...
␝ <chapter id="performance-tips">␟  <title>Performance Tips</title>␟  <title>性能に関するヒント</title>␞␞␞
␝  <para>␟   Query performance can be affected by many things. Some of these can
   be controlled by the user, while others are fundamental to the underlying
   design of the system.  This chapter provides some hints about understanding
   and tuning <productname>PostgreSQL</productname> performance.␟問い合わせの性能は多くの要因に影響されます。
ユーザが制御できるものもありますが、背後にあるシステム設計に起因する根本的な要因もあります。
本章では<productname>PostgreSQL</productname>の性能を理解し、チューニングするためのヒントを提供します。␞␞  </para>␞
...
␝ <sect1 id="using-explain">␟  <title>Using <command>EXPLAIN</command></title>␟  <title><command>EXPLAIN</command>の利用</title>␞␞␞
␝   <para>␟    <productname>PostgreSQL</productname> devises a <firstterm>query
    ...
    but this section attempts to cover the basics.␟<productname>PostgreSQL</productname>は受理した問い合わせから<firstterm>問い合わせ計画</firstterm>を作り出します。
...
本節ではその基本を提供しようと考えます。␞␞   </para>␞
␝   <para>␟    Examples in this section are drawn from the regression test database
    ...
    than exact, and because costs are inherently somewhat platform-dependent.␟本節の例は、v17の開発版ソースを用いて<command>VACUUM ANALYZE</command>を実行した後でリグレッションテストデータベースから取り出したものです。
実際にこの例を試すと、似たような結果になるはずですが、おそらく推定コストや行数は多少異なることになるでしょう。
<command>ANALYZE</command>による統計情報は厳密なものではなくランダムなサンプリングを行った結果であり、また、コストは本質的にプラットフォームに何かしら依存するためです。␞␞   </para>␞
␝   <para>␟    The examples use <command>EXPLAIN</command>'s default <quote>text</quote> output
    ...
    (XML, JSON, or YAML) instead.␟例では、簡潔で人が読みやすい<command>EXPLAIN</command>のデフォルトの<quote>text</quote>出力書式を使用します。
今後の解析で<command>EXPLAIN</command>の出力をプログラムに渡すことを考えているのであれば、代わりに機械読み取りが容易な出力書式（XML、JSON、YAML）のいずれかを使用する必要があります。␞␞   </para>␞
␝  <sect2 id="using-explain-basics">␟   <title><command>EXPLAIN</command> Basics</title>␟   <title><command>EXPLAIN</command>の基本</title>␞␞␞
␝   <para>␟    The structure of a query plan is a tree of <firstterm>plan nodes</firstterm>.
    ...
    number that the planner seeks to minimize.␟問い合わせ計画は<firstterm>計画ノード</firstterm>のツリー構造です。
...
プランナはこの値が最小になるように動作します。␞␞   </para>␞
␝   <para>␟    Here is a trivial example, just to show what the output looks like:␟どのような出力となるのかを示すためだけに、ここで簡単な例を示します。␞␞␞
␝   <para>␟    Since this query has no <literal>WHERE</literal> clause, it must scan all the
    ...
    to right):␟この問い合わせには<literal>WHERE</literal>句がありませんので、テーブル行をすべてスキャンしなければなりません。
このためプランナは単純なシーケンシャルスキャン計画を使用することを選びました。
（左から右に）括弧で囲まれた数値には以下のものがあります。␞␞␞
␝      <para>␟       Estimated start-up cost.  This is the time expended before the output
       phase can begin, e.g., time to do the sorting in a sort node.␟初期処理の推定コスト。
出力段階が開始できるようになる前に消費される時間、例えば、SORTノードで実行されるソート処理の時間です。␞␞      </para>␞
␝      <para>␟       Estimated total cost.  This is stated on the assumption that the plan
       ...
       available rows (see the <literal>LIMIT</literal> example below).␟全体推定コスト。
これは計画ノードが実行完了である、つまりすべての利用可能な行を受け取ることを前提として示されます。
実際には、ノードの親ノードはすべての利用可能な行を読む前に停止する可能性があります（以下の<literal>LIMIT</literal>の例を参照）。␞␞      </para>␞
␝      <para>␟       Estimated number of rows output by this plan node.  Again, the node
       is assumed to be run to completion.␟この計画ノードが出力する行の推定数。ここでも、ノードが実行を完了することを前提としています。␞␞      </para>␞
␝      <para>␟       Estimated average width of rows output by this plan node (in bytes).␟この計画ノードが出力する行の（バイト単位での）推定平均幅。␞␞      </para>␞
␝   <para>␟    The costs are measured in arbitrary units determined by the planner's
    ...
    parameters.␟コストはプランナのコストパラメータ（<xref linkend="runtime-config-query-constants"/>参照）によって決まる任意の単位で測定されます。
取り出すディスクページ単位でコストを測定することが、伝統的な方式です。
つまり、<xref linkend="guc-seq-page-cost"/>を慣習的に<literal>1.0</literal>に設定し、他のコストパラメータを相対的に設定します。
本節の例では、デフォルトのコストパラメータで実行しています。␞␞   </para>␞
␝   <para>␟    It's important to understand that the cost of an upper-level node includes
    ...
    plan.  (Every correct plan will output the same row set, we trust.)␟上位ノードのコストには、すべての子ノードのコストもその中に含まれていることを理解することは重要です。
...
（全ての正しい計画は同じ行セットを出力するものと期待されます。）␞␞   </para>␞
␝   <para>␟    The <literal>rows</literal> value is a little tricky because it is
    ...
    actually returned, updated, or deleted by the query.␟<literal>rows</literal>の値は、計画ノードによって処理あるいはスキャンされた行数を表しておらず、ノードによって発行された行数を表すので、多少扱いにくくなっています。
該当ノードに適用されるすべての<literal>WHERE</literal>句条件によるフィルタ処理の結果、スキャンされる行より少ない行数になることがよくあります。
理想的には、最上位の行数の推定値は、実際に問い合わせによって返され、更新され、あるいは削除された概算の行数となります。␞␞   </para>␞
␝   <para>␟    Returning to our example:␟例に戻ります。␞␞␞
␝   <para>␟    These numbers are derived very straightforwardly.  If you do:␟これらの数値はとても素直に導かれます。以下を実行すると、␞␞␞
␝␟    you will find that <classname>tenk1</classname> has 345 disk
    ...
    so the estimated cost is (345 * 1.0) + (10000 * 0.01) = 445.␟<classname>tenk1</classname>には345のディスクページと10000の行があることがわかります。
...
ですから、推定コストは(345 * 1.0) + (10000 * 0.01) = 445となります。␞␞   </para>␞
␝   <para>␟    Now let's modify the query to add a <literal>WHERE</literal> condition:␟では、<literal>WHERE</literal>条件を加えて、問い合わせを変更してみます。␞␞␞
␝␟    Notice that the <command>EXPLAIN</command> output shows the <literal>WHERE</literal>
    ...
    time spent checking the <literal>WHERE</literal> condition.␟<command>EXPLAIN</command>の出力が、Seq Scan計画ノードに付随する<quote>フィルタ</quote>条件として適用される<literal>WHERE</literal>句を表示していることに注意してください。
...
実際には、<literal>WHERE</literal>条件を検査するためにCPU時間が余計にかかることを反映して、ほんの少し（正確には10000 * <xref linkend="guc-cpu-operator-cost"/>）ですがコストが上昇しています。␞␞   </para>␞
␝   <para>␟    The actual number of rows this query would select is 7000, but the <literal>rows</literal>
    ...
    randomized sample of the table.␟この問い合わせが選択する実際の行数は7000です。
...
なぜなら、<command>ANALYZE</command>で生成される統計情報は、テーブルのランダムな標本から取り出されるからです。␞␞   </para>␞
␝   <para>␟    Now, let's make the condition more restrictive:␟では、条件をより強く制限してみます。␞␞␞
␝␟    Here the planner has decided to use a two-step plan: the child plan
    ...
    does the sorting.)␟ここでは、プランナは2段階の計画を使用することを決定しました。
...
ノードで記載されている<quote>bitmap</quote>は、ソートを行う機構の名前です。）␞␞   </para>␞
␝   <para>␟    Now let's add another condition to the <literal>WHERE</literal> clause:␟ここで<literal>WHERE</literal>句に別の条件を付与してみましょう。␞␞␞
␝␟    The added condition <literal>stringu1 = 'xxx'</literal> reduces the
    ...
    slightly to reflect this extra checking.␟追加された<literal>stringu1 = 'xxx'</literal>条件は出力行数推定値を減らしますが、同じ行集合にアクセスしなければなりませんので、コストは減りません。
...
これにより、追加の検査分を反映するため、コストは実際には少し上がります。␞␞   </para>␞
␝   <para>␟    In some cases the planner will prefer a <quote>simple</quote> index scan plan:␟場合によってはプランナは<quote>単純な</quote>インデックススキャン計画を選択します。␞␞␞
␝␟    In this type of plan the table rows are fetched in index order, which
    ...
    index already implicitly provides the requested ordering.␟この種の計画では、テーブル行はインデックス順で取り出されます。
...
この例では、<literal>ORDER BY unique1</literal>を追加すると、要求された順序がインデックスによってすでに暗黙的に提供されているため、同じ計画が使用されます。␞␞   </para>␞
␝   <para>␟     The planner may implement an <literal>ORDER BY</literal> clause in several
     ...
     <literal>Sort</literal> step:␟プランナは<literal>ORDER BY</literal>句をいくつかの方法で実装できます。
...
␝␟    If a part of the plan guarantees an ordering on a prefix of the
    ...
    <literal>Incremental Sort</literal> step:␟ソートキーで必要な接頭辞の順序がプランの一部で保証されている場合、<literal>インクリメンタルソート(Incremental Sort)</literal>ステップを使用することを決定できます。␞␞␞
␝␟    Compared to regular sorts, sorting incrementally allows returning tuples
    ...
    sorting batches.␟通常のソートと比較して、インクリメンタルソートは、結果セット全体がソートされる前にタプルを戻すことができます。
これにより、特に<literal>LIMIT</literal>がある問い合わせで最適化が可能になります。
また、メモリ使用量が削減され、ソートがディスクにオーバーフローする可能性が減少しますが、結果セットを複数のソートバッチに分割するオーバーヘッドが増加という代償を払うことになります。␞␞   </para>␞
␝   <para>␟    If there are separate indexes on several of the columns referenced
    in <literal>WHERE</literal>, the planner might choose to use an AND or OR
    combination of the indexes:␟<literal>WHERE</literal>句で参照される複数の列に対して別々のインデックスが存在する場合、プランナはインデックスをANDやORで組み合わせて使用することを選択する可能性があります。␞␞␞
␝␟    But this requires visiting both indexes, so it's not necessarily a win
    ...
    accordingly.␟しかし、これは両方のインデックスを参照する必要があります。
そのため、インデックスを1つ使用し、他の条件についてはフィルタとして扱う方法と比べて常に勝るとは限りません。
含まれる範囲を変更すると、それに伴い計画も変わることが分かるでしょう。␞␞   </para>␞
␝   <para>␟    Here is an example showing the effects of <literal>LIMIT</literal>:␟以下に<literal>LIMIT</literal>の影響を示す例を示します。␞␞␞
␝   <para>␟    This is the same query as above, but we added a <literal>LIMIT</literal> so that
    ...
    with that approach.␟これは上と同じ問い合わせですが、すべての行を取り出す必要がないため<literal>LIMIT</literal>を付けています。
...
以前の計画へのLimit追加でのLimitはビットマップスキャンの起動コストを払うことを避けることができないため、総コストは25単位を超えてしまうからです。␞␞   </para>␞
␝   <para>␟    Let's try joining two tables, using the columns we have been discussing:␟今まで説明に使ってきた列を使って2つのテーブルを結合してみましょう。␞␞␞
␝   <para>␟    In this plan, we have a nested-loop join node with two table scans as
    ...
    here), plus a little CPU time for join processing.␟この計画では、入力または子として２つのテーブルスキャンを持つネステッドループ結合ノードがあります。
...
ループノードのコストは、外部スキャンのコストと、各々の外部の行に対して内部スキャンが繰り返されることによるコスト（ここでは10 * 7.90）を加え、さらに結合処理を行うための少々のCPU時間を加えたものになります。␞␞   </para>␞
␝   <para>␟    In this example the join's output row count is the same as the product
    ...
    Here's an example:␟この例では、結合の出力行数は2つのスキャンの出力行数の積に等しくなっていますが、いつもそうなるわけではありません。
...
␝␟    The condition <literal>t1.hundred &lt; t2.hundred</literal> can't be
    ...
    but does not change either input scan.␟条件<literal>t1.hundred &lt; t2.hundred</literal>は<literal>tenk2_unique2</literal>インデックスの中では試験されません。
このため結合ノードで適用されます。
これは結合ノードの推定出力行数を減らしはしますが、入力スキャンには影響しません。␞␞   </para>␞
␝   <para>␟    Notice that here the planner has chosen to <quote>materialize</quote> the inner
    ...
    subsequent pass.␟ここではプランナが、具体化計画ノードをその上に挿入することで、結合の内部リレーションの<quote>具体化</quote>を選択していることに注意してください。
これは、たとえネステッドループ結合ノードが外部リレーションから各行につき一度、そのデータを10回読む必要があったとしても、<literal>t2</literal>インデックススキャンが一度だけ行なわれることを意味します。
具体化ノードはそのデータを読んだときにメモリに保存し、その後の読み出しではそのデータをメモリから返します。␞␞   </para>␞
␝   <para>␟    When dealing with outer joins, you might see join plan nodes with both
    ...
    join there is no semantic difference between these types of filters.␟外部結合を扱う時、<quote>結合フィルタ</quote>および通常の<quote>フィルタ</quote>の両方が付随する結合計画ノードが現れる可能性があります。
...
内部結合では、これらのフィルタ種類の間に意味的な違いはありません。␞␞   </para>␞
␝   <para>␟    If we change the query's selectivity a bit, we might get a very different
    join plan:␟問い合わせの選択性を少し変更すると、非常に異なる結合計画が得られるかもしれません。␞␞␞
␝   <para>␟    Here, the planner has chosen to use a hash join, in which rows of one
    ...
    rows from its outer child plan and searches the hash table for each one.␟ここでプランナはハッシュ結合の使用を選択しました。
...
外部の子計画から行を読み取り、各行に対してハッシュテーブルを検索します。␞␞   </para>␞
␝   <para>␟    Another possible type of join is a merge join, illustrated here:␟他にも、以下に示すようなマージ結合という結合があり得ます。␞␞␞
␝   <para>␟    Merge join requires its input data to be sorted on the join keys.  In this
    ...
    because of the nonsequential disk access required by the index scan.)␟マージ結合は、結合キーでソートされる入力データを必要とします。
この例では、各入力がインデックススキャンを使用して正しい順序で行にアクセスすることでソートされますが、シーケンシャルスキャンとソートも使用できます。
（多くの行をソートする場合、インデックススキャンでは非シーケンシャルなディスクアクセスが必要となるため、シーケンシャルスキャンとソートの方がインデックススキャンより優ります。）␞␞   </para>␞
␝   <para>␟    One way to look at variant plans is to force the planner to disregard
    ...
    type for the previous example, we could try␟<xref linkend="runtime-config-query-enable"/>に記載したenable/disableフラグを使用して、プランナが最も良いと考えている戦略を強制的に無視させる方法により、異なった計画を観察することができます。
...
␝␟    which shows that the planner thinks that hash join would be nearly 50%
    ...
    discussed <link linkend="using-explain-analyze">below</link>.␟これは、このケースではハッシュ結合がマージ結合よりも50%近く高価になるとプランナが考えていることを示しています。
当然ながら、次の疑問はこれが正しいかどうかでしょう。
<link linkend="using-explain-analyze">以下で</link>説明するように、<command>EXPLAIN ANALYZE</command>を使って調べることができます。␞␞   </para>␞
␝    </indexterm>␟    Some query plans involve <firstterm>subplans</firstterm>, which arise
    ...
    when they cannot be, we get plans like:␟一部の問い合わせ計画には、元の問い合わせ内の副<literal>SELECT</literal>から発生する <firstterm>副計画（subplans）</firstterm>が含まれています。
...
␝␟    This rather artificial example serves to illustrate a couple of
    ...
    the sub-<literal>SELECT</literal>.␟このかなり人工的な例は、二つの点を説明するのに役立ちます。すなわち、外側の計画階層からの値は副計画に渡すことができて（ここでは<literal>t.four</literal>が渡されます）、副SELECTの結果は外側の計画で利用できます。
これらの結果値は、<command>EXPLAIN</command>で<literal>(<replaceable>subplan_name</replaceable>).col<replaceable>N</replaceable></literal>のような記法で表示され、これは副<literal>SELECT</literal>の<replaceable>N</replaceable>番目の出力列を指します。␞␞   </para>␞
␝    </indexterm>␟    In the example above, the <literal>ALL</literal> operator runs the
    ...
    subplan</firstterm> to avoid that:␟上の例で、<literal>ALL</literal>演算子は、外側の問い合わせの各行に対して副計画を実行します（高い推定コストの原因となります）。
...
␝␟    Here, the subplan is run a single time and its output is loaded into
    ...
    amenable to hashing.␟ここでは、副計画が1回実行され、その出力がメモリ内のハッシュテーブルに置かれ、次に外側の<literal>ANY</literal>演算子によって検査されます。
これには、副<literal>SELECT</literal>が外側の問い合わせの変数を参照しないことと、<literal>ANY</literal>の比較演算子がハッシュ適用できることが必要です。␞␞   </para>␞
␝    </indexterm>␟    If, in addition to not referencing any variables of the outer query,
    the sub-<literal>SELECT</literal> cannot return more than one row,
    it may instead be implemented as an <firstterm>initplan</firstterm>:␟外側の問い合わせの変数を参照しないことに加えて、副<literal>SELECT</literal>が複数行を返せない場合は、代わりに<firstterm>initplan</firstterm>として次のように計画作成されます。␞␞␞
␝␟    An initplan is run only once per execution of the outer plan, and its
    ...
    happen without the sub-<literal>SELECT</literal> construct.␟initplanは外側の計画の実行ごとに1回だけ実行され、その結果は外側の計画の後の行で再利用するために保存されます。
したがって、この例では<literal>random()</literal>は1回だけ評価され、<literal>t1.10</literal>のすべての値がランダムに選択された同じ整数と比較されます。
これは、副<literal>SELECT</literal>構造がない場合とは大きく異なります。␞␞   </para>␞
␝   <para>␟    It is possible to check the accuracy of the planner's estimates
    ...
    shows.  For example, we might get a result like this:␟<command>EXPLAIN</command>の<literal>ANALYZE</literal>オプションを使用して、プランナが推定するコストの精度を点検することができます。
...
␝␟    Note that the <quote>actual time</quote> values are in milliseconds of
    ...
    the estimates were all dead-on, but that's quite unusual in practice.␟<quote>actual time</quote>値は実時間をミリ秒単位で表されていること、<literal>cost</literal>推定値は何らかの単位で表されていることに注意してください。
ですからそのまま比較することはできません。
注目すべきもっとも重要な点は通常、推定行数が実際の値と合理的に近いかどうかです。
この例では、推定はすべて正確ですが、現実的にはあまりありません。␞␞   </para>␞
␝   <para>␟    In some query plans, it is possible for a subplan node to be executed more
    ...
    executing the index scans on <literal>tenk2</literal>.␟問い合わせ計画の中には、何回も副計画ノードを実行する可能性のあるものがあります。
...
上の例では、<literal>tenk2</literal>に対するインデックススキャンの実行のために合計0.030ミリ秒要しています。␞␞   </para>␞
␝   <para>␟    In some cases <command>EXPLAIN ANALYZE</command> shows additional execution
    statistics beyond the plan node execution times and row counts.
    For example, Sort and Hash nodes provide extra information:␟場合によっては、<command>EXPLAIN ANALYZE</command>は計画ノードの実行時間と行数以上の実行統計情報をさらに表示します。
...
␝␟    The Sort node shows the sort method used (in particular, whether the sort
    ...
    shown.)␟ソートノードは使用されるソート方式（具体的にはソートがメモリ内かディスク上か）および必要なメモリまたはディスクの容量を表示します。
ハッシュノードでは、ハッシュバケット数とバッチ数、ハッシュテーブルで使用されるメモリのピーク容量が表示されます。
（バッチ数が１を超える場合、同時にディスクの使用容量も含まれますが、表示はされません。）␞␞   </para>␞
␝   <para>␟    Another type of extra information is the number of rows removed by a
    filter condition:␟他の種類の追加情報はフィルタ条件によって除外される行数があります。␞␞␞
␝␟    These counts can be particularly valuable for filter conditions applied at
    ...
    is rejected by the filter condition.␟特に結合ノードで適用されるフィルタ条件ではこれらの数が有用です。
<quote>Rows Removed</quote>行は、少なくともスキャンされた１行、結合ノードにおける結合組み合わせの可能性がフィルタ条件によって拒絶された時にのみ現れます。␞␞   </para>␞
␝   <para>␟    A case similar to filter conditions occurs with <quote>lossy</quote>
    index scans.  For example, consider this search for polygons containing a
    specific point:␟<quote>非可逆</quote>インデックススキャンはフィルタ条件に似た状況です。
...
␝␟    The planner thinks (quite correctly) that this sample table is too small
    ...
    index scan to be used, we see:␟プランナは（ほぼ正確に）、インデックススキャンを考慮するには例のテーブルが小さ過ぎるとみなします。
...
␝␟    Here we can see that the index returned one candidate row, which was
    ...
    to do the exact containment test on those rows.␟ここで、インデックスが１つの候補行を返し、それがインデックス条件の再検査により拒絶されることが分かります。
多角形の含有試験ではGiSTインデックスが<quote>非可逆</quote>であるため、これは発生します。
実際には対象と重なる多角形を持つ行を返し、そしてこれらの行が正確に含有関係であることを試験しなければなりません。␞␞   </para>␞
␝   <para>␟    <command>EXPLAIN</command> has a <literal>BUFFERS</literal> option that can be used with
    <literal>ANALYZE</literal> to get even more run time statistics:␟<command>EXPLAIN</command>には、より多くの実行時統計情報を取り出すために、<literal>ANALYZE</literal>に付与できる<literal>BUFFERS</literal>オプションがあります。␞␞␞
␝␟    The numbers provided by <literal>BUFFERS</literal> help to identify which parts
    of the query are the most I/O-intensive.␟<literal>BUFFERS</literal>により提供される数は、問い合わせのどの部分がもっとも大きいI/Oであるかを識別する役に立ちます。␞␞   </para>␞
␝   <para>␟    Keep in mind that because <command>EXPLAIN ANALYZE</command> actually
    ...
    roll the command back afterwards, for example:␟<command>EXPLAIN ANALYZE</command>が実際に問い合わせを実行しますので、<command>EXPLAIN</command>のデータを出力することを優先して問い合わせの出力が破棄されたとしても、何らかの副作用が通常通り発生することに注意してください。
テーブルを変更すること無くデータ変更問い合わせの解析を行いたければ、以下の例のように、実行後コマンドをロールバックしてください。␞␞␞
␝   <para>␟    As seen in this example, when the query is an <command>INSERT</command>,
    ...
    decisions.␟この例で分かるように、問い合わせが<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>、<command>MERGE</command>である場合、テーブル変更を行うための実作業は最上位のInsert、Update、Delete、Merge計画ノードで行われます。
...
これは、行われる作業がすべての正確な問い合わせ計画の作業と同一であるためであり、このため計画の決定に影響を与えません。␞␞   </para>␞
␝   <para>␟    When an <command>UPDATE</command>, <command>DELETE</command>, or
    <command>MERGE</command> command affects a partitioned table or
    inheritance hierarchy, the output might look like this:␟<command>UPDATE</command>、<command>DELETE</command>、<command>MERGE</command>コマンドがパーティションテーブルや継承階層に影響する場合には、出力は以下のようになるでしょう。␞␞␞
␝␟    In this example the Update node needs to consider three child tables,
    ...
    same order as the corresponding subplans.␟この例では、Updateノードは3つの子テーブルを考慮しますが、元の問い合わせに記述されていたパーティションテーブルを考慮しません（そこにデータ格納されることはないため）。
そのため、テーブル毎に3つの入力スキャン副計画があります。
明確にするため、Updateノードには対応する副計画と同じ順に更新される特定の対象テーブルを示す注釈が付けられています。␞␞   </para>␞
␝   <para>␟    The <literal>Planning time</literal> shown by <command>EXPLAIN
    ANALYZE</command> is the time it took to generate the query plan from the
    parsed query and optimize it. It does not include parsing or rewriting.␟<command>EXPLAIN ANALYZE</command>で表示される<literal>Planning time</literal>は、解析された問い合わせから問い合わせ計画を生成し最適化するのに掛かった時間です。
解析と書き換えは含みません。␞␞   </para>␞
␝   <para>␟    The <literal>Execution time</literal> shown by <command>EXPLAIN
    ...
    <command>EXPLAIN ANALYZE</command>.␟<command>EXPLAIN ANALYZE</command>で表示される<literal>Execution time</literal>（実行時間）にはエグゼキュータの起動、停止時間、発行される何らかのトリガの実行時間も含まれますが、解析や書き換え、計画作成の時間は含まれません。
...
しかし、遅延制約トリガはトランザクションが終わるまで実行されませんので、<command>EXPLAIN ANALYZE</command>では考慮されないことに注意してください。␞␞   </para>␞
␝   <para>␟    The time shown for the top-level node does not include any time needed
    ...
    total <literal>Execution time</literal>.␟最上位ノードに表示される時間には、問い合わせの出力データを表示可能な形式に変換したり、それをクライアントに送信したりするのに要する時間は含まれません。
...
その時間は分けて表示され、また、それは合計の<literal>Execution time</literal>にも含まれます。␞␞   </para>␞
␝  <sect2 id="using-explain-caveats">␟   <title>Caveats</title>␟   <title>警告</title>␞␞␞
␝   <para>␟    There are two significant ways in which run times measured by
    ...
    on your system.␟<command>EXPLAIN ANALYZE</command>により測定される実行時間が同じ問い合わせを普通に実行する場合と大きくそれる可能性がある、２つの重大な点があります。
...
<xref linkend="pgtesttiming"/>を用いて、使用中のシステムの時間測定にかかるオーバーヘッドを測ることができます。␞␞   </para>␞
␝   <para>␟    <command>EXPLAIN</command> results should not be extrapolated to situations
    ...
    <literal>polygon_tbl</literal> example above.)␟<command>EXPLAIN</command>の結果を試験を行ったものと大きく異なる状況の推定に使ってはいけません。
...
（上述の<literal>polygon_tbl</literal>の例でこれが起こることを示しています。）␞␞   </para>␞
␝   <para>␟    There are cases in which the actual and estimated values won't match up
    ...
    effect.  For example, in the <literal>LIMIT</literal> query we used before,␟実際の値と推定値がうまく合わないが本当は間違ったものがない場合があります。
...
␝␟    the estimated cost and row count for the Index Scan node are shown as
    ...
    values are displayed.␟インデックススキャンノードの推定コストと行数が実行完了したかのように表示されます。
しかし現実では、Limitノードが２行を取り出した後に行の要求を停止します。
このため実際の行数は２行のみであり、実行時間は提示された推定コストより小さくなります。
これは推定間違いではなく、単なる推定値と本当の値を表示する方法における矛盾です。␞␞   </para>␞
␝   <para>␟    Merge joins also have measurement artifacts that can confuse the unwary.
    ...
    of rows that are actually in the inner relation.␟またマージ結合には、注意しないと混乱を招く測定上の乱れがあります。
...
外部で多くの重複がある場合、内部の子計画ノードで繰り返される実際の行数は、内部リレーションにおける実際の行数より非常に多くなることがあり得ます。␞␞   </para>␞
␝   <para>␟    BitmapAnd and BitmapOr nodes always report their actual row counts as zero,
    due to implementation limitations.␟実装上の制限のため、BitmapAndおよびBitmapOrノードは常に実際の行数をゼロと報告します。␞␞   </para>␞
␝   <para>␟    Normally, <command>EXPLAIN</command> will display every plan node
    ...
    instead.␟通常<command>EXPLAIN</command>はプランナが生成したすべてのプランノードを表示します。
...
これが起きると、これらのプランノードは<command>EXPLAIN</command>の出力から削除され、<literal>Subplans Removed: <replaceable>N</replaceable></literal>という注釈が代わりに表示されます。␞␞   </para>␞
...
␝ <sect1 id="planner-stats">␟  <title>Statistics Used by the Planner</title>␟  <title>プランナで使用される統計情報</title>␞␞␞
␝  <sect2 id="planner-stats-single-column">␟   <title>Single-Column Statistics</title>␟   <title>単一列統計情報</title>␞␞  <para>␞
␝  <para>␟   As we saw in the previous section, the query planner needs to estimate
   the number of rows retrieved by a query in order to make good choices
   of query plans.  This section provides a quick look at the statistics
   that the system uses for these estimates.␟前節で説明した通り、問い合わせプランナは、より良い問い合わせ計画を選択するために問い合わせによって取り出される行数の推定値を必要としています。
本節では、システムがこの推定に使用する統計情報について簡単に説明します。␞␞  </para>␞
␝  <para>␟   One component of the statistics is the total number of entries in
   ...
   queries similar to this one:␟統計情報の1つの構成要素は、各テーブルとインデックスの項目の総数と、各テーブルとインデックスが占めるディスクブロック数です。
...
␝␟   Here we can see that <structname>tenk1</structname> contains 10000
   ...
   smaller than the table.␟ここで、<structname>tenk1</structname>とそのインデックスには10000行が存在し、そして、（驚くには値しませんが）インデックスはテーブルよりもかなり小さなものであることがわかります。␞␞  </para>␞
␝  <para>␟   For efficiency reasons, <structfield>reltuples</structfield>
   ...
   approximation.␟効率を上げるため、<structfield>reltuples</structfield>と<structfield>relpages</structfield>は処理の度には更新されず、したがって通常は多少古い値のみ所有しています。
...
いずれの場合でもプランナは、現在の物理的なテーブルサイズに合わせるために<structname>pg_class</structname>から検索した値を調整して、より高精度な近似値を得ます。␞␞  </para>␞
␝  <para>␟   Most queries retrieve only a fraction of the rows in a table, due
   ...
   updated.␟ほとんどの問い合わせは、検証される行を制限する<literal>WHERE</literal>句によって、テーブル内の行の一部のみを取り出します。
...
<structname>pg_statistic</structname>内の項目は、<command>ANALYZE</command>と<command>VACUUM ANALYZE</command>コマンドによって更新され、また１から更新がかかったとしても常に概算値になります。␞␞  </para>␞
␝  <para>␟   Rather than look at <structname>pg_statistic</structname> directly,
   ...
   For example, we might do:␟統計情報を手作業で確認する場合、<structname>pg_statistic</structname>を直接参照するのではなく、<link linkend="view-pg-stats"><structname>pg_stats</structname></link>ビューを参照する方が良いでしょう。
...
␝␟   Note that two rows are displayed for the same column, one corresponding
   ...
   the <literal>name</literal> column.)␟同じ列に対して2行が表示されていることに注意してください。
...
（簡潔にするために、<literal>name</literal>列について最頻値は最初の10個のみを示しました。）␞␞  </para>␞
␝  <para>␟   The amount of information stored in <structname>pg_statistic</structname>
   ...
   sufficient for columns with simple data distributions.␟<command>ANALYZE</command>により<structname>pg_statistic</structname>に格納される情報量、具体的には、それぞれの列に対する<structfield>most_common_vals</structfield>内と<structfield>histogram_bounds</structfield>配列のエントリの最大数は、<command>ALTER TABLE SET STATISTICS</command>コマンドによって列ごとに、<xref linkend="guc-default-statistics-target"/>設定パラメータを設定することによってグローバルに設定することができます。
...
反対に上限を下げることは、単純なデータ分布の列に対して順当です。␞␞  </para>␞
␝  <para>␟   Further details about the planner's use of statistics can be found in
   <xref linkend="planner-stats-details"/>.␟プランナによる統計情報の使用に関する詳細については<xref linkend="planner-stats-details"/>を参照してください。␞␞  </para>␞
...
␝  <sect2 id="planner-stats-extended">␟   <title>Extended Statistics</title>␟   <title>拡張統計情報</title>␞␞␞
␝   <para>␟    It is common to see slow queries running bad execution plans because
    ...
    such information.␟問い合わせ句で使われている複数列に相関性があることにより、悪い実行計画を実行する遅いクエリがしばしば観察されます。
プランナは通常複数の条件がお互いに独立であるとみなしますが、列の値に相関性がある場合はそれは成り立ちません。
通常の列ごとの統計情報は、それが個々の列ごとであるという性質上、列をまたがる相関性に関する知識を把握することはできません。
しかしながら、<productname>PostgreSQL</productname>は、<firstterm>多変量統計情報</firstterm>を計算することができ、それによってそうした情報を把握することができます。␞␞   </para>␞
␝   <para>␟    Because the number of possible column combinations is very large,
    ...
    the server to obtain statistics across interesting sets of columns.␟列の組み合わせの数は非常に大きいため、自動的に多変量統計情報を計算するのは現実的ではありません。
代わりに、サーバが興味のある列の集合にまたがる統計情報を得るように指示する目的で、<firstterm>拡張統計情報オブジェクト</firstterm>（しばしば単に<firstterm>統計情報オブジェクト</firstterm>と呼ばれます）を作成することができます。␞␞   </para>␞
␝   <para>␟    Statistics objects are created using the
    ...
    catalog.␟統計情報オブジェクトは<link linkend="sql-createstatistics"><command>CREATE STATISTICS</command></link>で作成します。
...
収集したデータは、<link linkend="catalog-pg-statistic-ext-data"><structname>pg_statistic_ext_data</structname></link>カタログで参照することができます。␞␞   </para>␞
␝   <para>␟    <command>ANALYZE</command> computes extended statistics based on the same
    ...
    them.␟通常の単一列統計情報の計算に使うのと同じテーブル行のサンプルに基づき、<command>ANALYZE</command>は、拡張統計情報を計算します。
（前節で述べたように）テーブルあるいはそのテーブルの対象となる列統計情報の増やすと、サンプルのサイズも増えるので、より大きな統計情報の対象を使うと、通常、より精度の高い拡張統計情報を得られますが、計算に費やす時間も増えます。␞␞   </para>␞
␝   <para>␟    The following subsections describe the kinds of extended statistics
    that are currently supported.␟次の副節では、現在サポートしている拡張統計情報の種類を説明します。␞␞   </para>␞
␝   <sect3 id="planner-stats-extended-functional-deps">␟    <title>Functional Dependencies</title>␟    <title>関数従属性</title>␞␞␞
␝    <para>␟     The simplest kind of extended statistics tracks <firstterm>functional
     ...
     dependency.␟もっとも単純な拡張統計情報は、データベースの正規形の定義で使われる考え方である、<firstterm>関数従属性</firstterm>を追跡します。
...
完全に正規化されたデータベースにおいても、ある列の間に部分的な相関関係が存在することがあり、これは部分的関数従属性として表現されます。␞␞    </para>␞
␝    <para>␟     The existence of functional dependencies directly affects the accuracy
     ...
     in underestimating the result size.␟ある問い合わせでは、関数従属性が存在することが見積もりの精度に直接影響を与えます。
問い合わせに独立した列と依存する列の両方に関する条件が含まれていると、依存する列に関する条件はそれ以上結果サイズを小さくしません。
しかし関数従属性に関する知識がなければ、クエリプランナはそれらの条件が独立であると見なし、結果サイズの過少見積もりすることになります。␞␞    </para>␞
␝    <para>␟     To inform the planner about functional dependencies, <command>ANALYZE</command>
     ...
     <command>ANALYZE</command> and later query planning.␟プランナに関数従属性について知らせるために、<command>ANALYZE</command>は列をまたがる依存性の強さを収集することができます。
...
<command>ANALYZE</command>および後々のクエリプランニングにおける不必要なオーバーヘッドを避けるために、強い相関関係のある列のグループのみを対象に、<literal>dependencies</literal>統計情報を作成することをお勧めします。␞␞    </para>␞
␝    <para>␟     Here is an example of collecting functional-dependency statistics:␟関数従属性統計情報の収集例です。␞␞<programlisting>␞
␝</programlisting>␟     Here it can be seen that column 1 (zip code) fully determines column
     ...
     represented by more than a single ZIP code.␟ここでは、列1（zip code）が完全に列5（city）を決定しているので、係数は1.0です。
一方、cityはzip codeを42%しか決定していないので、一つ以上のzip codeで表現されている多くのcity(58%)が存在するということになります。␞␞    </para>␞
␝    <para>␟     When computing the selectivity for a query involving functionally
     ...
     an underestimate.␟関数従属する列を伴うクエリの選択率を計算する際、過小評価を行わないように、プランナは依存性係数を使って条件ごとの選択率を調整します。␞␞    </para>␞
␝    <sect4 id="planner-stats-extended-functional-deps-limits">␟     <title>Limitations of Functional Dependencies</title>␟     <title>関数従属性の制限事項</title>␞␞␞
␝     <para>␟      Functional dependencies are currently only applied when considering
      ...
      range clauses, <literal>LIKE</literal> or any other type of condition.␟今のところ、列と定数を比較する単純な等価条件と、定数の<literal>IN</literal>句を考慮する際にしか関数従属性は適用されません。
二つの列を比較する、あるいは列を式と比較する等価条件、範囲句、<literal>LIKE</literal>その他の条件の見積もりを改善するのには使われません。␞␞     </para>␞
␝     <para>␟      When estimating with functional dependencies, the planner assumes that
      ...
      that possibility is not considered.  For example, given a query like␟関数従属性を含めた見積もりでは、プランナは関係する列に対する複数の条件が同時に成り立つ、つまり冗長であるとみなします。
それらの条件が同時に成り立たなければ、正しい見積もりは0行となりますが、その可能性は考慮されません。
たとえば次のクエリを見てください。␞␞<programlisting>␞
␝</programlisting>␟      the planner will disregard the <structfield>city</structfield> clause as not
      changing the selectivity, which is correct.  However, it will make
      the same assumption about␟プランナは、選択率が変わらないという正しい推定に基づき<structfield>city</structfield>句を無視します。
しかし、同じ前提を␞␞<programlisting>␞
␝</programlisting>␟      even though there will really be zero rows satisfying this query.
      Functional dependency statistics do not provide enough information
      to conclude that, however.␟が満たす行が0行であるにもかかわらず、同じ推測をします。
関数従属性統計情報は、これを結論付けるだけの十分な情報を提供しません。␞␞     </para>␞
␝     <para>␟      In many practical situations, this assumption is usually satisfied;
      ...
      option.␟多くの実用的な場合には、この前提は通常満たされます。
たとえば、あるアプリケーションには、クエリの中で両立するcityとZIP codeだけを許すGUIが備わっているかもしれません。
もしそうでなければ、関数従属性は実行可能なオプションではないかもしれません。␞␞     </para>␞
␝   <sect3 id="planner-stats-extended-n-distinct-counts">␟    <title>Multivariate N-Distinct Counts</title>␟    <title>多変量N個別値計数</title>␞␞␞
␝    <para>␟     Single-column statistics store the number of distinct values in each
     ...
     data, causing it to select bad plans.␟単一列統計情報は、それぞれの列で異なる値の数を保持します。
たとえば、<literal>GROUP BY a, b</literal>のように、二つ以上の列を組み合わせての異なる値の数の見積もりは、プランナに単一列の統計情報だけしか与えられない場合は、しばしば間違ったものになり、プランナは悪いプランの選択をしてしまいます。␞␞    </para>␞
␝    <para>␟     To improve such estimates, <command>ANALYZE</command> can collect n-distinct
     ...
     listed columns.␟見積もり改善のために、列のグループに対して<command>ANALYZE</command>はN個別統計情報を収集することができます。
以前述べたのと同様に、可能なすべての列のグループに対してこれを行なうのは現実的ではありません。
<literal>ndistinct</literal>オプションで定義された統計オブジェクト中に一緒に現れる列のグループに対してのみデータを
収集します。
列リストの中から、可能な二つ以上の列の組み合わせそれぞれに対してデータが収集されます。␞␞    </para>␞
␝    <para>␟     Continuing the previous example, the n-distinct counts in a
     table of ZIP codes might look like the following:␟先ほどの例の続きで、ZIP codeのテーブルのN個別値計数は次のようになります。␞␞<programlisting>␞
␝</programlisting>␟     This indicates that there are three combinations of columns that
     ...
     values.␟この例では、33178の異なる値を持つ列の組み合わせが三つあることを示しています。
ZIP codeとstate、ZIP codeとcity、cityとstateです。（これらが等しいという事実は、ZIP codeだけがテーブル中でユニークであることから期待されます。）
一方、cityとstateの組み合わせには、27435だけの異なる値があります。␞␞    </para>␞
␝    <para>␟     It's advisable to create <literal>ndistinct</literal> statistics objects only
     ...
     plans.  Otherwise, the <command>ANALYZE</command> cycles are just wasted.␟グループ化で実際に使用する列の組み合わせで、かつグループ数の見積もり間違いによって悪いプランをもたらすものに対してだけ、<literal>ndistinct</literal>統計情報オブジェクトを作ることをお勧めします。
さもないと、<command>ANALYZE</command>サイクルは単に無駄になります。␞␞    </para>␞
␝   <sect3 id="planner-stats-extended-mcv-lists">␟    <title>Multivariate MCV Lists</title>␟    <title>多変量MCVリスト</title>␞␞␞
␝    <para>␟     Another type of statistic stored for each column are most-common value
     ...
     multiple columns.␟列ごとに格納される別なタイプの統計値は最頻値リスト(most-common value list)です。
個々の列ごとには非常に正確な推測を可能にしますが、複数列に渡る条件を持つ問い合わせについては重大な誤った推定をもたらすことがあります。␞␞    </para>␞
␝    <para>␟     To improve such estimates, <command>ANALYZE</command> can collect MCV
     ...
     <literal>mcv</literal> option.␟こうした推定を改善するために、列の組み合わせのMCVリストを<command>ANALYZE</command>で収集することができます。
関数従属性とN個別値係数同様、考えられるすべての列のグループに対してこれを行うのは実用的ではありません。
MCVリストでは（関数従属性とN個別値係数と違って）列の頻値を格納するのでなおさらです。
ですから<literal>mcv</literal>オプションで定義された統計情報オブジェクト中に共通して現れる列のグループのデータだけが収集されます。␞␞    </para>␞
␝    <para>␟     Continuing the previous example, the MCV list for a table of ZIP codes
     might look like the following (unlike for simpler types of statistics,
     a function is required for inspection of MCV contents):␟前述の例を続けましょう。ZIPコードのテーブルのMCVリストは次のようになるでしょう。（単純な形式の統計情報とは違って、MCVの内容を解析する関数が必要になります）␞␞␞
␝</programlisting>␟     This indicates that the most common combination of city and state is
     ...
     magnitude under-estimates.␟これによると市と州のもっとも頻度の高い組み合わせはDCのWashingtonで、（サンプルにおける）実際の頻度は約0.35%でした。
比較の基準となる組み合わせの頻度（単純な列ごとの頻度から計算されたもの）はたった0.0027%で、2桁の過少見積になっています。␞␞    </para>␞
␝    <para>␟     It's advisable to create <acronym>MCV</acronym> statistics objects only
     ...
     are just wasted.␟そのグループの誤推定値が間違った計画をもたらしてしまうような、条件の中で実際に一緒に使われる列の組み合わせについてのみ<acronym>MCV</acronym>統計情報オブジェクトを作成することが望ましいです。
さもないと、<command>ANALYZE</command>とプラン処理は単に無駄になってしまいます。␞␞    </para>␞
...
␝ <sect1 id="explicit-joins">␟  <title>Controlling the Planner with Explicit <literal>JOIN</literal> Clauses</title>␟  <title>明示的な<literal>JOIN</literal>句でプランナを制御する</title>␞␞␞
␝  <para>␟   It is possible
   to control the query planner to some extent by using the explicit <literal>JOIN</literal>
   syntax.  To see why this matters, we first need some background.␟明示的な<literal>JOIN</literal>構文を使って問い合わせプランナをある程度制御できます。
どうしてこういうことが問題になるのか、まずその背景を見る必要があります。␞␞  </para>␞
␝  <para>␟   In a simple join query, such as:␟単純な問い合わせ、例えば␞␞<programlisting>␞
␝</programlisting>␟   the planner is free to join the given tables in any order.  For
   ...
   efficient query plan.␟では、プランナは自由に与えられたテーブルを任意の順で結合することができます。
...
ですから、プランナは最も効率の良い計画を探すために可能な計画をすべて検査します。␞␞  </para>␞
␝  <para>␟   When a query only involves two or three tables, there aren't many join
   ...
   necessarily find the best possible plan.␟結合の対象がせいぜい2、3個のテーブルなら心配するほど結合の種類は多くありません。
...
遺伝的探索は短い時間で探索を行いますが、必ずしも最適な計画を見つけるとは限りません。␞␞  </para>␞
␝  <para>␟   When the query involves outer joins, the planner has less freedom
   than it does for plain (inner) joins. For example, consider:␟外部結合が含まれるような問い合わせでは、通常の（内部）結合よりプランナの選択の余地が小さくなります。
例えば、次のような問い合わせを考えます。␞␞<programlisting>␞
␝</programlisting>␟   Although this query's restrictions are superficially similar to the
   ...
   For example, given:␟この問い合わせの検索条件は前述の例と表面的には似ているように思えますが、BとCの結合結果の行に適合しないAの各行が出力されなければならないため、意味的には異なります。
...
例えば、以下を考えてみます。␞␞<programlisting>␞
␝</programlisting>␟   it is valid to join A to either B or C first.  Currently, only
   ...
   can be rearranged to some extent.␟この場合、Aを先にBと結合してもCと結合しても有効です。
現時点では、<literal>FULL JOIN</literal>のみが完全に結合順を制限します。
<literal>LEFT JOIN</literal>や<literal>RIGHT JOIN</literal>を含む、ほとんどの実環境では、何らかの拡張に再調整することができます。␞␞  </para>␞
␝  <para>␟   Explicit inner join syntax (<literal>INNER JOIN</literal>, <literal>CROSS
   ...
   constrain the join order.␟明示的な内部結合構文（<literal>INNER JOIN</literal>、<literal>CROSS JOIN</literal>、装飾のない<literal>JOIN</literal>）は、意味的には<literal>FROM</literal>内の入力リレーションの列挙と同じです。
したがって、結合順を制約しません。␞␞  </para>␞
␝  <para>␟   Even though most kinds of <literal>JOIN</literal> don't completely constrain
   ...
   For example, these three queries are logically equivalent:␟ほとんどの種類の<literal>JOIN</literal>は完全に結合順を制約しませんが、<productname>PostgreSQL</productname>問い合わせプランナに、すべての<literal>JOIN</literal>句に対してとりあえず結合順を制限させることができます。
例えば、以下の3つの問い合わせは論理的には同一です。␞␞<programlisting>␞
␝</programlisting>␟   But if we tell the planner to honor the <literal>JOIN</literal> order,
   the second and third take less time to plan than the first.  This effect
   is not worth worrying about for only three tables, but it can be a
   lifesaver with many tables.␟しかし、プランナに<literal>JOIN</literal>の順番を守るように伝えた場合、2番目と3番目の問い合わせは最初のものよりも短い時間で計画を立てることができます。
この効果はたった3つのテーブルでは気にするほどのものではありませんが、多くのテーブルを結合する際には最後の頼みの綱になるかもしれません。␞␞  </para>␞
␝  <para>␟   To force the planner to follow the join order laid out by explicit
   <literal>JOIN</literal>s,
   set the <xref linkend="guc-join-collapse-limit"/> run-time parameter to 1.
   (Other possible values are discussed below.)␟プランナを強制的に明示的な<literal>JOIN</literal>に潜在する結合順に従わせるには、<xref linkend="guc-join-collapse-limit"/>実行時パラメータを1に設定してください。
（以下で他の取り得る値について説明します。）␞␞  </para>␞
␝  <para>␟   You do not need to constrain the join order completely in order to
   cut search time, because it's OK to use <literal>JOIN</literal> operators
   within items of a plain <literal>FROM</literal> list.  For example, consider:␟検索時間を節約するために、結合順を完全に束縛する必要はありません。
なぜなら、単純な<literal>FROM</literal>リストの項目内に<literal>JOIN</literal>演算子を使っても構わないからです。
例えば、次の例です。␞␞<programlisting>␞
␝</programlisting>␟   With <varname>join_collapse_limit</varname> = 1, this
   forces the planner to join A to B before joining them to other tables,
   but doesn't constrain its choices otherwise.  In this example, the
   number of possible join orders is reduced by a factor of 5.␟<varname>join_collapse_limit</varname> = 1とした場合、プランナは強制的に他のテーブルと結合する前にAとBを結合しますが、それ以外については特に拘束はありません。
この例では、結合順の候補は5の階乗分の1に減ります。␞␞  </para>␞
␝  <para>␟   Constraining the planner's search in this way is a useful technique
...
これには実験することを勧めます。␞␞  </para>␞
␝  <para>␟   A closely related issue that affects planning time is collapsing of
   subqueries into their parent query.  For example, consider:␟計画作成時間に影響する密接に関連した問題として、副問い合わせをその親問い合わせに折り畳むことがあります。
例えば、以下を考えてみます。␞␞<programlisting>␞
␝</programlisting>␟   This situation might arise from use of a view that contains a join;
   the view's <literal>SELECT</literal> rule will be inserted in place of the view
   reference, yielding a query much like the above.  Normally, the planner
   will try to collapse the subquery into the parent, yielding:␟こうした状況は、結合を含むビューを使用する際に現れます。
そのビューの<literal>SELECT</literal>ルールはビューを参照するところに挿入され、上のような問い合わせを生成します。
通常、プランナは副問い合わせを親問い合わせに折り畳み、以下を生成します。␞␞<programlisting>␞
␝</programlisting>␟   This usually results in a better plan than planning the subquery
   ...
   adjusting this run-time parameter up or down.␟これは通常、副問い合わせの計画を別途作成するより優れた計画を作成します。
...
この実行時パラメータの値を上下に調整することで計画作成時間と計画の質をトレードオフすることができます。␞␞  </para>␞
␝  <para>␟   <xref linkend="guc-from-collapse-limit"/> and <xref
   ...
   time and run time.␟両者はほとんど同じことを行うため、<xref linkend="guc-from-collapse-limit"/>と<xref linkend="guc-join-collapse-limit"/>は似たような名前になっています。
...
しかし、計画作成時間と実行時間の間のトレードオフを細かく調整するつもりであれば、これらを別の値に設定しても構いません。␞␞  </para>␞
␝ <sect1 id="populate">␟  <title>Populating a Database</title>␟  <title>データベースへのデータ投入</title>␞␞␞
␝  <para>␟   One might need to insert a large amount of data when first populating
   a database. This section contains some suggestions on how to make
   this process as efficient as possible.␟データベースにデータを初期投入するために、大量のテーブル挿入操作を行う必要がままあります。
本節では、この作業を効率良く行うためのちょっとした提言を示します。␞␞  </para>␞
␝   <indexterm>
...
␝  <sect2 id="disable-autocommit">␟   <title>Disable Autocommit</title>␟   <title>自動コミットをオフにする</title>␞␞␞
␝   <para>␟    When using multiple <command>INSERT</command>s, turn off autocommit and just do
    ...
    loaded data.␟複数回の<command>INSERT</command>を実行するのであれば、自動コミットを無効にして最後に1回だけコミットしてください。
...
その結果、一部のみがロードされたデータの対処に困ることはありません。␞␞   </para>␞
␝  <sect2 id="populate-copy-from">␟   <title>Use <command>COPY</command></title>␟   <title><command>COPY</command>の使用</title>␞␞␞
␝   <para>␟    Use <link linkend="sql-copy"><command>COPY</command></link> to load
    ...
    use this method to populate a table.␟単一コマンドですべての行をロードするために一連の<command>INSERT</command>コマンドではなく、<link linkend="sql-copy"><command>COPY</command></link>を使用してください。
...
<command>COPY</command>コマンドでテーブルにデータを投入する場合、コマンドは1つなので、自動コミットを無効にする必要はありません。␞␞   </para>␞
␝   <para>␟    If you cannot use <command>COPY</command>, it might help to use <link
    ...
    documentation.␟<command>COPY</command>を使用できない場合、準備された<command>INSERT</command>文を<link linkend="sql-prepare"><command>PREPARE</command></link>を使用して作成し、必要な回数だけ<command>EXECUTE</command>を実行する方が良いでしょう。
これにより、繰り返し行われる<command>INSERT</command>の解析と計画作成分のオーバーヘッドを省くことになります。
この機能のための方法はインタフェースによって異なります。
このインタフェースの文書の<quote>準備された文</quote>を参照してください。␞␞   </para>␞
␝   <para>␟    Note that loading a large number of rows using
    <command>COPY</command> is almost always faster than using
    <command>INSERT</command>, even if <command>PREPARE</command> is used and
    multiple insertions are batched into a single transaction.␟<command>COPY</command>を使用した大量の行のロードは、ほとんどすべての場合において、<command>INSERT</command>を使用するロードよりも高速です。
たとえ複数の挿入を単一トランザクションにまとめたとしても、またその際に<command>PREPARE</command>を使用したとしても、これは当てはまります。␞␞   </para>␞
␝   <para>␟    <command>COPY</command> is fastest when used within the same
    ...
    as all commands must write WAL otherwise.␟<command>COPY</command>は、前もって行われる<command>CREATE TABLE</command>または<command>TRUNCATE</command>コマンドと同一トランザクションで行った場合に、最速です。
...
この他の場合には、すべてのコマンドをWALに書き出さなければならないためです。␞␞   </para>␞
␝  <sect2 id="populate-rm-indexes">␟   <title>Remove Indexes</title>␟   <title>インデックスを削除する</title>␞␞␞
␝   <para>␟    If you are loading a freshly created table, the fastest method is to
    create the table, bulk load the table's data using
    <command>COPY</command>, then create any indexes needed for the
    table.  Creating an index on pre-existing data is quicker than
    updating it incrementally as each row is loaded.␟新規に作成したテーブルをロードする時、最速の方法は、テーブルを作成し、<command>COPY</command>を使用した一括ロードを行い、そのテーブルに必要なインデックスを作成することです。
既存のデータに対するインデックスを作成する方が、各行がロードされる度に段階的に更新するよりも高速です。␞␞   </para>␞
␝   <para>␟    If you are adding large amounts of data to an existing table,
    ...
    missing.␟既存のテーブルに大量のデータを追加しているのであれば、インデックスを削除し、テーブルをロード、その後にインデックスを再作成する方がよいかもしれません。
もちろん、他のユーザから見ると、インデックスが存在しない間データベースの性能は悪化します。
また、一意性インデックスを削除する前には熟考しなければなりません。
一意性制約によるエラー検査がその期間行われないからです。␞␞   </para>␞
␝  <sect2 id="populate-rm-fkeys">␟   <title>Remove Foreign Key Constraints</title>␟   <title>外部キー制約の削除</title>␞␞␞
␝   <para>␟    Just as with indexes, a foreign key constraint can be checked
    ...
    speed and loss of error checking while the constraint is missing.␟インデックスの場合と同様、外部キー制約は一行一行検査するよりも効率的に、<quote>まとめて</quote>検査することができます。
従って、外部キー制約を削除し、データをロード、そして、制約を再作成する方法は有用となることがあります。
ここでも、データロードの速度と、制約が存在しない間のエラー検査がないという点とのトレードオフがあります。␞␞   </para>␞
␝   <para>␟    What's more, when you load data into a table with existing foreign key
    ...
    up the load operation into smaller transactions.␟外部キー制約をすでに持つテーブルにデータをロードする時、新しい行はそれぞれ(行の外部キー制約を検査するトリガを発行しますので)サーバの待機中トリガイベントのリスト内に項目を要求します。
...
一時的な制約削除が受け入れられない場合に他に取り得る手段は、ロード操作をより小さなトランザクションに分割することだけかもしれません。␞␞   </para>␞
␝  <sect2 id="populate-work-mem">␟   <title>Increase <varname>maintenance_work_mem</varname></title>␟   <title><varname>maintenance_work_mem</varname>を増やす</title>␞␞␞
␝   <para>␟    Temporarily increasing the <xref linkend="guc-maintenance-work-mem"/>
    ...
    only useful when you are using one or both of the above techniques.␟大規模なデータをロードする時<xref linkend="guc-maintenance-work-mem"/>設定変数を一時的に増やすことで性能を向上させることができます。
これは、<command>CREATE INDEX</command>コマンドと<command>ALTER TABLE ADD FOREIGN KEY</command>の速度向上に役立ちます。
<command>COPY</command>自体には大して役立ちませんので、この助言は、上述の技法の片方または両方を使用している時にのみ有用です。␞␞   </para>␞
␝  <sect2 id="populate-max-wal-size">␟   <title>Increase <varname>max_wal_size</varname></title>␟   <title><varname>max_wal_size</varname>を増やす</title>␞␞␞
␝   <para>␟    Temporarily increasing the <xref linkend="guc-max-wal-size"/>
    ...
    reduced.␟大規模なデータをロードする時<xref linkend="guc-max-wal-size"/>設定変数を一時的に増やすことで高速化することができます。
...
大量のデータロードの際に一時的に<varname>max_wal_size</varname>を増加させることで、必要なチェックポイント数を減らすことができます。␞␞   </para>␞
␝  <sect2 id="populate-pitr">␟   <title>Disable WAL Archival and Streaming Replication</title>␟   <title>WALアーカイブ処理とストリーミングレプリケーションの無効化</title>␞␞␞
␝   <para>␟    When loading large amounts of data into an installation that uses
    ...
    recovery and standby server, which may lead to data loss.␟大量のデータをWALアーカイブ処理またはストリーミングレプリケーションを使用するインストレーションにロードする時、増加する大量のWALデータを処理するより、ロードが完了した後に新しくベースバックアップを取る方が高速です。
...
しかし、これらの変数を変更するにはサーバの再起動が必要となり、以前取得したベースバックアップがアーカイブリカバリやスタンバイサーバで使用できなくなりデータ消失につながる可能性があるため、注意してください。␞␞   </para>␞
␝   <para>␟    Aside from avoiding the time for the archiver or WAL sender to process the
    ...
    an <function>fsync</function> at the end than by writing WAL.)␟こうすると、アーカイブやWAL送信にWALデータを処理する時間を避けることの他に、実際のところ、特定のコマンドをより高速にします。
...
（これらは最後に<function>fsync</function>を実行することで、WALへの書き込みより安価にクラッシュした場合の安全性を保証することができます。）␞␞   </para>␞
␝  <sect2 id="populate-analyze">␟   <title>Run <command>ANALYZE</command> Afterwards</title>␟   <title>最後に<command>ANALYZE</command>を実行</title>␞␞␞
␝   <para>␟    Whenever you have significantly altered the distribution of data
    ...
    and <xref linkend="autovacuum"/> for more information.␟テーブル内のデータ分布を大きく変更した時は毎回、<link linkend="sql-analyze"><command>ANALYZE</command></link>を実行することを強く勧めます。
...
詳細は<xref linkend="vacuum-for-statistics"/>および<xref linkend="autovacuum"/>を参照してください。␞␞   </para>␞
␝  <sect2 id="populate-pg-dump">␟   <title>Some Notes about <application>pg_dump</application></title>␟   <title><application>pg_dump</application>に関するいくつかの注意</title>␞␞␞
␝   <para>␟    Dump scripts generated by <application>pg_dump</application> automatically apply
    ...
    from a <application>pg_dump</application> archive file.)␟<application>pg_dump</application>で生成されるダンプスクリプトは自動的に上のガイドラインのいくつかを適用します（すべてではありません）。
...
<application>psql</application>を使用してテキスト形式のダンプをロードする時と<application>pg_dump</application>のアーカイブファイルから<application>pg_restore</application>を使用してロードする時にも同じことが適用できます。）␞␞   </para>␞
␝   <para>␟    By default, <application>pg_dump</application> uses <command>COPY</command>, and when
    it is generating a complete schema-and-data dump, it is careful to
    load data before creating indexes and foreign keys.  So in this case
    several guidelines are handled automatically.  What is left
    for you to do is to:␟デフォルトでは、<application>pg_dump</application>は<command>COPY</command>を使用します。
スキーマとデータのダンプ全体を生成する場合、インデックスと外部キー制約を作成する前にデータをロードすることに注意してください。
ですので、この場合、ガイドラインのいくつかは自動的に行われます。
残された作業は以下のとおりです。␞␞    <itemizedlist>␞
␝      <para>␟       Set appropriate (i.e., larger than normal) values for
       <varname>maintenance_work_mem</varname> and
       <varname>max_wal_size</varname>.␟<varname>maintenance_work_mem</varname>および<varname>max_wal_size</varname>を適切な（つまり通常よりも大きな）値に設定します。␞␞      </para>␞
␝      <para>␟       If using WAL archiving or streaming replication, consider disabling
       ...
       base backup.␟WALアーカイブ処理またはストリーミングレプリケーションを使用する場合は、リストア時にこれを無効にすることを検討してください。
...
その後それらを正しい値に戻し、新規にベースバックアップを取ってください。␞␞      </para>␞
␝      <para>␟       Experiment with the parallel dump and restore modes of both
       ...
       significantly higher performance over the serial mode.␟<application>pg_dump</application>と<application>pg_restore</application>で、並列ダンプとリストア方式を実験して、利用する並列なジョブの最適な数を見つけて下さい。
<option>-j</option>オプションでダンプとリストアを並列に行なうのは逐次方式よりも大きく性能を向上させるでしょう。␞␞      </para>␞
␝      <para>␟       Consider whether the whole dump should be restored as a single
       ...
       transaction and have WAL archiving turned off.␟ダンプ全体を単一トランザクションとしてリストアすべきかどうか検討してください。
...
単一トランザクションで実行し、WALアーカイブを無効にしている場合、<command>COPY</command>コマンドは最も高速に行われます。␞␞      </para>␞
␝      <para>␟       If multiple CPUs are available in the database server, consider using
       <application>pg_restore</application>'s <option>--jobs</option> option.  This
       allows concurrent data loading and index creation.␟データベースサーバで複数のCPUが利用できるのであれば、<application>pg_restore</application>の<option>--jobs</option>オプションの利用を検討してください。
これによりデータのロードとインデックスの作成を同時に行うことができます。␞␞      </para>␞
␝      <para>␟       Run <command>ANALYZE</command> afterwards.␟この後で<command>ANALYZE</command>を実行してください。␞␞      </para>␞
␝   <para>␟    A data-only dump will still use <command>COPY</command>, but it does not
    drop or recreate indexes, and it does not normally touch foreign
    keys.␟データのみのダンプも<command>COPY</command>コマンドを使用しますが、インデックスの削除と再作成を行いません。
また、通常は外部キー制約を変更しません。␞␞␞
␝      <para>␟       You can get the effect of disabling foreign keys by using
       the <option>--disable-triggers</option> option &mdash; but realize that
       that eliminates, rather than just postpones, foreign key
       validation, and so it is possible to insert bad data if you use it.␟<option>--disable-triggers</option>オプションを使用して、外部キーを無効にさせることができます。
しかし、これは外部キー制約を遅らせるのではなく、除去することに注意してください。
そのため、これを使用すると不正なデータを挿入することができてしまいます。␞␞      </para>␞
␝␟    So when loading a data-only dump, it is up to you to drop and recreate
    ...
    <xref linkend="vacuum-for-statistics"/>
    and <xref linkend="autovacuum"/> for more information.␟したがって、データのみのダンプをロードする時、上の技法を使用したければ自らインデックスと外部キーを削除、再作成しなければなりません。
...
␝  <sect1 id="non-durability">␟   <title>Non-Durable Settings</title>␟   <title>永続性がない設定</title>␞␞␞
␝   <para>␟    Durability is a database feature that guarantees the recording of
    ...
    or corruption when these settings are used.␟永続性とは、サーバがクラッシュしたり電源が落ちたりしたとしても、コミットされたトランザクションが記録されていることを保証するデータベースの機能です。
...
␝      <para>␟       Place the database cluster's data directory in a memory-backed
       file system (i.e., <acronym>RAM</acronym> disk).  This eliminates all
       database disk I/O, but limits data storage to the amount of
       available memory (and perhaps swap).␟データベースクラスタのデータディレクトリをメモリ上のファイルシステム（つまり<acronym>RAM</acronym>ディスク）に設定します。
これはすべてのデータベースによるディスクI/Oを取り除きますが、データ量が利用可能なメモリ（およびスワップも使われるかもしれません）量までに制限されます。␞␞      </para>␞
␝      <para>␟       Turn off <xref linkend="guc-fsync"/>;  there is no need to flush
       data to disk.␟<xref linkend="guc-fsync"/>を無効にします。
データをディスクにフラッシュする必要がありません。␞␞      </para>␞
␝      <para>␟       Turn off <xref linkend="guc-synchronous-commit"/>;  there might be no
       need to force <acronym>WAL</acronym> writes to disk on every
       commit.  This setting does risk transaction loss (though not data
       corruption) in case of a crash of the <emphasis>database</emphasis>.␟<xref linkend="guc-synchronous-commit"/>を無効にします。
コミット毎にディスクに<acronym>WAL</acronym>書き出しを強制する必要がありません。
この設定は、<emphasis>データベース</emphasis>がクラッシュした場合、トランザクション損失（データ破損ではありません）の危険性があります。␞␞      </para>␞
␝      <para>␟       Turn off <xref linkend="guc-full-page-writes"/>;  there is no need
       to guard against partial page writes.␟<xref linkend="guc-full-page-writes"/>を無効にします。
部分的なページ書き出しから保護する必要がありません。␞␞      </para>␞
␝      <para>␟       Increase <xref linkend="guc-max-wal-size"/> and <xref
       linkend="guc-checkpoint-timeout"/>; this reduces the frequency
       of checkpoints, but increases the storage requirements of
       <filename>/pg_wal</filename>.␟<xref linkend="guc-max-wal-size"/>および<xref linkend="guc-checkpoint-timeout"/>を増加させます。
これによりチェックポイントの頻度が減少しますが、<filename>/pg_wal</filename>で必要とする容量が増加します。␞␞      </para>␞
␝      <para>␟       Create <link linkend="sql-createtable-unlogged">unlogged
       tables</link> to avoid <acronym>WAL</acronym> writes, though it
       makes the tables non-crash-safe.␟<acronym>WAL</acronym>書き出しを回避するためには、テーブルがクラッシュに対して安全ではなくなりますが、<link linkend="sql-createtable-unlogged">ログを取らないテーブル</link>を作成してください。␞␞      </para>␞
