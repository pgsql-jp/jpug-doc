...
␝  <refname>pg_verifybackup</refname>␟  <refpurpose>verify the integrity of a base backup of a
  <productname>PostgreSQL</productname> cluster</refpurpose>␟  <refpurpose><productname>PostgreSQL</productname>クラスタのベースバックアップの完全性を確認する</refpurpose>␞␞ </refnamediv>␞
␝ <refsect1>␟  <title>Description</title>␟  <title>説明</title>␞␞  <para>␞
␝  <para>␟   <application>pg_verifybackup</application> is used to check the
   ...
   format; a "tar" format backup can be checked after extracting it.␟<application>pg_verifybackup</application>は、<command>pg_basebackup</command>を使って取られたデータベースクラスタバックアップの完全性を、バックアップ時にサーバで生成された<literal>backup_manifest</literal>と比較して確認するために使われます。
バックアップは"plain"形式で保管されていなければなりません。"tar"形式のバックアップは展開後に確認できます。␞␞  </para>␞
␝  <para>␟   It is important to note that the validation which is performed by
   ...
   that commonly occur due to storage problems or user error.␟<application>pg_verifybackup</application>により実行される検証は、バックアップを利用しようとしている動作中のサーバにより実行される検査をすべて含む訳ではありませんし、含むことができる訳でもないことに注意するのは重要です。
このツールを使ったとしても、テストリストアを実行したり、結果のデータベースが期待した通りに動作し、正しいデータを含んでいるように見えることを検証したりすべきです。
しかしながら、<application>pg_verifybackup</application>は、ストレージの問題や利用者のエラーによる多くの問題を検出できます。␞␞  </para>␞
␝  <para>␟   Backup verification proceeds in four stages. First,
   ...
   <literal>pg_verifybackup</literal> will terminate with a fatal error.␟バックアップの検証は4つの段階で進みます。
第1に、<literal>pg_verifybackup</literal>が<literal>backup_manifest</literal>ファイルを読みます。
そのファイルが存在しない、読むことができない、不正である、バックアップディレクトリの<filename>pg_control</filename>とシステム識別子が一致しない、または、自身の内部チェックサムの検証に失敗した場合には、<literal>pg_verifybackup</literal>は致命的なエラーで終了します。␞␞  </para>␞
␝  <para>␟   Second, <literal>pg_verifybackup</literal> will attempt to verify that
   ...
   will necessarily also be missing.␟第2に、<literal>pg_verifybackup</literal>は、以下に記すようないくつかの例外を除いて、ディスクに保管されているデータファイルがサーバの送ろうと意図したデータファイルと完全に同一かを検証します。
...
ファイルだけが検査されます。ディレクトリの存在や欠如は、ディレクトリがなければ、そこに含まれるはずのファイルも必ずないという間接的なものを除き検証されません。␞␞  </para>␞
␝  <para>␟   Next, <literal>pg_verifybackup</literal> will checksum all the files,
   ...
   ignored in this step.␟次に、<literal>pg_verifybackup</literal>は、すべてのファイルのチェックサムを取り、そのチェックサムをマニフェスト内の値と比較し、計算されたチェックサムがマニフェストに保管されたチェックサムと一致しないファイルに対してエラーを出力します。
この段階は、前の段階でエラーとなったファイルに対しては実行されません。既に問題があると分かっているからです。
前の段階で無視されたファイルは、この段階でも無視されます。␞␞  </para>␞
␝  <para>␟   Finally, <literal>pg_verifybackup</literal> will use the manifest to
   ...
   by this method.␟最後に<literal>pg_verifybackup</literal>は、マニフェストを使って、バックアップを回復するのに必要な先行書き込みログ（WAL）レコードが存在し、それが読み込めて解析できるかを検証します。
<literal>backup_manifest</literal>は、必要となる先行書き込みログ（WAL）レコードに関する情報を含んでおり、<literal>pg_verifybackup</literal>は、その情報を使って、先行書き込みログ（WAL）レコードを解析するよう<literal>pg_waldump</literal>を呼び出します。
...
例えば、正しいチェックサムを持つものの馬鹿げた動作を指定する先行書き込みログ（WAL）レコードを生成するサーバのバグは、この方法では検出できません。␞␞  </para>␞
␝  <para>␟   Note that if extra WAL files which are not required to recover the backup
   ...
   of the server that generates a <literal>backup_manifest</literal> file.␟バックアップからの回復に必要でない余分なWALが存在すると、それはこのツールでは検査されないことに注意してください。その目的のために<literal>pg_waldump</literal>を別に呼び出して使うことはできます。
...
対照的に、データファイルの完全性の検査は、<literal>backup_manifest</literal>ファイルを生成したサーバのバージョンが何であれ動作します。␞␞  </para>␞
␝ <refsect1>␟  <title>Options</title>␟  <title>オプション</title>␞␞␞
␝   <para>␟    <application>pg_verifybackup</application> accepts the following
    command-line arguments:␟<application>pg_verifybackup</application>は以下のコマンドライン引数を受け付けます。␞␞␞
␝       <para>␟        Exit as soon as a problem with the backup is detected. If this option
        ...
        report all problems detected as errors.␟バックアップで問題が検出され次第、終了します。
このオプションが指定されていなければ、<literal>pg_verifybackup</literal>は問題が検出された後もバックアップの検査を続け、検出した問題をすべてエラーとして報告します。␞␞       </para>␞
␝       <para>␟        Ignore the specified file or directory, which should be expressed
        ...
        can be specified multiple times.␟バックアップ内に実際に存在するデータファイルの一覧と<literal>backup_manifest</literal>ファイル内の一覧を比較する時に、指定されたファイルやディレクトリを無視します。相対パス名で指定してください。
ディレクトリが指定された場合、このオプションは、その位置をルートとするサブツリー全体に影響します。
相対パス名が指定されたパス名に一致する場合、余分なファイル、足りないファイル、ファイルサイズの違い、チェックサムの不一致の報告は抑制されます。
このオプションは複数回指定できます。␞␞       </para>␞
␝       <para>␟        Use the manifest file at the specified path, rather than one located
        in the root of the backup directory.␟バックアップディレクトリのルートにあるものではなく、指定されたパスのマニフェストファイルを使用します。␞␞       </para>␞
␝       <para>␟        Don't attempt to parse write-ahead log data that will be needed
        to recover from this backup.␟このバックアップからの回復に必要な先行書き込みログ（WAL）データを解析しません。␞␞       </para>␞
␝       <para>␟        Enable progress reporting. Turning this on will deliver a progress
        report while verifying checksums.␟進行状況のレポートを有効にします。
このオプションを有効にすると、チェックサムの検証中に進行状況をレポートします。␞␞       </para>␞
␝       <para>␟        This option cannot be used together with the option
        <option>--quiet</option>.␟このオプションは、オプション<option>--quiet</option>と一緒には使用できません。␞␞       </para>␞
␝       <para>␟        Don't print anything when a backup is successfully verified.␟バックアップの検証に成功した場合、何も表示しません。␞␞       </para>␞
␝       <para>␟        Do not verify data file checksums. The presence or absence of
        files and the sizes of those files will still be checked. This is
        much faster, because the files themselves do not need to be read.␟データファイルのチェックサムを検証しません。
ファイルの存在、欠如とファイルのサイズは検査されます。
ファイル自身を読み込む必要がありませんので、これはずっと速いです。␞␞       </para>␞
␝       <para>␟        Try to parse WAL files stored in the specified directory, rather than
        in <literal>pg_wal</literal>. This may be useful if the backup is
        stored in a separate location from the WAL archive.␟<literal>pg_wal</literal>ではなく、指定されたディレクトリのWALファイルを解析しようとします。
バックアップがWALアーカイブとは別の場所に保管されている場合、これは有用でしょう。␞␞       </para>␞
␝   <para>␟    Other options are also available:␟他のオプションも使用可能です。␞␞␞
␝       <para>␟       Print the <application>pg_verifybackup</application> version and exit.␟<application>pg_verifybackup</application>のバージョンを表示し、終了します。␞␞       </para>␞
␝       <para>␟       Show help about <application>pg_verifybackup</application> command
       line arguments, and exit.␟<application>pg_verifybackup</application>のコマンドライン引数に関するヘルプを表示し、終了します。␞␞       </para>␞
␝ <refsect1>␟  <title>Examples</title>␟  <title>例</title>␞␞␞
␝  <para>␟   To create a base backup of the server at <literal>mydbserver</literal> and
   verify the integrity of the backup:␟<literal>mydbserver</literal>でサーバのバックアップを作成し、バックアップの完全性を検証します。␞␞<screen>␞
␝  <para>␟   To create a base backup of the server at <literal>mydbserver</literal>, move
   the manifest somewhere outside the backup directory, and verify the
   backup:␟<literal>mydbserver</literal>でサーバのバックアップを作成し、マニフェストをバックアップディレクトリの外のどこかに移動し、バックアップを検証します。␞␞<screen>␞
␝  <para>␟   To verify a backup while ignoring a file that was added manually to the
   backup directory, and also skipping checksum verification:␟バックアップディレクトリに手で追加されたファイルを無視し、チェックサムの検証も省略してバックアップを検証します。␞␞<screen>␞
␝ <refsect1>␟  <title>See Also</title>␟  <title>関連項目</title>␞␞␞
␝␟<prompt>$</prompt> <userinput>pg_basebackup -h mydbserver -D /usr/local/pgsql/data</userinput> <prompt>$</prompt> <userinput>edit /usr/local/pgsql/data/note.to.self</userinput> <prompt>$</prompt> <userinput>pg_verifybackup --ignore=note.to.self --skip-checksums /usr/local/pgsql/data</userinput> </screen></para> </screen></para>␟no translation␞␞␞
