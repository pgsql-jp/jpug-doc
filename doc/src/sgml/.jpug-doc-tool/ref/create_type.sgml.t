...
␝  <refname>CREATE TYPE</refname>␟  <refpurpose>define a new data type</refpurpose>␟  <refpurpose>新しいデータ型を定義する</refpurpose>␞␞ </refnamediv>␞
␝ <refsect1>␟  <title>Description</title>␟  <title>説明</title>␞␞␞
␝  <para>␟   <command>CREATE TYPE</command> registers a new data type for use in
   the current database.  The user who defines a type becomes its
   owner.␟<command>CREATE TYPE</command>は、現在のデータベースで使用できる新しいデータ型を登録します。
型を定義したユーザがその所有者となります。␞␞  </para>␞
␝  <para>␟   If a schema name is given then the type is created in the specified
   ...
   table in the same schema.)␟スキーマ名が与えられている場合、型は指定されたスキーマに作成されます。
スキーマ名がなければ、その型は現在のスキーマに作成されます。
型名は、同じスキーマにある既存の型もしくはドメインとは、異なる名前にする必要があります。
（さらに、テーブルはデータ型と関連しているため、データ型名は同じスキーマのテーブル名とも競合しないようにしてください。）␞␞  </para>␞
␝  <para>␟   There are five forms of <command>CREATE TYPE</command>, as shown in the
   ...
   as discussed in those sections.␟上の構文概要に示すように、<command>CREATE TYPE</command>には５つの構文があります。
...
シェル型は、範囲型と基本型を作成するときの前方参照として必要となるもので、それぞれの節で説明します。␞␞  </para>␞
␝  <refsect2>␟   <title>Composite Types</title>␟   <title>複合型</title>␞␞␞
␝  <para>␟   The first form of <command>CREATE TYPE</command>
   ...
   return type of a function.␟<command>CREATE TYPE</command>の最初の構文を使用すると、複合型を作成できます。
...
スタンドアローンの複合型は、例えば関数の引数や戻り値の型として有用です。␞␞  </para>␞
␝  <para>␟   To be able to create a composite type, you must
   have <literal>USAGE</literal> privilege on all attribute types.␟複合型を作成するためには、すべての属性型に対して<literal>USAGE</literal>権限を持たなければなりません。␞␞  </para>␞
␝  <refsect2 id="sql-createtype-enum">␟   <title>Enumerated Types</title>␟   <title>列挙型</title>␞␞␞
␝   <para>␟    The second form of <command>CREATE TYPE</command> creates an enumerated
    ...
    linkend="sql-altertype"><command>ALTER TYPE</command></link>.)␟<command>CREATE TYPE</command>の2つ目の構文を使用すると、<xref linkend="datatype-enum"/>で説明する列挙型（enum）を作成します。
...
(ラベルのない列挙型を作成できますが、<link linkend="sql-altertype"><command>ALTER TYPE</command></link>を使ってラベルを少なくとも1つ追加するまでは、そのような型は値を保持するのに使えません。)␞␞   </para>␞
␝  <refsect2 id="sql-createtype-range">␟   <title>Range Types</title>␟   <title>範囲型</title>␞␞␞
␝   <para>␟    The third form of <command>CREATE TYPE</command> creates a new
    range type, as described in <xref linkend="rangetypes"/>.␟<command>CREATE TYPE</command>の三番目の構文は、<xref linkend="rangetypes"/>で説明する範囲型を新規に作成します。␞␞   </para>␞
␝   <para>␟    The range type's <replaceable class="parameter">subtype</replaceable> can
    ...
    class="parameter">collation</replaceable> option.␟範囲型の<replaceable class="parameter">subtype</replaceable>は、関連する（範囲型の値を順序を決定するための）b-tree演算子クラスを持つ任意の型を取ることができます。
...
派生元型が照合順序変更可能であり、範囲の順序付けでデフォルト以外の照合順序を使用したい場合は、<replaceable class="parameter">collation</replaceable>オプションで使用したい照合順序を指定してください。␞␞   </para>␞
␝   <para>␟    The optional <replaceable class="parameter">canonical</replaceable>
    ...
    automatically replaces the shell type entry with a valid range type.␟<replaceable class="parameter">canonical</replaceable>関数(省略可能)は、定義する範囲型の引数を１つ取り、同じ型の値を返さなければなりません。
...
これは自動的にシェル型の項目を有効な範囲型に置き換えます。␞␞   </para>␞
␝   <para>␟    The optional <replaceable class="parameter">subtype_diff</replaceable>
    ...
    information.␟<replaceable class="parameter">subtype_diff</replaceable>関数(省略可能)は、<replaceable class="parameter">subtype</replaceable>型の２つの値を引数として取り、与えられた２つの値の差異を表す<type>double precision</type>型を返さなければなりません。
これは省略することができますが、提供することでその範囲型の列に対するGiSTインデックスの効率を大きく向上させることができます。
詳細については<xref linkend="rangetypes-defining"/>を参照してください。␞␞   </para>␞
␝   <para>␟    The optional <replaceable class="parameter">multirange_type_name</replaceable>
    ...
    <literal>_multirange</literal> suffix to the range type name.␟<replaceable class="parameter">multirange_type_name</replaceable>パラメータ(省略可能)は、対応する多重範囲型の名前を指定します。
指定されなければ、この名前は自動的に以下のように選ばれます。
範囲型の名前が部分文字列<literal>range</literal>を含んでいれば、多重範囲型の名前は、範囲型の名前における部分文字列<literal>range</literal>を<literal>multirange</literal>で置き換えることで作られます。
そうでなければ、多重範囲型の名前は範囲型の名前の末尾に<literal>_multirange</literal>を追加することで作られます。␞␞   </para>␞
␝  <refsect2>␟   <title>Base Types</title>␟   <title>基本型</title>␞␞␞
␝  <para>␟   The fourth form of <command>CREATE TYPE</command> creates a new base type
   ...
   confuse or even crash the server.)␟<command>CREATE TYPE</command>の４つ目の構文を使用すると、基本型（スカラ型）を新しく作成できます。
新しい基本型を作成するにはスーパーユーザでなければなりません。
（エラーがある型定義が混乱を招き、サーバがクラッシュすることすらあるため、この制限がなされました。）␞␞  </para>␞
␝  <para>␟   The parameters can appear in any order, not only that
   ...
   or another low-level language.␟パラメータは、上述の順番である必要はなく、任意の順番で指定することができ、多くは省略可能です。
...
<replaceable class="parameter">receive_function</replaceable>関数、<replaceable class="parameter">send_function</replaceable>関数、<replaceable class="parameter">type_modifier_input_function</replaceable>関数、<replaceable class="parameter">type_modifier_output_function</replaceable>関数、<replaceable class="parameter">analyze_function</replaceable>関数、および<replaceable class="parameter">subscript_function</replaceable>は省略可能です。
通常、これらの関数は、C言語やその他の低レベル言語で作成されなければなりません。␞␞  </para>␞
␝  <para>␟   The <replaceable class="parameter">input_function</replaceable>
   ...
   Output functions are not invoked for NULL values.␟<replaceable class="parameter">input_function</replaceable>は、型のテキストによる外部表現を内部表現形式に変換するものであり、その型用に定義される演算子や関数で使用されます。
...
出力関数はNULL値に対して呼び出されることはありません。␞␞  </para>␞
␝  <para>␟   The optional <replaceable class="parameter">receive_function</replaceable>
   ...
   Send functions are not invoked for NULL values.␟<replaceable class="parameter">receive_function</replaceable>では、型のバイナリによる外部表現を内部表現に変換します。この関数は省略可能です。
この関数が与えられない場合、この型ではバイナリ入力を行うことができません。
バイナリ表現の方法は、適度な可搬性を保ちつつ、内部表現への変換コストが小さくなるよう選択すべきです
（例えば標準の整数データ型は、外部バイナリ表現としてはネットワークバイトオーダーを使用し、内部表現ではマシン固有のバイトオーダーを使用します）。
...
送信関数はNULL値に対して呼び出されません。␞␞  </para>␞
␝  <para>␟   You should at this point be wondering how the input and output functions
   ...
   used normally.␟ここで、新しいデータ型を作成する前に入力関数と出力関数を作成する必要があるのに、どのようにしてそれらの関数で新しいデータ型を戻り値や入力として宣言できるのか、疑問に思うかもしれません。
...
最後に完全な定義を持った<command>CREATE TYPE</command>によって、シェル型の項目が完全かつ有効な型定義に置き換わり、新しい型を普通に使用することができるようになります。␞␞  </para>␞
␝  <para>␟   The optional
   <replaceable class="parameter">type_modifier_input_function</replaceable>
   ...
   value enclosed in parentheses.␟<replaceable class="parameter">type_modifier_input_function</replaceable>と<replaceable class="parameter">type_modifier_output_function</replaceable>は必須ではありませんが、型が修飾子をサポートする場合は必要です。
...
デフォルトの表示用書式が保管されたtypmod整数値を括弧で括ったものと一致している場合は、<replaceable class="parameter">type_modifier_output_function</replaceable>を省略することができます。␞␞  </para>␞
␝  <para>␟   The optional <replaceable class="parameter">analyze_function</replaceable>
   ...
   in <filename>src/include/commands/vacuum.h</filename>.␟オプションの<replaceable class="parameter">analyze_function</replaceable>は、このデータ型の列に対する、型固有の統計情報の収集を行います。
...
解析関数用のAPIの詳細は、<filename>src/include/commands/vacuum.h</filename>にあります。␞␞  </para>␞
␝  <para>␟   The optional <replaceable class="parameter">subscript_function</replaceable>
   ...
   <xref linkend="sql-createtype-array"/> below.␟オプションの<replaceable class="parameter">subscript_function</replaceable>は、SQLコマンド内で指定されたデータ型に添字をつけることを許可します。
この関数を指定することで、指定されたデータ型が<quote>真の</quote>配列型とみなされる訳ではありません。たとえば、<literal>ARRAY[]</literal>生成の結果型の候補にはなりません。
ですが、その型の値に添字をつけることが、そこからデータを取り出す自然な表記法であるなら、その意味を定義するのに<replaceable class="parameter">subscript_function</replaceable>を書くことができます。
添字関数は<type>internal</type>型の引数を1つ取り、添字を実装しているメソッド(関数)の構造体へのポインタである<type>internal</type>の結果を返すよう宣言しなければなりません。
添字関数の詳細なAPIは<filename>src/include/nodes/subscripting.h</filename>にあります。
<filename>src/backend/utils/adt/arraysubs.c</filename>にある配列の実装や<filename>contrib/hstore/hstore_subs.c</filename>にあるより単純なコードを読むのも有用かもしれません。
追加の情報は以下の<xref linkend="sql-createtype-array"/>にあります。␞␞  </para>␞
␝  <para>␟   While the details of the new type's internal representation are only
   ...
   to access it directly.)␟新しい型の内部表現の詳細を理解しなければならないのは、これらのI/O関数とその型に関連して動作するユーザ定義の関数のみですが、内部表現には、<productname>PostgreSQL</productname>に対し宣言しなければならない複数の属性値があります。
...
（長さフィールドは多くの場合<xref linkend="storage-toast"/>に記述されているようにエンコードされており、それに直接アクセスすることは賢明ではないことに注意してください。）␞␞  </para>␞
␝  <para>␟   The optional flag <literal>PASSEDBYVALUE</literal> indicates that
   ...
   (4 bytes on some machines, 8 bytes on others).␟オプションの<literal>PASSEDBYVALUE</literal>フラグは、このデータ型の値が参照ではなく値によって渡されることを示します。
値によって渡される型は固定長でなければならず、その内部表現は<type>Datum</type>型のサイズ（4バイトのマシンもあれば8バイトのマシンもあります）を超えてはいけません。␞␞  </para>␞
␝  <para>␟   The <replaceable class="parameter">alignment</replaceable> parameter
   specifies the storage alignment required for the data type.  The
   allowed values equate to alignment on 1, 2, 4, or 8 byte boundaries.
   Note that variable-length types must have an alignment of at least
   4, since they necessarily contain an <type>int4</type> as their first component.␟<replaceable class="parameter">alignment</replaceable>パラメータは、そのデータ型の格納の際に必要な整列を指定します。
設定可能な値は、1、2、4、8バイト境界での整列です。
可変長の型は最低でも4の整列を持たなければならないことに注意してください。
最初の要素として<type>int4</type>を持たなければならないからです。␞␞  </para>␞
␝  <para>␟   The <replaceable class="parameter">storage</replaceable> parameter
   ...
   <literal>external</literal> items.)␟<replaceable class="parameter">storage</replaceable>パラメータを使用することで、可変長データ型を格納する際の戦略を選択することができます。
...
（行を収めるために他に方法がない場合にはメインテーブルから削除されてしまう可能性がありますが、<literal>extended</literal>や<literal>external</literal>が指定されたアイテムよりも優先してメインテーブルに残されます。）␞␞  </para>␞
␝  <para>␟   All <replaceable class="parameter">storage</replaceable> values other
   ...
   SET STORAGE</literal>.␟<literal>plain</literal>を除くすべての<replaceable class="parameter">storage</replaceable>の値は、そのデータ型の関数が、<xref linkend="storage-toast"/>および<xref linkend="xtypes-toast"/>に記述されているように<firstterm>toast</firstterm>された値を処理できることを暗示します。
その他の特定の値を指定するのは、TOAST可能なデータ型の列について、単にデフォルトのTOAST戦略を決めるだけです。
ユーザは個々の列について<literal>ALTER TABLE SET STORAGE</literal>を使って他の戦略を選択できます。␞␞  </para>␞
␝  <para>␟   The <replaceable class="parameter">like_type</replaceable> parameter
   ...
   existing type in some fashion.␟<replaceable class="parameter">like_type</replaceable>パラメータは、何らかの既存のデータ型から複製するという、データ型の基本表現プロパティを指定する、別の方法を提供します。
...
新しい型の低レベル実装にある流儀に従った既存の型を<quote>移す</quote>時に、この方法で表現を指定することが特に有用です。␞␞  </para>␞
␝  <para>␟   The <replaceable class="parameter">category</replaceable> and
   ...
   ASCII character other than an upper-case letter to name such a category.␟<replaceable class="parameter">category</replaceable>と<replaceable class="parameter">preferred</replaceable>パラメータは、あいまいな状況でどの暗黙的なキャストが適用されるかについての制御を補助するために使用することができます。
...
そのようなカテゴリの命名には大文字以外の任意のASCII文字を選択してください。␞␞  </para>␞
␝  <para>␟   A default value can be specified, in case a user wants columns of the
   ...
   clause attached to a particular column.)␟ユーザがそのデータ型の列のデフォルトをNULL以外にしたい場合は、デフォルト値を指定することができます。
デフォルト値は<literal>DEFAULT</literal>キーワードで指定してください。
（この方法で指定されたデフォルト値を、特定の列に付与された、明示的な<literal>DEFAULT</literal>句によって上書きすることができます。）␞␞  </para>␞
␝  <para>␟   To indicate that a type is a fixed-length array type,
   ...
   see <xref linkend="sql-createtype-array"/> below.␟データ型が固定長の配列型であることを示すには、<literal>ELEMENT</literal>キーワードを使用して配列要素の型を指定してください。
例えば、4バイト整数（<type>int4</type>）の配列を定義するには、<literal>ELEMENT = int4</literal>と指定してください。
より詳細は以下の<xref linkend="sql-createtype-array"/>を参照してください。␞␞  </para>␞
␝  <para>␟   To indicate the delimiter to be used between values in the external
   ...
   with the array element type, not the array type itself.␟この型による配列の外部形式における値間の区切り文字を示すために、<replaceable class="parameter">delimiter</replaceable>で特定の文字を設定することができます。
デフォルトの区切り文字はカンマ（'<literal>,</literal>'）です。
この区切り文字は、配列要素の型に関係するものであり、配列型自体に関係するものでないことに注意してください。␞␞  </para>␞
␝  <para>␟   If the optional Boolean
   ...
   automatically merely by marking the type collatable.␟論理型の<replaceable class="parameter">collatable</replaceable>パラメータ(省略可能)が真の場合、<literal>COLLATE</literal>句を使用することによって、型の列定義と式は照合順序情報を持つことができます。
照合順序情報を実際に使用するかどうかは、型に対する操作を行う関数実装に任されています。
照合順序を設定可能な型を作成することにより、これが自動的に行われることはありません。␞␞  </para>␞
␝␟  <refsect2 id="sql-createtype-array" xreflabel="Array Types">
   <title>Array Types</title>␟  <refsect2 id="sql-createtype-array" xreflabel="配列型">
   <title>配列型</title>␞␞␞
␝   <para>␟    Whenever a user-defined type is created,
    ...
    element type's owner or schema, and is dropped if the element type is.␟ユーザ定義型が作成されると、<productname>PostgreSQL</productname>は、自動的に関連する配列型を作成します。
...
さらに、この型は、ユーザ定義型に対してシステムが<literal>ARRAY[]</literal>のように構成時に使うものです。
配列型はその要素となる型の所有者とスキーマのなんらかの変更に追従し、また、要素となる型が削除された場合に削除されます。␞␞   </para>␞
␝   <para>␟    You might reasonably ask why there is an <option>ELEMENT</option>
    ...
    zero, rather than from one as for variable-length arrays.␟「システムが自動的に配列型を正しく作成するのであれば、<option>ELEMENT</option>オプションはどうして存在するのだろう」と疑問に思うのも道理です。
<option>ELEMENT</option>が意味を持つ、主な場合は次のような条件を満たす固定長の型を作成する時です。
...
歴史的な理由（明らかに間違いなのですが、変更するには遅すぎたため）により、固定長配列型への要素番号指定は0から始まり、可変長配列の場合は1から始まります。␞␞   </para>␞
␝   <para>␟    Specifying the <option>SUBSCRIPT</option> option allows a data type to
    ...
    fixed-length type without also writing <option>SUBSCRIPT</option>.␟<option>SUBSCRIPT</option>オプションを指定すると、たとえシステムが配列型とみなさない場合でも、データ型に添字をつけることができるようになります。
固定長の配列について述べた振舞いは、実は<option>SUBSCRIPT</option>ハンドラ関数<function>raw_array_subscript_handler</function>により実装されており、固定長型に対して<option>SUBSCRIPT</option>を書かずに<option>ELEMENT</option>を指定した場合には自動的に、この関数が使われます。␞␞   </para>␞
␝   <para>␟    When specifying a custom <option>SUBSCRIPT</option> function, it is
    ...
    type won't be allowed if there are any columns of the dependent type.␟カスタムの<option>SUBSCRIPT</option>関数を指定した場合には、<option>SUBSCRIPT</option>ハンドラ関数が何を返すべきか見つけるために<structfield>typelem</structfield>を調べる必要がない限り、<option>ELEMENT</option>を指定することは必要ではありません。
<option>ELEMENT</option>を指定すると、新しい型が要素型を含んでいる、すなわち要素型に何らかの形で物理的に依存しているとシステムが仮定することに注意してください。このため、例えば、依存する型の列があれば要素型の属性を変更することはできません。␞␞   </para>␞
␝ <refsect1>␟  <title>Parameters</title>␟  <title>パラメータ</title>␞␞␞
␝     <para>␟      The name (optionally schema-qualified) of a type to be created.␟作成するデータ型の名前です（スキーマ修飾名も可）。␞␞     </para>␞
␝     <para>␟      The name of an attribute (column) for the composite type.␟複合型用の属性（列）名です。␞␞     </para>␞
␝     <para>␟      The name of an existing data type to become a column of the
      composite type.␟複合型の列となる、既存のデータ型の名前です。␞␞     </para>␞
␝     <para>␟      The name of an existing collation to be associated with a column of
      a composite type, or with a range type.␟複合型の列または範囲型に関連付けされる、既存の照合順序の名前です。␞␞     </para>␞
␝     <para>␟      A string literal representing the textual label associated with
      one value of an enum type.␟列挙型の1つの値に関連付けられるテキスト形式のラベルを表す、文字列リテラルです。␞␞     </para>␞
␝     <para>␟      The name of the element type that the range type will represent ranges
      of.␟範囲型がその範囲の対象として表現する、要素型の名前です。␞␞     </para>␞
␝     <para>␟      The name of a b-tree operator class for the subtype.␟派生元型のb-tree演算子クラスの名前です。␞␞     </para>␞
␝     <para>␟      The name of the canonicalization function for the range type.␟範囲型の正規化関数の名前です。␞␞     </para>␞
␝     <para>␟      The name of a difference function for the subtype.␟派生元型の差異をとる関数の名前です。␞␞     </para>␞
␝     <para>␟      The name of the corresponding multirange type.␟対応する多重範囲型の名前です。␞␞     </para>␞
␝     <para>␟      The name of a function that converts data from the type's
      external textual form to its internal form.␟指定された型のテキストによる外部形式のデータを内部形式に変換する関数の名前です。␞␞     </para>␞
␝     <para>␟      The name of a function that converts data from the type's
      internal form to its external textual form.␟指定された型の内部形式のデータをテキストによる外部形式に変換する関数の名前です。␞␞     </para>␞
␝     <para>␟      The name of a function that converts data from the type's
      external binary form to its internal form.␟指定された型のバイナリによる外部形式のデータを内部形式に変換する関数の名前です。␞␞     </para>␞
␝     <para>␟      The name of a function that converts data from the type's
      internal form to its external binary form.␟指定された型の内部形式のデータをバイナリによる外部形式に変換する関数の名前です。␞␞     </para>␞
␝     <para>␟      The name of a function that converts an array of modifier(s) for the type
      into internal form.␟型に関する修飾子の配列を内部形式に変換する関数の名前です。␞␞     </para>␞
␝     <para>␟      The name of a function that converts the internal form of the type's
      modifier(s) to external textual form.␟内部形式の型修飾子をテキストの外部形式に変換する関数の名前です。␞␞     </para>␞
␝     <para>␟      The name of a function that performs statistical analysis for the
      data type.␟指定したデータ型の統計情報解析を行う関数の名前です。␞␞     </para>␞
␝     <para>␟      The name of a function that defines what subscripting a value of the
      data type does.␟指定したデータ型の値に添字をつけることを定義する関数の名前です。␞␞     </para>␞
␝     <para>␟      A numeric constant that specifies the length in bytes of the new
      type's internal representation.  The default assumption is that
      it is variable-length.␟新しいデータ型の内部表現のバイト長を表す数値定数です。
デフォルトでは、可変長であるとみなされます。␞␞     </para>␞
␝     <para>␟      The storage alignment requirement of the data type.  If specified,
      it must be <literal>char</literal>, <literal>int2</literal>,
      <literal>int4</literal>, or <literal>double</literal>; the
      default is <literal>int4</literal>.␟データ型の格納整列条件です。
このオプションを指定する場合は、<literal>char</literal>、<literal>int2</literal>、<literal>int4</literal>、<literal>double</literal>のいずれかでなければなりません。
デフォルトは<literal>int4</literal>です。␞␞     </para>␞
␝     <para>␟      The storage strategy for the data type.  If specified, must be
      <literal>plain</literal>, <literal>external</literal>,
      <literal>extended</literal>, or <literal>main</literal>; the
      default is <literal>plain</literal>.␟データ型の格納戦略です。
...
デフォルトは<literal>plain</literal>です。␞␞     </para>␞
␝     <para>␟      The name of an existing data type that the new type will have the
      ...
      specification elsewhere in this <command>CREATE TYPE</command> command.␟新しい型に同じ表現を持たせる既存のデータ型の名前です。
<replaceable class="parameter">internallength</replaceable>、<replaceable class="parameter">passedbyvalue</replaceable>、<replaceable class="parameter">alignment</replaceable>、<replaceable class="parameter">storage</replaceable>の値が、この<command>CREATE TYPE</command>コマンドのどこかで明示的な指定により上書きされない限り、型から複製されます。␞␞     </para>␞
␝     <para>␟      The category code (a single ASCII character) for this type.
      ...
      other ASCII characters in order to create custom categories.␟この型用のカテゴリコード（単一のASCII文字）です。
デフォルトは<quote>ユーザ定義型</quote>を表す<literal>'U'</literal>です。
他の標準カテゴリコードを<xref linkend="catalog-typcategory-table"/>に示します。
独自のカテゴリを作成するために他のASCII文字を選択することもできます。␞␞     </para>␞
␝     <para>␟      True if this type is a preferred type within its type category,
      ...
      could cause surprising changes in behavior.␟この型がカテゴリ内で優先される型である場合に真、さもなくば偽です。
デフォルトは偽です。
動作に予想外の変化を引き起こしますので既存の型カテゴリに新しく優先される型を作成することには十分注意してください。␞␞     </para>␞
␝     <para>␟      The default value for the data type.  If this is omitted, the
      default is null.␟そのデータ型のデフォルト値です。
省略された場合、デフォルトはNULLです。␞␞     </para>␞
␝     <para>␟      The type being created is an array; this specifies the type of
      the array elements.␟配列型を作成する場合、その配列の要素の型を指定します。␞␞     </para>␞
␝     <para>␟      The delimiter character to be used between values in arrays made
      of this type.␟このデータ型による配列で、値間の区切り文字として使われる文字です。␞␞     </para>␞
␝     <para>␟      True if this type's operations can use collation information.
      The default is false.␟この型を操作する時に照合順序情報を使用することができる場合に真を取ります。
デフォルトは偽です。␞␞     </para>␞
␝ <refsect1 id="sql-createtype-notes">␟  <title>Notes</title>␟  <title>注釈</title>␞␞␞
␝  <para>␟   Because there are no restrictions on use of a data type once it's been
   ...
   while converting it to or from external form.␟一度作成したデータ型の使用には制限はありませんので、基本型または範囲型の作成は型定義で言及した関数の実行権をPUBLICに対して付与することと同じです。
この種の型定義において有用な関数では、これは通常問題になりません。
しかし、外部形式から、または、外部形式への変換を行う時に、その関数が<quote>秘密の</quote>情報を必要とする場合、型を設計する前に熟考してください。␞␞  </para>␞
␝  <para>␟   Before <productname>PostgreSQL</productname> version 8.3, the name of
   ...
   associated with a given type.␟<productname>PostgreSQL</productname>バージョン8.3より前のバージョンでは、生成される配列型の名前は常に要素型の名前の前に１つのアンダースコア文字（<literal>_</literal>）を付けたものになりました。
...
代わりに、<structname>pg_type</structname>.<structfield>typarray</structfield>を使用して、指定した型に関連した配列型を特定してください。␞␞  </para>␞
␝  <para>␟   It may be advisable to avoid using type and table names that begin with
   ...
   beginning with underscores always represent arrays.␟アンダースコアから始まる型やテーブル名の使用を避けることが賢明です。
サーバは生成された配列型名称をユーザ指定の名前と競合しないように変更しますが、混乱する危険があります。
特に古いクライアントソフトウェアを使用する場合、名前がアンダースコアから始まる型を常に配列を表すものと想定しているかもしれません。␞␞  </para>␞
␝  <para>␟   Before <productname>PostgreSQL</productname> version 8.2, the shell-type
   creation syntax
   ...
   function is written in C.␟<productname>PostgreSQL</productname>バージョン8.2より前まででは、シェル型を作成する<literal>CREATE TYPE <replaceable>name</replaceable></literal>構文は存在しません。
...
また、関数定義における単純なタイプミスの結果作成されるシェル型によって起こるカタログの混乱を防止するため、入力関数がCで作成された場合にのみこの方法によってシェル型が作成されます。␞␞  </para>␞
␝  <para>␟   In <productname>PostgreSQL</productname> version 16 and later,
   ...
   information.␟<productname>PostgreSQL</productname>バージョン16以降では、以前のバージョンのように<function>ereport()</function>例外を発生するのではなく、新しい<function>errsave()</function>/<function>ereturn()</function>機構を使用して、基本型の入力関数が<quote>ソフトな</quote>エラーを返すことが望ましいです。
詳細については、<filename>src/backend/utils/fmgr/README</filename>を参照してください。␞␞  </para>␞
␝ <refsect1>␟  <title>Examples</title>␟  <title>例</title>␞␞␞
␝  <para>␟   This example creates a composite type and uses it in
   a function definition:␟次の例では、複合型を作成し、それを関数定義で使用します。␞␞<programlisting>␞
␝  <para>␟   This example creates an enumerated type and uses it in
   a table definition:␟次の例では、列挙型を作成し、それをテーブル定義に使用します。␞␞<programlisting>␞
␝  <para>␟   This example creates a range type:␟次の例では、範囲型を作成します。␞␞<programlisting>␞
␝  <para>␟   This example creates the base data type <type>box</type> and then uses the
   type in a table definition:␟次の例では、基本データ型<type>box</type>を作成し、その型をテーブル定義の中で使用しています。␞␞<programlisting>␞
␝  <para>␟   If the internal structure of <type>box</type> were an array of four
   <type>float4</type> elements, we might instead use:␟<type>box</type>型の内部構造が<type>float4</type>型が4つの配列の場合、このように書き換えることもできます。␞␞<programlisting>␞
␝</programlisting>␟   which would allow a box value's component numbers to be accessed
   by subscripting.  Otherwise the type behaves the same as before.␟このようにすると、box値の要素に要素番号でアクセスできます。
その他は、上の例と同様の動作をします。␞␞  </para>␞
␝  <para>␟   This example creates a large object type and uses it in
   a table definition:␟次の例では、ラージオブジェクト型を作成し、テーブル定義にてそれを使用します。␞␞<programlisting>␞
␝  <para>␟   More examples, including suitable input and output functions, are
   in <xref linkend="xtypes"/>.␟その他の例は、<xref linkend="xtypes"/>を参照してください。ここには、入力関数、出力関数などを使った例があります。␞␞  </para>␞
␝ <refsect1 id="sql-createtype-compatibility">␟  <title>Compatibility</title>␟  <title>互換性</title>␞␞␞
␝  <para>␟   The first form of the <command>CREATE TYPE</command> command, which
   ...
   implemented in <productname>PostgreSQL</productname>.␟複合型を作成する、最初の<command>CREATE TYPE</command>コマンドの構文は標準<acronym>SQL</acronym>に準拠しています。
他の構文は<productname>PostgreSQL</productname>の拡張です。
標準<acronym>SQL</acronym>ではまた他の<command>CREATE TYPE</command>構文を定義していますが、<productname>PostgreSQL</productname>では実装されていません。␞␞  </para>␞
␝  <para>␟   The ability to create a composite type with zero attributes is
   a <productname>PostgreSQL</productname>-specific deviation from the
   standard (analogous to the same case in <command>CREATE TABLE</command>).␟ゼロ個の要素を持つ複合型を作成する機能は標準から派生した<productname>PostgreSQL</productname>固有のもの（<command>CREATE TABLE</command>の場合と同様）です。␞␞  </para>␞
␝ <refsect1 id="sql-createtype-see-also">␟  <title>See Also</title>␟  <title>関連項目</title>␞␞␞
