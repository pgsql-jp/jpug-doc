...
␝  <refname>EXPLAIN</refname>␟  <refpurpose>show the execution plan of a statement</refpurpose>␟  <refpurpose>問い合わせ文の実行計画を表示する</refpurpose>␞␞ </refnamediv>␞
...
␝  <para>␟   This command displays the execution plan that the
   ...
   each input table.␟与えられた文に対して、<productname>PostgreSQL</productname>プランナが生成する実行計画を表示します。
実行計画は、問い合わせ文が参照するテーブル（複数の場合もある）をスキャンする方法（単純なシーケンシャルスキャン、インデックススキャンなど）、複数のテーブルを参照する場合に、各テーブルから取り出した行を結合するために使用する結合アルゴリズムを示すものです。␞␞  </para>␞
␝  <para>␟   The most critical part of the display is the estimated statement execution
   ...
   costs to estimate which plan is really the cheapest.␟表示内容の中でも、最も重要なのは、文の実行にかかるコストの見積もりです。
...
また、<literal>LIMIT</literal>句を使って問い合わせが返す行数を制限する場合、プランナは実際にはどの計画が一番低コストになるかを概算するため、全体を処理した場合のコストの間で適切な補間を行います。␞␞  </para>␞
␝  <para>␟   The <literal>ANALYZE</literal> option causes the statement to be actually
   ...
   are close to reality.␟<literal>ANALYZE</literal>オプションを付けると、計画を作るだけでなく、文が実際に実行されます。
この場合は、各計画ノードで費された総経過時間（ミリ秒単位）と実際に返された全行数など、実際の実行時の統計情報が追加表示されます。
プランナの推測と実際の値が近いかどうかを確認するために、このオプションは有用です。␞␞  </para>␞
␝   <para>␟    Keep in mind that the statement is actually executed when
    ...
    without letting the command affect your data, use this approach:␟<literal>ANALYZE</literal>を使用した場合は、文が実際に実行されることを忘れないでください。
<command>EXPLAIN</command>は<command>SELECT</command>が返す出力をまったく表示しませんが、文に伴う副作用は通常通り発生します。
<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>、<command>MERGE</command>、<command>CREATE TABLE AS</command>、<command>EXECUTE</command>文に対して、データに影響を与えないように<command>EXPLAIN ANALYZE</command>を実行したい場合は、以下の方法を使用してください。␞␞<programlisting>␞
␝ <refsect1>␟  <title>Parameters</title>␟  <title>パラメータ</title>␞␞␞
␝     <para>␟      Carry out the command and show actual run times and other statistics.
      This parameter defaults to <literal>FALSE</literal>.␟コマンドを実行し、実際の実行時間やその他の統計情報を表示します。
このパラメータのデフォルトは<literal>FALSE</literal>です。␞␞     </para>␞
␝     <para>␟      Display additional information regarding the plan.  Specifically, include
      ...
      defaults to <literal>FALSE</literal>.␟計画についての追加情報を出力します。
...
このパラメータのデフォルトは<literal>FALSE</literal>です。␞␞     </para>␞
␝     <para>␟      Include information on the estimated startup and total cost of each
      plan node, as well as the estimated number of rows and the estimated
      width of each row.
      This parameter defaults to <literal>TRUE</literal>.␟各計画ノードの推定起動コストと総コスト、さらに推定行数と各行の推定幅に関する情報を含めます。
このパラメータのデフォルトは<literal>TRUE</literal>です。␞␞     </para>␞
␝     <para>␟      Include information on configuration parameters.  Specifically, include
      options affecting query planning with value different from the built-in
      default value.  This parameter defaults to <literal>FALSE</literal>.␟設定パラメータに関する情報を含めます。
特に、組み込みのデフォルト値と異なる値で問い合わせ計画に影響するオプションを含めます。
このパラメータのデフォルトは<literal>FALSE</literal>です。␞␞     </para>␞
␝     <para>␟      Allow the statement to contain parameter placeholders like
      ...
      It defaults to <literal>FALSE</literal>.␟文が<literal>$1</literal>のようなパラメータプレースホルダを含むことを許可し、これらのパラメータの値に依存しない汎用的な計画を生成します。
...
␝     <para>␟      Include information on buffer usage. Specifically, include the number of
      ...
      <literal>FALSE</literal>.␟バッファの使用状況に関する情報を含めます。
...
このパラメータのデフォルトは<literal>FALSE</literal>です。␞␞     </para>␞
␝     <para>␟      Include information on the cost
      ...
      argument, <literal>TEXT</literal> is assumed.␟問い合わせの出力データを<firstterm>シリアライズ</firstterm>、すなわち、クライアントに送信するためにテキスト形式またはバイナリ形式に変換する際のコストに関する情報を含めます。
...
引数を指定せずに<literal>SERIALIZE</literal>を書くと、<literal>TEXT</literal>が仮定されます。␞␞     </para>␞
␝     <para>␟      Include information on WAL record generation. Specifically, include the
      ...
      enabled.  It defaults to <literal>FALSE</literal>.␟WALレコード生成に関する情報を含めます。
具体的には、レコード数、ページ全体のイメージ(fpi)の数、生成されたWALのバイト単位での量が含められます。
テキスト形式では、非ゼロの値のみが出力されます。
このパラメータは<literal>ANALYZE</literal>パラメータも有効である場合にのみ使用できます。
デフォルトは<literal>FALSE</literal>です。␞␞     </para>␞
␝     <para>␟      Include actual startup time and time spent in each node in the output.
      ...
      option.
      This parameter may only be used when <literal>ANALYZE</literal> is also
      enabled.  It defaults to <literal>TRUE</literal>.␟実際のスタートアップ時間とノードで費やされた時間が追加表示されます。
...
␝     <para>␟      Include summary information (e.g., totaled timing information) after the
      ...
      necessary.␟要約情報（例えば、時間の情報の合計）を問い合わせ計画の後に出力します。
要約情報は<literal>ANALYZE</literal>が使われるときはデフォルトで含まれ、それ以外の場合はデフォルトでは含まれませんが、このオプションを使えば有効にできます。
<command>EXPLAIN EXECUTE</command>の計画時間には、計画をキャッシュから取得するのに要する時間、および必要なら再計画するのに要する時間も含まれます。␞␞     </para>␞
␝     <para>␟      Include information on memory consumption by the query planning phase.
      ...
      overhead.
      This parameter defaults to <literal>FALSE</literal>.␟問い合わせ計画フェーズによるメモリ消費量に関する情報を含めます。
具体的には、プランナのインメモリ構造体が使用する正確なメモリ量と、割り当てのオーバーヘッドを考慮した合計メモリを含めます。
このパラメータのデフォルトは<literal>FALSE</literal>です。␞␞     </para>␞
␝     <para>␟      Specify the output format, which can be TEXT, XML, JSON, or YAML.
      Non-text output contains the same information as the text output
      format, but is easier for programs to parse.  This parameter defaults to
      <literal>TEXT</literal>.␟出力形式を指定します。
TEXT、XML、JSON、YAMLを指定可能です。
TEXT以外の出力にはTEXT出力と同じ情報が含まれていますが、プログラムによる解析がより容易になります。
このパラメータのデフォルトは<literal>TEXT</literal>です。␞␞     </para>␞
...
␝     <para>␟      Any <command>SELECT</command>, <command>INSERT</command>, <command>UPDATE</command>,
      ...
      plan you wish to see.␟実行計画の表示対象となる、<command>SELECT</command>、<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>、<command>MERGE</command>、<command>VALUES</command>、<command>EXECUTE</command>、<command>DECLARE</command>、<command>CREATE TABLE AS</command>、<command>CREATE MATERIALIZED VIEW AS</command>のいずれかの文です。␞␞     </para>␞
␝ <refsect1>␟  <title>Outputs</title>␟  <title>出力</title>␞␞␞
␝   <para>␟    The command's result is a textual description of the plan selected
    for the <replaceable class="parameter">statement</replaceable>,
    optionally annotated with execution statistics.
    <xref linkend="using-explain"/> describes the information provided.␟コマンドの結果は、<replaceable class="parameter">statement</replaceable>に対して選択された計画をテキストで説明します。
オプションで、実行時の統計情報で注釈が付けられます。
<xref linkend="using-explain"/>では出力される情報について説明します。␞␞   </para>␞
␝ <refsect1>␟  <title>Notes</title>␟  <title>注釈</title>␞␞␞
␝  <para>␟   In order to allow the <productname>PostgreSQL</productname> query
   ...
   with the changes.␟<productname>PostgreSQL</productname>問い合わせプランナが十分な情報を使って問い合わせを最適化できるようにするには、問い合わせ内で使用されるすべてのテーブルに関する<link linkend="catalog-pg-statistic"><structname>pg_statistic</structname></link>のデータを最新状態にしなければなりません。
通常<link linkend="autovacuum">自動バキュームデーモン</link>により自動的に処理されます。
しかし最近その内容が大きく変更されたテーブルでは、自動バキュームがその変更に追いつくまで待つのではなく、手作業による<link linkend="sql-analyze"><command>ANALYZE</command></link>を実行する必要があるかもしれません。␞␞  </para>␞
␝  <para>␟   In order to measure the run-time cost of each node in the execution
   ...
   system calls for obtaining the time of day.␟実行計画内の各ノードの実行時コストを測定するために、現在の<command>EXPLAIN ANALYZE</command>実装は、問い合わせ実行に対し、情報収集のためのオーバーヘッドを加えます。
...
実行の間非常に短い時間を必要とする計画ノードに関して、時刻を得るためのシステムコールの操作が相対的に低速なプラットフォーム上で最悪な場合が発生します。␞␞  </para>␞
␝ <refsect1>␟  <title>Examples</title>␟  <title>例</title>␞␞␞
␝  <para>␟   To show the plan for a simple query on a table with a single
   <type>integer</type> column and 10000 rows:␟<type>integer</type>列を1つ持ち、10000行が存在するテーブルに対して、単純な問い合わせを行った場合の問い合わせ計画を表示します。␞␞␞
␝  <para>␟  Here is the same query, with JSON output formatting:␟以下は同じ問い合わせをJSON出力形式で出力したものです。␞␞<programlisting>␞
␝  <para>␟   If there is an index and we use a query with an indexable
   <literal>WHERE</literal> condition, <command>EXPLAIN</command>
   might show a different plan:␟インデックスが存在し、問い合わせの<literal>WHERE</literal>条件でインデックスを利用できる場合、<command>EXPLAIN</command>は異なる計画を表示する可能性があります。␞␞␞
␝  <para>␟  Here is the same query, but in YAML format:␟以下は同じ問い合わせをYAML形式で表したものです。␞␞<programlisting>␞
␝␟    XML format is left as an exercise for the reader.␟読者への演習としてXML形式については記載しません。␞␞  </para>␞
␝  <para>␟   Here is the same plan with cost estimates suppressed:␟以下は同じ計画ですが、コスト推定値を出力しません。␞␞␞
␝  <para>␟   Here is an example of a query plan for a query using an aggregate
   function:␟次に、集約関数を使用した問い合わせに対する問い合わせ計画の例を示します。␞␞␞
␝  <para>␟   Here is an example of using <command>EXPLAIN EXECUTE</command> to
   display the execution plan for a prepared query:␟以下は、<command>EXPLAIN EXECUTE</command>によってプリペアド文に対する実行計画を表示する例です。␞␞␞
␝  <para>␟   Of course, the specific numbers shown here depend on the actual
   ...
   in the table has not changed.␟もちろん、ここで示した具体的な数値は対象とするテーブルの実際の中身によって変わります。
また、この数値や選択された問い合わせ戦略は、プランナの改良のため、<productname>PostgreSQL</productname>のリリース間で異なる可能性がありますので注意してください。
さらに、<command>ANALYZE</command>コマンドは、データの統計情報を推定する際にランダムなサンプリングを行うため、実際のテーブル内の分布が変わっていなくても、新たに<command>ANALYZE</command>を実行すると推定コストが変わることがあります。␞␞  </para>␞
␝  <para>␟   Notice that the previous example showed a <quote>custom</quote> plan
   ...
   query, which can be done with <literal>GENERIC_PLAN</literal>:␟前の例では、<command>EXECUTE</command>で与えられた特定のパラメータに対する<quote>独自の</quote>計画が表示されていたことに注意してください。
...
␝␟   In this case the parser correctly inferred that <literal>$1</literal>
   ...
   symbols, which can be done by casting them, for example:␟この場合では、パーサは<literal>$1</literal>と<literal>$2</literal>が<literal>id</literal>と同じデータ型を持つべきだと正しく推測していたので、<command>PREPARE</command>からのパラメータ型情報の欠如は問題ではありませんでした。
...
␝  <para>␟   There is no <command>EXPLAIN</command> statement defined in the SQL standard.␟標準SQLでは<command>EXPLAIN</command>文は定義されていません。␞␞  </para>␞
...
␝</synopsis>␟   Note that in this syntax, the options must be specified in exactly the order
   shown.␟この構文では、オプションは、示した通りの順序で正確に指定しなければならないことに注意してください。␞␞  </para>␞
