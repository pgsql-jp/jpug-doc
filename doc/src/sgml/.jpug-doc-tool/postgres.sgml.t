...
␟␟   <!--
   ...
   -->␞␞<!ENTITY zwsp "&#x200B;">␞
␝␟<book id="postgres">␟<book id="postgres" lang="ja">␞␞␞
␝<book id="postgres">␟ <title>PostgreSQL &version; Documentation</title>␟ <title>PostgreSQL &version;文書</title>␞␞␞
␝ <bookinfo>␟  <corpauthor>The PostgreSQL Global Development Group</corpauthor>␟  <corpauthor>PostgreSQLグローバル開発グループ</corpauthor>␞␞  <productname>PostgreSQL</productname>␞
␝ <part id="tutorial">␟  <title>Tutorial</title>␟  <title>チュートリアル</title>␞␞␞
␝   <para>␟    Welcome to the <productname>PostgreSQL</productname> Tutorial.  The
    ...
    It makes no attempt to be a comprehensive treatment of the topics it covers.␟<productname>PostgreSQL</productname>のチュートリアルへようこそ。
...
扱っているトピックを網羅して記述しているものではありません。␞␞   </para>␞
␝   <para>␟    After you have successfully completed this tutorial you will want to
    ...
    manage their own PostgreSQL installation should also read <xref linkend="admin"/>.␟このチュートリアルを問題なく完了した後、SQL言語に関する理解を深めたい場合は<xref linkend="sql"/>を、<productname>PostgreSQL</productname>用のアプリケーションの開発に関する情報を学習したいのであれば、<xref linkend="client-interfaces"/>を読んでください。
また、PostgreSQLの導入を準備、提供、管理される方は、<xref linkend="admin"/>も参照してください。␞␞   </para>␞
␝ <part id="sql">␟  <title>The SQL Language</title>␟  <title>SQL言語</title>␞␞␞
␝   <para>␟    This part describes the use of the <acronym>SQL</acronym> language
    ...
    we address several aspects of importance for tuning a database.␟ここでは、<productname>PostgreSQL</productname>での<acronym>SQL</acronym>言語を使用する方法を説明します。
...
最後に、データベースをチューニングする際に重要となるいくつかの点について説明します。␞␞   </para>␞
␝   <para>␟    The information is arranged so that a novice user can
    ...
    the <xref linkend="reference"/>.␟ここでの内容は、初心者のユーザでも先のページを何度も参照することなく、最初から最後まで全てのトピックを理解できるような構成になっています。
これらの章は自己完結型であることを意図しており、上級ユーザは必要に応じて個別に章を読むことができます。
ここでの情報はトピック単位で記述されています。
特定のコマンドの完全な説明を探している読者は、<xref linkend="reference"/>を確認することをお勧めします。␞␞   </para>␞
␝   <para>␟    Readers should know how to connect to a
    ...
    similar functionality can be used as well.␟読者は<productname>PostgreSQL</productname>データベースへの接続および<acronym>SQL</acronym>コマンド発行の方法を知っている必要があります。
まだこれらについて慣れていない方は、先に<xref linkend="tutorial"/>を読むことをお勧めします。
<acronym>SQL</acronym>コマンドは通常、<productname>PostgreSQL</productname>の対話型端末<application>psql</application>を使用して入力しますが、同様の機能を備えた他のプログラムも使用することができます。␞␞   </para>␞
␝ <part id="admin">␟  <title>Server Administration</title>␟  <title>サーバの管理</title>␞␞␞
␝   <para>␟    This part covers topics that are of interest to a
    ...
    with these topics.␟ここでは、<productname>PostgreSQL</productname>管理者にとって関心のあるトピックを扱います。
これには、インストール、サーバの設定、ユーザとデータベースの管理、および保守作業が含まれます。
<productname>PostgreSQL</productname>サーバを個人的に使用している場合もそうですが、特に業務で使用している場合は、これらのトピックに精通している必要があります。␞␞   </para>␞
␝   <para>␟    The information attempts to be in the order in which
    ...
    <xref linkend="reference"/>.␟ここでの情報は、新規ユーザが読み進めるべき順番に並べられています。
各章は独立しており、必要に応じて個々の章を読むこともできます。
ここでの情報は項目単位で記述されています。
コマンドについて完全な説明を知りたい場合は、<xref linkend="reference"/>を確認することをお勧めします。␞␞   </para>␞
␝   <para>␟    The first few chapters are written so they can be understood
    ...
    linkend="sql"/> parts for additional information.␟最初の数章は、これからサーバを構築する新規ユーザにも読めるように、前提知識がなくても理解できるようになっています。
残りの部分では調整や管理について記されていますが、その資料は読者が<productname>PostgreSQL</productname>データベースシステムの一般的な使用について理解していることを前提としています。
詳細な情報については<xref linkend="tutorial"/>と<xref linkend="sql"/>を確認することをお勧めします。␞␞   </para>␞
␝ <part id="client-interfaces">␟  <title>Client Interfaces</title>␟  <title>クライアントインタフェース</title>␞␞␞
␝   <para>␟    This part describes the client programming interfaces distributed
    ...
    with the programming language of their choice.␟ここでは<productname>PostgreSQL</productname>に附属するクライアントプログラミングインタフェースについて説明します。
各章は独立して読むことができます。
...
読者は、データベースの操作や問い合わせを行うための<acronym>SQL</acronym>（<xref linkend="sql"/>を参照）、また、当然ながら、ご自身の選んだプログラミング言語に慣れ親しんでいることが必要です。␞␞   </para>␞
␝ <part id="server-programming">␟  <title>Server Programming</title>␟  <title>サーバプログラミング</title>␞␞␞
␝   <para>␟    This part is about extending the server functionality with
    ...
    material about server-side programming.␟ここでは、ユーザ定義関数、データ型、トリガなどを使用してサーバの機能を拡張する方法について説明します。
...
サーバサイドのプログラミング言語の章に進む前に、少なくとも、<xref linkend="extend"/>（関数も説明しています）の最初の数節を読破することは重要です。␞␞   </para>␞
␝ <part id="internals">␟  <title>Internals</title>␟  <title>内部情報</title>␞␞␞
␝   <para>␟    This part contains assorted information that might be of use to
    <productname>PostgreSQL</productname> developers.␟ここでは、<productname>PostgreSQL</productname>開発者が使用できる情報を分類して説明します。␞␞   </para>␞
␝  &limits;
  &acronyms;
  &glossary;
  &color;
  &obsolete;
␟␟  &jpug-doc;␞␞␞
␝ <part id="appendixes">␟  <title>Appendixes</title>␟  <title>付録</title>␞␞␞
