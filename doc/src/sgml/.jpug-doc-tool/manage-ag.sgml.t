...
␝<chapter id="managing-databases">␟ <title>Managing Databases</title>␟ <title>データベース管理</title>␞␞␞
␝ <para>␟  Every instance of a running <productname>PostgreSQL</productname>
  ...
  them.␟稼働している<productname>PostgreSQL</productname>サーバのすべてのインスタンスは、1つ以上のデータベースを管理します。
したがって、データベースは<acronym>SQL</acronym>オブジェクト（<quote>データベースオブジェクト</quote>）を組織化する場合に最上位の階層レベルとなります。
本章では、データベースの特性、作成方法、管理方法、および削除方法について説明します。␞␞ </para>␞
...
␝ <sect1 id="manage-ag-overview">␟  <title>Overview</title>␟  <title>概要</title>␞␞␞
␝  <para>␟   A small number of objects, like role, database, and tablespace
   ...
   such as a function).␟ロール、データベース、テーブル空間名のような少数のオブジェクトはクラスタレベルで定義されており、<literal>pg_global</literal>テーブル空間に格納されています。
クラスタの中には複数のデータベースがあり、互いに分離されているもののクラスタレベルのオブジェクトにはアクセスできます。
各データベースの中には複数のスキーマがあり、スキーマはテーブルや関数などのオブジェクトを含みます。
したがって階層の全体像は、クラスタ、データベース、スキーマ、テーブル（や関数などの何らかのオブジェクト）となります。␞␞  </para>␞
␝  <para>␟   When connecting to the database server, a client must specify the
   ...
   By default, all users can connect to all databases using all connection methods.␟データベースサーバに接続する時、クライアントはその接続要求の中で接続するデータベース名を指定しなければなりません。
...
デフォルトでは、すべてのユーザはすべてのデータベースにすべての接続方法で接続できます。␞␞  </para>␞
␝  <para>␟   If one <productname>PostgreSQL</productname> server cluster is planned to contain
   ...
   More information about managing schemas is in <xref linkend="ddl-schemas"/>.␟1つの<productname>PostgreSQL</productname>サーバクラスタに、たいていの場合お互いのことを意識しない、関係のないプロジェクトやユーザを含めるつもりなら、これらを別々のデータベースに含め、それに従って認証制御とアクセス制御を調整することが推奨されます。
...
スキーマの管理についての詳細は<xref linkend="ddl-schemas"/>に記載されています。␞␞  </para>␞
␝  <para>␟   While multiple databases can be created within a single cluster, it is advised
   ...
   from the database administrator's point-of-view.␟1つのクラスタ内に複数のデータベースを作成できますが、その利点がその危険性と制限に勝るかどうか慎重に検討することを勧めます。
特に、共有のWAL（<xref linkend="wal"/>参照）を持つことの影響がバックアップとリカバリのオプションにあります。
クラスタ内の個々のデータベースは、ユーザの視点から考えれば分離していても、データベース管理者の観点からは密接に結びついています。␞␞  </para>␞
␝  <para>␟   Databases are created with the <command>CREATE DATABASE</command> command
   ...
   <structname>pg_database</structname> system catalog, for example␟データベースは<command>CREATE DATABASE</command>コマンド（<xref linkend="manage-ag-createdb"/>を参照）で作成され、<command>DROP DATABASE</command>コマンド（<xref linkend="manage-ag-dropdb"/>を参照）で破棄されます。
既存のデータベース群を求めるには、以下の例のように<structname>pg_database</structname>システムカタログを確認してください。␞␞<synopsis>␞
␝</synopsis>␟   The <xref linkend="app-psql"/> program's <literal>\l</literal> meta-command
   and <option>-l</option> command-line option are also useful for listing the
   existing databases.␟また、<xref linkend="app-psql"/>プログラムの<literal>\l</literal>メタコマンドや<option>-l</option>コマンドラインオプションも既存のデータベースを列挙する際に役に立ちます。␞␞  </para>␞
␝   <para>␟    The <acronym>SQL</acronym> standard calls databases <quote>catalogs</quote>, but there
    is no difference in practice.␟標準<acronym>SQL</acronym>では、データベースを<quote>カタログ</quote>（catalog）と呼ぶこともありますが、実際のところ違いはありません。␞␞   </para>␞
␝ <sect1 id="manage-ag-createdb">␟  <title>Creating a Database</title>␟  <title>データベースの作成</title>␞␞␞
␝  <para>␟   In order to create a database, the <productname>PostgreSQL</productname>
   server must be up and running (see <xref
   linkend="server-start"/>).␟データベースを作成する場合、<productname>PostgreSQL</productname>サーバを起動している必要があります（<xref linkend="server-start"/>を参照してください）。␞␞  </para>␞
␝  <para>␟   Databases are created with the SQL command
   <xref linkend="sql-createdatabase"/>:␟<xref linkend="sql-createdatabase"/>SQLコマンドでデータベースを作成できます。␞␞<synopsis>␞
␝</synopsis>␟   where <replaceable>name</replaceable> follows the usual rules for
   ...
   the objects in it, even if they have a different owner).␟ここで、<replaceable>name</replaceable>は<acronym>SQL</acronym>識別子の通常の規則に従います。
現在のロールが自動的に新しいデータベースの所有者になります。
作成後、データベースを削除する権限はこの所有者にあります（この作業では、そのデータベースに属している、所有者のものではないオブジェクトでも、すべて削除されます）。␞␞  </para>␞
␝  <para>␟   The creation of databases is a restricted operation. See <xref
   linkend="role-attributes"/> for how to grant permission.␟データベースの作成は制限された作業です。
権限の付与に関する詳細は<xref linkend="role-attributes"/>を参照してください。␞␞  </para>␞
␝  <para>␟   Since you need to be connected to the database server in order to
   ...
   <literal>postgres</literal>.␟<command>CREATE DATABASE</command>コマンドを実行するためには、データベースサーバに接続している必要があります。
...
したがって、最初の<quote>通常の</quote>データベースを作成するには<literal>postgres</literal>に接続してください。␞␞  </para>␞
␝  <para>␟   Two additional databases,
   ...
   appear in <xref linkend="manage-ag-templatedbs"/>.␟<literal>template1</literal><indexterm><primary>template1</primary></indexterm>、<literal>template0</literal><indexterm><primary>template0</primary></indexterm>という2つの追加のデータベースもデータベースクラスタの初期化時に作成されます。
...
␝  <para>␟   As a convenience, there is a program you can
   execute from the shell to create new databases,
   <command>createdb</command>.<indexterm><primary>createdb</primary></indexterm>␟利便性のために、シェルから<command>createdb</command>を実行して、新しいデータベースを作成できます。
...
␝␟   <command>createdb</command> does no magic. It connects to the <literal>postgres</literal>
   ...
   a database with the current user name.␟<command>createdb</command>は魔法ではありません。
...
引数のない<command>createdb</command>は現在のユーザ名のデータベースを作成しますので、注意してください。␞␞  </para>␞
␝   <para>␟    <xref linkend="client-authentication"/> contains information about
    how to restrict who can connect to a given database.␟特定のデータベースに誰が接続できるかを制限する方法については<xref linkend="client-authentication"/>に記載されています。␞␞   </para>␞
␝  <para>␟   Sometimes you want to create a database for someone else, and have them
   become the owner of the new database, so they can
   configure and manage it themselves.  To achieve that, use one of the
   following commands:␟他のユーザのためにデータベースを作成し、そのユーザ自身が設定、管理できるように新しいデータベースの所有者にさせたい場合も考えられます。
そのためには、次のコマンドのいずれかを使用します。␞␞<programlisting>␞
␝</programlisting>␟   from the SQL environment, or:␟をSQL環境から実行するか、または␞␞<programlisting>␞
␝</programlisting>␟   from the shell.
   Only the superuser is allowed to create a database for
   someone else (that is, for a role you are not a member of).␟をシェルから実行します。
他のユーザのために（つまり、自身がメンバではないロールのために）データベースを作成できるのはスーパーユーザだけです。␞␞  </para>␞
␝ <sect1 id="manage-ag-templatedbs">␟  <title>Template Databases</title>␟  <title>テンプレートデータベース</title>␞␞␞
␝  <para>␟   <command>CREATE DATABASE</command> actually works by copying an existing
   ...
   action being taken when those databases are created.␟実際の<command>CREATE DATABASE</command>の動作は、既存のデータベースをコピーすることです。
...
例えば、<application>PL/Perl</application>手続き言語を<literal>template1</literal>にインストールした場合、データベースを作成する時に追加作業を行うことなく、自動的にこの言語をユーザデータベースで使用できます。␞␞  </para>␞
␝  <para>␟   However, <command>CREATE DATABASE</command> does not copy database-level
   <command>GRANT</command> permissions attached to the source database.
   The new database has default database-level permissions.␟しかし、<command>CREATE DATABASE</command>は、元のデータベースに付加されたデータベースレベルの<command>GRANT</command>権限をコピーしません。
新しいデータベースにはデフォルトのデータベースレベルの権限があります。␞␞  </para>␞
␝  <para>␟   There is a second standard system database named
   ...
   might have been added to <literal>template1</literal> later on.␟<literal>template0</literal>という名前の二次的な標準システムデータベースがあります。
...
このダンプスクリプトは、後で<literal>template1</literal>に追加される可能性のあるオブジェクトと衝突しないように、ダンプしたデータベースの内容を正しく再作成するために、汚れのないデータベースにリストアされなければなりません。␞␞  </para>␞
␝  <para>␟   Another common reason for copying <literal>template0</literal> instead
   ...
   or locale-specific data, while <literal>template0</literal> is known not to.␟<literal>template1</literal>の代わりに<literal>template0</literal>をコピーするその他の一般的な理由は、<literal>template0</literal>をコピーするときに新規の符号化方式とロケールを設定できることです。
一方、<literal>template1</literal>のコピーはそれが行ったと同一の設定を使用しなければなりません。
これは<literal>template0</literal>が認識されていない一方で、<literal>template1</literal>が符号化方式特有の、またはロケール特有のデータを含んでいる可能性があることに依ります。␞␞  </para>␞
␝  <para>␟   To create a database by copying <literal>template0</literal>, use:␟<literal>template0</literal>をコピーしてデータベースを作成するには、␞␞<programlisting>␞
␝</programlisting>␟   from the SQL environment, or:␟をSQL環境から実行するか、または␞␞<programlisting>␞
␝</programlisting>␟   from the shell.␟をシェルから実行します。␞␞  </para>␞
␝  <para>␟   It is possible to create additional template databases, and indeed
   ...
   are prevented.␟さらにテンプレートデータベースを作成することができます。
...
コピー操作中は元のデータベースへの新しい接続を許しません。␞␞  </para>␞
␝  <para>␟   Two useful flags exist in <literal>pg_database</literal><indexterm><primary>pg_database</primary></indexterm> for each
   ...
   should always be marked with <literal>datistemplate = true</literal>.␟<literal>datistemplate</literal>列と<literal>datallowconn</literal>列という、データベースそれぞれに有用なフラグが<literal>pg_database</literal>に存在します。
...
<literal>template0</literal>と<literal>template1</literal>の両方は、常に<literal>datistemplate</literal> = <literal>true</literal>とされていなければなりません。␞␞  </para>␞
␝   <para>␟    <literal>template1</literal> and <literal>template0</literal> do not have any special
    ...
    it must have <literal>pg_database.datistemplate = false</literal>.)␟<literal>template1</literal>と<literal>template0</literal>には、<literal>template1</literal>という名前が<command>CREATE DATABASE</command>のデフォルトのソースデータベースの名前であること以上の特別な地位はありません。
...
（<literal>template1</literal>を削除するには、<literal>pg_database.datistemplate = false</literal>としなければなりません。）␞␞   </para>␞
␝   <para>␟    The <literal>postgres</literal> database is also created when a database
    cluster is initialized.  This database is meant as a default database for
    users and applications to connect to. It is simply a copy of
    <literal>template1</literal> and can be dropped and recreated if necessary.␟データベースクラスタが初期化される時、<literal>postgres</literal>データベースも作成されます。
このデータベースは、ユーザとアプリケーションのデフォルトの接続先を意図したものです。
これは<literal>template1</literal>の単純なコピーで、必要に応じて削除したり再作成したりできます。␞␞   </para>␞
␝ <sect1 id="manage-ag-config">␟  <title>Database Configuration</title>␟  <title>データベースの設定</title>␞␞␞
␝  <para>␟   Recall from <xref linkend="runtime-config"/> that the
   ...
   default values for many of these settings.␟<xref linkend="runtime-config"/>で説明したように、<productname>PostgreSQL</productname>サーバには多数の実行時の設定変数が存在します。
これらの設定の多くに対して、データベース固有のデフォルト値を設定できます。␞␞  </para>␞
␝  <para>␟   For example, if for some reason you want to disable the
   ...
   database, you can execute the command:␟例えば、何らかの理由で特定のデータベースについて<acronym>GEQO</acronym>オプティマイザを無効にしたい場合、通常はすべてのデータベースでこれを無効にするか、またはすべての接続しているクライアントで間違いなく<literal>SET geqo TO off;</literal>が実行されていることを確認する必要があります。
特定のデータベースでこの設定をデフォルトにするには、次のコマンドを実行します。␞␞<programlisting>␞
␝</programlisting>␟   This will save the setting (but not set it immediately).  In
   ...
   <replaceable>varname</replaceable></literal>.␟これにより設定が保存されます（ただし、すぐに反映はされません）。
...
このような設定を解除するには、<literal>ALTER DATABASE <replaceable>dbname</replaceable> RESET <replaceable>varname</replaceable></literal>を使用します。␞␞  </para>␞
␝ <sect1 id="manage-ag-dropdb">␟  <title>Destroying a Database</title>␟  <title>データベースの削除</title>␞␞␞
␝  <para>␟   Databases are destroyed with the command
   <xref linkend="sql-dropdatabase"/>:<indexterm><primary>DROP DATABASE</primary></indexterm>␟データベースの削除には、以下の<xref linkend="sql-dropdatabase"/>コマンドを使用します。
<indexterm><primary>DROP DATABASE</primary></indexterm>␞␞<synopsis>␞
␝</synopsis>␟   Only the owner of the database, or
   a superuser, can drop a database. Dropping a database removes all objects
   that were
   contained within the database. The destruction of a database cannot
   be undone.␟データベースの所有者とスーパーユーザのみがデータベースを削除できます。
データベースの削除はそのデータベースに含まれるすべてのオブジェクトを削除します。
削除されたデータベースは復元できません。␞␞  </para>␞
␝  <para>␟   You cannot execute the <command>DROP DATABASE</command> command
   ...
   given cluster.␟削除しようとしているデータベースに接続している時には<command>DROP DATABASE</command>を実行することはできません。
しかし、<literal>template1</literal>などのその他のデータベースに接続すれば削除できます。
また、そのクラスタの最後のユーザデータベースを削除する時には、<literal>template1</literal>データベースに接続するしかありません。␞␞  </para>␞
␝  <para>␟   For convenience, there is also a shell program to drop
   databases, <xref linkend="app-dropdb"/>:<indexterm><primary>dropdb</primary></indexterm>␟利便性のため、データベースを削除する<xref linkend="app-dropdb"/>シェルプログラムもあります。
<indexterm><primary>dropdb</primary></indexterm>␞␞<synopsis>␞
␝</synopsis>␟   (Unlike <command>createdb</command>, it is not the default action to drop
   the database with the current user name.)␟（<command>createdb</command>と異なり、デフォルトで現在のユーザ名のデータベースを削除するようにはなっていません。）␞␞  </para>␞
...
␝ <sect1 id="manage-ag-tablespaces">␟  <title>Tablespaces</title>␟  <title>テーブル空間</title>␞␞␞
␝  <para>␟   Tablespaces in <productname>PostgreSQL</productname> allow database administrators to
   define locations in the file system where the files representing
   database objects can be stored. Once created, a tablespace can be referred
   to by name when creating database objects.␟<productname>PostgreSQL</productname>のテーブル空間により、データベース管理者はデータベースオブジェクトを表すファイルを格納できるファイルシステム上の場所を定義できます。
テーブル空間を一度作成すると、データベースオブジェクトを作成する時に名前により参照できます。␞␞  </para>␞
␝  <para>␟   By using tablespaces, an administrator can control the disk layout
   of a <productname>PostgreSQL</productname> installation. This is useful in at
   least two ways. First, if the partition or volume on which the
   cluster was initialized runs out of space and cannot be extended,
   a tablespace can be created on a different partition and used
   until the system can be reconfigured.␟テーブル空間を使用することで、管理者は<productname>PostgreSQL</productname>インストレーションのディスクレイアウトを制御できます。
これは、少なくとも2つの点で有用です。
1つは、クラスタを初期化したパーティションもしくはボリュームの容量が不足し、拡張を行うことができない場合、システムを再構成するまで、別のパーティションにテーブル空間を作成して、このテーブル空間を使用できます。␞␞  </para>␞
␝  <para>␟   Second, tablespaces allow an administrator to use knowledge of the
   ...
   expensive, slower disk system.␟もう1つは、テーブル空間により、管理者はデータベースオブジェクトの使用パターンに基づいてデータ格納場所を調整できることです。
例えば、非常によく使用されるインデックスを、例えば、高価なソリッドステートディスクなどの高速・高可用性ディスクに格納できます。
同時に、めったに使用されない保存用のデータや性能が求められていない保存用のデータを格納するテーブルを低価格・低速ディスクシステムに格納することもできます。␞␞  </para>␞
␝   <para>␟     Even though located outside the main PostgreSQL data directory,
     ...
     the entire cluster.␟主PostgreSQLデータディレクトリの外に位置していたとしても、テーブル空間はデータベースクラスタの不可欠な部分であり、データファイルの自律的な集合として扱うことは<emphasis>できません</emphasis>。
...
テーブル空間をRAMディスクのような一時ファイルシステムに置くことは、クラスタ全体の信頼性を危険にさらします。␞␞   </para>␞
␝  <para>␟   To define a tablespace, use the <xref
   linkend="sql-createtablespace"/>
   command, for example:<indexterm><primary>CREATE TABLESPACE</primary></indexterm>:␟テーブル空間を定義するには<xref linkend="sql-createtablespace"/>コマンドを使用してください。
以下に例を示します。
<indexterm><primary>CREATE TABLESPACE</primary></indexterm>␞␞<programlisting>␞
␝</programlisting>␟   The location must be an existing, empty directory that is owned by
   ...
   or lost.␟この場所は、既存かつ空のディレクトリでなければならず、また、<productname>PostgreSQL</productname>オペレーティングシステムユーザが所有していなければなりません。
その後に、テーブル空間内に作成されるオブジェクトはすべてこのディレクトリ以下のファイルに格納されます。
テーブル空間が見つからなかったり失われたりするとクラスタは機能しませんので、この場所は取り外し可能な記憶装置や一時的な記憶装置上にあってはいけません。␞␞  </para>␞
␝   <para>␟    There is usually not much point in making more than one
    ...
    system.  It just stores files in the directories you tell it to use.␟通常、論理ファイルシステム内の個々のファイルの場所を制御することができませんので、1つの論理ファイルシステムに複数のテーブル空間を作成することは意味がありません。
しかし、<productname>PostgreSQL</productname>にはこうした制限はありません。
実際、システムのファイルシステムの境を直接意識しません。
単に使用することを宣言したディレクトリにファイルを格納します。␞␞   </para>␞
␝  <para>␟   Creation of the tablespace itself must be done as a database superuser,
   but after that you can allow ordinary database users to use it.
   To do that, grant them the <literal>CREATE</literal> privilege on it.␟テーブル空間自体の生成はデータベーススーパーユーザが行う必要があります。
しかし、その後に、データベース一般ユーザにそのテーブル空間を使用させることができます。
これを行うには、ユーザにテーブル空間に対する<literal>CREATE</literal>権限を与えてください。␞␞  </para>␞
␝  <para>␟   Tables, indexes, and entire databases can be assigned to
   particular tablespaces. To do so, a user with the <literal>CREATE</literal>
   privilege on a given tablespace must pass the tablespace name as a
   parameter to the relevant command. For example, the following creates
   a table in the tablespace <literal>space1</literal>:␟テーブル、インデックス、およびデータベース全体は特定のテーブル空間に割り当て可能です。
これを行うには、指定テーブル空間に<literal>CREATE</literal>権限を持つユーザが関連するコマンドにテーブル空間をパラメータとして渡す必要があります。
例えば、以下は<literal>space1</literal>テーブル空間にあるテーブルを作成しています。␞␞<programlisting>␞
␝  <para>␟   Alternatively, use the <xref linkend="guc-default-tablespace"/> parameter:␟他の方法として、以下のように<xref linkend="guc-default-tablespace"/>パラメータの使用があります。␞␞<programlisting>␞
␝</programlisting>␟   When <varname>default_tablespace</varname> is set to anything but an empty
   ...
   do not have an explicit one.␟<varname>default_tablespace</varname>が空文字以外の何かに設定された場合、この値が、明示的なテーブル空間の指定がない<command>CREATE TABLE</command>コマンドや<command>CREATE INDEX</command>コマンドの暗黙的な <literal>TABLESPACE</literal>句として適用されます。␞␞  </para>␞
␝  <para>␟   There is also a <xref linkend="guc-temp-tablespaces"/> parameter, which
   ...
   a temporary object is to be created.␟<xref linkend="guc-temp-tablespaces"/>というパラメータも存在します。
...
一時的なオブジェクトを作成する度に、このリストから無作為に要素が選択されます。␞␞  </para>␞
␝  <para>␟   The tablespace associated with a database is used to store the system
   ...
   it uses the same tablespace as the template database it is copied from.␟データベースに関連付けされたテーブル空間は、そのデータベースのシステムカタログを格納するために使用されます。
...
テーブル空間の指定なしで作成されたデータベースは、コピー元のテンプレートデータベースのテーブル空間と同じものを使用します。␞␞  </para>␞
␝  <para>␟   Two tablespaces are automatically created when the database cluster
   is initialized.  The
   ...
   DATABASE</command>).␟データベースクラスタが初期化される時に2つのテーブル空間が自動的に作成されます。
...
（したがって、<command>CREATE DATABASE</command>の<literal>TABLESPACE</literal>句で変更されない限り、このテーブル空間が同様に他のデータベースに対するデフォルトのテーブル空間になります）。␞␞  </para>␞
␝  <para>␟   Once created, a tablespace can be used from any database, provided
   ...
   have been removed.␟テーブル空間は、一度作成すると、要求しているユーザが十分な権限を持っていればすべてのデータベースから使用できます。
これは、テーブル空間を使用するすべてのデータベースのすべてのオブジェクトが削除されるまで、そのテーブル空間を削除できないことを意味します。␞␞  </para>␞
␝  <para>␟   To remove an empty tablespace, use the <xref
   linkend="sql-droptablespace"/>
   command.␟空のテーブル空間を削除するには、<xref linkend="sql-droptablespace"/>コマンドを使用してください。␞␞  </para>␞
␝  <para>␟   To determine the set of existing tablespaces, examine the
   <link linkend="catalog-pg-tablespace"><structname>pg_tablespace</structname>
   </link> system catalog, for example␟既存のテーブル空間群を求めるには、以下の例のように<link linkend="catalog-pg-tablespace"><structname>pg_tablespace</structname></link>を確認してください。␞␞<synopsis>␞
␝</synopsis>␟   The <xref linkend="app-psql"/> program's <literal>\db</literal> meta-command
   is also useful for listing the existing tablespaces.␟また、<xref linkend="app-psql"/>プログラムの<literal>\db</literal>メタコマンドも既存のテーブル空間を列挙する際に役に立ちます。␞␞  </para>␞
␝  <para>␟   The directory <filename>$PGDATA/pg_tblspc</filename> contains symbolic links that
   ...
   continue to output the old tablespace locations.)␟<filename>$PGDATA/pg_tblspc</filename>ディレクトリには、そのクラスタで定義された非組み込みテーブル空間1つひとつを指し示すシンボリックリンクがあります。
...
（これを行わないと、<literal>pg_dump</literal>は古いテーブル空間の場所を出力し続けます。）␞␞  </para>␞
