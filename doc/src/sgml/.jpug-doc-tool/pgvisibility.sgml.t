␝<sect1 id="pgvisibility" xreflabel="pg_visibility">␟ <title>pg_visibility &mdash; visibility map information and utilities</title>␟ <title>pg_visibility &mdash; 可視性マップ情報とユーティリティ</title>␞␞␞
␝ <para>␟  The <filename>pg_visibility</filename> module provides a means for examining the
  ...
  force it to be rebuilt.␟<filename>pg_visibility</filename>モジュールは可視性マップ(Visibility Map, VM)およびテーブルのページレベルでの可視性情報を検査する手段を提供します。
このモジュールはまた、可視性マップの整合性を検査し、強制的に再構築する機能も提供します。␞␞ </para>␞
␝ <para>␟  Three different bits are used to store information about page-level
  ...
  can also cause these bits to disagree.␟ページレベルの可視性についての情報を格納するために、3つの異なるビットが使用されます。
...
データ破壊を起こすような何らかのイベントの後も、これらのビットが異なることがあり得ます。␞␞ </para>␞
␝ <para>␟  Functions that display information about <literal>PD_ALL_VISIBLE</literal> bits
  ...
  data blocks are similarly expensive.␟<literal>PD_ALL_VISIBLE</literal>ビットに関する情報を表示する関数は、可視性マップのみを参照する関数に比べるとずっと高価です。
これは、可視性マップだけではなく、（それよりずっと大きな）リレーションのデータブロックを読む必要があるからです。
リレーションのデータブロックを検査する関数は、同様に高価です。␞␞ </para>␞
␝ <sect2 id="pgvisibility-funcs">␟  <title>Functions</title>␟  <title>関数</title>␞␞␞
␝     <para>␟      Returns the all-visible and all-frozen bits in the visibility map for
      the given block of the given relation.␟指定のリレーションの指定のブロックについて、可視性マップ内の全可視ビットと全凍結ビットを返します。␞␞     </para>␞
␝     <para>␟      Returns the all-visible and all-frozen bits in the visibility map for
      the given block of the given relation, plus the
      <literal>PD_ALL_VISIBLE</literal> bit of that block.␟指定のリレーションの指定のブロックについて、可視性マップ内の全可視ビットと全凍結ビット、およびそのブロックの<literal>PD_ALL_VISIBLE</literal>を返します。␞␞     </para>␞
␝     <para>␟      Returns the all-visible and all-frozen bits in the visibility map for
      each block of the given relation.␟指定のリレーションの各ブロックについて、全可視ビットと全凍結ビットを返します。␞␞     </para>␞
␝     <para>␟      Returns the all-visible and all-frozen bits in the visibility map for
      each block of the given relation, plus the <literal>PD_ALL_VISIBLE</literal>
      bit of each block.␟指定のリレーションの各ブロックについて、全可視ビットと全凍結ビット、および各ブロックの<literal>PD_ALL_VISIBLE</literal>ビットを返します。␞␞     </para>␞
␝     <para>␟      Returns the number of all-visible pages and the number of all-frozen
      pages in the relation according to the visibility map.␟可視性マップに従って、リレーション内の全可視ページの数と全凍結ページの数を返します。␞␞     </para>␞
␝     <para>␟      Returns the TIDs of non-frozen tuples stored in pages marked all-frozen
      in the visibility map.  If this function returns a non-empty set of
      TIDs, the visibility map is corrupt.␟可視性マップ内で全凍結と印を付けられたページ内に格納されている非凍結タプルのTIDを返します。
この関数が返すTIDの集合が空でないなら、可視性マップは壊れています。␞␞     </para>␞
␝     <para>␟      Returns the TIDs of non-all-visible tuples stored in pages marked
      all-visible in the visibility map.  If this function returns a non-empty
      set of TIDs, the visibility map is corrupt.␟可視性マップ内で全可視と印を付けられたページ内に格納されている全可視でないタプルのTIDを返します。
この関数が返すTIDの集合が空でないなら、可視性マップは壊れています。␞␞     </para>␞
␝     <para>␟      Truncates the visibility map for the given relation.  This function is
      ...
      is done, queries will treat the visibility map as containing all zeroes.)␟指定のリレーションの可視性マップを切り詰めます。
...
（それが終わるまでは、可視性マップの中がすべてゼロになっているものとして問い合わせは動作します。）␞␞     </para>␞
␝  <para>␟   By default, these functions are executable only by superusers and roles with privileges
   ...
   be executed by superusers.␟デフォルトでは、これらの関数はスーパーユーザと<literal>pg_stat_scan_tables</literal>ロールの権限を持つロールのみが実行可能です。<function>pg_truncate_visibility_map(relation regclass)</function>は例外で、スーパーユーザのみが実行可能です。␞␞  </para>␞
␝ <sect2 id="pgvisibility-author">␟  <title>Author</title>␟  <title>作者</title>␞␞␞
