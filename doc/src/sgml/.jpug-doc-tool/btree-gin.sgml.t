␝<sect1 id="btree-gin" xreflabel="btree_gin">␟ <title>btree_gin &mdash; GIN operator classes with B-tree behavior</title>␟ <title>btree_gin &mdash; GIN演算子クラスとB-tree動作</title>␞␞␞
␝ <para>␟  <filename>btree_gin</filename> provides GIN operator classes that
...
␝ <para>␟  In general, these operator classes will not outperform the equivalent
  ...
  two separate indexes that would have to be combined via bitmap ANDing.␟一般的に、これらの演算子クラスは同等な標準B-treeインデックスメソッドを性能的に凌駕する物ではなく、標準B-treeコードの１つの重要機能である一意性を強制する能力を欠いています。
しかしながら、GINの試験、およびその他のGIN演算子クラスの開発の基礎として便利です。
同時に、GINインデックス化可能列およびB-treeインデックス化可能列双方を試験する問い合わせに対し、ビットマップを介してANDを取り一体化されるべき２つの別々のインデックスを作成するよりも、これらの演算子クラスの１つを使用する複数列GINインデックスを作成するほうがより効率的です。␞␞ </para>␞
...
␝ <sect2 id="btree-gin-example-usage">␟  <title>Example Usage</title>␟  <title>使用例</title>␞␞␞
␝-- create index␟-- create index␟-- インデックスの作成␞␞CREATE INDEX testidx ON test USING GIN (a);␞
␝-- query␟-- query␟-- 問い合わせ␞␞SELECT * FROM test WHERE a &lt; 10;␞
␝ <sect2 id="btree-gin-authors">␟  <title>Authors</title>␟  <title>作者</title>␞␞␞
␝  <para>␟   Teodor Sigaev (<email>teodor@stack.net</email>) and
   Oleg Bartunov (<email>oleg@sai.msu.su</email>).  See
   <ulink url="http://www.sai.msu.su/~megera/oddmuse/index.cgi/Gin"></ulink>
   for additional information.␟   Teodor Sigaev（<email>teodor@stack.net</email>）および
   Oleg Bartunov（<email>oleg@sai.msu.su</email>）。
追加情報は<ulink url="http://www.sai.msu.su/~megera/oddmuse/index.cgi/Gin"></ulink>を参照ください。␞␞  </para>␞
