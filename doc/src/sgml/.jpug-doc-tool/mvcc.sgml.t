...
␝ <chapter id="mvcc">␟  <title>Concurrency Control</title>␟  <title>同時実行制御</title>␞␞␞
␝  <para>␟   This chapter describes the behavior of the
   ...
   in this chapter.␟本章では同時に2つ以上のセッションが同じデータにアクセスしようとした場合、<productname>PostgreSQL</productname>データベースシステムがどう振舞うかについて説明します。
このような状況でデータの整合性を確実に保つ一方、全てのセッションに対して効果的なアクセスを許可するようにすることが目的です。
データベースアプリケーションを開発する方は、本章で扱われている内容を熟知していなければなりません。␞␞  </para>␞
␝   <indexterm>
...
␝  <sect1 id="mvcc-intro">␟   <title>Introduction</title>␟   <title>はじめに</title>␞␞␞
␝   <para>␟    <productname>PostgreSQL</productname> provides a rich set of tools
    ...
    performance in multiuser environments.␟<productname>PostgreSQL</productname>は、データへの同時アクセスを管理するために高度な開発者向けツール群を提供します。
...
<acronym>MVCC</acronym>は、マルチユーザ環境で理想的な性能を得るために、伝統的なデータベースシステムで行われるようなロック手法を避けることで、ロックの競合を最小化します。␞␞   </para>␞
␝   <para>␟    The main advantage of using the <acronym>MVCC</acronym> model of
    ...
    Snapshot Isolation</firstterm> (<acronym>SSI</acronym>) level.␟ロックではなく同時実行制御の<acronym>MVCC</acronym>モデルを使用する主な利点は、<acronym>MVCC</acronym>では問い合わせ（読み込み）ロックの獲得と、書き込みロックの獲得が競合しないことです。
したがって、読み込みは書き込みを絶対にブロックしませんし、書き込みも読み込みをブロックすることがありません。
革新的な<firstterm>シリアライザブルスナップショット分離</firstterm> (<acronym>SSI</acronym>)レベルの使用を通した最も厳密なトランザクションの分離レベルを提供する場合にも<productname>PostgreSQL</productname>はこれの保証を維持します。␞␞   </para>␞
␝   <para>␟    Table- and row-level locking facilities are also available in
    ...
    to a single transaction.␟全般的に完全なトランザクションの分離を必要とせず、明示的に競合する点を管理することを望むアプリケーションのために、<productname>PostgreSQL</productname>ではテーブルレベルおよび行レベルのロック機能も使用可能です。
とはいえ、<acronym>MVCC</acronym>を適切に使用すると通常ロックよりも性能が向上します。
さらに、アプリケーションが定義した勧告的ロックは単一トランザクションに拘束されないロックの獲得機構を提供します。␞␞   </para>␞
␝   <indexterm>
...
␝  <sect1 id="transaction-iso">␟   <title>Transaction Isolation</title>␟   <title>トランザクションの分離</title>␞␞␞
␝   <para>␟    The <acronym>SQL</acronym> standard defines four levels of
    ...
    phenomena caused by interactions?)␟標準<acronym>SQL</acronym>の規格では、トランザクションの分離について4つのレベルを定義しています。
標準規格で定義されているもののうち最も厳密なものはシリアライザブルです。
1セットのシリアライザブルなトランザクションを同時実行した場合には、ある順番でひとつずつそれらを実行した場合と同じ結果となることが保証されるものです。本文で詳しく述べます。
他の3レベルは、同時実行しているトランザクション間の相互作用に起因する、各レベルで発生してはならない現象面に基づき定義されます。
標準規格のシリアライザブルの定義では、このレベルではこれらの現象が起こりえないと述べています。
(これは驚くことではありません。トランザクションの効果がひとつずつ実行された場合と一貫性を持たなければならないとしたら、相互作用によって発生した現象はどうやっても見つけ出すことはできないでしょう。)␞␞   </para>␞
␝   <para>␟    The phenomena which are prohibited at various levels are:␟各種レベルにおける禁止される現象を以下に示します。␞␞␞
␝      <term>␟       dirty read
       <indexterm><primary>dirty read</primary></indexterm>␟ダーティリード
       <indexterm><primary>ダーティリード</primary></indexterm>␞␞      </term>␞
␝       <para>␟        A transaction reads data written by a concurrent uncommitted transaction.␟同時に実行されている他のトランザクションが書き込んで未だコミットしていないデータを読み込んでしまう。␞␞       </para>␞
␝      <term>␟       nonrepeatable read
       <indexterm><primary>nonrepeatable read</primary></indexterm>␟反復不能読み取り
       <indexterm><primary>反復不能読み取り</primary></indexterm>␞␞      </term>␞
␝       <para>␟        A transaction re-reads data it has previously read and finds that data
        has been modified by another transaction (that committed since the
        initial read).␟トランザクションが、以前読み込んだデータを再度読み込み、そのデータが（最初の読み込みの後にコミットした）別のトランザクションによって更新されたことを見出す。␞␞       </para>␞
␝      <term>␟       phantom read
       <indexterm><primary>phantom read</primary></indexterm>␟ファントムリード
       <indexterm><primary>ファントムリード</primary></indexterm>␞␞      </term>␞
␝       <para>␟        A transaction re-executes a query returning a set of rows that satisfy a
        search condition and finds that the set of rows satisfying the condition
        has changed due to another recently-committed transaction.␟トランザクションが、複数行のある集合を返す検索条件で問い合わせを再実行した時、別のトランザクションがコミットしてしまったために、同じ検索条件で問い合わせを実行しても異なる結果を得てしまう。␞␞       </para>␞
␝      <term>␟       serialization anomaly
       <indexterm><primary>serialization anomaly</primary></indexterm>␟直列化異常
       <indexterm><primary>直列化異常</primary></indexterm>␞␞      </term>␞
␝    <indexterm>
...
␝       <para>␟        The result of successfully committing a group of transactions
        ...
        transactions one at a time.␟複数のトランザクションを正常にコミットした結果が、それらのトランザクションを1つずつあらゆる可能な順序で実行する場合とは一貫性がない。␞␞       </para>␞
␝    </indexterm>␟    The SQL standard and PostgreSQL-implemented transaction isolation levels
    are described in <xref linkend="mvcc-isolevel-table"/>.␟標準SQLおよびPostgreSQLで実装されているトランザクション分離レベルを<xref linkend="mvcc-isolevel-table"/>に示します。␞␞   </para>␞
␝    <table tocentry="1" id="mvcc-isolevel-table">␟     <title>Transaction Isolation Levels</title>␟     <title>トランザクション分離レベル</title>␞␞     <tgroup cols="5">␞
␝        <entry>␟         Isolation Level␟分離レベル␞␞        </entry>␞
␝        <entry>␟         Dirty Read␟ダーティリード␞␞        </entry>␞
␝        <entry>␟         Nonrepeatable Read␟反復不能読み取り␞␞        </entry>␞
␝        <entry>␟         Phantom Read␟ファントムリード␞␞        </entry>␞
␝        <entry>␟         Serialization Anomaly␟直列化異常␞␞        </entry>␞
␝        <entry>␟         Read uncommitted␟リードアンコミッティド␞␞        </entry>␞
␝        <entry>␟         Allowed, but not in PG␟許容されるが、PostgreSQLでは発生しない␞␞        </entry>␞
␝        <entry>␟         Possible␟可能性あり␞␞        </entry>␞
␝        <entry>␟         Possible␟可能性あり␞␞        </entry>␞
␝        <entry>␟         Possible␟可能性あり␞␞        </entry>␞
␝        <entry>␟         Read committed␟リードコミッティド␞␞        </entry>␞
␝        <entry>␟         Not possible␟安全␞␞        </entry>␞
␝        <entry>␟         Possible␟可能性あり␞␞        </entry>␞
␝        <entry>␟         Possible␟可能性あり␞␞        </entry>␞
␝        <entry>␟         Possible␟可能性あり␞␞        </entry>␞
␝        <entry>␟         Repeatable read␟リピータブルリード␞␞        </entry>␞
␝        <entry>␟         Not possible␟安全␞␞        </entry>␞
␝        <entry>␟         Not possible␟安全␞␞        </entry>␞
␝        <entry>␟         Allowed, but not in PG␟許容されるが、PostgreSQLでは発生しない␞␞        </entry>␞
␝        <entry>␟         Possible␟可能性あり␞␞        </entry>␞
␝        <entry>␟         Serializable␟シリアライザブル␞␞        </entry>␞
␝        <entry>␟         Not possible␟安全␞␞        </entry>␞
␝        <entry>␟         Not possible␟安全␞␞        </entry>␞
␝        <entry>␟         Not possible␟安全␞␞        </entry>␞
␝        <entry>␟         Not possible␟安全␞␞        </entry>␞
␝   <para>␟    In <productname>PostgreSQL</productname>, you can request any of
    ...
    PostgreSQL's multiversion concurrency control architecture.␟<productname>PostgreSQL</productname>では、4つの標準トランザクション分離レベルを全て要求することができます。
しかし、内部的には3つの分離レベルしか実装されていません。
つまり、PostgreSQLのリードアンコミッティドモードは、リードコミッティドのように動作します。
これは、PostgreSQLの多版型同時実行制御という仕組みに標準の分離レベルを関連付ける実際的な方法がこれしかないからです。␞␞   </para>␞
␝   <para>␟    The table also shows that PostgreSQL's Repeatable Read implementation
    ...
    following subsections.␟このテーブルはまた、PostgreSQLのリピータブルリードの実装ではファントムリードが起こらないことを示しています。
標準SQLでは、ある分離レベルで発生しては<emphasis>ならない</emphasis>異常が指定されているので、これは許されています。
より高度な保障は許容されます。
利用可能な分離レベルでの動作については、以下の副節で詳しく説明します。␞␞   </para>␞
␝   <para>␟    To set the transaction isolation level of a transaction, use the
    command <xref linkend="sql-set-transaction"/>.␟トランザクションのトランザクション分離レベルを設定するには<xref linkend="sql-set-transaction"/>コマンドを使用してください。␞␞   </para>␞
␝     <para>␟       Some <productname>PostgreSQL</productname> data types and functions have
       ...
       and <xref linkend="datatype-serial"/>.␟いくつかの<productname>PostgreSQL</productname>データ型と関数はトランザクションの振る舞いに関して特別の規則があります。
特に、シーケンスに対しての変更は（従い、<type>serial</type>を使用して宣言された列のカウンタ）は直後に全ての他のトランザクションで可視となり、変更を行ったトランザクションが中止されるとロールバックはできません。
<xref linkend="functions-sequence"/>および<xref linkend="datatype-serial"/>を参照してください。␞␞     </para>␞
␝   <indexterm>
    <primary>transaction isolation level</primary>
...
␝  <sect2 id="xact-read-committed">␟   <title>Read Committed Isolation Level</title>␟   <title>リードコミッティド分離レベル</title>␞␞␞
␝   <para>␟    <firstterm>Read Committed</firstterm> is the default isolation
    ...
    before the second <command>SELECT</command> starts.␟<productname>PostgreSQL</productname>では<firstterm>リードコミッティド</firstterm>がデフォルトの分離レベルです。
トランザクションがこの分離レベルを使用すると、<command>SELECT</command>問い合わせ（<literal>FOR UPDATE/SHARE</literal>句を伴わない）はその問い合わせが実行される直前までにコミットされたデータのみを参照し、クエリの実行中にまだコミットされていないデータや、その問い合わせの実行中に別の同時実行トランザクションがコミットした更新は参照しません。
...
単一のトランザクション内であっても、<command>SELECT</command>文を2回連続して発行した場合、最初の<command>SELECT</command>文が開始した後で2番目の<command>SELECT</command>文が開始する前に他のトランザクションが更新をコミットすると、最初とその次に発行した<command>SELECT</command>問い合わせは異なるデータを参照してしまうことにも注意してください。␞␞   </para>␞
␝   <para>␟    <command>UPDATE</command>, <command>DELETE</command>, <command>SELECT
    ...
    locked and returned to the client.␟<command>UPDATE</command>、<command>DELETE</command>、<command>SELECT FOR UPDATE</command>、および<command>SELECT FOR SHARE</command>コマンドは対象行を検索する際に<command>SELECT</command>コマンドと同じように振舞います。
...
<command>SELECT FOR UPDATE</command>および<command>SELECT FOR SHARE</command>の場合、ロックされクライアントに返されるのは、更新されるバージョンの行であることを意味します。␞␞   </para>␞
␝   <para>␟    <command>INSERT</command> with an <literal>ON CONFLICT DO UPDATE</literal> clause
    ...
    conventionally visible to the command.␟<literal>ON CONFLICT DO UPDATE</literal>句のある<command>INSERT</command>は同じように動作します。
リードコミッティドモードでは、挿入を提案された各行が挿入または更新されます。
無関係なエラーが発生しなければ、それら2つの結果のうち1つが保証されます。
まだその結果が<command>INSERT</command>に対して可視になっていない他のトランザクションに起因する競合では、慣習的な意味でそのコマンドに対して可視のバージョンの行が存在<emphasis>しない</emphasis>にも関わらず、<command>UPDATE</command>句がその行に対して動作します。␞␞   </para>␞
␝   <para>␟    <command>INSERT</command> with an <literal>ON CONFLICT DO
    ...
    the case in Read Committed mode.␟<literal>ON CONFLICT DO NOTHING</literal>句のある<command>INSERT</command>では、<command>INSERT</command>のスナップショットに対してその結果が可視になっていない他のトランザクションの結果のために、行の挿入が処理されないかもしれません。
ここでも、問題になるのはリードコミッティドモードのときだけです。␞␞   </para>␞
␝   <para>␟    <command>MERGE</command> allows the user to specify various
    ...
    conditions.␟<command>MERGE</command>を使用すると、ユーザは<command>INSERT</command>、<command>UPDATE</command>および<command>DELETE</command>サブコマンドの様々な組み合わせを指定できます。
...
<command>MERGE</command>が<command>INSERT</command>を試行し、一意なインデックスが存在し、重複した行が同時に挿入された場合、一意性違反エラーが発生します。<command>MERGE</command>は<literal>MATCHED</literal>条件の評価を再開してこのようなエラーを回避しようとはしません。␞␞   </para>␞
␝   <para>␟    Because of the above rules, it is possible for an updating command to see
    ...
    like:␟このような仕組みにより、更新コマンドが、一貫しないスナップショットを参照する可能性があります。
...
␝␟    If two such transactions concurrently try to change the balance of account
    ...
    not create any troublesome inconsistency.␟2つのこのようなトランザクションが同時に口座番号12345の残高を変更しようとした場合、口座の行の更新されたバージョンに対して2番目のトランザクションが開始されることは明らかに望まれるところです。
各コマンドは事前に決定していた行に対してのみ処理を行うため、行の更新されたバージョンを見せることによって、何の問題となる不整合も引き起こしません。␞␞   </para>␞
␝   <para>␟    More complex usage can produce undesirable results in Read Committed
    ...
    <literal>10</literal>:␟より複雑な使用法により、リードコミッティドモードでは好ましくない結果を生成する場合があります。
...
␝-- run from another session:  DELETE FROM website WHERE hits = 10;␟-- run from another session:  DELETE FROM website WHERE hits = 10;␟-- 別のセッションから DELETE FROM website WHERE hits = 10; を実行します␞␞COMMIT;␞
␝␟    The <command>DELETE</command> will have no effect even though
    ...
    <literal>11</literal>, which no longer matches the criteria.␟<command>UPDATE</command>の前後の両方で<literal>website.hits = 10</literal>の行があるにも関わらず、<command>DELETE</command>は何もしません。
なぜこうなるのかと言うと、更新前の行値<literal>9</literal>は読み飛ばされ、また<command>UPDATE</command>が完了して<command>DELETE</command>がロックを獲得した時点では、新しい行値は<literal>10</literal>ではなく<literal>11</literal>となり、判定条件にもはやマッチしなくなっているからです。␞␞   </para>␞
␝   <para>␟    Because Read Committed mode starts each command with a new snapshot
    ...
    sees an absolutely consistent view of the database.␟リードコミッティドモードは、それぞれのコマンドをその時点までにコミットされた全てのトランザクションを含む新規スナップショットを伴って開始するので、同一のトランザクション内でそれに続くコマンドは、いかなる場合でもコミットされた同時実行トランザクションの結果を参照します。
上記問題の要点は<emphasis>単一の</emphasis>コマンドがデータベースの厳密に一貫性のある見え方を見るか否かです。␞␞   </para>␞
␝   <para>␟    The partial transaction isolation provided by Read Committed mode
    ...
    consistent view of the database than Read Committed mode provides.␟リードコミッティドモードで提供されている部分的なトランザクション分離は、多くのアプリケーションでは適切です。
またこのモードは高速で、使い方も簡単ですが、全ての場合に対して十分ではありません。
複雑な問い合わせや更新を行うアプリケーションは、リードコミッティドモードが提供する以上のより厳正なデータベースの厳密に一貫性のある見え方を必要とします。␞␞   </para>␞
␝   <indexterm>
    <primary>transaction isolation level</primary>
...
␝  <sect2 id="xact-repeatable-read">␟   <title>Repeatable Read Isolation Level</title>␟   <title>リピータブルリード分離レベル</title>␞␞␞
␝   <para>␟    The <firstterm>Repeatable Read</firstterm> isolation level only sees
    ...
    provide.␟<firstterm>リピータブルリード</firstterm>分離レベルは、トランザクションが開始される前までにコミットされたデータのみを参照します。
コミットされていないデータや、そのトランザクションの実行中に別のトランザクションでコミットされた変更を参照しません。
（しかし、その問い合わせと同じトランザクション内で行われた過去の更新は、まだコミットされていませんが、参照します。）
これは標準<acronym>SQL</acronym>の規格で求められるものよりもより強く保証するもので、直列化異常を除いて、<xref linkend="mvcc-isolevel-table"/>で述べている現象をすべて防ぎます。
上で述べたように、これは標準規格によって明示的に許容されているもので、標準ではそれぞれの分離レベルが提供しなくてはならない<emphasis>最小</emphasis>の保護のみが示されています。␞␞   </para>␞
␝   <para>␟    This level is different from Read Committed in that a query in a
    ...
    other transactions that committed after their own transaction started.␟リピータブルリードのトランザクション内の問い合わせは、トランザクション内の現在の文の開始時点ではなく、<emphasis>トランザクション</emphasis>の最初のトランザクション制御以外の文の開始時点のスナップショットを見る、という点でこのレベルはリードコミッティドと異なります。
従って、<emphasis>単一</emphasis>トランザクション内の連続する<command>SELECT</command>文は、同じデータを参照します。つまり、自身のトランザクションが開始した後にコミットされた他のトランザクションによる変更を参照しません。␞␞   </para>␞
␝   <para>␟    Applications using this level must be prepared to retry transactions
    due to serialization failures.␟このレベルを使ったアプリケーションでは、直列化の失敗によるトランザクションの再実行に備えておく必要があります。␞␞   </para>␞
␝   <para>␟    <command>UPDATE</command>, <command>DELETE</command>,
    ...
    then the repeatable read transaction will be rolled back with the message␟<command>UPDATE</command>、<command>DELETE</command>、<command>MERGE</command>、<command>SELECT FOR UPDATE</command>、および<command>SELECT FOR SHARE</command>コマンドでは、<command>SELECT</command>と同じように対象行を検索します。
...
␝␟    because a repeatable read transaction cannot modify or lock rows changed by
    other transactions after the repeatable read transaction began.␟これは、リピータブルリードトランザクションでは、トランザクションが開始された後に別のトランザクションによって更新されたデータを変更またはロックすることができないためです。␞␞   </para>␞
␝   <para>␟    When an application receives this error message, it should abort
    ...
    as the starting point for the new transaction's update.␟アプリケーションがこのエラーメッセージを受け取った場合、現在のトランザクションを中止して、トランザクション全体を始めからやり直されなければなりません。
2回目では、トランザクションはコミットされた変更を含めてデータベースの最初の状態とみなすので、新しいバージョンの行を新しいトランザクションにおける更新の始点としても、論理的矛盾は起こりません。␞␞   </para>␞
␝   <para>␟    Note that only updating transactions might need to be retried; read-only
    transactions will never have serialization conflicts.␟再実行する必要があるかもしれないのは、更新トランザクションのみです。
読み込み専用トランザクションでは直列化の衝突は決して起こりません。␞␞   </para>␞
␝   <para>␟    The Repeatable Read mode provides a rigorous guarantee that each
    ...
    of explicit locks to block conflicting transactions.␟リピータブルリードモードでは、全てのトランザクションがデータベースの一貫した不変のビューの状態を参照することが保証されます。
しかし、このビューは常にいくつかの同じレベルの同時実行トランザクションの直列（一度に一つずつの）実行と一貫性を持つとは限りません。
例えば、このレベルの読み取りのみのトランザクションは、バッチが完了したことを示すために更新された制御レコードを参照できますが、
制御レコードのより以前のバージョンを読み取るため、論理的にそのバッチの一部となる詳細なレコードの1つを参照することは<emphasis>できません</emphasis>。
この分離レベルで実行するトランザクションによりビジネスルールを強制しようとすることは、競合するトランザクションをブロックするために注意深く明示的なロックを持たないと、正確に動作しないことが多くあります。␞␞   </para>␞
␝   <para>␟    The Repeatable Read isolation level is implemented using a technique
    ...
    <xref linkend="berenson95"/>.␟リピータブルリード分離レベルは、学術的なデータベースの文献や他のデータベース製品のいくつかでは<firstterm>スナップショット分離</firstterm>として知られる技術を用いて実装されています。
...
詳細な取り扱いについては<xref linkend="berenson95"/>を参照してください。␞␞   </para>␞
␝    <para>␟     Prior to <productname>PostgreSQL</productname> version 9.1, a request
     ...
     behavior, Repeatable Read should now be requested.␟<productname>PostgreSQL</productname> version 9.1より前までは、シリアライザブル分離レベルの要求はここで説明した通りの動作をそのまま提供していました。
以前のシリアライザブルの動作を維持するためには、リピータブルリードを要求しなければならなくなりました。␞␞    </para>␞
␝   <indexterm>
    <primary>transaction isolation level</primary>
...
␝  <sect2 id="xact-serializable">␟   <title>Serializable Isolation Level</title>␟   <title>シリアライザブル分離レベル</title>␞␞␞
␝   <para>␟    The <firstterm>Serializable</firstterm> isolation level provides
    ...
    <firstterm>serialization failure</firstterm>.␟<firstterm>シリアライザブル</firstterm>分離レベルは、最も厳しいトランザクションの分離性を提供します。
...
実際、この分離レベルは、（ある時点で）逐次実行可能なすべてのトランザクションにおいて、シリアライザブルトランザクションの同時実行の組が一貫性のないような振る舞いをしていないかも監視することを除き、リピータブルリードと全く同じ動きをします。
この監視では、リピータブルリードが示すものを越えてブロックすることはありませんが、監視によりいくらかのオーバーヘッドがあり、<firstterm>直列化異常</firstterm>を引き起こすような状態の検知は、<firstterm>直列化の失敗</firstterm>を引き起こすでしょう。␞␞   </para>␞
␝   <para>␟    As an example,
    consider a table <structname>mytab</structname>, initially containing:␟例えば、以下の初期データを持つ<structname>mytab</structname>というテーブルを考えてみます。␞␞<screen>␞
␝</screen>␟    Suppose that serializable transaction A computes:␟ここでシリアライザブルトランザクションAが以下を計算し、␞␞<screen>␞
␝</screen>␟    and then inserts the result (30) as the <structfield>value</structfield> in a
    new row with <structfield>class</structfield><literal> = 2</literal>.  Concurrently, serializable
    transaction B computes:␟そして、<structfield>value</structfield>にその結果（30）を、<structfield>class</structfield><literal> = 2</literal>の行として新たに挿入したとします。
同時にシリアライザブルトランザクションBが以下を計算し、␞␞<screen>␞
␝</screen>␟    and obtains the result 300, which it inserts in a new row with
    ...
    transaction to commit and will roll the other back with this message:␟その結果300を得、そして、この結果を<structfield>class</structfield><literal> = 1</literal>の新たな行として挿入したとします。
...
␝␟    This is because if A had
    ...
    computed by A.␟この理由は、もしAがBよりも前に実行されていた場合、Bの総和は300ではなく330と計算され、また同様に逆の順序で実行されたとすればAで計算される総和が異なる結果になるからです。␞␞   </para>␞
␝   <para>␟    When relying on Serializable transactions to prevent anomalies, it is
    ...
    succeeds.␟異常を防止するためにシリアライザブルトランザクションを使用するのであれば、恒久的なユーザテーブルから読み取られたいかなるデータも、それを読んだトランザクションがコミットされるまで有効とは認められない点は重要です。
...
それ以外の全ての場合において、後に中止されたトランザクション内で読み込まれた結果をアプリケーションは信用してはならず、アプリケーションはトランザクションが成功するまで再試行すべきです。␞␞   </para>␞
␝   <para>␟    To guarantee true serializability <productname>PostgreSQL</productname>
    ...
    can block other transactions but cause disk access.␟真の直列性を保証するために<productname>PostgreSQL</productname>では、<firstterm>述語ロック</firstterm>を使います。
...
それとは対照的に、データの一貫性を保証したいリードコミッティドあるいはリピータブルリードトランザクションでは、テーブル全体のロック（そのテーブルを使用しようとしている他のユーザをブロックするかもしれません）を必要とするかもしれませんし、あるいは、他のトランザクションをブロックするだけでなくディスク・アクセスを引き起こす<literal>SELECT FOR UPDATE</literal>あるいは<literal>SELECT FOR SHARE</literal>を使用するかもしれません。␞␞   </para>␞
␝   <para>␟    Predicate locks in <productname>PostgreSQL</productname>, like in most
    ...
    write transactions complete.␟<productname>PostgreSQL</productname>の述語ロックは、他のほとんどのデータベースシステムと同様、トランザクションによって実際にアクセスされたデータを元にしています。
...
他方で、SIReadロックは、しばしば読み取りと書き込みが重なっているトランザクションが完了するまで、トランザクションのコミットが終わっても保持される必要があります。␞␞   </para>␞
␝   <para>␟    Consistent use of Serializable transactions can simplify development.
    ...
    for some environments.␟シリアライザブルトランザクションの一貫した使用は開発を単純化することができます。
...
しかしながら、このコストと、明示的なロックと<literal>SELECT FOR UPDATE</literal>または<literal>SELECT FOR SHARE</literal>を使用したブロッキングとで比較検討すると、シリアライザブルトランザクションはいくつかの環境において最良な実行を選択することになります。␞␞   </para>␞
␝   <para>␟    While <productname>PostgreSQL</productname>'s Serializable transaction isolation
    ...
    transactions.␟<productname>PostgreSQL</productname>のシリアライザブルトランザクション分離レベルが同じ結果を生む実行順序があることを証明できるときだけ、同時のトランザクションのコミットを許すとはいえ、本当のシリアル実行では起こらないエラーが常に防げるわけではありません。
...
もしいくつかのシリアライザブルトランザクションがこのプロトコルに沿わずに直接新しいキーを挿入すれば、たとえそれがシリアル実行の同時トランザクションでは起こりえないケースでも一意性制約違反が報告されることになります。␞␞   </para>␞
␝   <para>␟    For optimal performance when relying on Serializable transactions for
    concurrency control, these issues should be considered:␟同時実行制御のためにシリアライザブルトランザクションを使用する場合、最適な性能のためには、以下の問題を考慮すべきです。␞␞␞
␝      <para>␟       Declare transactions as <literal>READ ONLY</literal> when possible.␟可能であればトランザクションを<literal>READ ONLY</literal>として宣言してください。␞␞      </para>␞
␝      <para>␟       Control the number of active connections, using a connection pool if
       ...
       transactions.␟もし必要ならばコネクションプールを使用して、活動中の接続数を制御してください。
これは常に重要な性能上の考慮点ですが、シリアライザブルトランザクションを使用した多忙なシステムにおいては、特に重要になる可能性があります。␞␞      </para>␞
␝      <para>␟       Don't put more into a single transaction than needed for integrity
       purposes.␟完全性のために必要とされる以上のものを1つのトランザクションに入れないようにしてください。␞␞      </para>␞
␝      <para>␟       Don't leave connections dangling <quote>idle in transaction</quote>
       longer than necessary.  The configuration parameter
       <xref linkend="guc-idle-in-transaction-session-timeout"/> may be used to
       automatically disconnect lingering sessions.␟必要以上に長く<quote>トランザクション内で待機状態</quote>で接続したまま放置しておかないようにしてください。
長引くセッションを自動的に切断するために、設定パラメータ<xref linkend="guc-idle-in-transaction-session-timeout"/>を使うことができます。␞␞      </para>␞
␝      <para>␟       Eliminate explicit locks, <literal>SELECT FOR UPDATE</literal>, and
       <literal>SELECT FOR SHARE</literal> where no longer needed due to the
       protections automatically provided by Serializable transactions.␟シリアライザブルトランザクションにより自動的に提供される保護により、不必要な、明示的なロック、<literal>SELECT FOR UPDATE</literal>および<literal>SELECT FOR SHARE</literal>を取り除いてください。␞␞      </para>␞
␝      <para>␟       When the system is forced to combine multiple page-level predicate
       ...
       <xref linkend="guc-max-pred-locks-per-page"/>.␟述語ロックのテーブルがメモリ不足になると、複数のページレベルの述語ロックを単一のリレーションレベルの述語ロックへと結合するようシステムが強いられ、直列化失敗の発生割合が増加する恐れがあります。
これは、<xref linkend="guc-max-pred-locks-per-transaction"/>、<xref linkend="guc-max-pred-locks-per-relation"/>、<xref linkend="guc-max-pred-locks-per-page"/>のいずれか、あるいは、すべてを増やすことにより回避できます。␞␞      </para>␞
␝      <para>␟       A sequential scan will always necessitate a relation-level predicate
       ...
       query execution time.␟シーケンシャルスキャンは常にリレーションレベルでの述語ロックを必要とします。
...
トランザクションのロールバックや再実行の減少を、問い合わせ実行時間の全体的な変化と比較検討するようにしてください。␞␞      </para>␞
␝   <para>␟    The Serializable isolation level is implemented using a technique known
    ...
    <xref linkend="ports12"/> for detailed information.␟シリアライザブル分離レベルは、学術的なデータベースの文献ではシリアライザブルスナップショット分離として知られる技術を使って実装されています。シリアライザブルスナップショット分離は、スナップショット分離の上に直列化異常の確認を追加することで構築されています。
伝統的なロック技術を使う他のシステムと比較すると振舞いや性能の違いが観察されるかもしれません。
詳細な情報は<xref linkend="ports12"/>を参照してください。␞␞   </para>␞
␝   <indexterm>
...
␝  <sect1 id="explicit-locking">␟   <title>Explicit Locking</title>␟   <title>明示的ロック</title>␞␞␞
␝   <para>␟    <productname>PostgreSQL</productname> provides various lock modes
    ...
    enforce that.)␟<productname>PostgreSQL</productname>は、テーブル内のデータに対する同時アクセスを制御するために様々な種類のロックモードを備えています。
これらのモードは、<acronym>MVCC</acronym>では必要な動作を得られない場合、アプリケーション制御のロックに使用できます。
...
（例えば、<command>TRUNCATE</command>コマンドは、同じテーブルに対する他の操作と同時に安全に実行することはできないので、それを確実に実行するため、そのテーブルの<literal>ACCESS EXCLUSIVE</literal>ロックを獲得します。）␞␞   </para>␞
␝   <para>␟    To examine a list of the currently outstanding locks in a database
    server, use the
    <link linkend="view-pg-locks"><structname>pg_locks</structname></link>
    system view. For more information on monitoring the status of the lock
    manager subsystem, refer to <xref linkend="monitoring"/>.␟現在のデータベースサーバに残っているロックの一覧を確認するには、<link linkend="view-pg-locks"><structname>pg_locks</structname></link>システムビューを使用してください。
ロック管理サブシステムの状況監視についての詳細は<xref linkend="monitoring"/>を参照してください。␞␞   </para>␞
␝  <sect2 id="locking-tables">␟   <title>Table-Level Locks</title>␟   <title>テーブルレベルロック</title>␞␞␞
␝   <para>␟    The list below shows the available lock modes and the contexts in
    ...
    an <literal>ACCESS SHARE</literal> lock can be held by multiple transactions).␟以下のリストに、使用可能なロックモードとそれらが<productname>PostgreSQL</productname>で自動的に使用される文脈を示します。
...
競合しないロックモードは、多くのトランザクションで同時に保持できます。
特に、ロックモードには、自己競合するもの（例えば、<literal>ACCESS EXCLUSIVE</literal>は同時に複数のトランザクションで保持することは不可能）と、自己競合しないもの（例えば、<literal>ACCESS SHARE</literal>は複数のトランザクションで保持可能）があることに注意してください。␞␞   </para>␞
␝     <variablelist>␟      <title>Table-Level Lock Modes</title>␟      <title>テーブルレベルロックモード</title>␞␞      <varlistentry>␞
␝        <para>␟         Conflicts with the <literal>ACCESS EXCLUSIVE</literal> lock
         mode only.␟         <literal>ACCESS EXCLUSIVE</literal>ロックモードとのみ競合します。␞␞        </para>␞
␝        <para>␟         The <command>SELECT</command> command acquires a lock of this mode on
         referenced tables.  In general, any query that only <emphasis>reads</emphasis> a table
         and does not modify it will acquire this lock mode.␟<command>SELECT</command>コマンドにより、参照されるテーブルに対してこのモードのロックが獲得されます。
通常、テーブルの<emphasis>読み取り</emphasis>のみで変更を行わない問い合わせであれば全て、このロックモードを獲得します。␞␞        </para>␞
␝        <para>␟         Conflicts with the <literal>EXCLUSIVE</literal> and
         <literal>ACCESS EXCLUSIVE</literal> lock modes.␟<literal>EXCLUSIVE</literal>および<literal>ACCESS EXCLUSIVE</literal>ロックモードと競合します。␞␞        </para>␞
␝        <para>␟         The <command>SELECT</command> command acquires a lock of this mode
         ...
         <option>FOR ...</option> locking option).␟（明示的な<option>FOR ...</option>ロックオプションなしで参照される他のすべてのテーブルで取得される<literal>ACCESS SHARE</literal>ロックに加え）<command>SELECT</command>コマンドは、<option>FOR UPDATE</option>、<option>FOR NO KEY UPDATE</option>、<option>FOR SHARE</option>、または<option>FOR KEY SHARE</option>オプションのいずれかが指定されているすべてのテーブルでこのモードのロックを取得します。␞␞        </para>␞
␝        <para>␟         Conflicts with the <literal>SHARE</literal>, <literal>SHARE ROW
         EXCLUSIVE</literal>, <literal>EXCLUSIVE</literal>, and
         <literal>ACCESS EXCLUSIVE</literal> lock modes.␟<literal>SHARE</literal>、<literal>SHARE ROW EXCLUSIVE</literal>、<literal>EXCLUSIVE</literal>、および<literal>ACCESS EXCLUSIVE</literal>ロックモードと競合します。␞␞        </para>␞
␝        <para>␟         The commands <command>UPDATE</command>,
         <command>DELETE</command>, <command>INSERT</command>, and
         <command>MERGE</command>
         acquire this lock mode on the target table (in addition to
         <literal>ACCESS SHARE</literal> locks on any other referenced
         tables).  In general, this lock mode will be acquired by any
         command that <emphasis>modifies data</emphasis> in a table.␟<command>UPDATE</command>、<command>DELETE</command>、<command>INSERT</command>および<command>MERGE</command>コマンドは、（参照される他の全てのテーブルに対する<literal>ACCESS SHARE</literal>ロックに加えて）対象となるテーブル上にこのモードのロックを獲得します。
通常、このロックモードは、テーブルの<emphasis>データを変更する</emphasis>問い合わせにより獲得されます。␞␞        </para>␞
␝        <para>␟         Conflicts with the <literal>SHARE UPDATE EXCLUSIVE</literal>,
         <literal>SHARE</literal>, <literal>SHARE ROW
         EXCLUSIVE</literal>, <literal>EXCLUSIVE</literal>, and
         <literal>ACCESS EXCLUSIVE</literal> lock modes.
         This mode protects a table against
         concurrent schema changes and <command>VACUUM</command> runs.␟<literal>SHARE UPDATE EXCLUSIVE</literal>、<literal>SHARE</literal>、<literal>SHARE ROW EXCLUSIVE</literal>、<literal>EXCLUSIVE</literal>、および<literal>ACCESS EXCLUSIVE</literal>ロックモードと競合します。
このモードにより、同時実行されるスキーマの変更および<command>VACUUM</command>コマンドの実行から、テーブルを保護します。␞␞        </para>␞
␝        <para>␟         Acquired by <command>VACUUM</command> (without <option>FULL</option>),
         ...
         (for full details see the documentation of these commands).␟（<option>FULL</option>なしの）<command>VACUUM</command>、<command>ANALYZE</command>、<command>CREATE INDEX CONCURRENTLY</command>、<command>CREATE STATISTICS</command>、<command>COMMENT ON</command>、<command>REINDEX CONCURRENTLY</command>、ある種の<link linkend="sql-alterindex"><command>ALTER INDEX</command></link>、<link linkend="sql-altertable"><command>ALTER TABLE</command></link>の変種で取得されます（完全な詳細はこれらのコマンドの文書を見てください）。␞␞        </para>␞
␝        <para>␟         Conflicts with the <literal>ROW EXCLUSIVE</literal>,
         <literal>SHARE UPDATE EXCLUSIVE</literal>, <literal>SHARE ROW
         EXCLUSIVE</literal>, <literal>EXCLUSIVE</literal>, and
         <literal>ACCESS EXCLUSIVE</literal> lock modes.
         This mode protects a table against concurrent data changes.␟<literal>ROW EXCLUSIVE</literal>、<literal>SHARE UPDATE EXCLUSIVE</literal>、<literal>SHARE ROW EXCLUSIVE</literal>、<literal>EXCLUSIVE</literal>、および<literal>ACCESS EXCLUSIVE</literal>ロックモードと競合します。
このモードは、同時実行されるデータ変更からテーブルを保護します。␞␞        </para>␞
␝        <para>␟         Acquired by <command>CREATE INDEX</command>
         (without <option>CONCURRENTLY</option>).␟（<option>CONCURRENTLY</option>なしの）<command>CREATE INDEX</command>によって獲得されます。␞␞        </para>␞
␝        <para>␟         Conflicts with the <literal>ROW EXCLUSIVE</literal>,
         ...
         is self-exclusive so that only one session can hold it at a time.␟<literal>ROW EXCLUSIVE</literal>、<literal>SHARE UPDATE EXCLUSIVE</literal>、
...
このモードは、1つのセッションだけが一度にそれを保持できるよう、自己排他的に同時のデータ変更からテーブルを保護します。␞␞        </para>␞
␝        <para>␟         Acquired by <command>CREATE TRIGGER</command> and some forms of
         <link linkend="sql-altertable"><command>ALTER TABLE</command></link>.␟<command>CREATE TRIGGER</command>、および、<link linkend="sql-altertable"><command>ALTER TABLE</command></link>のいくつかの形式により獲得されます。␞␞        </para>␞
␝        <para>␟         Conflicts with the <literal>ROW SHARE</literal>, <literal>ROW
         ...
         transaction holding this lock mode.␟<literal>ROW SHARE</literal>、<literal>ROW EXCLUSIVE</literal>、
...
つまり、このロックモードを保持するトランザクションと並行して実行できる処理は、テーブルの読み取りだけです。␞␞        </para>␞
␝        <para>␟         Acquired by <command>REFRESH MATERIALIZED VIEW CONCURRENTLY</command>.␟<command>REFRESH MATERIALIZED VIEW CONCURRENTLY</command>により獲得されます。␞␞        </para>␞
␝        <para>␟         Conflicts with locks of all modes (<literal>ACCESS
         ...
         holder is the only transaction accessing the table in any way.␟全てのモードのロック（<literal>ACCESS
         SHARE</literal>、<literal>ROW SHARE</literal>、<literal>ROW
         EXCLUSIVE</literal>、<literal>SHARE UPDATE
         EXCLUSIVE</literal>、<literal>SHARE</literal>、<literal>SHARE
         ROW EXCLUSIVE</literal>、<literal>EXCLUSIVE</literal>、および
         <literal>ACCESS EXCLUSIVE</literal>）と競合します。
このモードにより、その保持者以外にテーブルにアクセスするトランザクションがないことを保証されます。␞␞        </para>␞
␝        <para>␟         Acquired by the <command>DROP TABLE</command>,
         ...
         a mode explicitly.␟<command>DROP TABLE</command>、<command>TRUNCATE</command>、<command>REINDEX</command>、<command>CLUSTER</command>、<command>VACUUM FULL</command>、（<option>CONCURRENTLY</option>なしの）<command>REFRESH MATERIALIZED VIEW</command>コマンドによって獲得されます。
<command>ALTER INDEX</command>と<command>ALTER TABLE</command>の多くの形式もこのレベルでロックを獲得します。
これはまた、明示的にモードを指定しない<command>LOCK TABLE</command>文のデフォルトのロックモードです。␞␞        </para>␞
␝      <para>␟       Only an <literal>ACCESS EXCLUSIVE</literal> lock blocks a
       <command>SELECT</command> (without <option>FOR UPDATE/SHARE</option>)
       statement.␟<literal>ACCESS EXCLUSIVE</literal>ロックのみが、<command>SELECT</command>（<option>FOR UPDATE/SHARE</option>なし）文をブロックします。␞␞      </para>␞
␝   <para>␟    Once acquired, a lock is normally held until the end of the transaction.  But if a
    ...
    releases locks acquired within it.␟通常ロックは獲得した後、トランザクションの終わりまで保持されます。
しかし、ロックがセーブポイントの確立後に獲得された場合、セーブポイントがロールバックされると、ロックは即座に解放されます。
これは、<command>ROLLBACK</command>がセーブポイント以降に行われたすべてのコマンドの効果を取り消すという原則と整合性が取れています。
<application>PL/pgSQL</application>例外ブロック内で獲得されたロックに対しても同様です。
そのブロックからエラーで抜けた後、獲得されたロックは解放されます。␞␞   </para>␞
␝    <table tocentry="1" id="table-lock-compatibility">␟     <title>Conflicting Lock Modes</title>␟     <title>ロックモードの競合</title>␞␞     <tgroup cols="9">␞
␝       <row>␟        <entry morerows="1">Requested Lock Mode</entry>
        <entry spanname="lockreq">Existing Lock Mode</entry>␟        <entry morerows="1">要求するロックモード</entry>
        <entry spanname="lockreq">既存のロックモード</entry>␞␞       </row>␞
␝   <sect2 id="locking-rows">␟    <title>Row-Level Locks</title>␟    <title>行レベルロック</title>␞␞␞
␝    <para>␟     In addition to table-level locks, there are row-level locks, which
     ...
     savepoint rollback, just like table-level locks.␟テーブルレベルロックに加えて、行レベルロックがあります。<productname>PostgreSQL</productname>が自動的に使う文脈付きで以下に行レベルロックの一覧があります。
...
␝     <variablelist>␟      <title>Row-Level Lock Modes</title>␟      <title>行レベルロックモード</title>␞␞      <varlistentry>␞
␝        <para>␟         <literal>FOR UPDATE</literal> causes the rows retrieved by the
         ...
         <xref linkend="applevel-consistency"/>.␟<literal>FOR UPDATE</literal>により<command>SELECT</command>文により取り出された行が更新用であるかのようにロックされます。
...
これ以上の説明は<xref linkend="applevel-consistency"/>を参照してください。␞␞        </para>␞
␝        <para>␟         The <literal>FOR UPDATE</literal> lock mode
         ...
         in the future.␟<literal>FOR UPDATE</literal>ロックモードは行に対する<command>DELETE</command>でも、ある列の値を変更する<command>UPDATE</command>でも獲得されます。
現時点では、<command>UPDATE</command>の場合に考慮される列の集合は、外部キーとして使うことのできる一意のインデックス（つまり部分インデックスや式インデックスは考慮されません）があるものですが、これは将来変わるかもしれません。␞␞        </para>␞
␝        <para>␟         Behaves similarly to <literal>FOR UPDATE</literal>, except that the lock
         ...
         <command>UPDATE</command> that does not acquire a <literal>FOR UPDATE</literal> lock.␟獲得するロックが弱い以外は<literal>FOR UPDATE</literal>と同じように振る舞います。このロックは同じ行のロックを獲得しようとする<literal>SELECT FOR KEY SHARE</literal>コマンドをブロックしません。
このロックモードは<literal>FOR UPDATE</literal>ロックを獲得しない<command>UPDATE</command>によっても獲得されます。␞␞        </para>␞
␝        <para>␟         Behaves similarly to <literal>FOR NO KEY UPDATE</literal>, except that it
         ...
         <command>SELECT FOR KEY SHARE</command>.␟取り出された各行に対して排他ロックではなく共有ロックを獲得する以外は、<literal>FOR NO KEY UPDATE</literal>と同じように振る舞います。
共有ロックは、他のトランザクションがこれらの行に対して<command>UPDATE</command>、<command>DELETE</command>、<command>SELECT FOR UPDATE</command>、<command>SELECT FOR NO KEY UPDATE</command>を実行するのをブロックしますが、<command>SELECT FOR SHARE</command>や<command>SELECT FOR KEY SHARE</command>を実行するのを阻害しません。␞␞        </para>␞
␝        <para>␟         Behaves similarly to <literal>FOR SHARE</literal>, except that the
         ...
         or <command>SELECT FOR KEY SHARE</command>.␟獲得するロックが弱い以外は<literal>FOR SHARE</literal>と同じように振る舞います。<literal>SELECT FOR UPDATE</literal>はブロックされますが、<literal>SELECT FOR NO KEY UPDATE</literal>はブロックされません。
キー共有ロックは、他のトランザクションが<command>DELETE</command>やキー値を変更する<command>UPDATE</command>を実行するのをブロックしますが、それ以外の<command>UPDATE</command>や、<command>SELECT FOR NO KEY UPDATE</command>、<command>SELECT FOR SHARE</command>、<command>SELECT FOR KEY SHARE</command>を阻害しません。␞␞        </para>␞
␝    <para>␟     <productname>PostgreSQL</productname> doesn't remember any
     ...
     will result in disk writes.␟<productname>PostgreSQL</productname>では、メモリ上に変更された行の情報を記憶しないため、同時にロックできる行数の上限はありません。
しかし、行をロックする際に、ディスクに書き込む作業が発生するかもしれません。
例えば<command>SELECT FOR UPDATE</command>は、選択された行をロックしたものと印を付けるために変更を行いますので、ディスクにその結果を書き込むことになります。␞␞    </para>␞
␝    <table tocentry="1" id="row-lock-compatibility">␟     <title>Conflicting Row-Level Locks</title>␟     <title>行レベルロックの競合</title>␞␞     <tgroup cols="5">␞
␝       <row>␟        <entry morerows="1">Requested Lock Mode</entry>
        <entry spanname="lockreq">Current Lock Mode</entry>␟        <entry morerows="1">要求するロックモード</entry>
        <entry spanname="lockreq">現在のロックモード</entry>␞␞       </row>␞
␝   <sect2 id="locking-pages">␟    <title>Page-Level Locks</title>␟    <title>ページレベルロック</title>␞␞␞
␝    <para>␟     In addition to table and row locks, page-level share/exclusive locks are
     used to control read/write access to table pages in the shared buffer
     pool.  These locks are released immediately after a row is fetched or
     updated.  Application developers normally need not be concerned with
     page-level locks, but they are mentioned here for completeness.␟テーブルと行ロックに加え、ページレベルの共有/排他ロックがあり、これらは共有バッファプールにあるテーブルページへの読み書きのアクセスを管理するために使用されます。
これらのロックは、行が取得された後や更新された後に即座に解除されます。
アプリケーション開発者は通常ページレベルロックを考慮する必要はありませんが、ロックについて全てを説明したかったためここで取り上げました。␞␞    </para>␞
...
␝   <sect2 id="locking-deadlocks">␟    <title>Deadlocks</title>␟    <title>デッドロック</title>␞␞␞
␝    <para>␟     The use of explicit locking can increase the likelihood of
     ...
     predict and should not be relied upon.)␟明示的なロックの使用は、<firstterm>デッドロック</firstterm>の原因となる可能性があります。
...
（どちらのトランザクションをアボートするかを正確に予期するのは難しく、これに依存すべきではありません）。␞␞    </para>␞
␝    <para>␟     Note that deadlocks can also occur as the result of row-level
     locks (and thus, they can occur even if explicit locking is not
     used). Consider the case in which two concurrent
     transactions modify a table. The first transaction executes:␟デッドロックは行レベルロックの結果として発生する可能性があります
...
␝␟     This acquires a row-level lock on the row with the specified
     account number. Then, the second transaction executes:␟これは、指定した口座番号の行に対し行レベルロックを獲得します。
...
␝␟     The first <command>UPDATE</command> statement successfully
     ...
     transaction one executes:␟1つ目の<command>UPDATE</command>文は指定された行に対する行レベルロックの獲得に成功し、この行の更新に成功します。
...
␝␟     Transaction one attempts to acquire a row-level lock on the
     ...
     situation and abort one of the transactions.␟トランザクション1は指定した行の行レベルロックを獲得しようとしますが、これは不可能です。
...
<productname>PostgreSQL</productname>はデッドロック状態を検知し、片方のトランザクションを中断させます。␞␞    </para>␞
␝    <para>␟     The best defense against deadlocks is generally to avoid them by
     ...
     transactions that abort due to deadlocks.␟デッドロックを防ぐ最も良い方法は、データベースを使用する全てのアプリケーションが、整合性のある順序で複数のオブジェクトに対するロックを獲得することです。
...
このことが事前に検証できない場合、デッドロックによりアボートするトランザクションを再試行すれば、デッドロックをデータベースを稼働させながらでも処理できます。␞␞    </para>␞
␝    <para>␟     So long as no deadlock situation is detected, a transaction seeking
     either a table-level or row-level lock will wait indefinitely for
     conflicting locks to be released.  This means it is a bad idea for
     applications to hold transactions open for long periods of time
     (e.g., while waiting for user input).␟デッドロック状況が検出されなければ、テーブルレベルロックもしくは行レベルロックを要求するトランザクションは、競合するロックが解放されるまで、無期限に待機します。
したがって、アプリケーションで長時間（例えば、ユーザの入力待ち）トランザクションを開いたまま保持しておくのは、推奨されません。␞␞    </para>␞
...
␝   <sect2 id="advisory-locks">␟    <title>Advisory Locks</title>␟    <title>勧告的ロック</title>␞␞␞
␝    <para>␟     <productname>PostgreSQL</productname> provides a means for
     ...
     cleaned up by the server at the end of the session.␟<productname>PostgreSQL</productname>は、アプリケーション独自の意味を持つロックを生成する手法を提供します。
...
また、セッション終了時にサーバによる自動整理を行うこともできるようになります。␞␞    </para>␞
␝    <para>␟     There are two ways to acquire an advisory lock in
     ...
     request are at session level or transaction level.␟<productname>PostgreSQL</productname>には、セッションレベルとトランザクションレベルという2つの勧告的ロックの獲得方法があります。
...
これは保持されているロックと新しい要求がセッションレベルかトランザクションレベルかに関わらず、この文は当てはまります。␞␞    </para>␞
␝    <para>␟     Like all locks in
     ...
     view.␟<productname>PostgreSQL</productname>におけるすべてのロックと同様に、現時点ですべてのセッションで保持されている勧告的ロックの全一覧は<link linkend="view-pg-locks"><structname>pg_locks</structname></link>システムビューにあります。␞␞    </para>␞
␝    <para>␟     Both advisory locks and regular locks are stored in a shared memory
     ...
     depending on how the server is configured.␟勧告的ロックと通常のロックは共有メモリプールに割り当てられ、その容量は<xref linkend="guc-max-locks-per-transaction"/>と<xref linkend="guc-max-connections"/>設定変数により決定されます。
このメモリを浪費しないように注意が必要です。
さもないと、サーバはロック獲得をまったく許可することができなくなります。
これは、サーバで許可できる勧告的ロック数に上限があることを意味します。
サーバの設定によりますが、通常、1万から10万程度になります。␞␞    </para>␞
␝    <para>␟     In certain cases using advisory locking methods, especially in queries
     involving explicit ordering and <literal>LIMIT</literal> clauses, care must be
     taken to control the locks acquired because of the order in which SQL
     expressions are evaluated.  For example:␟特に明示的な順序付けと<literal>LIMIT</literal>句を持つ問い合わせでは、この勧告ロックモードを使用する幾つかの場合において、SQL式が評価される順序を考慮し獲得されたロックを制御することに気を配らなければなりません。
以下に例を示します。␞␞<screen>␞
␝SELECT pg_advisory_lock(id) FROM foo WHERE id &gt; 12345 LIMIT 100; -- danger!␟SELECT pg_advisory_lock(id) FROM foo WHERE id = 12345; -- ok
SELECT pg_advisory_lock(id) FROM foo WHERE id &gt; 12345 LIMIT 100; -- danger!
SELECT pg_advisory_lock(q.id) FROM
(
  SELECT id FROM foo WHERE id &gt; 12345 LIMIT 100
) q; -- ok␟SELECT pg_advisory_lock(id) FROM foo WHERE id = 12345; -- 問題なし
...
␝</screen>␟     In the above queries, the second form is dangerous because the
     ...
     <structname>pg_locks</structname>.␟上の例では、ロック獲得関数が実行される前に<literal>LIMIT</literal> が適用されることを保障できないため、2番目の形式は危険です。
これにより、アプリケーションが想定していないなんらかのロックが生成される可能性があります。
そのため、（セッションが終了するまで）解放に失敗することになります。
アプリケーションから見ると、こうしたロックはただの飾りですが、<structname>pg_locks</structname>からは参照され続けます。␞␞    </para>␞
␝    <para>␟     The functions provided to manipulate advisory locks are described in
     <xref linkend="functions-advisory-locks"/>.␟勧告的ロックを扱うための関数については、<xref linkend="functions-advisory-locks"/>で説明します。␞␞    </para>␞
␝  <sect1 id="applevel-consistency">␟   <title>Data Consistency Checks at the Application Level</title>␟   <title>アプリケーションレベルでのデータの一貫性チェック</title>␞␞␞
␝   <para>␟    It is very difficult to enforce business rules regarding data integrity
    ...
    restrict itself to the statement's snapshot if a write conflict occurs.␟データの参照範囲は各ステートメントで変化するので、リードコミッティドトランザクションを使用して、データ保全性に関するビジネスルールを強化するのは非常に難しいことです。また、書き込み競合が生じる場合、単一のステートメントでさえステートメントのスナップショットに限定されないかもしれません。␞␞   </para>␞
␝   <para>␟    While a Repeatable Read transaction has a stable view of the data
    ...
    without some help.␟リピータブルリードトランザクションは実行全体にわたってデータの安定した参照範囲を持ちますが、<acronym>MVCC</acronym>スナップショットをデータ完全性チェックに使用することによる、<firstterm>読み取り/書き込み競合</firstterm>として知られるものを含む、微妙な問題があります。
...
そのような循環が出現する時、完全性のチェックはなにかしらの支援がなければ正しく動作しません。␞␞   </para>␞
␝   <para>␟    As mentioned in <xref linkend="xact-serializable"/>, Serializable
    ...
    break the cycle.␟<xref linkend="xact-serializable"/>により述べたように、シリアライザブルトランザクションは、危険なパターンの読み取り/書き込み競合のための非ブロッキング監視を加えたリピータブルリードトランザクションです。
明白に実行順が循環を引き起こすパターンが検知された場合、含まれていたトランザクションのうちの1つは循環を断ち切るためにロールバックされます。␞␞   </para>␞
␝   <sect2 id="serializable-consistency">␟    <title>Enforcing Consistency with Serializable Transactions</title>␟    <title>シリアライザブルトランザクションを用いた一貫性の強化</title>␞␞␞
␝    <para>␟     If the Serializable transaction isolation level is used for all writes
     ...
     <productname>PostgreSQL</productname>.␟シリアライザブルトランザクション分離レベルが、データの一貫性を必要とするすべての書き込みおよびすべての読み取りに使用される場合、一貫性を確実にするために必要なことは他にありません。
一貫性を保証するためにシリアライザブルトランザクションを使用するよう書かれている他の環境からのソフトウェアは、<productname>PostgreSQL</productname>でこの点に関して<quote>正しく動く</quote>べきです。␞␞    </para>␞
␝    <para>␟     When using this technique, it will avoid creating an unnecessary burden
     ...
     level in triggers.␟この技術を使用した場合、アプリケーションソフトウェアが直列化失敗でロールバックしたトランザクションを自動的に再試行するようなフレームワークを備えている場合、アプリケーションプログラマにとって不必要な負担を生み出さないようにするでしょう。
<literal>default_transaction_isolation</literal>を<literal>serializable</literal>にセットすることはよい考えかもしれません。
他のトランザクション分離レベルは使用されないことを保証する処置を講ずる、そうでなければ、不注意に完全位チェックを失わないよう、トリガでトランザクション分離レベルのチェックをすることも賢明でしょう。␞␞    </para>␞
␝    <para>␟     See <xref linkend="xact-serializable"/> for performance suggestions.␟実行に関する提言は<xref linkend="xact-serializable"/>を参照してください。␞␞    </para>␞
␝    <warning>␟     <title>Warning: Serializable Transactions and Data Replication</title>␟     <title>警告: シリアライザブルトランザクションとデータレプリケーション</title>␞␞     <para>␞
␝     <para>␟      This level of integrity protection using Serializable transactions
      ...
      may want to use Repeatable Read and explicit locking on the primary.␟シリアライザブルトランザクションを使用する整合性保護レベルは、まだホットスタンバイモード(<xref linkend="hot-standby"/>)や論理レプリケーションには拡張されていません。
そのために、ホットスタンバイや論理レプリケーションを使用する場合は、プライマリにおけるリピータブルリードと明示的なロック処理の利用が望まれるかもしれません。␞␞     </para>␞
␝   <sect2 id="non-serializable-consistency">␟    <title>Enforcing Consistency with Explicit Blocking Locks</title>␟    <title>明示的なブロッキングロックを用いた一貫性の強化</title>␞␞␞
␝    <para>␟     When non-serializable writes are possible,
     ...
     <productname>PostgreSQL</productname> from other environments.␟非シリアライザブルの書き込みが可能な場合、
...
これは<productname>PostgreSQL</productname>に他の環境からアプリケーションを移植する時に考慮されなければなりません␞␞    </para>␞
␝    <para>␟     Also of note to those converting from other environments is the fact
     ...
     the row was performed while the lock was held.␟他の環境から切り替えた場合のさらなる注意点としては、同時実行トランザクションが選択された行を更新しないか削除しないということを<command>SELECT FOR UPDATE</command>が保証しないという事実です。
...
しかしトランザクションがコミットするかロールバックして一度このロックを獲得すると、ロックが獲得されている間に、行の実際の<command>UPDATE</command>が行われなかった場合、ブロックされたトランザクションは、競合した操作を続けることになります。␞␞    </para>␞
␝    <para>␟     Global validity checks require extra thought under
     ...
     transaction.␟非シリアライザブル<acronym>MVCC</acronym>においては全体的な有効性チェックに特別な考慮を払わなければなりません。
...
<literal>SHARE</literal>モード（もしくはそれ以上）のロックにより、現在のトランザクションでの変更を除き、ロックされたテーブルにコミットされていない変更が存在しないことを保証されます。␞␞    </para>␞
␝    <para>␟     Note also that if one is relying on explicit locking to prevent concurrent
     ...
     before the snapshot is frozen.␟同時に、明示的なロック処理を使用して、同時に変更が実行されるのを防ごうとする場合、リードコミッティドモードを使用するか、または、リピータブルリードモードの場合は、問い合わせを実行する前にロックを獲得するよう留意してください。
...
したがって、スナップショットを取得する前に、明示的にロックを獲得できます。␞␞    </para>␞
␝   <indexterm>
    <primary>serialization failure</primary>
   </indexterm>
␟␟   <indexterm>
    <primary>直列化失敗</primary>
   </indexterm>␞␞␞
␝   <indexterm>
    <primary>retryable error</primary>
   </indexterm>
␟␟   <indexterm>
    <primary>再試行可能エラー</primary>
   </indexterm>␞␞␞
␝  <sect1 id="mvcc-serialization-failure-handling">␟   <title>Serialization Failure Handling</title>␟   <title>直列化失敗の扱い</title>␞␞␞
␝   <para>␟    Both Repeatable Read and Serializable isolation levels can produce
    ...
    (<literal>serialization_failure</literal>).␟リピータブルリード分離レベルとシリアライザブル分離レベルはどちらも、直列化異常を防止するように設計されたエラーを生成する可能性があります。
前述のように、これらのレベルを使用するアプリケーションは、直列化エラーが原因で失敗したトランザクションを再試行するように準備する必要があります。
このようなエラーのメッセージテキストは細かい状況によって異なりますが、常にSQLSTATEコードは<literal>40001</literal>(<literal>serialization_failure</literal>)となります。␞␞   </para>␞
␝   <para>␟    It may also be advisable to retry deadlock failures.
    These have the SQLSTATE code <literal>40P01</literal>
    (<literal>deadlock_detected</literal>).␟デッドロック障害を再試行することも推奨されます。
これらはSQLSTATEコード<literal>40P01</literal>(<literal>deadlock_detected</literal>)を持ちます。␞␞   </para>␞
␝   <para>␟    In some cases it is also appropriate to retry unique-key failures,
    ...
    failures.␟場合によっては、SQLSTATEコード<literal>23505</literal>(<literal>unique_violation</literal>)を持つ一意キーの失敗、およびSQLSTATEコード<literal>23P01</literal>(<literal>exclusion_violation</literal>)を持つ排他制約の失敗を再試行することも適切です。
たとえば、現在格納されているキーを検査した後にアプリケーションが主キー列に新しい値を選択した場合、別のアプリケーション・インスタンスが同じ新しいキーを同時に選択したため、一意キーの失敗が発生する可能性があります。
これは実質的に直列化の失敗ですが、サーバは挿入された値と前の読み取りとの間の関係を<quote>参照</quote>できないため、直列化の失敗を検出しません。
また、原則として直列化の問題が根本原因であると判断するのに十分な情報があるにもかかわらず、サーバが一意キーまたは排他制約のエラーを発行する場合もあります。
<literal>serialization_failure</literal>エラーを無条件に再試行することをお薦めしますが、これらの他のエラー・コードを再試行する場合は、一時的な失敗ではなく永続的なエラー条件を表す可能性があるため、より注意が必要です。␞␞   </para>␞
␝   <para>␟    It is important to retry the complete transaction, including all logic
    ...
    correctness.␟どのSQLを発行するか、どの値を使用するかを決定するすべてのロジックを含む完全なトランザクションを再試行することが重要です。
したがって、<productname>PostgreSQL</productname>は自動再試行機能を提供していません。
なぜなら、自動再試行機能は正当性を保証できないからです。␞␞   </para>␞
␝   <para>␟    Transaction retry does not guarantee that the retried transaction will
    ...
    commits or rolls back.␟トランザクションの再試行は、再試行されたトランザクションが完了することを保証するものではありません。
複数回の再試行が必要になる場合があります。
競合が非常に高い場合は、トランザクションの完了に多くの試行が必要になる可能性があります。
競合する準備済トランザクションが関係する場合は、準備済トランザクションがコミットまたはロールバックされるまで進行できない可能性があります。␞␞   </para>␞
␝  <sect1 id="mvcc-caveats">␟   <title>Caveats</title>␟   <title>警告</title>␞␞␞
␝   <para>␟    Some DDL commands, currently only <link linkend="sql-truncate"><command>TRUNCATE</command></link> and the
    ...
    table and other tables in the database.␟DDLコマンドの中には、現在は<link linkend="sql-truncate"><command>TRUNCATE</command></link>とテーブルを書き換える形の<link linkend="sql-altertable"><command>ALTER TABLE</command></link>だけですが、MVCCセーフでないものがあります。
...
ですので、対象のテーブルに対する連続した問い合わせで、このコマンドはテーブルの内容の見かけ上の不整合の原因とはなりません。しかし、対象のテーブルとデータベース内の他のテーブルの内容の間の可視の不整合の原因となるかもしれません。␞␞   </para>␞
␝   <para>␟    Support for the Serializable transaction isolation level has not yet
    ...
    of the transactions on the primary.␟シリアライザブルトランザクション分離レベルのサポートは、まだホットスタンバイレプリケーションは対象に加えられていません(<xref linkend="hot-standby"/>で述べます)。
ホットスタンバイモードで現在サポートされた最も厳しい分離レベルはリピータブルリードです。
プライマリ上でシリアライザブルトランザクション中にデータベースに永続的な書き込みを行えば、スタンバイはすべて最終的に一貫した状態に達するだろうということは保証されるでしょうが、スタンバイ上で実行されたリピータブルリードトランザクションは、ときどきプライマリのトランザクションの任意の連続する実行と一致しない過渡状態を見ることがあるでしょう。␞␞   </para>␞
␝   <para>␟    Internal access to the system catalogs is not done using the isolation
    ...
    isolation levels.␟システムカタログへの内部のアクセスは現在のトランザクションの分離レベルを使っては行われません。
これは、テーブルのような新しく作られたデータベースオブジェクトが、たとえシステムカタログが含む行が可視でないとしても、並行するリピータブルリードトランザクションやシリアライザブルトランザクションに対して可視であることを意味します。
対照的に、明示的にシステムカタログを確認する問い合わせは、より高い分離レベルで並行して作られているデータベースオブジェクトを表す行を見ることはできません。␞␞   </para>␞
...
␝  <sect1 id="locking-indexes">␟   <title>Locking and Indexes</title>␟   <title>ロックとインデックス</title>␞␞␞
␝   <para>␟    Though <productname>PostgreSQL</productname>
    provides nonblocking read/write access to table
    data, nonblocking read/write access is not currently offered for every
    index access method implemented
    in <productname>PostgreSQL</productname>.
    The various index types are handled as follows:␟<productname>PostgreSQL</productname>は、テーブルデータへのノンブロック読み込み/書き込みアクセスを備えています。しかし現在、この機能は<productname>PostgreSQL</productname>で実装されている全てのインデックスアクセスメソッドに対して実装されているわけではありません。
各種のインデックスでは下記のように扱われます。␞␞␞
␝      <term>␟       B-tree, <acronym>GiST</acronym> and <acronym>SP-GiST</acronym> indexes␟B-treeインデックス、<acronym>GiST</acronym>および<acronym>SP-GiST</acronym>インデックス␞␞      </term>␞
␝       <para>␟        Short-term share/exclusive page-level locks are used for
        read/write access. Locks are released immediately after each
        index row is fetched or inserted.  These index types provide
        the highest concurrency without deadlock conditions.␟読み込み/書き込みアクセスに短期の共有/排他モードのページレベルロックを使います。
ロックは、インデックス行が挿入または取り出されるとただちに解放されます。
これらのインデックス種類は、デッドロック状態になることなく、最も高い同時実行性を提供します。␞␞       </para>␞
␝      <term>␟       Hash indexes␟       ハッシュインデックス␞␞      </term>␞
␝       <para>␟        Share/exclusive hash-bucket-level locks are used for read/write
        ...
        than one index operation.␟読み込み/書き込みアクセスに共有/排他モードのハッシュバケットレベルロックを使います。
ロックは、バケット全体が処理された後に解放されます。
バケットレベルロックは、インデックスレベルのロックよりも同時実行性に優れていますが、1つのインデックス操作よりも長くロックが保持されますので、デッドロックに陥りやすくなります。␞␞       </para>␞
␝      <term>␟       <acronym>GIN</acronym> indexes␟       <acronym>GIN</acronym>インデックス␞␞      </term>␞
␝       <para>␟        Short-term share/exclusive page-level locks are used for
        read/write access. Locks are released immediately after each
        ...
        insertion.␟読み込み/書き込みアクセスに短期の共有/排他モードのページレベルロックを使います。
ロックは、インデックス行が挿入または取り出されるとただちに解放されます。
しかし、GINによりインデックス付けされた値の挿入は、通常1行当たり複数のインデックスキーの挿入をもたらすことに注意してください。
そのため、GINは単一の値を挿入する時にさらに多くの作業を行います。␞␞       </para>␞
␝   <para>␟    Currently, B-tree indexes offer the best performance for concurrent
    applications; since they also have more features than hash
    indexes, they are the recommended index type for concurrent
    applications that need to index scalar data. When dealing with
    non-scalar data, B-trees are not useful, and GiST, SP-GiST or GIN
    indexes should be used instead.␟現時点では、B-treeインデックスは同時実行アプリケーションにおいて最善の性能を提供します。
これはまた、ハッシュインデックスよりも多くの機能を持つため、スカラデータのインデックスが必要な同時実行アプリケーションで推奨するインデックス型です。
非スカラデータを扱う場合、B-treeを使用することができないことは明確です。
この場合は代わりにGiST、SP-GiSTもしくはGINインデックスを使用すべきです。␞␞   </para>␞
