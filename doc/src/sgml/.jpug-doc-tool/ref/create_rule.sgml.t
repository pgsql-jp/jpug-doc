...
␝  <refname>CREATE RULE</refname>␟  <refpurpose>define a new rewrite rule</refpurpose>␟  <refpurpose>
新しい書き換えルールを定義する
...
␝  <para>␟   <command>CREATE RULE</command> defines a new rule applying to a specified
   table or view.
   <command>CREATE OR REPLACE RULE</command> will either create a
   new rule, or replace an existing rule of the same name for the same
   table.␟<command>CREATE RULE</command>により、指定したテーブルまたはビューに適用するルールを新しく定義できます。
<command>CREATE OR REPLACE RULE</command>を使用すると、新しいルールの作成、または、同じテーブルの同じ名前の既存ルールの置き換えのいずれかを実行します。␞␞  </para>␞
␝  <para>␟   The <productname>PostgreSQL</productname> rule system allows one to
   ...
   More information about the rules system is in <xref linkend="rules"/>.␟<productname>PostgreSQL</productname>のルールシステムを使用すると、データベーステーブルに対する挿入、更新、削除時に本来の操作の代替として実行するアクションを定義できます。
...
ルールシステムについての詳細は、<xref linkend="rules"/>に記載されています。␞␞  </para>␞
␝  <para>␟   Presently, <literal>ON SELECT</literal> rules can only be attached
   ...
   VIEW</literal>.␟現時点では、<literal>ON SELECT</literal>ルールはビューにのみ付けられます。
...
ビューは<literal>CREATE OR REPLACE RULE "_RETURN" AS ...</literal>を使って再定義できますが、<literal>CREATE OR REPLACE VIEW</literal>を使う方が良いです。␞␞  </para>␞
␝  <para>␟   You can create the illusion of an updatable view by defining
   ...
   <literal>RETURNING</literal> clause into each of these rules.␟<literal>ON INSERT</literal>、<literal>ON UPDATE</literal>、<literal>ON DELETE</literal>ルールを必要に応じて定義し、ビューに対する更新操作を他のテーブルに対する適切な更新操作に置換することで、更新可能なビューという実在しないオブジェクトを作成することができます。
<command>INSERT RETURNING</command>などをサポートしたければ、これらのルールに適切な<literal>RETURNING</literal>句を確実につけてください。␞␞  </para>␞
␝  <para>␟   There is a catch if you try to use conditional rules for complex view
   ...
   currently work to support <literal>RETURNING</literal> queries, however.)␟複雑なビューの更新に条件付きルールを使用しようとする場合、落とし穴があります。
...
（しかし現在、この方法では<literal>RETURNING</literal>問い合わせのサポートがうまく動作しません。）␞␞  </para>␞
␝   <para>␟    A view that is simple enough to be automatically updatable (see <xref
    ...
    explicit rule.␟十分単純であり自動更新可能なビュー（<xref linkend="sql-createview"/>参照）は、更新可能とするためにユーザ作成のルールを必要としません。
とにかく明示的なルールを作成することもできますが、自動更新による変形は明示的なルールよりも通常高速です。␞␞   </para>␞
␝   <para>␟    Another alternative worth considering is to use <literal>INSTEAD OF</literal>
    triggers (see <xref linkend="sql-createtrigger"/>) in place of rules.␟検討すべき別の方法は、ルールの代わりに<literal>INSTEAD OF</literal>トリガ(<xref linkend="sql-createtrigger"/>参照)を使うことです。␞␞   </para>␞
␝ <refsect1>␟  <title>Parameters</title>␟  <title>パラメータ</title>␞␞␞
␝     <para>␟      The name of a rule to create.  This must be distinct from the
      ...
      name order.␟作成するルールの名前です。
この名前は、同じテーブルの他のルールとは異なる名前にする必要があります。
同一テーブルにイベントの種類が同じルールが複数あった場合、ルール名の順番（アルファベット順）に適用されます。␞␞     </para>␞
␝     <para>␟      The event is one of <literal>SELECT</literal>,
      ...
      rules.  Consider using an updatable view instead.␟イベントとは、<literal>SELECT</literal>、<literal>INSERT</literal>、<literal>UPDATE</literal>、<literal>DELETE</literal>のいずれかです。
...
代わりに、更新可能ビューを利用することを検討してください。␞␞     </para>␞
␝     <para>␟      The name (optionally schema-qualified) of the table or view the
      rule applies to.␟ルールを適用するテーブルまたはビューの名前です（スキーマ修飾名も可）。␞␞     </para>␞
␝     <para>␟      Any <acronym>SQL</acronym> conditional expression (returning
      ...
      cannot contain aggregate functions.␟任意の<acronym>SQL</acronym>条件式です（<type>boolean</type>型を返します）。
条件式では、<literal>NEW</literal>および<literal>OLD</literal>以外のテーブルは参照できません。
また、集約関数を含めることもできません。␞␞     </para>␞
␝    <listitem>␟     <para><literal>INSTEAD</literal> indicates that the commands should be
      executed <emphasis>instead of</emphasis> the original command.␟<para><literal>INSTEAD</literal>は、元のコマンドの<emphasis>代わりに</emphasis>このコマンドが実行されることを示します。␞␞     </para>␞
␝    <listitem>␟     <para><literal>ALSO</literal> indicates that the commands should be
      executed <emphasis>in addition to</emphasis> the original
      command.␟<para><literal>ALSO</literal>は元のコマンドに<emphasis>加えて</emphasis>このコマンドが実行されることを示します。␞␞     </para>␞
␝     <para>␟      If neither <literal>ALSO</literal> nor
      <literal>INSTEAD</literal> is specified, <literal>ALSO</literal>
      is the default.␟<literal>ALSO</literal>も<literal>INSTEAD</literal>も指定されなかった場合、<literal>ALSO</literal>がデフォルトです。␞␞     </para>␞
␝     <para>␟      The command or commands that make up the rule action.  Valid
      commands are <command>SELECT</command>,
      <command>INSERT</command>, <command>UPDATE</command>,
      <command>DELETE</command>, or <command>NOTIFY</command>.␟ルールのアクションを構成する単一または複数のコマンドです。
有効なコマンドは、<command>SELECT</command>、<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>、<command>NOTIFY</command>です。␞␞     </para>␞
␝  <para>␟   Within <replaceable class="parameter">condition</replaceable> and
   ...
   to refer to the existing row being updated or deleted.␟<replaceable class="parameter">condition</replaceable>と<replaceable class="parameter">command</replaceable>の内部では、対象とするテーブルの値を参照するために、特別なテーブル名<literal>NEW</literal>と<literal>OLD</literal>を使用できます。
<literal>NEW</literal>は、<literal>ON INSERT</literal>と<literal>ON UPDATE</literal>ルールで有効で、挿入または更新される新しい行を参照します。
<literal>OLD</literal>は、<literal>ON UPDATE</literal>と<literal>ON DELETE</literal>ルールで有効で、更新または削除される既存の行を参照します。␞␞  </para>␞
␝ <refsect1>␟  <title>Notes</title>␟  <title>注釈</title>␞␞␞
␝  <para>␟   You must be the owner of a table to create or change rules for it.␟テーブルにルールを定義する、または、そのルールを変更するためには所有者でなければなりません。␞␞  </para>␞
␝  <para>␟   In a rule for <literal>INSERT</literal>, <literal>UPDATE</literal>, or
   ...
   there is no <literal>RETURNING</literal> clause in any available rule.␟ビュー上の<literal>INSERT</literal>、<literal>UPDATE</literal>、<literal>DELETE</literal>ルールでは、<literal>RETURNING</literal>句を追加して、ビューの列を返すことができます。
...
ビュー上の<literal>RETURNING</literal>問い合わせは、利用可能なルールのどれにも<literal>RETURNING</literal>が存在しない場合に拒絶されます。␞␞  </para>␞
␝  <para>␟   It is very important to take care to avoid circular rules.  For
   ...
   of recursive expansion of a rule:␟ルールの循環は絶対に避けるよう注意してください。
...
␝  <para>␟   Presently, if a rule action contains a <command>NOTIFY</command>
   ...
   to.  For example, in:␟現在、ルールのアクションに<command>NOTIFY</command>コマンドが含まれる場合、<command>NOTIFY</command>コマンドは無条件に実行されます。
つまり、ルールを適用すべき行が存在しなかったとしても、<command>NOTIFY</command>が発行されます。
例えば、␞␞<programlisting>␞
␝</programlisting>␟   one <command>NOTIFY</command> event will be sent during the
   <command>UPDATE</command>, whether or not there are any rows that
   match the condition <literal>id = 42</literal>.  This is an
   implementation restriction that might be fixed in future releases.␟では、<literal>id = 42</literal>という条件に一致する行があってもなくても、<command>UPDATE</command>の際、1つの<command>NOTIFY</command>イベントが送信されます。
これは実装上の制限であり、将来のリリースでは修正されるかもしれません。␞␞  </para>␞
␝ <refsect1>␟  <title>Compatibility</title>␟  <title>互換性</title>␞␞␞
␝  <para>␟   <command>CREATE RULE</command> is a
   <productname>PostgreSQL</productname> language extension, as is the
   entire query rewrite system.␟<command>CREATE RULE</command>は<productname>PostgreSQL</productname>の言語拡張で、問い合わせ書き換えシステム全体が言語拡張です。␞␞  </para>␞
