...
␝<chapter id="indexam">␟ <title>Index Access Method Interface Definition</title>␟ <title>インデックスアクセスメソッドのインタフェース定義</title>␞␞␞
␝  <para>␟   This chapter defines the interface between the core
   ...
   possible to develop entirely new index types by writing add-on code.␟本章は、<productname>PostgreSQL</productname>のコアシステムと個々のインデックス種類を管理する<firstterm>インデックスアクセスメソッド</firstterm>との間のインタフェースを定義します。
コアシステムはインデックスの仕様のみを把握しています。
したがって、追加コードを記述することで完全に新しいインデックス種類を開発することができます。␞␞  </para>␞
␝  <para>␟   All indexes in <productname>PostgreSQL</productname> are what are known
   ...
   tuple headers; but these decisions are not forced on an access method.)␟<productname>PostgreSQL</productname>のインデックスはすべて、技術的には<firstterm>補助的なインデックス</firstterm>として知られるものです。
...
しかし、これはアクセスメソッドに強制されていることではありません。）␞␞  </para>␞
␝  <para>␟   An index is effectively a mapping from some data key values to
   ...
   are reclaimed.␟インデックスは効率的にあるデータキー値を、インデックスの親テーブル内の行バージョン（タプル）の<firstterm>タプル識別子</firstterm>言い換えると<acronym>TID</acronym>に関連付けます。
...
（<link linkend="storage-hot">HOTタプル</link>はこの説明の例外ですが、インデックスはこれらにも関与しません。）
（バキューム実行によって）無効タプル自身が回収された時に、無効タプルに対するインデックス項目は回収されます。␞␞  </para>␞
␝ <sect1 id="index-api">␟  <title>Basic API Structure for Indexes</title>␟  <title>インデックスの基本的API構造</title>␞␞␞
␝  <para>␟   Each index access method is described by a row in the
   ...
   <xref linkend="sql-drop-access-method"/> SQL commands.␟各インデックスメソッドは<link linkend="catalog-pg-am"><structname>pg_am</structname></link>システムカタログの行で説明されます。
<structname>pg_am</structname>エントリはインデックスアクセスメソッドの名前と<firstterm>ハンドラ関数</firstterm>を指定します。
これらのエントリはSQLコマンド<xref linkend="sql-create-access-method"/>と<xref linkend="sql-drop-access-method"/>を使って、作成および削除することができます。␞␞  </para>␞
␝  <para>␟   An index access method handler function must be declared to accept a
   ...
   in <xref linkend="index-functions"/>.␟インデックスメソッドのハンドラ関数は、<type>internal</type>型の引数を1つ取り、疑似型<type>index_am_handler</type>を返すものとして宣言しなければなりません。
...
サポート関数は<xref linkend="index-functions"/>で説明されています。␞␞  </para>␞
␝  <para>␟   The structure <structname>IndexAmRoutine</structname> is defined thus:␟構造体<structname>IndexAmRoutine</structname>は以下のように定義されています。␞␞<programlisting>␞
␝  <para>␟   To be useful, an index access method must also have one or more
   ...
   this chapter.␟使い易くするために、インデックスアクセスメソッドはまた、<link linkend="catalog-pg-opfamily"><structname>pg_opfamily</structname></link>、<link linkend="catalog-pg-opclass"><structname>pg_opclass</structname></link>、<link linkend="catalog-pg-amop"><structname>pg_amop</structname></link>および<link linkend="catalog-pg-amproc"><structname>pg_amproc</structname></link>内で定義される、複数の<firstterm>演算子族</firstterm>と<firstterm>演算子クラス</firstterm>を持ちます。
これらの項目により、プランナは、このアクセスメソッドのインデックスがどのような問い合わせ条件に対して使用できるかを決定することができます。
演算子族と演算子クラスについては、<xref linkend="xindex"/>で説明します。
これは本章を読む上で必要となる資料です。␞␞  </para>␞
␝  <para>␟   An individual index is defined by a
   ...
   passed to all operations on the index.␟個々のインデックスは、インデックスを物理的なリレーションとして記述する<link linkend="catalog-pg-class"><structname>pg_class</structname></link>項目と、インデックスの論理的な内容、つまり、インデックスが持つインデックス列の集合とその列の意味を、関連する演算子クラスで再現されたものとして表す<link linkend="catalog-pg-index"><structname>pg_index</structname></link>項目とで定義されます。
...
この両方のカタログ項目は、インデックスに対するすべての操作に渡される<structname>Relation</structname>データ構造の一部としてアクセスすることができます。␞␞  </para>␞
␝  <para>␟   Some of the flag fields of <structname>IndexAmRoutine</structname> have nonobvious
   ...
   conditions.␟<structname>IndexAmRoutine</structname>のフラグフィールドの中には、意味がわかりにくいものがあります。
...
これは検索条件として<literal>IS NULL</literal>および<literal>IS NOT NULL</literal>句をサポートすることを示します。␞␞  </para>␞
␝  <para>␟   The <structfield>amcaninclude</structfield> flag indicates whether the
   ...
   null, independently of <structfield>amoptionalkey</structfield>.␟<structfield>amcaninclude</structfield>フラグは、このアクセスメソッドが（処理することなく）キー列以外の追加の列を格納することができる<quote>included</quote>列をサポートしているかどうかを示します。
前段落の要件はキー列にのみ適用されます。
...
また、<structfield>amoptionalkey</structfield>とは独立して、include列はNULLにすることができなければなりません。␞␞  </para>␞
␝  <para>␟   The <structfield>amsummarizing</structfield> flag indicates whether the
   ...
   predicates, an update of such an attribute always disables <acronym>HOT</acronym>.␟<structfield>amsummarizing</structfield>フラグは、このアクセスメソッドがインデックス付きタプルを要約するかどうかを示します。
要約の粒度は少なくともブロック以上です。
個々のタプルを指すのではなく、ブロック範囲を指すアクセスメソッド（<acronym>BRIN</acronym>のような）は、<acronym>HOT</acronym>最適化を継続できる可能性があります。
これは、インデックス述語で参照される属性には適用されません。
そのような属性の更新は常に<acronym>HOT</acronym>が無効になります。␞␞  </para>␞
␝ <sect1 id="index-functions">␟  <title>Index Access Method Functions</title>␟  <title>インデックスアクセスメソッド関数</title>␞␞␞
␝  <para>␟   The index construction and maintenance functions that an index access
   method must provide in <structname>IndexAmRoutine</structname> are:␟インデックスアクセスメソッドが<structname>IndexAmRoutine</structname>で提供しなければならない、インデックス構築および保守関数を以下に示します。␞␞  </para>␞
␝</programlisting>␟   Build a new index.  The index relation has been physically created,
   ...
   this flag set to <literal>false</literal>.␟新しいインデックスを構築します。
...
パラレルインデックス作成をサポートしないアクセスメソッドでは、このフラグは<literal>false</literal>のままにするべきです。␞␞  </para>␞
␝</programlisting>␟   Build an empty index, and write it to the initialization fork (<symbol>INIT_FORKNUM</symbol>)
   ...
   relation fork on each server restart.␟空のインデックスを構築し、それを指定されたリレーションの初期フォーク(<symbol>INIT_FORKNUM</symbol>)に書き出します。
このメソッドはログを取らないインデックスに対してのみ呼び出されます。
初期フォークに書き出された空のインデックスは、サーバの再起動の度に主リレーションフォークにコピーされます。␞␞  </para>␞
␝</programlisting>␟   Insert a new tuple into an existing index.  The <literal>values</literal> and
   ...
   look into the heap to verify tuple liveness).␟既存のインデックスに新しいタプルを挿入します。
...
(タプルの有効性を検証するためにヒープ内を検索しなければなりません)。␞␞  </para>␞
␝  <para>␟   The <literal>indexUnchanged</literal> Boolean value gives a hint
   ...
   authoritative source of information about tuple visibility or versioning.␟<literal>indexUnchanged</literal>真偽値はインデックス付されるタプルの性質に関するヒントを与えます。
...
非キー列や部分インデックス述語にのみ現れる列を更新しても<literal>indexUnchanged</literal>の値には影響がないことに留意してください。
コアコードは、偽陽性と偽陰性の両方を許容する低オーバーヘッドのアプローチを使用して、各タプルの<literal>indexUnchanged</literal>値を決定します。
インデックスAMは、<literal>indexUnchanged</literal>を、タプルの可視性やバージョニングに関する信頼できる情報源として扱ってはいけません。␞␞  </para>␞
␝  <para>␟   The function's Boolean result value is significant only when
   <literal>checkUnique</literal> is <literal>UNIQUE_CHECK_PARTIAL</literal>.
   In this case a true result means the new entry is known unique, whereas
   false means it might be non-unique (and a deferred uniqueness check must
   be scheduled).  For other cases a constant false result is recommended.␟<literal>checkUnique</literal>が<literal>UNIQUE_CHECK_PARTIAL</literal>の場合、関数の論理型の結果値で十分です。
この場合、真の結果は新しい項目は一意であることが確認されたことを、一方偽の結果は一意でない可能性があること(遅延一意性検査を予定しなければならないこと)を意味します。
他の場合では、一定の偽という結果が推奨されます。␞␞  </para>␞
␝  <para>␟   Some indexes might not index all tuples.  If the tuple is not to be
   indexed, <function>aminsert</function> should just return without doing anything.␟一部のインデックスではすべてのタプルをインデックス付けしない可能性があります。
タプルがインデックス付けされない場合、<function>aminsert</function>は何も行わずに戻らなければなりません。␞␞  </para>␞
␝  <para>␟   If the index AM wishes to cache data across successive index insertions
   ...
   which will be called before the memory is released.␟SQL文の中で、インデックスAMがインデックスへの連続的な挿入をまたがってデータをキャッシュすることが望ましい場合は、<literal>indexInfo-&gt;ii_Context</literal>にメモリを確保し、そのデータへのポインタを<literal>indexInfo-&gt;ii_AmCache</literal>（初期値はNULLです）に格納することができます。
インデックスへの挿入の後に、メモリ以外のリソースを解放する必要がある場合は、メモリが解放される前に呼び出される<function>aminsertcleanup</function>が提供されます。␞␞  </para>␞
␝</programlisting>␟   Clean up state that was maintained across successive inserts in
   ...
   simply releasing the memory is not sufficient.␟<literal>indexInfo-&gt;ii_AmCache</literal>への連続する挿入で維持されていた状態をクリーンアップします。
これは、データに追加のクリーンアップステップ（たとえばピンの付いたバッファを解放するなど）が必要で、メモリを解放するだけでは不十分な場合に便利です。␞␞  </para>␞
␝</programlisting>␟   Delete tuple(s) from the index.  This is a <quote>bulk delete</quote> operation
   ...
   <function>amvacuumcleanup</function>.␟インデックスからタプル（複数可）を削除します。
...
<function>amvacuumcleanup</function>に渡さなければならない情報がなければ、NULLを返しても問題ありません。␞␞  </para>␞
␝  <para>␟   Because of limited <varname>maintenance_work_mem</varname>,
   ...
   null.␟<varname>maintenance_work_mem</varname>の制限により、多くのタプルが削除される時、<function>ambulkdelete</function>を複数回呼び出す必要があるかもしれません。
...
典型的に、渡された<literal>stats</literal>がNULLでない場合、<function>ambulkdelete</function>は同じ構造体を変更し、返します。␞␞  </para>␞
␝</programlisting>␟   Clean up after a <command>VACUUM</command> operation (zero or more
   ...
   be returned.␟<command>VACUUM</command>操作（0回以上の<function>ambulkdelete</function>呼び出し）後の整理を行います。
...
しかし、そうでなければ正しい統計情報を返さなければなりません。␞␞  </para>␞
␝  <para>␟   <function>amvacuumcleanup</function> will also be called at completion of an
   ...
   call, and that only in an autovacuum worker process.␟<function>amvacuumcleanup</function>も<command>ANALYZE</command>操作の完了時点にも呼び出されます。
この場合、<literal>stats</literal>は常にNULLで、戻り値はまったく無視されます。
この事象は<literal>info-&gt;analyze_only</literal>を検査することで識別されます。
アクセスメソッドがそのような呼び出しで挿入後の整理以外何もしないように、そしてそれは自動バキュームワーカープロセスのみであるようにすることを推奨します。␞␞  </para>␞
␝</programlisting>␟   Check whether the index can support <link
   ...
   struct can be set to NULL.␟列のインデックスされた元の値を返すことにより、そのインデックスが指定された列で<link linkend="indexes-index-only-scans"><firstterm>インデックスオンリースキャン</firstterm></link>をサポート可能かどうかを判断します。
...
アクセスメソッドがインデックスオンリースキャンをサポートしていない場合、<structname>IndexAmRoutine</structname>構造体の<structfield>amcanreturn</structfield>フィールドをNULLにセットすることができます。␞␞  </para>␞
␝</programlisting>␟   Estimate the costs of an index scan.  This function is described fully
   in <xref linkend="index-cost-estimation"/>, below.␟インデックススキャンのコストを推定します。
この関数については後述の<xref linkend="index-cost-estimation"/>で説明します。␞␞  </para>␞
␝</programlisting>␟   Parse and validate the reloptions array for an index.  This is called only
   ...
   It is OK to return NULL if default behavior is wanted.␟インデックス用のreloptionsの解析と検証を行います。
...
デフォルトの動作を行わせたい場合はNULLを返しても問題ありません。␞␞  </para>␞
␝</programlisting>␟   The <function>amproperty</function> method allows index access methods to override
   ...
   property test result.␟<function>amproperty</function>メソッドにより、インデックスメソッドは<function>pg_index_column_has_property</function>および関連する関数のデフォルトの動作を上書きすることができます。
...
<structfield>amproperty</structfield>メソッドが<literal>false</literal>を返すなら、コアコードは属性検査の結果を決定するために、通常の手続きを進めます。␞␞  </para>␞
␝  <para>␟   Access methods that support ordering operators should
   ...
   properties.␟順序付け演算子をサポートするアクセスメソッドは、<literal>AMPROP_DISTANCE_ORDERABLE</literal>の属性検査を実装する必要があります。
...
その他のすべての標準属性に対しては、デフォルトの動作が満足できるもののはずです。␞␞  </para>␞
␝</programlisting>␟   Return the textual name of the given build phase number.
   ...
   <structname>pg_stat_progress_create_index</structname> view.␟指定されたビルドフェーズ番号のテキスト名を返します。
フェーズ番号は、<function>pgstat_progress_update_param</function>インタフェースを介してインデックス構築中に報告されたものです。
それから、フェーズ名は<structname>pg_stat_progress_create_index</structname>ビューで公開されます。␞␞  </para>␞
␝</programlisting>␟   Validate the catalog entries for the specified operator class, so far as
   ...
   messages, typically at <literal>INFO</literal> level.␟指定の演算子クラスについて、アクセスメソッドが合理的に可能な範囲でカタログエントリを検証します。
例えば、これには必要なすべてのサポート関数が提供されていることのテストが含まれるかもしれません。
<function>amvalidate</function>関数は演算子クラスが無効なときは偽を返さなければなりません。
問題があれば典型的には<literal>INFO</literal>レベルで<function>ereport</function>メッセージにより報告されます。␞␞  </para>␞
␝</programlisting>␟   Validate proposed new operator and function members of an operator family,
   ...
   also given soft dependencies, so that they can be removed if necessary.␟アクセスメソッドが合理的に可能な範囲で提案された新しい演算子族の演算子と関数メンバを検証し、デフォルトが不十分なら依存型を設定します。
...
なぜなら、<function>amadjustmembers</function>はメンバの集合のすべてを観察しているとは仮定することができないからです。
...
たとえば、GIN、GiST、SP-GiSTのようなインデックス形式では演算子とopclassの関連性が相対的低く、演算子メンバの追加削除を自由に行うことが合理的であるため、これらの演算子メンバにおいてはopfamilyに常にソフト依存性が設定されます。
また、必要ならば削除可能にするために、追加のサポート関数にソフト依存性を設定するのが一般的です。␞␞  </para>␞
␝  <para>␟   The purpose of an index, of course, is to support scans for tuples matching
   ...
   index access method must or may provide are:␟当然ながらインデックスの目的は、よく<firstterm>修飾子</firstterm>や<firstterm>スキャンキー</firstterm>と呼ばれる、インデックス可能な<literal>WHERE</literal>条件を満たすタプルのスキャンをサポートすることです。
...
インデックスアクセスメソッドが提供しなければならない、もしくは提供する可能性のあるスキャン関連の関数を以下に示します。␞␞  </para>␞
␝</programlisting>␟   Prepare for an index scan.  The <literal>nkeys</literal> and <literal>norderbys</literal>
   ...
   acquiring locks;
   the interesting parts of index-scan startup are in <function>amrescan</function>.␟インデックススキャンを準備します。
...
インデックススキャンを始める際の興味深い部分は、<function>amrescan</function>にあります。␞␞  </para>␞
␝</programlisting>␟   Start or restart an index scan, possibly with new scan keys.  (To restart
   ...
   remains the same.␟インデックススキャンを起動または再起動します。
スキャンキーを新しくすることもできます。
...
実際には、ネステッドループ結合によって新しい外部タプルが選択され、同じスキャンキー構造体で新しいキー比較値が必要とされた場合に、この再起動機能は使用されます。␞␞  </para>␞
␝</programlisting>␟   Fetch the next tuple in the given scan, moving in the given
   ...
   callers.␟指定されたスキャン内から指定された方向（インデックス内の前方または後方）で次のタプルを取り出します。
タプルを取り出した場合は真を返します。
...
部分インデックス述語（もしあれば）は<function>amgettuple</function>呼び出し元で決して再検査されません。␞␞  </para>␞
␝  <para>␟   If the index supports <link linkend="indexes-index-only-scans">index-only
   ...
   call for the scan.␟そのインデックスが<link linkend="indexes-index-only-scans">インデックスオンリースキャン</link>をサポートしている場合（つまり<function>amcanreturn</function>がいずれかの列に対して真を返す場合）、そのアクセスメソッドはスキャンが成功したならば<literal>scan-&gt;xs_want_itup</literal>も確認し、それが真の場合、そのインデックスエントリに対応する元のインデックスされたデータを返さなければなりません。
...
データは少なくとも<function>amgettuple</function>、<function>amrescan</function>または<function>amendscan</function>によってスキャンされるまでよい状態を保たなくてはなりません。␞␞  </para>␞
␝  <para>␟   The <function>amgettuple</function> function need only be provided if the access
   ...
   struct must be set to NULL.␟<function>amgettuple</function>関数は、アクセスメソッドが<quote>単純</quote>インデックススキャンをサポートするときのみ提供される必要があります。
そうでなければ、<structname>IndexAmRoutine</structname>構造体の<structfield>amgettuple</structfield>フィールドはNULLに設定されなければなりません。␞␞  </para>␞
␝</programlisting>␟   Fetch all tuples in the given scan and add them to the caller-supplied
   ...
   in <xref linkend="index-scanning"/>.␟指定されたスキャンから全てのタプルを取り出し、呼び出し側が提供する<type>TIDBitmap</type>にそれらを付加します
...
<xref linkend="index-scanning"/>で説明した通り、<function>amgetbitmap</function>を使用する場合には他にも制限があります。␞␞  </para>␞
␝  <para>␟   The <function>amgetbitmap</function> function need only be provided if the access
   ...
   struct must be set to NULL.␟<function>amgetbitmap</function>関数はアクセスメソッドが<quote>ビットマップ</quote>インデックススキャンをサポートしている場合のみ必要です。
そうでなければ、<structname>IndexAmRoutine</structname>構造体の中の<structfield>amgetbitmap</structfield>フィールドはNULLに設定されなければなりません。␞␞  </para>␞
␝</programlisting>␟   End a scan and release resources.  The <literal>scan</literal> struct itself
   should not be freed, but any locks or pins taken internally by the
   access method must be released, as well as any other memory allocated
   by <function>ambeginscan</function> and other scan-related functions.␟スキャンを停止し、リソースを解放します。
<literal>scan</literal>構造体自体は解放すべきではありません。
アクセスメソッドで内部的に取られたロックやピンは、<function>ambeginscan</function>や他のスキャン関連の関数により確保されたメモリと同様に解放しなければなりません。␞␞  </para>␞
␝</programlisting>␟   Mark current scan position.  The access method need only support one
   remembered scan position per scan.␟現在のスキャン位置を記録します。
アクセスメソッドは1スキャン当たり1つの記録済みスキャンのみをサポートしなければなりません。␞␞  </para>␞
␝  <para>␟   The <function>ammarkpos</function> function need only be provided if the access
   ...
   struct may be set to NULL.␟<function>ammarkpos</function>関数はアクセスメソッドが順序付けされたスキャンをサポートする場合にのみ提供する必要があります。
そうでなければ、その<structname>IndexAmRoutine</structname>構造体の<structfield>ammarkpos</structfield>フィールドはNULLに設定しても構いません。␞␞  </para>␞
␝</programlisting>␟   Restore the scan to the most recently marked position.␟もっとも最近に記録された位置にスキャンを戻します。␞␞  </para>␞
␝  <para>␟   The <function>amrestrpos</function> function need only be provided if the access
   method supports ordered scans.  If it doesn't,
   the <structfield>amrestrpos</structfield> field in its <structname>IndexAmRoutine</structname>
   struct may be set to NULL.␟<function>amrestrpos</function>関数はアクセスメソッドが順序付けされたスキャンをサポートする場合にのみ提供する必要があります。
そうでなければ、その<structname>IndexAmRoutine</structname>構造体の<structfield>amrestrpos</structfield>フィールドはNULLに設定しても構いません。␞␞  </para>␞
␝  <para>␟   In addition to supporting ordinary index scans, some types of index
   ...
   functions may be implemented to support parallel index scans:␟通常のインデックススキャンのサポートに加え、ある種のインデックスは、複数のバックエンドが協調してインデックススキャンを実行する<firstterm>パラレルインデックススキャン</firstterm>をサポートすることができます。
...
パラレルインデックススキャンをサポートするために、以下の関数を実装することができます。␞␞  </para>␞
␝</programlisting>␟   Estimate and return the number of bytes of dynamic shared memory which
   ...
   AM-independent data in <structname>ParallelIndexScanDescData</structname>.)␟パラレルスキャンを実行するために、アクセスメソッドによって必要とされる動的共有メモリのバイト数を推測し、返します。
（この数値は、<structname>ParallelIndexScanDescData</structname>のAM独立データに必要となる量に追加するための値であり、それを置き換えるものではありません。）␞␞  </para>␞
␝  <para>␟   The <literal>nkeys</literal> and <literal>norderbys</literal>
   parameters indicate the number of quals and ordering operators that will be
   used in the scan; the same values will be passed to <function>amrescan</function>.
   Note that the actual values of the scan keys aren't provided yet.␟<literal>nkeys</literal>および<literal>norderbys</literal>パラメータは、スキャンで使用される等価性演算子と順序付け演算子の個数を表し、これらの値は<function>amrescan</function>にも渡されます。
スキャンキーの実際の値はまだ提供されていないことに注意してください。␞␞  </para>␞
␝  <para>␟   It is not necessary to implement this function for access methods which
   do not support parallel scans or for which the number of additional bytes
   of storage required is zero.␟パラレルスキャンをサポートしない、あるいはメモリ領域への追加のバイト数が0のアクセスメソッドでは、この関数を実装する必要はありません。␞␞  </para>␞
␝</programlisting>␟   This function will be called to initialize dynamic shared memory at the
   ...
   amount of space to store whatever data it wishes.␟この関数は、パラレルスキャンの最初に動的共有メモリを初期化するために呼ばれます。
<parameter>target</parameter>は、前もって<function>amestimateparallelscan</function>が返したバイト数を少なくとも持つ領域を指し、この関数はその分だけのスペースを使って必要なデータを保管することができます。␞␞  </para>␞
␝  <para>␟   It is not necessary to implement this function for access methods which
   do not support parallel scans or in cases where the shared memory space
   required needs no initialization.␟パラレルスキャンをサポートしない、あるいは共有メモリスペースの初期化が必要ないアクセスメソッドでは、この関数を実装する必要はありません。␞␞  </para>␞
␝</programlisting>␟   This function, if implemented, will be called when a parallel index scan
   ...
   the beginning.␟実装された場合、この関数はパラレルインデックススキャンを再起動しなければならない時に呼ばれます。
この関数は、<function>aminitparallelscan</function>が設定した共有状態を初期化し、スキャンが最初から再開できるようにします。␞␞  </para>␞
␝ <sect1 id="index-scanning">␟  <title>Index Scanning</title>␟  <title>インデックススキャン</title>␞␞␞
␝  <para>␟   In an index scan, the index access method is responsible for regurgitating
   ...
   conditions.␟インデックススキャンでは、<firstterm>スキャンキー</firstterm>に一致するものと示したすべてのタプルのTIDを繰り返すことに関する責任をインデックスアクセスメソッドが持ちます。
アクセスメソッドには、実際のインデックスの親テーブルからのタプルの取り出しやタプルがスキャンの可視性テストや他の条件を通過したかどうかの決定は含まれ<emphasis>ません</emphasis>。␞␞  </para>␞
␝  <para>␟   A scan key is the internal representation of a <literal>WHERE</literal> clause of
   ...
   to satisfy all the indicated conditions.␟スキャンキーは、<replaceable>index_key</replaceable> <replaceable>operator</replaceable> <replaceable>constant</replaceable>という形式の<literal>WHERE</literal>句の内部的表現です。
...
返されるタプルは指定された条件を満たすものと想定されます。␞␞  </para>␞
␝  <para>␟   The access method can report that the index is <firstterm>lossy</firstterm>, or
   ...
   specified, the index scan must return exactly the set of matching entries.␟アクセスメソッドはインデックスがある特定の問い合わせに対し<firstterm>非可逆</firstterm>、または再検査を要求するかどうかを報告することができます。
...
再検査オプションが指定されない場合、インデックススキャンは一致する項目の集合を返さなければなりません。␞␞  </para>␞
␝  <para>␟   Note that it is entirely up to the access method to ensure that it
   ...
   the scan keys to a <quote>normalized</quote> form.␟確実に、指定されたスキャンキーすべてに一致するもののみをすべて正しく見つけ出すことは、完全にアクセスメソッドの責任であることに注意してください。
...
<function>amrescan</function>における前処理の必要性は、インデックスアクセスメソッドがスキャンキーを<quote>正規化</quote>形式にする必要があるかどうかに依存します。␞␞  </para>␞
␝  <para>␟   Some access methods return index entries in a well-defined order, others
   do not.  There are actually two different ways that an access method can
   support sorted output:␟一部のアクセスメソッドは、他では行いませんが、十分に定義された順序でインデックス項目を返します。
アクセスメソッドが出力の順序付けをサポートできるようにする方法は、実質２種類存在します。␞␞␞
␝      <para>␟       Access methods that always return entries in the natural ordering
       ...
       numbers for their equality and ordering operators.␟常にそのデータ（btreeなど）の自然な順序で項目を返すアクセスメソッドは<structfield>amcanorder</structfield>を真に設定しなければなりません。
現在、こうしたアクセスメソッドは、その等価性と順序付け演算子でbtree互換の戦略番号を使用しなければなりません。␞␞      </para>␞
␝      <para>␟       Access methods that support ordering operators should set
       ...
       previously.␟順序付け演算子をサポートするアクセスメソッドは<structfield>amcanorderbyop</structfield>を真に設定しなければなりません。
...
前述の通り、この形式のスキャン修飾子を<function>amrescan</function>に渡すことができます。␞␞      </para>␞
␝  <para>␟   The <function>amgettuple</function> function has a <literal>direction</literal> argument,
   ...
   calls will have the same direction as the first one.)␟<function>amgettuple</function>関数は<literal>direction</literal>引数を持ちます。
...
（しかし<structfield>amcanbackward</structfield>が偽であれば、引き続くすべての呼び出しは最初のものと同じ方向を持ちます。）␞␞  </para>␞
␝  <para>␟   Access methods that support ordered scans must support <quote>marking</quote> a
   ...
   instead.␟順序付けされたスキャンを提供するアクセスメソッドはスキャン内位置の<quote>記録</quote>をサポートしなければならず、また、後でその記録された位置に戻ることをサポートしなければなりません。
同じ位置が複数回記録されるかもしれません。
しかし、スキャン内の1つの位置のみを記録する必要があります。
新しい<function>ammarkpos</function>呼び出しにより前回記録された位置は上書きされます。
順序付けされたスキャンをサポートしないアクセスメソッドは<structname>IndexAmRoutine</structname>で<function>ammarkpos</function>関数および<function>amrestrpos</function>関数を提供する必要はないので、これらのポインタをNULLにセットしてください。␞␞  </para>␞
␝  <para>␟   Both the scan position and the mark position (if any) must be maintained
   ...
   deleted.␟スキャン位置と記録された位置（もしあれば）の両方は、インデックス内の同時挿入や削除という観点における一貫性を保持しなければなりません。
...
重要なことは、挿入や削除によって、その項目自体が挿入・削除されていない項目がスキャンで失われたり二重になったりすることが起こらないという点です。␞␞  </para>␞
␝  <para>␟   If the index stores the original indexed data values (and not some lossy
   ...
   MVCC visibility.  But that is no concern of the access method's.␟インデックスが設定された列値がインデックスに格納されている(かつ、損失のある表現ではない)場合、ヒープタプルのTIDではなくインデックスに格納された実際のデータを返す<link linkend="indexes-index-only-scans">インデックスオンリースキャン</link>をサポートするのに有用です。
これは、可視性マップによってTIDが全可視のページ上にあると判断できる場合にI/Oを避けるだけのことです。
判断できない場合はMVCCを確認するためにヒープタプルにアクセスしなくてはなりません。
しかしその動作はアクセスメソッドでは考慮されていません。␞␞  </para>␞
␝  <para>␟   Instead of using <function>amgettuple</function>, an index scan can be done with
   ...
   spelled out in <xref linkend="index-locking"/>.␟<function>amgettuple</function>を使用する代わりに、<function>amgetbitmap</function>を使用して、一回の呼出しですべてのタプルを取り出してインデックススキャンを行うことができます。
...
最後に、<function>amgetbitmap</function>は返されたタプルに関し、<xref linkend="index-locking"/>に記載した意味でのロックを保証しません。␞␞  </para>␞
␝  <para>␟   Note that it is permitted for an access method to implement only
   <function>amgetbitmap</function> and not <function>amgettuple</function>, or vice versa,
   if its internal implementation is unsuited to one API or the other.␟アクセスメソッドの内部実装がどちらか片方のAPIにそぐわない場合、<function>amgettuple</function>を実装せず<function>amgetbitmap</function>のみを実装、またはその逆も許されていることに注意してください。␞␞  </para>␞
␝ <sect1 id="index-locking">␟  <title>Index Locking Considerations</title>␟  <title>インデックスのロック処理に関する検討</title>␞␞␞
␝  <para>␟   Index access methods must handle concurrent updates
   ...
   <literal>CONCURRENTLY</literal>).␟インデックスアクセスメソッドは、複数のプロセスによるインデックスの同時更新を取り扱えなければなりません。
...
インデックスの生成、破棄、<command>REINDEX</command>時にインデックス全体に対する<literal>ACCESS EXCLUSIVE</literal>ロックが獲得されます(<literal>CONCURRENTLY</literal>では代わりに<literal>SHARE UPDATE EXCLUSIVE</literal>が取得されます)。␞␞  </para>␞
␝  <para>␟   Building an index type that supports concurrent updates usually requires
   ...
   <filename>src/backend/access/hash/README</filename>.␟同時更新をサポートするインデックス種類を構築することは通常、必要な動作について広範かつ微細にわたる解析が必要です。
B-treeおよびハッシュインデックス種類では、<filename>src/backend/access/nbtree/README</filename>と <filename>src/backend/access/hash/README</filename>にある設計に関する決定事項を読むことができます。␞␞  </para>␞
␝  <para>␟   Aside from the index's own internal consistency requirements, concurrent
   ...
   with the following rules:␟インデックス自身の内部的な一貫性要求の他に、同時実行更新には、親テーブル（<firstterm>ヒープ</firstterm>）とインデックス間の一貫性に関する問題が発生します。
<productname>PostgreSQL</productname>はヒープへのアクセスおよび更新とインデックスへのアクセスおよび更新を分離していますので、インデックスとヒープとの間の一貫性が無くなる間隔が存在します。
以下の規則でこうした問題を扱います。␞␞␞
␝      <para>␟       A new heap entry is made before making its index entries.  (Therefore
       ...
       uncommitted row anyway.  But see <xref linkend="index-unique-checks"/>.)␟新しいヒープ項目はインデックス項目を作成する前に作成されます。
（このため、同時実行インデックススキャンはヒープエントリを確認する時によく失敗します。
インデックスの読み取りは、未コミットの行を対象としませんので問題ありません。
しかし、<xref linkend="index-unique-checks"/>を参照してください。）␞␞      </para>␞
␝      <para>␟       When a heap entry is to be deleted (by <command>VACUUM</command>), all its
       index entries must be removed first.␟ヒープエントリが（<command>VACUUM</command>によって）削除される時、これに対するすべてのインデックス項目が先に削除されます。␞␞      </para>␞
␝      <para>␟       An index scan must maintain a pin
       on the index page holding the item last returned by
       <function>amgettuple</function>, and <function>ambulkdelete</function> cannot delete
       entries from pages that are pinned by other backends.  The need
       for this rule is explained below.␟インデックススキャンは、最後に<function>amgettuple</function>が返した項目を保持するインデックスページ上のピンを管理しなければなりません。
また、<function>ambulkdelete</function>は、他のバックエンドがピンを持つページから項目を削除することはできません。
この規則の必要性については後で説明します。␞␞      </para>␞
␝␟   Without the third rule, it is possible for an index reader to
   ...
   overhead only in the rare cases where there actually is a conflict.␟３番目の規則がないと、<command>VACUUM</command>によって削除される直前に、インデックス読み取りがインデックス項目を見つけ、そして、<command>VACUUM</command>によって削除された後に対応するヒープ項目に達する可能性があります。
...
しかし、MVCCに則らないスナップショット(<literal>SnapshotAny</literal>など)では、実際にはスキャンキーに合わない行を受付け、返す可能性があります。
...
実行時におけるこの対策のコストは小さく、実際に競合が発生するごく稀な場合にのみブロックするためのオーバーヘッドが加わります。␞␞  </para>␞
␝  <para>␟   This solution requires that index scans be <quote>synchronous</quote>: we have
   ...
   for a query using an MVCC snapshot.␟この対策は、インデックススキャンが<quote>同期</quote>していることを要求します。
...
上の解析に従うと、MVCCに則らないスナップショットでは同期方式を使用しなければなりませんが、問い合わせがMVCCスナップショットを使用する場合は非同期スキャンを使用することができます。␞␞  </para>␞
␝  <para>␟   In an <function>amgetbitmap</function> index scan, the access method does not
   keep an index pin on any of the returned tuples.  Therefore
   it is only safe to use such scans with MVCC-compliant snapshots.␟<function>amgetbitmap</function>インデックススキャンでは、アクセスメソッドは返されるタプル上にインデックスピンをまったく保持しません。
したがって、MVCCに則ったスナップショットでこうしたスキャンを使用することのみが安全です。␞␞  </para>␞
␝  <para>␟   When the <structfield>ampredlocks</structfield> flag is not set, any scan using that
   ...
   reduce the frequency of such transaction cancellations.␟<structfield>ampredlocks</structfield>フラグが設定されていない場合、シリアライザブルトランザクション内でそのインデックスアクセスメソッドを使用するスキャンはいずれもインデックス全体に対するブロックしない述語ロックを獲得します。
...
このフラグが設定されている場合、こうしたトランザクションのキャンセルの頻度を低減することになる、より粒度の細かな述語ロックをインデックスアクセスメソッドが実装していることを示します。␞␞  </para>␞
␝ <sect1 id="index-unique-checks">␟  <title>Index Uniqueness Checks</title>␟  <title>インデックス一意性検査</title>␞␞␞
␝  <para>␟   <productname>PostgreSQL</productname> enforces SQL uniqueness constraints
   ...
   uniqueness.␟<productname>PostgreSQL</productname>は、SQLの一意性制約を<firstterm>一意性インデックス</firstterm>を使用して強制します。
このインデックスでは、同一キーに対し複数の項目を許しません。
この機能をサポートするアクセスメソッドは<structfield>amcanunique</structfield>を真に設定します。
（現時点ではb-treeのみがこれをサポートします。）
<literal>INCLUDE</literal>句内の列のリストは、一意性制約の強制時には考慮されません。␞␞  </para>␞
␝  <para>␟   Because of MVCC, it is always necessary to allow duplicate entries to
   ...
   checked when inserting a new row into a unique index:␟MVCCのため、インデックス内に物理的に重複した項目が存在できることが常に必要です。
...
␝      <para>␟       If a conflicting valid row has been deleted by the current transaction,
       ...
       a row without changing the key.)␟競合する有効な行が現在のトランザクションで削除された場合は問題ありません。
（具体的には、UPDATEは常に新しいバージョンを挿入する前に古い行バージョンを削除します。
これによりキーを変更することなく行をUPDATEすることができます。）␞␞      </para>␞
␝      <para>␟       If a conflicting row has been inserted by an as-yet-uncommitted
       ...
       end and then redo the visibility check in toto.)␟競合する行が未コミットのトランザクションで挿入された場合、挿入しようとしている方はトランザクションのコミットが分かるまで待機しなければなりません。
ロールバックした場合は競合しません。
競合する行が削除されずにコミットした場合、一意性違反となります。
（具体的には、他のトランザクションの終了をただ待機し、終了後に可視性の検査を完全に再実行します。）␞␞      </para>␞
␝      <para>␟       Similarly, if a conflicting valid row has been deleted by an
       as-yet-uncommitted transaction, the would-be inserter must wait
       for that transaction to commit or abort, and then repeat the test.␟同様に、競合する有効な行が未コミットのトランザクションで削除された場合、挿入しようとしている方はトランザクションのコミットまたはアボートを待機しなければならず、その後、試験を繰り返します。␞␞      </para>␞
␝  <para>␟   Furthermore, immediately before reporting a uniqueness violation
   ...
   <command>CREATE UNIQUE INDEX CONCURRENTLY</command>, however.)␟さらに、上記規則に従った一意性違反を報告する直前に、アクセスメソッドは挿入される行の有効性を再度検査しなければなりません。
もし、無効なコミットであれば、違反を報告してはいけません。
（現在のトランザクションによって作成された通常の行の挿入という状況では、これは発生することはありません。
しかし、これは<command>CREATE UNIQUE INDEX CONCURRENTLY</command>中に発生することがあります。）␞␞  </para>␞
␝  <para>␟   We require the index access method to apply these tests itself, which
   ...
   of insertion of the new index entry.␟インデックスアクセスメソッドにこうした試験を自身で行うことを要求します。
...
さらに、競合検査がインデックス行の挿入部分で統合されて行われない限り、競合状態を防ぐ明確な方法がありません。␞␞  </para>␞
␝  <para>␟   If the unique constraint is deferrable, there is additional complexity:
   ...
   <literal>checkUnique</literal> parameter having one of the following values:␟一意性制約が遅延可能である場合はさらに複雑になります。
...
␝      <para>␟       <literal>UNIQUE_CHECK_NO</literal> indicates that no uniqueness checking
       should be done (this is not a unique index).␟<literal>UNIQUE_CHECK_NO</literal>は、一意性検査を行うことはない(これは一意性インデックスではない)ことを示します。␞␞      </para>␞
␝      <para>␟       <literal>UNIQUE_CHECK_YES</literal> indicates that this is a non-deferrable
       unique index, and the uniqueness check must be done immediately, as
       described above.␟<literal>UNIQUE_CHECK_YES</literal>は、上述の通り遅延がない一意性インデックスであり、一意性検査を即時に行わなければならないことを示します。␞␞      </para>␞
␝      <para>␟       <literal>UNIQUE_CHECK_PARTIAL</literal> indicates that the unique
       ...
       be scheduled.␟<literal>UNIQUE_CHECK_PARTIAL</literal>は一意性制約が遅延可能であることを示します。
...
偽が返された行それぞれに対して、遅延再検査が予定されます。␞␞      </para>␞
␝      <para>␟       The access method must identify any rows which might violate the
       ...
       be conflicts.␟アクセスメソッドは一意性制約違反となるかもしれない行を識別しなければなりません。
しかし間違った偽を報告することはエラーではありません。
これにより他のトランザクションを待つことなく検査を行うことができます。
ここで報告された重複はエラーとして扱われず、後で再検査されます。
再検査時には重複しなくなっている可能性があります。␞␞      </para>␞
␝      <para>␟       <literal>UNIQUE_CHECK_EXISTING</literal> indicates that this is a deferred
       ...
       if the target row is also still live, report error.␟<literal>UNIQUE_CHECK_EXISTING</literal>は、一意性違反の可能性があると報告された行に対する遅延再検査であることを示します。
...
もし存在し、対象の行もまだ存在する場合エラーを報告します。␞␞      </para>␞
␝      <para>␟       It is recommended that in a <literal>UNIQUE_CHECK_EXISTING</literal> call,
       ...
       for the same tuple values as were used in the original insertion.␟<literal>UNIQUE_CHECK_EXISTING</literal>呼び出しでは、アクセスメソッドはさらに対象行が実際にインデックス内に既存の項目を持つか検証し、もしなければエラーを報告することを推奨します。
<function>aminsert</function>に渡されるインデックスタプル値が再計算されているため勧めます。
インデックス定義に実際には不変ではない関数が含まれる場合、インデックスの間違った領域を検査してしまうかもしれません。
再検査にて対象行の存在を検査することで、元の挿入で使用されたものと同じタプル値をスキャンしていることを検証します。␞␞      </para>␞
␝ <sect1 id="index-cost-estimation">␟  <title>Index Cost Estimation Functions</title>␟  <title>インデックスコスト推定関数</title>␞␞␞
␝  <para>␟   The <function>amcostestimate</function> function is given information describing
   ...
   in case it is possible to improve on the standard estimates.␟<function>amcostestimate</function>関数には、インデックスと共に使用できることが決まっているWHERE句およびORDER BY句のリストを含む、インデックススキャンの可能性を記述する情報が与えられます。
...
<function>amcostestimate</function>関数を持つことの意味は、標準の概算を改善することができる場合に、インデックスアクセスメソッドがインデックス型固有の知識体系を提供することができるということです。␞␞  </para>␞
␝  <para>␟   Each <function>amcostestimate</function> function must have the signature:␟それぞれの<function>amcostestimate</function>関数は以下のシグネチャを持たなければいけません。␞␞␞
␝␟   The first three parameters are inputs:␟最初の3つのパラメータは入力です。␞␞␞
␝      <para>␟       The planner's information about the query being processed.␟処理されている問い合わせに関するプランナの情報。␞␞      </para>␞
␝      <para>␟       The index access path being considered.  All fields except cost and
       selectivity values are valid.␟考慮されるインデックスアクセスパス。
コストと選択性値を除くすべてのフィールドが有効です。␞␞      </para>␞
␝      <para>␟       The number of repetitions of the index scan that should be factored
       ...
       to allow for some caching effects across multiple scans.␟コスト概算の算出対象となるインデックススキャンが繰り返された回数です。
これは通常、ネステッドループ結合の内部で利用されるパラメータ化されたスキャンの回数よりも大きい値になります。
コスト概算は1回のスキャンのための値であることに注意してください。<parameter>loop_count</parameter>がより大きい場合、複数のスキャンにより得られる効果をみるには十分な値といえるでしょう。␞␞      </para>␞
␝  <para>␟   The last five parameters are pass-by-reference outputs:␟最後の5つのパラメータは参照渡しの出力です。␞␞␞
␝      <para>␟       Set to cost of index start-up processing␟インデックスの起動処理にかかるコストに設定されます。␞␞      </para>␞
␝      <para>␟       Set to total cost of index processing␟インデックス処理の全体のコストに設定されます。␞␞      </para>␞
␝      <para>␟       Set to index selectivity␟インデックスの選択度に設定されます。␞␞      </para>␞
␝      <para>␟       Set to correlation coefficient between index scan order and
       underlying table's order␟インデックススキャンの順番と背後のテーブルの順番間の相関係数に設定されます。␞␞      </para>␞
␝      <para>␟       Set to number of index leaf pages␟インデックスのリーフページ数が設定されます。␞␞      </para>␞
␝  <para>␟   Note that cost estimate functions must be written in C, not in SQL or
   any available procedural language, because they must access internal
   data structures of the planner/optimizer.␟コスト概算関数は、SQLやその他の手続き言語ではなく、C言語で書かれなければいけないことに注意してください。
理由はプランナ/オプティマイザの内部データ構造にアクセスしなければいけないためです。␞␞  </para>␞
␝  <para>␟   The index access costs should be computed using the parameters used by
   ...
   (especially evaluation of the indexquals themselves).␟インデックスアクセスコストは<filename>src/backend/optimizer/path/costsize.c</filename>で使われる、逐次的なディスクブロックの取り出しには<varname>seq_page_cost</varname>のコストが、順不同の取り出しには<varname>random_page_cost</varname>のコストが、そして、1つのインデックス行の処理には通常<varname>cpu_index_tuple_cost</varname>というコストがかかる、というパラメータで計算されなければなりません。
さらに、インデックス処理（特にindexquals自体の評価）の間に呼び出される比較演算すべてに対して、<varname>cpu_operator_cost</varname>に適当な係数をかけたコストがかかります。␞␞  </para>␞
␝  <para>␟   The access costs should include all disk and CPU costs associated with
   scanning the index itself, but <emphasis>not</emphasis> the costs of retrieving or
   processing the parent-table rows that are identified by the index.␟アクセスコストは、インデックス自身のスキャンと関係するすべてのディスクとCPUコストも含むべきですが、インデックスで識別される親テーブルの行の処理や抽出にかかるコストは含めては<emphasis>いけません</emphasis>。␞␞  </para>␞
␝  <para>␟   The <quote>start-up cost</quote> is the part of the total scan cost that
   ...
   cost might want to set it nonzero.␟<quote>起動用コスト</quote>は、最初の行を取り出し始めることができるようになる前に費やされなければならない総スキャンコストの一部です。
ほとんどのインデックスでは、これはゼロとすることができます。
しかし、高い起動用コストを持つインデックス種類ではこれを非ゼロにすることを勧めます。␞␞  </para>␞
␝  <para>␟   The <parameter>indexSelectivity</parameter> should be set to the estimated fraction of the parent
   table rows that will be retrieved during the index scan.  In the case
   of a lossy query, this will typically be higher than the fraction of
   rows that actually pass the given qual conditions.␟<parameter>indexSelectivity</parameter>は、インデックススキャンの間に抽出される親テーブルの行の概算された割合として設定されるべきです。
非可逆問い合わせの場合はこの値が、与えられた制約条件を実際に通過する行の割合よりも高くなることがよくあります。␞␞  </para>␞
␝  <para>␟   The <parameter>indexCorrelation</parameter> should be set to the correlation (ranging between
   -1.0 and 1.0) between the index order and the table order.  This is used
   to adjust the estimate for the cost of fetching rows from the parent
   table.␟<parameter>indexCorrelation</parameter>は、インデックスの順番とテーブルの順番の間の（-1.0から1.0までの間の値を取る）相関として設定されるべきです。
この値は、メインテーブルから行を取り出すためのコスト概算を調整するために使用されます。␞␞  </para>␞
␝  <para>␟   The <parameter>indexPages</parameter> should be set to the number of leaf pages.
   This is used to estimate the number of workers for parallel index scan.␟<parameter>indexPages</parameter>は、リーフページ数が設定されるべきです。
これは、パラレルインデックススキャンのワーカー数の見積もりに使用されます。␞␞  </para>␞
␝  <para>␟   When <parameter>loop_count</parameter> is greater than one, the returned numbers
   should be averages expected for any one scan of the index.␟<parameter>loop_count</parameter>の値が1より大きい場合、戻り値はインデックスを利用した1回のスキャンを想定した平均値であるべきです。␞␞  </para>␞
␝  <procedure>␟   <title>Cost Estimation</title>␟   <title>コスト概算</title>␞␞   <para>␞
␝   <para>␟    A typical cost estimator will proceed as follows:␟典型的なコスト概算は次のように進められます。␞␞   </para>␞
␝    <para>␟     Estimate and return the fraction of parent-table rows that will be visited
     based on the given qual conditions.  In the absence of any index-type-specific
     knowledge, use the standard optimizer function <function>clauselist_selectivity()</function>:␟与えられた制約条件に基づいて訪れられるメインテーブルの行の割合を概算して返します。
インデックス型固有の知識体系を持たない場合、標準のオプティマイザの関数である<function>clauselist_selectivity()</function>を使用してください。␞␞␞
␝    <para>␟     Estimate the number of index rows that will be visited during the
     ...
     <literal>path-&gt;indexinfo</literal> struct.)␟スキャン中に訪れられるインデックスの行数を概算します。
多くのインデックス種類では、これは<parameter>indexSelectivity</parameter>とインデックスの中にある行数を掛けたものと等しいですが、それより多い場合もあります。
（ページおよび行内のインデックスのサイズは<literal>path-&gt;indexinfo</literal>構造体から得ることができることに注意してください。）␞␞    </para>␞
␝    <para>␟     Estimate the number of index pages that will be retrieved during the scan.
     This might be just <parameter>indexSelectivity</parameter> times the index's size in pages.␟スキャン中に抽出されるインデックスページ数を概算します。
これは単に<parameter>indexSelectivity</parameter>にページ内のインデックスのサイズを掛けたものになるでしょう。␞␞    </para>␞
␝    <para>␟     Compute the index access cost.  A generic estimator might do this:␟インデックスアクセスコストを計算します。
汎用的な概算においては以下のように行うでしょう。␞␞␞
␝/*␟ * Our generic assumption is that the index pages will be read
 * sequentially, so they cost seq_page_cost each, not random_page_cost.
 * Also, we charge for evaluation of the indexquals at each index row.
 * All the costs are assumed to be paid incrementally during the scan.␟ * 一般的な仮定は、インデックスページは逐次的に読まれるので、
...
␝␟     However, the above does not account for amortization of index reads
     across repeated index scans.␟しかし、上では繰り返されるインデックススキャンにかかるインデックス読み込みについて減価償却を考慮していません。␞␞    </para>␞
␝    <para>␟     Estimate the index correlation.  For a simple ordered index on a single
     field, this can be retrieved from pg_statistic.  If the correlation
     is not known, the conservative estimate is zero (no correlation).␟インデックスの相関を概算します。
1つのフィールドに対する単純な順番のインデックスでは、これはpg_statisticから入手することができます。
相関が未知の場合、概算を用心深く考えるとゼロ（無相関）となります。␞␞    </para>␞
␝  <para>␟   Examples of cost estimator functions can be found in
   <filename>src/backend/utils/adt/selfuncs.c</filename>.␟コスト概算関数の例は<filename>src/backend/utils/adt/selfuncs.c</filename>にあります。␞␞  </para>␞
