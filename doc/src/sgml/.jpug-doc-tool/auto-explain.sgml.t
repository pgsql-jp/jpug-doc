␝<sect1 id="auto-explain" xreflabel="auto_explain">␟ <title>auto_explain &mdash; log execution plans of slow queries</title>␟ <title>auto_explain &mdash; 低速な問い合わせ実行計画のログ</title>␞␞␞
␝ <para>␟  The <filename>auto_explain</filename> module provides a means for
  logging execution plans of slow statements automatically, without
  having to run <xref linkend="sql-explain"/>
  by hand.  This is especially helpful for tracking down un-optimized queries
  in large applications.␟<filename>auto_explain</filename>モジュールは、手動で<xref linkend="sql-explain"/>の実行を必要とせず、自動的に遅い文の実行計画をログ記録する手段を提供します。
大きなアプリケーションにおける最適化されていない問い合わせを追跡するのに特に有用です。␞␞ </para>␞
␝ <para>␟  The module provides no SQL-accessible functions.  To use it, simply
  load it into the server.  You can load it into an individual session:␟このモジュールはSQLでアクセスできる関数を提供しません。
...
␝␟  (You must be superuser to do that.)  More typical usage is to preload
  ...
  that.␟（実行するためにはスーパーユーザでなければなりません。）
...
当然ながらこのためのオーバーヘッドという代償があります。␞␞ </para>␞
␝ <sect2 id="auto-explain-configuration-parameters">␟  <title>Configuration Parameters</title>␟  <title>設定パラメータ</title>␞␞␞
␝     <indexterm>
...
␝ <para>␟  There are several configuration parameters that control the behavior of
  <filename>auto_explain</filename>.  Note that the default behavior is
  to do nothing, so you must set at least
  <varname>auto_explain.log_min_duration</varname> if you want any results.␟<filename>auto_explain</filename>の動作を制御するいくつかの設定パラメータが存在します。
デフォルトの動作は何もしないことなので、なんらかの結果を望むのであれば少なくとも<varname>auto_explain.log_min_duration</varname>を設定しなければならないことに注意してください。␞␞ </para>␞
␝     <indexterm>
      <primary><varname>auto_explain.log_parameter_max_length</varname> configuration parameter</primary>
     </indexterm>
␟␟     <indexterm>
      <primary><varname>auto_explain.log_parameter_max_length</varname>設定パラメータ</primary>
     </indexterm>␞␞     </para>␞
␝     <para>␟      <varname>auto_explain.log_min_duration</varname> is the minimum statement
      ...
      setting.␟<varname>auto_explain.log_min_duration</varname>は、文の実行計画がログに記録されるようになる、ミリ秒単位の最小の文実行時間です。
...
スーパーユーザのみがこの設定を変更することができます。␞␞     </para>␞
␝     <indexterm>
...
␝     <para>␟      <varname>auto_explain.log_parameter_max_length</varname> controls the
      ...
      setting.␟<varname>auto_explain.log_parameter_max_length</varname>は、問い合わせパラメータ値を記録するか制御します。
<literal>-1</literal>という値（デフォルト）は、パラメータ値を完全に記録します。
<literal>0</literal>はパラメータ値の記録を無効にします。
ゼロより大きい値は、各パラメータ値を指定されたバイト数に切り捨てます。
この設定を変更できるのは、スーパーユーザのみです。␞␞     </para>␞
␝     <para>␟      <varname>auto_explain.log_analyze</varname> causes <command>EXPLAIN ANALYZE</command>
      output, rather than just <command>EXPLAIN</command> output, to be printed
      when an execution plan is logged. This parameter is off by default.
      Only superusers can change this setting.␟<varname>auto_explain.log_analyze</varname>は、実行計画のログが取得されたときに出力されるものとして、単に<command>EXPLAIN</command>出力ではなく、<command>EXPLAIN ANALYZE</command>出力を行います。
このパラメータはデフォルトで無効です。
スーパーユーザのみがこの設定を変更できます。␞␞     </para>␞
␝     <indexterm>
...
␝      <para>␟       When this parameter is on, per-plan-node timing occurs for all
       ...
       performance cost, at the price of obtaining less information.␟このパラメータが有効の場合、計画ノードごとの時間的調整は事実上ログされるまで如何に時間が掛かろうと、全ての実行文に対して引き起こります。
極端に性能上のマイナスの影響が起こり得ます。
<varname>auto_explain.log_timing</varname>を無効にすれば、得られる情報が少なくなるという犠牲を払って、性能の損失が改善されます。␞␞      </para>␞
␝     <indexterm>
...
␝     <para>␟      <varname>auto_explain.log_buffers</varname> controls whether buffer
      usage statistics are printed when an execution plan is logged; it's
      equivalent to the <literal>BUFFERS</literal> option of <command>EXPLAIN</command>.
      This parameter has no effect
      unless <varname>auto_explain.log_analyze</varname> is enabled.
      This parameter is off by default.
      Only superusers can change this setting.␟<varname>auto_explain.log_buffers</varname>は実行計画のログを取得するときに、バッファ使用統計を出力するかどうかを制御します。
...
␝     <para>␟      <varname>auto_explain.log_wal</varname> controls whether WAL
      usage statistics are printed when an execution plan is logged; it's
      equivalent to the <literal>WAL</literal> option of <command>EXPLAIN</command>.
      This parameter has no effect
      unless <varname>auto_explain.log_analyze</varname> is enabled.
      This parameter is off by default.
      Only superusers can change this setting.␟<varname>auto_explain.log_wal</varname>は実行計画のログを取得するときに、WAL使用統計を出力するかどうかを制御します。
...
␝     <para>␟      <varname>auto_explain.log_timing</varname> controls whether per-node
      ...
      Only superusers can change this setting.␟<varname>auto_explain.log_timing</varname>は、実行計画のログが取得されたときに、ノード毎の時間的調整情報を出力するかどうかを制御します。
...
␝     <para>␟      <varname>auto_explain.log_triggers</varname> causes trigger
      execution statistics to be included when an execution plan is logged.
      This parameter has no effect
      unless <varname>auto_explain.log_analyze</varname> is enabled.
      This parameter is off by default.
      Only superusers can change this setting.␟<varname>auto_explain.log_triggers</varname>により、実行計画のログを記録するときに、トリガ実行の統計を含めるようになります。
<varname>auto_explain.log_analyze</varname>パラメータが設定されていなければ、このパラメータは効果がありません。
このパラメータはデフォルトで無効です。
スーパーユーザのみがこの設定を変更することができます。␞␞     </para>␞
␝     <indexterm>
...
␝     <para>␟      <varname>auto_explain.log_verbose</varname> controls whether verbose
      ...
      This parameter is off by default.
      Only superusers can change this setting.␟<varname>auto_explain.log_verbose</varname>は、実行計画のログが取得されたときに、冗長な詳細が出力されるかどうかを制御します。
...
␝     <para>␟      <varname>auto_explain.log_settings</varname> controls whether information
      ...
      Only superusers can change this setting.␟<varname>auto_explain.log_settings</varname>は、実行計画が記録される時に修正された設定オプションに関する情報を表示するかどうかを制御します。
...
␝     <para>␟      <varname>auto_explain.log_format</varname> selects the
      ...
      Only superusers can change this setting.␟<varname>auto_explain.log_format</varname>は使用する<command>EXPLAIN</command>出力書式を選びます。
...
␝     <para>␟      <varname>auto_explain.log_level</varname> selects the log level at which
      ...
      Only superusers can change this setting.␟<varname>auto_explain.log_level</varname>は、auto_explainが問い合わせ計画を記録するログレベルを選択します。
...
␝     <para>␟      <varname>auto_explain.log_nested_statements</varname> causes nested
      statements (statements executed inside a function) to be considered
      for logging.  When it is off, only top-level query plans are logged. This
      parameter is off by default. Only superusers can change this setting.␟<varname>auto_explain.log_nested_statements</varname>により、入れ子状の文（関数内から実行される文）を考慮して記録するようになります。
無効ならば、最上位の問い合わせ計画のみが記録されます。
このパラメータはデフォルトで無効です。
スーパーユーザのみがこの設定を変更することができます。␞␞     </para>␞
␝     <para>␟      <varname>auto_explain.sample_rate</varname> causes auto_explain to only
      ...
      will be explained or none. Only superusers can change this setting.␟<varname>auto_explain.sample_rate</varname>により、auto_explainは各セッションで一部の文の実行計画のみをログに記録するようになります。
デフォルトは1で、すべての問い合わせの実行計画をログに記録します。
入れ子になった文の場合には、実行計画がすべてログに記録されるか、全くされないかのどちらかです。
スーパーユーザのみがこの設定を変更できます。␞␞     </para>␞
␝  <para>␟   In ordinary usage, these parameters are set
   in <filename>postgresql.conf</filename>, although superusers can alter them
   on-the-fly within their own sessions.
   Typical usage might be:␟スーパーユーザは自身のセッション内でその場で変更できますが、通常の使用では、これらパラメータは<filename>postgresql.conf</filename>に設定しなければなりません。
典型的な使用方法は以下のようになります。␞␞  </para>␞
␝ <sect2 id="auto-explain-example">␟  <title>Example</title>␟  <title>例</title>␞␞␞
␝  <para>␟   This might produce log output such as:␟これにより、以下のようなログ出力が作成されます。␞␞  </para>␞
␝ <sect2 id="auto-explain-author">␟  <title>Author</title>␟  <title>作者</title>␞␞␞
␝  <para>␟   Takahiro Itagaki <email>itagaki.takahiro@oss.ntt.co.jp</email>␟   板垣 貴裕 <email>itagaki.takahiro.at.oss.ntt.co.jp</email>␞␞  </para>␞
