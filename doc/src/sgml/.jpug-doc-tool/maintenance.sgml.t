...
␝<chapter id="maintenance">␟ <title>Routine Database Maintenance Tasks</title>␟ <title>定常的なデータベース保守作業</title>␞␞␞
␝  <para>␟   <productname>PostgreSQL</productname>, like any database software, requires that certain tasks
   ...
   check that they execute successfully.␟他のデータベースソフトウェア同様、<productname>PostgreSQL</productname>も、最適な性能を得るために定常的に実施しなければならない作業があります。
...
適切なスクリプトを設定し、その実行がうまく行くかどうかを点検することは、データベース管理者の責任です。␞␞  </para>␞
␝  <para>␟   One obvious maintenance task is the creation of backup copies of the data on a
   regular schedule.  Without a recent backup, you have no chance of recovery
   after a catastrophe (disk failure, fire, mistakenly dropping a critical
   table, etc.).  The backup and recovery mechanisms available in
   <productname>PostgreSQL</productname> are discussed at length in
   <xref linkend="backup"/>.␟明らかに必要な保守作業の1つに、定期的なデータのバックアップコピーの作成があります。
最近のバックアップがなければ、（ディスクの破損、火災、重要なテーブルの間違った削除などの）破滅の後、復旧することができません。
<productname>PostgreSQL</productname>で可能なバックアップとリカバリ機構については、<xref linkend="backup"/>にて詳細に説明します。␞␞  </para>␞
␝  <para>␟   The other main category of maintenance task is periodic <quote>vacuuming</quote>
   of the database.  This activity is discussed in
   <xref linkend="routine-vacuuming"/>.  Closely related to this is updating
   the statistics that will be used by the query planner, as discussed in
   <xref linkend="vacuum-for-statistics"/>.␟他の保守作業の主なカテゴリには、定期的なデータベースの<quote>バキューム</quote>があります。
この作業については<xref linkend="routine-vacuuming"/>で説明します。
問い合わせプランナで使用される統計情報の更新も密接に関連しますが、こちらに関しては<xref linkend="vacuum-for-statistics"/>で説明します。␞␞  </para>␞
␝  <para>␟   Another task that might need periodic attention is log file management.
   This is discussed in <xref linkend="logfile-maintenance"/>.␟この他、定期的に行わなければならない作業にログファイルの管理があります。
これについては<xref linkend="logfile-maintenance"/>で説明します。␞␞  </para>␞
␝  <para>␟   <ulink
   ...
   Nagios and MRTG, but can be run standalone too.␟<ulink url="https://bucardo.org/check_postgres/"><application>check_postgres</application></ulink>が、データベースの健全性を監視し、異常な状態を報告するために用意されています。
<application>check_postgres</application>はNagiosおよびMRTGに組み込まれたものですが、独立して実行させることができます。␞␞  </para>␞
␝  <para>␟   <productname>PostgreSQL</productname> is low-maintenance compared
   to some other database management systems.  Nonetheless,
   appropriate attention to these tasks will go far towards ensuring a
   pleasant and productive experience with the system.␟<productname>PostgreSQL</productname>は他のデータベース管理システムに比べ、保守作業は少ないと言えます。
それでもなお、これらの作業に適切に注意することは、システムに対する快適かつ充実した経験を確実に得るのに効果があります。␞␞  </para>␞
...
␝ <sect1 id="routine-vacuuming">␟  <title>Routine Vacuuming</title>␟  <title>定常的なバキューム作業</title>␞␞␞
␝  <para>␟   <productname>PostgreSQL</productname> databases require periodic
   ...
   to skim this material to help them understand and adjust autovacuuming.␟<productname>PostgreSQL</productname>データベースは<firstterm>バキューム処理</firstterm>として知られている定期的な保守を必要とします。
...
自動バキューム処理に信頼をおいている管理者にとっても、この資料に目を通すことはそれらの理解と自動バキューム処理の調整に役に立つことでしょう。␞␞  </para>␞
␝  <sect2 id="vacuum-basics">␟   <title>Vacuuming Basics</title>␟   <title>バキューム作業の基本</title>␞␞␞
␝   <para>␟    <productname>PostgreSQL</productname>'s
    <link linkend="sql-vacuum"><command>VACUUM</command></link> command has to
    process each table on a regular basis for several reasons:␟<productname>PostgreSQL</productname>の<link linkend="sql-vacuum"><command>VACUUM</command></link>コマンドは以下の理由により定期的にそれぞれのテーブルを処理しなければなりません。␞␞␞
␝     <listitem>␟      <simpara>To recover or reuse disk space occupied by updated or deleted
      rows.</simpara>␟<simpara>
更新、あるいは削除された行によって占められたディスク領域の復旧または再利用。
</simpara>␞␞     </listitem>␞
␝     <listitem>␟      <simpara>To update data statistics used by the
      <productname>PostgreSQL</productname> query planner.</simpara>␟<simpara>
<productname>PostgreSQL</productname>問い合わせプランナによって使用されるデータ統計情報の更新。
</simpara>␞␞     </listitem>␞
␝     <listitem>␟      <simpara>To update the visibility map, which speeds
      up <link linkend="indexes-index-only-scans">index-only
      scans</link>.</simpara>␟      <simpara>
可視性マップの更新。
これにより<link linkend="indexes-index-only-scans">インデックスオンリースキャン</link>が高速化される。
      </simpara>␞␞     </listitem>␞
␝     <listitem>␟      <simpara>To protect against loss of very old data due to
      <firstterm>transaction ID wraparound</firstterm> or
      <firstterm>multixact ID wraparound</firstterm>.</simpara>␟<simpara>
...
␝␟    Each of these reasons dictates performing <command>VACUUM</command> operations
    of varying frequency and scope, as explained in the following subsections.␟以降の副節で説明するように、これらの理由の1つ1つは<command>VACUUM</command>操作の実行について、その頻度の変動や対象領域の変動に影響します。␞␞   </para>␞
␝   <para>␟    There are two variants of <command>VACUUM</command>: standard <command>VACUUM</command>
    ...
    avoid <command>VACUUM FULL</command>.␟<command>VACUUM</command>には、標準<command>VACUUM</command>と<command>VACUUM FULL</command>という２つの種類があります。
...
一般的に、管理者は標準<command>VACUUM</command>の使用に努め、<command>VACUUM FULL</command>の使用を避けるべきです。␞␞   </para>␞
␝   <para>␟    <command>VACUUM</command> creates a substantial amount of I/O
    traffic, which can cause poor performance for other active sessions.
    There are configuration parameters that can be adjusted to reduce the
    performance impact of background vacuuming &mdash; see
    <xref linkend="runtime-config-resource-vacuum-cost"/>.␟<command>VACUUM</command>は、かなりの量のI/Oトラフィックを発生させます。
このため、他の実行中のセッションの性能を劣化させる可能性があります。
バックグラウンドで実行されるバキューム処理による性能への影響を軽減させることを調整できるような設定パラメータがあります。
<xref linkend="runtime-config-resource-vacuum-cost"/>を参照してください。␞␞   </para>␞
...
␝  <sect2 id="vacuum-for-space-recovery">␟   <title>Recovering Disk Space</title>␟   <title>ディスク容量の復旧</title>␞␞␞
␝   <para>␟    In <productname>PostgreSQL</productname>, an
    ...
    space requirements. This is done by running <command>VACUUM</command>.␟<productname>PostgreSQL</productname>では、行の<command>UPDATE</command>もしくは<command>DELETE</command>は古い行を即座に削除しません。
...
これは<command>VACUUM</command>を実行することで行われます。␞␞   </para>␞
␝   <para>␟    The standard form of <command>VACUUM</command> removes dead row
    ...
    the operation completes.␟標準形式の<command>VACUUM</command>は、テーブルとインデックス内の無効な行バージョンを削除し、その領域を将来の再利用が可能であるものとして記録します。
...
また操作が終わるまで、テーブルの新しいコピー用に余計なディスク領域を必要とします。␞␞   </para>␞
␝   <para>␟    The usual goal of routine vacuuming is to do standard <command>VACUUM</command>s
    ...
    maintaining heavily-updated tables.␟定常的なバキューム作業の通例の目安は<command>VACUUM FULL</command>の必要性を避けるに充分な頻度で標準<command>VACUUM</command>を行うことです。
...
従って、程よい頻度の標準<command>VACUUM</command>を実行するほうが、不定期の<command>VACUUM FULL</command>を実行するより大量の更新テーブルを保守するにはより良い取り組みとなります。␞␞   </para>␞
␝   <para>␟    Some administrators prefer to schedule vacuuming themselves, for example
    ...
    work when the load is typical.␟例えば負荷が少ない夜間に全ての作業を行うように、一部の管理者は自身で計画したバキューム作業の方を選びます。
...
これにより、抑制可能な範囲を維持しつつ、負荷が標準的な場合に計画化された<command>VACUUM</command>がまとめて作業を行うことを想定することができます。␞␞   </para>␞
␝   <para>␟    For those not using autovacuum, a typical approach is to schedule a
    ...
    linkend="app-vacuumdb"/> might be helpful.␟自動バキュームを使用しない場合の典型的な方式は、データベース全体の<command>VACUUM</command>を1日1回使用頻度が低い時間帯にスケジュールすることです。
...
<xref linkend="app-vacuumdb"/>プログラムが役に立つかもしれません。␞␞   </para>␞
␝   <para>␟    Plain <command>VACUUM</command> may not be satisfactory when
    ...
    released until the new ones are complete.␟大規模な更新や削除作業の結果としてテーブルが無効な行バージョンを大量に含む場合、通常の<command>VACUUM</command>は満足のゆくものではないかもしれません。
...
新しいものが完成するまで、テーブルの旧コピーとインデックスは解放されませんので、元のテーブルと同程度の容量の余計なディスク領域も一時的に使用することに注意してください。␞␞   </para>␞
␝   <para>␟    If you have a table whose entire contents are deleted on a periodic
    ...
    The disadvantage is that strict MVCC semantics are violated.␟テーブルの内容が定期的に完全に削除される場合、<command>DELETE</command>の後に<command>VACUUM</command>を使用するよりも、<link linkend="sql-truncate"><command>TRUNCATE</command></link>を使用する方が良いでしょう。
...
不利な点は厳格なMVCCの意味論に違反することです。␞␞   </para>␞
...
␝  <sect2 id="vacuum-for-statistics">␟   <title>Updating Planner Statistics</title>␟   <title>プランナ用の統計情報の更新</title>␞␞␞
␝   <para>␟    The <productname>PostgreSQL</productname> query planner relies on
    ...
    degrade database performance.␟<productname>PostgreSQL</productname>問い合わせプランナは、優れた問い合わせ計画を作成するのに、テーブルの内容に関する統計情報に依存しています。
...
さもないと非効率的な計画を選択してしまい、データベースの性能を悪化させてしまいます。␞␞   </para>␞
␝   <para>␟    The autovacuum daemon, if enabled, will automatically issue
    ...
    to meaningful statistical changes.␟自動バキュームデーモンが有効になっている場合は、テーブルの内容が大きく変更されたときはいつでも自動的に<command>ANALYZE</command>コマンドを発行します。
...
しかし、意味のある統計情報の変更につながるかどうかは判りません。␞␞   </para>␞
␝   <para>␟    Tuples changed in partitions and inheritance children do not trigger
    ...
    keep the statistics up to date.␟パーティションや継承の子で変更されたタプルは親テーブルでの解析を誘発しません。
親テーブルが空であったり、まれにしか変更されなかったりする場合、自動バキュームにより処理されることはなく、継承ツリー全体としての統計情報は収集されないかもしれません。
統計情報を最新に保つためには、親テーブルで<command>ANALYZE</command>を手動で実行することが必要です。␞␞   </para>␞
␝   <para>␟    As with vacuuming for space recovery, frequent updates of statistics
    ...
    changes relatively slowly.␟領域復旧のためのバキューム処理と同様、頻繁な統計情報の更新は、滅多に更新されないテーブルよりも更新の激しいテーブルにとってより有益です。
...
このURL列の更新頻度も高いものかもしれませんが、その値の統計的な分布の変更は相対的に見ておそらく低いものです。␞␞   </para>␞
␝   <para>␟    It is possible to run <command>ANALYZE</command> on specific tables and even
    ...
    every single row.␟特定のテーブルに対して<command>ANALYZE</command>を実行することができます。
...
<command>ANALYZE</command>は、すべての行を読むのではなく、テーブルから統計的にランダムな行を抽出して使用します。␞␞   </para>␞
␝    <para>␟     Although per-column tweaking of <command>ANALYZE</command> frequency might not be
     ...
     linkend="guc-default-statistics-target"/> configuration parameter.␟列単位での<command>ANALYZE</command>実行頻度の調整はあまり実用的とは言えるものではありませんが、<command>ANALYZE</command>で集計される統計情報の詳細レベルの調整を列単位で行うことは価値がある場合があります。
<literal>WHERE</literal>句でよく使用され、データ分布の規則性がほとんどない列は、他の列よりもより細かいデータのヒストグラムが必要になるでしょう。
<command>ALTER TABLE SET STATISTICS</command>を参照するか、<xref linkend="guc-default-statistics-target"/>設定パラメータでデータベース全体のデフォルトを変更してください。␞␞    </para>␞
␝    <para>␟     Also, by default there is limited information available about
     the selectivity of functions.  However, if you create a statistics
     object or an expression
     index that uses a function call, useful statistics will be
     gathered about the function, which can greatly improve query
     plans that use the expression index.␟またデフォルトで、関数の選択性に関して利用可能な制限付きの情報があります。
しかし、統計情報オブジェクトや関数呼び出しを使用する式インデックスを作成する場合、有用な統計情報が関数に関して収集されます。
これにより式インデックスを使用する問い合わせ計画を大きく改良することができます。␞␞    </para>␞
␝    <para>␟     The autovacuum daemon does not issue <command>ANALYZE</command> commands for
     ...
     <command>ANALYZE</command> commands on those tables on a suitable schedule.␟自動バキュームデーモンは、有益になる頻度を決定する手段がありませんので、外部テーブルに対して<command>ANALYZE</command>コマンドを発行しません。
問い合わせが適切な計画作成のために外部テーブルの統計情報が必要であれば、適当なスケジュールでこれらのテーブルに対して手作業で管理する<command>ANALYZE</command>コマンドを実行することを勧めます。␞␞    </para>␞
␝    <para>␟     The autovacuum daemon does not issue <command>ANALYZE</command> commands
     ...
     up to date.␟自動バキュームデーモンはパーティション化テーブルに対して<command>ANALYZE</command>コマンドを発行しません。
継承の親は親自身が変更された場合にのみ解析されます。子テーブルの変更は親テーブルでの自動解析を誘発しません。
もし問い合わせが適切な計画のために親テーブルの統計情報を必要とするなら、統計情報を最新に保つために、それらのテーブルに対して定期的に手動で<command>ANALYZE</command>を実行することが必要です。␞␞    </para>␞
␝  <sect2 id="vacuum-for-visibility-map">␟   <title>Updating the Visibility Map</title>␟   <title>可視性マップの更新</title>␞␞␞
␝   <para>␟    Vacuum maintains a <link linkend="storage-vm">visibility map</link> for each
    ...
    clean up.␟バキュームは、どのページにすべての有効トランザクション（およびページが再度更新されるまでの将来のトランザクション）で可視であることが分かっているタプルのみが含まれるかを追跡するために、各テーブルの<link linkend="storage-vm">可視性マップ</link>の保守を行います。
２つの目的があります。
１つ目はバキューム自身が、整理するものがありませんので、こうしたページを次回飛ばすことができます。␞␞   </para>␞
␝   <para>␟    Second, it allows <productname>PostgreSQL</productname> to answer some
    ...
    cached even when the heap is very large.␟２つ目は、<productname>PostgreSQL</productname>が、背後にあるテーブルを参照することなく、インデックスのみを使用して一部の問い合わせに応えることができるようになります。
...
可視性マップはヒープより非常に小さいため、ヒープが非常に大きい場合であっても簡単にキャッシュすることができます。␞␞   </para>␞
...
␝  <sect2 id="vacuum-for-wraparound">␟   <title>Preventing Transaction ID Wraparound Failures</title>␟   <title>トランザクションIDの周回エラーの防止</title>␞␞␞
␝   <para>␟    <productname>PostgreSQL</productname>'s
    ...
    in every database at least once every two billion transactions.␟<productname>PostgreSQL</productname>の<link linkend="mvcc-intro">MVCC</link>トランザクションのセマンティクスは、トランザクションID（<acronym>XID</acronym>）番号の比較が可能であることに依存しています。
...
これを防ぐためには、すべてのデータベースにあるすべてのテーブルを少なくとも20億トランザクションごとにバキュームする必要があります。␞␞   </para>␞
␝   <para>␟    The reason that periodic vacuuming solves the problem is that
    ...
    how long that is.␟定期的なバキューム処理によりこの問題が解決する理由は、<command>VACUUM</command>が行に<emphasis>凍結状態</emphasis>という印をつけて、挿入トランザクションの効果が確実に可視になるような十分遠い過去にコミットされたトランザクションによりそれらが挿入されたことを表すからです。
...
これを防ぐために、凍結された行バージョンは挿入XIDが<literal>FrozenTransactionId</literal>であるかのように扱われ、それで、周回問題に関係なく、すべての通常のトランザクションから<quote>過去のもの</quote>として認識され、また、そのバージョンの行はどれだけ古いものであろうと、削除されるまで有効状態となります。␞␞   </para>␞
␝    <para>␟     In <productname>PostgreSQL</productname> versions before 9.4, freezing was
     ...
     in databases <application>pg_upgrade</application>'d from pre-9.4 versions.␟9.4より前のバージョンの<productname>PostgreSQL</productname>では、行の挿入XIDを実際に<literal>FrozenTransactionId</literal>で置換することで凍結が実装されており、これは行の<structname>xmin</structname>システム列として見えていました。
それより新しいバージョンでは単にフラグのビットをセットするだけで、行の元の<structname>xmin</structname>は後の検証での利用に備えて保存します。
しかし、9.4以前のバージョンから<application>pg_upgrade</application>でアップグレードしたデータベースでは、<structname>xmin</structname>が <literal>FrozenTransactionId</literal> (2)に等しい行がまだあるかもしれません。␞␞    </para>␞
␝    <para>␟     Also, system catalogs may contain rows with <structname>xmin</structname> equal
     ...
     older than every normal XID.␟また、システムカタログには<structname>xmin</structname>が<literal>BootstrapTransactionId</literal> (1)に等しい行が含まれる場合があり、これはその行が<application>initdb</application>の最初の段階で挿入されたことを意味します。
<literal>FrozenTransactionId</literal>と同様、この特別なXIDはすべての通常のXIDよりも古いものとして扱われます。␞␞    </para>␞
␝   <para>␟    <xref linkend="guc-vacuum-freeze-min-age"/>
    ...
    vacuumed again.␟<xref linkend="guc-vacuum-freeze-min-age"/>は、その行バージョンが凍結される前に、XID値がどのくらい経過しているのかを制御します。
この設定値を大きくすることで、そうでなければ凍結状態になる行がすぐに再び修正されるのであれば、不必要な作業を避けられるかもしれませんが、この設定値を小さくすることでテーブルを次にバキュームする必要が起こるまで継続できるトランザクション数が増加します。␞␞   </para>␞
␝   <para>␟    <command>VACUUM</command> uses the <link linkend="storage-vm">visibility map</link>
    ...
    always use its aggressive strategy.␟<command>VACUUM</command>は<link linkend="storage-vm">可視性マップ</link>を使用して、テーブルのどのページをスキャンする必要があるかを決定します。
...
<varname>vacuum_freeze_table_age</varname>を0に設定すると<command>VACUUM</command>は常にこの積極的な戦略を使うようになります。␞␞   </para>␞
␝   <para>␟    The maximum time that a table can go unvacuumed is two billion
    ...
    autovacuum is disabled.)␟テーブルをバキュームすることなく処理できる最大の時間は、20億トランザクションから最後に積極的なバキュームを実行した時点の<varname>vacuum_freeze_min_age</varname>の値を差し引いたものです。
...
（これは自動バキュームが無効であっても起こります。）␞␞   </para>␞
␝   <para>␟    This implies that if a table is not otherwise vacuumed,
    ...
    <varname>vacuum_freeze_min_age</varname>.␟これは、あるテーブルがバキュームされていなかったとしても、自動バキュームがおよそ<varname>autovacuum_freeze_max_age</varname> - <varname>vacuum_freeze_min_age</varname>トランザクション毎に呼び出されることを意味します。
...
記載するまでもありませんが、<varname>autovacuum_freeze_max_age</varname>を増やすことでも<varname>vacuum_freeze_min_age</varname>を減らすことでも、これを行うことができます。␞␞   </para>␞
␝   <para>␟    The effective maximum for <varname>vacuum_freeze_table_age</varname> is 0.95 *
    ...
    frequent aggressive vacuuming.␟<varname>vacuum_freeze_table_age</varname>に対する有効な最大値は0.95 * <varname>autovacuum_freeze_max_age</varname>です。
...
一方より低い値はより頻繁な積極的バキュームを引き起こします。␞␞   </para>␞
␝   <para>␟    The sole disadvantage of increasing <varname>autovacuum_freeze_max_age</varname>
    ...
    storage.)␟<varname>autovacuum_freeze_max_age</varname>（およびそれに付随する<varname>vacuum_freeze_table_age</varname>）を増やす唯一の欠点は、データベースクラスタのサブディレクトリ<filename>pg_xact</filename>と<filename>pg_commit_ts</filename>がより大きな容量となることです。
...
（デフォルトは2億トランザクションです。換算すると<filename>pg_xact</filename>はおよそ50MB、<filename>pg_commit_ts</filename>はおよそ2GBの容量となります。）␞␞   </para>␞
␝   <para>␟    One disadvantage of decreasing <varname>vacuum_freeze_min_age</varname> is that
    ...
    any more.␟<varname>vacuum_freeze_min_age</varname> を減らすことにも1つ欠点があります。
これにより<command>VACUUM</command>が大して役に立たなくなるかもしれません。
テーブル行がすぐに変更される場合（新しいXIDを獲得することになります）、行バージョンを凍結することは時間の無駄です。
そのため、この設定は、行の変更が起こらなくなるまで凍結されない程度に大きくすべきです。␞␞   </para>␞
␝   <para>␟    To track the age of the oldest unfrozen XIDs in a database,
    ...
    A convenient way to
    examine this information is to execute queries such as:␟データベース内のもっとも古い凍結されていないXIDの年代を追跡するために、<command>VACUUM</command>はシステムテーブル<structname>pg_class</structname>と<structname>pg_database</structname>にXID統計情報を保持します。
...
␝␟    The <literal>age</literal> column measures the number of transactions from the
    cutoff XID to the current transaction's XID.␟<literal>age</literal>列は切り捨てXIDから現在のトランザクションXIDまでのトランザクション数を測ります。␞␞   </para>␞
␝    <para>␟     When the <command>VACUUM</command> command's <literal>VERBOSE</literal>
     ...
     <command>VACUUM</command> operation executed by autovacuum.␟<command>VACUUM</command>コマンドの<literal>VERBOSE</literal>パラメータが指定されている場合、<command>VACUUM</command>はテーブルに関するさまざまな統計情報を出力します。
これには<structfield>relfrozenxid</structfield>および<structfield>relminmxid</structfield>をどのように繰り上げたかの情報や新しく凍結されたページの数が含まれます。
(<xref linkend="guc-log-autovacuum-min-duration"/>で制御される)自動バキュームロギングが自動バキュームによって実行された<command>VACUUM</command>操作を報告する場合にも、サーバログに同じ詳細が表示されます。␞␞    </para>␞
␝   <para>␟    <command>VACUUM</command> normally only scans pages that have been modified
    ...
    be forced for the table.␟<command>VACUUM</command>は通常は最後のバキュームの後で変更されたページのみスキャンしますが、<structfield>relfrozenxid</structfield>はテーブルの凍結されていないXIDを含むかもしれないすべてのページをスキャンしたときのみ繰り上がります。
...
<structfield>relfrozenxid</structfield>を繰り上げる<command>VACUUM</command>が<varname>autovacuum_freeze_max_age</varname>に達するまでにテーブルに対して発行されない場合、そのテーブルに対して自動バキュームが早急に強制されます。␞␞   </para>␞
␝   <para>␟    If for some reason autovacuum fails to clear old XIDs from a table, the
    system will begin to emit warning messages like this when the database's
    oldest XIDs reach forty million transactions from the wraparound point:␟何らかの理由により自動バキュームがテーブルの古いXIDの整理に失敗した場合、システムはデータベースの最古のXIDが周回ポイントから4000万トランザクションに達した場合と似たような警告メッセージを発行し始めます。␞␞␞
␝␟    (A manual <command>VACUUM</command> should fix the problem, as suggested by the
    ...
    there are fewer than three million transactions left until wraparound:␟（ヒントで示唆されたように手動<command>VACUUM</command>はこの問題を解決します。
...
␝␟    In this condition any transactions already in progress can continue,
    ...
    Instead, follow these steps:␟この状態では、すでに進行中のトランザクションは継続できますが、読み込み専用トランザクションのみを開始できます。
...
代わりに、次の手順を実行してください。␞␞␞
␝     <listitem>␟      <simpara>Resolve old prepared transactions. You can find these by checking
       ...
       committed or rolled back.</simpara>␟<simpara>古いプリペアドトランザクションを解決します。
<link linkend="view-pg-prepared-xacts">pg_prepared_xacts</link>の<literal>age(transactionid)</literal>が大きい行を確認して見つけることができます。
このようなトランザクションはコミットまたはロールバックされるべきです。</simpara>␞␞     </listitem>␞
␝     <listitem>␟      <simpara>End long-running open transactions. You can find these by checking
       ...
       can be terminated using <literal>pg_terminate_backend</literal>.</simpara>␟<simpara>長時間実行されているオープントランザクションを終了します。
...
このようなトランザクションはコミットまたはロールバックするか、<literal>pg_terminate_backend</literal>を使用してセッションを終了できます。</simpara>␞␞     </listitem>␞
␝     <listitem>␟      <simpara>Drop any old replication slots. Use
       ...
       need to be rebuilt.</simpara>␟<simpara>古いレプリケーションスロットを削除します。
...
存在するサーバに対してスロットを削除しても、そのスロットに接続しようとする可能性がある場合、そのレプリカは再構築する必要があるでしょう。</simpara>␞␞     </listitem>␞
␝     <listitem>␟      <simpara>Execute <command>VACUUM</command> in the target database. A database-wide
       ...
       more than the minimum amount of work required to restore normal operation.</simpara>␟<simpara>対象のデータベースで<command>VACUUM</command>を実行します。
...
通常の操作を回復するために必要な最小限の作業以上の作業を行うからです。</simpara>␞␞     </listitem>␞
␝     <listitem>␟      <simpara>Once normal operation is restored, ensure that autovacuum is properly configured
       in the target database in order to avoid future problems.</simpara>␟<simpara>通常の動作が回復したら、将来の問題を回避するために、対象のデータベースで自動バキュームが正しく設定されていることを確認してください。</simpara>␞␞     </listitem>␞
␝    <para>␟     In earlier versions, it was sometimes necessary to stop the postmaster and
     ...
     <xref linkend="app-postgres"/> reference page for details about using single-user mode.␟以前のバージョンでは、postmasterを停止して<command>VACUUM</command>をシングルユーザモードで実行する必要な場合がありました。
...
シングルユーザモードの使用の詳細については、<xref linkend="app-postgres"/>のリファレンスページを参照してください。␞␞    </para>␞
␝    <indexterm>
     <primary>wraparound</primary>
...
␝   <sect3 id="vacuum-for-multixact-wraparound">␟    <title>Multixacts and Wraparound</title>␟    <title>マルチトランザクションと周回</title>␞␞␞
␝    <para>␟     <firstterm>Multixact IDs</firstterm> are used to support row locking by
     ...
     transaction IDs associated with a multixact ID.␟<firstterm>マルチトランザクションID</firstterm>は複数のトランザクションによる行ロックをサポートするのに使われます。
...
<xref linkend="functions-pg-snapshot"/>で説明されているシステム関数<function>pg_get_multixact_members()</function>を使用して、マルチトランザクションIDに関連付けられたトランザクションIDを調べることができます。␞␞    </para>␞
␝    <para>␟     Whenever <command>VACUUM</command> scans any part of a table, it will replace
     ...
     <structname>pg_class</structname>.<structfield>relminmxid</structfield> to find its age.␟テーブルの何らかの部分に対し<command>VACUUM</command>スキャンされるときはいつでも、そのときに見つかった<xref linkend="guc-vacuum-multixact-freeze-min-age"/>よりも古いマルチトランザクションIDはすべて異なる値で置き換えられます。
...
<structname>pg_class</structname>.<structfield>relminmxid</structfield>に対してその年代を調べるのに<function>mxid_age()</function>を使えます。␞␞    </para>␞
␝    <para>␟     Aggressive <command>VACUUM</command>s, regardless of what causes
     ...
     multixacts can be removed.␟積極的な<command>VACUUM</command>は、その原因が何かに関わらず、そのテーブルの<structfield>relminmxid</structfield>を繰り上げできることが<emphasis>保証されています</emphasis>。
結局、すべてのデータベースのすべてのテーブルがスキャンされ、最も古いマルチトランザクション値が繰り上げられますので、ディスク上でより古いマルチトランザクションを保持している領域は削除できます。␞␞    </para>␞
␝    <para>␟     As a safety device, an aggressive vacuum scan will
     ...
     area can grow up to about 20GB before reaching wraparound.␟安全装置として、<xref linkend="guc-autovacuum-multixact-freeze-max-age"/>よりもそのマルチトランザクション年代が大きいどのテーブルに対しても、積極的なバキュームスキャンが起こります。
...
メンバ格納領域は、周回に達する前に、約20GBまで成長することがあります。␞␞    </para>␞
␝    <para>␟     Similar to the XID case, if autovacuum fails to clear old MXIDs from a table, the
     ...
     than three million left until wraparound.␟XIDの場合と同様に、自動バキュームがテーブルから古いMXIDをクリアできない場合、データベースの最も古いMXIDが周回ポイントから4000万トランザクションに達すると、システムは警告メッセージの出力を開始します。
そして、XIDの場合と同様に、こうした警告を無視し続け、周回するまで300万を切ると、システムは新しいMXIDの生成を拒絶します。␞␞    </para>␞
␝    <para>␟     Normal operation when MXIDs are exhausted can be restored in much the same way as
...
␝     <listitem>␟      <simpara>Running transactions and prepared transactions can be ignored if there
       is no chance that they might appear in a multixact.</simpara>␟<simpara>実行中のトランザクションとプリペアドトランザクションは、マルチトランザクションに現れる可能性がない場合は無視できます。</simpara>␞␞     </listitem>␞
␝     <listitem>␟      <simpara>MXID information is not directly visible in system views such as
       <literal>pg_stat_activity</literal>; however, looking for old XIDs is still a good
       way of determining which transactions are causing MXID wraparound problems.</simpara>␟<simpara><literal>pg_stat_activity</literal>などのシステムビューではMXID情報は直接表示されませんが、古いXIDを探すことはMXIDの周回問題の原因となっているトランザクションを判断する良い方法です。</simpara>␞␞     </listitem>␞
␝     <listitem>␟      <simpara>XID exhaustion will block all write transactions, but MXID exhaustion will
       only block a subset of write transactions, specifically those that involve
       row locks that require an MXID.</simpara>␟<simpara>XIDの枯渇はすべての書き込みトランザクションをブロックしますが、MXIDの枯渇は、特に MXIDを必要とする行ロックを含む書き込みトランザクションのサブセットのみをブロックします。</simpara>␞␞     </listitem>␞
␝   <indexterm>
...
␝  <sect2 id="autovacuum">␟   <title>The Autovacuum Daemon</title>␟   <title>自動バキュームデーモン</title>␞␞␞
␝   <para>␟    <productname>PostgreSQL</productname> has an optional but highly
    ...
    configuration parameters are appropriately set.␟<productname>PostgreSQL</productname>には、省略可能ですが強く推奨される<firstterm>自動バキューム</firstterm>という機能があります。
...
デフォルトの設定では、自動バキュームは有効で、関連するパラメータも適切に設定されています。␞␞   </para>␞
␝   <para>␟    The <quote>autovacuum daemon</quote> actually consists of multiple processes.
    ...
    autovacuum workers' activity.␟実際のところ<quote>自動バキュームデーモン</quote>は複数のプロセスから構成されます。
...
<xref linkend="guc-log-autovacuum-min-duration"/>も自動バキュームワーカーの活動を監視するために設定できます。␞␞   </para>␞
␝   <para>␟    If several large tables all become eligible for vacuuming in a short
    ...
    <xref linkend="guc-superuser-reserved-connections"/> limits.␟短期間にいくつかの大規模なテーブルがすべてバキューム対象として適切な状態になったとすると、すべての自動バキュームワーカーはこうしたテーブルに対するバキューム処理に長い期間占領される可能性があります。
...
ワーカーの実行数は<xref linkend="guc-max-connections"/>制限にも<xref linkend="guc-superuser-reserved-connections"/>制限にも計上されないことに注意してください。␞␞   </para>␞
␝   <para>␟    Tables whose <structfield>relfrozenxid</structfield> value is more than
    <xref linkend="guc-autovacuum-freeze-max-age"/> transactions old are always
    vacuumed (this also applies to those tables whose freeze max age has
    been modified via storage parameters; see below).  Otherwise, if the
    number of tuples obsoleted since the last
    <command>VACUUM</command> exceeds the <quote>vacuum threshold</quote>, the
    table is vacuumed.  The vacuum threshold is defined as:␟テーブルの<structfield>relfrozenxid</structfield>値が<xref linkend="guc-autovacuum-freeze-max-age"/>トランザクション年齢よりも古い場合、そのテーブルは常にバキュームされます
...
このバキューム閾値は以下のように定義されます。␞␞<programlisting>␞
␝<programlisting>␟vacuum threshold = vacuum base threshold + vacuum scale factor * number of tuples␟バキューム閾値 = バキューム基礎閾値 + バキューム規模係数 * タプル数␞␞</programlisting>␞
␝</programlisting>␟    where the vacuum base threshold is
    <xref linkend="guc-autovacuum-vacuum-threshold"/>,
    the vacuum scale factor is
    <xref linkend="guc-autovacuum-vacuum-scale-factor"/>,
    and the number of tuples is
    <structname>pg_class</structname>.<structfield>reltuples</structfield>.␟ここで、バキューム基礎閾値は<xref linkend="guc-autovacuum-vacuum-threshold"/>、バキューム規模係数は<xref linkend="guc-autovacuum-vacuum-scale-factor"/>、タプル数は<structname>pg_class</structname>.<structfield>reltuples</structfield>です。␞␞   </para>␞
␝   <para>␟    The table is also vacuumed if the number of tuples inserted since the last
    vacuum has exceeded the defined insert threshold, which is defined as:␟直前のバキュームの後に挿入されたタプル数が定義された挿入閾値を超えた場合も、テーブルはバキュームされます。ここで挿入閾値は以下のように定義されます。␞␞<programlisting>␞
␝<programlisting>␟vacuum insert threshold = vacuum base insert threshold + vacuum insert scale factor * number of tuples␟バキューム挿入閾値 = バキューム基礎挿入閾値 + バキューム挿入規模係数 * タプル数␞␞</programlisting>␞
␝</programlisting>␟    where the vacuum insert base threshold is
    ...
    since the last vacuum are scanned.␟ここで、バキューム挿入基礎閾値は<xref linkend="guc-autovacuum-vacuum-insert-threshold"/>、バキューム挿入規模係数は<xref linkend="guc-autovacuum-vacuum-insert-scale-factor"/>です。
...
そうでなければ最後のバキュームの後に変更されたページのみスキャンされます。␞␞   </para>␞
␝   <para>␟    For analyze, a similar condition is used: the threshold, defined as:␟解析でも似たような条件が使用されます。
以下で定義される閾値が、␞␞<programlisting>␞
␝<programlisting>␟analyze threshold = analyze base threshold + analyze scale factor * number of tuples␟解析閾値 = 解析基礎閾値 + 解析規模係数 * タプル数␞␞</programlisting>␞
␝</programlisting>␟    is compared to the total number of tuples inserted, updated, or deleted
    since the last <command>ANALYZE</command>.␟前回の<command>ANALYZE</command>の後に挿入、更新、削除されたタプル数と比較されます。␞␞   </para>␞
␝   <para>␟    Partitioned tables do not directly store tuples and consequently
    ...
    of data in their partitions changes significantly.␟パーティション化テーブルはタプルを直接格納しないため、自動バキュームによって処理されません。
...
この問題を回避するには、パーティション化テーブルに最初にデータが移入されたときに<command>ANALYZE</command>を手動で実行し、パーティション内のデータの分布が大きく変化した場合には必ず再度実行します。␞␞   </para>␞
␝   <para>␟    Temporary tables cannot be accessed by autovacuum.  Therefore,
    appropriate vacuum and analyze operations should be performed via
    session SQL commands.␟一時テーブルには自動バキュームでアクセスすることはできません。
したがってセッションのSQLコマンドを用いて適切なバキュームおよび解析操作を行わなければなりません。␞␞   </para>␞
␝   <para>␟    The default thresholds and scale factors are taken from
    ...
    the global settings.␟デフォルトの閾値と規模係数は、<filename>postgresql.conf</filename>から取られますが、(他の多くの自動バキューム制御パラメータと合わせて)テーブル毎に上書きすることができます。
より詳細な情報は<xref linkend="sql-createtable-storage-parameters"/>を参照してください。
テーブルの格納パラメータで設定が変更されると、そのテーブルを処理する時にその値が使用されます。
そうでなければ、全体設定が使われます。
全体設定についての詳細な情報は<xref linkend="runtime-config-autovacuum"/>を参照してください。␞␞   </para>␞
␝   <para>␟    When multiple workers are running, the autovacuum cost delay parameters
    ...
    are not considered in the balancing algorithm.␟複数のワーカープロセスが実行している場合、自動バキュームコスト遅延パラメータ(<xref linkend="runtime-config-resource-vacuum-cost"/>を参照してください)は実行中のワーカー全体に<quote>振り分け</quote>られます。
このため、ワーカーの実稼働数に関らず、システムに与えるI/Oの総影響は変わりありません。
しかし、テーブル毎の<literal>autovacuum_vacuum_cost_delay</literal>または<literal>autovacuum_vacuum_cost_limit</literal>格納パラメータが設定されたテーブルを処理するワーカーは振り分けアルゴリズムでは考慮されません。␞␞   </para>␞
␝   <para>␟    Autovacuum workers generally don't block other commands.  If a process
    ...
    automatically interrupted.␟自動バキュームワーカーは通常は他のコマンドをブロックしません。
...
しかしながら、自動バキュームがトランザクションIDの周回を防ぐために動作している(すなわち、<structname>pg_stat_activity</structname>ビューの自動バキューム問い合わせ名が<literal>(to prevent wraparound)</literal>で終わっている)場合には、自動バキュームは自動的には中断されません。␞␞   </para>␞
␝    <para>␟     Regularly running commands that acquire locks conflicting with a
     <literal>SHARE UPDATE EXCLUSIVE</literal> lock (e.g., ANALYZE) can
     effectively prevent autovacuums from ever completing.␟<literal>SHARE UPDATE EXCLUSIVE</literal>ロックと衝突するロックを獲得する、定期的に動作するコマンド(例えばANALYZE)により、自動バキュームが実質的に終わらなくなることがあります。␞␞    </para>␞
...
␝ <sect1 id="routine-reindex">␟  <title>Routine Reindexing</title>␟  <title>定常的なインデックスの再作成</title>␞␞␞
␝  <para>␟   In some situations it is worthwhile to rebuild indexes periodically
   with the <xref linkend="sql-reindex"/> command or a series of individual
   rebuilding steps.␟<xref linkend="sql-reindex"/>コマンドまたは一連の個々の再構築処理を使用して定期的にインデックスを再構築することが価値がある状況があります。␞␞␞
␝  <para>␟   B-tree index pages that have become completely empty are reclaimed for
   ...
   periodic reindexing is recommended.␟完全に空になったB-treeインデックスページは再利用のために回収されます。
...
こうした使用状況では、定期的なインデックス再構築を推奨します。␞␞  </para>␞
␝  <para>␟   The potential for bloat in non-B-tree indexes has not been well
   researched.  It is a good idea to periodically monitor the index's physical
   size when using any non-B-tree index type.␟B-tree以外のインデックスが膨張する可能性はまだよく調査されていません。
B-tree以外の任意の種類のインデックスを使用する際には、インデックスの物理容量を定期的に監視することを勧めます。␞␞  </para>␞
␝  <para>␟   Also, for B-tree indexes, a freshly-constructed index is slightly faster to
   access than one that has been updated many times because logically
   adjacent pages are usually also physically adjacent in a newly built index.
   (This consideration does not apply to non-B-tree indexes.)  It
   might be worthwhile to reindex periodically just to improve access speed.␟また、B-treeインデックスでは、新規に構築したインデックスの方が何度も更新されたインデックスよりもアクセスが多少高速です。
新しく構築されたインデックスでは論理的に近接するページが通常物理的にも近接するからです。
（これはB-tree以外のインデックスではあてはまりません。）
アクセス速度を向上させるためだけに周期的にインデックスを再構築することは価値があるかもしれません。␞␞  </para>␞
␝  <para>␟   <xref linkend="sql-reindex"/> can be used safely and easily in all cases.
   ...
   <literal>SHARE UPDATE EXCLUSIVE</literal> lock.␟<xref linkend="sql-reindex"/>はすべての状況で安全に簡単に使うことができます。
このコマンドはデフォルトで<literal>ACCESS EXCLUSIVE</literal>ロックを要求しますので、<literal>CONCURRENTLY</literal>オプションを付けて実行する方が好ましい場合がしばしばあります。その場合には<literal>SHARE UPDATE EXCLUSIVE</literal>ロックしか要求しません。␞␞  </para>␞
...
␝ <sect1 id="logfile-maintenance">␟  <title>Log File Maintenance</title>␟  <title>ログファイルの保守</title>␞␞␞
␝  <para>␟   It is a good idea to save the database server's log output
   somewhere, rather than just discarding it via <filename>/dev/null</filename>.
   The log output is invaluable when diagnosing
   problems.␟データベースサーバのログ出力を<filename>/dev/null</filename>に渡して単に破棄するのではなく、どこかに保存しておくことを推奨します。
問題の原因を究明する時にログ出力は貴重です。␞␞  </para>␞
␝   <para>␟    The server log can contain sensitive information and needs to be protected,
    ...
    appropriately authorized people.␟サーバログには機密情報が含まれている可能性があり、保存方法や保存場所、ルーティング先に関係なく保護する必要があります。
...
サーバログは、適切に権限を与えられたユーザにのみ見えるようにしてください。␞␞   </para>␞
␝  <para>␟   Log output tends to be voluminous
   (especially at higher debug levels) so you won't want to save it
   indefinitely.  You need to <emphasis>rotate</emphasis> the log files so that
   new log files are started and old ones removed after a reasonable
   period of time.␟ログ出力は（特により高いデバッグレベルの時に）巨大になる傾向があるので、際限なく保存したくはないでしょう。
適切な期間を経過した後に、新しいログファイルが開始され、古いログファイルが削除されるように、ログファイルを<emphasis>ローテーション</emphasis>する必要があります。␞␞  </para>␞
␝  <para>␟   If you simply direct the <systemitem>stderr</systemitem> of
   <command>postgres</command> into a
   file, you will have log output, but
   the only way to truncate the log file is to stop and restart
   the server. This might be acceptable if you are using
   <productname>PostgreSQL</productname> in a development environment,
   but few production servers would find this behavior acceptable.␟単に<command>postgres</command>の<systemitem>stderr</systemitem>をファイルに渡している場合、ログ出力を保持できますが、そのログファイルを切り詰めるためにはサーバを停止させ、再度起動させるしか方法がありません。
開発環境で<productname>PostgreSQL</productname>を使用しているのであればこれで構いませんが、実運用サーバでこの振舞いが適切となることはほぼありません。␞␞  </para>␞
␝  <para>␟   A better approach is to send the server's
   ...
   (comma-separated values) format.␟サーバの<systemitem>stderr</systemitem>を何らかのログローテーションプログラムに送信する方が良いでしょう。
...
また、この方法を使用して、機械読み取りしやすい<acronym>CSV</acronym>(カンマ区分値)書式でログデータを取り込むことができます。␞␞  </para>␞
␝  <para>␟   Alternatively, you might prefer to use an external log rotation
   ...
   pipe command, for example:␟また、既に他のサーバソフトウェアで使用している外部のログローテーションプログラムがあるのであれば、それを使用したいと考えるでしょう。
...
␝  <para>␟   You can combine these approaches by setting up <application>logrotate</application>
   ...
   (see <xref linkend="runtime-config-logging-where"/>).␟<productname>PostgreSQL</productname>組み込みのログ収集機構により生成されるログファイルを集めるのに<application>logrotate</application>を設定することで、上の方法を組み合わせることができます。
...
サーバはこのコマンドを受け取ると、ログ収集の設定に応じて新しいログファイルに切り替えるか、既存のファイルを再度開くかします(<xref linkend="runtime-config-logging-where"/>を参照してください)。␞␞  </para>␞
␝   <para>␟    When using static log file names, the server might fail to reopen the log
    ...
    and use a <literal>prerotate</literal> script to ignore open log files.␟静的なログファイル名を使う場合には、開けるファイルの最大数に達したりファイルテーブルのオーバーフローが起きた場合に、サーバがログファイルを再度開くのに失敗するかもしれません。
...
この問題を避けるため、ログファイル名を動的に割り当てて、開いているログファイルを無視する<literal>prerotate</literal>スクリプトを使うようにログ収集機構を設定できます。␞␞    </para>␞
␝  <para>␟   Another production-grade approach to managing log output is to
   ...
   <application>syslog</application>.␟この他の実運用レベルのログ出力の管理方法は、<application>syslog</application>に送信し、<application>syslog</application>にファイルのローテーションを行わせることです。
...
ログローテーションを自動化させたい場合は、<application>logrotate</application>プログラムを設定することで、<application>syslog</application>からのログファイルを扱うことができます。␞␞  </para>␞
␝  <para>␟   On many systems, however, <application>syslog</application> is not very reliable,
   ...
   in the <application>syslog</application> configuration file to disable syncing.)␟しかし、多くのシステムでは<application>syslog</application>は特に巨大なログメッセージに関してあまり信頼できません。
...
（同期化を無効にするため、<application>syslog</application>設定ファイル内のファイル名の先頭に<quote><literal>-</literal></quote>を使うことができます。）␞␞  </para>␞
␝  <para>␟   Note that all the solutions described above take care of starting new
   log files at configurable intervals, but they do not handle deletion
   of old, no-longer-useful log files.  You will probably want to set
   up a batch job to periodically delete old log files.  Another possibility
   is to configure the rotation program so that old log files are overwritten
   cyclically.␟上述の手法は全て、新しいログファイルを開始する周期を設定することができますが、古い、既に役に立たなくなったログファイルの削除は扱わないことに注意してください。
おそらく定期的に古いログファイルを削除するバッチジョブを設定することになるでしょう。
他に、ローテーション用プログラムを設定して古いログファイルを周期的に上書きさせるという方法もあります。␞␞  </para>␞
␝  <para>␟   <ulink url="https://pgbadger.darold.net/"><productname>pgBadger</productname></ulink>
   ...
   files, as well as detection of many other extraordinary conditions.␟<ulink url="https://pgbadger.darold.net/"><productname>pgBadger</productname></ulink>という外部プロジェクトは洗練されたログファイルの解析を行います。
<ulink url="https://bucardo.org/check_postgres/"><productname>check_postgres</productname></ulink>は、通常ではない多くの状態の検出を行うのと同時にログファイルに重要なメッセージが現れた時にNagiosで警告する機構を提供します。␞␞  </para>␞
