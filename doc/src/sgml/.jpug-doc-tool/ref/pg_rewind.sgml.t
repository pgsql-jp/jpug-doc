...
␝  <refname>pg_rewind</refname>␟  <refpurpose>synchronize a <productname>PostgreSQL</productname> data directory with another data directory that was forked from it</refpurpose>␟  <refpurpose><productname>PostgreSQL</productname>のデータディレクトリを、そこから派生した他のデータディレクトリと同期する</refpurpose>␞␞ </refnamediv>␞
␝ <refsect1>␟  <title>Description</title>␟  <title>説明</title>␞␞␞
␝  <para>␟   <application>pg_rewind</application> is a tool for synchronizing a PostgreSQL cluster
   with another copy of the same cluster, after the clusters' timelines have
   diverged. A typical scenario is to bring an old primary server back online
   after failover as a standby that follows the new primary.␟<application>pg_rewind</application>は、PostgreSQLのクラスタのタイムラインが分岐した後、クラスタをその複製のクラスタに同期するためのツールです。
典型的なシナリオとしては、フェイルオーバー後、新しいプライマリに追従するスタンバイとして、古いプライマリサーバをオンラインに戻す、というのがあります。␞␞  </para>␞
␝  <para>␟   After a successful rewind, the state of the target data directory is
   ...
   clusters.␟巻き戻し(rewind)が成功すれば、ターゲットデータディレクトリの状態はソースデータディレクトリのベースバックアップと類似したものになります。
...
そのため、データベースが大きく、クラスタ間で変更されているブロックの割合が小さい場合には、巻き戻し操作は他の方法に比べて極めて高速になります。␞␞  </para>␞
␝  <para>␟   <application>pg_rewind</application> examines the timeline histories of the source
   ...
   to become a standby again.␟<application>pg_rewind</application>はソースとターゲットクラスタ内のタイムラインヒストリーを調べ、それらがどの時点で異なるものになったのかを調べます。
...
たとえば、スタンバイサーバが昇格してから書き込みトランザクションを実行し、再びスタンバイになるために巻き戻すこともできます。␞␞  </para>␞
␝  <para>␟   After running <application>pg_rewind</application>, WAL replay needs to
   ...
   <filename>postgresql.conf</filename>.␟<application>pg_rewind</application>を実行した後、データディレクトリが整合のとれた状態になるためにはWALリプレイの完了が必要です。
...
<filename>recovery.signal</filename>ファイルをターゲットデータディレクトリに置き、<filename>postgresql.conf</filename>に適切な<xref linkend="guc-restore-command"/>を設定することで、これを達成できます。␞␞  </para>␞
␝  <para>␟   <application>pg_rewind</application> requires that the target server either has
   ...
   must also be set to <literal>on</literal>, but is enabled by default.␟<application>pg_rewind</application>を使用するには、ターゲットサーバ上で<filename>postgresql.conf</filename>の<xref linkend="guc-wal-log-hints"/>オプションが有効になっているか、<application>initdb</application>でクラスタを初期化した時にデータチェックサムが有効になっていなければなりません。
どちらもデフォルトではonではありません（無効です）。
<xref linkend="guc-full-page-writes"/>も<literal>on</literal>（有効）でなければなりませんが、これはデフォルトで有効です。␞␞  </para>␞
␝  <warning>␟   <title>Warning: Failures While Rewinding</title>␟   <title>警告: 巻き戻し中の失敗</title>␞␞   <para>␞
␝   <para>␟    If <application>pg_rewind</application> fails while processing, then
    the data folder of the target is likely not in a state that can be
    recovered.  In such a case, taking a new fresh backup is recommended.␟処理中に<application>pg_rewind</application>が失敗した場合、ターゲットのデータフォルダーはリカバリ可能な状態でない可能性があります。
このような場合は、最新のバックアップを取ることをお勧めします。␞␞   </para>␞
␝   <para>␟    As <application>pg_rewind</application> copies configuration files
    ...
    recovery, the target may again diverge from the primary.␟<application>pg_rewind</application>はソースから設定ファイルをそのままコピーするので、特にターゲットをソースのスタンバイとして再導入する場合には、ターゲットサーバを再起動する前にリカバリのために使われる設定を修正することが必要かもしれません。
巻き戻し操作は終わったもののリカバリの設定をせずにサーバを再起動すると、ターゲットは再びプライマリから分岐するでしょう。␞␞   </para>␞
␝   <para>␟    <application>pg_rewind</application> will fail immediately if it finds
    ...
    used before the rewind.␟<application>pg_rewind</application>は直接書き込めないファイルが見つかるとすぐに失敗します。
...
その場合は、コピーされたデータを削除して、巻き戻し前に使用していたリンクのセットを元に戻す必要があります。␞␞   </para>␞
␝ <refsect1>␟  <title>Options</title>␟  <title>オプション</title>␞␞␞
␝   <para>␟    <application>pg_rewind</application> accepts the following command-line
    arguments:␟    <application>pg_rewind</application>は以下のコマンドラインオプションを受け付けます。␞␞␞
␝       <para>␟        This option specifies the target data directory that is synchronized
        with the source. The target server must be shut down cleanly before
        running <application>pg_rewind</application>␟このオプションは、同期するターゲットデータディレクトリを指定します。
ターゲットサーバは、<application>pg_rewind</application>を実行する前に、正常にシャットダウンされていなければなりません。␞␞       </para>␞
␝       <para>␟        Specifies the file system path to the data directory of the source
        server to synchronize the target with. This option requires the
        source server to be cleanly shut down.␟同期するソースサーバのデータディレクトリへのファイルシステム上のパスを指定します。
このオプションを使用する場合は、ソースサーバは正常にシャットダウンされていなければなりません。␞␞       </para>␞
␝       <para>␟        Specifies a libpq connection string to connect to the source
        ...
        requires the source server to be running and accepting connections.␟ターゲットサーバに同期するソース<productname>PostgreSQL</productname>サーバに接続するlibpq接続文字列を指定します。
接続は、ソースサーバ（詳しくは注釈を参照）で<application>pg_rewind</application>で使われる関数を実行する権限を持つロールまたはスーパーユーザロールでの通常の（レプリケーションでない）接続でなければなりません。
このオプションはソースサーバが実行中であることと接続を受け付けることを必要とします。␞␞       </para>␞
␝       <para>␟        Create <filename>standby.signal</filename> and append connection
        ...
        this option.␟出力ディレクトリで<filename>standby.signal</filename>を作成し、<filename>postgresql.auto.conf</filename>に接続設定を追加します。
このオプションでは<literal>--source-server</literal>は必須です。␞␞       </para>␞
␝       <para>␟        Do everything except actually modifying the target directory.␟ターゲットディレクトリを実際に更新する以外はすべてのことを行います。␞␞       </para>␞
␝       <para>␟        By default, <command>pg_rewind</command> will wait for all files
        ...
        installation.␟デフォルトでは、<command>pg_rewind</command>はファイルがすべて安全にディスクに書き込まれるのを待ちます。
このオプションにより、<command>pg_rewind</command>は待つことなく戻るようになります。これは速いのですが、直後にオペレーティングシステムがクラッシュした場合、データディレクトリの破損が残るかもしれません。
一般に、このオプションはテストするためには有用ですが、稼働用のインストレーションでは使うべきではありません。␞␞       </para>␞
␝       <para>␟        Enables progress reporting. Turning this on will deliver an approximate
        progress report while copying data from the source cluster.␟進行状況のレポートを有効にします。このオプションを有効にすると、データをソースクラスタからコピーする際のおおよその進行状況をレポートします。␞␞       </para>␞
␝       <para>␟        Use <varname>restore_command</varname> defined in the target cluster
        ...
        directory.␟WALファイルが<filename>pg_wal</filename>ディレクトリにもはや存在しない場合、ターゲットクラスタ設定内で定義されている<varname>restore_command</varname>を使ってWALファイルをWALアーカイブから取り出します。␞␞       </para>␞
␝       <para>␟        Use the specified main server configuration file for the target
        ...
        completion of crash recovery).␟ターゲットクラスタ用に指定されたメインサーバ設定ファイルを使用します。
これは、<application>pg_rewind</application>がこのクラスタ上での巻き戻し操作のために<application>postgres</application>コマンドを内部的に使用している場合(<option>-c/--restore-target-wal</option>オプションを指定して<varname>restore_command</varname>を取得している場合や、クラッシュリカバリの完了を強制する場合)に影響します。␞␞       </para>␞
␝       <para>␟        Print verbose debugging output that is mostly useful for developers
        debugging <application>pg_rewind</application>.␟主に開発者が<application>pg_rewind</application>をデバッグするのに役立つ冗長なデバッグ出力を印字します。␞␞       </para>␞
␝       <para>␟        <application>pg_rewind</application> requires that the target server
        ...
        case.␟<application>pg_rewind</application>は、巻き戻す前にターゲットサーバが正常にシャットダウンされていることを要求します。
...
その場合、ユーザが自身で状況を扱うことが期待されます。␞␞       </para>␞
␝       <para>␟        When set to <literal>fsync</literal>, which is the default,
        ...
        links for the WAL directory and each configured tablespace.␟デフォルトの<literal>fsync</literal>に設定すると、<command>pg_rewind</command>はデータディレクトリ内のすべてのファイルを再帰的に開いて同期します。
...
␝      <term><option>--version</option></term>␟      <listitem><para>Display version information, then exit.</para></listitem>␟      <listitem><para>バージョン情報を表示して終了します。</para></listitem>␞␞     </varlistentry>␞
␝      <term><option>--help</option></term>␟      <listitem><para>Show help, then exit.</para></listitem>␟      <listitem><para>ヘルプを表示して終了します。</para></listitem>␞␞     </varlistentry>␞
␝ <refsect1>␟  <title>Environment</title>␟  <title>環境</title>␞␞␞
␝   When <option>--source-server</option> option is used,␟   When <option>--source-server</option> option is used,
   <application>pg_rewind</application> also uses the environment variables
   supported by <application>libpq</application> (see <xref linkend="libpq-envars"/>).␟<option>--source-server</option>オプションを使用する場合、<application>pg_rewind</application>は
<application>libpq</application>で利用できる環境変数を使用します(<xref linkend="libpq-envars"/>を参照)。␞␞  </para>␞
...
␝  <para>␟   When executing <application>pg_rewind</application> using an online
   ...
   a role, named <literal>rewind_user</literal> here:␟オンラインのクラスタをソースとして使用して<application>pg_rewind</application>を実行するとき、スーパーユーザの代わりにソースのクラスタ上で<application>pg_rewind</application>で使われる関数を実行できる権限を持ったロールを使うことができます。
<literal>rewind_user</literal>という名前のこのようなロールの作り方を以下に示します。␞␞<programlisting>␞
␝  <refsect2>␟   <title>How It Works</title>␟   <title>どうやって動くのか</title>␞␞␞
␝   <para>␟    The basic idea is to copy all file system-level changes from the source
    cluster to the target cluster:␟基本的なアイデアは、ファイルシステムレベルの変更を、すべてをソースクラスタからターゲットクラスタにコピーする、というものです。␞␞   </para>␞
␝     <para>␟      Scan the WAL log of the target cluster, starting from the last
      ...
      the WAL archive.␟ソースクラスタのタイムライン履歴がターゲットクラスタから分岐した時点より前の最後のチェックポイントから始めて、ターゲットクラスタのWALログをスキャンします。
各々のWALレコードについて、変更されたデータブロックを記録します。
これにより、ソースクラスタが分岐した以降に、ターゲットクラスタで変更されたすべてのデータブロックのリストが作成されます。
WALファイルの中にもう存在しないものがある場合は、失われたファイルをWALアーカイブで探すよう<option>-c</option>オプションを付けて<application>pg_rewind</application>を再実行してみます。␞␞     </para>␞
␝     <para>␟      Copy all those changed blocks from the source cluster to
      ...
      blocks changed on the target after that divergence.␟ファイルシステムへの直接アクセス（<option>--source-pgdata</option>）かSQL （<option>--source-server</option>）を使って、変更のあったすべてのブロックを、ソースクラスタからターゲットクラスタにコピーします。
これでリレーションファイルは、ソースとターゲットのWALタイムラインが分岐した時点より前で最後に完了したチェックポイントの時点に加えて、分岐後にターゲットで変更されたブロックを含んだソースでの現在の状態に相当する状態になります。␞␞     </para>␞
␝     <para>␟      Copy all other files, including new relation files, WAL segments,
      ...
      beginning with <filename>pgsql_tmp</filename>, are omitted.␟新しいリレーションファイルやWALセグメント、<filename>pg_xact</filename>や設定ファイルなどを含めて、それ以外のファイルをすべてソースクラスタからターゲットクラスタにコピーします。
...
ファイル<filename>backup_label</filename>、<filename>tablespace_map</filename>、<filename>pg_internal.init</filename>、<filename>postmaster.opts</filename>、<filename>postmaster.pid</filename>および<filename>.DS_Store</filename>も同様です。␞␞     </para>␞
␝     <para>␟      Create a <filename>backup_label</filename> file to begin WAL replay at
      ...
      rewinding from a stopped source.␟フェイルオーバーで作成されたチェックポイントでWALリプレイを開始するために<filename>backup_label</filename>ファイルを作成し、動作中のソースから巻き戻す場合には<literal>pg_current_wal_insert_lsn()</literal>の結果として定義される最小の整合のとれたLSNを、停止したソースから巻き戻す場合には最後のチェックポイントLSNを<filename>pg_control</filename>ファイルに設定します。␞␞     </para>␞
␝     <para>␟      When starting the target, <productname>PostgreSQL</productname> replays
      all the required WAL, resulting in a data directory in a consistent
      state.␟ターゲットが起動すると、<productname>PostgreSQL</productname>は必要なWALをすべてリプレイします。それにより、データディレクトリが整合のとれた状態になります。␞␞     </para>␞
