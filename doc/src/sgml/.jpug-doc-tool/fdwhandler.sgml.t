...
␝ <chapter id="fdwhandler">␟   <title>Writing a Foreign Data Wrapper</title>␟   <title>外部データラッパーの作成</title>␞␞␞
␝   <para>␟    All operations on a foreign table are handled through its foreign data
    ...
    This chapter outlines how to write a new foreign data wrapper.␟外部テーブルへの全ての操作は、コアサーバから呼び出される関数のセットで構成される、外部データラッパーで処理されます。
...
本章では、新しい外部データラッパーを作成する方法の概要を示します。␞␞   </para>␞
␝   <para>␟    The foreign data wrappers included in the standard distribution are good
    references when trying to write your own.  Look into the
    <filename>contrib</filename> subdirectory of the source tree.
    The <xref linkend="sql-createforeigndatawrapper"/> reference page also has
    some useful details.␟独自の外部データラッパーを作成する際、標準配布物に含まれているものは優れたリファレンスです。
ソースツリーの<filename>contrib</filename>サブディレクトリを調べてください。
<xref linkend="sql-createforeigndatawrapper"/>マニュアルページにも有用な情報があります。␞␞   </para>␞
␝    <para>␟     The SQL standard specifies an interface for writing foreign data wrappers.
     However, PostgreSQL does not implement that API, because the effort to
     accommodate it into PostgreSQL would be large, and the standard API hasn't
     gained wide adoption anyway.␟標準SQLでは外部データラッパーを作成するインタフェースを定義しています。
しかしながら、PostgreSQLに適応させる労力が大きく、また標準のAPIが広く採用されているわけでもないので、PostgreSQLはそのAPIを実装していません。␞␞    </para>␞
␝   <sect1 id="fdw-functions">␟    <title>Foreign Data Wrapper Functions</title>␟    <title>外部データラッパー関数</title>␞␞␞
␝    <para>␟     The FDW author needs to implement a handler function, and optionally
     a validator function. Both functions must be written in a compiled
     language such as C, using the version-1 interface.
     For details on C language calling conventions and dynamic loading,
     see <xref linkend="xfunc-c"/>.␟FDWの作者は、ハンドラ関数と、オプションで検証関数を実装する必要があります。
...
␝    <para>␟     The handler function simply returns a struct of function pointers to
     ...
     <xref linkend="fdw-callbacks"/>.␟ハンドラ関数は単に、プランナやエグゼキュータ、様々なメンテナンスコマンドから呼び出されるコールバックの関数ポインタを含む構造体を返します。
...
コールバック関数は通常のC言語関数で、SQLレベルでは参照も呼び出しもできません。コールバック関数の説明は<xref linkend="fdw-callbacks"/>にあります。␞␞    </para>␞
␝    <para>␟     The validator function is responsible for validating options given in
     ...
     would be stored in, one of:␟検証関数は、そのラッパーを使用する外部サーバ、ユーザマッピング、外部テーブルだけでなく、外部データラッパー自身の<command>CREATE</command>や<command>ALTER</command>といったコマンドで指定されたオプションの妥当性の検証を担当します。
検証関数は、検証するオプションを含むtext配列と、オプションを関連付けるオブジェクトの種類を表すOIDという二つの引数を取るものとして登録しなければなりません。
後者はそのオブジェクトが格納されるシステムカタログのOIDに対応するもので、以下のいずれかです。␞␞     <itemizedlist spacing="compact">␞
␝     </itemizedlist>␟     If no validator function is supplied, options are not checked at object
     creation time or object alteration time.␟検証関数が指定されなかった場合、オブジェクト作成時やオブジェクト変更時にオプションはチェックされません。␞␞    </para>␞
␝   <sect1 id="fdw-callbacks">␟    <title>Foreign Data Wrapper Callback Routines</title>␟    <title>外部データラッパーのコールバックルーチン</title>␞␞␞
␝    <para>␟     The FDW handler function returns a palloc'd <structname>FdwRoutine</structname>
     struct containing pointers to the callback functions described below.
     The scan-related functions are required, the rest are optional.␟FDWハンドラ関数は、以下で説明するコールバックの関数ポインタを含む、pallocされた<structname>FdwRoutine</structname>構造体を返します。
スキャンに関連した関数は必須で、それ以外は省略可能です。␞␞    </para>␞
␝    <para>␟     The <structname>FdwRoutine</structname> struct type is declared in
     <filename>src/include/foreign/fdwapi.h</filename>, which see for additional
     details.␟<structname>FdwRoutine</structname>構造体は<filename>src/include/foreign/fdwapi.h</filename>で宣言されていますので、追加情報はそちらを参照してください。␞␞    </para>␞
␝   <sect2 id="fdw-callbacks-scan">␟    <title>FDW Routines for Scanning Foreign Tables</title>␟    <title>外部テーブルスキャンのためのFDWルーチン</title>␞␞␞
␝␟     Obtain relation size estimates for a foreign table.  This is called
     ...
     planner data structures, but it's passed explicitly to save effort.)␟外部テーブルのリレーションサイズ見積もりを取得します。
...
(<literal>foreigntableid</literal>はプランナデータ構造体からも取得できますが、手間を省くために明示的に渡されます。)␞␞    </para>␞
␝    <para>␟     This function should update <literal>baserel-&gt;rows</literal> to be the
     ...
     no <command>ANALYZE</command> has been done on this foreign table.)␟この関数は、検索条件によるフィルタリングも考慮に入れた、そのテーブルスキャンが返すと見込まれる件数に<literal>baserel-&gt;rows</literal>を更新するべきです。
...
もしこの外部テーブルに<command>ANALYZE</command>が実行されていない場合は、<literal>-1</literal>になります。)␞␞    </para>␞
␝    <para>␟     See <xref linkend="fdw-planning"/> for additional information.␟追加情報については<xref linkend="fdw-planning"/>を参照してください。␞␞    </para>␞
␝␟     Create possible access paths for a scan on a foreign table.
     This is called during query planning.
     The parameters are the same as for <function>GetForeignRelSize</function>,
     which has already been called.␟外部テーブル対するスキャンとして可能なアクセスパスを作成します。
この関数は問い合わせのプラン作成中に呼び出されます。
引数は、先に呼ばれている<function>GetForeignRelSize</function>と同じです。␞␞    </para>␞
␝    <para>␟     This function must generate at least one access path
     ...
     identify the specific scan method intended.␟この関数は、外部テーブルのスキャンに対して少なくとも一つのアクセスパス(<structname>ForeignPath</structname>ノード)を作成して、それぞれのパスを<literal>baserel-&gt;pathlist</literal>に追加するために<function>add_path</function>を呼ばなければなりません。
...
それぞれのアクセスパスはコスト見積もりを含まねばならず、また意図した特定のスキャン方式を識別するのに必要なFDW固有の情報を持つことが出来ます。␞␞    </para>␞
␝    <para>␟     See <xref linkend="fdw-planning"/> for additional information.␟追加情報については<xref linkend="fdw-planning"/>を参照してください。␞␞    </para>␞
␝␟     Create a <structname>ForeignScan</structname> plan node from the selected foreign
     ...
     relation, <literal>foreigntableid</literal> is <literal>InvalidOid</literal>.)␟選択された外部アクセスパスから<structname>ForeignScan</structname>プランノードを作成します。
...
（パスがベースリレーションではなく結合のためのものの場合、<literal>foreigntableid</literal>は<literal>InvalidOid</literal>になります。）␞␞    </para>␞
␝    <para>␟     This function must create and return a <structname>ForeignScan</structname> plan
     node; it's recommended to use <function>make_foreignscan</function> to build the
     <structname>ForeignScan</structname> node.␟この関数は<structname>ForeignScan</structname>プランノードを作成して返さなければなりません。<structname>ForeignScan</structname>ノードを構築するには<function>make_foreignscan</function>を使うことが推奨されています。␞␞    </para>␞
␝    <para>␟     See <xref linkend="fdw-planning"/> for additional information.␟追加情報については<xref linkend="fdw-planning"/>を参照してください。␞␞    </para>␞
␝␟     Begin executing a foreign scan. This is called during executor startup.
     ...
     operating mode for this plan node.␟外部テーブルスキャンの実行を開始します。
...
<literal>eflags</literal>は、このプランノードに関するエグゼキュータの操作モードを表すフラグビットを含みます。␞␞    </para>␞
␝    <para>␟     Note that when <literal>(eflags &amp; EXEC_FLAG_EXPLAIN_ONLY)</literal> is
     true, this function should not perform any externally-visible actions;
     it should only do the minimum required to make the node state valid
     for <function>ExplainForeignScan</function> and <function>EndForeignScan</function>.␟<literal>(eflags &amp; EXEC_FLAG_EXPLAIN_ONLY)</literal>が真の場合、この関数は外部に見える処理を実行すべきではないことに注意してください。
<function>ExplainForeignScan</function>や<function>EndForeignScan</function>用にノード状態を有効にするのに必要とされる最小限のことだけをすべきです。␞␞    </para>␞
␝␟     Fetch one row from the foreign source, returning it in a tuple table slot
     ...
     the <structfield>es_query_cxt</structfield> of the node's <structname>EState</structname>.␟外部ソースから一行を取り出して、それをタプルテーブルスロットに入れて返します(この用途にはノードの<structfield>ScanTupleSlot</structfield>を使うべきです)。
...
より長命なストレージが必要な場合は、<function>BeginForeignScan</function>でメモリコンテキストを作成するか、ノードの<structname>EState</structname>に含まれる<structfield>es_query_cxt</structfield>を使用してください。␞␞    </para>␞
␝    <para>␟     The rows returned must match the <structfield>fdw_scan_tlist</structfield> target
     ...
     those columns omitted.␟返される行は、ターゲットリスト<structfield>fdw_scan_tlist</structfield>が提供されたなら、それとマッチしなければならず、提供されていない場合はスキャンされている外部テーブルの行型とマッチしなければなりません。
不要な列を取り出さないように最適化することを選ぶなら、それらの列の位置にNULLを入れるか、あるいはそれらの列を除いた<structfield>fdw_scan_tlist</structfield>リストを生成するべきです。␞␞    </para>␞
␝    <para>␟     Note that <productname>PostgreSQL</productname>'s executor doesn't care
     ...
     of a data type mismatch).␟<productname>PostgreSQL</productname>のエグゼキュータは返された行が外部テーブルに定義された制約に違反しているかどうかは気にしません。
しかし、プランナはそれに着目するので、宣言された制約に反する行が外部テーブル上にあった場合に、不正な最適化をするかもしれません。
ユーザが制約が成り立つと宣言したのに制約に違反した場合は(データ型が一致しなかった場合にする必要があるのと同様に)エラーを発生させるのが適切でしょう。␞␞    </para>␞
␝␟     Restart the scan from the beginning.  Note that any parameters the
     scan depends on may have changed value, so the new scan does not
     necessarily return exactly the same rows.␟先頭からスキャンを再開します。
スキャンが依存するいずれかのパラメータが値を変更しているかもしれないので、新しいスキャンが必ずしも厳密に同じ行を返すとは限らないことに注意してください。␞␞    </para>␞
␝␟     End the scan and release resources.  It is normally not important
     to release palloc'd memory, but for example open files and connections
     to remote servers should be cleaned up.␟スキャンを終了しリソースを解放します。
通常、pallocされたメモリを解放することは重要ではありませんが、たとえば開いたファイルやリモートサーバへの接続などはクリーンアップするべきです。␞␞    </para>␞
␝   <sect2 id="fdw-callbacks-join-scan">␟    <title>FDW Routines for Scanning Foreign Joins</title>␟    <title>外部テーブルの結合をスキャンするためのFDWルーチン</title>␞␞␞
␝    <para>␟     If an FDW supports performing foreign joins remotely (rather than
     by fetching both tables' data and doing the join locally), it should
     provide this callback function:␟FDWが外部テーブルの結合を（両方のテーブルのデータをフェッチして、ローカルで結合するのでなく）リモートで実行することをサポートする場合、次のコールバック関数を提供します。␞␞    </para>␞
␝</programlisting>␟     Create possible access paths for a join of two (or more) foreign tables
     ...
     joining are always possible.␟同じ外部サーバにある2つ（またはそれ以上）の外部テーブルの結合のための可能なアクセスパスを作成します。
...
なぜなら、ローカルの結合を含んだパスはいつでも可能だからです。␞␞    </para>␞
␝    <para>␟     Note that this function will be invoked repeatedly for the same join
     relation, with different combinations of inner and outer relations; it is
     the responsibility of the FDW to minimize duplicated work.␟この関数は、同じ結合のリレーションに対して、内側と外側のリレーションの異なる組み合わせで繰り返し呼び出されることに注意して下さい。
同じ作業の繰り返しを最小化することはFDWの責任です。␞␞    </para>␞
␝    <para>␟     Note also that the set of join clauses to apply to the join,
     ...
     by the planner as the best path for the <literal>joinrel</literal>.␟また、<literal>extra-&gt;restrictlist</literal>として渡される、結合に適用される結合句の集合は、内側と外側のリレーションの組み合わせによって異なることに注意してください。
<literal>joinrel</literal>に対して生成される<structname>ForeignPath</structname>パスには、使用する結合句の集合が含まれている必要があります。これは、<literal>joinrel</literal>に対する最適なパスとしてプランナによって選択された場合に、<structname>ForeignPath</structname>パスをプランに変換するためにプランナにより使用されます。␞␞    </para>␞
␝    <para>␟     If a <structname>ForeignPath</structname> path is chosen for the join, it will
     ...
     it will supply at run time in the tuples it returns.␟<structname>ForeignPath</structname>パスが結合のために選択されると、それは結合プロセス全体を代表することになり、構成テーブルとその関連の結合のために生成されたパスは使われなくなります。
...
他の相違点は、リモートの結合についての列リストがシステムカタログにはないため、FDWは<structfield>fdw_scan_tlist</structfield>に適切な<structfield>TargetEntry</structfield>ノードのリストを入れて、実行時に返されるタプル内の列の集合を表すようにしなければならないということです。␞␞    </para>␞
␝     <para>␟      Beginning with <productname>PostgreSQL</productname> 16,
      ...
      mimics <structfield>fs_relids</structfield>'s old semantics.␟<productname>PostgreSQL</productname> 16以降、<structfield>fs_relids</structfield>には、この結合に関連する外部結合がある場合、その範囲テーブルの添字が含まれます。
新しいフィールド<structfield>fs_base_relids</structfield>はベースリレーションの添字のみを含み、<structfield>fs_relids</structfield>の古いセマンティクスを模倣しています。␞␞     </para>␞
␝    <para>␟     See <xref linkend="fdw-planning"/> for additional information.␟追加情報については<xref linkend="fdw-planning"/>を参照してください。␞␞    </para>␞
␝   <sect2 id="fdw-callbacks-upper-planning">␟    <title>FDW Routines for Planning Post-Scan/Join Processing</title>␟    <title>スキャン/結合後の処理をプラン生成するためのFDWルーチン</title>␞␞␞
␝    <para>␟     If an FDW supports performing remote post-scan/join processing, such as
     remote aggregation, it should provide this callback function:␟FDWがリモート集約など、リモートでのスキャン/結合後の処理をサポートする場合、次のコールバック関数を提供します。␞␞    </para>␞
␝</programlisting>␟     Create possible access paths for <firstterm>upper relation</firstterm> processing,
     ...
     since paths involving local processing are always possible.␟<firstterm>上位リレーション</firstterm>処理のための、ありうるアクセスパスを作成します。上位リレーションはプランナ用語で、ウィンドウ関数、ソート、テーブル更新など、全てのスキャン/結合後の問い合わせのことです。
...
<function>GetForeignJoinPaths</function>の時と同様に、この関数が何らかのパス作成に成功する必要はありません。なぜなら、ローカル処理を含んでいるパスはいつでも可能だからです。␞␞    </para>␞
␝    <para>␟     The <literal>stage</literal> parameter identifies which post-scan/join step is
     ...
     work.)␟<literal>stage</literal>パラメータはどのスキャン/結合後の処理が現在考慮されているかを定めます。
...
しかしながら、手前の処理段階のために以前に生成されたパスを検査することは、冗長なプラン作成活動を回避するのに役立ちます。）␞␞    </para>␞
␝    <para>␟     See <xref linkend="fdw-planning"/> for additional information.␟追加情報については<xref linkend="fdw-planning"/>を参照してください。␞␞    </para>␞
␝   <sect2 id="fdw-callbacks-update">␟    <title>FDW Routines for Updating Foreign Tables</title>␟    <title>外部テーブル更新のためのFDWルーチン</title>␞␞␞
␝    <para>␟     If an FDW supports writable foreign tables, it should provide
     some or all of the following callback functions depending on
     the needs and capabilities of the FDW:␟もしFDWが更新可能な外部テーブルをサポートする場合、FDWのニーズと能力に応じて、以下のコールバック関数の一部または全てを提供する必要があります。␞␞    </para>␞
␝␟     <command>UPDATE</command> and <command>DELETE</command> operations are performed
     ...
     <command>DELETE</command>.␟<command>UPDATE</command>と<command>DELETE</command>の操作は、テーブルスキャン関数によって事前にフェッチされた行に対して実行されます。
FDWは、更新や削除の対象行を厳密に識別できるように行IDや主キー列の値といった追加情報を必要とするかもしれません。
それをサポートするために、この関数は<command>UPDATE</command>や<command>DELETE</command>の間に外部テーブルから取得される列のリストに追加の隠された(または<quote>ジャンクの</quote>)対象列を追加できます。␞␞    </para>␞
␝    <para>␟     To do that, construct a <structname>Var</structname> representing
     ...
     with your extension name, to avoid conflicts against other FDWs.␟それを行うには、必要な追加の値を表す<structname>Var</structname>を作成し、それをジャンク列の名前とともに<function>add_row_identity_var</function>に渡します。
...
もしこれらとは別の種類のジャンク列が必要なら、他のFDWとの衝突を避けるために拡張子名をプレフィックスとした名前を選ぶのが賢明かもしれません。␞␞    </para>␞
␝    <para>␟     If the <function>AddForeignUpdateTargets</function> pointer is set to
     ...
     relies on an unchanging primary key to identify rows.)␟もし<function>AddForeignUpdateTargets</function>ポインタが<literal>NULL</literal>に設定されている場合は、追加の対象式は追加されません。
(FDWが行を識別するのに不変の主キーに依存するのであれば<command>UPDATE</command>は依然として実現可能かもしれませんが、<command>DELETE</command>操作を実装することは不可能になるでしょう。)␞␞    </para>␞
␝␟     Perform any additional planning actions needed for an insert, update, or
     ...
     <function>BeginForeignModify</function> during the execution stage.␟外部テーブルに対する挿入、更新、削除に必要となる、追加のプラン生成アクションを実行します。
この関数は、更新処理を実行する<structname>ModifyTable</structname>プランノードに追加されるFDW固有の情報を生成します。この固有情報は<literal>List</literal>形式でなければならず、また実行段階の間に<function>BeginForeignModify</function>に渡されます。␞␞    </para>␞
␝    <para>␟     <literal>root</literal> is the planner's global information about the query.
     ...
     <literal>plan</literal> node.␟<literal>root</literal>はその問い合わせに関するプランナのグローバル情報です。
...
これは、<literal>plan</literal>ノードの対象リレーションごとのサブ構造にインデックスを付けたい場合に使用してください。␞␞    </para>␞
␝    <para>␟     See <xref linkend="fdw-planning"/> for additional information.␟追加情報は<xref linkend="fdw-planning"/>を参照してください。␞␞    </para>␞
␝    <para>␟     If the <function>PlanForeignModify</function> pointer is set to
     <literal>NULL</literal>, no additional plan-time actions are taken, and the
     <literal>fdw_private</literal> list delivered to
     <function>BeginForeignModify</function> will be NIL.␟もし<function>PlanForeignModify</function>ポインタが<literal>NULL</literal>に設定されている場合は、追加のプラン作成時処理は実行されず、<function>BeginForeignModify</function>に渡される<literal>fdw_private</literal>リストはNILになります。␞␞    </para>␞
␝␟     Begin executing a foreign table modification operation.  This routine is
     ...
     inserted, updated, or deleted.␟外部テーブルへの変更操作の実行を開始します。
このルーチンはエグゼキュータの起動中に呼び出されます。
実際のテーブル変更に先立って必要なあらゆる初期化処理を実行する必要があります。
その後、各タプルが挿入、更新、削除されるように<function>ExecForeignInsert/ExecForeignBatchInsert</function>、<function>ExecForeignUpdate</function>、<function>ExecForeignDelete</function>のいずれかが呼ばれます。␞␞    </para>␞
␝    <para>␟     <literal>mtstate</literal> is the overall state of the
     ...
     <literal>eflags</literal> contains flag bits describing the executor's
     operating mode for this plan node.␟<literal>mtstate</literal>は実行されている<structname>ModifyTable</structname>プランノード全体の状態です。プランに関する全般的なデータと実行状態はこの構造体経由で利用可能です。
...
␝    <para>␟     Note that when <literal>(eflags &amp; EXEC_FLAG_EXPLAIN_ONLY)</literal> is
     true, this function should not perform any externally-visible actions;
     it should only do the minimum required to make the node state valid
     for <function>ExplainForeignModify</function> and <function>EndForeignModify</function>.␟<literal>(eflags &amp; EXEC_FLAG_EXPLAIN_ONLY)</literal>が真の場合、この関数は外部に見える処理を実行すべきではないことに注意してください。
<function>ExplainForeignModify</function>や<function>EndForeignModify</function>用にノード状態を有効にするのに必要な最小限のことだけを実行するべきです。␞␞    </para>␞
␝    <para>␟     If the <function>BeginForeignModify</function> pointer is set to
     <literal>NULL</literal>, no action is taken during executor startup.␟もし<function>BeginForeignModify</function>ポインタが<literal>NULL</literal>に設定されている場合は、エグゼキュータ起動時には追加処理は何も実行されません。␞␞    </para>␞
␝␟     Insert one tuple into the foreign table.
     ...
     for <command>INSERT</command> cases, but is provided for completeness.)␟外部テーブルにタプルを一つ挿入します。
<literal>estate</literal>はその問い合わせのグローバルな実行状態です。
<literal>rinfo</literal>は対象の外部テーブルを表す<structname>ResultRelInfo</structname>構造体です。
<literal>slot</literal>には挿入されるタプルが含まれます。その行型定義は外部テーブルと一致します。
<literal>planSlot</literal>には<structname>ModifyTable</structname>プランノードのサブプランが生成したタプルが含まれます。追加の<quote>ジャンク</quote>列を含みうる点において、<literal>slot</literal>とは異なります。(<literal>planSlot</literal>は一般的に<command>INSERT</command>のケースにおいてはそれほど意味を持ちませんが、完全性のために提供されます。)␞␞    </para>␞
␝    <para>␟     The return value is either a slot containing the data that was actually
     inserted (this might differ from the data supplied, for example as a
     result of trigger actions), or NULL if no row was actually inserted
     (again, typically as a result of triggers).  The passed-in
     <literal>slot</literal> can be re-used for this purpose.␟戻り値は実際に挿入されたデータ(例えばトリガ処理の結果などにより、提供されたデータとは異なるかもしれません)を含むスロットか、または(こちらも一般的にトリガの結果)実際には挿入されなかった場合はNULLです。
渡された<literal>slot</literal>はこの用途に再利用可能です。␞␞    </para>␞
␝    <para>␟     The data in the returned slot is used only if the <command>INSERT</command>
     ...
     will be wrong.␟返却されたスロット内のデータは<command>INSERT</command>文が<literal>RETURNING</literal>句を持っているか、<literal>WITH CHECK OPTION</literal>を伴うビューに影響を及ぼす場合、もしくは、外部テーブルが<literal>AFTER ROW</literal>トリガを持っていた場合にのみ使われます。
...
それとは関係なく、処理成功を表すためになんらかのスロットは返却しなければなりません。さもないと、報告される問い合わせの結果行数が誤った値になってしまいます。␞␞    </para>␞
␝    <para>␟     If the <function>ExecForeignInsert</function> pointer is set to
     <literal>NULL</literal>, attempts to insert into the foreign table will fail
     with an error message.␟もし<function>ExecForeignInsert</function>ポインタが<literal>NULL</literal>に設定されている場合は、外部テーブルへの挿入の試みはエラーメッセージとともに失敗します。␞␞    </para>␞
␝    <para>␟     Note that this function is also called when inserting routed tuples into
...
FDWがそれをサポートすることを可能にする以下で説明するコールバック関数をご覧ください。␞␞    </para>␞
␝␟     Insert multiple tuples in bulk into the foreign table.
     ...
     tuples in those arrays.␟外部テーブルに一括で複数のタプルを挿入します。
<literal>slots</literal>と<literal>planSlots</literal>が複数のタプルを含むことと<literal>*numSlots</literal>がそれらの配列のタプルの数を指定すること以外は<function>ExecForeignInsert</function>とパラメータは同じです。␞␞    </para>␞
␝    <para>␟     The return value is an array of slots containing the data that was
     ...
     <literal>*numSlots</literal>.␟戻り値は実際に挿入されたデータを含んだスロットの配列です。（これは例えばトリガ実行の結果として、提供された結果と異なるかもしれません。）
渡された<literal>slots</literal>は、この目的のために再利用できます。
挿入に成功したタプルの数は、<literal>*numSlots</literal>に返されます。␞␞    </para>␞
␝    <para>␟     The data in the returned slot is used only if the <command>INSERT</command>
     ...
     <literal>WITH CHECK OPTION</literal> constraints.␟返却されたスロット内のデータは<command>INSERT</command>文がビュー<literal>WITH CHECK OPTION</literal>を含む場合、または外部テーブルに<literal>AFTER ROW</literal>トリガがある場合にのみ、使用されます。
トリガはすべての列を必要としますが、FDWは<literal>WITH CHECK OPTION</literal>制約の内容に応じて、一部またはすべての列を返さないように最適化することを選択することができます。␞␞    </para>␞
␝    <para>␟     If the <function>ExecForeignBatchInsert</function> or
     ...
     <literal>RETURNING</literal> clause.␟<function>ExecForeignBatchInsert</function>または<function>GetForeignModifyBatchSize</function>ポインタに<literal>NULL</literal>が設定された場合、外部テーブルには<function>ExecForeignInsert</function>を使って挿入を試みます。
この関数は<literal>RETURNING</literal>句をもつ<command>INSERT</command>の場合、使用されません。␞␞    </para>␞
␝    <para>␟     Note that this function is also called when inserting routed tuples into
     a foreign-table partition or executing <command>COPY FROM</command> on
     a foreign table, in which case it is called in a different way than it
     is in the <command>INSERT</command> case.  See the callback functions
...
␝␟     Report the maximum number of tuples that a single
     ...
     table option for the user to set this value, or some hard-coded value.␟指定された外部テーブルに対して、単一の<function>ExecForeignBatchInsert</function>呼び出しが処理できる最大のタプル数を報告します。
...
FDWは、ユーザがこの値またはハードコードされた値を設定するための外部サーバや外部テーブルオプションを提供することを想定しています。␞␞    </para>␞
␝    <para>␟     If the <function>ExecForeignBatchInsert</function> or
     <function>GetForeignModifyBatchSize</function> pointer is set to
     <literal>NULL</literal>, attempts to insert into the foreign table will
     use <function>ExecForeignInsert</function>.␟<function>ExecForeignBatchInsert</function>または<function>GetForeignModifyBatchSize</function>ポインタに<literal>NULL</literal>が設定された場合、外部テーブルには<function>ExecForeignInsert</function>を使って挿入を試みます。␞␞    </para>␞
␝␟     Update one tuple in the foreign table.
     ...
     be available from this slot.␟外部テーブル内のタプルを一つ更新します。
<literal>estate</literal>はその問い合わせのグローバルな実行状態です。
<literal>rinfo</literal>は対象の外部テーブルを表す<structname>ResultRelInfo</structname>構造体です。
...
特に、<function>AddForeignUpdateTargets</function>によって要求されたジャンク列は、このスロットから利用できるようになります。␞␞    </para>␞
␝    <para>␟     The return value is either a slot containing the row as it was actually
     updated (this might differ from the data supplied, for example as a
     result of trigger actions), or NULL if no row was actually updated
     (again, typically as a result of triggers).  The passed-in
     <literal>slot</literal> can be re-used for this purpose.␟戻り値は実際に更新されたデータ(例えばトリガ処理の結果などにより、提供されたデータとは異なるかもしれません)を含むスロットか、または(こちらも一般的にトリガの結果)実際には更新されなかった場合はNULLです。
渡された<literal>slot</literal>はこの用途に再利用可能です。␞␞    </para>␞
␝    <para>␟     The data in the returned slot is used only if the <command>UPDATE</command>
     statement has a <literal>RETURNING</literal> clause or involves a view
     <literal>WITH CHECK OPTION</literal>; or if the foreign table has
     an <literal>AFTER ROW</literal> trigger.  Triggers require all columns,
     but the FDW could choose to optimize away returning some or all columns
     ...
     <literal>WITH CHECK OPTION</literal> constraints.  Regardless, some slot
     must be returned to indicate success, or the query's reported row count
     will be wrong.␟返却されたスロット内のデータは<command>UPDATE</command>文が<literal>RETURNING</literal>句を持っているか、<literal>WITH CHECK OPTION</literal>を伴うビューに影響を及ぼす場合、もしくは外部テーブルが<literal>AFTER ROW</literal>トリガを持っていた場合にのみ使われます。
...
それとは関係なく、処理成功を表すためになんらかのスロットは返却しなければなりません。さもないと、報告される問い合わせの結果行数が誤った値になってしまいます。␞␞    </para>␞
␝    <para>␟     If the <function>ExecForeignUpdate</function> pointer is set to
     <literal>NULL</literal>, attempts to update the foreign table will fail
     with an error message.␟もし<function>ExecForeignUpdate</function>ポインタが<literal>NULL</literal>に設定されている場合は、外部テーブルへの更新の試みはエラーメッセージとともに失敗します。␞␞    </para>␞
␝␟     Delete one tuple from the foreign table.
     ...
     to identify the tuple to be deleted.␟外部テーブルからタプルを一つ削除します。
<literal>estate</literal>はその問い合わせのグローバルな実行状態です。
<literal>rinfo</literal>は対象の外部テーブルを表す<structname>ResultRelInfo</structname>構造体です。
<literal>slot</literal>にはタプルの新しいデータが含まれます。その行型定義は外部テーブルと一致します。
<literal>planSlot</literal>には<structname>ModifyTable</structname>プランノードのサブプランが生成したタプルが含まれます。実際、<function>AddForeignUpdateTargets</function>が要求するジャンク列はこのスロットが運びます。ジャンク列は削除されるタプルを識別するために使用しなければなりません。␞␞    </para>␞
␝    <para>␟     The return value is either a slot containing the row that was deleted,
     or NULL if no row was deleted (typically as a result of triggers).  The
     passed-in <literal>slot</literal> can be used to hold the tuple to be returned.␟戻り値は実際に削除されたデータを含むスロットか、または(一般的にトリガの結果)実際には削除されなかった場合はNULLです。
渡された<literal>slot</literal>は返却するタプルを保持する用途に利用可能です。␞␞    </para>␞
␝    <para>␟     The data in the returned slot is used only if the <command>DELETE</command>
     ...
     count will be wrong.␟返却されたスロット内のデータは<command>DELETE</command>問い合わせが<literal>RETURNING</literal>句を持っていた場合もしくは外部テーブルが<literal>AFTER ROW</literal>トリガを持っていた場合にのみ使われます。
トリガは全列を必要としますが、FDWは<literal>RETURNING</literal>句の内容に応じて返却する列を一部にするか全てにするかを最適化する余地があります。
それとは関係なく、処理成功を表すためになんらかのスロットは返却しなければなりません。さもないと、報告される問い合わせの結果行数が誤った値になってしまいます。␞␞    </para>␞
␝    <para>␟     If the <function>ExecForeignDelete</function> pointer is set to
     <literal>NULL</literal>, attempts to delete from the foreign table will fail
     with an error message.␟もし<function>ExecForeignDelete</function>ポインタが<literal>NULL</literal>に設定されている場合は、外部テーブルからの削除の試みはエラーメッセージとともに失敗します。␞␞    </para>␞
␝␟     End the table update and release resources.  It is normally not important
     to release palloc'd memory, but for example open files and connections
     to remote servers should be cleaned up.␟テーブル更新を終えてリソースを解放します。
通常、pallocされたメモリを解放することは重要ではありませんが、たとえば開いたファイルやリモートサーバへの接続などはクリーンアップするべきです。␞␞    </para>␞
␝    <para>␟     If the <function>EndForeignModify</function> pointer is set to
     <literal>NULL</literal>, no action is taken during executor shutdown.␟もし<function>EndForeignModify</function>ポインタが<literal>NULL</literal>に設定されている場合は、エグゼキュータ終了時には追加処理は何も実行されません。␞␞    </para>␞
␝    <para>␟     Tuples inserted into a partitioned table by <command>INSERT</command> or
     ...
     <command>COPY FROM</command> is executed on a foreign table.␟<command>INSERT</command>あるいは<command>COPY FROM</command>でパーティション化テーブルに挿入されたタプルはパーティションに転送されます。
FDWが外部テーブルのパーティションへの転送をサポートしているなら、以下のコールバック関数も提供すべきです。
これらの関数は、外部テーブルで<command>COPY FROM</command>が実行された時に呼び出されます。␞␞    </para>␞
␝␟     Begin executing an insert operation on a foreign table.  This routine is
     ...
     tuple(s) to be inserted into the foreign table.␟外部テーブルへの挿入操作の実行を開始します。
...
続いて、<function>ExecForeignInsert</function>または<function>ExecForeignBatchInsert</function>が外部テーブルにタプルを挿入するために呼び出されます。␞␞    </para>␞
␝    <para>␟     <literal>mtstate</literal> is the overall state of the
     <structname>ModifyTable</structname> plan node being executed; global data about
     the plan and execution state is available via this structure.
     <literal>rinfo</literal> is the <structname>ResultRelInfo</structname> struct describing
     the target foreign table.  (The <structfield>ri_FdwState</structfield> field of
     <structname>ResultRelInfo</structname> is available for the FDW to store any
     private state it needs for this operation.)␟<literal>mtstate</literal>は、実行中の<structname>ModifyTable</structname>プランノードの全体的な状態です。
プランのグローバルデータと実行状態がこの構造体を通じて得られます。
<literal>rinfo</literal>は<structname>ResultRelInfo</structname>構造体で、対象の外部テーブルを記述します。
（この操作中に必要なFDWのプライベート状態を保存するために<structname>ResultRelInfo</structname>の<structfield>ri_FdwState</structfield>フィールドが利用可能です。）␞␞    </para>␞
␝    <para>␟     When this is called by a <command>COPY FROM</command> command, the
     ...
     command.␟この関数が<command>COPY FROM</command>コマンドで呼ばれると、外部テーブルがタプル転送で選択された対象なのか、あるいはコマンドがターゲットを指定したのかに関わらず、<literal>mtstate</literal>中のプランに関係するグローバルデータは提供されず、次に個々の挿入されるタプルに対して呼び出される<function>ExecForeignInsert</function>の<literal>planSlot</literal>パラメータは<literal>NULL</literal>となります。␞␞    </para>␞
␝    <para>␟     If the <function>BeginForeignInsert</function> pointer is set to
     <literal>NULL</literal>, no action is taken for the initialization.␟<function>BeginForeignInsert</function>ポインタが<literal>NULL</literal>なら、初期化処理は実施されません。␞␞    </para>␞
␝    <para>␟     Note that if the FDW does not support routable foreign-table partitions
     ...
     subsequently called must throw error as needed.␟FDWが外部テーブルパーティションのタプル転送をサポートしていないか、または外部テーブルに対して<command>COPY FROM</command>の実行をサポートしていないか、あるいはその両方なら、この関数あるいは以後呼ばれた<function>ExecForeignInsert/ExecForeignBatchInsert</function>は、必ず必要なだけエラーを引き起こします。␞␞    </para>␞
␝␟     End the insert operation and release resources.  It is normally not important
     to release palloc'd memory, but for example open files and connections
     to remote servers should be cleaned up.␟挿入操作を終了してリソースを解放します。
通常、pallocされたメモリを解放することは重要ではありませんが、たとえば開いたファイルやリモートサーバへの接続などはクリーンアップするべきです。␞␞    </para>␞
␝    <para>␟     If the <function>EndForeignInsert</function> pointer is set to
     <literal>NULL</literal>, no action is taken for the termination.␟<function>EndForeignInsert</function>ポインタが<literal>NULL</literal>なら、終了処理は実施されません。␞␞    </para>␞
␝␟     Report which update operations the specified foreign table supports.
     ...
     <literal>(1 &lt;&lt; CMD_DELETE) = 16</literal> for <command>DELETE</command>.␟指定された外部テーブルがどの更新処理をサポートしているかを報告します。
戻り値は、その外部テーブルがサポートする操作を表すルールイベント番号のビットマスクである必要があります。
<command>UPDATE</command>用の<literal>(1 &lt;&lt; CMD_UPDATE) = 4</literal>、<command>INSERT</command>用の<literal>(1 &lt;&lt; CMD_INSERT) = 8</literal>、<command>DELETE</command>用の<literal>(1 &lt;&lt; CMD_DELETE) = 16</literal>といった<literal>CmdType</literal>列挙値を使います。␞␞    </para>␞
␝    <para>␟     If the <function>IsForeignRelUpdatable</function> pointer is set to
     ...
     updatability for display in the <literal>information_schema</literal> views.)␟もし<function>IsForeignRelUpdatable</function>ポインタが<literal>NULL</literal>に設定されていると、外部テーブルは<function>ExecForeignInsert</function>、<function>ExecForeignUpdate</function>、<function>ExecForeignDelete</function>を提供していると、それぞれ挿入、更新、削除をサポートしていると判断します。
この関数は、FDWが一部のテーブルについてのみ更新をサポートする場合にのみ必要です。
(そのような場合でも、この関数でチェックする代わりに問い合わせ実行関数でエラーにしても構いません。しかしながら、この関数は<literal>information_schema</literal>のビューの表示で更新可否を判定するのに使用されます。)␞␞    </para>␞
␝    <para>␟     Some inserts, updates, and deletes to foreign tables can be optimized
     ...
     interfaces described below make this possible.␟外部テーブルへの挿入、更新、削除は、代替インタフェース一式を実装することで最適化できます。
...
␝␟     Decide whether it is safe to execute a direct modification
     ...
     described above.
     The parameters are the same as for <function>PlanForeignModify</function>.␟リモートサーバ上で直接変更を実行することが安全かを判断します。
...
パラメータは<function>PlanForeignModify</function>に対するものと同じです。␞␞    </para>␞
␝    <para>␟     To execute the direct modification on the remote server, this function
     ...
     <structfield>resultRelation</structfield> field.␟リモートサーバで直接変更を実行するには、本関数は対象サブプランをリモートサーバ上で直接変更する<structname>ForeignScan</structname>プランノードで書き換えしなければなりません。
...
␝    <para>␟     See <xref linkend="fdw-planning"/> for additional information.␟追加情報は<xref linkend="fdw-planning"/>を参照してください。␞␞    </para>␞
␝    <para>␟     If the <function>PlanDirectModify</function> pointer is set to
     <literal>NULL</literal>, no attempts to execute a direct modification on the
     remote server are taken.␟<function>PlanDirectModify</function>ポインタに<literal>NULL</literal>が設定されている場合、
リモートサーバでの直接変更の実行は試みられません。␞␞    </para>␞
␝␟     Prepare to execute a direct modification on the remote server.
     ...
     <literal>eflags</literal> contains flag bits describing the executor's
     operating mode for this plan node.␟リモートサーバでの直接変更を実行する準備をします。
...
␝    <para>␟     Note that when <literal>(eflags &amp; EXEC_FLAG_EXPLAIN_ONLY)</literal> is
     true, this function should not perform any externally-visible actions;
     it should only do the minimum required to make the node state valid
     for <function>ExplainDirectModify</function> and <function>EndDirectModify</function>.␟<literal>(eflags &amp; EXEC_FLAG_EXPLAIN_ONLY)</literal>が真の場合、この関数は外部に見える処理を実行すべきではないことに注意してください。
<function>ExplainDirectModify</function>や<function>EndDirectModify</function>用にノード状態を有効にするのに必要な最小限のことだけを実行するべきです。␞␞    </para>␞
␝    <para>␟     If the <function>BeginDirectModify</function> pointer is set to
     <literal>NULL</literal>, no attempts to execute a direct modification on the
     remote server are taken.␟<function>BeginDirectModify</function>ポインタが<literal>NULL</literal>に設定されている場合、リモートサーバでの直接変更の実行は試みられません。␞␞    </para>␞
␝␟     When the <command>INSERT</command>, <command>UPDATE</command> or <command>DELETE</command>
     ...
     the <structfield>es_query_cxt</structfield> of the node's <structname>EState</structname>.␟<command>INSERT</command>、<command>UPDATE</command>、または、<command>DELETE</command>の問い合わせが<literal>RETURNING</literal>句を持たないときには、リモートサーバでの直接変更の後、単にNULLが返ります。
...
より長命な格納場所を必要とするなら、<function>BeginDirectModify</function>でメモリコンテキストを作るか、ノードの<structname>EState</structname>の<structfield>es_query_cxt</structfield>を使ってください。␞␞    </para>␞
␝    <para>␟     The rows returned must match the <structfield>fdw_scan_tlist</structfield> target
     list if one was supplied, otherwise they must match the row type of the
     ...
     <structfield>fdw_scan_tlist</structfield> list with those columns omitted.␟返される行は、ターゲットリスト<structfield>fdw_scan_tlist</structfield>が提供されたなら、それとマッチしなければならず、提供されていない場合は変更されている外部テーブルの行型とマッチしなければなりません。
<literal>RETURNING</literal>計算に不要な列を取り出さないように最適化することを選ぶなら、それらの列の位置にNULLを入れるか、あるいはそれらの列を除いた<structfield>fdw_scan_tlist</structfield>リストを生成するべきです。␞␞    </para>␞
␝    <para>␟     Whether the query has the clause or not, the query's reported row count
     ...
     case.␟問い合わせが句をもつかどうかによらず、問い合わせが報告する行数はFDW自身によって増加されなければなりません。
問い合わせが句を持たないときも、FDWは<command>EXPLAIN ANALYZE</command>の場合の<structname>ForeignScanState</structname> nodeむけに行数を増加させなければなりません。␞␞    </para>␞
␝    <para>␟     If the <function>IterateDirectModify</function> pointer is set to
     <literal>NULL</literal>, no attempts to execute a direct modification on the
     remote server are taken.␟<function>IterateDirectModify</function>ポインタが<literal>NULL</literal>に設定されている場合、リモートサーバでの直接変更の実行は試みられません。␞␞    </para>␞
␝␟     Clean up following a direct modification on the remote server.  It is
     ...
     files and connections to the remote server should be cleaned up.␟リモートサーバでの直接変更の後、クリーンアップします。
通常、pallocされたメモリを解放することは重要ではありませんが、たとえば開いたファイルやリモートサーバへの接続などはクリーンアップするべきです。␞␞    </para>␞
␝    <para>␟     If the <function>EndDirectModify</function> pointer is set to
     <literal>NULL</literal>, no attempts to execute a direct modification on the
     remote server are taken.␟<function>EndDirectModify</function>ポインタが<literal>NULL</literal>に設定されている場合、リモートサーバでの直接変更の実行は試みられません。␞␞    </para>␞
␝   <sect2 id="fdw-callbacks-truncate">␟    <title>FDW Routines for <command>TRUNCATE</command></title>␟    <title><command>TRUNCATE</command>のためのFDWルーチン</title>␞␞␞
␝␟     Truncate foreign tables.  This function is called when
     ...
     data structures of foreign tables to truncate.␟外部テーブルを切り捨てます。
この関数は<xref linkend="sql-truncate"/>が外部テーブルに対して実行されたときに呼び出されます。
<literal>rels</literal>は切り捨てられる外部テーブルの<structname>Relation</structname>データ構造のリストです。␞␞    </para>␞
␝    <para>␟     <literal>behavior</literal> is either <literal>DROP_RESTRICT</literal>
     ...
     respectively.␟<literal>behavior</literal>は<literal>DROP_RESTRICT</literal>か<literal>DROP_CASCADE</literal>のいずれかで、それぞれ元の<command>TRUNCATE</command>コマンドで<literal>RESTRICT</literal>また<literal>CASCADE</literal>オプションが要求されたことを表しています。␞␞    </para>␞
␝    <para>␟     If <literal>restart_seqs</literal> is <literal>true</literal>,
     the original <command>TRUNCATE</command> command requested the
     <literal>RESTART IDENTITY</literal> behavior, otherwise the
     <literal>CONTINUE IDENTITY</literal> behavior was requested.␟<literal>restart_seqs</literal>が<literal>true</literal> の場合、元の<command>TRUNCATE</command>コマンドが<literal>RESTART IDENTITY</literal>動作が要求され、それ以外の場合は<literal>CONTINUE IDENTITY</literal>動作が要求されていたことになります。␞␞    </para>␞
␝    <para>␟     Note that the <literal>ONLY</literal> options specified
     ...
     a foreign table.␟元の<command>TRUNCATE</command>コマンドで指定された<literal>ONLY</literal>オプションは<function>ExecForeignTruncate</function>に渡されないことに注意してください。
この動作は外部テーブルに対する<command>SELECT</command>と<command>UPDATE</command>、<command>DELETE</command>のコールバック関数と同様です。␞␞    </para>␞
␝    <para>␟     <function>ExecForeignTruncate</function> is invoked once per
     ...
     must belong to the same server.␟<function>ExecForeignTruncate</function>は、外部テーブルが切り捨てられる外部サーバごとに一度呼び出されます。
これは、<literal>rels</literal>に含まれるすべての外部テーブルが同じサーバに所属していなければならないことを意味しています。␞␞    </para>␞
␝    <para>␟     If the <function>ExecForeignTruncate</function> pointer is set to
     <literal>NULL</literal>, attempts to truncate foreign tables will
     fail with an error message.␟<function>ExecForeignTruncate</function>ポインタが<literal>NULL</literal>に設定されている場合、外部テーブルを切り捨てる試みはエラーメッセージとともに失敗します。␞␞    </para>␞
␝   <sect2 id="fdw-callbacks-row-locking">␟    <title>FDW Routines for Row Locking</title>␟    <title>行ロックのためのFDWルーチン</title>␞␞␞
␝    <para>␟     If an FDW wishes to support <firstterm>late row locking</firstterm> (as described
     in <xref linkend="fdw-row-locking"/>), it must provide the following
     callback functions:␟FDWが（<xref linkend="fdw-row-locking"/>で説明される）<firstterm>遅延行ロック</firstterm>をサポートする場合は、以下のコールバック関数を提供する必要があります。␞␞    </para>␞
␝␟     Report which row-marking option to use for a foreign table.
     ...
     a member of the <literal>RowMarkType</literal> enum type.␟行の印付けでどのオプションを外部テーブルに使うかを報告します。
...
その結果は、<literal>RowMarkType</literal>列挙型のメンバでなければなりません。␞␞    </para>␞
␝    <para>␟     This function is called during query planning for each foreign table that
     ...
     or <command>DELETE</command>.␟この関数は<command>UPDATE</command>、<command>DELETE</command>、<command>SELECT FOR UPDATE/SHARE</command>の問い合わせに現れ、かつ<command>UPDATE</command>あるいは<command>DELETE</command>の対象ではない各外部テーブルについて、問い合わせの計画時に呼び出されます。␞␞    </para>␞
␝    <para>␟     If the <function>GetForeignRowMarkType</function> pointer is set to
     ...
     so it need not be provided either.)␟<function>GetForeignRowMarkType</function>のポインタが<literal>NULL</literal>に設定されていると、必ず<literal>ROW_MARK_COPY</literal>オプションが使われます。
（これは<function>RefetchForeignRow</function>が決して呼び出されないので、それを提供する必要もない、ということを意味します。）␞␞    </para>␞
␝    <para>␟     See <xref linkend="fdw-row-locking"/> for more information.␟さらなる情報については<xref linkend="fdw-row-locking"/>を参照してください。␞␞    </para>␞
␝␟     Re-fetch one tuple slot from the foreign table, after locking it if required.
     ...
     <literal>slot</literal> contains nothing useful upon call, but can be used to
     hold the returned tuple. <literal>updated</literal> is an output parameter.␟必要ならロックした後で、外部テーブルから1つのタプルスロットを再フェッチします。
...
<literal>updated</literal>は出力パラメータです。␞␞    </para>␞
␝    <para>␟     This function should store the tuple into the provided slot, or clear it if
     ...
     never be seen by this routine.)␟この関数はタプルを与えられたスロットに格納するか、あるいは行ロックが取得できなければタプルをクリアします。
獲得する行ロックの種別は<literal>erm-&gt;markType</literal>で指定されます。この値は事前に<function>GetForeignRowMarkType</function>から返されたものです。
（<literal>ROW_MARK_REFERENCE</literal>は行のロックを獲得せずに、単にタプルを再フェッチすることを意味し、また、<literal>ROW_MARK_COPY</literal>はこのルーチンで使われることはありません。）␞␞    </para>␞
␝    <para>␟     In addition, <literal>*updated</literal> should be set to <literal>true</literal>
     ...
     this, always returning <literal>true</literal> is recommended.)␟そして、<literal>*updated</literal>はフェッチしたタプルが、以前に取得したものと同じではなく、更新されたバージョンであったときに<literal>true</literal>にセットされます。
（どちらなのかFDWが判断できない場合は、<literal>true</literal>を返すことが推奨されます）。␞␞    </para>␞
␝    <para>␟     Note that by default, failure to acquire a row lock should result in
     raising an error; returning with an empty slot is only appropriate if
     the <literal>SKIP LOCKED</literal> option is specified
     by <literal>erm-&gt;waitPolicy</literal>.␟デフォルトでは、行ロックの獲得に失敗したときはエラーを発生させるべきであることに注意してください。
空スロットを返すのが適切なのは、<literal>erm-&gt;waitPolicy</literal>で<literal>SKIP LOCKED</literal>オプションが指定されている場合だけです。␞␞    </para>␞
␝    <para>␟     The <literal>rowid</literal> is the <structfield>ctid</structfield> value previously read
     ...
     data types for row IDs in future.␟<literal>rowid</literal>は、再フェッチする行を以前読んだ時の<structfield>ctid</structfield>値です。
<literal>rowid</literal>値は<type>Datum</type>として渡されますが、現在は<type>tid</type>にしかなりません。
将来は行ID以外のデータ型が可能になることを期待して、関数APIとすることが選択されました。␞␞    </para>␞
␝    <para>␟     If the <function>RefetchForeignRow</function> pointer is set to
     <literal>NULL</literal>, attempts to re-fetch rows will fail
     with an error message.␟<function>RefetchForeignRow</function>ポインタが<literal>NULL</literal>の場合、行を再フェッチする試みは失敗し、エラーメッセージを発行します。␞␞    </para>␞
␝    <para>␟     See <xref linkend="fdw-row-locking"/> for more information.␟さらなる情報については<xref linkend="fdw-row-locking"/>を参照してください。␞␞    </para>␞
␝</programlisting>␟     Recheck that a previously-returned tuple still matches the relevant
     ...
     slot.␟以前に戻されたタプルが、関連するスキャンおよび結合の制約とまだ一致しているか再検査し、更新されたバージョンのタプルを提供する場合もあります。
...
それだけでなく、置換されたタプルを提供されたスロットに格納することもできます。␞␞    </para>␞
␝    <para>␟     To implement join pushdown, a foreign data wrapper will typically
     ...
     may choose not to create access paths for that join.)␟結合のプッシュダウンを実装する場合、外部データラッパーは通常、再検査のためだけに使用される代替のローカル結合プランを構築します。
...
（そのようなパスが見つからなかった場合はNULLを返します。この場合、外部データラッパーはそれ自身によりローカルパスを構築するかもしれず、あるいは、その結合むけのアクセスパスを作らないことを選択するかもしれません。）␞␞    </para>␞
␝   <sect2 id="fdw-callbacks-explain">␟    <title>FDW Routines for <command>EXPLAIN</command></title>␟    <title><command>EXPLAIN</command>のためのFDWルーチン</title>␞␞␞
␝␟     Print additional <command>EXPLAIN</command> output for a foreign table scan.
     ...
     ANALYZE</command> case.␟外部テーブルスキャンの追加の<command>EXPLAIN</command>出力を表示します。
<command>EXPLAIN</command>出力にフィールドを追加するために<function>ExplainPropertyText</function>や関連する関数を呼び出すことができます。
<literal>es</literal>の中のフラグフィールドは何を表示するかを決めるのに使用できます。また、<command>EXPLAIN ANALYZE</command>の場合には、実行時統計情報を提供するために<structname>ForeignScanState</structname>ノードの状態を調べることができます。␞␞    </para>␞
␝    <para>␟     If the <function>ExplainForeignScan</function> pointer is set to
     <literal>NULL</literal>, no additional information is printed during
     <command>EXPLAIN</command>.␟もし<function>ExplainForeignScan</function>ポインタが<literal>NULL</literal>に設定されている場合は、<command>EXPLAIN</command>中に追加情報は表示されません。␞␞    </para>␞
␝␟     Print additional <command>EXPLAIN</command> output for a foreign table update.
     ...
     <function>BeginForeignModify</function>.␟外部テーブル更新の追加の<command>EXPLAIN</command>出力を表示します。
<command>EXPLAIN</command>出力にフィールドを追加するために<function>ExplainPropertyText</function>や関連する関数を呼び出すことができます。
...
最初の4つの引数は<function>BeginForeignModify</function>と同じです。␞␞    </para>␞
␝    <para>␟     If the <function>ExplainForeignModify</function> pointer is set to
     <literal>NULL</literal>, no additional information is printed during
     <command>EXPLAIN</command>.␟もし<function>ExplainForeignModify</function>ポインタが<literal>NULL</literal>に設定されている場合は、<command>EXPLAIN</command>中に追加情報は表示されません。␞␞    </para>␞
␝␟     Print additional <command>EXPLAIN</command> output for a direct modification
     ...
     print, and the state of the <structname>ForeignScanState</structname> node
     can be inspected to provide run-time statistics in the <command>EXPLAIN
     ANALYZE</command> case.␟リモートサーバでの直接変更について追加<command>EXPLAIN</command>出力を表示します。
この関数は<command>EXPLAIN</command>出力にフィールドを加えるために<function>ExplainPropertyText</function>と関連の関数を呼ぶことができます。
<literal>es</literal>の中のフラグフィールドは何を表示するかを決めるのに使用できます。また、<command>EXPLAIN ANALYZE</command>の場合には、実行時統計情報を提供するために<structname>ForeignScanState</structname>ノードの状態を調べることができます。␞␞    </para>␞
␝    <para>␟     If the <function>ExplainDirectModify</function> pointer is set to
     <literal>NULL</literal>, no additional information is printed during
     <command>EXPLAIN</command>.␟<function>ExplainDirectModify</function>ポインタが<literal>NULL</literal>に設定されている場合は、<command>EXPLAIN</command>中に追加情報は表示されません。␞␞    </para>␞
␝   <sect2 id="fdw-callbacks-analyze">␟    <title>FDW Routines for <command>ANALYZE</command></title>␟    <title><command>ANALYZE</command>のためのFDWルーチン</title>␞␞␞
␝␟     This function is called when <xref linkend="sql-analyze"/> is executed on
     ...
     <parameter>totalpages</parameter>.  Otherwise, return <literal>false</literal>.␟この関数は<xref linkend="sql-analyze"/>が外部テーブルに対して実行されたときに呼び出されます。
...
そうでない場合は、<literal>false</literal>を返します。␞␞    </para>␞
␝    <para>␟     If the FDW does not support collecting statistics for any tables, the
     <function>AnalyzeForeignTable</function> pointer can be set to <literal>NULL</literal>.␟もしFDWが統計情報の収集をどのテーブルについてもサポートしない場合は、<function>AnalyzeForeignTable</function>ポインタを<literal>NULL</literal>にすることもできます。␞␞    </para>␞
␝    <para>␟     If provided, the sample collection function must have the signature␟もし提供される場合は、サンプル収集関数はこのようなシグネチャを持つ必要があります。␞␞<programlisting>␞
␝␟     A random sample of up to <parameter>targrows</parameter> rows should be collected
     ...
     if the FDW does not have any concept of dead rows.)␟最大<parameter>targrows</parameter>行のランダムサンプルをテーブルから収集し、呼び出し元が提供する<parameter>rows</parameter>配列に格納する必要があります。
実際に収集された行の数を返す必要があります。
さらに、テーブルに含まれる有効行と不要行の合計数の見積もりを出力パラメータの<parameter>totalrows</parameter>と<parameter>totaldeadrows</parameter>に返す必要があります。(もしFDWが不要行という概念を持たない場合は<parameter>totaldeadrows</parameter>を0に設定してください。)␞␞    </para>␞
␝   <sect2 id="fdw-callbacks-import">␟    <title>FDW Routines for <command>IMPORT FOREIGN SCHEMA</command></title>␟    <title><command>IMPORT FOREIGN SCHEMA</command>のためのFDWルーチン</title>␞␞␞
␝␟     Obtain a list of foreign table creation commands.  This function is
     ...
     These strings will be parsed and executed by the core server.␟外部テーブル作成コマンドのリストを取得します。
...
これらの文字列はコアサーバが解析して実行します。␞␞    </para>␞
␝    <para>␟     Within the <structname>ImportForeignSchemaStmt</structname> struct,
     ...
     FDW as database object options.␟<structname>ImportForeignSchemaStmt</structname>構造体において、<structfield>remote_schema</structfield>はリモートスキーマの名前で、そこからテーブルがインポートされます。
...
これらのオプションはFDWがデータベースオブジェクトのオプションとしてサポートするものと何ら関係ある必要はありません。␞␞    </para>␞
␝    <para>␟     The FDW may ignore the <structfield>local_schema</structfield> field of
     ...
     FOREIGN TABLE</command> commands.␟FDWは<structname>ImportForeignSchemaStmt</structname>の<structfield>local_schema</structfield>フィールドを無視しても良いです。
なぜなら、コアサーバは解析された<command>CREATE FOREIGN TABLE</command>コマンドにその名前を自動的に挿入するからです。␞␞    </para>␞
␝    <para>␟     The FDW does not have to concern itself with implementing the filtering
     ...
     useful to test whether a given foreign-table name will pass the filter.␟FDWは<structfield>list_type</structfield>および<structfield>table_list</structfield>で指定されるフィルタの実装にも注意する必要はありません。
なぜなら、コアサーバはそれらのオプションによって除外されるテーブルに対して戻されたコマンドをすべて自動的にスキップするからです。
しかし、除外されるテーブルについてコマンドを作成する作業を回避するのは、そもそも役立つことが多いです。
関数<function>IsImportableForeignTable()</function>は指定の外部テーブル名がフィルタを通るかどうかの検査に役立つかもしれません。␞␞    </para>␞
␝    <para>␟     If the FDW does not support importing table definitions, the
     <function>ImportForeignSchema</function> pointer can be set to <literal>NULL</literal>.␟FDWがテーブル定義のインポートをサポートしない場合は、<function>ImportForeignSchema</function>ポインタを<literal>NULL</literal>にセットすることができます。␞␞    </para>␞
␝   <sect2 id="fdw-callbacks-parallel">␟    <title>FDW Routines for Parallel Execution</title>␟    <title>パラレル実行のためのFDWルーチン</title>␞␞    <para>␞
␝    <para>␟     A <structname>ForeignScan</structname> node can, optionally, support parallel
     ...
     but most are required if parallel execution is to be supported.␟<structname>ForeignScan</structname>ノードは、オプションとして、パラレル実行をサポートします。
...
以下のコールバックは一般に全て省略可能ですが、パラレル実行をサポートするためには必要です。␞␞    </para>␞
␝</programlisting>␟    Test whether a scan can be performed within a parallel worker.  This
    ...
    context as the leader.␟スキャンがパラレルワーカーで実行できるかテストします。
この関数はプランナが並列プランが可能であろうと考えるときだけ呼ばれます。また、そのスキャンにとってパラレルワーカーで実行するのが安全であるとき真を返すべきです。
リモートデータソースがトランザクションのセマンティクスを持つ場合は、一般にあてはまりません。ただし、ワーカーのデータへの接続を何らかの形でリーダーとして同じトランザクション文脈を共有させることができる場合を除きます。␞␞    </para>␞
␝    <para>␟    If this function is not defined, it is assumed that the scan must take
    ...
    this method even when parallel execution is not supported.␟この関数が定義されていない場合、スキャンはパラレルリーダー内で実行しなければならないと想定されます。
真を返すことは、スキャンがパラレルワーカーで実行可能であるだけで、パラレルに実行可能であることを意味するのでは無いことに注意してください。
そのため、この関数を定義することはパラレル実行がサポートされていないときでも役立つ可能性があります。␞␞    </para>␞
␝</programlisting>␟    Estimate the amount of dynamic shared memory that will be required
    ...
    no shared memory will be allocated for the FDW's use.␟並列操作に必要とされるであろう動的共有メモリ量を推定します。
これは実際に使われる量よりも大きくてよいですが、小さくてはいけません。
戻り値はバイト単位です。
この関数はオプションであり、必要でない場合は省略することができます。
しかし省略された場合、FDWの使用のために共有メモリが割り当てられないため、次の3つの関数も省略しなければなりません。␞␞    </para>␞
␝</programlisting>␟    Initialize the dynamic shared memory that will be required for parallel
    ...
    This function is optional, and can be omitted if not needed.␟並列処理で必要とされる動的共有メモリを初期化します。
<literal>coordinate</literal>は<function>EstimateDSMForeignScan</function>の戻り値に等しいサイズの共有メモリ領域へのポインタです。
この関数はオプションであり、必要でない場合は省略することができます。␞␞   </para>␞
␝</programlisting>␟    Re-initialize the dynamic shared memory required for parallel operation
    ...
    that ordering.␟外部スキャンプランノードが再スキャンされようとしているときに、並列操作に必要な動的共有メモリを再初期化します。
この関数はオプションであり、必要でない場合は省略することができます。
<function>ReScanForeignScan</function>関数はローカル状態のみをリセットし、この関数は共有状態のみをリセットすることをお勧めします。
現在、この関数は<function>ReScanForeignScan</function>より前に呼び出されますが、その順序に依存しないようにする方が良いでしょう。␞␞   </para>␞
␝</programlisting>␟    Initialize a parallel worker's local state based on the shared state
    set up by the leader during <function>InitializeDSMForeignScan</function>.
    This function is optional, and can be omitted if not needed.␟<function>InitializeDSMForeignScan</function>でリーダーがセットアップした共有状態に基づくパラレルワーカーのローカル状態を初期化します。
この関数はオプションであり、必要でない場合は省略することができます。␞␞   </para>␞
␝</programlisting>␟    Release resources when it is anticipated the node will not be executed
    ...
    this method.␟ノードが完了するまで実行されないことが予想されるときにリソースを解放します。
これはすべてのケースで呼ばれるわけではありません。
<literal>EndForeignScan</literal>は、この関数が最初に呼び出されなくても呼び出されることがあります。
このコールバックが呼び出された直後に、パラレルクエリで使用されるDSM(動的共有メモリ)セグメントが破棄されるため、DSMセグメントがなくなる前に何らかのアクションを実行する外部データラッパーがこのメソッドを実装する必要があります。␞␞   </para>␞
␝   <sect2 id="fdw-callbacks-async">␟    <title>FDW Routines for Asynchronous Execution</title>␟    <title>非同期実行のためのFDWルーチン</title>␞␞    <para>␞
␝    <para>␟     A <structname>ForeignScan</structname> node can, optionally, support
     ...
     execution is to be supported.␟<structname>ForeignScan</structname>ノードは、オプションとして<filename>src/backend/executor/README</filename>に記述されているように、非同期実行をサポートすることが可能です。
以下の関数はすべてオプションですが、非同期実行をサポートする場合はすべて必須です。␞␞    </para>␞
␝</programlisting>␟     Test whether a given <structname>ForeignPath</structname> path can scan
     ...
     foreign relation asynchronously.␟与えられた<structname>ForeignPath</structname>パスが、そもそも外部リレーションを非同期でスキャンできるかどうかをテストします。
この関数は与えられたパスが<structname>AppendPath</structname>パスの直接の子であり、プランナが非同期実行が性能を向上させると考える場合にのみ、問い合わせ計画の最後で呼ばれ、与えられたパスが外部リレーションを非同期にスキャンできる場合に真を返さなければなりません。␞␞    </para>␞
␝    <para>␟     If this function is not defined, it is assumed that the given path scans
     ...
     called, so they need not be provided either.)␟この関数が定義されていない場合、与えられたパスは<function>IterateForeignScan</function>を使用して外部リレーションをスキャンすることが想定されます。
(これは以下で説明されるコールバック関数が決して呼ばれないことを意味します。呼ばれないことを意味するので、それらも提供される必要はありません。)␞␞    </para>␞
␝</programlisting>␟     Produce one tuple asynchronously from the
     ...
     in the <literal>areq</literal>.␟<structname>ForeignScan</structname>ノードから非同期に1つのタプルを生成します。
...
<function>ExecAsyncRequestDone</function>や<function>ExecAsyncRequestPending</function>を使って<literal>areq</literal>に出力パラメータを設定することが推奨されています。␞␞    </para>␞
␝</programlisting>␟     Configure a file descriptor event for which the
     ...
     <function>ForeignAsyncNotify</function> will be called.␟<structname>ForeignScan</structname>ノードが待機したいファイル記述子イベントを設定します。
...
ファイルディスクリプタのイベントが発生すると、<function>ForeignAsyncNotify</function>が呼ばれます。␞␞    </para>␞
␝</programlisting>␟     Process a relevant event that has occurred, then produce one tuple
     ...
     <function>ForeignAsyncRequest</function>.␟発生した関連イベントを処理し、<structname>ForeignScan</structname>ノードから非同期に1つのタプルを生成します。
この関数は<function>ForeignAsyncRequest</function>と同じように<literal>areq</literal>に出力パラメータを設定する必要があります。␞␞    </para>␞
␝   <sect2 id="fdw-callbacks-reparameterize-paths">␟    <title>FDW Routines for Reparameterization of Paths</title>␟    <title>パスの再パラメータ化のためのFDWルーチン</title>␞␞␞
␝</programlisting>␟    This function is called while converting a path parameterized by the
    ...
    <literal>adjust_appendrel_attrs_multilevel</literal> as required.␟この関数は、<literal>child_rel</literal>で指定された子リレーションの最上位の親によってパラメータ化されたパスを、子リレーションによってパラメータ化されたパスに変換する際に呼び出されます。
この関数はパスをパラメータ化する、あるいは<structname>ForeignPath</structname>の<literal>fdw_private</literal>メンバに保存されている式ノードを変換するために使用されます。
このコールバックは必要に応じて、<literal>reparameterize_path_by_child</literal>、<literal>adjust_appendrel_attrs</literal>あるいは<literal>adjust_appendrel_attrs_multilevel</literal>を呼び出すことができます。␞␞    </para>␞
␝   <sect1 id="fdw-helpers">␟    <title>Foreign Data Wrapper Helper Functions</title>␟    <title>外部データラッパーヘルパ関数</title>␞␞␞
␝    <para>␟     Several helper functions are exported from the core server so that
     ...
     these functions.␟FDWオプションのようなFDW関連オブジェクトの属性に外部データラッパーの作者が簡単にアクセスできるように、いくつかのヘルパ関数がコアサーバからエクスポートされています。
これらの関数を使用するには、ヘッダファイル<filename>foreign/foreign.h</filename>をあなたのソースファイルにインクルードする必要があります。
このヘッダはまたこれらの関数が返す構造体も定義しています。␞␞    </para>␞
␝␟     This function returns a <structname>ForeignDataWrapper</structname>
     ...
     object.␟この関数は、与えられたOIDの外部データラッパーに対して<structname>ForeignDataWrapper</structname>オブジェクトを返します。
...
ここには<literal>FDW_MISSING_OK</literal>値を指定できて、この場合、未定義オブジェクトに対するエラーの代わりに呼び出し元に<literal>NULL</literal>結果が返されます。␞␞    </para>␞
␝␟     This function returns a <structname>ForeignDataWrapper</structname>
     object for the foreign-data wrapper with the given OID.  A
     <structname>ForeignDataWrapper</structname> object contains properties
     of the FDW (see <filename>foreign/foreign.h</filename> for details).␟この関数は指定されたOIDを持つ外部データラッパーの<structname>ForeignDataWrapper</structname>オブジェクトを返します。<structname>ForeignDataWrapper</structname>オブジェクトはFDWのプロパティを含みます(詳細は<filename>foreign/foreign.h</filename>を参照して下さい)。␞␞    </para>␞
␝␟     This function returns a <structname>ForeignServer</structname> object
     ...
     result is returned to the caller instead of an error for an undefined
     object.␟この関数は、与えられたOIDの外部サーバに対して<structname>ForeignServer</structname>オブジェクトを返します。
<structname>ForeignServer</structname>オブジェクトにはサーバの属性（詳しくは<filename>foreign/foreign.h</filename>を参照）が含まれます。
<structfield>flags</structfield>は追加的なオプション一式を示すビット毎にorを取ったビットマスクです。
ここには<literal>FSV_MISSING_OK</literal>値を指定できて、この場合、未定義オブジェクトに対するエラーの代わりに呼び出し元に<literal>NULL</literal>結果が返されます。␞␞    </para>␞
␝␟     This function returns a <structname>ForeignServer</structname> object
     for the foreign server with the given OID.  A
     <structname>ForeignServer</structname> object contains properties
     of the server (see <filename>foreign/foreign.h</filename> for details).␟この関数は指定されたOIDを持つ外部サーバの<structname>ForeignServer</structname>オブジェクトを返します。<structname>ForeignServer</structname>オブジェクトは外部サーバのプロパティを含みます(詳細は<filename>foreign/foreign.h</filename>を参照して下さい)。␞␞    </para>␞
␝␟     This function returns a <structname>UserMapping</structname> object for
     ...
     user mapping (see <filename>foreign/foreign.h</filename> for details).␟この関数は指定されたロールと指定された外部サーバのユーザマッピングの<structname>UserMapping</structname>オブジェクトを返します。(もし特定のユーザのマッピングがない場合は、<literal>PUBLIC</literal>のためのマッピングを返すか、それもなければエラーを発生させます。)
<structname>UserMapping</structname>オブジェクトはユーザマッピングのプロパティを含みます(詳細は<filename>foreign/foreign.h</filename>を参照して下さい)。␞␞    </para>␞
␝␟     This function returns a <structname>ForeignTable</structname> object for
     ...
     foreign table (see <filename>foreign/foreign.h</filename> for details).␟この関数は指定されたOIDを持つ外部テーブルの<structname>ForeignTable</structname>オブジェクトを返します。<structname>ForeignTable</structname>オブジェクトは外部テーブルのプロパティを含みます(詳細は<filename>foreign/foreign.h</filename>を参照して下さい)。␞␞    </para>␞
␝␟     This function returns the per-column FDW options for the column with the
     given foreign table OID and attribute number, in the form of a list of
     <structname>DefElem</structname>.  NIL is returned if the column has no
     options.␟この関数は指定された外部テーブルOIDと属性番号に該当する列の列単位のFDWオプションを<structname>DefElem</structname>のリスト形式で返します。その列がオプションを持たない場合はNILが返ります。␞␞    </para>␞
␝    <para>␟     Some object types have name-based lookup functions in addition to the
     OID-based ones:␟いくつかのオブジェクト種別については、OIDベースのものに加えて名前ベースの検索関数もあります。␞␞    </para>␞
␝␟     This function returns a <structname>ForeignDataWrapper</structname>
     object for the foreign-data wrapper with the given name.  If the wrapper
     is not found, return NULL if missing_ok is true, otherwise raise an
     error.␟この関数は指定された名前の外部データラッパーの<structname>ForeignDataWrapper</structname>オブジェクトを返します。外部データラッパーが見つからない場合は、missing_okがtrueの場合はNULLを返し、それ以外の場合はエラーを発生させます。␞␞    </para>␞
␝␟     This function returns a <structname>ForeignServer</structname> object
     for the foreign server with the given name.  If the server is not found,
     return NULL if missing_ok is true, otherwise raise an error.␟この関数は指定された名前の外部サーバの<structname>ForeignServer</structname>オブジェクトを返します。外部サーバが見つからない場合は、missing_okがtrueの場合はNULLを返し、それ以外の場合はエラーを発生させます。␞␞    </para>␞
␝   <sect1 id="fdw-planning">␟    <title>Foreign Data Wrapper Query Planning</title>␟    <title>外部データラッパーの問い合わせプラン作成</title>␞␞␞
␝    <para>␟     The FDW callback functions <function>GetForeignRelSize</function>,
     ...
     Here are some notes about what they must do.␟FDWコールバック関数の<function>GetForeignRelSize</function>、<function>GetForeignPaths</function>、<function>GetForeignPlan</function>、<function>PlanForeignModify</function>、<function>GetForeignJoinPaths</function>、<function>GetForeignUpperPaths</function>、<function>PlanDirectModify</function>は<productname>PostgreSQL</productname>プランナの動作と協調しなければなりません。ここでは、これらの関数がすべき事に関するいくつかの注意事項を述べます。␞␞    </para>␞
␝    <para>␟     The information in <literal>root</literal> and <literal>baserel</literal> can be used
     ...
     columns that are used in qual evaluation but not output by the query.␟<literal>root</literal>と<literal>baserel</literal>に含まれる情報は、外部テーブルから取得する必要のある情報の量(とそれによるコスト)を削減するために使用できます。
...
<literal>baserel-&gt;reltarget-&gt;exprs</literal>はどの列が取得される必要があるかを決定するのに使用できます。ただし、このリストは<structname>ForeignScan</structname>プランノードから出力すべき列しか含んでおらず、条件検査には必要だが問い合わせからは出力されない列は含まないことに注意してください。␞␞    </para>␞
␝    <para>␟     Various private fields are available for the FDW planning functions to
     keep information in.  Generally, whatever you store in FDW private fields
     should be palloc'd, so that it will be reclaimed at the end of planning.␟様々なプライベートフィールドがFDWのプラン作成関数で情報を格納する目的で利用できます。
一般的に、プラン作成の最後に回収できるように、FDW固有フィールドに格納するものは全てpallocで確保すべきです。␞␞    </para>␞
␝    <para>␟     <literal>baserel-&gt;fdw_private</literal> is a <type>void</type> pointer that is
     ...
     avoiding recalculation.␟<literal>baserel-&gt;fdw_private</literal>は、<type>void</type>ポインタで、FDWのプラン作成関数で特定の外部テーブルに関する情報を格納する目的で利用できます。
コアプランナは、<literal>RelOptInfo</literal>ノードが作成されるときにNULLで初期化するときを除いて、このフィールドに一切に触れません。
このフィールドは、<function>GetForeignRelSize</function>から<function>GetForeignPaths</function>や<function>GetForeignPaths</function>から<function>GetForeignPlan</function>といったように情報を順次伝えるの便利で、結果として再計算を省くことができます。␞␞    </para>␞
␝    <para>␟     <function>GetForeignPaths</function> can identify the meaning of different
     ...
     in the backend.␟<function>GetForeignPaths</function>では、<structname>ForeignPath</structname>ノードの<structfield>fdw_private</structfield>フィールドに固有情報を格納することで、異なるアクセスパスを区別できます。<structfield>fdw_private</structfield>は<type>List</type>ポインタとして宣言されていますが、コアプランナがこのフィールドを操作することはないため、実際にはなんでも格納できます。
しかし、バックエンドのデバッグサポート機能を利用できるように<function>nodeToString</function>でダンプ出来る形式を使うのが最良の手法です。␞␞    </para>␞
␝    <para>␟     <function>GetForeignPlan</function> can examine the <structfield>fdw_private</structfield>
     ...
     executable.␟<function>GetForeignPlan</function>では、選択された<structname>ForeignPath</structname>ノードの<structfield>fdw_private</structfield>フィールドを調べて、<structname>ForeignScan</structname>プランノード内に格納されプラン実行時に利用可能な<structfield>fdw_exprs</structfield>と<structfield>fdw_private</structfield>の二つのリストを生成することができます。
...
これらのツリーは、完全に実行可能な状態にするためにプランナによる後処理を受けます。␞␞    </para>␞
␝    <para>␟     In <function>GetForeignPlan</function>, generally the passed-in target list can
     ...
     executor doesn't waste time rechecking them.␟<function>GetForeignPlan</function>では、一般的に渡されたターゲットリストはそのままプランノードにコピーできます。
...
より複雑なFDWは内部で一部の句をチェックできるかもしれませんが、そのような場合には、エグゼキュータが再チェックのために時間を無駄にしないように、それらの句はプランノードの条件リストから削除できます。␞␞    </para>␞
␝    <para>␟     As an example, the FDW might identify some restriction clauses of the
     ...
     evaluation of the <structfield>fdw_exprs</structfield> expression tree.␟たとえば、ローカル側で評価された<replaceable>sub_expression</replaceable>の値があればリモートサーバ側で実行出来るとFDWが判断するような、<replaceable>foreign_variable</replaceable> <literal>=</literal> <replaceable>sub_expression</replaceable>といった形式の条件句をFDWが識別するかもしれません。
...
リモートサーバに送られた問い合わせは、実行時に<structfield>fdw_exprs</structfield>式ツリーを評価して得られた値をパラメータ値とする<literal>WHERE <replaceable>foreign_variable</replaceable> = $1</literal>のようなものを伴うでしょう。␞␞    </para>␞
␝    <para>␟     Any clauses removed from the plan node's qual list must instead be added
     ...
     tuple entirely.␟<literal>READ COMMITTED</literal>分離レベルでの正しい動作を保証するため、プランノードの条件リストから除かれた句はすべて、代わりに<literal>fdw_recheck_quals</literal>に追加されるか、<literal>RecheckForeignScan</literal>で再検査される必要があります。
...
なぜなら、この場合の結合タプルはタプル全体を拒絶せずに、一部のフィールドをNULLにしてしまうからです。␞␞    </para>␞
␝    <para>␟     Another <structname>ForeignScan</structname> field that can be filled by FDWs
     ...
     columns they will return.␟FDWがセットできる別の<structname>ForeignScan</structname>フィールドに<structfield>fdw_scan_tlist</structfield>があります。
...
結合プラン（<function>GetForeignJoinPaths</function>が作るパスから作成される）は、それが返す列の集合を記述する<structfield>fdw_scan_tlist</structfield>を必ず提供しなければならないことに注意して下さい。␞␞    </para>␞
␝    <para>␟     The FDW should always construct at least one path that depends only on
     ...
     same as for an ordinary restriction clause.␟FDWはそのテーブルの条件句のみに依存するパスを常に少なくとも一つは生成すべきです。結合問い合わせでは、例えば<replaceable>foreign_variable</replaceable> <literal>=</literal> <replaceable>local_variable</replaceable>といった結合句に依存するパス(群)を生成することもできます。
...
<function>GetForeignPlan</function>では、結合句の<replaceable>local_variable</replaceable>部分が<structfield>fdw_exprs</structfield>に追加され、実行時には通常の条件句と同じように動作します。␞␞    </para>␞
␝    <para>␟     If an FDW supports remote joins, <function>GetForeignJoinPaths</function> should
     ...
     (<literal>extra-&gt;restrictlist</literal>).␟FDWがリモートでの結合をサポートする場合、<function>GetForeignPaths</function>がベーステーブルに対して処理するのとほぼ同じように、<function>GetForeignJoinPaths</function>は潜在的なリモートの結合に対して<structname>ForeignPath</structname>を生成することになります。
意図した結合に関する情報は、上記と同じ方法で<function>GetForeignPlan</function>に送ることができます。
しかし、<structfield>baserestrictinfo</structfield>は結合のリレーションには関連がなく、代わりに、特定の結合に関連するJOIN句は<function>GetForeignJoinPaths</function>に別のパラメータ(<literal>extra-&gt;restrictlist</literal>)として渡されます。␞␞    </para>␞
␝    <para>␟     An FDW might additionally support direct execution of some plan actions
     ...
     come from the same FDW.␟FDWはグルーピングや集約のような、スキャンや結合のレベルより上位のプラン動作の直接実行を追加的にサポートできるかもしれません。
...
もし問い合わせの全てのベースリレーションが同じFDWから来るなら、このようなパスを生成するのに推奨される場所は、各上位リレーション（すなわち各スキャン/結合後の処理の段階）に対して呼び出される<function>GetForeignUpperPaths</function>コールバック関数の中です。␞␞    </para>␞
␝    <para>␟     <function>PlanForeignModify</function> and the other callbacks described in
     ...
     implementing <emphasis>all</emphasis> behavior of the query.␟<function>PlanForeignModify</function>と<xref linkend="fdw-callbacks-update"/>で記述された他のコールバックは、外部リレーションは通常の方法でスキャンされ、それから個別の行変更がローカルの<literal>ModifyTable</literal>プランノードで駆動されるという想定をもとに設計されています。
...
<literal>UPPERREL_FINAL</literal>に挿入されたパスは問い合わせの<emphasis>全ての</emphasis>振る舞いの実装に責任があることに留意してください。␞␞    </para>␞
␝    <para>␟     When planning an <command>UPDATE</command> or <command>DELETE</command>,
     ...
     structures that <function>copyObject</function> knows how to copy.␟<command>UPDATE</command>や<command>DELETE</command>のプランを生成しているとき、
...
<function>PlanForeignModify</function>から返される<structname>List</structname>には、<structname>ForeignScan</structname>プランノードの<structfield>fdw_private</structfield>リストと同様に、<function>copyObject</function>がコピーの仕方を知っている構造体しか保持してはいけないという制約があります。␞␞    </para>␞
␝    <para>␟     <command>INSERT</command> with an <literal>ON CONFLICT</literal> clause does not
     ...
     since the specification is mandatory there.␟<literal>ON CONFLICT</literal>句のある<command>INSERT</command>は競合の対象の指定をサポートしません。
なぜなら、リモートのテーブルの一意性制約や排他制約についての情報がローカルにはないからです。
これは結果的に<literal>ON CONFLICT DO UPDATE</literal>がサポートされないことを意味します。
なぜなら、競合の対象の指定が必須だからです。␞␞    </para>␞
␝   <sect1 id="fdw-row-locking">␟    <title>Row Locking in Foreign Data Wrappers</title>␟    <title>外部データラッパーでの行ロック</title>␞␞␞
␝    <para>␟     If an FDW's underlying storage mechanism has a concept of locking
     ...
     considerations involved in this.␟FDWの元になる記憶機構が、行の同時更新を防ぐために個々の行をロックするという概念を持っているなら、<productname>PostgreSQL</productname>の通常のテーブルで使われている意味にできる限り現実的で近い行単位のロックをFDWが実施することは価値があるでしょう。
これに関していくつかの考慮点があります。␞␞    </para>␞
␝    <para>␟     One key decision to be made is whether to perform <firstterm>early
     ...
     row, as <productname>PostgreSQL</productname> TIDs do.␟なされるべき重要な決定の一つは、<firstterm>早いロック</firstterm>を実行するか<firstterm>遅いロック</firstterm>を実行するか、です。
...
できれば、<productname>PostgreSQL</productname>のTIDがそうしているように、行識別子は行の特定のバージョンを識別できるのが望ましいです。␞␞    </para>␞
␝    <para>␟     By default, <productname>PostgreSQL</productname> ignores locking considerations
     ...
     it wishes.␟デフォルトでは<productname>PostgreSQL</productname>はFDWとのやりとりにおいてロックの考慮をしませんが、FDWはコアのコードからの明示的なサポートなしに、早いロックを実行することができます。
<productname>PostgreSQL</productname>バージョン9.5で追加された<xref linkend="fdw-callbacks-row-locking"/>に記載されたAPI関数を使うことで、望むならFDWで遅いロックを使うことも可能です。␞␞    </para>␞
␝    <para>␟     An additional consideration is that in <literal>READ COMMITTED</literal>
     ...
     first way.␟さらなる考慮点は、<literal>READ COMMITTED</literal>分離モードにおいて、<productname>PostgreSQL</productname>は対象のタプルの更新されたバージョンに対して制約と結合条件の再検査を行う必要があるかもしれないということです。
...
再フェッチの要求を満たすことができるFDWでは最初の方法を選択するのも良いでしょう。␞␞    </para>␞
␝    <para>␟     For an <command>UPDATE</command> or <command>DELETE</command> on a foreign table, it
     ...
     <function>ExecForeignUpdate</function> or <function>ExecForeignDelete</function>
     callback, but no special support is provided for this.␟外部テーブルに対する<command>UPDATE</command>や<command>DELETE</command>では、対象テーブルに対する<literal>ForeignScan</literal>操作はフェッチする行を、恐らくは<command>SELECT FOR UPDATE</command>と同等なものを用いてロックすることが推奨されます。
...
これに代わる可能性として、<function>ExecForeignUpdate</function>または<function>ExecForeignDelete</function>のコールバック内で遅いロックを実行することがありますが、これについて特別なサポートは提供されません。␞␞    </para>␞
␝    <para>␟     For foreign tables that are specified to be locked by a <command>SELECT
     ...
     its <structfield>strength</structfield> field is not <literal>LCS_NONE</literal>.␟<command>SELECT FOR UPDATE/SHARE</command>コマンドによりロックすることが指定された外部テーブルについて、<literal>ForeignScan</literal>の操作では<command>SELECT FOR UPDATE/SHARE</command>と同等なものを使ってタプルをフェッチすることで、ここでも早いロックを実行できます。
...
このとき、NULLでないrowmark構造体が戻されるかどうかだけでなく、その<structfield>strength</structfield>フィールドが<literal>LCS_NONE</literal>でないことも確認しなければなりません。␞␞    </para>␞
␝    <para>␟     Lastly, for foreign tables that are used in an <command>UPDATE</command>,
     ...
     for <literal>LCS_NONE</literal>.)␟最後に、<command>UPDATE</command>、<command>DELETE</command>または<command>SELECT FOR UPDATE/SHARE</command>コマンドで使用されたが、行ロックの指定はされなかった外部テーブルについて、ロック強度が<literal>LCS_NONE</literal>になっているときに<function>GetForeignRowMarkType</function>でオプション<literal>ROW_MARK_REFERENCE</literal>を選択すれば、すべての行を複製するというデフォルトの動作を変更することができます。
...
（<function>GetForeignRowMarkType</function>関数を使うが、ロックしていない行を再フェッチしたくない場合は、<literal>LCS_NONE</literal>についてオプション<literal>ROW_MARK_COPY</literal>を選択して下さい。）␞␞    </para>␞
␝    <para>␟     See <filename>src/include/nodes/lockoptions.h</filename>, the comments
     ...
     additional information.␟さらなる情報は、<filename>src/include/nodes/lockoptions.h</filename>、<filename>src/include/nodes/plannodes.h</filename>での<type>RowMarkType</type>と<type>PlanRowMark</type>についてのコメント、<filename>src/include/nodes/execnodes.h</filename>での<type>ExecRowMark</type>についてのコメントを参照して下さい。␞␞    </para>␞
␝␟<listitem><para><literal>AttributeRelationId</literal></para></listitem> <listitem><para><literal>AttributeRelationId</literal></para></listitem> <listitem><para><literal>ForeignDataWrapperRelationId</literal></para></listitem> <listitem><para><literal>ForeignDataWrapperRelationId</literal></para></listitem> <listitem><para><literal>ForeignServerRelationId</literal></para></listitem> <listitem><para><literal>ForeignServerRelationId</literal></para></listitem> <listitem><para><literal>ForeignTableRelationId</literal></para></listitem> <listitem><para><literal>ForeignTableRelationId</literal></para></listitem> <listitem><para><literal>UserMappingRelationId</literal></para></listitem> <listitem><para><literal>UserMappingRelationId</literal></para></listitem>␟no translation␞␞␞
