␝␟ <title>Transaction Processing</title>␟ <title>トランザクション処理</title>␞␞␞
␝ <para>␟  This chapter provides an overview of the internals of
  <productname>PostgreSQL</productname>'s transaction management system.
  The word transaction is often abbreviated as <firstterm>xact</firstterm>.␟この章では、<productname>PostgreSQL</productname>のトランザクション制御システムの内部構造について概説します。
トランザクションという単語は、しばしば<firstterm>xact</firstterm>と省略されます。␞␞ </para>␞
␝␟  <title>Transactions and Identifiers</title>␟  <title>トランザクションと識別子</title>␞␞␞
␝  <para>␟   Transactions can be created explicitly using <command>BEGIN</command>
   ...
   single-statement transactions.␟トランザクションは、<command>BEGIN</command>または<command>START TRANSACTION</command>を使用して明示的に作成し、<command>COMMIT</command>または<command>ROLLBACK</command>を使用して終了します。
明示的なトランザクションの外側のSQL文は、自動的に単一文トランザクションを使用します。␞␞  </para>␞
␝  <para>␟   Every transaction is identified by a unique
   ...
   <literal>12532</literal>.␟各トランザクションは、一意の<literal>VirtualTransactionId</literal>（あるいは<literal>virtualXID</literal>または<literal>vxid</literal>とも呼ばれます）で識別します。
これは、バックエンドのプロセス番号（または<literal>procNumber</literal>）と、<literal>localXID</literal>と呼ばれる各バックエンドに固有の連続した番号で構成されます。
たとえば、仮想トランザクションID<literal>4/12532</literal>の<literal>procNumber</literal>は<literal>4</literal>で、<literal>localXID</literal>は<literal>12532</literal>です。␞␞  </para>␞
␝  <para>␟   Non-virtual <literal>TransactionId</literal>s (or <type>xid</type>),
   ...
   only performed database reads.␟非仮想<literal>TransactionId</literal>（または<type>xid</type>）、例えば<literal>278394</literal>は、<productname>PostgreSQL</productname>クラスタ内のすべてのデータベースが使用するグローバルカウンタからトランザクションに順番に割り当てられます。
...
特に、トランザクションがデータベース読み取りのみを実行する文で開始した場合にはそうなります。␞␞  </para>␞
␝  <para>␟   The internal transaction ID type <type>xid</type> is 32 bits wide
   ...
   replication.␟内部トランザクションID型<type>xid</type>は32ビット幅で、40億トランザクションごとに<link linkend="vacuum-for-wraparound">周回</link>します。
...
XIDは<productname>PostgreSQL</productname>の<link linkend="mvcc">MVCC</link>同時実行機構とストリーミングレプリケーションの基礎として使用されます。␞␞  </para>␞
␝  <para>␟   When a top-level transaction with a (non-virtual) xid commits,
   ...
   linkend="guc-track-commit-timestamp"/> is enabled.␟（非仮想）xidを持つトップレベルのトランザクションがコミットすると、<filename>pg_xact</filename>ディレクトリにコミット済みとしてマークされます。
<xref linkend="guc-track-commit-timestamp"/>が有効な場合、追加情報が<filename>pg_commit_ts</filename>ディレクトリに記録されます。␞␞  </para>␞
␝  <para>␟   In addition to <literal>vxid</literal> and <literal>xid</literal>,
   ...
   linkend="view-pg-prepared-xacts"><structname>pg_prepared_xacts</structname></link>.␟<literal>vxid</literal>と<literal>xid</literal>に加えて、準備されたトランザクションにはグローバルトランザクション識別子（<acronym>GID</acronym>）も割り当てられます。
GIDは最大200バイト長の文字列リテラルで、現在準備されている他のトランザクション中で一意でなければなりません。
GIDからxidへのマッピングは<link linkend="view-pg-prepared-xacts"><structname>pg_prepared_xacts</structname></link>に表示されます。␞␞  </para>␞
␝␟  <title>Transactions and Locking</title>␟  <title>トランザクションとロック</title>␞␞␞
␝  <para>␟   The transaction IDs of currently executing transactions are shown in
   ...
   set in read-write transactions.␟現在実行中のトランザクションのトランザクションIDは、<link linkend="view-pg-locks"><structname>pg_locks</structname></link>の<structfield>virtualxid</structfield>と<structfield>transactionid</structfield>に表示されます。
読み出し専用のトランザクションは<structfield>virtualxid</structfield>はありますが、<structfield>transactionid</structfield>はNULLです。
一方、読み書きのトランザクションは、両方の列が設定されます。␞␞  </para>␞
␝  <para>␟   Some lock types wait on <structfield>virtualxid</structfield>,
   ...
   linkend="vacuum-for-multixact-wraparound"/>).␟ロックのタイプによって<structfield>virtualxid</structfield>を待ちますが、他のタイプのロックは<structfield>transactionid</structfield>を待ちます。
行レベルの読み書きロックはロックされた行に直接記録され、<xref linkend="pgrowlocks"/>拡張を使用して検査できます。
行レベルの読み取りロックはマルチトランザクションID（<literal>mxid</literal>。<xref linkend="vacuum-for-multixact-wraparound"/>を参照）の割当ても必要とするかもしれません。␞␞  </para>␞
␝␟  <title>Subtransactions</title>␟  <title>サブトランザクション</title>␞␞␞
␝  <para>␟   Subtransactions are started inside transactions, allowing large
   ...
   <firstterm>subxact</firstterm>.␟サブトランザクションはトランザクション内で開始され、大きなトランザクションを小さな単位に分割できます。
サブトランザクションは、親トランザクションに影響を与えることなくコミットまたは中断できるため、親トランザクションを継続できます。
これにより、アプリケーションの開発パターンによくあるエラーの処理が容易になります。
この用語は、しばしば<firstterm>subxact</firstterm>と省略されます。␞␞  </para>␞
␝  <para>␟   Subtransactions can be started explicitly using the
   ...
   the top-level transaction.␟サブトランザクションは<command>SAVEPOINT</command>コマンドを使用して明示的に開始できますが、PL/pgSQLの<literal>EXCEPTION</literal>句などの他の方法でも開始できます。
PL/PythonとPL/Tclも明示的なサブトランザクションをサポートしています。
サブトランザクションは他のサブトランザクションから開始することもできます。
トップレベルのトランザクションとその子トランザクションは階層またはツリーを形成するため、メイントランザクションをトップレベルのトランザクションと呼びます。␞␞  </para>␞
␝  <para>␟   If a subtransaction is assigned a non-virtual transaction ID,
   ...
   xid is always lower than any of its child subxids.␟サブトランザクションに仮想トランザクションID以外のIDが割り当てられた場合、そのトランザクションIDは<quote>subxid</quote>と呼ばれます。
読取専用のサブトランザクションにはsubxidは割り当てられませんが、書込みを試みると割り当てられます。
これにより、トップレベルのトランザクションを含むすべての親サブトランザクションに、非仮想トランザクションIDが割り当てられます。
親xidは常に子subxidのいずれよりも小さいことが保証されます。␞␞  </para>␞
␝  <para>␟   The immediate parent xid of each subxid is recorded in the
   ...
   for read-only subtransactions.␟各subxidの直接の親xidは<filename>pg_subtrans</filename>ディレクトリに記録されます。
トップレベルのxidには親がないのでエントリは記録されません。
読み取り専用のサブトランザクションにもエントリは記録されません。␞␞  </para>␞
␝  <para>␟   When a subtransaction commits, all of its committed child
   ...
   subtransactions will also be considered aborted.␟サブトランザクションがコミットされると、その子サブトランザクションのすべての子サブトランザクションのxidがコミットされたものとみなされます。
サブトランザクションが中断された場合、その子サブトランザクションすべても中断されたものとみなされます。␞␞  </para>␞
␝  <para>␟   When a top-level transaction with an xid commits, all of its
   ...
   even if they were subcommitted.␟xidを持つトップレベルトランザクションがコミットすると、そのサブトランザクションの子サブトランザクションもすべて<filename>pg_xact</filename>サブディレクトリに永続的にコミット済みとして記録されます。
トップレベルトランザクションが中断された場合、そのサブトランザクションは、たとえサブトランザクションがコミットされていたとしても中断されます。␞␞  </para>␞
␝  <para>␟   The more subtransactions each transaction keeps open (not
   ...
   <filename>pg_subtrans</filename>.␟トランザクションがオープンしたサブトランザクション数が多いほど（ロールバックや解放されない）、トランザクション制御のオーバーヘッドが大きくなります。
各バックエンドで最大64個のオープンしたサブトランザクションが共有メモリにキャッシュされます。
その後、<filename>pg_subtrans</filename>内のサブxidエントリの追加検索により、ストレージI/Oオーバーヘッドが大幅に増加します。␞␞  </para>␞
␝␟  <title>Two-Phase Transactions</title>␟  <title>2相トランザクション</title>␞␞␞
␝  <para>␟   <productname>PostgreSQL</productname> supports a two-phase commit (2PC)
   ...
   often used aspects.␟<productname>PostgreSQL</productname>は、複数の分散システムがトランザクション処理として連携することを可能にする2相コミット（2PC）プロトコルをサポートしています。
...
<productname>PostgreSQL</productname>は、X/Open XA標準で提案されている機能とモデルに従いますが、あまり使用されていないいくつかの側面は実装しません。␞␞  </para>␞
␝  <para>␟   When the user executes <command>PREPARE TRANSACTION</command>, the
   ...
   linkend="view-pg-prepared-xacts"><structname>pg_prepared_xacts</structname></link>.␟ユーザが<command>PREPARE TRANSACTION</command>を実行すると、次のコマンドとしては<command>COMMIT PREPARED</command>または<command>ROLLBACK PREPARED</command>しか使用できません。
...
現在準備済みのトランザクションは<link linkend="view-pg-prepared-xacts"><structname>pg_prepared_xacts</structname></link>を使用して検査できます。␞␞  </para>␞
