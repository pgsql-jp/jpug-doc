␝␟<title>Database Physical Storage</title>␟<title>データベースの物理的な格納</title>␞␞␞
␝<para>␟This chapter provides an overview of the physical storage format used by
<productname>PostgreSQL</productname> databases.␟本章では<productname>PostgreSQL</productname>データベースで使用される物理的格納書式についての概要を説明します。␞␞</para>␞
␝␟<title>Database File Layout</title>␟<title>データベースファイルのレイアウト</title>␞␞␞
␝<para>␟This section describes the storage format at the level of files and
directories.␟本節ではファイルとディレクトリというレベルで格納書式について説明します。␞␞</para>␞
␝<para>␟Traditionally, the configuration and data files used by a database
...
managed by different server instances, can exist on the same machine.␟伝統的に、データベースクラスタで利用される制御ファイルとデータファイルは、クラスタのデータディレクトリ内に一緒に格納され、通常（このディレクトリを定義するために使用できる環境変数名にちなんで）<varname>PGDATA</varname>として参照されます。
通常の<varname>PGDATA</varname>の位置は<filename>/var/lib/pgsql/data</filename>です。
異なるサーバインスタンスによって管理することで、複数のクラスタを同一のマシン上に存在させることができます。␞␞</para>␞
␝<para>␟The <varname>PGDATA</varname> directory contains several subdirectories and control
files, as shown in <xref linkend="pgdata-contents-table"/>.  In addition to
these required items, the cluster configuration files
<filename>postgresql.conf</filename>, <filename>pg_hba.conf</filename>, and
<filename>pg_ident.conf</filename> are traditionally stored in
<varname>PGDATA</varname>, although it is possible to place them elsewhere.␟<xref linkend="pgdata-contents-table"/>に示すように、<varname>PGDATA</varname>ディレクトリには数個のサブディレクトリと制御ファイルがあります。
これら必要な項目に加え、クラスタの設定ファイルである<filename>postgresql.conf</filename>、<filename>pg_hba.conf</filename>および<filename>pg_ident.conf</filename>が、他の場所にも置くことができますが、伝統的に<varname>PGDATA</varname>内に格納されます。␞␞</para>␞
␝<table tocentry="1" id="pgdata-contents-table">␟<title>Contents of <varname>PGDATA</varname></title>␟<title><varname>PGDATA</varname>の内容</title>␞␞<tgroup cols="2">␞
␝<entry>␟Item␟項目␞␞</entry>␞
␝</entry>␟<entry>Description</entry>␟<entry>説明</entry>␞␞</row>␞
␝ <entry><filename>PG_VERSION</filename></entry>␟ <entry>A file containing the major version number of <productname>PostgreSQL</productname></entry>␟ <entry><productname>PostgreSQL</productname>の主バージョン番号を保有するファイル</entry>␞␞</row>␞
␝ <entry><filename>base</filename></entry>␟ <entry>Subdirectory containing per-database subdirectories</entry>␟ <entry>データベースごとのサブディレクトリを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>current_logfiles</filename></entry>␟ <entry>File recording the log file(s) currently written to by the logging
  collector</entry>␟ <entry>ログ収集機構が現在書き込んでいるログファイルを記録するファイル</entry>␞␞</row>␞
␝ <entry><filename>global</filename></entry>␟ <entry>Subdirectory containing cluster-wide tables, such as
 <structname>pg_database</structname></entry>␟ <entry><structname>pg_database</structname>のようなクラスタで共有するテーブルを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>pg_commit_ts</filename></entry>␟ <entry>Subdirectory containing transaction commit timestamp data</entry>␟ <entry>トランザクションのコミット時刻のデータを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>pg_dynshmem</filename></entry>␟ <entry>Subdirectory containing files used by the dynamic shared memory
  subsystem</entry>␟ <entry>動的共有メモリサブシステムで使われるファイルを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>pg_logical</filename></entry>␟ <entry>Subdirectory containing status data for logical decoding</entry>␟ <entry>ロジカルデコーディングのための状態データを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>pg_multixact</filename></entry>␟ <entry>Subdirectory containing multitransaction status data
  (used for shared row locks)</entry>␟ <entry>マルチトランザクションの状態のデータを保有するサブディレクトリ（共有行ロックで使用されます）</entry>␞␞</row>␞
␝ <entry><filename>pg_notify</filename></entry>␟ <entry>Subdirectory containing LISTEN/NOTIFY status data</entry>␟ <entry>LISTEN/NOTIFY状態データを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>pg_replslot</filename></entry>␟ <entry>Subdirectory containing replication slot data</entry>␟ <entry>レプリケーションスロットデータを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>pg_serial</filename></entry>␟ <entry>Subdirectory containing information about committed serializable transactions</entry>␟ <entry>コミットされたシリアライザブルトランザクションに関する情報を保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>pg_snapshots</filename></entry>␟ <entry>Subdirectory containing exported snapshots</entry>␟ <entry>エクスポートされたスナップショットを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>pg_stat</filename></entry>␟ <entry>Subdirectory containing permanent files for the statistics
  subsystem</entry>␟ <entry>統計サブシステム用の永続ファイルを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>pg_stat_tmp</filename></entry>␟ <entry>Subdirectory containing temporary files for the statistics
  subsystem</entry>␟ <entry>統計サブシステム用の一時ファイルを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>pg_subtrans</filename></entry>␟ <entry>Subdirectory containing subtransaction status data</entry>␟ <entry>サブトランザクションの状態のデータを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>pg_tblspc</filename></entry>␟ <entry>Subdirectory containing symbolic links to tablespaces</entry>␟ <entry>テーブル空間へのシンボリックリンクを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>pg_twophase</filename></entry>␟ <entry>Subdirectory containing state files for prepared transactions</entry>␟ <entry>プリペアドトランザクション用の状態ファイルを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>pg_wal</filename></entry>␟ <entry>Subdirectory containing WAL (Write Ahead Log) files</entry>␟ <entry> WAL（先行書き込みログ）ファイルを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>pg_xact</filename></entry>␟ <entry>Subdirectory containing transaction commit status data</entry>␟ <entry>トランザクションのコミット状態のデータを保有するサブディレクトリ</entry>␞␞</row>␞
␝ <entry><filename>postgresql.auto.conf</filename></entry>␟ <entry>A file used for storing configuration parameters that are set by
<command>ALTER SYSTEM</command></entry>␟ <entry><command>ALTER SYSTEM</command>により設定された設定パラメータを格納するのに使われるファイル</entry>␞␞</row>␞
␝ <entry><filename>postmaster.opts</filename></entry>␟ <entry>A file recording the command-line options the server was
last started with</entry>␟ <entry>最後にサーバを起動した時のコマンドラインオプションを記録するファイル</entry>␞␞</row>␞
␝ <entry><filename>postmaster.pid</filename></entry>␟ <entry>A lock file recording the current postmaster process ID (PID),
  ...
  (this file is not present after server shutdown)</entry>␟ <entry>
...
␝<para>␟For each database in the cluster there is a subdirectory within
...
there.␟クラスタ内の各データベースに対して、<varname>PGDATA</varname><filename>/base</filename>内にサブディレクトリが存在し、サブディレクトリ名は<structname>pg_database</structname>内のデータベースOIDとなります。
このサブディレクトリはデータベースファイルのデフォルトの位置であり、特にシステムカタログがそこに格納されます。␞␞</para>␞
␝<para>␟ Note that the following sections describe the behavior of the builtin
 ...
 access methods might work differently.␟以下の節では、組み込みの<literal>heap</literal><link linkend="tableam">テーブルアクセスメソッド</link>と組み込みの<link linkend="indexam">インデックスアクセスメソッド</link>の振舞いを説明していることに注意してください。
<productname>PostgreSQL</productname>の拡張性のため、他のアクセスメソッドは異なる動作をするかもしれません。␞␞</para>␞
␝<para>␟Each table and index is stored in a separate file.  For ordinary relations,
...
with the suffix <literal>_init</literal> (see <xref linkend="storage-init"/>).␟各テーブルおよびインデックスは別個のファイルに格納されます。
...
ここで<replaceable>BBB</replaceable>はファイルを生成したバックエンドのプロセス番号、<replaceable>FFF</replaceable>はファイルノード番号です。
...
ログを取らないテーブルとインデックスは、初期化フォークという第３のフォークを持ち、フォークに接尾辞<literal>_init</literal>を付けたファイルに格納します（<xref linkend="storage-init"/>参照）。␞␞</para>␞
␝<para>␟Note that while a table's filenode often matches its OID, this is
...
function.␟テーブルにおけるファイルノード番号とOIDは多くの場合一致しますが、常に一致するとは<emphasis>限らない</emphasis>ことに注意してください。
...
これらのカタログの実際のファイルノード番号は低レベルなデータ構造内に保管されており、<function>pg_relation_filenode()</function>関数を使用して入手できます。␞␞</para>␞
␝<para>␟When a table or index exceeds 1 GB, it is divided into gigabyte-sized
...
segments as well, though this is unlikely to happen in practice.␟テーブルまたはインデックスが１ギガバイトを超えると、ギガバイト単位の<firstterm>セグメント</firstterm>に分割されます。
...
原理上、空き領域マップと可視性マップのフォークにおいても複数のセグメントも必要とする可能性がありますが、実際のところは起こりそうにありません。␞␞</para>␞
␝<para>␟A table that has columns with potentially large entries will have an
...
See <xref linkend="storage-toast"/> for more information.␟項目が大きくなりそうな列を持ったテーブルは、連携した<firstterm>TOAST</firstterm>テーブルを保有する可能性があります。
...
<xref linkend="storage-toast"/>を参照してください。␞␞</para>␞
␝<para>␟The contents of tables and indexes are discussed further in
<xref linkend="storage-page-layout"/>.␟テーブルおよびインデックスの内容は、<xref linkend="storage-page-layout"/>においてさらに考察されています。␞␞</para>␞
␝<para>␟Tablespaces make the scenario more complicated.  Each user-defined tablespace
...
<varname>PGDATA</varname><filename>/global</filename>.␟テーブル空間は状況をさらに複雑にします。
...
同様に、<literal>pg_global</literal>テーブル空間は<filename>pg_tblspc</filename>を通してアクセスされるのではなく、<varname>PGDATA</varname><filename>/global</filename>と連携します。␞␞</para>␞
␝<para>␟The <function>pg_relation_filepath()</function> function shows the entire path
...
the files associated with the relation.␟<function>pg_relation_filepath()</function>関数は任意のリレーションの(<varname>PGDATA</varname>から相対的な)パス全体を示します。
これは上の規則の多くを記憶する必要がありませんので、しばしば有用です。
しかし、この関数がリレーションの主フォークの最初のセグメントの名前だけを返すことに注意して下さい。
リレーションに関したすべてのファイルを見つけるためにセグメント番号や<literal>_fsm</literal>や<literal>_vm</literal>、<literal>_init</literal>を追加する必要があるかもしれません。␞␞</para>␞
␝    <indexterm><primary>sliced bread</primary><see>TOAST</see></indexterm>
␟␟    <indexterm><primary>スライスパン</primary><see>TOAST</see></indexterm>␞␞</para>␞
␝<para>␟Temporary files (for operations such as sorting more data than can fit in
...
<replaceable>NNN</replaceable> distinguishes different temporary files of that backend.␟一時ファイル（メモリ内に収まりきらないデータのソートなどの操作用）は<varname>PGDATA</varname><filename>/base/pgsql_tmp</filename>内、または、<literal>pg_default</literal>以外のテーブル空間が指定されていた場合はテーブル空間ディレクトリ下の<filename>pgsql_tmp</filename>サブディレクトリ内に作成されます。
一時ファイルの名前は<filename>pgsql_tmp<replaceable>PPP</replaceable>.<replaceable>NNN</replaceable></filename>という形式です。
ここで、<replaceable>PPP</replaceable>は所有するバックエンドのPIDであり、<replaceable>NNN</replaceable>で同一バックエンドで作成された別の一時ファイルと区別します。␞␞</para>␞
␝<para>␟This section provides an overview of <acronym>TOAST</acronym> (The
Oversized-Attribute Storage Technique).␟本節では<acronym>TOAST</acronym>（過大属性格納技法：The Oversized-Attribute Storage Technique）の概要について説明します。␞␞</para>␞
␝<para>␟<productname>PostgreSQL</productname> uses a fixed page size (commonly
...
large data values in-memory.␟<productname>PostgreSQL</productname>は固定長のページサイズ（通常8キロバイト）を使用し、複数ページにまたがるタプルを許しません。
...
<acronym>TOAST</acronym>の基盤は大きなデータ値のインメモリで処理の改善にも使用されています。␞␞</para>␞
␝<para>␟Only certain data types support <acronym>TOAST</acronym> &mdash; there is no need to
...
See <xref linkend="xtypes-toast"/> for more detail.)␟一部のデータ型のみが<acronym>TOAST</acronym>をサポートします。
...
詳しくは<xref linkend="xtypes-toast"/>を参照してください。）␞␞</para>␞
␝<para>␟<acronym>TOAST</acronym> usurps two bits of the varlena length word (the high-order
...
the content of the <acronym>TOAST</acronym> pointer tells that, instead.␟<acronym>TOAST</acronym>はvarlenaの長さワードの2ビット(ビッグエンディアンのマシンでは上位ビット、リトルエンディアンのマシンでは下位ビット)を勝手に使用します。
...
その代わり<acronym>TOAST</acronym>ポインタの内容にこの情報が含まれています。␞␞</para>␞
␝<para>␟The compression technique used for either in-line or out-of-line compressed
...
inserted.␟行内あるいは行外の圧縮データで使用される圧縮技術は、<command>CREATE TABLE</command>または<command>ALTER TABLE</command>で<literal>COMPRESSION</literal>列オプションを設定することで各列に対して選択できます。
明示的な設定のない列に対するデフォルトは、データが挿入されるときに<xref linkend="guc-default-toast-compression"/>パラメータを参照することです。␞␞</para>␞
␝<para>␟As mentioned, there are multiple types of <acronym>TOAST</acronym> pointer datums.
...
Further details appear in <xref linkend="storage-toast-inmemory"/>.␟前に触れたように、<acronym>TOAST</acronym>ポインタデータにはいくつかの型があります。
...
更なる詳細は<xref linkend="storage-toast-inmemory"/>に記述されています。␞␞</para>␞
␝<sect2 id="storage-toast-ondisk">␟ <title>Out-of-Line, On-Disk TOAST Storage</title>␟ <title>行外ディスク上のTOAST格納</title>␞␞␞
␝<para>␟If any of the columns of a table are <acronym>TOAST</acronym>-able, the table will
...
described in more detail below.␟テーブルの列に1つでも<acronym>TOAST</acronym>可能なものがあれば、そのテーブルには連携した<acronym>TOAST</acronym>テーブルがあり、そのOIDがテーブルの<structname>pg_class</structname>.<structfield>reltoastrelid</structfield>エントリに格納されます。
ディスク上の<acronym>TOAST</acronym>化された値は以下で詳しく説明する通り、<acronym>TOAST</acronym>テーブルに保持されます。␞␞</para>␞
␝<para>␟Out-of-line values are divided (after compression if used) into chunks of at
...
bytes regardless of the actual size of the represented value.␟行外の値は（圧縮される場合は圧縮後に）最大<symbol>TOAST_MAX_CHUNK_SIZE</symbol>バイトの塊に分割されます
...
varlenaヘッダバイトに収納するためにディスク上の<acronym>TOAST</acronym>ポインタデータの総サイズは、表現される値の実サイズに関係なく、18バイトになります。␞␞</para>␞
␝<para>␟The <acronym>TOAST</acronym> management code is triggered only
...
none of the out-of-line values change.␟<acronym>TOAST</acronym>管理のコードは、テーブル内に格納される値が<symbol>TOAST_TUPLE_THRESHOLD</symbol>バイト（通常2キロバイト）を超える時にのみ実行されます。
...
行外の値を持つ行の更新では、行外の値の変更がなければ<acronym>TOAST</acronym>するコストはかかりません。␞␞</para>␞
␝<para>␟The <acronym>TOAST</acronym> management code recognizes four different strategies
for storing <acronym>TOAST</acronym>-able columns on disk:␟<acronym>TOAST</acronym>管理のコードでは、ディスク上に<acronym>TOAST</acronym>可能な列を格納するために、以下の4つの異なる戦略を認識します。␞␞␞
␝     <para>␟      <literal>PLAIN</literal> prevents either compression or
      out-of-line storage.  This is the only possible strategy for
      columns of non-<acronym>TOAST</acronym>-able data types.␟<literal>PLAIN</literal>は圧縮や行外の格納を防止します。
これは<acronym>TOAST</acronym>化不可能のデータ型の列に対してのみ取り得る戦略です。␞␞     </para>␞
␝     <para>␟      <literal>EXTENDED</literal> allows both compression and out-of-line
      ...
      the row is still too big.␟<literal>EXTENDED</literal>では、圧縮と行外の格納を許します。
これはほとんどの<acronym>TOAST</acronym>可能のデータ型のデフォルトです。
圧縮がまず行われ、それでも行が大き過ぎるのであれば行外の格納をします。␞␞     </para>␞
␝     <para>␟      <literal>EXTERNAL</literal> allows out-of-line storage but not
      ...
      required parts of the out-of-line value when it is not compressed.␟<literal>EXTERNAL</literal>は非圧縮の行外の格納を許します。
...
（格納領域が増加するという欠点があります）。␞␞     </para>␞
␝     <para>␟      <literal>MAIN</literal> allows compression but not out-of-line
      ...
      way to make the row small enough to fit on a page.)␟<literal>MAIN</literal>は圧縮を許しますが、行外の格納はできません。
（実際にはこうした列についても行外の格納は行われます。
しかし、他に行を縮小させページに合わせる方法がない場合の最後の手段としてのみです。）␞␞     </para>␞
...
with <link linkend="sql-altertable"><command>ALTER TABLE ... SET STORAGE</command></link>.␟<acronym>TOAST</acronym>可能なデータ型はそれぞれ、そのデータ型の列用のデフォルトの戦略を指定します。
しかし<link linkend="sql-altertable"><command>ALTER TABLE ... SET STORAGE</command></link>を使用して、あるテーブル列の戦略を変更することができます。␞␞</para>␞
␝<para>␟<symbol>TOAST_TUPLE_TARGET</symbol> can be adjusted for each table using
<link linkend="sql-altertable"><command>ALTER TABLE ... SET (toast_tuple_target = N)</command></link>␟<symbol>TOAST_TUPLE_TARGET</symbol>は<link linkend="sql-altertable"><command>ALTER TABLE ... SET (toast_tuple_target = N)</command></link>を使って各テーブルで調整できます。␞␞</para>␞
␝<para>␟This scheme has a number of advantages compared to a more straightforward
...
comparison table, in which all the HTML pages were cut down to 7 kB to fit.␟この機構には、ページをまたがる行の値を許可するといった素直な手法に比べて多くの利点があります。
...
すべてのHTMLページを7キロバイト程度に切り詰めた<acronym>TOAST</acronym>化されない比較用テーブルと比べ、実行時間に違いはありませんでした。␞␞</para>␞
␝<sect2 id="storage-toast-inmemory">␟ <title>Out-of-Line, In-Memory TOAST Storage</title>␟ <title>行外インメモリのTOAST格納</title>␞␞␞
...
pointers to <firstterm>expanded</firstterm> data.␟<acronym>TOAST</acronym>ポインタは、ディスク上にあるデータだけでなく、現在のサーバプロセスのメモリ内の場所を指すこともできます。
そのようなポインタは明らかに短命ですが、それでも有用です。
現在のところ、<firstterm>間接</firstterm>データへのポインタと、<firstterm>展開</firstterm>データへのポインタの2つのケースがあります。␞␞</para>␞
␝<para>␟Indirect <acronym>TOAST</acronym> pointers simply point at a non-indirect varlena
...
and there is no infrastructure to help with this.␟間接<acronym>TOAST</acronym>ポインタは、単にメモリ上のどこかに格納されている間接的でないvarlena値を指すだけです。
このケースは元々は単なる概念実証として作られたのですが、現在はロジカルデコーディング時に1GBを越える物理的タプルを作成する可能性を防ぐために使用されています。
（すべての行外フィールド値をタプルに持ってこようとすると、そうなるかもしれません。）
このケースでは、ポインタデータの作成者はポインタが存在可能な限り参照データが存在し続けることに全責任を負うため、利用が限られ、またこれを支援するための基盤もありません。␞␞</para>␞
␝<para>␟Expanded <acronym>TOAST</acronym> pointers are useful for complex data types
...
representation can be introduced incrementally, one function at a time.␟展開<acronym>TOAST</acronym>ポインタは、ディスク上の表現が計算目的にあまり適さない複雑なデータ型で有用です。
...
従って、展開表現のサポートは徐々に、1回に1つの関数だけ導入することができます。␞␞</para>␞
␝<para>␟<acronym>TOAST</acronym> pointers to expanded values are further broken down
...
unnecessary copying of expanded values during query execution.␟展開された値への<acronym>TOAST</acronym>ポインタは、さらに<firstterm>読み書き</firstterm>のポインタと<firstterm>読み取りのみ</firstterm>のポインタに分類されます。
...
この区別と、関連したいくつかの慣習により、問い合わせの実行時に展開された値を不必要に複製するのを避けることが可能になります。␞␞</para>␞
␝<para>␟For all types of in-memory <acronym>TOAST</acronym> pointer, the <acronym>TOAST</acronym>
...
tuple would otherwise be too big.␟すべてのタイプのインメモリの<acronym>TOAST</acronym>ポインタについて、<acronym>TOAST</acronym>管理のコードはそのようなポインタデータが偶然、ディスクに保存されてしまうことが決して起こらないようにします。
インメモリの<acronym>TOAST</acronym>ポインタは保存される前に自動的に展開されて通常の行内のvarlena値になります。
その後、含んでいるタプルが大きすぎるような時には、ディスク上の<acronym>TOAST</acronym>ポインタに変換されることもあります。␞␞</para>␞
␝<indexterm>
...
␝␟<title>Free Space Map</title>␟<title>空き領域マップ</title>␞␞␞
␝<para>␟Each heap and index relation, except for hash indexes, has a Free Space Map
...
<filename>12345_fsm</filename>, in the same directory as the main relation file.␟ヒープとハッシュインデックス以外のインデックスリレーションはそれぞれ、そのリレーション内で利用可能な領域を継続して追跡するために、空き領域マップ（<acronym>FSM</acronym>）を持ちます。
これは、個々のリレーションのフォーク内の主リレーションデータに沿って、リレーションのファイルノード番号に<literal>_fsm</literal>という接尾辞を付けた名前のファイルに格納されます。
例えばリレーションのファイルノードが12345であれば、<acronym>FSM</acronym>は主リレーションファイルと同じディレクトリ内の<filename>12345_fsm</filename>という名前のファイルに格納されます。␞␞</para>␞
␝<para>␟The Free Space Map is organized as a tree of <acronym>FSM</acronym> pages. The
...
levels aggregate information from the lower levels.␟空き領域マップは<acronym>FSM</acronym>ページのツリーとして編成されます。
最下位レベルの<acronym>FSM</acronym>ページはすべてのヒープ（またはインデックス）ページで利用可能な空き領域を、各ページ毎に1バイト使用して格納します。
上位レベルは下位レベルからの情報を集約します。␞␞</para>␞
␝<para>␟Within each <acronym>FSM</acronym> page is a binary tree, stored in an array with
...
at the root.␟各<acronym>FSM</acronym>ページの内部はノード当たり1バイトを持つ配列内に格納されたバイナリツリーです。
各リーフノードはヒープページ、または下位レベルの<acronym>FSM</acronym>ページを表現します。
各非リーフノード内には、子の値より大きな値が格納されます。
したがってリーフノード内の最大値がルートに格納されます。␞␞</para>␞
␝<para>␟See <filename>src/backend/storage/freespace/README</filename> for more details on
how the <acronym>FSM</acronym> is structured, and how it's updated and searched.
The <xref linkend="pgfreespacemap"/> module
can be used to examine the information stored in free space maps.␟<acronym>FSM</acronym>がどのように構築されるか、そしてどのように更新、検索されるかに関する詳細は<filename>src/backend/storage/freespace/README</filename>を参照してください。
<xref linkend="pgfreespacemap"/>モジュールを使用して、空き領域マップに格納された情報を調べることができます。␞␞</para>␞
␝<indexterm>
...
␝␟<title>Visibility Map</title>␟<title>可視性マップ</title>␞␞␞
␝<para>␟Each heap relation has a Visibility Map
...
Note that indexes do not have VMs.␟各ヒープリレーションは、どのページがすべての実行中のトランザクションから可視であることが分かっているタプルだけを含むかを追跡する、可視性マップ（VM）を持ちます。どのページが凍結状態のタプルだけを含むのかも追跡します。
...
インデックスはVMを持たないことに注意してください。␞␞</para>␞
␝<para>␟The visibility map stores two bits per heap page.  The first bit, if set,
...
That means that even an anti-wraparound vacuum need not revisit the page.␟可視性マップはヒープページ当たり2ビットを保持します。
...
これは、周回対策のバキュームすらそのページを再び訪れる必要はないことを意味します。␞␞</para>␞
␝<para>␟The map is conservative in the sense that we make sure that whenever a bit is
...
cleared by any data-modifying operations on a page.␟このマップは、ビットがセットされている時は常にこの条件が真であることを確実に把握できるという点で保守的ですが、ビットがセットされていない場合は、真かもしれませんし偽かもしれません。
可視性マップのビットはバキュームによってのみで設定されます。
しかしページに対する任意のデータ編集操作によってクリアされます。␞␞</para>␞
␝<para>␟The <xref linkend="pgvisibility"/> module can be used to examine the
information stored in the visibility map.␟<xref linkend="pgvisibility"/>モジュールは可視性マップに入っている情報を確かめるのに使えます。␞␞</para>␞
␝<indexterm>
...
␝␟<title>The Initialization Fork</title>␟<title>初期化フォーク</title>␞␞␞
␝<para>␟Each unlogged table, and each index on an unlogged table, has an initialization
...
erased (they will be recreated automatically as needed).␟ログを取らないテーブルと、ログを取らないテーブルに対するインデックスは、それぞれ初期化フォークを持ちます。
初期化フォークとは適切な種類の空テーブルと空インデックスです。
ログを取らないテーブルをクラッシュのために再度空にしなければならない場合、初期化フォークで主フォーク全体をコピーし、その他のフォークは消去されます（これらは必要に応じて自動的に再作成されます）。␞␞</para>␞
␝␟<title>Database Page Layout</title>␟<title>データベースページのレイアウト</title>␞␞␞
␝<para>␟This section provides an overview of the page format used within
<productname>PostgreSQL</productname> tables and indexes.<footnote>␟本節では<productname>PostgreSQL</productname>のテーブルおよびインデックスで使われるページ書式の概略について説明します。
<footnote>␞␞  <para>␞
␝  <para>␟    Actually, use of this page format is not required for either table or
    ...
    the item layout rules.␟実際には、テーブルアクセスメソッドもインデックスアクセスメソッドも、このページ書式を使用する必要はありません。
<literal>heap</literal>テーブルアクセスメソッドは常にこの書式を使用します。
既存のすべてのインデックスメソッドも、この基本書式を使用しています。しかし、インデックスメタページに保持されるデータは通常、アイテムレイアウト規則に従っていません。␞␞  </para>␞
␝</footnote>␟Sequences and <acronym>TOAST</acronym> tables are formatted just like a regular table.␟<acronym>TOAST</acronym>のテーブルとシーケンスは、通常のテーブルと同様に整形されています。␞␞</para>␞
␝<para>␟In the following explanation, a
<firstterm>byte</firstterm>
is assumed to contain 8 bits.  In addition, the term
<firstterm>item</firstterm>
refers to an individual data value that is stored on a page.  In a table,
an item is a row; in an index, an item is an index entry.␟以下の説明では1<firstterm>バイト</firstterm>は8ビットからなることを前提としています。
さらに、<firstterm>アイテム</firstterm>という単語は、ページに格納される個別のデータ値のことを指しています。
テーブル内ではアイテムは行であり、インデックス内ではアイテムはインデックスのエントリです。␞␞</para>␞
␝<para>␟Every table and index is stored as an array of <firstterm>pages</firstterm> of a
...
within the index, depending on the index access method.␟テーブルとインデックスはすべて、固定サイズ（通常8キロバイト。サーバのコンパイル時に異なるサイズを設定可能）の<firstterm>ページ</firstterm>の集まりとして格納されます。
...
また、インデックスではインデックスアクセスメソッドに依存した様々なページ種類があります。␞␞</para>␞
␝<para>␟<xref linkend="page-table"/> shows the overall layout of a page.
There are five parts to each page.␟<xref linkend="page-table"/>はページの全体的なレイアウトを示しています。
各ページには5つの部分があります。␞␞</para>␞
␝<table tocentry="1" id="page-table">␟<title>Overall Page Layout</title>
<titleabbrev>Page Layout</titleabbrev>␟<title>ページレイアウト全体</title>
<titleabbrev>ページレイアウト</titleabbrev>␞␞<tgroup cols="2">␞
␝<entry>␟Item␟アイテム␞␞</entry>␞
␝</entry>␟<entry>Description</entry>␟<entry>説明</entry>␞␞</row>␞
␝ <entry>PageHeaderData</entry>␟ <entry>24 bytes long. Contains general information about the page, including
free space pointers.</entry>␟ <entry>長さは24バイト。空き領域ポインタを含む、ページについての一般情報です。</entry>␞␞</row>␞
␝<entry>ItemIdData</entry>␟<entry>Array of item identifiers pointing to the actual items. Each
entry is an (offset,length) pair. 4 bytes per item.</entry>␟<entry>実際のアイテムを指すアイテム識別子の配列です。
各項目は（オフセットと長さの）ペアです。
1アイテムにつき4バイトです。</entry>␞␞</row>␞
␝<row>␟<entry>Free space</entry>
<entry>The unallocated space. New item identifiers are allocated from
the start of this area, new items from the end.</entry>␟<entry>空き領域</entry>
<entry>割り当てられていない空間です。
新規のアイテム識別子はこの領域の先頭から、新規のアイテムは最後から割り当てられます。
</entry>␞␞</row>␞
␝<row>␟<entry>Items</entry>␟<entry>アイテム</entry>␞␞␞
␝<entry>Items</entry>␟<entry>The actual items themselves.</entry>␟<entry>実際のアイテムそのものです。</entry>␞␞</row>␞
␝<row>␟<entry>Special space</entry>
<entry>Index access method specific data. Different methods store different
data. Empty in ordinary tables.</entry>␟<entry>特別な空間</entry>
<entry>インデックスアクセスメソッド特有のデータです。異なるメソッドは異なるデータを格納します。通常のテーブルでは空です。</entry>␞␞</row>␞
␝␟  The first 24 bytes of each page consists of a page header
  ...
  to be profitable: it tracks the oldest un-pruned XMAX on the page.␟それぞれのページの最初の24バイトはページヘッダ(<structname>PageHeaderData</structname>)から構成されています。
...
␝ <table tocentry="1" id="pageheaderdata-table">␟ <title>PageHeaderData Layout</title>
 <titleabbrev>PageHeaderData Layout</titleabbrev>␟ <title>PageHeaderDataのレイアウト</title>
 <titleabbrev>PageHeaderDataのレイアウト</titleabbrev>␞␞ <tgroup cols="4">␞
␝  <row>␟   <entry>Field</entry>␟   <entry>フィールド</entry>␞␞␞
␝   <entry>Field</entry>␟   <entry>Type</entry>␟   <entry>型</entry>␞␞␞
␝   <entry>Type</entry>␟   <entry>Length</entry>␟   <entry>長さ</entry>␞␞␞
␝   <entry>Length</entry>␟   <entry>Description</entry>␟   <entry>説明</entry>␞␞  </row>␞
␝   <entry>PageXLogRecPtr</entry>␟   <entry>8 bytes</entry>␟   <entry>8バイト</entry>␞␞␞
␝   <entry>8 bytes</entry>␟   <entry>LSN: next byte after last byte of WAL record for last change
   to this page</entry>␟   <entry>LSN: このページへの最終変更に対応するWALレコードの最後のバイトの次のバイト</entry>␞␞  </row>␞
␝   <entry>uint16</entry>␟   <entry>2 bytes</entry>␟   <entry>2バイト</entry>␞␞␞
␝   <entry>2 bytes</entry>␟   <entry>Page checksum</entry>␟   <entry>ページチェックサム</entry>␞␞  </row>␞
␝   <entry>uint16</entry>␟   <entry>2 bytes</entry>␟   <entry>2バイト</entry>␞␞␞
␝   <entry>2 bytes</entry>␟   <entry>Flag bits</entry>␟   <entry>フラグビット</entry>␞␞  </row>␞
␝   <entry>LocationIndex</entry>␟   <entry>2 bytes</entry>␟   <entry>2バイト</entry>␞␞␞
␝   <entry>2 bytes</entry>␟   <entry>Offset to start of free space</entry>␟   <entry>空き領域の始まりに対するオフセット</entry>␞␞  </row>␞
␝   <entry>LocationIndex</entry>␟   <entry>2 bytes</entry>␟   <entry>2バイト</entry>␞␞␞
␝   <entry>2 bytes</entry>␟   <entry>Offset to end of free space</entry>␟   <entry>空き領域の終わりに対するオフセット</entry>␞␞  </row>␞
␝   <entry>LocationIndex</entry>␟   <entry>2 bytes</entry>␟   <entry>2バイト</entry>␞␞␞
␝   <entry>2 bytes</entry>␟   <entry>Offset to start of special space</entry>␟   <entry>特別な空間の始まりに対するオフセット</entry>␞␞  </row>␞
␝   <entry>uint16</entry>␟   <entry>2 bytes</entry>␟   <entry>2バイト</entry>␞␞␞
␝   <entry>2 bytes</entry>␟   <entry>Page size and layout version number information</entry>␟   <entry>ページサイズおよびレイアウトのバージョン番号の情報</entry>␞␞  </row>␞
␝   <entry>TransactionId</entry>␟   <entry>4 bytes</entry>␟   <entry>4バイト</entry>␞␞␞
␝   <entry>4 bytes</entry>␟   <entry>Oldest unpruned XMAX on page, or zero if none</entry>␟   <entry>ページ上でもっとも古い切り詰められていないXMAX。存在しなければゼロ。</entry>␞␞  </row>␞
␝ <para>␟  All the details can be found in
  <filename>src/include/storage/bufpage.h</filename>.␟詳細情報については<filename>src/include/storage/bufpage.h</filename>を参照してください。␞␞ </para>␞
␝ <para>␟  Following the page header are item identifiers
  ...
  index of an item identifier.␟ページヘッダに続くのはアイテム識別子（<type>ItemIdData</type>）です。
識別子ごとに4バイトを必要とします。
...
␝␟  The items themselves are stored in space allocated backwards from the end
  ...
  <type>HeapTupleHeaderData</type>, described below.␟アイテム自体は、未割り当て空間の最後から順番に割り当てられた空間に格納されます。
...
␝␟  The final section is the <quote>special section</quote> which can
  ...
  <structfield>pd_special</structfield> to equal the page size).␟最後のセクションは、アクセスメソッドが格納しようとするものを何でも含めることのできる<quote>特別なセクション</quote>です。
...
␝ <para>␟  <xref linkend="storage-page-layout-figure"/> illustrates how these parts are
  laid out in a page.␟<xref linkend="storage-page-layout-figure"/>は、これらの部分がページ内でどのようにレイアウトされているかを図解しています。␞␞ </para>␞
␝ <figure id="storage-page-layout-figure">␟  <title>Page Layout</title>␟  <title>ページレイアウト</title>␞␞  <mediaobject>␞
␝␟ <title>Table Row Layout</title>␟ <title>テーブル行のレイアウト</title>␞␞␞
␝␟  All table rows are structured in the same way. There is a fixed-size
  ...
  suitably aligned.)␟テーブル行はすべて同じ方法で構成されています。
...
␝ <table tocentry="1" id="heaptupleheaderdata-table">␟ <title>HeapTupleHeaderData Layout</title>
 <titleabbrev>HeapTupleHeaderData Layout</titleabbrev>␟ <title>HeapTupleHeaderDataのレイアウト</title>
 <titleabbrev>HeapTupleHeaderDataのレイアウト</titleabbrev>␞␞ <tgroup cols="4">␞
␝  <row>␟   <entry>Field</entry>␟   <entry>フィールド</entry>␞␞␞
␝   <entry>Field</entry>␟   <entry>Type</entry>␟   <entry>型</entry>␞␞␞
␝   <entry>Type</entry>␟   <entry>Length</entry>␟   <entry>長さ</entry>␞␞␞
␝   <entry>Length</entry>␟   <entry>Description</entry>␟   <entry>説明</entry>␞␞  </row>␞
␝   <entry>TransactionId</entry>␟   <entry>4 bytes</entry>␟   <entry>4バイト</entry>␞␞␞
␝   <entry>4 bytes</entry>␟   <entry>insert XID stamp</entry>␟   <entry>挿入XIDスタンプ</entry>␞␞  </row>␞
␝   <entry>TransactionId</entry>␟   <entry>4 bytes</entry>␟   <entry>4バイト</entry>␞␞␞
␝   <entry>4 bytes</entry>␟   <entry>delete XID stamp</entry>␟   <entry>削除XIDスタンプ</entry>␞␞  </row>␞
␝   <entry>CommandId</entry>␟   <entry>4 bytes</entry>␟   <entry>4バイト</entry>␞␞␞
␝   <entry>4 bytes</entry>␟   <entry>insert and/or delete CID stamp (overlays with t_xvac)</entry>␟   <entry>挿入、削除の両方または片方のCIDスタンプ（t_xvacと共有）</entry>␞␞  </row>␞
␝   <entry>TransactionId</entry>␟   <entry>4 bytes</entry>␟   <entry>4バイト</entry>␞␞␞
␝   <entry>4 bytes</entry>␟   <entry>XID for VACUUM operation moving a row version</entry>␟   <entry>行バージョンを移すVACUUM操作用XID</entry>␞␞  </row>␞
␝   <entry>ItemPointerData</entry>␟   <entry>6 bytes</entry>␟   <entry>6バイト</entry>␞␞␞
␝   <entry>6 bytes</entry>␟   <entry>current TID of this or newer row version</entry>␟   <entry>この行または最新バージョンの行の現在のTID</entry>␞␞  </row>␞
␝   <entry>uint16</entry>␟   <entry>2 bytes</entry>␟   <entry>2バイト</entry>␞␞␞
␝   <entry>2 bytes</entry>␟   <entry>number of attributes, plus various flag bits</entry>␟   <entry>属性の数と各種フラグビット</entry>␞␞  </row>␞
␝   <entry>uint16</entry>␟   <entry>2 bytes</entry>␟   <entry>2バイト</entry>␞␞␞
␝   <entry>2 bytes</entry>␟   <entry>various flag bits</entry>␟   <entry>様々なフラグビット</entry>␞␞  </row>␞
␝   <entry>uint8</entry>␟   <entry>1 byte</entry>␟   <entry>1バイト</entry>␞␞␞
␝   <entry>1 byte</entry>␟   <entry>offset to user data</entry>␟   <entry>ユーザデータに対するオフセット</entry>␞␞  </row>␞
␝ <para>␟   All the details can be found in
   <filename>src/include/access/htup_details.h</filename>.␟詳細情報については<filename>src/include/access/htup_details.h</filename>を参照してください。␞␞ </para>␞
␝␟  Interpreting the actual data can only be done with information obtained
  ...
  and <firstterm>heap_getsysattr</firstterm>.␟実際のデータの解釈は、他のテーブル、ほとんどの場合、<structname>pg_attribute</structname>から取得された情報でのみ行うことができます。
...
␝␟  To read the data you need to examine each attribute in turn. First check
  ...
  it might be compressed, too (see <xref linkend="storage-toast"/>).␟データを読むためには、それぞれの属性を順番に検査する必要があります。
...
␝␟ <title>Heap-Only Tuples (<acronym>HOT</acronym>)</title>␟ <title>ヒープ専用タプル(<acronym>HOT</acronym>)</title>␞␞␞
␝ <para>␟  To allow for high concurrency, <productname>PostgreSQL</productname>
  ...
  expensive.␟高い同時実行性を可能にするために、<productname>PostgreSQL</productname>は<link linkend="mvcc-intro">マルチバージョン同時実行制御</link>(<acronym>MVCC</acronym>)を使用して行を格納します。
しかし、<acronym>MVCC</acronym>には更新クエリに対していくつかの欠点があります。
特に、更新では、テーブルに新しいバージョンの行を追加する必要があります。
また、更新された行ごとに新しいインデックスエントリが必要になる可能性があり、古いバージョンの行とそのインデックスエントリを削除するとコストが高くなります。␞␞ </para>␞
␝ <para>␟  To help reduce the overhead of updates,
  <productname>PostgreSQL</productname> has an optimization called
  heap-only tuples (<acronym>HOT</acronym>).  This optimization is
  possible when:␟更新のオーバーヘッドを減らすために、<productname>PostgreSQL</productname>にはヒープ専用タプル<acronym>HOT</acronym>と呼ばれる最適化があります。
この最適化は以下の場合に可能です。␞␞␞
␝    <para>␟     The update does not modify any columns referenced by the table's indexes,
     ...
     linkend="brin">BRIN</link>.␟更新は、集約インデックスを除き、テーブルのインデックスによって参照される列を変更しません。
<productname>PostgreSQL</productname>のコア配布物で唯一の集約インデックスメソッドは<link linkend="brin">BRIN</link>です。␞␞     </para>␞
␝    <para>␟     There is sufficient free space on the page containing the old row
     for the updated row.␟古い行を含むページには、更新された行に対して十分な空き領域があります。␞␞    </para>␞
␝␟  In such cases, heap-only tuples provide two optimizations:␟そのような場合に、ヒープ専用タプルは2つの最適化を提供します。␞␞␞
␝    <para>␟     New index entries are not needed to represent updated rows, however,
     summary indexes may still need to be updated.␟更新された行を表すために新しいインデックスエントリは必要ありませんが、集約インデックスは更新が必要でしょう。␞␞    </para>␞
␝    <para>␟     When a row is updated multiple times, row versions other than the oldest
     ...
     are made available for reuse.)␟行が複数回更新されると、最も古いものと最も新しいもの以外の行バージョンは、定期的なバキューム操作を必要とせずに、<command>SELECT</command>を含む通常の操作で完全に削除されることがあります。
（インデックスは常に元の行バージョンの<link linkend="storage-page-layout">ページアイテム識別子</link>を参照します。
その行バージョンに関連付けられたタプルデータは削除され、そのアイテム識別子は、依然としていくつかの同時トランザクションに見える可能性のある最も古いバージョンを指すリダイレクトに変換されます。
もはや誰にも見えない中間行バージョンは完全に削除され、関連するページアイテム識別子は再利用可能になります。）␞␞    </para>␞
␝ <para>␟  You can increase the likelihood of sufficient page space for
  ...
  allows monitoring of the occurrence of HOT and non-HOT updates.␟テーブルの<link linkend="reloption-fillfactor"><literal>fillfactor</literal></link>を減らすことで、<acronym>HOT</acronym>更新のための十分なページ領域の可能性を高めることができます。
そうしない場合でも、<acronym>HOT</acronym>更新は発生します。
なぜなら、新しい行は新しいページや、新しい行バージョンのために十分な空き領域を持つ既存のページに自然に移動するからです。
システムビュー<link linkend="monitoring-pg-stat-all-tables-view">pg_stat_all_tables</link>は、HOTおよび非HOT更新の発生を監視できます。␞␞ </para>␞
