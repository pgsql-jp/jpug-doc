...
␝ <chapter id="extend">␟  <title>Extending <acronym>SQL</acronym></title>␟  <title><acronym>SQL</acronym>の拡張</title>␞␞␞
␝  <para>␟   In  the  sections  that follow, we will discuss how you
   can extend the <productname>PostgreSQL</productname>
   <acronym>SQL</acronym> query language by adding:␟本節では以下を追加することで<productname>PostgreSQL</productname>の<acronym>SQL</acronym>問い合わせ言語をどのように拡張できるかを説明します。␞␞␞
␝     <para>␟      functions (starting in <xref linkend="xfunc"/>)␟関数（<xref linkend="xfunc"/>から）。␞␞     </para>␞
␝     <para>␟      aggregates (starting in <xref linkend="xaggr"/>)␟集約（<xref linkend="xaggr"/>から）。␞␞     </para>␞
␝     <para>␟      data types (starting in <xref linkend="xtypes"/>)␟データ型（<xref linkend="xtypes"/>から）。␞␞     </para>␞
␝     <para>␟      operators (starting in <xref linkend="xoper"/>)␟演算子（<xref linkend="xoper"/>から）。␞␞     </para>␞
␝     <para>␟      operator classes for indexes (starting in <xref linkend="xindex"/>)␟インデックス用演算子クラス（<xref linkend="xindex"/>から）。␞␞     </para>␞
␝     <para>␟      packages of related objects (starting in <xref linkend="extend-extensions"/>)␟      関連オブジェクトのパッケージ（<xref linkend="extend-extensions"/>から）。␞␞     </para>␞
␝  <sect1 id="extend-how">␟   <title>How Extensibility Works</title>␟   <title>拡張の作用法</title>␞␞␞
␝   <para>␟    <productname>PostgreSQL</productname> is extensible because its operation  is
    ...
    specially written by the <acronym>DBMS</acronym> vendor.␟<productname>PostgreSQL</productname>の動作は、カタログに定義された方法で駆動されているため拡張が可能です。
...
これらのテーブルはユーザが変更できます。そして、<productname>PostgreSQL</productname>は操作をこれらのテーブルに基づいて行うので、これは<productname>PostgreSQL</productname>がユーザによって拡張できることを意味します。
これに対して、一般のデータベースシステムでは、ソースコード内にハードコーディングされたプロシージャを変えるか、<acronym>DBMS</acronym>ベンダによって特別に書かれたモジュールをロードしなければ拡張できません。␞␞   </para>␞
␝   <para>␟    The <productname>PostgreSQL</productname> server can moreover
    ...
    structures.␟さらに<productname>PostgreSQL</productname>サーバは動的ロードによってユーザの作成したコードを取り入れることができます。
つまり、ユーザが新しい型か関数を実装するオブジェクトコードファイル（例えば共有ライブラリ）を指定でき、<productname>PostgreSQL</productname>は要求された時にロードします。
<acronym>SQL</acronym>で作成されたコードをサーバに追加するのはさらに簡単です。
このように演算を<quote>その場で</quote>変えることができるため、<productname>PostgreSQL</productname>は新しいアプリケーションや格納構造をラピッドプロトタイプする場合に適しています。␞␞   </para>␞
...
␝  <sect1 id="extend-type-system">␟   <title>The <productname>PostgreSQL</productname> Type System</title>␟   <title><productname>PostgreSQL</productname>の型システム</title>␞␞␞
␝   <para>␟    <productname>PostgreSQL</productname> data types can be divided into base
    types, container types, domains, and pseudo-types.␟<productname>PostgreSQL</productname>のデータ型は、基本型、コンテナ型、ドメイン、疑似型に分類されます。␞␞   </para>␞
␝   <sect2 id="extend-type-system-base">␟    <title>Base Types</title>␟    <title>基本型</title>␞␞␞
␝    <para>␟     Base types are those, like <type>integer</type>, that are
     ...
     them.
     The built-in base types are described in <xref linkend="datatype"/>.␟基本型は<type>integer</type>のように、<acronym>SQL</acronym>言語レベル以下で実装されたものです
...
組み込みの基本型は、<xref linkend="datatype"/>に記載されています。␞␞    </para>␞
␝    <para>␟     Enumerated (enum) types can be considered as a subcategory of base
     ...
     Refer to <xref linkend="datatype-enum"/> for more information.␟列挙(enum)型は基本型の一種とみなすことができます。
主な違いは、列挙型は低レベルプログラミング無しに、<acronym>SQL</acronym>コマンドだけで作ることができることです。
より詳細については、<xref linkend="datatype-enum"/>を参照してください。␞␞    </para>␞
␝   <sect2 id="extend-type-system-container">␟    <title>Container Types</title>␟    <title>コンテナ型</title>␞␞␞
␝    <para>␟     <productname>PostgreSQL</productname> has three kinds
     of <quote>container</quote> types, which are types that contain multiple
     values of other types.  These are arrays, composites, and ranges.␟<productname>PostgreSQL</productname>には三種類の<quote>コンテナ</quote>型があります。これは他の型の複数の値を含む型です。
配列、複合型、範囲型があります。␞␞    </para>␞
␝    <para>␟     Arrays can hold multiple values that are all of the same type.  An array
     ...
     information.␟配列は、全て同じ型の複数の値を保持できます。
配列型は各基本型、複合型、範囲型およびドメイン型に対して自動的に作られます。
しかし、配列の配列はありません。
この型システムにおいては多次元配列は一次元配列と同じです。
より詳細については、<xref linkend="arrays"/>を参照してください。␞␞    </para>␞
␝    <para>␟     Composite types, or row types, are created whenever the user
     ...
     for more information.␟ユーザがテーブルを作成すると、複合型、もしくは行型が作成されます。
...
より詳細については、<xref linkend="rowtypes"/>を参照してください。␞␞    </para>␞
␝    <para>␟     A range type can hold two values of the same type, which are the lower
     ...
     for more information.␟範囲型は同じ型の二つの値を保持できます。これらは範囲の下限と上限です。
範囲型はユーザによって作られますが、少数の組み込みの型もあります。
より詳細については、<xref linkend="rangetypes"/>を参照してください。␞␞    </para>␞
␝   <sect2 id="extend-type-system-domains">␟    <title>Domains</title>␟    <title>ドメイン</title>␞␞␞
␝    <para>␟     A domain is based on a particular underlying type and for many purposes
     ...
     Refer to <xref linkend="domains"/> for more information.␟ドメインは、特定の元となる型に基づいたもので、多くの目的では、その元となる型と交換可能です。
しかし、ドメインは元となる基本型で許可される範囲内で値の有効範囲を制限する制約を持つことができます。
ドメインは<acronym>SQL</acronym>コマンドの<xref linkend="sql-createdomain"/>を使って作られます。
より詳細については、<xref linkend="domains"/>を参照してください。␞␞    </para>␞
␝   <sect2 id="extend-type-system-pseudo">␟    <title>Pseudo-Types</title>␟    <title>疑似データ型</title>␞␞␞
␝    <para>␟     There are a few <quote>pseudo-types</quote> for special purposes.
     ...
     pseudo-types.␟特殊な目的用に数個の<quote>疑似型</quote>があります。
疑似型はテーブルの列やコンテナ型の構成要素として現れることはありません。
しかし、関数の引数や結果型を宣言する際に使用できます。
これは、型システム内で特殊な関数クラスを識別するための機構を提供します。
<xref linkend="datatype-pseudotypes-table"/>に既存の疑似型を列挙します。␞␞    </para>␞
...
␝   <sect2 id="extend-types-polymorphic">␟    <title>Polymorphic Types</title>␟    <title>多様型</title>␞␞␞
␝    <para>␟     Some pseudo-types of special interest are the <firstterm>polymorphic
     ...
     their use appear in <xref linkend="xfunc-sql-polymorphic-functions"/>.␟いくつかの特殊な用途を持つ疑似型は<firstterm>多様型</firstterm>で、<firstterm>多様関数</firstterm>を宣言するのに使われます。
...
これらの使用例は<xref linkend="xfunc-sql-polymorphic-functions"/>にあります。␞␞    </para>␞
␝    <table id="extend-types-polymorphic-table">␟     <title>Polymorphic Types</title>␟     <title>多様型</title>␞␞     <tgroup cols="3">␞
␝       <row>␟        <entry>Name</entry>␟        <entry>名前</entry>␞␞␞
␝        <entry>Name</entry>␟        <entry>Family</entry>␟        <entry>族</entry>␞␞␞
␝        <entry>Family</entry>␟        <entry>Description</entry>␟        <entry>説明</entry>␞␞       </row>␞
␝        <entry>Simple</entry>␟        <entry>Indicates that a function accepts any data type</entry>␟        <entry>関数があらゆるデータ型を受け付けることを示します</entry>␞␞       </row>␞
␝        <entry>Simple</entry>␟        <entry>Indicates that a function accepts any array data type</entry>␟        <entry>関数があらゆる配列データ型を受け付けることを示します</entry>␞␞       </row>␞
␝        <entry>Simple</entry>␟        <entry>Indicates that a function accepts any non-array data type</entry>␟        <entry>関数があらゆる配列でないデータ型を受け付けることを示します</entry>␞␞       </row>␞
␝        <entry>Simple</entry>␟        <entry>Indicates that a function accepts any enum data type
        (see <xref linkend="datatype-enum"/>)␟        <entry>関数があらゆる列挙型（<xref linkend="datatype-enum"/>を参照）を受け付けることを示します␞␞        </entry>␞
␝        <entry>Simple</entry>␟        <entry>Indicates that a function accepts any range data type
        (see <xref linkend="rangetypes"/>)␟        <entry>関数があらゆる範囲データ型（<xref linkend="rangetypes"/>を参照）を受け付けることを示します␞␞        </entry>␞
␝        <entry>Simple</entry>␟        <entry>Indicates that a function accepts any multirange data type
        (see <xref linkend="rangetypes"/>)␟        <entry>関数があらゆる多重範囲データ型（<xref linkend="rangetypes"/>を参照）を受け付けることを示します␞␞        </entry>␞
␝        <entry>Common</entry>␟        <entry>Indicates that a function accepts any data type,
        with automatic promotion of multiple arguments to a common data type␟        <entry>さまざまな引数の共通データ型への自動昇格によって、関数があらゆるデータ型を受け付けることを示します␞␞        </entry>␞
␝        <entry>Common</entry>␟        <entry>Indicates that a function accepts any array data type,
        with automatic promotion of multiple arguments to a common data type␟        <entry>さまざまな引数の共通データ型への自動昇格によって、関数があらゆる配列データ型を受け付けることを示します␞␞        </entry>␞
␝        <entry>Common</entry>␟        <entry>Indicates that a function accepts any non-array data type,
        with automatic promotion of multiple arguments to a common data type␟        <entry>さまざまな引数の共通データ型への自動昇格によって、関数があらゆる配列でないデータ型を受け付けることを示します␞␞        </entry>␞
␝        <entry>Common</entry>␟        <entry>Indicates that a function accepts any range data type,
        with automatic promotion of multiple arguments to a common data type␟        <entry>さまざまな引数の共通データ型への自動昇格によって、配列があらゆる範囲データ型を受け付けることを示します␞␞        </entry>␞
␝        <entry>Common</entry>␟        <entry>Indicates that a function accepts any multirange data type,
        with automatic promotion of multiple arguments to a common data type␟        <entry>さまざまな引数の共通データ型への自動昇格によって、配列があらゆる多重範囲データ型を受け付けることを示します␞␞        </entry>␞
␝    <para>␟     Polymorphic arguments and results are tied to each other and are resolved
     ...
     actual types of the polymorphic inputs as described below.␟多様の引数と結果は互いに結びつけられていて、多様関数を呼ぶ問い合わせが解析されるときに特定のデータ型に決定されます。
2つ以上の多様引数がある時には、入力値の実データ型は後述するように合わせなければなりません。
関数の結果型が多様、あるいは、多様型の出力パラメータを持つ場合には、それらの結果の型は後述するように多様入力の実際の型から導出されます。␞␞    </para>␞
␝    <para>␟     For the <quote>simple</quote> family of polymorphic types, the
     matching and deduction rules work like this:␟多様型の<quote>simple</quote>族では、一致と導出の規則は以下のように動作します。␞␞    </para>␞
␝    <para>␟     Each position (either argument or return value) declared as
     ...
     type.  Likewise for <type>anymultirange</type>.␟<type>anyelement</type>として宣言された位置（引数もしくは戻り値）にはそれぞれ、任意の実データ型を指定できますが、1つの呼び出しでは、これらはすべて<emphasis>同一の</emphasis>実データ型でなければなりません。
...
<type>anymultirange</type>についても同様です。␞␞    </para>␞
␝    <para>␟     Furthermore, if there are
     ...
     be an enum type.␟さらに、<type>anyarray</type>と宣言された位置と<type>anyelement</type>と宣言された位置の両方がある場合、<type>anyarray</type>の位置の実際の配列型は、その要素の型が<type>anyelement</type>位置に現れる型と同じでなければなりません。
<type>anynonarray</type>は、実際の型が配列型であってはならないという制限が加わっている点を除き、<type>anyelement</type>とまったく同様に扱われます。
<type>anyenum</type>は、実際の型が列挙型でなければならないという制約が加わっている点を除き、<type>anyelement</type>とまったく同様に扱われます。␞␞    </para>␞
␝    <para>␟     Similarly, if there are positions declared <type>anyrange</type>
     ...
     <type>anyelement</type> and <type>anyarray</type>.␟同様に<type>anyrange</type>と宣言された位置と<type>anyelement</type>もしくは<type>anyarray</type>と宣言された位置の両方がある場合、<type>anyrange</type>の位置の実際の範囲型は、その範囲の派生元型が<type>anyelement</type>位置に現れる型と同じであり、<type>anyarray</type>位置の要素の型と同じでなければなりません。
<type>anymultirange</type>と宣言された位置がある場合、実際の多重範囲型には、宣言された位置と<type>anyrange</type>が一致する範囲と、宣言された位置<type>anyelement</type>および<type>anyarray</type>と一致する基本要素が含まれている必要があります。␞␞    </para>␞
␝    <para>␟     Thus, when more than one argument position is declared with a polymorphic
     type, the net effect is that only certain combinations of actual argument
     types are allowed.  For example, a function declared as
     <literal>equal(anyelement, anyelement)</literal> will take any two input values,
     so long as they are of the same data type.␟このように、2つ以上の引数位置が多様型と宣言されると、全体の効果として、実引数型の特定の組み合わせのみが許されるようになります。
例えば、<literal>equal(anyelement, anyelement)</literal>と宣言された関数は、2つの引数が同じデータ型である限り、任意の入力値を2つ取ることになります。␞␞    </para>␞
␝    <para>␟     When the return value of a function is declared as a polymorphic type,
     ...
     will only accept arrays of enum types.␟関数の戻り値を多様型として宣言する時、少なくとも1つの引数位置も多様でなければなりません。
...
他にも例えば、<literal>f(anyarray) returns anyenum</literal>と宣言された関数は列挙型の配列のみを受け付けます。␞␞    </para>␞
␝    <para>␟     In most cases, the parser can infer the actual data type for a
     ...
     is because there could be multiple range types with the same subtype.␟ほとんどの場合、パーサは同じ族の異なる多様型の引数から多様結果型の実データ型を推論できます。
...
これは、同じ派生元型の複数の範囲型が存在する可能性があるためです。␞␞    </para>␞
␝    <para>␟     Note that <type>anynonarray</type> and <type>anyenum</type> do not represent
     ...
     both actual arguments have to be the same enum type.␟<type>anynonarray</type>型と<type>anyenum</type>型が、別個の型変数を表していないことに注意してください。
...
両方の実引数は同じ列挙型でなければなりません。␞␞    </para>␞
␝    <para>␟     For the <quote>common</quote> family of polymorphic types, the
     ...
     positions are automatically cast to the array type for that type.␟多様型の<quote>common</quote>族については、一致と導出の規則は概ね<quote>simple</quote>と同じに動作しますが、一つの大きな違いがあります。
...
共通データ型が特定されたなら、<type>anycompatible</type>および<type>anycompatiblenonarray</type>の位置の引数は自動的にその型にキャストされ、<type>anycompatiblearray</type>位置の引数は自動的にその型に対する配列にキャストされます。␞␞    </para>␞
␝    <para>␟     Since there is no way to select a range type knowing only its subtype,
     ...
     argument.␟その派生元型だけ分かっている範囲型を選択する方法がないため、<type>anycompatiblerange</type>および/または<type>anycompatiblemultirange</type>の使用には、その型で宣言されているすべての引数が同じ実際の範囲型および/または多重範囲型を持ち、また、その型の派生元型は選択された共通型に即したものである必要があり、そのため、範囲値のキャストは必要ありません。
<type>anyrange</type>および<type>anymultirange</type>と同様に、<type>anycompatiblerange</type>および<type>anymultirange</type>を関数の結果型として使用するには、<type>anycompatiblerange</type>または<type>anycompatiblemultirange</type>の引数がある必要があります。␞␞    </para>␞
␝    <para>␟     Notice that there is no <type>anycompatibleenum</type> type.  Such a
     ...
     resolve a common type for dissimilar enum inputs.␟<type>anycompatibleenum</type>型は無いことに注意してください。
通常、列挙型への暗黙キャストはありません。これは異なる列挙入力に対する共通型を決める方法が無いことを意味します。そのため、このような型はあまり有益ではないでしょう。␞␞    </para>␞
␝    <para>␟     The <quote>simple</quote> and <quote>common</quote> polymorphic
     families represent two independent sets of type variables.  Consider
     for example␟<quote>simple</quote>と<quote>common</quote>の多様族は型変数の二つの独立したセットに相当します。
例えば以下を考えてください。␞␞<programlisting>␞
␝</programlisting>␟     In an actual call of this function, the first two inputs must have
     ...
     last two inputs.␟この関数の実際の呼び出しでは、最初の2つの入力は正確に同じ型を持たなければなりません。
最後の2つの入力は共通型に昇格できなければなりませんが、この型が最初の2つの入力型と何らか関係がある必要はありません。
結果は最後の2つの入力の共通型を持ちます。␞␞    </para>␞
␝    <para>␟     A variadic function (one taking a variable number of arguments, as in
     ...
     parameters.␟可変長引数の関数（<xref linkend="xfunc-sql-variadic-functions"/>で説明する可変個の引数を取る関数）を多様とすることができます。
...
引数を一致させ、実際の結果型を決めるために、こうした関数は<type>anynonarray</type>または<type>anycompatiblenonarray</type>パラメータをあたかも適切な個数記述した場合と同様に動作します。␞␞    </para>␞
...
␝  <sect1 id="extend-extensions">␟   <title>Packaging Related Objects into an Extension</title>␟   <title>関連するオブジェクトを拡張としてパッケージ化</title>␞␞␞
␝   <para>␟    A useful extension to <productname>PostgreSQL</productname> typically includes
    ...
    your database.␟<productname>PostgreSQL</productname>への有用な拡張は通常、複数のSQLオブジェクトを含んでいます。
...
これらのファイルがあれば、単純な<link linkend="sql-createextension"><command>CREATE EXTENSION</command></link>コマンドがそのオブジェクトをデータベース内に読み込みます。␞␞   </para>␞
␝   <para>␟    The main advantage of using an extension, rather than just running the
    ...
    other files available when loading such a dump into a new database.␟拡張を使用する主な利点は、<acronym>SQL</acronym>スクリプトを実行するだけでデータベースに<quote>粗な</quote>なオブジェクトの群をロードできることではなく、<productname>PostgreSQL</productname>が拡張のオブジェクトをまとまったものと理解できることです。
単一の<link linkend="sql-dropextension"><command>DROP EXTENSION</command></link>コマンドでオブジェクトすべてを削除できます（個々の<quote>アンインストール</quote>スクリプトを保守する必要はありません）。
...
しかし、こうしたダンプを新しいデータベースにロードする際には、拡張の制御ファイル、スクリプトファイル、その他のファイルが利用できるようにしておく必要があります。␞␞   </para>␞
␝   <para>␟    <productname>PostgreSQL</productname> will not let you drop an individual object
    ...
    update script to perform changes to extension member objects.␟<productname>PostgreSQL</productname>はユーザに、拡張全体を削除させる以外に、拡張内に含まれる個々のオブジェクトを削除させません。
...
本番環境では、拡張メンバオブジェクトへの変更を処理するために拡張更新スクリプトを作成するのが一般により良い方法です。␞␞   </para>␞
␝   <para>␟    The extension script may set privileges on objects that are part of the
    ...
    at the time the dump was taken.␟拡張スクリプトは、<command>GRANT</command>文と<command>REVOKE</command>文を使って拡張の一部のオブジェクトに権限を設定するかもしれません。
...
<application>pg_dump</application>が使用されると、<command>CREATE EXTENSION</command>コマンドがダンプ内に含まれ、オブジェクトの権限をダンプが取られた時点のものに設定するために必要となる<command>GRANT</command>文と<command>REVOKE</command>文が後に続きます。␞␞   </para>␞
␝   <para>␟    <productname>PostgreSQL</productname> does not currently support extension scripts
    ...
    included in dumps created by <application>pg_dump</application>.␟<productname>PostgreSQL</productname>は、現在拡張スクリプトにて<command>CREATE POLICY</command>文や<command>SECURITY LABEL</command>文の発行をサポートしていません。
これらは拡張が作成された後に設定されるべきです。
拡張オブジェクトのすべての行セキュリティポリシーとセキュリティラベルは<application>pg_dump</application>によって作成されたダンプに含まれます。␞␞   </para>␞
␝   <para>␟    The extension mechanism also has provisions for packaging modification
    ...
    is actually installed in a given database.␟また拡張機構は、拡張に含まれるSQLオブジェクトの定義を調整するパッケージ調整スクリプトを準備しています。
例えば、拡張のバージョン1.1でバージョン1.0と比べて１つの関数を追加し、他の関数本体を変更する場合、拡張の作成者はこれらの２つの変更のみを行う<firstterm>更新スクリプト</firstterm>を提供できます。
そして<command>ALTER EXTENSION UPDATE</command>コマンドを使用して、これらの変更を適用し、指定されたデータベース内に実際にインストールされた拡張のバージョンが何かを記録します。␞␞   </para>␞
␝   <para>␟    The kinds of SQL objects that can be members of an extension are shown in
    ...
    schema(s) its member objects are within.␟拡張のメンバとなり得るSQLオブジェクトの種類を<link linkend="sql-alterextension"><command>ALTER EXTENSION</command></link>で説明します。
...
拡張が自身のメンバオブジェクトが属するスキーマを所有することは適切かも知れませんし、そうでないかも知れません。␞␞   </para>␞
␝   <para>␟    If an extension's script creates any temporary objects (such as temp
    ...
    dropping the whole extension.␟ある拡張のスクリプトが（一時テーブルのような）一時オブジェクトを作成する場合、現在のセッションで、以降そのオブジェクトは拡張のメンバとして扱われます。
しかしすべての一時オブジェクト同様、セッションの終わりに削除されます。
これは、拡張全体を削除することなしに、拡張のメンバオブジェクトは削除できない、という規則の例外です。␞␞   </para>␞
␝   <indexterm>
...
␝   <sect2 id="extend-extensions-files">␟    <title>Extension Files</title>␟    <title>拡張のファイル</title>␞␞␞
␝    <para>␟     The <command>CREATE EXTENSION</command> command relies on a control
     ...
     control file can specify a different directory for the script file(s).␟<command>CREATE EXTENSION</command>コマンドは各拡張に関して、拡張と同じ名前に<literal>.control</literal>という拡張子を持つファイル名である必要がある、制御ファイルに依存します。
...
デフォルトでは、このスクリプトファイルも<literal>SHAREDIR/extension</literal>ディレクトリに格納されますが、制御ファイルでスクリプトファイルを別のディレクトリに指定できます。␞␞    </para>␞
␝    <para>␟     The file format for an extension control file is the same as for the
     ...
     a single word or number.␟拡張の制御ファイルのファイル書式は<filename>postgresql.conf</filename>ファイルと同じです。
...
単一の単語または数字ではない値にはすべて引用符で確実にくくってください。␞␞    </para>␞
␝    <para>␟     A control file can set the following parameters:␟制御ファイルは以下のパラメータを設定できます。␞␞    </para>␞
␝       <para>␟        The directory containing the extension's <acronym>SQL</acronym> script
        file(s).  Unless an absolute path is given, the name is relative to
        the installation's <literal>SHAREDIR</literal> directory.  The
        default behavior is equivalent to specifying
        <literal>directory = 'extension'</literal>.␟拡張の<acronym>SQL</acronym>スクリプトファイルを含むディレクトリです。
絶対パスで指定されていない限り、この名前はインストレーションの<literal>SHAREDIR</literal>ディレクトリからの相対パスになります。
デフォルトの動作は<literal>directory = 'extension'</literal>と指定した場合と同じです。␞␞       </para>␞
␝       <para>␟        The default version of the extension (the one that will be installed
        ...
        don't want to do that.␟拡張のデフォルトのバージョン（<command>CREATE EXTENSION</command>でバージョン指定がない場合にインストールされるバージョン）です。
これは省略できますが、その場合<literal>VERSION</literal>オプションがない<command>CREATE EXTENSION</command>は失敗します。
ですので通常省略しようとは思わないでしょう。␞␞       </para>␞
␝       <para>␟        A comment (any string) about the extension.  The comment is applied
        ...
        a <xref linkend="sql-comment"/> command in the script file.␟拡張に関するコメント（任意の文字列）です。
最初に拡張が作成されるときにコメントは適用されますが、拡張が更新される間はされません（ユーザが追加したコメントを上書いてしまうため）。
この他の方法として、スクリプトファイル内で<xref linkend="sql-comment"/>コマンドを使用してコメントを設定できます。␞␞       </para>␞
␝       <para>␟        The character set encoding used by the script file(s).  This should
        be specified if the script files contain any non-ASCII characters.
        Otherwise the files will be assumed to be in the database encoding.␟スクリプトファイルで使用される文字セット符号化方式です。
スクリプトファイルに何らかの非ASCII文字が含まれる場合に指定しなければなりません。
指定がなければ、ファイルはデータベース符号化方式であると仮定されます。␞␞       </para>␞
␝       <para>␟        The value of this parameter will be substituted for each occurrence
        ...
        files do not need to hard-wire the name of the shared library.␟このパラメータの値でスクリプトファイル内の<literal>MODULE_PATHNAME</literal>の出現箇所が置換されます。
設定されていない場合は置換は行われません。
通常これは、スクリプトファイル内で共有ライブラリの名前を直接書き込む必要がなくなるように<literal>$libdir/<replaceable>shared_library_name</replaceable></literal>に設定され、C言語関数では<command>CREATE FUNCTION</command>コマンド中で<literal>MODULE_PATHNAME</literal>を使用します。␞␞       </para>␞
␝       <para>␟        A list of names of extensions that this extension depends on,
        for example <literal>requires = 'foo, bar'</literal>.  Those
        extensions must be installed before this one can be installed.␟拡張が依存する拡張の名前のリストです。
例えば<literal>requires = 'foo, bar'</literal>です。
対象の拡張がインストールできるようになる前に、これらの拡張がインストールされていなければなりません。␞␞       </para>␞
␝       <para>␟        A list of names of extensions that this extension depends on that
        ...
        syntax) in a way that cannot track renames.␟この拡張が依存していて、<command>ALTER EXTENSION ... SET SCHEMA</command>でスキーマを変更することを禁止される拡張の名前の一覧です。
この拡張のスクリプトが、(<literal>@extschema:<replaceable>name</replaceable>@</literal>構文を使って)必要な拡張のスキーマの名前を参照し、名前変更を追跡できない場合に必要です。␞␞       </para>␞
␝       <para>␟        If this parameter is <literal>true</literal> (which is the default),
        ...
        fail anyway, but it's more user-friendly to give the error up front.)␟このパラメータが<literal>true</literal>（デフォルト）の場合、スーパーユーザのみが拡張を作成または新しいバージョンに更新できます（ただし、後述する<varname>trusted</varname>も参照してください）。
...
（このようなコマンドはいずれにせよ失敗するでしょうけれども、前もってエラーを出す方がよりユーザフレンドリです。）␞␞       </para>␞
␝       <para>␟        This parameter, if set to <literal>true</literal> (which is not the
        ...
        see <xref linkend="extend-extensions-security"/>.␟このパラメータは、<literal>true</literal>（デフォルトではありません）に設定されている場合、一部の非スーパーユーザが<varname>superuser</varname>に<literal>true</literal>を設定している拡張をインストールできるようにします。
...
また、拡張をtrustedとして作成するには、拡張のインストールとスクリプト更新を安全に記述するために、かなりの追加の労力が影響が必要です。<xref linkend="extend-extensions-security"/>を参照してください。␞␞       </para>␞
␝       <para>␟        An extension is <firstterm>relocatable</firstterm> if it is possible to move
        its contained objects into a different schema after initial creation
        of the extension.  The default is <literal>false</literal>, i.e., the
        extension is not relocatable.
        See <xref linkend="extend-extensions-relocation"/> for more information.␟拡張を最初に作成した後に拡張により含まれるオブジェクトを別のスキーマに移動できる場合、拡張は<firstterm>再配置可能</firstterm>です。
デフォルトは<literal>false</literal>、つまり、拡張は再配置可能ではありません。
詳しくは<xref linkend="extend-extensions-relocation"/>を参照してください。␞␞       </para>␞
␝       <para>␟        This parameter can only be set for non-relocatable extensions.
        ...
        See <xref linkend="extend-extensions-relocation"/> for more information.␟このパラメータは再配置可能ではない拡張に対してのみ設定できます。
拡張が指名したスキーマのみにロードされ、他にはロードされないことを強制します。
<varname>schema</varname>パラメータは、拡張を最初に作成するときにのみ参照され、拡張が更新される間はされません。
詳しくは<xref linkend="extend-extensions-relocation"/>を参照してください。␞␞       </para>␞
␝    <para>␟     In addition to the primary control file
     ...
     <varname>default_version</varname> cannot be set in a secondary control file.␟主制御ファイル<literal><replaceable>extension</replaceable>.control</literal>に加え、拡張は<literal><replaceable>extension</replaceable>--<replaceable>version</replaceable>.control</literal>という形の名前の副制御ファイルを持つことができます。
...
しかし<varname>directory</varname>および<varname>default_version</varname>パラメータは副制御ファイルで設定することはできません。␞␞    </para>␞
␝    <para>␟     An extension's <acronym>SQL</acronym> script files can contain any SQL commands,
     ...
     script files are implicitly executed within a transaction block.␟拡張の<acronym>SQL</acronym>スクリプトファイルにはトランザクション制御コマンド（<command>BEGIN</command>、<command>COMMIT</command>など）およびトランザクションブロックの内側で実行できないコマンド（<command>VACUUM</command>など）を除く任意のSQLコマンドを含めることができます。
スクリプトファイルが暗黙的にトランザクションブロック内で実行されるためです。␞␞    </para>␞
␝    <para>␟     An extension's <acronym>SQL</acronym> script files can also contain lines
     ...
     extension, a state of affairs that's a bit tedious to recover from.␟拡張の<acronym>SQL</acronym>スクリプトファイルには、<literal>\echo</literal>から始まる行を含めることができます。
...
これがないと、ユーザは間違って拡張としてではなく、<quote>まとまっていない</quote>オブジェクトとして拡張の内容をロードしてしまい、復旧が多少困難な状態になる可能性があります。␞␞    </para>␞
␝    <para>␟     If the extension script contains the
     ...
     non-superuser would create a privilege escalation path for that user.)␟拡張のスクリプトに文字列<literal>@extowner@</literal>が含まれている場合、この文字列は、<command>CREATE EXTENSION</command>や<command>ALTER EXTENSION</command>を実行した（適切にクォートされた）ユーザ名で置き換えられます。
典型的には、この機能はtrustedと印付けされた拡張が選択されたオブジェクトにサービス起動のスーパーユーザではなく実行したユーザを所有者として割り当てる際に使われます。
（とはいえ、このようにするには注意深くすべきです。例えば、C言語関数の所有者を非スーパーユーザに割り当てると、そのユーザに権限昇格の経路を作ることになるでしょう）␞␞    </para>␞
␝    <para>␟     While the script files can contain any characters allowed by the specified
     ...
     within a script file to set the comment.␟スクリプトファイルは指定した符号化方式で認められる任意の文字を含めることができますが、<productname>PostgreSQL</productname>が制御ファイルの符号化方式が何かを把握する方法がありませんので、制御ファイルにはASCII文字のみを含めなければなりません。
実際には、拡張のコメントに非ASCII文字を含めたい場合にのみ、これが問題になります。
このような場合には、制御ファイルの<varname>comment</varname>を使用せず、代わりにコメントを設定するためにスクリプトファイル内で<command>COMMENT ON EXTENSION</command>を使用することを勧めます。␞␞    </para>␞
␝   <sect2 id="extend-extensions-relocation">␟    <title>Extension Relocatability</title>␟    <title>拡張の再配置性</title>␞␞␞
␝    <para>␟     Users often wish to load the objects contained in an extension into a
     different schema than the extension's author had in mind.  There are
     three supported levels of relocatability:␟ユーザは拡張に含まれるオブジェクトを拡張の作成者が考えていたスキーマとは別のスキーマにロードしたいとよく考えます。
再配置性に関して３つのレベルがサポートされます。␞␞    </para>␞
␝      <para>␟       A fully relocatable extension can be moved into another schema
       ...
       file.␟完全な再配置可能な拡張は、いつでも、データベースにロードされた後であっても、他のスキーマに移動させることができます。
...
制御ファイル内で<literal>relocatable = true</literal>と設定することで、完全な再配置可能と印付けます。␞␞      </para>␞
␝      <para>␟       An extension might be relocatable during installation but not
       ...
       <literal>SCHEMA</literal> option of <command>CREATE EXTENSION</command>.␟拡張はインストール処理の間再配置可能ですが、その後再配置することはできません。
...
ユーザは<command>CREATE EXTENSION</command>の<literal>SCHEMA</literal>オプションを使用して対象のスキーマを設定できます。␞␞      </para>␞
␝      <para>␟       If the extension does not support relocation at all, set
       ...
       of limited use since the schema name is determined by the control file.␟拡張が再配置をまったくサポートしない場合、制御ファイルで<literal>relocatable = false</literal>を設定し、かつ、<literal>schema</literal>を意図している対象スキーマの名前に設定してください。
...
<literal>@extschema@</literal>置換機構はこの場合でも使用できますが、スキーマ名が制御ファイルによって決定されますので、用途は限定されます。␞␞      </para>␞
␝    <para>␟     In all cases, the script file will be executed with
     ...
     this:␟すべての場合において、スクリプトファイルは対象のスキーマを指し示すようにあらかじめ設定した<xref linkend="guc-search-path"/>を用いて実行されます。
つまり<command>CREATE EXTENSION</command>は以下と同じことを行います。␞␞<programlisting>␞
␝</programlisting>␟     This allows the objects created by the script file to go into the target
     ...
     to its previous setting upon completion of <command>CREATE EXTENSION</command>.␟これによりスクリプトファイルで作成されるオブジェクトを対象のスキーマ内に格納できます。
スクリプトファイルは要望に応じて<varname>search_path</varname>を変更できますが、一般的には望まれません。
<command>CREATE EXTENSION</command>の実行後、<varname>search_path</varname>は以前の設定に戻されます。␞␞    </para>␞
␝    <para>␟     The target schema is determined by the <varname>schema</varname> parameter in
     ...
     already exist, but in the other two cases it must already exist.␟対象のスキーマは制御ファイル内の<varname>schema</varname>パラメータがあればこのパラメータにより決定されます。
...
制御ファイルの<varname>schema</varname>パラメータが使用される時、対象のスキーマが存在しない場合は作成されますが、これ以外の２つの場合ではすでに存在しなければなりません。␞␞    </para>␞
␝    <para>␟     If any prerequisite extensions are listed in <varname>requires</varname>
     ...
     the new extension's script file.␟何らかの事前に必要な拡張が制御ファイル内の<varname>requires</varname>に列挙されていた場合、それらのターゲットスキーマは新しい機能拡張のターゲットスキーマに続いて<varname>search_path</varname>の初期設定に追加されます。
これにより新しい拡張のスクリプトファイルからそれらのオブジェクトが可視になります。␞␞    </para>␞
␝    <para>␟     For security, <literal>pg_temp</literal> is automatically appended to
     the end of <varname>search_path</varname> in all cases.␟安全のため、全てのケースにおいて<literal>pg_temp</literal>は自動的に<varname>search_path</varname>の最後に追記されます。␞␞    </para>␞
␝    <para>␟     Although a non-relocatable extension can contain objects spread across
     ...
     extensions.␟再配置不可能な拡張は複数スキーマにまたがるオブジェクトを含めることができますが、通常、外部使用を意図したオブジェクトはすべて単一スキーマに格納することが望まれます。
この単一スキーマが拡張の対象のスキーマとみなされます。
こうした調整は依存する拡張を作成する間、デフォルトの<varname>search_path</varname>設定を都合に合わせて扱います。␞␞    </para>␞
␝    <para>␟     If an extension references objects belonging to another extension,
     ...
     this one's <literal>no_relocate</literal> list.␟拡張が別の拡張に属するオブジェクトを参照する場合は、それらの参照をスキーマ修飾することをお勧めします。
これを行うには、拡張のスクリプトファイルに<literal>@extschema:<replaceable>name</replaceable>@</literal>と記述します。ここで、<replaceable>name</replaceable>は他の拡張の名前です(他の拡張はこの拡張の<literal>requires</literal>リストに記載されていることが必要です)。
...
（通常、これは<literal>@extschema:<replaceable>name</replaceable>@</literal>が関数本体や<varname>search_path</varname>設定などの文字列リテラルの内部で使用される場合に発生します。
その他の場合では、オブジェクト参照は解析中にOIDへと縮小され、その後の検索は必要ありません。）
他の拡張のスキーマ名がそのように埋め込まれている場合は、インストール後に他の拡張が再配置されないようにすることが必要です。これを行うには、他の拡張の名前をこの拡張の<literal>no_relocate</literal>リストに追加します。␞␞    </para>␞
␝   <sect2 id="extend-extensions-config-tables">␟    <title>Extension Configuration Tables</title>␟    <title>拡張設定テーブル</title>␞␞␞
␝    <para>␟     Some extensions include configuration tables, which contain data that
     ...
     and restore.␟一部の拡張は、拡張をインストールした後でユーザにより追加または変更される可能性があるデータを持つ設定テーブルを含みます。
...
さもないとダンプしリストアした後で拡張の動作が変わってしまいます。␞␞    </para>␞
␝    <para>␟     To solve this problem, an extension's script file can mark a table
     ...
     table or the sequence, for example␟この問題を解消するために、拡張のスクリプトファイルでは設定リレーションとして作成されるテーブル、またはシーケンスに印を付け、<application>pg_dump</application>にテーブルの、またはシーケンスの内容をダンプに含める（定義は含まれません）ようにさせることができます。
このためには、以下の例のようにテーブル、またはシーケンスを作成した後に<function>pg_extension_config_dump(regclass, text)</function>関数を呼び出してください。␞␞<programlisting>␞
␝</programlisting>␟     Any number of tables or sequences can be marked this way. Sequences
     associated with <type>serial</type> or <type>bigserial</type> columns can
     be marked as well.␟任意数のテーブル、またはシーケンスをこの方法で印付けることができます。
<type>serial</type>列または<type>bigserial</type>列に関連したシーケンスが、同様に印付けることができます。␞␞    </para>␞
␝    <para>␟     When the second argument of <function>pg_extension_config_dump</function> is
     ...
     For example, you might do␟<function>pg_extension_config_dump</function>の第２引数が空文字列である場合、テーブルのすべての内容が<application>pg_dump</application>によりダンプされます。
...
␝</programlisting>␟     and then make sure that <structfield>standard_entry</structfield> is true only
     in the rows created by the extension's script.␟このようにした後、拡張のスクリプトで作成される行のみで<structfield>standard_entry</structfield>が確実に真になるようにします。␞␞    </para>␞
␝    <para>␟     For sequences, the second argument of <function>pg_extension_config_dump</function>
     has no effect.␟シーケンスにおいて、<function>pg_extension_config_dump</function>の第２引数は何も影響を及ぼしません。␞␞    </para>␞
␝    <para>␟     More complicated situations, such as initially-provided rows that might
     be modified by users, can be handled by creating triggers on the
     configuration table to ensure that modified rows are marked correctly.␟初期状態で提供される行がユーザによって変更されるようなもっと複雑な状況では、設定テーブルに対するトリガを作成して、変更された行が正しく印付けられることを確実にするように取り扱うことができます。␞␞    </para>␞
␝    <para>␟     You can alter the filter condition associated with a configuration table
     ...
     extension with <command>ALTER EXTENSION ... DROP TABLE</command>.␟<function>pg_extension_config_dump</function>を再度呼び出すことにより、設定テーブルに関連付いたフィルタ条件を変更できます。
（通常これは拡張の更新スクリプト内で役に立つでしょう。）
設定ファイルからテーブルを取り除くように印付ける方法は、<command>ALTER EXTENSION ... DROP TABLE</command>を用いてテーブルを拡張から分離するしかありません。␞␞    </para>␞
␝    <para>␟     Note that foreign key relationships between these tables will dictate the
     ...
     intervention will be required.␟このテーブルとの外部キーの関係は、テーブルがpg_dumpによってダンプされる順序に影響します。
特に、pg_dumpは参照しているテーブルの前に参照されているテーブルをダンプしようとします。
外部キーの関係はCREATE EXTENSION時(データがテーブルにロードされる前)に設定されますので、循環依存はサポートされません。
循環依存が存在すれば、データはダンプされますが、そのダンプを直接はリストアできず、ユーザの介入が必要になります。␞␞    </para>␞
␝    <para>␟     Sequences associated with <type>serial</type> or <type>bigserial</type> columns
     need to be directly marked to dump their state. Marking their parent
     relation is not enough for this purpose.␟<type>serial</type>列または<type>bigserial</type>列に関連したシーケンスは、それらの状態をダンプするために直接印付けする必要があります。
親リレーションを印付けすることは、この目的に十分ではありません。␞␞    </para>␞
␝   <sect2 id="extend-extensions-updates">␟    <title>Extension Updates</title>␟    <title>拡張の更新</title>␞␞␞
␝    <para>␟     One advantage of the extension mechanism is that it provides convenient
     ...
     <literal>1.1</literal>).␟拡張機構の１つの利点は、拡張のオブジェクトを定義するSQLコマンドの更新を簡便に管理する方法を提供していることです。
...
更新スクリプトは<literal><replaceable>extension</replaceable>--<replaceable>old_version</replaceable>--<replaceable>target_version</replaceable>.sql</literal>というパターンに従った名前（例えば、<literal>foo--1.0--1.1.sql</literal>は<literal>foo</literal>拡張のバージョン<literal>1.0</literal>からバージョン<literal>1.1</literal>に変更するコマンドを含みます。）を持たなければなりません。␞␞    </para>␞
␝    <para>␟     Given that a suitable update script is available, the command
     ...
     objects, they are automatically dissociated from the extension.␟適切な更新スクリプトが利用可能である場合、<command>ALTER EXTENSION UPDATE</command>コマンドはインストール済みの拡張を指定した新しいバージョンへ更新します。
...
また、スクリプトが拡張のメンバオブジェクトを削除する場合には、それらのメンバオブジェクトは拡張から自動的に分離されます。␞␞    </para>␞
␝    <para>␟     If an extension has secondary control files, the control parameters
     that are used for an update script are those associated with the script's
     target (new) version.␟拡張が副制御ファイルを持つ場合、更新スクリプトで使用される制御パラメータは、スクリプトの対象の（新しい）バージョンに関連付けされたものになります。␞␞    </para>␞
␝    <para>␟     <command>ALTER EXTENSION</command> is able to execute sequences of update
     ...
     currently installed.␟<command>ALTER EXTENSION</command>は、要求される更新を実現するために更新スクリプトファイルを連続して実行できます。
例えば<literal>foo--1.0--1.1.sql</literal>と<literal>foo--1.1--2.0.sql</literal>のみが利用可能であるとすると、現在<literal>1.0</literal>がインストールされている時にバージョン<literal>2.0</literal>への更新が要求された場合、<command>ALTER EXTENSION</command>はこれらを順番に適用します。␞␞    </para>␞
␝    <para>␟     <productname>PostgreSQL</productname> doesn't assume anything about the properties
     ...
     <literal>--</literal> or leading or trailing <literal>-</literal>.)␟<productname>PostgreSQL</productname>はバージョン名称の特性についてまったく仮定を行いません。
例えば<literal>1.0</literal>の次が<literal>1.1</literal>であるかどうかを把握しません。
これは利用可能なバージョン名をかみ合わせ、もっとも少ない数の更新スクリプトを適用するために必要な経路を続けるだけです。
（バージョン名には、<literal>--</literal>を含まず先頭または最後に<literal>-</literal>が付かなければ、任意の文字を取ることができます。）␞␞    </para>␞
␝    <para>␟     Sometimes it is useful to provide <quote>downgrade</quote> scripts, for
     ...
     drops any irreplaceable objects, this will yield undesirable results.␟<quote>ダウングレード</quote>スクリプトを提供することが便利な場合があります。
...
ダウングレードスクリプトが取り返しがつかないオブジェクトを何か削除してしまう場合、望まない結果になってしまいます。␞␞    </para>␞
␝    <para>␟     To check for unexpected update paths, use this command:␟想定外の更新経路かどうかを検査するためには、以下のコマンドを使用してください。␞␞<programlisting>␞
␝</programlisting>␟     This shows each pair of distinct known version names for the specified
     ...
     format.␟これは指定した拡張の個々の既知のバージョン名の組み合わせをそれぞれ、元のバージョンから対象のバージョンへ進む時に取られる更新経路順、またはもし利用できる更新経路がなければ<literal>NULL</literal>を付けて、表示します。
経路は<literal>--</literal>を区切り文字として使用したテキスト形式で表示されます。
配列形式の方が良ければ<literal>regexp_split_to_array(path,'--')</literal>を使用できます。␞␞    </para>␞
␝   <sect2 id="extend-extensions-update-scripts">␟    <title>Installing Extensions Using Update Scripts</title>␟    <title>更新スクリプトを利用した拡張のインストール</title>␞␞␞
␝    <para>␟     An extension that has been around for awhile will probably exist in
     ...
     needed to produce small updates.␟以前から存在している拡張は、おそらく複数のバージョンに渡って存在しているので、拡張の作者は更新スクリプトを開発する必要性が出てきます。
...
この方法で拡張のスクリプトを調整することにより、小さな更新を複数作成するための保守の手間を減らすことができます。␞␞    </para>␞
␝    <para>␟     If you use secondary (version-specific) control files with an extension
     ...
     from <literal>1.0</literal> to another version.␟この方法で保守している拡張に二次的な（バージョン固有の）制御ファイルがある場合は、スタンドアローンのインストールスクリプトがない場合でも、各バージョンで制御ファイルが必要になることに注意してください。
そのバージョンへと更新する暗黙的な方法を、制御ファイルが決定するからです。
たとえば、<filename>foo--1.0.control</filename>が<literal>requires = 'bar'</literal>を指定しているのに、<literal>foo</literal>の他の制御ファイルが指定していないとすると、<literal>1.0</literal>から他のバージョンに更新した際に<literal>bar</literal>への依存性が削除されてしまうでしょう。␞␞    </para>␞
␝   <sect2 id="extend-extensions-security">␟    <title>Security Considerations for Extensions</title>␟    <title>拡張のためのセキュリティに関する考慮事項</title>␞␞␞
␝    <para>␟     Widely-distributed extensions should assume little about the database
     ...
     search-path-based attacks.␟広く配布される拡張では、インストールされるデータベースについて想定していないはずです。
このため、拡張はサーチパスに基づく攻撃を受けないよう、安全なスタイルで拡張によって提供される関数記述するのが適切です。␞␞    </para>␞
␝    <para>␟     An extension that has the <varname>superuser</varname> property set to
     ...
     user to acquire superuser privileges.␟<varname>superuser</varname>プロパティを真にしている拡張はインストールや更新スクリプトの中で行われるアクションのセキュリティ面での危険も考慮しなければなりません。
悪意あるユーザが不用意に書かれた拡張スクリプトを悪用してトロイの木馬を作成し、スーパーユーザ権限を獲得できるようにすることは、そう難しくありません。␞␞    </para>␞
␝    <para>␟     If an extension is marked <varname>trusted</varname>, then its
     ...
     examined to ensure that no compromise is possible.␟拡張が<varname>trusted</varname>と印付けされている場合、そのインストールスキーマはインストールするユーザにより選択できます。そのユーザはスーパーユーザ権限を獲得することを狙って意図的に安全でないスキーマを使用するかもしれません。
したがって、trustedの拡張はセキュリティ観点から極めて危険で、そのスクリプトのコマンドは危険性がないことを確実にするため注意深く検証されなければなりません。␞␞    </para>␞
␝    <para>␟     Advice about writing functions securely is provided in
     <xref linkend="extend-extensions-security-funcs"/> below, and advice
     about writing installation scripts securely is provided in
     <xref linkend="extend-extensions-security-scripts"/>.␟関数を安全に書くためのアドバイスは以下のリンクから提供されます。
<xref linkend="extend-extensions-security-funcs"/>
また、インストールスクリプトを安全に書くためのアドバイスは以下のリンクから提供されます。
<xref linkend="extend-extensions-security-scripts"/>␞␞    </para>␞
␝    <sect3 id="extend-extensions-security-funcs">␟     <title>Security Considerations for Extension Functions</title>␟     <title>拡張関数のためのセキュリティに関する考慮事項</title>␞␞␞
␝     <para>␟      SQL-language and PL-language functions provided by extensions are at
      risk of search-path-based attacks when they are executed, since
      parsing of these functions occurs at execution time not creation time.␟拡張により提供されるSQL言語とPL言語関数は実行されるときにサーチパスに基づく攻撃を受ける危険性があります。
これらの関数は作成時ではなく、実行時に解析されるためです。␞␞     </para>␞
...
␟␟     <!--
     ...
     -->␞␞     <para>␞
␝     <para>␟      The <link linkend="sql-createfunction-security"><command>CREATE
      ...
      user.␟<link linkend="sql-createfunction-security"><command>CREATE FUNCTION</command></link>のリファレンスページには<literal>SECURITY DEFINER</literal>関数を安全に書くためのアドバイスが記載されています。
拡張が提供するあらゆる関数は、強い権限を持つユーザから実行されることがあるので、これらのテクニックを適用することは、良い習慣です。␞␞     </para>␞
␝     <para>␟      If you cannot set the <varname>search_path</varname> to contain only
      ...
      and <literal>CASE WHEN <replaceable>expression</replaceable></literal>.␟<varname>search_path</varname>に安全なスキーマだけを設定できない場合は、修飾されていない名前は悪意あるユーザが定義したオブジェクトとして名前解決されうることを想定してください。
...
これらの場所には、<literal>OPERATOR(<replaceable>スキーマ</replaceable>.=) ANY</literal>や<literal>CASE WHEN <replaceable>式</replaceable></literal>を使用してください。␞␞     </para>␞
␝     <para>␟      A general-purpose extension usually should not assume that it's been
      ...
      match the declared argument types, using explicit casts where necessary.␟汎用の拡張は通常、安全なスキーマにインストールされることを想定するべきではありません。これはスキーマ修飾された自身のオブジェクトであっても完全にリスクがないわけではないことを意味しています。
...
必要に応じて明示的なキャストを使用して関数と演算子のデータ型が引数の型と厳密に一致するように注意してください。␞␞     </para>␞
␝    <sect3 id="extend-extensions-security-scripts">␟     <title>Security Considerations for Extension Scripts</title>␟     <title>拡張スクリプトのためのセキュリティに関する考慮事項</title>␞␞␞
␝     <para>␟      An extension installation or update script should be written to guard
      ...
      used.␟拡張のインストールや更新スクリプトはスクリプト実行時にサーチパスに基づく攻撃を防ぐように記述されなければなりません。
スクリプトが参照するオブジェクトがスクリプトの作者が意図したものではないオブジェクトとして解決されうる場合、即座もしくは、誤って定義された拡張オブジェクトが使われたときに攻撃を受ける可能性があります。␞␞     </para>␞
␝     <para>␟      DDL commands such as <command>CREATE FUNCTION</command>
      ...
      in <command>CREATE FUNCTION</command>.␟<command>CREATE FUNCTION</command>や<command>CREATE OPERATOR CLASS</command>などのDDLコマンドは一般的には安全ですが、汎用的な式を構成に持つコマンドには注意が必要です。
例えば、<command>CREATE FUNCTION</command>の<literal>DEFAULT</literal>式に行うのと同様に<command>CREATE VIEW</command>には審査が必要です。␞␞     </para>␞
␝     <para>␟      Sometimes an extension script might need to execute general-purpose
      ...
      the <productname>PostgreSQL</productname> source code distribution.␟拡張スクリプトには汎用SQLを実行する必要があることがあります。
...
例は配布される<productname>PostgreSQL</productname>ソースコードの<filename>contrib</filename>に見つけることができます。␞␞     </para>␞
␝     <para>␟      Cross-extension references are extremely difficult to make fully
      ...
      installed in <literal>pg_catalog</literal>.␟拡張をまたがる参照を完璧に安全にすることは極めて困難です。
...
今のところ、最良の手段は他の拡張に依存する拡張であるなら、依存する拡張が常に<literal>pg_catalog</literal>にインストールされるのでない限り、拡張をtrustedと印付けしないことです。␞␞     </para>␞
␝   <sect2 id="extend-extensions-example">␟    <title>Extension Example</title>␟    <title>拡張の例</title>␞␞␞
␝    <para>␟     Here is a complete example of an <acronym>SQL</acronym>-only
     ...
     values are automatically coerced to text for storage.␟ここでは、<acronym>SQL</acronym>のみの拡張の完全な例を示します。
<quote>k</quote>と<quote>v</quote>という名称の２つの要素からなる複合型であり、そのスロットには任意の型の値を格納できるものです。
格納の際テキスト以外の値は自動的にテキストに変換されます。␞␞    </para>␞
␝    <para>
     The script file <filename>pair--1.0.sql</filename> looks like this:

<programlisting><![CDATA[
-- complain if script is sourced in psql, rather than via CREATE EXTENSION
␟␟-- スクリプトが、CREATE EXTENSION経由ではなく、psqlのソースとして使われた場合には文句を言う␞␞\echo Use "CREATE EXTENSION pair" to load this file. \quit␞
␝     The script file <filename>pair--1.0.sql</filename> looks like this:␟     The script file <filename>pair--1.0.sql</filename> looks like this:␟<filename>pair--1.0.sql</filename>スクリプトファイルは以下のようになります。␞␞␞
␝    <para>␟     The control file <filename>pair.control</filename> looks like this:␟<filename>pair.control</filename>制御ファイルは以下のようになります。␞␞␞
␝    <para>␟     While you hardly need a makefile to install these two files into the
     correct directory, you could use a <filename>Makefile</filename> containing this:␟これらの２つのファイルを正しいディレクトリにインストールするためにmakefileを作成する必要はほとんどありませんが、以下を含む<filename>Makefile</filename>を使用できます。␞␞␞
␝␟     This makefile relies on <acronym>PGXS</acronym>, which is described
     ...
     directory as reported by <application>pg_config</application>.␟このmakefileは<xref linkend="extend-pgxs"/>で説明する<acronym>PGXS</acronym>に依存します。
<literal>make install</literal>コマンドは制御ファイルとスクリプトファイルを<application>pg_config</application>で報告される正しいディレクトリにインストールします。␞␞    </para>␞
␝    <para>␟     Once the files are installed, use the
     <command>CREATE EXTENSION</command> command to load the objects into
     any particular database.␟ファイルがインストールされた後、<command>CREATE EXTENSION</command>コマンドを使用してオブジェクトを任意の特定のデータベースにロードしてください。␞␞    </para>␞
␝  <sect1 id="extend-pgxs">␟   <title>Extension Building Infrastructure</title>␟   <title>拡張構築基盤</title>␞␞␞
␝   <para>␟    If you are thinking about distributing your
    ...
    own build system.␟<productname>PostgreSQL</productname>拡張モジュールの配布を考えているのであれば、移植可能な構築システムを準備することはかなり難しいものになるかもしれません。
このため<productname>PostgreSQL</productname>インストレーションは単純な拡張モジュールをすでにインストールされているサーバに対して簡単に構築できるように、<acronym>PGXS</acronym>と呼ばれる拡張向けの構築基盤を提供します。
<acronym>PGXS</acronym>は主にCコードを含む拡張を意図していますが、SQLのみからなる拡張でも使用できます。
...
より複雑なパッケージでは、独自の構築システムを作成する必要があるかもしれません。␞␞   </para>␞
␝   <para>␟    To use the <acronym>PGXS</acronym> infrastructure for your extension,
    ...
    without going via SQL), and a documentation text file:␟独自の拡張で<acronym>PGXS</acronym>基盤を使用するためには、簡単なmakefileを作成する必要があります。
...
このモジュールはいくつかのCコードを含む共有ライブラリ、拡張の制御ファイル、SQLスクリプト、インクルードファイル（他のモジュールが拡張の関数にSQLを経由せずにアクセスする必要があるかもしれない場合にのみ必要です）、ドキュメントテキストファイルから構成されます。␞␞<programlisting>␞
␝</programlisting>␟    The last three lines should always be the same.  Earlier in the
    file, you assign variables or add custom
    <application>make</application> rules.␟最後の３行は常に同じです。
ファイルのこの前に変数の設定と独自の<application>make</application>ルールを記載してください。␞␞   </para>␞
␝   <para>␟    Set one of these three variables to specify what is built:␟以下の３個の変数の１つを構築対象に指定してください。␞␞␞
␝       <para>␟        list of shared-library objects to be built from source files with same
        stem (do not include library suffixes in this list)␟同じ家系のソースファイルから構築される共有ライブラリのリストです。
（このリストにはライブラリ接頭辞を含めないでください。）␞␞       </para>␞
␝       <para>␟        a shared library to build from multiple source files
        (list object files in <varname>OBJS</varname>)␟複数のソースファイルから構築される共有ライブラリです。
（<varname>OBJS</varname>にオブジェクトファイルを列挙します。）␞␞       </para>␞
␝       <para>␟        an executable program to build
        (list object files in <varname>OBJS</varname>)␟構築する実行プログラムです。
（<varname>OBJS</varname>にオブジェクトファイルを列挙します。）␞␞       </para>␞
␝␟    The following variables can also be set:␟以下の変数も設定できます。␞␞␞
␝       <para>␟        extension name(s); for each name you must provide an
        <literal><replaceable>extension</replaceable>.control</literal> file,
        which will be installed into
        <literal><replaceable>prefix</replaceable>/share/extension</literal>␟拡張の名前です。
各名前に対して、<literal><replaceable>prefix</replaceable>/share/extension</literal>にインストールされる<literal><replaceable>extension</replaceable>.control</literal>ファイルを提供しなければなりません。␞␞       </para>␞
␝       <para>␟        subdirectory of <literal><replaceable>prefix</replaceable>/share</literal>
        ...
        or <literal>contrib</literal> if not)␟DATAおよびDOCSファイルのインストール先となるはずの<literal><replaceable>prefix</replaceable>/share</literal>サブディレクトリです。
（設定がない場合、デフォルトは<varname>EXTENSION</varname>が設定されている場合は<literal>extension</literal>に、設定されていない場合は<literal>contrib</literal>になります。）␞␞       </para>␞
␝       <para>␟        random files to install into <literal><replaceable>prefix</replaceable>/share/$MODULEDIR</literal>␟<literal><replaceable>prefix</replaceable>/share/$MODULEDIR</literal>にインストールされる様々なファイルです。␞␞       </para>␞
␝       <para>␟        random files to install into
        <literal><replaceable>prefix</replaceable>/share/$MODULEDIR</literal>,
        which need to be built first␟<literal><replaceable>prefix</replaceable>/share/$MODULEDIR</literal>にインストールされる、最初に構築しなければならない様々なファイルです。␞␞       </para>␞
␝       <para>␟        random files to install under
        <literal><replaceable>prefix</replaceable>/share/tsearch_data</literal>␟<literal><replaceable>prefix</replaceable>/share/tsearch_data</literal>以下にインストールされる様々なファイルです。␞␞       </para>␞
␝       <para>␟        random files to install under
        <literal><replaceable>prefix</replaceable>/doc/$MODULEDIR</literal>␟<literal><replaceable>prefix</replaceable>/doc/$MODULEDIR</literal>以下にインストールされる様々なファイルです。␞␞       </para>␞
␝       <para>␟        Files to (optionally build and) install under
        <literal><replaceable>prefix</replaceable>/include/server/$MODULEDIR/$MODULE_big</literal>.␟（必要に応じてビルドして）<literal><replaceable>prefix</replaceable>/include/server/$MODULEDIR/$MODULE_big</literal>以下にインストールをするファイル。␞␞       </para>␞
␝       <para>␟        Unlike <literal>DATA_built</literal>, files in <literal>HEADERS_built</literal>
        are not removed by the <literal>clean</literal> target; if you want them removed,
        also add them to <literal>EXTRA_CLEAN</literal> or add your own rules to do it.␟<literal>DATA_built</literal>と違って、<literal>HEADERS_built</literal>のファイルは<literal>clean</literal>ターゲットによって削除されません。削除したい場合には、それらを<literal>EXTRA_CLEAN</literal>にも加えるか、削除を行う独自のルールを追加してください。␞␞       </para>␞
␝       <para>␟        Files to install (after building if specified) under
        ...
        in <literal>MODULES</literal> or <literal>MODULE_big</literal>.␟（指定されていたならビルド後に）<literal><replaceable>prefix</replaceable>/include/server/$MODULEDIR/$MODULE</literal>の下にインストールするファイル。ここでの<literal>$MODULE</literal>は<literal>MODULES</literal>か<literal>MODULE_big</literal>で使われているモジュール名でなければなりません。␞␞       </para>␞
␝       <para>␟        Unlike <literal>DATA_built</literal>, files in <literal>HEADERS_built_$MODULE</literal>
        are not removed by the <literal>clean</literal> target; if you want them removed,
        also add them to <literal>EXTRA_CLEAN</literal> or add your own rules to do it.␟<literal>DATA_built</literal>と違って、<literal>HEADERS_built_$MODULE</literal>のファイルは<literal>clean</literal>ターゲットによって削除されません。削除したい場合には、これらを<literal>EXTRA_CLEAN</literal>にも加えるか、削除を行う独自のルールを追加してください。␞␞       </para>␞
␝       <para>␟        It is legal to use both variables for the same module, or any
        ...
        <literal>HEADERS_built_$MODULE</literal> variables.␟同じモジュールあるいは任意の組み合わせに対して両方の変数を使うことは正当ですが、<literal>MODULES</literal>リストにプレフィックス<literal>built_</literal>の有無しか異ならない二つのモジュール名を書く場合を除きます。これは両義解釈をひき起こすでしょう。
このような（おそらくありそうにない）場合、<literal>HEADERS_built_$MODULE</literal>変数だけを使うべきです。␞␞       </para>␞
␝       <para>␟        script files (not binaries) to install into
        <literal><replaceable>prefix</replaceable>/bin</literal>␟<literal><replaceable>prefix</replaceable>/bin</literal>にインストールされるスクリプトファイルです（バイナリファイルではありません）。␞␞       </para>␞
␝       <para>␟        script files (not binaries) to install into
        <literal><replaceable>prefix</replaceable>/bin</literal>,
        which need to be built first␟<literal><replaceable>prefix</replaceable>/bin</literal>にインストールされる、最初に構築しなければならないスクリプトファイルです（バイナリファイルではありません）。␞␞       </para>␞
␝       <para>␟        list of regression test cases (without suffix), see below␟リグレッションテストケース（接尾辞がない）のリストです。
後述します。␞␞       </para>␞
␝       <para>␟        additional switches to pass to <application>pg_regress</application>␟<application>pg_regress</application>に渡す追加オプションです。␞␞       </para>␞
␝       <para>␟        list of isolation test cases, see below for more details␟分離性試験のリストです。
詳細は後述します。␞␞       </para>␞
␝       <para>␟        additional switches to pass to
        <application>pg_isolation_regress</application>␟<application>pg_isolation_regress</application>に渡す追加オプションです。␞␞       </para>␞
␝       <para>␟        switch defining if TAP tests need to be run, see below␟TAPテストを実行する必要があるかどうかを定義するオプションです。
後述します。␞␞       </para>␞
␝       <para>␟        don't define an <literal>install</literal> target, useful for test
        modules that don't need their build products to be installed␟<literal>install</literal>ターゲットを定義しません。
構築物をインストールする必要のないテストモジュールに有用です。␞␞       </para>␞
␝       <para>␟        don't define an <literal>installcheck</literal> target, useful e.g., if tests require special configuration, or don't use <application>pg_regress</application>␟<literal>installcheck</literal>ターゲットを定義しません。
テストの際に特殊な設定が必要、あるいは<application>pg_regress</application>を使用しない場合などに有用です。␞␞       </para>␞
␝       <para>␟        extra files to remove in <literal>make clean</literal>␟<literal>make clean</literal>で削除される追加ファイルです。␞␞       </para>␞
␝       <para>␟        will be prepended to <varname>CPPFLAGS</varname>␟<varname>CPPFLAGS</varname>の先頭に加えられます。␞␞       </para>␞
␝       <para>␟        will be appended to <varname>CFLAGS</varname>␟<varname>CFLAGS</varname>に加えられます。␞␞       </para>␞
␝       <para>␟        will be appended to <varname>CXXFLAGS</varname>␟<varname>CXXFLAGS</varname>に加えられます。␞␞       </para>␞
␝       <para>␟        will be prepended to <varname>LDFLAGS</varname>␟<varname>LDFLAGS</varname>の先頭に加えられます。␞␞       </para>␞
␝       <para>␟        will be added to <varname>PROGRAM</varname> link line␟<varname>PROGRAM</varname>のリンク行に追加されます。␞␞       </para>␞
␝       <para>␟        will be added to <varname>MODULE_big</varname> link line␟<varname>MODULE_big</varname>リンク行に追加されます。␞␞       </para>␞
␝       <para>␟        path to <application>pg_config</application> program for the
        <productname>PostgreSQL</productname> installation to build against
        (typically just <literal>pg_config</literal> to use the first one in your
        <varname>PATH</varname>)␟構築対象の<productname>PostgreSQL</productname>インストレーション用の<application>pg_config</application>プログラムへのパスです。
（通常は<varname>PATH</varname>内の最初に見つかる<literal>pg_config</literal>が単純に使用されます）␞␞       </para>␞
␝   <para>␟    Put this makefile as <literal>Makefile</literal> in the directory
    ...
    or on the <literal>make</literal> command line.␟このmakefileを<literal>Makefile</literal>として拡張を保管するディレクトリ内に保管してください。
...
makefileまたは<literal>make</literal>のコマンドラインのいずれかで<varname>PG_CONFIG</varname>を別の<command>pg_config</command>プログラムを指し示すように設定することで、別のインストレーションを使用できます。␞␞   </para>␞
␝   <para>␟    You can also run <literal>make</literal> in a directory outside the source
    tree of your extension, if you want to keep the build directory separate.
    This procedure is also called a
    <indexterm><primary>VPATH</primary></indexterm><firstterm>VPATH</firstterm>
    build.  Here's how:␟構築ディレクトリを別にしておきたいのであれば、拡張のソースツリーの外のディレクトリで<literal>make</literal>を実行することもできます。
この方法は<indexterm><primary>VPATH</primary></indexterm><firstterm>VPATH</firstterm>構築とも呼ばれます。
以下にやり方を示します。␞␞<programlisting>␞
␝   <para>␟    Alternatively, you can set up a directory for a VPATH build in a similar
    ...
    <varname>VPATH</varname> like this:␟あるいは、コアコードと同様な方法でVPATH構築用のディレクトリを設定できます。
そのようにする1つの方法は、コアスクリプト<filename>config/prep_buildtree</filename>を使うことです。
一度そうすれば、<literal>make</literal>変数<varname>VPATH</varname>を以下のように設定することで、構築できます。␞␞<programlisting>␞
␝</programlisting>␟    This procedure can work with a greater variety of directory layouts.␟この方法はより様々なディレクトリのレイアウトで機能します。␞␞   </para>␞
␝   <para>␟    The scripts listed in the <varname>REGRESS</varname> variable are used for
    ...
    have all expected files.␟<varname>REGRESS</varname>変数に列挙されたスクリプトは、<literal>make install</literal>を実行した後で<literal>make installcheck</literal>によって呼び出すことができる、作成したモジュールのリグレッションテストで使用されます。
...
このためすべての想定ファイルがあることを確認してください。␞␞   </para>␞
␝   <para>␟    The scripts listed in the <varname>ISOLATION</varname> variable are used
    ...
    make sure you have all expected files.␟<varname>ISOLATION</varname>変数に列挙されたスクリプトは、<literal>make install</literal>を実行した後で<literal>make installcheck</literal>によって呼び出すことができるモジュールでの同時実行中のセッションの振舞いの負荷テストで使用されます。
これが動作するためには、<productname>PostgreSQL</productname>サーバが実行していなければなりません。
...
想定ファイルがない試験を実行しようとすると<quote>問題</quote>として報告されます。
このためすべての想定ファイルがあることを確認してください。␞␞   </para>␞
␝   <para>␟    <literal>TAP_TESTS</literal> enables the use of TAP tests.  Data from each
    run is present in a subdirectory named <literal>tmp_check/</literal>.
    See also <xref linkend="regress-tap"/> for more details.␟ <literal>TAP_TESTS</literal>はTAPテストの指定を有効にします。
各試験の実行によるデータは<literal>tmp_check/</literal>という名前のサブディレクトリに含まれます。
詳細は<xref linkend="regress-tap"/>を参照してください。␞␞   </para>␞
␝    <para>␟     The easiest way to create the expected files is to create empty files,
     ...
     <literal>expected/</literal> if they match what you expect from the test.␟想定ファイルを作成する最も簡単な方法は、空のファイルを作成し、試験を実行する（当然差異が報告されます）ことです。
（<literal>REGRESS</literal>の試験による）<literal>results/</literal>ディレクトリまたは（<literal>ISOLATION</literal>の試験による）<literal>output_iso/results/</literal>ディレクトリ内で見つかる実際の結果ファイルを確認し、テストの想定結果と合致するのであれば、<literal>expected/</literal>にコピーしてください。␞␞    </para>␞
