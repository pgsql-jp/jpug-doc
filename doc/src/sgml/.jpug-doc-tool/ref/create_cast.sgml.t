...
␝  <refname>CREATE CAST</refname>␟  <refpurpose>define a new cast</refpurpose>␟  <refpurpose>新しいキャストを定義する</refpurpose>␞␞ </refnamediv>␞
␝ <refsect1 id="sql-createcast-description">␟  <title>Description</title>␟  <title>説明</title>␞␞␞
␝  <para>␟   <command>CREATE CAST</command> defines a new cast.  A cast
   specifies how to perform a conversion between
   two data types.  For example,␟<command>CREATE CAST</command>を使用すると、新しいキャストを定義できます。
キャストは、2つのデータ型間の変換処理方法を指定するものです。
以下に例を示します。␞␞<programlisting>␞
␝</programlisting>␟   converts the integer constant 42 to type <type>float8</type> by
   invoking a previously specified function, in this case
   <literal>float8(int4)</literal>. (If no suitable cast has been defined, the
   conversion fails.)␟この文を実行すると、事前に指定された関数（この場合<literal>float8(int4)</literal>）が呼び出され、整数定数42が<type>float8</type>型に変換されます。
（適切なキャストが定義されていない場合、変換処理は失敗します。）␞␞  </para>␞
␝  <para>␟   Two types can be <firstterm>binary coercible</firstterm>, which
   ...
   binary compatible.)␟2つのデータ型を<firstterm>バイナリ強制互換</firstterm>とすることができます。
...
（2つの型が両方向でバイナリ強制互換であることは、バイナリ互換性と呼ばれます。）␞␞  </para>␞
␝  <para>␟   You can define a cast as an <firstterm>I/O conversion cast</firstterm> by using
   ...
   a regular function-based cast; only the implementation is different.␟<literal>WITH INOUT</literal>構文を使用して<firstterm>I/O変換キャスト</firstterm>としてキャスト定義を行うことができます。
I/O変換キャストは、元データ型の出力関数を呼び出し、その結果文字列を対象データ型の入力関数に渡すことで行われます。
多くの一般的な場合では、この機能により変換用に別個のキャスト関数を作成する必要性がなくなります。
I/O変換キャストは通常の関数を基にしたキャストと同様に動作します。ただ実装が異なるだけです。␞␞  </para>␞
␝  <para>␟   By default, a cast can be invoked only by an explicit cast request,
   ...
   construct.␟デフォルトでは、キャストは明示的なキャスト要求があった場合のみ発生します。
明示的なキャスト要求の構文は、<literal>CAST(<replaceable>x</replaceable> AS <replaceable>typename</replaceable>)</literal>、もしくは、<replaceable>x</replaceable><literal>::</literal><replaceable>typename</replaceable>式です。␞␞  </para>␞
␝  <para>␟   If the cast is marked <literal>AS ASSIGNMENT</literal> then it can be invoked
   implicitly when assigning a value to a column of the target data type.
   For example, supposing that <literal>foo.f1</literal> is a column of
   type <type>text</type>, then:␟キャストに<literal>AS ASSIGNMENT</literal>オプションを付けると、対象データ型の列に代入する際、暗黙的にそのキャストを発生させることができます。
例えば、<literal>foo.f1</literal>が<type>text</type>型の列であるとします。␞␞<programlisting>␞
␝</programlisting>␟   will be allowed if the cast from type <type>integer</type> to type
   <type>text</type> is marked <literal>AS ASSIGNMENT</literal>, otherwise not.
   (We generally use the term <firstterm>assignment
   cast</firstterm> to describe this kind of cast.)␟<type>integer</type>型を<type>text</type>型に変換するキャストに<literal>AS ASSIGNMENT</literal>オプションが付けられていれば、上記のSQL文が実行できます。
しかし、<literal>AS ASSIGNMENT</literal>オプションが付いていなければ、実行できません。
（一般的に、この種のキャストを<firstterm>代入キャスト</firstterm>と呼びます。）␞␞  </para>␞
␝  <para>␟   If the cast is marked <literal>AS IMPLICIT</literal> then it can be invoked
   implicitly in any context, whether assignment or internally in an
   expression.  (We generally use the term <firstterm>implicit
   cast</firstterm> to describe this kind of cast.)
   For example, consider this query:␟キャストに<literal>AS IMPLICIT</literal>オプションを付けると、代入の場合だけでなく、式の中にある場合でも、全てのコンテキストで暗黙的にそのキャストを呼び出すことができます。
（一般的に、この種のキャストを<firstterm>暗黙キャスト</firstterm>と呼びます。）
例えば次のような問い合わせを考えてみます。␞␞<programlisting>␞
␝</programlisting>␟   The parser initially marks the constants as being of type <type>integer</type>
   ...
   the query as if it had been written␟パーサはまず定数にそれぞれ<type>integer</type>と<type>numeric</type>であると印を付けます。
...
パーサは暗黙的なキャストを行い、問い合わせをあたかも次のように記載されたものとして解決します。␞␞<programlisting>␞
␝  <para>␟   Now, the catalogs also provide a cast from <type>numeric</type> to
   ...
   <type>numeric</type>; there is no built-in knowledge about that.␟ここで、カタログはまた<type>numeric</type>から<type>integer</type>へのキャストも提供しています。
...
これに関する組み込まれた知見は存在しません。␞␞  </para>␞
␝  <para>␟   It is wise to be conservative about marking casts as implicit.  An
   ...
   to <type>int4</type>, are best made explicit-only.␟暗黙キャストは、多用しない方が賢明です。
...
<type>text</type>型から<type>int4</type>型への変換のような、カテゴリを越えるデータ型のキャストは、明示的にのみ使用するのが適切です。␞␞  </para>␞
␝   <para>␟    Sometimes it is necessary for usability or standards-compliance reasons
    ...
    more information.␟型の集合の中で複数の暗黙的なキャストを提供することが、有用性や標準との互換性上の理由により必要となることがあり、これにより、上で説明した通り防ぐことができないあいまいさが引き起こされます。
パーサは、こうした状況でも望ましい動作の提供を補助できる<firstterm>型カテゴリ</firstterm>と<firstterm>優先される型</firstterm>に基づいた発見的手法を用意しています。
詳細は<xref linkend="sql-createtype"/>を参照してください。␞␞   </para>␞
␝  <para>␟   To be able to create a cast, you must own the source or the target data type
   ...
   server.)␟キャストを作成するためには、変換元または変換先（の内の一方）のデータ型を所有し、もう一方の型に対する<literal>USAGE</literal>権限を持つ必要があります。
また、バイナリ強制互換性を持つキャストを作成できるのは、スーパーユーザでなければなりません。
（バイナリ強制互換性があるキャスト変換を誤って使用するとサーバがクラッシュしてしまう可能性が高いことから、この制限が付けられました）。␞␞  </para>␞
␝ <refsect1>␟  <title>Parameters</title>␟  <title>パラメータ</title>␞␞␞
␝      <para>␟       The name of the source data type of the cast.␟キャストする変換元のデータ型の名前です。␞␞      </para>␞
␝      <para>␟       The name of the target data type of the cast.␟キャストする変換先のデータ型の名前です。␞␞      </para>␞
␝      <para>␟       The function used to perform the cast.  The function name can
       be schema-qualified.  If it is not, the function will be looked
       up in the schema search path.  The function's result data type must
       match the target type of the cast.   Its arguments are discussed below.
       If no argument list is specified, the function name must be unique in
       its schema.␟キャストを実行するために使用される関数です。
関数名はスキーマ修飾することができます。
スキーマ修飾されていない場合、関数はスキーマ検索パスから検索されます。
関数の結果のデータ型は、キャストの変換先のデータ型と一致する必要があります。
引数については後で説明します。
引数リストが指定されない場合、関数名はスキーマ内で一意でなければなりません。␞␞      </para>␞
␝      <para>␟       Indicates that the source type is binary-coercible to the target type,
       so no function is required to perform the cast.␟変換元データ型から変換先データ型への間に、バイナリ強制互換性があることを示します。
この場合、キャストを実行するのに関数は必要ありません。␞␞      </para>␞
␝      <para>␟       Indicates that the cast is an I/O conversion cast, performed by
       invoking the output function of the source data type, and passing the
       resulting string to the input function of the target data type.␟キャストが、変換元データ型の出力関数を呼び出し、その結果の文字列を変換先データ型の入力関数に渡すことで行われる、I/O変換キャストであることを示します。␞␞      </para>␞
␝      <para>␟       Indicates that the cast can be invoked implicitly in assignment
       contexts.␟代入コンテキストで、暗黙的にキャストを呼び出せることを示します。␞␞      </para>␞
␝      <para>␟       Indicates that the cast can be invoked implicitly in any context.␟任意のコンテキストで、暗黙的にキャストを呼び出せることを示します。␞␞      </para>␞
␝  <para>␟   Cast implementation functions can have one to three arguments.
   ...
   your own data types so that this matters.)␟キャストを実装する関数は1〜3個の引数を取ることができます。
...
独自のデータ型をこの流儀に従うように設計することは勧められません。）␞␞  </para>␞
␝  <para>␟   The return type of a cast function must be identical to or
   binary-coercible to the cast's target type.␟キャスト関数の戻り値は、キャストの対象型と同一またはバイナリ強制互換性を持たなければなりません。␞␞  </para>␞
␝  <para>␟   Ordinarily a cast must have different source and target data types.
   However, it is allowed to declare a cast with identical source and
   target types if it has a cast implementation function with more than one
   argument.  This is used to represent type-specific length coercion
   functions in the system catalogs.  The named function is used to
   coerce a value of the type to the type modifier value given by its
   second argument.␟通常、キャストにおける変換元データ型と変換先データ型は異なる必要があります。
しかし、2つ以上の引数を持つ関数でキャストを実装した場合は、変換元と変換先とで同一のデータ型を持つキャストを宣言することができます。
これは、システムカタログにおいて型固有の長さ強制関数を表現するために使用されています。
指定された関数は、型の値を強制的に2番目の引数で与えられた型修飾子の値にするために使用されます。␞␞  </para>␞
␝  <para>␟   When a cast has different source and
   ...
   convert between data types and a second to apply the modifier.␟キャストが変換元と変換先のデータ型が異なり、複数の引数を取る関数を持つ場合、あるデータ型から他のデータ型への変換と長さの強制を1つの操作にまとめたものをサポートします。
引数を1つしか取らない場合は、型修飾子を使用して型を強制するために、データ型間の変換と修飾子の適用という2つのキャスト操作が必要となります。␞␞  </para>␞
␝  <para>␟   A cast to or from a domain type currently has no effect.  Casting
   to or from a domain uses the casts associated with its underlying type.␟ドメイン型へのキャスト、ドメイン型からのキャストは現在は効果がありません。
ドメインへのキャスト、ドメインからのキャストは、基となる型と関連したキャストを使用します。␞␞  </para>␞
␝ <refsect1 id="sql-createcast-notes">␟  <title>Notes</title>␟  <title>注釈</title>␞␞␞
␝  <para>␟   Use <link linkend="sql-dropcast"><command>DROP CAST</command></link> to remove user-defined casts.␟ユーザ定義のキャストを削除するには<link linkend="sql-dropcast"><command>DROP CAST</command></link>を使用してください。␞␞  </para>␞
...
␝  <para>␟   Remember that if you want to be able to convert types both ways you
   need to declare casts both ways explicitly.␟データ型を双方向に変更可能にするには、双方向のキャストを明示的に宣言する必要があることに注意してください。␞␞  </para>␞
␝  <para>␟   It is normally not necessary to create casts between user-defined types
   ...
   mostly because of requirements of the SQL standard.)␟ユーザ定義型と標準文字列型（<type>text</type>、<type>varchar</type>、<type>char(<replaceable>n</replaceable>)</type>）、および文字列カテゴリとして定義されたユーザ定義型との間のキャストを作成することは、通常必要ありません。
...
（組み込み型のごく一部は実際変換用に異なった振舞いをしますが、ほとんどは標準SQLの仕様のためのものです。）␞␞  </para>␞
␝  <para>␟   While not required, it is recommended that you continue to follow this old
   ...
   target type's name.␟必須ではありませんが、キャストを実装する関数には変換先のデータ型の名前を付けるという以前からの慣習に従っておくことを推奨します。
...
<productname>PostgreSQL</productname>は引数として異なる型を取る同じ名前の関数をオーバーロードすることができるので、様々な型から特定の変換先型への変換関数の名前を全て変換先の型名にしても特に問題は発生しません。␞␞  </para>␞
␝   <para>␟    Actually the preceding paragraph is an oversimplification: there are
    ...
    syntax.␟実際のところ、前の段落は単純化しすぎたものです。
...
この例外により関数のような構文で入出力変換キャストができるようになります。␞␞   </para>␞
␝   <para>␟    There is also an exception to the exception: I/O conversion casts from
    ...
    a function or column reference was intended.␟この例外にも例外があります。
複合型から文字列型へのI/O変換キャストでは関数構文を使用して呼び出すことができず、明示的なキャスト構文（<literal>CAST</literal>記法または<literal>::</literal>記法のいずれか）で記述しなければなりません。
この例外は、自動提供I/O変換キャストを導入した後、関数または列参照を意図した時に非常に簡単に間違って呼び出されることが判明したため追加されました。␞␞   </para>␞
␝ <refsect1 id="sql-createcast-examples">␟  <title>Examples</title>␟  <title>例</title>␞␞␞
␝  <para>␟   To create an assignment cast from type <type>bigint</type> to type
   <type>int4</type> using the function <literal>int4(bigint)</literal>:␟関数<literal>int4(bigint)</literal>を使用した<type>bigint</type>型から<type>int4</type>型への代入キャストを作成します。␞␞<programlisting>␞
␝</programlisting>␟   (This cast is already predefined in the system.)␟（このキャストは、システムに既に定義されています。）␞␞  </para>␞
␝ <refsect1 id="sql-createcast-compat">␟  <title>Compatibility</title>␟  <title>互換性</title>␞␞␞
␝  <para>␟   The <command>CREATE CAST</command> command conforms to the
   ...
   extension, too.␟SQLではバイナリ強制互換性があるデータ型や実装関数の追加の引数について規定されていません。さらに、<literal>AS IMPLICIT</literal>は、<productname>PostgreSQL</productname>の拡張です。
これらの点以外では、<command>CREATE CAST</command>は標準<acronym>SQL</acronym>に準拠しています。␞␞  </para>␞
␝ <refsect1 id="sql-createcast-seealso">␟  <title>See Also</title>␟  <title>関連項目</title>␞␞␞
␝␟<xref linkend="sql-createfunction"/>, <xref linkend="sql-createtype"/>,␟no translation␞␞␞
