...
␝  <refname>NOTIFY</refname>␟  <refpurpose>generate a notification</refpurpose>␟  <refpurpose>通知を生成する</refpurpose>␞␞ </refnamediv>␞
␝ <refsect1>␟  <title>Description</title>␟  <title>説明</title>␞␞␞
␝  <para>␟   The <command>NOTIFY</command> command sends a notification event together
   ...
   Notifications are visible to all users.␟<command>NOTIFY</command>コマンドは、現在のデータベース内で事前に指定チャネル名について<command>LISTEN <replaceable class="parameter">channel</replaceable></command>コマンドを実行したクライアントアプリケーションに<quote>ペイロード</quote>文字列(省略可能)を持つ通知イベントを送ります。
通知はすべてのユーザから可視です。␞␞  </para>␞
␝  <para>␟   <command>NOTIFY</command> provides a simple
   ...
   tables in the database to pass additional data from notifier to listener(s).␟<command>NOTIFY</command>は同一の<productname>PostgreSQL</productname>データベースにアクセスするプロセスの集合に対する単純なプロセス間通信の仕組みを提供します。
通知の際にペイロード文字列を送信することができます。
また、データベース内のテーブルを使用して通知者から（1つまたは複数の）リスナに追加的なデータを渡すことにより、構造化されたデータを渡す高度な仕組みを構築することができます。␞␞  </para>␞
␝  <para>␟   The information passed to the client for a notification event includes the
   notification channel
   name, the notifying session's server process <acronym>PID</acronym>, and the
   payload string, which is an empty string if it has not been specified.␟通知イベントとしてクライアントに渡される情報には、通知チャネル名と通知を行うセッションのサーバプロセスの<acronym>PID</acronym>、ペイロード文字列(指定されていなければ空文字列)が含まれます。␞␞  </para>␞
␝  <para>␟   It is up to the database designer to define the channel names that will
   ...
   the payload string could be used to differentiate various cases.␟各データベースにおいて使用される通知チャネル名とその意味についての定義は、データベース設計者に任されています。
...
他の方法としてペイロード文字列を使用して各種様々な状況に対応させることもできます。␞␞  </para>␞
␝  <para>␟   When <command>NOTIFY</command> is used to signal the occurrence of changes
   to a particular table, a useful programming technique is to put the
   <command>NOTIFY</command> in a statement trigger that is triggered by table updates.
   In this way, notification happens automatically when the table is changed,
   and the application programmer cannot accidentally forget to do it.␟特定のテーブルが変更されたことを通知するために<command>NOTIFY</command>を使用する場合、<command>NOTIFY</command>をテーブル更新時に発行される文トリガ内に配置すると便利です。
こうすると、通知はテーブルが変更された時に自動的に行われるので、アプリケーションプログラマが通知の実行を忘れるといった事故を防ぐことができます。␞␞  </para>␞
␝  <para>␟   <command>NOTIFY</command> interacts with SQL transactions in some important
   ...
   should try to keep their transactions short.␟<command>NOTIFY</command>とSQLトランザクションの間には、いくつかの重要な相互作用があります。
...
結論として、<command>NOTIFY</command>を使用してシグナルの実時間処理をするアプリケーションではトランザクションを短くしておかなければなりません。␞␞  </para>␞
␝  <para>␟   If the same channel name is signaled multiple times with identical
   ...
   the order in which the transactions committed.␟同じチャネル名が、同一トランザクションから同じペイロード文字列で複数回通知される場合、1つの通知インスタンスのみをリスナに伝えます。
...
また異なるトランザクションからのメッセージがトランザクションのコミット順で配送されることも保証します。␞␞  </para>␞
␝  <para>␟   It is common for a client that executes <command>NOTIFY</command>
   ...
   back, and can be ignored.␟<command>NOTIFY</command>を実行するクライアント自身が、その通知の通知チャネルを監視していることはよくあります。
...
<acronym>PID</acronym>が同じであれば、その通知イベントは自分自身から跳ね返ってきたものであり、無視することができます。␞␞  </para>␞
␝ <refsect1>␟  <title>Parameters</title>␟  <title>パラメータ</title>␞␞␞
␝     <para>␟      Name of the notification channel to be signaled (any identifier).␟シグナルとして送られる通知チャネル名です（任意の識別子）。␞␞     </para>␞
␝     <para>␟      The <quote>payload</quote> string to be communicated along with the
      ...
      it's best to put it in a database table and send the key of the record.)␟通知と一緒に通信される<quote>ペイロード</quote>文字列です。
これは単純な文字列リテラルとして指定されなければなりません。
デフォルトの設定では、8000バイト未満でなければなりません。
(バイナリデータまたは大規模な情報を渡さなければならないのであれば、データベーステーブル内に格納しレコードのキーを送信することが最善です。)␞␞     </para>␞
␝ <refsect1>␟  <title>Notes</title>␟  <title>注釈</title>␞␞␞
␝  <para>␟   There is a queue that holds notifications that have been sent but not
   ...
   current transaction so that cleanup can proceed.␟送信済みだがすべての監視セッションでは処理されていない通知を保持するためのキューが存在します。
...
この場合、クリーンアップ処理が進むように、確実にそのセッションでその現在のトランザクションを完了させるようにしなければなりません。␞␞  </para>␞
␝  <para>␟   The function <function>pg_notification_queue_usage</function> returns the
   fraction of the queue that is currently occupied by pending notifications.
   See <xref linkend="functions-info"/> for more information.␟関数<function>pg_notification_queue_usage</function>は現在、保留中の通知によって占められているキューの割合を返します。
詳細な情報については<xref linkend="functions-info"/>を参照してください。␞␞  </para>␞
␝  <para>␟   A transaction that has executed <command>NOTIFY</command> cannot be
   prepared for two-phase commit.␟<command>NOTIFY</command>を実行したトランザクションでは二相コミットを準備することはできません。␞␞  </para>␞
␝   <para>␟    To send a notification you can also use the function
    ...
    non-constant channel names and payloads.␟通知を送信するために関数<literal><function>pg_notify</function>(<type>text</type>,<type>text</type>)</literal>を使用することもできます。
この関数は第1引数としてチャネル名、第2引数としてペイロードを取ります。
不定のチャネル名、ペイロードで作業しなければならない場合は、<command>NOTIFY</command>コマンドよりこの関数を使用する方がかなり簡単です。␞␞   </para>␞
␝ <refsect1>␟  <title>Examples</title>␟  <title>例</title>␞␞␞
␝  <para>␟   Configure and execute a listen/notify sequence from
   <application>psql</application>:␟<application>psql</application>から監視/通知処理の設定と実行を行います。␞␞␞
␝ <refsect1>␟  <title>Compatibility</title>␟  <title>互換性</title>␞␞␞
␝  <para>␟   There is no <command>NOTIFY</command> statement in the SQL
   standard.␟標準SQLには<command>NOTIFY</command>はありません。␞␞  </para>␞
␝ <refsect1>␟  <title>See Also</title>␟  <title>関連項目</title>␞␞␞
␝␟LISTEN virtual; NOTIFY virtual; Asynchronous notification "virtual" received from server process with PID 8448. NOTIFY virtual, 'This is the payload'; Asynchronous notification "virtual" with payload "This is the payload" received from server process with PID 8448. LISTEN foo; SELECT pg_notify('fo' || 'o', 'pay' || 'load'); Asynchronous notification "foo" with payload "payload" received from server process with PID 14728. </programlisting></para> </programlisting></para>␟no translation␞␞␞
