...
␝  <refname>LOCK</refname>␟  <refpurpose>lock a table</refpurpose>␟  <refpurpose>テーブルをロックする</refpurpose>␞␞ </refnamediv>␞
␝␟<phrase>where <replaceable class="parameter">lockmode</replaceable> is one of:</phrase>␟<phrase>ここで<replaceable class="parameter">lockmode</replaceable>は以下のいずれかです。</phrase>␞␞␞
␝ <refsect1>␟  <title>Description</title>␟  <title>説明</title>␞␞␞
␝  <para>␟   <command>LOCK TABLE</command> obtains a table-level lock, waiting
   ...
   end.)␟<command>LOCK TABLE</command>はテーブルレベルのロックを取得します。必要であれば競合するロックが解除されるまで待機します。
...
ロックが解除されるのは常にトランザクションの終了時です。）␞␞  </para>␞
␝  <para>␟   When a view is locked, all relations appearing in the view definition
   query are also locked recursively with the same lock mode.␟ビューがロックされると、ビューを定義する問い合わせに現れるテーブルもすべて同じロックモードで再帰的にロックされます。␞␞  </para>␞
␝  <para>␟   When acquiring locks automatically for commands that reference
   ...
   furthermore none can begin until you release the lock.␟テーブルを参照するコマンドのために自動的にロックを取得する場合、<productname>PostgreSQL</productname>は使用可能な一番弱いロックモードを常に使用します。
...
したがって、一度ロックを取得してしまえば、コミットされていない状態の書き込みは存在しないことになります。さらに、ロックを解除するまで他のアプリケーションは書き込みを開始することができません。␞␞  </para>␞
␝  <para>␟   To achieve a similar effect when running a transaction at the
   ...
   the latest committed values.␟<literal>REPEATABLE READ</literal>または<literal>SERIALIZABLE</literal>分離レベルで実行しているトランザクションで同様の効果を得るには、全ての<command>SELECT</command>文とデータを更新する文を実行する前に<command>LOCK TABLE</command>文を実行する必要があります。
...
後からトランザクション内で<command>LOCK TABLE</command>を実行した場合も同時書き込みを防ぐことはできますが、トランザクションの読み込み対象データの値がコミットされた最新の値であることは保証されません。␞␞  </para>␞
␝  <para>␟   If a transaction of this sort is going to change the data in the
   ...
   restrictive mode first.␟このようなトランザクションでテーブルのデータを変更する場合は、<literal>SHARE</literal>モードではなく<literal>SHARE ROW EXCLUSIVE</literal>ロックモードを使用する必要があります。
...
また、1つのオブジェクトに対して複数のロックモードを呼び出す場合、トランザクションは常に最も制限の強いモードを最初に取得するべきです。␞␞  </para>␞
␝  <para>␟   More information about the lock modes and locking strategies can be
   found in <xref linkend="explicit-locking"/>.␟ロックモードとロック取得方針についてのより詳細については<xref linkend="explicit-locking"/>を参照してください。␞␞  </para>␞
...
␝     <para>␟      The name (optionally schema-qualified) of an existing table to
      ...
      descendant tables are included.␟ロックする既存のテーブルの名前です（スキーマ修飾名も可）。
...
␝     <para>␟      The command <literal>LOCK TABLE a, b;</literal> is equivalent to
      <literal>LOCK TABLE a; LOCK TABLE b;</literal>. The tables are locked
      one-by-one in the order specified in the <command>LOCK
      TABLE</command> command.␟<literal>LOCK a, b;</literal>というコマンドは<literal>LOCK TABLE a; LOCK TABLE b;</literal>と同じです。
テーブルは1つひとつ<command>LOCK</command>で指定された順番でロックされます。␞␞     </para>␞
␝     <para>␟      The lock mode specifies which locks this lock conflicts with.
      Lock modes are described in <xref linkend="explicit-locking"/>.␟ロックモードには、取得するロックと競合するロックを指定します。
ロックモードについては、<xref linkend="explicit-locking"/>で説明します。␞␞     </para>␞
␝     <para>␟      If no lock mode is specified, then <literal>ACCESS
      EXCLUSIVE</literal>, the most restrictive mode, is used.␟ロックモードを指定しない場合、最も制限が強い<literal>ACCESS EXCLUSIVE</literal>が使用されます。␞␞     </para>␞
␝     <para>␟      Specifies that <command>LOCK TABLE</command> should not wait for
      ...
      is aborted.␟<command>LOCK TABLE</command>が競合するロックの解放まで待機しないことを指定します。
指定したロックがすぐに取得できない場合、トランザクションはアボートされます。␞␞     </para>␞
␝ <refsect1>␟  <title>Notes</title>␟  <title>注釈</title>␞␞␞
␝   <para>␟    To lock a table, the user must have the right privilege for the specified
    ...
    MODE</literal> is permitted.␟テーブルをロックするには、ユーザが指定した<replaceable class="parameter">lockmode</replaceable>に対して適切な権限を持っていることが必要です。
...
ユーザがテーブルに対して<literal>SELECT</literal>権限を持っている場合、<literal>ACCESS SHARE MODE</literal>が許可されます。␞␞   </para>␞
␝   <para>␟    The user performing the lock on the view must have the corresponding
    ...
    relevant privileges on the underlying base relations.␟ビューに対してロックを実行するユーザはビューに対して対応する権限を持っていなければなりません。
さらに、デフォルトでは、ビューの所有者は元になる基底リレーションに対する関連する権限を持っていなければなりませんが、ロックを実行するユーザは元になる基底リレーションに対する権限を必要としません。
ただし、ビューの<literal>security_invoker</literal>が<literal>true</literal>に設定されている場合(<link linkend="sql-createview"><command>CREATE VIEW</command></link>を参照してください)、ビュー所有者ではなくロックを実行するユーザは元になる基底リレーションに対する関連する権限を持っていなければなりません。␞␞   </para>␞
␝   <para>␟    <command>LOCK TABLE</command> is useless outside a transaction block: the lock
    ...
    to define a transaction block.␟<command>LOCK TABLE</command>はトランザクションブロックの外側では意味がありません。
文が完了するまでしかロックは保持されません。
したがって<productname>PostgreSQL</productname>は<command>LOCK</command>がトランザクションブロックの外側で使用された場合にエラーを報告します。
トランザクションブロックを定義するためには<link linkend="sql-begin"><command>BEGIN</command></link>および<link linkend="sql-commit"><command>COMMIT</command></link>（または<link linkend="sql-rollback"><command>ROLLBACK</command></link>）を使用してください。␞␞   </para>␞
␝  <para>␟   <command>LOCK TABLE</command> only deals with table-level locks, and so
   ...
   in the <xref linkend="sql-select"/> documentation.␟<command>LOCK</command>が扱うのはテーブルレベルのロックのみです。
...
実際の行レベルでのロックを獲得する方法については、<xref linkend="sql-select"/>の文書の<xref linkend="locking-rows"/>と<xref linkend="sql-for-update-share"/>を参照してください。␞␞  </para>␞
␝ <refsect1>␟  <title>Examples</title>␟  <title>例</title>␞␞␞
␝  <para>␟   Obtain a <literal>SHARE</literal> lock on a primary key table when going to perform
   inserts into a foreign key table:␟外部キーテーブルへの挿入を行う際に、主キーテーブルへの<literal>SHARE</literal>ロックを獲得します。␞␞␞
␝-- Do ROLLBACK if record was not returned␟-- Do ROLLBACK if record was not returned␟-- レコードがなければROLLBACKしてください。␞␞INSERT INTO films_user_comments VALUES␞
␝  <para>␟   Take a <literal>SHARE ROW EXCLUSIVE</literal> lock on a primary key table when going to perform
   a delete operation:␟削除操作を行う際に主キーテーブルの<literal>SHARE ROW EXCLUSIVE</literal>ロックを取得します。␞␞␞
␝ <refsect1>␟  <title>Compatibility</title>␟  <title>互換性</title>␞␞␞
␝  <para>␟   There is no <command>LOCK TABLE</command> in the SQL standard,
   which instead uses <command>SET TRANSACTION</command> to specify
   concurrency levels on transactions.  <productname>PostgreSQL</productname> supports that too;
   see <xref linkend="sql-set-transaction"/> for details.␟標準SQLには<command>LOCK TABLE</command>はありません。
その代わりにトランザクションの同時性レベルを指定する<command>SET TRANSACTION</command>が使用されます。
<productname>PostgreSQL</productname>はこのコマンドもサポートしています。詳細は<xref linkend="sql-set-transaction"/>を参照してください。␞␞  </para>␞
␝  <para>␟   Except for <literal>ACCESS SHARE</literal>, <literal>ACCESS EXCLUSIVE</literal>,
   ...
   present in <productname>Oracle</productname>.␟<literal>ACCESS SHARE</literal>、<literal>ACCESS EXCLUSIVE</literal>、<literal>SHARE UPDATE EXCLUSIVE</literal>ロックモードを除き、<productname>PostgreSQL</productname>のロックモードと<command>LOCK TABLE</command>構文は<productname>Oracle</productname>のものと互換性があります。␞␞  </para>␞
␝␟BEGIN WORK; LOCK TABLE films IN SHARE ROW EXCLUSIVE MODE; DELETE FROM films_user_comments WHERE id IN (SELECT id FROM films WHERE rating &lt; 5); DELETE FROM films WHERE rating &lt; 5; COMMIT WORK; </programlisting></para> </programlisting></para>␟no translation␞␞␞
