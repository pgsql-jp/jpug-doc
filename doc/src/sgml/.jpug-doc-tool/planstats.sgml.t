␝<chapter id="planner-stats-details">␟ <title>How the Planner Uses Statistics</title>␟ <title>プランナは統計情報をどのように使用するか</title>␞␞␞
␝  <para>␟   This chapter builds on the material covered in <xref
   ...
   providing much of the raw material for cost calculation.␟本章は、<xref linkend="using-explain"/>と<xref linkend="planner-stats"/>で扱われている題材を基にしていて、問い合わせの各段階において返される行数を推定するために、プランナがシステムの統計情報をどのように使用するかについて更なる詳細をいくつか説明します。
これは計画作成処理において重要な部分で、コスト計算用の多くの情報を提供します。␞␞  </para>␞
␝  <para>␟   The intent of this chapter is not to document the code in detail,
   but to present an overview of how it works.
   This will perhaps ease the learning curve for someone who subsequently
   wishes to read the code.␟本章の目的はコードを詳しく文書化することではありません。
どのように動作するのかに関する概要を表すことが目的です。
これによりおそらく、後にコードを参照するユーザの習得速度が向上するでしょう。␞␞  </para>␞
...
␝ <sect1 id="row-estimation-examples">␟  <title>Row Estimation Examples</title>␟  <title>行数推定の例</title>␞␞␞
␝  <para>␟   The examples shown below use tables in the <productname>PostgreSQL</productname>
   ...
   any new <command>ANALYZE</command>.␟以下の例は<productname>PostgreSQL</productname>リグレッションテストデータベースのテーブルを使用しています。
また、<command>ANALYZE</command>は統計を生成する際にランダムサンプリングを使用するため、新しい<command>ANALYZE</command>を実行した後は結果がわずかに変化することに注意してください。␞␞  </para>␞
␝  <para>␟   Let's start with a very simple query:␟非常に簡単な問い合わせから始めましょう。␞␞␞
␝␟   How the planner determines the cardinality of <structname>tenk1</structname>
   ...
   <structname>pg_class</structname>:␟プランナがどのように<structname>tenk1</structname>のカーディナリティを決定するかについては<xref linkend="planner-stats"/>で説明しました。
...
␝␟    These numbers are current as of the last <command>VACUUM</command> or
    ...
    the same as <structfield>reltuples</structfield>.␟これらの値は最後にそのテーブルを<command>VACUUM</command>または<command>ANALYZE</command>を行った時点のものです。
...
上の例では、<structfield>relpages</structfield>の値は最新のものなので、推定行数は<structfield>reltuples</structfield>と同じです。␞␞  </para>␞
␝  <para>␟   Let's move on to an example with a range condition in its
   <literal>WHERE</literal> clause:␟次に<literal>WHERE</literal>句に範囲条件を持つ例に進みましょう。␞␞␞
␝␟   The planner examines the <literal>WHERE</literal> clause condition
   ...
   view:␟プランナは<literal>WHERE</literal>句の条件を検査し、<structname>pg_operator</structname>内の<literal>&lt;</literal>演算子用の選択度関数を検索します。
...
␝␟   Next the fraction of the histogram occupied by <quote>&lt; 1000</quote>
   ...
   values inside each bucket, we can calculate the selectivity as:␟次に、<quote>&lt; 1000</quote>で占められるヒストグラムの割合を取り出します。
...
␝␟   that is, one whole bucket plus a linear fraction of the second, divided by
   ...
   <structname>tenk1</structname>:␟つまり、1つのバケット全体に、2番目のバケットとの線形比率を加えたものを、バケット数で割ったものとなります。
ここで、行の推定値は、選択度と<structname>tenk1</structname>のカーディナリティを掛け合わせたものとして計算されます。␞␞␞
␝     = 10000 * 0.100697␟     = 1007  (rounding off)␟     = 1007  (四捨五入)␞␞</programlisting>␞
␝  <para>␟   Next let's consider an example with an equality condition in its
   <literal>WHERE</literal> clause:␟次に、<literal>WHERE</literal>句に等価条件を持つ例を検討してみましょう。␞␞␞
␝␟   Again the planner examines the <literal>WHERE</literal> clause condition
   ...
   that will be useful later:␟ここでも、プランナは<literal>WHERE</literal>句の条件を検査し、<literal>=</literal>用の選択度関数、この場合は<function>eqsel</function>を検索します。
...
␝␟   Since <literal>CRAAAA</literal> appears in the list of MCVs, the selectivity is
   ...
   (<acronym>MCF</acronym>s):␟<literal>CRAAAA</literal>がMCVのリスト内にありますので、選択度は単に頻出値の頻度（<acronym>MCF</acronym>）のリスト内の対応する項目になります。␞␞␞
␝␟   As before, the estimated number of rows is just the product of this with the
   cardinality of <structname>tenk1</structname>:␟前と同様、推定される行数は単に前回同様、この値と<structname>tenk1</structname>のカーディナリティとの積です。␞␞␞
␝  <para>␟   Now consider the same query, but with a constant that is not in the
   <acronym>MCV</acronym> list:␟ここで、同じ問い合わせを見てみます。
...
␝␟   This is quite a different problem: how to estimate the selectivity when the
   ...
   <acronym>MCV</acronym>s:␟値が<acronym>MCV</acronym>の一覧に<emphasis>ない</emphasis>場合、選択度をどのように推定するかは大きく異なります。
...
␝␟   That is, add up all the frequencies for the <acronym>MCV</acronym>s and
   ...
   The estimated number of rows is then calculated as usual:␟つまり、<acronym>MCV</acronym>の頻度をすべて足し合わせたものを1から差し引き、そして、<emphasis>この他の</emphasis>個別値の数で除算します。
...
␝rows = 10000 * 0.0014559␟     = 15  (rounding off)␟     = 15  (四捨五入)␞␞</programlisting>␞
␝  <para>␟   The previous example with <literal>unique1 &lt; 1000</literal> was an
   ...
   selectivity.  For example, consider␟前述の<literal>unique1 &lt; 1000</literal>を使用した例は<function>scalarltsel</function>が本当は何を行うかについて、単純化しすぎたものでした。
...
␝␟   We already saw the MCV information for <structfield>stringu1</structfield>,
   and here is its histogram:␟すでに<structfield>stringu1</structfield>のMCV情報は確認していますので、ここではヒストグラムを見てみます。␞␞␞
␝␟   Checking the MCV list, we find that the condition <literal>stringu1 &lt;
   ...
   so the selectivity within the MCV part of the population is␟MCVリストを検査すると、<literal>stringu1 &lt; 'IAAAAA'</literal>条件は先頭の6項目で満たされ、最後の4項目で満たされないことがわかります。
...
␝␟   Summing all the MCFs also tells us that the total fraction of the
   ...
   for the MCV and non-MCV populations:␟MCFの総和はまた、MCVで表される母集団の合計割合が0.03033333であり、したがってヒストグラムで表される割合が0.96966667であることがわかります。
...
␝rows        = 10000 * 0.307669␟            = 3077  (rounding off)␟            = 3077  (四捨五入)␞␞</programlisting>␞
␝␟   In this particular example, the correction from the MCV list is fairly
   ...
   for the most common values is found exactly.␟列の分布がかなり平坦ですので、この特定の例におけるMCVリストによる訂正はかなり小さなものです。
（これらの特定の値が他より頻出するものと示す統計情報はほとんどサンプリングエラーによります。）
より一般的な、一部の値が他より非常に多く頻出する場合では、頻出値に対する選択度が正確に検出されますので、この複雑な処理により精度が改良されます。␞␞  </para>␞
␝  <para>␟   Now let's consider a case with more than one
   condition in the <literal>WHERE</literal> clause:␟次に<literal>WHERE</literal>句に複数の条件を持つ場合を検討しましょう。␞␞␞
␝␟   The planner assumes that the two conditions are independent, so that
   the individual selectivities of the clauses can be multiplied together:␟プランナは2つの条件が独立していると仮定します。
このため、個々の句の選択度が掛け合わされます。␞␞␞
␝rows        = 10000 * 0.0001466␟            = 1  (rounding off)␟            = 1  (四捨五入)␞␞</programlisting>␞
␝␟   Notice that the number of rows estimated to be returned from the bitmap
   ...
   fetches.␟ビットマップインデックススキャンにより返されるものと推定される行数は、インデックスで使用される条件のみを反映することに注意してください。
後続のヒープ取り出しのコスト推定に影響しますので、これは重要です。␞␞  </para>␞
␝  <para>␟   Finally we will examine a query that involves a join:␟最後に、結合を含む問い合わせを見てみましょう。␞␞␞
␝␟   The restriction on <structname>tenk1</structname>,
   ...
   <structfield>unique1</structfield> histogram:␟<structname>tenk1</structname> <literal>unique1 &lt; 50</literal>に関する制限が、ネステッドループ結合の前に評価されます。
これは、前の範囲に関する例と同様に扱われます。
しかし、今回の値50は<structfield>unique1</structfield>ヒストグラムの最初のバケットにありますので、以下のようになります。␞␞␞
␝rows        = 10000 * 0.005035␟            = 50  (rounding off)␟            = 50  (四捨五入)␞␞</programlisting>␞
␝␟   The restriction for the join is <literal>t2.unique2 = t1.unique2</literal>.
   ...
   <structname>tenk2</structname> and <structname>tenk1</structname>:␟結合の制限は<literal>t2.unique2 = t1.unique2</literal>です。
...
␝␟   In this case there is no <acronym>MCV</acronym> information for
   ...
   together with the column null fractions (zero for both):␟この場合、<structname>unique2</structname>に関する<acronym>MCV</acronym>の情報はなく、すべての値が一意であるように見えます（n_distinct = -1）。
...
␝␟   This is, subtract the null fraction from one for each of the relations,
   ...
   selectivity:␟これは、各リレーションにおいて、1からNULL部分を差し引き、それを大きい方のリレーションの行数で割ります（この値は一意でない場合にはスケーリングされます）。
この結合が生成するはずの行数は、2つの入力のデカルト積のカーディナリティに、この選択度を掛けたものとして計算されます。␞␞␞
␝  <para>␟   Had there been MCV lists for the two columns,
   ...
   populations follows the same approach shown here.␟2つの列に対するMCVリストがありますので、<function>eqjoinsel</function>はMCVで表される列母集団部分の結合選択度を決めるために、MCVリストを直接比較します。
残りの母集団に対する推定はここで示した同じ手法に従います。␞␞  </para>␞
␝  <para>␟   Notice that we showed <literal>inner_cardinality</literal> as 10000, that is,
   ...
   other factors they sometimes diverge significantly.␟<literal>inner_cardinality</literal>を10000、つまり<structname>tenk2</structname>の変更がないサイズと示していることに注意してください。
...
しかし、四捨五入誤差などの要因により多少異なる場合があります。␞␞  </para>␞
␝  <para>␟   For those interested in further details, estimation of the size of
   ...
   in <filename>src/backend/utils/adt/selfuncs.c</filename>.␟詳細に興味を持った方向けに、テーブル（すべての<literal>WHERE</literal>句の前にあるもの）のサイズ推定は<filename>src/backend/optimizer/util/plancat.c</filename>で行われます。
句の選択度に関する一般的なロジックについては<filename>src/backend/optimizer/path/clausesel.c</filename>にあります。
演算子固有の選択度関数についてはたいてい<filename>src/backend/utils/adt/selfuncs.c</filename>内にあります。␞␞  </para>␞
␝  <indexterm>
   <primary>row estimation</primary>
...
␝ <sect1 id="multivariate-statistics-examples">␟  <title>Multivariate Statistics Examples</title>␟  <title>多変量統計の例</title>␞␞␞
␝  <sect2 id="functional-dependencies">␟   <title>Functional Dependencies</title>␟   <title>関数従属性</title>␞␞␞
␝   <para>␟    Multivariate correlation can be demonstrated with a very simple data set
    &mdash; a table with two columns, both containing the same values:␟非常に単純なデータ集合で、多変量相関係数の例を示すことができます。
...
␝␟    As explained in <xref linkend="planner-stats"/>, the planner can determine
    ...
    rows obtained from <structname>pg_class</structname>:␟<xref linkend="planner-stats"/>で説明されているように、<structname>pg_class</structname>から得られるページ数と行数を使って、<structname>t</structname>のカーディナリティを決定できます。␞␞␞
␝␟    The data distribution is very simple; there are only 100 distinct values
    in each column, uniformly distributed.␟データの分布はとても単純です。各々の列にはわずか100の異なる値があるだけであり、かつ均一に分布しています。␞␞   </para>␞
␝   <para>␟    The following example shows the result of estimating a <literal>WHERE</literal>
    condition on the <structfield>a</structfield> column:␟次の例では、<structfield>a</structfield>列に関する<literal>WHERE</literal>条件の見積もり結果を示しています。␞␞␞
␝␟    The planner examines the condition and determines the selectivity
    ...
    condition on both columns, combining them with <literal>AND</literal>:␟プランナは、この条件を調べ、この句の選択度を1%と決定しました。
...
␝␟    The planner estimates the selectivity for each condition individually,
    ...
    matching the conditions (100) is two orders of magnitude higher.␟個別に選択度を見積もった結果、上記と同じ1%の見積もりとなります。
次に、その条件が独立であると見なし、それらの選択度を掛けあわせ、最終的な選択度の見積もりをわずか0.01%であるとします。
その条件に一致する実際の行数は2桁多いので(100)、これはかなり過小見積もりです。␞␞   </para>␞
␝   <para>␟    This problem can be fixed by creating a statistics object that
    directs <command>ANALYZE</command> to calculate functional-dependency
    multivariate statistics on the two columns:␟この問題は、<command>ANALYZE</command>に二つの列について関数従属性多変量統計を計算させる、統計オブジェクトを作成することによって解決できます。␞␞␞
␝  <sect2 id="multivariate-ndistinct-counts">␟   <title>Multivariate N-Distinct Counts</title>␟   <title>多変量N個別値計数</title>␞␞␞
␝   <para>␟    A similar problem occurs with estimation of the cardinality of sets of
    ...
    accurate:␟<command>GROUP BY</command>句が生成するグループ数のような、複数列の集合のカーディナリティの見積もりについても、同様の問題が起きます。
<command>GROUP BY</command>の対象が単一の列なら、N個別値の推定（HashAggregateノードが返す推定行数で示されます）はとても正確です。␞␞<programlisting>␞
␝</programlisting>␟    But without multivariate statistics, the estimate for the number of
    groups in a query with two columns in <command>GROUP BY</command>, as
    in the following example, is off by an order of magnitude:␟しかし、多変量統計がないと、二つの列についての<command>GROUP BY</command>問い合わせにおけるグループ数の見積もりは、次の例のようにひと桁ずれてしまいます。␞␞<programlisting>␞
␝</programlisting>␟    By redefining the statistics object to include n-distinct counts for the
    two columns, the estimate is much improved:␟二つの列についてのN個別値計数を含むように統計オブジェクトを再定義することにより、見積もりは大きく改善されます。␞␞<programlisting>␞
␝  <sect2 id="mcv-lists">␟   <title>MCV Lists</title>␟   <title>MCVリスト</title>␞␞␞
␝   <para>␟    As explained in <xref linkend="functional-dependencies"/>, functional
    ...
    the column level, not between individual column values).␟<xref linkend="functional-dependencies"/>で説明したように、関数従属性は非常に安価で効率的な統計情報ですが、主要な制限はその大域的な性質です（列レベルだけでの従属性の追跡であり、個々の列の値の間のものではありません）。␞␞   </para>␞
␝   <para>␟    This section introduces multivariate variant of <acronym>MCV</acronym>
    ...
    <command>ANALYZE</command>, storage and planning time.␟この節では<acronym>MCV</acronym>(最頻値)リストの多変量のもの、<xref linkend="row-estimation-examples"/>で述べた行毎の統計情報の素直な拡張を導入します。
この統計情報は格納された個々の値による制限を解決しますが、<command>ANALYZE</command>での統計情報の構築や容量や計画作成時間に関して当然より高価です。␞␞   </para>␞
␝   <para>␟    Let's look at the query from <xref linkend="functional-dependencies"/>
    ...
    make sure the planner uses the newly created statistics).␟再び<xref linkend="functional-dependencies"/>の問い合わせを見てみましょう。ですが、今回は列の同じ集合に対して<acronym>MCV</acronym>リストを作ります（プランナが新しく作られた統計情報を確実に利用するよう、関数従属性を確実に削除してください）。␞␞␞
␝␟    The estimate is as accurate as with the functional dependencies, mostly
    ...
    let's inspect the <acronym>MCV</acronym> list a bit.␟主に、テーブルがかなり小さく、異なる値の少ない単純な分布のおかげで、推定は関数従属性と同じくらい正確です。
関数従属性では特に上手く扱えない2番目の問い合わせを見る前に、<acronym>MCV</acronym>リストを少し調べてみましょう。␞␞   </para>␞
␝   <para>␟    Inspecting the <acronym>MCV</acronym> list is possible using
    <function>pg_mcv_list_items</function> set-returning function.␟<acronym>MCV</acronym>を調べるのは、集合を返す<function>pg_mcv_list_items</function>関数でできます。␞␞␞
␝␟    This confirms there are 100 distinct combinations in the two columns, and
    ...
    <structfield>nulls</structfield> column.␟これで、2つの列の100個の個別の組み合わせがあり、すべてほぼ同様に確からしい(それぞれ1%の頻度)ことが確かめられます。
基準となる頻度(base frequency)は、複数列の統計情報がないとして、列毎の統計情報から計算された頻度です。
列のどちらか一方にでもNULL値があれば、<structfield>nulls</structfield>列で見分けられます。␞␞   </para>␞
␝   <para>␟    When estimating the selectivity, the planner applies all the conditions
    ...
    in <filename>src/backend/statistics/mcv.c</filename> for details.␟選択性を推定する場合、プランナは<acronym>MCV</acronym>リストの項目にすべての条件を適用してから、一致するものの頻度を合計します。
詳細は<filename>src/backend/statistics/mcv.c</filename>の<function>mcv_clauselist_selectivity</function>を参照してください。␞␞   </para>␞
␝   <para>␟    Compared to functional dependencies, <acronym>MCV</acronym> lists have two
    major advantages. Firstly, the list stores actual values, making it possible
    to decide which combinations are compatible.␟関数従属性に比べて、<acronym>MCV</acronym>は主要な利点が2つあります。1つ目は、リストが実際の値を格納していることで、これによりどの組み合わせが適合するのか決定できます。␞␞␞
␝␟    Secondly, <acronym>MCV</acronym> lists handle a wider range of clause types,
    ...
    consider the following range query for the same table:␟2つ目は、<acronym>MCV</acronym>リストが、関数従属性のような等式句だけでなく、より広い範囲の形の句を扱うことです。
例えば、以下のような同じテーブルに対する範囲の問い合わせを考えてみましょう。␞␞␞
␝ <sect1 id="planner-stats-security">␟  <title>Planner Statistics and Security</title>␟  <title>プランナの統計情報とセキュリティ</title>␞␞␞
␝  <para>␟   Access to the table <structname>pg_statistic</structname> is restricted to
   ...
   see it.␟テーブル<structname>pg_statistic</structname>へのアクセスはスーパーユーザのみに制限されているため、一般ユーザはこのテーブルを使って他のユーザのテーブル内容について調べることはできません。
...
巧妙に作られた演算子を使うと、渡された引数を意図的に漏らす（例えば、それをログに出力する、他のテーブルに書き出すなど）、あるいはその値をエラーメッセージに出力することで偶然に漏らすことが可能で、いずれにせよ<structname>pg_statistic</structname>のデータを、それを見ることができないはずのユーザに対して露出する可能性があります。␞␞  </para>␞
␝  <para>␟   In order to prevent this, the following applies to all built-in selectivity
   ...
   the planner will proceed with default or fall-back assumptions.␟このことを防ぐため、すべての組み込みの選択性推定関数には以下のことが適用されます。
...
そうでないときは、選択性推定はあたかも利用可能な統計情報がないかのような動作をし、プランナはデフォルトあるいは代替の推定に従って処理をします。␞␞  </para>␞
␝  <para>␟   If a user does not have the required privilege on the table or columns,
   ...
   to see if a different plan results.␟ユーザがテーブルや列について必要な権限を持っていない場合、最終的には権限不足のエラーを受け取ることが多いでしょう。
...
このことが起きているかもしれないと思った場合は、より権限のあるユーザで問い合わせを実行して、異なる計画が得られるかどうか調べることができます。␞␞  </para>␞
␝  <para>␟   This restriction applies only to cases where the planner would need to
   ...
   privileges.␟この制限は、プランナが<structname>pg_statistic</structname>の1つ以上の値についてユーザ定義演算子を実行する必要がある場合にのみ適用されます。
従って、列内でのNULL値の割合や異なる値の個数といった一般的な統計情報については、プランナはアクセス権限に関わらず使用することが許されています。␞␞  </para>␞
␝  <para>␟   Selectivity estimation functions contained in third-party extensions that
   potentially operate on statistics with user-defined operators should follow
   the same security rules.  Consult the PostgreSQL source code for guidance.␟サードパーティの拡張に含まれる選択性推定関数で、ユーザ定義演算子で統計情報の演算をする可能性のあるものは、同じセキュリティ規則に従うべきです。
そのための手引については、PostgreSQLのソースコードを参照してください。␞␞  </para>␞
