...
␝<sect1 id="history">␟ <title>A Brief History of <productname>PostgreSQL</productname></title>␟ <title><productname>PostgreSQL</productname>小史</title>␞␞␞
␝ <para>␟  The object-relational database management system now known as
  ...
  the most advanced open-source database available anywhere.␟現在<productname>PostgreSQL</productname>として知られるオブジェクト指向リレーショナルデータベース管理システムは、カリフォルニア大学バークレイ校で作成された<productname>POSTGRES</productname>パッケージから派生しています。
数十年にわたる開発の背景を持ち、<productname>PostgreSQL</productname>は現在最も先端的な、どこからでも入手可能なオープンソースデータベースです。␞␞ </para>␞
␝ <sect2 id="history-berkeley">␟  <title>The Berkeley <productname>POSTGRES</productname> Project</title>␟  <title>バークレイ校<productname>POSTGRES</productname>プロジェクト</title>␞␞␞
␝  <para>␟   The <productname>POSTGRES</productname> project, led by Professor
   ...
   linkend="ston87b"/>.␟Michael Stonebraker教授率いる<productname>POSTGRES</productname>プロジェクトにはその後援者としてDefense Advanced Research Projects Agency（<acronym>DARPA</acronym>）、 Army Research Office (<acronym>ARO</acronym>)、National Science Foundation（<acronym>NSF</acronym>）、そしてESL, Inc.が名を連ねていました。
...
ストレージ管理の理論や構造は<xref linkend="ston87b"/>で詳しく示されました。␞␞  </para>␞
␝  <para>␟   <productname>POSTGRES</productname> has undergone several major
   ...
   portability and reliability.␟<productname>POSTGRES</productname>はそれ以来いくつかの主要なリリースを重ねてきました。
...
<productname>Postgres95</productname>まで引き続いた各リリース（下記を参照）のほとんどの部分では、移植性と信頼性に焦点を合わせていました。␞␞  </para>␞
␝  <para>␟   <productname>POSTGRES</productname> has been used to implement many
   ...
   <xref linkend="ston92"/>.␟<productname>POSTGRES</productname>は様々な研究用、そして実際の業務アプリケーションを実装するために使われてきています。
...
1992年後半には、<productname>POSTGRES</productname>は<xref linkend="ston92"/>に書かれているSequoia 2000 scientific computing projectの主要なデータ管理システムになりました。␞␞  </para>␞
␝  <para>␟   The size of the external user community nearly doubled during 1993.
   ...
   Version 4.2.␟1993年に外部ユーザコミュニティの規模はほぼ倍増しました。
データベースの研究に費やされるべき時間がプロトタイプコードの保守とサポートに取られていることが次第に明らかになってきました。
このサポートの重荷を減らすために、バークレイ<productname>POSTGRES</productname>プロジェクトはバージョン4.2をもって公式に終了しました。␞␞  </para>␞
␝  <para>␟   In 1994, Andrew Yu and Jolly Chen added an SQL language interpreter
   ...
   Berkeley code.␟1994年、Andrew YuとJolly Chenが<productname>POSTGRES</productname>にSQL言語インタプリタを追加しました。
引き続いて<productname>Postgres95</productname>がWeb上でリリースされ、オリジナルの<productname>POSTGRES</productname>バークレイコードのオープンソースによる後続として世界への独自の道を歩み始めました。␞␞  </para>␞
␝  <para>␟   <productname>Postgres95</productname> code was completely ANSI C
   ...
   bug fixes, the following were the major enhancements:␟<productname>Postgres95</productname>のコードは全てANSI C準拠となるように書き直され、これまでに膨れ上がったコードの25%を整理することで身軽になりました。
...
␝     <para>␟      The query language PostQUEL was replaced with
      ...
      query clause was also added.␟（サーバに実装された）<acronym>SQL</acronym>が問い合わせ言語PostQUELに取って代わりました。
...
集約は再実装されました。
<literal>GROUP BY</literal>問い合わせ句のサポートも追加されました。␞␞     </para>␞
␝     <para>␟      A new program
      ...
      the old <application>monitor</application> program.␟<acronym>GNU</acronym>の<application>Readline</application>を使った新しいプログラム（<application>psql</application>）が、対話式SQL問い合わせのために用意されました。
これは古い<application>monitor</application>プログラムにほぼ取って代わるものになりました。␞␞     </para>␞
␝     <para>␟      A new front-end library, <filename>libpgtcl</filename>,
      ...
      <productname>Postgres95</productname> server.␟新しいフロントエンドライブラリ、<filename>libpgtcl</filename>が<acronym>Tcl</acronym>ベースのクライアントをサポートしました。
サンプルシェル<command>pgtclsh</command>は<application>Tcl</application>と<productname>Postgres95</productname>サーバとインタフェースをとる新規Tclコマンドを提供しました。␞␞     </para>␞
␝     <para>␟      The large-object interface was overhauled. The inversion large
      objects were the only mechanism for storing large objects.  (The
      inversion file system was removed.)␟ラージオブジェクトインタフェースがオーバーホールされました。
転置ラージオブジェクトが唯一のラージオブジェクト格納機構でした。
（転置ファイルシステムは削除されました。）␞␞     </para>␞
␝     <para>␟      The instance-level rule system was removed.  Rules were still
      available as rewrite rules.␟インスタンスレベルのルールシステムが削除されました。
ルールは書き換えルールとしてまだ利用できました。␞␞     </para>␞
␝     <para>␟      A short tutorial introducing regular <acronym>SQL</acronym>
      features as well as those of
      <productname>Postgres95</productname> was distributed with the
      source code␟標準<acronym>SQL</acronym>の機能や<productname>Postgres95</productname>の機能を紹介したチュートリアルがソースコードとともに配布されました。␞␞     </para>␞
␝     <para>␟      <acronym>GNU</acronym> make (instead of <acronym>BSD</acronym>
      ...
      doubles was fixed).␟<acronym>GNU</acronym> makeが（<acronym>BSD</acronym> makeの代わりに）構築に使われました。
また、<productname>Postgres95</productname>はパッチの当たっていない<productname>GCC</productname>でコンパイルできました
（doubleにおけるデータ整列が修正されたおかげです）。␞␞     </para>␞
␝  <para>␟   By 1996, it became clear that the name <quote>Postgres95</quote>
   ...
   Berkeley <productname>POSTGRES</productname> project.␟1996年になると<quote>Postgres95</quote>という名前が時代の試練に耐えられなくなったことが明らかになりました。
...
同時に、もともとバークレイ<productname>POSTGRES</productname>プロジェクトで始まった連番に戻す番号の6.0で始まるバージョン番号を設定しました。␞␞  </para>␞
␝  <para>␟   <productname>Postgres</productname> is still considered an official
   ...
   <productname>PostgreSQL</productname>.␟<productname>Postgres</productname>は、伝統があることと、人々が<productname>PostgreSQL</productname>よりも<productname>Postgres</productname>と発音しやすいことから、今でもプロジェクトの正式な名前と考えられています。␞␞  </para>␞
␝  <para>␟   The emphasis during development of
   ...
   all areas.␟<productname>Postgres95</productname>開発で重視されたのは、サーバのコードに内在する問題点を特定し、原因を理解することでした。
<productname>PostgreSQL</productname>においては、全ての分野に目を留めているとしても、保守作業を続けつつ特徴や能力を強化することに重点が移りました。␞␞  </para>␞
␝  <para>␟   Details about what has happened in <productname>PostgreSQL</productname> since
   then can be found in <xref linkend="release"/>.␟その後<productname>PostgreSQL</productname>がたどった足跡の詳細は<xref linkend="release"/>を参照してください。␞␞  </para>␞
