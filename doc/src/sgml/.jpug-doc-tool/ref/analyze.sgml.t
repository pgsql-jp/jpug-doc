...
␝  <refname>ANALYZE</refname>␟  <refpurpose>collect statistics about a database</refpurpose>␟  <refpurpose>データベースに関する統計を収集する</refpurpose>␞␞ </refnamediv>␞
...
␝  <para>␟   <command>ANALYZE</command> collects statistics about the contents
   ...
   queries.␟<command>ANALYZE</command>はデータベース内のテーブルの内容に関する統計情報を収集し、その結果を<link linkend="catalog-pg-statistic"><structname>pg_statistic</structname></link>システムカタログに保存します。
問い合わせプランナが最も効率の良い問い合わせの実行計画を決定する際、この統計情報が使用されます。␞␞  </para>␞
␝  <para>␟   Without a <replaceable class="parameter">table_and_columns</replaceable>
   ...
   in which case only the statistics for those columns are collected.␟<replaceable class="parameter">table_and_columns</replaceable>リストがない場合、<command>ANALYZE</command>は現在のデータベース内で現在のユーザが解析する権限のあるすべてのテーブルとマテリアライズドビューを処理します。
リストがある場合、<command>ANALYZE</command>は指定されたテーブルのみを処理します。
さらにテーブルの列名のリストを与え、その列の統計情報のみを収集することも可能です。␞␞  </para>␞
␝ <refsect1>␟  <title>Parameters</title>␟  <title>パラメータ</title>␞␞␞
␝     <para>␟      Enables display of progress messages.␟進行状況の表示を有効にします。␞␞     </para>␞
␝     <para>␟      Specifies that <command>ANALYZE</command> should not wait for any
      ...
      partitions if there is a conflicting lock on the partitioned table.␟リレーション上で動作を開始する時に、<command>ANALYZE</command>は衝突しているロックが解放されるのを待たないことを指定します。リレーションを待つことなく即時にロックできない場合、そのリレーションは飛ばされます。
...
また、通常<command>ANALYZE</command>は指定されたパーティションテーブルのパーティションをすべて処理しますが、このオプションにより<command>ANALYZE</command>は、パーティションテーブル上に衝突するロックがあれば、パーティションをすべてスキップします。␞␞     </para>␞
␝     <para>␟      Specifies the
      ...
      maximum value is <literal>16 GB</literal>.␟<command>ANALYZE</command>の<glossterm linkend="glossary-buffer-access-strategy">バッファアクセスストラテジ</glossterm>リングバッファサイズを指定します。
...
最小値は<literal>128 kB</literal>で、最大値は<literal>16 GB</literal>です。␞␞     </para>␞
...
␝     <para>␟      The name (possibly schema-qualified) of a specific table to
      ...
      statistics of the individual partitions are updated.␟解析の対象とするテーブルの名前です（スキーマ修飾名も可）。
省略された場合、現在のデータベースの中のすべての通常のテーブル、パーティションテーブル、マテリアライズドビュー（外部テーブルは除く）が解析されます。
指定したテーブルがパーティションテーブルの場合、パーティションテーブル全体としての継承の統計と、個々のパーティションの統計の両方が更新されます。␞␞     </para>␞
␝     <para>␟      The name of a specific column to analyze. Defaults to all columns.␟解析の対象とする列名です。
デフォルトは全ての列です。␞␞     </para>␞
␝ <refsect1>␟  <title>Outputs</title>␟  <title>出力</title>␞␞␞
␝   <para>␟    When <literal>VERBOSE</literal> is specified, <command>ANALYZE</command> emits
    progress messages to indicate which table is currently being
    processed.  Various statistics about the tables are printed as well.␟<literal>VERBOSE</literal>が指定された場合、<command>ANALYZE</command>は進捗メッセージとして処理中のテーブルを表示します。
さらに、テーブルについての各種統計情報も表示されます。␞␞   </para>␞
␝ <refsect1>␟  <title>Notes</title>␟  <title>注釈</title>␞␞␞
␝  <para>␟   To analyze a table, one must ordinarily have the <literal>MAINTAIN</literal>
   privilege on the table.  However, database owners are allowed to
   analyze all tables in their databases, except shared catalogs.
   <command>ANALYZE</command> will skip over any tables that the calling user
   does not have permission to analyze.␟テーブルを解析するためには、通常そのテーブルに対する<literal>MAINTAIN</literal>権限が必要です。
しかしながら、データベースの所有者は、共有カタログを除いて、そのデータベースのテーブルをすべて解析できます。
<command>ANALYZE</command>は呼び出したユーザが解析する権限のないテーブルをスキップします。␞␞  </para>␞
␝  <para>␟   Foreign tables are analyzed only when explicitly selected.  Not all
   ...
   warning and does nothing.␟外部テーブルは明示的に選択された場合にのみ解析されます。
すべての外部データラッパーが<command>ANALYZE</command>をサポートしているとは限りません。
テーブルのラッパーが<command>ANALYZE</command>をサポートしない場合、コマンドは警告を出力し、何も行いません。␞␞  </para>␞
␝  <para>␟   In the default <productname>PostgreSQL</productname> configuration,
   ...
   (This will not be sufficient if there is heavy update activity.)␟デフォルトの<productname>PostgreSQL</productname>の設定では、自動バキュームデーモン（<xref linkend="autovacuum"/>参照）が、データが最初にロードされた時や通常の操作を通して変更された時にテーブルの自動解析まで面倒をみます。
...
（非常に更新が激しい場合、これでは十分ではありません。）␞␞  </para>␞
␝  <para>␟   While <command>ANALYZE</command> is running, the <xref
   linkend="guc-search-path"/> is temporarily changed to <literal>pg_catalog,
   pg_temp</literal>.␟<command>ANALYZE</command>の実行中、<xref linkend="guc-search-path"/>は一時的に<literal>pg_catalog, pg_temp</literal>に変更されます。␞␞  </para>␞
␝  <para>␟   <command>ANALYZE</command>
   requires only a read lock on the target table, so it can run in
   parallel with other non-DDL activity on the table.␟<command>ANALYZE</command>は、対象とするテーブルの読み取りロックのみを必要とします。したがって、そのテーブルに対する他の非DDL操作と並行して実行できます。␞␞  </para>␞
␝  <para>␟   The statistics collected by <command>ANALYZE</command> usually
   ...
   linkend="maintenance"/>.␟通常、<command>ANALYZE</command>によって収集される統計情報には、各列の典型的な値と各列のデータ分布の概要を示すヒストグラムが含まれます。
...
<xref linkend="maintenance"/>に、統計情報についての詳細が記載されています。␞␞  </para>␞
␝  <para>␟   For large tables, <command>ANALYZE</command> takes a random sample
   ...
   <command>ANALYZE</command>, as described below.␟巨大なテーブルでは、<command>ANALYZE</command>は、全ての行を検査するのではなく、テーブルの中からランダムにサンプルを取り出して使用します。
...
これを防止するには、以下に示すように<command>ANALYZE</command>で収集される統計情報の量を増やしてください。␞␞  </para>␞
␝  <para>␟   The extent of analysis can be controlled by adjusting the
   ...
   have no use for statistics on such columns.␟設定パラメータ変数<xref linkend="guc-default-statistics-target"/>を調整するか、もしくは<link linkend="sql-altertable"><command>ALTER TABLE ... ALTER COLUMN ... SET STATISTICS</command></link>を使用して列単位の統計目標を列毎に設定することで、解析の範囲を制御することができます。
...
プランナにとってそのような列の統計情報は不要だからです。␞␞  </para>␞
␝  <para>␟   The largest statistics target among the columns being analyzed determines
   the number of table rows sampled to prepare the statistics.  Increasing
   the target causes a proportional increase in the time and space needed
   to do <command>ANALYZE</command>.␟解析対象列の統計情報目標値の最大値によって、統計情報を作成するためにテーブルから抽出する行数が決定します。
目標値を大きくすると、比例して、<command>ANALYZE</command>に要する時間とディスク容量が増加します。␞␞  </para>␞
␝  <para>␟   One of the values estimated by <command>ANALYZE</command> is the number of
   ...
   installed with
   <link linkend="sql-altertable"><command>ALTER TABLE ... ALTER COLUMN ... SET (n_distinct = ...)</command></link>.␟<command>ANALYZE</command>で推定される値の1つに各列に出現する個別値の個数があります。
行の部分集合のみしか検査されませんので、統計情報の対象をできる限り大きくしたとしても、この推定値はかなり不正確になることが時々あり得ます。
この不正確性のために悪い問い合わせ計画となる場合、より正確な値を手作業で求めて、<link linkend="sql-altertable"><command>ALTER TABLE ... ALTER COLUMN ... SET (n_distinct = ...)</command></link>で設定することができます。␞␞  </para>␞
␝  <para>␟    If the table being analyzed has inheritance children,
    ...
    run <command>ANALYZE</command> manually.␟解析中のテーブルが継承の子テーブルを持つ場合、<command>ANALYZE</command>は統計情報を2組収集します。1組目は親テーブルのみの行に関するもので、2組目は親テーブルとその子テーブルすべての両方の行を含むものです。
...
このテーブルへの挿入や更新がほとんどなければ、継承関係に対する統計情報は手作業で<command>ANALYZE</command>を実行しない限り最新状態にはなりません。␞␞  </para>␞
␝  <para>␟    For partitioned tables, <command>ANALYZE</command> gathers statistics by
    ...
    partitioning it's guaranteed to be empty.␟パーティション化テーブルでは、<command>ANALYZE</command>はすべてのパーティションから行を抽出して統計情報を集めます。それに加えて、再帰的に各パーティションに行って、統計情報を更新します。
複数レベルのパーティショニングが行なわれていたとしても、末端の各パーティションは1度だけ解析されます。
パーティショニングにより空であることが保証されていますので、(パーティションからのデータのない)親テーブルのみの統計情報は収集されません。␞␞  </para>␞
␝  <para>␟    The autovacuum daemon does not process partitioned tables, nor does it
    ...
    up to date.␟自動バキュームデーモンはパーティション化テーブルを処理しませんし、子だけが変更された場合に継承の親を処理することもありません。
テーブル階層の統計情報を最新に保つためには、周期的に手動で<command>ANALYZE</command>を実行することが通常は必要です。␞␞  </para>␞
␝  <para>␟    If any child tables or partitions are foreign tables whose foreign
    data wrappers do not support <command>ANALYZE</command>, those tables are
    ignored while gathering inheritance statistics.␟子テーブルやパーティションが、外部データラッパーが<command>ANALYZE</command>をサポートしない外部テーブルであった場合には、そのテーブルは継承統計情報を集めるときに無視されます。␞␞  </para>␞
␝  <para>␟    If the table being analyzed is completely empty, <command>ANALYZE</command>
    will not record new statistics for that table.  Any existing statistics
    will be retained.␟解析しようとするテーブルが完全に空である場合、<command>ANALYZE</command>はそのテーブルに対する新しい解析情報を記録しません。
これまでの統計情報はすべて保持されます。␞␞  </para>␞
␝  <para>␟    Each backend running <command>ANALYZE</command> will report its progress
    in the <structname>pg_stat_progress_analyze</structname> view. See
    <xref linkend="analyze-progress-reporting"/> for details.␟<command>ANALYZE</command>を実行している各バックエンドはその進捗を<structname>pg_stat_progress_analyze</structname>ビューで報告します。
詳細は<xref linkend="analyze-progress-reporting"/>を参照してください。␞␞  </para>␞
␝ <refsect1>␟  <title>Compatibility</title>␟  <title>互換性</title>␞␞␞
␝  <para>␟   There is no <command>ANALYZE</command> statement in the SQL standard.␟標準SQLには<command>ANALYZE</command>文はありません。␞␞  </para>␞
␝  <para>␟   The following syntax was used before <productname>PostgreSQL</productname>
   version 11 and is still supported:␟<productname>PostgreSQL</productname>バージョン11より前では次の構文が使われていましたが、今でもサポートされています。␞␞<synopsis>␞
