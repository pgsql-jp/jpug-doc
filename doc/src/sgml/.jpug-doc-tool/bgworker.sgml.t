...
␝<chapter id="bgworker">␟ <title>Background Worker Processes</title>␟ <title>バックグラウンドワーカープロセス</title>␞␞␞
␝ <para>␟  PostgreSQL can be extended to run user-supplied code in separate processes.
  ...
  server and behave like a regular client application.␟PostgreSQLはユーザ提供のコードを別のプロセスとして実行するように拡張できます。
...
また、アプリケーションは<application>libpq</application>とリンクすることにより通常のクライアントアプリケーションのようにサーバに接続して動作できます。␞␞ </para>␞
␝  <para>␟   There are considerable robustness and security risks in using background
   ...
   background worker processes.␟バックグラウンドワーカーを使うにあたっては、堅牢性とセキュリティリスクを考慮しなくてはなりません。なぜならば、<literal>C</literal>言語で書かれており、データへのアクセスが制限されていないためです。
バックグラウンドワーカープロセスを含むモジュールを有効にしたいと思っている管理者は、細心の注意を払って実践してください。
バックグラウンドワーカープロセスの実行は、注意深く検査されたモジュールだけを許可する必要があります。␞␞  </para>␞
␝ <para>␟  Background workers can be initialized at the time that
  ...
  **<parameter>handle</parameter>)</function>.  Unlike
  ...
  from a regular backend or another background worker.␟バックグラウンドワーカーは、モジュールを<varname>shared_preload_libraries</varname>に記すことによって、<productname>PostgreSQL</productname>スタート時に初期化できます。
...
postmasterプロセスからのみ呼び出すことができる<function>RegisterBackgroundWorker</function>とは異なり、<function>RegisterDynamicBackgroundWorker</function>は通常のバックエンドまたは他のバックグラウンドワーカーから呼び出す必要があります。␞␞ </para>␞
␝ <para>␟  The structure <structname>BackgroundWorker</structname> is defined thus:␟<structname>BackgroundWorker</structname>の構造体は以下のように定義されます。␞␞<programlisting>␞
␝  <para>␟   <structfield>bgw_name</structfield> and <structfield>bgw_type</structfield> are
   ...
   the type somehow, but that is not strictly required.)␟<structfield>bgw_name</structfield>や<structfield>bgw_type</structfield>は、ログメッセージ、プロセス一覧、および同様の場面で使用される文字列です。
...
（通常、<structfield>bgw_name</structfield>の文字列は何らかの形で種類に関する情報を含んでいますが、必須であるというわけではありません。）␞␞  </para>␞
␝  <para>␟   <structfield>bgw_flags</structfield> is a bitwise-or'd bit mask indicating the
   capabilities that the module wants.  Possible values are:␟<structfield>bgw_flags</structfield>は、モジュールが要求する機能をOR演算したビットマスクです。可能な値は以下の通りです。␞␞   <variablelist>␞
␝       <indexterm><primary>BGWORKER_SHMEM_ACCESS</primary></indexterm>␟       Requests shared memory access.  This flag is required.␟共有メモリへのアクセスを要求します。
このフラグは必須です。␞␞      </para>␞
␝      <para>␟       <indexterm><primary>BGWORKER_BACKEND_&zwsp;DATABASE_CONNECTION</primary></indexterm>
       ...
       <literal>BGWORKER_SHMEM_ACCESS</literal>, or worker start-up will fail.␟<indexterm><primary>BGWORKER_BACKEND_&zwsp;DATABASE_CONNECTION</primary></indexterm>
トランザクションや問い合わせの実行が出来るデータベース接続を要求します。
<literal>BGWORKER_BACKEND_DATABASE_CONNECTION</literal>を使用してデータベースに接続するバックグラウンドワーカーは<literal>BGWORKER_SHMEM_ACCESS</literal>を使用して共有メモリにアタッチしなければなりません。さもなければ起動時に失敗します。␞␞      </para>␞
␝  <para>␟   <structfield>bgw_start_time</structfield> is the server state during which
   ...
   is reached.␟<structfield>bgw_start_time</structfield>は、<command>postgres</command>がプロセスを起動するべきタイミングを指定します。
そのタイミングは、以下のうちの１つです。
...
これらのプロセスは、違う状態になったときに停止するわけではありません。␞␞  </para>␞
␝  <para>␟   <structfield>bgw_restart_time</structfield> is the interval, in seconds, that
   <command>postgres</command> should wait before restarting the process in
   the event that it crashes.  It can be any positive value,
   or <literal>BGW_NEVER_RESTART</literal>, indicating not to restart the
   process in case of a crash.␟<structfield>bgw_restart_time</structfield>は、万が一プロセスがクラッシュした場合に<command>postgres</command>がそのプロセスを再起動するために待つ必要のある間隔を秒単位で指定します。
これは任意の正の値、またはクラッシュしても再起動させない場合に<literal>BGW_NEVER_RESTART</literal>を指定します。␞␞  </para>␞
␝  <para>␟   <structfield>bgw_library_name</structfield> is the name of a library in
   ...
   be set to <literal>"postgres"</literal>.␟<structfield>bgw_library_name</structfield>は、バックグラウンドワーカーの初期エントリポイントのためのライブラリ名です。
その指定されたライブラリがワーカープロセスによって動的にロードされます。呼び出すべき関数を特定するために<structfield>bgw_function_name</structfield>が使用されます。
コアコード内の関数を呼び出す場合、<literal>"postgres"</literal>を設定する必要があります。␞␞  </para>␞
␝  <para>␟   <structfield>bgw_function_name</structfield> is the name of the function
   ...
   <literal>PGDLLEXPORT</literal> (and not <literal>static</literal>).␟<structfield>bgw_function_name</structfield>は、新しいバックグラウンドワーカーの初期エントリポイントとして使用する関数の名前です。
この関数が動的にロードされたライブラリ内にある場合、<literal>PGDLLEXPORT</literal>（<literal>static</literal>ではない）とマークする必要があります。␞␞  </para>␞
␝  <para>␟   <structfield>bgw_main_arg</structfield> is the <type>Datum</type> argument
   ...
   this structure.␟<structfield>bgw_main_arg</structfield>は、バックグラウンドワーカーのメイン関数の<type>Datum</type>引数です。
...
ワーカーはこの構造を調べることがあり、役に立ちます。␞␞  </para>␞
␝  <para>␟   On Windows (and anywhere else where <literal>EXEC_BACKEND</literal> is
   ...
   new background worker process.␟Windowsの（どこか他の場所で<literal>EXEC_BACKEND</literal>が定義されている）場合、または動的バックグラウンドワーカーは、<type>Datum</type>を参照で渡すのは安全ではありません。値のみで渡してください。
引数が必要な場合は、int32型または他の小さな値を渡し、共有メモリに割り当てられた配列へのインデックスとしてそれを使用するのが最も安全です。
<type>cstring</type>や<type>text</type>のようなポインタを渡された場合は、新しいバックグラウンドワーカープロセスから有効になりません。␞␞  </para>␞
␝  <para>␟   <structfield>bgw_extra</structfield> can contain extra data to be passed
   ...
   accessed via <literal>MyBgworkerEntry</literal>, as discussed above.␟<structfield>bgw_extra</structfield>はバックグラウンドワーカーに渡す追加データを含めることが出来ます。
<structfield>bgw_main_arg</structfield>とは異なり、このデータはワーカーのメイン関数の引数として渡されていませんが、上述したように<literal>MyBgworkerEntry</literal>を介してアクセスできます。␞␞  </para>␞
␝  <para>␟   <structfield>bgw_notify_pid</structfield> is the PID of a PostgreSQL
   backend process to which the postmaster should send <literal>SIGUSR1</literal>
   when the process is started or exits.  It should be 0 for workers registered
   at postmaster startup time, or when the backend registering the worker does
   not wish to wait for the worker to start up.  Otherwise, it should be
   initialized to <literal>MyProcPid</literal>.␟<structfield>bgw_notify_pid</structfield>は、プロセスの開始時と終了時にpostmasterが<literal>SIGUSR1</literal>を送信するPostgreSQLバックエンドプロセスのPIDです。
それはpostmasterの起動時に登録されたワーカーの場合、またはワーカーを登録しているバックエンドがワーカーの起動を待ちたくない場合は0にする必要があります。
それ以外の場合は、<literal>MyProcPid</literal>で初期化する必要があります。␞␞  </para>␞
␝␟  <para>Once running, the process can connect to a database by calling
   ...
   once.  It is not possible to switch databases.␟  <para>
...
データベースを切り替えることができません。␞␞  </para>␞
␝  <para>␟   Signals are initially blocked when control reaches the
   background worker's main function, and must be unblocked by it; this is to
   allow the process to customize its signal handlers, if necessary.
   Signals can be unblocked in the new process by calling
   <function>BackgroundWorkerUnblockSignals</function> and blocked by calling
   <function>BackgroundWorkerBlockSignals</function>.␟バックグラウンドワーカーのメイン関数に制御が達したとき、シグナルは最初にブロックされています。このブロックは解除されなければなりません。
これは、必要に応じてプロセスがシグナルハンドラをカスタマイズできるようにするためです。
シグナルは、新しいプロセスで<function>BackgroundWorkerUnblockSignals</function>を呼び出すことにより解除でき、<function>BackgroundWorkerBlockSignals</function>を呼び出すことでブロックできます。␞␞  </para>␞
␝  <para>␟   If <structfield>bgw_restart_time</structfield> for a background worker is
   ...
   <command>postgres</command> itself has terminated.␟バックグラウンドワーカーは、<structfield>bgw_restart_time</structfield>が<literal>BGW_NEVER_RESTART</literal>に設定されている場合、または終了コード0で終了した場合、または<function>TerminateBackgroundWorker</function>によって終了した場合、postmasterに自動的に登録が解除されて終了します。
...
この関数を呼び出すときには<literal>WL_POSTMASTER_DEATH</literal>フラグが設定されているか確認し、<command>postgres</command>自身が終了する緊急事態には、リターンコードを確認するようにしてください。␞␞  </para>␞
␝  <para>␟   When a background worker is registered using the
   ...
   as soon as it is not.␟バックグラウンドワーカーを<function>RegisterDynamicBackgroundWorker</function>関数により登録している場合、登録を実行するバックエンドはワーカーの状態に関する情報を取得することが可能です。
...
<function>GetBackgroundWorkerPid</function>はワーカーの状態を監視できます。以下の戻り値が得られます。
...
<function>TerminateBackgroundWorker</function>はワーカーが実行していた場合postmasterがワーカーに<literal>SIGTERM</literal>を送信し、実行が終了次第すぐに登録を解除します。␞␞  </para>␞
␝  <para>␟   In some cases, a process which registers a background worker may wish to
   ...
   *handle</parameter>, <parameter>pid_t *</parameter>)</function> function.
   ...
   <literal>BGWH_POSTMASTER_DIED</literal>.␟場合によっては、バックグラウンドワーカーが起動するのを待ってから、ワーカーを登録したい場合もあるでしょう。
...
そうでない場合、戻り値は<literal>BGWH_STOPPED</literal>または<literal>BGWH_POSTMASTER_DIED</literal>になります。␞␞  </para>␞
␝  <para>␟   A process can also wait for a background worker to shut down, by using the
   ...
   *handle</parameter>)</function> function and passing the
   ...
   <literal>BGWH_POSTMASTER_DIED</literal>.␟登録時に得られた<type>BackgroundWorkerHandle *</type>を使用して<function>WaitForBackgroundWorkerShutdown(<parameter>BackgroundWorkerHandle *handle</parameter>)</function>関数を呼び出すことで、バックグラウンドワーカーがシャットダウンするのを待つこともできます。
バックグラウンドワーカーが終了するか、postmasterが死ぬまで、この関数はブロックします。
バックグラウンドワーカーが終了した場合の戻り値は<literal>BGWH_STOPPED</literal>、postmasterが死んだ場合の戻り値は<literal>BGWH_POSTMASTER_DIED</literal>になります。␞␞  </para>␞
␝  <para>␟   Background workers can send asynchronous notification messages, either by
   ...
   infrastructure for a worker to consume such notifications.␟バックグラウンドワーカーは、<acronym>SPI</acronym>経由で<command>NOTIFY</command>コマンドを使用して、あるいは<function>Async_Notify()</function>で直接、非同期通知メッセージを送ることができます。
そのような通知はトランザクションのコミット時に送信されます。
バックグラウンドワーカーは、<command>LISTEN</command>コマンドによる非同期通知メッセージの受信登録をすべきではありません。
ワーカーがそのような通知を消費する基盤が存在しないからです。␞␞  </para>␞
␝  <para>␟   The <filename>src/test/modules/worker_spi</filename> module
   contains a working example,
   which demonstrates some useful techniques.␟バックグラウンドワーカーの実例として、<filename>src/test/modules/worker_spi</filename>というモジュールがあります。
これはいくつかの有用な技術を示しています。␞␞  </para>␞
␝  <para>␟   The maximum number of registered background workers is limited by
   <xref linkend="guc-max-worker-processes"/>.␟登録できるバックグラウンドワーカーの最大数は<xref linkend="guc-max-worker-processes"/>によって制限されています。␞␞  </para>␞
