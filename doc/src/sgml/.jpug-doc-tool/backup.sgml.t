...
␝<chapter id="backup">␟ <title>Backup and Restore</title>␟ <title>バックアップとリストア</title>␞␞␞
␝ <para>␟  As with everything that contains valuable data, <productname>PostgreSQL</productname>
  databases should be backed up regularly. While the procedure is
  essentially simple, it is important to have a clear understanding of
  the underlying techniques and assumptions.␟貴重なデータを保持しているあらゆるもの同様、<productname>PostgreSQL</productname>データベースも定期的にバックアップされなければなりません。
バックアップの手順は基本的に簡単ですが、使用されている諸技術といくつかの前提条件を明確に理解しておくことが重要です。␞␞ </para>␞
␝ <para>␟  There are three fundamentally different approaches to backing up
  <productname>PostgreSQL</productname> data:␟<productname>PostgreSQL</productname>のデータをバックアップする場合、3つの異なる手法があります。␞␞  <itemizedlist>␞
␝  <itemizedlist>␟   <listitem><para><acronym>SQL</acronym> dump</para></listitem>
   <listitem><para>File system level backup</para></listitem>
   <listitem><para>Continuous archiving</para></listitem>␟   <listitem><para><acronym>SQL</acronym>によるダンプ</para></listitem>
   <listitem><para>ファイルシステムレベルのバックアップ</para></listitem>
   <listitem><para>継続的アーカイブ</para></listitem>␞␞  </itemizedlist>␞
␝  </itemizedlist>␟  Each has its own strengths and weaknesses; each is discussed in turn
  in the following sections.␟それぞれ長所と短所があります。
ひとつひとつ順を追って以下の節で説明します。␞␞ </para>␞
␝ <sect1 id="backup-dump">␟  <title><acronym>SQL</acronym> Dump</title>␟  <title><acronym>SQL</acronym>によるダンプ</title>␞␞␞
␝  <para>␟   The idea behind this dump method is to generate a file with SQL
   commands that, when fed back to the server, will recreate the
   database in the same state as it was at the time of the dump.
   <productname>PostgreSQL</productname> provides the utility program
   <xref linkend="app-pgdump"/> for this purpose. The basic usage of this
   command is:␟このダンプ方法の背景にはSQLコマンドでファイルを生成し、そのファイルをサーバが再度読み込みを行った時に、ダンプした時点と同じ状態が再構築されるという意図があります。
この目的のため、<productname>PostgreSQL</productname>は<xref linkend="app-pgdump"/>ユーティリティプログラムを提供しています。
このコマンドの基本となる使い方は以下の通りです。␞␞<synopsis>␞
␝</synopsis>␟   As you see, <application>pg_dump</application> writes its result to the
   ...
   fine-grained control of object restoration.␟見てわかる通り、<application>pg_dump</application>は結果を標準出力に書き出します。
これがどのように活用できるかをこれから説明します。
上記のコマンドはテキストファイルを作成しますが、<application>pg_dump</application>は並列処理を可能にしたり、オブジェクトのリストアをより細かく制御できる他のフォーマットでファイルを作れます。␞␞  </para>␞
␝  <para>␟   <application>pg_dump</application> is a regular <productname>PostgreSQL</productname>
   ...
   or <option>-t <replaceable>table</replaceable></option>.)␟<application>pg_dump</application>は、<productname>PostgreSQL</productname>の通常のクライアントアプリケーションです（その中でも特に優れた機能を発揮するものですが）。
...
（もしデータベース全体のバックアップを取るのに十分な権限を持っていない場合には、<option>-n <replaceable>schema</replaceable></option>もしくは、<option>-t <replaceable>table</replaceable></option>のようなオプションを使って、データベースのアクセス権のある部分をバックアップできます。）␞␞  </para>␞
␝  <para>␟   To specify which database server <application>pg_dump</application> should
   ...
   default.)␟<application>pg_dump</application>を行うデータベースサーバを特定するにはコマンドラインの<option>-h <replaceable>host</replaceable></option>オプションと<option>-p <replaceable>port</replaceable></option>オプションを使用します。
デフォルトのホストはローカルホスト、または<envar>PGHOST</envar>環境変数で指定したものです。
同様に、デフォルトのポートは<envar>PGPORT</envar>環境変数で指定されているか、うまく行かない場合にはコンパイル時の設定がデフォルトとなります。（そこはうまくできていて、サーバは通常コンパイル時の設定をデフォルトとします。）␞␞  </para>␞
␝  <para>␟   Like any other <productname>PostgreSQL</productname> client application,
   ...
   linkend="client-authentication"/>).␟他の<productname>PostgreSQL</productname>のクライアントアプリケーションのように、<application>pg_dump</application>はデフォルトでオペレーティングシステムの現在のユーザ名と同じデータベースユーザ名で接続します。
これを書き換えるには<option>-U</option>オプションを付けるか<envar>PGUSER</envar>環境変数を設定します。
<application>pg_dump</application>の接続は（<xref linkend="client-authentication"/>で説明されている）通常のクライアント認証方法によることを思い出してください。␞␞  </para>␞
␝  <para>␟   An important advantage of <application>pg_dump</application> over the other backup
   ...
   architecture, such as going from a 32-bit to a 64-bit server.␟後で述べる他のバックアップ手法に対する<application>pg_dump</application>の重要な利点は、<application>pg_dump</application>の出力は一般に新しいバージョンの<productname>PostgreSQL</productname>に再ロードできるということです。
一方、ファイルレベルのバックアップと継続的アーカイブは両方とも非常にサーバ、バージョン依存です。
<application>pg_dump</application>は、32ビットから64ビットのサーバに移行するなどの異なるマシンアーキテクチャにデータベースを移す場合に上手くいく唯一の方法でもあります。␞␞  </para>␞
␝  <para>␟   Dumps created by <application>pg_dump</application> are internally consistent,
   ...
   exclusive lock, such as most forms of <command>ALTER TABLE</command>.)␟<application>pg_dump</application>で作成されたダンプは、内部的に整合性があります。
...
（<command>ALTER TABLE</command>のほとんどの形態であるような排他的ロックが必要な作業は例外です。）␞␞  </para>␞
␝  <sect2 id="backup-dump-restore">␟   <title>Restoring the Dump</title>␟   <title>ダンプのリストア</title>␞␞␞
␝   <para>␟    Text files created by <application>pg_dump</application> are intended to
    be read by the <application>psql</application> program using its default
    settings. The general command form to restore a text dump is␟<application>pg_dump</application>で作成されたテキストファイルは、デフォルト設定の<application>psql</application>プログラムで読み込まれることを意図しています。
以下に、テキストダンプをリストアする一般的なコマンドを示します。␞␞<synopsis>␞
␝</synopsis>␟    where <replaceable class="parameter">dumpfile</replaceable> is the
    ...
    the <xref linkend="app-psql"/> reference page for more information.␟ここで<replaceable class="parameter">dumpfile</replaceable>は<application>pg_dump</application>コマンドにより出力されたファイルです。
...
詳細については、<xref linkend="app-psql"/>のリファレンスページを参照してください。␞␞   </para>␞
␝   <para>␟    Non-text file dumps should be restored using the <xref
    linkend="app-pgrestore"/> utility.␟テキスト形式ではないダンプファイルは<xref linkend="app-pgrestore"/>ユーティリティを使いリストアします。␞␞   </para>␞
␝   <para>␟    Before restoring an SQL dump, all the users who own objects or were
    ...
    (Sometimes this is what you want, but usually it is not.)␟SQLダンプのリストアを実行する前に、ダンプされたデータベース内のオブジェクトを所有するユーザやそのオブジェクト上に権限を与えられたユーザも存在しなければなりません。
存在していない場合、リストアはそのオブジェクトの元々の所有権や付与された権限を再作成することができません。
（このようにしたい場合もあるでしょうが、通常そうではありません。）␞␞   </para>␞
␝   <para>␟    By default, the <application>psql</application> script will continue to
    ...
    exit status of 3 if an SQL error occurs:␟デフォルトで<application>psql</application>スクリプトは、SQLエラーが起きた後も実行を継続します。
<literal>ON_ERROR_STOP</literal>変数を設定して<application>psql</application>を実行することで、その動作を変更し、SQLエラーが起きた場合に<application>psql</application>が、終了ステータス3で終了するようにしたいと思うかもしれません。␞␞<programlisting>␞
␝</programlisting>␟    Either way, you will only have a partially restored database.
    ...
    after a partially restored dump.␟どちらにしても、部分的にリストアされたデータベースにしかなりません。
...
しかし、部分的にリストアされたダンプから手作業で複雑なデータベースを整理するよりまだましかもしれません。␞␞   </para>␞
␝   <para>␟    The ability of <application>pg_dump</application> and <application>psql</application> to
    write to or read from pipes makes it possible to dump a database
    directly from one server to another, for example:␟<application>pg_dump</application>と<application>psql</application>ではパイプから読み書きができるので、あるサーバから別のサーバへデータベースを直接ダンプできます。
以下に例を示します。␞␞<programlisting>␞
␝    <para>␟     The dumps produced by <application>pg_dump</application> are relative to
     ...
     above.␟<application>pg_dump</application>で作成されるダンプは<literal>template0</literal>と相対関係にあります。
つまり<literal>template1</literal>を経由して追加されたあらゆる言語、プロシージャなども<application>pg_dump</application>によりダンプされます。
その結果としてリストアする際に、カスタマイズされた<literal>template1</literal>を使用している場合は、上記の例のように、<literal>template0</literal>から空のデータベースを作成する必要があります。␞␞    </para>␞
␝   <para>␟    After restoring a backup, it is wise to run <link
    ...
    linkend="populate"/>.␟バックアップをリストアした後、問い合わせオプティマイザが有用な統計情報を使用できるように、各データベースに対して<link linkend="sql-analyze"><command>ANALYZE</command></link>を実行することを勧めます。
より詳しくは、<xref linkend="vacuum-for-statistics"/> と <xref linkend="autovacuum"/>を参照してください。
効率的に大規模なデータを<productname>PostgreSQL</productname>にロードする方法に関するより多くの勧告については、<xref linkend="populate"/>を参照してください。␞␞   </para>␞
␝  <sect2 id="backup-dump-all">␟   <title>Using <application>pg_dumpall</application></title>␟   <title><application>pg_dumpall</application>の使用</title>␞␞␞
␝   <para>␟    <application>pg_dump</application> dumps only a single database at a time,
    ...
    tablespace definitions. The basic usage of this command is:␟<application>pg_dump</application>は一度に単一のデータベースのみをダンプします。
...
␟␟です。␞␞<synopsis>␞
␝</synopsis>␟    The resulting dump can be restored with <application>psql</application>:␟ダンプの結果は<application>psql</application>でリストアできます。␞␞<synopsis>␞
␝</synopsis>␟    (Actually, you can specify any existing database name to start from,
    ...
    dump are appropriate for the new installation.␟（実際、開始時に任意の既存のデータベース名を指定することができますが、空のクラスタ内にロードする場合は、通常 <literal>postgres</literal> を使用すべきです。）
ロールやテーブル空間の情報をリストアしなければならないので、<application>pg_dumpall</application>のダンプをリストアする時には、データベーススーパーユーザのアクセス権限を確実に必要とします。
テーブル空間を使用している場合、ダンプ内のテーブル空間のパスが新しいインストレーションで適切であることを確認してください。␞␞   </para>␞
␝   <para>␟    <application>pg_dumpall</application> works by emitting commands to re-create
    roles, tablespaces, and empty databases, then invoking
    <application>pg_dump</application> for each database.  This means that while
    each database will be internally consistent, the snapshots of
    different databases are not synchronized.␟<application>pg_dumpall</application>はコマンドを発令することによりロール、テーブル空間、およびデータベースを再作成し、それぞれのデータベースに対して<application>pg_dump</application>を起動します。
このことは、それぞれのデータベースには内部的に矛盾がない一方、異なるデータベースのスナップショットは完全に同期しないことを示しています。␞␞   </para>␞
␝   <para>␟    Cluster-wide data can be dumped alone using the
    ...
    <application>pg_dump</application> command on individual databases.␟クラスタレベルでのデータは<application>pg_dumpall</application> の<option>--globals-only</option> オプションを使用して出力することができます。
このコマンドは個々のデータベースに<application>pg_dump</application> コマンドを実行しつつ、フルバックアップを取得する際に必要です。␞␞   </para>␞
␝  <sect2 id="backup-dump-large">␟   <title>Handling Large Databases</title>␟   <title>大規模データベースの扱い</title>␞␞␞
␝   <para>␟    Some operating systems have maximum file size limits that cause
    ...
    potential problem.  There are several possible methods:␟オペレーティングシステムの中には最大ファイルサイズに制限があるものがあり、大きな<application>pg_dump</application>出力ファイルを作成しているときに問題を引き起こします。
幸運なことに、<application>pg_dump</application>は標準出力に書き出すことができますので、Unix標準のツールを使ってこの潜在的な問題を解決できます。
取りうる方法がいくつか存在します。␞␞   </para>␞
␝   <formalpara>␟    <title>Use compressed dumps.</title>␟    <title>圧縮ダンプの使用</title>␞␞    <para>␞
␝    <para>␟     You can use your favorite compression program, for example
     <application>gzip</application>:␟たとえば、自分が愛用している<application>gzip</application>のような圧縮プログラムが使えます。␞␞␞
␝␟     Reload with:␟元に戻すには次のようにします。␞␞␞
␝␟     or:␟あるいは次のようにもできます。␞␞␞
␝   <formalpara>␟    <title>Use <command>split</command>.</title>␟    <title><command>split</command>の使用</title>␞␞    <para>␞
␝    <para>␟     The <command>split</command> command
     allows you to split the output into smaller files that are
     acceptable in size to the underlying file system. For example, to
     make 2 gigabyte chunks:␟<command>split</command>コマンドで結果を使用しているファイルシステムが受け付けられる大きさに分割することができます。
例えば2ギガバイトずつに分割するには次のようにします。␞␞␞
␝␟     Reload with:␟元に戻すには次のようにします。␞␞␞
␝␟     If using GNU <application>split</application>, it is possible to
     use it and <application>gzip</application> together:␟GNU <application>split</application>を使用している場合は、次のように<application>gzip</application>を一緒に使うことでファイルの圧縮が出来ます。␞␞␞
␝␟     It can be restored using <command>zcat</command>.␟圧縮されたファイルは<command>zcat</command>を使ってリストア出来ます。␞␞    </para>␞
␝   <formalpara>␟    <title>Use <application>pg_dump</application>'s custom dump format.</title>␟    <title><application>pg_dump</application>のカスタムダンプ書式の使用</title>␞␞    <para>␞
␝    <para>␟     If <productname>PostgreSQL</productname> was built on a system with the
     ...
     following command dumps a database using the custom dump format:␟もし<productname>PostgreSQL</productname>が<application>zlib</application>圧縮ライブラリインストール済みのシステム上で構築されたのなら、カスタムダンプ書式では出力ファイルに書き出す時にデータを圧縮します。
...
␝␟     A custom-format dump is not a script for <application>psql</application>, but
     instead must be restored with <application>pg_restore</application>, for example:␟カスタム書式のダンプは<application>psql</application>用のスクリプトではありませんので、代わりに<application>pg_restore</application>でリストアしなければなりません。
例えば以下のようにします。␞␞␞
␝␟     See the <xref linkend="app-pgdump"/> and <xref
     linkend="app-pgrestore"/> reference pages for details.␟詳細は<xref linkend="app-pgdump"/>と<xref linkend="app-pgrestore"/>のリファレンスページを参照してください。␞␞    </para>␞
␝   <para>␟    For very large databases, you might need to combine <command>split</command>
    with one of the other two approaches.␟巨大なデータベースに対しては、そのほかの２つの手法のうちの１つと一緒に<command>split</command>を組み合わせる必要があるかもしれません。␞␞   </para>␞
␝   <formalpara>␟    <title>Use <application>pg_dump</application>'s parallel dump feature.</title>␟    <title><application>pg_dump</application>の並列実行</title>␞␞    <para>␞
␝    <para>␟     To speed up the dump of a large database, you can use
     ...
     are only supported for the "directory" archive format.␟<application>pg_dump</application>を並列実行することで、大きなデータベースのダンプを高速に実行することができます。
...
␝␟     You can use <command>pg_restore -j</command> to restore a dump in parallel.
     ...
     archive mode, whether or not it has been created with <command>pg_dump -j</command>.␟<command>pg_restore -j</command>コマンドでダンプファイルを並列でリストアすることができます。
これは<command>pg_dump -j</command>でダンプファイルが作成されたか、否かにかかわらず、カスタムもしくはディレクトリダンプ書式で作成されたダンプファイルに使用できます。␞␞    </para>␞
␝ <sect1 id="backup-file">␟  <title>File System Level Backup</title>␟  <title>ファイルシステムレベルのバックアップ</title>␞␞␞
␝  <para>␟   An alternative backup strategy is to directly copy the files that
   ...
   for doing file system backups; for example:␟バックアップ戦略の代替案として<productname>PostgreSQL</productname>がデータベース内のデータを保存するために使用しているファイルを直接コピーする方法があります。
<xref linkend="creating-cluster"/>にこれらのファイルがどこにあるか解説されています。
下記のような通常のファイルシステムのバックアップを行うどんな方法でも問題ありません。␞␞␞
␝  <para>␟   There are two restrictions, however, which make this method
   impractical, or at least inferior to the <application>pg_dump</application>
   method:␟しかしこの方法には2つの制約があり、そのためにあまり実用的ではなく、少なくとも<application>pg_dump</application>より劣ると言わざるを得ません。␞␞␞
␝     <para>␟      The database server <emphasis>must</emphasis> be shut down in order to
      ...
      also need to shut down the server before restoring the data.␟有効なバックアップを行うにはデータベースサーバを<emphasis>必ず</emphasis>停止しなければなりません。
...
言うまでもありませんが、データをリストアする前にもサーバを停止させる必要があります。␞␞     </para>␞
␝     <para>␟      If you have dug into the details of the file system layout of the
      ...
      backup and restoration of an entire database cluster.␟データベースのファイルシステムレイアウトの詳細を熟知している場合、ある個別のテーブルやデータベースをそれぞれのファイルやディレクトリからバックアップしたり復元したりすることを試みたいと思うかもしれません。
...
ですので、ファイルシステムバックアップは、データベースクラスタ全体の完全なバックアップとリストア処理にのみ動作します。␞␞     </para>␞
␝  <para>␟   An alternative file-system backup approach is to make a
   ...
   snapshot to reduce recovery time.␟その他のファイルシステムバックアップ方法として、ファイルシステムが<quote>整合性を維持したスナップショット</quote>機能をサポートしている場合（かつ、正しく実装されていると信用する場合）、データディレクトリのスナップショットを作成する方法があります。
...
<command>CHECKPOINT</command>コマンドをスナップショット取得前に発行することで復旧時間を減らすこともできます。␞␞  </para>␞
␝  <para>␟   If your database is spread across multiple file systems, there might not
   ...
   the consistent-snapshot technique in such situations.␟対象のデータベースが複数のファイルシステムにまたがって分散している場合、全てのボリュームに対して完全に同期した凍結スナップショットを得る方法が存在しない可能性があります。
...
こうした状況では、整合性を維持したスナップショット技術を信用する前に使用するファイルシステムの文書を熟読してください。␞␞  </para>␞
␝  <para>␟   If simultaneous snapshots are not possible, one option is to shut down
   ...
   continuous archive recovery (<xref linkend="backup-pitr-recovery"/>).␟同時実行のスナップショットができない場合、選択肢の１つとして、全ての機能の停止したスナップショットを確定させるのに充分な時間、データベースサーバをシャットダウンさせることが挙げられます。
他の選択肢は、継続的なベースバックアップの保管（<xref linkend="backup-base-backup"/>）を行うことです。
こうしたバックアップには、バックアップ中のファイルシステムの変更を心配する必要がないためです。
これにはバックアップ処理期間のみに継続的な保管を行う必要があり、継続的なアーカイブリカバリ（<xref linkend="backup-pitr-recovery"/>）を使用してリストアを行います。␞␞  </para>␞
␝  <para>␟   Another option is to use <application>rsync</application> to perform a file
   ...
   allows a file system backup to be performed with minimal downtime.␟ファイルシステムをバックアップするその他の選択肢として<application>rsync</application>の使用が挙げられます。
...
この方法で最小の稼働停止時間でファイルシステムのバックアップを行う事ができます。␞␞  </para>␞
␝  <para>␟   Note that a file system backup will typically be larger
   than an SQL dump. (<application>pg_dump</application> does not need to dump
   the contents of indexes for example, just the commands to recreate
   them.)  However, taking a file system backup might be faster.␟ファイルシステムバックアップは、概してSQLによるダンプより大きくなることに注意してください。
（<application>pg_dump</application>では、例えばインデックスの内容をダンプする必要はありません。単にコマンドで再作成します。）
しかし、ファイルシステムのバックアップを取るほうがより高速でしょう。␞␞  </para>␞
...
␝ <sect1 id="continuous-archiving">␟  <title>Continuous Archiving and Point-in-Time Recovery (PITR)</title>␟  <title>継続的アーカイブとポイントインタイムリカバリ（PITR）</title>␞␞␞
␝  <para>␟   At all times, <productname>PostgreSQL</productname> maintains a
   ...
   benefits:␟<productname>PostgreSQL</productname>は常に、クラスタのデータディレクトリ以下の<filename>pg_wal/</filename>ディレクトリ内で<firstterm>先行書き込みログ</firstterm>（WAL）を管理しています。
...
管理者にとって、この方法はこれまで説明した方法よりかなり複雑になりますが、以下のような大きな利点が複数あります。␞␞  <itemizedlist>␞
␝    <para>␟     We do not need a perfectly consistent file system backup as the starting point.
     Any internal inconsistency in the backup will be corrected by log
     replay (this is not significantly different from what happens during
     crash recovery).  So we do not need a file system snapshot capability,
     just <application>tar</application> or a similar archiving tool.␟開始時点のファイルシステムバックアップは完全な整合状態である必要はありません。
そのバックアップ内の内部的な不整合はログのやり直しによって修正されます
（これは、クラッシュからの復旧時に行われることと大きな違いはありません）。
ですので、ファイルシステムのスナップショット機能を必要としません。
単に<application>tar</application>などのアーカイブツールが必要です。␞␞    </para>␞
␝    <para>␟     Since we can combine an indefinitely long sequence of WAL files
     for replay, continuous backup can be achieved simply by continuing to archive
     the WAL files.  This is particularly valuable for large databases, where
     it might not be convenient to take a full backup frequently.␟再生の際にWALファイルの並びを数に制限なく連ねて組み合わせられますので、単にWALファイルのアーカイブを続けることで連続したバックアップを達成できます。
これは、頻繁に完全なバックアップを行うことが困難な、大規模なデータベースでは特に価値があります。␞␞    </para>␞
␝    <para>␟     It is not necessary to replay the WAL entries all the
     ...
     backup was taken.␟WAL項目の再生を最後まで行わなければならないということはありません。
やり直しを任意の時点までで停止することができ、それにより、その時点までのデータベースの整合性を持ったスナップショットを得ることができます。
このような技術が<firstterm>ポイントインタイムリカバリ</firstterm>を補助するものであり、元となるベースバックアップの取得時点以降の任意の時点の状態にデータベースをリストアすることが可能になります。␞␞    </para>␞
␝    <para>␟     If we continuously feed the series of WAL files to another
     machine that has been loaded with the same base backup file, we
     have a <firstterm>warm standby</firstterm> system: at any point we can bring up
     the second machine and it will have a nearly-current copy of the
     database.␟連続的に一連のWALファイルを、同一のベースバックアップをロードしている別のマシンに配送することで、<firstterm>ウォームスタンバイ</firstterm>システムを保有することができます。
つまり、任意の時点でその2番目のマシンを、ほぼ現時点のデータベースの複製を持った状態で有効にすることができます。␞␞    </para>␞
␝   <para>␟    <application>pg_dump</application> and
    ...
    information to be used by WAL replay.␟<application>pg_dump</application>と<application>pg_dumpall</application>はファイルシステムレベルのバックアップを生成しませんので、継続的アーカイブ方式の一部として使うことはできません。
そのダンプは<emphasis>論理的</emphasis>なものであり、WALのやり直しで使うのに十分な情報を含んでいません。␞␞   </para>␞
␝  <para>␟   As with the plain file-system-backup technique, this method can only
   support restoration of an entire database cluster, not a subset.
   Also, it requires a lot of archival storage: the base backup might be bulky,
   and a busy system will generate many megabytes of WAL traffic that
   have to be archived.  Still, it is the preferred backup technique in
   many situations where high reliability is needed.␟通常のファイルシステムバックアップ技術の場合と同様、この方法は、一部ではなく、データベースクラスタ全体のリストア処理のみをサポートできます。
また、アーカイブ用に大量の格納領域を必要とします。
ベースバックアップはかさばる場合があり、また、高負荷なシステムではアーカイブしなければならないWALの流量をメガバイト単位で生成します。
しかし、これは、高信頼性が必要な、多くの状況でむしろ好まれるバックアップ手法です。␞␞  </para>␞
␝  <para>␟   To recover successfully using continuous archiving (also called
   ...
   archiving WAL files.␟継続的アーカイブ（多くのデータベースベンダで<quote>オンラインバックアップ</quote>とも呼ばれます）を使用して復旧を成功させるためには、少なくともバックアップの開始時点まで遡る、連続した一連のアーカイブ済みWALファイルが必要です。
ですので、運用するためには、最初のベースバックアップを取得する<emphasis>前</emphasis>にWALファイルをアーカイブする手順を設定し試験しなければなりません。
したがって、まずWALファイルのアーカイブ機構について説明します。␞␞  </para>␞
␝  <sect2 id="backup-archiving-wal">␟   <title>Setting Up WAL Archiving</title>␟   <title>WALアーカイブの設定</title>␞␞␞
␝   <para>␟    In an abstract sense, a running <productname>PostgreSQL</productname> system
    ...
    interest and can be recycled.␟抽象的な意味では、実行中の<productname>PostgreSQL</productname>システムは無限に長い一連のWALレコードを生成します。
...
最後のチェックポイントより前の内容を持つセグメントファイルはもはや重要でなく、リサイクルできると見なされます。␞␞   </para>␞
␝   <para>␟    When archiving WAL data, we need to capture the contents of each segment
    ...
    complex C function &mdash; it's all up to you.␟WALデータをアーカイブする場合、完成したセグメントファイルのそれぞれの内容を取り出し、再利用のために回収される前にそのデータをどこかに保存することが必要です。
...
全て管理者に任されています。␞␞   </para>␞
␝   <para>␟    To enable WAL archiving, set the <xref linkend="guc-wal-level"/>
    ...
    <filename>postgresql.conf</filename> file.␟WALアーカイブを有効にするには、<xref linkend="guc-wal-level"/>設定パラメータを<literal>replica</literal>以上に、<xref linkend="guc-archive-mode"/>を<literal>on</literal>に設定し、<xref linkend="guc-archive-command"/>設定パラメータで使用するシェルコマンドを指定するか、<xref linkend="guc-archive-library"/>設定パラメータで使用するライブラリを指定します。
実際には、これらの設定は常に<filename>postgresql.conf</filename>ファイルに置かれます。␞␞   </para>␞
␝   <para>␟    In <varname>archive_command</varname>,
    ...
    like:␟<varname>archive_command</varname>では、<literal>%p</literal>はアーカイブするファイルのパス名に置き換えられますが、<literal>%f</literal>はファイル名のみに置き換えられます。
（パス名は現在の作業ディレクトリ、つまりクラスタのデータディレクトリからの相対パスです。）
実際の<literal>%</literal>文字をコマンドに埋め込む必要がある場合は<literal>%%</literal>を使用してください。
最も簡単で便利なコマンドは以下のようなものです。␞␞<programlisting>␞
␝</programlisting>␟    which will copy archivable WAL segments to the directory
    ...
    the actual command executed might look like this:␟これは、アーカイブ可能なWALセグメントを<filename>/mnt/server/archivedir</filename>ディレクトリにコピーします。
（これは一例です。
推奨するものではなく、また、全てのプラットフォームで動作しない可能性があります。）
<literal>%p</literal>および<literal>%f</literal>パラメータが置き換えられたあと、実行された実コマンドは以下のようになります。␞␞<programlisting>␞
␝</programlisting>␟    A similar command will be generated for each new file to be archived.␟類似したコマンドがアーカイブされるそれぞれの新規ファイルに生成されます。␞␞   </para>␞
␝   <para>␟    The archive command will be executed under the ownership of the same
    user that the <productname>PostgreSQL</productname> server is running as.  Since
    the series of WAL files being archived contains effectively everything
    in your database, you will want to be sure that the archived data is
    protected from prying eyes; for example, archive into a directory that
    does not have group or world read access.␟このアーカイブ用コマンドは<productname>PostgreSQL</productname>サーバを稼働させるユーザと同じ所有権で実行されます。
アーカイブされる一連のWALファイルには、実質、データベース内の全てが含まれていますので、アーカイブしたデータをのぞき見から確実に保護しなければならないでしょう。
例えば、グループや全員に読み込み権限を付与していないディレクトリにデータをアーカイブしてください。␞␞   </para>␞
␝   <para>␟    It is important that the archive command return zero exit status if and
    ...
    it will try again periodically until it succeeds.␟アーカイブ用コマンドが成功した場合のみにゼロという終了ステータスを返すことが重要です。
<productname>PostgreSQL</productname>は、ゼロという結果に基づいて、そのファイルのアーカイブが成功したことを想定し、そのファイルを削除したり回収するかもしれません。
しかし、非ゼロのステータスは、<productname>PostgreSQL</productname>に対してファイルがアーカイブされなかったことを通知し、成功するまで定期的に再試行させます。␞␞   </para>␞
␝   <para>␟    Another way to archive is to use a custom archive module as the
    ...
    <xref linkend="archive-modules"/>.␟アーカイブするための別の方法は、<varname>archive_library</varname>としてカスタムアーカイブモジュールを使用することです。
...
アーカイブモジュールの詳細は<xref linkend="archive-modules"/>を参照してください。␞␞   </para>␞
␝   <para>␟    When the archive command is terminated by a signal (other than
    ...
    not reported in <xref linkend="pg-stat-archiver-view"/>.␟アーカイブコマンドがシグナル（サーバのシャットダウンの一部として使用される<systemitem>SIGTERM</systemitem>以外）やシェルによる125より大きい終了ステータスを持つエラー（command not foundなど）、あるいはアーカイブ関数が<literal>ERROR</literal>または<literal>FATAL</literal>を出力したことによって終了すると、アーカイバプロセスは中止され、postmasterによって再起動されます。
このような場合、失敗は<xref linkend="pg-stat-archiver-view"/>では報告されません。␞␞   </para>␞
␝   <para>␟    Archive commands and libraries should generally be designed to refuse to overwrite
    any pre-existing archive file.  This is an important safety feature to
    preserve the integrity of your archive in case of administrator error
    (such as sending the output of two different servers to the same archive
    directory).  It is advisable to test your proposed archive library to ensure
    that it does not overwrite an existing file.␟通常アーカイブ用コマンドあるいはライブラリは、既存のアーカイブ済みファイルの上書きを行わないように設計されなければなりません。
これは、管理者のミス（例えば2つの異なるサーバの出力を同一のアーカイブ用ディレクトリに送信してしまうなど）といった場合からアーカイブ状況の整合性を保護するための安全策として重要です。
確実に既存のファイルを上書きしないように、使用予定のアーカイブライブラリを試験することをお勧めします。␞␞   </para>␞
␝   <para>␟    In rare cases, <productname>PostgreSQL</productname> may attempt to
    ...
    <literal>false</literal>, respectively.␟まれに、<productname>PostgreSQL</productname>は以前にアーカイブされたWALファイルを再アーカイブしようとすることがあります。
...
既に存在するファイルがアーカイブされるWALファイルと異なる内容を含む場合、アーカイブコマンドやライブラリは、それぞれ0のステータス、<literal>false</literal>を返さ<emphasis>なければなりません</emphasis>。␞␞   </para>␞
␝   <para>␟    The example command above for Unix avoids overwriting a pre-existing archive
    ...
    already exists, which is <emphasis>not</emphasis> the desired behavior.)␟上のUnix用のコマンド例では、別途<command>test</command>という段階を含めることで、既に存在するアーカイブへの上書きを防いでいます。
...
（具体的にはGNUの<command>cp</command>コマンドは<option>-i</option> オプションを使い、ターゲットファイルがすでに存在している場合、ゼロのステータスを返します。これは<emphasis>期待していない</emphasis>動作です。）␞␞   </para>␞
␝   <para>␟    While designing your archiving setup, consider what will happen if
    ...
    you free some space.)␟アーカイブ設定を設計する時には、操作者の介入が必要であったり、アーカイブ場所の容量不足の理由でアーカイブ用コマンドあるいはライブラリが繰り返し失敗した時にどうなるかを考慮してください。
...
（<filename>pg_wal/</filename>を含むファイルシステムがいっぱいになると、<productname>PostgreSQL</productname>はパニック停止します。コミットされたトランザクションは失われませんが、データベースはいくらかの容量を解放するまでオフラインのままです。）␞␞   </para>␞
␝   <para>␟    The speed of the archive command or library is unimportant as long as it can keep up
    ...
    it is working as you intend.␟サーバのWALデータの生成に要する平均速度に追いついている限り、アーカイブ用コマンドあるいはライブラリの処理速度は重要ではありません。
...
アーカイブ処理が確実に意図通りに動作しているかを監視することを推奨します。␞␞   </para>␞
␝   <para>␟    In writing your archive command or library, you should assume that the file names to
    be archived can be up to 64 characters long and can contain any
    combination of ASCII letters, digits, and dots.  It is not necessary to
    preserve the original relative path (<literal>%p</literal>) but it is necessary to
    preserve the file name (<literal>%f</literal>).␟アーカイブ用コマンドあるいはライブラリを作成する時、アーカイブされるファイル名は最長64文字までで、ASCII文字と数字とドットのどんな組合せを使用しても構いません。
元の相対パス（<literal>%p</literal>）を保存する必要はありませんが、ファイル名（<literal>%f</literal>）を保存する必要はあります。␞␞   </para>␞
␝   <para>␟    Note that although WAL archiving will allow you to restore any
    ...
    configuration files.␟WALアーカイブによって<productname>PostgreSQL</productname>データベースでなされた変更は全てリストアすることができますが、設定ファイルはSQL操作ではなく手作業で変更されますので、設定ファイル（<filename>postgresql.conf</filename>、<filename>pg_hba.conf</filename>、および<filename>pg_ident.conf</filename>）になされた変更までリストアしないことに注意してください。
通常のファイルシステムバックアップ手続きでバックアップされる場所に設定ファイルを保持したい場合があります。
設定ファイルの設置場所を変更するには<xref linkend="runtime-config-file-locations"/>を参照してください。␞␞   </para>␞
␝   <para>␟    The archive command or function is only invoked on completed WAL segments.  Hence,
    ...
    usually reasonable.␟アーカイブコマンドあるいは関数は完全なWALセグメントに対してのみ呼び出されます。
...
格納領域を膨張させてしまいます。
通常ならば分単位の<varname>archive_timeout</varname>設定が合理的です。␞␞   </para>␞
␝   <para>␟    Also, you can force a segment switch manually with
    ...
    linkend="functions-admin-backup-table"/>.␟終わったばかりのトランザクションをできるだけ早くアーカイブさせたい場合、<function>pg_switch_wal</function>を使用して手作業でセグメント切り替えを強制することができます。
この他のWAL管理に関連した関数を<xref linkend="functions-admin-backup-table"/>に列挙します。␞␞   </para>␞
␝   <para>␟    When <varname>wal_level</varname> is <literal>minimal</literal> some SQL commands
    ...
    working <varname>archive_command</varname> is re-established.␟<varname>wal_level</varname>が<literal>minimal</literal>の場合、<xref linkend="populate-pitr"/>に書かれているように、いくつかのSQLコマンドはWALロギングを回避するため最適化されます。
...
このようにすると、動作する<varname>archive_command</varname>が再構築されるまでWALファイルは<filename>pg_wal/</filename>に蓄積します。␞␞   </para>␞
␝  <sect2 id="backup-base-backup">␟   <title>Making a Base Backup</title>␟   <title>ベースバックアップの作成</title>␞␞␞
␝   <para>␟    The easiest way to perform a base backup is to use the
    ...
    (see <xref linkend="backup-lowlevel-base-backup"/>).␟ベースバックアップを取得する最も簡単な方法は<xref linkend="app-pgbasebackup"/> を実行する方法です。
通常のファイルやTAR形式のファイルとしてベースバックアップを取得することができます。
もし、<xref linkend="app-pgbasebackup"/>より柔軟性が求められる場合は、低レベルなAPIを使ってバックアップを作成することもできます（詳細は <xref linkend="backup-lowlevel-base-backup"/>を参照）。␞␞   </para>␞
␝   <para>␟    It is not necessary to be concerned about the amount of time it takes
    ...
    effectively forced on during backup mode.␟ベースバックアップを取得するための時間を考慮する必要はありません。
しかし、普段、<varname>full_page_writes</varname>を無効にして運用している場合、バックアップ取得中は強制的に<varname>full_page_writes</varname>が有効になるため、パフォーマンスが落ちていると感じる可能性があります。␞␞   </para>␞
␝   <para>␟    To make use of the backup, you will need to keep all the WAL
    ...
    you can recover your data.␟バックアップを使用するためには、ファイルシステムのバックアップ取得中、および、その後に生成されるWALセグメントファイル全てが保存されている必要があります。
...
しかし、データを確実に復旧させるためには数世代のバックアップセットを保持することを考慮すべきです。␞␞   </para>␞
␝   <para>␟    The backup history file is just a small text file. It contains the
    ...
    restore.␟バックアップ履歴ファイルは、ほんの小さなテキストファイルです。
これには<xref linkend="app-pgbasebackup"/>で与えたラベル文字列の他、バックアップの開始、終了時間およびバックアップのWALセグメントが含まれます。
このラベルをバックアップを構成するために使うことで、アーカイブ履歴ファイルはどのバックアップをリストアするべきか間違いなく判断することができます。␞␞   </para>␞
␝   <para>␟    Since you have to keep around all the archived WAL files back to your
    ...
    been a long time since the last base backup.␟最後のベースバックアップ以降のアーカイブされたWALファイルを保持し続ける必要があるため、通常、ベースバックアップを取得すべき期間は、アーカイブされたWALファイルを保持するためにどのくらいのストレージを拡張できるかによって決定されます。
また、復旧が必要になった場合に、どのくらいの時間を復旧に使うと覚悟するのかも考慮すべきです。&mdash;
システムは全てのWALセグメントを適用する必要があるため、もし、最後のベースバックアップを取得してから長い時間が経過している場合、適用に時間を要する可能性があります。␞␞   </para>␞
␝  <sect2 id="backup-incremental-backup">␟   <title>Making an Incremental Backup</title>␟   <title>増分バックアップを作成する</title>␞␞␞
␝   <para>␟    You can use <xref linkend="app-pgbasebackup"/> to take an incremental
    ...
    metadata to reconstruct the current version of the file.␟<xref linkend="app-pgbasebackup"/>を使用して増分バックアップを作成するには、<literal>--incremental</literal>オプションを指定します。
<literal>--incremental</literal>の引数として、同じサーバの以前のバックアップへのバックアップマニフェストを指定する必要があります。
作成されたバックアップには、リレーション以外のファイルはそのまま含まれますが、一部のリレーションファイルは、以前のバックアップ以降に変更されたブロックのみと、ファイルの現在のバージョンを再構築するのに十分なメタデータを含む、より小さな増分ファイルに置き換えられる場合があります。␞␞   </para>␞
␝   <para>␟    To figure out which blocks need to be backed up, the server uses WAL
    ...
    fail.␟どのブロックをバックアップする必要があるかを判断するために、サーバは、データディレクトリ内のディレクトリ<literal>pg_wal/summaries</literal>に格納されているWAL要約を使用します。
...
しかし、必要なファイルがすでに削除されている場合や、WAL要約が十分な速さで追いつかないときは、増分バックアップは失敗します。␞␞   </para>␞
␝   <para>␟    When restoring an incremental backup, it will be necessary to have not
    ...
    limitations</link>.␟増分バックアップをリストアする場合は、増分バックアップ自体だけでなく、増分バックアップから除外されたブロックを提供するために必要な以前のすべてのバックアップも必要になります。
この要件の詳細については、<xref linkend="app-pgcombinebackup"/>を参照してください。
クラスタのチェックサム状態が変更された場合、<literal>pg_combinebackup</literal>の使用に制限があることに注意してください。<link linkend="app-pgcombinebackup-limitations">pg_combinebackupの制限</link>を参照してください。␞␞   </para>␞
␝   <para>␟    Note that all of the requirements for making use of a full backup also
    ...
    restoring later incremental backups.␟完全バックアップを使用するためのすべての要件は、増分バックアップにも適用されることに注意してください。
...
完全バックアップと増分バックアップの関係は自分で追跡する必要があり、後で増分バックアップをリストアするときに必要になる前のバックアップを削除しないようにしてください。␞␞   </para>␞
␝   <para>␟    Incremental backups typically only make sense for relatively large
    ...
    incremental backups won't be much smaller than full backups.␟通常、増分バックアップは、データのかなりの部分が変更されないか、または徐々にしか変更されない比較的大きなデータベースに対してのみ意味があります。
小規模なデータベースの場合は、増分バックアップの存在を無視して、管理が簡単な完全バックアップを実行する方が簡単です。
頻繁に変更される大きなデータベースの場合、増分バックアップは完全バックアップよりも大幅に小さくはなりません。␞␞   </para>␞
␝   <para>␟    An incremental backup is only possible if replay would begin from a later
    ...
    have been created.␟増分バックアップは、増分バックアップが依存しているバックアップよりも後のチェックポイントから再生を開始する場合にのみ可能です。
プライマリで増分バックアップを実行する場合、バックアップごとに新しいチェックポイントが起こるため、この条件は常に満たされます。
スタンバイでは、再生は最新のリスタートポイントから開始します。
そのため、前回のバックアップ以降のアクティビティが非常に少ない場合は、新しいリスタートポイントが作成されていない可能性があるため、増分バックアップは失敗するかもしれません。␞␞   </para>␞
␝  <sect2 id="backup-lowlevel-base-backup">␟   <title>Making a Base Backup Using the Low Level API</title>␟   <title>低レベルAPIを使用したベースバックアップの作成</title>␞␞   <para>␞
␝   <para>␟    Instead of taking a full or incremental base backup using
    ...
    proceeding to the next step.␟<xref linkend="app-pgbasebackup"/>を使って完全、あるいは増分バックアップを取る代わりに、低レベルのAPIを使ってベースバックアップを取得できます。
この手順は<application>pg_basebackup</application>を使う方法よりも少し余分の手順が必要ですが、比較的単純です。
これらのステップを順番に実行すること、また次のステップに進む前にこれらのステップが成功していることを確認することが非常に重要です。␞␞   </para>␞
␝    <para>␟     Multiple backups are able to be run concurrently (both those
     started using this backup API and those started using
     <xref linkend="app-pgbasebackup"/>).␟複数のバックアップを同時に実行できます（このバックアップAPIを使用して開始されたバックアップと<xref linkend="app-pgbasebackup"/>を使用して開始されたバックアップの両方）。␞␞    </para>␞
␝    <para>␟     Ensure that WAL archiving is enabled and working.␟WALアーカイブが有効であり、正常に動作することを確認してください。␞␞    </para>␞
␝    <para>␟     Connect to the server (it does not matter which database) as a user with
     rights to run <function>pg_backup_start</function> (superuser,
     or a user who has been granted <literal>EXECUTE</literal> on the
     function) and issue the command:␟<function>pg_backup_start</function>の実行権限を持つユーザとしてサーバ（どのデータベースであってもかまいません）に接続します。ユーザはスーパーユーザか、またはこの関数に<literal>EXECUTE</literal>権限を与えられたユーザです。以下のコマンドを発行します。␞␞<programlisting>␞
␝</programlisting>␟     where <literal>label</literal> is any string you want to use to uniquely
     identify this backup operation. The connection
     calling <function>pg_backup_start</function> must be maintained until the end of
     the backup, or the backup will be automatically aborted.␟ここで<literal>label</literal>は、このバックアップ操作を一意に識別するために使用したい文字列です。
<function>pg_backup_start</function>を呼び出す接続は、バックアップが終了するまで維持されなければなりません。
さもないと、バックアップは自動的に打ち切られます。␞␞    </para>␞
␝    <para>␟     Online backups are always started at the beginning of a checkpoint.
     ...
     as much I/O as possible.␟オンラインバックアップは、常にチェックポイントの先頭から開始されます。
...
このチェックポイントは、できるだけ多くのI/Oを使用してできるだけ早く完了します。␞␞    </para>␞
␝    <para>␟     Perform the backup, using any convenient file-system-backup tool
     ...
     consider during this backup.␟（<application>pg_dump</application>や<application>pg_dumpall</application>ではなく）<application>tar</application>や<application>cpio</application>などの使い慣れた任意のファイルシステムバックアップツールを使用して、バックアップを実行してください。
この作業時に、データベースの通常の操作を停止することは不要ですし、望ましい方法でもありません。
このバックアップの実行中に考慮すべき点は<xref linkend="backup-lowlevel-base-backup-data"/>を参照してください。␞␞    </para>␞
␝    <para>␟     In the same connection as before, issue the command:␟以前と同じ接続の中で、以下のコマンドを実行します。␞␞<programlisting>␞
␝</programlisting>␟     This terminates backup mode. On a primary, it also performs an automatic
     ...
     ready to archive.␟これはバックアップモードを終了し、プライマリでは、次のWALセグメントへの自動切換えを行います。
スタンバイでは、WALセグメントを自動的に切り替えることはできません。
ですから、手動切り替えを行うためにプライマリで<function>pg_switch_wal</function>を実行することをお勧めします。
この切換えの理由は、バックアップ期間中に書き出された最後のWALファイルがアーカイブできるよう準備することです。␞␞    </para>␞
␝    <para>␟     <function>pg_backup_stop</function> will return one row with three
     ...
     modification, which may require opening the file in binary mode.␟<function>pg_backup_stop</function>は3つの値を含んだ1行を返します。
...
これらのファイルはバックアップが動作するために極めて重要であり、1バイトも変更なしに書き込まれる必要があるため、バイナリモードで開かれる必要があるかもしれません。␞␞    </para>␞
␝    <para>␟     Once the WAL segment files active during the backup are archived, you are
     ...
     may not be valid.␟バックアップ中に使用されたWALセグメントファイルがアーカイブされれば完了です。
<function>pg_backup_stop</function>の戻り値の1番目の値で識別されるファイルは、バックアップファイル一式を完結させるのに必要となる最終セグメントです。
...
<function>pg_backup_stop</function>実行においての時間期限を設けたい場合、適切な<varname>statement_timeout</varname>の値を設定できますが、この設定値によって<function>pg_backup_stop</function>が中断したときにバックアップが正当ではない可能性があるということを肝に銘じてください。␞␞    </para>␞
␝    <para>␟     If the backup process monitors and ensures that all WAL segment files
     ...
     therefore be incomplete and not able to be restored.␟バックアップに必要なすべてのWALセグメントファイルのアーカイブが成功したことを、バックアップ作業の中で監視して確認するのであれば、<literal>wait_for_archive</literal>パラメータ（デフォルトでtrueです）をfalseに設定し、バックアップレコードがWALに書き込まれたら即座に<function>pg_backup_stop</function>が戻るようにすることができます。
...
そうなると、リストアできません。␞␞    </para>␞
␝   <sect3 id="backup-lowlevel-base-backup-data">␟   <title>Backing Up the Data Directory</title>␟   <title>データディレクトリのバックアップ</title>␞␞   <para>␞
␝   <para>␟    Some file system backup tools emit warnings or errors
    ...
    --warning=no-file-removed</literal> to hide the related warning messages.␟ファイルシステムのバックアップツール中には複写している途中でファイルが変更されると警告もしくはエラーを報告するものがあります。
...
GNUの <application>tar</application>で1.23以降のバージョンを使用しているのであれば、<literal>--warning=no-file-changed --warning=no-file-removed</literal>オプションをつけることで関連する警告メッセージを隠すオプションを使用することができます。␞␞   </para>␞
␝   <para>␟    Be certain that your backup includes all of the files under
    ...
    your tablespaces).␟バックアップに、データベースクラスタディレクトリ（例えば<filename>/usr/local/pgsql/data</filename>）以下にある全てのファイルが含まれていることを確認してください。
このディレクトリ以下に存在しないテーブル空間を使用している場合、注意して、同様にそれらを含めてください
（そして、バックアップがリンクとしてシンボリックリンクをアーカイブしていることを確認してください。
さもないとリストアはテーブル空間を壊してしまいます）。␞␞   </para>␞
␝   <para>␟    You should, however, omit from the backup the files within the
    ...
    (These files can confuse <application>pg_ctl</application>.)␟しかし、クラスタの<filename>pg_wal/</filename>サブディレクトリにあるファイルをバックアップから省いてください。
...
（これらのファイルは<application>pg_ctl</application>を誤作動させる可能性があります。）␞␞   </para>␞
␝   <para>␟    It is often a good idea to also omit from the backup the files
    ...
    the new primary comes on line.␟プライマリ上に存在するレプリケーションスロットがバックアップに含まれないようにするために、クラスタの中の<filename>pg_replslot/</filename>ディレクトリをバックアップから除くのもしばしば良い考えです。
...
このようにバックアップにレプリケーションスロットを含むことは、新しいプライマリがオンラインになる頃には、スロットの内容がかなり古くなっている可能性があります。␞␞   </para>␞
␝   <para>␟    The contents of the directories <filename>pg_dynshmem/</filename>,
    ...
    omitted from the backup as they will be initialized on postmaster startup.␟ディレクトリ<filename>pg_dynshmem/</filename>、<filename>pg_notify/</filename>、<filename>pg_serial/</filename>、<filename>pg_snapshots/</filename>、<filename>pg_stat_tmp/</filename>、<filename>pg_subtrans/</filename>の中身はバックアップから除外できます。（ただし、ディレクトリ自体は除外できません。）
というのも、postmaster起動時に初期化されるからです。␞␞   </para>␞
␝   <para>␟    Any file or directory beginning with <filename>pgsql_tmp</filename> can be
    omitted from the backup.  These files are removed on postmaster start and
    the directories will be recreated as needed.␟<filename>pgsql_tmp</filename>で始まるすべてのファイルとディレクトリはバックアップから除外できます。
これらのファイルはpostmasterの起動時に削除されますし、ディレクトリも必要なら再作成されます。␞␞   </para>␞
␝   <para>␟    <filename>pg_internal.init</filename> files can be omitted from the
    backup whenever a file of that name is found.  These files contain
    relation cache data that is always rebuilt when recovering.␟<filename>pg_internal.init</filename>という名前のファイルが見つかった場合、それはバックアップから省くことができます。
このファイルはリレーションキャッシュデータを含んでおり、常にリカバリの際に再構築されます。␞␞   </para>␞
␝   <para>␟    The backup label
    ...
    process.␟バックアップラベルファイルには、<function>pg_backup_start</function>に付与したラベル文字列と<function>pg_backup_start</function>が実行された時刻、最初のWALファイルの名前が含まれます。
...
その存在と内容はシステムのリカバリプロセスが適切に動作するために非常に重要です。␞␞   </para>␞
␝   <para>␟    It is also possible to make a backup while the server is
    ...
    It is generally better to follow the continuous archiving procedure above.␟サーバが停止している時にバックアップを作成することも可能です。
...
通常は、上述の継続的アーカイブ手順に従う方をお勧めします。␞␞   </para>␞
␝  <sect2 id="backup-pitr-recovery">␟   <title>Recovering Using a Continuous Archive Backup</title>␟   <title>継続的アーカイブによるバックアップを使用した復旧</title>␞␞␞
␝   <para>␟    Okay, the worst has happened and you need to recover from your backup.
    Here is the procedure:␟さて、最悪の事態が発生し、バックアップから復旧する必要が出てきたものとします。
以下にその手順を説明します。␞␞  <orderedlist>␞
␝    <para>␟     Stop the server, if it's running.␟もし稼働しているのであればサーバを停止してください。␞␞    </para>␞
␝    <para>␟     If you have the space to do so,
     ...
     were not archived before the system went down.␟もし容量があるのであれば、後で必要になる場合に備えてクラスタデータディレクトリ全体とテーブル空間を全て一時的な場所にコピーしてください。
この予防措置は、既存のデータベースを2つ分保持できるだけの空き領域を必要とします。
十分な領域がない場合でも、少なくともクラスタの<filename>pg_wal</filename>サブディレクトリの内容は保存すべきです。
ここには、システムが停止する前にアーカイブされなかったWALファイルが含まれているかも知れないからです。␞␞    </para>␞
␝    <para>␟     Remove all existing files and subdirectories under the cluster data
     directory and under the root directories of any tablespaces you are using.␟クラスタデータディレクトリ以下、および、使用中のテーブル空間の最上位ディレクトリ以下にある既存の全てのファイルとサブディレクトリを削除してください。␞␞    </para>␞
␝    <para>␟     If you're restoring a full backup, you can restore the database files
     ...
     were correctly restored.␟完全バックアップをリストアする場合は、データベースファイルを直接ターゲットディレクトリにリストアすることができます。
ファイルが正しい所有権（<literal>root</literal>ではなくデータベースシステムユーザです！）でリストアされていることを確認してください。
テーブル空間を使用している場合は、<filename>pg_tblspc/</filename>内のシンボリックリンクが正しくリストアされていることを検証する必要があります。␞␞    </para>␞
␝    <para>␟     If you're restoring an incremental backup, you'll need to restore the
     ...
     verify that permissions and tablespace links are correct.␟増分バックアップをリストアする場合は、増分バックアップと、それが直接または間接的に依存している以前のすべてのバックアップを、リストアを実行しているマシンにリストアする必要があります。
...
上記のように、権限とテーブル空間のリンクが正しいことを確認します。␞␞    </para>␞
␝    <para>␟     Remove any files present in <filename>pg_wal/</filename>; these came from the
     ...
     if you had it set up that way before.␟<filename>pg_wal/</filename>内にあるファイルをすべて削除してください。
これらはファイルシステムバックアップから生成されたものであり、おそらく現在のものより古く使用できないものです。
<filename>pg_wal/</filename>をまったくアーカイブしていなければ、適切な権限で再作成してください。
以前シンボリックリンクとして設定していたのであれば、そのように確実に再構築するように注意してください。␞␞    </para>␞
␝    <para>␟     If you have unarchived WAL segment files that you saved in step 2,
     copy them into <filename>pg_wal/</filename>.  (It is best to copy them,
     not move them, so you still have the unmodified files if a
     problem occurs and you have to start over.)␟手順2で退避させた未アーカイブのWALセグメントファイルがあるのであれば、<filename>pg_wal/</filename>にコピーしてください。
（問題が発生し、初めからやり直さなければならない場合に未変更のファイルが残るように、移動させるのではなくコピーすることが最善です。）␞␞    </para>␞
␝    <para>␟     Set recovery configuration settings in
     ...
     ordinary users from connecting until you are sure the recovery was successful.␟<filename>postgresql.conf</filename>（<xref linkend="runtime-config-wal-archive-recovery"/>を参照してください）に復旧の設定を記述し、クラスタデータディレクトリに<filename>recovery.signal</filename>ファイルを作成します。
また、一時的に<filename>pg_hba.conf</filename>を変更し、復旧の成功を確認できるまで一般ユーザが接続できないようにする必要があるかもしれません。␞␞    </para>␞
␝    <para>␟     Start the server.  The server will go into recovery mode and
     ...
     commence normal database operations.␟サーバを起動してください。
...
その後通常のデータベース操作を開始します。␞␞    </para>␞
␝    <para>␟     Inspect the contents of the database to ensure you have recovered to
     the desired state.  If not, return to step 1.  If all is well,
     allow your users to connect by restoring <filename>pg_hba.conf</filename> to normal.␟データベースの内容を検査し、希望する状態まで復旧できていることを確認してください。
復旧できなかった場合は手順1に戻ってください。
全て問題なければ、ユーザが接続できるように<filename>pg_hba.conf</filename>を正常状態に戻してください。␞␞    </para>␞
␝   <para>␟    The key part of all this is to set up a recovery configuration that
    ...
    something like:␟ここで重要となるのは、どのように復旧させたいのかやどこまで復旧させたいかを記述する復旧設定を設定することです。
...
最も簡単でよく使われるコマンドは以下のようなものです。␞␞<programlisting>␞
␝</programlisting>␟    which will copy previously archived WAL segments from the directory
    <filename>/mnt/server/archivedir</filename>.  Of course, you can use something
    much more complicated, perhaps even a shell script that requests the
    operator to mount an appropriate tape.␟これは事前にアーカイブされたWALセグメントを<filename>/mnt/server/archivedir</filename>ディレクトリからコピーします。
当然ながら、もっと複雑なものを使用することができます。
例えば、操作者に適切なテープをマウントさせることを要求するようなシェルスクリプトでさえ可能です。␞␞   </para>␞
␝   <para>␟    It is important that the command return nonzero exit status on failure.
    ...
    up.␟このコマンドが失敗した時に非ゼロの終了ステータスを返すことが重要です。
このコマンドは、アーカイブに存在しないファイルを要求する<emphasis>かもしれません</emphasis>が、その場合でも非ゼロを返さなければなりません。
これはエラー状態ではありません。
例外は、コマンドがシグナルによって中断された場合（データベースの停止に使用される<systemitem>SIGTERM</systemitem>以外）か、シェルによるエラー（コマンドが見つかりませんなど）で復旧が中断され、サーバが起動しない場合です。␞␞   </para>␞
␝   <para>␟    Not all of the requested files will be WAL segment
    files; you should also expect requests for files with a suffix of
    <literal>.history</literal>. Also be aware that
    the base name of the <literal>%p</literal> path will be different from
    <literal>%f</literal>; do not expect them to be interchangeable.␟要求されるファイルはWALセグメントファイルだけではありません。
...
これらが相互に置き換え可能であるとは考えないでください。␞␞   </para>␞
␝   <para>␟    WAL segments that cannot be found in the archive will be sought in
    <filename>pg_wal/</filename>; this allows use of recent un-archived segments.
    However, segments that are available from the archive will be used in
    preference to files in <filename>pg_wal/</filename>.␟アーカイブ場所で見つけられなかったWALセグメントは<filename>pg_wal/</filename>から検索されます。
これにより、最近の未アーカイブのセグメントを使用することができます。
しかし、アーカイブ場所から利用できるセグメントは<filename>pg_wal/</filename>内のファイルよりも優先的に使用されます。␞␞   </para>␞
␝   <para>␟    Normally, recovery will proceed through all available WAL segments,
    ...
    <xref linkend="backup-timelines"/> for discussion.␟通常は利用可能な全てのWALセグメントを使用して復旧処理が行われます。
...
␝   <para>␟    If you want to recover to some previous point in time (say, right before
    ...
    accuracy which transaction ID to use.␟もし以前のある時点まで復旧させたい場合（例えば、経験不足のデータベース管理者が主トランザクションテーブルを消去した直前）、要求する<link linkend="runtime-config-wal-recovery-target">停止時点</link>を指定するだけです。
停止時点は、<quote>recovery target</quote>として既知の停止時点で指定することも、日付と時刻で指定することも、リストアポイントか完了した特定のトランザクションIDで指定することもできます。
本ドキュメントの執筆時点では使用するトランザクションIDの識別を補助するツールがありませんので、ほとんどの場合は日付と時刻による指定のみを使用することになるでしょう。␞␞   </para>␞
␝     <para>␟      The stop point must be after the ending time of the base backup, i.e.,
      ...
      and roll forward from there.)␟停止時点はバックアップの終了時刻、つまり、<function>pg_backup_stop</function>の最終時刻より後の時点でなければなりません。
バックアップを行っている最中のある時点までベースバックアップを使用して復旧させることはできません。
（こうした時点まで復旧させるには、その前のベースバックアップまで戻って、そこからロールフォワードしてください。）␞␞     </para>␞
␝   <para>␟    If recovery finds corrupted WAL data, recovery will
    ...
    WAL data need not be scanned again.␟復旧時にWALデータの破損がわかると、復旧はその時点で止まり、サーバは起動しません。
...
サーバは定期的にすべての状態をディスクに強制し、再度スキャンする必要がない処理済みのWALデータを示す<filename>pg_control</filename>ファイルを更新します。␞␞   </para>␞
␝  <indexterm zone="backup">
...
␝  <sect2 id="backup-timelines">␟   <title>Timelines</title>␟   <title>タイムライン</title>␞␞␞
␝   <para>␟    The ability to restore the database to a previous point in time creates
    ...
    those that were generated in the original database history.␟過去のある時点までデータベースを復旧できる機能は、タイムトラベルやパラレルユニバースといったSFの物語に類似した、多少の複雑性があります。
...
ですので、このことを避けるために、ポイントインタイムで復旧させた後に生成された一連のWALレコードと元のデータベースの履歴において生成されたWALレコードとを区別する必要があります。␞␞   </para>␞
␝   <para>␟    To deal with this problem, <productname>PostgreSQL</productname> has a notion
    ...
    usually printed in decimal.)␟こうした問題を扱うために<productname>PostgreSQL</productname>には<firstterm>タイムライン</firstterm>という概念があります。
...
（たとえばサーバログメッセージのような他のコンテキストの場合、タイムラインIDは通常10進数で表示されることに注意してください。）␞␞   </para>␞
␝   <para>␟    It is
...
タイムラインを使用して、以前捨てたタイムライン分岐における状態を含む、過去の<emphasis>任意</emphasis>の状態に復旧させることができます。␞␞   </para>␞
␝   <para>␟    Every time a new timeline is created, <productname>PostgreSQL</productname> creates
    ...
    a result of experimentation.␟新しいタイムラインが生成される度に、<productname>PostgreSQL</productname>は、どのタイムラインがいつどこから分岐したかを示す<quote>タイムライン履歴</quote>ファイルを作成します。
...
特にこうしたコメントは、実験の結果いくつものタイムラインのもつれがある場合に有用です。␞␞   </para>␞
␝   <para>␟    The default behavior of recovery is to recover to the latest timeline found
    ...
    cannot recover into timelines that branched off earlier than the base backup.␟復旧処理のデフォルトは、アーカイブで見つかった最新のタイムラインへの復旧です。
...
ベースバックアップより前に分岐したタイムラインに沿って復旧することはできません。␞␞   </para>␞
␝  <sect2 id="backup-tips">␟   <title>Tips and Examples</title>␟   <title>ヒントと例</title>␞␞␞
␝   <para>␟    Some tips for configuring continuous archiving are given here.␟継続的アーカイブを構成するいくつかのヒントを以下にあげます。␞␞   </para>␞
␝    <sect3 id="backup-standalone">␟     <title>Standalone Hot Backups</title>␟     <title>スタンドアローンホットバックアップ</title>␞␞␞
␝     <para>␟      It is possible to use <productname>PostgreSQL</productname>'s backup facilities to
      ...
      might be negated.)␟スタンドアローンホットバックアップを形成するため<productname>PostgreSQL</productname>のバックアップ基盤を使用することができます。
これらのバックアップはポイントインタイムリカバリに使用することはできないのですが、<application>pg_dump</application>によるダンプよりバックアップとリストアが概してより速く行われます。
（同時に<application>pg_dump</application>のダンプより大きくなるので、場合によっては速度による利点が打ち消されるかもしれません。）␞␞     </para>␞
␝     <para>␟      As with base backups, the easiest way to produce a standalone
      ...
      required to restore the backup.␟ベースバックアップと同様に、スタンドアローンホットバックアップを作成する最も簡単な方法は <xref linkend="app-pgbasebackup"/>ツールを使用する方法です。
実行時に<literal>-X</literal>オプションをつけることでバックアップに必要な全ての先行書き込みログ（WAL）を自動的にバックアップに含めることができ、リストアするときには特に特別な作業を行う必要がありません。␞␞     </para>␞
␝    <sect3 id="compressed-archive-logs">␟     <title>Compressed Archive Logs</title>␟     <title>圧縮アーカイブログ</title>␞␞␞
␝     <para>␟      If archive storage size is a concern, you can use
      <application>gzip</application> to compress the archive files:␟もし、アーカイブのストレージ容量に懸念がある場合、アーカイブファイルを圧縮するために<application>gzip</application>を使用することもできます。␞␞<programlisting>␞
␝</programlisting>␟      You will then need to use <application>gunzip</application> during recovery:␟復旧時は <application>gunzip</application>を使う必要があります。␞␞<programlisting>␞
␝    <sect3 id="backup-scripts">␟     <title><varname>archive_command</varname> Scripts</title>␟     <title><varname>archive_command</varname>スクリプト</title>␞␞␞
␝     <para>␟      Many people choose to use scripts to define their
      <varname>archive_command</varname>, so that their
      <filename>postgresql.conf</filename> entry looks very simple:␟<filename>postgresql.conf</filename>の記入事項が以下のように簡素となるため、多くの人が<varname>archive_command</varname>の定義にスクリプトの使用を選択します。␞␞<programlisting>␞
␝</programlisting>␟      Using a separate script file is advisable any time you want to use
      more than a single command in the archiving process.
      This allows all complexity to be managed within the script, which
      can be written in a popular scripting language such as
      <application>bash</application> or <application>perl</application>.␟アーカイブ処理手順において単一ではなくそれ以上の数のコマンドを使用したい場合はいつでも、別のスクリプトファイルの使用が推奨されます。
そうするとスクリプト内で全ての複雑性が管理されます。
スクリプトは<application>bash</application>または<application>perl</application>のようなよくあるスクリプト言語で記載できます。␞␞     </para>␞
␝     <para>␟      Examples of requirements that might be solved within a script include:␟スクリプト内で解決される要件の例として以下があります。␞␞      <itemizedlist>␞
␝        <para>␟         Copying data to secure off-site data storage␟セキュアなオフサイトデータストレージへのデータのコピー␞␞        </para>␞
␝        <para>␟         Batching WAL files so that they are transferred every three hours,
         rather than one at a time␟一回に全てではなく３時間毎に転送されるようにWALファイルのバッチ␞␞        </para>␞
␝        <para>␟         Interfacing with other backup and recovery software␟その他のバックアップとリカバリのソフトウェアとのインタフェース␞␞        </para>␞
␝        <para>␟         Interfacing with monitoring software to report errors␟エラー報告を行う監視ソフトとのインタフェース␞␞        </para>␞
␝      <para>␟       When using an <varname>archive_command</varname> script, it's desirable
       ...
       be diagnosed easily if they fail.␟<varname>archive_command</varname>スクリプトを使うときは<xref linkend="guc-logging-collector"/>を使えるようにすることが望ましい方法です。
そのスクリプトが<systemitem>stderr</systemitem>に書き出したメッセージはすべて、データベースのサーバログとして書かれます。
このため複雑な設定でエラーが発生した時に、簡単に原因を突き止められます。␞␞      </para>␞
␝  <sect2 id="continuous-archiving-caveats">␟   <title>Caveats</title>␟   <title>警告</title>␞␞␞
␝   <para>␟    At this writing, there are several limitations of the continuous archiving
    technique.  These will probably be fixed in future releases:␟本ドキュメント作成時点では、継続的アーカイブ技術にいくつかの制限があります。
将来のリリースでは修正されるはずです。␞␞␞
␝    <para>␟     If a <link linkend="sql-createdatabase"><command>CREATE DATABASE</command></link>
     ...
     template databases while taking a base backup.␟もしもベースバックアップが行われている時、<link linkend="sql-createdatabase"><command>CREATE DATABASE</command></link>コマンドが実行され、ベースバックアップが処理を実行している期間に<command>CREATE DATABASE</command>がコピーしているtemplateデータベースが変更されると、復旧処理により、これらの変更が作成されたデータベースにも伝播される可能性があります。
もちろん、これは望まれる事ではありません。
この危険を回避するには、ベースバックアップ期間中にはすべてのtemplateデータベースを変更しないことが一番です。␞␞    </para>␞
␝    <para>␟     <link linkend="sql-createtablespace"><command>CREATE TABLESPACE</command></link>
     ...
     dropping tablespaces.␟<link linkend="sql-createtablespace"><command>CREATE TABLESPACE</command></link>コマンドはリテラルの絶対パス付でWALにログが記録され、したがって、同じ絶対パスでのテーブル空間作成の時に再生されます。
これは、もしWALが異なったマシン上で再生される場合には好ましくありません。
WAL再生がたとえ同一のマシンであっても、新規のデータディレクトリであれば危険です。
なぜなら、再生は元のテーブル空間の内容を上書きしてしまうからです。
この種の潜在的な振舞いを防ぐためには、テーブル空間を作成もしくは削除後に新規ベースバックアップを行うのが最良の手段です。␞␞    </para>␞
␝   <para>␟    It should also be noted that the default <acronym>WAL</acronym>
    ...
    interval parameters as much as feasible.␟また、デフォルトの<acronym>WAL</acronym>フォーマットは数多くのディスクページのスナップショットを含んでいるため、かなりかさばるものになってしまっていることに触れておくべきでしょう。
...
差し当たり管理者は、可能な限りチェックポイント間隔パラメータを大きくすることによって、WALに含まれるページスナップショットの数を削減することができます。␞␞   </para>␞
