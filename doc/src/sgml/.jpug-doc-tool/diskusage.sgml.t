␝<chapter id="diskusage">␟ <title>Monitoring Disk Usage</title>␟ <title>ディスク使用量の監視</title>␞␞␞
␝ <para>␟  This chapter discusses how to monitor the disk usage of a
  <productname>PostgreSQL</productname> database system.␟本章では<productname>PostgreSQL</productname>データベースシステムのディスク使用量を監視する方法について説明します。␞␞ </para>␞
...
␝ <sect1 id="disk-usage">␟  <title>Determining Disk Usage</title>␟  <title>ディスク使用量の決定</title>␞␞␞
␝  <para>␟   Each table has a primary heap disk file where most of the data is
   ...
   in <xref linkend="storage-file-layout"/>.␟各テーブルには、データの大部分が格納されるプライマリヒープディスクファイルが備わっています。
...
これらのファイルの命名規約について<xref linkend="storage-file-layout"/>で説明します。␞␞  </para>␞
␝  <para>␟   You can monitor disk space in three ways:
   ...
   system catalogs.␟ディスクスペースの監視は、次の3つの方法で行えます。
...
本節の残りの部分で、システムカタログを調査することによりこの方法を示します。␞␞  </para>␞
␝  <para>␟   Using <application>psql</application> on a recently vacuumed or analyzed database,
   you can issue queries to see the disk usage of any table:␟バキュームされて間もないデータベース、もしくは解析されたデータベース上で<application>psql</application>を使用することにより、どのようなテーブルでもディスクの使用量を調べる問い合わせを発行できます。␞␞<programlisting>␞
...
␝  <para>␟   To show the space used by <acronym>TOAST</acronym> tables, use a query
   like the following:␟<acronym>TOAST</acronym>テーブルで使用されている容量を示すには、以下のような問い合わせを使用してください。␞␞<programlisting>␞
␝  <para>␟   You can easily display index sizes, too:␟インデックスサイズについても、以下のように簡単に表示できます。␞␞<programlisting>␞
␝  <para>␟   It is easy to find your largest tables and indexes using this
   information:␟この情報を使用して、以下のように簡単に最大のテーブルとインデックスを見つけ出すことができます。␞␞<programlisting>␞
␝ <sect1 id="disk-full">␟  <title>Disk Full Failure</title>␟  <title>ディスク容量不足による問題</title>␞␞␞
...
もしWALファイルを持つディスクが一杯になった場合は、データベースサーバはパニックを起こし結果的にシャットダウンします。␞␞  </para>␞
␝  <para>␟   If you cannot free up additional space on the disk by deleting
   ...
   linkend="manage-ag-tablespaces"/> for more information about that.␟他のデータを削除しても、ディスクに空き容量を用意できない場合、
