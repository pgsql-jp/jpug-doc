...
␝  <refname>CREATE AGGREGATE</refname>␟  <refpurpose>define a new aggregate function</refpurpose>␟  <refpurpose>新しい集約関数を定義する</refpurpose>␞␞ </refnamediv>␞
␝␟<phrase>or the old syntax</phrase>␟<phrase>または以下の旧構文</phrase>␞␞␞
␝ <refsect1>␟  <title>Description</title>␟  <title>説明</title>␞␞␞
␝  <para>␟   <command>CREATE AGGREGATE</command> defines a new aggregate function.
   ...
   features.␟<command>CREATE AGGREGATE</command>は、新しい集約関数を定義します。
...
新しい型を定義する場合、またはまだ提供されていない集約関数が必要な場合、必要な機能を実現するために<command>CREATE AGGREGATE</command>を使うことができます。␞␞  </para>␞
␝  <para>␟   When replacing an existing definition, the argument types, result type,
   ...
   hypothetical-set aggregate) as the old one.␟既存の定義を置き換える場合には、引数の型、結果の型、直接引数の数を変えることはできません。
また、新しい定義は古いものと同じ種類(通常の集約、順序集合集約、仮想集合集約)でなければなりません。␞␞  </para>␞
␝  <para>␟   If a schema name is given (for example, <literal>CREATE AGGREGATE
   myschema.myagg ...</literal>) then the aggregate function is created in the
   specified schema.  Otherwise it is created in the current schema.␟スキーマ名が付けられている場合（例えば、<literal>CREATE AGGREGATE myschema.myagg ...</literal>）、集約関数は指定されたスキーマに作成されます。
スキーマ名がなければ、集約関数は現在のスキーマに作成されます。␞␞  </para>␞
␝  <para>␟   An aggregate function is identified by its name and input data type(s).
   ...
   (see <xref linkend="sql-createfunction"/>).␟集約関数は名前と入力データ型(複数可)の組み合わせによって識別されます。
演算の対象となる入力データ型が異なっていれば、同じスキーマ内に同じ名前の集約関数があっても構いません。
1つのスキーマ内では、集約関数の名前と入力データ型の組み合わせは、通常の関数の名前と入力データ型の組み合わせと異なる必要があります。
この動作は通常の関数名についてオーバーロードする時と同じです（<xref linkend="sql-createfunction"/>を参照してください）。␞␞  </para>␞
␝  <para>␟   A simple aggregate function is made from one or two ordinary
   functions:
   a state transition function
   <replaceable class="parameter">sfunc</replaceable>,
   and an optional final calculation function
   <replaceable class="parameter">ffunc</replaceable>.
   These are used as follows:␟単純な集約関数は1つか2つの通常の関数から作られます。
状態遷移関数<replaceable class="parameter">sfunc</replaceable>と最終計算関数<replaceable class="parameter">ffunc</replaceable>(省略可能)です。
これらは以下のように使われます。␞␞<programlisting>␞
␝<replaceable class="parameter">ffunc</replaceable>( internal-state ) ---> aggregate-value␟<replaceable class="parameter">sfunc</replaceable>( internal-state, next-data-values ) ---> next-internal-state
<replaceable class="parameter">ffunc</replaceable>( internal-state ) ---> aggregate-value␟<replaceable class="parameter">sfunc</replaceable>( 内部状態, 次のデータ値 ) ---> 次の内部状態
<replaceable class="parameter">ffunc</replaceable>( 内部状態 ) ---> 集約の結果␞␞</programlisting>␞
␝  <para>␟   <productname>PostgreSQL</productname> creates a temporary variable
   ...
   is returned as-is.␟<productname>PostgreSQL</productname>は、集約の現在の内部状態を保持する<replaceable class="parameter">stype</replaceable>データ型の一時変数を作成します。
それぞれの入力行に対して、集約引数の値が計算され、現在の状態値と新しい引数値で状態遷移関数が呼び出され、新しい内部状態変数が計算されます。
全ての行が処理されると、最終関数が1回呼び出され、集約の出力値が計算されます。
最終関数がない場合は、終了時の状態値がそのまま返されます。␞␞  </para>␞
␝  <para>␟   An aggregate function can provide an initial condition,
   that is, an initial value for the internal state value.
   This is specified and stored in the database as a value of type
   <type>text</type>, but it must be a valid external representation
   of a constant of the state value data type.  If it is not supplied
   then the state value starts out null.␟集約関数は、初期条件、つまり内部状態値の初期値を提供することができます。
これは<type>text</type>型の値としてデータベースに格納されますが、状態値データ型の定数として有効な外部表現でなければいけません。
初期状態が与えられていない場合、状態値はNULLから始まります。␞␞  </para>␞
␝  <para>␟   If the state transition function is declared <quote>strict</quote>,
   ...
   condition or use a nonstrict transition function.␟状態遷移関数が<quote>strict</quote>と宣言されている場合、NULLを入力値にして呼び出すことはできません。
...
これらの型が異なる時は、非NULL初期値を供給するか、strictでない遷移関数を使わなければいけません。␞␞  </para>␞
␝  <para>␟   If the state transition function is not strict, then it will be called
   unconditionally at each input row, and must deal with null inputs
   and null state values for itself.  This allows the aggregate
   author to have full control over the aggregate's handling of null values.␟状態遷移関数がstrictでない場合は、それぞれの入力行に対してその関数が無条件に呼び出されるので、NULL入力とNULL状態値を自分で処理しなければいけません。
これは、関数の作成者が、集約関数におけるNULL値の扱いを完全に制御できることを意味します。␞␞  </para>␞
␝  <para>␟   If the final function is declared <quote>strict</quote>, then it will not
   be called when the ending state value is null; instead a null result
   will be returned automatically.  (Of course this is just the normal
   behavior of strict functions.)  In any case the final function has
   the option of returning a null value.  For example, the final function for
   <function>avg</function> returns null when it sees there were zero
   input rows.␟最終関数が<quote>strict</quote>と宣言されていると、終了状態値がNULLの時は、最終関数が呼び出されません。
その場合、NULLという結果が自動的に出力されます。
（もちろんこれは、strictな関数の一般的な動作に過ぎません。）
いずれにせよ、最終関数はNULLを返すことができます。
例えば、<function>avg</function>の最終関数は、入力が0行だとわかるとNULLを返します。␞␞  </para>␞
␝  <para>␟   Sometimes it is useful to declare the final function as taking not just
   ...
   in the current call.␟最終関数を、状態値だけでなく、集約の入力値に対応する追加パラメータも取るように宣言すると便利なことがあります。
...
最終関数は、現在の呼び出しでの実際の引数の型を特定するために、例えば<function>get_fn_expr_argtype</function>を使うことができます。␞␞  </para>␞
␝  <para>␟   An aggregate can optionally support <firstterm>moving-aggregate mode</firstterm>,
   ...
   aggregate that includes an inverse transition function.␟集約は、<xref linkend="xaggr-moving-aggregates"/>に記述されているように<firstterm>移動集約モード</firstterm>をサポートすることができます。
...
<literal>MINVFUNC</literal>を除き、これらのパラメータは、<literal>M</literal>のない単純集約の対応するパラメータのように動作しますが、集約について逆変換関数を含む別の実装を定義します。␞␞  </para>␞
␝  <para>␟   The syntax with <literal>ORDER BY</literal> in the parameter list creates
   ...
   aggregate-input rows as an additional <quote>hypothetical</quote> row.␟パラメータのリストに<literal>ORDER BY</literal>を含む構文は、<firstterm>順序集合集約</firstterm>と呼ばれる特別な種類の集約を作ります。
...
これにより、直接引数の値を、<quote>仮想的な</quote>行として、集約の入力行の集合に加えることができます。␞␞  </para>␞
␝  <para>␟   An aggregate can optionally support <firstterm>partial aggregation</firstterm>,
   ...
   marked <literal>PARALLEL SAFE</literal> to enable parallel aggregation.␟<xref linkend="xaggr-partial-aggregates"/>で説明されている通り、集約では<firstterm>部分集約</firstterm>をサポートすることができます。
...
並列集約を可能にするには、集約に<literal>PARALLEL SAFE</literal>の印をつける必要もあることに注意してください。␞␞  </para>␞
␝  <para>␟   Aggregates that behave like <function>MIN</function> or <function>MAX</function> can
   sometimes be optimized by looking into an index instead of scanning every
   input row.  If this aggregate can be so optimized, indicate it by
   specifying a <firstterm>sort operator</firstterm>.  The basic requirement is that
   the aggregate must yield the first element in the sort ordering induced by
   the operator; in other words:␟<function>MIN</function>や<function>MAX</function>のような振舞いをする集約では、すべての入力行をスキャンせずにインデックスを検索することで最適化できることがあります。
このように最適化される集約の場合、<firstterm>ソート演算子</firstterm>を指定することで明示してください。
その演算子で生成されるソート順で集約の最初の要素が生成されなければならないということが基本的な必要条件です。
言い換えると、␞␞<programlisting>␞
␝</programlisting>␟   must be equivalent to:␟が以下と同じでなければならないということです。␞␞<programlisting>␞
␝</programlisting>␟   Further assumptions are that the aggregate ignores null inputs, and that
   ...
   index operator class.␟更に、集約がNULL入力を無視すること、および、NULL以外の入力がまったくなかった時にのみNULLという結果を返すことも前提となります。
...
指定した演算子がB-treeインデックス演算子クラスの<quote>より小さい</quote>ストラテジか<quote>より大きい</quote>ストラテジのメンバでない限り、最適化が実際には効果がないことに注意してください。␞␞  </para>␞
␝  <para>␟   To be able to create an aggregate function, you must
   ...
   privilege on the supporting functions.␟集約関数を作成するためには、引数の型、状態の型、戻り値の型に対する<literal>USAGE</literal>権限およびサポート関数に対する<literal>EXECUTE</literal>権限を持たなければなりません。␞␞  </para>␞
␝ <refsect1>␟  <title>Parameters</title>␟  <title>パラメータ</title>␞␞␞
␝     <para>␟      The name (optionally schema-qualified) of the aggregate function
      to create.␟      作成する集約関数の名前です（スキーマ修飾名も可）。␞␞     </para>␞
␝     <para>␟      The mode of an argument: <literal>IN</literal> or <literal>VARIADIC</literal>.
      ...
      can be marked <literal>VARIADIC</literal>.␟引数のモードで、<literal>IN</literal>または<literal>VARIADIC</literal>です。
(集約関数は<literal>OUT</literal>の引数をサポートしません。)
省略した場合のデフォルトは<literal>IN</literal>です。
<literal>VARIADIC</literal>を指定できるのは、最後の引数だけです。␞␞     </para>␞
␝     <para>␟      The name of an argument.  This is currently only useful for
      documentation purposes.  If omitted, the argument has no name.␟引数の名前です。
現在は、文書化を目的とする場合にのみ有効です。
省略した場合、引数には名前がありません。␞␞     </para>␞
␝     <para>␟      An input data type on which this aggregate function operates.
      ...
      aggregate is <function>count(*)</function>.)␟集約関数が演算する入力データ型です。
引数が存在しない集約関数を作成するには、引数指定のリストに<literal>*</literal>と記載してください。
（例えば <function>count(*)</function>などの集約です。）␞␞     </para>␞
␝     <para>␟      In the old syntax for <command>CREATE AGGREGATE</command>, the input data type
      ...
      Ordered-set aggregates cannot be defined with the old syntax.␟<command>CREATE AGGREGATE</command>の旧構文では、入力データ型は集約の名前の次に記載されたものではなく<literal>basetype</literal>パラメータにより指定されます。
...
␝     <para>␟      The name of the state transition function to be called for each
      ...
      and returns the next state value.␟それぞれの入力行に対して呼び出される状態遷移関数の名前です。
...
この関数は、現在の状態値と現在の入力データ値を受け取り、次の状態値を返します。␞␞     </para>␞
␝     <para>␟      For ordered-set (including hypothetical-set) aggregates, the state
      ...
      same.␟順序集合(仮想集合を含む)集約では、状態遷移関数は現在値と集約引数のみを受け取り、直接引数は受け取りません。
それ以外の点は全く同じです。␞␞     </para>␞
␝     <para>␟      The data type for the aggregate's state value.␟集約の状態値のデータ型です。␞␞     </para>␞
␝     <para>␟      The approximate average size (in bytes) of the aggregate's state value.
      ...
      grouped aggregate query.␟集約の状態値のおおよその平均サイズ(単位はバイト)です。
このパラメータを省略した場合、あるいはゼロを指定した場合、<replaceable>state_data_type</replaceable>に基づいたデフォルトの推定が使われます。
プランナは、グループ化された集約のクエリに必要なメモリを推定するのに、この値を使います。␞␞     </para>␞
␝     <para>␟      The name of the final function called to compute the aggregate's
      ...
      class="parameter">state_data_type</replaceable>.␟最終関数の名前です。最終関数は、全ての入力行に対する処理が終わった後、集約の結果を計算するために呼び出されます。
通常の集約では、この関数は<replaceable class="parameter">state_data_type</replaceable>型の引数を1つ取らなければなりません。
集約の出力データ型はこの関数の戻り値として定義されます。
<replaceable class="parameter">ffunc</replaceable>が指定されない場合には、集約の結果として終了時の状態値が使われます。出力型は<replaceable class="parameter">state_data_type</replaceable>になります。␞␞     </para>␞
␝     <para>␟      For ordered-set (including hypothetical-set) aggregates, the
      final function receives not only the final state value,
      but also the values of all the direct arguments.␟順序集合(仮想集合を含む)集約では、最終関数は終了時の状態値だけでなく、すべての直接引数の値も受け取ります。␞␞     </para>␞
␝     <para>␟      If <literal>FINALFUNC_EXTRA</literal> is specified, then in addition to the
      ...
      is being defined.␟<literal>FINALFUNC_EXTRA</literal>が指定された場合、最終関数は、終了時の状態値と直接引数に加えて、集約の通常の(集約された)引数に対応する追加のNULL値を受け取ります。
これは主に、多様型の集約が定義されているときに、集約の結果の型を正しく解決するのに役立ちます。␞␞     </para>␞
␝     <para>␟      This option specifies whether the final function is a pure function
      ...
      for which the default is <literal>READ_WRITE</literal>.␟このオプションは、最終関数が引数を変更しない純粋な関数であるかどうかを指定します。
...
順序集合集約ではデフォルトが<literal>READ_WRITE</literal>であることを除き、デフォルトは<literal>READ_ONLY</literal>です。␞␞     </para>␞
␝     <para>␟      The <replaceable class="parameter">combinefunc</replaceable> function
      ...
      aggregate state, it adds another aggregate state to the running state.␟集約関数が部分集約をサポートできるようにするために、<replaceable class="parameter">combinefunc</replaceable>を指定することができます。
...
この関数は、個々の入力行に対して作用してそれを集約中の状態に追加するのではなく、実行中の状態に別の集約状態を追加する<replaceable class="parameter">sfunc</replaceable>として考えることができます。␞␞     </para>␞
␝     <para>␟      The <replaceable class="parameter">combinefunc</replaceable> must be
      ...
      the other state will be taken as the correct result.␟<replaceable class="parameter">combinefunc</replaceable>は、<replaceable class="parameter">state_data_type</replaceable>の引数を2つ取り、<replaceable class="parameter">state_data_type</replaceable>の値を返すものとして宣言されなければなりません。
この関数は<quote>strict</quote>とすることもできます。
その場合、入力状態の一方がNULLのときは関数が呼び出されず、他方の状態が正しい結果であると見なされます。␞␞     </para>␞
␝     <para>␟      For aggregate functions
      ...
      returned is properly stored in the aggregate memory context.␟<replaceable class="parameter">state_data_type</replaceable>が<type>internal</type>の集約関数では、<replaceable class="parameter">combinefunc</replaceable>をSTRICTにすることはできません。
この場合、<replaceable class="parameter">combinefunc</replaceable>はNULL状態を正しく処理し、返される状態が集約のメモリコンテキスト内に適切に保存されることを確実にする必要があります。␞␞     </para>␞
␝     <para>␟      An aggregate function
      whose <replaceable class="parameter">state_data_type</replaceable>
      ...
      is also required.␟<replaceable class="parameter">state_data_type</replaceable>が<type>internal</type>の集約関数は、<replaceable class="parameter">serialfunc</replaceable>関数がある場合に限り、並列集約に参加することができます。
...
これに対応する<replaceable class="parameter">deserialfunc</replaceable>も必要です。␞␞     </para>␞
␝     <para>␟      Deserialize a previously serialized aggregate state back into
      ...
      for type safety reasons.)␟以前にシリアライズされた集約状態をデシリアライズして<replaceable class="parameter">state_data_type</replaceable>に戻します。
この関数は<type>bytea</type>型および<type>internal</type>型の2つの引数を取り、<type>internal</type>型の結果を生成しなければなりません。
（注意：2番目の<type>internal</type>の引数は使用されませんが、型の安全性の理由から必要となっています。）␞␞     </para>␞
␝     <para>␟      The initial setting for the state value.  This must be a string
      constant in the form accepted for the data type <replaceable
      class="parameter">state_data_type</replaceable>.  If not
      specified, the state value starts out null.␟状態値の初期設定です。
データ型<replaceable class="parameter">state_data_type</replaceable>として受け取り可能な文字列定数でなければいけません。
このパラメータが指定されない場合、状態値はNULLから始まります。␞␞     </para>␞
␝     <para>␟      The name of the forward state transition function to be called for each
      ...
      from <replaceable>state_data_type</replaceable>.␟移動集約モードにおいて、それぞれの入力行に対して呼び出される前方状態遷移関数の名前です。
これは最初の引数と結果が<replaceable>mstate_data_type</replaceable>型で、<replaceable>state_data_type</replaceable>とは異なるかもしれないことを除けば、通常の遷移関数と全く同じです。␞␞     </para>␞
␝     <para>␟      The name of the inverse state transition function to be used in
      ...
      attribute as the forward state transition function.␟移動集約モードで使われる逆状態遷移関数の名前です。
この関数は<replaceable>msfunc</replaceable>と同じ引数および結果型を持ちますが、現在の集約状態に対して、値を追加するのではなく、取り除くために使われます。
逆遷移関数は前方状態遷移関数と同じstrictさの属性を持っていなければなりません。␞␞     </para>␞
␝     <para>␟      The data type for the aggregate's state value, when using
      moving-aggregate mode.␟移動集約モードを使うときの、集約状態値のデータ型です。␞␞     </para>␞
␝     <para>␟      The approximate average size (in bytes) of the aggregate's state
      value, when using moving-aggregate mode.  This works the same as
      <replaceable>state_data_size</replaceable>.␟移動集約モードを使うときの、集約状態値のおおよその平均サイズ(バイト単位)です。
<replaceable>state_data_size</replaceable>と同じように作用します。␞␞     </para>␞
␝     <para>␟      The name of the final function called to compute the aggregate's
      ...
      aggregate's regular implementation.␟移動集約モードを使うときに、すべての入力行がスキャンされた後で、集約結果を計算するために呼び出される最終関数の名前です。
...
<replaceable>mffunc</replaceable>または<replaceable>mstate_data_type</replaceable>によって決定される集約結果の型は、集約の通常の実装によって決定される型と適合しなければなりません。␞␞     </para>␞
␝     <para>␟      This option is like <literal>FINALFUNC_MODIFY</literal>, but it describes
      the behavior of the moving-aggregate final function.␟このオプションは<literal>FINALFUNC_MODIFY</literal>と似ていますが、移動集約最終関数の挙動を記述します。␞␞     </para>␞
␝     <para>␟      The initial setting for the state value, when using moving-aggregate
      mode.  This works the same as <replaceable>initial_condition</replaceable>.␟移動集約モードを使うときの、状態値の初期設定です。
これは、<replaceable>initial_condition</replaceable>と同じように作用します。␞␞     </para>␞
␝     <para>␟      The associated sort operator for a <function>MIN</function>- or
      <function>MAX</function>-like aggregate.
      This is just an operator name (possibly schema-qualified).
      The operator is assumed to have the same input data types as
      the aggregate (which must be a single-argument normal aggregate).␟<function>MIN</function>または<function>MAX</function>のような集約に対して関連付けされるソート演算子です。
これは単なる演算子の名前です（スキーマ修飾可能）。
この演算子は集約（これは単一引数の通常の集約でなければなりません）と同じ入力データ型を持つと前提されています。␞␞     </para>␞
␝     <para>␟      The meanings of <literal>PARALLEL SAFE</literal>, <literal>PARALLEL
      ...
      aggregate itself.␟<literal>PARALLEL SAFE</literal>、<literal>PARALLEL RESTRICTED</literal>、<literal>PARALLEL UNSAFE</literal>の意味は<link linkend="sql-createfunction"><command>CREATE FUNCTION</command></link>におけるものと同じです。
集約は、その印が<literal>PARALLEL UNSAFE</literal>（これがデフォルトです！）あるいは<literal>PARALLEL RESTRICTED</literal>となっている場合、並列処理での使用を考慮されません。
プランナは集約のサポート関数の並列処理安全性の印を考慮せず、集約自体の印のみを参照することに注意してください。␞␞     </para>␞
␝     <para>␟      For ordered-set aggregates only, this flag specifies that the aggregate
      ...
      collations of the aggregate's arguments.␟順序集合集約についてのみ、このフラグは、仮想集合集約の要求に従って集約の引数が処理されることを指定します。
つまり、最後のいくつかの引数が、集約される(<literal>WITHIN GROUP</literal>の)引数と適合しなければなりません。
<literal>HYPOTHETICAL</literal>フラグは実行時の動作には何の影響もなく、集約の引数のデータ型と照合についての解析時の解決にのみ影響します。␞␞     </para>␞
␝  <para>␟   The parameters of <command>CREATE AGGREGATE</command> can be
   written in any order, not just the order illustrated above.␟<command>CREATE AGGREGATE</command>のパラメータは、任意の順番で記述することができます。上記の順番で記述する必要はありません。␞␞  </para>␞
␝␟ <refsect1 id="sql-createaggregate-notes" xreflabel="Notes">␟ <refsect1 id="sql-createaggregate-notes" xreflabel="注釈">␞␞␞
␝ <refsect1 id="sql-createaggregate-notes" xreflabel="Notes">␟  <title>Notes</title>␟  <title>注釈</title>␞␞␞
␝   <para>␟    In parameters that specify support function names, you can write
    ...
    of the support functions are determined from other parameters.␟サポート関数名を指定するパラメータでは、必要なら、<literal>SFUNC = public.sum</literal>のようにスキーマ名を書くことができます。
しかし、引数の型をそこに書くことはできません。
サポート関数の引数の型は、他のパラメータにより決定されるからです。␞␞   </para>␞
␝   <para>␟    Ordinarily, PostgreSQL functions are expected to be true functions that
    ...
    a fresh copy of the transition state each time.␟通常PostgreSQL関数は入力値を変更しない純粋な関数であることが期待されます。
しかし<emphasis>集約のコンテキストで使用される場合には</emphasis>、集約遷移関数は、これを偽ってその状態遷移引数を直接変更することが許されます。
これにより、遷移状態の新しいコピーを都度作るのに比べると、かなりの性能上の利点が期待できます。␞␞   </para>␞
␝   <para>␟    Likewise, while an aggregate final function is normally expected not to
    ...
    long as none of the final functions are marked <literal>READ_WRITE</literal>.)␟同様に、集約最終関数は入力値を変更しない純粋な関数であることが期待されますが、状態遷移引数を変更するのを避けることが実用的でないことがあります。
...
これは最終関数のうち<literal>READ_WRITE</literal>と印付けられているものが一つもない限り許容されます。）␞␞   </para>␞
␝   <para>␟    If an aggregate supports moving-aggregate mode, it will improve
    ...
    fresh aggregation, rather than using the inverse transition function.)␟集約が移動集約モードをサポートしていると、移動フレームの開始のあるウィンドウ（つまり、<literal>UNBOUNDED PRECEDING</literal>以外のフレーム開始モード）として集約が使われる場合に計算の効率が向上します。
...
(そうならない場合は、ウィンドウ関数の仕組みは、逆遷移関数を使うのでなく、単純に新しい集約を開始します。)␞␞   </para>␞
␝   <para>␟    The forward transition function for moving-aggregate mode is not
    ...
    running state value.␟移動集約モードの前方遷移関数は、新しい集約値としてNULLを返すことは許されません。
逆遷移関数がNULLを返した場合、それは、逆関数がその入力値について状態計算を元に戻すことができなかったと見なされるため、集約の計算は現在のフレームの開始位置からやり直しとなります。
こうすることで、実行中の状態値を元に戻すのが現実的でないということが稀に起こるような場合でも、移動集約モードを使うことができます。␞␞   </para>␞
␝   <para>␟    If no moving-aggregate implementation is supplied,
    ...
    one set of frame boundaries.␟移動集約が実装されていない場合でも、集約は移動フレームを使うことができますが、フレームの開始が移動した場合、<productname>PostgreSQL</productname>は必ず集約全体を再計算します。
...
最終関数は集約の状態値を破壊しないものとされるので、フレームの境界の集合に対して集約結果の値が得られた後でも、集約を続行することが可能です。␞␞   </para>␞
␝   <para>␟    The syntax for ordered-set aggregates allows <literal>VARIADIC</literal>
    ...
    that are not constrained to match the aggregated arguments.␟順序集合集約の構文では、<literal>VARIADIC</literal>を最後の直接パラメータと、最後の集約(<literal>WITHIN GROUP</literal>)パラメータの両方について指定することができます。
...
仮想集合集約の場合、<literal>VARIADIC "any"</literal>パラメータに対応する直接引数は仮想的なパラメータで、それより前のパラメータは、集約引数に対応する制約のない、追加の直接引数となります。␞␞   </para>␞
␝   <para>␟    Currently, ordered-set aggregates do not need to support
    moving-aggregate mode, since they cannot be used as window functions.␟現在は、順序集合集約は、ウィンドウ関数として使うことはできないので、移動集約モードをサポートする必要はありません。␞␞   </para>␞
␝   <para>␟    Partial (including parallel) aggregation is currently not supported for
    ...
    those semantics cannot be supported during partial aggregation.␟部分集約（並列集約を含む）は現在のところ、順序集約ではサポートされません。
また、部分集約は<literal>DISTINCT</literal>あるいは<literal>ORDER BY</literal>句を含む集約の呼び出しでは決して使われることはありません。
なぜなら、部分集約ではそれらを意味論的にサポートできないからです。␞␞  </para>␞
␝ <refsect1>␟  <title>Examples</title>␟  <title>例</title>␞␞␞
␝  <para>␟   See <xref linkend="xaggr"/>.␟<xref linkend="xaggr"/>を参照してください。␞␞  </para>␞
␝ <refsect1>␟  <title>Compatibility</title>␟  <title>互換性</title>␞␞␞
␝  <para>␟   <command>CREATE AGGREGATE</command> is a
   <productname>PostgreSQL</productname> language extension.  The SQL
   standard does not provide for user-defined aggregate functions.␟<command>CREATE AGGREGATE</command>は<productname>PostgreSQL</productname>の言語拡張です。
標準SQLには、ユーザ定義の集約関数を使用する機能はありません。␞␞  </para>␞
