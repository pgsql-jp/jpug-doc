...
␝<chapter id="tablesample-method">␟ <title>Writing a Table Sampling Method</title>␟ <title>テーブルサンプリングメソッドの書き方</title>␞␞␞
␝ <para>␟  <productname>PostgreSQL</productname>'s implementation of the <literal>TABLESAMPLE</literal>
  ...
  table will be selected when the <literal>TABLESAMPLE</literal> clause is used.␟<productname>PostgreSQL</productname>による<literal>TABLESAMPLE</literal>句の実装は、標準SQLが求める<literal>BERNOULLI</literal>と<literal>SYSTEM</literal>のメソッドに加え、ユーザ定義のテーブルサンプリングメソッドをサポートしています。
サンプリングメソッドは、<literal>TABLESAMPLE</literal>句が使用された際にどの行が選択されるかを決定します。␞␞ </para>␞
␝ <para>␟  At the SQL level, a table sampling method is represented by a single SQL
  function, typically implemented in C, having the signature␟SQLレベルでは、テーブルサンプリングメソッドは、以下の呼び出し形式を持ち、典型的にはCで実装された単一のSQLの関数で表現されます。␞␞<programlisting>␞
␝</programlisting>␟  The name of the function is the same method name appearing in the
  ...
  described in <xref linkend="tablesample-support-functions"/>.␟関数の名前は<literal>TABLESAMPLE</literal>句に現れるメソッド名と同じです。
...
サポート関数は<xref linkend="tablesample-support-functions"/>で説明されています。␞␞ </para>␞
␝ <para>␟  In addition to function pointers, the <type>TsmRoutine</type> struct must
  provide these additional fields:␟関数へのポインタに加え、<type>TsmRoutine</type>構造体は以下の追加のフィールドを提供しなければなりません。␞␞ </para>␞
␝    <para>␟     This is an OID list containing the data type OIDs of the parameter(s)
     ...
     more or different parameters.␟このサンプリングメソッドが使用される際に、<literal>TABLESAMPLE</literal>句が受け付ける引数のデータ型のOIDが格納された、OIDのリストです。
たとえば、組み込みのメソッドに対しては、このリストは、サンプリングのパーセンテージを表す<literal>FLOAT4OID</literal>という値を持つ単一の要素が含まれています。
カスタムサンプリングメソッドは、複数の異なるパラメータを持つことができます。␞␞    </para>␞
␝    <para>␟     If <literal>true</literal>, the sampling method can deliver identical samples
     ...
     sampling method.␟<literal>true</literal>の場合、もし毎回同じ引数と<literal>REPEATABLE</literal>シード値が提供され、テーブル内容に変更がないならば、サンプリングメソッドは実行されたどの問い合わせに対しても、同一のサンプルを出力することができます。
<literal>false</literal>の場合は、サンプリングメソッドを使用する際に<literal>REPEATABLE</literal>句を受け付けません。␞␞    </para>␞
␝    <para>␟     If <literal>true</literal>, the sampling method can deliver identical samples
     ...
     might result in inconsistent query output.␟<literal>true</literal>の場合、サンプリングメソッドは同じ問い合わせ内で実行されたどのスキャンに対しても、同一のサンプルを出力することができます(パラメータ、シード値、スナップショットに変更がない、という前提で)。
<literal>false</literal>の場合、プランナは、サンプル対象のテーブルを2度以上スキャンする必要のあるようなプランは選択しません。
問い合わせの結果に不整合が発生する可能性があるからです。␞␞    </para>␞
␝ <para>␟  The <type>TsmRoutine</type> struct type is declared
  in <filename>src/include/access/tsmapi.h</filename>, which see for additional
  details.␟<type>TsmRoutine</type>構造体は<filename>src/include/access/tsmapi.h</filename>で宣言されています。
詳細はそちらをご覧ください。␞␞ </para>␞
␝ <para>␟  The table sampling methods included in the standard distribution are good
  ...
  subdirectory for add-on methods.␟標準配布物に含まれるテーブルサンプリングメソッドは、自分でサンプリングメソッドを書く際に良いお手本になります。
組み込みのサンプリングメソッドに関しては、ソースツリー中の<filename>src/backend/access/tablesample</filename>サブディレクトリを見てください。
追加のメソッドに関しては<filename>contrib</filename>サブディレクトリを見てください。␞␞ </para>␞
␝ <sect1 id="tablesample-support-functions">␟  <title>Sampling Method Support Functions</title>␟  <title>サンプリングメソッドサポート関数</title>␞␞␞
␝  <para>␟   The TSM handler function returns a palloc'd <type>TsmRoutine</type> struct
   ...
   be NULL.␟TSMハンドラ関数は、以下に示すサポート関数へのポインタを含むpallocされた<type>TsmRoutine</type>構造体を返します。
大半の関数は必須ですが、いくつかオプションのものがあり、そうした関数へのポインタはNULLにできます。␞␞  </para>␞
␝␟   This function is called during planning.  It must estimate the number of
   ...
   The <literal>pages</literal> and <literal>tuples</literal> parameters are outputs.␟この関数はプランニングの際に呼び出されます。
...
<literal>pages</literal>と<literal>tuples</literal>パラメータは出力です。␞␞  </para>␞
␝␟   Initialize for execution of a SampleScan plan node.
   ...
   operating mode for this plan node.␟SampleScan計画ノードを実行するための初期化を行います。
...
<literal>eflags</literal>には、この計画ノードにおけるエグゼキュータの動作モードを記述するフラグビットが含まれます。␞␞  </para>␞
␝  <para>␟   When <literal>(eflags &amp; EXEC_FLAG_EXPLAIN_ONLY)</literal> is true,
   ...
   and <function>EndSampleScan</function>.␟<literal>(eflags &amp; EXEC_FLAG_EXPLAIN_ONLY)</literal>が真ならば、スキャンは実際には行われず、この関数は<command>EXPLAIN</command>と<function>EndSampleScan</function>にとってノードの状態が意味のあるように最低限必要な処理を行うことになります。␞␞  </para>␞
␝  <para>␟   This function can be omitted (set the pointer to NULL), in which case
   <function>BeginSampleScan</function> must perform all initialization needed
   by the sampling method.␟この関数は(ポインタをNULLにすることにより)省略できますが、この場合、<function>BeginSampleScan</function>がサンプリングメソッドに必要なすべての初期化を行なわなければなりません。␞␞  </para>␞
␝␟   Begin execution of a sampling scan.
   ...
   of <literal>random()</literal> if not.␟サンプルスキャンの実行を開始します。
...
これは、<literal>REPEATABLE</literal>の値が指定されている場合はそこから派生したハッシュか、でなければ<literal>random()</literal>の結果です。␞␞  </para>␞
␝  <para>␟   This function may adjust the fields <literal>node-&gt;use_bulkread</literal>
   ...
   will require more locking.␟この関数は<literal>node-&gt;use_bulkread</literal>と<literal>node-&gt;use_pagemode</literal>フィールドによって動作を変更します。
...
ただし、個々の操作はより高くつきます。
というのも、より多くのロックが必要になるからです。␞␞  </para>␞
␝  <para>␟   If the sampling method is
   ...
   and seed don't change).␟サンプリングメソッドに<literal>repeatable_across_scans</literal>という印があれば、最初にスキャンした時と同じタプルの集合を、再スキャンでも選択できることになります。
つまり、新しい<function>BeginSampleScan</function>が、前回と同じタプルを選択することになるわけです。
(もし<literal>TABLESAMPLE</literal>の引数とシードが変わらなければ、の話ですが)␞␞  </para>␞
␝␟   Returns the block number of the next page to be scanned, or
   <literal>InvalidBlockNumber</literal> if no pages remain to be scanned.␟次にスキャンするページのブロック番号を返します。
もはやスキャンするページがない場合には<literal>InvalidBlockNumber</literal>を返します。␞␞  </para>␞
␝  <para>␟   This function can be omitted (set the pointer to NULL), in which case
   ...
   each scan.␟この関数は(ポインタをNULLにすることにより)省略できます。
この場合コアのコードはリレーション全体をシーケンシャルスキャンします。
そのようなスキャンは同期スキャンを行う可能性があるので、毎回のスキャンで同じ順番でリレーションのページをアクセスするとは、サンプリングメソッドは仮定できません。␞␞  </para>␞
␝␟   Returns the offset number of the next tuple to be sampled on the
   ...
   on the page.␟サンプル対象の指定ページ内の次のタプルのオフセット番号を返します。
サンプル対象のタプルが残っていない場合は、<literal>InvalidOffsetNumber</literal>を返します。
<literal>maxoffset</literal>は、使用中のページ中の最大オフセット番号です。␞␞  </para>␞
␝   <para>␟    <function>NextSampleTuple</function> is not explicitly told which of the offset
    ...
    it returned were valid and visible.␟<function>NextSampleTuple</function>は、範囲<literal>1 .. maxoffset</literal>の中のどのオフセット番号が有効なタプルにあたるのかは明示的には教えてくれません。
コアのコードは、存在しない、あるいは不可視のタプルを対象とするサンプルの要求は無視するため、通常これは問題にはなりません。サンプルの偏りも起きません。
それでも必要ならば、関数は<literal>node-&gt;donetuples</literal>を使って、返されたタプルのうちのいくつが有効で可視であったのかを調べることができます。␞␞   </para>␞
␝   <para>␟    <function>NextSampleTuple</function> must <emphasis>not</emphasis> assume
    ...
    returned.␟<function>NextSampleTuple</function>は、直近の<function>NextSampleBlock</function>の呼び出しが返したページ番号と<literal>blockno</literal>が同じであると見なすべきでは<emphasis>ありません</emphasis>。
...
一旦あるページのサンプリングが開始すれば、<literal>InvalidOffsetNumber</literal>が返るまでは、続く<function>NextSampleTuple</function>に呼び出しがすべて同じページを参照すると見なすことは問題ありません。␞␞   </para>␞
␝␟   End the scan and release resources.  It is normally not important
   ...
   case where no such resources exist.␟スキャンを終了し、リソースを解放します。
通常pallocされたメモリを解放するのは重要なことではありませんが、外部から見えるリソースはすべて解放しなければなりません。
そのようなリソースが存在しない場合は、この関数は(ポインタをNULLにすることにより)省略できます。␞␞  </para>␞
