...
␝ <sect1 id="xtypes">␟  <title>User-Defined Types</title>␟  <title>ユーザ定義の型</title>␞␞␞
␝  <para>␟   As described in <xref linkend="extend-type-system"/>,
   ...
   to operate on the type in a low-level language, usually C.␟<xref linkend="extend-type-system"/>に述べられているように、<productname>PostgreSQL</productname>は、新しい型をサポートするように拡張することができます。
本節では、<acronym>SQL</acronym>言語以下のレベルで定義されるデータ型である基本型を新しく定義する方法について説明します。
新しい基本型の作成には、低レベル言語、通常Cで作成された型を操作する関数の実装が必要です。␞␞  </para>␞
␝  <indexterm>
...
␝  <para>␟   The examples in this section can be found in
   ...
   about running the examples.␟本節で使用する例は、ソース配布物内の<filename>src/tutorial</filename>ディレクトリに<filename>complex.sql</filename>と<filename>complex.c</filename>という名前で置いてあります。
この例の実行方法についてはディレクトリ内の<filename>README</filename>を参照してください。␞␞  </para>␞
␝  </indexterm>␟  A user-defined type must always have input and output functions.
  ...
  operations we'd like to have for the type.␟ユーザ定義データ型では必ず入力関数と出力関数が必要です。
...
単に格納するだけではなく、その型に操作を加えたいのであれば、その型に持たせたいすべての操作を実装した関数をさらに提供しなければなりません。␞␞ </para>␞
␝ <para>␟  Suppose we want to define a type <type>complex</type> that represents
  complex numbers. A natural way to represent a complex number in
  memory would be the following C structure:␟例えば、複素数を表現する<type>complex</type>型を定義することを考えます。
おそらく、次のようなC構造体で複素数をメモリ中で表現することがごく自然な方法です。␞␞␞
␝␟  We will need to make this a pass-by-reference type, since it's too
  large to fit into a single <type>Datum</type> value.␟単一の<type>Datum</type>値で扱うには大き過ぎるので、これは参照渡し型にしなければなりません。␞␞ </para>␞
␝ <para>␟  As the external string representation of the type, we choose a
  string of the form <literal>(x,y)</literal>.␟この型の外部文字列表現として<literal>(x,y)</literal>形式の文字列を使用することを選択します。␞␞ </para>␞
␝ <para>␟  The input and output functions are usually not hard to write,
  ...
  input function.  For instance:␟入出力関数、特に出力関数を作成することは困難ではありません。
しかし、この型の外部表現文字列を定義する時、その表現のための完全で堅牢なパーサを入力関数として作成しなければなりません。
以下に例を示します。␞␞␞
␝␟  The output function can simply be:␟出力関数は以下のように簡単にできます。␞␞␞
␝ <para>␟  You should be careful to make the input and output functions inverses of
  ...
  involved.␟入出力関数は各々の逆関数になるように注意しなければなりません。
そうしないと、データをファイルにダンプし、それを読み戻そうとする際に、深刻な問題が発生するでしょう。
これは特に浮動小数点数が関係する際によく発生する問題です。␞␞ </para>␞
␝ <para>␟  Optionally, a user-defined type can provide binary input and output
  ...
  for type <type>float8</type>:␟省略することができますが、ユーザ定義型はバイナリ入出力関数を提供することができます。
...
␝ <para>␟  Once we have written the I/O functions and compiled them into a shared
  library, we can define the <type>complex</type> type in SQL.
  First we declare it as a shell type:␟入出力関数を作成し共有ライブラリ内にコンパイルすれば、SQLで<type>complex</type>型を定義することができます。
まずシェル型として宣言します。␞␞␞
␝␟  This serves as a placeholder that allows us to reference the type while
  defining its I/O functions.  Now we can define the I/O functions:␟これは、入出力関数を定義する時にこの型を参照することができるプレースホルダとして動作します。
...
␝ <para>␟  Finally, we can provide the full definition of the data type:␟最後にデータ型の完全な定義を提供することができます。␞␞<programlisting>␞
␝  </indexterm>␟  When you define a new base type,
  ...
  (<literal>_</literal>) prepended.␟新しい基本型を定義すると、<productname>PostgreSQL</productname>は自動的にその型の配列のサポートを提供します。
配列型は通常、基本型の名前の前にアンダースコア文字<literal>_</literal>が付いた名前になります。␞␞ </para>␞
␝ <para>␟  Once the data type exists, we can declare additional functions to
  ...
  layers are discussed in following sections.␟データ型が存在するようになると、そのデータ型に対する有用な操作を提供する関数を宣言することができます。
そしてその関数を使用する演算子も定義できます。
また、必要に応じて、そのデータ型用のインデックスをサポートするための演算子クラスも作成することができます。
こうした追加層については後の節で説明します。␞␞ </para>␞
␝ <para>␟  If the internal representation of the data type is variable-length, the
  ...
  because the length field may be encoded depending on platform.)␟データ型の内部表現が可変長であるなら、内部表現は可変長データの標準配置に従わなければなりません。先頭の4バイトは<type>char[4]</type>フィールドで、直接アクセスされることは決してありません（慣習的に<structfield>vl_len_</structfield>と呼ばれます）。
<function>SET_VARSIZE()</function>マクロを使用してデータの総量をこのフィールドに格納し、また、<function>VARSIZE()</function>を使用して取り出さなければなりません。
(長さフィールドはプラットフォームに依存してエンコードされるかもしれませんので、このマクロが存在します。)␞␞ </para>␞
␝ <para>␟  For further details see the description of the
  <xref linkend="sql-createtype"/> command.␟詳細については<xref linkend="sql-createtype"/>コマンドの説明を参照してください。␞␞ </para>␞
␝   <indexterm>
...
␝ <sect2 id="xtypes-toast">␟  <title>TOAST Considerations</title>␟  <title>TOASTの考慮</title>␞␞   <indexterm>␞
␝ <para>␟  If the values of your data type vary in size (in internal form), it's
  ...
  overhead.␟データ型の値により（内部形式で）容量が変動する場合、そのデータ型を<acronym>TOAST</acronym>可能とすることが通常は望ましいです（<xref linkend="storage-toast"/>を参照してください）。
ヘッダのオーバーヘッドを減らすことで<acronym>TOAST</acronym>は小さなデータに対しても容量を抑えることができますので、データが常に圧縮したり外部に格納したりするには小さ過ぎる場合でも、これを行なうことを推奨します。␞␞ </para>␞
␝ <para>␟  To support <acronym>TOAST</acronym> storage, the C functions operating on the data
  ...
  option other than <literal>plain</literal>.␟<acronym>TOAST</acronym>格納をサポートするために、そのデータ型を扱うC関数は常に、<function>PG_DETOAST_DATUM</function>を使用して、渡されたTOAST化値を注意深く展開しなければなりません。
（通常、こうした詳細は型独自の<function>GETARG_DATATYPE_P</function>マクロを定義して隠蔽します。）
その後、<command>CREATE TYPE</command>コマンドを実行する際に、内部長を<literal>variable</literal>と指定し、また、<literal>plain</literal>以外の適当な格納オプションを選択してください。␞␞ </para>␞
␝ <para>␟  If data alignment is unimportant (either just for a specific function or
  ...
  must go through the regular <function>PG_DETOAST_DATUM</function> interface.␟データの整列が（単なる特定の関数向けやデータ型が常にバイト単位の整列を規定しているため）重要でない場合、<function>PG_DETOAST_DATUM</function>のオーバーヘッドの一部を省くことができます。
...
整列が重要であれば、通常の<function>PG_DETOAST_DATUM</function>インタフェースを介して実行してください。␞␞ </para>␞
␝  <para>␟   Older code frequently declares <structfield>vl_len_</structfield> as an
   ...
   architectures that are strict about alignment.␟古めのコードではしばしば<structfield>vl_len_</structfield>を<type>char[4]</type>ではなく<type>int32</type>として宣言しています。
この構造体定義が少なくとも<type>int32</type>で整列されたフィールドを持っている限り、これは問題ありません。
しかし、整列されていない可能性があるデータを扱う場合に、こうした構造体定義を使用することは危険です。
データが実際に整列されていると仮定することをコンパイラの規則としているかもしれず、この場合、整列に厳密なアーキテクチャではコアダンプしてしまいます。␞␞  </para>␞
␝ <para>␟  Another feature that's enabled by <acronym>TOAST</acronym> support is the
  ...
  and then pass that format in-memory between functions of the data type.␟<acronym>TOAST</acronym>のサポートにより有効になるもう一つの機能は以下のような可能性です。ディスクに格納されたフォーマットよりも扱うのにより便利な<firstterm>展開された</firstterm>インメモリデータ表現を持てるかもしれません。
通常のもしくは<quote>単純な</quote>varlena格納フォーマットは結局のところ単なるバイトのblobです。例えば、メモリの別の場所にコピーされるかもしれませんのでポインタを含むことができません。
複雑なデータ型に対しては、単純なフォーマットは扱うのにかなり高価になるかもしれません。そこで、<productname>PostgreSQL</productname>は計算するのにより適した表現に単純なフォーマットを<quote>展開する</quote>方法を提供し、そのフォーマットをインメモリでそのデータ型の関数から関数へと渡します。␞␞ </para>␞
␝ <para>␟  To use expanded storage, a data type must define an expanded format that
  ...
  unless better performance is important.␟展開された格納を使うためには、データ型は<filename>src/include/utils/expandeddatum.h</filename>にある規則に従う展開されたフォーマットを定義し、単純なvarlenaの値を展開されたフォーマットに<quote>展開する</quote>関数や展開されたフォーマットを通常のvarlena表現に<quote>戻す</quote>関数を提供しなければなりません。
...
そのため、単純なvarlenaフォーマットを扱う既存の関数は、わずかに非効率ではありますが、展開された入力も続けて扱えるでしょう。より良いパフォーマンスが重要になるまで、変更は必要ありません。␞␞ </para>␞
␝ <para>␟  C functions that know how to work with an expanded representation
  ...
  determine which format they received.␟展開された表現の扱い方を知っているC関数は典型的には以下の2つに分類されます。展開されたフォーマットのみを扱えるものと、展開されたものも単純なvarlena入力も扱えるものです。
...
呼び出し側はどちらのフォーマットを受け取ったのか確定するのに<function>VARATT_IS_EXPANDED_HEADER()</function>マクロを使えます。␞␞ </para>␞
␝ <para>␟  The <acronym>TOAST</acronym> infrastructure not only allows regular varlena
  ...
  in a sane state if it fails partway through.␟<acronym>TOAST</acronym>基盤により、通常のvarlenaの値を展開された値から区別できるようになるだけでなく、展開された値への<quote>読み書き可能</quote>なポインタと<quote>読み取りのみ</quote>のポインタを区別できるようになります。
...
また、読み書き可能な展開された値をその場で修正するC関数は、途中で失敗した場合に気をつけて値を健全な状態のままにしておくことを推奨します。␞␞ </para>␞
␝ <para>␟  For examples of working with expanded values, see the standard array
  infrastructure, particularly
  <filename>src/backend/utils/adt/array_expanded.c</filename>.␟展開された値を扱う例は、標準配列基盤、特に<filename>src/backend/utils/adt/array_expanded.c</filename>を見てください。␞␞ </para>␞
