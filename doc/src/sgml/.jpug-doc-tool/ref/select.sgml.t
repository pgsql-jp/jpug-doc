...
␝  <refname>WITH</refname>␟  <refpurpose>retrieve rows from a table or view</refpurpose>␟  <refpurpose>テーブルもしくはビューから行を検索する</refpurpose>␞␞ </refnamediv>␞
␝␟<phrase>where <replaceable class="parameter">from_item</replaceable> can be one of:</phrase>␟<phrase>ここで<replaceable class="parameter">from_item</replaceable>は以下の一つです。</phrase>␞␞␞
␝␟<phrase>and <replaceable class="parameter">grouping_element</replaceable> can be one of:</phrase>␟<phrase>また<replaceable class="parameter">grouping_element</replaceable>は以下の一つです。</phrase>␞␞␞
␝␟<phrase>and <replaceable class="parameter">with_query</replaceable> is:</phrase>␟<phrase>また<replaceable class="parameter">with_query</replaceable>は以下の通りです。</phrase>␞␞␞
␝ <refsect1>␟  <title>Description</title>␟  <title>説明</title>␞␞␞
␝  <para>␟   <command>SELECT</command> retrieves rows from zero or more tables.
   The general processing of <command>SELECT</command> is as follows:␟<command>SELECT</command>は0個以上のテーブルから行を返します。
<command>SELECT</command>の一般的な処理は以下の通りです。␞␞␞
␝     <para>␟      All queries in the <literal>WITH</literal> list are computed.
      ...
      (See <xref linkend="sql-with"/> below.)␟<literal>WITH</literal>リスト内のすべての問い合わせが計算されます。
...
（後述の<xref linkend="sql-with"/>を参照してください。）␞␞     </para>␞
␝     <para>␟      All elements in the <literal>FROM</literal> list are computed.
      ...
      (See <xref linkend="sql-from"/> below.)␟<literal>FROM</literal>リストにある全要素が計算されます。
（<literal>FROM</literal>リストの要素は実テーブルか仮想テーブルのいずれかです。）
<literal>FROM</literal>リストに複数の要素が指定された場合、それらはクロス結合されます。
（後述の<xref linkend="sql-from"/>を参照してください。）␞␞     </para>␞
␝     <para>␟      If the <literal>WHERE</literal> clause is specified, all rows
      that do not satisfy the condition are eliminated from the
      output.  (See <xref linkend="sql-where"/> below.)␟<literal>WHERE</literal>句が指定された場合、条件を満たさない行は全て出力から取り除かれます。
（後述の<xref linkend="sql-where"/>を参照してください。）␞␞     </para>␞
␝     <para>␟      If the <literal>GROUP BY</literal> clause is specified,
      ...
      in the <literal>GROUP BY</literal> clause.␟<literal>GROUP BY</literal>句が指定された場合、および集約関数の呼び出しがある場合は、1つまたは複数の値が条件に合う行ごとにグループに組み合わせて出力され、また集約関数の結果が計算されます。
...
問い合わせ出力列は、名目上は次のステップで計算されますが、<literal>GROUP BY</literal>句で（名前または序数によって）参照することもできます。␞␞     </para>␞
␝     <para>␟      The actual output rows are computed using the
      <command>SELECT</command> output expressions for each selected
      row or row group.  (See <xref linkend="sql-select-list"/> below.)␟実際には、選択された各行または行グループに対して、<command>SELECT</command>の出力式を使用して計算した結果の行が出力されます。
（後述の<xref linkend="sql-select-list"/>を参照してください。）␞␞     </para>␞
␝    <listitem>␟     <para><literal>SELECT DISTINCT</literal> eliminates duplicate rows from the
      ...
      duplicates.  (See <xref linkend="sql-distinct"/> below.)␟<para><literal>SELECT DISTINCT</literal>は結果から重複行を取り除きます。
<literal>SELECT DISTINCT ON</literal>は指定した全ての式に一致する行を取り除きます。
<literal>SELECT ALL</literal>（これがデフォルトです）では、重複行も含め、全ての候補行を返します。
（詳しくは、後述の<xref linkend="sql-distinct"/>を参照してください。）␞␞     </para>␞
␝     <para>␟      Using the operators <literal>UNION</literal>,
      ...
      <xref linkend="sql-except"/> below.)␟<literal>UNION</literal>、<literal>INTERSECT</literal>、<literal>EXCEPT</literal>演算子を使用すると、複数の<command>SELECT</command>文の出力を1つの結果集合にまとめることができます。
...
（後述の<xref linkend="sql-union"/>、<xref linkend="sql-intersect"/>、<xref linkend="sql-except"/>を参照してください。）␞␞     </para>␞
␝     <para>␟      If the <literal>ORDER BY</literal> clause is specified, the
      returned rows are sorted in the specified order.  If
      <literal>ORDER BY</literal> is not given, the rows are returned
      in whatever order the system finds fastest to produce.  (See
      <xref linkend="sql-orderby"/> below.)␟<literal>ORDER BY</literal>句が指定された場合、返される行は指定した順番でソートされます。
<literal>ORDER BY</literal>が指定されない場合は、システムが計算過程で見つけた順番で行が返されます。
（後述の<xref linkend="sql-orderby"/>を参照してください。）␞␞     </para>␞
␝     <para>␟      If the <literal>LIMIT</literal> (or <literal>FETCH FIRST</literal>) or <literal>OFFSET</literal>
      clause is specified, the <command>SELECT</command> statement
      only returns a subset of the result rows. (See <xref
      linkend="sql-limit"/> below.)␟<literal>LIMIT</literal>（または<literal>FETCH FIRST</literal>）あるいは<literal>OFFSET</literal>句が指定された場合、<command>SELECT</command>文は結果行の一部分のみを返します。
（詳しくは、後述の<xref linkend="sql-limit"/>を参照してください。）␞␞     </para>␞
␝     <para>␟      If <literal>FOR UPDATE</literal>, <literal>FOR NO KEY UPDATE</literal>, <literal>FOR SHARE</literal>
      or <literal>FOR KEY SHARE</literal>
      is specified, the
      <command>SELECT</command> statement locks the selected rows
      against concurrent updates.  (See <xref linkend="sql-for-update-share"/>
      below.)␟<literal>FOR UPDATE</literal>、<literal>FOR NO KEY UPDATE</literal>、<literal>FOR SHARE</literal>または<literal>FOR KEY SHARE</literal>句を指定すると、<command>SELECT</command>文は引き続き行われる更新に備えて選択行をロックします。
（詳しくは、後述の<xref linkend="sql-for-update-share"/>を参照してください。）␞␞     </para>␞
␝  <para>␟   You must have <literal>SELECT</literal> privilege on each column used
   ...
   of each table so selected).␟<command>SELECT</command>コマンド内で使われる列それぞれに対する<literal>SELECT</literal>権限が必要です。
<literal>FOR NO KEY UPDATE</literal>、<literal>FOR UPDATE</literal>、<literal>FOR SHARE</literal>または<literal>FOR KEY SHARE</literal>を使用するためには、さらに、（選択された各テーブルで少なくとも1列に対する）<literal>UPDATE</literal>権限が必要です。␞␞  </para>␞
␝ <refsect1>␟  <title>Parameters</title>␟  <title>パラメータ</title>␞␞␞
␝␟  <refsect2 id="sql-with" xreflabel="WITH Clause">␟  <refsect2 id="sql-with" xreflabel="WITH句">␞␞␞
␝  <refsect2 id="sql-with" xreflabel="WITH Clause">␟   <title><literal>WITH</literal> Clause</title>␟   <title><literal>WITH</literal>句</title>␞␞␞
␝   <para>␟    The <literal>WITH</literal> clause allows you to specify one or more
    ...
    referenced as a table by the primary query.␟<literal>WITH</literal>句により主問い合わせ内で名前により参照可能な、1つ以上の副問い合わせを指定することができます。
副問い合わせは実質的に主問い合わせの間の一時的なテーブルかビューのように動作します。
各副問い合わせは<command>SELECT</command>、<command>TABLE</command>、<command>VALUES</command>、<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>、<command>MERGE</command>文にできます。
<literal>WITH</literal>内でデータ変更文（<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>、<command>MERGE</command>）を記述する場合は、<literal>RETURNING</literal>句を含めるのが普通です。
主問い合わせで読み取られる一時テーブルを形成するのは、<literal>RETURNING</literal>の出力であり、文が変更する背後のテーブルでは<emphasis>ありません</emphasis>。
<literal>RETURNING</literal>を省いても文は実行されますが、出力を生成しませんので、主問い合わせでテーブルとして参照することができません。␞␞   </para>␞
␝   <para>␟    A name (without schema qualification) must be specified for each
    <literal>WITH</literal> query.  Optionally, a list of column names
    can be specified; if this is omitted,
    the column names are inferred from the subquery.␟（スキーマ修飾がない）名前を各<literal>WITH</literal>問い合わせで指定しなければなりません。
列名のリストをオプションで指定することもできます。
これを省略すると、列名は副問い合わせから推定されます。␞␞   </para>␞
␝   <para>␟    If <literal>RECURSIVE</literal> is specified, it allows a
    <command>SELECT</command> subquery to reference itself by name.  Such a
    subquery must have the form␟<literal>RECURSIVE</literal>が指定されると、<command>SELECT</command>副問い合わせは自身で名前により参照することができます。
こうした副問い合わせは以下のような形式でなければなりません。␞␞<synopsis>␞
␝</synopsis>␟    where the recursive self-reference must appear on the right-hand
    ...
    an example.␟ここで再帰的な自己参照は<literal>UNION</literal>の右辺に現れなければなりません。
問い合わせ当たり1つの再帰的な自己参照のみが許されます。
再帰的なデータ変更文はサポートされていませんが、データ変更文で再帰的な<command>SELECT</command>の結果を使用することができます。
例は<xref linkend="queries-with"/>を参照してください。␞␞   </para>␞
␝   <para>␟    Another effect of <literal>RECURSIVE</literal> is that
    ...
    that are earlier in the <literal>WITH</literal> list.␟<literal>RECURSIVE</literal>には他にも、<literal>WITH</literal>問い合わせが順序通りでなくても構わないという効果があります。
つまり、問い合わせはリストの後にある別のものを参照することができます。
（しかし巡回する参照や相互的な参照は実装されていません。）
<literal>RECURSIVE</literal>がないと、<literal>WITH</literal>問い合わせは主問い合わせが共通する<literal>WITH</literal>問い合わせのうち、<literal>WITH</literal>リストの前方にあるもののみを参照することができます。␞␞   </para>␞
␝   <para>␟    When there are multiple queries in the <literal>WITH</literal>
    ...
    queries that do not use recursion or forward references.␟<literal>WITH</literal>句に複数の問い合わせがある場合、<literal>RECURSIVE</literal>は<literal>WITH</literal>の直後に一度だけ書くべきです。
再帰や前方参照を使わない問い合わせには効果はないですが、<literal>WITH</literal>句内の問い合わせすべてに適用されます。␞␞   </para>␞
␝   <para>␟    The optional <literal>SEARCH</literal> clause computes a <firstterm>search
    ...
    <xref linkend="queries-with-search"/> for examples.␟省略可能な<literal>SEARCH</literal>句では、再帰問い合わせの結果を幅優先順か深さ優先順で並べるのに使える<firstterm>検索シーケンス列</firstterm>を計算します。
...
例は<xref linkend="queries-with-search"/>を参照してください。␞␞   </para>␞
␝   <para>␟    The optional <literal>CYCLE</literal> clause is used to detect cycles in
    ...
    linkend="queries-with-cycle"/> for examples.␟省略可能な<literal>CYCLE</literal>句は、再帰問い合わせで循環を検出するのに使われます。
与えられた列名のリストは、訪れた行を追跡するのに使う行キーを指定します。
...
例は<xref linkend="queries-with-cycle"/>を参照してください。␞␞   </para>␞
␝   <para>␟    Both the <literal>SEARCH</literal> and the <literal>CYCLE</literal> clause
    ...
    appears before the columns added by the <literal>CYCLE</literal> clause.␟<literal>SEARCH</literal>句、<literal>CYCLE</literal>句とも再帰的な<literal>WITH</literal>問い合わせに対してのみ有効です。
...
両方の句が使われた場合、<literal>SEARCH</literal>句で追加された列は<literal>CYCLE</literal>句で追加された列の前に現れます。␞␞   </para>␞
␝   <para>␟    The primary query and the <literal>WITH</literal> queries are all
    ...
    row, the results are unspecified.␟主問い合わせと<literal>WITH</literal>問い合わせは（理論的には）同時に実行されます。
このことは、<literal>WITH</literal>中のデータ更新文の効果は、<literal>RETURNING</literal>出力の読み込みを行ったことによるものを除き、問い合わせ中の他の部分から見えないことを意味します。
2つのそうしたデータ更新文が同じ行を更新しようとした時の結果は不定です。␞␞   </para>␞
␝   <para>␟    A key property of <literal>WITH</literal> queries is that they
    ...
    reads all or any of their output.␟<literal>WITH</literal>問い合わせの重要な特性は、これらを主問い合わせが複数回参照していたとしても、主問い合わせの実行当たり通常一度のみ評価される点です。
特にデータ変更文は、主問い合わせがその出力のすべてまたは一部を読み取るかに関係なく、本当に一度のみ実行されることが保証されています。␞␞   </para>␞
␝   <para>␟    However, a <literal>WITH</literal> query can be marked
    ...
    containing no volatile functions).␟しかし、<literal>WITH</literal>問い合わせに<literal>NOT MATERIALIZED</literal>と印を付けることにより、この保証を取り除くことができます。
...
再帰的であるか、あるいは副作用のある（すなわち揮発性の関数を含まない単純<literal>SELECT</literal>ではない）<literal>WITH</literal>問い合わせに<literal>NOT MATERIALIZED</literal>を適用しても無視されます。␞␞   </para>␞
␝   <para>␟    By default, a side-effect-free <literal>WITH</literal> query is folded
    ...
    <literal>WITH</literal> to act as an optimization fence.␟デフォルトでは、主問い合わせ中の<literal>FROM</literal>句で正確に一度だけ使われているなら、副作用のない<literal>WITH</literal>問い合わせは主問い合わせに畳み込まれます。
...
<productname>PostgreSQL</productname>バージョン12よりも前ではそうした畳込みは決して行われていなかったので、古いバージョン用に書かれた問い合わせは<literal>WITH</literal>が最適化障壁として働くことに依存しているかもしれません。␞␞   </para>␞
␝   <para>␟    See <xref linkend="queries-with"/> for additional information.␟追加情報については<xref linkend="queries-with"/>を参照してください。␞␞   </para>␞
␝␟  <refsect2 id="sql-from" xreflabel="FROM Clause">␟  <refsect2 id="sql-from" xreflabel="FROM句">␞␞␞
␝  <refsect2 id="sql-from" xreflabel="FROM Clause">␟   <title><literal>FROM</literal> Clause</title>␟   <title><literal>FROM</literal>句</title>␞␞␞
␝   <para>␟    The <literal>FROM</literal> clause specifies one or more source
    ...
    Cartesian product.␟<literal>FROM</literal>句には<command>SELECT</command>の対象となるソーステーブルを1つ以上指定します。
複数のソースが指定された場合、結果は全てのソースのデカルト積（クロス結合）となります。
しかし、通常は（<literal>WHERE</literal>を介して）制約条件を付けて、デカルト積のごく一部を返すように結果行を限定します。␞␞   </para>␞
␝   <para>␟    The <literal>FROM</literal> clause can contain the following
    elements:␟<literal>FROM</literal>句には以下の要素を指定できます。␞␞␞
␝       <para>␟        The name (optionally schema-qualified) of an existing table or view.
        ...
        indicate that descendant tables are included.␟既存のテーブルもしくはビューの名前です（スキーマ修飾名も可）。
...
省略することもできますが、テーブル名の後に<literal>*</literal>を指定することで、明示的に継承するテーブルも含まれることを示すことができます。␞␞       </para>␞
␝       <para>␟        A substitute name for the <literal>FROM</literal> item containing the
        ...
        substitute names for one or more columns of the table.␟別名を含む<literal>FROM</literal>項目の代替名です。
...
テーブルの別名があれば、そのテーブルの複数の列の名前を置き換える列の別名リストを記述することができます。␞␞       </para>␞
␝       <para>␟        A <literal>TABLESAMPLE</literal> clause after
        ...
        installed in the database via extensions.␟<replaceable class="parameter">table_name</replaceable>の後の<literal>TABLESAMPLE</literal>句は、そのテーブルの行の部分集合を取り出すときに、指定した<replaceable class="parameter">sampling_method</replaceable>を使うべきであることを示唆します。
...
他のサンプリングメソッドも拡張(extension)によりデータベースにインストールすることができます。␞␞       </para>␞
␝       <para>␟        The <literal>BERNOULLI</literal> and <literal>SYSTEM</literal> sampling methods
        ...
        the table as a result of clustering effects.␟サンプリングメソッド<literal>BERNOULLI</literal>と<literal>SYSTEM</literal>はいずれも1つだけ<replaceable class="parameter">argument</replaceable>を取り、これはテーブルからサンプリングする割合で0から100までのパーセントで表現されます。
...
<literal>BERNOULLI</literal>では、テーブル全体をスキャンし、個々の行を別々に、指定された確率に従って、選択あるいは無視します。
<literal>SYSTEM</literal>ではブロックレベルのサンプリングを行います。
各ブロックは指定された確率で選択され、選択されたブロック内のすべての行が返されます。
サンプリングに小さな割合が指定された場合、<literal>SYSTEM</literal>は<literal>BERNOULLI</literal>よりもかなり高速ですが、クラスタリング効果により、<literal>BERNOULLI</literal>に比べてランダムでないサンプルを返すかもしれません。␞␞       </para>␞
␝       <para>␟        The optional <literal>REPEATABLE</literal> clause specifies
        ...
        samples on each use.␟省略可能な<literal>REPEATABLE</literal>句では、サンプリングメソッドで乱数を生成するための<replaceable class="parameter">seed</replaceable>の数あるいは式を指定します。
...
一部のアドオンのサンプリングメソッドでは<literal>REPEATABLE</literal>が利用できず、使用の度に常に新しいサンプルを生成することに注意してください。␞␞       </para>␞
␝       <para>␟        A sub-<command>SELECT</command> can appear in the
        ...
        can also be used here.␟<literal>FROM</literal>句では、副<command>SELECT</command>を使うことができます。
<command>SELECT</command>コマンドの実行中、副<command>SELECT</command>の出力は一時テーブルであるかのように動作します。
副<command>SELECT</command>は括弧で囲まれなければなりません。また、テーブルと同じようにして別名を与えることができます。
<link linkend="sql-values"><command>VALUES</command></link>コマンドをここで使用することもできます。␞␞       </para>␞
␝       <para>␟        A <literal>WITH</literal> query is referenced by writing its name,
        just as though the query's name were a table name.  (In fact,
        the <literal>WITH</literal> query hides any real table of the same name
        for the purposes of the primary query.  If necessary, you can
        refer to a real table of the same name by schema-qualifying
        the table's name.)
        An alias can be provided in the same way as for a table.␟<literal>WITH</literal>問い合わせは、問い合わせの名前があたかもテーブル名であるかのように、名前を記述することで参照されます。
（実際には<literal>WITH</literal>問い合わせは主問い合わせの対象とするテーブルと同じ名前の実テーブルを隠蔽します。
必要ならばテーブル名をスキーマ修飾することで同じ名前の実テーブルを参照することができます。）
テーブルと同様の方法で別名を提供することができます。␞␞       </para>␞
␝       <para>␟        Function calls can appear in the <literal>FROM</literal>
        ...
        attribute becomes a separate column in the implicit table.␟<literal>FROM</literal>句では、関数呼び出しを使用できます。
（これは特に関数が結果セットを返す場合に有用ですが、任意の関数を使用できます。）
<command>SELECT</command>コマンドの実行中は、この関数の出力は一時テーブルであるかのように動作します。
関数の結果型が(複数の<literal>OUT</literal>パラメータを持つ関数の場合を含む)複合型なら、各属性はその暗黙のテーブルの別々の列になります。␞␞       </para>␞
␝       <para>␟        When the optional <command>WITH ORDINALITY</command> clause is added
        ...
        By default, this column is named <literal>ordinality</literal>.␟関数呼び出しに省略可能な<command>WITH ORDINALITY</command>句を追加した時は、<type>bigint</type>型の追加の列が関数の結果列に追加されます。
この列は関数の結果セットの行に1から始まる番号を付けます。
デフォルトでは、この列は<literal>ordinality</literal>という名前です。␞␞       </para>␞
␝       <para>␟        An alias can be provided in the same way as for a table.
        ...
        type, including the ordinality column if present.␟テーブルに対するのと同じように、別名を使用することができます。
別名が記述されていれば、列の別名リストを記述して、関数の複合型の戻り値の１つ以上の、存在する場合にはordinality列を含め、属性に対する代替名を提供することもできます。␞␞       </para>␞
␝       <para>␟        Multiple function calls can be combined into a
        ...
        function that produced the most rows.␟複数の関数呼び出しを<literal>ROWS FROM( ... )</literal>で括ることにより、1つの<literal>FROM</literal>句の項目にまとめることができます。
このような項目の出力は各関数の最初の行を結合した項目、次いで各関数の2番目の行、といった具合になります。
一部の関数が他の関数より少ない行数を出力した場合は、存在しないデータについてNULL値が代用され、戻される行数はいつでも最大の行数を返した関数と同じになります。␞␞       </para>␞
␝      <para>␟        If the function has been defined as returning the
        ...
        actual number and types of columns returned by the function.␟関数が<type>record</type>データ型を返すと定義されている場合は、別名すなわち<literal>AS</literal>キーワードと、それに続く<literal>（<replaceable class="parameter">column_name</replaceable> <replaceable class="parameter">data_type</replaceable> <optional>, ... </optional>）</literal>という形式の列定義リストが必要です。
列定義リストは、関数によって返される実際の列の数およびデータ型に一致していなければなりません。␞␞       </para>␞
␝       <para>␟        When using the <literal>ROWS FROM( ... )</literal> syntax, if one of the
        ...
        a single function and no <literal>WITH ORDINALITY</literal> clause.␟<literal>ROWS FROM( ... )</literal>の構文を使う時、関数の1つが列定義のリストを必要としている場合は、<literal>ROWS FROM( ... )</literal>内の関数呼び出しの後に列定義のリストを置くのが望ましいです。
関数が1つだけで、<literal>WITH ORDINALITY</literal>句がない場合に限り、列定義のリストを<literal>ROWS FROM( ... )</literal>の後に置くことができます。␞␞       </para>␞
␝       <para>␟        To use <literal>ORDINALITY</literal> together with a column definition
        list, you must use the <literal>ROWS FROM( ... )</literal> syntax and put the
        column definition list inside <literal>ROWS FROM( ... )</literal>.␟<literal>ORDINALITY</literal>を列定義のリストと一緒に使うには、<literal>ROWS FROM( ... )</literal>構文を使い、列定義のリストを<literal>ROWS FROM( ... )</literal>の内側に置かなければなりません。␞␞       </para>␞
␝       <para>␟        One of␟        以下のいずれかです。␞␞        <itemizedlist>␞
␝␟        For the <literal>INNER</literal> and <literal>OUTER</literal> join types, a
        ...
        or <literal>NATURAL</literal>.  See below for the meaning.␟<literal>INNER</literal>および<literal>OUTER</literal>結合型では、結合条件、すなわち、<literal>ON <replaceable class="parameter">join_condition</replaceable></literal>、<literal>USING (<replaceable class="parameter">join_column</replaceable> [, ...])</literal>、<literal>NATURAL</literal>のいずれか1つのみを指定する必要があります。
それぞれの意味は後述します。␞␞       </para>␞
␝       <para>␟        A <literal>JOIN</literal> clause combines two <literal>FROM</literal>
        ...
        <literal>FROM</literal> and <literal>WHERE</literal>.␟<literal>JOIN</literal>句は、2つの<literal>FROM</literal>項目を結び付けます。
...
なぜなら、通常の<literal>FROM</literal>と<literal>WHERE</literal>でできないことは何もしないからです。␞␞       </para>␞
␝␟       <para><literal>LEFT OUTER JOIN</literal> returns all rows in the qualified
        ...
        have matches.  Outer conditions are applied afterwards.␟<para><literal>LEFT OUTER JOIN</literal>は、条件に合うデカルト積の全ての行（つまり、その結合条件を満たす全ての組み合わせ）に加え、左側テーブルの中で、右側テーブルには結合条件を満たす行が存在しなかった行のコピーも返します。
この左側テーブルの行を結合結果のテーブルの幅に拡張するために、右側テーブルが入る列にはNULL値が挿入されます。
マッチする行を決める時は、<literal>JOIN</literal>句自身の条件のみが考慮されることに注意してください。
外部結合条件は後で適用されます。␞␞       </para>␞
␝       <para>␟        Conversely, <literal>RIGHT OUTER JOIN</literal> returns all the
        joined rows, plus one row for each unmatched right-hand row
        (extended with nulls on the left).  This is just a notational
        convenience, since you could convert it to a <literal>LEFT
        OUTER JOIN</literal> by switching the left and right tables.␟逆に、<literal>RIGHT OUTER JOIN</literal>は、全ての結合行と、左側テーブルに当てはまるものがなかった右側の行（左側はNULLで拡張されています）の1行ずつを返します。
左右のテーブルを入れ替えれば<literal>LEFT OUTER JOIN</literal>に変換できるので、<literal>RIGHT OUTER JOIN</literal>は記述上の便宜を図るため用意されているに過ぎません。␞␞       </para>␞
␝␟       <para><literal>FULL OUTER JOIN</literal> returns all the joined rows, plus
        one row for each unmatched left-hand row (extended with nulls
        on the right), plus one row for each unmatched right-hand row
        (extended with nulls on the left).␟<para><literal>FULL OUTER JOIN</literal>は、全ての結合行に加え、一致しなかった左側の行（右側はNULLで拡張）、一致しなかった右側の行（左側はNULLで拡張）を全て返します。␞␞       </para>␞
␝      <listitem>␟       <para><replaceable class="parameter">join_condition</replaceable> is
        an expression resulting in a value of type
        <type>boolean</type> (similar to a <literal>WHERE</literal>
        clause) that specifies which rows in a join are considered to
        match.␟<para><replaceable class="parameter">join_condition</replaceable>は、結合においてどの行が一致するかを指定する、<type>boolean</type>型の値を返す式です（<literal>WHERE</literal>句に類似しています）。␞␞       </para>␞
␝       <para>␟        A clause of the form <literal>USING ( a, b, ... )</literal> is
        ...
        both.␟<literal>USING ( a, b, ... )</literal>という形式の句は<literal>ON left_table.a = right_table.a AND left_table.b = right_table.b ...</literal>の省略形です。
また<literal>USING</literal>は等価な列の両方ではなく片方のみが結合の出力に含まれることを意味します。␞␞       </para>␞
␝       <para>␟        If a <replaceable class="parameter">join_using_alias</replaceable>
        ...
        same as they appear in the <literal>USING</literal> list.␟<replaceable class="parameter">join_using_alias</replaceable>名が指定されれば、結合列に対するテーブルの別名を提供します。
...
また、普通の<replaceable class="parameter">alias</replaceable>とは異なり、列の別名リストを書くことはできません &mdash; 結合列の出力名は<literal>USING</literal>リストに現れるのと同じです。␞␞       </para>␞
␝       <para>␟        <literal>NATURAL</literal> is shorthand for a
        ...
        to <literal>ON TRUE</literal>.␟<literal>NATURAL</literal>は、2つのテーブル内の同じ名前を持つ列を全て指定した<literal>USING</literal>リストの省略形です。
共通の列名がない場合、<literal>NATURAL</literal>は<literal>ON TRUE</literal>と同等になります。␞␞       </para>␞
␝       <para>␟        <literal>CROSS JOIN</literal> is equivalent to <literal>INNER JOIN ON
        ...
        but restricted by the join condition (if any).␟<literal>CROSS JOIN</literal>は<literal>INNER JOIN ON(TRUE)</literal>と同じです。
つまり、条件によって削除される行はありません。
これらは単純なデカルト積を生成します。
これは、<literal>FROM</literal>の最上位レベルにある2つのテーブルをリストした場合と同じ結果ですが、(存在すれば)結合条件によって制限されます。␞␞       </para>␞
␝       <para>␟        The <literal>LATERAL</literal> key word can precede a
        ...
        <literal>FROM</literal> item.)␟<literal>LATERAL</literal>キーワードを副<command>SELECT</command>の<literal>FROM</literal>項目の前に付けることができます。
これにより、副<command>SELECT</command>が<literal>FROM</literal>リストの中で前に現れる<literal>FROM</literal>項目の列を参照することができます。
（<literal>LATERAL</literal>がないと、副<command>SELECT</command>それぞれが個別に評価され、他の<literal>FROM</literal>項目とのクロス参照を行うことができません。）␞␞       </para>␞
...
しかしこの場合、無意味な単語になります。
関数式はどのような場合でもより前の<literal>FROM</literal>項目を参照することができるからです。␞␞       </para>␞
␝       <para>␟        A <literal>LATERAL</literal> item can appear at top level in the
        ...
        side of a <literal>JOIN</literal> that it is on the right-hand side of.␟<literal>LATERAL</literal>項目は<literal>FROM</literal>の最上位レベルや<literal>JOIN</literal>ツリー内に記述することができます。
後者の場合、<literal>JOIN</literal>の右辺にあれば、左辺にある任意の項目を参照することができます。␞␞       </para>␞
␝       <para>␟        When a <literal>FROM</literal> item contains <literal>LATERAL</literal>
        ...
        repeated for each row or set of rows from the column source table(s).␟<literal>FROM</literal>項目が<literal>LATERAL</literal>クロス参照を含む場合、評価は次のように行われます。
...
これが各行または列ソーステーブルからの行集合に対して繰り返されます。␞␞       </para>␞
␝       <para>␟        The column source table(s) must be <literal>INNER</literal> or
        ...
        <replaceable>X</replaceable>.␟列ソーステーブルは<literal>LATERAL</literal>項目と<literal>INNER</literal>または<literal>LEFT</literal>結合されていなければなりません。
さもないと、
<literal>LATERAL</literal>項目において各行集合を計算するための行集合が完全に定義することができません。
したがって<literal><replaceable>X</replaceable> RIGHT JOIN LATERAL <replaceable>Y</replaceable></literal>という式は構文としては有効ですが、実際には<replaceable>Y</replaceable>では<replaceable>X</replaceable>を参照することができません。␞␞       </para>␞
␝␟  <refsect2 id="sql-where" xreflabel="WHERE Clause">␟  <refsect2 id="sql-where" xreflabel="WHERE句">␞␞␞
␝  <refsect2 id="sql-where" xreflabel="WHERE Clause">␟   <title><literal>WHERE</literal> Clause</title>␟   <title><literal>WHERE</literal>句</title>␞␞␞
␝   <para>␟    The optional <literal>WHERE</literal> clause has the general form␟<literal>WHERE</literal>句の一般的な構文は以下の通りです（この句は省略可能です）。␞␞<synopsis>␞
␝</synopsis>␟    where <replaceable class="parameter">condition</replaceable> is
    any expression that evaluates to a result of type
    <type>boolean</type>.  Any row that does not satisfy this
    condition will be eliminated from the output.  A row satisfies the
    condition if it returns true when the actual row values are
    substituted for any variable references.␟<replaceable class="parameter">condition</replaceable>は、評価の結果として<type>boolean</type>型を返す任意の式です。
この条件を満たさない行は全て出力から取り除かれます。
全ての変数に実際の行の値を代入して、式が真を返す場合、その行は条件を満たすとみなされます。␞␞   </para>␞
␝␟  <refsect2 id="sql-groupby" xreflabel="GROUP BY Clause">␟  <refsect2 id="sql-groupby" xreflabel="GROUP BY句">␞␞␞
␝  <refsect2 id="sql-groupby" xreflabel="GROUP BY Clause">␟   <title><literal>GROUP BY</literal> Clause</title>␟   <title><literal>GROUP BY</literal>句</title>␞␞␞
␝   <para>␟    The optional <literal>GROUP BY</literal> clause has the general form␟<literal>GROUP BY</literal>句の一般的な構文は以下の通りです（この句は省略可能です）。␞␞<synopsis>␞
␝   <para>␟    <literal>GROUP BY</literal> will condense into a single row all
    ...
    input-column name rather than an output column name.␟<literal>GROUP BY</literal>は、グループ化のために与えられた式を評価し、結果が同じ値になった行を1つの行にまとめる機能を持ちます。
...
判断がつかない時は、<literal>GROUP BY</literal>の名前は出力列名ではなく入力列名として解釈されます。␞␞   </para>␞
␝   <para>␟    If any of <literal>GROUPING SETS</literal>, <literal>ROLLUP</literal> or
    ...
    of grouping sets see <xref linkend="queries-grouping-sets"/>.␟グループ化の要素として<literal>GROUPING SETS</literal>、<literal>ROLLUP</literal>、<literal>CUBE</literal>のいずれかが指定されている場合、<literal>GROUP BY</literal>句は全体でいくつかの独立した<replaceable>グループ化セット</replaceable>を定義します。
...
グループ化セットの処理の詳細については、<xref linkend="queries-grouping-sets"/>を参照してください。␞␞   </para>␞
␝   <para>␟    Aggregate functions, if any are used, are computed across all rows
    ...
    are included in the input to that aggregate function.␟集約関数が使用された場合、各グループ内の全ての行を対象に計算が行われ、グループごとに別々の値が生成されます。
...
<literal>FILTER</literal>句があると、その条件に適合する行だけが集約関数の入力行に取り込まれます。␞␞   </para>␞
␝   <para>␟    When <literal>GROUP BY</literal> is present,
    ...
    the table containing the ungrouped column.␟<literal>GROUP BY</literal>が存在する場合、あるいは集約関数が存在する場合、集約関数内部以外で、グループ化されていない列を参照する、あるいはグループ化されていない列がグループ化された列に関数依存する<command>SELECT</command>リストの式は無効になります。
こうしないとグループ化されていない列について返される値は複数の値になってしまう可能性があるからです。
グループ化された列(またはその部分集合)がグループ化されていない列を含むテーブルの主キーである場合、関数従属性が存在します。␞␞   </para>␞
␝   <para>␟    Keep in mind that all aggregate functions are evaluated before
    ...
    an aggregate function; see <xref linkend="syntax-express-eval"/>.␟すべての集約関数は、<literal>HAVING</literal>句や<literal>SELECT</literal>リストのどの<quote>スカラ</quote>式よりも先に評価されることに注意してください。
これは例えば、<literal>CASE</literal>式を集約関数の評価をスキップするために使うことはできない、ということを意味します。
<xref linkend="syntax-express-eval"/>を参照してください。␞␞   </para>␞
␝   <para>␟    Currently, <literal>FOR NO KEY UPDATE</literal>, <literal>FOR UPDATE</literal>,
    <literal>FOR SHARE</literal> and <literal>FOR KEY SHARE</literal> cannot be
    specified with <literal>GROUP BY</literal>.␟現在は、<literal>FOR NO KEY UPDATE</literal>、<literal>FOR UPDATE</literal>、<literal>FOR SHARE</literal>、<literal>FOR KEY SHARE</literal>を<literal>GROUP BY</literal>と合わせて使うことはできません。␞␞   </para>␞
␝␟  <refsect2 id="sql-having" xreflabel="HAVING Clause">␟  <refsect2 id="sql-having" xreflabel="HAVING句">␞␞␞
␝  <refsect2 id="sql-having" xreflabel="HAVING Clause">␟   <title><literal>HAVING</literal> Clause</title>␟   <title><literal>HAVING</literal>句</title>␞␞␞
␝   <para>␟    The optional <literal>HAVING</literal> clause has the general form␟<literal>HAVING</literal>句の一般的な構文は以下の通りです（この句は省略可能です）。␞␞<synopsis>␞
␝</synopsis>␟    where <replaceable class="parameter">condition</replaceable> is
    the same as specified for the <literal>WHERE</literal> clause.␟<replaceable class="parameter">condition</replaceable>は<literal>WHERE</literal>句で指定するものと同じです。␞␞   </para>␞
␝   <para>␟    <literal>HAVING</literal> eliminates group rows that do not
    ...
    functionally dependent on the grouping columns.␟<literal>HAVING</literal>は、グループ化された行の中で、条件を満たさない行を取り除く機能を持ちます。
...
<replaceable class="parameter">condition</replaceable>内で使用する列は、集約関数内で使用される場合とグループ化されない列がグループ化される列に関数依存する場合を除き、グループ化された列を一意に参照するものでなければなりません。␞␞   </para>␞
␝   <para>␟    The presence of <literal>HAVING</literal> turns a query into a grouped
    ...
    <literal>HAVING</literal> condition is true, zero rows if it is not true.␟<literal>HAVING</literal>句があると、<literal>GROUP BY</literal>句がなかったとしても問い合わせはグループ化された問い合わせになります。
...
こうした問い合わせでは、<literal>HAVING</literal>が真の場合には単一の行を、真以外の場合は0行を出力します。␞␞   </para>␞
␝   <para>␟    Currently, <literal>FOR NO KEY UPDATE</literal>, <literal>FOR UPDATE</literal>,
    <literal>FOR SHARE</literal> and <literal>FOR KEY SHARE</literal> cannot be
    specified with <literal>HAVING</literal>.␟現在は、<literal>FOR NO KEY UPDATE</literal>、<literal>FOR UPDATE</literal>、<literal>FOR SHARE</literal>、<literal>FOR KEY SHARE</literal>を<literal>HAVING</literal>と合わせて使うことはできません。␞␞   </para>␞
␝␟  <refsect2 id="sql-window" xreflabel="WINDOW Clause">␟  <refsect2 id="sql-window" xreflabel="WINDOW句">␞␞␞
␝  <refsect2 id="sql-window" xreflabel="WINDOW Clause">␟   <title><literal>WINDOW</literal> Clause</title>␟   <title><literal>WINDOW</literal>句</title>␞␞␞
␝   <para>␟    The optional <literal>WINDOW</literal> clause has the general form␟<literal>WINDOW</literal>句の一般的な構文は以下の通りです（この句は省略可能です）。␞␞<synopsis>␞
␝</synopsis>␟    where <replaceable class="parameter">window_name</replaceable> is
    a name that can be referenced from <literal>OVER</literal> clauses or
    subsequent window definitions, and
    <replaceable class="parameter">window_definition</replaceable> is␟ここで<replaceable class="parameter">window_name</replaceable>は、<literal>OVER</literal>句やこの後のウィンドウ定義で参照することができる名前です。
また、<replaceable class="parameter">window_definition</replaceable>は以下の通りです。␞␞<synopsis>␞
␝   <para>␟    If an <replaceable class="parameter">existing_window_name</replaceable>
    ...
    must not specify a frame clause.␟<replaceable class="parameter">existing_window_name</replaceable>を指定する場合、それは<literal>WINDOW</literal>リスト内のそれより前にある項目を参照しなければなりません。
...
コピーされたウィンドウはフレーム句を指定してはなりません。␞␞   </para>␞
␝   <para>␟    The elements of the <literal>PARTITION BY</literal> list are interpreted in
    ...
    and aggregation.␟<literal>PARTITION BY</literal>リストの要素は<link linkend="sql-groupby"><literal>GROUP BY</literal></link>句の要素とほとんど同じように解釈されます。
ただし、こちらは常に単純な式であり、出力列の名前や番号ではないことが異なります。
他にも違いがあり、これらの式は、通常の<literal>GROUP BY</literal>句では許されない、集約関数を含めることができるという点です。
グループ化および集約処理の後にウィンドウ処理が動作するため、これらでは許されています。␞␞   </para>␞
␝   <para>␟    Similarly, the elements of the <literal>ORDER BY</literal> list are interpreted
    ...
    or number of an output column.␟同様に、<literal>ORDER BY</literal>リストの要素は文レベルの<link linkend="sql-orderby"><literal>ORDER BY</literal></link>句の要素とほとんど同じように解釈されます。
ただし、この式は常に単純な式であり、出力列の名前や番号ではないことが異なります。␞␞   </para>␞
␝   <para>␟    The optional <replaceable class="parameter">frame_clause</replaceable> defines
    ...
    The <replaceable class="parameter">frame_clause</replaceable> can be one of␟<replaceable class="parameter">frame_clause</replaceable>を指定すると、（すべてではありませんが）フレームに依存するウィンドウ関数用の<firstterm>ウィンドウフレーム</firstterm>を定義できます。
...
␝␟    where <replaceable>frame_start</replaceable>
    and <replaceable>frame_end</replaceable> can be one of␟ここで<replaceable>frame_start</replaceable>と<replaceable>frame_end</replaceable>は以下のいずれかを取ることができます。␞␞␞
␝␟    and <replaceable>frame_exclusion</replaceable> can be one of␟また、<replaceable>frame_exclusion</replaceable>には以下のいずれかを取ることができます。␞␞␞
␝␟    If <replaceable>frame_end</replaceable> is omitted it defaults to <literal>CURRENT
    ...
    PRECEDING</literal> is not allowed.␟<replaceable>frame_end</replaceable>が省略された場合、デフォルトで<literal>CURRENT ROW</literal>となります。
...
例えば<literal>RANGE BETWEEN CURRENT ROW AND <replaceable>offset</replaceable> PRECEDING</literal>は許されません。␞␞   </para>␞
␝   <para>␟    The default framing option is <literal>RANGE UNBOUNDED PRECEDING</literal>,
    ...
    functions.␟デフォルトのフレーム化オプションは<literal>RANGE UNBOUNDED PRECEDING</literal>です。
...
また、<replaceable>offset</replaceable>が単純な定数である必要はありませんが、変数や集約関数、ウィンドウ関数を含めることはできません。␞␞   </para>␞
␝   <para>␟    The <replaceable>frame_exclusion</replaceable> option allows rows around
    ...
    default behavior of not excluding the current row or its peers.␟<replaceable>frame_exclusion</replaceable>オプションは現在行の周辺の行を、フレーム開始とフレーム終了のオプションにより含まれるものであっても、フレームから除外することができます。
...
<literal>EXCLUDE NO OTHERS</literal>は単に、現在行もそのピアも除外しないというデフォルトの振る舞いを明示的に指定します。␞␞   </para>␞
␝   <para>␟    Beware that the <literal>ROWS</literal> mode can produce unpredictable
    ...
    a given peer group will be in the frame or excluded from it.␟<literal>ORDER BY</literal>順序によりその行を一意に順序付けできない場合、<literal>ROWS</literal>モードが予期できない結果をもたらす可能性があることに注意して下さい。
<literal>RANGE</literal>および<literal>GROUPS</literal>モードは、<literal>ORDER BY</literal>順序におけるピアとなる行が同等に扱われる、すなわち、与えられたピアグループの全行がフレームに入るか除外されるように設計されています。␞␞   </para>␞
␝   <para>␟    The purpose of a <literal>WINDOW</literal> clause is to specify the
    ...
    than one window function.␟<literal>WINDOW</literal>句の目的は、問い合わせの<link linkend="sql-select-list"><command>SELECT</command>リスト</link>または<link linkend="sql-orderby"><literal>ORDER BY</literal></link>句に記載される<firstterm>ウィンドウ関数</firstterm>の動作を規定することです。
...
しかし<literal>WINDOW</literal>句は、同じウィンドウ定義が複数のウィンドウ関数で必要とされる場合に入力量を省くことができます。␞␞   </para>␞
␝   <para>␟    Currently, <literal>FOR NO KEY UPDATE</literal>, <literal>FOR UPDATE</literal>,
    <literal>FOR SHARE</literal> and <literal>FOR KEY SHARE</literal> cannot be
    specified with <literal>WINDOW</literal>.␟現在は、<literal>FOR NO KEY UPDATE</literal>、<literal>FOR UPDATE</literal>、<literal>FOR SHARE</literal>、<literal>FOR KEY SHARE</literal>を<literal>WINDOW</literal>と合わせて使うことはできません。␞␞   </para>␞
␝   <para>␟    Window functions are described in detail in
    <xref linkend="tutorial-window"/>,
    <xref linkend="syntax-window-functions"/>, and
    <xref linkend="queries-window"/>.␟ウィンドウ関数に関する詳細については<xref linkend="tutorial-window"/>、<xref linkend="syntax-window-functions"/>、<xref linkend="queries-window"/>を参照してください。␞␞   </para>␞
␝␟  <refsect2 id="sql-select-list" xreflabel="SELECT List">␟  <refsect2 id="sql-select-list" xreflabel="SELECTリスト">␞␞␞
␝  <refsect2 id="sql-select-list" xreflabel="SELECT List">␟   <title><command>SELECT</command> List</title>␟   <title><command>SELECT</command>リスト</title>␞␞␞
␝   <para>␟    The <command>SELECT</command> list (between the key words
    ...
    computed in the <literal>FROM</literal> clause.␟<command>SELECT</command>リスト（<literal>SELECT</literal>キーワードと<literal>FROM</literal>キーワードの間にあるもの）は、<command>SELECT</command>文の出力行を形成する式を指定するものです。
この式では、<literal>FROM</literal>句で処理後の列を参照することができます（通常は実際に参照します）。␞␞   </para>␞
␝   <para>␟    Just as in a table, every output column of a <command>SELECT</command>
    ...
    <literal>?column?</literal>.␟テーブルの場合と同様に、<command>SELECT</command>の出力列はすべて名前を持ちます。
...
より複雑な場合では、関数名または型名が使用されるかもしれません。さもなければ<literal>?column?</literal>のように生成される名前になるかもしれません。␞␞   </para>␞
␝   <para>␟    An output column's name can be used to refer to the column's value in
    ...
    out the expression instead.␟<literal>ORDER BY</literal>句と<literal>GROUP BY</literal>句内で列の値を参照する時も、出力列名を使用できます。
しかし、<literal>WHERE</literal>や<literal>HAVING</literal>句では使用できません。これらでは式を書かなければなりません。␞␞   </para>␞
␝   <para>␟    Instead of an expression, <literal>*</literal> can be written in
    ...
    the output column names will be the same as the table columns' names.␟リストには、選択された行の全ての列を表す省略形として、式ではなく<literal>*</literal>と書くことができます。
...
出力列名はテーブルの列名と同一になります。␞␞   </para>␞
␝   <para>␟    According to the SQL standard, the expressions in the output list should
    ...
    set-returning function.␟標準SQLによれば、出力リスト内の式は、<literal>DISTINCT</literal>、<literal>ORDER BY</literal>、<literal>LIMIT</literal>を適用する前に計算することになっています。
...
集合を返す関数を含む出力式は、ソートの後、制限の前に実際の評価が行われ、これにより<literal>LIMIT</literal>が集合を返す関数の出力を制限することになります。␞␞   </para>␞
␝    <para>␟     <productname>PostgreSQL</productname> versions before 9.6 did not provide any
     guarantees about the timing of evaluation of output expressions versus
     sorting and limiting; it depended on the form of the chosen query plan.␟<productname>PostgreSQL</productname>のバージョン9.6より前では、出力式がソートや制限に対して評価されるタイミングについて何の保証もしていませんでした。
それは選択された問い合わせの計画の形式に依存します。␞␞    </para>␞
␝␟  <refsect2 id="sql-distinct" xreflabel="DISTINCT Clause">␟  <refsect2 id="sql-distinct" xreflabel="DISTINCT句">␞␞␞
␝  <refsect2 id="sql-distinct" xreflabel="DISTINCT Clause">␟   <title><literal>DISTINCT</literal> Clause</title>␟   <title><literal>DISTINCT</literal>句</title>␞␞␞
␝   <para>␟    If <literal>SELECT DISTINCT</literal> is specified, all duplicate rows are
    removed from the result set (one row is kept from each group of
    duplicates).  <literal>SELECT ALL</literal> specifies the opposite: all rows are
    kept; that is the default.␟<literal>SELECT DISTINCT</literal>が指定されると、重複する行は全て結果セットから削除されます
（重複するグループの中で1行が保持されます）。
<literal>SELECT ALL</literal>はこの反対で、全ての行が保持されます。
デフォルトはこちらです。␞␞   </para>␞
␝   <para>␟    <literal>SELECT DISTINCT ON ( <replaceable
    ...
    example:␟<literal>SELECT DISTINCT ON ( <replaceable class="parameter">expression</replaceable> [, ...] )</literal>は指定した式が等しいと評価した各行集合の中で、最初の行のみを保持します。
...
例えば。␞␞<programlisting>␞
␝</programlisting>␟    retrieves the most recent weather report for each location.  But
    if we had not used <literal>ORDER BY</literal> to force descending order
    of time values for each location, we'd have gotten a report from
    an unpredictable time for each location.␟これは各地点の最新の気象情報を取り出します。
しかし<literal>ORDER BY</literal>を使用して各地点を時間によって降順にソートしなければ、各地点について得られる情報がいつのものかはわかりません。␞␞   </para>␞
␝   <para>␟    The <literal>DISTINCT ON</literal> expression(s) must match the leftmost
    <literal>ORDER BY</literal> expression(s).  The <literal>ORDER BY</literal> clause
    will normally contain additional expression(s) that determine the
    desired precedence of rows within each <literal>DISTINCT ON</literal> group.␟<literal>DISTINCT ON</literal>に指定する式は<literal>ORDER BY</literal>の最も左側の式と一致しなければなりません。
<literal>ORDER BY</literal>句は、通常、各<literal>DISTINCT ON</literal>グループの中での行の優先順位を決定する追加的な式を含みます。␞␞   </para>␞
␝   <para>␟    Currently, <literal>FOR NO KEY UPDATE</literal>, <literal>FOR UPDATE</literal>,
    <literal>FOR SHARE</literal> and <literal>FOR KEY SHARE</literal> cannot be
    specified with <literal>DISTINCT</literal>.␟現在は、<literal>FOR NO KEY UPDATE</literal>、<literal>FOR UPDATE</literal>、<literal>FOR SHARE</literal>、<literal>FOR KEY SHARE</literal>を<literal>DISTINCT</literal>と合わせて使うことはできません。␞␞   </para>␞
␝␟  <refsect2 id="sql-union" xreflabel="UNION Clause">␟  <refsect2 id="sql-union" xreflabel="UNION句">␞␞␞
␝  <refsect2 id="sql-union" xreflabel="UNION Clause">␟   <title><literal>UNION</literal> Clause</title>␟   <title><literal>UNION</literal>句</title>␞␞␞
␝   <para>␟    The <literal>UNION</literal> clause has this general form:␟<literal>UNION</literal>句の一般的な構文は以下の通りです。␞␞<synopsis>␞
␝<replaceable class="parameter">select_statement</replaceable> UNION [ ALL | DISTINCT ] <replaceable class="parameter">select_statement</replaceable>␟</synopsis><replaceable class="parameter">select_statement</replaceable> is
    ...
    expression.)␟</synopsis><replaceable class="parameter">select_statement</replaceable>には、<literal>ORDER BY</literal>、<literal>LIMIT</literal>、<literal>FOR NO KEY UPDATE</literal>、<literal>FOR UPDATE</literal>、<literal>FOR SHARE</literal>、<literal>FOR KEY SHARE</literal>句を持たない任意の<command>SELECT</command>文が入ります。
（<literal>ORDER BY</literal>と<literal>LIMIT</literal>は、括弧で囲めば副式として付与することができます。
括弧がない場合、これらの句は右側に置かれた入力式ではなく、<literal>UNION</literal>の結果に対して適用されてしまいます。）␞␞   </para>␞
␝   <para>␟    The <literal>UNION</literal> operator computes the set union of
    ...
    data types.␟<literal>UNION</literal>演算子は、2つの<command>SELECT</command>文が返す行の和集合を作成します。
この和集合には、2つの<command>SELECT</command>文の結果集合のいずれか（または両方）に存在する行が全て含まれています。
<literal>UNION</literal>の直接のオペランドとなる2つの<command>SELECT</command>文が返す列数は、同じでなければなりません。また、対応する列のデータ型には互換性が存在する必要があります。␞␞   </para>␞
␝   <para>␟    The result of <literal>UNION</literal> does not contain any duplicate
    ...
    default behavior of eliminating duplicate rows.␟<literal>ALL</literal>オプションが指定されていない限り、<literal>UNION</literal>の結果には重複行は含まれません。
...
重複行を除去するデフォルトの動作を明示的に指定するために<literal>DISTINCT</literal>を記述することができます。␞␞   </para>␞
␝   <para>␟    Multiple <literal>UNION</literal> operators in the same
    <command>SELECT</command> statement are evaluated left to right,
    unless otherwise indicated by parentheses.␟1つの<command>SELECT</command>文に複数の<literal>UNION</literal>演算子がある場合、括弧がない限り、それらは左から右に評価されます。␞␞   </para>␞
␝   <para>␟    Currently, <literal>FOR NO KEY UPDATE</literal>, <literal>FOR UPDATE</literal>, <literal>FOR SHARE</literal> and
    <literal>FOR KEY SHARE</literal> cannot be
    specified either for a <literal>UNION</literal> result or for any input of a
    <literal>UNION</literal>.␟現時点では、<literal>UNION</literal>の結果や<literal>UNION</literal>に対する入力に、<literal>FOR NO KEY UPDATE</literal>、<literal>FOR UPDATE</literal>、<literal>FOR SHARE</literal>、<literal>FOR KEY SHARE</literal>を指定することはできません。␞␞   </para>␞
␝␟  <refsect2 id="sql-intersect" xreflabel="INTERSECT Clause">␟  <refsect2 id="sql-intersect" xreflabel="INTERSECT句">␞␞␞
␝  <refsect2 id="sql-intersect" xreflabel="INTERSECT Clause">␟   <title><literal>INTERSECT</literal> Clause</title>␟   <title><literal>INTERSECT</literal>句</title>␞␞␞
␝   <para>␟    The <literal>INTERSECT</literal> clause has this general form:␟<literal>INTERSECT</literal>句の一般的な構文は以下の通りです。␞␞<synopsis>␞
␝<replaceable class="parameter">select_statement</replaceable> INTERSECT [ ALL | DISTINCT ] <replaceable class="parameter">select_statement</replaceable>␟</synopsis><replaceable class="parameter">select_statement</replaceable> is
    any <command>SELECT</command> statement without an <literal>ORDER
    BY</literal>, <literal>LIMIT</literal>, <literal>FOR NO KEY UPDATE</literal>, <literal>FOR UPDATE</literal>,
    <literal>FOR SHARE</literal>, or <literal>FOR KEY SHARE</literal> clause.␟</synopsis><replaceable class="parameter">select_statement</replaceable>には、<literal>ORDER BY</literal>、<literal>LIMIT</literal>、<literal>FOR NO KEY UPDATE</literal>、<literal>FOR UPDATE</literal>、<literal>FOR SHARE</literal>、<literal>FOR KEY SHARE</literal>句を持たない、任意の<command>SELECT</command>文が入ります。␞␞   </para>␞
␝   <para>␟    The <literal>INTERSECT</literal> operator computes the set
    intersection of the rows returned by the involved
    <command>SELECT</command> statements.  A row is in the
    intersection of two result sets if it appears in both result sets.␟<literal>INTERSECT</literal>は、2つの<command>SELECT</command>文が返す行の積集合を計算します。
この積集合に含まれるのは、2つの<command>SELECT</command>文の結果集合の両方に存在する行です。␞␞   </para>␞
␝   <para>␟    The result of <literal>INTERSECT</literal> does not contain any
    ...
    <literal>DISTINCT</literal> can be written to explicitly specify the
    default behavior of eliminating duplicate rows.␟<literal>ALL</literal>オプションを指定しない限り、<literal>INTERSECT</literal>の結果に重複行は含まれません。
...
重複行を除去するデフォルトの動作を明示的に指定するために<literal>DISTINCT</literal>を記述することができます。␞␞   </para>␞
␝   <para>␟    Multiple <literal>INTERSECT</literal> operators in the same
    <command>SELECT</command> statement are evaluated left to right,
    ...
    C)</literal>.␟1つの<command>SELECT</command>文に複数の<literal>INTERSECT</literal>演算子がある場合、括弧がない限り、それらは左から右に評価されます。
<literal>INTERSECT</literal>は<literal>UNION</literal>よりも強い結び付きを持ちます。
つまり、<literal>A UNION B INTERSECT C</literal> は<literal>A UNION (B INTERSECT C)</literal>と解釈されます。␞␞   </para>␞
␝   <para>␟    Currently, <literal>FOR NO KEY UPDATE</literal>, <literal>FOR UPDATE</literal>, <literal>FOR SHARE</literal> and
    <literal>FOR KEY SHARE</literal> cannot be
    specified either for an <literal>INTERSECT</literal> result or for any input of
    an <literal>INTERSECT</literal>.␟現時点では、<literal>INTERSECT</literal>の結果や<literal>INTERSECT</literal>に対する入力に、<literal>FOR NO KEY UPDATE</literal>、<literal>FOR UPDATE</literal>、<literal>FOR SHARE</literal>または<literal>FOR KEY SHARE</literal>を指定することはできません。␞␞   </para>␞
␝␟  <refsect2 id="sql-except" xreflabel="EXCEPT Clause">␟  <refsect2 id="sql-except" xreflabel="EXCEPT句">␞␞␞
␝  <refsect2 id="sql-except" xreflabel="EXCEPT Clause">␟   <title><literal>EXCEPT</literal> Clause</title>␟   <title><literal>EXCEPT</literal>句</title>␞␞␞
␝   <para>␟    The <literal>EXCEPT</literal> clause has this general form:␟<literal>EXCEPT</literal>句の一般的な構文は以下の通りです。␞␞<synopsis>␞
␝<replaceable class="parameter">select_statement</replaceable> EXCEPT [ ALL | DISTINCT ] <replaceable class="parameter">select_statement</replaceable>␟</synopsis><replaceable class="parameter">select_statement</replaceable> is
    any <command>SELECT</command> statement without an <literal>ORDER
    BY</literal>, <literal>LIMIT</literal>, <literal>FOR NO KEY UPDATE</literal>, <literal>FOR UPDATE</literal>,
    <literal>FOR SHARE</literal>, or <literal>FOR KEY SHARE</literal> clause.␟</synopsis><replaceable class="parameter">select_statement</replaceable>には、<literal>ORDER BY</literal>、<literal>LIMIT</literal>、<literal>FOR NO KEY UPDATE</literal>、<literal>FOR UPDATE</literal>、<literal>FOR SHARE</literal>、<literal>FOR KEY SHARE</literal>句を持たない、任意の<command>SELECT</command>文が入ります。␞␞   </para>␞
␝   <para>␟    The <literal>EXCEPT</literal> operator computes the set of rows
    that are in the result of the left <command>SELECT</command>
    statement but not in the result of the right one.␟<literal>EXCEPT</literal>は、左側の<command>SELECT</command>文の結果には存在し、右側の<command>SELECT</command>文の結果には存在しない行の集合を生成します。␞␞   </para>␞
␝   <para>␟    The result of <literal>EXCEPT</literal> does not contain any
    duplicate rows unless the <literal>ALL</literal> option is specified.
    With <literal>ALL</literal>, a row that has <replaceable>m</replaceable> duplicates in the
    left table and <replaceable>n</replaceable> duplicates in the right table will appear
    max(<replaceable>m</replaceable>-<replaceable>n</replaceable>,0) times in the result set.
    <literal>DISTINCT</literal> can be written to explicitly specify the
    default behavior of eliminating duplicate rows.␟<literal>ALL</literal>オプションが指定されていない限り、<literal>EXCEPT</literal>の結果には重複行は含まれません。
...
重複行を除去するデフォルトの動作を明示的に指定するために<literal>DISTINCT</literal>を記述することができます。␞␞   </para>␞
␝   <para>␟    Multiple <literal>EXCEPT</literal> operators in the same
    <command>SELECT</command> statement are evaluated left to right,
    unless parentheses dictate otherwise.  <literal>EXCEPT</literal> binds at
    the same level as <literal>UNION</literal>.␟1つの<command>SELECT</command>文に複数の<literal>EXCEPT</literal>演算子がある場合、括弧がない限り、それらは左から右に評価されます。
<literal>EXCEPT</literal>の結び付きの強さは<literal>UNION</literal>と同じです。␞␞   </para>␞
␝   <para>␟    Currently, <literal>FOR NO KEY UPDATE</literal>, <literal>FOR UPDATE</literal>, <literal>FOR SHARE</literal> and
    <literal>FOR KEY SHARE</literal> cannot be
    specified either for an <literal>EXCEPT</literal> result or for any input of
    an <literal>EXCEPT</literal>.␟現時点では、<literal>EXCEPT</literal>の結果や<literal>EXCEPT</literal>に対する入力に、<literal>FOR NO KEY UPDATE</literal>、<literal>FOR UPDATE</literal>、<literal>FOR SHARE</literal>または<literal>FOR KEY SHARE</literal>を指定することはできません。␞␞   </para>␞
␝␟  <refsect2 id="sql-orderby" xreflabel="ORDER BY Clause">␟  <refsect2 id="sql-orderby" xreflabel="ORDER BY句">␞␞␞
␝  <refsect2 id="sql-orderby" xreflabel="ORDER BY Clause">␟   <title><literal>ORDER BY</literal> Clause</title>␟   <title><literal>ORDER BY</literal>句</title>␞␞␞
␝   <para>␟    The optional <literal>ORDER BY</literal> clause has this general form:␟<literal>ORDER BY</literal>句の一般的な構文は以下の通りです（この句は省略可能です）。␞␞<synopsis>␞
␝</synopsis>␟    The <literal>ORDER BY</literal> clause causes the result rows to
    ...
    an implementation-dependent order.␟<literal>ORDER BY</literal>句を使うと、結果行を指定した式（複数可）に従ってソートすることができます。
最も左側の式を使って比較した結果、2つの行が等しいと判断された場合は、1つ右側の式を使って比較します。その結果も等しければ、さらに次の式に進みます。
指定した全ての式で等しいと判断された場合は、実装に依存した順番で返されます。␞␞   </para>␞
␝   <para>␟    Each <replaceable class="parameter">expression</replaceable> can be the
    name or ordinal number of an output column
    (<command>SELECT</command> list item), or it can be an arbitrary
    expression formed from input-column values.␟<replaceable class="parameter">expression</replaceable>には、出力列（<command>SELECT</command>リスト項目）の名前または序数、あるいは入力列値から形成される任意の式を取ることができます。␞␞   </para>␞
␝   <para>␟    The ordinal number refers to the ordinal (left-to-right) position
    of the output column. This feature makes it possible to define an
    ordering on the basis of a column that does not have a unique
    name.  This is never absolutely necessary because it is always
    possible to assign a name to an output column using the
    <literal>AS</literal> clause.␟序数は、出力列の位置（左から右に割り当てられます）を示します。
これを使うと、一意な名前を持たない列の順序を定義することができます。
<literal>AS</literal>句を使用すれば出力列に名前を割り当てることができるので、これはどうしても必要な機能というわけではありません。␞␞   </para>␞
␝   <para>␟    It is also possible to use arbitrary expressions in the
    ...
    following statement is valid:␟また、<literal>ORDER BY</literal>句には、<command>SELECT</command>出力リストに出現しない列を含む、任意の式を使用できます。
したがって、以下の文は有効です。␞␞<programlisting>␞
␝</programlisting>␟    A limitation of this feature is that an <literal>ORDER BY</literal>
    ...
    specify an output column name or number, not an expression.␟ただし、<literal>UNION</literal>、<literal>INTERSECT</literal>、<literal>EXCEPT</literal>句の結果に<literal>ORDER BY</literal>を適用する場合は、式は使用できず、出力列の名前か序数のみを指定できるという制限があります。␞␞   </para>␞
␝   <para>␟    If an <literal>ORDER BY</literal> expression is a simple name that
    ...
    compatible with the SQL standard.␟<literal>ORDER BY</literal>の式として出力列名と入力列名の両方に一致する単なる名前が与えられた場合、<literal>ORDER BY</literal>はそれを出力列名として扱います。
これは、同じ状況における<literal>GROUP BY</literal>の選択とは反対です。
この不整合は、標準SQLとの互換性を保持するために発生しています。␞␞   </para>␞
␝   <para>␟    Optionally one can add the key word <literal>ASC</literal> (ascending) or
    ...
    names.)␟<literal>ORDER BY</literal>中の任意の式の後に、キーワード<literal>ASC</literal>（昇順）、<literal>DESC</literal>（降順）を付加することができます（省略可能）。
...
（ただし、ユーザ定義データ型の作成時には、デフォルトのソート順を定義することができます。また、異なる名前の演算子と対応付けすることもできます。）␞␞   </para>␞
␝   <para>␟    If <literal>NULLS LAST</literal> is specified, null values sort after all
    ...
    on whether the operator is a less-than or greater-than operator.␟<literal>NULLS LAST</literal>が指定されると、NULL値はすべての非NULL値の後にソートされます。
...
<literal>USING</literal>が指定されると、デフォルトのNULLの順序は、演算子が小なり演算子か大なり演算子によって変わります。␞␞   </para>␞
␝   <para>␟    Note that ordering options apply only to the expression they follow;
    for example <literal>ORDER BY x, y DESC</literal> does not mean
    the same thing as <literal>ORDER BY x DESC, y DESC</literal>.␟順序付けオプションは直前の演算子にのみ適用されます。
たとえば、<literal>ORDER BY x, y DESC</literal>は<literal>ORDER BY x DESC, y DESC</literal>と同一の意味ではありません。␞␞   </para>␞
␝   <para>␟    Character-string data is sorted according to the collation that applies
    ...
    <xref linkend="collation"/>.␟文字型データでは、格納する列に適用された照合順序に従ってソートされます。
...
より詳細については<xref linkend="sql-syntax-collate-exprs"/>および<xref linkend="collation"/>を参照してください。␞␞   </para>␞
␝␟  <refsect2 id="sql-limit" xreflabel="LIMIT Clause">␟  <refsect2 id="sql-limit" xreflabel="LIMIT句">␞␞␞
␝  <refsect2 id="sql-limit" xreflabel="LIMIT Clause">␟   <title><literal>LIMIT</literal> Clause</title>␟   <title><literal>LIMIT</literal>句</title>␞␞␞
␝   <para>␟    The <literal>LIMIT</literal> clause consists of two independent
    sub-clauses:␟<literal>LIMIT</literal>句は2つの独立した副句から構成されます。␞␞<synopsis>␞
␝</synopsis>␟    The parameter <replaceable class="parameter">count</replaceable> specifies the
    ...
    class="parameter">count</replaceable> rows to be returned.␟パラメータ<replaceable class="parameter">count</replaceable>には返される行の最大数を、一方、<replaceable class="parameter">start</replaceable>には行を返し始める前に飛ばす行数を指定します。
両方とも指定された場合、<replaceable class="parameter">start</replaceable>行分が飛ばされ、そこから数えて<replaceable class="parameter">count</replaceable>行が返されます。␞␞   </para>␞
␝   <para>␟    If the <replaceable class="parameter">count</replaceable> expression
    ...
    to NULL, it is treated the same as <literal>OFFSET 0</literal>.␟<replaceable class="parameter">count</replaceable>式がNULLと評価された場合、<literal>LIMIT ALL</literal>として、つまり制限無しとして扱われます。
<replaceable class="parameter">start</replaceable>がNULLと評価された場合、<literal>OFFSET 0</literal>と同様に扱われます。␞␞   </para>␞
␝   <para>␟    SQL:2008 introduced a different syntax to achieve the same result,
    which <productname>PostgreSQL</productname> also supports.  It is:␟SQL:2008では同じ結果を実現する異なる構文が導入されました。
<productname>PostgreSQL</productname>でもサポートしています。
以下の構文です。␞␞<synopsis>␞
␝</synopsis>␟    In this syntax, the <replaceable class="parameter">start</replaceable>
    ...
    <productname>PostgreSQL</productname> is laxer and allows either order.␟この構文において、<replaceable class="parameter">start</replaceable>または<replaceable class="parameter">count</replaceable>の値は標準SQLでは、リテラル定数、パラメータもしくは変数名を要求します。
...
標準SQLでは<literal>OFFSET</literal>句は、<literal>FETCH</literal>句と同時に使用する場合、これより前に存在しなければなりません。
しかし<productname>PostgreSQL</productname>は厳密ではなく、どちらが先でも許されます。␞␞   </para>␞
␝   <para>␟    When using <literal>LIMIT</literal>, it is a good idea to use an
    ...
    don't know what ordering unless you specify <literal>ORDER BY</literal>.␟<literal>LIMIT</literal>を使う時は、結果行を一意な順番に強制する<literal>ORDER BY</literal>句を使うとよいでしょう。
そうしないと、問い合わせ結果のどの部分が返されるのかがわかりません。
10〜20行目までを出力するとしても、どの順番で並べた時の10〜20行目なのでしょうか。
<literal>ORDER BY</literal>を指定しない限り、行が返される順番は不明です。␞␞   </para>␞
␝   <para>␟    The query planner takes <literal>LIMIT</literal> into account when
    ...
    <literal>ORDER BY</literal> is used to constrain the order.␟問い合わせプランナは問い合わせ計画を作成する時に<literal>LIMIT</literal>を考慮するので、<literal>LIMIT</literal>と<literal>OFFSET</literal>の指定によって異なった計画を得ることになるでしょう。計画が異なれば、異なる順番で行が返ります。
...
これはバグではありません。
「SQLは、<literal>ORDER BY</literal>で順序を制御されない限り、問い合わせ結果が返す順序を約束しない」という事実の当然の帰結なのです。␞␞   </para>␞
␝   <para>␟    It is even possible for repeated executions of the same <literal>LIMIT</literal>
    ...
    simply not guaranteed in such a case.␟厳密的に部分集合の選択を強制する<literal>ORDER BY</literal>がなければ、同じ<literal>LIMIT</literal>問い合わせを繰り返し実行してもテーブル行から異なる部分集合が取り出される可能性すらあります。
これも不具合ではありません。
こうした場合に確定した結果は単に保証されていないのです。␞␞   </para>␞
␝␟  <refsect2 id="sql-for-update-share" xreflabel="The Locking Clause">␟  <refsect2 id="sql-for-update-share" xreflabel="ロック処理句">␞␞␞
␝  <refsect2 id="sql-for-update-share" xreflabel="The Locking Clause">␟   <title>The Locking Clause</title>␟   <title>ロック処理句</title>␞␞␞
␝   <para>␟    <literal>FOR UPDATE</literal>, <literal>FOR NO KEY UPDATE</literal>, <literal>FOR SHARE</literal>
    and <literal>FOR KEY SHARE</literal>
    are <firstterm>locking clauses</firstterm>; they affect how <literal>SELECT</literal>
    locks rows as they are obtained from the table.␟<literal>FOR UPDATE</literal>、<literal>FOR NO KEY UPDATE</literal>、<literal>FOR SHARE</literal>および<literal>FOR KEY SHARE</literal>は<firstterm>ロック処理句</firstterm>です。
これらはテーブルから行を入手する時にどのように<literal>SELECT</literal>がその行をロックするかに影響します。␞␞   </para>␞
␝   <para>␟    The locking clause has the general form␟ロック処理句の一般的な構文は以下の通りです。␞␞␞
␝␟    where <replaceable>lock_strength</replaceable> can be one of␟ここで<replaceable>lock_strength</replaceable>は以下のいずれかを取ることができます。␞␞␞
␝   <para>␟    <replaceable class="parameter">from_reference</replaceable> must be a
    ...
    row-level lock mode, refer to <xref linkend="locking-rows"/>.␟<replaceable class="parameter">from_reference</replaceable>は、<literal>FROM</literal>句で参照されるテーブルの<replaceable class="parameter">alias</replaceable>または隠されていない<replaceable class="parameter">table_name</replaceable>でなければなりません。
それぞれの行レベルロックモードについての詳しい説明は<xref linkend="locking-rows"/>を参照してください。␞␞   </para>␞
␝   <para>␟    To prevent the operation from waiting for other transactions to commit,
    ...
    if you need to acquire the table-level lock without waiting.␟他のトランザクションのコミットを待機することなく操作を進めるには、<literal>NOWAIT</literal>あるいは<literal>SKIP LOCKED</literal>オプションを使用してください。
...
もし、テーブルレベルのロックを待機せずに獲得しなければならないのであれば、最初に<link linkend="sql-lock"><command>LOCK</command></link>の<literal>NOWAIT</literal>オプションを使用してください。␞␞   </para>␞
␝   <para>␟    If specific tables are named in a locking clause,
    ...
    a locking clause within the <literal>WITH</literal> query.␟ロック処理句内に特定のテーブルが指定されている場合は、そのテーブルの行のみがロックされます。
...
<literal>WITH</literal>問い合わせ内での行ロックを行いたい場合は、<literal>WITH</literal>問い合わせ内でロック処理句を指定してください。␞␞   </para>␞
␝   <para>␟    Multiple locking
    ...
    clauses affecting it.␟異なるロック方式を異なるテーブルに指定する必要があれば、複数のロック処理句を記述することができます。
...
それ以外の場合、あるテーブルに影響を与える句のいずれかで<literal>SKIP LOCKED</literal>が指定されていれば、そのテーブルは<literal>SKIP LOCKED</literal>として処理されます。␞␞   </para>␞
␝   <para>␟    The locking clauses cannot be
    used in contexts where returned rows cannot be clearly identified with
    individual table rows; for example they cannot be used with aggregation.␟ロック処理句は、返される行がテーブルのどの行に対応するのかが明確に識別できない場合には使用することができません。
例えば、集約には使用できません。␞␞   </para>␞
␝   <para>␟    When a locking clause
    ...
    by the cursor will be locked.␟ロック処理句が<command>SELECT</command>問い合わせの最上位レベルに存在する場合、ロック対象行は問い合わせが返す行に正確に一致します。
...
同様に、ロック処理句がカーソル問い合わせで使用された場合、カーソルにより実際に取り込んだ行または通り過ぎた行のみがロックされます。␞␞   </para>␞
␝   <para>␟    When a locking clause
    ...
    execution of the sub-query.  For example,␟ロック処理句が副<command>SELECT</command>に存在する場合、ロック対象行は副問い合わせの外側の問い合わせに返される行となります。
外側の問い合わせからの条件が副問い合わせ実行の最適化に使用される可能性がありますので、これには副問い合わせ自体の検査が提示する行より少なくなるかもしれません。
例えば、␞␞<programlisting>␞
␝</programlisting>␟    will lock only rows having <literal>col1 = 5</literal>, even though that
    condition is not textually within the sub-query.␟は、副問い合わせ内では文字として条件が記載されていなくても、<literal>col1 = 5</literal>を持つ行のみがロックされます。␞␞   </para>␞
␝  <para>␟   Previous releases failed to preserve a lock which is upgraded by a later
   savepoint.  For example, this code:␟以前のリリースでは、セーブポイント以降に更新されるロックの保持は失敗しました。
例えば以下のコードです。␞␞<programlisting>␞
␝</programlisting>␟   would fail to preserve the <literal>FOR UPDATE</literal> lock after the
   <command>ROLLBACK TO</command>.  This has been fixed in release 9.3.␟<command>ROLLBACK TO</command>後の<literal>FOR UPDATE</literal>ロックの保持に失敗します。
これはリリース9.3で修正されました。␞␞  </para>␞
␝   <para>␟    It is possible for a <command>SELECT</command> command running at the <literal>READ
    ...
    for example␟<literal>ORDER BY</literal>句とロック処理句を使用した、<literal>READ COMMITTED</literal>トランザクション分離レベルで実行する<command>SELECT</command>コマンドでは、順序通りにならない行を返す可能性があります。
...
必要に応じて、これは以下のように副問い合わせ内に<literal>FOR UPDATE/SHARE</literal>句を記述することで、回避することができます。␞␞<programlisting>␞
␝</programlisting>␟    Note that this will result in locking all rows of <structname>mytable</structname>,
    ...
    strictly sorted result is required.␟最上位レベルにおける<literal>FOR UPDATE</literal>は実際に返される行のみをロックするのに対して、これは結果として<structname>mytable</structname>のすべての行をロックすることに注意してください。
これは、特に<literal>ORDER BY</literal>が<literal>LIMIT</literal>やその他の制限と組み合わせている場合、性能上大きな違いを生み出す可能性があります。
このため、この技法は、順序付け対象の列に対する同時実行の更新が想定され、かつ、厳密にソートされた結果が要求される場合にのみ推奨されます。␞␞   </para>␞
␝   <para>␟    At the <literal>REPEATABLE READ</literal> or <literal>SERIALIZABLE</literal>
    ...
    no possibility of receiving rows out of order under these isolation levels.␟<literal>REPEATABLE READ</literal>または<literal>SERIALIZABLE</literal>トランザクション分離レベルでは、（<literal>'40001'</literal>という<literal>SQLSTATE</literal>を持つ）シリアライゼーション失敗が発生します。
このためこれらの分離レベルでは順序通りでない行を受け取る可能性はありません。␞␞   </para>␞
␝  <refsect2 id="sql-table">␟   <title><literal>TABLE</literal> Command</title>␟   <title><literal>TABLE</literal>コマンド</title>␞␞␞
␝   <para>␟    The command␟コマンド␞␞<programlisting>␞
␝</programlisting>␟    is equivalent to␟は以下と同じです。␞␞<programlisting>␞
␝</programlisting>␟    It can be used as a top-level command or as a space-saving syntax
    ...
    aggregation cannot
    be used.␟これは、最上位のコマンドとして、あるいは複雑な問い合わせの一部として、入力を省略する構文の一種としても使用することができます。
...
<literal>WHERE</literal>句およびいかなる形式の集約も使うことはできません。␞␞   </para>␞
␝ <refsect1>␟  <title>Examples</title>␟  <title>例</title>␞␞␞
␝  <para>␟   To join the table <literal>films</literal> with the table
   <literal>distributors</literal>:␟<literal>films</literal>テーブルを<literal>distributors</literal>テーブルと結合します。␞␞␞
␝  <para>␟   To sum the column <literal>len</literal> of all films and group
   the results by <literal>kind</literal>:␟全ての映画の<literal>len</literal>列を合計し<literal>kind</literal>列によって結果をグループ化します。␞␞␞
␝  <para>␟   To sum the column <literal>len</literal> of all films, group
   the results by <literal>kind</literal> and show those group totals
   that are less than 5 hours:␟全ての映画の<literal>len</literal>列を合計し<literal>kind</literal>列によって結果をグループ化し、合計が5時間より少ないグループの合計を表示します。␞␞␞
␝  <para>␟   The following two examples are identical ways of sorting the individual
   results according to the contents of the second column
   (<literal>name</literal>):␟次に、結果を2番目の列（<literal>name</literal>）の内容に基づいてソートする方法を2つ例示します。␞␞␞
␝  <para>␟   The next example shows how to obtain the union of the tables
   <literal>distributors</literal> and
   <literal>actors</literal>, restricting the results to those that begin
   with the letter W in each table.  Only distinct rows are wanted, so the
   key word <literal>ALL</literal> is omitted.␟次の例は、<literal>distributors</literal>テーブルと<literal>actors</literal>テーブルの和集合を取得する方法を示しています。さらに、両方のテーブルで結果をWという文字で始まる行のみに限定しています。
重複しない行のみが必要なので、<literal>ALL</literal>キーワードは省略されています。␞␞␞
␝  <para>␟   This example shows how to use a function in the <literal>FROM</literal>
   clause, both with and without a column definition list:␟次に、<literal>FROM</literal>句内での関数の使用方法について、列定義リストがある場合とない場合の両方の例を示します。␞␞␞
␝  <para>␟   Here is an example of a function with an ordinality column added:␟以下は序数列が追加された関数の例です。␞␞␞
␝  <para>␟   This example shows how to use a simple <literal>WITH</literal> clause:␟以下の例では簡単な<literal>WITH</literal>句の使用方法を示します。␞␞␞
␝␟   Notice that the <literal>WITH</literal> query was evaluated only once,
   so that we got two sets of the same three random values.␟<literal>WITH</literal>問い合わせが一度だけ評価されることに注意してください。
このため3つのランダムな値の同じ集合2組を得ることになります。␞␞  </para>␞
␝  <para>␟   This example uses <literal>WITH RECURSIVE</literal> to find all
   subordinates (direct or indirect) of the employee Mary, and their
   level of indirectness, from a table that shows only direct
   subordinates:␟以下の例では<literal>WITH RECURSIVE</literal>を使用して、直接の部下しか表示しないテーブルから、従業員Maryの（直接または間接的な）部下とその間接度を見つけ出します。␞␞␞
␝␟   Notice the typical form of recursive queries:
   an initial condition, followed by <literal>UNION</literal>,
   followed by the recursive part of the query. Be sure that the
   recursive part of the query will eventually return no tuples, or
   else the query will loop indefinitely.  (See <xref linkend="queries-with"/>
   for more examples.)␟初期条件、続いて<literal>UNION</literal>、さらに問い合わせの再帰部分という再帰問い合わせの典型的な構文に注意してください。
問い合わせの再帰部分は最終的にはタプルを返さないことを確実にしてください。
さもないと問い合わせは無限にループします。
（より多くの例については<xref linkend="queries-with"/>を参照してください。）␞␞  </para>␞
␝  <para>␟   This example uses <literal>LATERAL</literal> to apply a set-returning function
   <function>get_product_names()</function> for each row of the
   <structname>manufacturers</structname> table:␟以下の例では、<structname>manufacturers</structname>テーブルの各行に対して集合を返す<function>get_product_names()</function>関数を適用するために<literal>LATERAL</literal>を使用します。␞␞␞
␝␟    Manufacturers not currently having any products would not appear in the
    ...
    such manufacturers in the result, we could do:␟これは内部結合ですので、現時点で製品をまったく持たないメーカーは結果に現れません。
こうしたメーカーの名前も結果に含めたければ以下のようにします。␞␞␞
␝ <refsect1>␟  <title>Compatibility</title>␟  <title>互換性</title>␞␞␞
␝  <para>␟   Of course, the <command>SELECT</command> statement is compatible
   with the SQL standard.  But there are some extensions and some
   missing features.␟当然ながら、<command>SELECT</command>文は標準SQLと互換性があります。
しかし、拡張機能や実現されていない機能もいくつかあります。␞␞  </para>␞
␝  <refsect2>␟   <title>Omitted <literal>FROM</literal> Clauses</title>␟   <title><literal>FROM</literal>句の省略</title>␞␞␞
␝   <para>␟    <productname>PostgreSQL</productname> allows one to omit the
    <literal>FROM</literal> clause.  It has a straightforward use to
    compute the results of simple expressions:␟<productname>PostgreSQL</productname>では、<literal>FROM</literal>句を省略することができます。
これによって、以下のように単純な式を計算させることができます。␞␞<programlisting>␞
␝</programlisting>␟    Some other <acronym>SQL</acronym> databases cannot do this except
    by introducing a dummy one-row table from which to do the
    <command>SELECT</command>.␟他の<acronym>SQL</acronym>データベースでは、このような<command>SELECT</command>を行うためにはダミーの1行テーブルを使わなければならないものもあります。␞␞   </para>␞
␝  <refsect2>␟   <title>Empty <literal>SELECT</literal> Lists</title>␟   <title>空の<literal>SELECT</literal>リスト</title>␞␞␞
␝   <para>␟    The list of output expressions after <literal>SELECT</literal> can be
    ...
    However, an empty list is not allowed when <literal>DISTINCT</literal> is used.␟<literal>SELECT</literal>の後の出力式のリストは空でも良く、このとき列数がゼロの結果テーブルが生成されます。
これは標準SQLでは有効な構文ではありません。
<productname>PostgreSQL</productname>は列数がゼロのテーブルを許すので、それと整合性を保つために許しています。
しかし、<literal>DISTINCT</literal>を使う時は、空のリストを使うことはできません。␞␞   </para>␞
␝  <refsect2>␟   <title>Omitting the <literal>AS</literal> Key Word</title>␟   <title><literal>AS</literal>キーワードの省略</title>␞␞␞
␝   <para>␟    In the SQL standard, the optional key word <literal>AS</literal> can be
    ...
    any possible conflict against future keyword additions.␟標準SQLでは、キーワード<literal>AS</literal>（省略可能）は、新しい列名が有効な列名（つまり予約済みのどのキーワードとも異なるもの）である場合は常に、出力列名の前から省くことができます。
<productname>PostgreSQL</productname>には多少より強い制限があります。
新しい列名が予約済みか否かに関わらず何らかのキーワードに一致する場合は<literal>AS</literal>が必要です。
推奨する実践方法は、今後のキーワードの追加と競合する可能性に備え、<literal>AS</literal>を使用する、または出力列名を二重引用符で括ることです。␞␞   </para>␞
␝   <para>␟    In <literal>FROM</literal> items, both the standard and
    ...
    ambiguities.␟<literal>FROM</literal>項目において標準および<productname>PostgreSQL</productname>では、未予約のキーワードである別名の前の<literal>AS</literal>を省略することができます。
しかし、構文があいまいになるため、出力列名では実践的ではありません。␞␞   </para>␞
␝  <refsect2>␟   <title>Omitting Sub-<command>SELECT</command> Aliases in <literal>FROM</literal></title>␟   <title><literal>FROM</literal>での副<command>SELECT</command>の別名の省略</title>␞␞␞
␝   <para>␟    According to the SQL standard, a sub-<command>SELECT</command> in the
    <literal>FROM</literal> list must have an alias.  In
    <productname>PostgreSQL</productname>, this alias may be omitted.␟標準SQLでは、<literal>FROM</literal>リスト内の副<command>SELECT</command>には別名が必要です。
<productname>PostgreSQL</productname>では、この別名を省略できます。␞␞   </para>␞
␝  <refsect2>␟   <title><literal>ONLY</literal> and Inheritance</title>␟   <title><literal>ONLY</literal>と継承関係</title>␞␞␞
␝   <para>␟    The SQL standard requires parentheses around the table name when
    ...
    considers these parentheses to be optional.␟標準SQLでは、<literal>SELECT * FROM ONLY (tab1), ONLY (tab2) WHERE ...</literal>のように、<literal>ONLY</literal>を記述する時にテーブル名の前後を括弧でくくることを要求します。
<productname>PostgreSQL</productname>ではこの括弧を省略可能であるとみなしています。␞␞   </para>␞
␝   <para>␟    <productname>PostgreSQL</productname> allows a trailing <literal>*</literal> to be written to
    explicitly specify the non-<literal>ONLY</literal> behavior of including
    child tables.  The standard does not allow this.␟<productname>PostgreSQL</productname>では最後に<literal>*</literal>を付けることで
明示的に子テーブルを含めるという<literal>ONLY</literal>ではない動作を指定することができます。
標準ではこれを許していません。␞␞   </para>␞
␝   <para>␟    (These points apply equally to all SQL commands supporting the
    <literal>ONLY</literal> option.)␟（これらの点は<literal>ONLY</literal>オプションをサポートするすべてのSQLコマンドで同様に適用されます。）␞␞   </para>␞
␝  <refsect2>␟   <title><literal>TABLESAMPLE</literal> Clause Restrictions</title>␟   <title><literal>TABLESAMPLE</literal>句の制限</title>␞␞␞
␝   <para>␟    The <literal>TABLESAMPLE</literal> clause is currently accepted only on
    regular tables and materialized views.  According to the SQL standard
    it should be possible to apply it to any <literal>FROM</literal> item.␟現在のところ、<literal>TABLESAMPLE</literal>句は通常のテーブルとマテリアライズドビューでのみ受け付けられます。
標準SQLでは、<literal>FROM</literal>句の任意の要素について適用可能であるべきとされています。␞␞   </para>␞
␝  <refsect2>␟   <title>Function Calls in <literal>FROM</literal></title>␟   <title><literal>FROM</literal>内の関数呼び出し</title>␞␞␞
␝   <para>␟    <productname>PostgreSQL</productname> allows a function call to be
    ...
    same as other set-returning functions.␟<productname>PostgreSQL</productname>では、<literal>FROM</literal>リストのメンバとして直接関数呼び出しを記述することができます。
...
<productname>PostgreSQL</productname>では<literal>UNNEST()</literal>を他の集合を返す関数と同じものとして扱います。␞␞   </para>␞
␝  <refsect2>␟   <title>Namespace Available to <literal>GROUP BY</literal> and <literal>ORDER BY</literal></title>␟   <title><literal>GROUP BY</literal>と<literal>ORDER BY</literal>における利用可能な名前空間</title>␞␞␞
␝   <para>␟    In the SQL-92 standard, an <literal>ORDER BY</literal> clause can
    ...
    output-column names.␟標準SQL-92では、<literal>ORDER BY</literal>句で使用できるのは、出力列名か序数のみであり、<literal>GROUP BY</literal>句で使用できるのは、入力列名からなる式のみです。
...
式で使われる名前は、常に出力列名ではなく入力列の名前とみなされることに注意してください。␞␞   </para>␞
␝   <para>␟    SQL:1999 and later use a slightly different definition which is not
    entirely upward compatible with SQL-92.
    In most cases, however, <productname>PostgreSQL</productname>
    will interpret an <literal>ORDER BY</literal> or <literal>GROUP
    BY</literal> expression the same way SQL:1999 does.␟SQL:1999以降では、SQL-92と完全には上位互換でない、多少異なる定義が採用されています。
しかし、ほとんどの場合、<productname>PostgreSQL</productname>はSQL:1999と同じ方法で<literal>ORDER BY</literal>や<literal>GROUP BY</literal>を解釈します。␞␞   </para>␞
␝  <refsect2>␟   <title>Functional Dependencies</title>␟   <title>関数従属性</title>␞␞␞
␝   <para>␟    <productname>PostgreSQL</productname> recognizes functional dependency
    ...
    recognized.␟テーブルの主キーが<literal>GROUP BY</literal>リストに含まれる場合に限り、<productname>PostgreSQL</productname>は（<literal>GROUP BY</literal>で列を省くことができる）関数従属性を認識します。
標準SQLでは、認識しなければならない追加の条件を規定しています。␞␞   </para>␞
␝  <refsect2>␟   <title><literal>LIMIT</literal> and <literal>OFFSET</literal></title>␟   <title><literal>LIMIT</literal>および<literal>OFFSET</literal></title>␞␞␞
␝   <para>␟    The clauses <literal>LIMIT</literal> and <literal>OFFSET</literal>
    ...
    PostgreSQL, to implement the effects of these clauses.)␟<literal>LIMIT</literal>および<literal>OFFSET</literal>句は<productname>PostgreSQL</productname>独自の構文ですが、<productname>MySQL</productname>でも使用されています。
...
（<productname>Oracle</productname>用に開発されたアプリケーションでは、これらの句の機能を実装するために自動生成される<literal>rownum</literal>列を含めるという回避策を使用することが多いですが、PostgreSQLでは利用できません。）␞␞   </para>␞
␝  <refsect2>␟   <title><literal>FOR NO KEY UPDATE</literal>, <literal>FOR UPDATE</literal>, <literal>FOR SHARE</literal>, <literal>FOR KEY SHARE</literal></title>␟   <title><literal>FOR NO KEY UPDATE</literal>、<literal>FOR UPDATE</literal>、<literal>FOR SHARE</literal>、<literal>FOR KEY SHARE</literal></title>␞␞␞
␝   <para>␟    Although <literal>FOR UPDATE</literal> appears in the SQL standard, the
    ...
    standard.␟<literal>FOR UPDATE</literal>は標準SQLに存在しますが、標準では、<command>DECLARE CURSOR</command>のオプションとしてしか許されていません。
<productname>PostgreSQL</productname>では、副<command>SELECT</command>など任意の<command>SELECT</command>で許されます。
これは拡張です。
<literal>FOR NO KEY UPDATE</literal>、<literal>FOR SHARE</literal>、<literal>FOR KEY SHARE</literal>の亜種、および<literal>NOWAIT</literal>と<literal>SKIP LOCKED</literal>オプションは標準にはありません。␞␞   </para>␞
␝  <refsect2>␟   <title>Data-Modifying Statements in <literal>WITH</literal></title>␟   <title><literal>WITH</literal>内のデータ変更文</title>␞␞␞
␝   <para>␟    <productname>PostgreSQL</productname> allows <command>INSERT</command>,
    <command>UPDATE</command>, <command>DELETE</command>, and
    <command>MERGE</command> to be used as <literal>WITH</literal>
    queries.  This is not found in the SQL standard.␟<productname>PostgreSQL</productname>では<literal>WITH</literal>問い合わせとして<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>および<command>MERGE</command>を使用できます。
これは標準SQLにはありません。␞␞   </para>␞
␝  <refsect2>␟   <title>Nonstandard Clauses</title>␟   <title>非標準句</title>␞␞␞
␝   <para>␟    <literal>DISTINCT ON ( ... )</literal> is an extension of the
    SQL standard.␟<literal>DISTINCT ON ( ... )</literal>は標準SQLの拡張です。␞␞   </para>␞
␝   <para>␟    <literal>ROWS FROM( ... )</literal> is an extension of the SQL standard.␟<literal>ROWS FROM( ... )</literal>は標準SQLの拡張です。␞␞   </para>␞
␝   <para>␟    The <literal>MATERIALIZED</literal> and <literal>NOT
    MATERIALIZED</literal> options of <literal>WITH</literal> are extensions
    of the SQL standard.␟<literal>WITH</literal>の<literal>MATERIALIZED</literal>と<literal>NOT MATERIALIZED</literal>オプションは標準SQLの拡張です。␞␞   </para>␞
␝␟<para><literal>[ INNER ] JOIN</literal></para> <para><literal>[ INNER ] JOIN</literal></para>␟no translation␞␞␞
␝␟<para><literal>LEFT [ OUTER ] JOIN</literal></para> <para><literal>LEFT [ OUTER ] JOIN</literal></para>␟no translation␞␞␞
␝␟<para><literal>RIGHT [ OUTER ] JOIN</literal></para> <para><literal>RIGHT [ OUTER ] JOIN</literal></para>␟no translation␞␞␞
␝␟<para><literal>FULL [ OUTER ] JOIN</literal></para> <para><literal>FULL [ OUTER ] JOIN</literal></para>␟no translation␞␞␞
␝␟SELECT m.name AS mname, pname FROM manufacturers m LEFT JOIN LATERAL get_product_names(m.id) pname ON true; </programlisting></para> </programlisting></para>␟no translation␞␞␞
