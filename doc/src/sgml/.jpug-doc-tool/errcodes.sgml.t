...
␝<appendix id="errcodes-appendix">␟ <title><productname>PostgreSQL</productname> Error Codes</title>␟ <title><productname>PostgreSQL</productname>エラーコード</title>␞␞␞
␝ <para>␟  All messages emitted by the <productname>PostgreSQL</productname>
  ...
  borrowed from other databases.␟<productname>PostgreSQL</productname>サーバによって発行されるメッセージは全て、標準SQLにおける<quote>SQLSTATE</quote>コードの記述方法に従った、5文字のエラーコードが割り当てられています。
...
標準SQLで定義されていない追加のエラーコードは、独自のものであったり他のデータベースから取り入れたものです。␞␞ </para>␞
␝ <para>␟  According to the standard, the first two characters of an error code
  ...
  what to do from the error class.␟標準に従い、エラーコードの最初の2文字はエラーのクラスを表し、残り3文字がそのクラス内の特定条件を表します。
したがって、特定のエラーコードを認識できないアプリケーションであっても、エラークラスから何をすべきか推定できることがあります。␞␞ </para>␞
␝ <para>␟  <xref linkend="errcodes-table"/> lists all the error codes defined in
  ...
  within the class but do not have any more-specific code assigned.␟<xref linkend="errcodes-table"/>は、<productname>PostgreSQL</productname> &version;で定義されたエラーコードを全て一覧で示しています。
（標準SQLでは定義されていますが、現在実際に使用されていないものもあります。）
エラークラスも示しています。
各エラークラスには、残りの3文字が<literal>000</literal>となる<quote>標準</quote>エラーコードが存在します。
あるクラスの範囲内で発生したが、より特定のコードが割り当てられていないエラー条件のためだけに、このコードが使用されます。␞␞ </para>␞
␝ <para>␟  The symbol shown in the column <quote>Condition Name</quote> is
  ...
  condition names; those are classes 00, 01, and 02.)␟<quote>条件名</quote>列に示されているシンボルは<application>PL/pgSQL</application>で使用している条件名です。
条件名は大文字もしくは小文字でも記述されます。（<application>PL/pgSQL</application>は、エラーの場合と異なり、警告の場合にはその状態名を認識しません。これらはクラス00と01と02です）。␞␞ </para>␞
␝ <para>␟  For some types of errors, the server reports the name of a database object
  ...
  violation), but this is likely to be expanded in future.␟いくつかのエラー型に対し、サーバはそのエラーに起因するデータベースオブジェクト（テーブル、テーブル列、データ型、もしくは制約）の名前を報告します。たとえば、<symbol>unique_violation</symbol>エラーの原因となった一意性制約の名前です。
...
␝<table id="errcodes-table">␟ <title><productname>PostgreSQL</productname> Error Codes</title>␟ <title><productname>PostgreSQL</productname>エラーコード</title>␞␞␞
␝   <row>␟    <entry>Error Code</entry>␟    <entry>エラーコード</entry>␞␞␞
␝    <entry>Error Code</entry>␟    <entry>Condition Name</entry>␟    <entry>条件名</entry>␞␞   </row>␞
