␝<sect1 id="pgwalinspect" xreflabel="pg_walinspect">␟ <title>pg_walinspect &mdash; low-level WAL inspection</title>␟ <title>pg_walinspect &mdash; 低レベルのWAL検査</title>␞␞␞
␝ <para>␟  The <filename>pg_walinspect</filename> module provides SQL functions that
  ...
  accessible through SQL rather than a separate utility.␟<filename>pg_walinspect</filename>モジュールは、実行中の<productname>PostgreSQL</productname>データベースクラスタの先行書き込みログ（WAL）の内容を低レベルで検査することを可能にするSQL関数を提供します。
これはデバッグ、分析、報告、教育目的に有用です。
<xref linkend="pgwaldump"/>と似ていますが、独立したユーティリティではなくSQLを通してアクセスできます。␞␞ </para>␞
␝ <para>␟  All the functions of this module will provide the WAL information using the
  server's current timeline ID.␟このモジュールのすべての関数は、サーバの現在のタイムラインIDを使用してWAL情報を提供します。␞␞ </para>␞
␝  <para>␟   The <filename>pg_walinspect</filename> functions are often called
   ...
   inserted.␟<filename>pg_walinspect</filename>関数は、しばしば、関心のある既知のWALレコードの<emphasis>開始</emphasis>位置を指定するLSN引数を使用して呼び出されます。
しかし、<function><link linkend="pg-logical-emit-message">pg_logical_emit_message</link></function>のようないくつかの関数は、挿入されたレコードの<emphasis>後</emphasis>のLSNを返します。␞␞  </para>␞
␝  <para>␟   All of the <filename>pg_walinspect</filename> functions that show
   ...
   will not raise an error.␟あるLSN範囲内にあるレコードに関する情報を表示する<filename>pg_walinspect</filename>関数はすべて、サーバの現在のLSNより後の<replaceable>end_lsn</replaceable>引数を受け入れることに対して寛容です。
<quote>未来の</quote><replaceable>end_lsn</replaceable>を使用することはエラーを引き起こしません。␞␞  </para>␞
␝  <para>␟   It may be convenient to provide the value
   ...
   current LSN.␟<literal>FFFFFFFF/FFFFFFFF</literal>（最大有効<type>pg_lsn</type>値）を<replaceable>end_lsn</replaceable>引数として指定すると便利です。
これは、サーバの現在のLSNに一致する<replaceable>end_lsn</replaceable>引数を指定することと同じです。␞␞  </para>␞
␝ <para>␟  By default, use of these functions is restricted to superusers and members of
  the <literal>pg_read_server_files</literal> role. Access may be granted by
  superusers to others using <command>GRANT</command>.␟デフォルトでは、これらの関数の使用はスーパーユーザと<literal>pg_read_server_files</literal>ロールのメンバに制限されています。
スーパーユーザは<command>GRANT</command>を使用して他のユーザにアクセスを許可できます。␞␞ </para>␞
␝ <sect2 id="pgwalinspect-funcs">␟  <title>General Functions</title>␟  <title>一般的な関数</title>␞␞␞
␝     <para>␟      Gets WAL record information about a record that is located at or
      after the <replaceable>in_lsn</replaceable> argument.  For
      example:␟<replaceable>in_lsn</replaceable>引数以降のレコードに関するWALレコード情報を取得します。
例を示します。␞␞<screen>␞
␝     <para>␟      If <replaceable>in_lsn</replaceable> isn't at the start of a WAL
      record, information about the next valid WAL record is shown
      instead.  If there is no next valid WAL record, the function
      raises an error.␟<replaceable>in_lsn</replaceable>がWALレコードの先頭にない場合、次の有効なWALレコードに関する情報が代わりに表示されます。
次の有効なWALレコードがない場合、関数はエラーを発生します。␞␞     </para>␞
␝     <para>␟      Gets information of all the valid WAL records between
      <replaceable>start_lsn</replaceable> and <replaceable>end_lsn</replaceable>.
      Returns one row per WAL record.  For example:␟<replaceable>start_lsn</replaceable>と<replaceable>end_lsn</replaceable>の間のすべての有効なWALレコードの情報を取得します。
...
␝     <para>␟      The function raises an error if
      <replaceable>start_lsn</replaceable> is not available.␟<replaceable>start_lsn</replaceable>が利用できない場合、この関数はエラーを発生します。␞␞     </para>␞
␝     <para>␟      Gets information about each block reference from all the valid
      ...
      For example:␟<replaceable>start_lsn</replaceable>と<replaceable>end_lsn</replaceable>の間のすべての有効なWALレコードから、1つ以上のブロック参照を持つすべてのブロック参照に関する情報を取得します。
WALレコードのブロック参照ごとに1行返します。
例を示します。␞␞<screen>␞
␝     <para>␟      This example involves a WAL record that only contains one block
      ...
      <replaceable>block_id</replaceable> values.␟この例は、1つのブロック参照のみを含むWALレコードを含んでいますが、多くのWALレコードには複数のブロック参照が含まれています。
 <function>pg_get_wal_block_info</function>によって出力される行は、一意の<replaceable>start_lsn</replaceable>と<replaceable>block_id</replaceable>値の組み合わせを持つことが保証されています。␞␞     </para>␞
␝     <para>␟      Much of the information shown here matches the output that
      ...
      <function>pg_get_wal_records_info</function>.␟ここに示す情報の多くは、<function>pg_get_wal_records_info</function>に同じ引数を与えた場合に出力される出力と一致します。
...
ブロック参照を持たないレコード（例えば、<literal>COMMIT</literal>WALレコード）は行を返さないので、<function>pg_get_wal_block_info</function>は実際には<function>pg_get_wal_records_info</function>よりも<emphasis>少ない</emphasis>行を返すかもしれません。␞␞     </para>␞
␝     <para>␟      The <structfield>reltablespace</structfield>,
...
␝     <para>␟      It is possible for clients to avoid the overhead of
      ...
      block data isn't truly required.␟クライアントは、ブロックデータの取り出しによるオーバーヘッドを回避することができます。
...
明らかに、この最適化はブロックデータが本当に必要でない問い合わせでのみ許されます。␞␞     </para>␞
␝     <para>␟      The function raises an error if
      <replaceable>start_lsn</replaceable> is not available.␟<replaceable>start_lsn</replaceable>が利用できない場合、この関数はエラーを発生します。␞␞     </para>␞
␝     <para>␟      Gets statistics of all the valid WAL records between
      <replaceable>start_lsn</replaceable> and
      ...
      For example:␟<replaceable>start_lsn</replaceable>と<replaceable>end_lsn</replaceable>の間のすべての有効なWALレコードの統計を取得します。
...
␝     <para>␟      The function raises an error if
      <replaceable>start_lsn</replaceable> is not available.␟<replaceable>start_lsn</replaceable>が利用できない場合、この関数はエラーを発生します。␞␞     </para>␞
␝ <sect2 id="pgwalinspect-author">␟  <title>Author</title>␟  <title>作者</title>␞␞␞
␝␟Bharath Rupireddy <email>bharath.rupireddyforpostgres@gmail.com</email>␟no translation␞␞␞
