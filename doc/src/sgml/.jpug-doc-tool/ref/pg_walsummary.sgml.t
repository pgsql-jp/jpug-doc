␝  <refname>pg_walsummary</refname>␟  <refpurpose>print contents of WAL summary files</refpurpose>␟  <refpurpose>WAL要約ファイルの内容を表示する</refpurpose>␞␞ </refnamediv>␞
␝ <refsect1>␟  <title>Description</title>␟  <title>説明</title>␞␞  <para>␞
␝  <para>␟   <application>pg_walsummary</application> is used to print the contents of
   ...
   but it may be useful for debugging purposes.␟<application>pg_walsummary</application>は、WAL要約ファイルの内容を出力するために使用します。
これらのバイナリファイルは、データディレクトリの<literal>pg_wal/summaries</literal>サブディレクトリにあり、このツールを使用してテキストに変換できます。
これは通常は必要ありません。
WAL要約ファイルは主に<link linkend="backup-incremental-backup">増分バックアップ</link>をサポートするために存在しますが、デバッグ目的で有用な場合があります。␞␞  </para>␞
␝  <para>␟   A WAL summary file is indexed by tablespace OID, relation OID, and relation
   ...
   block is undefined or infinite and will not be printed by this tool.␟WAL要約ファイルは、テーブル空間OID、リレーションOID、およびリレーションフォークによってインデックス付けされます。
...
リレーションフォークが関連するWAL範囲内で作成、削除、または切り捨てられていない場合、limitブロックは未定義または無限であり、このツールでは出力されません。␞␞  </para>␞
...
␝       <para>␟        By default, <literal>pg_walsummary</literal> prints one line of output
        ...
        output for every modified block.␟デフォルトでは、<literal>pg_walsummary</literal>は、連続する1つ以上の変更されたブロックの範囲ごとに1行の出力を出力します。
これにより、0から999までのすべてのブロックが変更されたリレーションは、1000の別々の行ではなく、1行の出力しか生成しないため、出力を非常に簡潔にすることができます。
このオプションは、変更されたブロックごとに別々の行を出力するように要求します。␞␞       </para>␞
␝       <para>␟        Do not print any output, except for errors. This can be useful
        when you want to know whether a WAL summary file can be successfully
        parsed but don't care about the contents.␟エラー以外の出力を表示しません。
これは、WAL要約ファイルが正常に解析できたかどうかを知りたいがレコードの内容を気にしない場合には有用でしょう。␞␞       </para>␞
␝       <para>␟        Display version information, then exit.␟バージョン情報を表示して終了します。␞␞       </para>␞
␝       <para>␟       Shows help about <application>pg_walsummary</application> command line
       arguments, and exits.␟<application>pg_walsummary</application>コマンドライン引数の使用方法を表示し、終了します。␞␞       </para>␞
