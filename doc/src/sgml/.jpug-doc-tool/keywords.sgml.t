...
␝<appendix id="sql-keywords-appendix">␟ <title><acronym>SQL</acronym> Key Words</title>␟ <title><acronym>SQL</acronym>キーワード</title>␞␞␞
␝ <para>␟  <xref linkend="keywords-table"/> lists all tokens that are key words
  ...
  those and the other intermediate standard versions are small.)␟<xref linkend="keywords-table"/>に標準SQLと<productname>PostgreSQL</productname> &version;においてキーワードとされているすべてのトークンを示します。
この背景となる情報は<xref linkend="sql-syntax-identifiers"/>にあります。
（スペースの都合上、標準SQLのより新しい２つのバージョンと歴史的な比較のためのSQL-92のみを含めています。
これらと他の標準の中間のバージョンの違いはわずかです。）␞␞ </para>␞
␝ <para>␟  SQL distinguishes between <firstterm>reserved</firstterm> and
  ...
  is attached to a word in some contexts.␟SQLは<firstterm>予約</firstterm>と<firstterm>未予約</firstterm>キーワードを区別します。
...
未予約キーワードの本質的な存在意義は、ある文脈においては前もって定義された意味があることを宣言することだけです。␞␞ </para>␞
␝ <para>␟  In the <productname>PostgreSQL</productname> parser, life is a bit
  ...
  word).␟<productname>PostgreSQL</productname>の構文解析の過程では、少々複雑になります。
複数の異なったトークンのクラスがあり、それらは識別子としてまったく使用できないものから、普通の識別子と見なされるものの、パーサ内で特別なステータスを持たないものまであります。
（後者は一般的にはSQLで規定された関数です。）
<productname>PostgreSQL</productname>では予約キーワードは完全に予約されたものではなく、列ラベルとして使用することができます（例：<token>CHECK</token>は予約キーワードですが、<literal>SELECT 55 AS CHECK</literal>のようにすることができます）。␞␞ </para>␞
␝ <para>␟  In <xref linkend="keywords-table"/> in the column for
  ...
  ordinary identifier by <productname>PostgreSQL</productname>.␟<xref linkend="keywords-table"/>の<productname>PostgreSQL</productname>の列では、パーサがはっきりと理解しているキーワードではあるが、列名やテーブル名としても使用できるものを<quote>未予約</quote>としています。
...
列が空白のエントリは、<productname>PostgreSQL</productname>が通常の識別子として扱う単語であることを示します。␞␞ </para>␞
␝ <para>␟  Furthermore, while most key words can be used as <quote>bare</quote>
  ...
  are marked in the table as <quote>requires <literal>AS</literal></quote>.␟さらに、（<xref linkend="queries-column-labels"/>に記載されているように）ほとんどのキーワードはその前に<literal>AS</literal>を書かなくても<quote>裸の</quote>列ラベルとして使用することができますが、曖昧さを避けるために前置<literal>AS</literal>を必要とするものが多少あります。
これらは表の中で<quote><literal>AS</literal>が必要</quote>と印を付けられています。␞␞ </para>␞
␝ <para>␟  As a general rule, if you get spurious parser errors for commands
  that use any of the listed key words as an identifier, you should
  try quoting the identifier to see if the problem goes away.␟一般的な規則として、以下に示すキーワードのいずれかを識別子として使うコマンドで、おかしなパーサエラーが発生した場合、その識別子を引用符でくくって問題が解決するかどうか確認してください。␞␞ </para>␞
␝ <para>␟  It is important to understand before studying <xref
  ...
  presence of a key word does not indicate the existence of a feature.␟<xref linkend="keywords-table"/>を見る前に、<productname>PostgreSQL</productname>において予約されていないキーワードが、その単語に関連する機能を実装していないということを意味していないことを理解しておいてください。
逆に、キーワードがあるということも機能が存在することを意味していません。␞␞ </para>␞
