...
␝<chapter id="runtime-config">␟  <title>Server Configuration</title>␟  <title>サーバ設定</title>␞␞␞
␝  <para>␟   There are many configuration parameters that affect the behavior of
   the database system. In the first section of this chapter we
   describe how to interact with configuration parameters. The subsequent sections
   discuss each parameter in detail.␟データベースシステムの動作に影響を与える数多くのパラメータがあります。
この章の最初の節で、どのように設定パラメータを操作するのかについて説明します。
引き続く節で、それぞれのパラメータの詳細を説明します。␞␞  </para>␞
␝  <sect1 id="config-setting">␟   <title>Setting Parameters</title>␟   <title>パラメータの設定</title>␞␞␞
␝   <sect2 id="config-setting-names-values">␟    <title>Parameter Names and Values</title>␟    <title>パラメータ名とその値</title>␞␞␞
␝    <para>␟     All parameter names are case-insensitive. Every parameter takes a
     ...
     parameter:␟全てのパラメータの名前は大文字と小文字を区別しません。
それぞれのパラメータは、論理値、整数、浮動小数点、文字列、またはenum（列挙型）の5つの型のいずれかの値を取ります。
型はパラメータをセットするための記法を定義します。␞␞    </para>␞
␝      <para>␟       <emphasis>Boolean:</emphasis>
       ...
       <literal>1</literal>,
       <literal>0</literal>
       (all case-insensitive) or any unambiguous prefix of one of these.␟<emphasis>論理型:</emphasis>
値は<literal>on</literal>、<literal>off</literal>、<literal>true</literal>、<literal>false</literal>、<literal>yes</literal>、<literal>no</literal>、<literal>1</literal>、<literal>0</literal>（すべて大文字小文字の区別なし）、あるいは、曖昧でなければ、これらの先頭から数文字を省略形として使うこともできます。␞␞      </para>␞
␝      <para>␟       <emphasis>String:</emphasis>
       In general, enclose the value in single quotes, doubling any single
       quotes within the value.  Quotes can usually be omitted if the value
       is a simple number or identifier, however.
       (Values that match an SQL keyword require quoting in some contexts.)␟<emphasis>文字列型:</emphasis>
一般に、単一引用符の中に値を入れます。
単一引用符を値に含める場合は単一引用符を２つ続けます。
なお、値が単純な数字や識別子である場合は、通常は引用符を省略できます。
（使用する場所によっては、SQLキーワードと一致する値に引用符が必要になることがあります。）␞␞      </para>␞
␝      <para>␟       <emphasis>Numeric (integer and floating point):</emphasis>
       ...
       Quotes are not required, except for hexadecimal input.␟<emphasis>数値型(整数型と浮動小数点型):</emphasis>
...
16進数入力を除き引用符は必要ありません。␞␞      </para>␞
␝      <para>␟       <emphasis>Numeric with Unit:</emphasis>
       ...
       the numeric value and the unit.␟<emphasis>単位付きの数値:</emphasis>
...
␝         <para>␟          Valid memory units are <literal>B</literal> (bytes),
          <literal>kB</literal> (kilobytes),
          <literal>MB</literal> (megabytes), <literal>GB</literal>
          (gigabytes), and <literal>TB</literal> (terabytes).
          The multiplier for memory units is 1024, not 1000.␟有効なメモリの単位は<literal>B</literal> (バイト) <literal>kB</literal> (キロバイト)、<literal>MB</literal> (メガバイト)、 <literal>GB</literal> (ギガバイト), <literal>TB</literal> (テラバイト)です。
メモリ単位の乗数は1024です。1000ではありません。␞␞         </para>␞
␝         <para>␟          Valid time units are
          <literal>us</literal> (microseconds),
          <literal>ms</literal> (milliseconds),
          <literal>s</literal> (seconds), <literal>min</literal> (minutes),
          <literal>h</literal> (hours), and <literal>d</literal> (days).␟有効な時間の単位は <literal>us</literal> (マイクロ秒)、<literal>ms</literal> (ミリ秒)、<literal>s</literal> (秒)、<literal>min</literal> (分)、<literal>h</literal> (時間)、<literal>d</literal> (日数) です。␞␞         </para>␞
␝␟       If a fractional value is specified with a unit, it will be rounded
       ...
       occurs after any unit conversion.␟単位に添えて小数点以下が指定された場合、より小さな単位が存在すれば、値はその小さな単位の積に丸められます。
たとえば、<literal>30.1 GB</literal>は<literal>32319628902 B</literal>ではなく<literal>30822 MB</literal>に変換されます。
整数型のパラメータでは、単位変換の後で最終的な整数への丸めが行われます。␞␞      </para>␞
␝      <para>␟       <emphasis>Enumerated:</emphasis>
       ...
       Enum parameter values are case-insensitive.␟<emphasis>列挙型:</emphasis>
...
列挙型の値は大文字小文字を区別しません。␞␞      </para>␞
␝   <sect2 id="config-setting-configuration-file">␟    <title>Parameter Interaction via the Configuration File</title>␟    <title>設定ファイルによるパラメータ操作</title>␞␞␞
␝    <para>␟     The most fundamental way to set these parameters is to edit the file
     ...
     An example of what this file might look like is:␟これらのパラメータを設定する最も基本的な方法は、<filename>postgresql.conf</filename><indexterm><primary>postgresql.conf</primary></indexterm>ファイルを編集することで、これは通常 data ディレクトリに格納されています。
デフォルトのコピーはデータベースクラスタディレクトリが初期化されるときそこにインストールされます。このファイルがどういったものかの例を示します。␞␞<programlisting>␞
␝</programlisting>␟     One parameter is specified per line. The equal sign between name and
     ...
     all but the last one are ignored.␟1つの行毎に1つのパラメータが指定されます。
名前と値の間の等号は省略可能です。
...
ファイル中、同じパラメータに対して複数のエントリが指定されている場合は、最後のエントリ以外は無視されます。␞␞    </para>␞
␝    <para>␟     Parameters set in this way provide default values for the cluster.
     ...
     administrator or user can override these defaults.␟この方法によりクラスタに対してデフォルト値が設定されます。
上書きされない限り、アクティブなセッションが見るのはこの値です。
次節以降では、管理者やユーザがこれらのデフォルト値を上書きする方法を説明します。␞␞    </para>␞
␝     </indexterm>␟     The configuration file is reread whenever the main server process
     ...
     ignored (but logged) during <systemitem>SIGHUP</systemitem> processing.␟設定ファイルは、メインサーバプロセスが<systemitem>SIGHUP</systemitem>シグナルを受け取るたびに再読み込みされます。
...
設定ファイル内で無効なパラメータが設定された場合も、同じように（ログには残りますが）<systemitem>SIGHUP</systemitem> 処理中は無視されます。␞␞    </para>␞
␝    <para>␟     In addition to <filename>postgresql.conf</filename>,
     ...
     in <filename>postgresql.conf</filename>.␟<filename>postgresql.conf</filename>に加え、<productname>PostgreSQL</productname>のデータディレクトリには <filename>postgresql.auto.conf</filename><indexterm><primary>postgresql.auto.conf</primary></indexterm>というファイルがあります。
...
<filename>postgresql.auto.conf</filename>は、<filename>postgresql.conf</filename>の設定を上書きします。␞␞    </para>␞
␝    <para>␟     External tools may also
     ...
     (as <command>ALTER SYSTEM</command> will).␟外部ツールも<filename>postgresql.auto.conf</filename>を変更するかも知れません。
...
そのようなツールは、単に新しい設定を最後に追加するか、重複した設定あるいはコメント(<command>ALTER SYSTEM</command>が行います)を削除することを選択するかも知れません。␞␞    </para>␞
␝    <para>␟     The system view
     ...
     desired effects.␟システムビューの<link linkend="view-pg-file-settings"><structname>pg_file_settings</structname></link>は、設定ファイルへの変更を前もってテストしたい場合や、<systemitem>SIGHUP</systemitem>シグナルで望み通りの効果がなかった場合に問題を調査する際に役立ちます。␞␞    </para>␞
␝   <sect2 id="config-setting-sql">␟    <title>Parameter Interaction via SQL</title>␟    <title>SQLを通じたパラメータ操作</title>␞␞␞
␝     <para>␟      <productname>PostgreSQL</productname> provides three SQL
      ...
      on a per-database or per-role basis:␟<productname>PostgreSQL</productname>は3つのSQLコマンドでデフォルト値を設定します。
すでに説明した<command>ALTER SYSTEM</command>コマンドは、SQLによってグローバルな設定値を変更する方法を提供します; <filename>postgresql.conf</filename>を編集するのと等価です。これに加え、データベース単位あるいはロール単位で設定するためのコマンドがあります:␞␞     </para>␞
␝      <para>␟       The <link linkend="sql-alterdatabase"><command>ALTER DATABASE</command></link> command allows global
       settings to be overridden on a per-database basis.␟<link linkend="sql-alterdatabase"><command>ALTER DATABASE</command></link>コマンドはデータベース単位でグローバルな設定値を上書きします。␞␞      </para>␞
␝      <para>␟       The <link linkend="sql-alterrole"><command>ALTER ROLE</command></link> command allows both global and
       per-database settings to be overridden with user-specific values.␟<link linkend="sql-alterrole"><command>ALTER ROLE</command></link>コマンドはグローバルと、データベース単位の両方をユーザ固有の設定値で上書きします。␞␞      </para>␞
␝     <para>␟      Values set with <command>ALTER DATABASE</command> and <command>ALTER ROLE</command>
      ...
      so cannot be set with these commands (or the ones listed below).␟<command>ALTER DATABASE</command>と<command>ALTER ROLE</command>による設定値は新しくデータベースセッションを開始した時にのみ適用されます。
これらのコマンドは設定ファイルやサーバのコマンドラインから取得した値を上書きし、セッションの残りの部分のデフォルト値を構成します。
なお、一部の設定はサーバを起動した後では変更できず、これらのコマンドを使っては設定できません（以下に記述するコマンドでも同じことが言えます）。␞␞    </para>␞
␝     <para>␟      Once a client is connected to the database, <productname>PostgreSQL</productname>
      provides two additional SQL commands (and equivalent functions) to
      interact with session-local configuration settings:␟クライアントがデータベースに接続すると、<productname>PostgreSQL</productname>では更に2つのSQL(そして同等の関数)を使ってセッションローカルの設定変更を行うことができます。␞␞    </para>␞
␝     <para>␟      The <link linkend="sql-show"><command>SHOW</command></link> command allows inspection of the
      current value of any parameter.  The corresponding SQL function is
      <function>current_setting(setting_name text)</function>
      (see <xref linkend="functions-admin-set"/>).␟<link linkend="sql-show"><command>SHOW</command></link>コマンドを使ってすべてのパラメータの現在の値を調べることができます。
対応する関数は<function>current_setting(setting_name text)</function>です（<xref linkend="functions-admin-set"/>を参照してください）。␞␞     </para>␞
␝      <para>␟       The <link linkend="sql-set"><command>SET</command></link> command allows modification of the
       ...
       (see <xref linkend="functions-admin-set"/>).␟<link linkend="sql-set"><command>SET</command></link>コマンドでセッション内でローカルに変更できるパラメータの値を変更することができます。他のセッションには影響がありません。
...
（<xref linkend="functions-admin-set"/>を参照してください）。␞␞      </para>␞
␝    <para>␟     In addition, the system view <link
     linkend="view-pg-settings"><structname>pg_settings</structname></link> can be
     used to view and change session-local values:␟更にシステムビューの<link linkend="view-pg-settings"><structname>pg_settings</structname></link>を使ってセッションローカルな値を参照したり変更することができます。␞␞    </para>␞
␝      <para>␟       Querying this view is similar to using <command>SHOW ALL</command> but
       provides more detail.  It is also more flexible, since it's possible
       to specify filter conditions or join against other relations.␟このビューを問い合わせるのは、<command>SHOW ALL</command>を使うのと同じですが、更に詳細な情報を提供します。
フィルタ条件を指定したり他のリレーションと結合ができるので、より柔軟です。␞␞      </para>␞
␝      <para>␟       Using <command>UPDATE</command> on this view, specifically
       updating the <structname>setting</structname> column, is the equivalent
       of issuing <command>SET</command> commands.  For example, the equivalent of␟このビューに対して<command>UPDATE</command>を実行する、具体的には<structname>setting</structname>列を更新することは、<command>SET</command>コマンドを実行するのと同等です。
たとえば、␞␞<programlisting>␞
␝</programlisting>␟       is:␟は以下と同じです。␞␞<programlisting>␞
␝   <sect2 id="config-setting-shell">␟    <title>Parameter Interaction via the Shell</title>␟    <title>シェルによるパラメータ操作</title>␞␞␞
␝     <para>␟      In addition to setting global defaults or attaching
      ...
      accept parameter values via the shell.␟グローバルなデフォルト値を設定したりデータベース、ロール単位で上書きを行えるだけでなく、シェル機能を使って<productname>PostgreSQL</productname>に設定値を渡すことができます。
サーバも<application>libpq</application>クライアントライブラリもシェル経由でパラメータ値を受けとることができます。␞␞     </para>␞
␝      <para>␟       During server startup, parameter settings can be
       ...
       <option>--name=value</option> variation.  For example,␟サーバ起動時に、<option>-c</option>コマンドラインパラメータか、それと同等で別の形式の<option>--name=value</option>を使ってパラメータ設定値を<command>postgres</command>に渡すことができます。たとえば、␞␞<programlisting>␞
␝</programlisting>␟       Settings provided in this way override those set via
       <filename>postgresql.conf</filename> or <command>ALTER SYSTEM</command>,
       so they cannot be changed globally without restarting the server.␟このようにして渡された設定値は、<filename>postgresql.conf</filename>や<command>ALTER SYSTEM</command>による設定を上書きします。
したがってサーバを再起動しない限りこれらの設定値をグローバルに変更することはできません。␞␞     </para>␞
␝     <para>␟      When starting a client session via <application>libpq</application>,
      ...
      <literal>--</literal>, before the name must be specified. For example,␟<application>libpq</application>を使ってクライアントセッションを開始するときに<envar>PGOPTIONS</envar>環境変数を使って設定値を指定できます。
このようにして渡された設定値はセッションのデフォルトとなりますが、他のセッションには影響を与えません。
歴史的な理由により、<envar>PGOPTIONS</envar>の形式は<command>postgres</command>を起動するときのものと似ています。とりわけ、<option>-c</option>を指定するか、先頭に<literal>--</literal>を付加した名前を指定しなければならない点です。たとえば、␞␞<programlisting>␞
␝     <para>␟      Other clients and libraries might provide their own mechanisms,
      via the shell or otherwise, that allow the user to alter session
      settings without direct use of SQL commands.␟他のクライアントやライブラリではそれぞれ固有の方法でシェルなどを経由して、SQLコマンドを直接使わずにセッションの設定を変更することができるかもしれません。␞␞     </para>␞
␝   <sect2 id="config-includes">␟    <title>Managing Configuration File Contents</title>␟    <title>設定ファイルの内容の管理</title>␞␞␞
␝      <indexterm>
...
␝     <para>␟      <productname>PostgreSQL</productname> provides several features for breaking
      ...
      with related, but not identical, configurations.␟<productname>PostgreSQL</productname>は複雑な<filename>postgresql.conf</filename>ファイルを複数の小さなファイルに分割する複数の方法を提供しています。
これは、とりわけお互いに関連しているものの設定が同じではない複数のサーバを管理する際に有用です。␞␞     </para>␞
␝      </indexterm>␟      In addition to individual parameter settings,
      ...
      separate parts.  Include directives simply look like:␟パラメータ設定に加え、<filename>postgresql.conf</filename>ファイルに<firstterm>includeディレクティブ</firstterm>を入れることができます。
...
␝      separate parts.  Include directives simply look like:␟<programlisting>
include 'filename'
...
␝</programlisting>␟      If the file name is not an absolute path, it is taken as relative to
      the directory containing the referencing configuration file.
      Inclusions can be nested.␟ファイル名が絶対パスでない場合、参照する設定ファイルを含むディレクトリからの相対パスであると受け取られます。
Includeコマンドは入れ子にすることができます。␞␞     </para>␞
␝      <indexterm>
...
␝      </indexterm>␟      There is also an <literal>include_if_exists</literal> directive, which acts
      ...
      processing the referencing configuration file.␟<literal>include_if_exists</literal>ディレクティブもあります。
これは参照ファイルが存在しないか、または読み込むことができない場合の動作を除き、<literal>include</literal>ディレクティブと同一の動作をします。
通常の<literal>include</literal>はこれをエラーと解釈しますが、<literal>include_if_exists</literal>はただ単にメッセージをログ出力し、そして参照している設定ファイルの処理を続けます。␞␞     </para>␞
␝      </indexterm>␟      The <filename>postgresql.conf</filename> file can also contain
      <literal>include_dir</literal> directives, which specify an entire
      directory of configuration files to include.  These look like␟includeする設定ファイルを含むディレクトリ全体を指定する<literal>include_dir</literal>ディレクティブを、<filename>postgresql.conf</filename>ファイルに含めることもできます。
このような感じです。␞␞␞
␝      directory of configuration files to include.  These look like␟<programlisting>
include_dir 'directory'
</programlisting>␟<programlisting>
include_dir 'ディレクトリ名'
</programlisting>␞␞␞
␝</programlisting>␟      Non-absolute directory names are taken as relative to the directory
      ...
      uppercase letters before lowercase ones).␟絶対パスではないディレクトリ名はその設定ファイルがあるディレクトリへの相対パスと見なされます。
...
つまり、文字より数字、小文字より大文字が先になります)。␞␞     </para>␞
␝     <para>␟      Include files or directories can be used to logically separate portions
      ...
      them:␟includeされるファイルもしくはディレクトリは、大きな単一の<filename>postgresql.conf</filename>ファイルを使う代わりに、データベース設定の一部分を論理的に分離するために使用することが可能です。
...
それらをincludeするためには<filename>postgresql.conf</filename>ファイルの最後に以下を追加します。␞␞<programlisting>␞
␝</programlisting>␟      All systems would have the same <filename>shared.conf</filename>.  Each
      ...
      configuration information in it.␟全てのシステムは同一の<filename>shared.conf</filename>を所有する様になるでしょう。
...
そして最後の<filename>server.conf</filename>には、本当にサーバ固有となる設定情報を記載します。␞␞     </para>␞
␝     <para>␟      Another possibility is to create a configuration file directory and
      put this information into files there. For example, a <filename>conf.d</filename>
      directory could be referenced at the end of <filename>postgresql.conf</filename>:␟別の方法として、設定ファイルディレクトリを作成し、この情報をそこのファイルに格納することができます。
たとえば、<filename>conf.d</filename>ディレクトリを<filename>postgresql.conf</filename>の最後で参照するようにできます。␞␞<programlisting>␞
␝</programlisting>␟      Then you could name the files in the <filename>conf.d</filename> directory
      like this:␟そして、<filename>conf.d</filename>の中のファイルを以下のような名前にすることができます。␞␞<programlisting>␞
␝</programlisting>␟       This naming convention establishes a clear order in which these
       ...
       value set in <filename>conf.d/01memory.conf</filename>.␟この命名規則により、これらのファイルが読み込まれる順序が明確になります。
サーバが設定を読み込んでいるときに各パラメータについて最後にあった設定だけが使用されるので、このことは重要です。
この例では、<filename>conf.d/02server.conf</filename>でされた指定は<filename>conf.d/01memory.conf</filename>の設定値よりも優先します。␞␞     </para>␞
␝     <para>␟      You might instead use this approach to naming the files
      descriptively:␟代わりに以下の方法を使って、ファイルにわかりやすい名前をつけることもできます。␞␞<programlisting>␞
␝</programlisting>␟      This sort of arrangement gives a unique name for each configuration file
      ...
      control is another good practice to consider.)␟こういった工夫で、設定ファイルのバリエーションに対して固有の名前を付与することができます。
また、バージョン管理リポジトリのリポジトリに複数のサーバの設定ファイルを置く場合に生じる曖昧さを排除することができます。
（データベース設定ファイルをバージョン管理することは、これもまた検討に値するやり方です。）␞␞     </para>␞
␝   <sect1 id="runtime-config-file-locations">␟    <title>File Locations</title>␟    <title>ファイルの場所</title>␞␞␞
␝      <indexterm>
...
␝     <para>␟      In addition to the <filename>postgresql.conf</filename> file
      ...
      kept separate.)␟すでに説明した<filename>postgresql.conf</filename>ファイルに加え、<productname>PostgreSQL</productname>は、クライアント認証の管理を行うために、他の2つの手作業で編集される設定ファイルを使用します（これらの使用方法は<xref linkend="client-authentication"/>で説明します）。
全ての3つの設定ファイルは、デフォルトではデータベースクラスタのdataディレクトリに格納されます。
本節で説明するパラメータにより、設定ファイルを他の場所に置くことが可能になります。
（そのようにすると管理がしやすくなります。
とりわけ、設定ファイルを分けて保存することで、設定ファイルの適切なバックアップを確実に行うことがしばしば容易になります。）␞␞     </para>␞
␝      <indexterm>
...
␝       <para>␟         Specifies the directory to use for data storage.
         This parameter can only be set at server start.␟データ格納に使用するディレクトリを指定します。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟         Specifies the main server configuration file
         (customarily called <filename>postgresql.conf</filename>).
         This parameter can only be set on the <command>postgres</command> command line.␟メインサーバ設定ファイルを指定します（通例<filename>postgresql.conf</filename>と呼ばれます）。
このパラメータは<command>postgres</command>コマンドライン上でのみ設定可能です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟         Specifies the configuration file for host-based authentication
         (customarily called <filename>pg_hba.conf</filename>).
         This parameter can only be set at server start.␟ホストベース認証（HBA）用のファイルを指定します（通例<filename>pg_hba.conf</filename>と呼ばれます）。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟         Specifies the configuration file for user name mapping
         (customarily called <filename>pg_ident.conf</filename>).
         This parameter can only be set at server start.
         See also <xref linkend="auth-username-maps"/>.␟ユーザ名マッピングの設定ファイルを指定します（通例<filename>pg_ident.conf</filename>と呼ばれます）。
このパラメータはサーバ起動時のみ設定可能です。
<xref linkend="auth-username-maps"/>もご覧ください。␞␞       </para>␞
␝       <para>␟        Specifies the name of an additional process-ID (PID) file that the
        server should create for use by server administration programs.
        This parameter can only be set at server start.␟サーバ管理プログラムで使用するためにサーバが作成する、追加のプロセス識別子（PID）ファイルの名前を指定します。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝     <para>␟      In a default installation, none of the above parameters are set
      ...
      configuration files are all found within the data directory.␟デフォルトのインストールでは、上記のいかなるパラメータも明示的に設定されません。
その代わり、data ディレクトリは<option>-D</option>コマンドラインオプション、または<envar>PGDATA</envar>環境変数で指定され、設定ファイル全てはその data ディレクトリ内に格納されます。␞␞     </para>␞
␝     <para>␟      If you wish to keep the configuration files elsewhere than the
      ...
      files.␟dataディレクトリ以外の場所に設定ファイルを格納したいのであれば、<command>postgres</command>の<option>-D</option>コマンドラインオプション、または<envar>PGDATA</envar>環境変数で設定ファイルの場所を指し示し、そしてdataディレクトリが実際どこに存在するのかを示すため、<filename>postgresql.conf</filename>の（もしくはコマンドライン上で）<varname>data_directory</varname>パラメータを設定しなければなりません。
<varname>data_directory</varname>は、設定ファイルの場所ではなく、data ディレクトリの位置に関して、<option>-D</option>および<envar>PGDATA</envar>を上書きすることに注意してください。␞␞     </para>␞
␝     <para>␟      If you wish, you can specify the configuration file names and locations
      ...
      to specify <option>-D</option> or <envar>PGDATA</envar>.␟必要に応じて、パラメータ<varname>config_file</varname>、<varname>hba_file</varname>、<varname>ident_file</varname>を使用し、設定ファイルの名前と場所を個別に指定することができます。
<varname>config_file</varname>は<command>postgres</command>コマンドラインによってのみ指定されますが、その他は主設定ファイル内で設定できます。
全ての3つのパラメータと<varname>data_directory</varname>が明示的に設定されていれば、<option>-D</option>または<envar>PGDATA</envar>を指定する必要はありません。␞␞     </para>␞
␝     <para>␟      When setting any of these parameters, a relative path will be interpreted
      with respect to the directory in which <command>postgres</command>
      is started.␟これらのパラメータのどれを設定する場合でも、相対パスは、<command>postgres</command>が起動されるディレクトリから見た相対パスとして解釈されます。␞␞     </para>␞
␝   <sect1 id="runtime-config-connection">␟    <title>Connections and Authentication</title>␟    <title>接続と認証</title>␞␞␞
␝      <indexterm>
...
␝    <sect2 id="runtime-config-connection-settings">␟     <title>Connection Settings</title>␟     <title>接続設定</title>␞␞␞
␝       <para>␟         Specifies the TCP/IP address(es) on which the server is
         ...
         made.␟クライアントアプリケーションからの接続をサーバが監視するTCP/IPアドレスを指定します。
...
デフォルトの値は<systemitem class="systemname">localhost</systemitem>で、ローカルなTCP/IP <quote>loopback</quote>接続のみ許可します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟         While client authentication (<xref
         linkend="client-authentication"/>) allows fine-grained control
         over who can access the server, <varname>listen_addresses</varname>
         controls which interfaces accept connection attempts, which
         can help prevent repeated malicious connection requests on
         insecure network interfaces.  This parameter can only be set
         at server start.␟クライアント認証(<xref linkend="client-authentication"/>)は、誰がサーバにアクセスできるかを細かく制御できますが、<varname>listen_addresses</varname>は接続要求を受け付けるインタフェースを制御します。
これは、セキュアでないネットワークインタフェース上での悪意のある接続試行の繰り返しを防ぐのに役立ちます。
このパラメータはサーバ起動時にのみ設定できます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The TCP port the server listens on; 5432 by default.  Note that the
        same port number is used for all IP addresses the server listens on.
        This parameter can only be set at server start.␟サーバが監視するTCPポートで、デフォルトは 5432です。
サーバが監視する全てのIPアドレスに対し、同じポート番号が使用されることを覚えておいてください。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝       <para>␟        Determines the maximum number of concurrent connections to the
        database server. The default is typically 100 connections, but
        might be less if your kernel settings will not support it (as
        determined during <application>initdb</application>).  This parameter can
        only be set at server start.␟データベースサーバに同時接続する最大数を決定します。
デフォルトは典型的に100接続ですが、カーネルの設定が（<application>initdb</application>の過程で）それをサポートしていない場合、もっと少なくなることがあります。
このパラメータはサーバ起動時のみに設定可能です。␞␞       </para>␞
␝       <para>␟        PostgreSQL sizes certain resources based directly on the value of
        <varname>max_connections</varname>. Increasing its value leads to
        higher allocation of those resources, including shared memory.␟PostgreSQLは、<varname>max_connections</varname>の値に直接基づいてある種のリソースのサイズを設定します。
その値を大きくすると、共有メモリを含み、より大きなリソースがアロケーションされます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        When running a standby server, you must set this parameter to the
        same or higher value than on the primary server. Otherwise, queries
        will not be allowed in the standby server.␟スタンバイサーバを運用している場合、このパラメータはプライマリサーバでの設定と同じ、もしくはより高い値に設定しなければなりません。そうしないと問い合わせがスタンバイサーバ内で受け入れられません。␞␞       </para>␞
␝       <para>␟        Determines the number of connection <quote>slots</quote> that are
        ...
        superusers.␟<link linkend="predefined-roles-table"><literal>pg_use_reserved_connections</literal></link>権限を持つロールによる接続のために予約される接続<quote>スロット</quote>の数を決定します。
...
<varname>superuser_reserved_connections</varname>以下の接続スロットが利用可能な場合、新しい接続はスーパーユーザに対してのみ受け入れられます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Determines the number of connection <quote>slots</quote> that
        ...
        <xref linkend="guc-reserved-connections"/> have been exhausted.␟<productname>PostgreSQL</productname>のスーパーユーザによる接続のために予約されている接続<quote>スロット</quote>の数を決定します。
...
このパラメータによる接続スロット予約は、<xref linkend="guc-reserved-connections"/>によるスロット予約が使い果たされた後の緊急使用のための最終的な予備を意図しています。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The default value is three connections. The value must be less
        than <varname>max_connections</varname> minus
        <varname>reserved_connections</varname>.
        This parameter can only be set at server start.␟デフォルトの値は3接続です。
この値は <varname>max_connections</varname>から<varname>reserved_connections</varname>を引いたものより小さくなくてはなりません。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝       <para>␟        Specifies the directory of the Unix-domain socket(s) on which the
        ...
        only TCP/IP sockets can be used to connect to the server.␟クライアントアプリケーションからの接続をlistenするサーバ上のUnixドメインソケットのディレクトリを指定します。
複数のディレクトリをカンマで区切って指定することにより、複数のソケットを作ることができます。
項目の間の空白は無視されます。
名前の中に空白かカンマが必要なら、ディレクトリ名を二重引用符で囲ってください。
空の値はUnixドメインソケットをlistenしないことを意味します。
この場合、TCP/IPソケットを使ってサーバに接続することだけが可能になります。␞␞       </para>␞
␝       <para>␟        A value that starts with <literal>@</literal> specifies that a
        ...
        <literal>@/tmp</literal>.␟<literal>@</literal>で始まる値は、抽象名前空間にUnixドメインソケットを作ることを指定します（今の所これはLinuxでのみサポートされています）。
この場合、この値は<quote>ディレクトリ</quote>ではなくファイルシステムの名前空間と同様の方法で、実際のソケット名の接頭辞が求められます。
ファイルシステムの位置ではないため、抽象ソケット名接頭辞は自由に選択できますが、それにもかかわらず<literal>@/tmp</literal>のようなファイルシステム的な値を使うのが慣例です。␞␞       </para>␞
␝       <para>␟        The default value is normally
        <filename>/tmp</filename>, but that can be changed at build time.
        On Windows, the default is empty, which means no Unix-domain socket is
        created by default.
        This parameter can only be set at server start.␟デフォルト値は通常<filename>/tmp</filename>ですが、ビルド時に変更できます。
...
␝       <para>␟        In addition to the socket file itself, which is named
        ...
        For sockets in the abstract namespace, no lock file is created.␟<literal>.s.PGSQL.<replaceable>nnnn</replaceable></literal>という名前のソケットファイル（<replaceable>nnnn</replaceable>はポート番号）のほかに、<literal>.s.PGSQL.<replaceable>nnnn</replaceable>.lock</literal>という通常ファイルがそれぞれの<varname>unix_socket_directories</varname>ディレクトリの中に作成されます。
いずれのファイルも手作業で削除してはいけません。
抽象名前空間にあるソケットにはロックファイルは作成されません。␞␞       </para>␞
␝       <para>␟        Sets the owning group of the Unix-domain socket(s).  (The owning
        ...
        server start.␟Unixドメインソケット（複数も）を所有するグループを設定します。
...
␝       <para>␟        This parameter is not supported on Windows.  Any setting will be
        ignored.  Also, sockets in the abstract namespace have no file owner,
        so this setting is also ignored in that case.␟このパラメータはWindowsではサポートされていません。
すべての設定は無視されます。
また、抽象名前空間にあるソケットはファイル所有者を持たないので、この場合も設定は無視されます。␞␞       </para>␞
␝       <para>␟        Sets the access permissions of the Unix-domain socket(s).  Unix-domain
        sockets use the usual Unix file system permission set.
        The parameter value is expected to be a numeric mode
        specified in the format accepted by the
        <function>chmod</function> and <function>umask</function>
        system calls.  (To use the customary octal format the number
        must start with a <literal>0</literal> (zero).)␟Unixドメインソケット（複数も）のアクセスパーミッションを設定します。
...
（通常使われる8進数形式を使用するのであれば、<literal>0</literal>（ゼロ）で始まらなければなりません。）␞␞       </para>␞
␝       <para>␟        The default permissions are <literal>0777</literal>, meaning
        ...
        read or execute permissions.)␟デフォルトのパーミッションは、誰でも接続できる<literal>0777</literal>になっています。
変更するならば<literal>0770</literal>（ユーザとグループのみです。<varname>unix_socket_group</varname>も参照してください）や<literal>0700</literal>（ユーザのみ）が適切です。
（Unixドメインソケットでは書き込み権限だけが問題になるため、読み込みや実行のパーミッションを設定または解除する意味はありません。）␞␞       </para>␞
␝       <para>␟        This access control mechanism is independent of the one
        described in <xref linkend="client-authentication"/>.␟このアクセス制御機構は <xref linkend="client-authentication"/>で記述されたものとは別個のものです。␞␞       </para>␞
␝       <para>␟        This parameter can only be set at server start.␟このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝       <para>␟        This parameter is irrelevant on systems, notably Solaris as of Solaris
        10, that ignore socket permissions entirely.  There, one can achieve a
        similar effect by pointing <varname>unix_socket_directories</varname> to a
        directory having search permission limited to the desired audience.␟このパラメータはSolaris 10の時点でのSolarisなど、ソケットのパーミッションを完全に無視するシステムでは無関係です。
こうしたシステムでは、許可したいユーザだけが検索パーミッションを持つディレクトリを<varname>unix_socket_directories</varname>で指すようにすることによって同じような効果を得ることができます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sockets in the abstract namespace have no file permissions, so this
        setting is also ignored in that case.␟抽象名前空間にあるソケットはファイルパーミッションを持たないので、この設定も無視されます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables advertising the server's existence via
        <productname>Bonjour</productname>.  The default is off.
        This parameter can only be set at server start.␟<productname>Bonjour</productname>によりサーバの存在を公表することを可能にします。デフォルトはoffです。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the <productname>Bonjour</productname> service
        ...
        This parameter can only be set at server start.␟<productname>Bonjour</productname>サービス名を指定します。
...
␝       <para>␟        Specifies the amount of time with no network activity after which
        ...
        ignored and always reads as zero.␟クライアントとのやり取りがなくなった後、オペレーティングシステムがTCPのkeepaliveパケットをクライアントに送信するまでの時間を指定します。
...
Unixドメインソケット経由で接続されたセッションでは、このパラメータは無視され、常にゼロとして読み取られます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the amount of time after which a TCP keepalive message
        ...
        Windows; on other systems, it must be zero.
        In sessions connected via a Unix-domain socket, this parameter is
        ignored and always reads as zero.␟TCPのkeepaliveメッセージに対してクライアントから応答がない場合に、再送を行うまでの時間を指定します。
この値が単位なしで指定された場合は、秒単位であるとみなします。
...
␝       <para>␟        Specifies the number of TCP keepalive messages that can be lost before
        ...
        In sessions connected via a Unix-domain socket, this parameter is
        ignored and always reads as zero.␟サーバのクライアントへの接続が切れたと判断されるまでのTCP keepaliveメッセージの数を指定します。
0（デフォルトです）の場合はオペレーティングシステムのデフォルト値を使用します。
...
␝       <para>␟        Specifies the amount of time that transmitted data may
        ...
        In sessions connected via a Unix-domain socket, this parameter is
        ignored and always reads as zero.␟未応答の送信データが残ったままの接続が強制的に閉じられるまでの時間を指定します。
...
UNIXドメインソケットで接続しているセッションではこのパラメータは無視され、常に0として扱われます。␞␞       </para>␞
␝       <para>␟        Sets the time interval between optional checks that the client is still
        ...
        the kernel reports that the connection is closed.␟問い合わせを実行中に、クライアントがまだ接続しているどうかの追加のチェックを行う時間間隔を指定します。
このチェックはソケットをポーリングすることによって行われ、その接続がクローズされていることをカーネルが報告することによって、長時間実行中の問い合わせをより早くアボートさせることができます。␞␞       </para>␞
␝       <para>␟        This option relies on kernel events exposed by Linux, macOS, illumos
        and the BSD family of operating systems, and is not currently available
        on other systems.␟このオプションは、Linux、macOS、illumos、BSD系のオペレーティングシステムによって公開されるカーネルイベントに依存しており、現在他のシステムでは利用できません。␞␞       </para>␞
␝       <para>␟        If the value is specified without units, it is taken as milliseconds.
        ...
        waits for, receives or sends data.␟値が単位なしに指定されると、ミリ秒であるとみなされます。
デフォルト値は<literal>0</literal>で、接続のチェックは無効になります。
接続チェックがない場合、サーバは接続が失われたことを、次のソケットへのアクセス、すなわちソケットの待受、データの受信、送信のときにだけ検出します。␞␞       </para>␞
␝       <para>␟        For the kernel itself to detect lost TCP connections reliably and within
        ...
        <productname>PostgreSQL</productname>.␟ネットワーク障害を含めて、TCP接続が失われたことをすべてのシナリオにおいて既知のタイムフレームの中で確実にカーネルが検出するためには、オペレーティングシステムのTCP keepalive設定、あるいは<productname>PostgreSQL</productname>の<xref linkend="guc-tcp-keepalives-idle"/>、<xref linkend="guc-tcp-keepalives-interval"/>、<xref linkend="guc-tcp-keepalives-count"/>設定を調整することが必要になるかもしれません。␞␞       </para>␞
...
␝     <sect2 id="runtime-config-connection-authentication">␟     <title>Authentication</title>␟     <title>認証</title>␞␞␞
␝      <indexterm>
...
␝       <para>␟        Maximum amount of time allowed to complete client authentication. If a
        ...
        file or on the server command line.␟クライアント認証を完了するまでの最大時間です。
もし、この時間内に自称クライアントが認証プロトコルを完了しない場合、サーバは接続を閉じます。
これはハングしたクライアントが接続を永久に占有することを防ぎます。
この値が単位なしで指定された場合は、秒単位であるとみなします。
デフォルトは1分（<literal>1m</literal>）です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        When a password is specified in <xref linkend="sql-createrole"/> or
        ...
        as an MD5 hash.  The default is <literal>scram-sha-256</literal>.␟<xref linkend="sql-createrole"/>あるいは<xref linkend="sql-alterrole"/>でパスワードを設定する際に、このパラメータはパスワードを暗号化するアルゴリズムを指定します。
可能な値は、SCRAM-SHA-256でパスワードを暗号化する<literal>scram-sha-256</literal>とパスワードをMD5ハッシュとして格納する<literal>md5</literal>です。
デフォルト値は<literal>scram-sha-256</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Note that older clients might lack support for the SCRAM authentication
        mechanism, and hence not work with passwords encrypted with
        SCRAM-SHA-256.  See <xref linkend="auth-password"/> for more details.␟古いクライアントはSCRAM認証機構をサポートしていない可能性があり、したがってSCRAM-SHA-256による暗号化は動作しないかもしれないことに注意してください。
さらなる詳細については<xref linkend="auth-password"/>をご覧ください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The number of computational iterations to be performed when encrypting
        ...
        password must be set.␟SCRAM-SHA-256を使用してパスワードを暗号化するときに実行される計算の反復回数です。
...
変更された値を有効にするには、新しいパスワードを設定する必要があります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the location of the server's Kerberos key file.  The default is
        ...
        See <xref linkend="gssapi-auth"/> for more information.␟Kerberosサーバキーファイルの場所を設定します。
...
詳細は<xref linkend="gssapi-auth"/>をご覧ください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets whether GSSAPI user names should be treated
        case-insensitively.
        The default is <literal>off</literal> (case sensitive). This parameter can only be
        set in the <filename>postgresql.conf</filename> file or on the server command line.␟GSSAPIユーザ名を大文字小文字の区別なく取り扱うかどうかを設定します。
デフォルトは<literal>off</literal>（大文字小文字を区別する）です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        Sets whether GSSAPI delegation should be accepted from the client.
        ...
        set in the <filename>postgresql.conf</filename> file or on the server command line.␟クライアントからのGSSAPI委任を受け入れるかどうかを設定します。
...
␝     <para>␟      See <xref linkend="ssl-tcp"/> for more information about setting up
      ...
      <acronym>TLS</acronym>.␟<acronym>SSL</acronym>の設定の詳細は、<xref linkend="ssl-tcp"/>を参照してください。
...
<acronym>SSL</acronym>はこの文脈で<acronym>TLS</acronym>と同じ意味で使用されます。␞␞     </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables <acronym>SSL</acronym> connections.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.
        The default is <literal>off</literal>.␟<acronym>SSL</acronym>による接続を有効にします。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
デフォルトは<literal>off</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the name of the file containing the SSL server certificate
        authority (CA).
        Relative paths are relative to the data directory.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.
        The default is empty, meaning no CA file is loaded,
        and client certificate verification is not performed.␟SSLサーバ認証局（CA）が入っているファイル名を設定します。
相対パスの場合は、データディレクトリからの相対パスになります。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
デフォルトは空で、この場合CAファイルは読み込まれず、クライアントのサーバ検証は行われません。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the name of the file containing the SSL server certificate.
        ...
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.
        The default is <filename>server.crt</filename>.␟SSLサーバ証明書が入っているファイル名を設定します。
相対パスの場合は、データディレクトリからの相対パスになります。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
デフォルトは <filename>server.crt</filename> です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the name of the file containing the SSL client certificate
        revocation list (CRL).
        Relative paths are relative to the data directory.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.
        The default is empty, meaning no CRL file is loaded (unless
        <xref linkend="guc-ssl-crl-dir"/> is set).␟SSLクライアント証明書失効リスト（CRL）が入っているファイル名を設定します。
相対パスの場合は、データディレクトリからの相対パスになります。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
デフォルトは空で、この場合CRLファイルは読み込まれません（<xref linkend="guc-ssl-crl-dir"/>が設定されていない限り）。␞␞       </para>␞
␝       <para>␟        Specifies the name of the directory containing the SSL client
        ...
        <xref linkend="guc-ssl-crl-file"/> is set).␟SSLクライアント証明書失効リスト（CRL）が入っているディレクトリ名を設定します。
相対パスの場合は、データディレクトリからの相対パスになります。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
デフォルトは空で、この場合CRLファイルは読み込まれません（<xref linkend="guc-ssl-crl-file"/>が設定されていない限り）。␞␞       </para>␞
␝       <para>␟        The directory needs to be prepared with the
        <productname>OpenSSL</productname> command
        <literal>openssl rehash</literal> or <literal>c_rehash</literal>.  See
        its documentation for details.␟このディレクトリは<productname>OpenSSL</productname>コマンドの<literal>openssl rehash</literal>あるいは<literal>c_rehash</literal>で準備されなければなりません。
詳細はそれらのドキュメントをご覧ください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        When using this setting, CRLs in the specified directory are loaded
...
␝       <para>␟        Specifies the name of the file containing the SSL server private key.
        Relative paths are relative to the data directory.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.
        The default is <filename>server.key</filename>.␟SSLサーバの秘密鍵が入っているファイル名を設定します。
相対パスの場合は、データディレクトリからの相対パスになります。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
デフォルトは <filename>server.key</filename> です。␞␞       </para>␞
␝       <para>␟        Specifies a list of <acronym>SSL</acronym> cipher suites that are
        ...
        reasonable choice unless you have specific security requirements.␟SSL接続で使うことのできる<acronym>SSL</acronym>暗号スイートのリストを指定します。
...
特別なセキュリティ要件でなければ通常これが適当です。␞␞       </para>␞
␝       <para>␟        This parameter can only be set in the
        <filename>postgresql.conf</filename> file or on the server command
        line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        Explanation of the default value:␟デフォルト値の説明:␞␞        <variablelist>␞
␝           <para>␟            Cipher suites that use ciphers from <literal>HIGH</literal> group (e.g.,
            AES, Camellia, 3DES)␟<literal>HIGH</literal>グループ(たとえばAES, Camellia, 3DES)を使用する暗号スイート␞␞           </para>␞
␝           <para>␟            Cipher suites that use ciphers from <literal>MEDIUM</literal> group
            (e.g., RC4, SEED)␟<literal>MEDIUM</literal>グループ(たとえば RC4, SEED)を使用する暗号スイート␞␞           </para>␞
␝           <para>␟            The <productname>OpenSSL</productname> default order for
            ...
            <literal>HIGH</literal> and <literal>MEDIUM</literal> ciphers.␟<productname>OpenSSL</productname>の<literal>HIGH</literal>に対するデフォルトの並び順には問題があります。
3DESがAES128より高いとしているからです。
3DESはAES128よりもセキュアではなく、またずっと遅いので、これは間違っています。
<literal>+3DES</literal>ではそれを他のすべての<literal>HIGH</literal>と<literal>MEDIUM</literal>暗号よりも後に位置づけます。␞␞           </para>␞
␝           <para>␟            Disables anonymous cipher suites that do no authentication.  Such
            cipher suites are vulnerable to <acronym>MITM</acronym> attacks and
            therefore should not be used.␟認証を行わない匿名暗号スイートを無効にします。
そういった暗号スイートは<acronym>MITM</acronym>攻撃に対して脆弱で、使用すべきではありません。␞␞           </para>␞
␝      <indexterm>
...
␝       <para>␟        Available cipher suite details will vary across
        ...
        filtered at run time based on the server key type.␟<productname>OpenSSL</productname>のバージョンにより、利用可能な暗号スイートの詳細は異なります。
...
ここで得られるリストは、サーバキータイプにより実行時にフィルタされることに注意してください。␞␞       </para>␞
␝       <para>␟        Specifies whether to use the server's SSL cipher preferences, rather
        than the client's.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.
        The default is <literal>on</literal>.␟サーバのSSL暗号設定をクライアントに優先して使うかどうかを指定します。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        <productname>PostgreSQL</productname> versions before 9.4 do not have
        ...
        the server is appropriately configured.␟バージョン9.4より前の<productname>PostgreSQL</productname>にはこの設定がなく、常にクライアントの設定が使用されます。
この設定は、主に古いバージョンとの互換性のために設けられています。
通常サーバの設定に従うほうが良いです。大抵の場合、サーバはより適切に設定されているからです。␞␞       </para>␞
␝       <para>␟        Specifies the name of the curve to use in <acronym>ECDH</acronym> key
        exchange.  It needs to be supported by all clients that connect.
        It does not need to be the same curve used by the server's Elliptic
        Curve key.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.
        The default is <literal>prime256v1</literal>.␟<acronym>ECDH</acronym>キー交換で使われる曲線の名前を指定します。
接続するすべてのクライアントがこの設定をサポートしている必要があります。
サーバの楕円曲線キーで使用されるのと同じ曲線である必要はありません。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
デフォルト値は<literal>prime256v1</literal>です。␞␞       </para>␞
␝       <para>␟        <productname>OpenSSL</productname> names for the most common curves
        are:␟<productname>OpenSSL</productname>はよく使われる曲線に名前を付けています。␞␞        <literal>prime256v1</literal> (NIST P-256),␞
␝      <indexterm>
...
␝        <literal>secp521r1</literal> (NIST P-521).␟        The full list of available curves can be shown with the command
        <command>openssl ecparam -list_curves</command>.  Not all of them
        are usable in <acronym>TLS</acronym> though.␟利用できる曲線の完全なリストは<command>openssl ecparam -list_curves</command>で得られます。ただし、<acronym>TLS</acronym>ではこのすべてが利用できるわけではありません。␞␞       </para>␞
␝       <para>␟        Sets the minimum SSL/TLS protocol version to use.  Valid values are
        ...
        3, are always disabled.␟使用するSSL/TLSプロトコルバージョンの最小値を設定します。
...
TLS 1.0より前のプロトコルバージョン、すなわちSSLバージョン2あるいは3は常に無効となります。␞␞       </para>␞
␝       <para>␟        The default is <literal>TLSv1.2</literal>, which satisfies industry
        best practices as of this writing.␟デフォルトは<literal>TLSv1.2</literal>で、本稿執筆時点では業界のベストプラクティスを満たしています。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        Sets the maximum SSL/TLS protocol version to use.  Valid values are as
        ...
        newer protocol.␟使用するSSL/TLSプロトコルバージョンの最大値を設定します。
使用できる値は<xref linkend="guc-ssl-min-protocol-version"/>と、すべてのプロトコルバージョンを許可する空文字です。
デフォルトはすべてのプロトコルバージョンを許可する設定です。
最大プロトコルバージョンの設定は主にテスト、あるいは新しいプロトコルを使った時にコンポーネントのどこかに問題がある時に有用です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the name of the file containing Diffie-Hellman parameters
        ...
        <command>openssl dhparam -out dhparams.pem 2048</command>.␟いわゆる短命DH系SSL暗号で使用するディフィー・ヘルマンパラメータを格納するファイル名を指定します。
デフォルトは空で、この場合はコンパイル時に決められたデフォルトのDHパラメータが使用されます。
攻撃者が、よく知られたコンパイル時設定のDHパラメータを解読しようとしている場合には、カスタムDHパラメータを使うことでその危険性を低減できます。
<command>openssl dhparam -out dhparams.pem 2048</command>を使って、独自のDHパラメータファイルを作ることができます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        Sets an external command to be invoked when a passphrase for
        ...
        mechanism is used.␟秘密鍵などのSSLファイルを復号する際に、パスフレーズの入手が必要な時に起動される外部コマンドを設定します。
デフォルトではこのパラメータは空文字で、組み込みのプロンプト機構が使用されます。␞␞       </para>␞
␝       <para>␟        The command must print the passphrase to the standard output and exit
        ...
        newline is stripped from the end of the output if present.␟このコマンドは、パスフレーズを標準出力に書き出し、コード0で終了しなければなりません。
...
出力の最後に一個の改行があれば、削除されます。␞␞       </para>␞
␝       <para>␟        The command does not actually have to prompt the user for a
        ...
        mechanism is adequately secure.␟このコマンドは実際にはパスフレーズ用にユーザにプロンプトを表示する必要はありません。
ファイルからパスフレーズが読めるなら、キーチェーン機構やその他から取得します。
選択された仕組みが適切にセキュアかどうかを確認するのはユーザ次第です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        This parameter determines whether the passphrase command set by
        ...
        passphrase is obtained from a file, for example.␟このパラメータは、キーにパスフレーズが必要な場合、設定ファイルの再読み込み中に<varname>ssl_passphrase_command</varname>で設定されたパスフレーズコマンドも呼び出されるかどうかを設定します。
...
たとえばパスフレーズがファイルから読み込める場合には、この設定をonにするのが適切です。␞␞       </para>␞
␝       <para>␟        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝   <sect1 id="runtime-config-resource">␟    <title>Resource Consumption</title>␟    <title>資源の消費</title>␞␞␞
␝      <indexterm>
...
␝    <sect2 id="runtime-config-resource-memory">␟     <title>Memory</title>␟     <title>メモリ</title>␞␞␞
␝       <para>␟        Sets the amount of memory the database server uses for shared
        ...
        value.)
        This parameter can only be set at server start.␟データベースサーバが共有メモリバッファのために使用するメモリ量を設定します。
...
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝       <para>␟        If you have a dedicated database server with 1GB or more of RAM, a
        ...
        longer period of time.␟1GB以上のRAMを載せた専用データベースサーバを使用している場合、<varname>shared_buffers</varname>に対する妥当な初期値はシステムメモリの25%です。
...
<varname>shared_buffers</varname>をより大きく設定する場合は、大抵<varname>max_wal_size</varname>も合わせて増やす必要があります。これは、新規または変更された多量のデータを書き出す処理をより長い時間に渡って分散させるためです。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        On systems with less than 1GB of RAM, a smaller percentage of RAM is
        appropriate, so as to leave adequate space for the operating system.␟1GB未満のRAMのシステムでは、オペレーティングシステムに十分な余裕を残すために、RAMに対してより小さい割合を設定することが適切です。␞␞       </para>␞
␝       <para>␟        Controls whether huge pages are requested for the main shared memory
        ...
        <xref linkend="guc-huge-pages-status"/>.␟主共有メモリ領域に対してhuge pageを要求するかどうかを管理します。
...
huge pageの実際の状態は、サーバ変数<xref linkend="guc-huge-pages-status"/>で示されています。␞␞       </para>␞
␝       <para>␟        At present, this setting is supported only on Linux and Windows. The
        ...
        (the default).␟今のところこの機能はLinuxとWindowsでのみサポートされています。
他のシステムでは<literal>try</literal>と設定しても無視されます。
Linuxではこの機能は<varname>shared_memory_type</varname>が<literal>mmap</literal>（デフォルトです）に設定されている時にのみサポートされます。␞␞       </para>␞
␝       <para>␟        The use of huge pages results in smaller page tables and less CPU time
        spent on memory management, increasing performance. For more details about
        using huge pages on Linux, see <xref linkend="linux-huge-pages"/>.␟huge pageを使うと、ページテーブルが小さくなり、メモリ管理に使用されるCPU時間が少なくなり、性能が向上します。
詳細は、<xref linkend="linux-huge-pages"/>を見てください。␞␞       </para>␞
␝       <para>␟        Huge pages are known as large pages on Windows.  To use them, you need to
        ...
        command prompt revokes the user right <quote>Lock pages in memory</quote> when started.␟huge pageはWindowsではlarge pageとして知られています。
...
UACが有効ならば、通常のコマンドプロンプトは起動時にユーザの<quote>メモリ中のロックページ</quote>権限を剥奪します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Note that this setting only affects the main shared memory area.
        ...
        <varname>huge_pages</varname>).␟この設定は主共有メモリ領域にのみ影響することに注意してください。
...
ですから、この機能の利用は（<varname>huge_pages</varname>の明示的な利用と違って）今の所推奨されていません。␞␞       </para>␞
␝       <para>␟        Controls the size of huge pages, when they are enabled with
        ...
        system will be used. This parameter can only be set at server start.␟<xref linkend="guc-huge-pages"/>が有効なときにhuge pageのサイズを制御します。
デフォルトはゼロ(<literal>0</literal>)です。
<literal>0</literal>に設定すると、システムのデフォルトのhuge pageのサイズが使われます。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝       <para>␟        Some commonly available page sizes on modern 64 bit server architectures include:
        ...
        about usage and support, see <xref linkend="linux-huge-pages"/>.␟現代の64ビットサーバアーキテクチャにおける可能な一般的なページサイズには以下が含まれます。
...
使い方とサポートに関する詳細な情報に関しては<xref linkend="linux-huge-pages"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Non-default settings are currently supported only on Linux.␟今の所Linuxでサポートされるデフォルトの設定値はありません。␞␞       </para>␞
␝       <para>␟        Sets the maximum amount of memory used for temporary buffers within
        ...
        have no effect on that session.␟それぞれのデータベースセッションが使用する一時バッファの最大メモリ量を設定します。
...
設定はそれぞれのセッション内で変更できますが、そのセッション内で一時テーブルが最初に使用されるまでになります。それより後に値の変更を試みても、そのセッションでは効果がありません。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        A session will allocate temporary buffers as needed up to the limit
        ...
        (or in general, <symbol>BLCKSZ</symbol> bytes).␟セッションは、<varname>temp_buffers</varname>を上限として、必要に応じて一時バッファを確保します。
多くの一時バッファを実際に必要としないセッションで大きな値を設定するコストとは、<varname>temp_buffers</varname>の増分毎に、1つのバッファ記述子、約64バイトだけです。
しかし、バッファが実際に使用されると、それに対して追加の8192バイト（汎用的に言えば<symbol>BLCKSZ</symbol>バイト）が消費されます。␞␞       </para>␞
␝       <para>␟        Sets the maximum number of transactions that can be in the
        <quote>prepared</quote> state simultaneously (see <xref
        linkend="sql-prepare-transaction"/>).
        Setting this parameter to zero (which is the default)
        disables the prepared-transaction feature.
        This parameter can only be set at server start.␟同時に<quote>プリペアド</quote>状態にできるトランザクションの最大数を設定します（<xref linkend="sql-prepare-transaction"/>を参照してください）。
このパラメータをゼロ（これがデフォルトです）に設定すると、プリペアドトランザクション機能が無効になります。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝       <para>␟        If you are not planning to use prepared transactions, this parameter
        ...
        session can have a prepared transaction pending.␟プリペアドトランザクションの使用を意図しないのであれば、このパラメータはプリペアドトランザクションが偶然に作成されないようゼロに設定すべきです。
プリペアドトランザクションを使用する場合、全てのセッションがプリペアドトランザクションを保留できるように、<varname>max_prepared_transactions</varname>を少なくとも<xref linkend="guc-max-connections"/>と同じ大きさに設定するのが良いでしょう。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        When running a standby server, you must set this parameter to the
        same or higher value than on the primary server. Otherwise, queries
        will not be allowed in the standby server.␟スタンバイサーバを運用している場合、このパラメータはプライマリサーバ上の設定よりも同等かもしくはより高水準に設定しなければなりません。そうしないと問い合わせがスタンバイサーバ内で受け入れられません。␞␞       </para>␞
␝       <para>␟        Sets the base maximum amount of memory to be used by a query operation
        ...
        nodes and hash-based processing of <literal>IN</literal> subqueries.␟一時ディスクファイルに書き込む前に、問い合わせ操作（ソートやハッシュなど）で使用される最大メモリ量を設定します。
...
ハッシュテーブルはハッシュ結合、ハッシュに基づいた集約、メモ化（memoize）ノードおよび<literal>IN</literal>副問い合わせのハッシュに基づいた処理で使用されます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Hash-based operations are generally more sensitive to memory
        ...
        amount.␟一般的に、ハッシュに基づく操作はソートに基づく操作よりも利用可能なメモリに敏感です。
ハッシュテーブルのメモリ制限は、<varname>work_mem</varname>に<varname>hash_mem_multiplier</varname>を乗算することで計算されます。
これにより、ハッシュに基づく操作では通常の<varname>work_mem</varname>に基づく量を超える量のメモリが使用される可能性があります。␞␞       </para>␞
␝       <para>␟        Used to compute the maximum amount of memory that hash-based
        ...
        <varname>work_mem</varname> base amount.␟ハッシュに基づく操作が利用できる最大のメモリ量を計算するために使用します。
最終的な制限は<varname>work_mem</varname>に<varname>hash_mem_multiplier</varname>を掛けて決定されます。
デフォルト値は2.0で、ハッシュに基づく操作が通常の<varname>work_mem</varname>に基づく値の2倍使用することになります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Consider increasing <varname>hash_mem_multiplier</varname> in
        ...
        or more.␟問い合わせ操作によって日常的にメモリ不足になるような環境、とりわけ単に<varname>work_mem</varname>を増やしたことによってメモリ逼迫（メモリ逼迫が典型的には間欠的なメモリ不足エラーの発生の形で起こる）が起きる場合には<varname>hash_mem_multiplier</varname>を増やすことを考慮してください。
デフォルトの2.0が色々なワークロードが混在している場合には効果的かも知れません。
より大きな2.0から8.0、あるいはそれ以上の設定は<varname>work_mem</varname>がすでに40MB以上に増やしてあるような環境で効果的かも知れません。␞␞       </para>␞
␝       <para>␟        Specifies the maximum amount of memory to be used by maintenance
        ...
        performance for vacuuming and for restoring database dumps.␟<command>VACUUM</command>、<command>CREATE INDEX</command>、および<command>ALTER TABLE ADD FOREIGN KEY</command>の様な保守操作で使用されるメモリの最大容量を指定します。
この値が単位なしで指定された場合は、キロバイト単位であるとみなします。
...
大きい値を設定することでvacuum処理と、ダンプしたデータベースのリストア性能が向上します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Note that when autovacuum runs, up to
        ...
        setting <xref linkend="guc-autovacuum-work-mem"/>.␟自動バキュームが稼働すると、最大でこのメモリの<xref linkend="guc-autovacuum-max-workers"/>倍が配分されるので、デフォルトの値をあまり高く設定しないよう注意してください。
別の設定項目<xref linkend="guc-autovacuum-work-mem"/>で制御するのが良いかもしれません。␞␞       </para>␞
␝       <indexterm>
...
␝       <para>␟        Specifies the maximum amount of memory to be used by each
        ...
        This parameter can only be set in the
        <filename>postgresql.conf</filename> file or on the server command
        line.␟個々の自動バキュームワーカープロセスが使用する最大のメモリ量を指定します。
この値が単位なしで指定された場合は、キロバイト単位であるとみなします。
...
␝       <para>␟        Specifies the size of the
        ...
        shared buffers.␟<command>VACUUM</command>コマンドおよび<command>ANALYZE</command>コマンドで使用される<glossterm linkend="glossary-buffer-access-strategy">バッファアクセスストラテジ</glossterm>の大きさを指定します。
...
設定を高くすると、<command>VACUUM</command>および<command>ANALYZE</command>の実行速度が速くなりますが、設定を高くしすぎると、他の多くの有用なページが共有バッファから削除されてしまう可能性があります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the maximum amount of memory to be used by logical decoding,
        ...
        reducing the amount of decoded changes written to disk.␟デコードされた更新がローカルディスクに書かれる前にロジカルデコーディングが使用する最大のメモリ量を指定します。
これにより、ロジカルストリーミングレプリケーションの接続が使用する最大メモリが制限されます。
デフォルトは64メガバイト(<literal>64MB</literal>)です。
個々のレプリケーション接続がここで指定した単一のバッファだけを使用し、インストールは通常たくさんの接続を並行して使わないので（<varname>max_wal_senders</varname>で制限されます）、この値を<varname>work_mem</varname>よりもずっと大きくしても安全で、それによってデコードされた更新がディスクに書かれる量が削減されます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the amount of memory to use to cache the contents of
        ...
        This parameter can only be set at server start.␟<literal>pg_commit_ts</literal>の内容をキャッシュするために使用するメモリ量を指定します（<xref linkend="pgdata-contents-table"/>を参照してください）。
...
␝       <para>␟        Specifies the amount of shared memory to use to cache the contents
        of <literal>pg_multixact/members</literal> (see
        <xref linkend="pgdata-contents-table"/>).
        If this value is specified without units, it is taken as blocks,
        that is <symbol>BLCKSZ</symbol> bytes, typically 8kB.
        The default value is <literal>32</literal>.
        This parameter can only be set at server start.␟<literal>pg_multixact/members</literal>の内容をキャッシュするために使用する共有メモリの量を指定します（<xref linkend="pgdata-contents-table"/>を参照してください）。
この値が単位なしで指定された場合、ブロック、つまり<symbol>BLCKSZ</symbol>バイト、通常は8キロバイトとして扱われます。
...
␝       <para>␟        Specifies the amount of shared memory to use to cache the contents
        ...
        This parameter can only be set at server start.␟<literal>pg_multixact/offsets</literal>の内容をキャッシュするために使用する共有メモリの量を指定します（<xref linkend="pgdata-contents-table"/>を参照してください）。
この値が単位なしで指定された場合、ブロック、つまり<symbol>BLCKSZ</symbol>バイト、通常は8キロバイトとして扱われます。
...
␝       <para>␟        Specifies the amount of shared memory to use to cache the contents
        ...
        that is <symbol>BLCKSZ</symbol> bytes, typically 8kB.
        The default value is <literal>16</literal>.
        This parameter can only be set at server start.␟<literal>pg_notify</literal>の内容をキャッシュするために使用する共有メモリの量を指定します（<xref linkend="pgdata-contents-table"/>を参照してください）。
この値が単位なしで指定された場合、ブロック、つまり<symbol>BLCKSZ</symbol>バイト、通常は8キロバイトとして扱われます。
デフォルト値は<literal>16</literal>です。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the amount of shared memory to use to cache the contents
        ...
        that is <symbol>BLCKSZ</symbol> bytes, typically 8kB.
        The default value is <literal>32</literal>.
        This parameter can only be set at server start.␟<literal>pg_serial</literal>の内容をキャッシュするために使用する共有メモリの量を指定します（<xref linkend="pgdata-contents-table"/>を参照してください）。
この値が単位なしで指定された場合、ブロック、つまり<symbol>BLCKSZ</symbol>バイト、通常は8キロバイトとして扱われます。
デフォルト値は<literal>32</literal>です。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the amount of shared memory to use to cache the contents
        ...
        <varname>shared_buffers</varname>/512 up to 1024 blocks,
        but not fewer than 16 blocks.
        This parameter can only be set at server start.␟<literal>pg_subtrans</literal>の内容をキャッシュするために使用する共有メモリの量を指定します（<xref linkend="pgdata-contents-table"/>を参照してください）。
この値が単位なしで指定された場合、ブロック、つまり<symbol>BLCKSZ</symbol>バイト、通常は8キロバイトとして扱われます。
デフォルト値は<literal>0</literal>で、最大1024ブロック、16ブロック以上の<varname>shared_buffers</varname>/512を要求します。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the amount of shared memory to use to cache the contents
        ...
        <varname>shared_buffers</varname>/512 up to 1024 blocks,
        but not fewer than 16 blocks.
        This parameter can only be set at server start.␟<literal>pg_xact</literal>の内容をキャッシュするために使用する共有メモリの量を指定します（<xref linkend="pgdata-contents-table"/>を参照してください）。
この値が単位なしで指定された場合、ブロック、つまり<symbol>BLCKSZ</symbol>バイト、通常は8キロバイトとして扱われます。
デフォルト値は<literal>0</literal>で、最大1024ブロック、16ブロック以上の<varname>shared_buffers</varname>/512を要求します。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝       <para>␟        Specifies the maximum safe depth of the server's execution stack.
        ...
        privilege can change this setting.␟サーバの実行スタックの最大安全深度を指定します。
...
スーパーユーザと、適切な<literal>SET</literal>権限を持つユーザのみがこの設定を変更することができます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Setting <varname>max_stack_depth</varname> higher than
        ...
        so caution is recommended in selecting a value.␟<varname>max_stack_depth</varname>を実際のカーネルの制限よりも高い値に設定した場合、暴走した再帰関数により、個々のバックエンドプロセスがクラッシュするかもしれません。
<productname>PostgreSQL</productname>がカーネルの制限を決定することができるプラットフォームでは、この変数を危険な値に設定させません。
しかし、すべてのプラットフォームがこの情報を提供できるわけではありません。
このため、値を選ぶ時には注意が必要です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the shared memory implementation that the server
        ...
        linkend="sysvipc"/>).␟<productname>PostgreSQL</productname>の共有バッファおよび他の共有データを保持する主共有メモリ領域のためにサーバが使用すべき共有メモリの実装を指定します。
...
␝       <para>␟        Specifies the dynamic shared memory implementation that the server
        ...
        other shared memory facilities are not available.␟サーバが使う動的共有メモリの実装を指定します。可能な値は<literal>posix</literal> (<literal>shm_open</literal>で獲得するPOSIX共有メモリ)、<literal>sysv</literal>(<literal>shmget</literal>で獲得するSystem V共有メモリ)、<literal>windows</literal> (Windows共有メモリ)、 <literal>mmap</literal>(データディレクトリ内のメモリマップファイルを使ってシミュレートする共有メモリ)です。
すべての値がすべてのプラットフォームでサポートされているわけではありません。
...
しかし、デバッグ目的のために<literal>pg_dynshmem</literal>ディレクトリがRAMディスク上にある場合や、他の共有メモリ機能が使えない場合は有用かもしれません。␞␞       </para>␞
␝       <para>␟        Specifies the amount of memory that should be allocated at server
        ...
        only be set at server start.␟サーバ起動時にパラレルクエリ用に獲得するメモリの量を指定します。
...
␝     <sect2 id="runtime-config-resource-disk">␟     <title>Disk</title>␟     <title>ディスク</title>␞␞␞
␝       <para>␟        Specifies the maximum amount of disk space that a process can use
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟あるプロセスが一時ファイルとして使用できるディスクの最大容量を設定します。
例えば、ソートやハッシュの一時ファイルであったり、カーソルを保持する格納ファイルです。
この制限値を超えようとするトランザクションはキャンセルされます。
この値が単位なしで指定された場合は、キロバイト単位であるとみなします。
<literal>-1</literal>（デフォルトです）の場合は制限がありません。
この設定はスーパーユーザと、適切な<literal>SET</literal>権限を持つユーザのみ変更可能です。␞␞       </para>␞
␝       <para>␟        This setting constrains the total space used at any instant by all
        ...
        execution, does <emphasis>not</emphasis> count against this limit.␟この設定により、ある <productname>PostgreSQL</productname> セッションによって使用される一時ファイルの合計の容量が常に制約されることになります。
なお、問い合わせの実行において暗黙的に使用される一時ファイルとは異なり、一時テーブルとして明示的に使用されるディスク容量は、この制限には<emphasis>含まれません</emphasis>。␞␞       </para>␞
␝       <para>␟        Specifies the maximum amount of allocated pages for
        <xref linkend="sql-notify"/> / <xref linkend="sql-listen"/> queue.
        The default value is 1048576. For 8 KB pages it allows to consume
        up to 8 GB of disk space.␟<xref linkend="sql-notify"/>/<xref linkend="sql-listen"/>キューのために割り当てられたページの最大量を指定します。
デフォルト値は1048576です。
8キロバイトページの場合、最大8ギガバイトのディスクスペースが消費されます。␞␞       </para>␞
␝      <indexterm>
...
␝     <sect2 id="runtime-config-resource-kernel">␟     <title>Kernel Resource Usage</title>␟     <title>カーネル資源使用</title>␞␞␞
␝       <para>␟        Sets the maximum number of simultaneously open files allowed to each
        ...
        This parameter can only be set at server start.␟それぞれのサーバ子プロセスが同時にオープンできるファイル数の最大値をセットします。
デフォルトは1000ファイルです。
...
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝    <sect2 id="runtime-config-resource-vacuum-cost">␟     <title>Cost-based Vacuum Delay</title>␟     <title>コストに基づくVacuum遅延</title>␞␞␞
␝     <para>␟      During the execution of <xref linkend="sql-vacuum"/>
      ...
      counter and continue execution.␟<xref linkend="sql-vacuum"/> および <xref linkend="sql-analyze"/> コマンドの実行中、実行される各種I/O操作の予測コストを追跡し続ける内部カウンタをシステムが保守します。
累積されたコストが（<varname>vacuum_cost_limit</varname>で指定された）限度に達すると、操作を実行しているプロセスは<varname>vacuum_cost_delay</varname>で指定されたちょっとの間スリープします。その後、カウンタをリセットし、実行を継続します。␞␞     </para>␞
␝     <para>␟      The intent of this feature is to allow administrators to reduce
      ...
      delay provides a way for administrators to achieve this.␟この機能の目的は、同時に実行されているデータベースの活動に対するこれらコマンドによるI/Oへの影響を、管理者が軽減できるようにすることです。
...
コストに基づいたvacuum遅延はこれを実現するための方法を管理者に提供します。␞␞     </para>␞
␝       <indexterm>
...
␝     <para>␟      This feature is disabled by default for manually issued
      <command>VACUUM</command> commands. To enable it, set the
      <varname>vacuum_cost_delay</varname> variable to a nonzero
      value.␟手動で実行した<command>VACUUM</command>コマンドについては、デフォルトでこの機能は無効になっています。
有効にするには、<varname>vacuum_cost_delay</varname>変数をゼロでない値に設定します。␞␞     </para>␞
␝        <para>␟         The amount of time that the process will sleep
         when the cost limit has been exceeded.
         If this value is specified without units, it is taken as milliseconds.
         The default value is zero, which disables the cost-based vacuum
         delay feature.  Positive values enable cost-based vacuuming.␟コストの限度を越えた場合、プロセスがスリープする時間の長さです。
この値が単位なしで指定された場合は、ミリ秒単位であるとみなします。
デフォルトの値は0で、コストに基づいたvacuum遅延機能を無効にします。
正の整数はコストに基づいたvacuumを有効にします。␞␞        </para>␞
␝       <indexterm>
...
␝        <para>␟         When using cost-based vacuuming, appropriate values for
         ...
         will consistently measure; large delays are not helpful.␟コストに基づいたバキューム処理を使用する場合、<varname>vacuum_cost_delay</varname>の適切な値は通常かなり小さくなり、おそらく1ミリ秒以下になります。
...
␝        <para>␟         The estimated cost for vacuuming a buffer found in the shared buffer
         cache. It represents the cost to lock the buffer pool, lookup
         the shared hash table and scan the content of the page. The
         default value is one.␟共有バッファキャッシュの中のバッファにvacuumを掛ける予測コストです。バッファプールのロック、共有ハッシュテーブルの検索、およびページ内容のスキャンのコストを示します。デフォルトの値は1です。␞␞        </para>␞
␝       <indexterm>
...
␝        <para>␟         The estimated cost for vacuuming a buffer that has to be read from
         disk.  This represents the effort to lock the buffer pool,
         lookup the shared hash table, read the desired block in from
         the disk and scan its content. The default value is 2.␟ディスクから読み込まれなければならないバッファにvacuumを掛ける予測コストです。これが示すものは、バッファプールロックの試み、共有ハッシュテーブルの参照、ディスクから目的ブロックの読み込み、そしてその内容のスキャンです。デフォルトの値は2です。␞␞        </para>␞
␝       <indexterm>
...
␝        <para>␟         The estimated cost charged when vacuum modifies a block that was
         previously clean. It represents the extra I/O required to
         flush the dirty block out to disk again. The default value is
         20.␟vacuumが、先だって掃除したブロックを変更するのに必要な推定コストです。
ダーティブロックを再度ディスクにフラッシュするのに必要な余分なI/Oを表します。デフォルトの値は20です。␞␞        </para>␞
␝        <para>␟         This is the accumulated cost that will cause the vacuuming process to sleep
         for <varname>vacuum_cost_delay</varname>.  The default is 200.␟vacuumを掛けるプロセスを<varname>vacuum_cost_delay</varname>の間スリープさせることになる累計されたコストです。
デフォルトの値は200です。␞␞        </para>␞
␝      <para>␟       There are certain operations that hold critical locks and should
       ...
       <varname>vacuum_cost_delay</varname> * 4.␟重要なロックを保有し可能なかぎり早急に完了しなければならないある種の操作があります。コストに基づいたvacuum遅延はこの様な操作では起こりません。
...
<varname>vacuum_cost_delay</varname> * <varname>accumulated_balance</varname> / <varname>vacuum_cost_limit</varname>␞␞      </para>␞
␝    <sect2 id="runtime-config-resource-background-writer">␟     <title>Background Writer</title>␟     <title>バックグラウンドライタ</title>␞␞␞
␝       <indexterm>
...
␝     <para>␟      There is a separate server
      ...
      can be used to tune the behavior for local needs.␟<firstterm>バックグラウンドライタ</firstterm>と呼ばれる個別のサーバプロセスがあり、その機能は（新規または更新された）<quote>ダーティ</quote>な共有バッファの書き込みを行うことです。
...
この副節で説明する各パラメータは、サイト独自の必要に応じて動作を調整することに使用できます。␞␞     </para>␞
␝       <indexterm>
...
␝        <para>␟         Specifies the delay between activity rounds for the
         ...
         <filename>postgresql.conf</filename> file or on the server command line.␟バックグラウンドライタの動作周期間の遅延を指定します。
...
このパラメータは<filename>postgresql.conf</filename>ファイル、または、サーバのコマンドラインで設定可能です。␞␞        </para>␞
␝       <indexterm>
...
␝        <para>␟         In each round, no more than this many buffers will be written
         ...
         This parameter can only be set in the <filename>postgresql.conf</filename>
         file or on the server command line.␟それぞれの周期で、この数以上のバッファはバックグラウンドライタにより書き込まれません。
...
␝        <para>␟         The number of dirty buffers written in each round is based on the
         ...
         The default is 2.0.
         This parameter can only be set in the <filename>postgresql.conf</filename>
         file or on the server command line.␟各周期で書き出されるダーティバッファ数は、最近の周期でサーバプロセスが必要とした新しいバッファ数を基にします。
...
デフォルトは2.0です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞        </para>␞
␝        <para>␟         Whenever more than this amount of data has
         ...
         This parameter can only be set in the <filename>postgresql.conf</filename>
         file or on the server command line.␟バックグラウンドライタがこの値より多く書く度に、OSが記憶装置に書き込むことを強制しようとします。
...
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞        </para>␞
␝     <para>␟      Smaller values of <varname>bgwriter_lru_maxpages</varname> and
      <varname>bgwriter_lru_multiplier</varname> reduce the extra I/O load
      caused by the background writer, but make it more likely that server
      processes will have to issue writes for themselves, delaying interactive
      queries.␟<varname>bgwriter_lru_maxpages</varname>および<varname>bgwriter_lru_multiplier</varname>の値がより少ないと、バックグラウンドライタで引き起こされる追加のI/O負荷を軽減しますが、サーバプロセスが自分自身で行わなければならない書き込みが増加することになり、会話型問い合わせを遅らせることになります。␞␞     </para>␞
␝       <indexterm>
...
␝    <sect2 id="runtime-config-resource-async-behavior">␟     <title>Asynchronous Behavior</title>␟     <title>非同期動作</title>␞␞␞
␝       <indexterm>
...
␝        <para>␟         Whenever more than this amount of data has
         ...
         the maximum value scales proportionally to it.)␟この量を超えるデータが単一のバックエンドによって書き込まれたときはいつでも、OSが記憶装置に書き込むことを強制します。
...
（<symbol>BLCKSZ</symbol>が8kBでなければ、最大値が<symbol>BLCKSZ</symbol>に比例して変更されます。）␞␞        </para>␞
␝        <para>␟         Sets the number of concurrent disk I/O operations that
         ...
         this setting only affects bitmap heap scans.␟<productname>PostgreSQL</productname>が同時実行可能であると想定する同時ディスクI/O操作の数を設定します。
...
現在、この設定はビットマップヒープスキャンのみに影響します。␞␞        </para>␞
␝        <para>␟         For magnetic drives, a good starting point for this setting is the
         number of separate
         ...
         concurrent requests, so the best value might be in the hundreds.␟磁気ディスクドライブにおいては、データベースに使用されるRAID 0ストライプ、RAID 1ミラーを構成する個々のドライブ数から始めると良いでしょう。
...
SSDやそれ以外のメモリベースの記憶装置は、多くの同時リクエストをこなすことができるので、最適な値は数百になるかもしれません。␞␞        </para>␞
␝        <para>␟         Asynchronous I/O depends on an effective <function>posix_fadvise</function>
         function, which some operating systems lack.  If the function is not
         present then setting this parameter to anything but zero will result
         in an error.  On some operating systems (e.g., Solaris), the function
         is present but does not actually do anything.␟非同期I/Oは実質的に<function>posix_fadvise</function>関数に依存します。
これは一部のオペレーティングシステムには存在しません。
この関数が存在しない場合、この値をゼロ以外に設定するとエラーとなります。
一部のオペレーティングシステム（例えばSolaris）では存在するけれども、実際何も行わないものもあります。␞␞        </para>␞
␝       <indexterm>
...
␝        <para>␟         The default is 1 on supported systems, otherwise 0.  This value can
         be overridden for tables in a particular tablespace by setting the
         tablespace parameter of the same name (see
         <xref linkend="sql-altertablespace"/>).␟デフォルトは、サポートされているシステムでは1、そうでなければ0です。
この値は、テーブル空間パラメータの同じ名前のパラメータを設定することで、特定のテーブル空間内のテーブルに対して上書きできます。
(<xref linkend="sql-altertablespace"/>を参照ください)。␞␞        </para>␞
␝        <para>␟         Similar to <varname>effective_io_concurrency</varname>, but used
         for maintenance work that is done on behalf of many client sessions.␟<varname>effective_io_concurrency</varname>と似ていますが、多くのクライアントセッションのために行われる保守作業で適用されるところが異なります。␞␞        </para>␞
␝        <para>␟         The default is 10 on supported systems, otherwise 0.  This value can
         be overridden for tables in a particular tablespace by setting the
         tablespace parameter of the same name (see
         <xref linkend="sql-altertablespace"/>).␟サポートしているシステムではデフォルトは10で、それ以外は0です。
この値は同じ名前のテーブル空間パラメータを使って、特定のテーブル空間にあるテーブルのために置き換えることができます。（<xref linkend="sql-altertablespace"/>を参照してください。）␞␞        </para>␞
␝       <indexterm>
...
␝        <para>␟         Controls the largest I/O size in operations that combine I/O.
         The default is 128kB.␟I/Oを結合する操作で最大のI/Oサイズを制御します。
デフォルトは128キロバイトです。␞␞        </para>␞
␝        <para>␟         Sets the maximum number of background processes that the cluster
         can support.  This parameter can only be set at server start.  The
         default is 8.␟クラスタがサポートするバックグラウンドプロセスの最大数を指定します。
このパラメータはサーバ起動時のみ設定可能です。
デフォルトは8です。␞␞        </para>␞
␝        <para>␟         When running a standby server, you must set this parameter to the
         same or higher value than on the primary server. Otherwise, queries
         will not be allowed in the standby server.␟スタンバイサーバを起動しているときは、このパラメータを、プライマリサーバの設定値と同じかそれ以上にしなければなりません。
さもなければ、スタンバイサーバで問い合わせの実行ができなくなります。␞␞        </para>␞
␝       <indexterm>
...
␝        <para>␟         When changing this value, consider also adjusting
         <xref linkend="guc-max-parallel-workers"/>,
         <xref linkend="guc-max-parallel-maintenance-workers"/>, and
         <xref linkend="guc-max-parallel-workers-per-gather"/>.␟この値を変更する際は、<xref linkend="guc-max-parallel-workers"/>、<xref linkend="guc-max-parallel-maintenance-workers"/>、<xref linkend="guc-max-parallel-workers-per-gather"/>を変更することも考慮してください。␞␞        </para>␞
␝        <para>␟         Sets the maximum number of workers that can be started by a single
         ...
         disables parallel query execution.␟一つの<literal>Gather</literal>または<literal>Gather Merge</literal>ノードに対して起動できるワーカー数の最大値を設定します。
...
デフォルト値は2です。
この設定値を0にすると、パラレルクエリの実行は行われません。␞␞        </para>␞
␝        <para>␟         Note that parallel queries may consume very substantially more
         ...
         uses no workers at all.␟パラレルクエリの実行により、パラレルクエリではない場合に比べて非常に多くのリソースが使用されるかもしれないことに注意してください。
...
たとえば、あるパラレルクエリが4つのワーカーを使っているとすると、ワーカーを使わない場合に比べて、最大5倍のCPU時間、メモリ、I/Oバンド幅、その他を使うかもしれません。␞␞        </para>␞
␝       <indexterm>
...
␝        <para>␟         For more information on parallel query, see
         <xref linkend="parallel-query"/>.␟パラレルクエリに関する更なる情報については、<xref linkend="parallel-query"/>をご覧ください。␞␞        </para>␞
␝        <para>␟         Sets the maximum number of parallel workers that can be
         ...
         commands.␟単一のユーティリティコマンドで使用されるパラレルワーカーの最大数を設定します。
...
デフォルト値は2です。
0に設定すると、ユーティリティコマンドはパラレルワーカーを使用しません。␞␞        </para>␞
␝       <indexterm>
...
␝        <para>␟         Note that parallel utility commands should not consume
         ...
         and I/O bandwidth.␟パラレルユーティリティコマンドは同等の非パラレル操作よりもかなり多くのメモリを消費すべきでないことに留意してください。
...
しかし、パラレルユーティリティコマンドは、依然としてかなり多くのCPUリソースとI/Oバンド幅を消費するかも知れません。␞␞        </para>␞
␝       <indexterm>
...
␝        <para>␟         Sets the maximum number of workers that the cluster can support for
         ...
         established by that setting.␟パラレルクエリ操作用にクラスタがサポートできる最大のワーカー数を設定します。
デフォルト値は8です。
...
<xref linkend="guc-max-worker-processes"/>で決まるワーカープロセスのプールから、パラレルワーカーが使われるからです。␞␞        </para>␞
␝      </varlistentry>
     </variablelist>
    </sect2>
   </sect1>
...
␝        <para>␟         Allows the leader process to execute the query plan under
         ...
         query duration.␟ワーカープロセスを待つ代わりに、<literal>Gather</literal>ノードと<literal>Gather Merge</literal>ノード配下の問い合わせプランをリーダープロセスが実行できるようにします。
...
これがリーダーの性能を助けるのか、阻害要因になるかは計画型、ワーカーの数、問い合わせの実行時間の長さによります。␞␞        </para>␞
␝   <sect1 id="runtime-config-wal">␟    <title>Write Ahead Log</title>␟    <title>先行書き込みログ（WAL）</title>␞␞␞
␝   <para>␟    For additional information on tuning these settings,
    see <xref linkend="wal-configuration"/>.␟これらの設定をチューニングする追加情報は<xref linkend="wal-configuration"/>を参照してください。␞␞   </para>␞
␝      <indexterm>
       <primary><varname>wal_level</varname> configuration parameter</primary>
      </indexterm>
␟␟      <indexterm>
       <primary><varname>wal_level</varname>設定パラメータ</primary>
      </indexterm>␞␞     <variablelist>␞
␝    <sect2 id="runtime-config-wal-settings">␟     <title>Settings</title>␟     <title>諸設定</title>␞␞     <variablelist>␞
␝       <para>␟        <varname>wal_level</varname> determines how much information is written to
        ...
        levels.  This parameter can only be set at server start.␟<varname>wal_level</varname>はどれだけの情報がWALに書かれるかを決定します。
...
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝       <para>␟        The <literal>minimal</literal> level generates the least WAL
        ...
        optimization include:␟<literal>minimal</literal>レベルでは、最小限のWALの量しか生成されません。
永続リレーションを作成あるいは書き換えるトランザクションの差分に関する情報は記録されません。
これにより、それらの操作が大幅に高速になります（<xref linkend="populate-pitr"/>を参照してください）。
この最適化が適用される操作には以下のものがあげられます。␞␞        <simplelist>␞
␝         <member><command>REFRESH MATERIALIZED VIEW</command>␟         (without <option>CONCURRENTLY</option>)</member>␟         (<option>CONCURRENTLY</option>なし)</member>␞␞         <member><command>REINDEX</command></member>␞
␝        </simplelist>␟        However, minimal WAL does not contain sufficient information for
        ...
        for point-in-time recovery and standby servers.␟しかし、minimal WALにはポイントインタイムリカバリのための十分な情報が含まれていないので、<literal>replica</literal>以上を使って継続的アーカイブ(<xref linkend="guc-archive-mode"/>)とストリーミングバイナリレプリケーションを可能にしなければなりません。
実際、<varname>max_wal_senders</varname>が0以外の場合、サーバはこのモードで起動することさえありません。
<varname>wal_level</varname>を<literal>minimal</literal>に変更すると、以前のベースバックアップがポイントインタイムリカバリとスタンバイサーバで使用できなくなることに注意してください。␞␞       </para>␞
␝       <para>␟        In <literal>logical</literal> level, the same information is logged as
        ...
        executed.␟<literal>logical</literal>レベルでは、<literal>replica</literal>と同じ情報が記録されるのに加え、ロジカルチェンジセットをWALから取り出すのに必要な情報が追加されます。
<literal>logical</literal>を使うとWALの量が増えます。
とりわけ、多数のテーブルが<literal>REPLICA IDENTITY FULL</literal>と設定されていて(訳注: ALTER TABLE参照)、多くの<command>UPDATE</command>と<command>DELETE</command>文が実行される場合はこのことが言えます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        In releases prior to 9.6, this parameter also allowed the
        values <literal>archive</literal> and <literal>hot_standby</literal>.
        These are still accepted but mapped to <literal>replica</literal>.␟9.6よりも前のリリースでは、このパラメータは<literal>archive</literal>と<literal>hot_standby</literal>という設定値も可能でした。
引き続きこれらも受け付けられますが、<literal>replica</literal>へとマップされます。␞␞       </para>␞
␝       <para>␟        If this parameter is on, the <productname>PostgreSQL</productname> server
        ...
        consistent state after an operating system or hardware crash.␟このパラメータがオンの場合、<productname>PostgreSQL</productname>サーバは<function>fsync()</function>システムコールを発行するか、もしくはこれに相当する方法（<xref linkend="guc-wal-sync-method"/>を参照）で、更新が物理的にディスクに確実に書き込まれるように試みます。
これは、オペレーティングシステムもしくはハードウェアがクラッシュした後、データベースクラスタを一貫した状態に復旧させることを確実にします。␞␞       </para>␞
␝       <para>␟        While turning off <varname>fsync</varname> is often a performance
        ...
        data.␟<varname>fsync</varname>を停止することはしばしば性能上の利益になるとは言っても、停電やクラッシュの際に回復不可能なデータ破壊になることがあります。
従って外部データから全てのデータベースを簡単に再構築できる場合のみ<varname>fsync</varname>を停止してください。␞␞       </para>␞
␝       <para>␟        Examples of safe circumstances for turning off
        ...
        turning off <varname>fsync</varname>.␟<varname>fsync</varname>を停止しても安全な状況の例としては、以下があげられます。
...
高性能なハードウェアであるからと言って、<varname>fsync</varname>を停止することは正当性を主張する十分な理由とはなりません。␞␞       </para>␞
␝       <para>␟        For reliable recovery when changing <varname>fsync</varname>
        ...
        file system, or rebooting the server.␟<varname>fsync</varname>を無効(off)から有効(on)に変更したときの信頼できるリカバリのためには、カーネル内の全ての変更されたバッファを恒久的ストレージに強制的に吐き出させることが必要です。
これは、クラスタがシャットダウンしている間、または<varname>fsync</varname>が有効のときに、<command>initdb --sync-only</command>を実行する、<command>sync</command>を実行する、ファイルシステムをアンマウントする、またはサーバを再起動することによって可能となります。␞␞       </para>␞
␝       <para>␟        In many situations, turning off <xref linkend="guc-synchronous-commit"/>
        for noncritical transactions can provide much of the potential
        performance benefit of turning off <varname>fsync</varname>, without
        the attendant risks of data corruption.␟多くの場合、重要でないトランザクションに対して<xref linkend="guc-synchronous-commit"/>を無効にすることにより、データ破壊という付随的危険性を伴うことなく、<varname>fsync</varname>を無効にすることで得られるであろう性能上のメリットの多くを得ることができます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        <varname>fsync</varname> can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.
        If you turn this parameter off, also consider turning off
        <xref linkend="guc-full-page-writes"/>.␟<varname>fsync</varname> は<filename>postgresql.conf</filename>ファイル、または、サーバのコマンドラインでのみ設定可能です。
このパラメータを無効にする場合、<xref linkend="guc-full-page-writes"/>も同時に無効にすることを検討してください。␞␞       </para>␞
␝       <para>␟        Specifies how much WAL processing must complete before
        ...
        <literal>local</literal>, and <literal>off</literal>.␟トランザクションのコミットがクライアントに<quote>成功</quote>の報告を返す前に、どれだけのWAL処理を完了しなければならないかを指定をします。
有効な値は<literal>remote_apply</literal>、<literal>on</literal>(デフォルト)、<literal>remote_write</literal>、<literal>local</literal>、<literal>off</literal>です。␞␞       </para>␞
␝       <para>␟        If <varname>synchronous_standby_names</varname> is empty,
        ...
        discussion see <xref linkend="wal-async-commit"/>.␟<varname>synchronous_standby_names</varname>が空文字なら、意味のある設定は<literal>on</literal>と<literal>off</literal>だけです。
...
詳細は<xref linkend="wal-async-commit"/>を参照してください。␞␞       </para>␞
␝       <para>␟        If <xref linkend="guc-synchronous-standby-names"/> is non-empty,
        <varname>synchronous_commit</varname> also controls whether
        transaction commits will wait for their WAL records to be
        processed on the standby server(s).␟<xref linkend="guc-synchronous-standby-names"/>が空文字でない場合は、<varname>synchronous_commit</varname>は、WALレコードが、スタンバイサーバに複製されるまでトランザクションコミットを待機するか否かも制御します。␞␞       </para>␞
␝       <para>␟        When set to <literal>remote_apply</literal>, commits will wait
        ...
        completeness.␟<literal>remote_apply</literal>に設定すると、現在の同期スタンバイがトランザクションのコミットレコードを受け取って適用し、スタンバイ上で発行されたクエリから見えるようになり、スタンバイ上の永続的な記憶装置に書き込まれたことを報告するまでコミットは待機します。
...
これは通常同期レプリケーションが使用されている場合は望ましい設定ではありませんが、完全さのために提供されています。␞␞       </para>␞
␝       <para>␟        This parameter can be changed at any time; the behavior for any
        one transaction is determined by the setting in effect when it
        commits.  It is therefore possible, and useful, to have some
        transactions commit synchronously and others asynchronously.
        For example, to make a single multistatement transaction commit
        asynchronously when the default is the opposite, issue <command>SET
        LOCAL synchronous_commit TO OFF</command> within the transaction.␟このパラメータはいつでも変更可能です。
どのトランザクションの動作も、コミット時に有効であった設定によって決まります。
したがって、一部のトランザクションのコミットを同期的に、その他を非同期的にすることが可能で、かつ、有用です。
例えば、デフォルトが同期コミットの場合に複数文トランザクションを一つだけ非同期にコミットさせるためには、トランザクション内で<command>SET LOCAL synchronous_commit TO OFF</command>を発行します。␞␞       </para>␞
␝       <para>␟        <xref linkend="synchronous-commit-matrix"/> summarizes the
        capabilities of the <varname>synchronous_commit</varname> settings.␟<varname>synchronous_commit</varname>設定の機能のまとめが<xref linkend="synchronous-commit-matrix"/>にあります。␞␞       </para>␞
␝       <table id="synchronous-commit-matrix">␟        <title>synchronous_commit Modes</title>␟        <title>synchronous_commitモード</title>␞␞        <tgroup cols="5">␞
␝          <row>␟           <entry>synchronous_commit setting</entry>␟           <entry>synchronous_commit設定</entry>␞␞␞
␝           <entry>synchronous_commit setting</entry>␟           <entry>local durable commit</entry>␟           <entry>localの永続的なコミット</entry>␞␞␞
␝           <entry>local durable commit</entry>␟           <entry>standby durable commit after PG crash</entry>␟           <entry>PGがクラッシュした後のスタンバイの永続的なコミット</entry>␞␞␞
␝           <entry>standby durable commit after PG crash</entry>␟           <entry>standby durable commit after OS crash</entry>␟           <entry>OSがクラッシュした後のスタンバイの永続的なコミット</entry>␞␞␞
␝           <entry>standby durable commit after OS crash</entry>␟           <entry>standby query consistency</entry>␟           <entry>スタンバイのクエリの一貫性</entry>␞␞          </row>␞
␝           <entry>remote_apply</entry>␟           <entry align="center">&bull;</entry>␟           <entry align="center">○</entry>␞␞␞
␝           <entry align="center">&bull;</entry>␟           <entry align="center">&bull;</entry>␟           <entry align="center">○</entry>␞␞␞
␝           <entry align="center">&bull;</entry>␟           <entry align="center">&bull;</entry>␟           <entry align="center">○</entry>␞␞␞
␝           <entry align="center">&bull;</entry>␟           <entry align="center">&bull;</entry>␟           <entry align="center">○</entry>␞␞          </row>␞
␝           <entry>on</entry>␟           <entry align="center">&bull;</entry>␟           <entry align="center">○</entry>␞␞␞
␝           <entry align="center">&bull;</entry>␟           <entry align="center">&bull;</entry>␟           <entry align="center">○</entry>␞␞␞
␝           <entry align="center">&bull;</entry>␟           <entry align="center">&bull;</entry>␟           <entry align="center">○</entry>␞␞           <entry align="center"></entry>␞
␝           <entry>remote_write</entry>␟           <entry align="center">&bull;</entry>␟           <entry align="center">○</entry>␞␞␞
␝           <entry align="center">&bull;</entry>␟           <entry align="center">&bull;</entry>␟           <entry align="center">○</entry>␞␞           <entry align="center"></entry>␞
␝      <indexterm>
...
␟␟      <indexterm>
      ...
      </indexterm>␞␞           <entry align="center"></entry>␞
␝           <entry>local</entry>␟           <entry align="center">&bull;</entry>␟           <entry align="center">○</entry>␞␞           <entry align="center"></entry>␞
␝       <para>␟        Method used for forcing WAL updates out to disk.
        If <varname>fsync</varname> is off then this setting is irrelevant,
        since WAL file updates will not be forced out at all.
        Possible values are:␟WALの更新をディスクへ強制するのに使用される方法です。
<varname>fsync</varname>がオフの場合この設定は役に立ちません。と言うのはWALファイルの更新が全く強制されないからです。取り得る値は以下のものです。␞␞       </para>␞
␝        <para>␟         <literal>open_datasync</literal> (write WAL files with <function>open()</function> option <symbol>O_DSYNC</symbol>)␟<literal>open_datasync</literal>（<function>open()</function>のオプション<symbol>O_DSYNC</symbol>でWALファイルに書き込む）␞␞        </para>␞
␝        <para>␟         <literal>fdatasync</literal> (call <function>fdatasync()</function> at each commit)␟<literal>fdatasync</literal>（コミット毎に<function>fdatasync()</function>を呼び出す）␞␞        </para>␞
␝        <para>␟         <literal>fsync</literal> (call <function>fsync()</function> at each commit)␟<literal>fsync</literal>（コミット毎に<function>fsync()</function>を呼び出す）␞␞        </para>␞
␝        <para>␟         <literal>fsync_writethrough</literal> (call <function>fsync()</function> at each commit, forcing write-through of any disk write cache)␟<literal>fsync_writethrough</literal>（すべてのディスク書き込みキャッシュをライトスルーさせるため、コミット毎に<function>fsync()</function>を呼び出す）␞␞        </para>␞
␝        <para>␟         <literal>open_sync</literal> (write WAL files with <function>open()</function> option <symbol>O_SYNC</symbol>)␟<literal>open_sync</literal>（<function>open()</function>のオプション<symbol>O_SYNC</symbol>でWALファイルに書き込む）␞␞        </para>␞
␝      <indexterm>
...
␝       <para>␟        Not all of these choices are available on all platforms.
        ...
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟全てのプラットフォームでこれら全ての選択肢が使えるわけではありません。
...
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        When this parameter is on, the <productname>PostgreSQL</productname> server
        ...
        increase the checkpoint interval parameters.)␟このパラメータが有効の場合、<productname>PostgreSQL</productname>サーバは、チェックポイントの後にそのページが最初に変更された過程で、ディスクページの全ての内容をWALに書き込みます。
...
従って、完全ページ書き出しのコストを低減する方法の1つは、チェックポイント間隔パラメータを大きくすることです。）␞␞       </para>␞
␝       <para>␟        Turning this parameter off speeds normal operation, but
        might lead to either unrecoverable data corruption, or silent
        data corruption, after a system failure. The risks are similar to turning off
        <varname>fsync</varname>, though smaller, and it should be turned off
        only based on the same circumstances recommended for that parameter.␟このパラメータを無効にすると、通常の操作速度が上がりますが、システム障害後に、回復不能なデータ破損、あるいは警告なしのデータ損壊をもたらすかもしれません。
このリスクは小さいながら<varname>fsync</varname>を無効にした場合と似ています。そしてその<varname>fsync</varname>に対して推奨されている同一の状況に基づく限りにおいて停止されなければなりません。␞␞       </para>␞
␝       <para>␟        Turning off this parameter does not affect use of
        WAL archiving for point-in-time recovery (PITR)
        (see <xref linkend="continuous-archiving"/>).␟このパラメータを無効にしてもポイントインタイムリカバリ（PITR）用のWALアーカイブの使用に影響ありません（ <xref linkend="continuous-archiving"/>を参照してください）。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.
        The default is <literal>on</literal>.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝       <para>␟        When this parameter is <literal>on</literal>, the <productname>PostgreSQL</productname>
        ...
        non-critical modifications of so-called hint bits.␟このパラメータが<literal>on</literal>の場合、<productname>PostgreSQL</productname>サーバはチェックポイント後にはじめてページを変更する際に、ディスクページの全内容をWALに書き出します。
これは、あまり重要でない、ヒントビットと呼ばれるものに対する変更にさえ当てはまります。␞␞       </para>␞
␝       <para>␟        If data checksums are enabled, hint bit updates are always WAL-logged
        ...
        enabled.␟データチェックサムが有効であると、ヒントビットの更新は常にWALにログされ、この設定パラメータは無視されます。この設定パラメータを使って、データチェックサムが有効なときにどれだけのWALログは余計に書きだされるかをテストすることができます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter can only be set at server start. The default value is <literal>off</literal>.␟このパラメータはサーバ起動時のみ設定可能です。
デフォルト値は<literal>off</literal>です。␞␞       </para>␞
␝       <para>␟        This parameter enables compression of WAL using the specified
        compression method.
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟このパラメータは、指定された圧縮方式を使用したWALの圧縮を有効にします。
...
スーパーユーザと適切な<literal>SET</literal>権限を持つユーザのみがこの設定を変更できます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Enabling compression can reduce the WAL volume without
        increasing the risk of unrecoverable data corruption,
        but at the cost of some extra CPU spent on the compression during
        WAL logging and on the decompression during WAL replay.␟このパラメータを有効にすると、回復不可能なデータ破壊のリスクを増やすこと無しにWALの量を減らすことができます。
しかし、WALロギングの際の圧縮のため、またWALリプレイの際には伸張のために余分なCPUを使用するというコストが発生します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        If set to <literal>on</literal> (the default), this option causes new
        ...
        when the file is created so that it has the expected size.␟このオプションが<literal>on</literal>（デフォルト）に設定されると、新しいWALファイルはゼロで初期化されます。
...
<literal>off</literal>に設定すると、ファイルを作る時に期待する大きさになるように最後のバイトだけが書かれます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        If set to <literal>on</literal> (the default), this option causes WAL
        ...
        option is given to disable this behavior.␟このオプションが<literal>on</literal>（デフォルト）に設定されると、新しくファイルを作るのを避けるために、名前を変えてWALファイルが再利用されます。
COWファイルシステムでは、新しいファイルを作るほうが速いかも知れないので、この挙動を無効にできるようにオプションとなっています。␞␞       </para>␞
␝       <para>␟        The amount of shared memory used for WAL data that has not yet been
        ...
        This parameter can only be set at server start.␟未だディスクに書き込まれていないWALデータに対して使用される共有メモリ容量です。
...
␝       <para>␟        The contents of the WAL buffers are written out to disk at every
        transaction commit, so extremely large values are unlikely to
        provide a significant benefit.  However, setting this value to at
        least a few megabytes can improve write performance on a busy
        server where many clients are committing at once.  The auto-tuning
        selected by the default setting of -1 should give reasonable
        results in most cases.␟WALバッファの内容はトランザクションのコミット毎にディスクに書き込まれます。
したがって、極端に大きな値は有意な効果を期待できません。
しかし、この値を数メガバイトに設定することにより、多くのクライアントが同時にコミットするトラフィック量の多いサーバでは書き込み性能が向上します。
デフォルト設定の-1で選択される自動チューニングによると、ほとんどの場合妥当な結果が得られます。␞␞       </para>␞
␝      <indexterm>
...
␝      <para>␟        Specifies how often the WAL writer flushes WAL, in time terms.
        ...
        <filename>postgresql.conf</filename> file or on the server command line.␟WALライタがWALをフラッシュする頻度を時間で指定します。
...
␝      <para>␟        Specifies how often the WAL writer flushes WAL, in volume terms.
        ...
        This parameter can only be set in the
        <filename>postgresql.conf</filename> file or on the server command line.␟WALライタがWALをフラッシュする頻度を量で指定します。
最後のフラッシュが過去<varname>wal_writer_delay</varname>以内に行われ、かつそれ以降<varname>wal_writer_flush_after</varname>相当のWALが生成されていない場合は、WALはオペレーティングシステムに書き込まれますが、ディスクにはフラッシュされません。
...
␝       <para>␟        When <varname>wal_level</varname> is <literal>minimal</literal> and a
        ...
        (<literal>2MB</literal>).␟<varname>wal_level</varname>が<literal>minimal</literal>で、トランザクションのコミットが永続リレーションを作るかあるいは書き換えた場合に、この設定は新しいデータをどのように永続させるかを決定します。
...
デフォルトは2メガバイト(<literal>2MB</literal>)です。␞␞       </para>␞
␝       <para>␟        Setting <varname>commit_delay</varname> adds a time delay
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟<varname>commit_delay</varname>を設定することにより、WALフラッシュを開始する前の時間遅延が追加されます。
...
␝       <para>␟        In <productname>PostgreSQL</productname> releases prior to 9.3,
        ...
        completes the flush operation.␟9.3より前の<productname>PostgreSQL</productname>では、<varname>commit_delay</varname>の振る舞いは異なっており、あまり効果がありませんでした。
全てのWALフラッシュではなく、コミットだけに影響していました。また、そしてWALフラッシュが早めに完了しても設定された遅延分待機していました。
<productname>PostgreSQL</productname> 9.3以降では、フラッシュの準備が整った最初のプロセスが設定値分待機し、後続のプロセスは最初のプロセスがフラッシュ操作を完了するまでの間だけ待機をします。␞␞       </para>␞
␝       <para>␟        Minimum number of concurrent open transactions to require
        before performing the <varname>commit_delay</varname> delay. A larger
        value makes it more probable that at least one other
        transaction will become ready to commit during the delay
        interval. The default is five transactions.␟<varname>commit_delay</varname>の遅延を実行するときに必要とされる同時に開いているトランザクションの最小数です。
より大きい値にすると、遅延周期の間に、少なくとも1つの他のトランザクションのコミットの準備が整う確率が高くなります。
デフォルトは5トランザクションです。␞␞       </para>␞
␝      <indexterm>
...
␝     <sect2 id="runtime-config-wal-checkpoints">␟     <title>Checkpoints</title>␟     <title>チェックポイント</title>␞␞␞
␝      <indexterm>
...
␝       <para>␟        Maximum time between automatic WAL checkpoints.
        ...
        for crash recovery.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟自動的WALチェックポイント間の最大間隔を指定します。
この値が単位なしで指定された場合は、秒単位であるとみなします。
有効な範囲は、30秒から1日の間です。
...
␝       <para>␟        Specifies the target of checkpoint completion, as a fraction of
        ...
        or on the server command line.␟チェックポイントの間の時間のうち、どの程度の割合を使うかをチェックポイントの完了目標として指定します。
...
␝       <para>␟        Whenever more than this amount of data has been
        ...
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟チェックポイント実行中にこのデータ量よりも多く書く度に、OSが記憶装置に書き込むことを強制しようとします。
このことにより、カーネルのページキャッシュが持つダーティデータの量を一定量に制限し、チェックポイントの最後に<function>fsync</function>が実行される際、あるいはOSがバックグラウンドでデータを大きな塊で書き出す際に性能の急激な低下を招く可能性を減らします。
多くの場合これによってトランザクションの遅延が大幅に少なくなりますが、あるケース、特にワークロードが<xref linkend="guc-shared-buffers"/>よりも大きく、OSのページキャッシュよりも小さい時には性能が低下するかもしれません。
この設定が無効なプラットフォームがあります。
この値が単位なしで指定された場合は、ブロック単位であるとみなします。すなわち、<symbol>BLCKSZ</symbol>バイト、一般的には8kBです。
...
␝       <para>␟        Write a message to the server log if checkpoints caused by
        ...
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟WALセグメントファイルが溢れることが原因で起きるチェックポイントが、ここで指定した時間よりも短い間隔で発生したとき、サーバログにメッセージを書き出します
...
␝       <para>␟        Maximum size to let the WAL grow during automatic
        ...
        crash recovery.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟自動WALチェックポイントの際にWALが増加する最大サイズです。
...
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        As long as WAL disk usage stays below this setting, old WAL files are
        ...
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟この設定以下にWALのディスク使用量が保たれる限り、古いWALファイルは、消去されることなく今後のチェックポイントで使用するために常にリサイクルされます。
...
␝     <sect2 id="runtime-config-wal-archiving">␟     <title>Archiving</title>␟     <title>アーカイビング</title>␞␞␞
␝       <para>␟        When <varname>archive_mode</varname> is enabled, completed WAL segments
        ...
        <xref linkend="continuous-archiving-in-standby"/> for details.␟<varname>archive_mode</varname>が有効な場合、<xref linkend="guc-archive-command"/>あるいは<xref linkend="guc-archive-library"/>を設定することにより、完了したWALセグメントはアーカイブ格納領域に送信されます。
...
詳細は<xref linkend="continuous-archiving-in-standby"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        <varname>archive_mode</varname> is a separate setting from
        ...
        <varname>wal_level</varname> is set to <literal>minimal</literal>.␟<varname>archive_command</varname>と<varname>archive_library</varname>がアーカイブモードを停止することなく変更できるように、<varname>archive_mode</varname>は、<varname>archive_command</varname>と<varname>archive_library</varname>とは別の設定項目になっています。
このパラメータはサーバ起動時のみ設定可能です。
<varname>wal_level</varname>が<literal>minimal</literal>に設定されている場合、<varname>archive_mode</varname>を有効にすることはできません。␞␞       </para>␞
␝       <para>␟        The local shell command to execute to archive a completed WAL file
        ...
        <xref linkend="backup-archiving-wal"/>.␟完了したWALファイルセグメントのアーカイブを実行するローカルのシェルコマンドです。
...
詳しくは<xref linkend="backup-archiving-wal"/>を参照ください。␞␞       </para>␞
␝       <para>␟        This parameter can only be set in the <filename>postgresql.conf</filename>
        ...
        archive recovery, so it should only be used in unusual circumstances.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
...
例えば、<literal>/bin/true</literal>（Windowsでは<literal>REM</literal>）のように、真を返すだけで何もしないコマンドを<varname>archive_command</varname>に設定すると、実質的にアーカイブ処理が無効になりますが、アーカイブからの復帰に必要なWALファイルの連鎖も同時に断ち切るため、特別な場合のみ使用するようにしなければなりません。␞␞       </para>␞
␝       <para>␟        The library to use for archiving completed WAL file segments.  If set to
        ...
        <xref linkend="archive-modules"/>.␟アーカイビングが完了したWALファイルセグメントに使用するライブラリです。
...
詳細については、<xref linkend="backup-archiving-wal"/>および<xref linkend="archive-modules"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter can only be set in the
        <filename>postgresql.conf</filename> file or on the server command line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        The <xref linkend="guc-archive-command"/> or <xref linkend="guc-archive-library"/> is only invoked for
        ...
        If this value is specified without units, it is taken as seconds.
        This parameter can only be set in the
        <filename>postgresql.conf</filename> file or on the server command line.␟<xref linkend="guc-archive-command"/>または<xref linkend="guc-archive-library"/> は、完了したWALセグメントに対してのみ呼び出されます。
...
␝␟    <title>Recovery</title>␟    <title>リカバリ</title>␞␞␞
␝      <indexterm>
...
␝    <para>␟     This section describes the settings that apply to recovery in general,
     affecting crash recovery, streaming replication and archive-based
     replication.␟この節では、クラッシュリカバリ、ストリーミングレプリケーション、およびアーカイブベースのレプリケーションに影響する一般的なリカバリに適用される設定について説明します。␞␞    </para>␞
␝       <para>␟        Whether to try to prefetch blocks that are referenced in the WAL that
        ...
        function, but it doesn't do anything.␟リカバリ中に、まだバッファプールにない、WALで参照されているブロックをプリフェッチしようとするかどうか。
有効な値は<literal>off</literal>、<literal>on</literal>、<literal>try</literal>(デフォルト)です。
<literal>try</literal>設定は、オペレーティングシステムが<function>posix_fadvise</function>関数を提供している場合にのみプリフェッチを有効にします。
この関数は現在、プリフェッチの実装に使用されています。
一部のオペレーティングシステムはこの関数を提供していますが、何も行わないことに注意してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Prefetching blocks that will soon be needed can reduce I/O wait times
        ...
        prefetching activity.␟すぐに必要になるブロックをプリフェッチすると、一部のワークロードでリカバリ中のI/O待機時間を短縮できます。
プリフェッチ・アクティビティを制限する<xref linkend="guc-wal-decode-buffer-size"/>および<xref linkend="guc-maintenance-io-concurrency"/>設定も参照してください。␞␞       </para>␞
␝       <para>␟        A limit on how far ahead the server can look in the WAL, to find
        blocks to prefetch.  If this value is specified without units, it is
        taken as bytes.
        The default is 512kB.␟サーバがプリフェッチするブロックを見つけるためにWAL内をどれだけ先まで見ることができるかの制限です。
...
␝␟    <title>Archive Recovery</title>␟    <title>アーカイブからのリカバリ</title>␞␞␞
␝    <para>␟     This section describes the settings that apply only for the duration of
     the recovery.  They must be reset for any subsequent recovery you wish to
     perform.␟この節では、リカバリの間だけ適用する設定について説明します。
その後実施するリカバリでは、その設定はリセットしなければなりません。␞␞    </para>␞
␝    <para>␟     <quote>Recovery</quote> covers using the server as a standby or for
     ...
     recovery is used to recover from data loss.␟<quote>リカバリ</quote>は、サーバをスタンバイとして使用するとき、あるいはターゲットを指定したリカバリで適用されます。
通常、スタンバイモードは、高可用性または読み出しスケーラビリティ、あるいはその両方を提供するために使用します。
一方ターゲットを指定したリカバリは失なわれたデータを回復するために使用します。␞␞    </para>␞
␝    <para>␟     To start the server in standby mode, create a file called
     ...
     also be applied but are typically not useful in this mode.␟スタンバイモードでサーバを起動するには、<filename>standby.signal</filename><indexterm><primary>standby.signal</primary></indexterm>と呼ばれるファイルをデータディレクトリに作ります。
...
<xref linkend="runtime-config-wal-recovery-target"/>のパラメータも適用できますが、このモードでは通常有用ではありません。␞␞    </para>␞
␝      <indexterm>
...
␝    <para>␟     To start the server in targeted recovery mode, create a file called
     ...
     linkend="runtime-config-wal-recovery-target"/> will be used.␟サーバをターゲットリカバリモードで起動するには、<filename>recovery.signal</filename><indexterm><primary>recovery.signal</primary></indexterm>という名前のファイルをデータディレクトリに作ります。
...
このモードでは、この節と<xref linkend="runtime-config-wal-recovery-target"/>のパラメータの両方が使用されます。␞␞    </para>␞
␝       <para>␟        The local shell command to execute to retrieve an archived segment of
        ...
        Write <literal>%%</literal> to embed an actual <literal>%</literal> character.␟一連のWALファイルからアーカイブセグメントを取り出すために実行するローカルのシェルコマンドです。
...
<literal>%</literal>文字自体を埋め込むには<literal>%%</literal>と書いてください。␞␞       </para>␞
␝       <para>␟        It is important for the command to return a zero exit status
        only if it succeeds.  The command <emphasis>will</emphasis> be asked for file
        names that are not present in the archive; it must return nonzero
        when so asked.  Examples:␟コマンドは、成功した時のみ終了コードのゼロを返却することが重要です。
コマンドはアーカイブにないファイル名を聞かれる<emphasis>ことになります</emphasis>。
その場合には、非ゼロの値を返却しなければなりません。以下に例を示します。␞␞<programlisting>␞
␝</programlisting>␟        An exception is that if the command was terminated by a signal (other
        than <systemitem>SIGTERM</systemitem>, which is used as part of a
        database server shutdown) or an error by the shell (such as command
        not found), then recovery will abort and the server will not start up.␟例外は、データベースサーバのシャットダウンの一部として、<systemitem>SIGTERM</systemitem>以外のシグナルでコマンドが終了させられたり、シェルによってエラーが発生した（コマンドが見つからない場合など）場合で、その場合はリカバリは中断され、サーバはスタートアップしなくなります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        This optional parameter specifies a shell command that will be executed
        ...
        single-standby configurations, for example:␟オプションのパラメータは、すべてのリスタートポイントで実行されるシェルコマンドを指定します。
...
単一のスタンバイ構成用の<varname>archive_cleanup_command</varname>で、たとえば以下のように、しばしば<xref linkend="pgarchivecleanup"/>モジュールが使われます。␞␞<programlisting>archive_cleanup_command = 'pg_archivecleanup /mnt/server/archivedir %r'</programlisting>␞
␝<programlisting>archive_cleanup_command = 'pg_archivecleanup /mnt/server/archivedir %r'</programlisting>␟        Note however that if multiple standby servers are restoring from the
        ...
        command.␟だたし、複数のスタンバイサーバが同じアーカイブディレクトリからリストアしている場合は、どのサーバにおいてももはや必要がなくなるまでWALファイルが削除されることのないようにする必要があることに留意してください。
<varname>archive_cleanup_command</varname>は通常ウォームスタンバイ構成で使用されます。
（<xref linkend="warm-standby"/>参照。）
<literal>%</literal>文字自体を埋め込むには<literal>%%</literal>と書いてください。␞␞       </para>␞
␝       <para>␟        If the command returns a nonzero exit status then a warning log
        message will be written.  An exception is that if the command was
        terminated by a signal or an error by the shell (such as command not
        found), a fatal error will be raised.␟コマンドが非ゼロの終了ステータスを返した場合、警告ログメッセージが出力されます。
例外は、コマンドがシグナルで終了されたとき、あるいはシェルがエラーを起こしたとき（コマンドが見つからないなど）で、その場合致命的エラーが生じます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        This parameter specifies a shell command that will be executed once only
        at the end of recovery. This parameter is optional. The purpose of the
        <varname>recovery_end_command</varname> is to provide a mechanism for cleanup
        following replication or recovery.
        Any <literal>%r</literal> is replaced by the name of the file containing the
        last valid restart point, like in <xref linkend="guc-archive-cleanup-command"/>.␟このパラメータは、リカバリの終了時に一度だけ起動されるシェルコマンドを指定します。
このパラメータはオプションです。
<varname>recovery_end_command</varname>の目的はレプリケーションあるいはリカバリの後のクリーンアップのための機構を提供することにあります。
<literal>%r</literal>は、<xref linkend="guc-archive-cleanup-command"/>と同じように、有効な最後のリスタートポイントを含むWALファイル名に置換されます。␞␞       </para>␞
␝       <para>␟        If the command returns a nonzero exit status then a warning log
        message will be written and the database will proceed to start up
        anyway.  An exception is that if the command was terminated by a
        signal or an error by the shell (such as command not found), the
        database will not proceed with startup.␟コマンドが非ゼロの終了ステータスを返した場合、警告ログメッセージが出力されますが、データベースはスタートアップ処理を続けます。
例外は、コマンドがシグナルによって終了させられたか、シェルによってエラーが発生した（そのようなコマンドは見つからない）場合で、その場合はデータベースはスタートアップ処理を継続させません。␞␞       </para>␞
␝       <para>␟        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝␟    <title>Recovery Target</title>␟    <title>リカバリターゲット</title>␞␞␞
␝      <indexterm>
...
␝     <para>␟      By default, recovery will recover to the end of the WAL log. The
      ...
      These parameters can only be set at server start.␟デフォルトではWALログの最後までリカバリを行います。
...
これらのパラメータはサーバ起動時のみ設定可能です。␞␞     </para>␞
␝       <para>␟        This parameter specifies that recovery should end as soon as a
        consistent state is reached, i.e., as early as possible. When restoring
        from an online backup, this means the point where taking the backup
        ended.␟このパラメータは、リカバリが一貫した状態になり次第、すなわちできるだけ早く終了することを指定します。
オンラインバックアップからリストアした場合、これはバックアップが終了した時点を意味します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Technically, this is a string parameter, but <literal>'immediate'</literal>
        is currently the only allowed value.␟技術的にはこれは文字列型のパラメータですが、現時点では<literal>'immediate'</literal>だけが許容されている値です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter specifies the named restore point (created with
        <function>pg_create_restore_point()</function>) to which recovery will proceed.␟このパラメータは、指定した（<function>pg_create_restore_point()</function>により作成された）名前付きリストアポイントまでリカバリを進行させます。␞␞       </para>␞
␝       <para>␟        This parameter specifies the time stamp up to which recovery
        will proceed.
        The precise stopping point is also influenced by
        <xref linkend="guc-recovery-target-inclusive"/>.␟このパラメータは、指定したタイムスタンプまでリカバリを進行させます。
正確な停止点は<xref linkend="guc-recovery-target-inclusive"/>にも影響されます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The value of this parameter is a time stamp in the same format
        ...
        e.g., <literal>Europe/Helsinki</literal> not <literal>EEST</literal>.␟（<xref linkend="guc-timezone-abbreviations"/>変数が設定ファイルで先に設定されていない限り）時間帯略語を使えないことを除けば、このパラメータの値は<type>timestamp with time zone</type>データ型が受け付けるのと同じ形式のタイムスタンプです。
おすすめの形式はUTCからのオフセットか、<literal>EEST</literal>ではなく<literal>Europe/Helsinki</literal>のような完全な時間帯名です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter specifies the transaction ID up to which recovery
        ...
        The precise stopping point is also influenced by
        <xref linkend="guc-recovery-target-inclusive"/>.␟このパラメータは、指定したトランザクションIDまでリカバリを進行させます。
...
正確な停止点は<xref linkend="guc-recovery-target-inclusive"/>にも影響されます。␞␞       </para>␞
␝       <para>␟        This parameter specifies the LSN of the write-ahead log location up
        to which recovery will proceed. The precise stopping point is also
        influenced by <xref linkend="guc-recovery-target-inclusive"/>. This
        parameter is parsed using the system data type
        <link linkend="datatype-pg-lsn"><type>pg_lsn</type></link>.␟このパラメータは、指定した先行書き込みログ（WAL）の場所のLSNまでリカバリを進行させます。
正確な停止点は、<xref linkend="guc-recovery-target-inclusive"/>の影響も受けます。
このパラメータは、システムデータ型<link linkend="datatype-pg-lsn"><type>pg_lsn</type></link>を使用して解析されます。␞␞       </para>␞
␝      <indexterm>
...
␝     <para>␟       The following options further specify the recovery target, and affect
       what happens when the target is reached:␟以下のオプションはリカバリ対象をより詳細に指定し、リカバリが対象に達した時の動作に影響を与えます。␞␞     </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies whether to stop just after the specified recovery target
        ...
        be included in the recovery.  Default is <literal>on</literal>.␟指定したリカバリ対象のちょうど後に停止するか(<literal>on</literal>)、ちょうどその前に停止するか(<literal>off</literal>)を指定します。
...
デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝       <para>␟        Specifies recovering into a particular timeline.  The value can be a
        ...
        a standby server.  <literal>latest</literal> is the default.␟リカバリが作成する個別のタイムラインを指定します。
...
デフォルトは<literal>latest</literal>です。␞␞       </para>␞
␝       <para>␟        To specify a timeline ID in hexadecimal (for example, if extracted
        ...
        <literal>0x11</literal> (or 17 decimal).␟タイムラインIDを16進数で指定するには(たとえば、WALファイル名前または歴史ファイルから抽出した場合)、<literal>0x</literal>を前に置きます。
例として、WALファイル名前が<filename>00000011000000A10000004F</filename>であれば、タイムラインIDは<literal>0x11</literal>(または10進数で17)になります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        You usually only need to set this parameter
        in complex re-recovery situations, where you need to return to
        a state that itself was reached after a point-in-time recovery.
        See <xref linkend="backup-timelines"/> for discussion.␟通常このパラメータの設定が必要となるのは、ポイントインタイムリカバリの実施後に到達した状態に戻す場合など、複雑なリカバリの状況のみです。
説明は<xref linkend="backup-timelines"/>を参照してください。␞␞       </para>␞
␝       <para>␟        Specifies what action the server should take once the recovery target is
        ...
        recovery target.␟リカバリ対象に到達した場合に、サーバがする動作を指定します。
デフォルトは<literal>pause</literal>で、リカバリを休止することを意味します。
<literal>promote</literal>は、リカバリの過程が終われば、サーバは接続の受け付けを始めることを意味します。
最後に、<literal>shutdown</literal>は、リカバリ対象に到達した後にサーバを停止します。␞␞       </para>␞
␝       <para>␟        The intended use of the <literal>pause</literal> setting is to allow queries
        ...
        continue recovery.␟<literal>pause</literal>の設定の意図した使い方は、このリカバリ対象がリカバリのための最も望ましいポイントかどうかチェックするために、データベースに対して問い合わせを実行できるようにすることです。
休止された状態は、<function>pg_wal_replay_resume()</function>(<xref linkend="functions-recovery-control-table"/>参照)の使用により再開することができます。
その後、それはリカバリを終了させます。
このリカバリ対象が希望の止まるポイントでない場合、サーバをシャットダウンし、リカバリ対象の設定をより後の対象に変更し、リカバリを継続するために再起動してください。␞␞       </para>␞
␝       <para>␟        The <literal>shutdown</literal> setting is useful to have the instance ready
        ...
        since the last checkpoint next time it is started).␟<literal>shutdown</literal>の設定はインスタンスを正確に望ましい再生ポイントで準備するのに有用です。
インスタンスはさらに多くのWALレコードを再生できます(実際、次に起動するときには最後のチェックポイントからWALレコードを再生しなければなりません)。␞␞       </para>␞
␝       <para>␟        Note that because <filename>recovery.signal</filename> will not be
        ...
        file is removed manually.␟<filename>recovery.signal</filename>は<varname>recovery_target_action</varname>が<literal>shutdown</literal>に設定されていると削除されないことに留意してください。
設定が変更されるか、<filename>recovery.signal</filename>が手動で削除されない限り、以降起動しても直ちに停止されてしまいます。␞␞       </para>␞
␝       <para>␟        This setting has no effect if no recovery target is set.
        ...
        <literal>promote</literal>.␟この設定はリカバリ対象が設定されていない場合には効果がありません。
<xref linkend="guc-hot-standby"/>が有効になっていない場合、<literal>pause</literal>の設定は<literal>shutdown</literal>と同じように動作します。
昇格中にリカバリ対象に到達した場合は、<literal>pause</literal>は<literal>promote</literal>と同じように働きます。␞␞       </para>␞
␝       <para>␟        In any case, if a recovery target is configured but the archive
        recovery ends before the target is reached, the server will shut down
        with a fatal error.␟どのような場合でも、リカバリ対象が設定されていて、アーカイブリカバリがそのリカバリ対象に到達する前に終了すると、サーバはフェイタルエラーで停止します。␞␞       </para>␞
␝   <sect2 id="runtime-config-wal-summarization">␟    <title>WAL Summarization</title>␟    <title>WAL要約</title>␞␞␞
␝      <indexterm>
...
␝    <para>␟     These settings control WAL summarization, a feature which must be
     enabled in order to perform an
     <link linkend="backup-incremental-backup">incremental backup</link>.␟これらの設定は、WAL要約を制御します。この機能は、<link linkend="backup-incremental-backup">インクリメンタルバックアップ</link>を実行するために有効にする必要があります。␞␞    </para>␞
␝      <para>␟       Enables the WAL summarizer process. Note that WAL summarization can
       ...
       command line.  The default is <literal>off</literal>.␟WAL要約プロセスを有効にします。
...
␝      <para>␟       The server cannot be started with <literal>summarize_wal=on</literal>
       ...
       <literal>wal_level=minimal</literal>.␟<literal>wal_level</literal>が<literal>minimal</literal>に設定されている場合、サーバを<literal>summarize_wal=on</literal>で起動することはできません。
<literal>wal_level=minimal</literal>の状態でサーバを起動し、その後<literal>summarize_wal=on</literal>に設定した場合、要約処理は動作しますが、<literal>wal_level=minimal</literal>で生成されたWALの要約ファイルの生成は拒否します。␞␞      </para>␞
␝      <para>␟       Configures the amount of time after which the WAL summarizer
       ...
       this parameter, because the WAL summarizer will not run.␟WAL要約処理が自動的に古いWAL要約を削除するまでの時間を設定します。
...
デフォルトは10日間です。
<literal>summarize_wal = off</literal>の場合、WAL要約処理が動作しないため、このパラメータの値に関わらず、既存のWAL要約は削除されません。␞␞      </para>␞
␝   <sect1 id="runtime-config-replication">␟    <title>Replication</title>␟    <title>レプリケーション</title>␞␞␞
␝    <para>␟     These settings control the behavior of the built-in
     ...
     <xref linkend="logical-replication"/>).␟これらの設定は、組み込み<firstterm>ストリーミングレプリケーション</firstterm>機能（<xref linkend="streaming-replication"/>参照）および組み込み<firstterm>論理レプリケーション</firstterm>機能（<xref linkend="logical-replication"/>を参照）の動作を制御します。
<xref linkend="streaming-replication"/>␞␞    </para>␞
␝    <para>␟     For <emphasis>streaming replication</emphasis>, servers will be either a
     ...
     across the cluster without problems if that is required.␟<emphasis>ストリーミングレプリケーション</emphasis>では、サーバ群のサーバはプライマリかスタンバイのいずれかです。
...
必要とあればクラスタに渡って問題なく設定を変化させることができます。␞␞    </para>␞
␝    <para>␟     For <emphasis>logical replication</emphasis>, <firstterm>publishers</firstterm>
     ...
     <xref linkend="logical-replication-config"/>.␟<emphasis>論理レプリケーション</emphasis>の場合、<firstterm>パブリッシャー（</firstterm><link linkend="sql-createpublication"><command>CREATE PUBLICATION</command></link>を実行するサーバ）は、データを<firstterm>サブスクライバー</firstterm>（<link linkend="sql-createsubscription"><command>CREATE SUBSCRIPTION</command></link>を実行するサーバ）に複製します。
サーバは、パブリッシャーとサブスクライバーを同時に兼ねることもできます。
以降の節でパブリッシャーを「送信者」と呼ぶことに注意してください。
論理レプリケーション設定設定の詳細は、<xref linkend="logical-replication-config"/>を参照してください。␞␞    </para>␞
␝    <sect2 id="runtime-config-replication-sender">␟     <title>Sending Servers</title>␟     <title>送出サーバ群</title>␞␞␞
␝       <indexterm>
...
␝     <para>␟      These parameters can be set on any server that is
      ...
      standby becomes the primary.␟これらのパラメータはレプリケーションデータを１つ、またはそれ以上複数のスタンバイサーバに送るすべてのサーバ上で設定することができます。
プライマリは常に送出サーバであるため、パラメータは常にプライマリ上に設定されなければなりません。
これらのパラメータの役割と意味はスタンバイが後にプライマリに昇格しても変わりません。␞␞     </para>␞
␝       <para>␟        Specifies the maximum number of concurrent connections from standby
        ...
        servers.␟複数のスタンバイサーバあるいは、ストリーミングを使ったベースバックアップクライアントからの同時接続を受ける接続最大値を設定します（つまり、同時に稼働するWAL送信プロセスの最大値です）。
...
このパラメータはサーバ起動時のみ設定可能です。
また、スタンバイサーバからの接続を許可するには、<varname>wal_level</varname>を<literal>replica</literal>以上に設定しておかなければなりません。␞␞       </para>␞
␝       <para>␟         When running a standby server, you must set this parameter to the
         same or higher value than on the primary server. Otherwise, queries
         will not be allowed in the standby server.␟スタンバイサーバを実行する際は、このパラメータをプライマリサーバと同じか高い値にしなければなりません。
さもなければ、スタンバイサーバでクエリを実行できなくなります。␞␞        </para>␞
␝        <para>␟         Specifies the maximum number of replication slots
         ...
         be used.␟サーバが使用できるレプリケーションスロット(<xref linkend="streaming-replication-slots"/>参照)の最大数を指定します。
デフォルトは10です。
このパラメータはサーバ起動時のみ設定可能です。
現在存在しているレプリケーションスロットの数よりも少ない値を設定すると、サーバは起動しません。
また、レプリケーションスロットが使用できるためには、<varname>wal_level</varname>を<literal>replica</literal>以上に設定しなければなりません。␞␞        </para>␞
␝       <indexterm>
...
␝        <para>␟         Note that this parameter also applies on the subscriber side, but with
         a different meaning.␟このパラメータはサブスクライバー側にも適用されますが、意味が異なることに注意してください。␞␞        </para>␞
␝       <para>␟        Specifies the minimum size of past WAL files kept in the
        ...
        archiving is in use.)␟<filename>pg_wal</filename>ディレクトリに保持する過去のWALファイルの最小サイズを指定します。
もし送出サーバに接続しているスタンバイサーバが<varname>wal_keep_size</varname>メガバイトを越えて遅延した場合、送出サーバはスタンバイサーバが今後とも必要とするWALセグメントを削除する可能性があります。
この場合、レプリケーション接続は終了させられます。結果として下流に対する接続も結局は終了されることがあります。（しかし、WALアーカイブが使用されていれば、スタンバイサーバはアーカイブからセグメントを取り出し、復旧することができます。）␞␞       </para>␞
␝       <indexterm>
...
␝       <para>␟        This sets only the minimum size of segments retained in
        ...
        If this value is specified without units, it is taken as megabytes.
        This parameter can only be set in the
        <filename>postgresql.conf</filename> file or on the server command line.␟<filename>pg_wal</filename>に保持され続けるセグメントの最小値のみを設定します。
...
␝       <para>␟        Specify the maximum size of WAL files
        ...
        If this value is specified without units, it is taken as megabytes.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟チェックポイント時に<link linkend="streaming-replication-slots">レプリケーションスロット</link>が<filename>pg_wal</filename>ディレクトリに残すことのできる
...
この値が単位無しで指定されると、メガバイトであると見なします。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        Terminate replication connections that are inactive for longer
        than this amount of time. This is useful for
        the sending server to detect a standby crash or network outage.
        If this value is specified without units, it is taken as milliseconds.
        The default value is 60 seconds.
        A value of zero disables the timeout mechanism.␟指定された時間より長く非活動であるレプリケーション接続を停止します。
スタンバイサーバのクラッシュ、またはネットワークの停止を送出サーバが検出することにこれが役立ちます。
この値が単位なしで指定された場合は、ミリ秒単位であるとみなします。
デフォルトの値は60秒です。
値ゼロはこのタイムアウト機能を無効にします。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        With a cluster distributed across multiple geographic
        ...
        network connection.␟地理的に複数の場所に分散したクラスタでは、場所によって異なる値を使うことでクラスタ管理がより柔軟にできるようになります。
より小さな値は低遅延ネットワーク接続上のスタンバイの障害検知をより高速にするのに役立ちます。
遅延の大きなネットワーク接続に設置されたスタンバイの健全性の判断にはより大きな値が助けになるでしょう。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Record commit time of transactions. This parameter
        can only be set in <filename>postgresql.conf</filename> file or on the server
        command line. The default value is <literal>off</literal>.␟トランザクションのコミットタイムを記録します。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
デフォルトは<literal>off</literal>です。␞␞       </para>␞
␝       <para>␟        A comma-separated list of streaming replication standby server slot names
        ...
        Additionally, the replication management functions␟ロジカルWAL送信プロセスが待機する、カンマで区切られたストリーミングレプリケーションのスタンバイサーバのスロット名です。
...
また、レプリケーション管理関数である␞␞␞
␝        Additionally, the replication management functions␟        <link linkend="pg-replication-slot-advance">
        ...
        confirmed WAL receipt.␟<link linkend="pg-replication-slot-advance">、<function>pg_replication_slot_advance</function></link>、<link linkend="pg-logical-slot-get-changes">、<function>pg_logical_slot_get_changes</function></link>、<link linkend="pg-logical-slot-peek-changes">、<function>pg_logical_slot_peek_changes</function></link>がロジカルフェイルオーバースロットに使用されると、<varname>synchronized_standby_slots</varname>で指定されたすべての物理スロットでWAL受信が確認されるまでブロックします。␞␞       </para>␞
␝       <para>␟        The standbys corresponding to the physical replication slots in
        <varname>synchronized_standby_slots</varname> must configure
        <literal>sync_replication_slots = true</literal> so they can receive
        logical failover slot changes from the primary.␟<varname>synchronized_standby_slots</varname>内の物理レプリケーションスロットに対応するスタンバイは、<literal>sync_replication_slots = true</literal>を設定して、プライマリからロジカルフェイルオーバースロットの変更を受信できるようにする必要があります。␞␞       </para>␞
␝    <sect2 id="runtime-config-replication-primary">␟     <title>Primary Server</title>␟     <title>プライマリサーバ</title>␞␞␞
␝      <indexterm>
...
␝     <para>␟      These parameters can be set on the primary server that is
      to send replication data to one or more standby servers.
      ...
      possibility of a standby becoming the primary.␟これらのパラメータはレプリケーションデータを１つ、またはそれ以上複数のスタンバイサーバに送るプライマリサーバ上で設定することができます。
...
スタンバイサーバがプライマリサーバになるかもしれない状況に備え、それらのパラメータをスタンバイサーバで設定したいと考えたとしても、スタンバイサーバ上でのパラメータの値は意味をなしません。␞␞     </para>␞
␝       <para>␟        Specifies a list of standby servers that can support
        ...
        availability and protection against data loss.␟<xref linkend="synchronous-replication"/>で説明されているように、<firstterm>同期レプリケーション</firstterm>をサポート可能なスタンバイサーバのリストを指定します。
...
二つ以上の同期スタンバイサーバ名を指定することで、かなりの高可用性とデータ損失に対する保護が得られます。␞␞       </para>␞
␝       <para>␟        The name of a standby server for this purpose is the
        ...
        consult their documentation.␟この目的のためのスタンバイサーバ名は、スタンバイの接続情報で指定された、スタンバイの<varname>application_name</varname>設定です。
...
それ以外のレプリケーションストリームの消費者については、それぞれのドキュメントをご覧ください。␞␞       </para>␞
␝       <para>␟        This parameter specifies a list of standby servers using
        either of the following syntaxes:␟このパラメータは、以下の構文のいずれかを用いてスタンバイサーバのリストを指定します。␞␞<synopsis>␞
␝</synopsis>␟        where <replaceable class="parameter">num_sync</replaceable> is
        ...
        synchronous standbys from the listed servers.␟ここで、<replaceable class="parameter">num_sync</replaceable>は、トランザクションが応答を待機する必要のある同期スタンバイの数です。
<replaceable class="parameter">standby_name</replaceable>は、スタンバイサーバの名前です。
<literal>FIRST</literal>と<literal>ANY</literal>は、リスト中のサーバから同期スタンバイを選ぶ方法を指定します。␞␞       </para>␞
␝       <para>␟        The keyword <literal>FIRST</literal>, coupled with
        ...
        standby. The keyword <literal>FIRST</literal> is optional.␟キーワード<literal>FIRST</literal>を<replaceable class="parameter">num_sync</replaceable>と組み合わせると、優先度に基づく同期レプリケーションを指定し、優先度に基づいて選ばれた<replaceable class="parameter">num_sync</replaceable>個の同期スタンバイにWALレコードがレプリケーションされるまで、トランザクションのコミットは待機します。
...
キーワード<literal>FIRST</literal>はオプションです。␞␞       </para>␞
␝       <para>␟        The keyword <literal>ANY</literal>, coupled with
        <replaceable class="parameter">num_sync</replaceable>, specifies a
        ...
        <literal>s1</literal>, <literal>s2</literal>, <literal>s3</literal> and <literal>s4</literal>
        reply.␟キーワード<literal>ANY</literal>を<replaceable class="parameter">num_sync</replaceable>と組み合わせると、クォーラムに基づく同期レプリケーションを指定し、列挙されたスタンバイのうち<emphasis>少なくとも</emphasis><replaceable class="parameter">num_sync</replaceable>個の同期スタンバイにWALレコードがレプリケーションされるまで、トランザクションのコミットを待たせます。
たとえば<literal>ANY 3 (s1, s2, s3, s4)</literal>とすると、<literal>s1</literal>、<literal>s2</literal>、<literal>s3</literal>、<literal>s4</literal>のうちの少なくとも3つが応答を返した時点でコミットが進行します。␞␞       </para>␞
␝       <para>␟        <literal>FIRST</literal> and <literal>ANY</literal> are case-insensitive. If these
        keywords are used as the name of a standby server,
        its <replaceable class="parameter">standby_name</replaceable> must
        be double-quoted.␟<literal>FIRST</literal>と<literal>ANY</literal>は、大文字小文字を区別しません。
もしこれらのキーワードをスタンバイサーバの名前に使う場合は、<replaceable class="parameter">standby_name</replaceable>は二重引用符で囲わなければなりません。␞␞       </para>␞
␝       <para>␟        The third syntax was used before <productname>PostgreSQL</productname>
        ...
        as a synchronous standby.␟3番目の構文は、<productname>PostgreSQL</productname> 9.6よりも前のバージョンで用いられていたもので、依然としてサポートされています。
...
<literal>s1</literal>か<literal>s2</literal>が同期スタンバイとして選ばれます。␞␞       </para>␞
␝       <para>␟        The special entry <literal>*</literal> matches any standby name.␟特別なエントリ<literal>*</literal>は、すべてのスタンバイ名に一致します。␞␞       </para>␞
␝       <para>␟        There is no mechanism to enforce uniqueness of standby names.  In case
        of duplicates one of the matching standbys will be considered as
        higher priority, though exactly which one is indeterminate.␟スタンバイの一意性を強制する仕組みはありません。
重複があった場合、一致したスタンバイは優先順位が高いと見なされますが、どれが選ばれるかは非決定的です。␞␞       </para>␞
␝        <para>␟         Each <replaceable class="parameter">standby_name</replaceable>
         ...
         double-quoted or not.␟各々の<replaceable class="parameter">standby_name</replaceable>は、<literal>*</literal>である場合を除き、SQL識別子の形式を取らなければなりません。
二重引用符を用いることもできます。
しかし、二重引用符の有無に関わらず、<replaceable class="parameter">standby_name</replaceable>とスタンバイのアプリケーション名の比較は、大文字小文字の区別なしに行われることに注意してください。␞␞        </para>␞
␝       <para>␟        If no synchronous standby names are specified here, then synchronous
        ...
        <literal>local</literal> or <literal>off</literal>.␟ここに同期スタンバイ名が指定されていない場合、同期レプリケーションは有効とはならず、トランザクションコミットはレプリケーションを待機しません。これがデフォルトの設定です。同期レプリケーションが有効であっても、<xref linkend="guc-synchronous-commit"/>パラメータを<literal>local</literal> または <literal>off</literal>に設定することにより、個別のトランザクションをレプリケーションに対して待機しないように設定できます。␞␞       </para>␞
␝       <para>␟        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝    <sect2 id="runtime-config-replication-standby">␟     <title>Standby Servers</title>␟     <title>スタンバイサーバ</title>␞␞␞
␝        <indexterm>
...
␝     <para>␟      These settings control the behavior of a
      <link linkend="standby-server-operation">standby server</link>
      that is
      to receive replication data.  Their values on the primary server
      are irrelevant.␟これらの設定はレプリケーションデータを受け取る<link linkend="standby-server-operation">スタンバイサーバ</link>の動作を管理します。
プライマリサーバ上のこれらの値は無意味です。␞␞     </para>␞
␝         <para>␟          Specifies a connection string to be used for the standby server
          to connect with a sending server. This string is in the format
          described in <xref linkend="libpq-connstring"/>. If any option is
          unspecified in this string, then the corresponding environment
          variable (see <xref linkend="libpq-envars"/>) is checked. If the
          environment variable is not set either, then
          defaults are used.␟スタンバイサーバが送信サーバに接続するための接続文字列を指定します。
...
環境変数も設定されていなければデフォルトの値が使われます。␞␞         </para>␞
␝         <para>␟          The connection string should specify the host name (or address)
          ...
          <literal>replication</literal> as the database name).␟接続文字列では、プライマリサーバのホスト名（またはアドレス）、スタンバイサーバのデフォルトと異なるのであればポート番号も指定する必要があります。
...
パスワードは <varname>primary_conninfo</varname>に記述することもできますし、スタンバイサーバ上の分離されたファイル<filename>~/.pgpass</filename>に記述することもできます (データベース名には <literal>replication</literal> を使います)。␞␞         </para>␞
␝         <para>␟          For replication slot synchronization (see
          ...
          used for slot synchronization. It is ignored for streaming.␟レプリケーションスロットを同期（<xref linkend="logicaldecoding-replication-slots-synchronization"/>を参照）するために、<varname>primary_conninfo</varname>文字列に有効な<literal>dbname</literal>を指定する必要もあります。
これはスロットの同期にのみ使用されます。
ストリーミングでは無視されます。␞␞         </para>␞
␝        <indexterm>
...
␝         <para>␟          This parameter can only be set in the <filename>postgresql.conf</filename>
          ...
          is an empty string).
          This setting has no effect if the server is not in standby mode.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
WAL受信プロセスが実行中にこのパラメータが変更されると、そのプロセスにシグナルが送られ、新しい設定で再起動するために停止します（<varname>primary_conninfo</varname>が空文字の場合を除きます。）
サーバがスタンバイモードでなければこの設定は無効となります。␞␞         </para>␞
␝      <indexterm>
...
␝         <para>␟          Optionally specifies an existing replication slot to be used when
          ...
          set or the server is not in standby mode.␟上流ノードのリソース削除を制御するためにストリーミングレプリケーション経由でプライマリに接続した場合、既存のレプリケーションスロットを使うように、必要に応じて指定します（<xref linkend="streaming-replication-slots"/>を参照）。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
WAL受信プロセスが実行中にこのパラメータが変更されると、そのプロセスにシグナルが送られ、新しい設定で再起動するために停止します。
<varname>primary_conninfo</varname>が設定されていない場合、この設定は無効です。␞␞         </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies whether or not you can connect and run queries during
        recovery, as described in <xref linkend="hot-standby"/>.
        The default value is <literal>on</literal>.
        This parameter can only be set at server start. It only has effect
        during archive recovery or in standby mode.␟<xref linkend="hot-standby"/>に記載されている通り、リカバリの最中に接続し、そして問い合わせを実行できるか否かを設定します。デフォルト値は<literal>on</literal>です。
このパラメータはサーバ起動時のみ設定可能です。
これは、アーカイブリカバリ期間、又はスタンバイモードにある場合にのみ効果をもたらします。␞␞       </para>␞
␝       <para>␟        When hot standby is active, this parameter determines how long the
        ...
        queries to complete.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟ホットスタンバイがアクティブな場合、このパラメータは、<xref linkend="hot-standby-conflict"/>で説明されているように、適用されようとしているWALエントリと競合するスタンバイクエリをキャンセルするまで待機する時間を決定します。
...
␝       <para>␟        Note that <varname>max_standby_archive_delay</varname> is not the same as the
        ...
        time.␟<varname>max_standby_archive_delay</varname>はキャンセル前に問い合わせが実行できる最大の時間の長さと同じでないことに注意してください。
むしろ、任意の１つのWALセグメントのデータの適用のために許される最大合計時間です。
従って、ある問い合わせによりWALセグメント内の前の部分で大幅な遅延となった場合、その後の衝突する問い合わせの猶予時間はずっと短くなります。␞␞       </para>␞
␝       <para>␟        When hot standby is active, this parameter determines how long the
        standby server should wait before canceling standby queries that
        conflict with about-to-be-applied WAL entries, as described in
        <xref linkend="hot-standby-conflict"/>.
...
␝       <para>␟        Note that <varname>max_standby_streaming_delay</varname> is not the same as
        ...
        again.␟<varname>max_standby_streaming_delay</varname>はキャンセル前に問い合わせが実行できる最大の時間の長さと同じでないことに注意してください。
むしろ、プライマリサーバから一度受け取られたWALデータを適用するために許される最大合計時間です。
従って、ある問い合わせが大幅な遅延を起こした場合、その後の衝突する問い合わせは、スタンバイサーバがふたたび遅れを取り戻すまでの間、猶予時間はずっと短くなります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies whether the WAL receiver process should create a temporary replication
        ...
        the new setting.␟永続レプリケーションスロットが(<xref linkend="guc-primary-slot-name"/>を使って)作成されない設定になっている時に、WAL受信プロセスがリモートインスタンス上に一時レプリケーションスロットを作るかどうかを指定します。
デフォルトはoffです。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
WAL受信プロセスが実行中にこのパラメータが変更されると、そのプロセスにシグナルが送られ、新しい設定で再起動するために停止します。␞␞       </para>␞
␝      <indexterm>
...
␝      <para>␟       Specifies the minimum frequency for the WAL receiver
       ...
       command line.␟スタンバイサーバ上のWAL受信プロセスがプライマリ、または上位サーバに対してレプリケーションの進捗情報を送信する最小頻度を指定します。
...
デフォルトの値は10秒です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞      </para>␞
␝       <para>␟        Specifies whether or not a hot standby will send feedback to the primary
        or upstream standby
        ...
        <filename>postgresql.conf</filename> file or on the server command line.␟ホットスタンバイがスタンバイサーバ上で現在処理を行っている問い合わせについて、プライマリまたは上位サーバにフィードバックを送るか否かを指定します。
...
␝       <para>␟        If cascaded replication is in use the feedback is passed upstream
        until it eventually reaches the primary.  Standbys make no other use
        of feedback they receive other than to pass upstream.␟カスケードレプリケーションが使用されている場合、フィードバックは最終的にプライマリに到達するまで上位サーバに転送されます。スタンバイは上位に転送する以外、受け取ったフィードバックを他に使用しません。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Terminate replication connections that are inactive for longer
        ...
        the <filename>postgresql.conf</filename> file or on the server
        command line.␟指定された時間より長い間、活動していないレプリケーション接続は停止します。
このことは受信するスタンバイサーバがプライマリノードの機能停止、またはネットワーク停止を検出するのに便利です。
この値が単位なしで指定された場合は、ミリ秒単位であるとみなします。
デフォルト値は60秒です。
値ゼロは時間切れメカニズムを無効にします。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        Specifies how long the standby server should wait when WAL data is not
        ...
        This parameter can only be set in
        the <filename>postgresql.conf</filename> file or on the server
        command line.␟WALデータがソース(ストリーミングレプリケーション、ローカルの<filename>pg_wal</filename>、またはWALアーカイブ)から取得できない時に、スタンバイサーバがWALデータ受信をリトライするまでにどの位の時間待つべきかを指定します。
この値が単位なしで指定された場合は、ミリ秒単位であるとみなします。
デフォルト値は5秒です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        This parameter is useful in configurations where a node in recovery
        ...
        is taken into account.␟このパラメータは、リカバリ対象のノードにおいて、新しいWALデータが読み込み可能になるまでの待ち時間を制御する必要のある時に有用です。
...
これは、たとえば基盤へのアクセス回数が課金対象になるクラウド環境において、有用です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        In logical replication, this parameter also limits how often a failing
        replication apply worker will be respawned.␟論理レプリケーションでは、このパラメータによって、失敗したレプリケーション適用ワーカーが再生成される頻度も制限されます。␞␞       </para>␞
␝       <para>␟        By default, a standby server restores WAL records from the
        ...
        If this value is specified without units, it is taken as milliseconds.
        The default is zero, adding no delay.␟デフォルトでは、スタンバイサーバは可能な限り早くプライマリからWALレコードをリストアします。
...
デフォルトは0で、遅延を与えません。␞␞       </para>␞
␝       <para>␟        It is possible that the replication delay between servers exceeds the
        ...
        than typical time deviations between servers.␟サーバ間のレプリケーション遅延はパラメータの値を上回る可能性があり、その場合には遅延は追加されません。
...
しかし、このパラメータの有用な設定値は典型的なサーバ間の時間のずれよりもずっと大きいので、それらは大きな問題ではありません。␞␞       </para>␞
␝       <para>␟        The delay occurs only on WAL records for transaction commits.
        ...
        are not visible until the corresponding commit record is applied.␟遅延はトランザクションコミットのWALレコードだけで発生します。
他のレコードは可能な限り早く再生されるでしょう。
対応する（トランザクション）コミットレコードが適用されるまではその効果が不可視であることがMVCCの可視ルールによって保証されているため、他のレコードが可能な限り早く再生されることは問題にはなりません。␞␞       </para>␞
␝       <para>␟        The delay occurs once the database in recovery has reached a consistent
        state, until the standby is promoted or triggered. After that the standby
        will end recovery without further waiting.␟ひとたびリカバリ中のデータベースが整合性のとれた状態になれば、スタンバイサーバが昇格またはトリガになるまで、遅延が発生します。
その後、スタンバイサーバはそれ以上待たずにリカバリを終了します。␞␞       </para>␞
␝       <para>␟        WAL records must be kept on the standby until they are ready to be
        ...
        <filename>pg_wal</filename> directory.␟WALレコードは、適用される準備が整うまでスタンバイに保持されなければなりません。
したがって、遅延が長くなるとWALファイルの蓄積量が増加し、スタンバイの<filename>pg_wal</filename>ディレクトリに必要なディスク容量が増加します。␞␞       </para>␞
␝       <para>␟        This parameter is intended for use with streaming replication deployments;
        however, if the parameter is specified it will be honored in all cases
        except crash recovery.

        <varname>hot_standby_feedback</varname> will be delayed by use of this feature
        which could lead to bloat on the primary; use both together with care.␟このパラメータはストリーミングレプリケーション配信で使われることを目的としていますが、パラメータが指定されていると、クラッシュリカバリを除くすべてのケースで使用されます。
この機能を使うことによって<varname>hot_standby_feedback</varname>が遅延され、プライマリサーバの肥大化に繋がる可能性があります。両方同時に使う場合には注意して使ってください。␞␞␞
␝         <para>␟          Synchronous replication is affected by this setting when <varname>synchronous_commit</varname>
          is set to <literal>remote_apply</literal>; every <literal>COMMIT</literal>
          will need to wait to be applied.␟<varname>synchronous_commit</varname>が<literal>remote_apply</literal>に設定されていれば、同期レプリケーションは、この設定に影響を受けます。各<literal>COMMIT</literal>は適用されるのを待つことが必要です。␞␞         </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        It enables a physical standby to synchronize logical failover slots
        from the primary server so that logical subscribers can resume
        replication from the new primary server after failover.␟これにより、物理スタンバイはプライマリサーバからのロジカルフェイルオーバースロットの同期が可能となり、ロジカルサブスクライバーはフェイルオーバー後に新しいプライマリサーバからレプリケーションを再開できます。␞␞       </para>␞
␝       <para>␟        It is disabled by default. This parameter can only be set in the
        <filename>postgresql.conf</filename> file or on the server command line.␟このパラメータはデフォルトで無効です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝    <sect2 id="runtime-config-replication-subscriber">␟     <title>Subscribers</title>␟     <title>サブスクライバー</title>␞␞␞
␝     <para>␟      These settings control the behavior of a logical replication subscriber.
      Their values on the publisher are irrelevant.
      See <xref linkend="logical-replication-config"/> for more details.␟これらの設定項目は、論理レプリケーションのサブスクライバーの挙動を制御します。
パブリッシャーにおける設定値とは無関係です。
詳細については<xref linkend="logical-replication-config"/>を参照してください。␞␞     </para>␞
␝       <para>␟        Specifies how many replication origins (see
        ...
        reserve for table synchronization.␟サブスクライバー側ではレプリケーション原点(origin)（<xref linkend="replication-origins"/>参照）をいくつ並行して追跡できるかを指定します。
...
<xref linkend="replication-origins"/>␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Note that this parameter also applies on a sending server, but with
        a different meaning.␟このパラメータは送信サーバにも適用されますが、意味は異なることに注意してください。␞␞       </para>␞
␝       <para>␟        Specifies maximum number of logical replication workers. This includes
        leader apply workers, parallel apply workers, and table synchronization
        workers.␟論理レプリケーションワーカーの最大数を指定します。
これには、リーダー適用ワーカー、パラレル適用ワーカーおよびテーブル同期ワーカーが含まれます。␞␞       </para>␞
␝       <para>␟        Logical replication workers are taken from the pool defined by
        <varname>max_worker_processes</varname>.␟論理レプリケーションワーカーは、<varname>max_worker_processes</varname>で定義されたプールから取得されます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The default value is 4. This parameter can only be set at server
        start.␟デフォルト値は4です。
このパラメータはサーバ起動時にのみ設定可能です。␞␞       </para>␞
␝       <para>␟        Maximum number of synchronization workers per subscription. This
        parameter controls the amount of parallelism of the initial data copy
        during the subscription initialization or when new tables are added.␟サブスクリプションごとの同期ワーカーの最大数です。
このパラメータは、サブスクリプションの初期化中、あるいは新しいテーブルが追加されたときの初期データコピーの並列度を制御します。␞␞       </para>␞
␝       <para>␟        Currently, there can be only one synchronization worker per table.␟今のところ、一つのテーブルにつき、同期ワーカーは一つだけです。␞␞       </para>␞
␝       <para>␟        The synchronization workers are taken from the pool defined by
        <varname>max_logical_replication_workers</varname>.␟同期ワーカーは、<varname>max_logical_replication_workers</varname>で定義されたプールから取得されます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The default value is 2. This parameter can only be set in the
        <filename>postgresql.conf</filename> file or on the server command
        line.␟デフォルト値は2です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        Maximum number of parallel apply workers per subscription. This
        parameter controls the amount of parallelism for streaming of
        in-progress transactions with subscription parameter
        <literal>streaming = parallel</literal>.␟サブスクリプションごとの適用並列ワーカーの最大数です。
このパラメータは、サブスクリプションパラメータの<literal>streaming = parallel</literal>が指定されている進行中のトランザクションのストリーミングに対する並列度の数を制御します。␞␞       </para>␞
␝       <para>␟        The parallel apply workers are taken from the pool defined by
        <varname>max_logical_replication_workers</varname>.␟並列同期ワーカーは、<varname>max_logical_replication_workers</varname>で定義されたプールから取得されます。␞␞       </para>␞
␝       <para>␟        The default value is 2. This parameter can only be set in the
        <filename>postgresql.conf</filename> file or on the server command
        line.␟デフォルト値は2です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝   <sect1 id="runtime-config-query">␟    <title>Query Planning</title>␟    <title>問い合わせ計画</title>␞␞␞
␝    <sect2 id="runtime-config-query-enable">␟     <title>Planner Method Configuration</title>␟     <title>プランナメソッド設定</title>␞␞␞
␝      <indexterm>
...
␝      <para>␟       These configuration parameters provide a crude method of
       ...
       STATISTICS</command>.␟以下の設定パラメータは、問い合わせオプティマイザが選択する問い合わせ計画に影響する大雑把な手法を提供します。
ある問い合わせに対してオプティマイザが選択したデフォルト計画が最適でない場合、<emphasis>暫定的な</emphasis>解決策は、これらの設定パラメータの1つを使用し、オプティマイザに異なる計画を選択するように仕向けることです。
オプティマイザが選択する計画の品質を改善するためのより良い方法には、プランナコスト定数を調節する（<xref linkend="runtime-config-query-constants"/>を参照）、<link linkend="sql-analyze"><command>ANALYZE</command></link>を手作業で実行する、<xref linkend="guc-default-statistics-target"/>設定パラメータの値を大きくする、<command>ALTER TABLE SET STATISTICS</command>を使用して、特定の列に対して収集される統計情報を増やす、などがあります。␞␞      </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables or disables the query planner's use of async-aware
        append plan types. The default is <literal>on</literal>.␟問い合わせプランナが非同期を意識したアペンド計画型を使うのを可能あるいは不可能にします。
デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables or disables the query planner's use of bitmap-scan plan
        types. The default is <literal>on</literal>.␟問い合わせプランナがビットマップスキャン計画型を選択することを有効もしくは無効にします。デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables or disables the query planner's use of gather
        merge plan types. The default is <literal>on</literal>.␟問い合わせプランナがギャザーマージ計画型を選択することを有効もしくは無効にします。
デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Controls if the query planner will produce a plan which will provide
        ...
        efficient plan. The default value is <literal>on</literal>.␟問い合わせプランナが、インデックススキャンのようなプランの子ノードのキーの順でソートされた<literal>GROUP BY</literal>キーを提供するプランを生成するかどうかを制御します。
...
デフォルト値は<literal>on</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables or disables the query planner's use of hashed
        aggregation plan types. The default is <literal>on</literal>.␟問い合わせプランナがハッシュ集約計画型を選択することを有効もしくは無効にします。
デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables or disables the query planner's use of hash-join plan
        types. The default is <literal>on</literal>.␟問い合わせプランナがハッシュ結合計画型を選択することを有効もしくは無効にします。デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables or disables the query planner's use of incremental sort steps.
        The default is <literal>on</literal>.␟問い合わせプランナが増分ソートステップを使うのを有効あるいは無効にします。
デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables or disables the query planner's use of index-scan and
        index-only-scan plan types.  The default is <literal>on</literal>.
        Also see <xref linkend="guc-enable-indexonlyscan"/>.␟問い合わせプランナがインデックススキャン計画型とインデックスオンリースキャン計画型を選択することを有効もしくは無効にします。
デフォルトは<literal>on</literal>です。
<xref linkend="guc-enable-indexonlyscan"/>も参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables or disables the query planner's use of index-only-scan plan
        ...
        enabled to have the query planner consider index-only-scans.␟問い合わせプランナがインデックスオンリースキャン計画型を選択することを有効もしくは無効にします（<xref linkend="indexes-index-only-scans"/>を参照）。
デフォルトは<literal>on</literal>です。
問い合わせプランナがインデックスオンリースキャンを考慮するようにするには、<xref linkend="guc-enable-indexscan"/>設定も有効にすることが必要です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables or disables the query planner's use of materialization.
        ...
        The default is <literal>on</literal>.␟問い合わせプランナの具体化の使用を有効、または無効にします。
全体にわたって具体化を差し止めることはできませんが、この値をoffにすることにより、正確性が要求される場合を除いて、具体化ノードをプランナが挿入することを防止します。デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables or disables the query planner's use of memoize plans for
        ...
        <literal>on</literal>.␟問い合わせプランナが、ネステッドループ結合の中のパラメータ付きスキャンの結果をキャッシュするメモ化計画を使用するのを有効あるいは無効にします。
...
␝       <para>␟        Enables or disables the query planner's use of merge-join plan
        types. The default is <literal>on</literal>.␟問い合わせプランナがマージ結合計画型を選択することを有効もしくは無効にします。デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables or disables the query planner's use of nested-loop join
        plans. It is impossible to suppress nested-loop joins entirely,
        but turning this variable off discourages the planner from using
        one if there are other methods available. The default is
        <literal>on</literal>.␟問い合わせプランナがネステッドループ結合計画を選択することを有効もしくは無効にします。
...
␝       <para>␟        Enables or disables the query planner's use of parallel-aware
        append plan types. The default is <literal>on</literal>.␟問い合わせプランナによるパラレル認識なアペンド計画型の利用を有効あるいは無効にします。
デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝       <indexterm>
...
␝       <para>␟        Enables or disables the query planner's use of hash-join plan
        ...
        also enabled. The default is <literal>on</literal>.␟問い合わせプランナによる、パラレルハッシュを使ったハッシュ結合計画型の利用を有効あるいは無効にします。
...
␝       <para>␟        Enables or disables the query planner's ability to eliminate a
        ...
        See <xref linkend="ddl-partition-pruning"/> for details.␟問い合わせプランナが、クエリプランからパーティション化テーブルのパーティションを除く機能を有効あるいは無効にします。
これはまた、クエリエグゼキュータがクエリ実行中にパーティションを削除（無視）することができるプランを生成するプランナの機能を制御します。
デフォルトは<literal>on</literal>です。
詳細は<xref linkend="ddl-partition-pruning"/>をご覧ください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables or disables the query planner's use of partitionwise join,
        ...
        terms of memory and CPU.  The default value is <literal>off</literal>.␟パーティション同士の結合(partitionwise join)を問い合わせプランナが使用するのを有効あるいは無効にします。
...
␝       <para>␟        Enables or disables the query planner's use of partitionwise grouping
        ...
        <literal>off</literal>.␟パーティション化されたテーブルに対してグループ化または集約を個別に実行できるように、問い合わせプランナのパーティションごと（partitionwise）のグループ化または集約の使用を有効または無効にします。
...
␝       <para>␟        Controls if the query planner will produce a plan which will provide
        ...
        <literal>on</literal>.␟問い合わせプランナが、問い合わせの<literal>ORDER BY</literal>/<literal>DISTINCT</literal>集約関数に必要なオーダーで事前にソートされた行を生成するかどうかを制御します。
...
␝       <para>␟        Enables or disables the query planner's use of sequential scan
        plan types. It is impossible to suppress sequential scans
        entirely, but turning this variable off discourages the planner
        from using one if there are other methods available. The
        default is <literal>on</literal>.␟問い合わせプランナがシーケンシャルスキャン計画型を選択することを有効もしくは無効にします。
...
␝       <para>␟        Enables or disables the query planner's use of explicit sort
        steps. It is impossible to suppress explicit sorts entirely,
        but turning this variable off discourages the planner from
        using one if there are other methods available. The default
        is <literal>on</literal>.␟問い合わせプランナが明示的並べ替え手順を選択することを有効もしくは無効にします。
明示的並べ替えを完全に禁止することは不可能ですが、この変数をオフにすると、もし他の方法が利用できるのであれば、プランナはその使用を行わないようになります。
デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝       <para>␟        Enables or disables the query planner's use of <acronym>TID</acronym>
        scan plan types. The default is <literal>on</literal>.␟問い合わせプランナが<acronym>TID</acronym>スキャン計画型を選択することを有効もしくは無効にします。
デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝     <sect2 id="runtime-config-query-constants">␟     <title>Planner Cost Constants</title>␟     <title>プランナコスト定数</title>␞␞␞
␝    <para>␟     The <firstterm>cost</firstterm> variables described in this section are measured
     ...
     times in milliseconds on a particular machine.␟本節で扱う<firstterm>コスト</firstterm>変数は、任意の尺度で測られます。
...
しかし望むなら、特定のマシンにおけるミリ秒単位の実行時間など、異なる尺度を使用することができます。␞␞    </para>␞
␝      <indexterm>
...
␝    <para>␟     Unfortunately, there is no well-defined method for determining ideal
     ...
     risky.␟残念ながら、コスト変数に対する理想的な値を決定する、上手く定義された方法がありません。
特定のインストレーションが受け取る問い合わせ全体を混在させたものの平均として扱うのが最善でしょう。
数回の実験のみを根拠にこの値を変更することは危険であるといえます。␞␞    </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the planner's estimate of the cost of a disk page fetch
        ...
        (see <xref linkend="sql-altertablespace"/>).␟シーケンシャルな一連の取り出しの一部となる、ディスクページ取り出しに関する、プランナの推定コストを設定します。
デフォルトは1.0です。
この値は同じ名前のテーブル空間パラメータを設定することで、特定のテーブル空間の中にあるテーブルとインデックスに対して上書きできます（<xref linkend="sql-altertablespace"/>を参照してください）。␞␞       </para>␞
␝       <para>␟        Sets the planner's estimate of the cost of a
        non-sequentially-fetched disk page.  The default is 4.0.
        This value can be overridden for tables and indexes in a particular
        tablespace by setting the tablespace parameter of the same name
        (see <xref linkend="sql-altertablespace"/>).␟非シーケンシャル的に取り出されるディスクページのコストに対するプランナの推測を設定します。
デフォルトは4です。
この値は同じ名前のテーブル空間パラメータを設定することで、特定のテーブル空間の中にあるテーブルとインデックスに対して上書きできます（<xref linkend="sql-altertablespace"/>を参照してください）。␞␞       </para>␞
␝       <para>␟        Reducing this value relative to <varname>seq_page_cost</varname>
        ...
        parameters.␟この値を<varname>seq_page_cost</varname>と比較して小さくすると、システムはなるべくインデックススキャンを使用するようになります。
大きくすると、インデックススキャンが相対的に高価になります。
両方の値を増減させることで、CPUコストに対するディスクI/Oコストの重要性を変更させることができます。
これについては、後述のパラメータで説明します。␞␞       </para>␞
␝       <para>␟        Random access to mechanical disk storage is normally much more expensive
        than four times sequential access.  However, a lower default is used
        (4.0) because the majority of random accesses to disk, such as indexed
        reads, are assumed to be in cache.  The default value can be thought of
        as modeling random access as 40 times slower than sequential, while
        expecting 90% of random reads to be cached.␟機械的ディスク記憶装置に対するランダムアクセスは通常はシーケンシャルアクセスの4倍よりもかなり高価です。
しかし、より低いデフォルト（4.0）が使用されます。というのはインデックスのついた読み取りのようなディスクに対するランダムアクセスのほとんどはキャッシュにあると想定されるからです。
このデフォルト値は、ランダムアクセスがシーケンシャルアクセスより40倍遅い一方で、ランダム読み込みの90%はキャッシュされていることが期待されるというモデルとして考えることができます。␞␞       </para>␞
␝       <para>␟        If you believe a 90% cache rate is an incorrect assumption
        ...
        e.g., <literal>1.1</literal>.␟自環境の作業負荷において、90％のキャッシュ率は誤った仮定と考えられるのであれば、ランダム記憶装置読み込みのコストをより良く反映するため random_page_cost を大きくすることができます。
反対に、データが完全にキャッシュされていると思われるのであれば、random_page_cost を小さくすることが適切です。例えば、データベースの容量がサーバのメモリより小さい場合などです。
例えばSSDのような、シーケンシャルアクセスに比べてランダム読み込みコストがあまり大きくない記憶装置の場合も、random_page_cost に対し<literal>1.1</literal>のようにより低い値のモデル化の方が良いでしょう。␞␞       </para>␞
␝      <indexterm>
...
␝        <para>␟         Although the system will let you set <varname>random_page_cost</varname> to
         ...
         than it would normally be.␟システムは<varname>random_page_cost</varname>を<varname>seq_page_cost</varname>よりも小さな値に設定することを許しますが、そのようにすることは物理的にはおかしなことです。
...
RAM内に存在するページの取り出しコストは通常よりもかなり小さくなるためです。␞␞        </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the planner's estimate of the cost of processing
        each row during a query.
        The default is 0.01.␟問い合わせ時のそれぞれの行の処理コストに対するプランナの推測を設定します。
デフォルトは0.01です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the planner's estimate of the cost of processing
        ...
        The default is 0.005.␟インデックススキャン時のそれぞれのインデックス行の処理コストに対するプランナの推測を設定します。
デフォルトは0.005です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the planner's estimate of the cost of processing each
        operator or function executed during a query.
        The default is 0.0025.␟問い合わせ時に実行される各演算子や関数の処理コストに対するプランナの推測を設定します。デフォルトは0.0025です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the planner's estimate of the cost of launching parallel worker
        processes.
        The default is 1000.␟パラレルワーカープロセスを起動するためのコストに対するプランナの推測値を設定します。
デフォルトは1000です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the planner's estimate of the cost of transferring one tuple
        from a parallel worker process to another process.
        The default is 0.1.␟あるパラレルワーカープロセスから、1タプルを他のプロセスに転送するためのコストに対するプランナの推測値を設定します。
デフォルトは0.1です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the minimum amount of table data that must be scanned in order
        ...
        If this value is specified without units, it is taken as blocks,
        that is <symbol>BLCKSZ</symbol> bytes, typically 8kB.
        The default is 8 megabytes (<literal>8MB</literal>).␟パラレルスキャンを考慮する最小のテーブルデータのサイズを指定します。
パラレルシーケンシャルスキャンでは、スキャンされるテーブルのデータ量は、常にテーブルのサイズと同じです。
しかし、インデックスが使われる場合は、スキャンされるテーブルの量は通常少なくなるでしょう。
この値が単位なしで指定された場合は、ブロック単位であるとみなします。すなわち、<symbol>BLCKSZ</symbol>バイト、一般的には8kBです。
デフォルトは8メガバイト（ <literal>8MB</literal>）です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the minimum amount of index data that must be scanned in order
        ...
        If this value is specified without units, it is taken as blocks,
        that is <symbol>BLCKSZ</symbol> bytes, typically 8kB.
        The default is 512 kilobytes (<literal>512kB</literal>).␟パラレルスキャンが考慮されるために、スキャンされなければならないインデックスデータの最小量を設定します。
...
この値が単位なしで指定された場合は、ブロック単位であるとみなします。すなわち、<symbol>BLCKSZ</symbol>バイト、一般的には8kBです。
デフォルトは512キロバイト（<literal>512kB</literal>）です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the planner's assumption about the effective size of the
        ...
        (If <symbol>BLCKSZ</symbol> is not 8kB, the default value scales
        proportionally to it.)␟単一の問い合わせで利用できるディスクキャッシュの実効容量に関するプランナの条件を設定します。
...
（<symbol>BLCKSZ</symbol>が8kbでなければ、この設定のデフォルト値と最大値が<symbol>BLCKSZ</symbol>に比例して変更されます。）␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the query cost above which JIT compilation is activated, if
        ...
        The default is <literal>100000</literal>.␟JITが有効な場合（<xref linkend="jit"/>参照）、それ以上ならJITコンパイルが起動する問い合わせコストを設定します。
<acronym>JIT</acronym>を実行するとプラン時間がかかりますが、問い合わせの実行を高速化することができます。
これを<literal>-1</literal>にすると、JITコンパイルは無効になります。
デフォルトは<literal>100000</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the query cost above which JIT compilation attempts to inline
        ...
        The default is <literal>500000</literal>.␟それ以上ならJITコンパイルが関数と演算子のインライン化を試みる問い合わせコストを設定します。
...
デフォルトは<literal>500000</literal>です。␞␞       </para>␞
␝       <para>␟        Sets the query cost above which JIT compilation applies expensive
        ...
        The default is <literal>500000</literal>.␟それ以上ならJITコンパイルが高価な最適化を実行する問い合わせコストを設定します。
...
デフォルトは<literal>500000</literal>です。␞␞       </para>␞
␝     <sect2 id="runtime-config-query-geqo">␟     <title>Genetic Query Optimizer</title>␟     <title>遺伝的問い合わせオプティマイザ</title>␞␞␞
␝      <indexterm>
...
␝     <para>␟      The genetic query optimizer (GEQO) is an algorithm that does query
      ...
      For more information see <xref linkend="geqo"/>.␟遺伝的問い合わせオプティマイザ（GEQO）はヒューリスティック（発見的）検索法を用いた問い合わせ計画を行なう演算手法です。
通常のしらみつぶしの検索演算手法で見いだされる計画よりも時として劣った計画を作成するという代償を払いますが、この手法は（多くのリレーションを結合するような）複雑な問い合わせに対し計画時間を軽減します。
より詳細は<xref linkend="geqo"/>を参照してください。␞␞     </para>␞
␝      <indexterm>
...
␝       <para>␟        Enables or disables genetic query optimization.
        ...
        more granular control of GEQO.␟遺伝的問い合わせ最適化を有効もしくは無効にします。デフォルトは有効です。
運用時には無効にしないことが通常最善です。<varname>geqo_threshold</varname>変数は、GEQOを制御するためよりきめ細かな方法を提供します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Use genetic query optimization to plan queries with at least
        ...
        use of GEQO.␟少なくともこれだけの数の<literal>FROM</literal>項目数があるときに、問い合わせを計画するのに遺伝的問い合わせ最適化を使用します。
...
従って、問い合わせの大きさに対する閾値はGEQOの使用を管理するのに便利な方法です。␞␞       </para>␞
␝       <para>␟        Controls the trade-off between planning time and query plan
        ...
        chosen.␟GEQOにおける計画時間と問い合わせ計画の品質間のトレードオフを制御します。この変数は1から10までの範囲の整数でなければなりません。
デフォルトの値は5です。値を大きくすると、問い合わせ計画作成により多くの時間を費すことになりますが、より効率的な問い合わせ計画が選択される可能性が増加します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        <varname>geqo_effort</varname> doesn't actually do anything
        directly; it is only used to compute the default values for
        the other variables that influence GEQO behavior (described
        below). If you prefer, you can set the other parameters by
        hand instead.␟実際<varname>geqo_effort</varname>は直接何も行いません。それはGEQOの動作に影響を与える他の変数に対し、デフォルトの値を計算するためにのみ使用されます（以下で説明します）。もしよければ、代わりに手作業で他のパラメータを設定できます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Controls the pool size used by GEQO, that is the
        number of individuals in the genetic population.  It must be
        at least two, and useful values are typically 100 to 1000.  If
        it is set to zero (the default setting) then a suitable
        value is chosen based on <varname>geqo_effort</varname> and
        the number of tables in the query.␟GEQOで使用されるプール容量を管理します。それは遺伝的個体群内の個体数です。最低でも2つはなければならず、よく100から1000までの値が使用されます。
もし（デフォルトの設定である）零に設定されると、<varname>geqo_effort</varname>および問い合わせの中のテーブル数に基づいて、適切な値が選択されます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Controls the number of generations used by GEQO, that is
        the number of iterations of the algorithm.  It must
        be at least one, and useful values are in the same range as
        the pool size.  If it is set to zero (the default setting)
        then a suitable value is chosen based on
        <varname>geqo_pool_size</varname>.␟GEQOで使用される世代の数を管理します。それはアルゴリズムの反復数です。最低でも1はなければならず、よくプールサイズと同じ範囲の値が使用されます。
これを0に設定（デフォルトの設定）すると、適切な値が<varname>geqo_pool_size</varname>に基づいて選択されます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Controls the selection bias used by GEQO. The selection bias
        is the selective pressure within the population. Values can be
        from 1.50 to 2.00; the latter is the default.␟GEQOで使用される淘汰の偏りを管理します。
淘汰の偏りは個体群内の（遺伝的な）自然淘汰です。値は1.50から2.00で、2.00がデフォルトです。␞␞       </para>␞
␝       <para>␟        Controls the initial value of the random number generator used
        ...
        better or worse best path being found.␟結合順序検索空間にわたって、GEQOが無作為のパスを選択するために使用される乱数発生器の初期値を制御します。
値は0（デフォルト）から1までの範囲です。
値を変動させると探査される結合パスの集合が変化するため、見つかる最善のパスが良くなる場合も悪くなる場合もあります。␞␞       </para>␞
␝      <indexterm>
...
␝     <sect2 id="runtime-config-query-other">␟     <title>Other Planner Options</title>␟     <title>その他のプランナオプション</title>␞␞␞
␝      <indexterm>
...
␝       <para>␟        Sets the default statistics target for table columns without
        ...
        query planner, refer to <xref linkend="planner-stats"/>.␟<command>ALTER TABLE SET STATISTICS</command>で列特定の目的セットの無いテーブル列に対し、デフォルトの統計対象を設定します。
より大きい値は<command>ANALYZE</command>に必要な時間を増加させますが、プランナの予測の品質を向上させます。
デフォルトは100です。
<productname>PostgreSQL</productname>の問い合わせプランナによる統計情報の使用方法に関するより詳細な情報は、<xref linkend="planner-stats"/>を参照してください。␞␞       </para>␞
␝       <para>␟        Controls the query planner's use of table constraints to
        optimize queries.
        ...
        performance.␟問い合わせプランナが問い合わせを最適化する際のテーブル制約の使用を制御します。
...
伝統的な継承ツリーでしばしば性能向上のために使用されます。␞␞      </para>␞
␝       <para>␟        When this parameter allows it for a particular table, the planner
        ...
        contradict the constraints.  For example:␟このパラメータが特定のテーブルに対して許される時、プランナはそのテーブルの<literal>CHECK</literal>制約で問い合わせ条件を比較し、制約と矛盾する条件のテーブルのスキャンを省きます。
例えば以下のようになります。␞␞␞
␝␟        With constraint exclusion enabled, this <command>SELECT</command>
        will not scan <structname>child1000</structname> at all, improving performance.␟制約排除が有効であると、この<command>SELECT</command>は全く<structname>child1000</structname>をスキャンせず、性能を向上させます。␞␞       </para>␞
␝       <para>␟        Currently, constraint exclusion is enabled by default
        ...
        <xref linkend="guc-enable-partition-pruning"/>.)␟現在、継承ツリーを使ってテーブルパーティショニングを実装するために使用されるよくある場合のことだけを考慮して、制約排除はデフォルトで有効です。
...
（パーティショニングされたテーブル用の同等の機能は別のパラメータ<xref linkend="guc-enable-partition-pruning"/>で制御します。）␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Refer to <xref linkend="ddl-partitioning-constraint-exclusion"/> for
        more information on using constraint exclusion to implement
        partitioning.␟パーティショニングを実装するための制約排除の利用についてのより進んだ情報は<xref linkend="ddl-partitioning-constraint-exclusion"/>を参照ください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the planner's estimate of the fraction of a cursor's rows that
        ...
        first rows might be delivered.␟検索されるカーソル行の割合のプランナの見積もりを設定します。
デフォルトは0.1です。
...
つまり、推定時間全体のみが考慮され、先頭の行の取り出しにかかる時間は考慮されなくなります。␞␞       </para>␞
␝       <para>␟        The planner will merge sub-queries into upper queries if the
        resulting <literal>FROM</literal> list would have no more than
        this many items.  Smaller values reduce planning time but might
        yield inferior query plans.  The default is eight.
        For more information see <xref linkend="explicit-joins"/>.␟プランナは、<literal>FROM</literal>リストがこの数の項目より少ない結果の場合、副問い合わせを上位の問い合わせに併合します。
より小さい値は計画時間を縮小させますが、劣った問い合わせ計画をもたらす可能性があります。
デフォルトは8です。
詳細は<xref linkend="explicit-joins"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Setting this value to <xref linkend="guc-geqo-threshold"/> or more
        may trigger use of the GEQO planner, resulting in non-optimal
        plans.  See <xref linkend="runtime-config-query-geqo"/>.␟この値を<xref linkend="guc-geqo-threshold"/>か、それ以上に設定するとGEQOプランナ使用の誘引となり、最適ではない計画をもたらします。<xref linkend="runtime-config-query-geqo"/>を参照してください。␞␞       </para>␞
␝        linkend="jit"/>).
...
␝       <para>␟        Determines whether <acronym>JIT</acronym> compilation may be used by
        <productname>PostgreSQL</productname>, if available (see <xref
        linkend="jit"/>).
        The default is <literal>on</literal>.␟<productname>PostgreSQL</productname>が、可能ならば<acronym>JIT</acronym>コンパイルを使うかどうかを決定します（<xref linkend="jit"/>を参照）。
デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝       <para>␟        The planner will rewrite explicit <literal>JOIN</literal>
        ...
        yield inferior query plans.␟最終的にリストがこの項目数以下になる時、プランナは、明示的な<literal>JOIN</literal>構文（<literal>FULL JOIN</literal>を除く）を<literal>FROM</literal>項目のリストに直します。
この値を小さくすれば計画作成時間は減少しますが、劣った問い合わせ計画が作成される可能性があります。␞␞       </para>␞
␝       <para>␟        By default, this variable is set the same as
        ...
        For more information see <xref linkend="explicit-joins"/>.␟デフォルトでは、この値は<varname>from_collapse_limit</varname>と同じ値に設定されており、殆どの場合に適切です。
...
␝       <para>␟        Prepared statements (either explicitly prepared or implicitly
        ...
        For more information see <xref linkend="sql-prepare"/>.␟準備された文（明示的に準備されたものあるいはPL/pgSQLのように暗黙的に生成されたもののどちらにおいても）は、カスタムあるいは汎用(generic)プランで実行することができます。
...
詳細は<xref linkend="sql-prepare"/>をご覧ください。␞␞       </para>␞
␝       <para>␟        Sets the planner's estimate of the average size of the working
        ...
        benefit from larger-than-default values.␟<link linkend="queries-with-recursive">再帰問い合わせ</link>のワーキング・テーブルの平均サイズのプランナの推定値を、問い合わせの最初の非再帰項の推定サイズの倍数として設定します。
...
グラフ分析の問い合わせでは、デフォルト値より大きい値が有効な場合があります。␞␞       </para>␞
...
␝   <sect1 id="runtime-config-logging">␟    <title>Error Reporting and Logging</title>␟    <title>エラー報告とログ出力</title>␞␞␞
...
␝    <sect2 id="runtime-config-logging-where">␟     <title>Where to Log</title>␟     <title>どこにログを出力するか</title>␞␞␞
␝       <para>␟        <productname>PostgreSQL</productname> supports several methods
         ...
         only.
         This parameter can only be set in the <filename>postgresql.conf</filename>
         file or on the server command line.␟<productname>PostgreSQL</productname>は、<systemitem>stderr</systemitem>、<systemitem>jsonlog</systemitem>、<systemitem>csvlog</systemitem>、<systemitem>jsonlog</systemitem>および<systemitem>syslog</systemitem>を含めて、サーバメッセージのログ出力に対し数種類の方法を提供します。
...
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        If <systemitem>csvlog</systemitem> is included in <varname>log_destination</varname>,
        ...
        CSV-format log output.␟<systemitem>csvlog</systemitem>が<varname>log_destination</varname>に含まれる場合、ログ項目はプログラムへの読み込みが簡便な<quote>カンマ区切り値</quote>書式（<acronym>CSV</acronym>）で出力されます。
詳細は<xref linkend="runtime-config-logging-csvlog"/>を参照してください。
CSV書式のログ出力を生成するためには<xref linkend="guc-logging-collector"/>を有効にする必要があります。␞␞       </para>␞
␝       <para>␟        If <systemitem>jsonlog</systemitem> is included in
        ...
        <xref linkend="guc-logging-collector"/> must be enabled to generate
        JSON-format log output.␟<systemitem>jsonlog</systemitem>が<varname>log_destination</varname>に含まれる場合、ログ項目はプログラムへの読み込みが簡便な<acronym>JSON</acronym>書式で出力されます。
詳細は<xref linkend="runtime-config-logging-jsonlog"/>を参照してください。
JSON書式のログ出力を生成するためには<xref linkend="guc-logging-collector"/>を有効にする必要があります。␞␞       </para>␞
␝       <para>␟        When either <systemitem>stderr</systemitem>,
        ...
        is an example of this file's content:␟<systemitem>stderr</systemitem>、<systemitem>csvlog</systemitem>、または<systemitem>jsonlog</systemitem>のいずれかが含まれている場合、ファイル<filename>current_logfiles</filename>が作成され、ロギングコレクタによって現在使用されているログファイルの場所と関連付けられたロギング先が記録されます。
これにより、インスタンスによって現在使用されているログを簡単に見つけることができます。
このファイルの内容の例を次に示します:␞␞<programlisting>␞
␝␟        <filename>current_logfiles</filename> is recreated when a new log file
        ...
        collector is disabled.␟<filename>current_logfiles</filename>は、ローテーションの効果として新しいログファイルが作成されたとき、および<varname>log_destination</varname>がリロードされたときに再作成されます。
<systemitem>stderr</systemitem>、<systemitem>csvlog</systemitem>、<systemitem>jsonlog</systemitem>のいずれも<varname>log_destination</varname>に含まれていないとき、およびロギングコレクタが無効になっているときに削除されます。␞␞       </para>␞
␝        <para>␟         On most Unix systems, you will need to alter the configuration of
         ...
         will discard all such messages.  You will need to add something like:␟<varname>log_destination</varname>で<systemitem>syslog</systemitem>オプションを使用できるようにするために、ほとんどのUnixシステムではシステムの<application>syslog</application>デーモンの設定を変更しなければならないでしょう。
...
うまく動作させるために␞␞<programlisting>␞
␝</programlisting>␟         to the  <application>syslog</application> daemon's configuration file
         to make it work.␟<application>syslog</application>デーモンの設定ファイルに追加しなければならないでしょう。␞␞        </para>␞
␝      <indexterm>
...
␝        <para>␟         On Windows, when you use the <literal>eventlog</literal>
         ...
         See <xref linkend="event-log-registration"/> for details.␟Windowsで<varname>log_destination</varname>に対し<literal>eventlog</literal>オプションを使用する場合、Windows Event Viewer がイベントログメッセージを手際良く表示できるよう、オペレーティングシステムでイベントソースとそのライブラリを登録しなければなりません。
詳細は<xref linkend="event-log-registration"/>を参照ください。␞␞        </para>␞
␝       <para>␟         This parameter enables the <firstterm>logging collector</firstterm>, which
         ...
         This parameter can only be set at server start.␟このパラメータは<firstterm>ログ収集機構</firstterm>を有効にします。
...
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝        <para>␟         It is possible to log to <systemitem>stderr</systemitem> without using the
         ...
         overwrite each other's output.␟ログ収集機構を使用せずに<systemitem>stderr</systemitem>のログを取ることは可能です。
ログメッセージはサーバの<systemitem>stderr</systemitem>が指し示すいかなる場所にも向かうだけです。
しかし、その方法はログファイルを巡回させる都合のよい方法を提供しないので、ログ容量が小さい場合のみに適しています。
同時に、ログ収集機構を使用しないいくつかのプラットフォームにおいては、ログ出力が失われたり、文字化けします。なぜなら、同一のログファイルに同時に書き込みを行うマルチプロセッサはそれぞれの出力を上書きできるからです。␞␞        </para>␞
␝      <indexterm>
...
␝        <para>␟          The logging collector is designed to never lose messages.  This means
          ...
          the rest of the system.␟ログ収集機構はメッセージを決して失わないために設計されています。
...
これらの場合にはいくつかのログメッセージを失うことになりますが、残ったシステムを阻止しません。␞␞        </para>␞
␝      <indexterm>
...
␝       <para>␟        When <varname>logging_collector</varname> is enabled,
        this parameter determines the directory in which log files will be created.
        It can be specified as an absolute path, or relative to the
        cluster data directory.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.
        The default is <literal>log</literal>.␟<varname>logging_collector</varname>を有効と設定した場合、このパラメータはログファイルが作成されるディレクトリを確定します。
ディレクトリは、絶対パス、もしくはデータベースクラスタのディレクトリに対する相対パスで指定することができます。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
デフォルトは<literal>log</literal>です。␞␞       </para>␞
␝       <para>␟        When <varname>logging_collector</varname> is enabled,
        ...
        The default is <literal>postgresql-%Y-%m-%d_%H%M%S.log</literal>.␟<varname>logging_collector</varname>が有効な場合、このパラメータは作成されたログファイルのファイル名を設定します。
...
デフォルトは<literal>postgresql-%Y-%m-%d_%H%M%S.log</literal>です。␞␞       </para>␞
␝       <para>␟        If you specify a file name without escapes, you should plan to
        ...
        longer the case.␟エスケープすることなくファイル名を指定する場合、ディスク全体を使い切ってしまうことを防止するためにログローテーションを行うユーティリティを使用することを計画しなければなりません。
8.4より前のリリースの<productname>PostgreSQL</productname>では、<literal>%</literal>エスケープがなければ、新しいログファイルの生成時のエポック時刻を付与しますが、これはもはや当てはまりません。␞␞       </para>␞
␝       <para>␟        If CSV-format output is enabled in <varname>log_destination</varname>,
        ...
        replaced instead.)␟CSV書式の出力が<varname>log_destination</varname>で有効な場合、タイムスタンプ付きのログファイル名に<literal>.csv</literal>を付与し、最終的なCSV書式出力用のファイル名が作成されます。
（<varname>log_filename</varname>が<literal>.log</literal>で終わる場合は後置詞が置き換えられます。）␞␞       </para>␞
␝       <para>␟        If JSON-format output is enabled in <varname>log_destination</varname>,
        <literal>.json</literal> will be appended to the timestamped
        log file name to create the file name for JSON-format output.
        (If <varname>log_filename</varname> ends in <literal>.log</literal>, the suffix is
        replaced instead.)␟JSON書式の出力が<varname>log_destination</varname>で有効な場合、タイムスタンプ付きのログファイル名に<literal>.json</literal>を付与し、最終的なJSON書式出力用のファイル名が作成されます。
（<varname>log_filename</varname>が<literal>.log</literal>で終わる場合は後置詞が置き換えられます。）␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        On Unix systems this parameter sets the permissions for log files
        ...
        <function>chmod</function> and <function>umask</function>
        system calls.  (To use the customary octal format the number
        must start with a <literal>0</literal> (zero).)␟Unixシステムにおいては、<varname>logging_collector</varname>が有効になっている場合、このパラメータはログファイルのパーミッションを設定します。
...
（慣例的な8進数フォーマットを使用する場合、番号は<literal>0</literal>（ゼロ）で始まらなければなりません。␞␞       </para>␞
␝       <para>␟        The default permissions are <literal>0600</literal>, meaning only the
        ...
        they might contain sensitive data.␟デフォルトのパーミッションは<literal>0600</literal>で、意味するところはサーバの所有者のみログファイルの読み書きが可能です。
...
いずれにせよ、それらは機密データを含む可能性があるため、ログファイルを誰もが読み取り可能にすることは賢明ではありません。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        When <varname>logging_collector</varname> is enabled,
        ...
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟<varname>logging_collector</varname>が有効な場合、このパラメータは個々のログファイルの最大寿命を決定します。
...
␝       <para>␟        When <varname>logging_collector</varname> is enabled,
        ...
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟<varname>logging_collector</varname>が有効な場合、このパラメータは個々のログファイルの最大容量を決定します。
...
␝       <para>␟        When <varname>logging_collector</varname> is enabled,
        ...
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟<varname>logging_collector</varname>が有効な場合、このパラメータにより、<productname>PostgreSQL</productname>は既存の同名のファイルに追加するのではなく、そのファイルを切り詰める（上書きする）ようになります。
...
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        Example:  To keep 7 days of logs, one log file per day named
        <literal>server_log.Mon</literal>, <literal>server_log.Tue</literal>,
        etc., and automatically overwrite last week's log with this week's log,
        set <varname>log_filename</varname> to <literal>server_log.%a</literal>,
        <varname>log_truncate_on_rotation</varname> to <literal>on</literal>, and
        <varname>log_rotation_age</varname> to <literal>1440</literal>.␟例：7日間のログを保存し、毎日のログを <literal>server_log.Mon</literal>、<literal>server_log.Tue</literal>、等とし、そして自動的に前週のログを今週のログで上書きするには以下のように設定します。
<varname>log_filename</varname> を <literal>server_log.%a</literal>とし、<varname>log_truncate_on_rotation</varname> を <literal>on</literal>にし、そして<varname>log_rotation_age</varname> を <literal>1440</literal>に設定します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Example: To keep 24 hours of logs, one log file per hour, but
        ...
        different from the hour's initial file name.␟例：24時間のログを保持、1時間おきに1つのログファイルを作成、ただし、ログファイルのサイズが1ギガバイトを超えた場合それより早く切り替えさせるには、<varname>log_filename</varname>を<literal>server_log.%H%M</literal>にし、<varname>log_truncate_on_rotation</varname>を<literal>on</literal>にし、<varname>log_rotation_age</varname>を<literal>60</literal>にし、そして<varname>log_rotation_size</varname>を<literal>1000000</literal>に設定します。
<varname>log_filename</varname>に<literal>%M</literal>を含めると、サイズを元にしたローテーションが時間毎の始めのファイル名とは異なる名前のファイルを選択するようにできます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        When logging to <application>syslog</application> is enabled, this parameter
        ...
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟<application>syslog</application>へのログ出力が有効な場合、このパラメータは<application>syslog</application>の<quote>facility</quote>が使われるように確定します。
...
␝        <para>␟         When logging to <application>syslog</application> is enabled, this parameter
         ...
         This parameter can only be set in the <filename>postgresql.conf</filename>
         file or on the server command line.␟<application>syslog</application>にログ出力が有効な場合、このパラメータは<application>syslog</application>ログ内の<productname>PostgreSQL</productname>メッセージを特定するのに使用するプログラム名を確定します。デフォルトは<literal>postgres</literal>です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞        </para>␞
␝        <para>␟         When logging to <application>syslog</application> and this is on (the
         ...
         suppress repeated messages.␟<application>syslog</application>にログを出力している場合で、これがオン（デフォルト）であると、各メッセージには（<literal>[2]</literal>のような）増加する順序数が頭に追加されます。
...
繰り返されるメッセージを抑止したい場合には、これをオフにできます。␞␞        </para>␞
␝      <indexterm>
...
␝       <para>␟        When logging to <application>syslog</application> is enabled, this parameter
        ...
        the syslog service to cope with the potentially bulky messages.␟<application>syslog</application>へのログ出力が有効な場合、このパラメータはメッセージがどのようにsyslogに送られるかを規定します。
オンなら（デフォルト）、メッセージは行に分割され、長い行は、伝統的なsyslog実装のサイズ上限である1024バイト以内に分割されます。
オフならば、PostgreSQLサーバログメッセージは、そのままsyslogサービスに送られます。
大きなサイズになるかもしれないメッセージにどう対応するかは、syslogサービス次第となります。␞␞       </para>␞
␝       <para>␟        If syslog is ultimately logging to a text file, then the effect will
        ...
        more useful to keep messages logically together.␟もしsyslogが最終的にテキストファイルにログを出力するのであれば、どちらに設定しても効果は同じです。
設定値をオンにしておくのが最善です。
多くのsyslogの実装では、長いメッセージを扱えないか、長いメッセージを扱うための特別な設定が必要だからです。
しかし、syslogが最終的に他のメディアに書き込むのであれば、メッセージを論理的に一緒にしておくことが必要か、もしくは有用です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        When logging to <application>event log</application> is enabled, this parameter
        determines the program name used to identify
        <productname>PostgreSQL</productname> messages in
        the log. The default is <literal>PostgreSQL</literal>.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟<application>event log</application>へのログ出力が有効になっていると、このパラメータはログ中の<productname>PostgreSQL</productname>メッセージを特定するのに使用されるプログラム名を決定します。デフォルトは<literal>PostgreSQL</literal>です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝      <indexterm>
...
␝     <sect2 id="runtime-config-logging-when">␟     <title>When to Log</title>␟     <title>いつログを出力するか</title>␞␞␞
␝      <indexterm>
...
␝       <para>␟        Controls which <link linkend="runtime-config-severity-levels">message
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟どの<link linkend="runtime-config-severity-levels">メッセージレベル</link>をサーバログに書き込むかを管理します。
...
␝       <para>␟        Controls which SQL statements that cause an error
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟エラー条件の原因となったどのSQL文をサーバログに記録するかを制御します。
...
スーパーユーザと、適切な<literal>SET</literal>権限を持つユーザのみがこの設定を変更することができます。␞␞       </para>␞
␝        <para>␟         Causes the duration of each completed statement to be logged
         ...
         durations.
         Only superusers and users with the appropriate <literal>SET</literal>
         privilege can change this setting.␟文の実行に少なくとも指定した時間かかった場合、それぞれの文の実行に要した時間をログに記録します。
...
スーパーユーザと、適切な<literal>SET</literal>権限を持つユーザのみがこの設定を変更することができます。␞␞        </para>␞
␝        <para>␟         This overrides <xref linkend="guc-log-min-duration-sample"/>,
         meaning that queries with duration exceeding this setting are not
         subject to sampling and are always logged.␟これは<xref linkend="guc-log-min-duration-sample"/>を上書きします。つまり、問い合わせがこの設定を越えると、実行時間のサンプリングの対象にならず、常に記録されます。␞␞        </para>␞
␝        <para>␟         For clients using extended query protocol, durations of the Parse,
         Bind, and Execute steps are logged independently.␟拡張問い合わせプロトコルを使用するクライアントでは、Parse、Bind、Executeそれぞれの段階で要した時間が独立して記録されます。␞␞        </para>␞
␝      <indexterm>
...
␝        <para>␟         When using this option together with
         ...
         duration message using the process ID or session ID.␟このオプションと<xref linkend="guc-log-statement"/>を一緒に使用する時、<varname>log_statement</varname>によって記録されるテキスト文は、実行時間のログには重複されません。
<application>syslog</application>を使用していなければ、プロセスIDとセッションIDを使用して、文メッセージと後の実行時間メッセージを関連付けできるように、<xref linkend="guc-log-line-prefix"/>を使用してPIDまたはセッションIDをログに記録することを勧めます。␞␞        </para>␞
␝        <para>␟         Allows sampling the duration of completed statements that ran for
         ...
         durations.
         Only superusers and users with the appropriate <literal>SET</literal>
         privilege can change this setting.␟指定した時間以上で実行完了した文の実行時間のサンプルを許可します。
...
スーパーユーザと、適切な<literal>SET</literal>権限を持つユーザのみがこの設定を変更することができます。␞␞        </para>␞
␝        <para>␟         This setting has lower priority
         ...
         subject to sampling and are always logged.␟この設定は<varname>log_min_duration_statement</varname>よりも優先度が低いです。つまり、<varname>log_min_duration_statement</varname>を超えた実行時間の文はサンプリングの対象となり、常に記録されます。␞␞        </para>␞
␝      <indexterm>
...
␝        <para>␟         Other notes for <varname>log_min_duration_statement</varname>
         apply also to this setting.␟<varname>log_min_duration_statement</varname>のその他の注意事項もこの設定に適用されます。␞␞        </para>␞
␝      <indexterm>
...
␝        <para>␟         Determines the fraction of statements with duration exceeding
         ...
         <literal>-1</literal>.
         Only superusers and users with the appropriate <literal>SET</literal>
         privilege can change this setting.␟<xref linkend="guc-log-min-duration-sample"/>を越え、記録対象となる文の割合を決定します。
...
スーパーユーザと、適切な<literal>SET</literal>権限を持つユーザのみがこの設定を変更することができます。␞␞        </para>␞
␝        <para>␟         Sets the fraction of transactions whose statements are all logged,
         ...
         Only superusers and users with the appropriate <literal>SET</literal>
         privilege can change this setting.␟他の理由に加え、トランザクションの文のうち、ログの対象となる割合を設定します。
...
␝        <para>␟         Like all statement-logging options, this option can add significant
         overhead.␟他の文のログを取るオプション同様、このオプションも大きなオーバーヘッドを与える可能性があります。␞␞        </para>␞
␝        <para>␟         Sets the amount of time after which the startup process will log
         ...
         each operation.
         This parameter can only be set in the <filename>postgresql.conf</filename>
         file or on the server command line.␟起動プロセスが実行中の長時間実行操作に関するメッセージをログに記録するまでの時間と、その操作に関する次の進行状況メッセージの間隔を設定します。
デフォルトは10秒です。
<literal>0</literal>に設定すると、この機能は無効になります。この値を単位なしで指定すると、ミリ秒とみなされます。
この設定は、各操作に個別に適用されます。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞        </para>␞
␝        <para>␟         For example, if syncing the data directory takes 25 seconds and
         ...
         but nothing will be logged for resetting unlogged relations.␟たとえば、データディレクトリの同期に25秒かかり、その後、unloggedリレーションのリセットに8秒かかったとします。
この設定がデフォルト値の10秒である場合、データディレクトリを同期するためのメッセージは、10秒間処理された後と20秒間処理された後に記録されますが、unloggedリレーションのリセットに関しては何も記録されません。␞␞        </para>␞
␝    <para>␟     <xref linkend="runtime-config-severity-levels"/> explains the message
     ...
     as shown in the table.␟<xref linkend="runtime-config-severity-levels"/>で、<productname>PostgreSQL</productname>で使用されるメッセージ深刻度レベルを説明します。
ログ出力が<systemitem>syslog</systemitem>またはWindowsの<systemitem>eventlog</systemitem>に送られる場合、この深刻度レベルは表で示すように変換されます。␞␞    </para>␞
␝    <table id="runtime-config-severity-levels">␟     <title>Message Severity Levels</title>␟     <title>メッセージ深刻度レベル</title>␞␞     <tgroup cols="4">␞
␝       <row>␟        <entry>Severity</entry>␟        <entry>深刻度</entry>␞␞␞
␝        <entry>Severity</entry>␟        <entry>Usage</entry>␟        <entry>使用方法</entry>␞␞        <entry><systemitem>syslog</systemitem></entry>␞
␝        <entry><literal>DEBUG1 .. DEBUG5</literal></entry>␟        <entry>Provides successively-more-detailed information for use by
         developers.</entry>␟        <entry>開発者が使用する連続的かつより詳細な情報を提供します。</entry>␞␞        <entry><literal>DEBUG</literal></entry>␞
␝        <entry><literal>INFO</literal></entry>␟        <entry>Provides information implicitly requested by the user,
         e.g., output from <command>VACUUM VERBOSE</command>.</entry>␟        <entry><command>VACUUM VERBOSE</command>の出力などの、ユーザによって暗黙的に要求された情報を提供します。</entry>␞␞        <entry><literal>INFO</literal></entry>␞
␝        <entry><literal>NOTICE</literal></entry>␟        <entry>Provides information that might be helpful to users, e.g.,
         notice of truncation of long identifiers.</entry>␟        <entry>長い識別子の切り詰めに関する注意など、ユーザの補助になる情報を提供します。</entry>␞␞        <entry><literal>NOTICE</literal></entry>␞
␝        <entry><literal>WARNING</literal></entry>␟        <entry>Provides warnings of likely problems, e.g., <command>COMMIT</command>
         outside a transaction block.</entry>␟        <entry>トランザクションブロック外での<command>COMMIT</command>の様な、ユーザへの警告を提供します。</entry>␞␞        <entry><literal>NOTICE</literal></entry>␞
␝        <entry><literal>ERROR</literal></entry>␟        <entry>Reports an error that caused the current command to
         abort.</entry>␟        <entry>現在のコマンドを中断させる原因となったエラーを報告します。</entry>␞␞        <entry><literal>WARNING</literal></entry>␞
␝        <entry><literal>LOG</literal></entry>␟        <entry>Reports information of interest to administrators, e.g.,
         checkpoint activity.</entry>␟        <entry>チェックポイントの活動の様な、管理者に関心のある情報を報告します。</entry>␞␞        <entry><literal>INFO</literal></entry>␞
␝        <entry><literal>FATAL</literal></entry>␟        <entry>Reports an error that caused the current session to
         abort.</entry>␟        <entry>現在のセッションを中断させる原因となったエラーを報告します。</entry>␞␞        <entry><literal>ERR</literal></entry>␞
␝        <entry><literal>PANIC</literal></entry>␟        <entry>Reports an error that caused all database sessions to abort.</entry>␟        <entry>全てのデータベースセッションを中断させる原因となったエラーを報告します。</entry>␞␞        <entry><literal>CRIT</literal></entry>␞
␝     <sect2 id="runtime-config-logging-what">␟     <title>What to Log</title>␟     <title>なにをログに出力するか</title>␞␞␞
␝      <indexterm>
...
␝      <para>␟       What you choose to log can have security implications;  see
       <xref linkend="logfile-maintenance"/>.␟ログに出力する内容は、セキュリティに影響を与える可能性があります。
<xref linkend="logfile-maintenance"/>を参照してください。␞␞      </para>␞
␝      <indexterm>
...
␝       <para>␟        The <varname>application_name</varname> can be any string of less than
        ...
        linkend="sql-syntax-strings-escape">C-style hexadecimal escapes</link>.␟<varname>application_name</varname>は<symbol>NAMEDATALEN</symbol>文字未満の任意の文字列（標準ビルドでは64文字）にすることができます。
...
他の文字は<link linkend="sql-syntax-strings-escape">Cスタイルの16進エスケープ文字</link>に置き換えられます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        These parameters enable various debugging output to be emitted.
        ...
        These parameters are off by default.␟これらのパラメータは生成される各種デバッグ出力を有効にします。
...
␝       <para>␟        When set, <varname>debug_pretty_print</varname> indents the messages
        ...
        it is off.  It is on by default.␟設定された場合、<varname>debug_pretty_print</varname>は<varname>debug_print_parse</varname>、<varname>debug_print_rewritten</varname>、または<varname>debug_print_plan</varname>で生成されたメッセージを字下げします。
設定されない場合の<quote>コンパクト</quote>形式よりもより見やすく、しかしより長いものとなります。デフォルトは有効です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Causes each action executed by autovacuum to be logged if it ran for at
        ...
        individual tables by changing table storage parameters.␟少なくとも指定時間実行した場合、autovacuumで実行される各活動がログに残るようになります。
...
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
ただし、この設定はテーブルストレージパラメータの変更により、それぞれのテーブルに対して上書きすることができます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Causes checkpoints and restartpoints to be logged in the server log.
        Some statistics are included in the log messages, including the number
        of buffers written and the time spent writing them.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line. The default is on.␟チェックポイントおよびリスタートポイントをサーバログに記録するようにします。
書き出されたバッファ数や書き出しに要した時間など、いくつかの統計情報がこのログメッセージに含まれます。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
デフォルトはonです。␞␞       </para>␞
␝       <para>␟        Causes each attempted connection to the server to be logged,
        as well as successful completion of both client authentication (if
        necessary) and authorization.
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this parameter at session start,
        and it cannot be changed at all within a session.
        The default is <literal>off</literal>.␟サーバへの接続試行とともに、クライアント認証の成功終了と（必要ならば）承認をログに残します。
...
␝        <para>␟         Some client programs, like <application>psql</application>, attempt
         to connect twice while determining if a password is required, so
         duplicate <quote>connection received</quote> messages do not
         necessarily indicate a problem.␟<application>psql</application>などクライアントプログラムは、パスワードが要求されているかどうか確認するまで2回接続を試みるので、二重の<quote>connection received</quote>メッセージは必ずしも問題を示すものではありません。␞␞        </para>␞
␝      <indexterm>
...
␝       <para>␟        Causes session terminations to be logged.  The log output
        provides information similar to <varname>log_connections</varname>,
        plus the duration of the session.
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this parameter at session start,
        and it cannot be changed at all within a session.
        The default is <literal>off</literal>.␟セッションの終了をログします。
ログ出力の情報は<varname>log_connections</varname>と同様で、更にセッションの経過時間が追加されます。
スーパーユーザと、適切な<literal>SET</literal>権限を持つユーザだけがセッション開始時にこのパラメータを変更でき、セッションが開始された後は変更できません。
デフォルトは<literal>off</literal>です。␞␞       </para>␞
␝       <para>␟        Causes the duration of every completed statement to be logged.
        The default is <literal>off</literal>.
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟すべての完了した文について、その経過時間をログするようにします。
デフォルトは<literal>off</literal>です。
スーパーユーザと、適切な<literal>SET</literal>権限を持つユーザのみがこの設定を変更することができます。␞␞       </para>␞
␝       <para>␟        For clients using extended query protocol, durations of the Parse,
        Bind, and Execute steps are logged independently.␟拡張問い合わせプロトコルを使用するクライアントでは、Parse、Bind、Executeそれぞれの段階で要した時間が独立して記録されます。␞␞       </para>␞
...
␝        <para>␟         The difference between enabling <varname>log_duration</varname> and setting
         ...
         gathering statistics in high-load installations.␟<varname>log_duration</varname>を有効にするのと<xref linkend="guc-log-min-duration-statement"/>を0に設定する方法との違いは、<varname>log_min_duration_statement</varname>を超えた場合、テキスト版の問い合わせが強制的に出力されるのに対して、このオプションでは出力されないという点です。
...
この動作は、高負荷なインストレーションで統計情報を収集する際に有用です。␞␞        </para>␞
␝      <indexterm>
...
␝       <para>␟        Controls the amount of detail written in the server log for each
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟ログ出力されるそれぞれのメッセージに対し、サーバログに書き込まれる詳細の量を制御します。
...
␝       <para>␟        By default, connection log messages only show the IP address of the
        connecting host. Turning this parameter on causes logging of the
        host name as well.  Note that depending on your host name resolution
        setup this might impose a non-negligible performance penalty.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟デフォルトでは、接続ログメッセージは接続元ホストのIPアドレスのみを表示します。
このパラメータを有効にすると、ホスト名もログに残るようになります。
ホスト名解決方法の設定に依存しますが、これが無視できないほどの性能劣化を起こす可能性があることに注意してください。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟         This is a <function>printf</function>-style string that is output at the
         ...
         readability in log files.␟これは、各ログ行の先頭に出力する<function>printf</function>の書式文字列です。
...
ログファイルではパディングは人間の視認性を向上させるので有用です。␞␞       </para>␞
␝       <para>␟         This parameter can only be set in the <filename>postgresql.conf</filename>
         file or on the server command line. The default is
         <literal>'%m [%p] '</literal> which logs a time stamp and the process ID.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
デフォルトは、タイムスタンプとプロセスIDをログ出力する<literal>'%m [%p] '</literal>です。␞␞       </para>␞
␝            <row>␟             <entry>Escape</entry>␟             <entry>エスケープ</entry>␞␞␞
␝             <entry>Escape</entry>␟             <entry>Effect</entry>␟             <entry>効果</entry>␞␞␞
␝             <entry>Effect</entry>␟             <entry>Session only</entry>␟             <entry>セッションのみ</entry>␞␞             </row>␞
␝             <entry><literal>%a</literal></entry>␟             <entry>Application name</entry>␟             <entry>アプリケーション名</entry>␞␞␞
␝             <entry>Application name</entry>␟             <entry>yes</entry>␟             <entry>○</entry>␞␞            </row>␞
␝             <entry><literal>%u</literal></entry>␟             <entry>User name</entry>␟             <entry>ユーザ名</entry>␞␞␞
␝             <entry>User name</entry>␟             <entry>yes</entry>␟             <entry>○</entry>␞␞            </row>␞
␝             <entry><literal>%d</literal></entry>␟             <entry>Database name</entry>␟             <entry>データベース名</entry>␞␞␞
␝             <entry>Database name</entry>␟             <entry>yes</entry>␟             <entry>○</entry>␞␞            </row>␞
␝             <entry><literal>%r</literal></entry>␟             <entry>Remote host name or IP address, and remote port</entry>␟             <entry>遠隔ホスト名、またはIPアドレス、およびポート番号</entry>␞␞␞
␝             <entry>Remote host name or IP address, and remote port</entry>␟             <entry>yes</entry>␟             <entry>○</entry>␞␞            </row>␞
␝             <entry><literal>%h</literal></entry>␟             <entry>Remote host name or IP address</entry>␟             <entry>遠隔ホスト名、またはIPアドレス</entry>␞␞␞
␝             <entry>Remote host name or IP address</entry>␟             <entry>yes</entry>␟             <entry>○</entry>␞␞            </row>␞
␝             <entry><literal>%b</literal></entry>␟             <entry>Backend type</entry>␟             <entry>バックエンドタイプ</entry>␞␞␞
␝             <entry>Backend type</entry>␟             <entry>no</entry>␟             <entry>×</entry>␞␞            </row>␞
␝             <entry><literal>%p</literal></entry>␟             <entry>Process ID</entry>␟             <entry>プロセス識別子</entry>␞␞␞
␝             <entry>Process ID</entry>␟             <entry>no</entry>␟             <entry>×</entry>␞␞            </row>␞
␝             <entry><literal>%P</literal></entry>␟             <entry>Process ID of the parallel group leader, if this process
              is a parallel query worker</entry>␟             <entry>このプロセスがパラレルクエリワーカーである場合に、パラレルグループリーダーのプロセス識別子</entry>␞␞␞
␝              is a parallel query worker</entry>␟             <entry>no</entry>␟             <entry>×</entry>␞␞            </row>␞
␝             <entry><literal>%t</literal></entry>␟             <entry>Time stamp without milliseconds</entry>␟             <entry>ミリ秒無しのタイムスタンプ</entry>␞␞␞
␝             <entry>Time stamp without milliseconds</entry>␟             <entry>no</entry>␟             <entry>×</entry>␞␞            </row>␞
␝             <entry><literal>%m</literal></entry>␟             <entry>Time stamp with milliseconds</entry>␟             <entry>ミリ秒付きタイムスタンプ</entry>␞␞␞
␝             <entry>Time stamp with milliseconds</entry>␟             <entry>no</entry>␟             <entry>×</entry>␞␞            </row>␞
␝             <entry><literal>%n</literal></entry>␟             <entry>Time stamp with milliseconds (as a Unix epoch)</entry>␟             <entry>ミリ秒付きタイムスタンプ（Unixエポックとして）</entry>␞␞␞
␝             <entry>Time stamp with milliseconds (as a Unix epoch)</entry>␟             <entry>no</entry>␟             <entry>×</entry>␞␞            </row>␞
␝             <entry><literal>%i</literal></entry>␟             <entry>Command tag: type of session's current command</entry>␟             <entry>コマンドタグ。セッションの現在のコマンド種類</entry>␞␞␞
␝             <entry>Command tag: type of session's current command</entry>␟             <entry>yes</entry>␟             <entry>○</entry>␞␞            </row>␞
␝             <entry><literal>%e</literal></entry>␟             <entry>SQLSTATE error code</entry>␟             <entry>SQLSTATE エラーコード</entry>␞␞␞
␝             <entry>SQLSTATE error code</entry>␟             <entry>no</entry>␟             <entry>×</entry>␞␞            </row>␞
␝             <entry><literal>%c</literal></entry>␟             <entry>Session ID: see below</entry>␟             <entry>セッションID。下記参照</entry>␞␞␞
␝             <entry>Session ID: see below</entry>␟             <entry>no</entry>␟             <entry>×</entry>␞␞            </row>␞
␝             <entry><literal>%l</literal></entry>␟             <entry>Number of the log line for each session or process, starting at 1</entry>␟             <entry>各セッションまたは各プロセスのログ行の番号。1から始まります。</entry>␞␞␞
␝             <entry>Number of the log line for each session or process, starting at 1</entry>␟             <entry>no</entry>␟             <entry>×</entry>␞␞            </row>␞
␝             <entry><literal>%s</literal></entry>␟             <entry>Process start time stamp</entry>␟             <entry>プロセスの開始タイムスタンプ</entry>␞␞␞
␝             <entry>Process start time stamp</entry>␟             <entry>no</entry>␟             <entry>×</entry>␞␞            </row>␞
␝             <entry><literal>%v</literal></entry>␟             <entry>Virtual transaction ID (procNumber/localXID);  see
             <xref linkend="transaction-id"/></entry>␟             <entry>仮想トランザクションID(procNumber/localXID)。
             <xref linkend="transaction-id"/>を参照してください</entry>␞␞␞
␝             <xref linkend="transaction-id"/></entry>␟             <entry>no</entry>␟             <entry>×</entry>␞␞            </row>␞
␝             <entry><literal>%x</literal></entry>␟             <entry>Transaction ID (0 if none is assigned);  see
             <xref linkend="transaction-id"/></entry>␟             <entry>トランザクションID（何もアサインされていない場合0）。
             <xref linkend="transaction-id"/>参照</entry>␞␞␞
␝             <xref linkend="transaction-id"/></entry>␟             <entry>no</entry>␟             <entry>×</entry>␞␞            </row>␞
␝             <entry><literal>%q</literal></entry>␟             <entry>Produces no output, but tells non-session
             processes to stop at this point in the string; ignored by
             session processes</entry>␟             <entry>何も出力しません。
             非セッションプロセスではこのエスケープ以降の出力を停止します。
             セッションプロセスでは無視されます。</entry>␞␞␞
␝             session processes</entry>␟             <entry>no</entry>␟             <entry>×</entry>␞␞            </row>␞
␝             <entry><literal>%Q</literal></entry>␟             <entry>Query identifier of the current query.  Query
             ...
             query identifiers is configured.</entry>␟             <entry>現在の問い合わせの問い合わせ識別子。問い合わせ識別子はデフォルトでは計算されません。
             ですから<xref linkend="guc-compute-query-id"/>パラメータが有効であるか、あるいは問い合わせ識別子を計算するサードパーティのモジュールが設定されていないければゼロとなります。</entry>␞␞␞
␝             query identifiers is configured.</entry>␟             <entry>yes</entry>␟             <entry>○</entry>␞␞            </row>␞
␝             <entry><literal>%%</literal></entry>␟             <entry>Literal <literal>%</literal></entry>␟             <entry><literal>%</literal>文字そのもの</entry>␞␞␞
␝             <entry>Literal <literal>%</literal></entry>␟             <entry>no</entry>␟             <entry>×</entry>␞␞            </row>␞
␝         <para>␟          The backend type corresponds to the column
          <structfield>backend_type</structfield> in the view
          <link linkend="monitoring-pg-stat-activity-view">
          <structname>pg_stat_activity</structname></link>,
          but additional types can appear
          in the log that don't show in that view.␟バックエンドタイプは<link linkend="monitoring-pg-stat-activity-view"><structname>pg_stat_activity</structname></link>ビューの<structfield>backend_type</structfield>列に関連します。しかし、ビューにない他のタイプがログに現れることがあります。␞␞         </para>␞
␝         <para>␟         The <literal>%c</literal> escape prints a quasi-unique session identifier,
         consisting of two 4-byte hexadecimal numbers (without leading zeros)
         separated by a dot.  The numbers are the process start time and the
         process ID, so <literal>%c</literal> can also be used as a space saving way
         of printing those items.  For example, to generate the session
         identifier from <literal>pg_stat_activity</literal>, use this query:␟<literal>%c</literal>エスケープは、2つの4バイトの16進数（先頭のゼロは省略）をドットで区切った構成の、準一意なセッション識別子を表示します。
この数値はプロセスの起動時間とそのプロセスIDです。
したがって、<literal>%c</literal>を使用して、これらの項目を出力するための文字数を省略することができます。例として、<literal>pg_stat_activity</literal>からセッション識別子を生成するには以下の問い合わせを行ないます。␞␞<programlisting>␞
␝        <para>␟         If you set a nonempty value for <varname>log_line_prefix</varname>,
         ...
         character can be used too.␟<varname>log_line_prefix</varname>に空白文字以外の値を設定する場合、通常、ログ行の残りとの区切りを明確にするために、その最後の文字を空白文字にすべきです。
句読点用の文字も使用できます。␞␞        </para>␞
␝        <para>␟         <application>Syslog</application> produces its own
         time stamp and process ID information, so you probably do not want to
         include those escapes if you are logging to <application>syslog</application>.␟<application>Syslog</application>は独自にタイムスタンプとプロセスID情報を生成します。
ですのでおそらく、<application>Syslog</application>にログを保管する場合は、こうしたエスケープを含めるとは考えないでしょう。␞␞        </para>␞
␝        <para>␟         The <literal>%q</literal> escape is useful when including information that is
         only available in session (backend) context like user or database
         name.  For example:␟<literal>%q</literal>エスケープは、ユーザやデータベース名のように、セッション（バックエンド）コンテキストでのみ存在する情報を含める場合に有用です。
<literal>%q</literal>␞␞<programlisting>␞
␝      <indexterm>
...
␝        <para>␟         The <literal>%Q</literal> escape always reports a zero identifier
         ...
         which an identifier cannot be calculated.␟識別子が計算できない不正な文も含め、<varname>log_statement</varname>は識別子が計算可能になる前に出力を生成するため、<literal>%Q</literal>エスケープは、<xref linkend="guc-log-statement"/>が生成する行では常にゼロの識別子を報告します。␞␞        </para>␞
␝      <indexterm>
...
␝       <para>␟        Controls whether a log message is produced when a session waits
        longer than <xref linkend="guc-deadlock-timeout"/> to acquire a
        lock.  This is useful in determining if lock waits are causing
        poor performance.  The default is <literal>off</literal>.
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟セッションがロックの獲得までの間に<xref linkend="guc-deadlock-timeout"/>より長く待機する場合にログメッセージを生成するかどうかを制御します。
これは、ロック待ちによって性能がでていないのかどうか確認する時に有用です。
デフォルトは<literal>off</literal>です。
スーパーユーザと、適切な<literal>SET</literal>権限を持つユーザのみがこの設定を変更することができます。␞␞       </para>␞
␝       <para>␟        Controls whether a log message is produced when the startup process
        waits longer than <varname>deadlock_timeout</varname>
        for recovery conflicts.  This is useful in determining if recovery
        conflicts prevent the recovery from applying WAL.␟スタートアッププロセスが<varname>deadlock_timeout</varname>よりも長くリカバリコンフリクトを待つ場合にログメッセージを出力するかどうかを制御します。
リカバリコンフリクトによってリカバリがWALを適用するのを妨げられているかどうかを決定するのに有用です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The default is <literal>off</literal>.  This parameter can only be set
        in the <filename>postgresql.conf</filename> file or on the server
        command line.␟デフォルトは<literal>off</literal>です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        If greater than zero, each bind parameter value logged with a
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟ゼロよりも大きければ、エラーではない時にバインドパラメータ値が、文とともにこの指定バイト数に短縮されて記録されます。
...
␝       <para>␟        This setting only affects log messages printed as a result of
        <xref linkend="guc-log-statement"/>,
        ...
        are sent in binary form, since then conversion to text is required.␟この設定は、<xref linkend="guc-log-statement"/>、<xref linkend="guc-log-duration"/>および関連設定の結果を表示するログメッセージにのみ影響します。
ゼロ以外の値の設定は、とりわけパラメータがバイナリ形式で送信される際に多少のオーバーヘッドをもたらします。
テキストへの変換が必要になるからです。␞␞       </para>␞
␝       <para>␟        If greater than zero, each bind parameter value reported in error
        ...
        If this value is specified without units, it is taken as bytes.␟ゼロよりも大きければ、エラー時のバインドパラメータ値が、エラーメッセージ中にこの指定バイト数に短縮されて記録されます。
ゼロ（デフォルトです）ならエラーメッセージ中のバインドパラメータは記録されません。
<literal>-1</literal>ならバインドパラメータはすべて記録されます。
この値が単位なしに指定されると、バイト単位と見なされます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Non-zero values of this setting add overhead, as
        ...
        conversion while the latter only requires copying the string.␟ゼロ以外の値の設定は多少のオーバーヘッドをもたらします。
...
前者はデータ変換が必要なのに対し、後者は単に文字列をコピーするだけだからです。␞␞       </para>␞
␝       <para>␟        Controls which SQL statements are logged. Valid values are
        ...
        (with any embedded single-quote marks doubled).␟どのSQL文をログに記録するかを制御します。
...
拡張問い合わせプロトコルを使用するクライアントでは、Executeメッセージを受け取った時にBindパラメータの値が（すべての単一引用符が二重にされた状態で）含まれていた場合、ログに記録されます。␞␞       </para>␞
␝       <para>␟        The default is <literal>none</literal>.
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟デフォルトは<literal>none</literal>です。
スーパーユーザおよび適切な<literal>SET</literal>権限を持つユーザのみがこの設定を変更できます。␞␞       </para>␞
␝        <para>␟         Statements that contain simple syntax errors are not logged
         ...
         <literal>ERROR</literal> (or lower) to log such statements.␟ログメッセージの発行は、基本解析により文の種類が決まった後に行われますので、<varname>log_statement</varname> = <literal>all</literal>という設定を行ったとしても、単純な構文エラーを持つ文は記録されません。
拡張問い合わせプロトコルの場合も同様に、この設定ではExecute段階以前（つまり、解析や計画作成期間）に失敗した文は記録されません。
こうした文のログを記録するには、<varname>log_min_error_statement</varname>を<literal>ERROR</literal>（以下）に設定してください。␞␞        </para>␞
␝      <indexterm>
...
␝        <para>␟         Logged statements might reveal sensitive data and even contain
         plaintext passwords.␟ログに記録された文は、機密データを明らかにし、プレーンテキストのパスワードを含むことさえあります。␞␞        </para>␞
␝      <indexterm>
...
␝       <para>␟        Causes each replication command and <literal>walsender</literal>
        ...
        <literal>SET</literal> privilege can change this setting.␟サーバログに各レプリケーションコマンドと、<literal>walsender</literal>プロセスのレプリケーションスロットの獲得／解放を記録します。
...
␝       <para>␟        Controls logging of temporary file names and sizes.
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟一時ファイルのファイル名とサイズのログ出力を制御します。
...
この設定を変更できるのは、スーパーユーザと適切な<literal>SET</literal>権限を持つユーザだけです。␞␞       </para>␞
␝       <para>␟        Sets the time zone used for timestamps written in the server log.
        ...
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟サーバログに書き出す際に使用される時間帯を設定します。
...
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝     <sect2 id="runtime-config-logging-csvlog">␟     <title>Using CSV-Format Log Output</title>␟     <title>CSV書式のログ出力の利用</title>␞␞␞
␝       <para>␟        Including <literal>csvlog</literal> in the <varname>log_destination</varname> list
        ...
        and query id.
        Here is a sample table definition for storing CSV-format log output:␟<varname>log_destination</varname>リストに<literal>csvlog</literal>を含めることは、ログファイルをデータベーステーブルにインポートする簡便な方法を提供します。このオプションはカンマ区切り値書式（<acronym>CSV</acronym>）で以下の列を含むログ行を生成します。
ミリ秒単位のtimestamp、
ユーザ名、
データベース名、
プロセスID、
...
␝       <para>␟        To import a log file into this table, use the <command>COPY FROM</command>
        command:␟このテーブルにログファイルをインポートするためには、<command>COPY FROM</command>コマンドを使用してください。␞␞␞
␝</programlisting>␟        It is also possible to access the file as a foreign table, using
        the supplied <xref linkend="file-fdw"/> module.␟提供された<xref linkend="file-fdw"/>モジュールを使って外部テーブルとしてファイルをアクセスすることも可能です。␞␞       </para>␞
␝       <para>␟       There are a few things you need to do to simplify importing CSV log
       files:␟CSVログファイルをインポートする作業を単純にするためにいくつか必要な作業があります。␞␞␞
␝           <para>␟            Set <varname>log_filename</varname> and
            ...
            file is complete and therefore ready to be imported.␟一貫性があり、予測可能なログファイル命名機構を提供するために、<varname>log_filename</varname>および<varname>log_rotation_age</varname>を設定してください。
これによりどのようなファイル名になると、個々のログファイルが完了しインポートする準備が整ったかが推測できるようになります。␞␞         </para>␞
␝           <para>␟            Set <varname>log_rotation_size</varname> to 0 to disable
            size-based log rotation, as it makes the log file name difficult
            to predict.␟ログファイル名の予測が困難になりますので、<varname>log_rotation_size</varname>を0にして容量を基にしたログの回転を無効にしてください。␞␞           </para>␞
␝          <para>␟           Set <varname>log_truncate_on_rotation</varname> to <literal>on</literal> so
           that old log data isn't mixed with the new in the same file.␟同じファイルに古いログデータと新しいログデータが混在しないようにするために、<varname>log_truncate_on_rotation</varname>を<literal>on</literal>に設定してください。␞␞          </para>␞
␝          <para>␟           The table definition above includes a primary key specification.
           ...
           written, which would also cause <command>COPY</command> to fail.␟上のテーブル定義には主キーの指定が含まれています。
...
この手順は、<command>COPY</command>が失敗する原因となる、完全に書き込まれなかった欠落した行をインポートするという事故も防止します。␞␞          </para>␞
␝     <sect2 id="runtime-config-logging-jsonlog">␟     <title>Using JSON-Format Log Output</title>␟     <title>JSON書式のログ出力の利用</title>␞␞␞
␝     <para>␟      Including <literal>jsonlog</literal> in the
      ...
      lines in <acronym>JSON</acronym> format.␟<varname>log_destination</varname>リストに<literal>jsonlog</literal>を含めると、ログファイルをさまざまなプログラムにインポートするのに便利です。
このオプションは、<acronym>JSON</acronym>書式のログ行を出力します。␞␞     </para>␞
␝     <para>␟      String fields with null values are excluded from output.
      Additional fields may be added in the future. User applications that
      process <literal>jsonlog</literal> output should ignore unknown fields.␟NULL値を持つ文字列フィールドは出力から除外されます。
将来、フィールドが追加される可能性があります。
<literal>jsonlog</literal>出力を処理するユーザアプリケーションは不明なフィールドを無視する必要があります。␞␞     </para>␞
␝     <para>␟      Each log line is serialized as a JSON object with the set of keys and
      their associated values shown in <xref
      linkend="runtime-config-logging-jsonlog-keys-values"/>.␟各ログ行はJSONオブジェクトとしてシリアライズされ、一連のキーとそれに関連する値が<xref linkend="runtime-config-logging-jsonlog-keys-values"/>で示されます。␞␞     </para>␞
␝     <table id="runtime-config-logging-jsonlog-keys-values">␟      <title>Keys and Values of JSON Log Entries</title>␟      <title>JSONログ項目のキーと値</title>␞␞      <tgroup cols="3">␞
␝        <row>␟         <entry>Key name</entry>␟         <entry>キー名</entry>␞␞␞
␝         <entry>Key name</entry>␟         <entry>Type</entry>␟         <entry>型</entry>␞␞␞
␝         <entry>Type</entry>␟         <entry>Description</entry>␟         <entry>説明</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Time stamp with milliseconds</entry>␟         <entry>ミリ秒付きのタイムスタンプ</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>User name</entry>␟         <entry>ユーザ名</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Database name</entry>␟         <entry>データベース名</entry>␞␞        </row>␞
␝         <entry>number</entry>␟         <entry>Process ID</entry>␟         <entry>プロセスID</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Client host</entry>␟         <entry>クライアントホスト</entry>␞␞        </row>␞
␝         <entry>number</entry>␟         <entry>Client port</entry>␟         <entry>クライアントポート</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Session ID</entry>␟         <entry>セッションID</entry>␞␞        </row>␞
␝         <entry>number</entry>␟         <entry>Per-session line number</entry>␟         <entry>セッション単位の行番号</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Current ps display</entry>␟         <entry>現在のps表示</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Session start time</entry>␟         <entry>セッション開始時刻</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Virtual transaction ID</entry>␟         <entry>仮想トランザクションID</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Regular transaction ID</entry>␟         <entry>通常トランザクションID</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Error severity</entry>␟         <entry>エラー深刻度</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>SQLSTATE code</entry>␟         <entry>SQLSTATEコード</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Error message</entry>␟         <entry>エラーメッセージ</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Error message detail</entry>␟         <entry>エラーメッセージ詳細</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Error message hint</entry>␟         <entry>エラーメッセージヒント</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Internal query that led to the error</entry>␟         <entry>このエラーの原因となった内部問い合わせ</entry>␞␞        </row>␞
␝         <entry>number</entry>␟         <entry>Cursor index into internal query</entry>␟         <entry>内部問い合わせへのカーソルインデックス</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Error context</entry>␟         <entry>エラーコンテキスト</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Client-supplied query string</entry>␟         <entry>クライアント提供の問い合わせ文字列</entry>␞␞        </row>␞
␝         <entry>number</entry>␟         <entry>Cursor index into query string</entry>␟         <entry>問い合わせ文字列へのカーソルインデックス</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Error location function name</entry>␟         <entry>エラー位置関数名</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>File name of error location</entry>␟         <entry>エラー位置のファイル名</entry>␞␞        </row>␞
␝         <entry>number</entry>␟         <entry>File line number of the error location</entry>␟         <entry>エラー位置のファイル行番号</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Client application name</entry>␟         <entry>クライアントアプリケーション名</entry>␞␞        </row>␞
␝         <entry>string</entry>␟         <entry>Type of backend</entry>␟         <entry>バックエンドの種類</entry>␞␞        </row>␞
␝         <entry>number</entry>␟         <entry>Process ID of leader for active parallel workers</entry>␟         <entry>活動中のパラレルワーカーのリーダーのプロセスID</entry>␞␞        </row>␞
␝         <entry>number</entry>␟         <entry>Query ID</entry>␟         <entry>問い合わせ識別子</entry>␞␞        </row>␞
␝   <sect2 id="runtime-config-logging-proc-title">␟    <title>Process Title</title>␟    <title>プロセスのタイトル</title>␞␞␞
␝      <indexterm>
...
␝    <para>␟     These settings control how process titles of server processes are
     ...
     See <xref linkend="monitoring-ps"/> for details.␟これらの設定項目は、サーバプロセスのタイトルを変更します。
プロセスのタイトルは、典型的には<application>ps</application>、あるいはWindowsにおいては<application>Process Explorer</application>で見ることができます。
詳細については、<xref linkend="monitoring-ps"/>を参照してください。␞␞    </para>␞
␝       <para>␟        Sets a name that identifies this database cluster (instance) for
        ...
        linkend="guc-synchronous-standby-names"/>).␟様々な目的のために、このデータベースクラスタ（インスタンス）を識別する名前を設定します。
クラスタ名はこのクラスタのすべてのサーバプロセスのプロセスタイトルに現れます。
更に、スタンバイ接続のデフォルトのアプリケーション名となります（<xref linkend="guc-synchronous-standby-names"/>参照）。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The name can be any string of less
        ...
        This parameter can only be set at server start.␟名前は<symbol>NAMEDATALEN</symbol>文字（標準のビルドでは64文字）より少ない文字列です。
...
␝       <para>␟        Enables updating of the process title every time a new SQL command
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟サーバが新しいSQLコマンドを受け取る時に毎回、プロセスタイトルを更新できるようにします。
...
スーパーユーザと、適切な<literal>SET</literal>権限を持つユーザのみがこの設定を変更することができます。␞␞       </para>␞
␝   <sect1 id="runtime-config-statistics">␟    <title>Run-time Statistics</title>␟    <title>実行時統計情報</title>␞␞␞
␝    <sect2 id="runtime-config-cumulative-statistics">␟     <title>Cumulative Query and Index Statistics</title>␟     <title>累積的な問い合わせ及びインデックスの統計情報</title>␞␞␞
␝      <indexterm>
...
␝     <para>␟      These parameters control the server-wide cumulative statistics system.
...
␝       <para>␟        Enables the collection of information on the currently
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟各セッションで実行中のコマンドに関する情報と、そのコマンドの識別子および実行開始時刻の収集を有効にします。
このパラメータはデフォルトで有効です。
...
␝       <para>␟       Specifies the amount of memory reserved to store the text of the
       ...
       This parameter can only be set at server start.␟<structname>pg_stat_activity</structname>.<structfield>query</structfield>フィールドに対し、それぞれの活動中のセッションで現在実行されているコマンドを追跡記録するため予約されるメモリ量を指定します。
...
␝       <para>␟        Enables collection of statistics on database activity.
        This parameter is on by default, because the autovacuum
        daemon needs the collected information.
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟データベースの活動についての統計情報の収集を有効にします。
...
␝       <para>␟        Enables timing of database I/O calls.  This parameter is off by
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟データベースによるI/O呼び出しの記録を有効にします。
...
␝       <para>␟        Enables timing of WAL I/O calls. This parameter is off by default,
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟データベースによるWAL I/O呼び出しの記録を有効にします。
このパラメータはデフォルトで無効になっています。その理由は、現時点の時刻をオペレーティングシステムに繰り返し問い合わせるので、プラットフォームによっては深刻な負荷の原因になるからです。
...
スーパーユーザと、適切な<literal>SET</literal>権限を持つユーザのみがこの設定を変更することができます。␞␞       </para>␞
␝       <para>␟        Enables tracking of function call counts and time used. Specify
        ...
        statistics tracking.
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟関数の呼び出し数と費やされた時間の追跡を有効にします。
...
␝        <para>␟         SQL-language functions that are simple enough to be <quote>inlined</quote>
         into the calling query will not be tracked, regardless of this
         setting.␟呼び出す問い合わせ内に<quote>インライン化</quote>できる位単純なSQL言語関数は、この設定と関係なく、追跡されません。␞␞        </para>␞
␝       <para>␟        Determines the behavior when cumulative statistics are accessed
        ...
        The default is <literal>cache</literal>.␟トランザクション内で累積的統計に複数回アクセスする場合の動作を決定します。
...
デフォルトは<literal>cache</literal>です。␞␞       </para>␞
␝        <para>␟         <literal>none</literal> is most suitable for monitoring systems. If
         ...
         particularly if many database objects exist.␟<literal>none</literal>は、システムの監視に最も適しています。
値が一度しかアクセスされない場合、最も効率的です。
<literal>cache</literal>は、繰り返しアクセスによって同じ値が生成されることを保証します。
これは、自己結合などを含むクエリで重要です。
<literal>snapshot</literal>は、統計情報を対話的に検査する場合に便利ですが、特に多くのデータベースオブジェクトが存在する場合にはオーバーヘッドが高くなります。␞␞        </para>␞
␝      <indexterm>
...
␝    <sect2 id="runtime-config-statistics-monitor">␟     <title>Statistics Monitoring</title>␟     <title>統計情報の監視</title>␞␞     <variablelist>␞
␝       <para>␟        Enables in-core computation of a query identifier.
        ...
        The default is <literal>auto</literal>.␟組み込みの問い合わせ識別子の計算を有効にします。
...
デフォルトは<literal>auto</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝        <para>␟         To ensure that only one query identifier is calculated and
         displayed, extensions that calculate query identifiers should
         throw an error if a query identifier has already been computed.␟一つの問い合わせ識別子のみが計算されて表示されることを確実にするために、問い合わせ識別子を計算する拡張は、問い合わせ識別子がすでに計算済みのときにエラーを生じるようにすべきです。␞␞        </para>␞
␝       <para>␟        For each query, output performance statistics of the respective
        ...
        default.
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change these settings.␟各問い合わせに対し、対応するモジュールの性能に関する統計情報をサーバログに出力します。
...
␝   <sect1 id="runtime-config-autovacuum">␟    <title>Automatic Vacuuming</title>␟    <title>自動Vacuum作業</title>␞␞␞
␝      <indexterm>
...
␝     <para>␟      These settings control the behavior of the <firstterm>autovacuum</firstterm>
      ...
      basis; see <xref linkend="sql-createtable-storage-parameters"/>.␟以下に示す設定は<firstterm>自動バキューム</firstterm>機能の動作を制御します。
詳細は<xref linkend="autovacuum"/>を参照してください。
これらの設定の多くは、テーブル単位で変更できることに注意してください。
<xref linkend="sql-createtable-storage-parameters"/>を参照してください。␞␞     </para>␞
␝       <para>␟        Controls whether the server should run the
        ...
        disabled for individual tables by changing table storage parameters.␟サーバがautovacuumランチャデーモンを実行すべきかどうかを管理します。
デフォルトでは有効です。
しかしautovacuumを作動させるためには<xref linkend="guc-track-counts"/>も有効でなければなりません。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
ただし、テーブルストレージパラメータを変更することにより、autovacuumは個々のテーブルに対して無効にできます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Note that even when this parameter is disabled, the system
        will launch autovacuum processes if necessary to
        prevent transaction ID wraparound.  See <xref
        linkend="vacuum-for-wraparound"/> for more information.␟このパラメータが無効であったとしてもシステムは、トランザクションIDの周回を防止する必要があれば、autovacuumプロセスを起動することに注意してください。
詳細は<xref linkend="vacuum-for-wraparound"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the maximum number of autovacuum processes (other than the
        autovacuum launcher) that may be running at any one time.  The default
        is three.  This parameter can only be set at server start.␟同時に実行することができるautovacuumプロセス（autovacuumランチャ以外）の最大数を指定します。
デフォルトは3です。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the minimum delay between autovacuum runs on any given
        ...
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟あるデータベースについて実行されるautovacuumデーモンの最小遅延を指定します。
...
␝       <para>␟        Specifies the minimum number of updated or deleted tuples needed
        to trigger a <command>VACUUM</command> in any one table.
        The default is 50 tuples.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line;
        but the setting can be overridden for individual tables by
        changing table storage parameters.␟どのテーブルに対しても<command>VACUUM</command>を起動するために必要な、更新もしくは削除されたタプルの最小数を指定します。
デフォルトは50タプルです。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
ただし、この設定はテーブルストレージパラメータの変更により、それぞれのテーブルに対して上書きすることができます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the number of inserted tuples needed to trigger a
        ...
        file or on the server command line;
        but the setting can be overridden for individual tables by
        changing table storage parameters.␟あるテーブルで<command>VACUUM</command>を起動するきっかけとなるのに必要な挿入タプル数を設定します。
デフォルトは1000タプルです。
...
␝       <para>␟        Specifies the minimum number of inserted, updated or deleted tuples
        needed to trigger an <command>ANALYZE</command> in any one table.
        The default is 50 tuples.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line;
        but the setting can be overridden for individual tables by
        changing table storage parameters.␟どのテーブルに対しても<command>ANALYZE</command>を起動するのに必要な、挿入、更新、もしくは削除されたタプルの最小数を指定します。
デフォルトは50タプルです。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
ただし、この設定はテーブルストレージパラメータの変更により、それぞれのテーブルに対して上書きすることができます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies a fraction of the table size to add to
        ...
        file or on the server command line;
        but the setting can be overridden for individual tables by
        changing table storage parameters.␟<command>VACUUM</command>を起動するか否かを決定するときに、<varname>autovacuum_vacuum_threshold</varname>に足し算するテーブル容量の割合を指定します。
...
␝       <para>␟        Specifies a fraction of the table size to add to
        ...
        file or on the server command line;
        but the setting can be overridden for individual tables by
        changing table storage parameters.␟<command>VACUUM</command>を起動するかどうか決める際の<varname>autovacuum_vacuum_insert_threshold</varname>に追加するテーブルサイズの割合を指定します。
デフォルトは0.2（テーブルサイズの20%）です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
ただし、この設定はテーブルストレージパラメータの変更により、それぞれのテーブルに対して上書きすることができます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies a fraction of the table size to add to
        ...
        file or on the server command line;
        but the setting can be overridden for individual tables by
        changing table storage parameters.␟<command>ANALYZE</command>を起動するか否かを決定するときに、<varname>autovacuum_analyze_threshold</varname>に足し算するテーブル容量の割合を指定します。
デフォルトは0.1（テーブルサイズの10%）です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
ただし、この設定はテーブルストレージパラメータの変更により、それぞれのテーブルに対して上書きすることができます。␞␞       </para>␞
␝       <para>␟        Specifies the maximum age (in transactions) that a table's
        ...
        prevent wraparound even when autovacuum is otherwise disabled.␟トランザクションID周回を防ぐために<command>VACUUM</command>操作が強制される前までにテーブルの<structname>pg_class</structname>.<structfield>relfrozenxid</structfield> フィールドが到達できる最大（トランザクションにおける）年代を指定します。
自動バキュームが無効であった時でも、システムは周回を防ぐために自動バキューム子プロセスを起動することに注意してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Vacuum also allows removal of old files from the
        <filename>pg_xact</filename> subdirectory, which is why the default
        is a relatively low 200 million transactions.
        This parameter can only be set at server start, but the setting
        can be reduced for individual tables by
        changing table storage parameters.
        For more information see <xref linkend="vacuum-for-wraparound"/>.␟vacuumは同時に<filename>pg_xact</filename>サブディレクトリから古いファイルの削除を許可します。
これが、比較的低い2億トランザクションがデフォルトである理由です。
このパラメータはサーバ起動時にのみ設定可能です。
しかし、この設定はテーブルストレージパラメータの変更により、それぞれのテーブルで減らすことができます。
詳細は<xref linkend="vacuum-for-wraparound"/>を参照してください。␞␞       </para>␞
␝       <para>␟        Specifies the maximum age (in multixacts) that a table's
        ...
        Note that the system will launch autovacuum processes to
        prevent wraparound even when autovacuum is otherwise disabled.␟トランザクションID周回を防ぐために<command>VACUUM</command>操作が強制される前までにテーブルの<structname>pg_class</structname>.<structfield>relminmxid</structfield> フィールドが到達できる最大（マルチトランザクションにおける）年代を指定します。
自動バキュームが無効であった時でも、システムは周回を防ぐために自動バキューム子プロセスを起動することに注意してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Vacuuming multixacts also allows removal of old files from the
        ...
        For more information see <xref linkend="vacuum-for-multixact-wraparound"/>.␟またマルチトランザクションIDのvacuumは<filename>pg_multixact</filename>と<filename>pg_multixact/offsets</filename>サブディレクトリから古いファイルの削除します。
これがデフォルトが4億トランザクションをやや下回る理由です。
このパラメータはサーバ起動時にのみ設定可能です。
しかし、この設定はテーブルストレージパラメータの変更により、それぞれのテーブルで減らすことができます。
詳細は<xref linkend="vacuum-for-multixact-wraparound"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the cost delay value that will be used in automatic
        ...
        file or on the server command line;
        but the setting can be overridden for individual tables by
        changing table storage parameters.␟自動<command>VACUUM</command>操作に使用されるコスト遅延値を指定します。
-1に指定されると、一定の <xref linkend="guc-vacuum-cost-delay"/>の値が使用されます。
この値が単位なしで指定された場合は、ミリ秒単位であるとみなします。
デフォルト値は2ミリ秒です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
ただし、この設定はテーブルストレージパラメータの変更により、それぞれのテーブルに対して上書きすることができます。␞␞       </para>␞
␝       <para>␟        Specifies the cost limit value that will be used in automatic
        ...
        file or on the server command line;
        but the setting can be overridden for individual tables by
        changing table storage parameters.␟自動<command>VACUUM</command>操作に使用されるコスト限界値を指定します。
...
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。
ただし、この設定はテーブルストレージパラメータの変更により、それぞれのテーブルに対して上書きすることができます。␞␞       </para>␞
␝   <sect1 id="runtime-config-client">␟    <title>Client Connection Defaults</title>␟    <title>クライアント接続デフォルト</title>␞␞␞
␝      <indexterm>
...
␝    <sect2 id="runtime-config-client-statement">␟     <title>Statement Behavior</title>␟     <title>文の動作</title>␞␞     <variablelist>␞
␝       <para>␟        Controls which
        ...
        rank here than in <xref linkend="guc-log-min-messages"/>.␟どの<link linkend="runtime-config-severity-levels">メッセージレベル</link>をクライアントに送るかを管理します。有効な値は、<literal>DEBUG5</literal>、<literal>DEBUG4</literal>、<literal>DEBUG3</literal>、<literal>DEBUG2</literal>、<literal>DEBUG1</literal>、<literal>LOG</literal>、<literal>NOTICE</literal>、<literal>WARNING</literal>および<literal>ERROR</literal>です。
それぞれのレベルはそれに続く全てのレベルを包含します。
レベルが後の方になるにつれ、より少ないメッセージが送られます。
デフォルトは<literal>NOTICE</literal>です。
ここでの<literal>LOG</literal>の優先順位が<xref linkend="guc-log-min-messages"/>の場合と異なることに注意してください。␞␞       </para>␞
␝      <indexterm>
...
␟␟      <indexterm><primary>パス</primary><secondary>スキーマへの</secondary></indexterm>␞␞       </para>␞
␝       <para>␟        <literal>INFO</literal> level messages are always sent to the client.␟<literal>INFO</literal>レベルのメッセージは常にクライアントに送られます。␞␞       </para>␞
␝       <para>␟        This variable specifies the order in which schemas are searched
        when an object (table, data type, function, etc.) is referenced by a
        simple name with no schema specified.  When there are objects of
        identical names in different schemas, the one found first
        in the search path is used.  An object that is not in any of the
        schemas in the search path can only be referenced by specifying
        its containing schema with a qualified (dotted) name.␟この変数は、オブジェクト（テーブル、データ型、関数など）がスキーマを指定されていない単純な名前で参照されている場合に、スキーマを検索する順番を指定します。
異なるスキーマに同じ名前のオブジェクトがある場合、検索パスで最初に見つかったものが使用されます。
検索パス内のどのスキーマにも存在しないオブジェクトを参照するには、修飾名（ドット付き）でそのオブジェクトが含まれるスキーマを指定する必要があります。␞␞       </para>␞
␝       <para>␟        The value for <varname>search_path</varname> must be a comma-separated
        ...
        permission, is silently ignored.␟<varname>search_path</varname>の値は、スキーマの名前をカンマで区切った一覧でなければなりません。
存在していないスキーマ、またはユーザが<literal>USAGE</literal>権限を所有していないスキーマは警告なしに無視されます。␞␞       </para>␞
␝       <para>␟        If one of the list items is the special name
        ...
        (If not, <literal>$user</literal> is ignored.)␟もしそのようなスキーマが存在し、ユーザがそれにたいして<literal>USAGE</literal>権限を所有している場合、一覧内の項目の1つが特別な名前である<literal>$user</literal>の場合、<function>CURRENT_USER</function>と同じ名前を持つスキーマがあれば、そのスキーマが置換されます。
（このような名前空間がない場合は<literal>$user</literal>は無視されます。）␞␞       </para>␞
...
␟␟       <!-- 細かい話ですが、typnameが'funcname'である場合でも、funcname('foo')は一時スキーマを使いません。
            この段落では、関数名とは「pg_proc.pronameあるいは文法生成物のfunc_name」のどちらでも良いという意味で使用しています。-->␞␞       <para>␞
␝       <para>␟        The system catalog schema, <literal>pg_catalog</literal>, is always
        searched, whether it is mentioned in the path or not.  If it is
        mentioned in the path then it will be searched in the specified
        order.  If <literal>pg_catalog</literal> is not in the path then it will
        be searched <emphasis>before</emphasis> searching any of the path items.␟システムカタログのスキーマである<literal>pg_catalog</literal>は、パスでの指定の有無にかかわらず、常に検索されます。
パスで指定されている場合は、指定された順序で検索されます。
<literal>pg_catalog</literal>がパスに含まれていない場合、パスに含まれる項目を検索する<emphasis>前に</emphasis>検索が行われます␞␞       </para>␞
␝       <para>␟        Likewise, the current session's temporary-table schema,
        ...
        function or operator names.␟同様に、現在のセッションの一時テーブルスキーマ<literal>pg_temp_<replaceable>nnn</replaceable></literal>も、存在すれば常に検索されます。
...
関数や演算子名に対してはまったく検索されません。␞␞       </para>␞
␝       <para>␟        When objects are created without specifying a particular target
        ...
        path is empty.␟対象となる特定のスキーマを指定せずにオブジェクトが作成された場合、それらのオブジェクトは<varname>search_path</varname>で名前を付けられた最初に有効となっているスキーマに配置されます。
検索パスが空の場合、エラーが報告されます。␞␞       </para>␞
␝       <para>␟        The default value for this parameter is
        ...
        setting, either globally or per-user.␟このパラメータのデフォルト値は<literal>"$user", public</literal>です。
この設定はデータベースの共有（どのユーザも非公開のスキーマを持たず、全員が<literal>public</literal>を共有）、ユーザごとの非公開のスキーマ、およびこれらの組み合わせがサポートします。
デフォルトの検索パスの設定を全体的またはユーザごとに変更することで、その他の効果を得ることもできます。␞␞       </para>␞
␝       <para>␟        For more information on schema handling, see
        <xref linkend="ddl-schemas"/>.  In particular, the default
        configuration is suitable only when the database has a single user or
        a few mutually-trusting users.␟スキーマの扱いについての詳細は、<xref linkend="ddl-schemas"/>をご覧ください。
とりわけ、デフォルトの設定はデータベースのユーザが、一人あるいはお互いに信頼できる少数のユーザだけである場合にのみ適切です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The current effective value of the search path can be examined
        ...
        appearing in <varname>search_path</varname> were resolved.␟<acronym>SQL</acronym>関数の<function>current_schemas</function> によって、検索パスの現在の有効な値を調べることができます（<xref linkend="functions-info"/>を参照してください）。
これは、<varname>search_path</varname> の値を調べるのとは異なります。
<function>current_schemas</function>は、<varname>search_path</varname>に現れる項目がどのように解決されたかを表すからです。␞␞       </para>␞
␝       <para>␟        This variable controls whether to raise an error in lieu of applying a
        ...
        the <literal>BYPASSRLS</literal> attribute.␟この設定値は、行セキュリティポリシーの適用によってエラーを生じさせるかどうかを制御します。
...
それはすなわち、<literal>BYPASSRLS</literal>アトリビュートを持つスーパーユーザです。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        For more information on row security policies,
        see <xref linkend="sql-createpolicy"/>.␟行セキュリティポリシーについての更なる情報は<xref linkend="sql-createpolicy"/>をご覧ください。␞␞       </para>␞
␝      <indexterm>
...
␟␟      <indexterm><primary>テーブル空間</primary><secondary>デフォルト</secondary></indexterm>␞␞       </para>␞
␝       <para>␟        This parameter specifies the default table access method to use when
        ...
        specifying a table access method. The default is <literal>heap</literal>.␟このパラメータは、テーブルあるいはマテリアライズドビューの作成時に<command>CREATE</command>コマンドが明示的にアクセスメソッドを指定しないか、あるいはテーブルアクセスメソッドを指定できない<command>SELECT ... INTO</command>が利用された時に使われるデフォルトのテーブルアクセスメソッドを指定します。
デフォルトは<literal>heap</literal>です。␞␞       </para>␞
␝       <para>␟        This variable specifies the default tablespace in which to create
        objects (tables and indexes) when a <command>CREATE</command> command does
        not explicitly specify a tablespace.␟この変数は、<command>CREATE</command>コマンドで明示的にテーブル空間を指定していない場合にオブジェクト（テーブルとインデックス）の作成先となるデフォルトのテーブル空間を指定します。␞␞       </para>␞
␝       <para>␟        The value is either the name of a tablespace, or an empty string
        ...
        for it, or creation attempts will fail.␟値はテーブル空間名、もしくは現在のデータベースのデフォルトのテーブル空間を使用することを意味する空文字列です。
...
さもなくば作成に失敗します。␞␞       </para>␞
␝       <para>␟        This variable is not used for temporary tables; for them,
        <xref linkend="guc-temp-tablespaces"/> is consulted instead.␟この変数は一時テーブル向けには使用されません。
一時テーブル向けには代わりに<xref linkend="guc-temp-tablespaces"/>が考慮されます。␞␞       </para>␞
␝       <para>␟        This variable is also not used when creating databases.
        By default, a new database inherits its tablespace setting from
        the template database it is copied from.␟また、この変数はデータベース作成時には使用されません。
デフォルトでは、新しいデータベースはコピー元のテンプレートデータベースからテーブル空間の設定を引き継ぎます。␞␞       </para>␞
␝       <para>␟        If this parameter is set to a value other than the empty string
        ...
        even if <varname>default_tablespace</varname> has changed since then.␟パーティションテーブルが作られたときにこのパラメータが空文字以外に設定されていると、<varname>default_tablespace</varname>がその後変更されたとしても、パーティションテーブルのテーブル空間はその値になり、今後作られるパーティションのデフォルトテーブル空間として使われます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        For more information on tablespaces,
        see <xref linkend="manage-ag-tablespaces"/>.␟テーブル空間に付いてより詳細な情報は<xref linkend="manage-ag-tablespaces"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␟␟      <indexterm><primary>テーブル空間</primary><secondary>一時的</secondary></indexterm>␞␞       </para>␞
␝       <para>␟        This variable sets the default
        ...
        The default is <literal>pglz</literal>.␟この変数は圧縮可能な列の値のデフォルト<link linkend="storage-toast">TOAST</link>圧縮方式を設定します。
...
デフォルトは<literal>pglz</literal>です。␞␞       </para>␞
␝       <para>␟        This variable specifies tablespaces in which to create temporary
        objects (temp tables and indexes on temp tables) when a
        <command>CREATE</command> command does not explicitly specify a tablespace.
        Temporary files for purposes such as sorting large data sets
        are also created in these tablespaces.␟この変数は、<command>CREATE</command>コマンドで明示的にテーブル空間が指定されない場合に、生成する一時オブジェクト（一時テーブルと一時テーブル上のインデックス）を格納するテーブル空間（複数可）を指定します。
大規模データ集合のソートなどを目的とした一時ファイルもまた、このテーブル空間（複数可）に作成されます。␞␞       </para>␞
␝       <para>␟        The value is a list of names of tablespaces.  When there is more than
        ...
        <productname>PostgreSQL</productname> will automatically use the default
        tablespace of the current database instead.␟この値はテーブル空間名のリストです。
...
リスト内の選択された要素が空文字列だった場合、<productname>PostgreSQL</productname>は自動的に現在のデータベースのデフォルトのテーブル空間を代わりに使用します。␞␞       </para>␞
␝       <para>␟        When <varname>temp_tablespaces</varname> is set interactively, specifying a
        ...
        using a value set in <filename>postgresql.conf</filename>.␟<varname>temp_tablespaces</varname>を対話式に設定する場合、存在しないテーブル空間を指定するとエラーになります。
...
具体的には、この規則は<filename>postgresql.conf</filename>内で設定した値を使用する場合に適用されます。␞␞       </para>␞
␝       <para>␟        The default value is an empty string, which results in all temporary
        objects being created in the default tablespace of the current
        database.␟デフォルト値は空文字列です。
この結果、すべての一時オブジェクトは現在のデータベースのデフォルトのテーブル空間内に作成されます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        See also <xref linkend="guc-default-tablespace"/>.␟<xref linkend="guc-default-tablespace"/>も参照してください␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter is normally on. When set to <literal>off</literal>, it
        ...
        users; <application>pg_dump</application> does so automatically.␟このパラメータは通常オンです。
...
<application>pg_dump</application>はこれを自動的に行います。␞␞       </para>␞
␝       <para>␟        Each SQL transaction has an isolation level, which can be
        ...
        is <quote>read committed</quote>.␟SQLトランザクションはそれぞれ、<quote>read uncommitted</quote>、<quote>read committed</quote>、<quote>repeatable read</quote>、または<quote>serializable</quote>のいずれかの分離レベルを持ちます。
このパラメータは各新規トランザクションのデフォルトの分離レベルを制御します。
デフォルトは<quote>read committed</quote>です。␞␞       </para>␞
...
␟␟<!-- split-config2-end -->
<!-- split-config3-start -->␞␞     <varlistentry id="guc-default-transaction-read-only" xreflabel="default_transaction_read_only">␞
␝      <indexterm>
...
␝       <para>␟        Consult <xref linkend="mvcc"/> and <xref
        linkend="sql-set-transaction"/> for more information.␟より詳細は <xref linkend="mvcc"/> および <xref linkend="sql-set-transaction"/> を調べてください。␞␞       </para>␞
␝       <para>␟        A read-only SQL transaction cannot alter non-temporary tables.
        This parameter controls the default read-only status of each new
        transaction. The default is <literal>off</literal> (read/write).␟読み取り専用のSQLトランザクションでは、非一時的テーブルを変更することができません。
このパラメータは、各新規トランザクションのデフォルトの読み取りのみ状況を制御します。
デフォルト<literal>off</literal>（読み書き）です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Consult <xref linkend="sql-set-transaction"/> for more information.␟より詳細な情報は<xref linkend="sql-set-transaction"/>を調べてください。␞␞       </para>␞
␝       <para>␟        When running at the <literal>serializable</literal> isolation level,
        ...
        option suitable for long-running read-only transactions.␟<literal>シリアライザブル</literal>分離レベルで運用されている場合、繰り延べ読み取り専用SQLトランザクションは、その処理の許可の前に遅延されることがあります。
しかし、ひとたび処理が開始されるとシリアライザブル可能性を保障するために必要ないかなるオーバーヘッドも発生させません。
従って、シリアル化（直列化）のコードは、このオプションを長期間にわたる読み取り専用トランザクションに対して適切な処置と位置づけ、同時実行の更新の観点から中断を強制する理由はありません。␞␞        </para>␞
␝        <para>␟        This parameter controls the default deferrable status of each
        new transaction.  It currently has no effect on read-write
        transactions or those operating at isolation levels lower
        than <literal>serializable</literal>. The default is <literal>off</literal>.␟このパラメータはそれぞれの新規トランザクションのデフォルトでの繰り延べ状態を制御します。
...
␝       <para>␟        Consult <xref linkend="sql-set-transaction"/> for more information.␟より詳細は<xref linkend="sql-set-transaction"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter reflects the current transaction's isolation level.
        ...
        linkend="sql-set-transaction"/> command.␟このパラメータは現在のトランザクションの隔離レベルを反映します。
個々のトランザクションの開始時、これに<xref linkend="guc-default-transaction-isolation"/>の現在の値が設定されます。
その後のこの変数の値を変更する試みは、<xref linkend="sql-set-transaction"/>と同等です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter reflects the current transaction's read-only status.
        At the beginning of each transaction, it is set to the current value
        of <xref linkend="guc-default-transaction-read-only"/>.
        Any subsequent attempt to change it is equivalent to a <xref
        linkend="sql-set-transaction"/> command.␟このパラメータは現在のトランザクションの読み込み専用状態を反映します。
...
␝       <para>␟        This parameter reflects the current transaction's deferrability status.
        At the beginning of each transaction, it is set to the current value
        of <xref linkend="guc-default-transaction-deferrable"/>.
        Any subsequent attempt to change it is equivalent to a <xref
        linkend="sql-set-transaction"/> command.␟このパラメータは現在のトランザクションの延期状態(deferrable status)を反映します。
個々のトランザクションの開始時、これに<xref linkend="guc-default-transaction-deferrable"/>の現在の値が設定されます。
その後のこの変数の値を変更する試みは、<xref linkend="sql-set-transaction"/>と同等です。␞␞       </para>␞
␝       <para>␟        Controls firing of replication-related triggers and rules for the
        current session.
        ...
        query plans.
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟現在のセッションに対するレプリケーション関連のトリガおよびルールの起動を制御します。
...
スーパーユーザおよび適切な<literal>SET</literal>権限を持つユーザのみがこの設定を変更できます。␞␞       </para>␞
␝       <para>␟        The intended use of this setting is that logical replication systems
        ...
        for more information.␟この設定の使い方の趣旨としては、レプリケーションされた更新を適用する際に論理レプリケーションシステムが<literal>replica</literal>に設定するということです。
このことによる効果としては、（デフォルトの設定から変更されていない）トリガとルールはレプリカ上では起動されない、ということです。
更なる情報は、<link linkend="sql-altertable"><command>ALTER TABLE</command></link>節の<literal>ENABLE TRIGGER</literal>と<literal>ENABLE RULE</literal>をご覧ください。␞␞       </para>␞
␝       <para>␟        PostgreSQL treats the settings <literal>origin</literal> and
        ...
        changes should not be replicated.␟PostgreSQLは<literal>origin</literal>と<literal>local</literal>の設定を内部的に同じものとして扱います。
サードパーティのレプリケーションシステムは内部的な目的、たとえば<literal>local</literal>を使ってレプリケーションされるべきでないセッションを指定するためにこれら2つの値を使って構いません。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Since foreign keys are implemented as triggers, setting this parameter
        to <literal>replica</literal> also disables all foreign key checks,
        which can leave data in an inconsistent state if improperly used.␟外部キーはトリガとして実装されているため、このパラメータを<literal>replica</literal>とすることによって同時にすべての外部キー検査が無効になります。
このことにより、正しく使用しないと、データを不整合状態にしてしまう可能性があります。␞␞       </para>␞
␝       <para>␟        Abort any statement that takes more than the specified amount of time.
        If <varname>log_min_error_statement</varname> is set
        to <literal>ERROR</literal> or lower, the statement that timed out
        will also be logged.
        If this value is specified without units, it is taken as milliseconds.
        A value of zero (the default) disables the timeout.␟コマンドがクライアントからサーバに届いた時から数えて、実行時間が指定された時間を越えた文を停止します。
<varname>log_min_error_statement</varname>が<literal>ERROR</literal>もしくはそれ以下に設定されている場合は、タイムアウトした文はログに書き込まれます。
この値が単位なしで指定された場合は、ミリ秒単位であるとみなします。
値がゼロ（デフォルト）の場合、これを無効にします。␞␞       </para>␞
␝       <para>␟        The timeout is measured from the time a command arrives at the
        ...
        it is canceled by completion of an Execute or Sync message.␟タイムアウトは、コマンドがサーバに到着したときから、サーバがそのコマンドの実行を完了するまでを基準にします。
...
拡張問い合わせでは、タイムアウトは問い合わせに関するメッセージ（Parse、Bind、Execute、Describe）が到着したときに開始し、ExecuteあるいはSyncメッセージが完了した時にキャンセルされます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Setting <varname>statement_timeout</varname> in
        <filename>postgresql.conf</filename> is not recommended because it would
        affect all sessions.␟すべてのセッションに影響することがあるので、<filename>postgresql.conf</filename>内で<varname>statement_timeout</varname>を設定することは推奨されません。␞␞       </para>␞
␝       <para>␟        Terminate any session that spans longer than the specified amount of
        ...
        If this value is specified without units, it is taken as milliseconds.
        A value of zero (the default) disables the timeout.␟トランザクションが指定された時間を超えていた場合に、このセッションを終了します。
この時間制限は、明示的なトランザクション（<command>BEGIN</command>で開始）と、単一の文に対応する暗黙的に開始されたトランザクションの両方に適用されます。
この値が単位なしで指定された場合は、ミリ秒単位であるとみなします。
値がゼロ（デフォルト）の場合はタイムアウトは無効になります。␞␞       </para>␞
␝       <para>␟        If <varname>transaction_timeout</varname> is shorter or equal to
        <varname>idle_in_transaction_session_timeout</varname> or <varname>statement_timeout</varname>
        then the longer timeout is ignored.␟<varname>transaction_timeout</varname>が<varname>idle_in_transaction_session_timeout</varname>または<varname>statement_timeout</varname>より短いか、等しい場合、長い方のタイムアウトは無視されます。␞␞       </para>␞
␝       <para>␟        Setting <varname>transaction_timeout</varname> in
        <filename>postgresql.conf</filename> is not recommended because it would
        affect all sessions.␟すべてのセッションに影響することがあるので、<filename>postgresql.conf</filename>内で<varname>transaction_timeout</varname>を設定することは推奨されません。␞␞       </para>␞
␝      <indexterm>
...
␝        <para>␟         Prepared transactions are not subject to this timeout.␟準備されたトランザクションには、このタイムアウトは影響ありません。␞␞        </para>␞
␝       <para>␟        Abort any statement that waits longer than the specified amount of
        ...
        locks.
        If this value is specified without units, it is taken as milliseconds.
        A value of zero (the default) disables the timeout.␟テーブル、インデックス、行、またはその他のデータベースオブジェクトに対してロック獲得を試みている最中、指定された時間を超えて待機するいかなる命令も停止されます。
...
この値が単位なしで指定された場合は、ミリ秒単位であるとみなします。
値ゼロ（デフォルト）はこの機能を無効にします。␞␞       </para>␞
␝       <para>␟        Unlike <varname>statement_timeout</varname>, this timeout can only occur
        ...
        logged.␟<varname>statement_timeout</varname>と異なり、このタイムアウトはロックを待機しているときのみ発生します。
...
<varname>log_min_error_statement</varname>が<literal>ERROR</literal>またはそれより低く設定されると、時間制限を超えた命令はログに記録されます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Setting <varname>lock_timeout</varname> in
        <filename>postgresql.conf</filename> is not recommended because it would
        affect all sessions.␟<varname>lock_timeout</varname>を<filename>postgresql.conf</filename>にて設定することは、すべてのセッションに影響を与える可能性があるため推奨されません。␞␞       </para>␞
␝       <para>␟        Terminate any session that has been idle (that is, waiting for a
        client query) within an open transaction for longer than the
        specified amount of time.
        If this value is specified without units, it is taken as milliseconds.
        A value of zero (the default) disables the timeout.␟開いているトランザクションが、指定された時間を超えてアイドルだった場合（つまりクライアントからの問い合わせを待っている場合）に、セッションを終了します。
この値が単位なしで指定された場合は、ミリ秒単位であるとみなします。
値がゼロ（デフォルト）の場合はタイムアウトは無効になります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This option can be used to ensure that idle sessions do not hold
        ...
        See <xref linkend="routine-vacuuming"/> for more details.␟このオプションにより、アイドル状態のセッションが不合理なほど長い時間ロックを保持することを防ぐことができます。
強いロックが保持されていなくても、開いたトランザクションは、自分にしか可視でない、最近無効になったタプルをvacuumが掃除するのを妨げます。
ですから、長時間アイドルのままになっていると、テーブルの溢れにつながります。
更なる詳細は<xref linkend="routine-vacuuming"/>を見てください。␞␞       </para>␞
␝       <para>␟        Terminate any session that has been idle (that is, waiting for a
        client query), but not within an open transaction, for longer than
        the specified amount of time.
        If this value is specified without units, it is taken as milliseconds.
        A value of zero (the default) disables the timeout.␟トランザクションが開いた状態ではなく、指定した時間よりも長い時間アイドル（すなわちクライアントからの問い合わせを待っている）であるセッションを終了させます。
この値が単位なしで指定された場合は、ミリ秒単位であるとみなします。
値ゼロ（デフォルトです）はタイムアウトを無効にします。␞␞       </para>␞
␝       <para>␟        Unlike the case with an open transaction, an idle session without a
        transaction imposes no large costs on the server, so there is less
        need to enable this timeout
        than <varname>idle_in_transaction_session_timeout</varname>.␟開いたトランザクションの場合と違って、トランザクションを伴わないアイドルなセッションは、サーバに対して大きなコストにはなりません。
ですからこのタイムアウトを有効にする必要性は、<varname>idle_in_transaction_session_timeout</varname>よりも低いと言えます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Be wary of enforcing this timeout on connections made through
        ...
        perhaps by applying it only to particular users.␟コネクションプーリングソフトウェアその他のミドルウェアを通じたコネクションに対してこのタイムアウトを強制することには慎重になってください。
そのようなレイヤは、突然のコネクション切断に対して好ましくない振る舞いをするかもしれないからです。
おそらく特定のユーザに対してのみ適用し、対話形式のセッションに対してのみこのタイムアウトを有効にするのが助けになるかもしれません。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        <command>VACUUM</command> performs an aggressive scan if the table's
        ...
        information see
        <xref linkend="vacuum-for-wraparound"/>.␟テーブルの<structname>pg_class</structname>.<structfield>relfrozenxid</structfield>フィールドがこの設定で指定した時期に達すると、<command>VACUUM</command>は積極的なテーブルスキャンを行います。
...
␝       <para>␟        Specifies the cutoff age (in transactions) that
        ...
        linkend="vacuum-for-wraparound"/>.␟<command>VACUUM</command>が、古いXIDを持つページをフリーズするきっかけとするかどうかを決定する、カットオフ年代を（トランザクション単位で）指定します。
デフォルトは5千万トランザクションです。
...
␝       <para>␟        Specifies the maximum age (in transactions) that a table's
        ...
        the failsafe to trigger during any <command>VACUUM</command>.␟<command>VACUUM</command>がシステム全体に渡るトランザクションID周回障害を避けるために異例の措置を取るようになる、テーブルの<structname>pg_class</structname>.<structfield>relfrozenxid</structfield>フィールドが到達する最大の（トランザクション数単位での）年齢を指定します。
これは<command>VACUUM</command>の最後の手段となる戦略です。
この安全機構は典型的には、トランザクションID周回を防ぐための自動バキュームがすでに走っているときに起動されます。
しかし、すべての<command>VACUUM</command>の実行中にこの安全機構が起動する可能性があります。␞␞       </para>␞
␝       <para>␟        When the failsafe is triggered, any cost-based delay that is
        ...
        <glossterm linkend="glossary-shared-memory">shared buffers</glossterm>.␟この安全機構が起動すると、すべての有効なコストに基づく遅延はもはや適用されず、（インデックスのバキュームのような）必須ではない保守タスクは迂回されます。
使用中の<glossterm linkend="glossary-buffer-access-strategy">バッファアクセスストラテジ</glossterm>は無効になり、その結果<command>VACUUM</command>は<glossterm linkend="glossary-shared-memory">共有バッファ</glossterm>のすべてを自由に使用することになります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The default is 1.6 billion transactions.  Although users can
        ...
        value to no less than 105% of <xref
         linkend="guc-autovacuum-freeze-max-age"/>.␟デフォルトは16億トランザクションです。
ユーザはこの値をゼロから21億までの間で設定できますが、<command>VACUUM</command>は<xref linkend="guc-autovacuum-freeze-max-age"/>の105%よりも小さくならないように実際の値を調整します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        <command>VACUUM</command> performs an aggressive scan if the table's
        ...
        For more information see <xref linkend="vacuum-for-multixact-wraparound"/>.␟<structname>pg_class</structname>.<structfield>relminmxid</structfield>フィールドがこの設定値で指定した年代に達すると<command>VACUUM</command>はテーブルの積極的なスキャンを行います。
...
詳細は<xref linkend="vacuum-for-multixact-wraparound"/>をご覧ください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Specifies the cutoff age (in multixacts) that <command>VACUUM</command>
        ...
        autovacuums.
        For more information see <xref linkend="vacuum-for-multixact-wraparound"/>.␟<command>VACUUM</command>がテーブルをスキャンする際に、ページを凍結するきっかけとするかどうかを決める下限値をマルチトランザクション単位で指定します。
...
␝       <para>␟        Specifies the maximum age (in multixacts) that a table's
        ...
        failsafe to trigger during any <command>VACUUM</command>.␟<command>VACUUM</command>がシステム全体マルチトランザクションID周回障害を避けるために異例の措置を取るようになる、テーブルの<structname>pg_class</structname>.<structfield>relminmxid</structfield>フィールドが到達する最大の（マルチトランザクション数単位での）年齢を指定します。
これは<command>VACUUM</command>最後の手段となる戦略です。
この安全機構は典型的には、トランザクションID周回を防ぐための自動バキュームがすでに走っているときに起動されます。
しかし、すべての<command>VACUUM</command>の実行中にこの安全機構が起動する可能性があります。␞␞       </para>␞
␝       <para>␟        When the failsafe is triggered, any cost-based delay that is
        in effect will no longer be applied, and further non-essential
        maintenance tasks (such as index vacuuming) are bypassed.␟この安全機構が起動すると、すべての有効なコストに基づく遅延はもはや適用されず、（インデックスのバキュームのような）必須ではない保守タスクは迂回されます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The default is 1.6 billion multixacts.  Although users can set
        this value anywhere from zero to 2.1 billion,
        <command>VACUUM</command> will silently adjust the effective
        value to no less than 105% of <xref
         linkend="guc-autovacuum-multixact-freeze-max-age"/>.␟デフォルトは16億マルチトランザクションです。
ユーザはこの値をゼロから21億までの間で設定できますが、<command>VACUUM</command>は<xref linkend="guc-autovacuum-multixact-freeze-max-age"/>の105%よりも小さくならないように実際の値を調整します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the output format for values of type <type>bytea</type>.
        ...
        accepts both formats on input, regardless of this setting.␟<type>bytea</type>型の値の出力形式を設定します。
...
<type>bytea</type>型は常にこの設定に係わらず、入力時に双方の書式を受け付けます。␞␞       </para>␞
␝       <para>␟        Sets how binary values are to be encoded in XML.  This applies
        ...
        XML-related functions, see <xref linkend="functions-xml"/>.␟バイナリデータをXMLに符号化する方法を設定します。
...
XMLに関連した関数については<xref linkend="functions-xml"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The actual choice here is mostly a matter of taste,
        ...
        base64 encoding.␟実のところこの選択はほとんど趣味の問題で、クライアントアプリケーションで起こり得る制限のみに制約されます。
どちらの方法もすべての値をサポートしますが、hex符号化方式はbase64符号化方式より少し大きくなります。␞␞       </para>␞
␝       <para>␟        Sets whether <literal>DOCUMENT</literal> or
        ...
        <literal>CONTENT</literal>.␟XMLと文字列値との変換時に<literal>DOCUMENT</literal>とするか<literal>CONTENT</literal>とするかを設定します。
この説明については<xref linkend="datatype-xml"/>を参照してください。
有効な値は<literal>DOCUMENT</literal>と<literal>CONTENT</literal>です。
デフォルトは<literal>CONTENT</literal>です。␞␞       </para>␞
␝       <para>␟        According to the SQL standard, the command to set this option is␟標準SQLに従うと、このオプションを設定するコマンドは以下のようになります。␞␞<synopsis>␞
␝      <indexterm>
...
␝</synopsis>␟        This syntax is also available in PostgreSQL.␟この構文はPostgreSQLでも使用可能です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the maximum size of a GIN index's pending list, which is used
        ...
        index storage parameters.
         ...
         for more information.␟<literal>fastupdate</literal>が有効なときに使用されるGINインデックスのペンディングリストの最大サイズを設定します。
...
更なる情報については、<xref linkend="gin-fast-update"/>と<xref linkend="gin-tips"/>を参照してください。␞␞       </para>␞
␝       <para>␟        If a user who has <literal>CREATEROLE</literal> but not
        ...
        which disables the feature.␟<literal>CREATEROLE</literal>を持ち、<literal>SUPERUSER</literal>を持たないユーザがロールを作成し、これが空でない値に設定されている場合、新しく作成されたロールは、この指定されたオプションを持ち、ロールを作成したユーザに付与されます。
値は、<literal>set</literal>、<literal>inherit</literal>、またはこれらのリストをカンマで区切ったものである必要があります。
デフォルト値は空の文字列であるため、この機能は無効になります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The purpose of this option is to allow a <literal>CREATEROLE</literal>
        ...
        for users which they create.␟このオプションの目的は、<literal>CREATEROLE</literal>が許可されているスーパーユーザではないユーザが、作成されたユーザを自動的に継承するか、または<literal>SET ROLE</literal>権限を自動的に取得できるようにすることです。
...
また、この設定を使用して、<literal>CREATEROLE</literal>ユーザが作成したユーザに対して同様の動作を生成できます。␞␞       </para>␞
␝      <indexterm>
...
␝      <para>␟       Allow temporarily disabling execution of event triggers in order to
       ...
       <literal>SET</literal> privilege can change this setting.␟イベントトリガの実行を一時的に無効化して、障害のあるイベントトリガのトラブルシューティングおよび修復を可能にします。
<literal>false</literal>に設定することですべてのイベントトリガが無効になります。
値を<literal>true</literal>に設定すると、すべてのイベントトリガが起動します。
これがデフォルト値です。
スーパーユーザおよび適切な<literal>SET</literal>権限を持つユーザのみがこの設定を変更できます。␞␞      </para>␞
␝      <para>␟       Set relation kinds for which access to non-system relations is prohibited.
       ...
       <literal>foreign-table</literal>.␟非システムリレーションへのアクセスが禁止されているリレーションの種類を設定します。
値は、カンマで区切られたリレーションの種類のリストのフォームを取ります。
現在サポートされているリレーション種別は<literal>view</literal>と<literal>foreign-table</literal>です。␞␞      </para>␞
␝      <indexterm>
...
␝     <sect2 id="runtime-config-client-format">␟     <title>Locale and Formatting</title>␟     <title>ロケールと書式設定</title>␞␞␞
␝      <indexterm>
...
␝       <para>␟        Sets the display format for date and time values, as well as the
        ...
        behavior of the chosen <varname>lc_time</varname> locale.␟日付時刻値の表示書式を設定し、曖昧な日付入力の解釈規則を設定します。
...
組み込みのデフォルトは<literal>ISO, MDY</literal>ですが、<application>initdb</application>により、選択された<varname>lc_time</varname>ロケールの動作に対応した設定で設定ファイルが初期化されます。␞␞       </para>␞
␝       <para>␟        Sets the display format for interval values.
        ...
        4.4.3.2 of ISO 8601.␟間隔の値の表示形式を設定します。<literal>sql_standard</literal>値は、標準<acronym>SQL</acronym>間隔リテラルに一致する出力を生成します。
...
値<literal>iso_8601</literal>は、ISO 8601の4.4.3.2節で定義されている時間間隔<quote>format with designators</quote>に一致する出力を生成します。␞␞       </para>␞
␝      <indexterm>
...
␟␟      <indexterm><primary>時間帯</primary></indexterm>␞␞       </para>␞
␝       <para>␟        The <varname>IntervalStyle</varname> parameter also affects the
        interpretation of ambiguous interval input.  See
        <xref linkend="datatype-interval-input"/> for more information.␟また<varname>IntervalStyle</varname>パラメータはあいまいに入力された時間間隔の解釈に影響を与えます。
詳細については<xref linkend="datatype-interval-input"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␟␟      <indexterm><primary>時間帯名</primary></indexterm>␞␞       </para>␞
␝       <para>␟        Sets the time zone for displaying and interpreting time stamps.
        The built-in default is <literal>GMT</literal>, but that is typically
        overridden in <filename>postgresql.conf</filename>; <application>initdb</application>
        will install a setting there corresponding to its system environment.
        See <xref linkend="datatype-timezones"/> for more information.␟表示用およびタイムスタンプ解釈用の時間帯を設定します。
...
詳細は<xref linkend="datatype-timezones"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the collection of time zone abbreviations that will be accepted
        ...
        See <xref linkend="datetime-config-files"/> for more information.␟サーバで日付時刻の入力として受付け可能となる時間帯省略形の集合を設定します。
デフォルトは<literal>'Default'</literal>です。
これはほぼ全世界で通じる集合です。
また、<literal>Australia</literal>、<literal>India</literal>、その他特定のインストレーションで定義可能な集合が存在します。
詳細は<xref linkend="datetime-config-files"/>を参照してください。␞␞       </para>␞
␝       <para>␟        This parameter adjusts the number of digits used for textual output of
        floating-point values, including <type>float4</type>, <type>float8</type>,
        and geometric data types.␟このパラメータは、<type>float4</type>、<type>float8</type>、幾何データ型などにおいて、浮動小数点数値のテキスト形式の出力で使用される桁数を調整します。␞␞       </para>␞
␝       <para>␟        If the value is 1 (the default) or above, float values are output in
        ...
        compatibility, values up to 3 are permitted.␟この値が1（デフォルト）あるいはそれ以上なら、浮動小数点数値の出力は最も短い精度の形式で出力されます。
...
歴史的な互換性の理由により、3までの値が許容されています。␞␞       </para>␞
␝       <para>␟        If the value is zero or negative, then the output is rounded to a
        ...
        the bits of the binary float value, but may be more human-readable.␟この値がゼロまたは負なら、出力は与えられた10進精度に丸められます。
...
この形式は低速、かつ元の2進数における浮動小数点値のビットを保存しませんが、人間にとってより読みやすいかも知れません。␞␞       </para>␞
␝      <indexterm>
...
␟␟      <indexterm><primary>文字セット</primary></indexterm>␞␞        </para>␞
␝        <para>␟         The meaning of this parameter, and its default value, changed
         in <productname>PostgreSQL</productname> 12;
         see <xref linkend="datatype-float"/> for further discussion.␟このパラメータの意味とデフォルト値は<productname>PostgreSQL</productname> 12で変更されました。
更なる説明については<xref linkend="datatype-float"/>をご覧ください。␞␞        </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the client-side encoding (character set).
        The default is to use the database encoding.
        The character sets supported by the <productname>PostgreSQL</productname>
        server are described in <xref linkend="multibyte-charset-supported"/>.␟クライアント側符号化方式（文字セット）を設定します。デフォルトはデータベース符号化方式を使用します。
<productname>PostgreSQL</productname>サーバでサポートされている文字セットは<xref linkend="multibyte-charset-supported"/>に記載されています。␞␞       </para>␞
␝       <para>␟        Sets the language in which messages are displayed.  Acceptable
        values are system-dependent; see <xref linkend="locale"/> for
        more information.  If this variable is set to the empty string
        (which is the default) then the value is inherited from the
        execution environment of the server in a system-dependent way.␟メッセージが表示される言語を設定します。使用可能な値はシステムに依存します。詳細については<xref linkend="locale"/>を参照してください。
この変数が空に設定された場合（これがデフォルトです）、値はシステムに依存する方法でサーバの実行環境から継承されます。␞␞       </para>␞
␝       <para>␟        On some systems, this locale category does not exist.  Setting
        this variable will still work, but there will be no effect.
        Also, there is a chance that no translated messages for the
        desired language exist.  In that case you will continue to see
        the English messages.␟システムによっては、このロケールのカテゴリが存在しません。この変数を設定することはできますが、実効性はありません。
また、指定の言語に翻訳されたメッセージが存在しないこともあります。
その場合は、引き続き英語のメッセージが表示されます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟この設定を変更できるのは、スーパーユーザと適切な<literal>SET</literal>権限を持つユーザだけです。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Sets the locale to use for formatting monetary amounts, for
        example with the <function>to_char</function> family of
        functions.  Acceptable values are system-dependent; see <xref
        linkend="locale"/> for more information.  If this variable is
        set to the empty string (which is the default) then the value
        is inherited from the execution environment of the server in a
        system-dependent way.␟通貨書式で使用するロケールを設定します。
...
␝       <para>␟        Sets the locale to use for formatting numbers, for example
        with the <function>to_char</function> family of
        functions. Acceptable values are system-dependent; see <xref
        linkend="locale"/> for more information.  If this variable is
        set to the empty string (which is the default) then the value
        is inherited from the execution environment of the server in a
        system-dependent way.␟数字の書式で使用するロケールを設定します。
例えば、<function>to_char</function>系の関数で使用します。
使用可能な値はシステムに依存します。
詳細については<xref linkend="locale"/>を参照してください。
この変数が空に設定された場合（これがデフォルトです）、値はシステムに依存する方法でサーバの実行環境から継承されます。␞␞       </para>␞
...
␝       <para>␟        Sets the locale to use for formatting dates and times, for example
        with the <function>to_char</function> family of
        functions. Acceptable values are system-dependent; see <xref
        linkend="locale"/> for more information.  If this variable is
        set to the empty string (which is the default) then the value
        is inherited from the execution environment of the server in a
        system-dependent way.␟例えば<function>to_char</function>系関数における、日付と時間の書式で使用するロケールを設定します。
使用可能な値はシステムに依存します。
詳細については<xref linkend="locale"/>を参照してください。
この変数が空に設定された場合（これがデフォルトです）、値はシステムに依存する方法でサーバの実行環境から継承されます。␞␞       </para>␞
␝       <para>␟        When ICU locale validation problems are encountered, controls which
        ...
        <literal>LOG</literal>.␟ICUロケール検証の問題が発生した場合、問題を報告するために使用される<link linkend="runtime-config-severity-levels">メッセージレベル</link>を制御します。
有効な値は、<literal>DISABLED</literal>、<literal>DEBUG5</literal>、<literal>DEBUG4</literal>、<literal>DEBUG3</literal>、<literal>DEBUG2</literal>、<literal>DEBUG1</literal>、<literal>INFO</literal>、<literal>NOTICE</literal>、<literal>WARNING</literal>、<literal>ERROR</literal>、および <literal>LOG</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        If set to <literal>DISABLED</literal>, does not report validation
        problems at all. Otherwise reports problems at the given message
        level. The default is <literal>WARNING</literal>.␟<literal>DISABLED</literal>に設定すると、検証で見つかった問題は全く報告されません。
それ以外の場合は、指定したメッセージレベルの問題が報告されます。
デフォルトは<literal>WARNING</literal>です。␞␞       </para>␞
␝       <para>␟        Selects the text search configuration that is used by those variants
        ...
        matching that locale can be identified.␟明示的な設定指定引数を持たないテキスト検索関数の亜種で使用される、テキスト検索設定を選択します。
詳細は<xref linkend="textsearch"/>を参照してください。
組み込みのデフォルトは<literal>pg_catalog.simple</literal>ですが、<application>initdb</application>は、ロケールに合う設定を認識することができれば、選択された<varname>lc_ctype</varname>ロケールに対応した設定で設定ファイルを初期化します。␞␞       </para>␞
␝    <sect2 id="runtime-config-client-preload">␟     <title>Shared Library Preloading</title>␟     <title>共有ライブラリのプリロード</title>␞␞␞
␝     <para>␟      Several settings are available for preloading shared libraries into the
      ...
      take effect and what privileges are required to change them.␟追加機能や性能改良の目的で共有ライブラリをプリロードするいくつかの設定があります。
...
各設定の違いは、設定変更を行うためにいつ、どのような権限が必要かにあります。␞␞     </para>␞
␝     <para>␟      <productname>PostgreSQL</productname> procedural language libraries can
      ...
      <literal>tcl</literal>, or <literal>python</literal>.␟典型的には<literal>'$libdir/plXXX'</literal>のような構文を用いて<productname>PostgreSQL</productname>手続き言語ライブラリをこの方法でプリロードできます。
<literal>XXX</literal>は<literal>pgsql</literal>、<literal>perl</literal>、<literal>tcl</literal>、<literal>python</literal>です。␞␞     </para>␞
␝     <para>␟      Only shared libraries specifically intended to be used with PostgreSQL
      ...
      as <envar>LD_PRELOAD</envar> for that.␟PostgreSQLで使用することを意図したライブラリだけがこの方法でロードできます。
すべてのPostgreSQL用のライブラリは<quote>magic block</quote>を持ち、互換性を保証するためにチェックされます。
ですからPostgreSQL用ではないライブラリはこの方法ではロードできません。
<envar>LD_PRELOAD</envar>のようなOSの機能を使えばあるいは使用できるかもしれません。␞␞     </para>␞
␝      <indexterm>
...
␝     <para>␟      In general, refer to the documentation of a specific module for the
      recommended way to load that module.␟一般的に言ってモジュールのドキュメントを参照し、推奨される方法でロードしてください。␞␞     </para>␞
␝       <para>␟        This variable specifies one or more shared libraries that are to be
        ...
        found, the connection attempt will fail.␟この変数は、接続時に事前読み込みされる、1つまたは複数の共有ライブラリを指定します。
...
以降の変更は効果がありません。
もし指定したライブラリが見つからない場合は、接続は失敗します。␞␞       </para>␞
␝       <para>␟        This option can be set by any user.  Because of that, the libraries
        ...
        the same effect as <literal>$libdir/plugins/mylib</literal>.␟このオプションはすべてのユーザが設定できます。
...
<literal>mylib</literal> は<literal>$libdir/plugins/mylib</literal>と同じ効果です。␞␞       </para>␞
␝       <para>␟        The intent of this feature is to allow unprivileged users to load
        ...
        using
        <command>ALTER ROLE SET</command>.␟この機能の意図するところは、明示的な<command>LOAD</command>コマンドを使わずに、特定のセッションにおいて非特権ユーザがデバッグ用あるいは性能計測用のライブラリをロードできるようにすることにあります。
そのためにも、クライアント側で<envar>PGOPTIONS</envar>環境変数を使う、あるいは<command>ALTER ROLE SET</command>を使うことが典型的になるでしょう。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        However, unless a module is specifically designed to be used in this way by
        non-superusers, this is usually not the right setting to use.  Look
        at <xref linkend="guc-session-preload-libraries"/> instead.␟しかし、モジュールが特にスーパーユーザ以外に使われることを意図しているのでない限り、通常この方法は正しい使い方ではありません。
代わりに<xref linkend="guc-session-preload-libraries"/>を見てください。␞␞       </para>␞
␝       <para>␟        This variable specifies one or more shared libraries that are to be
        preloaded at connection start.
        It contains a comma-separated list of library names, where each name
        is interpreted as for the <link linkend="sql-load"><command>LOAD</command></link> command.
...
␝        found, the connection attempt will fail.␟        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟この設定を変更できるのは、スーパーユーザと適切な<literal>SET</literal>権限を持つユーザだけです。␞␞       </para>␞
␝       <para>␟        The intent of this feature is to allow debugging or
        ...
        if they should apply to all sessions.␟この機能は、デバッグや性能測定の目的で<command>LOAD</command>コマンドを使わずに特定のセッションでライブラリをロードする目的で使われます。
たとえば<command>ALTER ROLE SET</command>で設定することにより、特定のユーザが開始するすべてのセッションで<xref linkend="auto-explain"/>が有効になります。
また、このパラメータはサーバを再起動せずに変更できます(しかし変更は新しいセッションが開始するときにのみ有効となります)。すべてのセッションで有効にしたいのであれば、この方法で新しいモジュールを容易に追加できます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Unlike <xref linkend="guc-shared-preload-libraries"/>, there is no large
        ...
        connection pooling is used.␟<xref linkend="guc-shared-preload-libraries"/>と違って、ライブラリがはじめて使われるときにロードする方法と比べてセッションが開始するときにライブラリをロードする方法には大きな性能的な優位性はありません。
しかし、コネクションプーリングを使うのであれば、いくらか優位性があります。␞␞       </para>␞
␝       <para>␟        This variable specifies one or more shared libraries to be preloaded at
        server start.
        It contains a comma-separated list of library names, where each name
        is interpreted as for the <link linkend="sql-load"><command>LOAD</command></link> command.
        Whitespace between entries is ignored; surround a library name with
        ...
        library is not found, the server will fail to start.␟この変数はサーバ起動時にプリロードされる一つ以上の共有ライブラリを指定します。
ここにはカンマ区切りでライブラリ名のリストを格納し、各々の名前は<link linkend="sql-load"><command>LOAD</command></link>コマンドで解釈されます。
項目の間の空白は無視されます。
名前の中に空白あるいはカンマを含める場合は、二重引用符で囲ってください。
このパラメータはサーバ起動時のみ設定可能です。
もし指定したライブラリが見つからない場合は、サーバの起動は失敗します。␞␞       </para>␞
␝       <para>␟        Some libraries need to perform certain operations that can only take
        ...
        documentation of each library for details.␟ライブラリによってはpostmaster起動時にのみ可能な操作を実行する必要があるものがあります。
たとえば、共有メモリの獲得、軽量ロックの予約、バックグラウンドワーカーの起動などです。
このようなライブラリはこのパラメータを使ってサーバ起動時にロードしなければなりません。
詳細は各ライブラリのドキュメントを見てください。␞␞       </para>␞
␝       <para>␟        Other libraries can also be preloaded.  By preloading a shared library,
...
<xref linkend="guc-session-preload-libraries"/>を代わりに使ってください。␞␞       </para>␞
␝       <indexterm>
...
␝       <para>␟        On Windows hosts, preloading a library at server start will not reduce
        ...
        perform operations at postmaster start time.␟Windowsのホストでは、ライブラリのプリロードは、新しいサーバプロセスの起動に要する時間を短縮しません。
個々のサーバプロセスは、すべてのプリロードライブラリを再読み込みします。
それでもpostmaster起動時に操作を実行しなければならないライブラリを使用するWindowsホストにとっては<varname>shared_preload_libraries</varname>は有用です。␞␞       </para>␞
␝       <para>␟        This variable is the name of the JIT provider library to be used
        (see <xref linkend="jit-pluggable"/>).
        The default is <literal>llvmjit</literal>.
        This parameter can only be set at server start.␟この変数は、使用するJITプロバイダライブラリ(<xref linkend="jit-pluggable"/>参照)の名前です。
デフォルトは<literal>llvmjit</literal>です。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝       <para>␟        If set to a non-existent library, <acronym>JIT</acronym> will not be
        available, but no error will be raised. This allows JIT support to be
        installed separately from the main
        <productname>PostgreSQL</productname> package.␟存在しないライブラリが指定されると、<acronym>JIT</acronym>は利用できませんが、エラーは起こりません。
これにより、<productname>PostgreSQL</productname>パッケージとは別にJITサポートをインストールできるようになります。␞␞       </para>␞
␝      <indexterm>
...
␝     <sect2 id="runtime-config-client-other">␟     <title>Other Defaults</title>␟     <title>その他のデフォルト</title>␞␞␞
␝       <para>␟        If a dynamically loadable module needs to be opened and the
        file name specified in the <command>CREATE FUNCTION</command> or
        <command>LOAD</command> command
        does not have a directory component (i.e., the
        name does not contain a slash), the system will search this
        path for the required file.␟オープンする必要がある動的ロード可能なモジュールについて、その<command>CREATE FUNCTION</command>や<command>LOAD</command>コマンドで指定されたファイル名にディレクトリ要素がなく（つまり、名前にスラッシュが含まれずに）指定された場合、システムは必要なファイルをこのパスから検索します。␞␞       </para>␞
␝       <para>␟        The value for <varname>dynamic_library_path</varname> must be a
        ...
        this directory.) For example:␟<varname>dynamic_library_path</varname>の値は、絶対パスのディレクトリ名をコロン（Windowsの場合はセミコロン）で区切った一覧です。
...
例を以下に示します。␞␞<programlisting>␞
␝</programlisting>␟        or, in a Windows environment:␟Windows環境の場合は以下です。␞␞<programlisting>␞
␝       <para>␟        The default value for this parameter is
        <literal>'$libdir'</literal>. If the value is set to an empty
        string, the automatic path search is turned off.␟このパラメータのデフォルト値は<literal>'$libdir'</literal>です。
この値が空に設定された場合、自動的なパス検索は無効になります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter can be changed at run time by superusers and users
        with the appropriate <literal>SET</literal> privilege, but a
        setting done that way will only persist until the end of the
        client connection, so this method should be reserved for
        development purposes. The recommended way to set this parameter
        is in the <filename>postgresql.conf</filename> configuration
        file.␟このパラメータはスーパーユーザと、適切な<literal>SET</literal>権限を持つユーザによって実行時に変更することができますが、この方法での設定は、そのクライアント接続が終わるまでしか有効になりません。
ですので、この方法は開発目的でのみ使用すべきです。
推奨方法はこのパラメータを<filename>postgresql.conf</filename>設定ファイル内で設定することです。␞␞       </para>␞
␝       <para>␟        Soft upper limit of the size of the set returned by GIN index scans. For more
        information see <xref linkend="gin-tips"/>.␟GINインデックススキャンにより返されるセットのソフトな上限サイズです。
詳細は<xref linkend="gin-tips"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝   <sect1 id="runtime-config-locks">␟    <title>Lock Management</title>␟    <title>ロック管理</title>␞␞␞
␝       <para>␟        This is the amount of time to wait on a lock
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟これは、デッドロック状態があるかどうかを調べる前にロックを待つ時間です。
...
␝       <para>␟        When <xref linkend="guc-log-lock-waits"/> is set,
        this parameter also determines the amount of time to wait before
        a log message is issued about the lock wait.  If you are trying
        to investigate locking delays you might want to set a shorter than
        normal <varname>deadlock_timeout</varname>.␟<xref linkend="guc-log-lock-waits"/>が設定された場合、このパラメータはロック待機に関するログメッセージを出力する前の待機時間を決定します。
ロック遅延の調査を行う場合は、通常の<varname>deadlock_timeout</varname>よりも短い値を設定することを勧めます。␞␞       </para>␞
␝       <para>␟        The shared lock table has space for
        ...
        many children.  This parameter can only be set at server start.␟共有ロックテーブルは、プロセスごと、あるいは準備されたトランザクションごとの<varname>max_locks_per_transaction</varname>オブジェクト（たとえばテーブル）空間を持っています。
...
␝       <para>␟        When running a standby server, you must set this parameter to have the
...
␝       <para>␟        The shared predicate lock table has space for
        ...
        only be set at server start.␟共有述語ロックテーブルは、プロセスごと、あるいは準備されたトランザクションごとの<varname>max_pred_locks_per_transaction</varname>オブジェクト（たとえばテーブル）空間を持っています。
したがって、ある時点でこの数以上の個々のオブジェクトをロックすることはできません。
このパラメータは各トランザクションで使用するオブジェクトロックの平均値を制限します。
個々のトランザクションでは、このロックテーブルにすべてのトランザクションのロックが収まる限りオブジェクトのロックを獲得できます。
...
␝       <para>␟        This controls how many pages or tuples of a single relation can be
        ...
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟リレーション全体をカバーするロックに昇格する前に、一つリレーションの中で述語ロックできるページ数あるいはタプル数を指定します。
...
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        This controls how many rows on a single page can be predicate-locked
        before the lock is promoted to covering the whole page.  The default
        is 2.  This parameter can only be set in
        the <filename>postgresql.conf</filename> file or on the server command line.␟ページ全体をカバーするロックに昇格する前に、一つページの中で述語ロックできる行数を指定します。
デフォルトは2です。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝   <sect1 id="runtime-config-compatible">␟    <title>Version and Platform Compatibility</title>␟    <title>バージョンとプラットフォーム互換性</title>␞␞␞
␝      <indexterm>
...
␝    <sect2 id="runtime-config-compatible-version">␟     <title>Previous PostgreSQL Versions</title>␟     <title>以前のPostgreSQLバージョン</title>␞␞␞
␝       <para>␟        This controls whether the array input parser recognizes
        ...
        turned <literal>off</literal>.␟これは、配列入力パーサが引用符のない<literal>NULL</literal>をNULL配列要素として認識するかどうかを制御します。
...
古い動作を必要とするアプリケーションの後方互換性のため、この変数を<literal>off</literal>にすることができます。␞␞       </para>␞
␝      <indexterm><primary>strings</primary><secondary>backslash quotes</secondary></indexterm>
␟␟      <indexterm><primary>文字列</primary><secondary>バックスラッシュによる引用</secondary></indexterm>␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Note that it is possible to create array values containing null values
        even when this variable is <literal>off</literal>.␟この変数が<literal>off</literal>であっても、NULL値を含む配列値を作成することができることに注意してください。␞␞       </para>␞
␝       <para>␟        This controls whether a quote mark can be represented by
        ...
        <literal>safe_encoding</literal> is the default setting.␟文字列リテラルの中で引用符が<literal>\'</literal>で表現されるかどうかを管理します。
...
<literal>safe_encoding</literal> がデフォルトの設定。␞␞       </para>␞
␝      <indexterm><primary>strings</primary><secondary>escape warning</secondary></indexterm>
␟␟      <indexterm><primary>文字列</primary><secondary>エスケープに関する警告</secondary></indexterm>␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Note that in a standard-conforming string literal, <literal>\</literal> just
        means <literal>\</literal> anyway.  This parameter only affects the handling of
        non-standard-conforming literals, including
        escape string syntax (<literal>E'...'</literal>).␟標準に従った文字列リテラルでは、<literal>\</literal>は単に<literal>\</literal>を意味するものです。
このパラメータのみが、エスケープ文字列構文（<literal>E'...'</literal>）を含む標準に従わないリテラルの取り扱いに影響します。␞␞       </para>␞
␝       <para>␟        When on, a warning is issued if a backslash (<literal>\</literal>)
        appears in an ordinary string literal (<literal>'...'</literal>
        syntax) and <varname>standard_conforming_strings</varname> is off.
        The default is <literal>on</literal>.␟有効の場合、通常の文字列リテラル（<literal>'...'</literal>構文）にバックスラッシュ（<literal>\</literal>）があり、<varname>standard_conforming_strings</varname>が無効な場合、警告が発せられます。
デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Applications that wish to use backslash as escape should be
        ...
        can be enabled to help locate code that needs to be changed.␟通常文字列のデフォルトの振る舞いは、標準SQLではバックスラッシュを通常文字として取り扱うため、バックスラッシュをエスケープとして使用したいアプリケーションは、エスケープ文字列構文(<literal>E'...'</literal>)を使用するように変更すべきです。
この変数は変更すべきコードを突き止めるのに役立つよう、有効にすることができます。␞␞       </para>␞
␝       <para>␟        In <productname>PostgreSQL</productname> releases prior to 9.0, large objects
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟9.0以前の<productname>PostgreSQL</productname>リリースでは、ラージオブジェクトはアクセス権限が無く、従って全てのユーザが常に読み込み、書き込みが可能でした。
...
␝       <para>␟        Setting this variable does not disable all security checks related to
        large objects &mdash; only those for which the default behavior has
        changed in <productname>PostgreSQL</productname> 9.0.␟この変数を設定しても、ラージオブジェクトに関連した全ての安全性チェックを無効にする訳ではありません。
<productname>PostgreSQL</productname> 9.0で変更されたデフォルトの動きに対してのみです。␞␞       </para>␞
␝      <indexterm><primary>strings</primary><secondary>standard conforming</secondary></indexterm>
␟␟      <indexterm><primary>文字列</primary><secondary>標準に従う</secondary></indexterm>␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        When the database generates SQL, force all identifiers to be quoted,
        ...
        <xref linkend="app-pgdump"/> and <xref linkend="app-pg-dumpall"/>.␟データベースがSQLを生成する時、たとえ（現在）キーワードになっていなくても、全ての識別子を引用符で囲むことを強制します。
これは <command>EXPLAIN</command>の出力に影響を与えるのみならず、<function>pg_get_viewdef</function>のような関数の結果にも影響します。
<xref linkend="app-pgdump"/> および <xref linkend="app-pg-dumpall"/>の<option>--quote-all-identifiers</option>オプションも参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This controls whether ordinary string literals
        ...
        backslashes to be treated as escape characters.␟標準SQLで規定されたように、通常の文字列リテラル（<literal>'...'</literal>）がバックスラッシュをそのまま取り扱うか否かを制御します。
...
エスケープ文字列構文 (<xref linkend="sql-syntax-strings-escape"/>)は、アプリケーションでバックスラッシュをエスケープ文字として扱いたい場合に使用すべきです。␞␞       </para>␞
␝       <para>␟        This allows sequential scans of large tables to synchronize with each
        ...
        is <literal>on</literal>.␟これにより、同時実行スキャンがほぼ同じ時間に同じブロックを読み取り、I/Oへの負荷を分散できるように、互いに同期して、大規模テーブルをシーケンシャルスキャンすることができます。
...
␝    <sect2 id="runtime-config-compatible-clients">␟     <title>Platform and Client Compatibility</title>␟     <title>プラットフォームとクライアント互換性</title>␞␞     <variablelist>␞
␝       <para>␟        When on, expressions of the form <literal><replaceable>expr</replaceable> =
        ...
        <literal>off</literal>.␟有効の場合、<literal><replaceable>expr</replaceable> = NULL</literal>（もしくは<literal>NULL = <replaceable>expr</replaceable></literal>）形式の式は<literal><replaceable>expr</replaceable> IS NULL</literal>として取り扱われ、それは、もし<replaceable>expr</replaceable>がNULL値と評価すれば真を返し、そうでなければ偽を返します。
<literal><replaceable>expr</replaceable> = NULL</literal>の正しいSQL仕様準拠の動作は常にNULL（判らない）を返すことです。
従って、このパラメータのデフォルトは<literal>off</literal>になっています。␞␞       </para>␞
␝       <para>␟        However, filtered forms in <productname>Microsoft
        ...
        involving null values, so this option is off by default.␟しかし、<productname>Microsoft Access</productname>のフィルタ形式はNULL値を検査するために<literal><replaceable>expr</replaceable> = NULL</literal>を使用する問い合わせを生成しますので、そのインタフェースを使用してデータベースにアクセスする場合は、このオプションを有効にする方が良いでしょう。
...
しかし、慣れていないユーザはしばしばNULL値に関する式の意味に戸惑いますので、デフォルトでこのオプションはoffです。␞␞       </para>␞
␝       <para>␟        Note that this option only affects the exact form <literal>= NULL</literal>,
        not other comparison operators or other expressions
        that are computationally equivalent to some expression
        involving the equals operator (such as <literal>IN</literal>).
        Thus, this option is not a general fix for bad programming.␟このオプションは<literal>= NULL</literal>という形式にのみ影響することに注意してください。
他の比較演算子や等価演算子を呼び出す他の（<literal>IN</literal>のような）式と計算する上で等価となる式には影響を与えません。
したがって、このオプションは間違ったプログラミングの汎用的な問題解決を行いません。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Refer to <xref linkend="functions-comparison"/> for related information.␟関連する情報は<xref linkend="functions-comparison"/>を参照してください。␞␞       </para>␞
␝       <para>␟        When <literal>allow_alter_system</literal> is set to
        ...
        the <filename>postgresql.conf</filename> file or on the server command
        line. The default value is <literal>on</literal>.␟<literal>allow_alter_system</literal>が<literal>off</literal>に設定されている場合、<command>ALTER SYSTEM</command>コマンドが実行されるとエラーが返されます。
このパラメータは、<filename>PostgreSQL.conf</filename>ファイルまたはサーバコマンドラインでのみ設定できます。
デフォルト値は<literal>on</literal>です。␞␞       </para>␞
␝       <para>␟        Note that this setting must not be regarded as a security feature. It
        ...
        this setting.␟この設定をセキュリティ機能とみなしてはならないことに注意してください。
これは<literal>ALTER SYSTEM</literal>コマンドを無効にするだけです。
スーパーユーザが他のSQLコマンドを使用して設定を変更することを妨げるものではありません。
スーパーユーザにはオペレーティングシステムレベルでシェルコマンドを実行する多くの方法があるため、この設定の値に関係なく<literal>postgresql.auto.conf</literal>を変更できます。␞␞       </para>␞
␝       <para>␟        Turning this setting off is intended for environments where the
        ...
        help avoid such mistakes.␟この設定をオフにすることは、<productname>PostgreSQL</productname>の設定が外部のツールによって管理されている環境を対象としています。
...
このパラメータを<literal>オフ</literal>に設定すると、そのような間違いを避けるための助けになります。␞␞       </para>␞
␝       <para>␟        This parameter only controls the use of <command>ALTER SYSTEM</command>.
        ...
        <literal>off</literal>.␟このパラメータは<command>ALTER SYSTEM</command>の使用のみを制御します。
<filename>postgresql.auto.conf</filename>に保存された設定は、<literal>allow_alter_system</literal>が<literal>off</literal>に設定されていても有効になります。␞␞       </para>␞
␝      <indexterm>
...
␝   <sect1 id="runtime-config-error-handling">␟    <title>Error Handling</title>␟    <title>エラー処理</title>␞␞␞
␝      <indexterm>
...
␝       <para>␟        If on, any error will terminate the current session.  By default,
        this is set to off, so that only FATAL errors will terminate the
        session.␟onなら、全てのエラーは現在のセッションを中止させます。
デフォルトではこれはoffに設定されているので、 FATALエラーのみがセッションを中止させます。␞␞       </para>␞
␝       <para>␟        When set to on, which is the default, <productname>PostgreSQL</productname>
        ...
        control and take any actions it deems appropriate.␟デフォルトであるonの場合、<productname>PostgreSQL</productname>はバックエンドのクラッシュの後、自動的に再初期化を行います。
この値を真のままにしておくことが、通常データベースの可用性を最大化する最適の方法です。
しかし、 <productname>PostgreSQL</productname>がクラスタウェアにより起動された時のような状況では、クラスタウェアが制御を獲得して、適切とみなすいかなる振る舞いをも行えるように再起動を無効にすることが有益かもしれません。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        When set to off, which is the default, <productname>PostgreSQL</productname>
        will raise a PANIC-level error on failure to flush modified data files
        to the file system.  This causes the database server to crash.  This
        parameter can only be set at server start.␟デフォルトであるoffに設定すると、<productname>PostgreSQL</productname>は変更されたデータファイルのファイルシステムへのフラッシュの失敗に対してPANICレベルのエラーを発生させます。
これによりデータベースサーバのクラッシュが引き起こされます。
このパラメータはサーバ起動時のみ設定可能です。␞␞       </para>␞
␝       <para>␟        On some operating systems, the status of data in the kernel's page
        ...
        faulty hardware.␟オペレーティングシステムによっては、カーネルキャッシュのページ内のデータの状態は、書き戻しの失敗の後は不明です。
完全に忘れられてしまった場合には、再試行することが危険であるかもしれません。
二度目の試行は成功と報告されるかもしれませんが、実際にはデータが失われているかもしれません。
このような状況では、データロスを避ける唯一の方法は、失敗が報告された後、可能ならば失敗の根本原因を調査して故障したハードウェアを交換したのち、WALからの回復することだけです。␞␞       </para>␞
␝       <indexterm>
...
␝       <para>␟        If set to on, <productname>PostgreSQL</productname> will instead
        ...
        in case of write-back failure.␟onに設定すると、代わりに<productname>PostgreSQL</productname>はエラーを報告して実行を継続し、後のチェックポイントでデータのフラッシュをリトライします。
書き戻しの失敗が起きたときのオペレーティングシステムのバッファデータの扱いを調査した後でのみonに設定してください。␞␞       </para>␞
␝       <para>␟        When set to <literal>fsync</literal>, which is the default,
        ...
        copies created with <application>pg_basebackup</application>.␟デフォルトである<literal>fsync</literal>に設定すると、<productname>PostgreSQL</productname>はクラッシュリカバリを開始する前に、再帰的にデータディレクトリ内のすべてのファイルを開いて同期します。
...
これは、<application>pg_basebackup</application>で作られた複製も含めて、正しく停止されなかったデータベースクラスタを起動する際には必ず適用されます。␞␞       </para>␞
␝       <para>␟        On Linux, <literal>syncfs</literal> may be used instead, to ask the
        ...
        appear only in kernel logs.␟Linuxでは代わりに、オペレーティングシステムに対して、データディレクトリ、WALファイル、各々のテーブル空間（しかし、シンボリックリンクを通じて到達可能な他のファイルシステムを含みません）を含むファイルシステムを同期することを依頼する<literal>syncfs</literal>が使えるかもしれません。
...
更に、Linuxのバージョン5.8以前では、ディスクへの書き込み中に発生したI/Oエラーが<productname>PostgreSQL</productname>に報告されないことがあり、関連エラーメッセージはカーネルログにのみ現れるかもしれません。␞␞       </para>␞
␝       <para>␟        This parameter can only be set in the
        <filename>postgresql.conf</filename> file or on the server command line.␟このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝   <sect1 id="runtime-config-preset">␟    <title>Preset Options</title>␟    <title>設定済みのオプション</title>␞␞␞
␝      <indexterm>
...
␝    <para>␟     The following <quote>parameters</quote> are read-only.
     ...
     is compiled or when it is installed.␟以下の<quote>パラメータ</quote>は読み取り専用です。
...
これらのうち多くは<productname>PostgreSQL</productname>のコンパイル時、もしくはインストール時に決定されます。␞␞    </para>␞
␝      <indexterm>
...
␝       <para>␟        Reports the size of a disk block.  It is determined by the value
        ...
        linkend="runtime-config-resource"/> for information.␟ディスクブロックの容量を報告します。
...
これに関しての情報は<xref linkend="runtime-config-resource"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Reports whether data checksums are enabled for this cluster.
        See <xref linkend="app-initdb-data-checksums"/> for more information.␟このクラスタでデータチェックサムが有効になっているかどうかを報告します。
詳細は<xref linkend="app-initdb-data-checksums"/>を見てください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        On Unix systems this parameter reports the permissions the data
        ...
        <xref linkend="app-initdb-allow-group-access"/> for more information.␟このパラメータは、Unixシステムでは起動時のデータディレクトリのパーミッション（<xref linkend="guc-data-directory"/>により定義されます）を報告します。
（Microsoft Windowsではこのパラメータは常に<literal>0700</literal>を表示します。）
さらなる情報は<xref linkend="app-initdb-allow-group-access"/>をご覧ください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Reports whether <productname>PostgreSQL</productname> has been built
        ...
        assertions.␟<productname>PostgreSQL</productname>がアサーションを有効にしてビルドされているかどうかを報告します。
...
デフォルトでは<productname>PostgreSQL</productname>は、アサーションなしにビルドされます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Reports the state of huge pages in the current instance:
        ...
        See <xref linkend="guc-huge-pages"/> for more information.␟現在インスタンスのhuge pagesの状態を報告します:<literal>on</literal>、<literal>off</literal>、または<literal>unknown</literal>のどれかです（<literal>postgres -C</literal>で表示される場合）。
このパラメータは、<literal>huge_pages=try</literal>でhuge pagesのアロケーションが成功したかどうかを判断するのに役立ちます。
詳細については、<xref linkend="guc-huge-pages"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Reports whether <productname>PostgreSQL</productname> was built with support for
        64-bit-integer dates and times.  As of <productname>PostgreSQL</productname> 10,
        this is always <literal>on</literal>.␟<productname>PostgreSQL</productname>が64ビット整数による日付と時刻のサポート付きで構築されたかどうかを報告します。
<productname>PostgreSQL</productname> 10では、これは常に<literal>on</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Reports whether the server is currently in hot standby mode.  When
...
␝       <para>␟        Reports the maximum number of function arguments. It is determined by
        the value of <literal>FUNC_MAX_ARGS</literal> when building the server. The
        default value is 100 arguments.␟関数の引数の最大数を報告します。
サーバを構築する時、<literal>FUNC_MAX_ARGS</literal>の値で決定されます。
デフォルトの値は100引数です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Reports the maximum identifier length. It is determined as one
        ...
        can be less than 63 characters when using multibyte encodings.␟最長の識別子の長さを報告します。
サーバ構築時の<literal>NAMEDATALEN</literal>の値より一つ少なく設定されます。
デフォルトの<literal>NAMEDATALEN</literal>の値は64ですので、デフォルトの<varname>max_identifier_length</varname>は63バイトで、マルチバイト符号化方式を使用している場合、63文字以下になることがあります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Reports the maximum number of index keys. It is determined by
        the value of <literal>INDEX_MAX_KEYS</literal> when building the server. The
        default value is 32 keys.␟インデックスキーの最大数を報告します。サーバをビルドする際に<literal>INDEX_MAX_KEYS</literal>の値で決定されます。デフォルトの値は32キーです。␞␞       </para>␞
␝      <indexterm>
...
␝      <indexterm><primary>character set</primary></indexterm>
␟␟      <indexterm><primary>文字セット</primary></indexterm>␞␞       </para>␞
␝       <para>␟        Reports the number of blocks (pages) that can be stored within a file
        ...
        <varname>block_size</varname>; by default this is 1GB.␟あるファイルセグメントの中に格納できるブロック数（ページ数）を報告します。
サーバ構築時に<literal>RELSEG_SIZE</literal>の値で決定されます。
バイト単位の一セグメントファイルの最大容量は、<varname>block_size</varname>倍の<varname>segment_size</varname>と等しくなります。デフォルトでは1GBです。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Reports the database encoding (character set).
        It is determined when the database is created.  Ordinarily,
        clients need only be concerned with the value of <xref
        linkend="guc-client-encoding"/>.␟データベース符号化方式（文字セット）を報告します。
データベースが作成された時に決定されます。通常クライアントは<xref linkend="guc-client-encoding"/>の値にのみ注意する必要があります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Reports the version number of the server. It is determined by the
        value of <literal>PG_VERSION</literal> when building the server.␟サーバのバージョン番号を報告します。
サーバ構築の際の<literal>PG_VERSION</literal>の値によって決定されます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Reports the version number of the server as an integer. It is determined
        by the value of <literal>PG_VERSION_NUM</literal> when building the server.␟サーバのバージョン番号を整数として返します。
この値は、サーバ構築時の<literal>PG_VERSION_NUM</literal>の値により決まります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Reports the size of the main shared memory area, rounded up to the
        nearest megabyte.␟主共有メモリ領域のサイズを報告します。
最も近いメガバイト単位に切り上げられます。␞␞       </para>␞
␝       <para>␟        Reports the number of huge pages that are needed for the main shared
        memory area based on the specified <xref linkend="guc-huge-page-size"/>.
        If huge pages are not supported, this will be <literal>-1</literal>.␟指定された<xref linkend="guc-huge-page-size"/>に基づいて、主共有メモリ領域に必要な巨大ページの数を報告します。
巨大ページがサポートされていない場合、<literal>-1</literal>になります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This setting is supported only on <productname>Linux</productname>.  It
        ...
        <xref linkend="linux-huge-pages"/>.␟この設定は<productname>Linux</productname>でのみサポートされています。
他のプラットフォームでは常に<literal>-1</literal>に設定されます。
<productname>Linux</productname>で巨大なページを使うための詳細は<xref linkend="linux-huge-pages"/>を参照してください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Reports the name of the SSL library that this
        ...
        example <literal>OpenSSL</literal>, or an empty string if none.␟（このインスタンスでSSLが設定あるいは使用されていなくても）この<productname>PostgreSQL</productname>サーバの構築時に使用されたSSLライブラリの名前、たとえば<literal>OpenSSL</literal>あるいは空文字列、を報告します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Reports the size of a WAL disk block.  It is determined by the value
        of <literal>XLOG_BLCKSZ</literal> when building the server. The default value
        is 8192 bytes.␟WALディスクブロックの容量を報告します。
サーバ構築時に<literal>XLOG_BLCKSZ</literal>の値で決定されます。デフォルトの値は8192バイトです。␞␞       </para>␞
␝       <para>␟        Reports the size of write ahead log segments.  The default value is
        16MB. See <xref linkend="wal-configuration"/> for more information.␟先行書き込みログ（WAL）のセグメントの大きさを報告します。
デフォルト値は16MBです。
さらなる詳細については<xref linkend="wal-configuration"/>をご覧ください。␞␞       </para>␞
␝   <sect1 id="runtime-config-custom">␟    <title>Customized Options</title>␟    <title>独自のオプション</title>␞␞␞
␝    <para>␟     This feature was designed to allow parameters not normally known to
     ...
     configured in the standard ways.␟この機能は追加モジュール（手続き言語など）によって追加される<productname>PostgreSQL</productname>が識別できないパラメータを使えるように設計されたものです。
これにより拡張モジュールは標準の方法で構成されます。␞␞    </para>␞
␝    <para>␟     Custom options have two-part names: an extension name, then a dot, then
     the parameter name proper, much like qualified names in SQL.  An example
     is <literal>plpgsql.variable_conflict</literal>.␟カスタムオプションには２つに分かれた名称があります。拡張名につづいてドット、そして特定のパラメータ名です。SQLの修飾名に良く似ています。
例として<literal>plpgsql.variable_conflict</literal>が挙げられます。␞␞    </para>␞
␝    <para>␟     Because custom options may need to be set in processes that have not
     ...
     placeholders are removed.␟カスタムオプションは読み込まれていない関連性のある拡張モジュールのプロセスに設定される必要がある場合があるので、<productname>PostgreSQL</productname>はどんな２つの部分のパラメータ名による設定を受け付けます。これらの変数は代替物として取り扱われ、それらを定義したモジュールが読み込まれるまで機能しません。
拡張モジュールが読み込まれた時、その変数定義が追加され、それら定義に基づいてプレースホルダ値が変換されます。
拡張名で始まる認識できないプレースホルダに対しては警告が発せられ、そのプレースホルダは削除されます。␞␞    </para>␞
␝   <sect1 id="runtime-config-developer">␟    <title>Developer Options</title>␟    <title>開発者向けオプション</title>␞␞␞
␝      <indexterm>
...
␝    <para>␟     The following parameters are intended for developer testing, and
     ...
     parameters require special source compilation flags to work at all.␟以下のパラメータは、開発者のテスト用であり、決して実運用のデータベースに使わないでください。
しかし、中には深刻な損傷を負ったデータベースの復旧に役立つものもあります。
したがって、これらはサンプルの<filename>postgresql.conf</filename>からは除外されています。
これらのパラメータの多くは、それを動作させるために特殊なソースコンパイルを必要としていることに注意してください。␞␞    </para>␞
␝      <indexterm>
...
␝       <para>␟        Allows tablespaces to be created as directories inside
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟<command>CREATE TABLESPACE</command>コマンドに空の位置文字列が渡された時に、<filename>pg_tblspc</filename>内にテーブル空間をディレクトリとして作ることを可能にします。
...
␝       <para>␟        Allows modification of the structure of system tables as well as
        ...
        system.
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟システムテーブルの構造変更とその他の危険性の高いシステムテーブルに対するアクションを許可します。
これは通常スーパーユーザにさえ許可されません。
この設定の無分別な使用は回復不能なデータ喪失やデータベースシステムの重大な破損を招きます。
スーパーユーザと、適切な<literal>SET</literal>権限を持つユーザのみがこの設定を変更することができます。␞␞       </para>␞
␝       <para>␟        This parameter contains a comma-separated list of C function names.
        ...
        be used to debug specific areas of the source code.␟このパラメータはカンマ区切りのC関数名を含みます。
エラーが発生し、エラー発生箇所の内部C関数がこのリストの値と一致すると、エラーメッセージとともにバックトレースが一緒にサーバログに書かれます。
これはソースコードの特定箇所をデバッグするのに役立ちます。␞␞       </para>␞
␝       <para>␟        Backtrace support is not available on all platforms, and the quality
        of the backtraces depends on compilation options.␟バックトレースのサポートはすべてのプラットフォームで提供されているわけではありませんし、バックトレースの品質はコンパイルオプションに依存します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟この設定を変更できるのは、スーパーユーザと適切な<literal>SET</literal>権限を持つユーザだけです。␞␞       </para>␞
␝       <para>␟        When set to <literal>1</literal>, each system catalog cache entry is
        ...
        selects normal catalog caching behavior.␟<literal>1</literal>なら、可能な最初の機会にシステムカタログのキャッシュ項目が破棄されます。
...
デフォルト値の<literal>0</literal>で正常なカタログキャッシュは通常の動作になります。␞␞       </para>␞
␝       <para>␟        This parameter can be very helpful when trying to trigger
        ...
        <filename>pg_config_manual.h</filename> for details.␟このパラメータは並行的なカタログの変更を伴う再現しにくいバグを引き起こす際にとても役に立ちますが、それ以外にはめったに必要になりません。
詳細は<filename>inval.c</filename>と<filename>pg_config_manual.h</filename>のソースコードファイルを見てください。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter is supported when
        ...
        value will raise an error.␟このパラメータはコンパイル時に<symbol>DISCARD_CACHES_ENABLED</symbol>が定義されたとき（これは<application>configure</application>オプションの<option>--enable-cassert</option>が使われたときに自動的に起こります）にサポートされます。
実運用のビルドでは<literal>0</literal>となり、それ以外の値に設定しようとするとエラーが起こります。␞␞       </para>␞
␝       <para>␟        Ask the kernel to minimize caching effects for relation data and WAL
        files using <literal>O_DIRECT</literal> (most Unix-like systems),
        <literal>F_NOCACHE</literal> (macOS) or
        <literal>FILE_FLAG_NO_BUFFERING</literal> (Windows).␟リレーションデータとWALファイルに対して、<literal>O_DIRECT</literal>（ほとんどのUnixライクなシステム）、<literal>F_NOCACHE</literal>（macOS）、<literal>FILE_FLAG_NO_BUFFERING</literal> （Windows）を使って、キャッシュ効果を最小にするようカーネルに依頼します。␞␞       </para>␞
␝       <para>␟        May be set to an empty string (the default) to disable use of direct
        ...
        allocated.␟空文字列（デフォルト）を設定してダイレクトI/Oの使用を無効にすることも、カンマで区切られたリストを設定してダイレクトI/Oを使用するようにすることもできます。
有効なオプションは、メインデータファイルの場合は<literal>data</literal>、WALファイルの場合は<literal>wal</literal>、最初に割り当てられたWALファイルの場合は<literal>wal_init</literal>です。␞␞       </para>␞
␝       <para>␟        Some operating systems and file systems do not support direct I/O, so
        non-default settings may be rejected at startup or cause errors.␟一部のオペレーティングシステムおよびファイルシステムでは、ダイレクトI/Oが行われないため、デフォルト以外の設定が起動時に拒否されたり、エラーが発生したりする場合があります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Currently this feature reduces performance, and is intended for
        developer testing only.␟今の所、この機能はパフォーマンスを悪くするので、開発者のテストのみを目的としています。␞␞       </para>␞
␝       <para>␟        Allows the use of parallel queries for testing purposes even in cases
        ...
        below).␟性能改善が期待できなくても、テスト目的のためにパラレルクエリを利用できるようにします。
<varname>debug_parallel_query</varname>に設定できる値は、<literal>off</literal>（性能改善が期待できるときにだけパラレルクエリを使用する）、<literal>on</literal>（安全なクエリに対しては常にパラレルクエリを強制する）、<literal>regress</literal>（<literal>on</literal>と同様だが、下記のような振る舞いの変更を伴う）です。␞␞       </para>␞
␝       <para>␟        More specifically, setting this value to <literal>on</literal> will add
        ...
        (or, possibly, <literal>PARALLEL RESTRICTED</literal>).␟正確に言えば、この値を<literal>on</literal>にすると、安全と見なされるすべての問い合わせ計画の上に<literal>Gather</literal>ノードを追加し、クエリをパラレルワーカー上で実行するようにします。
プランナがこれによってクエリが失敗すると思わない限り、パラレルワーカーが利用できない、あるいは使用できないような場合でも、たとえばサブトランザクションの開始のように、パラレルクエリコンテキストでは許可されない操作は不許可となります。
このオプションを設定することによって、エラーとなったり、あるいは期待していなかった結果がもたらされる場合には、クエリで使用されている関数は<literal>PARALLEL UNSAFE</literal>（もしくは、<literal>PARALLEL RESTRICTED</literal>）と印を付ける必要があるかもしません。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Setting this value to <literal>regress</literal> has all of the same effects
        ...
        would be obtained if this setting were turned <literal>off</literal>.␟この設定値を<literal>regress</literal>とすると、<literal>on</literal>とするのに加え、自動リグレッションテストを助けるための付加的な効果が現れます。
通常パラレルワーカーからのメッセージは、そのことを表すコンテキスト行を表示しますが、<literal>regress</literal>と設定すると、非パラレル実行と同じ出力になるように、これを抑止します。
また、プランに追加された<literal>Gather</literal>ノードは、<literal>EXPLAIN</literal>出力から隠され、<literal>off</literal>に設定したときと同じ出力が得られるようにします。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Ignore system indexes when reading system tables (but still
        update the indexes when modifying the tables).  This is useful
        when recovering from damaged system indexes.
        This parameter cannot be changed after session start.␟システムテーブルの読み込み時にシステムインデックスを無視します（しかしテーブルが更新された時はインデックスを更新します）。
障害があるシステムインデックスを復旧する時、これは有用です。
セッションが始まった後に、このパラメータを変更することはできません。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The amount of time to delay when a new
        ...
        This parameter cannot be changed after session start.␟サーバプロセスが始まり認証手続きが終わった後の遅延時間です。
...
␝       <para>␟        The amount of time to delay just after a
        ...
        A value of zero (the default) disables the delay.
        This parameter can only be set in the <filename>postgresql.conf</filename>
        file or on the server command line.␟新しくサーバプロセスがforkした後、認証手続きに入る前の遅延時間です。
...
␝       <para>␟        Generates a great amount of debugging output for the
        ...
        client or server logs, respectively.␟<command>LISTEN</command>と<command>NOTIFY</command>コマンドのための大量なデバッグ出力を生成します。
この出力をクライアントもしくはサーバログに送信するためには、それぞれ、<xref linkend="guc-client-min-messages"/>もしくは<xref linkend="guc-log-min-messages"/>は<literal>DEBUG1</literal>以下でなければなりません。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        If on, emit information about resource usage during sort operations.
        ...
        (However, <symbol>TRACE_SORT</symbol> is currently defined by default.)␟もしも有効であれば、並べ替え操作の間のリソース使用についての情報を放出します。
このパラメータは <productname>PostgreSQL</productname>がコンパイルされた時、<symbol>TRACE_SORT</symbol>マクロが定義されている場合にのみ有効です。
（とは言っても、現在<symbol>TRACE_SORT</symbol>はデフォルトで定義されています。）␞␞       </para>␞
␝       <para>␟        If on, emit information about lock usage.  Information dumped
        ...
        is shown here:␟有効な場合、ロックの使用状況に関する情報を出力します。
...
ログファイル出力例を以下に示します。␞␞<screen>␞
␝</screen>␟        Details of the structure being dumped may be found in
        <filename>src/include/storage/lock.h</filename>.␟ダンプされる構造の詳細は、<filename>src/include/storage/lock.h</filename> にあります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter is only available if the <symbol>LOCK_DEBUG</symbol>
        macro was defined when <productname>PostgreSQL</productname> was
        compiled.␟このパラメータは<productname>PostgreSQL</productname>がコンパイル時に<symbol>LOCK_DEBUG</symbol>マクロが定義された場合のみ有効です。␞␞       </para>␞
␝       <para>␟        If on, emit information about lightweight lock usage.  Lightweight
        ...
        to shared-memory data structures.␟有効な場合、軽量ロックの使用状況に関する情報を出力します。
軽量ロックは主に、共有メモリ上のデータ構造へのアクセスに関する排他制御機能を提供することを意図したものです。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter is only available if the <symbol>LOCK_DEBUG</symbol>
        macro was defined when <productname>PostgreSQL</productname> was
        compiled.␟このパラメータは<productname>PostgreSQL</productname>がコンパイル時に<symbol>LOCK_DEBUG</symbol>マクロが定義された場合のみ有効です。␞␞       </para>␞
␝       <para>␟        If on, emit information about user lock usage.  Output is the same
        as for <symbol>trace_locks</symbol>, only for advisory locks.␟有効な場合、ユーザロックの使用状況に関する情報を出力します。
出力は<symbol>trace_locks</symbol>と同じですが、勧告的ロックに関するもののみを出力します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter is only available if the <symbol>LOCK_DEBUG</symbol>
        macro was defined when <productname>PostgreSQL</productname> was
        compiled.␟このパラメータは<productname>PostgreSQL</productname>がコンパイル時に<symbol>LOCK_DEBUG</symbol>マクロが定義された場合のみ有効です。␞␞       </para>␞
␝       <para>␟        If set, do not trace locks for tables below this OID (used to avoid
        output on system tables).␟設定すると、このOID未満のテーブルに関するロックの追跡を行いません。
（システムテーブルに関する出力を抑えるために使用します。）␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter is only available if the <symbol>LOCK_DEBUG</symbol>
        macro was defined when <productname>PostgreSQL</productname> was
        compiled.␟このパラメータは<productname>PostgreSQL</productname>がコンパイル時に<symbol>LOCK_DEBUG</symbol>マクロが定義された場合のみ有効です。␞␞       </para>␞
␝       <para>␟        Unconditionally trace locks on this table (OID).␟このテーブル（OID）に対し無条件でロックを追跡します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter is only available if the <symbol>LOCK_DEBUG</symbol>
        macro was defined when <productname>PostgreSQL</productname> was
        compiled.␟このパラメータは<productname>PostgreSQL</productname>がコンパイル時に<symbol>LOCK_DEBUG</symbol>マクロが定義された場合のみ有効です。␞␞       </para>␞
␝       <para>␟        If set, dumps information about all current locks when a
        deadlock timeout occurs.␟設定すると、デッドロックタイムアウトが発生した時全ての進行中のロックについての情報がダンプされます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter is only available if the <symbol>LOCK_DEBUG</symbol>
        macro was defined when <productname>PostgreSQL</productname> was
        compiled.␟このパラメータは<productname>PostgreSQL</productname>がコンパイル時に<symbol>LOCK_DEBUG</symbol>マクロが定義された場合のみ有効です。␞␞       </para>␞
␝       <para>␟        If set, logs system resource usage statistics (memory and CPU) on
        various B-tree operations.␟設定すると、各種B-tree操作に関するシステムリソース（メモリとCPU）の使用についての統計情報をログに出力します。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        This parameter is only available if the <symbol>BTREE_BUILD_STATS</symbol>
        macro was defined when <productname>PostgreSQL</productname> was
        compiled.␟このパラメータは<productname>PostgreSQL</productname>がコンパイル時に<symbol>BTREE_BUILD_STATS</symbol>マクロが定義された場合のみ有効です。␞␞       </para>␞
␝       <para>␟        This parameter is intended to be used to check for bugs in the WAL
        ...
        differences will result in a fatal error, terminating recovery.␟このパラメータは、WALのREDOルーチンのバグをチェックするために使うことを意図しています。
...
予期しない差異は、致命的エラーを引き起こし、リカバリが中断されます。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        The default value of this setting is the empty string, which disables
        ...
        the appropriate <literal>SET</literal> privilege can change this setting.␟デフォルト値は空文字で、この機能を無効にします。
...
␝       <para>␟        If on, emit WAL-related debugging output. This parameter is
        only available if the <symbol>WAL_DEBUG</symbol> macro was
        defined when <productname>PostgreSQL</productname> was
        compiled.␟もしonであれば、WALに関連したデバッグ出力が有効になります。このパラメータは<symbol>WAL_DEBUG</symbol>マクロが <productname>PostgreSQL</productname>のコンパイルの時に定義された場合にのみ有効です。␞␞       </para>␞
␝       <para>␟        Only has effect if <xref linkend="app-initdb-data-checksums"/> are enabled.␟<xref linkend="app-initdb-data-checksums"/>が有効の時のみ効果があります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        Detection of a checksum failure during a read normally causes
        ...
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟読み込み過程でチェックサム障害が検出されると、通常<productname>PostgreSQL</productname>はエラーを報告し、現時点のトランザクションを停止します。
...
␝       <para>␟        Detection of a damaged page header normally causes
        <productname>PostgreSQL</productname> to report an error, aborting the current
        ...
        default setting is <literal>off</literal>.
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟ページヘッダの障害がわかると、通常<productname>PostgreSQL</productname>はエラーの報告を行い、現在のトランザクションを中断させます。
...
␝       <para>␟        If set to <literal>off</literal> (the default), detection of
        ...
        during recovery or in standby mode.␟<literal>off</literal>（デフォルトです）に設定すると、無効なページを参照しているWALレコードは<productname>PostgreSQL</productname>に対してPANICレベルのエラーを引き起こし、リカバリをアボートします。
...
リカバリ中、あるいはスタンバイモードでのみ効果があります。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        If LLVM has the required functionality, register generated functions
        with <productname>GDB</productname>.  This makes debugging easier.
        The default setting is <literal>off</literal>.
        This parameter can only be set at server start.␟LLVMに要求された機能がある場合は、生成した関数を<productname>GDB</productname>用に登録します。
これにより、デバッグが容易になります。
...
␝       <para>␟        Writes the generated <productname>LLVM</productname> IR out to the
        ...
        The default setting is <literal>off</literal>.
        Only superusers and users with the appropriate <literal>SET</literal>
        privilege can change this setting.␟生成された<productname>LLVM</productname> IRを<xref linkend="guc-data-directory"/>内のファイルシステムに出力します。
...
␝       <para>␟        Determines whether expressions are JIT compiled, when JIT compilation
        is activated (see <xref linkend="jit-decision"/>).  The default is
        <literal>on</literal>.␟JITコンパイルが有効な時に式がJITコンパイルされるかどうかを決定します。
...
␝       <para>␟        If LLVM has the required functionality, emit the data needed to allow
        ...
        The default setting is <literal>off</literal>.
        This parameter can only be set at server start.␟LLVMに要求された機能がある場合は、JITが生成した関数を<productname>perf</productname>でプロファイルすることができるデータを出力します。
...
␝       <para>␟        Determines whether tuple deforming is JIT compiled, when JIT
        compilation is activated (see <xref linkend="jit-decision"/>).
        The default is <literal>on</literal>.␟JITコンパイルが有効な時にタプルデフォーミングがJITコンパイルされるかどうかを決定します。
（<xref linkend="jit-decision"/>参照。）
デフォルトは<literal>on</literal>です。␞␞       </para>␞
␝      <indexterm>
...
␝       <para>␟        When set to <literal>on</literal>, which is the default,
        ...
        the server command line.␟デフォルトである<literal>on</literal>に設定すると、<productname>PostgreSQL</productname>はバックエンドがクラッシュした後に自動的に一時ファイルを削除します。
...
␝       <para>␟        By default, after a backend crash the postmaster will stop remaining
        ...
        This parameter can only be set in
        the <filename>postgresql.conf</filename> file or on the server
        command line.␟デフォルトでは、バックエンドクラッシュの後、postmasterは残りの子プロセスを<systemitem>SIGQUIT</systemitem>シグナルを送ることで停止します。
...
␝       <para>␟        By default, after attempting to stop a child process with
        ...
        This parameter can only be set in
        the <filename>postgresql.conf</filename> file or on the server
        command line.␟デフォルトでは、<systemitem>SIGQUIT</systemitem>で子プロセスを停止しようとした後、postmasterは5秒間待ってから<systemitem>SIGKILL</systemitem>を送信して即時終了を強制します。
...
自動的にコアファイルを削除する機能は提供されていないことに注意してください。
このパラメータは、<filename>postgresql.conf</filename>ファイルか、サーバのコマンドラインでのみ設定可能です。␞␞       </para>␞
␝       <para>␟        The allowed values are <literal>buffered</literal> and
        ...
        publisher and subscriber:␟指定できる値は、<literal>buffered</literal>および<literal>immediate</literal>です。
デフォルトは<literal>buffered</literal>です。
このパラメータは、大きなトランザクションにおけるロジカルデコーディングおよびレプリケーションのテストに使用することを目的としています。
<varname>debug_logical_replication_streaming</varname>の効果は、パブリッシャーとサブスクライバーで異なります。␞␞       </para>␞
␝       <para>␟        On the publisher side, <varname>debug_logical_replication_streaming</varname>
        ...
        changes when <varname>logical_decoding_work_mem</varname> is reached.␟パブリッシャー側では、<varname>debug_logical_replication_streaming</varname>を使用すると、ロジカルデコーディングでただちに変更をストリーミングまたはシリアル化できます。
...
<literal>buffered</literal>に設定すると、<varname>logical_decoding_work_mem</varname>に達したときに、デコーディング処理がストリームまたはシリアル化します。␞␞       </para>␞
␝       <para>␟        On the subscriber side, if the <literal>streaming</literal> option is set to
        ...
        end of the transaction.␟サブスクライバー側では、<literal>streaming</literal>オプションが<literal>parallel</literal>に設定されている場合、<varname>debug_logical_replication_streaming</varname>を使用して、リーダー適用ワーカーに変更を共有メモリキューに送信するか、すべての変更をファイルにシリアル化するように指示できます。
<literal>buffered</literal>に設定されている場合、リーダーは共有メモリキューを介してパラレル適用ワーカーに変更を送信します。
<literal>immediate</literal>に設定されている場合、リーダーはファイルに対するすべての変更をシリアル化し、パラレル適用ワーカーにファイルを読み取ってトランザクションの最後に適用するように通知します。␞␞       </para>␞
␝  <sect1 id="runtime-config-short">␟   <title>Short Options</title>␟   <title>短いオプション</title>␞␞␞
␝   <para>␟    For convenience there are also single letter command-line option
    ...
    to use the option heavily.␟簡便性のために、一文字のコマンドラインオプションスイッチも、幾つかのパラメータのために用意されています。
それらは<xref linkend="runtime-config-short-table"/>に解説されています。
一部のオプションは歴史的な理由のために存在します。
また、この一文字オプションが存在することが、このオプションを多く使用することを支持することを示しているわけではありません。␞␞   </para>␞
␝    <table id="runtime-config-short-table">␟     <title>Short Option Key</title>␟     <title>短いオプションキー</title>␞␞     <tgroup cols="2">␞
␝       <row>␟        <entry>Short Option</entry>␟        <entry>短いオプション</entry>␞␞␞
␝      </tbody>
     </tgroup>
    </table>

  </sect1>
␟␟<!-- split-config3-end -->␞␞</chapter>␞
␝        <entry>Short Option</entry>␟        <entry>Equivalent</entry>␟        <entry>同義</entry>␞␞       </row>␞
