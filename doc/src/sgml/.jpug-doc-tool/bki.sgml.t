␝<chapter id="bki">␟ <title>System Catalog Declarations and Initial Contents</title>␟ <title>システムカタログの宣言と初期内容</title>␞␞␞
␝ <para>␟  <productname>PostgreSQL</productname> uses many different system catalogs
  ...
  of the catalogs can change between major versions.␟<productname>PostgreSQL</productname>は、テーブルや関数のようなデータベースオブジェクトの存在の有無と特性を追跡するために、多くの異なるシステムカタログを使用します。
...
とはいえ、カタログの構造はメジャーバージョン間で変更されることがあります。␞␞ </para>␞
␝ <para>␟  The structures of the catalogs are declared in specially formatted C
  ...
  the catalog has, as well as some other basic properties such as its OID.␟カタログの構造は、ソースツリーの<filename>src/include/catalog/</filename>ディレクトリの中の特殊な形式のCヘッダファイルに宣言されています。
個々のカタログに対応して、カタログと同じ名前のヘッダファイルが存在し（たとえば、<structname>pg_class</structname>に対して<filename>pg_class.h</filename>というように）、カタログが持つ一連の列やOIDのような基本的な特性を定義しています。␞␞ </para>␞
␝ <para>␟  Many of the catalogs have initial data that must be loaded into them
  ...
  be inserted into the <structname>pg_proc</structname> catalog.␟SQLコマンドを実行可能な状態にまでシステムを持っていくために、多くのカタログは<application>initdb</application>の<quote>bootstrap</quote>フェーズで読み込まなければならない初期データを持っています。
...
たとえば、<filename>pg_proc.dat</filename>は、<structname>pg_proc</structname>カタログに挿入されるべき初期の行を記述しています。␞␞ </para>␞
␝ <para>␟  To create the catalog files and load this initial data into them, a
  ...
  installation tree.␟カタログファイルを作り初期データをそこにロードするために、ブートストラップモードで実行中のバックエンドは、コマンドと初期データを含む<acronym>BKI</acronym>（Backend Interface: バックエンドインタフェース）ファイルを読み込みます。
このモードで使用される<filename>postgres.bki</filename>は、<filename>genbki.pl</filename>というPerlスクリプトを使って、<productname>PostgreSQL</productname>ディストリビューションを構築する過程で前述のヘッダとデータファイルから作成されます。
<filename>postgres.bki</filename>は<productname>PostgreSQL</productname>の特定のリリースに固有のものですが、プラットフォームからは独立しており、インストレーションツリーの<filename>share</filename>サブディレクトリにインストールされます。␞␞ </para>␞
␝ <para>␟  <filename>genbki.pl</filename> also produces a derived header file for
  ...
  reads a particular catalog.␟<filename>genbki.pl</filename>は、他にも各々のカタログで使用する、たとえば<structname>pg_class</structname>のための<filename>pg_class_d.h</filename>のような派生ファイルを生成します。
このファイルには自動生成されたマクロ定義が含まれているほか、他のマクロとenum宣言も含まれており、特定のカタログを読み込むクライアントCコードに役立ちます。␞␞ </para>␞
␝ <para>␟  Most PostgreSQL developers don't need to be directly concerned with
  ...
  the <acronym>BKI</acronym> file format.␟ほとんどのPostgreSQL開発者は直接<acronym>BKI</acronym>ファイルのことを気にかける必要はありませんが、バックエンドに些細ではない機能を追加する際にはカタログヘッダファイル、あるいはまた初期データファイルの変更が必要になるでしょう。
この章の残りの部分でそれについての情報をお届けします。
また網羅性のために、<acronym>BKI</acronym>ファイルのフォーマットも説明します。␞␞ </para>␞
␝ <sect1 id="system-catalog-declarations">␟  <title>System Catalog Declaration Rules</title>␟  <title>システムカタログの宣言ルール</title>␞␞␞
␝  <para>␟   The key part of a catalog header file is a C structure definition
   ...
   it is a shared relation.␟カタログヘッダファイルの肝心な部分は、カタログにおける行の配置を記述するC構造体定義です。
...
<filename>genbki.h</filename>に記述されたBKIプロパティマクロを使って、共有リレーションであるかどうかといった、そのカタログ全体のプロパティを定義することができます。␞␞  </para>␞
␝  <para>␟   The system catalog cache code (and most catalog-munging code in general)
   ...
   random errors or even segmentation violations.␟システムカタログキャッシュのコード（そして一般的にたいていのカタログを触るコード）は、すべてのシステムカタログタプルに固定長部分が実際に存在するとみなします。
...
これはランダムなエラーとなるか、あるいはセグメンテーション違反にすらなってしまいます。␞␞  </para>␞
␝  <para>␟   As a partial guard against this type of error, variable-length or
   ...
   and <literal>BKI_FORCE_NULL</literal> annotations as needed.␟この種のエラーから部分的に身を守るためには、可変長あるいはNULLを許容するフィールドはCコンパイラから直接見えないようにすべきです。
...
不正な行を作るのを防ぐ独立したガードとして、NULLを許容しないすべての列を<structname>pg_attribute</structname>でそのように宣言することを要求します。
ブートストラップコードは、固定長で、かつ、NULLを許容したり可変幅である列の次ではないカタログ列に対して自動的に<literal>NOT NULL</literal>のマークを付けます。
このルールが不適切なら、<literal>BKI_FORCE_NOT_NULL</literal>と<literal>BKI_FORCE_NULL</literal>を必要に応じて使ってマーキングを修正できます。␞␞  </para>␞
␝  <para>␟   Frontend code should not include any <filename>pg_xxx.h</filename>
   ...
   to the <filename>pg_xxx_d.h</filename> header.␟フロントエンドのコードはすべての<filename>pg_xxx.h</filename>カタログヘッダファイルをincludeすべきではありません。
バックエンド以外ではコンパイルできないCコードを含んでいるかもしれないからです。
（典型的には、<filename>src/backend/catalog/</filename>ファイル中に関数宣言を含んでいることによって起こります。）
その代わりに、フロントエンドは生成された<filename>pg_xxx_d.h</filename>ヘッダをincludeできます。
このファイルは、OIDの<literal>#define</literal>と、クライアント側で必要になるデータを含んでいます。
カタログヘッダ中のマクロやその他のコードをフロントエンドから見えるようにしたい場合は、<literal>#ifdef EXPOSE_TO_CLIENT_CODE</literal> ... <literal>#endif</literal>で該当セクションを囲むことにより、<filename>genbki.pl</filename>がそのセクションを<filename>pg_xxx_d.h</filename>にコピーするように指示してください。␞␞  </para>␞
␝  <para>␟   A few of the catalogs are so fundamental that they can't even be created
   ...
   Avoid making new catalogs be bootstrap catalogs if at all possible.␟少数のカタログは非常に基本的なものなので、ほとんどのカタログで使用される<acronym>BKI</acronym> <literal>create</literal>コマンドですら作成できません。そのコマンドが、新しいカタログの記述をこれらのカタログに書き込む必要があるからです。
...
可能ならば、新しいカタログをブートストラップカタログとして作るのは避けてください。␞␞  </para>␞
␝ <sect1 id="system-catalog-initial-data">␟  <title>System Catalog Initial Data</title>␟  <title>システムカタログ初期データ</title>␞␞␞
␝  <para>␟   Each catalog that has any manually-created initial data (some do not)
   has a corresponding <literal>.dat</literal> file that contains its
   initial data in an editable format.␟手動で生成した初期データを持つ（いくつかのものは持っていません）各々のカタログには、編集可能なデータ形式の初期データを含み、対応する<literal>.dat</literal>ファイルがあります。␞␞  </para>␞
␝  <sect2 id="system-catalog-initial-data-format">␟   <title>Data File Format</title>␟   <title>データファイル形式</title>␞␞␞
␝   <para>␟    Each <literal>.dat</literal> file contains Perl data structure literals
    ...
    will demonstrate the key features:␟個々の<literal>.dat</literal>ファイルにはPerlのデータ構造文字列が含まれます。
それらは単に評価されることによって1個がカタログの1行に対応するハッシュ参照の配列を含むメモリ上のデータ構造を生成します。
<filename>pg_database.dat</filename>から抜きだしたものに些細な変更を加えたものを使って、鍵となる機能を示します。␞␞   </para>␞
␝   <para>␟    Points to note:␟特筆すべきポイント：␞␞   </para>␞
␝     <para>␟      The overall file layout is: open square bracket, one or more sets of
      curly braces each of which represents a catalog row, close square
      bracket.  Write a comma after each closing curly brace.␟全体的なファイルレイアウトは次のようになります。
開き大括弧、カタログの行を表現する一つ以上の中括弧、閉じ大括弧。
各々の閉じ中括弧の後にはカンマを書きます。␞␞     </para>␞
␝     <para>␟      Within each catalog row, write comma-separated
      ...
      suitable default value for it.)␟各々のカタログ行にカンマ区切りで<replaceable>key</replaceable><literal>=&gt;</literal><replaceable>value</replaceable>ペアを書きます。
記述可能な<replaceable>key</replaceable>は、カタログの列に加えてメタデータキーである<literal>oid</literal>、<literal>oid_symbol</literal>、<literal>array_type_oid</literal>、<literal>descr</literal>です。
...
メタデータキーは省略可能であるのに対し、カタログの<literal>.h</literal>ファイルが列のデフォルト値を指定する場合を除いてカタログの定義済み列はすべて提供されなければなりません。
（上記の例では<filename>pg_database.h</filename>が適切なデフォルト値を供するので<structfield>datdba</structfield>フィールドは省略されました。）␞␞     </para>␞
␝     <para>␟      All values must be single-quoted.  Escape single quotes used within a
      ...
      bootstrap scanner to see.␟すべての値は単一引用符で囲まなければなりません。
...
Perlが２つ削除し、ブートストラップスキャナが認識するために<literal>\\</literal>が残ります。␞␞     </para>␞
␝     <para>␟      Null values are represented by <literal>_null_</literal>.
      (Note that there is no way to create a value that is just that
      string.)␟NULL値は<literal>_null_</literal>で表します。
（それと同じ文字列を作る方法はないことに注意してください。）␞␞     </para>␞
␝     <para>␟      Comments are preceded by <literal>#</literal>, and must be on their
      own lines.␟コメントは<literal>#</literal>を前に置いてください。また同じ行上に置かなければなりません。␞␞     </para>␞
␝     <para>␟      Field values that are OIDs of other catalog entries should be
      ...
      below.␟他のカタログエントリのOIDであるフィールド値は、実際の数値のOIDではなくシンボル名で記述されるべきです。
（上記の例では<structfield>dattablespace</structfield>がこのような参照を含みます。）
これは後述の<xref linkend="system-catalog-oid-references"/>で説明します。␞␞     </para>␞
␝     <para>␟      Since hashes are unordered data structures, field order and line
      ...
      formatting script <filename>reformat_dat_file.pl</filename>:␟ハッシュは順序付けられないデータ構造なので、フィールドの順や行の配置には重要な意味はありません。
しかし、見た目を統一するために、フォーマットスクリプト<filename>reformat_dat_file.pl</filename>が適用される少数のルールを設定しました。␞␞␞
␝        <para>␟         Within each pair of curly braces, the metadata
         fields <literal>oid</literal>, <literal>oid_symbol</literal>,
         <literal>array_type_oid</literal>, and <literal>descr</literal>
         (if present) come first, in that order, then the catalog's own
         fields appear in their defined order.␟中括弧のペアの中で、メタデータフィールドの<literal>oid</literal>、<literal>oid_symbol</literal>、<literal>array_type_oid</literal>、および、（もし存在するなら）<literal>descr</literal>がこの順で最初に来ます。
そして、定義された順にカタログ自身のフィールドが現れます。␞␞        </para>␞
␝        <para>␟         Newlines are inserted between fields as needed to limit line length
         to 80 characters, if possible.  A newline is also inserted between
         the metadata fields and the regular fields.␟可能ならば、行の長さを80文字に制限するために、必要に応じてフィールドの間に改行を挿入します。
改行はメタデータフィールドと通常のフィールドの間にも挿入します。␞␞        </para>␞
␝        <para>␟         If the catalog's <literal>.h</literal> file specifies a default
         value for a column, and a data entry has that same
         value, <filename>reformat_dat_file.pl</filename> will omit it from
         the data file.  This keeps the data representation compact.␟カタログの<literal>.h</literal>ファイルが列のデフォルト値を指定していて、データエントリが同じ値なら、<filename>reformat_dat_file.pl</filename>はデータファイルからデータエントリを省略します。
これでデータ表現が小さくなります。␞␞        </para>␞
␝        <para>␟         <filename>reformat_dat_file.pl</filename> preserves blank lines
         and comment lines as-is.␟<filename>reformat_dat_file.pl</filename>は空白行とコメント行をそのまま維持します。␞␞        </para>␞
␝␟      It's recommended to run <filename>reformat_dat_file.pl</filename>
      ...
      run <literal>make reformat-dat-files</literal>.␟カタログデータパッチを投稿する前に、<filename>reformat_dat_file.pl</filename>を実行することをお勧めします。
便利さのために、単に<filename>src/include/catalog/</filename>に変更を加えて<literal>make reformat-dat-files</literal>を実行することができます。␞␞     </para>␞
␝     <para>␟      If you want to add a new method of making the data representation
      ...
      data back into the full representation.␟データ表現をより小さくする新しい方法を付け加えたいのであれば、<filename>reformat_dat_file.pl</filename>で実装し、また
データを完全な表現に戻す方法を<function>Catalog::ParseData()</function>に指示しなければなりません。␞␞     </para>␞
␝  <sect2 id="system-catalog-oid-assignment">␟   <title>OID Assignment</title>␟   <title>OIDの割当</title>␞␞␞
␝   <para>␟    A catalog row appearing in the initial data can be given a
    ...
    =&gt; <replaceable>name</replaceable></literal> metadata field.␟初期データに現れるカタログ行には<literal>oid=&gt; <replaceable>nnnn</replaceable></literal>メタデータフィールドを書くことで手動で割り当てたOIDを与えることができます。
それだけでなく、OIDを割り当てられたならば、<literal>oid_symbol =&gt; <replaceable>name</replaceable></literal>メタデータフィールドを書くことでそのOID用のCマクロを作ることができます。␞␞   </para>␞
␝   <para>␟    Pre-loaded catalog rows must have preassigned OIDs if there are OID
    ...
    cross-referenced.␟他のプリロードカタログ行の中にそのOIDへの参照がある場合には、プリロードカタログ行は割当済みのOIDを持たなければなりません。
Cコードから行OIDが参照されるときにも割当済みのOIDは必要です。
どちらも当てはまらない場合は、<literal>oid</literal>メタデータフィールドは省略可能です。
その場合、ブートストラップコードが自動的にOIDを割り当てます。
実用的には、カタログの一部のみが実際に相互参照されている場合でも、与えられたプリロードカタログ行のOIDをすべて割当済みにするか、一つも割当済みにしないかのどちらかに通常します。␞␞   </para>␞
␝   <para>␟    Writing the actual numeric value of any OID in C code is considered
    ...
    via <literal>oid_symbol</literal> entries.␟Cコード中でOIDの実際の数値を書くのは非常に良くないと考えられます。
...
他のすべてのシステムカタログでは、<literal>oid_symbol</literal>を使って必要なマクロを手動で指定しなければなりません。␞␞   </para>␞
␝   <para>␟    To find an available OID for a new pre-loaded row, run the
    ...
    OIDs at compile time.)␟新しいプリロード行のために利用可能なOIDを見つけるには、<filename>src/include/catalog/unused_oids</filename>スクリプトを実行してください。
未使用のOIDの範囲が表示されます。
（たとえば、出力行<literal>45-900</literal>はOIDs 45から900が利用されていないことを示します。）
今の所OID 1&ndash;9999 は手動での割当のために予約されています。
<filename>unused_oids</filename>スクリプトは、単にカタログヘッダと<filename>.dat</filename>を見てそこに出現していないOIDを探しているだけです。
間違い見つけるために<filename>duplicate_oids</filename>を利用することもできます。
（<filename>genbki.pl</filename>は手動アサインされていない全ての行にOIDを割り当て、また、コンパイル時に重複OIDを検出します。）␞␞   </para>␞
␝   <para>␟    When choosing OIDs for a patch that is not expected to be committed
    ...
    also be useful for recovering from that situation.␟即座にコミットされるとは期待できないパッチ用にOIDを選ぶときの最良の手法は、8000&mdash;9999の範囲でランダムに選択したところから始まるおおむね連続したOIDのグループを使うことです。
...
コミットされていないパッチが最近コミットされたパッチとOID衝突していることに気づいた場合に、このような状況から回復するのにも<filename>renumber_oids.pl</filename>がおそらく役立つでしょう。␞␞   </para>␞
␝   <para>␟    Because of this convention of possibly renumbering OIDs assigned by
    ...
    would create assorted compatibility problems.␟パッチに割り当てられたOIDを番号付け替えすることがあるこの慣習のため、パッチに割り当てられたOIDはそのパッチが正式リリースに含まれるまでは永続的と考えるべきではありません。
さまざまな互換性の問題を生み出すかもしれないため、一度リリースされた手動でアサインされたオブジェクトのOIDは変更しません。␞␞   </para>␞
␝   <para>␟    If <filename>genbki.pl</filename> needs to assign an OID to a catalog
    ...
    usual OID assignment mechanism takes care of preventing any conflicts.)␟<filename>genbki.pl</filename>は、手動アサインされたOIDを持たないカタログエントリにOIDを割り当てる必要がある場合、10000&mdash;11999範囲の値を使います。
ブートストラップの実行開始時にその場で作成されるオブジェクトもこの範囲のOIDが設定されるように、ブートストラップ実行開始の際にはサーバのOIDカウンタは10000に設定されます。（通常のOID割当機構は、衝突を回避するように調整を行います。）␞␞   </para>␞
␝   <para>␟    Objects with OIDs below <symbol>FirstUnpinnedObjectId</symbol> (12000)
    ...
    to <filename>genbki.pl</filename> will be.␟<symbol>FirstUnpinnedObjectId</symbol> (12000)以下のOIDを持つオブジェクトは<quote>固定</quote>と見なされ、削除されません。
（<function>IsPinnedObject()</function>に固定されている少数の例外があります。）
<application>initdb</application>は、固定されていないオブジェクトを作成する準備ができるとすぐに、OIDカウンタを<symbol>FirstUnpinnedObjectId</symbol>まで強制的に増加させます。
このため、<filename>information_schema.sql</filename>スクリプトの実行中に作成されたオブジェクトなど、<application>initdb</application>の後半のフェーズで作成されたオブジェクトは固定されませんが、<filename>genbki.pl</filename>で認識されているすべてのオブジェクトは固定されます。␞␞   </para>␞
␝   <para>␟    OIDs assigned during normal database operation are constrained to be
    ...
    from one installation to another.␟通常のデータベース操作で割り当てられたOIDは16384以上に限定されます。
これは<filename>genbki.pl</filename>や<application>initdb</application>中に自動的に割り当てられたOIDに対して10000&mdash;16383の範囲が空いていること保証します。
これらの自動割り当てされたOIDは今後も変更がないとはいえず、あるインストレーションから他のインストレーションで変更されるかもしれません。␞␞   </para>␞
␝  <sect2 id="system-catalog-oid-references">␟   <title>OID Reference Lookup</title>␟   <title>OID参照検索</title>␞␞␞
␝   <para>␟    In principle, cross-references from one initial catalog row to another
    ...
    The rules are as follows:␟原則としては、ある初期カタログ行から他への相互参照は、参照しているフィールドで参照されている行の事前割り当てされたOIDを書くことだけで記述できます。
...
そのルールは以下のとおりです。␞␞   </para>␞
␝     <para>␟      Use of symbolic references is enabled in a particular catalog column
      ...
      lookup on each element of the array.␟<literal>BKI_LOOKUP(<replaceable>lookuprule</replaceable>)</literal>を列の定義に加えることで、特定のカタログ列でのシンボル参照が利用可能になります。
ここで<replaceable>lookuprule</replaceable>は参照されているカタログ名で、例えば<literal>pg_proc</literal>です。
<literal>BKI_LOOKUP</literal>を、<type>Oid</type>、<type>regproc</type>、<type>oidvector</type>、<type>Oid[]</type>の列に加えることができます。
最後の２つにおいては、配列の個々の要素を検索することを暗に意味します。␞␞     </para>␞
␝     <para>␟      It's also permissible to attach <literal>BKI_LOOKUP(encoding)</literal>
      ...
      known to <filename>genbki.pl</filename>.␟文字セット符号化方式を参照する整数の列に<literal>BKI_LOOKUP(encoding)</literal>を加えることも許容されます。これは今のところカタログのOIDとして現れませんが、値の集合を<filename>genbki.pl</filename>に知らせます。␞␞     </para>␞
␝     <para>␟      In some catalog columns, it's allowed for entries to be zero instead
      ...
      <filename>genbki.pl</filename> will warn about unrecognized names.␟カタログ列の中には、エントリを有効な参照の代わりにゼロとすることが許されているものがあります。
これが許されているなら、<literal>BKI_LOOKUP</literal>の代わりに<literal>BKI_LOOKUP_OPT</literal>を書いてください。
そうすれば、エントリに<literal>0</literal>を書くことができます。
（列が<type>regproc</type>と宣言されている場合は、<literal>0</literal>の代わりに<literal>-</literal>と書くことができます。）
この特別な場合を除いて、<literal>BKI_LOOKUP</literal>列内のすべてのエントリはシンボル参照でなければなりません。
<filename>genbki.pl</filename>は認識できない名前には警告を出します。␞␞     </para>␞
␝     <para>␟      Most kinds of catalog objects are simply referenced by their names.
      ...
      for <literal>int4</literal>.␟たいていのカタログオブジェクト類は単純にその名前で参照されます。
型名は参照されている<structname>pg_type</structname>のエントリの<structfield>typname</structfield>と正確に一致しなければならないことに注意してください。<literal>int4</literal>に対する<literal>integer</literal>などの別名は使えません。␞␞     </para>␞
␝     <para>␟      A function can be represented by
      ...
      spaces.␟それが<filename>pg_proc.dat</filename>内でユニークなら、関数は<structfield>proname</structfield>で表現できます。
...
空白は挿入しないでください。␞␞     </para>␞
␝     <para>␟      Operators are represented
      ...
      operator.)␟演算子は<replaceable>oprname(lefttype,righttype)</replaceable>で表現します。
...
（省略された単項演算子のオペランドは<literal>0</literal>と書きます。）␞␞     </para>␞
␝     <para>␟      The names of opclasses and opfamilies are only unique within an
      access method, so they are represented
      by <replaceable>access_method_name</replaceable><literal>/</literal><replaceable>object_name</replaceable>.␟opclassesとopfamiliesの名前はアクセスメソッド内でのみユニークなので、<replaceable>access_method_name</replaceable><literal>/</literal><replaceable>object_name</replaceable>で表します。␞␞     </para>␞
␝     <para>␟      In none of these cases is there any provision for
      schema-qualification; all objects created during bootstrap are
      expected to be in the <literal>pg_catalog</literal> schema.␟以上のいずれの場合にもスキーマ修飾の規定はありません。
ブートストラップ中に作成されるすべてのオブジェクトは、<literal>pg_catalog</literal>スキーマにあると期待されます。␞␞     </para>␞
␝   <para>␟    <filename>genbki.pl</filename> resolves all symbolic references while it
    ...
    references.␟<filename>genbki.pl</filename>は実行中にすべてのシンボル参照を解決し、生成したBKIファイルの中に単純な数字のOIDを設定します。
ですから、ブートストラップバックエンドはシンボル参照にかかわる必要はありません。␞␞   </para>␞
␝   <para>␟    It's desirable to mark OID reference columns
    ...
    keys).␟カタログに検索の必要な初期データがなかったとしても、OID参照列に<literal>BKI_LOOKUP</literal>や<literal>BKI_LOOKUP_OPT</literal>の印を付けておくことが望ましいです。
これにより、<filename>genbki.pl</filename>がシステムカタログ内に存在する外部キー関係を記録できるようになります。
その情報は間違ったエントリを確認するリグレッションテストで使われます。
マクロ<literal>DECLARE_FOREIGN_KEY</literal>、<literal>DECLARE_FOREIGN_KEY_OPT</literal>、<literal>DECLARE_ARRAY_FOREIGN_KEY</literal>、<literal>DECLARE_ARRAY_FOREIGN_KEY_OPT</literal>も参照してください。<literal>BKI_LOOKUP</literal>には複雑すぎる外部キー関係（典型的には、複数列の外部キー）を宣言するのに使われます。␞␞   </para>␞
␝  <sect2 id="system-catalog-auto-array-types">␟   <title>Automatic Creation of Array Types</title>␟   <title>配列型の自動作成</title>␞␞␞
␝   <para>␟    Most scalar data types should have a corresponding array type (that is,
    ...
    automatically in most cases.␟たいていのスカラデータ型は対応する配列型を持つはずです（すなわち、要素がスカラ型の標準varlena配列型で、スカラ型の<structname>pg_type</structname>エントリの<structfield>typarray</structfield>フィールドから参照されているもの）。
<filename>genbki.pl</filename>はたいていの場合に配列型に対する<structname>pg_type</structname>エントリも自動的に生成できます。␞␞   </para>␞
␝   <para>␟    To use this facility, just write an <literal>array_type_oid
    ...
    automatically with that OID.␟この機能を使うには、スカラ型の<structname>pg_type</structname>エントリに<literal>array_type_oid =&gt; <replaceable>nnnn</replaceable></literal>というメタデータフィールドを記述して、配列型に使用するOIDを指定します。
自動的にそのOIDが書かれるため、このとき<structfield>typarray</structfield>フィールドを省いてもよいです。␞␞   </para>␞
␝   <para>␟    The generated array type's name is the scalar type's name with an
    ...
    set to cross-reference each other.␟生成された配列型の名前は、スカラ型の名前の手前にアンダースコアを付けたものです。
...
さらに両エントリの<structfield>typelem</structfield>および<structfield>typarray</structfield>フィールドは相互参照するように設定されます。␞␞   </para>␞
␝  <sect2 id="system-catalog-recipes">␟   <title>Recipes for Editing Data Files</title>␟   <title>データファイルの編集方法</title>␞␞␞
␝   <para>␟    Here are some suggestions about the easiest ways to perform common tasks
    when updating catalog data files.␟カタログデータファイルを更新する共通の作業を実施するためのもっとも簡単な方法の提案を示します。␞␞   </para>␞
␝   <formalpara>␟    <title>Add a new column with a default to a catalog:</title>␟    <title>カタログにデフォルト付きの新しい列を追加する</title>␞␞    <para>␞
␝    <para>␟     Add the column to the header file with
     ...
     in existing rows where a non-default value is needed.␟<literal>BKI_DEFAULT(<replaceable>value</replaceable>)</literal>注釈付きでヘッダファイルに列を追加します。
非デフォルト値が必要な既存の行に対してのみフィールドを追加によるデータファイルの調整が必要です。␞␞    </para>␞
␝   <formalpara>␟    <title>Add a default value to an existing column that doesn't have
     one:</title>␟    <title>デフォルト値を持たない既存の列にデフォルト値を追加する</title>␞␞    <para>␞
␝    <para>␟     Add a <literal>BKI_DEFAULT</literal> annotation to the header file,
     then run <literal>make reformat-dat-files</literal> to remove
     now-redundant field entries.␟<literal>BKI_DEFAULT</literal>注釈をヘッダファイルに追加し、冗長になったフィールドエントリを削除するために<literal>make reformat-dat-files</literal>を実行します。␞␞    </para>␞
␝   <formalpara>␟    <title>Remove a column, whether it has a default or not:</title>␟    <title>デフォルト値の有無にかかわらず、列を削除する</title>␞␞    <para>␞
␝    <para>␟     Remove the column from the header, then run <literal>make
     reformat-dat-files</literal> to remove now-useless field entries.␟ヘッダから列を削除し、<literal>make reformat-dat-files</literal>を実行して不要になったフィールドエントリを削除します。␞␞    </para>␞
␝   <formalpara>␟    <title>Change or remove an existing default value:</title>␟    <title>既存のデフォルト値を変更もしくは削除する</title>␞␞    <para>␞
␝    <para>␟     You cannot simply change the header file, since that will cause the
     ...
     reformat-dat-files</literal> to remove superfluous fields again.␟現在のデータが正しく解釈されなくなるため、単にヘッダファイルを変更することはできません。
まず<literal>make expand-dat-files</literal>を実行し、すべてのデフォルト値が明示的に挿入されるようにデータファイルを書き換えます。
次に<literal>BKI_DEFAULT</literal>注釈を変更もしくは削除し、<literal>make reformat-dat-files</literal>を実行して余分のフィールドを再び削除します。␞␞    </para>␞
␝   <formalpara>␟    <title>Ad-hoc bulk editing:</title>␟    <title>特定の目的のための大量の編集:</title>␞␞    <para>␞
␝    <para>␟     <filename>reformat_dat_file.pl</filename> can be adapted to perform
     ...
     into a char field:␟<filename>reformat_dat_file.pl</filename>を使って色々な大量の変更を実施できます。
一度限りのコードを挿入できることを示すブロックコメントを見つけます。
次の例では、<structname>pg_proc</structname>中の２つの論理値型フィールドを一つの文字フィールドに統合します。␞␞␞
␝       <para>␟        Add the new column, with a default,
        to <filename>pg_proc.h</filename>:␟デフォルトがある新しい列を<filename>pg_proc.h</filename>に追加します。␞␞<programlisting>␞
␝       <para>␟        Create a new script based on <filename>reformat_dat_file.pl</filename>
        to insert appropriate values on-the-fly:␟臨機応変に適当な値を挿入するために、<filename>reformat_dat_file.pl</filename>を元に新しいスクリプトを作ります。␞␞<programlisting>␞
␝       <para>␟        Run the new script:␟スクリプトを実行します。␞␞<programlisting>␞
␝</programlisting>␟        At this point <filename>pg_proc.dat</filename> has all three
        ...
        only in rows where they have non-default values.␟この時点で<filename>pg_proc.dat</filename>には<structfield>prokind</structfield>、<structfield>proisagg</structfield>、<structfield>proiswindow</structfield>のすべての3つの列がありますが、非デフォルト値を持つ行だけに表れます。␞␞       </para>␞
␝       <para>␟        Remove the old columns from <filename>pg_proc.h</filename>:␟<filename>pg_proc.h</filename>から古い列を削除します。␞␞<programlisting>␞
␝       <para>␟        Finally, run <literal>make reformat-dat-files</literal> to remove
        the useless old entries from <filename>pg_proc.dat</filename>.␟最後に、<literal>make reformat-dat-files</literal>を実行して<filename>pg_proc.dat</filename>から不要になった古いエントリを削除します。␞␞       </para>␞
␝␟     For further examples of scripts used for bulk editing, see
     ...
     message:
     <ulink url="https://www.postgresql.org/message-id/CAJVSVGVX8gXnPm+Xa=DxR7kFYprcQ1tNcCT5D0O3ShfnM6jehA@mail.gmail.com"></ulink>␟さらなる大量編集スクリプトの例については、<ulink url="https://www.postgresql.org/message-id/CAJVSVGVX8gXnPm+Xa=DxR7kFYprcQ1tNcCT5D0O3ShfnM6jehA@mail.gmail.com"></ulink>に付随する<filename>convert_oid2name.pl</filename>と<filename>remove_pg_type_oid_symbols.pl</filename>を見てください。␞␞    </para>␞
␝ <sect1 id="bki-format">␟  <title><acronym>BKI</acronym> File Format</title>␟  <title><acronym>BKI</acronym>ファイル形式</title>␞␞␞
␝  <para>␟   This section describes how the <productname>PostgreSQL</productname>
   ...
   file is at hand as an example.␟本節では<productname>PostgreSQL</productname>のバックエンドがどのようにして<acronym>BKI</acronym>ファイルを解釈するのかを説明します。
例として<filename>postgres.bki</filename>ファイルが手元にあると、説明が一層理解しやすくなるでしょう。␞␞  </para>␞
␝  <para>␟   <acronym>BKI</acronym> input consists of a sequence of commands.  Commands are made up
   ...
   single-quoted strings.  Everything is case sensitive.␟<acronym>BKI</acronym>の入力は一連のコマンドで構成されます。
...
大文字/小文字は全て区別されます。␞␞  </para>␞
␝  <para>␟   Lines starting with <literal>#</literal> are ignored.␟<literal>#</literal>で始まる行は無視されます。␞␞  </para>␞
␝ <sect1 id="bki-commands">␟  <title><acronym>BKI</acronym> Commands</title>␟  <title><acronym>BKI</acronym>コマンド</title>␞␞␞
␝     <para>␟      Create a table named <replaceable
      class="parameter">tablename</replaceable>, and having the OID
      <replaceable class="parameter">tableoid</replaceable>,
      with the columns given in parentheses.␟括弧で与えられた列と、OID <replaceable class="parameter">tableoid</replaceable>を持つ<replaceable class="parameter">tablename</replaceable>というテーブルを作成します。␞␞     </para>␞
␝     <para>␟      The following column types are supported directly by
      ...
      catalogs can contain any built-in type.)␟次の列型は<filename>bootstrap.c</filename>で直接サポートされます。
...
この他の型の列を持つテーブルを作成することはできますが、<structname>pg_type</structname>が完了し適切な項目で埋められるまで完了させることができません。
（これらの列型のみブートストラップカタログで使用されますが、非ブートストラップカタログは如何なる組み込み型も含む事があるという事を実際に意味しています。）␞␞     </para>␞
␝     <para>␟      When <literal>bootstrap</literal> is specified,
      ...
      <structname>pg_class</structname> etc. themselves.␟<literal>bootstrap</literal>が指定された場合、テーブルはディスク上に作成されるだけで、<structname>pg_class</structname>や<structname>pg_attribute</structname>などにその項目は登録されません。
したがって、これらの項目が（<literal>insert</literal>コマンドで）固定化されるまで、普通のSQL操作でこのテーブルにアクセスできません。
このオプションは<structname>pg_class</structname>など自身を作成するために使用されます。␞␞     </para>␞
␝     <para>␟      The table is created as shared if <literal>shared_relation</literal> is
      specified.
      ...
      provided anyway for documentation.)␟<literal>shared_relation</literal>が指定された場合、テーブルは共有として作成されます。
テーブルの行型OID（<structname>pg_type</structname> OID）は<literal>rowtype_oid</literal>句で指定できます。
指定されなければ、OIDは自動的に生成されます。
（<literal>bootstrap</literal> が指定されていれば、<literal>rowtype_oid</literal>句は役に立ちません。しかし、文書化のためにともかく指定はできます。）␞␞     </para>␞
␝     <para>␟      Open the table named
      <replaceable class="parameter">tablename</replaceable>
      for insertion of data.  Any currently open table is closed.␟データを挿入するために<replaceable class="parameter">tablename</replaceable>と名前が付けられたテーブルを開きます。
現在開いているテーブルは閉じられます。␞␞     </para>␞
␝     <para>␟      Close the open table.  The name of the table must be given as a
      cross-check.␟開いているテーブルを閉じます。
照合用にテーブル名を指定しなければなりません。␞␞     </para>␞
␝     <para>␟      Insert a new row into the open table using <replaceable
      class="parameter">value1</replaceable>, <replaceable
      class="parameter">value2</replaceable>, etc., for its column
      values.␟<replaceable class="parameter">value1</replaceable>や<replaceable class="parameter">value2</replaceable>などを列の値として、開いているテーブルに行を挿入します。␞␞     </para>␞
␝     <para>␟      NULL values can be specified using the special key word
      ...
      Escape-string-style backslash escapes are allowed in the string, too.)␟NULL値は特別なキーワード、<literal>_null_</literal>によって指定できます。
識別子に見えない値、あるいは数値文字列は単一引用符で囲まなければなりません。
(単一引用符を値に含めるには、二重に書いてください。
エスケープ文字列形式のバックスラッシュエスケープも、文字列内で許されています。)␞␞     </para>␞
␝     <para>␟      Create an index named <replaceable
      ...
      made for it, but the index contents are not initialized by this command.␟<replaceable class="parameter">amname</replaceable>アクセスメソッドを使用して、<replaceable class="parameter">tablename</replaceable>と名付けられたテーブル上に、OID <replaceable class="parameter">indexoid</replaceable>を所有する、<replaceable class="parameter">indexname</replaceable>という名前のインデックスを作成します。
...
このインデックスファイルは作成され、適切なカタログ項目が作成されますが、このコマンドではインデックスの内容の初期化を行いません。␞␞     </para>␞
␝     <para>␟      Create a TOAST table for the table named
      <replaceable class="parameter">tablename</replaceable>.
      ...
      is postponed.␟<replaceable class="parameter">tablename</replaceable>という名前のテーブル用のTOASTテーブルを作成します。
...
<literal>declare index</literal>と同様、インデックスの作成は遅延されます。␞␞     </para>␞
␝     <para>␟      Fill in the indices that have previously been declared.␟前に宣言されたインデックスを作成します。␞␞     </para>␞
␝ <sect1 id="bki-structure">␟  <title>Structure of the Bootstrap <acronym>BKI</acronym> File</title>␟  <title><acronym>BKI</acronym>ファイルのブートストラップの構成</title>␞␞␞
␝  <para>␟   The <literal>open</literal> command cannot be used until the tables it uses
   ...
   the created table for data insertion.␟<literal>open</literal>コマンドは、テーブルが、使用するテーブルが存在し、開かれるテーブルに対しエントリを所有するまで使用できません。（これら最小限度のテーブルは、<structname>pg_class</structname>、<structname>pg_attribute</structname>、<structname>pg_proc</structname>、および<structname>pg_type</structname>です。）これらのテーブル自体が充填されるようにするには、<literal>bootstrap</literal>オプションを伴った<literal>create</literal>が明示的にデータの挿入のために作成されたテーブルを開きます。␞␞  </para>␞
␝  <para>␟   Also, the <literal>declare index</literal> and <literal>declare toast</literal>
   commands cannot be used until the system catalogs they need have been
   created and filled in.␟また、必要とするシステムカタログが作成され、値が設定されるまで、<literal>declare index</literal>および<literal>declare toast</literal>コマンドは使用できません。␞␞  </para>␞
␝  <para>␟   Thus, the structure of the <filename>postgres.bki</filename> file has to
   be:␟従い、<filename>postgres.bki</filename>の構造は以下でなければなりません。␞␞   <orderedlist>␞
␝     <para>␟      <literal>create bootstrap</literal> one of the critical tables␟      1つの重要なテーブルを<literal>create bootstrap</literal>␞␞     </para>␞
␝     <para>␟      <literal>insert</literal> data describing at least the critical tables␟      少なくとも重要なテーブルを記述するデータを<literal>insert</literal>␞␞     </para>␞
␝     <para>␟      Repeat for the other critical tables.␟      その他の重要テーブルに対して反復。␞␞     </para>␞
␝     <para>␟      <literal>create</literal> (without <literal>bootstrap</literal>) a noncritical table␟      重要でないテーブルを（<literal>bootstrap</literal>無しで）<literal>create</literal>␞␞     </para>␞
␝     <para>␟      <literal>insert</literal> desired data␟      求められるデータの<literal>insert</literal>␞␞     </para>␞
␝     <para>␟      Repeat for the other noncritical tables.␟     その他の重要でないテーブルに対して反復。␞␞     </para>␞
␝     <para>␟      Define indexes and toast tables.␟      インデックスおよびTOASTテーブルの定義。␞␞     </para>␞
␝  <para>␟   There are doubtless other, undocumented ordering dependencies.␟他にもドキュメント化されていない順序に関する依存性があることは疑いの余地がありません。␞␞  </para>␞
␝ <sect1 id="bki-example">␟  <title>BKI Example</title>␟  <title>BKIの例</title>␞␞␞
␝  <para>␟   The following sequence of commands will create the table
   ...
   respectively, and insert two rows into the table:␟次の一連のコマンドは、それぞれ<type>oid</type>型、<type>int4</type>型、<type>text</type>型の3つの列、<literal>oid</literal>、<literal>cola</literal>、<literal>colb</literal>を持ち、OID 420 が付いた<literal>test_table</literal>テーブルを作成し、そして2つの行をテーブルに挿入します。␞␞<programlisting>␞
␝␟<literal>close</literal>␟no translation␞␞␞
␝␟<literal>open</literal>␟no translation␞␞␞
␝␟<literal>close</literal>␟no translation␞␞␞
␝␟<literal>build indices</literal>␟no translation␞␞␞
