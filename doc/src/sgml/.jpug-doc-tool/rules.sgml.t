...
␝<chapter id="rules">␟<title>The Rule System</title>␟<title>ルールシステム</title>␞␞␞
␝<para>␟     This chapter discusses the rule system in
     <productname>PostgreSQL</productname>.  Production rule systems
     are conceptually simple, but there are many subtle points
     involved in actually using them.␟本章では<productname>PostgreSQL</productname>のルールシステムについて説明します。
本番で稼働するルールシステムは概念としては単純ですが、実際に使ってみると、わかりにくいところが少なからずあります。␞␞</para>␞
␝<para>␟     Some other database systems define active database rules, which
     are usually stored procedures and triggers.  In
     <productname>PostgreSQL</productname>, these can be implemented
     using functions and triggers as well.␟通常それらはストアドプロシージャとトリガですが、他のいくつかのデータベースシステムは能動的データベースルールを定義しています。
<productname>PostgreSQL</productname>では関数とトリガとして実装されています。␞␞</para>␞
␝<para>␟     The rule system (more precisely speaking, the query rewrite rule
     ...
     linkend="ong90"/>.␟ルールシステム（より正確に言うと問い合わせ書き換えルールシステム）はストアドプロシージャとトリガとはまったく異なります。
...
このルールシステムの基礎理論と能力は<xref linkend="ston90b"/>および<xref linkend="ong90"/>で解説されています。␞␞</para>␞
...
␝<sect1 id="querytree">␟<title>The Query Tree</title>␟<title>問い合わせツリーとは</title>␞␞␞
␝<para>␟    To understand how the rule system works it is necessary to know
    when it is invoked and what its input and results are.␟どのようにルールシステムが機能するかを理解するためには、ルールがどのように起動され、その入力と結果は何かを理解しなければなりません。␞␞</para>␞
␝<para>␟    The rule system is located between the parser and the planner.
    ...
    is basically representable as an <acronym>SQL</acronym> statement.␟ルールシステムは問い合わせパーサとプランナの中間に位置します。
ルールシステムは、入力としてパーサの出力、単一の問い合わせツリー、および何らかの特別な情報を持つ問い合わせツリーでもあるユーザ定義の書き換えルールを取り、結果として0個以上の問い合わせツリーを生成します。
ルールシステムの入力と出力は常にパーサ自体でも生成することができるもので、参照する対象は基本的に<acronym>SQL</acronym>文として表現できるものです。␞␞</para>␞
␝<para>␟    Now what is a query tree? It is an internal representation of an
    ...
    the log output, but they contain exactly the same information.␟では問い合わせツリーとは何でしょうか。
...
これはログ出力のように整形されていませんが、まったく同じ情報を持っています。␞␞</para>␞
␝<para>␟    Reading a raw query tree requires some experience.  But since
    <acronym>SQL</acronym> representations of query trees are
    sufficient to understand the rule system, this chapter will not
    teach how to read them.␟問い合わせツリーそのものを読むためにはある程度の経験が必要です。
ルールシステムを理解するためには問い合わせツリーの<acronym>SQL</acronym>表現で十分ですので、本章ではその読み方については説明しません。␞␞</para>␞
␝<para>␟    When reading the <acronym>SQL</acronym> representations of the
    ...
    structure. The parts of a query tree are␟本章の問い合わせツリーの<acronym>SQL</acronym>表現形式を読む時に必要なのは、問い合わせツリー構造の中に分解された、ある文の部品を識別できることです。
問い合わせツリーには以下の部品があります。␞␞␞
␝    <term>␟        the command type␟コマンド種類␞␞    </term>␞
␝    <para>␟        This is a simple value telling which command
        (<command>SELECT</command>, <command>INSERT</command>,
        <command>UPDATE</command>, <command>DELETE</command>) produced
        the query tree.␟これはどのコマンド（<command>SELECT</command>、<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>）が構文解析ツリーを作ったかを示す単純な値です。␞␞    </para>␞
␝    <term>␟        the range table
      <indexterm><primary>range table</primary></indexterm>␟範囲テーブル
      <indexterm><primary>範囲テーブル</primary></indexterm>␞␞    </term>␞
␝    <para>␟        The range table is a list of relations that are used in the query.
        In a <command>SELECT</command> statement these are the relations given after
        the <literal>FROM</literal> key word.␟範囲テーブルは問い合わせで使われるリレーションのリストです。
<command>SELECT</command>文ではこれは<literal>FROM</literal>キーワードの後で与えられるリレーションになります。␞␞    </para>␞
␝    <para>␟        Every range table entry identifies a table or view and tells
        ...
        this situation.␟範囲テーブルのそれぞれの項目はテーブルもしくはビューを識別し、問い合わせの別の部品ではどんな名前で呼び出されるかを示します。
...
本章の例ではその状況を含んでいません。␞␞    </para>␞
␝    <term>␟        the result relation␟結果リレーション␞␞    </term>␞
␝    <para>␟        This is an index into the range table that identifies the
        relation where the results of the query go.␟問い合わせの結果が格納されるリレーションを識別する範囲テーブルへのインデックスです。␞␞    </para>␞
␝    <para>␟        <command>SELECT</command> queries don't have a result
        ...
        separately here.)␟<command>SELECT</command>問い合わせは結果リレーションを持ちません。
（<command>SELECT INTO</command>の場合は特別ですが、<literal>INSERT ... SELECT</literal>が付いた<command>CREATE TABLE</command>とほぼ同じですので、ここでは個別には説明しません。）␞␞    </para>␞
␝    <para>␟        For <command>INSERT</command>, <command>UPDATE</command>, and
        <command>DELETE</command> commands, the result relation is the table
        (or view!) where the changes are to take effect.␟<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>コマンドでは、結果リレーションは変更が有効になるテーブル（もしくはビュー）です。␞␞    </para>␞
␝    <term>␟        the target list
    <indexterm><primary>target list</primary></indexterm>␟目的リスト
    <indexterm><primary>目的リスト</primary></indexterm>␞␞    </term>␞
␝    <para>␟        The target list is a list of expressions that define the
        ...
        rule system never sees it.)␟目的リストは問い合わせの結果を定義する式のリストです。
...
これはパーサによって個別の列に展開されますので、ルールシステムが見ることはありません。）␞␞    </para>␞
␝    <para>␟        <command>DELETE</command> commands don't need a normal target list
        ...
        the rule system, as described in <xref linkend="rules-views-update"/>.)␟<command>DELETE</command>コマンドは結果を返しませんので、通常の目的リストは必要ありません。
...
もしビューであれば<xref linkend="rules-views-update"/>で述べるように、代わりに行全体の変数がルールシステムによって追加されます。）␞␞    </para>␞
␝    <para>␟        For <command>INSERT</command> commands, the target list describes
        ...
        planner with a constant null expression.␟<command>INSERT</command>問い合わせでは、目的リストは結果リレーションへ入る新規の行を示します。
...
残った列（値が与えられていない列、かつデフォルト値を持たない列）は全て、プランナによって定数NULL式で埋められます。␞␞    </para>␞
␝    <para>␟        For <command>UPDATE</command> commands, the target list
        ...
        the executor can identify the old row to be updated.␟<command>UPDATE</command>コマンドでは、目的リストは古いものを置き換えるべき新しい行を示します。
ルールシステムではコマンド内の<literal>SET column = expression</literal>部分にある式だけを持っています。
プランナは、古い行から新しい行へ値をコピーする式を挿入することにより、抜けている列を処理します。
<command>DELETE</command>の場合と同様、エグゼキュータが更新すべき行を見つけられるように、<acronym>CTID</acronym>もしくは行全体の変数が追加されます。␞␞    </para>␞
␝    <para>␟        Every entry in the target list contains an expression that can
        ...
        tree made of function calls, constants, variables, operators, etc.␟目的リストの各項目は、定数値、範囲テーブル内のリレーション中の1つの列を指し示す変数、パラメータ等の式を保持するか、または、関数呼び出し、定数、変数、演算子などにより作られた式のツリーを保持します。␞␞    </para>␞
␝    <term>␟        the qualification␟条件␞␞    </term>␞
␝    <para>␟        The query's qualification is an expression much like one of
        ...
        of an <acronym>SQL</acronym> statement.␟問い合わせの条件は目的リストの項目に含まれている式によく似た式です。
...
それは<acronym>SQL</acronym>文の中の<literal>WHERE</literal>句に対応します。␞␞    </para>␞
␝    <term>␟        the join tree␟結合ツリー␞␞    </term>␞
␝    <para>␟        The query's join tree shows the structure of the <literal>FROM</literal> clause.
        ...
        both the <literal>FROM</literal> and <literal>WHERE</literal> clauses of a <command>SELECT</command>.␟問い合わせの結合ツリーは<literal>FROM</literal>句の構造を表します。
...
ですから、結合ツリーは<command>SELECT</command>の<literal>FROM</literal>句と<literal>WHERE</literal>句の両方を表しているわけです。␞␞    </para>␞
␝    <term>␟        the others␟その他␞␞    </term>␞
␝    <para>␟        The other parts of the query tree like the <literal>ORDER BY</literal>
        ...
        system.␟<literal>ORDER BY</literal>句のような、問い合わせツリーのその他の部品は、ここでは取り上げません。
ルールシステムはルールを適用している時にそこで項目を入れ替えることもありますが、これはルールシステムの基本とはあまり関係しません。␞␞    </para>␞
...
␝<sect1 id="rules-views">␟<title>Views and the Rule System</title>␟<title>ビューとルールシステム</title>␞␞␞
␝<para>␟    Views in <productname>PostgreSQL</productname> are implemented
    ...
    So a view like␟<productname>PostgreSQL</productname>におけるビューはルールシステムを使って実装されています。
...
␝␟    although you can't actually write that, because tables are not
    allowed to have <literal>ON SELECT</literal> rules.␟ですが、テーブルは<literal>ON SELECT</literal>ルールを持つことができませんので、これを実際には書くことはできません。␞␞</para>␞
␝<para>␟    A view can also have other kinds of <literal>DO INSTEAD</literal>
    ...
    <xref linkend="rules-views-update"/>.␟ビューには他の種類の<literal>DO INSTEAD</literal>ルールもあり、基礎となる記憶域がないにもかかわらず、ビューに対して<command>INSERT</command>、<command>UPDATE</command>、または<command>DELETE</command>コマンドを実行できるようにします。
これについては、以下の<xref linkend="rules-views-update"/>でさらに説明します。␞␞</para>␞
...
␝<sect2 id="rules-select">␟<title>How <command>SELECT</command> Rules Work</title>␟<title><command>SELECT</command>ルールの動き</title>␞␞␞
␝<para>␟    Rules <literal>ON SELECT</literal> are applied to all queries as the last step, even
    ...
    <command>SELECT</command> rules are described first.␟たとえコマンドが<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>などであっても、<literal>ON SELECT</literal>ルールは全ての問い合わせに対し最後に適用されます。
そして、このルールは他のコマンド種類のルールと異なるセマンティクスを持っていて、問い合わせツリーを新規に生成せずに、そこにあるものを修正します。
したがって<command>SELECT</command>ルールを一番初めに記述します。␞␞</para>␞
␝<para>␟    Currently, there can be only one action in an <literal>ON SELECT</literal> rule, and it must
    ...
    it restricts <literal>ON SELECT</literal> rules to act like views.␟現在のところ、<literal>ON SELECT</literal>ルールでは1つのアクションしか許されず、それは<literal>INSTEAD</literal>である無条件の<command>SELECT</command>アクションでなければいけません。
この制約は、一般のユーザが何をしても、ルールシステムが堅牢であるために必要であり、<literal>ON SELECT</literal>のルールはビュー同様の動作に限定されます。␞␞</para>␞
␝<para>␟    The examples for this chapter are two join views that do some
    ...
    than having many different ones that might mix up in mind.␟本章の例として挙げているのは、ちょっとした演算をする2つの結合のビューと、次にこれらの機能を利用するいくつかのビューです。
...
初めて学ぶための例としては決して簡単ではなく先に進むことを躊躇させるかもしれません。
しかし、多くの別々の例を持ち出して頭の混乱を招くよりも、全ての論点をステップごとに追う1つの例を挙げる方が良いでしょう。␞␞</para>␞
␝<para>␟    The real tables we need in the first two rule system descriptions
    are these:␟最初の2つのルールシステムの説明で必要とする実テーブルを以下に示します。␞␞␞
␝<programlisting>␟CREATE TABLE shoe_data (
...
␝␟    As you can see, they represent shoe-store data.␟これでわかるかもしれませんが、これらは靴屋のデータを表しています。␞␞</para>␞
␝<para>␟    The views are created as:␟ビューを以下のように作成します。␞␞␞
␝␟    The <command>CREATE VIEW</command> command for the
    ...
    <command>SELECT</command> statement in the view creation command.␟<literal>shoelace</literal>ビュー（今ある一番簡単なビュー）用の<command>CREATE VIEW</command>コマンドは、<literal>shoelace</literal>リレーションと、問い合わせ範囲テーブルの中で<literal>shoelace</literal>リレーションが参照される時はいつでも、適用されるべき書き換えルールの存在を示す項目を<structname>pg_rewrite</structname>に作ります。
...
このルールアクションは、ビュー作成コマンド内の<command>SELECT</command>のコピーである、1つの問い合わせツリーです。␞␞</para>␞
␝    <para>␟    The two extra range
    ...
    for <command>SELECT</command> rules.␟<structname>pg_rewrite</structname>項目の<literal>NEW</literal>と<literal>OLD</literal>に対する2つの特別な範囲テーブル項目は<command>SELECT</command>ルールには関係ありません。␞␞    </para>␞
␝<para>␟    Now we populate <literal>unit</literal>, <literal>shoe_data</literal>
    and <literal>shoelace_data</literal> and run a simple query on a view:␟ではここで<literal>unit</literal>、<literal>shoe_data</literal>、<literal>shoelace_data</literal>にデータを入れ、ビューに簡単な問い合わせを行います。␞␞␞
␝   <para>␟    This is the simplest <command>SELECT</command> you can do on our
    views, so we take this opportunity to explain the basics of view
    rules.  The <literal>SELECT * FROM shoelace</literal> was
    interpreted by the parser and produced the query tree:␟これは、ビューに対する最も簡単な<command>SELECT</command>ですので、この機会にビュールールの基本を説明します。
...
␝␟    and this is given to the rule system. The rule system walks through the
    ...
    <literal>_RETURN</literal> rule with the query tree:␟このツリーがルールシステムに伝えられます。
...
␝<para>␟    To expand the view, the rewriter simply creates a subquery range-table
    entry containing the rule's action query tree, and substitutes this
    range table entry for the original one that referenced the view.  The
    resulting rewritten query tree is almost the same as if you had typed:␟ビューを展開するために、リライタは単純にルールのアクション問い合わせツリーを持つ副問い合わせ範囲テーブルの項目を作り、ビューを参照していた元の範囲テーブルを置き換えます。
...
この方法で、書き換えられた問い合わせで直接ビューを使用していなくても、エグゼキュータはユーザがそのビューにアクセスするための正しい権限を持っているか確認します。␞␞</para>␞
␝<para>␟    That was the first rule applied.  The rule system will continue checking
...
ですから書き換えは完結し、上記がプランナに渡される最終的な結果となります。␞␞</para>␞
␝<para>␟    Now we want to write a query that finds out for which shoes currently in the store
    we have the matching shoelaces (color and length) and where the
    total number of exactly matching pairs is greater than or equal to two.␟さて、店に置いてある靴紐（の色とサイズ）に一致する靴が店にあるか、完全に一致する靴の在庫数が2以上あるかどうかを把握する問い合わせを書いてみましょう。␞␞␞
␝<para>␟    The output of the parser this time is the query tree:␟今回のパーサの出力は以下の問い合わせツリーです。␞␞␞
␝␟    The first rule applied will be the one for the
    <literal>shoe_ready</literal> view and it results in the
    query tree:␟最初に適用されるルールは<literal>shoe_ready</literal>ビュー用のもので、問い合わせツリーにおける結果は以下のようになります。␞␞␞
␝␟    Similarly, the rules for <literal>shoe</literal> and
    <literal>shoelace</literal> are substituted into the range table of
    the subquery, leading to a three-level final query tree:␟同じように、<literal>shoe</literal>と<literal>shoelace</literal>用のルールは副問い合わせの範囲テーブルとして代用され、3レベルの最終問い合わせツリーへと導きます。␞␞␞
␝   <para>␟    This might look inefficient, but the planner will collapse this into a
    ...
    rewrite system doesn't have to concern itself with.␟これは非効率的に見えるかもしれませんが、プランナは副問い合わせを<quote>引っ張り上げること</quote>で、これを単一レベルの問い合わせツリーに縮めてから、手で書き出したかのように結合を計画します。
そのため、問い合わせツリーを縮めるという最適化を、書き換えシステム自身で意識する必要はありません。␞␞   </para>␞
␝<sect2 id="rules-views-non-select">␟<title>View Rules in Non-<command>SELECT</command> Statements</title>␟<title>非<command>SELECT</command>文のビュールール</title>␞␞␞
␝<para>␟    Two details of the query tree aren't touched in the description of
    ...
    special care needs to be taken if the result relation is a view.␟これまでのビュールールの説明では問い合わせツリーの2つの詳細について触れませんでした。
それらは、コマンドタイプと結果リレーションです。
実際、コマンドタイプはビュールールでは必要とされませんが、結果リレーションがビューの場合には特別な考慮が必要ですので、結果リレーションは問い合わせリライタの動作に影響するかもしれません。␞␞</para>␞
␝<para>␟    There are only a few differences between a query tree for a
    ...
    <literal>b</literal>, the query trees for the two statements:␟<command>SELECT</command>と他のコマンドに対する問い合わせツリーの間には大きな違いはありません。
明らかに、それらは違うコマンドタイプを持っていて、<command>SELECT</command>以外のコマンドでは、結果リレーションは結果の格納先となる範囲テーブルの項目を指し示します。
それ以外ではまったく同じです。
ですから、<literal>a</literal>と<literal>b</literal>の列を持つテーブル<literal>t1</literal>、<literal>t2</literal>に対する以下の2つの文の問い合わせツリーは、␞␞␞
␝␟    are nearly identical.  In particular:␟ほとんど同じです。
以下に、具体的に示します。␞␞␞
␝        <para>␟            The range tables contain entries for the tables <literal>t1</literal> and <literal>t2</literal>.␟範囲テーブルには、テーブル<literal>t1</literal>と<literal>t2</literal>に対する項目があります。␞␞        </para>␞
␝        <para>␟            The target lists contain one variable that points to column
            <literal>b</literal> of the range table entry for table <literal>t2</literal>.␟目的リストにはテーブル<literal>t2</literal>に対する範囲テーブル項目の<literal>b</literal>列を指し示す1つの変数があります。␞␞        </para>␞
␝        <para>␟            The qualification expressions compare the columns <literal>a</literal> of both
            range-table entries for equality.␟条件式は、範囲テーブルの両項目の<literal>a</literal>列の等価性を比較します。␞␞        </para>␞
␝        <para>␟            The join trees show a simple join between <literal>t1</literal> and <literal>t2</literal>.␟結合ツリーは<literal>t1</literal>と<literal>t2</literal>の単純な結合を表しています。␞␞        </para>␞
␝   <para>␟    The consequence is, that both query trees result in similar
...
␝␟    But there is a little problem in
    ...
    that are there has to be replaced by the new row?␟の結果集合とまったく同じ結果集合を作成します。
...
しかし、そこにあるどの行が新しい行によって置換されなければならないのでしょうか。␞␞</para>␞
␝<para>␟    To resolve this problem, another entry is added to the target list
    ...
    <acronym>CTID</acronym> to the target list, the query actually looks like:␟この問題を解決するため、<command>UPDATE</command>文（<command>DELETE</command>文の場合も同様）の目的リストに別の項目が付け加えられます。
...
␝␟    Now another detail of <productname>PostgreSQL</productname> enters
    ...
    the dead row.␟では、<productname>PostgreSQL</productname>の別の詳細説明に入りましょう。
...
このようにして、古い行は隠され、トランザクションがコミットされた後、vacuum掃除機が不必要になった行をそのうちに削除できます。␞␞</para>␞
␝<para>␟    Knowing all that, we can simply apply view rules in absolutely
    the same way to any command. There is no difference.␟これらの詳細が全部理解できれば、どんなコマンドに対してもまったく同じようにしてビューのルールを簡単に適用することができます。
そこには差異がありません。␞␞</para>␞
␝<sect2 id="rules-views-power">␟<title>The Power of Views in <productname>PostgreSQL</productname></title>␟<title><productname>PostgreSQL</productname>におけるビューの能力</title>␞␞␞
␝<para>␟    The above demonstrates how the rule system incorporates view
    ...
    different names).␟ここまでで、ルールシステムがどのようにビューの諸定義を元の問い合わせツリーに組み入れるかを解説しました。
第2の例では、1つのビューからの単純な<command>SELECT</command>によって、最終的に4つのテーブルを結合する問い合わせツリーが生成されました（<literal>unit</literal>は違った名前で2回使われました）。␞␞</para>␞
␝<para>␟    The benefit of implementing views with the rule system is
    ...
    up to that point.␟ビューをルールシステムで実装する利点は、どのテーブルをスキャンすべきか、それらのテーブル間の関連性、ビューからの制約条件、元の問い合わせ条件に関する情報を全て、プランナが1つの問い合わせツリーの中に持っていることです。
元の問い合わせが既にビューに対する結合である時も同様です。
プランナはここでどれが問い合わせ処理の最適経路かを決定しなければなりません。
プランナは保持する情報が多ければ多いほど、より良い決定を下すことができます。
そして<productname>PostgreSQL</productname>に実装されているルールシステムはこれが現時点で、提供されている全ての情報であることを保証します。␞␞</para>␞
␝<sect2 id="rules-views-update">␟<title>Updating a View</title>␟<title>ビューの更新について</title>␞␞␞
␝<para>␟    What happens if a view is named as the target relation for an
    <command>INSERT</command>, <command>UPDATE</command>,
    ...
    These options are discussed below.␟ビューが<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>、<command>MERGE</command>などの対象リレーションとして名付けられた場合はどうなるのでしょうか？
...
これらのオプションについては、以下で説明します。␞␞</para>␞
␝<para>␟    If the subquery selects from a single base relation and is simple
    ...
    <xref linkend="sql-createview"/>.␟副問い合わせが単一の基底リレーションを参照しかつ十分に単純である時、リライタは副問い合わせを基となる基底リレーションに自動的に置き換え、したがって、<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>あるいは<command>MERGE</command>を適切な方法で基底リレーションに適用できます。
この場合の<quote>十分に単純</quote>なビューは<firstterm>自動的に更新可能</firstterm>であると呼ばれます。
自動的に更新可能なビューに関するより詳細な情報については、<xref linkend="sql-createview"/>を参照してください。␞␞</para>␞
␝<para>␟    Alternatively, the operation may be handled by a user-provided
    ...
    to represent the view in its capacity as the result relation.␟もう一つの方法として、ビューに対するユーザ定義の<literal>INSTEAD OF</literal>トリガによってこれらのコマンドを処理できます（<xref linkend="sql-createtrigger"/>を参照してください）。
この場合、書き換えは少々違う形で行われます。
<command>INSERT</command>に対しては、リライタはビューに全く何もせず、問い合わせの結果リレーションをそのままにします。
<command>UPDATE</command>、<command>DELETE</command>、<command>MERGE</command>に対しては、コマンドが更新、削除もしくはマージしようとする<quote>古い</quote>行を生成するためにビュー問い合わせを展開する必要がまだあります。
そのため、ビューは通常通り展開されますが、もう一つの展開されない範囲テーブル項目が結果リレーションとしてビューを表す問い合わせに追加されます。␞␞</para>␞
␝<para>␟    The problem that now arises is how to identify the rows to be
    ...
    out what to update based on the old and new row values.␟ここで起こる問題はビューで更新される行をどのように特定するかということです。
...
新旧の行の値に基づいて更新するものを計算するのはトリガの責任です。␞␞</para>␞
␝<para>␟    Another possibility is for the user to define <literal>INSTEAD</literal>
    ...
    the target relation other than <command>SELECT</command> rules.␟別の方法としては、ビューに対する<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>コマンドに代替の動作を指定する<literal>INSTEAD</literal>ルールを定義する事です。
これらのルールは、ビューではなくコマンドを、通常は1つもしくは複数のテーブルを更新するコマンドに書き換えます。
それが<xref linkend="rules-update"/>の論題になります。
これは<command>MERGE</command>では機能しないことに注意してください。現在、<command>SELECT</command>ルール以外の対象リレーションでのルールをサポートしていません。␞␞</para>␞
␝<para>␟    Note that rules are evaluated first, rewriting the original query
    ...
    used at all.␟ルールが最初に評価され、元の問い合わせが計画され実行される前にそれを書き換えることに注意して下さい。
そのためビューに<literal>INSTEAD OF</literal>トリガと<command>INSERT</command>や<command>UPDATE</command>や<command>DELETE</command>に関するルールがあった場合、ルールが最初に評価され、その結果よってはトリガが全く使われないかもしれません。␞␞</para>␞
␝<para>␟    Automatic rewriting of an <command>INSERT</command>,
    <command>UPDATE</command>, <command>DELETE</command>, or
    ...
    updatable views.␟単純なビューに対する<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>あるいは<command>MERGE</command>問い合わせの自動書き換えは常に最後に試みられます。
したがって、ビューがルールもしくはトリガを持っていた場合、これらは自動的に更新可能なビューのデフォルト動作を上書きします。␞␞</para>␞
␝<para>␟    If there are no <literal>INSTEAD</literal> rules or <literal>INSTEAD OF</literal>
    ...
    be thrown because the executor cannot update a view as such.␟ビューに<literal>INSTEAD</literal>ルールも<literal>INSTEAD OF</literal>トリガも定義されておらず、かつ、リライタが問い合わせを自動的に基となる基底リレーションへの更新に書き換える事ができなかった場合、エグゼキュータはビューを更新できませんのでエラーが発生します。␞␞</para>␞
...
␝<sect1 id="rules-materializedviews">␟<title>Materialized Views</title>␟<title>マテリアライズドビュー</title>␞␞␞
␝<para>␟    Materialized views in <productname>PostgreSQL</productname> use the
    rule system like views do, but persist the results in a table-like form.
    The main differences between:␟<productname>PostgreSQL</productname>におけるマテリアライズドビューはビューのようにルールシステムを使用しますが、あたかもテーブルであるかのような形態で結果を保持します。
以下の␞␞␞
␝␟    and:␟と␞␞␞
␝␟    are that the materialized view cannot subsequently be directly updated
    ...
    can be generated for the materialized view with:␟の間の主な違いは、その後にマテリアライズドビューを直接更新できない事と、マテリアライズドビューを作成するために使われた問い合わせがビューと全く同様の方法で保持され、以下のコマンドを用いて最新のデータでマテリアライズドビューを再構築できる事です。␞␞␞
␝␟    The information about a materialized view in the
    ...
    only used for populating the materialized view.␟マテリアライズドビューに関する情報は<productname>PostgreSQL</productname>システムカタログでビューやテーブルに対するものと全く同様に保持されています。
そのため、パーサにとってマテリアライズドビューはテーブルやビューと同じリレーションです。
問い合わせでマテリアライズドビューが参照された時、あたかもテーブルのように、データはマテリアライズドビューから直接返されます。ルールはマテリアライズドビューにデータを投入する時にだけ使用されます。␞␞</para>␞
␝<para>␟    While access to the data stored in a materialized view is often much
    ...
    Consider a table which records sales:␟多くの場合、マテリアライズドビューに格納されているデータの参照は、ビューを通して、あるいはビューから参照されているテーブルを直接参照するよりも高速ですが、データが常に最新であるとは限りません。ですが、時には最新のデータは必要でない事もあります。
販売履歴を記録するテーブルの例を考えてみましょう。␞␞␞
␝<programlisting>␟CREATE TABLE invoice (
...
␝␟    If people want to be able to quickly graph historical sales data, they
    ...
    for the current date:␟もし利用者が過去の販売データを速やかにグラフ化可能であってほしいと考えるなら、彼らはデータの要約を望むのであって、最新のデータが不完全である事は気にしないでしょう。␞␞␞
␝␟    This materialized view might be useful for displaying a graph in the
    ...
    the statistics each night using this SQL statement:␟このマテリアライズドビューは営業担当用に作成されるダッシュボードのグラフを表示するのにぴったりでしょう。
以下のSQLを使った統計情報を更新するジョブを毎晩スケジュールしておくことができます。␞␞␞
␝<para>␟    Another use for a materialized view is to allow faster access to data
    ...
    foreign data access.␟それ以外のマテリアライズドビューの用途として、外部データラッパーを通じてリモートシステムから取得したデータの高速化が挙げられます。
...
この有利な点は、他の種類の外部データアクセスには当てはまらないでしょう。␞␞</para>␞
␝<para>␟    Setup:␟セットアップ:␞␞␞
␝␟    Now let's spell-check a word.  Using <literal>file_fdw</literal> directly:␟字句のスペルチェックをしてみましょう。
直接<literal>file_fdw</literal>を使って：␞␞␞
␝␟    With <command>EXPLAIN ANALYZE</command>, we see:␟<command>EXPLAIN ANALYZE</command>によれば以下の通りです:␞␞␞
...
␝␟    Either way, the word is spelled wrong, so let's look for what we might
    ...
    <literal>pg_trgm</literal>:␟どちらの場合でも、wordの綴りは間違っています。では、我々が望んでいたであろう結果を得るために、もう一度<literal>file_fdw</literal>と<literal>pg_trgm</literal>を使ってみます。
...
␝␟    If you can tolerate periodic update of the remote data to the local
    database, the performance benefit can be substantial.␟定期的にリモートのデータをローカルに更新せねばならない事を許容できるのであれば、代わりに性能上の便益を得られることでしょう。␞␞</para>␞
...
␝<sect1 id="rules-update">␟<title>Rules on <command>INSERT</command>, <command>UPDATE</command>, and <command>DELETE</command></title>␟<title><command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>についてのルール</title>␞␞␞
␝<para>␟    Rules that are defined on <command>INSERT</command>, <command>UPDATE</command>,
    ...
    RULE</command> command allows more:␟<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>に定義するルールは前節で解説したビューのルールとはまったく異なります。
第一点として、これらの<command>CREATE RULE</command>コマンドでは以下を行うことができます。␞␞␞
␝        <para>␟            They are allowed to have no action.␟アクションがないルールも可能です。␞␞        </para>␞
␝        <para>␟            They can have multiple actions.␟複数のアクションを持てます。␞␞        </para>␞
␝        <para>␟            They can be <literal>INSTEAD</literal> or <literal>ALSO</literal> (the default).␟<literal>INSTEAD</literal>もしくは<literal>ALSO</literal>（デフォルト）を取ることができます。␞␞        </para>␞
␝        <para>␟            The pseudorelations <literal>NEW</literal> and <literal>OLD</literal> become useful.␟疑似リレーション<literal>NEW</literal>と<literal>OLD</literal>が役立つようになります。␞␞        </para>␞
␝        <para>␟            They can have rule qualifications.␟ルール条件を持たせることができます。␞␞        </para>␞
␝␟    Second, they don't modify the query tree in place. Instead they
    create zero or more new query trees and can throw away the
    original one.␟第二点として、その場で問い合わせツリーを変更しません。
その代わりに新規の0個以上の問い合わせツリーを生成して、オリジナルを破棄することができます。␞␞</para>␞
␝ <para>␟  In many cases, tasks that could be performed by rules
  ...
  carrying out the rules.␟多くの場合、<command>INSERT</command>/<command>UPDATE</command>/<command>DELETE</command>におけるルールによって実行できるタスクは、トリガで実行した方が良いでしょう。
トリガは概念としては少し複雑ですが、意味を理解するにはとても単純です。
元の問い合わせにvolatile関数を含む場合、ルールは驚かせる結果を返すことがよくあります。（volatile関数はルールを遂行する過程で予期していた回数より多く実行されてしまうかもしれません）␞␞ </para>␞
␝ <para>␟  Also, there are some cases that are not supported by these types of rules at
  ...
  contrary to the express intent of the query's author.␟また、これらのタイプのルールが全くサポートしない場合もあります。
...
これはルール問い合わせにこれらの構造を複製すると副問い合わせを複数回評価し、問い合わせの作者が表現したかった意図と異なる結果となるためです。␞␞ </para>␞
␝<sect2 id="rules-update-how">␟<title>How Update Rules Work</title>␟<title>更新ルールの動作</title>␞␞␞
␝<para>␟    Keep the syntax:␟␞␞␞
␝␟    in mind.
    In the following, <firstterm>update rules</firstterm> means rules that are defined
    on <command>INSERT</command>, <command>UPDATE</command>, or <command>DELETE</command>.␟上記の構文を覚えておいてください。
以下では、<firstterm>更新ルール</firstterm>は<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>に定義されたルールを意味します。␞␞</para>␞
␝<para>␟    Update rules get applied by the rule system when the result
    ...
    <literal>ALSO</literal> (the default).␟更新ルールは、問い合わせツリーの結果リレーションとコマンド種類が<command>CREATE RULE</command>で与えられるオブジェクトとイベントと等しい場合にルールシステムによって適用されます。
更新ルールに対してルールシステムは問い合わせツリーのリストを生成します。
最初は問い合わせツリーリストは空です。
0（<literal>NOTHING</literal>キーワード）、1つまたは複数のアクションが有効です。
簡単にするため、ここでは1つのアクションのルールを取り上げます。
このルールは条件を持っていても持っていなくても構いませんし、また<literal>INSTEAD</literal>か<literal>ALSO</literal>（デフォルト）のいずれかを取ることができます。␞␞</para>␞
␝<para>␟    What is a rule qualification? It is a restriction that tells
    ...
    special meaning).␟ルール条件とはどんなものでしょうか。
それはルールのアクションを行わなければならない時と、行ってはならない時を指定する条件です。
基本的に（特別な意味合いを持った）オブジェクトとして与えられるリレーションである<literal>NEW</literal>疑似リレーションか<literal>OLD</literal>疑似リレーション、または、その両者のみをこの条件は参照することができます。␞␞</para>␞
␝   <para>␟    So we have three cases that produce the following query trees for
    a one-action rule.␟1アクションのルールに対し、以下の問い合わせツリーを生成する3つの場合があります。␞␞␞
␝     <varlistentry>␟      <term>No qualification, with either <literal>ALSO</literal> or
      <literal>INSTEAD</literal></term>␟      <term><literal>ALSO</literal>または<literal>INSTEAD</literal>で条件がない場合。</term>␞␞      <listitem>␞
␝       <para>␟        the query tree from the rule action with the original query
        tree's qualification added␟元の問い合わせツリーの条件が追加された、ルールアクションからの問い合わせツリー␞␞       </para>␞
␝     <varlistentry>␟      <term>Qualification given and <literal>ALSO</literal></term>␟      <term>条件付き、かつ<literal>ALSO</literal></term>␞␞      <listitem>␞
␝       <para>␟        the query tree from the rule action with the rule
        qualification and the original query tree's qualification
        added␟ルール条件と元の問い合わせツリーの条件が追加された、ルールアクションからの問い合わせツリー␞␞       </para>␞
␝     <varlistentry>␟      <term>Qualification given and <literal>INSTEAD</literal></term>␟      <term>条件付き、かつ<literal>INSTEAD</literal></term>␞␞      <listitem>␞
␝       <para>␟        the query tree from the rule action with the rule
        qualification and the original query tree's qualification; and
        the original query tree with the negated rule qualification
        added␟ルール条件と元の問い合わせツリーの条件が追加された、ルールアクションからの問い合わせツリーとルール条件の否定条件が追加された元の問い合わせツリー␞␞       </para>␞
␝␟    Finally, if the rule is <literal>ALSO</literal>, the unchanged original query tree is
    ...
    for a rule with one action.␟最後に、もしルールが<literal>ALSO</literal>の場合、変更されていない元の問い合わせツリーがリストに付け加えられます。
条件付きの<literal>INSTEAD</literal>ルールのみが既に元の構文解析ツリーに追加をしているので、最後は1つのアクションを持つルールに対して1つもしくは2つの問い合わせツリーにたどり着きます。␞␞</para>␞
␝<para>␟    For <literal>ON INSERT</literal> rules, the original query (if not suppressed by <literal>INSTEAD</literal>)
    ...
    matching their qualifications.␟<literal>ON INSERT</literal>ルールでは、元の問い合わせは、（<literal>INSTEAD</literal>により止められていない限り）ルールによって追加されたアクションの前に実行されます。
...
さもないと、条件に一致する行を見つけることができないためにアクションが作動しなくなる可能性が起こります。␞␞</para>␞
␝<para>␟    The query trees generated from rule actions are thrown into the
    ...
    error.)␟ルールアクションで生成された問い合わせツリーは、再度書き換えシステムに渡され、より多くのルールの適用を受けて問い合わせツリーが増やされたり減らされたりするかもしれません。
ですから、ルールのアクションはルール自身とは異なるコマンド種類か、異なる結果リレーションを持っていなければなりません。
さもないと、この再帰的処理により無限ループに陥ってしまいます。
(ルールの再帰展開は検知され、エラーとして報告されます。)␞␞</para>␞
␝<para>␟    The query trees found in the actions of the
    ...
    result relation.␟<structname>pg_rewrite</structname>システムカタログのアクションにある問い合わせツリーは単なるテンプレートです。
...
<literal>OLD</literal>に対する参照は全て結果リレーションである範囲テーブルの項目への参照に置き換えられます。␞␞</para>␞
␝<para>␟    After the system is done applying update rules, it applies view rules to the
    produced query tree(s).  Views cannot insert new update actions so
    there is no need to apply update rules to the output of view rewriting.␟更新ルールの適用が終わると、システムはそこで作られた構文解析ツリーにビュールールを適用します。
ビューは、新しい更新アクションを挿入できないため、ビュー書き換えの結果に更新ルールを適用する必要はありません。␞␞</para>␞
␝<sect3 id="rules-update-how-first">␟<title>A First Rule Step by Step</title>␟<title>最初のルール、ステップバイステップ</title>␞␞␞
␝<para>␟    Say we want to trace changes to the <literal>sl_avail</literal> column in the
    ...
    <literal>shoelace_data</literal>.␟<literal>shoelace_data</literal>リレーションの<literal>sl_avail</literal>列の変化を追跡してみたいと思います。
そこでログ用テーブルと、<literal>shoelace_data</literal>に対して行われる<command>UPDATE</command>をログに記録するルールを用意しました。␞␞␞
␝<programlisting>␟CREATE TABLE shoelace_log (
...
␝<para>␟    Now someone does:␟ここで誰かが以下を実行します。␞␞␞
␝␟    and we look at the log table:␟ログテーブルを見てみましょう。␞␞␞
␝   <para>␟    That's what we expected. What happened in the background is the following.
    The parser created the query tree:␟思った通りの結果が出ました。
以下に裏で何が起こったのかを説明します。
パーサがまず以下の構文解析ツリーを生成しました。␞␞␞
␝␟    There is a rule <literal>log_shoelace</literal> that is <literal>ON UPDATE</literal> with the rule
    qualification expression:␟ルール<literal>log_shoelace</literal>には、<literal>ON UPDATE</literal>を持つルール条件式␞␞␞
...
␝␟    (This looks a little strange since you cannot normally write
    ...
    the <command>INSERT</command> command's query tree.)␟（通常、<literal>INSERT ... VALUES ... FROM</literal>を書くことはできないのでちょっと奇妙に見えるかもしれません。
ここの<literal>FROM</literal>句は単に<literal>new</literal>と<literal>old</literal>の問い合わせツリーの範囲テーブル項目があることを示しているだけです。
これらは、<command>INSERT</command>コマンドの問い合わせツリー中の変数から参照されるために必要なのです。）␞␞</para>␞
␝<para>␟    The rule is a qualified <literal>ALSO</literal> rule, so the rule system
    ...
    incorporated into the rule's action query tree. This results in:␟このルールは条件付きの<literal>ALSO</literal>ルールですので、ルールシステムは変更されたルールアクションと元の問い合わせツリーという2つの問い合わせツリーを返さなければなりません。
第1の段階で元の問い合わせの範囲テーブルはルールアクション問い合わせツリーに取り込まれます。
そして、次の結果を生みます。␞␞␞
␝␟    In step 2, the rule qualification is added to it, so the result set
    is restricted to rows where <literal>sl_avail</literal> changes:␟第2段階で、以下のようにルール条件が付け加えられます。
...
␝␟    (This looks even stranger, since <literal>INSERT ... VALUES</literal> doesn't have
    ...
    anyway for <literal>INSERT ... SELECT</literal>.)␟（<literal>INSERT ... VALUES</literal>は<literal>WHERE</literal>句を持たないため、これはさらに奇妙です。
しかし、プランナとエグゼキュータには問題はありません。
これらはどのみち<literal>INSERT ... SELECT</literal>のために同じ機能をサポートしなければなりません。）␞␞   </para>␞
␝   <para>␟    In step 3, the original query tree's qualification is added,
    restricting the result set further to only the rows that would have been touched
    by the original query:␟第3段階で、以下のように元の問い合わせツリーの条件が付け加えられ、結果集合は元の問い合わせで変更された行のみにさらに限定されます。␞␞␞
␝   <para>␟    Step 4 replaces references to <literal>NEW</literal> by the target list entries from the
    original query tree or by the matching variable references
    from the result relation:␟第4段階では、以下のように元の問い合わせツリーの目的リスト項目、または結果リレーションの該当する変数参照で、<literal>NEW</literal>の参照を置換します。␞␞␞
␝   <para>␟    Step 5 changes <literal>OLD</literal> references into result relation references:␟第5段階は、以下のように<literal>OLD</literal>参照を結果リレーション参照に置き換えます。␞␞␞
␝   <para>␟    That's it.  Since the rule is <literal>ALSO</literal>, we also output the
    original query tree.  In short, the output from the rule system
    is a list of two query trees that correspond to these statements:␟これで終わりです。このルールは<literal>ALSO</literal>のため、元の問い合わせツリーも出力します。
まとめると、ルールシステムからの出力は以下の文に対応する2つの問い合わせツリーのリストです。␞␞␞
␝␟    These are executed in this order, and that is exactly what
    the rule was meant to do.␟この2つは順番通りに処理され、正確にルールが定義した通りです。␞␞   </para>␞
␝   <para>␟    The substitutions and the added qualifications
    ensure that, if the original query would be, say:␟元の問い合わせが例えば下記のような場合に、置換と追加された条件は、␞␞␞
␝␟    no log entry would get written.  In that case, the original query
    ...
    command generated by the rule is:␟ログには何も書かれないことを確実にします。
...
␝␟    and that qualification will never be true.␟そしてこの条件は決して真にはなりません。␞␞   </para>␞
␝   <para>␟    It will also work if the original query modifies multiple rows. So
    if someone issued the command:␟もし元の問い合わせが複数の行を変更してもうまくいきます。
...
␝␟    four rows in fact get updated (<literal>sl1</literal>, <literal>sl2</literal>, <literal>sl3</literal>, and <literal>sl4</literal>).
    ...
    in the extra query tree:␟この場合、実際には4行が更新されます（<literal>sl1</literal>、<literal>sl2</literal>、<literal>sl3</literal>および<literal>sl4</literal>）。
...
␝␟    being generated by the rule.  This query tree will surely insert
    three new log entries. And that's absolutely correct.␟この構文解析ツリーは確実に3つの新しいログ項目を挿入します。
これはまったく正しい動作です。
[訳注：sl3行はWHERE 0 != shoelace_data.sl_avail条件を満たさない（0!=0）ので、実際に更新される4行-1の3行分のログ項目が挿入されます]。␞␞</para>␞
␝<para>␟    Here we can see why it is important that the original query tree
    ...
    <literal>0 &lt;&gt; shoelace_data.sl_avail</literal>.␟ここで元の構文解析ツリーが最後に実行されるということが重要な理由がわかります。
もし<command>UPDATE</command>が先に実行されたとしたら、全ての行は0にセットされ、<literal>0 &lt;&gt; shoelace_data.sl_avail</literal>である行をログ書き込み時の<command>INSERT</command>の段階で見つけられなくなります。␞␞</para>␞
...
␝<sect2 id="rules-update-views">␟<title>Cooperation with Views</title>␟<title>ビューとの協調</title>␞␞␞
␝<para>␟    A simple way to protect view relations from the mentioned
    ...
    to let those query trees get thrown away.  So we could create the rules:␟誰かがビューに対して<command>INSERT</command>、<command>UPDATE</command>、<command>DELETE</command>を発行するといった、前述の可能性からビューリレーションを保護する簡単な方法は、それらの構文解析ツリーを破棄してしまうことです。
...
␝␟    If someone now tries to do any of these operations on the view
    ...
    executed after the rule system is done with it.␟誰かが<literal>shoe</literal>ビューリレーションに上記の操作を行おうとすると、ルールシステムはルールを適用します。
ルールにはアクションがなく、かつ、<literal>INSTEAD</literal>ですから、結果の問い合わせツリーリストは空になります。
ルールシステムの処理が完了した後に最適化されるものや実行されるべきものが何も残っていませんので、問い合わせ全体が無効になります。␞␞</para>␞
␝<para>␟    A more sophisticated way to use the rule system is to
    ...
    the following rules:␟より洗練されたルールシステムの使用方法は、実テーブルに適当な操作を行う問い合わせツリーへの書き換えを行うルールを作ることです。
<literal>shoelace</literal>ビューにこれを適用するために以下のルールを作ります。␞␞␞
␝   <para>␟    If you want to support <literal>RETURNING</literal> queries on the view,
    ...
    <literal>shoelace</literal>.  An example for the insert case is:␟このビュー上で<literal>RETURNING</literal>問い合わせをサポートしたい場合、ビューの行を計算する<literal>RETURNING</literal>句を含むルールを作成しなければなりません。
...
␝␟    Note that this one rule supports both <command>INSERT</command> and
    ...
    <literal>RETURNING</literal> clause is simply ignored for <command>INSERT</command>.␟この1つのルールが、ビューに対する<command>INSERT</command>問い合わせと<command>INSERT RETURNING</command>問い合わせルールをサポートすることに注意してください。
<command>INSERT</command>では<literal>RETURNING</literal>句が無視されるだけです。␞␞   </para>␞
␝   <para>␟    Now assume that once in a while, a pack of shoelaces arrives at
    ...
    trick. The creation commands for these are:␟ここで店には不定期に靴紐のケースが分厚い送り状とともに届けられると仮定します。
...
␝␟    Now you can fill the table <literal>shoelace_arrive</literal> with
    the data from the parts list:␟これで、送り状のデータを<literal>shoelace_arrive</literal>テーブルに投入することができます。␞␞␞
...
␝   <para>␟    It's a long way from the one <literal>INSERT ... SELECT</literal>
    to these results. And the description of the query-tree
    transformation will be the last in this chapter.  First, there is
    the parser's output:␟1つの<literal>INSERT ... SELECT</literal>からこの結果になるには、長い道のりがあります。
...
␝␟    Now the first rule <literal>shoelace_ok_ins</literal> is applied and turns this
    into:␟最初の<literal>shoelace_ok_ins</literal>ルールが適用され、結果は以下のようになります。␞␞␞
␝␟    and throws away the original <command>INSERT</command> on
    ...
    <literal>shoelace_upd</literal> produces:␟そして、<literal>shoelace_ok</literal>に対する元の<command>INSERT</command>を破棄します。
...
␝␟    Again it's an <literal>INSTEAD</literal> rule and the previous query tree is trashed.
    ...
    and applies the <literal>_RETURN</literal> rule on it, and we get:␟これは再び<literal>INSTEAD</literal>ルールですので、以前の問い合わせツリーは破棄されます。
...
␝␟    Finally, the rule <literal>log_shoelace</literal> gets applied,
    producing the extra query tree:␟最後に、<literal>log_shoelace</literal>ルールが適用され、以下のような特別な問い合わせツリーが生成されます。␞␞␞
␝␟    After that the rule system runs out of rules and returns the
    generated query trees.␟この後、ルールシステムはルールを使い切り、生成された問い合わせツリーを返します。␞␞   </para>␞
␝   <para>␟    So we end up with two final query trees that are equivalent to the
    <acronym>SQL</acronym> statements:␟そして、以下の<acronym>SQL</acronym>文と等価となる2つの最終問い合わせツリーで終結します。␞␞␞
␝␟    The result is that data coming from one relation inserted into another,
    ...
    gets reduced into two queries.␟結果は、1つのリレーションから来たデータが別のリレーションに挿入され、3つ目のリレーションの更新に変更され、4つ目の更新と5つ目への最終更新のログ記録に変更され、最終的に2つの問い合わせに縮小されます。␞␞</para>␞
␝<para>␟    There is a little detail that's a bit ugly. Looking at the two
    ...
    <command>INSERT</command> will be␟ちょっと見苦しい小さな事項があります。
...
␝␟    while omitting the extra range table entry would result in a␟一方、余計な範囲テーブル項目を省略することで、␞␞␞
␝␟    which produces exactly the same entries in the log table.  Thus,
    ...
    that all possible at all.␟ログテーブルにまったく同じ項目が作られます。
...
しかしながら、これらを全て可能にするのは大変な仕事です。␞␞</para>␞
␝<para>␟    Now we make a final demonstration of the
    <productname>PostgreSQL</productname> rule system and its power.
    Say you add some shoelaces with extraordinary colors to your
    database:␟最後に<productname>PostgreSQL</productname>のルールシステムとその効力を示しましょう。
...
␝␟    We would like to make a view to check which
    ...
    The view for this is:␟全ての靴に合わない色が<literal>shoelace</literal>項目にあるかどうかを検査するビューを作成したいと考えます。
ビューは以下のようになります。␞␞␞
␝␟    Its output is:␟この出力は以下のようになります。␞␞␞
␝   <para>␟    Now we want to set it up so that mismatching shoelaces that are
    not in stock are deleted from the database.
    To make it a little harder for <productname>PostgreSQL</productname>,
    we don't delete it directly. Instead we create one more view:␟ここで、合う靴がない靴紐のうち、在庫がないものをデータベースから削除するように設定してみます。
...
␝   <para>␟    A <command>DELETE</command> on a view, with a subquery qualification that
    ...
    from a real table.␟合計4つのネスト/結合されたビューを副問い合わせの条件として持ち、その中の1つはビューを含む副問い合わせ条件を持ち、かつ演算を施されたビューの列が使われる場合の、ビューに対する<command>DELETE</command>が、実テーブルから要求されたデータを削除する単一の問い合わせツリーに書き換えられます。␞␞</para>␞
␝<para>␟    There are probably only a few situations out in the real world
    where such a construct is necessary. But it makes you feel
    comfortable that it works.␟このような構造が必要な状況は実社会ではほとんどないと思われます。
しかし、実際に動くことを確認できれば安心できます。␞␞</para>␞
...
␝<sect1 id="rules-privileges">␟<title>Rules and Privileges</title>␟<title>ルールと権限</title>␞␞␞
␝<para>␟    Due to rewriting of queries by the <productname>PostgreSQL</productname>
    ...
    to tables.␟<productname>PostgreSQL</productname>のルールシステムによる問い合わせの書き換えによって、オリジナルの問い合わせで使われたものではない他のテーブル/ビューがアクセスされます。
更新ルールを使うことによってテーブルへの書き込みアクセスを包含することができます。␞␞</para>␞
␝<para>␟    Rewrite rules don't have a separate owner. The owner of
    ...
    their queries.␟書き換えルールに別々の所有者はいません。
...
このことは、セキュリティ実行者ビューを除き、ユーザは問い合わせで明記するテーブル/ビューに対しての権限だけあればよいことを示しています。␞␞</para>␞
␝<para>␟    For example: A user has a list of phone numbers where some of
    them are private, the others are of interest for the assistant of the office.
    The user can construct the following:␟例えば、以下のようにします。
...
␝␟    Nobody except that user (and the database superusers) can access the
    ...
    assistant can use it.␟そのユーザ（とデータベースのスーパーユーザ）以外は<literal>phone_data</literal>テーブルにアクセスできません。
...
<literal>phone_number</literal>へのアクセスもチェックされますが、これは呼び出したユーザに対して行われますので、秘書とユーザ以外は使うことができません。␞␞</para>␞
␝<para>␟    The privileges are checked rule by rule. So the assistant is for now the
    ...
    access to the assistant's view would fail.␟権限はルールごとにチェックされます。
...
秘書のビューへのアクセスは即座に失敗に終わります。␞␞</para>␞
␝<para>␟    One might think that this rule-by-rule checking is a security
    ...
    think it over and then use <command>REVOKE</command>.␟このルールごとのチェックがセキュリティホールになると考える人がいるかもしれませんが、実際にはそうではありません。
...
信用している誰かがこのようなことを行った場合は、考えを変えて<command>REVOKE</command>してください。␞␞</para>␞
␝<para>␟    Note that while views can be used to hide the contents of certain
    ...
    the following view is insecure:␟上に示したような手法を使ってある列の内容を隠すのにビューは使えますが、<literal>security_barrier</literal>フラグが設定されていない限り、見えない行にあるデータを隠す信頼できる方法ではない事に注意してください。
例えば、以下のビューは安全ではありません。␞␞<programlisting>␞
␝</programlisting>␟    This view might seem secure, since the rule system will rewrite any
    <command>SELECT</command> from <literal>phone_number</literal> into a
    ...
    For example:␟ルールシステムが<literal>phone_number</literal>からの<command>SELECT</command>を<literal>phone_data</literal>からの<command>SELECT</command>に書き換え、<literal>phone</literal>が412で始まらない項目のみが必要だという条件を追加しますので、このビューは安全に見えます。
しかし、ユーザが自身の関数を作成できるのであれば、<function>NOT LIKE</function>式の前にユーザ定義の関数を実行するようプランナを説得することは難しくありません。
例えば以下の通りです。␞␞<programlisting>␞
␝</programlisting>␟    Every person and phone number in the <literal>phone_data</literal> table will be
    ...
    input values in the error messages they produce.)␟プランナはより高価な<function>NOT LIKE</function>の前に安価な<function>tricky</function>関数を実行することを選びますので、<literal>phone_data</literal>テーブルの人と電話番号はすべて<literal>NOTICE</literal>として表示されます。
たとえユーザが新しい関数を定義できない場合でも、同様の攻撃で組み込み関数が使えます。
（例えば、ほとんどの型変換関数は生成するエラーメッセージに入力値を含んでいます。）␞␞</para>␞
␝<para>␟    Similar considerations apply to update rules. In the examples of
    ...
    <literal>INSERT</literal>.  This might not be true in more complex scenarios.␟同様の考慮は更新ルールにも適用できます。
...
これはより複雑な状況では正しくないかもしれません。␞␞</para>␞
␝<para>␟    When it is necessary for a view to provide row-level security, the
    ...
    be secure:␟ビューが行単位セキュリティを提供する場合には、そのビューには<literal>security_barrier</literal>属性を付与するべきです。
これは、悪意を持って選ばれた関数や演算子が、ビューが適用されるより前に渡された行に対して実行されないようにします。
例えば、上記のビューが次のように定義された場合、これは安全です。␞␞<programlisting>␞
␝</programlisting>␟    Views created with the <literal>security_barrier</literal> may perform
    ...
    enabled by default.␟<literal>security_barrier</literal>を付けて定義されたビューは、このオプションなしのビューよりも性能の劣る問い合わせ実行プランを生成します。一般的に、最も高速だが、セキュリティ上の問題がある問い合わせ実行プランを破棄しなければならないという状況は不可避です。そのため、このオプションはデフォルトでは有効になっていません。␞␞</para>␞
␝<para>␟    The query planner has more flexibility when dealing with functions that
    ...
    view's row filters.␟プランナは副作用が無い関数をもう少し柔軟に扱います。
...
これがもしセキュリティ・ビューの条件句でフィルタリングされるより前に適用されたなら、不可視行に関する何か重要な情報を与える事ができてしまいます。␞␞</para>␞
␝<para>␟    It is important to understand that even a view created with the
    ...
    to the data at all.␟ビューが<literal>security_barrier</literal>属性付きで定義されたとしても、それは限定的な意味で安全である、つまり不可視な行の内容が潜在的に安全でない関数に渡される事がないという事を意図しているにすぎません。
...
もし、この種の"隠れチャネル"攻撃に対する対策が必要であれば、とにかくデータに対するアクセスを許可するのは得策ではありません。␞␞</para>␞
␝<sect1 id="rules-status">␟<title>Rules and Command Status</title>␟<title>ルールおよびコマンドの状態</title>␞␞␞
␝<para>␟    The <productname>PostgreSQL</productname> server returns a command
    ...
    involved, but what happens when the query is rewritten by rules?␟<productname>PostgreSQL</productname>サーバでは、受け取った問い合わせのそれぞれに対して、<literal>INSERT 149592 1</literal>のようなコマンド状態文字列を返します。
これは簡単ですが、ルールが使用されていない場合には十分なものです。
しかし、問い合わせがルールにより書き換えられた場合、どのようになるでしょうか。␞␞</para>␞
␝<para>␟    Rules affect the command status as follows:␟ルールはコマンド状態に以下のように影響を与えます。␞␞␞
␝      <para>␟       If there is no unconditional <literal>INSTEAD</literal> rule for the query, then
       ...
       be affected.)␟問い合わせに無条件の<literal>INSTEAD</literal>ルールが使用されていない場合、元々与えられていた問い合わせが実行され、そのコマンド状態は通常通り返されます。
（しかし、条件付き<literal>INSTEAD</literal>ルールが使用されている場合、その条件の否定が元の問い合わせに追加されることに注意してください。
これにより、処理する行の数が減り、その結果報告される状態が影響を受けるかもしれません）。␞␞      </para>␞
␝      <para>␟       If there is any unconditional <literal>INSTEAD</literal> rule for the query, then
       ...
       zeroes for the row-count and OID fields.␟問い合わせに無条件の<literal>INSTEAD</literal>ルールが使用されている場合、元の問い合わせはまったく実行されません。
...
この条件に合致する問い合わせがルールによって追加されない場合、返されるコマンド状態は、元の問い合わせの種類と行数およびOIDフィールドに0が表示されます。␞␞      </para>␞
␝<para>␟    The programmer can ensure that any desired <literal>INSTEAD</literal> rule is the one
    ...
    gets applied last.␟後者の場合、プログラマは、有効ルールの中でアルファベット順の最後のルール名を与えることによって、必要な<literal>INSTEAD</literal>ルールを最後に実行することができます。そして、そのことによって、コマンド状態が確実にそのルールで設定されるようにできます。␞␞</para>␞
...
␝<sect1 id="rules-triggers">␟<title>Rules Versus Triggers</title>␟<title>ルール対トリガ</title>␞␞␞
␝<para>␟    Many things that can be done using triggers can also be
    ...
    be generated, it must be done by a trigger.␟トリガによって行われる多くの操作は<productname>PostgreSQL</productname>のルールシステムで実装可能です。
ルールで実装できないものの1つはある種の制約、特に外部キーに関してです。
もし他のテーブルに列の値がなかった場合、条件ルールでコマンドを<literal>NOTHING</literal>に書き換えてしまうことも可能です。
しかし、これではデータがだまって消去されてしまい、良いアイディアとは言えません。
有効な値のチェックが必要で、無効な値の場合にエラーメッセージを生成する必要がある場合、トリガで行う必要があります。␞␞</para>␞
␝<para>␟    In this chapter, we focused on using rules to update views. All of
    ...
    logic is required to perform the update.␟この章ではビューを更新するのにルールを使うことに焦点を当ててきました。
この章の更新ルールの例はすべてビューの<literal>INSTEAD OF</literal>トリガを使っても実装できます。
特に更新を実行するのに複雑な論理が要求される場合には、そのようなトリガを書くことはしばしばルールを書くよりも簡単です。␞␞</para>␞
␝<para>␟    For the things that can be implemented by both, which is best
    ...
    simpler than the rule approach, and is easier for novices to get right.␟どちらでも実装できる事項に関してどちらがベストかはデータベースの使用方法によります。
トリガは各行に対して一度起動します。
...
しかし、トリガ方式は概念的にルールシステムよりかなり単純であり、初心者は簡単に正しく扱うことができます。␞␞</para>␞
␝<para>␟    Here we show an example of how the choice of rules versus triggers
    plays out in one situation.  There are two tables:␟ここで、ある状況下でルールとトリガのどちらを選択するかを示す例を挙げます。
例えば、2つのテーブルがあるとします。␞␞␞
␝<programlisting>␟CREATE TABLE computer (
...
␝␟    Both tables have many thousands of rows and the indexes on
    ...
    that reference a deleted computer.  The trigger would use this command:␟2つのテーブルにはともに数千の行があって、<structfield>hostname</structfield>上のインデックスは一意です。
ルール/トリガは削除されたホストを参照する、<literal>software</literal>の行を削除する制約を実装しなければなりません。
...
␝␟    Since the trigger is called for each individual row deleted from
    ...
    parameter.  The rule would be written as:␟<literal>computer</literal>から削除された行1つひとつに対してこのトリガが呼び出されますので、このコマンドの準備を行い、計画を保存し、パラメータとして<structfield>hostname</structfield>を渡すことができます。
ルールの場合は以下のように作成されます。␞␞␞
␝   <para>␟    Now we look at different types of deletes. In the case of a:␟ここで別の類の削除を考えてみましょう。以下のような場合には、␞␞␞
␝␟    the table <literal>computer</literal> is scanned by index (fast), and the
    ...
    The extra command from the rule would be:␟<literal>computer</literal>はインデックスにより（高速に）スキャンされます。
...
␝␟    Since there are appropriate indexes set up, the planner
    will create a plan of␟適切なインデックスが設定されていますので、プランナは以下の計画を作成します。␞␞␞
␝␟    So there would be not that much difference in speed between
    the trigger and the rule implementation.␟ですので、トリガとルールの実装間での速度差はあまりありません。␞␞   </para>␞
␝   <para>␟    With the next delete we want to get rid of all the 2000 computers
    where the <structfield>hostname</structfield> starts with
    <literal>old</literal>. There are two possible commands to do that. One
    is:␟次の削除処理では<structfield>hostname</structfield>が<literal>old</literal>で始まる2,000台全てのcomputerを削除しようと思います。
方法として2つの有効な問い合わせがあります。
1つは以下のようなものです。␞␞␞
...
␝␟    which results in the following executing plan for the command
    added by the rule:␟これにより、ルールによって追加されるコマンド用の実行計画は以下のようになります。␞␞␞
␝␟    This shows, that the planner does not realize that the
    ...
    manager take some time, even if all the index blocks will soon be in the cache.␟これが示していることは、<literal>AND</literal>で結合された複数の検索条件が存在する場合、プランナは正規表現版のコマンドでは行っていることですが、<literal>computer</literal>上の<structfield>hostname</structfield>に対する検索条件を<literal>software</literal>上のインデックススキャンにも同様に使用できることを理解しないということです。
...
参照する全てのインデックスブロックがすぐにキャッシュに現れるとしても、トリガによるSPIマネージャ経由の2,000回のコマンドの実行には時間を要します。␞␞</para>␞
␝<para>␟    The last command we look at is:␟最後のコマンドを見てみましょう。␞␞␞
␝␟    Again this could result in many rows to be deleted from
    <literal>computer</literal>. So the trigger will again run many commands
    through the executor.  The command generated by the rule will be:␟この文でも<literal>computer</literal>から多くの行が削除される結果となります。
...
␝␟    The plan for that command will again be the nested loop over two
    index scans, only using a different index on <literal>computer</literal>:␟このコマンド用の計画もまた前回同様2つのインデックススキャンのネステッドループとなります。
...
␝␟    In any of these cases, the extra commands from the rule system
    ...
    in a command.␟いずれの場合においても、ルールシステムが生成する追加コマンドは影響を受ける行数からは多かれ少なかれ独立しています。␞␞</para>␞
␝<para>␟    The summary is, rules will only be significantly slower than
    triggers if their actions result in large and badly qualified
    joins, a situation where the planner fails.␟まとめると、問い合わせ結果が大きく、プランナがうまく結合条件を設定できないような状況下でのみルールはトリガに比べて明らかに遅くなります。␞␞</para>␞
