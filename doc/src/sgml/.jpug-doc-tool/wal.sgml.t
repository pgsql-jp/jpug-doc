␝<chapter id="wal">␟ <title>Reliability and the Write-Ahead Log</title>␟ <title>信頼性と先行書き込みログ（WAL）</title>␞␞␞
␝ <para>␟  This chapter explains how to control the reliability of
  <productname>PostgreSQL</productname>, including details about the
  Write-Ahead Log.␟本章では、先行書き込みログに関する詳細を含めて、<productname>PostgreSQL</productname>の信頼性を制御する方法を説明します。␞␞ </para>␞
␝ <sect1 id="wal-reliability">␟  <title>Reliability</title>␟  <title>信頼性</title>␞␞␞
␝  <para>␟   Reliability is an important property of any serious database
   ...
   transactions will remain intact.␟信頼性は、すべての本格的なデータベースシステムで重要な特性です。
...
実際、コンピュータに致命的な障害が発生したとしても、もしディスク装置が無事ならば、類似のハードウェアを持つ別のコンピュータに移すことができ、コミットされたトランザクションを元通りに復元できます。␞␞  </para>␞
␝  <para>␟   While forcing data to the disk platters periodically might seem like
   ...
   to adjust how this is done.)␟データを周期的にディスクプラッタに書き出すことは簡単な操作に思われるかもしれませんが、そうではありません。
...
（これを調整する方法については<xref linkend="guc-wal-sync-method"/>パラメータを参照してください。）␞␞  </para>␞
␝  <para>␟   Next, there might be a cache in the disk drive controller; this is
   ...
   is restored the data will be written to the disk drives.␟次に、ディスク装置のコントローラキャッシュが存在する可能性があります。
...
後で電源が復旧した後に、データがディスク装置に書き出されます。␞␞  </para>␞
␝  <para>␟   And finally, most disk drives have caches. Some are write-through
   ...
   solid-state drives (SSD) also have volatile write-back caches.␟最後に、ほとんどのディスク装置がキャッシュを持っています。一部はwrite-throughであり、一部はwrite-backです。
...
多くのソリッドステートドライブ(SSD)も同様に揮発性のwrite-backキャッシュを持っています。␞␞  </para>␞
␝  <para>␟   These caches can typically be disabled; however, the method for doing
   this varies by operating system and drive type:␟これらのキャッシュは、大抵は無効にできます。しかしながらオペレーティングシステムやドライブの種類によってその方法は異なります。␞␞  </para>␞
␝      <para>␟        On <productname>Linux</productname>, IDE and SATA drives can be queried using
        ...
        to disable it.␟<productname>Linux</productname>上で<command>hdparm -I</command>を使用することでIDEおよびSATAドライブのキャッシュについて調べることができます。
...
<command>sdparm --get=WCE</command>によりキャッシュが有効かどうかの確認ができ、<command>sdparm --clear=WCE</command>により無効にすることができます。␞␞      </para>␞
␝      <para>␟        On <productname>FreeBSD</productname>, IDE drives can be queried using
        ...
        <command>sdparm</command> when available.␟<productname>FreeBSD</productname>では、IDEドライブに対して<command>camcontrol identify</command>により確認ができ、そして書き込みキャッシュを無効にするには<filename>/boot/loader.conf</filename>の<literal>hw.ata.wc=0</literal>を利用します。SCSIドライブに対しては<command>camcontrol identify</command>を確認に使用することができ、<command>sdparm</command>を使用できる場合にはそれを用いて書き込みキャッシュの確認と変更が可能です。␞␞      </para>␞
␝      <para>␟        On <productname>Solaris</productname>, the disk write cache is controlled by
        <command>format -e</command>.
        (The Solaris <acronym>ZFS</acronym> file system is safe with disk write-cache
        enabled because it issues its own disk cache flush commands.)␟<productname>Solaris</productname>では、ディスクの書き込みキャッシュは<command>format -e</command>で制御できます。
(Solarisの<acronym>ZFS</acronym>ファイルシステムは、独自のディスクキャッシュ書き出しコマンドを発行しているため、ディスクの書き込みキャッシュを有効にしても安全です。)␞␞      </para>␞
␝      <para>␟        On <productname>Windows</productname>, if <varname>wal_sync_method</varname> is
        ...
        which prevent write caching.␟<productname>Windows</productname>では、もし<varname>wal_sync_method</varname>が<literal>open_datasync</literal>（デフォルト）の場合、<literal>My Computer\Open\<replaceable>disk drive</replaceable>\Properties\Hardware\Properties\Policies\Enable write caching on the disk</literal>のチェックを外すことで、書き込みキャッシュを無効にできます。
もう一つの方法としては、<varname>wal_sync_method</varname>を<literal>fdatasync</literal>（NTFSのみ）または<literal>fsync</literal>に設定し、書き込みキャッシュを使用しないようにします。␞␞      </para>␞
␝      <para>␟        On <productname>macOS</productname>, write caching can be prevented by
        setting <varname>wal_sync_method</varname> to <literal>fsync_writethrough</literal>.␟<productname>macOS</productname>では、<varname>wal_sync_method</varname>を<literal>fsync_writethrough</literal>に設定することで書き込みキャッシュを使用しないようにします。␞␞      </para>␞
␝  <para>␟   Recent SATA drives (those following <acronym>ATAPI-6</acronym> or later)
   ...
   address this suboptimal behavior.␟最近のSATAドライブ(<acronym>ATAPI-6</acronym>またはそれ以降)はドライブキャッシュの書き出しコマンド(<command>FLUSH CACHE EXT</command>)を提供している一方、SCSIドライブでは従来から類似の<command>SYNCHRONIZE CACHE</command>コマンドをサポートしていました。
...
ファイルシステムやディスクコントローラの設計者が、いずれはこの動作を修正してくれることが望まれます。␞␞  </para>␞
␝  <para>␟   When the operating system sends a write request to the storage hardware,
   ...
   url="https://brad.livejournal.com/2116715.html"><filename>diskchecker.pl</filename></ulink>.␟オペレーティングシステムが、ストレージハードウェアに書き込み要求を送信した時、データが不揮発性のストレージ領域に本当に届いたかどうかを確認することはほぼできません。
...
<ulink url="https://brad.livejournal.com/2116715.html"><filename>diskchecker.pl</filename></ulink>を使うことで、I/Oサブシステムの動作の信頼性をテストすることができます。␞␞  </para>␞
␝  <para>␟   Another risk of data loss is posed by the disk platter write
   ...
   they guarantee that data is written to the BBU as full (8kB) pages.␟ディスクプラッタの書き込み操作自体によってもデータ損失が発生することがあります。
...
もし、部分的なページ書き込みを防止できるファイルシステムソフトウェア（例えばZFS）を使うのであれば、<xref linkend="guc-full-page-writes"/>を無効にしてページイメージ作成を無効にすることができます。バッテリバックアップ付き（BBU）のディスクコントローラでは、フルページ（8kB）がBBUへ書き込まれることを保証できなければ、部分的なページ書き出しを防止できません。␞␞  </para>␞
␝  <para>␟   <productname>PostgreSQL</productname> also protects against some kinds of data corruption
   on storage devices that may occur because of hardware errors or media failure over time,
   such as reading/writing garbage data.␟さらに<productname>PostgreSQL</productname>は、ハードウェアエラーや経時変化によるメディア障害により発生する、ごみデータを読み書きしてしまうようなストレージ装置内のある種のデータ破損を防ぎます。␞␞   <itemizedlist>␞
␝     <para>␟      Each individual record in a WAL file is protected by a CRC-32C (32-bit) check
      ...
      archive recovery and replication.␟WALファイルのそれぞれのレコードは、レコードの内容が正確かどうかを伝えるためCRC-32C (32-bit)チェックにより保護されています。
CRCの値はそれぞれのWALレコードを書き込む時に設定され、クラッシュリカバリ、アーカイブリカバリとレプリケーション時に検証されます。␞␞     </para>␞
␝     <para>␟      Data pages are not currently checksummed by default, though full page images
      recorded in WAL records will be protected; see <link
      linkend="app-initdb-data-checksums"><application>initdb</application></link>
      for details about enabling data checksums.␟今のところ、デフォルトではデータページはチェックサム計算はされませんが、WALレコードに記録されているページ全体のイメージは保護されます。
データチェックサム有効化についての詳細は<link linkend="app-initdb-data-checksums"><application>initdb</application></link>を参照してください。␞␞     </para>␞
␝     <para>␟      Internal data structures such as <filename>pg_xact</filename>, <filename>pg_subtrans</filename>, <filename>pg_multixact</filename>,
      ...
      WAL records are protected as discussed above.␟<filename>pg_xact</filename>、<filename>pg_subtrans</filename>、<filename>pg_multixact</filename>、<filename>pg_serial</filename>、<filename>pg_notify</filename>、<filename>pg_stat</filename>、<filename>pg_snapshots</filename>のような内部データ構造は直接チェックサム計算もされず、全ページ書き込みによる保護もされていません。
しかし、そのようなデータ構造が永続する場所では、クラッシュリカバリ時に直近の更新が正確に再構築されるようにWALレコードが書き出され、それらのWALレコードは上記のように保護されます。␞␞     </para>␞
␝     <para>␟      Individual state files in <filename>pg_twophase</filename> are protected by CRC-32C.␟<filename>pg_twophase</filename>にある個別の状態ファイルはCRC-32Cで保護されています。␞␞     </para>␞
␝     <para>␟      Temporary data files used in larger SQL queries for sorts,
      materializations and intermediate results are not currently checksummed,
      nor will WAL records be written for changes to those files.␟大きな問い合わせの中でソート、具現化、および中間結果用に使用される暫定的なデータファイルは現在チェックサム計算されず、それらのファイルに対する変更もWALレコードに書き込まれません。␞␞     </para>␞
␝  <para>␟   <productname>PostgreSQL</productname> does not protect against correctable memory errors
   and it is assumed you will operate using RAM that uses industry standard
   Error Correcting Codes (ECC) or better protection.␟<productname>PostgreSQL</productname>は修復可能なメモリエラーに対して保護を行いません。業界標準の誤り検出訂正（Error Correcting Codes -ECC-）またはそれ以上の保護付きのRAM使用が想定されています。␞␞  </para>␞
␝ <sect1 id="checksums">␟  <title>Data Checksums</title>␟  <title>データチェックサム</title>␞␞  <indexterm>␞
␝  <para>␟   By default, data pages are not protected by checksums, but this can
   ...
   structures and temporary files are not.␟デフォルトでは、データページはチェックサムで保護されていませんが、オプションでデータベースクラスタに対して有効にすることができます。
チェックサムを有効にすると、各データページにチェックサムが含まれます。
チェックサムは、ページが書き込まれるときに更新され、ページが読み取られるたびに検証されます。
チェックサムによってデータページのみ保護されます。
内部データ構造と一時ファイルは保護されません。␞␞  </para>␞
␝  <para>␟   Checksums can be enabled when the cluster is initialized using <link
   linkend="app-initdb-data-checksums"><application>initdb</application></link>.
   They can also be enabled or disabled at a later time as an offline
   operation. Data checksums are enabled or disabled at the full cluster
   level, and cannot be specified individually for databases or tables.␟チェックサムは、<link linkend="app-initdb-data-checksums"><application>initdb</application></link>を使用してデータベースクラスタを初期化するときに有効にできます。
また、オフライン操作で後から有効化または無効化することもできます。
データチェックサムは、データベースクラスタ全体のレベルで有効または無効になり、データベースやテーブルに対して個別に指定することはできません。␞␞  </para>␞
␝  <para>␟   The current state of checksums in the cluster can be verified by viewing the
   value of the read-only configuration variable <xref
   linkend="guc-data-checksums" /> by issuing the command <command>SHOW
   data_checksums</command>.␟データベースクラスタのチェックサムの現在の状態は、<command>SHOW data_checksums</command>コマンドを実行して読み取り専用設定変数<xref linkend="guc-data-checksums" />の値を参照することで確認できます。␞␞  </para>␞
␝  <para>␟   When attempting to recover from page corruptions, it may be necessary to
   bypass the checksum protection. To do this, temporarily set the
   configuration parameter <xref linkend="guc-ignore-checksum-failure" />.␟ページ破損からの復旧を試みる場合、チェックサム保護のバイパスが必要な場合があります。
これを行うには、一時的に設定パラメータ<xref linkend="guc-ignore-checksum-failure" />を設定します。␞␞  </para>␞
␝  <sect2 id="checksums-offline-enable-disable">␟   <title>Off-line Enabling of Checksums</title>␟   <title>オフラインでのチェックサムの有効化</title>␞␞␞
␝   <para>␟    The <link linkend="app-pgchecksums"><application>pg_checksums</application></link>
    application can be used to enable or disable data checksums, as well as
    verify checksums, on an offline cluster.␟<link linkend="app-pgchecksums"><application>pg_checksums</application></link>アプリケーションは、オフラインのデータベースクラスタ上でデータチェックサムを有効または無効にしたり、チェックサムを検証したりできます。␞␞   </para>␞
␝   <indexterm>
...
␝  <sect1 id="wal-intro">␟   <title>Write-Ahead Logging (<acronym>WAL</acronym>)</title>␟   <title>先行書き込みログ(<acronym>WAL</acronym>)</title>␞␞␞
␝   <para>␟    <firstterm>Write-Ahead Logging</firstterm> (<acronym>WAL</acronym>)
    ...
    roll-forward recovery, also known as REDO.)␟<firstterm>先行書き込みログ</firstterm>（<acronym>WAL</acronym>）はデータの一貫性を確実にするための標準的な手法です。
...
リカバリの時点でデータページに対してまだ行われていない変更分は、WALレコードを使って再実行されます（これがREDOとして知られているロールフォワードリカバリです）。␞␞   </para>␞
␝    <para>␟     Because <acronym>WAL</acronym> restores database file
     ...
     Journaled file systems do improve boot speed after a crash.␟<acronym>WAL</acronym>によりデータベースファイルの中身を障害後にリストアするため、信頼性のある格納領域にあるデータファイルやWALファイルに対しては、ジャーナルファイルシステムは必要ありません。
...
ジャーナルファイルシステムは障害後の起動速度を改善します。␞␞    </para>␞
␝   <para>␟    Using <acronym>WAL</acronym> results in a
    ...
    suffice to commit many transactions.␟<acronym>WAL</acronym>を使用することでディスクへの書き込み回数が大幅に減少します。
...
さらに、サーバが小規模なトランザクションを同時に多く処理する時、WALファイルを一度<function>fsync</function>することで、多くのトランザクションをコミットすることができる場合もあります。␞␞   </para>␞
␝   <para>␟    <acronym>WAL</acronym> also makes it possible to support on-line
    ...
    inconsistencies.␟また、<acronym>WAL</acronym>により、<xref linkend="continuous-archiving"/>で説明するオンラインバックアップとポイントインタイムリカバリをサポートできます。
...
ある程度の時間を経過して作成されたバックアップであっても、その期間用のWALを再生することにより、内部の不整合を修復します。␞␞   </para>␞
␝   <indexterm>
...
␝ <sect1 id="wal-async-commit">␟  <title>Asynchronous Commit</title>␟  <title>非同期コミット</title>␞␞␞
␝  <para>␟   <firstterm>Asynchronous commit</firstterm> is an option that allows transactions
   ...
   acceptable trade-off.␟<firstterm>非同期コミット</firstterm>とは、トランザクションをより高速に完了することができるオプションです。
もっとも最近のトランザクションがデータベースがクラッシュしてしまった場合に失われるという危険があります。
これは、多くのアプリケーションで受け入れられるトレードオフです。␞␞  </para>␞
␝  <para>␟   As described in the previous section, transaction commit is normally
   ...
   in throughput for small transactions.␟前節で説明した通り、通常トランザクションのコミットは<firstterm>同期的</firstterm>です。
...
これにより、小規模なトランザクションでスループットがかなり向上します。␞␞  </para>␞
␝  <para>␟   Asynchronous commit introduces the risk of data loss. There is a short
   ...
   guarantee of this kind.␟非同期コミットにはデータ損失の危険があります。
...
しかし、イベントロギングなど多くのシナリオでは、この種の強力な保証は必要ありません。␞␞  </para>␞
␝  <para>␟   The risk that is taken by using asynchronous commit is of data loss,
   ...
   effects are preserved.␟非同期コミットによりもたらされる危険性は、データの破損ではなくデータの損失です。
...
例えば、トランザクションBが以前に行われたトランザクションAの結果に依存した変更を行った場合、Bの影響が保存されている限り、Aの影響が失われることは起こり得ません。␞␞  </para>␞
␝  <para>␟   The user can select the commit mode of each transaction, so that
   ...
   <varname>synchronous_commit</varname> when transaction commit begins.␟ユーザは各トランザクションでコミットモードを選択することができます。
...
あるひとつのトランザクションで使用されるモードは、トランザクションのコミットが始まった時の<varname>synchronous_commit</varname>の値に依存します。␞␞  </para>␞
␝  <para>␟   Certain utility commands, for instance <command>DROP TABLE</command>, are
   ...
   TRANSACTION</command>, are also always synchronous.␟例えば<command>DROP TABLE</command>などの特定のユーティリティコマンドでは、<varname>synchronous_commit</varname>の設定に関わらず、強制的に同期的コミットが行われます。
これにより、サーバのファイルシステムとデータベースの論理的な状態との間の一貫性が保証されます。
<command>PREPARE TRANSACTION</command>などの二相コミットをサポートするコマンドもまた、常に同期的です。␞␞  </para>␞
␝  <para>␟   If the database crashes during the risk window between an
   ...
   designed to favor writing whole pages at a time during busy periods.␟もし非同期コミットとそのトランザクションの<acronym>WAL</acronym>レコードの書き込みの間の危険期間にデータベースがクラッシュしたとすると、そのトランザクションでなされた変更は失われる<emphasis>でしょう</emphasis>。
...
WALライタは稼働中に一回ページ全体を書き込むように設計されているため、危険期間の実際の最大の長さは<varname>wal_writer_delay</varname>の３倍です。␞␞  </para>␞
␝   <para>␟    An immediate-mode shutdown is equivalent to a server crash, and will
    therefore cause loss of any unflushed asynchronous commits.␟即時モードのシャットダウンはサーバクラッシュと同じことですので、フラッシュされていない非同期コミットが失われることになります。␞␞   </para>␞
␝  <para>␟   Asynchronous commit provides behavior different from setting
   ...
   of data corruption.␟非同期コミットでは<xref linkend="guc-fsync"/> = offという設定とは異なる動作になります。
...
非同期コミットはデータ破損の危険性はなく、多くの状況では<varname>fsync</varname>を無効にした場合に得られる性能向上とほぼ同等の性能を提供します。␞␞  </para>␞
␝  <para>␟   <xref linkend="guc-commit-delay"/> also sounds very similar to
   ...
   flush, to amortize the cost of the flush among multiple transactions.␟また<xref linkend="guc-commit-delay"/>も非同期コミットと類似のように見えますが、これは実のところ同期コミットの一方法です。
...
この設定は、複数のトランザクションの中でフラッシュのコストを償却するために、トランザクションが一回のフラッシュに参加しようとするグループに参加できる時間的猶予を広げる方法として考えることができます。␞␞  </para>␞
␝ <sect1 id="wal-configuration">␟  <title><acronym>WAL</acronym> Configuration</title>␟  <title><acronym>WAL</acronym>の設定</title>␞␞␞
␝  <para>␟   There are several <acronym>WAL</acronym>-related configuration parameters that
   affect database performance. This section explains their use.
   Consult <xref linkend="runtime-config"/> for general information about
   setting server configuration parameters.␟データベースの性能に影響するような<acronym>WAL</acronym>に関連した設定パラメータが複数あります。
本節では、その使い方を説明します。
サーバ設定パラメータの設定方法についての詳細は<xref linkend="runtime-config"/>を参照してください。␞␞  </para>␞
␝  <para>␟   <firstterm>Checkpoints</firstterm><indexterm><primary>checkpoint</primary></indexterm>
   ...
   archived before being recycled or removed.)␟<firstterm>チェックポイント</firstterm><indexterm><primary>checkpoint</primary></indexterm>は、一連のトランザクションにおいて、そのチェックポイント以前に書かれた全ての情報によりヒープとインデックスファイルがすでに更新されていることを保証する時点です。
...
(<acronym>WAL</acronym>アーカイブが行われる場合、このWALセグメントは削除もしくは再利用される前に保存されなければなりません。)␞␞  </para>␞
␝  <para>␟   The checkpoint requirement of flushing all dirty data pages to disk
   ...
   degradation during checkpoints.␟チェックポイント処理は、全てのダーティデータページをディスクへフラッシュするため、大きなI/O負荷を発生させます。
チェックポイント処理においては、I/Oはチェックポイント開始時に始まり、次のチェックポイントが開始する前に完了するように調節されます。
これは、チェックポイント処理中の性能劣化を極力抑える効果があります。␞␞  </para>␞
␝  <para>␟   The server's checkpointer process automatically performs
   ...
   command <command>CHECKPOINT</command>.␟サーバのチェックポインタプロセスは、自動的にチェックポイントを時々実行します。
...
また、<command>CHECKPOINT</command> SQLコマンドで強制的にチェックポイントを作成することもできます。␞␞  </para>␞
␝  <para>␟   Reducing <varname>checkpoint_timeout</varname> and/or
   ...
   and in any case causing more disk I/O.␟<varname>checkpoint_timeout</varname>または<varname>max_wal_size</varname>、あるいはその両者を減少させると、チェックポイントはより頻繁に行われます。
...
また、確実により多くのディスクI/Oが発生します。␞␞  </para>␞
␝  <para>␟   Checkpoints are fairly expensive, first because they require writing
...
<varname>max_wal_size</varname>を十分高く設定していないと、大規模な<command>COPY</command>転送などのまとまった操作でこうした警告が多く発生するかもしれません。␞␞  </para>␞
␝  <para>␟   To avoid flooding the I/O system with a burst of page writes,
   ...
   unexpected variation in the number of WAL segments needed.␟ページ書き出しの集中によるI/Oシステムの溢れを防ぐために、チェックポイント期間のダーティバッファの書き出しは一定の期間に分散されます。
...
これは必要なWALセグメント数が想定以上に変動することになり、性能の劣化が発生することになります。␞␞  </para>␞
␝  <para>␟   On Linux and POSIX platforms <xref linkend="guc-checkpoint-flush-after"/>
   ...
   <xref linkend="guc-shared-buffers"/>, but smaller than the OS's page cache.␟LinuxおよびPOSIXプラットフォームでは、チェックポイントによって書かれたページを、設定したバイト数の後にディスクにフラッシュさせるように<xref linkend="guc-checkpoint-flush-after"/>を使ってOSに強制させることができます。
...
しかし、とりわけワークロードが<xref linkend="guc-shared-buffers"/>よりも大きく、かつOSのページキャッシュよりも小さい場合には性能上不利になることもあります。␞␞  </para>␞
␝  <para>␟   The number of WAL segment files in <filename>pg_wal</filename> directory depends on
   ...
   WAL is needed.␟<filename>pg_wal</filename>ディレクトリ内のWALセグメントファイルの数は、<varname>min_wal_size</varname>、<varname>max_wal_size</varname>、それに前回のチェックポイントで生成されたWALの量に依存します。
...
システムがアイドル状態にあり、WALの使用量を見積った結果、少ないWALしか必要ないとなったとしても、こうした量のWALファイルは必ず再利用されます。␞␞  </para>␞
␝  <para>␟   Independently of <varname>max_wal_size</varname>,
   ...
   until they are summarized.␟<varname>max_wal_size</varname>に関わらず、最新の<xref linkend="guc-wal-keep-size"/>メガバイトのWALファイルに加えて、もう一つのWALファイルが常に保持されます。
...
同様に、<link linkend="runtime-config-wal-summarization">WAL要約</link>が有効な場合、古いセグメントは要約されるまで保持されます。␞␞  </para>␞
␝  <para>␟   In archive recovery or standby mode, the server periodically performs
   ...
   <link linkend="monitoring-pg-stat-checkpointer-view"><structname>pg_stat_checkpointer</structname></link>
   view counts the restartpoints that have really been performed.␟アーカイブリカバリもしくはスタンバイモードにおいて、サーバでは定期的に通常運用でのチェックポイント処理と似た<firstterm>リスタートポイント</firstterm><indexterm><primary>restartpoint</primary></indexterm>処理を行います。これは、すでに再生されたWALを再度読み込む必要がないよう、ディスクに現在の状態を強制的に書き込み、<filename>pg_control</filename>ファイルを更新します。また<filename>pg_wal</filename>ディレクトリの中の古いWALセグメントを再利用できるようにします。
リスタートポイント処理はチェックポイントレコードに基づいてのみ実行されるため、プライマリ側のチェックポイント処理よりも頻繁に実行されることはありません。
リスタートポイントはスケジュールまたは外部リクエストによって要求されます。
<link linkend="monitoring-pg-stat-checkpointer-view"><structname>pg_stat_checkpointer</structname></link>ビューの中の<structfield>restartpoints_timed</structfield>カウンタは最初のリスタートポイントを数え、<structfield>restartpoints_req</structfield>は2番目のリスタートポイントを数えます。
リスタートポイントは、チェックポイントレコードに到達し、前回のリスタートポイント処理から<xref linkend="guc-checkpoint-timeout"/>秒以上経過している場合、または前回のリスタートポイント処理が失敗した場合にスケジュールされます。
この場合、次のリスタートポイントは15秒後にスケジュールされます。
リスタートポイントは、チェックポイントと同様の理由で要求されることがありますが、ほとんどはWALサイズが<xref linkend="guc-max-wal-size"/>を超えそうな場合です。
しかし、リスタートポイント処理が実行できるタイミングに制限があるため、リカバリ中には、1回のチェックポイント分のWAL相当が<varname>max_wal_size</varname>を超えることが頻繁にあります。
（どのみち<varname>max_wal_size</varname>はハードリミットではないので、ディスクスペースを使い尽くしてしまわないように、常に十分な余裕を持っておくべきです。）
<link linkend="monitoring-pg-stat-checkpointer-view"><structname>pg_stat_checkpointer</structname></link>ビューの中の<structfield>restartpoints_done</structfield>カウンタは、実際に実行されたリスタートポイントを数えます。␞␞  </para>␞
␝  <para>␟   In some cases, when the WAL size on the primary increases quickly,
   ...
   resources have been spent.␟場合によっては、プライマリのWALサイズが急速に増加すると、例えば大規模なINSERT中のインスタンスでは、スタンバイの<structfield>restartpoints_req</structfield>カウンタがピークの増加を示すことがあります。
これは、最後のリスタートポイント以降の安全なチェックポイントレコードがスタンバイでまだ再生されていないため、XLOG消費の増加による新規リスタートポイントの作成要求を実行できないために発生します。
この動作は正常であり、システムリソースの消費の増加にはつながりません。
リスタートポイントに関連するカウンタの中で、顕著なシステムリソースの消費を示すのは<structfield>restartpoints_done</structfield>カウンタのみです。␞␞  </para>␞
␝  <para>␟   There are two commonly used internal <acronym>WAL</acronym> functions:
   ...
   during the period immediately following each checkpoint.␟よく使われる2つの内部用<acronym>WAL</acronym>関数があります。
...
<xref linkend="guc-full-page-writes"/>が設定され、かつ、システムが高負荷状態である場合、<varname>wal_buffers</varname>を高くすることで、各チェックポイントの直後の応答時間を滑らかにすることができます。␞␞  </para>␞
␝  <para>␟   The <xref linkend="guc-commit-delay"/> parameter defines for how many
   ...
   the parameter.␟<xref linkend="guc-commit-delay"/>パラメータは、<function>XLogFlush</function>内でロックを取得してからグループコミット上位者が何マイクロ秒休止するかを定義します。一方、グループコミット追従者は上位者の後に並びます。
...
いくつかのプラットフォームで、休止操作はパラメータによって要求された時間よりわずかに長くなることも覚えておいてください。␞␞  </para>␞
␝  <para>␟   Since the purpose of <varname>commit_delay</varname> is to allow the
   ...
   throughput suffers.␟<varname>commit_delay</varname>の目的は、それぞれのフラッシュ操作のコストを並列にコミット中のトランザクションに（潜在的にはトランザクションの待ち時間と引き換えに）分散させることにあり、うまく設定を行うためには、まずそのコストを測る必要があります。
...
余りにも高い値の<varname>commit_delay</varname>を設定すると、トランザクション遅延を増加させかねないことになり、トランザクションの総スループットが低下します。␞␞  </para>␞
␝  <para>␟   When <varname>commit_delay</varname> is set to zero (the default), it
   ...
   committing client with one sibling transaction).␟<varname>commit_delay</varname>が(デフォルトの)ゼロに設定されても、グループコミットが起こることがあります。
...
しかし、回転待ち時間が長い場合、この設定はわずか二つのクライアントにおいてさえトランザクションスループットを向上させる効果があるかもしれません（言いかえれば、一つの兄弟（sibling）トランザクションを所有する単一のコミット中のクライアントです）。␞␞  </para>␞
␝  <para>␟   The <xref linkend="guc-wal-sync-method"/> parameter determines how
   ...
   has been turned off.␟<xref linkend="guc-wal-sync-method"/>パラメータは<productname>PostgreSQL</productname>がカーネルに対して<acronym>WAL</acronym>更新のディスクへの書き込みを要求する方法を決定します。
...
ちなみに、このパラメータは<varname>fsync</varname>が無効になっている場合は役に立ちません。␞␞  </para>␞
␝  <para>␟   Enabling the <xref linkend="guc-wal-debug"/> configuration parameter
   (provided that <productname>PostgreSQL</productname> has been
   compiled with support for it) will result in each
   <function>XLogInsertRecord</function> and <function>XLogFlush</function>
   <acronym>WAL</acronym> call being logged to the server log. This
   option might be replaced by a more general mechanism in the future.␟<xref linkend="guc-wal-debug"/>設定パラメータを有効にすることで、<function>XLogInsertRecord</function>と<function>XLogFlush</function>という<acronym>WAL</acronym>呼び出しは毎回サーバログにログが残ります
（このパラメータをサポートするように<productname>PostgreSQL</productname>をコンパイルする必要があります）。
将来このオプションはより一般的な機構に置き換わる可能性があります。␞␞  </para>␞
␝  <para>␟   There are two internal functions to write WAL data to disk:
   ...
   in <structname>pg_stat_wal</structname>, respectively.␟WALデータをディスクに書き込むための2つの内部関数があります。
...
<varname>track_wal_io_timing</varname>の設定に関係なく、<function>XLogWrite</function>の書き込み回数と<function>issue_xlog_fsync</function>のディスクへのWALデータの同期回数もそれぞれ<structname>pg_stat_wal</structname>の<literal>wal_write</literal>と<literal>wal_sync</literal>としてカウントされます。␞␞  </para>␞
␝  <para>␟   The <xref linkend="guc-recovery-prefetch"/> parameter can be used to reduce
   ...
   <function>posix_fadvise</function> is available.␟<xref linkend="guc-recovery-prefetch"/>パラメータは、すぐに必要になるが現在<productname>PostgreSQL</productname>のバッファプールにないディスクブロックの読み取りを開始するようカーネルに指示することにより、リカバリ中の入出力待ち時間を減らすために使用できます。
<xref linkend="guc-maintenance-io-concurrency"/>と<xref linkend="guc-wal-decode-buffer-size"/>の設定は、プリフェッチの並列度と先読み量をそれぞれ制限します。
デフォルトでは<literal>try</literal>に設定されており、<function>posix_fadvise</function>が利用可能なシステムでこの機能が有効になります。␞␞  </para>␞
␝ <sect1 id="wal-internals">␟  <title>WAL Internals</title>␟  <title>WALの内部</title>␞␞␞
␝  <para>␟   <acronym>WAL</acronym> is automatically enabled; no action is
   ...
   linkend="wal-configuration"/>).␟<acronym>WAL</acronym>は自動的に有効になります。
<acronym>WAL</acronym>ファイルが必要とするディスク容量を確保することと、必要なチューニングを実施すること（<xref linkend="wal-configuration"/>を参照）以外は、管理者は何もする必要はありません。␞␞  </para>␞
␝  <para>␟   <acronym>WAL</acronym> records are appended to the <acronym>WAL</acronym>
   ...
   and recovery.␟新しいレコードが作成されるごとに、<acronym>WAL</acronym>レコードが<acronym>WAL</acronym>ファイルに追加されます。
...
2つの<acronym>LSN</acronym>値を比較することで<acronym>WAL</acronym>データの差分量を計算することができるので、レプリケーションやリカバリの進捗状況を測定できます。␞␞  </para>␞
␝  <para>␟   <acronym>WAL</acronym> files are stored in the directory
   ...
   available stock of numbers.␟<acronym>WAL</acronym>ファイルは、データディレクトリ以下の<filename>pg_wal</filename>ディレクトリに、通常16メガバイトのサイズを持つセグメントファイルの集合として格納されています(ただし、このサイズは<application>initdb</application>の<option>--wal-segsize</option>オプションで変更できます)。
...
数字は巡回しませんが、利用可能な数字を使い尽くすには非常に長い時間がかかるはずです。␞␞  </para>␞
␝  <para>␟   It is advantageous if the WAL is located on a different disk from the
   main database files.  This can be achieved by moving the
   <filename>pg_wal</filename> directory to another location (while the server
   is shut down, of course) and creating a symbolic link from the
   original location in the main data directory to the new location.␟主要なデータベースファイルが置いてあるディスクとは別のディスクにWALを置くと利点があります。
これは<filename>pg_wal</filename>ディレクトリを別の場所に（もちろんサーバを終了しておいてから）移動し、主データディレクトリ以下の元々の場所から新しい場所へのシンボリックリンクを張ることによって可能となります。␞␞  </para>␞
␝  <para>␟   The aim of <acronym>WAL</acronym> is to ensure that the log is
   ...
   (See <xref linkend="wal-reliability"/>.)␟<acronym>WAL</acronym>の目的である、確実にデータベースレコードが変更される前にログが書き出されることは、実際にはキャッシュにしかデータがなく、ディスクには格納されていない時にディスクドライブ<indexterm><primary>ディスクドライブ</primary></indexterm>が格納に成功したとカーネルに虚偽の報告を行うことによって失われる可能性があります。
そのような状況では、電源が落ちた際に、復旧不可能なデータ破損が起こることがあります。
管理者は、<productname>PostgreSQL</productname>の<acronym>WAL</acronym>ファイルを保持しているディスク装置がそのような嘘の報告をしないように保証するべきです。(<xref linkend="wal-reliability"/>を参照して下さい。)␞␞  </para>␞
␝  <para>␟   After a checkpoint has been made and the WAL flushed, the
   ...
   state.␟チェックポイントが実行され、WALがフラッシュされた後、チェックポイントの位置は<filename>pg_control</filename>に保存されます。
...
データページの内容全体は、チェックポイント後の最初のページ変更時にWAL内に保存されますので(<xref linkend="guc-full-page-writes"/>パラメータが無効にされていないという前提です)、そのチェックポイント以降に変更された全てのページは一貫した状態に復旧されます。␞␞  </para>␞
␝  <para>␟   To deal with the case where <filename>pg_control</filename> is
   ...
   seem to be a problem in practice.␟<filename>pg_control</filename>が壊れた場合に備え、既存のWALセグメントを逆順に読み（すなわち新しいものから古いものへと）、最終チェックポイントを見つける方法を実際には実装した方が良いと思われます。
まだこれはできていません。
...
このため、<filename>pg_control</filename>は理屈では弱点ですが、実質問題になりません。␞␞  </para>␞
