...
␝  <refname>TRUNCATE</refname>␟  <refpurpose>empty a table or set of tables</refpurpose>␟  <refpurpose>1テーブルまたはテーブル群を空にする</refpurpose>␞␞ </refnamediv>␞
␝ <refsect1>␟  <title>Description</title>␟  <title>説明</title>␞␞␞
␝  <para>␟   <command>TRUNCATE</command> quickly removes all rows from a set of
   ...
   operation. This is most useful on large tables.␟<command>TRUNCATE</command>はテーブル群から全ての行を素早く削除します。
...
このコマンドは、大きなテーブルを対象とする場合に最も有用です。␞␞  </para>␞
␝ <refsect1>␟  <title>Parameters</title>␟  <title>パラメータ</title>␞␞␞
␝     <para>␟      The name (optionally schema-qualified) of a table to truncate.
      ...
      descendant tables are included.␟空にするテーブルの名前です（スキーマ修飾名も可）。
...
␝     <para>␟      Automatically restart sequences owned by columns of
      the truncated table(s).␟消去されるテーブルの列により所有されるシーケンスを自動的に再開始させます。␞␞     </para>␞
␝     <para>␟      Do not change the values of sequences.  This is the default.␟シーケンスの値を変更しません。これがデフォルトです。␞␞     </para>␞
␝     <para>␟      Automatically truncate all tables that have foreign-key references
      to any of the named tables, or to any tables added to the group
      due to <literal>CASCADE</literal>.␟指定されたテーブル、または、<literal>CASCADE</literal>により削除対象テーブルとされたテーブルを参照する外部キーを持つテーブルすべてを自動的に空にします。␞␞     </para>␞
␝     <para>␟      Refuse to truncate if any of the tables have foreign-key references
      from tables that are not listed in the command.  This is the default.␟外部キーにより対象のテーブルを参照するテーブルのいずれかがこのコマンドで指定されていない場合、操作を拒否します。
これがデフォルトです。␞␞     </para>␞
␝ <refsect1>␟  <title>Notes</title>␟  <title>注釈</title>␞␞␞
␝  <para>␟   You must have the <literal>TRUNCATE</literal> privilege on a table
   to truncate it.␟テーブルを空にするためにはそのテーブルに<literal>TRUNCATE</literal>権限を持たなければなりません。␞␞  </para>␞
␝  <para>␟   <command>TRUNCATE</command> acquires an <literal>ACCESS EXCLUSIVE</literal> lock on each
   ...
   the <command>DELETE</command> command should be used instead.␟<command>TRUNCATE</command>は操作対象の各テーブルに対する<literal>ACCESS EXCLUSIVE</literal>ロックを獲得します。
これは、この他のそのテーブルに対する同時操作をすべてブロックします。
<literal>RESTART IDENTITY</literal>が指定された場合、初期化対象のシーケンスがあると、それは同様に排他ロックされます。
テーブルへの同時アクセスが必要ならば、代わりに <command>DELETE</command>コマンドを使用しなければなりません。␞␞  </para>␞
␝  <para>␟   <command>TRUNCATE</command> cannot be used on a table that has foreign-key
   ...
   referencing tables and all their partitions with no distinction.␟そのテーブルが他のテーブルから外部キーで参照されている場合、その同じコマンドでそれらのテーブルをすべて空にするように指定していない限り、<command>TRUNCATE</command>を使用することはできません。
...
空にするテーブルがパーティションの場合、兄弟のパーティションには手をつけませんが、参照しているテーブルすべてとそのパーティションすべてに対しては、区別することなくカスケードが起こります。␞␞  </para>␞
␝  <para>␟   <command>TRUNCATE</command> will not fire any <literal>ON DELETE</literal>
   ...
   that were added due to cascading).␟<command>TRUNCATE</command>は、テーブルに<literal>ON DELETE</literal>トリガがあっても、それを発行しません。
...
トリガは処理されるテーブルの順番（コマンドに列挙されたものが先、その後にカスケードのために追加されたもの）に発行されます。␞␞  </para>␞
␝  <para>␟   <command>TRUNCATE</command> is not MVCC-safe.  After truncation, the table will
   appear empty to concurrent transactions, if they are using a snapshot
   taken before the truncation occurred.
   See <xref linkend="mvcc-caveats"/> for more details.␟<command>TRUNCATE</command>はMVCC的に安全ではありません。
同時実行中のトランザクションが、削除の前に取得したスナップショットを使っている場合、削除の後、テーブルはそのトランザクションからは空に見えます。
（詳しくは<xref linkend="mvcc-caveats"/>を参照してください。）␞␞  </para>␞
␝  <para>␟   <command>TRUNCATE</command> is transaction-safe with respect to the data
   in the tables: the truncation will be safely rolled back if the surrounding
   transaction does not commit.␟テーブル内のデータという観点では、<command>TRUNCATE</command>はトランザクション的に安全です。
前後のトランザクションがコミットされなければ消去は安全にロールバックされます。␞␞  </para>␞
␝  <para>␟   When <literal>RESTART IDENTITY</literal> is specified, the implied
   ...
   a failed transaction.␟<literal>RESTART IDENTITY</literal>が指定された場合、暗黙的に<command>ALTER SEQUENCE RESTART</command>操作がトランザクション的に行われます。
...
これは、失敗したトランザクションの後の<function>currval()</function>の通常の動作と同じです。␞␞  </para>␞
␝  <para>␟   <command>TRUNCATE</command> can be used for foreign tables if
   supported by the foreign data wrapper, for instance,
   see <xref linkend="postgres-fdw"/>.␟<command>TRUNCATE</command>は、外部データラッパーによりサポートされていれば、外部テーブルに対して使えます。例えば、<xref linkend="postgres-fdw"/>を参照してください。␞␞  </para>␞
␝ <refsect1>␟  <title>Examples</title>␟  <title>例</title>␞␞␞
␝  <para>␟   Truncate the tables <literal>bigtable</literal> and
   <literal>fattable</literal>:␟<literal>bigtable</literal>テーブルおよび<literal>fattable</literal>テーブルを空にします。␞␞␞
␝  <para>␟   The same, and also reset any associated sequence generators:␟以下も同じですが、ここでは関連するシーケンスジェネレータをすべてリセットします。␞␞␞
␝  <para>␟   Truncate the table <literal>othertable</literal>, and cascade to any tables
   that reference <literal>othertable</literal> via foreign-key
   constraints:␟<literal>othertable</literal>テーブル、および、外部キー制約により<literal>othertable</literal>を参照するすべてのテーブルを空にします。␞␞␞
␝ <refsect1>␟  <title>Compatibility</title>␟  <title>互換性</title>␞␞␞
␝  <para>␟   The SQL:2008 standard includes a <command>TRUNCATE</command> command
   ...
   considered and compared with other implementations if necessary.␟標準SQL:2008には、<literal>TRUNCATE TABLE <replaceable>tablename</replaceable></literal>という構文の<command>TRUNCATE</command>コマンドが含まれます。
...
このため、上記注釈を検討し、必要に応じて他の実装と比べなければなりません。␞␞  </para>␞
␝ <refsect1>␟  <title>See Also</title>␟  <title>関連項目</title>␞␞␞
␝␟TRUNCATE othertable CASCADE; </programlisting></para> </programlisting></para>␟no translation␞␞␞
