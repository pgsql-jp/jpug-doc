...
␝  <refname>REINDEX</refname>␟  <refpurpose>rebuild indexes</refpurpose>␟  <refpurpose>インデックスを再構築する</refpurpose>␞␞ </refnamediv>␞
...
␝  <para>␟   <command>REINDEX</command> rebuilds an index using the data
   stored in the index's table, replacing the old copy of the index. There are
   several scenarios in which to use <command>REINDEX</command>:␟<command>REINDEX</command>は、インデックスのテーブルに保存されたデータを使用してインデックスを再構築し、古いインデックスのコピーと置き換えます。
以下に<command>REINDEX</command>が使用される状況を示します。␞␞␞
␝     <para>␟      An index has become corrupted, and no longer contains valid
      ...
      recovery method.␟インデックスが破損してしまい、有効なデータがなくなった場合です。
理論的には決して起こらないはずですが、実際には、ソフトウェアのバグやハードウェアの障害によりインデックスが破損することがあります。
<command>REINDEX</command>はこの修復手段を提供します。␞␞     </para>␞
␝     <para>␟      An index has become <quote>bloated</quote>, that is it contains many
      ...
      linkend="routine-reindex"/> for more information.␟インデックスが<quote>膨張状態</quote>、つまり、多くの空、もしくは、ほとんど空のページを持つ状態になっている場合です。
...
詳細は<xref linkend="routine-reindex"/>を参照してください。␞␞     </para>␞
␝     <para>␟      You have altered a storage parameter (such as fillfactor)
      for an index, and wish to ensure that the change has taken full effect.␟インデックスの格納パラメータ（フィルファクタなど）を変更し、この変更を確実に有効にしたい場合です。␞␞     </para>␞
␝     <para>␟      If an index build fails with the <literal>CONCURRENTLY</literal> option,
      ...
      to perform a concurrent build on an invalid index.␟<literal>CONCURRENTLY</literal>オプションをつけたインデックス作成が失敗すると、このインデックスは<quote>無効</quote>として残されます。
こうしたインデックスは使用されませんが、<command>REINDEX</command>を使用して再作成するのが便利かもしれません。
<command>REINDEX INDEX</command>だけが無効なインデックスでの同時構築を実行できることに注意してください。␞␞     </para>␞
␝ <refsect1>␟  <title>Parameters</title>␟  <title>パラメータ</title>␞␞␞
␝     <para>␟      Recreate the specified index. This form of <command>REINDEX</command>
      cannot be executed inside a transaction block when used with a
      partitioned index.␟指定したインデックスを再作成します。
この構文の<command>REINDEX</command>は、パーティションインデックスと使われる場合にはトランザクションブロック内で実行できません。␞␞     </para>␞
␝     <para>␟      Recreate all indexes of the specified table.  If the table has a
      secondary <quote>TOAST</quote> table, that is reindexed as well.
      This form of <command>REINDEX</command> cannot be executed inside a
      transaction block when used with a partitioned table.␟指定したテーブルの全インデックスを再作成します。
テーブルに2次的な<quote>TOAST</quote>テーブルがある場合、それについてもインデックスを再作成します。
この構文の<command>REINDEX</command>は、パーティションテーブルと使われる場合にはトランザクションブロック内で実行できません。␞␞     </para>␞
␝     <para>␟      Recreate all indexes of the specified schema.  If a table of this
      ...
      This form of <command>REINDEX</command> cannot be executed inside a
      transaction block.␟指定したスキーマのすべてのインデックスを再作成します。
このスキーマのテーブルが二次的な<quote>TOAST</quote>テーブルを持っている場合は、そのインデックスも再作成されます。
共有システムカタログのインデックスも処理されます。
この構文の<command>REINDEX</command>はトランザクションブロック内で実行できません。␞␞     </para>␞
␝     <para>␟      Recreate all indexes within the current database, except system
      catalogs.
      Indexes on system catalogs are not processed.
      This form of <command>REINDEX</command> cannot be executed inside a
      transaction block.␟システムカタログを除く、現在のデータベースのすべてのインデックスを再作成します。
システムカタログのインデックスは処理されません。
この構文の<command>REINDEX</command>をトランザクションブロック内で実行できません。␞␞     </para>␞
␝     <para>␟      Recreate all indexes on system catalogs within the current database.
      Indexes on shared system catalogs are included.
      Indexes on user tables are not processed.
      This form of <command>REINDEX</command> cannot be executed inside a
      transaction block.␟現在のデータベースのシステムカタログに対するすべてのインデックスを再作成します。
共有システムカタログのインデックスも含みます。
ユーザテーブルのインデックスは処理されません。
この構文の<command>REINDEX</command>をトランザクションブロック内で実行できません。␞␞     </para>␞
␝     <para>␟      The name of the specific index, table, or database to be
      ...
      and it must match the current database's name.␟インデックスを再作成するインデックス、テーブル、データベースの名前です。
...
このパラメータは省略可能で、現在のデータベース名と一致する必要があります。␞␞     </para>␞
␝     <para>␟      When this option is used, <productname>PostgreSQL</productname> will rebuild the
      index without taking any locks that prevent concurrent inserts,
      updates, or deletes on the table; whereas a standard index rebuild
      locks out writes (but not reads) on the table until it's done.
      There are several caveats to be aware of when using this option
      &mdash; see <xref linkend="sql-reindex-concurrently"/> below.␟このオプションが使われると、<productname>PostgreSQL</productname>は、そのテーブルで同時実行される挿入、更新、削除を妨げるようなロックを取得せずにインデックスを再構築します。一方、標準のインデックス再構築は終了するまでテーブルの書き込みをロックします（読み込みはロックしません）。
このオプションを使用する場合に注意すべき点がいくつかあります&mdash;下記の<xref linkend="sql-reindex-concurrently"/>を参照してください。␞␞     </para>␞
␝     <para>␟      For temporary tables, <command>REINDEX</command> is always
      non-concurrent, as no other session can access them, and
      non-concurrent reindex is cheaper.␟一時テーブルに対しては<command>REINDEX</command>は常に同時再作成ではありません。他のセッションはアクセスできませんし、同時でないインデックス再作成の方がより安価だからです。␞␞     </para>␞
␝     <para>␟      Specifies that indexes will be rebuilt on a new tablespace.␟新しいテーブル空間でインデックスを再構築することを指定します。␞␞     </para>␞
␝     <para>␟      Prints a progress report as each index is reindexed.␟各インデックスが再作成されるときに、進捗レポートを表示します。␞␞     </para>␞
...
␝     <para>␟      The tablespace where indexes will be rebuilt.␟インデックスが再構築されるテーブル空間です。␞␞     </para>␞
␝ <refsect1>␟  <title>Notes</title>␟  <title>注釈</title>␞␞␞
␝  <para>␟   If you suspect corruption of an index on a user table, you can
   simply rebuild that index, or all indexes on the table, using
   <command>REINDEX INDEX</command> or <command>REINDEX TABLE</command>.␟ユーザテーブル上の特定のインデックスに破損の疑いがある場合、<command>REINDEX INDEX</command>を使ってそのインデックスを再構築することもできますし、<command>REINDEX TABLE</command>を使ってそのテーブルのすべてのインデックスを再構築することもできます。␞␞  </para>␞
␝  <para>␟   Things are more difficult if you need to recover from corruption of
   ...
   indexes for system catalog lookups.␟システムテーブルのインデックスの破損を復旧する場合の手順はより複雑になります。
この場合、システムによって破損の可能性があるインデックス自体が使用されないようにすることが重要です。
（実際は、このようなケースでは、破損したインデックスに依存していたため、サーバプロセスが起動時に強制終了してしまう可能性があります。）
安全に復旧させるには、システムカタログ検索時のインデックスの使用を禁止する<option>-P</option>オプションを使用してサーバを起動しなければなりません。␞␞  </para>␞
␝  <para>␟   One way to do this is to shut down the server and start a single-user
   ...
   interface.␟考えられる方法の1つは次の方法です。まず、サーバを停止して、コマンドラインから<option>-P</option>オプションを指定してシングルユーザ状態の<productname>PostgreSQL</productname>サーバを起動します。
...
シングルユーザ状態のサーバインタフェースの操作方法についての詳細は、<xref linkend="app-postgres"/>リファレンスページを参照してください。␞␞  </para>␞
␝  <para>␟   Alternatively, a regular server session can be started with
   ...
   have been completed.␟その他、コマンドラインで<option>-P</option>を指定して通常のサーバセッションを起動することもできます。
...
この方法では他のクライアントを締め出す必要はありませんが、修復が終わるまで破損したデータベースへの他のユーザの接続を防止する方が良いことに注意してください。␞␞  </para>␞
␝  <para>␟   <command>REINDEX</command> is similar to a drop and recreate of the index
   ...
   into expensive sequential scans.␟<command>REINDEX</command>は、インデックスの中身を1から作り直すという点では、インデックスを削除してから再作成する処理と似ています。
しかし、ロックに関しては異なります。
...
したがって、読み込みがブロックされることはありませんが、コストが高いシーケンシャルスキャンの使用を強制されることになるかもしれません。␞␞  </para>␞
␝  <para>␟   While <command>REINDEX</command> is running, the <xref
   linkend="guc-search-path"/> is temporarily changed to <literal>pg_catalog,
   pg_temp</literal>.␟<command>REINDEX</command>の実行中、<xref linkend="guc-search-path"/>は一時的に<literal>pg_catalog, pg_temp</literal>に変更されます。␞␞  </para>␞
␝  <para>␟   Reindexing a single index or table requires
   ...
   catalog.␟単一のインデックスまたはテーブルを再インデックスするには、テーブルに対する<literal>MAINTAIN</literal>権限が必要です。
...
ただし、特別な例外として、ユーザがカタログに対して<literal>MAINTAIN</literal>権限を持っていない場合、<command>REINDEX DATABASE</command>、<command>REINDEX SCHEMA</command>、および<command>REINDEX SYSTEM</command>は共有カタログ上のインデックスをスキップします。␞␞  </para>␞
␝  <para>␟   Reindexing partitioned indexes or partitioned tables is supported
   ...
   a transaction block when working on a partitioned table or index.␟パーティションインデックスやパーティションテーブルのインデックス再作成は、それぞれ<command>REINDEX INDEX</command>、<command>REINDEX TABLE</command>でサポートされています。
指定されたパーティションリレーションの個々のパーティションが、別々のトランザクションでインデックスを再作成されます。
パーティションテーブルやパーティションインデックスに対して操作する場合、このコマンドをトランザクションブロック内では使えません。␞␞  </para>␞
␝  <para>␟   When using the <literal>TABLESPACE</literal> clause with
   ...
   tablespace.␟パーティションインデックスやパーティションテーブルで<literal>TABLESPACE</literal>句を使って<command>REINDEX</command>を行なう場合、リーフパーティションのテーブル空間参照のみが更新されます。
...
コマンドを再実行すればリーフパーティションはすべて再構築され、先ほどは処理されなかったインデックスが新しいテーブル空間へと移動します。␞␞  </para>␞
␝  <para>␟   If <literal>SCHEMA</literal>, <literal>DATABASE</literal> or
   ...
   to the new tablespace.␟<literal>SCHEMA</literal>、<literal>DATABASE</literal>、<literal>SYSTEM</literal>が<literal>TABLESPACE</literal>と一緒に使われた場合、システムリレーションは飛ばされ、<literal>WARNING</literal>が1度出ます。
TOASTテーブルのインデックスは再構築されますが、新しいテーブル空間には移動しません。␞␞  </para>␞
␝␟  <refsect2 id="sql-reindex-concurrently" xreflabel="Rebuilding Indexes Concurrently">␟  <refsect2 id="sql-reindex-concurrently" xreflabel="インデックスを同時に再構築">␞␞␞
...
␝  <refsect2 id="sql-reindex-concurrently" xreflabel="Rebuilding Indexes Concurrently">␟   <title>Rebuilding Indexes Concurrently</title>␟   <title>インデックスを同時に再構築</title>␞␞␞
␝   <para>␟    Rebuilding an index can interfere with regular operation of a database.
    ...
    for periods that are unacceptably long for a production system.␟インデックスの再構築は、通常のデータベース操作を妨げることがあります。
...
非常に大規模なテーブルに対するインデックス作成は何時間もかかることがあり得ます。また小規模なテーブルであっても、インデックス再構築により、運用状態のシステムとしては受け入れられないほど長い時間、書き込みロックがかかる可能性があります。␞␞   </para>␞
␝   <para>␟    <productname>PostgreSQL</productname> supports rebuilding indexes with minimum locking
    ...
    may slow down other operations.␟<productname>PostgreSQL</productname>は最小限の書き込みロックでのインデックス再構築をサポートしています。
...
もちろん、インデックス再構築によりCPUやメモリ、入出力に余分に負荷がかかりますので、他の操作が低速になる可能性があります。␞␞   </para>␞
␝   <para>␟    The following steps occur in a concurrent reindex.  Each step is run in a
    separate transaction.  If there are multiple indexes to be rebuilt, then
    each step loops through all the indexes before moving to the next step.␟同時実行再インデックスは以下のような段階で行なわれます。
各段階は分離したトランザクション内で実行されます。
複数のインデックスを再構築する場合、次の段階に移る前にすべてのインデックスに対して各段階が繰り返されます。␞␞␞
␝      <para>␟       A new transient index definition is added to the catalog
       ...
       associated tables to prevent any schema modification while processing.␟カタログ<literal>pg_index</literal>に新しく一時的なインデックス定義が追加されます。
この定義は古いインデックスを置き換えるのに使われます。
処理中は、再インデックスされるインデックスと関連するテーブルに対して、セッションレベルでの<literal>SHARE UPDATE EXCLUSIVE</literal>ロックを取得します。スキーマが修正されないようにするためです。␞␞      </para>␞
␝      <para>␟       A first pass to build the index is done for each new index.  Once the
       ...
       index.␟インデックス構築の第1段階は新しいインデックスそれぞれに対して行なわれます。
...
この過程は各インデックスに対して分離したトランザクションで行なわれます。␞␞      </para>␞
␝      <para>␟       Then a second pass is performed to add tuples that were added while the
       first pass was running.  This step is also done in a separate
       transaction for each index.␟次に、第1段階実行中に追加されたタプルを追加する第2段階が行なわれます。
この過程は各インデックスに対して分離したトランザクションで行なわれます。␞␞      </para>␞
␝      <para>␟       All the constraints that refer to the index are changed to refer to the
       ...
       referenced the old index to be invalidated.␟インデックスを参照する制約は、すべて新しいインデックス定義を参照するよう変更され、インデックスの名前が変更されます。
この時点で、<literal>pg_index.indisvalid</literal>は新しいインデックスに対しては<quote>true</quote>に切り替えられ、古いものに対しては<quote>false</quote>に切り替えられます。そして、古いインデックスを参照するセッションをすべて無効にするためキャッシュの無効化が行なわれます。␞␞      </para>␞
␝      <para>␟       The old indexes have <literal>pg_index.indisready</literal> switched to
       <quote>false</quote> to prevent any new tuple insertions, after waiting
       for running queries that might reference the old index to complete.␟古いインデックスを参照している可能性のある実行中の問い合わせが完了するのをまってから、新しいタプルが挿入されないように古いインデックスは<literal>pg_index.indisready</literal>が<quote>false</quote>に切り替えられます。␞␞      </para>␞
␝      <para>␟       The old indexes are dropped.  The <literal>SHARE UPDATE
       EXCLUSIVE</literal> session locks for the indexes and the table are
       released.␟古いインデックスが削除されます。
インデックスやテーブルに対する<literal>SHARE UPDATE EXCLUSIVE</literal>セッションロックは解放されます。␞␞      </para>␞
␝   <para>␟    If a problem arises while rebuilding the indexes, such as a
    ...
    such an index as <literal>INVALID</literal>:␟インデックスの再構築中に、一意性インデックスでの一意性違反などの問題が発生したら、<command>REINDEX</command>コマンドは失敗しますが、既に存在しているものに加えて<quote>無効な</quote>新しいインデックスを残します。
...
␝␟    If the index marked <literal>INVALID</literal> is suffixed
    ...
    <literal>_ccold2</literal>, etc.␟<literal>INVALID</literal>と印づけられたインデックスに接尾辞<literal>_ccnew</literal>がついている場合、それは同時実行操作中に作られた一時的なインデックスに対応します。お勧めの回復法は<literal>DROP INDEX</literal>を使ってそれを削除して、再度<command>REINDEX CONCURRENTLY</command>を試みることです。
...
一意性を保つため、<literal>_ccnew1</literal>、<literal>_ccold2</literal>などのように、無効なインデックス名の接尾辞に非ゼロの番号が追加されることがあります。␞␞   </para>␞
␝   <para>␟    Regular index builds permit other regular index builds on the same table
    ...
    CONCURRENTLY</command> cannot.␟通常のインデックス構築は、同じテーブルでの他の通常のインデックス構築を許しますが、同時実行インデックス構築は1つだけが一度に1つのテーブルでできます。
どちらの場合でも、その間のそのテーブルでの他の種類のスキーマ修正は認められていません。
もう一つの違いは、通常の<command>REINDEX TABLE</command>や<command>REINDEX INDEX</command>コマンドはトランザクションブロックの内側で実行できますが、<command>REINDEX CONCURRENTLY</command>はできないことです。␞␞   </para>␞
␝   <para>␟    Like any long-running transaction, <command>REINDEX</command> on a table
    can affect which tuples can be removed by concurrent
    <command>VACUUM</command> on any other table.␟他の時間のかかるトランザクションと同じく、あるテーブルに対する<command>REINDEX</command>は、その他のテーブルに対する同時実行中の<command>VACUUM</command>によりどのタプルが削除できるかに影響します。␞␞   </para>␞
␝   <para>␟    <command>REINDEX SYSTEM</command> does not support
    <command>CONCURRENTLY</command> since system catalogs cannot be reindexed
    concurrently.␟システムカタログは同時実行で再インデックスできませんので、<command>REINDEX SYSTEM</command>は<command>CONCURRENTLY</command>をサポートしません。␞␞   </para>␞
␝   <para>␟    Furthermore, indexes for exclusion constraints cannot be reindexed
    ...
    to reindex such indexes without the <command>CONCURRENTLY</command> option.)␟さらに、排他制約に対するインデックスは同時実行で再インデックスできません。
このコマンドでそのようなインデックスの名前が直接指定されたら、エラーが起きます。
排他制約インデックスのあるテーブルやデータベースが同時実行で再インデックスされる場合、そのインデックスはスキップされます。
（そのようなインデックスを<command>CONCURRENTLY</command>オプションなしで再インデックスすることは可能です。）␞␞   </para>␞
␝  <para>␟    Each backend running <command>REINDEX</command> will report its progress
    in the <structname>pg_stat_progress_create_index</structname> view. See
    <xref linkend="create-index-progress-reporting"/> for details.␟<command>REINDEX</command>を実行している各バックエンドはその進捗を<structname>pg_stat_progress_create_index</structname>ビューで報告します。
詳細は<xref linkend="create-index-progress-reporting"/>を参照してください。␞␞  </para>␞
␝ <refsect1>␟  <title>Examples</title>␟  <title>例</title>␞␞␞
␝  <para>␟   Rebuild a single index:␟単一のインデックスを再構築します。␞␞␞
␝  <para>␟   Rebuild all the indexes on the table <literal>my_table</literal>:␟テーブル<literal>my_table</literal>上のすべてのインデックスを再構築します。␞␞␞
␝  <para>␟   Rebuild all indexes in a particular database, without trusting the
   system indexes to be valid already:␟システムインデックスが有効かどうかを確認することなく、あるデータベース内の全てのインデックスを再構築します。␞␞␞
␝  <para>␟   Rebuild indexes for a table, without blocking read and write operations
   on involved relations while reindexing is in progress:␟再インデックスの進行中に、関連するリレーションの読み書きをブロックすることなく、テーブルに対するインデックスを再構築します。␞␞␞
␝ <refsect1>␟  <title>Compatibility</title>␟  <title>互換性</title>␞␞␞
␝  <para>␟   There is no <command>REINDEX</command> command in the SQL standard.␟標準SQLには<command>REINDEX</command>はありません。␞␞  </para>␞
␝ <refsect1>␟  <title>See Also</title>␟  <title>関連項目</title>␞␞␞
␝␟</itemizedlist></para> </itemizedlist></para>␟no translation␞␞␞
␝␟$ <userinput>export PGOPTIONS="-P"</userinput> $ <userinput>psql broken_db</userinput> ... broken_db=&gt; REINDEX DATABASE broken_db; broken_db=&gt; \q </programlisting></para> </programlisting></para>␟no translation␞␞␞
␝␟REINDEX TABLE CONCURRENTLY my_broken_table; </programlisting></para> </programlisting></para>␟no translation␞␞␞
