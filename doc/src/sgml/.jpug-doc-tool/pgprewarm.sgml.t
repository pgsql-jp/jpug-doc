␝<sect1 id="pgprewarm" xreflabel="pg_prewarm">␟ <title>pg_prewarm &mdash; preload relation data into buffer caches</title>␟ <title>pg_prewarm &mdash; リレーションデータをバッファキャッシュにプリロードする</title>␞␞␞
␝ <para>␟  The <filename>pg_prewarm</filename> module provides a convenient way
  ...
  will, using 2 background workers, reload those same blocks after a restart.␟ <filename>pg_prewarm</filename>はオペレーティングシステムのバッファキャッシュまたは<productname>PostgreSQL</productname>のバッファキャッシュいずれかにリレーションデータをロードするための便利な方法を提供します。
...
後者の場合には、システムは、定期的に<filename>autoprewarm.blocks</filename>という名前のファイルに共有バッファの内容を記録するバックグラウンドワーカーを実行し、再起動後には2つのバックグラウンドワーカー使って同じブロックを再ロードします。␞␞ </para>␞
␝ <sect2 id="pgprewarm-funcs">␟  <title>Functions</title>␟  <title>関数</title>␞␞␞
␝  <para>␟   The first argument is the relation to be prewarmed.  The second argument
   ...
   is the number of blocks prewarmed.␟第１引数はプレウォーム(あらかじめロード)したいリレーションを指定します。
...
戻り値は、プレウォームされたブロック数です。␞␞  </para>␞
␝  <para>␟   There are three available prewarming methods.  <literal>prefetch</literal>
   ...
   database buffer cache.␟ プレウォームに使用する方法としては以下の３種類が使用可能です。
...
<literal>buffer</literal>は、データベースのバッファキャッシュに要求された範囲を読み込みます。␞␞  </para>␞
␝  <para>␟   Note that with any of these methods, attempting to prewarm more blocks than
   ...
   when caches are largely empty.␟これらの方法のいずれかでもキャッシュ出来るよりも多くのブロックをプレウォームしようとすると注意が必要です。<literal>prefetch</literal>や<literal>read</literal>のようなOSのキャッシュを使用する場合、または <productname>PostgreSQL</productname>の<literal>buffer</literal>にキャッシュする場合、高い番号のブロックが読み込まれると低い番号のブロックが追い出されます。
...
これらの理由から、プレウォームはキャッシュが主に空のとき、一般的には起動時にもっとも有用です。␞␞  </para>␞
␝  <para>␟   Launch the main autoprewarm worker.  This will normally happen
   automatically, but is useful if automatic prewarm was not configured at
   server startup time and you wish to start up the worker at a later time.␟自動プレウォームワーカーを起動します。
通常これは自動的に行なわれますが、サーバのスタート時に自動プレウォームが設定されておらず、後でワーカーを起動したい場合に有用です。␞␞  </para>␞
␝  <para>␟   Update <filename>autoprewarm.blocks</filename> immediately.  This may be useful
   ...
   to <filename>autoprewarm.blocks</filename>.␟直ちに<filename>autoprewarm.blocks</filename>を更新します。
これは、自動プレウォームワーカーが動いていなくて、次の再起動後に自動プレウォームワーカーを動かそうと考えているときに有用かもしれません。
戻り値は<filename>autoprewarm.blocks</filename>に書き込まれたブロック数です。␞␞  </para>␞
␝     <indexterm>
...
␝ <sect2 id="pgprewarm-config-params">␟  <title>Configuration Parameters</title>␟  <title>設定パラメータ</title>␞␞␞
␝     <indexterm>
...
␝     <para>␟      Controls whether the server should run the autoprewarm worker. This is
      on by default. This parameter can only be set at server start.␟サーバが自動プレウォームワーカーを起動すべきかどうかを制御します。
デフォルトはonです。
このパラメータはサーバ起動時のみ設定可能です。␞␞     </para>␞
␝     <para>␟      This is the interval between updates to <literal>autoprewarm.blocks</literal>.
      The default is 300 seconds. If set to 0, the file will not be
      dumped at regular intervals, but only when the server is shut down.␟これは<literal>autoprewarm.blocks</literal>を更新する間隔です。
デフォルトは300秒です。
0に設定すると、このファイルは定常間隔では更新されず、サーバが停止する時にだけ更新されます。␞␞     </para>␞
␝  <para>␟   These parameters must be set in <filename>postgresql.conf</filename>.
   Typical usage might be:␟これらのパラメータは<filename>postgresql.conf</filename>で設定しなければなりません。
典型的な使用法は以下の通りです。␞␞  </para>␞
␝ <sect2 id="pgprewarm-author">␟  <title>Author</title>␟  <title>作者</title>␞␞␞
␝␟Robert Haas <email>rhaas@postgresql.org</email>␟no translation␞␞␞
