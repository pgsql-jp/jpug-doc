␝  <refname>pg_createsubscriber</refname>␟  <refpurpose>convert a physical replica into a new logical replica</refpurpose>␟  <refpurpose>物理レプリカを新しい論理レプリカに変換する</refpurpose>␞␞ </refnamediv>␞
␝ <refsect1>␟  <title>Description</title>␟  <title>説明</title>␞␞␞
␝  <para>␟   <application>pg_createsubscriber</application> creates a new logical
   ...
   must be run at the target server.␟<application>pg_createsubscriber</application>は、物理スタンバイサーバから新しい論理レプリカを作成します。
...
ターゲットサーバで実行する必要があります。␞␞  </para>␞
␝  <para>␟   After a successful run, the state of the target server is analogous to a
   ...
   which ensures each table is brought up to a synchronized state.␟実行が成功した後のターゲットサーバの状態は、新しい論理レプリケーションセットアップに似ています。
...
各テーブルが同期化された状態になることを保証する同期化フェーズのみを行います。␞␞  </para>␞
␝  <para>␟   <application>pg_createsubscriber</application> targets large database
   ...
   <literal>copy_data</literal></link> option.␟<application>pg_createsubscriber</application>は大きなデータベースシステムを対象としています。
...
詳細については、<command>CREATE SUBSCRIPTION</command><link linkend="sql-createsubscription-params-with-copy-data"><literal>copy_data</literal></link>オプションを参照してください。␞␞  </para>␞
␝ <refsect1>␟  <title>Options</title>␟  <title>オプション</title>␞␞␞
␝  <para>␟   <application>pg_createsubscriber</application> accepts the following
   command-line arguments:␟<application>pg_createsubscriber</application>は、以下のコマンドライン引数を受け付けます。␞␞␞
␝      <para>␟       The name of the database in which to create a subscription.  Multiple
       databases can be selected by writing multiple <option>-d</option>
       switches.␟サブスクリプションを作成するデータベースの名前。
複数の<option>-d</option>スイッチを記述することによって、複数のデータベースを選択できます。␞␞      </para>␞
␝      <para>␟       The target directory that contains a cluster directory from a physical
       replica.␟物理レプリカのクラスタディレクトリを含むターゲットディレクトリ。␞␞      </para>␞
␝      <para>␟       Do everything except actually modifying the target directory.␟ターゲットディレクトリを実際に更新する以外はすべてのことを行います。␞␞      </para>␞
␝      <para>␟       The port number on which the target server is listening for
       connections.  Defaults to running the target server on port 50432 to
       avoid unintended client connections.␟ターゲットサーバが接続をリスニングするポート番号。
デフォルトでは、ターゲットサーバはポート50432で実行され、意図しないクライアント接続を回避します。␞␞      </para>␞
␝      <para>␟       The connection string to the publisher.  For details see <xref
       linkend="libpq-connstring"/>.␟パブリッシャーへの接続文字列。
詳細は<xref linkend="libpq-connstring"/>を参照してください。␞␞      </para>␞
␝      <para>␟       The directory to use for postmaster sockets on target server.  The
       default is current directory.␟ターゲットサーバ上のpostmasterソケットに使用するディレクトリ。
デフォルトは現在のディレクトリです。␞␞      </para>␞
␝      <para>␟       The maximum number of seconds to wait for recovery to end.  Setting to
       0 disables.  The default is 0.␟リカバリーが終了するまでの最大待ち時間（秒）。
0に設定すると無効になります。
デフォルトは0です。␞␞      </para>␞
␝      <para>␟       The user name to connect as on target server.  Defaults to the current
       operating system user name.␟ターゲットサーバに接続するためのユーザ名。
デフォルトは、現在のオペレーティングシステムユーザ名です。␞␞      </para>␞
␝      <para>␟       Enables verbose mode.  This will cause
       ...
       on standard error.␟冗長モードを有効にします。
これにより<application>pg_createsubscriber</application>は進捗メッセージと各ステップの詳細情報を標準エラーに出力します。
オプションを繰り返すと、追加のデバッグレベルメッセージが標準エラーに出力されます。␞␞      </para>␞
␝      <para>␟       Use the specified main server configuration file for the target data
       ...
       outside the data directory.␟ターゲットデータディレクトリ用に指定されたメインサーバの設定ファイルを使用します。
<application>pg_createsubscriber</application>は、内部的に<application>pg_ctl</application>コマンドをスタートに使用し、ターゲットサーバを起動、停止します。
データディレクトリ外に保存されている場合は、実際の<filename>postgresql.conf</filename>設定ファイルを指定できます。␞␞      </para>␞
␝      <para>␟       The publication name to set up the logical replication.  Multiple
       ...
       a generated name is assigned to the publication name.␟論理レプリケーションを設定するパブリケーション名。
...
このオプションを指定しない場合、生成された名前がパブリケーション名に割り当てられます。␞␞      </para>␞
␝      <para>␟       The replication slot name to set up the logical replication.  Multiple
       ...
       replication slot name.␟論理レプリケーションを設定するレプリケーションスロット名。
...
このオプションを指定しない場合、サブスクリプション名がレプリケーションスロット名に割り当てられます。␞␞      </para>␞
␝      <para>␟       The subscription name to set up the logical replication.  Multiple
       ...
       match the order of database switches.  If this option is not specified,
       a generated name is assigned to the subscription name.␟論理レプリケーションを設定するサブスクリプション名。
...
このオプションを指定しない場合、生成された名前がサブスクリプション名に割り当てられます。␞␞      </para>␞
␝      <para>␟       Print the <application>pg_createsubscriber</application> version and exit.␟<application>pg_createsubscriber</application>のバージョンを表示し、終了します。␞␞      </para>␞
␝      <para>␟       Show help about <application>pg_createsubscriber</application> command
       line arguments, and exit.␟<application>pg_createsubscriber</application>コマンドライン引数についてのヘルプを表示し、終了します。␞␞      </para>␞
␝ <refsect1>␟  <title>Notes</title>␟  <title>注釈</title>␞␞␞
␝  <refsect2>␟   <title>Prerequisites</title>␟   <title>前提条件</title>␞␞␞
␝   <para>␟    There are some prerequisites for
    ...
    linkend="pg-replication-origin-advance"><function>pg_replication_origin_advance()</function></link>.␟<application>pg_createsubscriber</application>がターゲットサーバを論理レプリカに変換するためには、いくつかの前提条件があります。
...
指定されたターゲットデータディレクトリのデータベースユーザは、<link linkend="sql-createsubscription">サブスクリプションの作成</link>と<link linkend="pg-replication-origin-advance"><function>pg_replication_origin_advance()</function></link>を使用するための権限を持たなければなりません。␞␞   </para>␞
␝   <para>␟    The target server must be used as a physical standby.  The target server
    ...
    must accept local connections.␟ターゲットサーバは、物理スタンバイとして使用する必要があります。
...
ターゲットサーバは、ローカル接続を受け入れる必要があります。␞␞   </para>␞
␝   <para>␟    The source server must accept connections from the target server.  The
    ...
    to the number of specified databases and existing WAL sender processes.␟ソースサーバはターゲットサーバからの接続を受け入れなければなりません。
...
ソースサーバは、指定されたデータベースの数と既存のWAL送信プロセス以上の値に<xref linkend="guc-max-wal-senders"/>を設定する必要があります。␞␞   </para>␞
␝  <refsect2>␟   <title>Warnings</title>␟   <title>警告</title>␞␞␞
␝   <para>␟    If <application>pg_createsubscriber</application> fails after the target
    ...
    recommended.␟ターゲットサーバの昇格後に<application>pg_createsubscriber</application>が失敗した場合、データディレクトリはリカバリ可能な状態ではない可能性が高いです。
このような場合は、新しいスタンバイサーバを作成することをお勧めします。␞␞   </para>␞
␝   <para>␟    <application>pg_createsubscriber</application> usually starts the target
    server with different connection settings during transformation.  Hence,
    connections to the target server should fail.␟<application>pg_createsubscriber</application>は通常、変換中に異なる接続設定でターゲットサーバを起動します。
したがって、ターゲットサーバへの接続は失敗するはずです。␞␞   </para>␞
␝   <para>␟    Since DDL commands are not replicated by logical replication, avoid
    ...
    replicated, which might cause an error.␟DDLコマンドは論理レプリケーションではレプリケートされないため、<application>pg_createsubscriber</application>の実行中にデータベーススキーマを変更するDDLコマンドは実行しないでください。
ターゲットサーバがすでに論理レプリカに変換されている場合、DDLコマンドはレプリケートされない可能性があり、エラーが発生する可能性があります。␞␞   </para>␞
␝   <para>␟    If <application>pg_createsubscriber</application> fails while processing,
    ...
    objects left.  If the target server is running, it will be stopped.␟処理中に<application>pg_createsubscriber</application>が失敗した場合、ソースサーバ上に作成されたオブジェクト（パブリケーション、レプリケーションスロット）は削除されます。
ターゲットサーバがソースサーバに接続できない場合、削除は失敗する可能性があります。
このような場合、警告メッセージが残されたオブジェクトを通知します。
ターゲットサーバが実行中の場合は、停止されます。␞␞   </para>␞
␝   <para>␟    If the replication is using <xref linkend="guc-primary-slot-name"/>, it
    will be removed from the source server after the logical replication
    setup.␟レプリケーションが<xref linkend="guc-primary-slot-name"/>を使用している場合、論理レプリケーション設定後にソースサーバから削除されます。␞␞   </para>␞
␝   <para>␟    If the target server is a synchronous replica, transaction commits on the
    primary might wait for replication while running
    <application>pg_createsubscriber</application>.␟ターゲットサーバが同期レプリカの場合、<application>pg_createsubscriber</application>の実行中にプライマリでのトランザクションコミットがレプリケーションを待つことがあります。␞␞   </para>␞
␝   <para>␟    <application>pg_createsubscriber</application> sets up logical
    ...
    option enabled.␟<application>pg_createsubscriber</application>は二相コミットを無効にして論理レプリケーションを設定します。
これは、準備されたトランザクションが事前の準備なしに<command>COMMIT PREPARED</command>の時点で複製されることを意味します。
セットアップが完了したら、<link linkend="sql-createsubscription-params-with-two-phase"><literal>two_phase</literal></link>オプションを有効にしてサブスクリプションを手動で削除し、再作成することができます。␞␞   </para>␞
␝   <para>␟    <application>pg_createsubscriber</application> changes the system
    ...
    fresh standby should be created.␟<application>pg_createsubscriber</application>は<application>pg_resetwal</application>を使用してシステム識別子を変更します。
これは、ターゲットサーバがソースサーバからのWALファイルを使用する可能性がある状況を回避します。
ターゲットサーバにスタンバイがある場合、レプリケーションが中断され、新しいスタンバイを作成する必要があります。␞␞   </para>␞
␝  <refsect2>␟   <title>How It Works</title>␟   <title>どうやって動くのか</title>␞␞␞
␝   <para>␟    The basic idea is to have a replication start point from the source server
    and set up a logical replication to start from this point:␟基本的な考え方は、ソースサーバからレプリケーションの開始ポイントを設定し、このポイントから開始する論理レプリケーションをセットアップすることです。␞␞   </para>␞
␝     <para>␟      Start the target server with the specified command-line options.  If the
      target server is already running,
      <application>pg_createsubscriber</application> will terminate with an
      error.␟指定したコマンドラインオプションでターゲットサーバを起動します。
ターゲットサーバがすでに実行中の場合、<application>pg_createsubscriber</application>はエラーで終了します。␞␞     </para>␞
␝     <para>␟      Check if the target server can be converted.  There are also a few
      checks on the source server.  If any of the prerequisites are not met,
      <application>pg_createsubscriber</application> will terminate with an
      error.␟ターゲットサーバを変換できるかどうかをチェックします。
ソースサーバに対してもいくつかのチェックが行われます。
前提条件のいずれかが満たされていない場合、<application>pg_createsubscriber</application>はエラーで終了します。␞␞     </para>␞
␝     <para>␟      Create a publication and replication slot for each specified database on
      ...
      transaction will be lost.␟ソースサーバ上の指定されたデータベースごとに、パブリケーションとレプリケーションスロットを作成します。
...
これにより、トランザクションが失われることがなくなります。␞␞     </para>␞
␝     <para>␟      Write recovery parameters into the target data directory and restart the
      ...
      does not end until the given number of seconds.␟ターゲットデータディレクトリにリカバリパラメータを書き込み、ターゲットサーバを再起動します。
...
<option>--recovery-timeout</option>オプションが設定されている場合、指定された秒数が経過するまでリカバリが終了しない場合、<application>pg_createsubscriber</application>は終了します。␞␞     </para>␞
␝     <para>␟      Create a subscription for each specified database on the target server.
      ...
      the replication start point before starting the replication.␟ターゲットサーバ上の指定した各データベースに対してサブスクリプションを作成します。
...
これは、レプリケーションを開始する前に、レプリケーションの進行状況をレプリケーション開始点に設定する必要があるためです。␞␞     </para>␞
␝     <para>␟      Drop publications on the target server that were replicated because they
      were created before the replication start location.  It has no use on
      the subscriber.␟レプリケーション開始位置より前に作成されていたために複製された、ターゲットサーバ上のパブリケーション削除します。
これはサブスクライバー側では使用されません。␞␞     </para>␞
␝     <para>␟      Set the replication progress to the replication start point for each
      ...
      to set up the initial replication location.␟各サブスクリプションのレプリケーション開始点にレプリケーションの進行状況を設定します。
...
レプリケーションオリジン名とレプリケーション開始点は、<link linkend="pg-replication-origin-advance"><function>pg_replication_origin_advance()</function></link>で初期レプリケーション場所を設定するために使用されます。␞␞     </para>␞
␝     <para>␟      Enable the subscription for each specified database on the target server.
      The subscription starts applying transactions from the replication start
      point.␟ターゲットサーバ上の指定した各データベースのサブスクリプションを使用可能にします。
サブスクリプションは、レプリケーション開始ポイントからトランザクションを適用し始めます。␞␞     </para>␞
␝     <para>␟      If the standby server was using <xref linkend="guc-primary-slot-name"/>,
      it has no use from now on so drop it.␟スタンバイサーバが<xref linkend="guc-primary-slot-name"/>を使用していた場合、それは今後は使用されないので削除します。␞␞     </para>␞
␝     <para>␟      If the standby server contains <link
      ...
      them.␟スタンバイサーバに<link linkend="logicaldecoding-replication-slots-synchronization">フェイルオーバーレプリケーションスロット</link>がある場合、もはや同期できないので削除します。␞␞     </para>␞
␝     <para>␟      Update the system identifier on the target server. The
      <xref linkend="app-pgresetwal"/> is run to modify the system identifier.
      The target server is stopped as a <command>pg_resetwal</command> requirement.␟ターゲットサーバのシステム識別子を更新します。
<xref linkend="app-pgresetwal"/>が実行され、システム識別子が変更されます。
<command>pg_resetwal</command>の必要条件なので、ターゲットサーバは停止されます。␞␞     </para>␞
␝ <refsect1>␟  <title>Examples</title>␟  <title>例</title>␞␞␞
␝  <para>␟   To create a logical replica for databases <literal>hr</literal> and
   <literal>finance</literal> from a physical replica at
   <literal>foo</literal>:␟<literal>foo</literal>での物理レプリカからデータベース<literal>hr</literal>と<literal>finance</literal>の論理レプリカを作成するには、次のようにします。␞␞<screen>␞
