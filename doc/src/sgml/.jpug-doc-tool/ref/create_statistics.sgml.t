...
␝  <refname>CREATE STATISTICS</refname>␟  <refpurpose>define extended statistics</refpurpose>␟  <refpurpose>拡張統計情報を定義する</refpurpose>␞␞ </refnamediv>␞
␝ <refsect1 id="sql-createstatistics-description">␟  <title>Description</title>␟  <title>説明</title>␞␞␞
␝  <para>␟   <command>CREATE STATISTICS</command> will create a new extended statistics
   ...
   database and will be owned by the user issuing the command.␟<command>CREATE STATISTICS</command>は指定したテーブル、外部テーブル、マテリアライズドビューのデータを追跡する新しい拡張統計オブジェクトを作成します。
統計オブジェクトは現在のデータベースに作成され、コマンドを実行したユーザに所有されます。␞␞  </para>␞
␝  <para>␟   The <command>CREATE STATISTICS</command> command has two basic forms. The
   ...
   any expressions included in the list.␟<command>CREATE STATISTICS</command>コマンドには2つの基本的な形式があります。
...
この形式では、自動的にリストに含まれる式の単変量統計も集められます。␞␞  </para>␞
␝  <para>␟   If a schema name is given (for example, <literal>CREATE STATISTICS
   ...
   of any other statistics object in the same schema.␟スキーマ名が指定された場合（例：<literal>CREATE STATISTICS myschema.mystat ...</literal>）、統計オブジェクトは指定したスキーマ内に作成されます。
スキーマ名を指定しなければ、現在のスキーマ内に作成されます。
統計オブジェクトの名前を指定する場合は、同じスキーマ内のどの統計オブジェクトとも異なるものでなければなりません。␞␞  </para>␞
␝ <refsect1>␟  <title>Parameters</title>␟  <title>パラメータ</title>␞␞␞
␝     <para>␟      Do not throw an error if a statistics object with the same name already
      ...
      definition.
      Statistics name is required when <literal>IF NOT EXISTS</literal> is specified.␟同じ名前の統計オブジェクトが既に存在していてもエラーを発生させません。
この場合、注意メッセージが発行されます。
この場合、統計オブジェクトの名前だけが問題にされ、その定義の詳細は考慮されないことに注意してください。
<literal>IF NOT EXISTS</literal>が指定された場合、統計の名前が必要です。␞␞     </para>␞
␝     <para>␟      The name (optionally schema-qualified) of the statistics object to be
      ...
      name(s) and/or expression(s).␟作成する統計オブジェクトの名前です（スキーマ修飾も可）。
名前が省略された場合、<productname>PostgreSQL</productname>は、親テーブルの名前と定義された列名、式に基づいた適切な名前を選択します。␞␞     </para>␞
␝     <para>␟      A multivariate statistics kind to be computed in this statistics object.
      ...
      and <xref linkend="multivariate-statistics-examples"/>.␟この統計オブジェクト内で計算する多変量統計の種別です。
...
より詳細な情報は<xref linkend="planner-stats-extended"/>および<xref linkend="multivariate-statistics-examples"/>を参照してください。␞␞     </para>␞
␝     <para>␟      The name of a table column to be covered by the computed statistics.
      ...
      not significant.␟統計計算の対象となるテーブル列の名前です。
これは多変量統計を構築するときにのみ可能です。
少なくとも2つの列名または式を指定しなければなりません。順序は重要ではありません。␞␞     </para>␞
␝     <para>␟      An expression to be covered by the computed statistics.  This may be
      ...
      statistics are built automatically for each expression in the list.␟統計計算の対象となる式です。
これは1つの式に対して単変量統計を構築する場合や、多変量統計を構築する複数の列名や式のリストの一部として使われます。
後者の場合、リスト内の各式に対して自動的にそれぞれの単変量統計が構築されます。␞␞     </para>␞
␝     <para>␟      The name (optionally schema-qualified) of the table containing the
      column(s) the statistics are computed on;  see <xref
      linkend="sql-analyze"/> for an explanation of the handling of
      inheritance and partitions.␟統計情報が計算される列があるテーブルの名前（オプションでスキーマ修飾可）です。継承とパーティションの取り扱いについては<xref linkend="sql-analyze"/>を参照してください。␞␞     </para>␞
␝ <refsect1>␟  <title>Notes</title>␟  <title>注釈</title>␞␞␞
␝  <para>␟   You must be the owner of a table to create a statistics object
   reading it.  Once created, however, the ownership of the statistics
   object is independent of the underlying table(s).␟テーブルを読み取る統計オブジェクトを作るには、そのテーブルの所有者でなければなりません。
しかし、統計オブジェクトが作成された後は、その所有者と対象となるテーブルは無関係になります。␞␞  </para>␞
␝  <para>␟   Expression statistics are per-expression and are similar to creating an
   ...
   expression in the statistics object definition.␟式統計は式ごとのものであり、インデックスの保守のオーバーヘッドを避けられるということ以外は式にインデックスを作るのと似ています。
式統計は、統計オブジェクト定義内の各式に対して自動的に構築されます。␞␞  </para>␞
␝  <para>␟   Extended statistics are not currently used by the planner for selectivity
   estimations made for table joins.  This limitation will likely be removed
   in a future version of <productname>PostgreSQL</productname>.␟拡張統計処理は現在、プランナによってテーブル結合に対して行なわれる選択性の見積もりには使用されていません。
この制限は、<productname>PostgreSQL</productname>の将来のバージョンで削除される可能性があります。␞␞  </para>␞
␝ <refsect1 id="sql-createstatistics-examples">␟  <title>Examples</title>␟  <title>例</title>␞␞␞
␝  <para>␟   Create table <structname>t1</structname> with two functionally dependent columns, i.e.,
   knowledge of a value in the first column is sufficient for determining the
   value in the other column. Then functional dependency statistics are built
   on those columns:␟関数従属性のある2つの列を含むテーブル<structname>t1</structname>を作成します。
つまり、第1の列の値を知っていれば、それだけでもう一方の列の値がわかる、というものです。
その次に、これらの列の間に関数的依存統計を構築します。␞␞␞
␝-- the number of matching rows will be drastically underestimated:␟-- the number of matching rows will be drastically underestimated:␟-- マッチする行の数は非常に低く見積もられる␞␞EXPLAIN ANALYZE SELECT * FROM t1 WHERE (a = 1) AND (b = 0);␞
␝-- now the row count estimate is more accurate:␟-- now the row count estimate is more accurate:␟-- 行数の見積もりがより正確になる␞␞EXPLAIN ANALYZE SELECT * FROM t1 WHERE (a = 1) AND (b = 0);␞
␝␟   Without functional-dependency statistics, the planner would assume
   ...
   conditions are redundant and does not underestimate the row count.␟関数的依存統計がなければ、プランナは2つの<literal>WHERE</literal>条件を独立なものとみなすため、それらの選択性を掛け算して、非常に小さな行数見積もりを導きます。
このような統計があれば、プランナは<literal>WHERE</literal>条件が冗長であることを認識し、行数を低く見積もりません。␞␞  </para>␞
␝  <para>␟   Create table <structname>t2</structname> with two perfectly correlated columns
   (containing identical data), and an MCV list on those columns:␟(同一のデータの入った)完全に相関のある2つの列を持つテーブル<structname>t2</structname>を作成し、2つの列の最頻値(MCV)の一覧を作成します。␞␞␞
␝-- valid combination (found in MCV)␟-- valid combination (found in MCV)␟-- 有効な組み合わせ(MCV内で見つかる)␞␞EXPLAIN ANALYZE SELECT * FROM t2 WHERE (a = 1) AND (b = 1);␞
␝-- invalid combination (not found in MCV)␟-- invalid combination (not found in MCV)␟-- 無効な組み合わせ(MCV内で見つからない)␞␞EXPLAIN ANALYZE SELECT * FROM t2 WHERE (a = 1) AND (b = 2);␞
␝␟   The MCV list gives the planner more detailed information about the
   ...
   in the table, allowing it to generate better estimates in both cases.␟最頻値の一覧は、テーブルによく現れる特定の値に関するものだけでなく、テーブルに現れない値の組み合わせの選択の上限に関するより詳細な情報もプランナに与えますので、両方の場合に対してより良く見積もりができるようになります。␞␞  </para>␞
␝  <para>␟   Create table <structname>t3</structname> with a single timestamp column,
   ...
   and ndistinct statistics are built on those two expressions:␟タイムスタンプの列1つのテーブル<structname>t3</structname>を作成し、その列の式を使う問い合わせを実行します。
...
␝-- the number of matching rows will be drastically underestimated:␟-- the number of matching rows will be drastically underestimated:␟-- マッチする行数は大幅に過小評価されます。␞␞EXPLAIN ANALYZE SELECT * FROM t3␞
␝-- statistics are built automatically)␟-- build ndistinct statistics on the pair of expressions (per-expression
-- statistics are built automatically)␟-- 式の組み合わせに対してN個別値統計を構築します
-- (式ごとの統計は自動的に構築されます)␞␞CREATE STATISTICS s3 (ndistinct) ON date_trunc('month', a), date_trunc('day', a) FROM t3;␞
␝-- now the row count estimates are more accurate:␟-- now the row count estimates are more accurate:␟-- 行数の評価はより正確になります。␞␞EXPLAIN ANALYZE SELECT * FROM t3␞
␝␟   Without expression and ndistinct statistics, the planner has no information
   ...
   more accurate estimates.␟式統計とN個別値統計がなければ、プランナは式の異なる値の数についての情報がなく、デフォルトの評価に頼らなければなりません。
...
そのような統計があれば、プランナは条件が相互に関係していることを認識し、ずっとより正確な評価をします。␞␞  </para>␞
␝ <refsect1>␟  <title>Compatibility</title>␟  <title>互換性</title>␞␞␞
␝  <para>␟   There is no <command>CREATE STATISTICS</command> command in the SQL standard.␟標準SQLに<command>CREATE STATISTICS</command>コマンドはありません。␞␞  </para>␞
