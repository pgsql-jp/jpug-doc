...
␝ <sect1 id="xaggr">␟  <title>User-Defined Aggregates</title>␟  <title>ユーザ定義の集約</title>␞␞␞
␝  <para>␟   Aggregate functions in <productname>PostgreSQL</productname>
   ...
   called as part of an aggregate.)␟<productname>PostgreSQL</productname>における集約関数は、<firstterm>状態値</firstterm>と<firstterm>状態遷移関数</firstterm>で定義されています。
...
（実際には、集約の一部として呼び出されて動作する専用の遷移関数を作成することは、多くの場合パフォーマンス上の理由から役立ちます。）␞␞  </para>␞
␝  <para>␟   Thus, in addition to the argument and result data types seen by a user
   of the aggregate, there is an internal state-value data type that
   might be different from both the argument and result types.␟したがって、集約のユーザに見える引数と結果のデータ型に加え、引数と結果の型のどちらとも違う可能性がある内部状態値のデータ型があります。␞␞  </para>␞
␝  <para>␟   If we define an aggregate that does not use a final function,
   ...
   The aggregate definition would be:␟最終関数を使わない集約を定義した場合は、列の値を行ごとに計算する関数を実行することで集約ができます。
<function>sum</function>はそのような集約の一例です。
<function>sum</function>は0から始まり、常に現在の行の値をその時点までの総和に追加します。
例えば、もし<function>sum</function>集約を複素数(complex)のデータ型で動作するようにしたければ、そのデータ型の加算関数だけが必要になります。
集約の定義は以下のようになります。␞␞␞
...
␝␟   (Notice that we are relying on function overloading: there is more than
   ...
   of sum applies to a column of type <type>complex</type>.)␟（関数のオーバーロード機能に依存していることに注意してください。
<function>sum</function>という名前の集約関数は複数存在しますが、<productname>PostgreSQL</productname>は列の<type>complex</type>型に適用できるsum関数を見つけ出すことができます。）␞␞  </para>␞
␝  <para>␟   The above definition of <function>sum</function> will return zero
   ...
   for null inputs).␟上記の<function>sum</function>の定義は、もし非NULLの入力値がなければ0（初期状態）を返します。
...
<productname>PostgreSQL</productname>は、もし初期状態がNULLで状態遷移関数が<quote>strict（厳密）</quote>と宣言されている場合、自動的にそのように動作します（つまりNULL入力では呼び出されないようになります）。␞␞  </para>␞
␝  <para>␟   Another bit of default behavior for a <quote>strict</quote> transition function
   ...
   do whatever is needed.␟もう1つの<quote>strict</quote>な状態遷移関数のデフォルト動作としては、NULL入力値が現れると前の状態値が変わらずに維持されるということがあります。
したがって、NULL値は無視されます。
もしNULL入力に対し他の動作が必要な場合は、状態遷移関数をstrict宣言しないようにします。その代わりにNULL入力の検査をおこなうようにコーディングし、必要なことをすればよいのです。␞␞  </para>␞
␝  <para>␟   <function>avg</function> (average) is a more complex example of an aggregate.
   It requires
   ...
   looks like:␟<function>avg</function>（平均値計算）はもっと複雑な集約の一例です。
...
␝   <para>␟   <function>float8_accum</function> requires a three-element array, not just
   ...
   some other aggregates as well as <function>avg</function>.␟<function>float8_accum</function>は、入力の総和と個数だけではなく二乗和も蓄積しますので、２要素ではなく、３要素の配列を必要とします。
それは、<function>avg</function>以外の他の集約でも使用できるようにするためです。␞␞   </para>␞
␝  <para>␟   Aggregate function calls in SQL allow <literal>DISTINCT</literal>
   ...
   of the aggregate's support functions.␟SQLの集約関数はオプションにより<literal>DISTINCT</literal>と<literal>ORDER BY</literal>を許可します。それは集約の遷移関数に渡される行や順序を制御します。これらのオプションは裏側で実装されるので、集約のサポート関数が気にする必要はありません。␞␞  </para>␞
␝  <para>␟   For further details see the
   <xref linkend="sql-createaggregate"/>
   command.␟さらなる詳細については、<xref linkend="sql-createaggregate"/>コマンドを参照してください。␞␞  </para>␞
␝  <indexterm>
...
␝ <sect2 id="xaggr-moving-aggregates">␟  <title>Moving-Aggregate Mode</title>␟  <title>移動集約モード</title>␞␞␞
␝  <para>␟   Aggregate functions can optionally support <firstterm>moving-aggregate
   ...
   input rows.␟集約関数は、<firstterm>移動集約モード</firstterm>をオプションでサポートします。それは、ウィンドウ内のフレーム開始点を移動することで、集約関数の実行を大幅に高速にすることができます。
(集約関数としてのウィンドウ関数の使用に関する情報は<xref linkend="tutorial-window"/>と <xref linkend="syntax-window-functions"/> を参照してください。)
基本的な考え方は、通常の<quote>順方向</quote>の遷移関数に加えて、集約は<firstterm>逆方向遷移関数</firstterm>を提供します。これによりウィンドウフレームが終了した時点で、集約の実行中の状態値から行を除外することが可能になります。
例えば、<function>sum</function>集約では、順方向遷移関数として加算を使用しており、逆方向遷移関数として減算を使用します。
逆方向遷移関数を持たないとウィンドウ関数は、フレームの開始点に移動するたびに一から集約を再計算しなければなりません。
その実行時間は、入力行の数のフレーム長の平均回数倍に比例します。
逆遷移関数を使用すると実行時間は、入力行の数にのみ比例します。␞␞  </para>␞
␝  <para>␟   The inverse transition function is passed the current state value and the
   ...
   state.␟逆遷移関数には、現在の状態値と現在の状態が含まれる最も古い行の集約入力値（複数可）を渡されます。
...
これらは必要がない場合、通常モードのデータ型および関数と同じでも構いません。␞␞  </para>␞
␝  <para>␟   As an example, we could extend the <function>sum</function> aggregate given above
   to support moving-aggregate mode like this:␟例として、移動集約モードをサポートするために、以下のように<function>sum</function>集約を拡張できます。␞␞␞
␝␟   The parameters whose names begin with <literal>m</literal> define the
   ...
   parameters without <literal>m</literal>.␟<literal>m</literal>で始まる名前のパラメータは、移動集約の実装を定義します。
逆遷移関数<literal>minvfunc</literal>以外は<literal>m</literal>のない通常の集約パラメータに対応しています。␞␞  </para>␞
␝  <para>␟   The forward transition function for moving-aggregate mode is not allowed
   ...
   state value.␟移動集約モードのための順方向遷移関数は、新しい状態値としてnullを返すことが許されていません。
...
逆遷移関数はこれらのケースで<quote>諦め</quote>ますが、大部分のケースで動作することが出来ます。
例として、浮動小数点数を扱う集約は、<literal>NaN</literal>(非数)の入力が実行されている状態値から除去されなければならない時に諦めることを選択するかもしれません。␞␞  </para>␞
␝  <para>␟   When writing moving-aggregate support functions, it is important to be
   ...
   naive declaration of <function>sum(<type>float8</type>)</function> could be␟移動集約サポート関数を記述する際には、逆遷移関数が正しい状態値を正確に再構築できていることを確認することが重要です。
それ以外の場合は、移動集約モードが使用されているかどうかに応じてユーザに見える結果に違いがあるかもしれません。
逆遷移関数を追加する最初の簡単な例は、要件を満たせていない<type>float4</type>や<type>float8</type>入力の<function>sum</function>です。
稚拙な<function>sum(<type>float8</type>)</function>の宣言です。␞␞␞
␝␟   This aggregate, however, can give wildly different results than it would
   have without the inverse transition function. For example, consider␟しかし、この集約は、逆遷移関数を持たない場合と比較すると著しく異なる結果になります。
例を考えます。␞␞␞
␝␟   This query returns <literal>0</literal> as its second result, rather than the
   ...
   of <productname>PostgreSQL</productname>.␟この問い合わせは ２行目の結果が期待した<literal>1</literal>ではなく<literal>0</literal>を返します。
 原因は、浮動小数点値で制限された精度です：<literal>1e20</literal>に<literal>1</literal>を加えても結果は再び<literal>1e20</literal>になります。その結果から<literal>1e20</literal>を引くと<literal>1</literal>ではなく<literal>0</literal>になります。
これは、<productname>PostgreSQL</productname>限定ではなくて、一般的な浮動小数点演算の制限であることに注意してください。␞␞  </para>␞
␝  <indexterm>
   <primary>aggregate function</primary>
...
␝ <sect2 id="xaggr-polymorphic-aggregates">␟  <title>Polymorphic and Variadic Aggregates</title>␟  <title>多様引数と可変長引数集約</title>␞␞␞
␝  <para>␟   Aggregate functions can use polymorphic
   ...
   Here is an example of a polymorphic aggregate:␟集約関数は多様状態遷移関数や多様最終関数を使用することができます。これにより、同じ関数を使用して複数の集約を実装することができます。
...
␝␟   Here, the actual state type for any given aggregate call is the array type
   ...
   functionality, with better performance than this definition would have.)␟ここでは、任意の呼び出しが出来る集約として実際の状態型を（実際の入力型がその要素となる）配列型にしています。
この集約の動作は、その配列型に全ての入力を連結することです。
（組み込みの集約関数<function>array_agg</function>は、この定義での動作よりもより良い性能で、類似の機能を提供しています。）␞␞  </para>␞
␝  <para>␟   Here's the output using two different actual data types as arguments:␟以下に2つの異なる実データ型を引数として使用した出力例を示します。␞␞␞
␝  <para>␟   Ordinarily, an aggregate function with a polymorphic result type has a
   ...
   aggregate <function>array_agg</function> is equivalent to␟通常、上記の例のように多様型の結果を返す集約関数は多様状態型を持ちます。
それは、最終関数を適正に宣言するために以下が必要になります。結果の型は多様型であり、引数の型は多様型でない必要があります。そうでないと<command>CREATE FUNCTION</command>は、呼び出しから結果の型を推定することができないので拒否されます。
しかし、状態型として多様型を使用するのは時に不便です。
最も一般的なケースでは集約サポート関数は、C言語で状態型を<type>internal（内部データ）</type>と宣言して書かれる必要があります。なぜなら、SQLには同等のものがないためです。
このケースに対処するために、集約の入力引数と一致する追加の<quote>ダミー</quote>引数を取るように最終関数を宣言することが可能です。
最終関数が呼び出されたときに特定の値を使用できないため、このようなダミー引数は常にnull値として渡されます。
それらは、多様最終関数の結果の型を集約の入力型（複数可）に合わせる場合のみ使用します。
 例えば以下の定義は、組み込み集約の<function>array_agg</function>と等価です。␞␞␞
␝␟   Here, the <literal>finalfunc_extra</literal> option specifies that the final
   ...
   of <function>array_agg_finalfn</function> to be valid.␟ここで、<literal>finalfunc_extra</literal>オプションは最終関数が状態値に加えて、集約の入力引数（複数可）に対応する追加のダミー引数（複数可）を受け取れることを指定します。
<function>array_agg_finalfn</function>の追加引数<type>anynonarray</type>により有効であると宣言をすることができます。␞␞  </para>␞
␝  <para>␟   An aggregate function can be made to accept a varying number of arguments
   ...
   but this is not strictly required.␟集約関数は、通常の関数の場合とほとんど同じ方法で、最後の引数を<literal>VARIADIC</literal>配列として宣言することで、可変長の引数を受け入れるようにすることができます。 <xref linkend="xfunc-sql-variadic-functions"/>を参照してください。
集約の遷移関数(複数可)は、それら最後の引数と同じ配列型を持っている必要があります。
遷移関数(複数可)は、典型的には、<literal>VARIADIC</literal>付きになりますが、これは必須ではありません。␞␞  </para>␞
␝   <para>␟    Variadic aggregates are easily misused in connection with
    ...
    aggregate.  For example, in␟可変長集約は、<literal>ORDER BY</literal>オプション（<xref linkend="syntax-aggregates"/>を参照してください）との組み合わせでは、パーサが実引数かどうかを見分けることができないので、簡単に誤用されるようになります。
<literal>ORDER BY</literal>の右側にあるすべてのものは、集約への引数ではなく、ソートキーであることに留意してください。
例えば、␞␞<programlisting>␞
␝</programlisting>␟    the parser will see this as a single aggregate function argument and
    three sort keys.  However, the user might have intended␟パーサには集約関数の引数１つと３つのソートキーと見えます。
しかし、これは以下のようにユーザが意図している可能性があります。␞␞<programlisting>␞
␝</programlisting>␟    If <literal>myaggregate</literal> is variadic, both these calls could be
    perfectly valid.␟もし<literal>myaggregate</literal>が可変長引数の場合、これらの呼び出しが両方とも妥当かもしれません。␞␞   </para>␞
␝   <para>␟    For the same reason, it's wise to think twice before creating aggregate
    functions with the same names and different numbers of regular arguments.␟同じ理由で、通常の引数の数とは違う同じ名前の集約関数を作成する前に二度考えるのが賢明です。␞␞   </para>␞
␝  <indexterm>
   <primary>aggregate function</primary>
...
␝ <sect2 id="xaggr-ordered-set-aggregates">␟  <title>Ordered-Set Aggregates</title>␟  <title>順序集合の集約</title>␞␞␞
␝  <para>␟   The aggregates we have been describing so far are <quote>normal</quote>
   ...
   definition of <function>percentile_disc</function> is equivalent to:␟これまでに記述された集約は<quote>通常の</quote>集約です。
<productname>PostgreSQL</productname>は、<firstterm>順序集合集約</firstterm>もサポートします。それは、通常の集約とは２つの大きな違いがあります。
第一に、入力行ごとに評価される通常の集約引数に加えて、順序集合集約は、集約の呼び出しの時に一度だけ評価される<quote>直接</quote>引数を持つことが出来ます。
第二に、集約引数の構文は通常、明示的にソート順を指定します。
順序集合集約は通常、呼び出すソート順が必要な局面、例えば順位や百分位数(パーセンタイル)のような特定の行の順序に依存して計算する実装のために使用されます。
例えば、以下は組み込み関数<function>percentile_disc</function>の定義と同じです。␞␞␞
␝␟   This aggregate takes a <type>float8</type> direct argument (the percentile
   ...
   It could be used to obtain a median household income like this:␟この集約は、<type>float8</type>型の直接引数（百分位数）と、任意のソート可能なデータ型を集約の入力として取ります。
それは、以下のように家計所得の中央値を得ることができます。␞␞␞
␝␟   Here, <literal>0.5</literal> is a direct argument; it would make no sense
   for the percentile fraction to be a value varying across rows.␟ここで<literal>0.5</literal>は直接の引数です。百分位数が行毎に変化する値であったら意味がありません。␞␞  </para>␞
␝  <para>␟   Unlike the case for normal aggregates, the sorting of input rows for
   ...
   already-sorted state.␟通常の集約の場合とは違って、順序集合集約のための入力行のソートは、裏側でおこなわれて<emphasis>いません</emphasis>。それは集約のサポート関数の責任です。
典型的な実装方法は、集約の状態値に<quote>tuplesort</quote>オブジェクトへの参照を保持し、そのオブジェクトに入ってくる行を供給した後、ソートを完了し、最終関数内でデータを読み出すことです。
この設計は、最終関数がソートされるデータに追加の<quote>架空</quote>行を注入するなどの特別な操作を実行するのを可能にします。
...
(上の例では、状態値が<type>内部</type>型 &mdash; として宣言されていることに気づくでしょう。これは典型的なものです。)
また、最終関数がソートを実行しますので、遷移関数を後で再び実行し、引き続き入力行を追加することはできません。
これは最終関数が<literal>READ_ONLY</literal>ではないことを意味します。追加の最終関数呼び出しで既にソートされた状態を使えるのなら、<link linkend="sql-createaggregate"><command>CREATE AGGREGATE</command></link>で<literal>READ_WRITE</literal>または<literal>SHAREABLE</literal>と宣言しなければなりません。␞␞  </para>␞
␝  <para>␟   The state transition function for an ordered-set aggregate receives
   ...
   type(s).␟順序集合集約のための状態遷移関数は、現在の状態値を加えた行ごとに集約入力値を受信し、更新された状態値を返します。
これは通常の集約と同じ定義ですが、（もしあっても）直接の引数が提供されていないことに注意してください。
最終関数は、最後の状態値、もしあれば直接の引数の値、および（<literal>finalfunc_extra</literal>が指定された場合）集約入力（複数）に対応するnull値を受信します。
通常の集約と同様に、<literal>finalfunc_extra</literal>は集約が多様(型)である場合にのみ便利です。そのとき集約の入力型(複数可)が、最終関数の結果の型と合わせるために追加のダミー引数が必要になります。␞␞  </para>␞
␝  <para>␟   Currently, ordered-set aggregates cannot be used as window functions,
   and therefore there is no need for them to support moving-aggregate mode.␟現在、順序集合集約は、ウィンドウ関数として使用することができないので移動集約モードをサポートする必要はありません。␞␞  </para>␞
␝  <indexterm>
   <primary>aggregate function</primary>
...
␝  <sect2 id="xaggr-partial-aggregates">␟  <title>Partial Aggregation</title>␟  <title>部分集約</title>␞␞␞
␝  <para>␟   Optionally, an aggregate function can support <firstterm>partial
   ...
   yet.)␟省略可能ですが、集約関数は<firstterm>部分集約</firstterm>をサポート出来ます。
...
（将来このモードは、ローカルとリモートのテーブルの集計を結合させるなどの目的のために使用されるかもしれません。それはまだ実装されていません。）␞␞  </para>␞
␝  <para>␟   To support partial aggregation, the aggregate definition must provide
   ...
   aggregates that are sensitive to input row order.␟部分集約をサポートするためには、集約定義が<firstterm>結合関数</firstterm>を提供しなければなりません。
...
これは入力行の順序に敏感な集約のための結合関数を定義することは通常不可能だということを意味します。␞␞  </para>␞
␝  <para>␟   As simple examples, <literal>MAX</literal> and <literal>MIN</literal> aggregates can be
   ...
   type.)␟簡単な例を示します。<literal>MAX</literal>と<literal>MIN</literal>集約は、その遷移関数として使用される「２つの大なり」比較、又は「２つの小なり」比較関数と同じ結合関数を指定することで部分集約をサポートすることが出来ます。
<literal>SUM</literal>集約は結合関数として加算関数が必要になります。
（ここでも、入力データ型よりも状態値が広い場合を除き遷移関数と同じです。）␞␞  </para>␞
␝  <para>␟   The combine function is treated much like a transition function that
   ...
   which will be called to combine each partial result into that state.␟結合関数は、２番目の引数として、基本となる入力型ではなく状態型の値を取りますが、遷移関数のように扱われています。
具体的には、null値とstrict関数に対処するためのルールは似ています。
また、<literal>initcond</literal>が非nullである集約定義を指定する場合、各部分集約の実行のための初期状態として使用されるだけでなく、各部分の結果をその状態に結合するために呼び出される結合関数の初期状態としても使用されることに留意してください。␞␞  </para>␞
␝  <para>␟   If the aggregate's state type is declared as <type>internal</type>, it is
   ...
   context and will not have sufficient lifespan.␟集約の状態型が<type>internal</type>で宣言されている場合、その結果が集約状態値の正しいメモリコンテキストに割り当てられていることは結合関数の責任です。
これは特に、以下のことを意味します。最初の入力が<literal>NULL</literal>だと、単純に２番目の入力を返すのは無効です。なぜなら、その値が間違ったコンテキストになり、そして十分な寿命を持っていないことになります。␞␞  </para>␞
␝  <para>␟   When the aggregate's state type is declared as <type>internal</type>, it is
   ...
   probably not work either.␟集約の状態型を<type>internal</type>で宣言することは、<firstterm>シリアライズ関数</firstterm>と<firstterm>デシリアライズ関数</firstterm>を提供するために通常適切です。これらの関数は、状態値を１つのプロセスから別のプロセスにコピーすることを可能にします。
これらの関数がなければ、並列集約を行うことができず、ローカル/リモート集約などの将来のアプリケーションも、おそらく動作しません。␞␞  </para>␞
␝  <para>␟   A serialization function must take a single argument of
   ...
   long-lived.␟シリアライズ関数は、<type>internal</type>の単一の引数を取り、フラットなblobのバイト状態値パッケージを表わす<type>bytea</type>型を返します。
逆にデシリアライズ関数はその変換を逆にします。
<type>bytea</type>型と<type>internal</type>型の２つの引数を取り、<type>internal</type>型を返します。
（第２引数は使用せず常に０ですが、型の安全性の理由のために必要とされます。）
デシリアライズ関数の結果は単純に、現在のメモリコンテキストに割り当てる必要があります。結合関数の結果とは異なり、長寿命でありません。␞␞  </para>␞
␝  <para>␟   Worth noting also is that for an aggregate to be executed in parallel,
   the aggregate itself must be marked <literal>PARALLEL SAFE</literal>.  The
   parallel-safety markings on its support functions are not consulted.␟集約を並列に実行するために、集約自体に<literal>PARALLEL SAFE</literal>マークが、されなければならないというのは注目する価値があります。
それのサポート関数のパラレルセーフマークは参照されません。␞␞  </para>␞
␝  <indexterm>
   <primary>aggregate function</primary>
...
␝ <sect2 id="xaggr-support-functions">␟  <title>Support Functions for Aggregates</title>␟  <title>集約サポート関数</title>␞␞␞
␝  <para>␟   A function written in C can detect that it is being called as an
   aggregate support function by calling
   <function>AggCheckCallContext</function>, for example:␟C言語で作成された関数は、<function>AggCheckCallContext</function>を呼び出して、集約サポート関数として呼び出されているかを検出することができます。
例えば、␞␞<programlisting>␞
␝</programlisting>␟   One reason for checking this is that when it is true, the first input
   ...
   for more detail.)␟この検査を行う理由の１つは、これが真の場合、先頭の入力は一時的な状態値であるはずなので、新規に割り当ててコピーを持つことなくそのまま変更しても安全だからです。
例として<function>int8inc()</function>を参照してください。
（集約遷移関数は常に遷移値をその場で変更できますが、集約最終関数ではそのようなことをするのは一般には勧められません。もし、そうするなら集約を定義する時にその振舞いを宣言しなければなりません。
より詳しくは<xref linkend="sql-createaggregate"/>を見てください。）␞␞  </para>␞
␝  <para>␟   The second argument of <function>AggCheckCallContext</function> can be used to
   ...
   aggregate.)␟<function>AggCheckCallContext</function>の第２引数は、集約の状態値が保管されているメモリコンテキストを取得するために使用できます。
これは状態値として<quote>展開された</quote>オブジェクト（<xref linkend="xtypes-toast"/>を参照）を使用する遷移関数に便利です。
最初の呼び出しで、遷移関数はメモリコンテキストが集約状態のコンテキストの子である展開されたオブジェクトを返し、その後の呼び出しで同じ展開されたオブジェクトを保持し続ける必要があります。
<function>array_append()</function>の例を参照してください。
（<function>array_append()</function>は組み込み集約の遷移関数ではありませんが、カスタム集約の遷移関数で使用すると効率的に動作するように書かれています。）␞␞  </para>␞
␝  <para>␟   Another support routine available to aggregate functions written in C
   ...
   source code.␟別のサポートルーチンとしてC言語で書かれた<function>AggGetAggref</function>集約関数が利用可能です。それは、集約の呼び出しを定義する<literal>Aggref</literal>パースノードを返します。
これは主に順序集合集約で有用です。これはソートの順序をどう実現するか<literal>Aggref</literal>ノードの内部構造まで検査することができます。
その例は、<productname>PostgreSQL</productname>ソースコード中の<filename>orderedsetaggs.c</filename>から見つけることができます。␞␞  </para>␞
