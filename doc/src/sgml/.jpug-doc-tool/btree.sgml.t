...
␝<sect1 id="btree">␟<title>B-Tree Indexes</title>␟<title>B-Treeインデックス</title>␞␞␞
␝<sect2 id="btree-intro">␟ <title>Introduction</title>␟ <title>はじめに</title>␞␞␞
␝ <para>␟  <productname>PostgreSQL</productname> includes an implementation of the
  ...
  compression, if applicable).␟<productname>PostgreSQL</productname>は、標準的な<acronym>btree</acronym>（multi-way balanced tree）インデックスデータ構造を実装しています。
明確に定義された線形順にソート可能なデータ型は、すべてbtreeインデックスで索引付できます。
唯一の制限は、一つのインデックスエントリが（適用可能であれば、TOAST圧縮後）ページの約1/3を超えられないことです。␞␞ </para>␞
␝ <para>␟  Because each btree operator class imposes a sort order on its data type,
  ...
  btree <acronym>AM</acronym> make use of them.␟btree演算子クラスはそのデータ型がソート順を持つことが必要なので、btree演算子クラス（実際には演算子族）は、<productname>PostgreSQL</productname>の一般的表現として、およびソートセマンティクスを理解するものとして利用されてきました。
ですから、単にbtreeインデックスをサポートするだけに必要なもの以上の機能と、btree <acronym>AM</acronym>が使用するものからはかけ離れたシステムの部品を備えなければなりません。␞␞ </para>␞
␝<sect2 id="btree-behavior">␟ <title>Behavior of B-Tree Operator Classes</title>␟ <title>B-Tree演算子クラスの振る舞い</title>␞␞␞
␝ <para>␟  As shown in <xref linkend="xindex-btree-strat-table"/>, a btree operator
  ...
  from <structname>pg_amop</structname>.)␟<xref linkend="xindex-btree-strat-table"/>で示すように、btree演算子クラスは次の5つの比較演算子を提供しなければなりません。
...
しかし、プランナは<structname>pg_amop</structname>から検索するのではなく<literal>=</literal>の否定子リンクから検索します。）␞␞ </para>␞
␝ <para>␟  When several data types share near-identical sorting semantics, their
  ...
  comparison conditions that it deduces from transitivity.␟複数のデータ型がほとんど同じソートセマンティクスを共有している場合、それらの演算子クラスは演算子族にまとめることができます。
そうすることによりプランナが型をまたがる比較を推論できるので、これはメリットがあります。
演算子族内の各演算子クラスは、入力データ型のための単一型演算子（および関連するサポート関数）を含むべきです。
一方、型をまたがる比較演算子とサポート関数は演算子族中で<quote>ゆるやか</quote>です。
プランナが推移関係から推論するすべての比較条件を提示できるように、型をまたがる演算子の完全な集合を演算子族に入れておくことをお勧めします。␞␞ </para>␞
␝ <para>␟  There are some basic assumptions that a btree operator family must
  satisfy:␟btree演算子族が満たさなければならない基本的な前提条件があります。␞␞ </para>␞
␝   <para>␟    An <literal>=</literal> operator must be an equivalence relation; that
    is, for all non-null values <replaceable>A</replaceable>,
    <replaceable>B</replaceable>, <replaceable>C</replaceable> of the
    data type:␟<literal>=</literal>演算子は等号関係でなければなりません。
つまり、そのデータ型のすべての非NULL値<replaceable>A</replaceable>、<replaceable>B</replaceable>、<replaceable>C</replaceable>について、␞␞␞
␝      <para>␟       <replaceable>A</replaceable> <literal>=</literal>
       <replaceable>A</replaceable> is true
       (<firstterm>reflexive law</firstterm>)␟<replaceable>A</replaceable> <literal>=</literal> <replaceable>A</replaceable>が真である（<firstterm>反射律</firstterm>）␞␞      </para>␞
␝      <para>␟       if <replaceable>A</replaceable> <literal>=</literal>
       <replaceable>B</replaceable>,
       then <replaceable>B</replaceable> <literal>=</literal>
       <replaceable>A</replaceable>
       (<firstterm>symmetric law</firstterm>)␟<replaceable>A</replaceable> <literal>=</literal> <replaceable>B</replaceable>なら、<replaceable>B</replaceable> <literal>=</literal> <replaceable>A</replaceable>である（<firstterm>対称律</firstterm>）␞␞      </para>␞
␝      <para>␟       if <replaceable>A</replaceable> <literal>=</literal>
       ...
       (<firstterm>transitive law</firstterm>)␟<replaceable>A</replaceable> <literal>=</literal> <replaceable>B</replaceable>かつ<replaceable>B</replaceable><literal>=</literal> <replaceable>C</replaceable>なら、<replaceable>A</replaceable> <literal>=</literal> <replaceable>C</replaceable>である（<firstterm>推移律</firstterm>）␞␞      </para>␞
␝   <para>␟    A <literal>&lt;</literal> operator must be a strong ordering relation;
    that is, for all non-null values <replaceable>A</replaceable>,
    <replaceable>B</replaceable>, <replaceable>C</replaceable>:␟<literal>&lt;</literal>は強順序関係でなければなりません。つまり、すべての非NULL値<replaceable>A</replaceable>、<replaceable>B</replaceable>、<replaceable>C</replaceable>に対して、␞␞␞
␝      <para>␟       <replaceable>A</replaceable> <literal>&lt;</literal>
       <replaceable>A</replaceable> is false
       (<firstterm>irreflexive law</firstterm>)␟<replaceable>A</replaceable> <literal>&lt;</literal> <replaceable>A</replaceable>は偽である（<firstterm>非反射律</firstterm>）␞␞      </para>␞
␝      <para>␟       if <replaceable>A</replaceable> <literal>&lt;</literal>
       <replaceable>B</replaceable>
       ...
       <replaceable>C</replaceable>
       (<firstterm>transitive law</firstterm>)␟<replaceable>A</replaceable> <literal>&lt;</literal> <replaceable>B</replaceable>かつ<replaceable>B</replaceable> <literal>&lt;</literal> <replaceable>C</replaceable>なら、<replaceable>A</replaceable> <literal>&lt;</literal> <replaceable>C</replaceable>である（<firstterm>推移律</firstterm>）␞␞      </para>␞
␝   <para>␟    Furthermore, the ordering is total; that is, for all non-null
    values <replaceable>A</replaceable>, <replaceable>B</replaceable>:␟更に、順序は全である。すなわち、すべての非NULL値<replaceable>A</replaceable>、<replaceable>B</replaceable>に対して、␞␞␞
␝      <para>␟       exactly one of <replaceable>A</replaceable> <literal>&lt;</literal>
       ...
       <replaceable>A</replaceable> is true
       (<firstterm>trichotomy law</firstterm>)␟厳密に<replaceable>A</replaceable> <literal>&lt;</literal> <replaceable>B</replaceable>、<replaceable>A</replaceable> <literal>=</literal> <replaceable>B</replaceable>、<replaceable>B</replaceable> <literal>&lt;</literal> <replaceable>A</replaceable>のうちどれか一つが真（<firstterm>三分律</firstterm>）␞␞      </para>␞
␝␟    (The trichotomy law justifies the definition of the comparison support
    function, of course.)␟（もちろん、三分律は比較サポート関数の定義を正当化します。）␞␞   </para>␞
␝ <para>␟  The other three operators are defined in terms of <literal>=</literal>
  and <literal>&lt;</literal> in the obvious way, and must act consistently
  with them.␟他の3つの演算子は<literal>=</literal>と<literal>&lt;</literal>に沿って自明に定義され、それらと一貫していなければなりません。␞␞ </para>␞
␝ <para>␟  For an operator family supporting multiple data types, the above laws must
  ...
  same <type>float8</type> value, and thus the transitive law would fail.␟複数のデータ型をサポートする演算子族について、演算子族中のデータ型であるどんな<replaceable>A</replaceable>、<replaceable>B</replaceable>、<replaceable>C</replaceable>も上記の法則を満たさなければなりません。
...
これは同じ<type>float8</type>値に対して等号比較する複数の異なる<type>numeric</type>値が存在することを意味し、したがって推移律は満たされません。␞␞ </para>␞
␝ <para>␟  Another requirement for a multiple-data-type family is that any implicit
  or binary-coercion casts that are defined between data types included in
  the operator family must not change the associated sort ordering.␟複数データ型族に関する別な要件は、演算子族に含まれるデータ型間に定義される暗黙的あるいは二値型強制(binary-coercion)キャストは、関係するソート順を変更してはならないことです。␞␞ </para>␞
␝ <para>␟  It should be fairly clear why a btree index requires these laws to hold
  ...
  planner relies on them for optimization purposes.␟単一のデータ型において、btreeインデックスがこれらの法則を守ることを要求するのはかなり明確です。
これらの法則なしにはキー並べる順序がなくなってしまうからです。
また、異なるデータ型の比較キーを使うインデックス検索では、2つのデータ型またがる比較が正常に動作することが必要です。
演算子族中で3つ以上のデータ型に対する拡張はbtreeインデックスの機構自体では要求されませんが、プランナは最適化の目的でそれらに依存します。␞␞ </para>␞
␝<sect2 id="btree-support-funcs">␟ <title>B-Tree Support Functions</title>␟ <title>B-Treeサポート関数</title>␞␞␞
␝ <para>␟  As shown in <xref linkend="xindex-btree-support-table"/>, btree defines
  one required and four optional support functions.  The five
  user-defined methods are:␟<xref linkend="xindex-btree-support-table"/>で示すように、btreeでは一つの必須サポート関数と、4つの省略可能なサポート関数を定義します。
5つのユーザ定義メソッドは以下の通りです。␞␞ </para>␞
␝    <para>␟     For each combination of data types that a btree operator family
     ...
     examples.␟btreeの演算子族が比較演算子を提供する各データ型の組み合わせに対して、比較サポート関数を提供しなければなりません。それらはサポート関数1番で<structname>pg_amproc</structname>に、また、比較での左右のデータ型と等しい<structfield>amproclefttype</structfield>/<structfield>amprocrighttype</structfield>に、登録されます（すなわち、<structname>pg_amop</structname>に登録されている演算子が対応するものと同じデータ型です）。
...
例として<filename>src/backend/access/nbtree/nbtcompare.c</filename>を参照してください。␞␞    </para>␞
␝    <para>␟     If the compared values are of a collatable data type, the
     appropriate collation OID will be passed to the comparison
     support function, using the standard
     <function>PG_GET_COLLATION()</function> mechanism.␟比較される値が照合順序が適用可能なデータ型のものである場合、比較サポート関数に適切な照合順序のOIDが渡され、標準の<function>PG_GET_COLLATION()</function>機構が使用されます。␞␞    </para>␞
␝    <indexterm>
...
␝    <para>␟     Optionally, a btree operator family may provide <firstterm>sort
     ...
     <filename>src/include/utils/sortsupport.h</filename>.␟任意で、btree演算子族は<firstterm>ソートサポート</firstterm>関数を提供してもよいです。これはサポート関数2番で登録されます。
この関数は、素朴に比較サポート関数を呼び出すよりも、ソート目的により効果的な方法での比較の実装を可能にします。
これに関するAPIは<filename>src/include/utils/sortsupport.h</filename>で定義されています。␞␞    </para>␞
␝    <para>␟     Optionally, a btree operator family may provide
     ...
     compatible with the family's data ordering.␟任意で、btree演算子族は<firstterm>in_range</firstterm>サポート関数を提供してもよいです。これはサポート関数3番に登録されます。
...
基本的には、提供される拡張情報はどのように演算子族のデータ並び順と互換性のある方法で<replaceable>offset</replaceable>値を足すか引くかです。␞␞    </para>␞
␝    <para>␟     An <function>in_range</function> function must have the signature␟<function>in_range</function>関数は以下のシグネチャを持たなければなりません。␞␞<synopsis>␞
␝</synopsis>␟     <replaceable>val</replaceable> and
     ...
     equal to <type>type2</type>.␟<replaceable>val</replaceable>と<replaceable>base</replaceable>は同じ型でなければならず、これは演算子族でサポートされる型の一つ（すなわち、並び順を提供する対象の型）です。
...
各<function>in_range</function>関数は、<structname>pg_amproc</structname>に<type>type1</type>と等しい<structfield>amproclefttype</structfield>と<type>type2</type>に等しい<structfield>amprocrighttype</structfield>で登録されるべきです。␞␞    </para>␞
␝    <para>␟     The essential semantics of an <function>in_range</function>
     ...
     <replaceable>val</replaceable> to the result, as follows:␟<function>in_range</function>関数の本質的なセマンティクスは2つのBooleanフラグパラメータに依存します。
これは以下のように、<replaceable>base</replaceable>に<replaceable>offset</replaceable>を加算または減算して、それから<replaceable>val</replaceable>を結果と比較すべきです。␞␞     <itemizedlist>␞
␝       <para>␟        if <literal>!</literal><replaceable>sub</replaceable> and
        ...
        <replaceable>offset</replaceable>)␟        <literal>!</literal><replaceable>sub</replaceable>かつ
        <literal>!</literal><replaceable>less</replaceable>であるなら、
        <replaceable>val</replaceable> <literal>&gt;=</literal>
        (<replaceable>base</replaceable> <literal>+</literal>
        <replaceable>offset</replaceable>)を返します␞␞       </para>␞
␝       <para>␟        if <literal>!</literal><replaceable>sub</replaceable> and
        ...
        (<replaceable>base</replaceable> <literal>+</literal>
        <replaceable>offset</replaceable>)を返します␞␞       </para>␞
␝       <para>␟        if <replaceable>sub</replaceable> and
        <literal>!</literal><replaceable>less</replaceable>, return
        <replaceable>val</replaceable> <literal>&gt;=</literal>
        (<replaceable>base</replaceable> <literal>-</literal>
        <replaceable>offset</replaceable>)␟        <replaceable>sub</replaceable>かつ
        <literal>!</literal><replaceable>less</replaceable>であるなら、
        <replaceable>val</replaceable> <literal>&gt;=</literal>
        (<replaceable>base</replaceable> <literal>-</literal>
        <replaceable>offset</replaceable>)を返します␞␞       </para>␞
␝       <para>␟        if <replaceable>sub</replaceable> and
        ...
        (<replaceable>base</replaceable> <literal>-</literal>
        <replaceable>offset</replaceable>)を返します␞␞       </para>␞
␝     </itemizedlist>␟     Before doing so, the function should check the sign of
     ...
     the core code needn't understand what <quote>less than
      zero</quote> means for a particular data type.␟このように実行する前に、本関数は、<replaceable>offset</replaceable>の符号を検査すべきです。
...
中核コードが特定のデータ型における<quote>ゼロより小さい</quote>ことの意味を理解しなくても良いように、この要件は<function>in_range</function>関数に委託されます。␞␞    </para>␞
␝    <para>␟     An additional expectation is that <function>in_range</function>
     ...
     sort order of the operator family.␟さらに期待されることは、<function>in_range</function>関数は、実用的には、<replaceable>base</replaceable> <literal>+</literal> <replaceable>offset</replaceable>や<replaceable>base</replaceable> <literal>-</literal> <replaceable>offset</replaceable>がオーバーフローする場合にエラーを投げるのを避けるべきです。
たとえ値がデータ型の範囲を超えたとしても正しい比較結果は決定できます。
データ型が<quote>infinity</quote>や<quote>NaN</quote>などの概念を含む場合には、<function>in_range</function>の結果が演算子族の通常のソート順序と一致するように特別な対応が必要となることに注意してください。␞␞    </para>␞
␝    <para>␟     The results of the <function>in_range</function> function must be
     consistent with the sort ordering imposed by the operator family.
     To be precise, given any fixed values of
     <replaceable>offset</replaceable> and
     <replaceable>sub</replaceable>, then:␟<function>in_range</function>関数の結果は、演算子族で規定されるソート順序と整合していなければなりません。
正確には、与えられた任意の<replaceable>offset</replaceable>と<replaceable>sub</replaceable>の固定値については以下のようになります。␞␞     <itemizedlist>␞
␝       <para>␟        If <function>in_range</function> with
        ...
        <replaceable>base</replaceable>.␟<replaceable>less</replaceable> = trueの<function>in_range</function>がいくつかの<replaceable>val1</replaceable>と<replaceable>base</replaceable>に対して真であるなら、同じ<replaceable>base</replaceable>の全ての<replaceable>val2</replaceable> <literal>&lt;=</literal> <replaceable>val1</replaceable>に対して真でなければなりません。␞␞       </para>␞
␝       <para>␟        If <function>in_range</function> with
        ...
        <replaceable>val2</replaceable> <literal>&gt;=</literal>
        <replaceable>val1</replaceable> with the same
        <replaceable>base</replaceable>.␟<replaceable>less</replaceable> = trueの<function>in_range</function>が、いくつかの<replaceable>val1</replaceable>と<replaceable>base</replaceable>に対して偽であるなら、同じ<replaceable>base</replaceable>の全ての<replaceable>val2</replaceable> <literal>&gt;=</literal> <replaceable>val1</replaceable>に対して偽でなければなりません。␞␞       </para>␞
␝       <para>␟        If <function>in_range</function> with
        <replaceable>less</replaceable> = true is true for some
        ...
        <replaceable>val</replaceable>.␟<replaceable>less</replaceable> = trueの<function>in_range</function>がいくつかの<replaceable>val</replaceable>と<replaceable>base1</replaceable>に対して真であるなら、同じ<replaceable>val</replaceable>の全ての<replaceable>base2</replaceable> <literal>&gt;=</literal> <replaceable>base1</replaceable>に対して真でなければなりません。␞␞       </para>␞
␝       <para>␟        If <function>in_range</function> with
        <replaceable>less</replaceable> = true is false for some
        <replaceable>val</replaceable> and
        ...
        <replaceable>base2</replaceable> <literal>&lt;=</literal>
        <replaceable>base1</replaceable> with the same
        <replaceable>val</replaceable>.␟<replaceable>less</replaceable> = trueの<function>in_range</function>が一部の<replaceable>val</replaceable>と<replaceable>base1</replaceable>に対して偽であるなら、同じ<replaceable>val</replaceable>の全ての<replaceable>base2</replaceable> <literal>&lt;=</literal> <replaceable>base1</replaceable>に対して偽でなければなりません。␞␞       </para>␞
␝     </itemizedlist>␟     Analogous statements with inverted conditions hold when
     <replaceable>less</replaceable> = false.␟<replaceable>less</replaceable> = falseのときには、逆条件の類似した命題が適用できます。␞␞    </para>␞
␝    <para>␟     If the type being ordered (<type>type1</type>) is collatable, the
     appropriate collation OID will be passed to the
     <function>in_range</function> function, using the standard
     PG_GET_COLLATION() mechanism.␟整列しようとしている型(<type>type1</type>)が照合可能であるなら、標準のPG_GET_COLLATION()機構を使って、<function>in_range</function>関数に適切な照合順序のOIDが渡されます。␞␞    </para>␞
␝    <para>␟     <function>in_range</function> functions need not handle NULL
     inputs, and typically will be marked strict.␟<function>in_range</function>関数は、通例STRICTと印付けされ、NULL入力を処理する必要がありません。␞␞    </para>␞
␝    <para>␟     Optionally, a btree operator family may provide
     ...
     functions are only called when building or rebuilding an index.␟省略可能ですが、btree演算子族は<function>equalimage</function> (<quote>イメージ等価を意味する等価</quote>)サポート関数を提供してもよいです。これはサポート関数4番で登録されます。
この関数は、中核コードがbtree重複排除の最適化を適用するのが安全かを決定できるようにします。
今のところ、<function>equalimage</function>関数はインデックスの構築または再構築時にのみ呼び出されます。␞␞    </para>␞
␝    <para>␟     An <function>equalimage</function> function must have the
     signature␟<function>equalimage</function>関数は以下のシグネチャを持たなければなりません。␞␞<synopsis>␞
␝</synopsis>␟     The return value is static information about an operator class
     ...
     assumed to hold.␟戻り値は演算子クラスと照合順序についての静的な情報です。
...
<function>equalimage</function>関数が登録されていなかったり、<literal>false</literal>を返すことは、この条件は守られないであろうことを示します。␞␞    </para>␞
␝    <para>␟     The <replaceable>opcintype</replaceable> argument is the
     ...
     <function>PG_GET_COLLATION()</function> mechanism.␟<replaceable>opcintype</replaceable>引数は演算子クラスタがインデックスを作るデータ型の<literal><structname>pg_type</structname>.oid</literal>です。
これは同じ基となる<function>equalimage</function>関数を演算子クラスを横断して再利用できるようになる利便性があります。
<replaceable>opcintype</replaceable>が照合可能なデータ型である場合には、適切な照合順序のOIDが、標準の<function>PG_GET_COLLATION()</function>機構を使って、<function>equalimage</function>関数に渡されます。␞␞    </para>␞
␝    <para>␟     As far as the operator class is concerned, returning
     ...
     called.␟演算子クラスに関する限り、<literal>true</literal>を返すことは、重複排除が安全（あるいは<function>equalimage</function>関数に渡されたOIDの照合順序について安全）であることを示します。
しかしながら、コアコードは、<emphasis>全ての</emphasis>インデックス列が<function>equalimage</function>関数を登録する演算子クラスを使っていて、各関数が呼ばれたとき実際に<literal>true</literal>を返すときに、そのインデックスに対して重複排除を安全と見做すだけです。␞␞    </para>␞
␝    <para>␟     Image equality is <emphasis>almost</emphasis> the same condition
     ...
     functions).␟イメージ等価は単純にビット毎に等しいことと<emphasis>ほとんど</emphasis>同じ条件です。
...
形式的には、演算子クラスの<function>equalimage</function>関数が<literal>true</literal>を返すときには、<literal>datum_image_eq()</literal> C関数が常に演算子クラスの<function>order</function>関数と一致すると想定して安全でした（同じ照合順序のOIDが<function>equalimage</function>と<function>order</function>の両関数に渡されるとして）。␞␞    </para>␞
␝    <para>␟     The core code is fundamentally unable to deduce anything about
     ...
     particular data type implements must be considered separately.␟コアコードは基本的に、複数データ型の族の中の演算子クラスの<quote>等価性がイメージ等価性を含む</quote>状態について、同族の他の演算子クラスの詳細に基づいた、いかなる推測もできません。
...
一般に、ある特定データ型の実装によるセマンティクスは別個に考慮されなければなりません。␞␞    </para>␞
␝    <para>␟     The convention followed by the operator classes included with the
     ...
     custom function to retain control.␟<productname>PostgreSQL</productname>のコア配布物に含まれる演算子クラスが従う慣習は、標準品、すなわち、一般的な<function>equalimage</function>関数を登録することです。
...
サードパーティ拡張におけるベストプラクティスは制御を保つためにそれら自身のカスタム関数を登録することです。␞␞    </para>␞
␝    <para>␟     Optionally, a B-tree operator family may provide
     ...
     parameters that control operator class behavior.␟省略可能ですが、B-treeの演算子族は<function>options</function>（<quote>演算子クラス固有オプション</quote>）サポート関数を提供してもよいです。これはサポート関数5番に登録されます。
この関数はユーザに見える演算子クラスの振る舞いを制御するパラメータの集合を定義します。␞␞    </para>␞
␝    <para>␟     An <function>options</function> support function must have the
     signature␟<function>options</function>サポート関数は以下のシグネチャを持たなければなりません。␞␞<synopsis>␞
␝</synopsis>␟     The function is passed a pointer to a <structname>local_relopts</structname>
     struct, which needs to be filled with a set of operator class
     specific options.  The options can be accessed from other support
     functions using the <literal>PG_HAS_OPCLASS_OPTIONS()</literal> and
     <literal>PG_GET_OPCLASS_OPTIONS()</literal> macros.␟関数には<structname>local_relopts</structname>構造体へのポインタが渡されます。ここには演算子クラス固有のオプションの集合が書かれている必要があります。
このオプションには<literal>PG_HAS_OPCLASS_OPTIONS()</literal>および<literal>PG_GET_OPCLASS_OPTIONS()</literal>マクロを使って他のサポート関数からアクセスが可能です。␞␞    </para>␞
␝    <para>␟     Currently, no B-Tree operator class has an <function>options</function>
     ...
     evolution of B-tree in <productname>PostgreSQL</productname>.␟今のところ、<function>options</function>サポート関数を持ったB-Treeの演算子クラスはありません。
...
それでも、統一性のためにサポート関数がB-treeに追加されました。おそらく<productname>PostgreSQL</productname>でのB-treeの更なる進化の過程で使用法を見つけ出すでしょう。␞␞    </para>␞
␝<sect2 id="btree-implementation">␟ <title>Implementation</title>␟ <title>実装</title>␞␞␞
␝ <para>␟  This section covers B-Tree index implementation details that may be
  of use to advanced users.  See
  <filename>src/backend/access/nbtree/README</filename> in the source
  distribution for a much more detailed, internals-focused description
  of the B-Tree implementation.␟本節では、上級ユーザに役立つかもしれない、B-Treeインデックスの実装の詳細について説明します。
更なる詳細、B-Tree実装の内部に焦点をあてた記述については、ソース配布物の<filename>src/backend/access/nbtree/README</filename>を参照してください。␞␞ </para>␞
␝ <sect3 id="btree-structure">␟  <title>B-Tree Structure</title>␟  <title>B-Treeの構造</title>␞␞  <para>␞
␝  <para>␟   <productname>PostgreSQL</productname> B-Tree indexes are
   ...
   pages use the standard page format described in <xref
    linkend="storage-page-layout"/>.␟<productname>PostgreSQL</productname>のB-Treeインデックスは複数階層のツリー構造で、ツリーの各階層はページの双方向連結リストとして使用できます。
...
内部ページとリーフページは共に、<xref linkend="storage-page-layout"/>に記載されている標準のページ書式を使用します。␞␞  </para>␞
␝  <para>␟   New leaf pages are added to a B-Tree index when an existing leaf
   ...
   above the original root page.␟既存リーフページがやってくるタプルをはめ込むことができないとき、新たなリーフページがB-Treeインデックスに追加されます。
...
これは元のルートページの一つ上の階層に新たなルートページを作ることで、ツリー構造に新しい階層を加えます。␞␞  </para>␞
␝ <sect3 id="btree-deletion">␟  <title>Bottom-up Index Deletion</title>␟  <title>ボトムアップインデックスの削除</title>␞␞  <para>␞
␝  <para>␟   B-Tree indexes are not directly aware that under MVCC, there might
   ...
   commits).␟B-Treeインデックスは、MVCCの下で同じ論理テーブル行の複数の現存するバージョンが存在する可能性があることを直接認識していません。
...
各インデックス中のそれぞれの新しいタプルは通常元の<quote>更新された</quote>タプルと短期間共存する必要があります（通常、<command>UPDATE</command>トランザクションのコミットした直後までです）。␞␞  </para>␞
␝  <para>␟   B-Tree indexes incrementally delete version churn index tuples by
   ...
   thresholds are exceeded (see <xref linkend="autovacuum"/>).␟B-Treeインデックスは、<firstterm>ボトムアップインデックスの削除</firstterm>パスの実行によって、バージョンチャーンのインデックスタプルを徐々に削除します。
...
これは、一定の<emphasis>定量的な</emphasis>テーブルレベルの閾値が超えられたとき（<xref linkend="autovacuum"/>参照）に起動されるautovacuumワーカーによって実行された<quote>トップダウン</quote>インデックスのクリーンアップと対照的です。␞␞  </para>␞
␝   <para>␟    Not all deletion operations that are performed within B-Tree
    ...
    as a way of avoiding the split.␟B-Treeインデックス内で実行されたすべての削除操作がボトムアップ削除操作とは限りません。
...
ボトムアップインデックス削除と同様に、単純インデックス削除は分割を回避する方法としてページ分割が予測された時点で実行されます。␞␞   </para>␞
␝   <para>␟    Simple deletion is opportunistic in the sense that it can only
    ...
    indexed columns.␟単純削除は、最近のインデックススキャンでは影響があるアイテムに<literal>LP_DEAD</literal>ビットをセットする際に、ついでに実行できる機会の場合のみに実行されるという意味で、機会主義と言えます。
<productname>PostgreSQL</productname> 14より前では、B-Treeの削除の種類は単純な削除のみでした。
単純な削除とボトムアップ削除の主な違いは、前者だけがインデックススキャンの動きによって機会を狙って駆動されることに対して、後者だけがインデックスカラムが論理的に変更されない<command>UPDATE</command>からのバージョンチャーンを具体的に対象とすることです。␞␞   </para>␞
␝  <para>␟   Bottom-up index deletion performs the vast majority of all garbage
   ...
   each of its indexes.␟ボトムアップインデックス削除は、明確なワークロードによる特定インデックスのすべてのゴミインデックスタプルの掃除の大多数を実行します。
...
そのような場合でも、<command>VACUUM</command>操作（通常、自動バキュームワーカープロセスで実行します）による、徹底的な<quote>一掃</quote>が、テーブルとその各インデックスの<emphasis>共通の</emphasis>クリーンアップの一部として最終的に要求されます。␞␞  </para>␞
␝  <para>␟   Unlike <command>VACUUM</command>, bottom-up index deletion does not
   ...
   <command>VACUUM</command> cycle).␟ <command>VACUUM</command>とは異なり、ボトムアップインデックス削除は最も古いゴミのインデックスタプルがどのくらい経過しているかについて強い保証を提供しません。
...
これにより、時間の経過と共に異なる論理行が同じテーブル<acronym>TID</acronym>を再利用することが可能です（ただし、これは存続期間が同じ<command>VACUUM</command>サイクルにまたがる、二つの論理行に同時には発生しません）。␞␞  </para>␞
␝ <sect3 id="btree-deduplication">␟  <title>Deduplication</title>␟  <title>重複排除</title>␞␞  <para>␞
␝  <para>␟   A duplicate is a leaf page tuple (a tuple that points to a table
   ...
   enabled: <firstterm>deduplication</firstterm>.␟重複とは、同じインデックスで<emphasis>全ての</emphasis>インデックスキー列が少なくとも一つの他のリーフページタプルの該当する列の値と一致する値をもっている、リーフページタプル（テーブルの行を指すタプル）です。
重複タプルは実際によくあります。
オプションの技法「<firstterm>重複排除</firstterm>」が有効にされているとき、B-Treeインデックスは、特別な重複に対する空間効率の良い表現方法を使用できます。␞␞  </para>␞
␝  <para>␟   Deduplication works by periodically merging groups of duplicate
   ...
   reduced significantly.␟重複排除は重複タプルのグループを定期的に合併して、各グループに対する単一の<firstterm>ポスティングリスト</firstterm>タプルを形成することで機能します。
...
インデックスのバキューム処理のオーバーヘッドも顕著に削減されるかもしれません。␞␞  </para>␞
␝   <para>␟    B-Tree deduplication is just as effective with
    ...
    domain of indexed values.␟B-Tree重複排除は、B-Tree演算子クラスの<literal>=</literal>項に従ってNULL値が決して互いに等しくならないとしても、NULL値を含む<quote>重複</quote>に対しても同様に効果的です。
ディスク上のB-Tree構造を理解するいかなる実装部分に関しても、NULLはインデックス値の定義域からの他の値に過ぎません。␞␞   </para>␞
␝  <para>␟   The deduplication process occurs lazily, when a new item is
   ...
   default.␟既存のリーフページに収まらない新たな要素が挿入されたとき、重複排除の処理は怠惰に実行されますが、インデックスタプルの削除は新しいアイテムのための十分なスペースを解放できなかった場合に限ります（通常、削除は簡易に検討した上で無視されます）。
...
重複排除はデフォルトで有効になっています。␞␞  </para>␞
␝  <para>␟   <command>CREATE INDEX</command> and <command>REINDEX</command>
   ...
   operations.␟<command>CREATE INDEX</command>と<command>REINDEX</command>は、使用する手順が若干異なりますが、ポスティングリストタプルを作って重複排除を適用します。
...
この戦略は<command>CREATE INDEX</command>と<command>REINDEX</command>に良く適合します。これらは1回で終わるバッチ操作であるからです。␞␞  </para>␞
␝  <para>␟   Write-heavy workloads that don't benefit from deduplication due to
   ...
   deduplication isn't usually helpful.␟インデックスの値に重複が無いか殆ど無いために重複排除から利益を得られない、書き込みの多いワークロードには、（重複排除が明示的に無効化されて居ない限り）固定のペナルティによる小さい負荷増があります。
...
通常は重複排除を無効化することは有益ではありません。␞␞  </para>␞
␝  <para>␟   It is sometimes possible for unique indexes (as well as unique
   ...
   to succeed where an earlier deletion pass failed.␟一意性インデックス（や一意性制約）が重複排除に使用できる場合があります。
...
一つの時間のかかるトランザクションが自然に消えるまでページ分割を遅らせることで、以前の削除パスが失敗した場所でボトムアップ削除パスを成功することができます。␞␞  </para>␞
␝   <para>␟    A special heuristic is applied to determine whether a
    ...
    little downside.␟一意性インデックスで重複排除パスを実行すべきかどうかの判断には、特別なヒューリスティックが適用されます。
...
一意性インデックスで重複排除を有効にしておいてもほとんどデメリットはありません。␞␞   </para>␞
␝  <para>␟   Deduplication cannot be used in all cases due to
   implementation-level restrictions.  Deduplication safety is
   determined when <command>CREATE INDEX</command> or
   <command>REINDEX</command> is run.␟実装レベルの制限により、重複排除は全ての場合に使えるわけではありません。
重複排除の安全性は<command>CREATE INDEX</command>あるいは<command>REINDEX</command>が実行されたときに決定されます。␞␞  </para>␞
␝  <para>␟   Note that deduplication is deemed unsafe and cannot be used in the
   following cases involving semantically significant differences
   among equal datums:␟等しいデータの間で意味的に明らかな違いを伴う以下の場合には、重複排除は安全でないと見做されて使用できないことに注意してください。␞␞  </para>␞
␝     <para>␟      <type>text</type>, <type>varchar</type>, and <type>char</type>
      cannot use deduplication when a
      <emphasis>nondeterministic</emphasis> collation is used.  Case
      and accent differences must be preserved among equal datums.␟<emphasis>非決定的な</emphasis>照合順序が使われているとき<type>text</type>、<type>varchar</type>、および、<type>char</type>は重複排除を使えません。
等しいデータの間で大文字小文字やアクセントの違いが維持されなければなりません。␞␞     </para>␞
␝     <para>␟      <type>numeric</type> cannot use deduplication.  Numeric display
      scale must be preserved among equal datums.␟<type>numeric</type>は重複排除を使えません。
等しいデータの間で数の表示スケールが維持されなければなりません。␞␞     </para>␞
␝     <para>␟      <type>jsonb</type> cannot use deduplication, since the
      <type>jsonb</type> B-Tree operator class uses
      <type>numeric</type> internally.␟<type>jsonb</type>のB-Tree演算子クラスは内部的に<type>numeric</type>を使っているため、<type>jsonb</type>は重複排除を使えません。␞␞     </para>␞
␝     <para>␟      <type>float4</type> and <type>float8</type> cannot use
      ...
      preserved.␟<type>float4</type>および<type>float8</type>は重複排除を使えません。
これらの型は<literal>-0</literal>と<literal>0</literal>に異なる表現を持ち、にもかかわらずこれらは等しいと見做されます。
この違いは維持されなければなりません。␞␞     </para>␞
␝  <para>␟   There is one further implementation-level restriction that may be
   lifted in a future version of
   <productname>PostgreSQL</productname>:␟さらに以下の実装レベルの制限があります。これは<productname>PostgreSQL</productname>の将来バージョンで解消されるかもしれません。␞␞  </para>␞
␝     <para>␟      Container types (such as composite types, arrays, or range
      types) cannot use deduplication.␟コンテナ型（複合型、配列型、あるいは、範囲型など）は、重複排除を使えません。␞␞     </para>␞
␝  <para>␟   There is one further implementation-level restriction that applies
   regardless of the operator class or collation used:␟さらに以下の実装レベルの制限があります。これは使われている演算子クラスや照合順序にかかわりなく該当します。␞␞  </para>␞
␝     <para>␟      <literal>INCLUDE</literal> indexes can never use deduplication.␟      <literal>INCLUDE</literal>インデックスには重複排除は使えません。␞␞     </para>␞
