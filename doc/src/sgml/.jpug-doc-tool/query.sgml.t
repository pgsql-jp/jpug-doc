␝ <chapter id="tutorial-sql">␟  <title>The <acronym>SQL</acronym> Language</title>␟  <title><acronym>SQL</acronym>言語</title>␞␞␞
␝  <sect1 id="tutorial-sql-intro">␟   <title>Introduction</title>␟   <title>はじめに</title>␞␞␞
␝   <para>␟    This chapter provides an overview of how to use
    ...
    language features are extensions to the standard.␟本章では、<acronym>SQL</acronym>を使用した簡単な操作方法について、その概要を説明します。
...
<productname>PostgreSQL</productname>の言語機能の中には標準を拡張したものがあることには注意してください。␞␞   </para>␞
␝   <para>␟    In the examples that follow, we assume that you have created a
    database named <literal>mydb</literal>, as described in the previous
    chapter, and have been able to start <application>psql</application>.␟以下で示す例では、前章で説明した<literal>mydb</literal>という名前のデータベースを作成し、<application>psql</application>を起動できるようになっていることを前提としています。␞␞   </para>␞
␝   <para>␟    Examples in this manual can also be found in the
    ...
    files, first change to that directory and run <application>make</application>:␟このマニュアルで示す例は、<productname>PostgreSQL</productname>ソース配布物に含まれており、<filename>src/tutorial/</filename>以下に展開されます。
...
␝␟    This creates the scripts and compiles the C files containing user-defined
    functions and types.  Then, to start the tutorial, do the following:␟これによりスクリプトが作成され、そして、ユーザ定義の関数と型を含むCのファイルがコンパイルされます。
...
␝␟    The <literal>\i</literal> command reads in commands from the
    ...
    <filename>basics.sql</filename>.␟<literal>\i</literal>は、指定したファイルからコマンドを読み込みます。
<command>psql</command>の<literal>-s</literal>オプションによって、シングルステップモードとなり、それぞれの文をサーバに送る前に一時停止します。
本節で使用するコマンドは<filename>basics.sql</filename>ファイル内にあります。␞␞   </para>␞
...
␝  <sect1 id="tutorial-concepts">␟   <title>Concepts</title>␟   <title>概念</title>␞␞␞
␝    <indexterm><primary>row</primary></indexterm>
␟␟    <indexterm><primary>行</primary></indexterm>␞␞   </para>␞
␝    <indexterm><primary>column</primary></indexterm>
␟␟    <indexterm><primary>列</primary></indexterm>␞␞   </para>␞
␝␟    <productname>PostgreSQL</productname> is a <firstterm>relational
    ...
    more modern development is the object-oriented database.␟<productname>PostgreSQL</productname>は<firstterm>リレーショナルデータベース管理システム</firstterm>（<acronym>RDBMS</acronym>）です。
...
より近代的な成果はオブジェクト指向データベースです。␞␞   </para>␞
␝    <indexterm><primary>database cluster</primary></indexterm>
␟␟    <indexterm><primary>データベースクラスタ</primary></indexterm>␞␞   </para>␞
␝    <indexterm><primary>cluster</primary><secondary>of databases</secondary><see>database cluster</see></indexterm>
␟␟    <indexterm><primary>クラスタ</primary><secondary>データベースの</secondary><see>データベースクラスタ</see></indexterm>␞␞   </para>␞
␝␟    Each table is a named collection of <firstterm>rows</firstterm>.
    ...
    way (although they can be explicitly sorted for display).␟各テーブルは、<firstterm>行</firstterm>の集合に名前を付けたものです。
あるテーブルの各行は、名前を付けた<firstterm>列</firstterm>の集合ということができます。
各列は特定のデータ型を持ちます。
列は行において固定の順番を持ちますが、SQLはテーブルにある行の順番をまったく保証しないことを覚えておくことは重要です
（しかし、表示用に明示的にソートさせることは可能です）。␞␞   </para>␞
␝␟    Tables are grouped into databases, and a collection of databases
    managed by a single <productname>PostgreSQL</productname> server
    instance constitutes a database <firstterm>cluster</firstterm>.␟テーブルはデータベースとしてまとめられ、1つの<productname>PostgreSQL</productname>サーバインスタンスで管理されるデータベースの集合はデータベース<firstterm>クラスタ</firstterm>を構成します。␞␞   </para>␞
␝  <sect1 id="tutorial-table">␟   <title>Creating a New Table</title>␟   <title>新しいテーブルの作成</title>␞␞␞
␝   <para>␟    You  can  create  a  new  table by specifying the table
    name, along with all column names and their types:␟テーブル名と、テーブルの全ての列の名前と型を指定することで、新しいテーブルを作成できます。␞␞␞
␝    prcp            real,          -- precipitation␟    temp_lo         int,           -- low temperature
    temp_hi         int,           -- high temperature
    prcp            real,          -- precipitation␟    temp_lo         int,           -- 最低気温
    temp_hi         int,           -- 最高気温
    prcp            real,          -- 降水量␞␞    date            date␞
␝␟    You can enter this into <command>psql</command> with the line
    breaks.  <command>psql</command> will recognize that the command
    is not terminated until the semicolon.␟上のコマンドを複数の行に分けて<command>psql</command>に入力できます。
<command>psql</command>は、セミコロンで終わるまでそのコマンドは継続するものと認識します。␞␞   </para>␞
␝   <para>␟    White space (i.e., spaces, tabs, and newlines) can be used freely
    ...
    above).␟SQLコマンドでは空白文字（つまり空白、タブ、改行）を自由に使用できます。
...
ただし、（上では行っていませんが）識別子が二重引用符で括られていた場合は大文字小文字を区別します。␞␞   </para>␞
␝   <para>␟    <type>varchar(80)</type> specifies a data type that can store
    ...
    This might be convenient or confusing &mdash; you choose.)␟<type>varchar(80)</type>は、80文字までの任意の文字列を格納できるデータ型を指定しています。
...
これは好みによります）。␞␞   </para>␞
␝   <para>␟    <productname>PostgreSQL</productname> supports the standard
    ...
    support special cases in the <acronym>SQL</acronym> standard.␟<productname>PostgreSQL</productname>は標準<acronym>SQL</acronym>のデータ型、<type>int</type>、<type>smallint</type>、<type>real</type>、<type>double precision</type>、<type>char(<replaceable>N</replaceable>)</type>、<type>varchar(<replaceable>N</replaceable>)</type>、<type>date</type>、<type>time</type>、<type>timestamp</type>や<type>interval</type>をサポートします。
また、一般的なユーティリティ用の型や高度な幾何データ型もサポートします。
任意の数のユーザ定義のデータ型を使用して、<productname>PostgreSQL</productname>をカスタマイズできます。
したがって、標準<acronym>SQL</acronym>における特殊な場合をサポートするために必要な場所を除き、型名は構文内でキーワードではありません。␞␞   </para>␞
␝   <para>␟    The second example will store cities and their associated
    geographical location:␟以下に示す2番目の例では、都市とその地理的な位置情報を格納します。␞␞<programlisting>␞
␝</programlisting>␟    The <type>point</type> type is an example of a
    <productname>PostgreSQL</productname>-specific data type.␟<type>point</type>型は、<productname>PostgreSQL</productname>独自のデータ型の一例です。␞␞   </para>␞
␝␟    Finally, it should be mentioned that if you don't need a table any
    ...
    the following command:␟最後に、テーブルが不要になった場合や別のものに作り直したい場合、以下のコマンドを使用して削除できることを示します。␞␞<synopsis>␞
␝  <sect1 id="tutorial-populate">␟   <title>Populating a Table With Rows</title>␟   <title>テーブルに行を挿入</title>␞␞␞
␝   <para>␟    The <command>INSERT</command> statement is used to populate a table  with
    rows:␟以下のように、<command>INSERT</command>文を使用して、テーブルに行を挿入します。␞␞␞
␝␟    Note that all data types use rather obvious input formats.
    ...
    format shown here.␟全てのデータ型でどちらかといえばわかりやすい入力書式を使用していることに注意してください。
通常、単純な数値以外の定数は、この例のように、単一引用符（<literal>'</literal>）で括らなければなりません。
<type>date</type>型で受け付けられるものは実際はかなり柔軟です。
しかし、このチュートリアルの段階では、曖昧さがない書式にこだわることにします。␞␞   </para>␞
␝   <para>␟    The <type>point</type> type requires a coordinate pair as input,
    as shown here:␟<type>point</type>型では、入力として次のような座標の組み合わせが必要です。␞␞<programlisting>␞
␝   <para>␟    The syntax used so far requires you to remember the order of the
    columns.  An alternative syntax allows you to list the columns
    explicitly:␟ここまでの構文では、列の順番を覚えておく必要がありました。
以下に示す他の構文では、列のリストを明示的に与えることができます。␞␞<programlisting>␞
␝</programlisting>␟    You can list the columns in a different order if you wish or
    even omit some columns, e.g., if the precipitation is unknown:␟リスト内の列は好きな順番で指定できます。
また、一部の列を省略することもできます。
例えば、降水量がわからない場合は以下のようにできます。␞␞<programlisting>␞
␝</programlisting>␟    Many developers consider explicitly listing the columns better
    style than relying on the order implicitly.␟多くの開発者は、暗黙的な順番に依存するよりも、列のリストを明示的に指定する方が良いやり方だと考えています。␞␞   </para>␞
␝   <para>␟    Please enter all the commands shown above so you have some data to
    work with in the following sections.␟以降の節でもデータを使用しますので、上のコマンドを全て入力してください。␞␞   </para>␞
␝␟    You could also have used <command>COPY</command> to load large
    ...
    <command>INSERT</command>.  An example would be:␟また、<command>COPY</command>を使用して大量のデータを平文テキストファイルからロードすることもできます。
...
␝␟    where the file name for the source file must be available on the
    ...
    tab character):␟ここで元となるファイルを表すファイル名は、クライアントではなく、バックエンドプロセスを動かしているマシンで利用できるものでなければなりません。
...
␝␟    You can read more about the <command>COPY</command> command in
    <xref linkend="sql-copy"/>.␟<xref linkend="sql-copy"/>には<command>COPY</command>コマンドについてのより詳しい説明があります。␞␞   </para>␞
␝    <indexterm><primary>query</primary></indexterm>
␟␟    <indexterm><primary>問い合わせ</primary></indexterm>␞␞␞
␝  <sect1 id="tutorial-select">␟   <title>Querying a Table</title>␟   <title>テーブルへの問い合わせ</title>␞␞␞
␝␟    To retrieve data from a table, the table is
    ...
    <structname>weather</structname>, type:␟テーブルからデータを取り出すために、テーブルへ<firstterm>問い合わせ</firstterm>をします。
...
例えば、<structname>weather</structname>の全ての行を取り出すには、以下を入力します。␞␞<programlisting>␞
␝</programlisting>␟    Here <literal>*</literal> is a shorthand for <quote>all columns</quote>.␟ここで<literal>*</literal>は<quote>全ての列</quote>の省略形です。␞␞     <footnote>␞
␝      <para>␟       While <literal>SELECT *</literal> is useful for off-the-cuff
       queries, it is widely considered bad style in production code,
       since adding a column to the table would change the results.␟<literal>SELECT *</literal>は即興的な問い合わせで有用ですが、テーブルに列を追加することにより結果が変わってしまいますので、実用システムのコードでは悪いやり方であると一般的には考えられています。␞␞      </para>␞
␝     </footnote>␟    So the same result would be had with:␟したがって、以下のようにしても同じ結果になります。␞␞<programlisting>␞
␝␟    The output should be:␟出力は、以下のようになります。␞␞␞
␝   <para>␟    You can write expressions, not just simple column references, in the
    select list.  For example, you can do:␟選択リストには、単なる列参照だけではなく式を指定することもできます。
例えば、以下を行うことができます。␞␞<programlisting>␞
␝</programlisting>␟    This should give:␟この結果は次のようになります。␞␞<screen>␞
␝</screen>␟    Notice how the <literal>AS</literal> clause is used to relabel the
    output column.  (The <literal>AS</literal> clause is optional.)␟<literal>AS</literal>句を使用した出力列の再ラベル付けの部分に注意してください。
（<literal>AS</literal>句は省略できます。）␞␞   </para>␞
␝   <para>␟    A query can be <quote>qualified</quote> by adding a <literal>WHERE</literal>
    ...
    retrieves the weather of San Francisco on rainy days:␟必要な行が何かを指定する<literal>WHERE</literal>句を追加して問い合わせに<quote>条件付け</quote>できます。
...
␝</programlisting>␟    Result:␟結果は次のようになります。␞␞<screen>␞
␝␟    You can request that the results of a query
    be returned in sorted order:␟問い合わせの結果をソートして返すように指定できます。␞␞␞
...
␝␟    In this example, the sort order isn't fully specified, and so you
    ...
    get the results shown above if you do:␟この例では、ソート順は十分に指定されていません。
...
␝␟    You can request that duplicate rows be removed from the result of
    a query:␟問い合わせの結果から重複行を除くように指定できます。␞␞␞
␝␟    Here again, the result row ordering might vary.
    You can ensure consistent results by using <literal>DISTINCT</literal> and
    <literal>ORDER BY</literal> together:␟ここでも、結果行の順序は変動するかもしれません。
<literal>DISTINCT</literal>と<literal>ORDER BY</literal>を一緒に使用することで確実に一貫した結果を得ることができます。␞␞     <footnote>␞
␝      <para>␟       In some database systems, including older versions of
       ...
       <literal>DISTINCT</literal> causes the rows to be ordered.␟<productname>PostgreSQL</productname>の古めのバージョンを含む一部のデータベースシステムでは、<literal>DISTINCT</literal>の実装に行の自動順序付けが含まれており、<literal>ORDER BY</literal>は不要です。
しかし、これは標準SQLにおける要求ではなく、現在の<productname>PostgreSQL</productname>では<literal>DISTINCT</literal>句が行の順序付けを行うことを保証していません。␞␞      </para>␞
...
␝  <sect1 id="tutorial-join">␟   <title>Joins Between Tables</title>␟   <title>テーブル間を結合</title>␞␞␞
␝   <para>␟    Thus far, our queries have only accessed one table at a time.
    ...
    table, and select the pairs of rows where these values match.<footnote>␟ここまでの問い合わせは、一度に1つのテーブルにのみアクセスするものでした。
...
例えば、すべての気象データを関連する都市の位置情報と一緒にすべて返すためには、データベースは<structname>weather</structname>テーブルの各行の<structfield>city</structfield>列を、<structname>cities</structname>テーブルの全ての行の<structfield>name</structfield>列と比較することが必要です。
    <footnote>␞␞     <para>␞
␝     <para>␟      This  is only a conceptual model.  The join is usually performed
      in a more efficient manner than actually comparing each possible
      pair of rows, but this is invisible to the user.␟これは概念的なモデルでしかありません。
実際の結合は通常、1つひとつの行の組み合わせを比べるよりも、もっと効率的な方法で行われます。
しかし、これはユーザからはわかりません。␞␞     </para>␞
␝    </footnote>␟    This would be accomplished by the following query:␟これは、以下の問い合わせによって行うことができます。␞␞␞
␝   <para>␟    Observe two things about the result set:␟この結果について2つのことに注目してください。␞␞    <itemizedlist>␞
␝      <para>␟       There is no result row for the city of Hayward.  This is
       ...
       shortly how this can be fixed.␟Hayward市についての結果行がありません。
これは<structname>cities</structname>テーブルにはHaywardに一致する項目がないからで、結合の際に<structname>weather</structname>テーブル内の一致されなかった行は無視されるのです。
これをどうしたら解決できるかは、しばらく後で説明します。␞␞      </para>␞
␝      <para>␟       There are two columns containing the city name.  This is
       ...
       <literal>*</literal>:␟都市名を持つ2つの列があります。
<structname>weather</structname>テーブルと<structname>cities</structname>テーブルからの列のリストが連結されるため、これは正しい動作です。
しかし実際には、これは望ましい結果ではないため、<literal>*</literal>を使わずに、明示的に出力列のリストを指定することになるでしょう。␞␞<programlisting>␞
␝   <para>␟    Since the columns all had different names, the parser
    ...
    meant, as in:␟列がすべて異なる名前だったので、パーサは自動的にどのテーブルの列かを見つけることができました。
...
␝␟    It is widely considered good style to qualify all column names
    ...
    column name is later added to one of the tables.␟結合問い合わせではすべての列名を修飾するのが良いやり方であると一般に考えられています。
そうすれば、テーブルのいずれかに後で重複する名前を持つ列が追加されても、問い合わせが失敗しません。␞␞   </para>␞
␝   <para>␟    Join queries of the kind seen thus far can also be written in this
    form:␟ここまでに示したような結合問い合わせは、以下のような形で表すことができます。␞␞␞
␝   <indexterm><primary>join</primary><secondary>outer</secondary></indexterm>
␟␟   <indexterm><primary>結合</primary><secondary>外部</secondary></indexterm>␞␞   </para>␞
␝␟    This syntax pre-dates the <literal>JOIN</literal>/<literal>ON</literal>
    ...
    clause together with other conditions.␟この構文は<literal>JOIN</literal>/<literal>ON</literal>より以前のもので、SQL-92で導入されました。
...
結合条件はそれ独自のキーワードにより導入されるのに対して、以前は条件は他の条件と一緒に<literal>WHERE</literal>句の中に混ざっていました。␞␞   </para>␞
␝   <para>␟    Now we will figure out how we can get the Hayward records back in.
    ...
    The command looks like this:␟ここで、どのようにすればHaywardのレコードを得ることができるようになるのかを明らかにします。
...
␝␟    This query is called a <firstterm>left outer
    ...
    values are substituted for the right-table columns.␟この問い合わせは<firstterm>左外部結合</firstterm>と呼ばれます。
結合演算子の左側に指定したテーブルの各行が最低でも一度出力され、一方で、右側のテーブルでは左側のテーブルの行に一致するもののみが出力されるからです。
右側のテーブルに一致するものがない、左側のテーブルの行を出力する時、右側のテーブルの列は空の値（NULL）で置換されます。␞␞   </para>␞
␝   <formalpara>␟    <title>Exercise:</title>␟    <title>練習:</title>␞␞␞
␝   <indexterm><primary>join</primary><secondary>self</secondary></indexterm>
␟␟   <indexterm><primary>結合</primary><secondary>自己</secondary></indexterm>␞␞    </para>␞
␝   <indexterm><primary>alias</primary><secondary>for table name in query</secondary></indexterm>
␟␟   <indexterm><primary>別名</primary><secondary>問い合わせ内のテーブル名用の</secondary></indexterm>␞␞    </para>␞
␝    <para>␟     There are also right outer joins and full outer joins.  Try to
     find out what those do.␟右外部結合や完全外部結合も存在します。
これらが何を行うかを考えてください。␞␞    </para>␞
␝   <para>␟    We can also join a table against itself.  This is called a
    ...
    following query:␟テーブルを自分自身に対して結合させることができます。
...
␝␟    Here we have relabeled the weather table as <literal>w1</literal> and
    ...
    queries to save some typing, e.g.:␟ここで、結合の左側と右側を区別できるように、weatherテーブルに<literal>w1</literal>と<literal>w2</literal>というラベルを付けています。
また、入力量を省くために、他の問い合わせでもこの種の別名を使用できます。
以下に例を示します。␞␞<programlisting>␞
␝</programlisting>␟    You will encounter this style of abbreviating quite frequently.␟こういった形の省略はかなりよく行われます。␞␞   </para>␞
...
␝  <sect1 id="tutorial-agg">␟   <title>Aggregate Functions</title>␟   <title>集約関数</title>␞␞␞
␝   <para>␟    Like  most  other relational database products,
    ...
    <function>min</function> (minimum) over a set of rows.␟他のほとんどのリレーショナルデータベース製品同様、<productname>PostgreSQL</productname>は<firstterm>集約関数</firstterm>をサポートします。
集約関数は複数の入力行から1つの結果を計算します。
例えば、行の集合に対して、<function>count</function>（総数）、<function>sum</function>（総和）、<function>avg</function>（平均）、<function>max</function>（最大）、<function>min</function>（最小）といった演算を行う集約があります。␞␞   </para>␞
␝    <indexterm><primary>subquery</primary></indexterm>
␟␟    <indexterm><primary>副問い合わせ</primary></indexterm>␞␞␞
␝   <para>␟    As an example, we can find the highest low-temperature reading anywhere
    with:␟例として、次のように全ての都市の最低気温から最も高い気温を求めることができます。␞␞␞
␝␟    If we wanted to know what city (or cities) that reading occurred in,
    we might try:␟どの都市のデータなのかを知りたいとしたら、下記のような問い合わせを試行するかもしれません。␞␞␞
␝<programlisting>␟SELECT city FROM weather WHERE temp_lo = max(temp_lo);     <lineannotation>WRONG</lineannotation>␟SELECT city FROM weather WHERE temp_lo = max(temp_lo);     <lineannotation>間違い</lineannotation>␞␞</programlisting>␞
␝␟    but this will not work since the aggregate
    ...
    by using a <firstterm>subquery</firstterm>:␟しかし、<function>max</function>集約を<literal>WHERE</literal>句で使用することができませんので、このコマンドは動作しません。
...
␝␟    This is OK because the subquery is an independent computation
    ...
    in the outer query.␟副問い合わせは、外側の問い合わせで起こることとは別々に集約を計算する独立した演算ですので、この問い合わせは問題ありません。␞␞   </para>␞
␝␟    Aggregates are also very useful in combination with <literal>GROUP
    ...
    and the maximum low temperature observed in each city with:␟また、<literal>GROUP BY</literal>句と組み合わせた集約も非常に役に立ちます。
例えば、以下のコマンドで都市ごとにデータの数と最低気温の最大値を求めることができます。␞␞␞
␝␟    which gives us one output row per city.  Each aggregate result is
    computed over the table rows matching that city.
    We can filter these grouped
    rows using <literal>HAVING</literal>:␟ここには都市ごとに1行の出力があります。
...
␝␟    which gives us the same results for only the cities that have all
    ...
    cities whose
    names begin with <quote><literal>S</literal></quote>, we might do:␟このコマンドは上と同じ計算を行うものですが、全ての<structfield>temp_lo</structfield>の値が40未満の都市のみを出力します。
最後になりますが、<quote><literal>S</literal></quote>から始まる名前の都市のみを対象にしたい場合は、以下を行います。␞␞␞
␝     <para>␟      The <literal>LIKE</literal> operator does pattern matching and
      is explained in <xref linkend="functions-matching"/>.␟<literal>LIKE</literal>演算子はパターンマッチを行います。これについては<xref linkend="functions-matching"/>で説明します。␞␞     </para>␞
␝   <para>␟    It is important to understand the interaction between aggregates and
    ...
    stage.)␟集約と<acronym>SQL</acronym>の<literal>WHERE</literal>と<literal>HAVING</literal>句の間の相互作用を理解することが重要です。
...
同じ条件は<literal>WHERE</literal>の段階でもっと効率良く使用できます）。␞␞   </para>␞
␝   <para>␟    In the previous example, we can apply the city name restriction in
    ...
    for all rows that fail the <literal>WHERE</literal> check.␟前の例では<literal>WHERE</literal>内に都市名の制限を適用できます。
集約を行う必要がないからです。
これは<literal>HAVING</literal>に制限を追加するよりも効率的です。
なぜなら<literal>WHERE</literal>の検査で失敗する全ての行についてグループ化や集約演算が行われないからです。␞␞   </para>␞
␝   <para>␟    Another way to select the rows that go into an aggregate
    computation is to use <literal>FILTER</literal>, which is a
    per-aggregate option:␟集約計算に使用する行を選択するもう1つの方法は、集約ごとのオプションである<literal>FILTER</literal>を使用することです。␞␞␞
␝␟    <literal>FILTER</literal> is much like <literal>WHERE</literal>,
    ...
    so it still finds the reading of 46.␟<literal>FILTER</literal>は<literal>WHERE</literal>によく似ていますが、結び付けられている特定の集約関数の入力からのみ行を削除する点が異なります。
ここでは、<literal>count</literal>集約は<literal>temp_lo</literal>が45未満の行のみを数えますが、<literal>max</literal>集約はすべての行に適用されるため、読み取り値46が検出されます。␞␞   </para>␞
␝  <sect1 id="tutorial-update">␟   <title>Updates</title>␟   <title>更新</title>␞␞␞
␝   <para>␟    You can update existing rows using the
    <command>UPDATE</command> command.
    Suppose you discover the temperature readings are
    all off by 2 degrees after November 28.  You can correct the
    data as follows:␟<command>UPDATE</command>コマンドを使用して、既存の行を更新できます。
11月28日以降の全ての気温の読み取りが2度高くなっていることがわかったとします。
その場合、以下のコマンドによって、データを修正できます。␞␞␞
␝   <para>␟    Look at the new state of the data:␟データの更新後の状態を確認します。␞␞<programlisting>␞
␝  <sect1 id="tutorial-delete">␟   <title>Deletions</title>␟   <title>削除</title>␞␞␞
␝   <para>␟    Rows can be removed from a table using the <command>DELETE</command>
    command.
    Suppose you are no longer interested in the weather of Hayward.
    Then you can do the following to delete those rows from the table:␟<command>DELETE</command>コマンドを使用してテーブルから行を削除できます。
Haywardの気象を対象としなくなったとします。
その場合、以下のコマンドを使用して、テーブルから行を削除できます。␞␞<programlisting>␞
␝␟    All weather records belonging to Hayward are removed.␟Haywardに関する気象データは全て削除されました。␞␞␞
␝   <para>␟    One should be wary of statements of the form␟以下の形式の文には注意しなければなりません。␞␞<synopsis>␞
␝␟    Without a qualification, <command>DELETE</command> will
    ...
    doing this!␟条件がない場合、<command>DELETE</command>は指定したテーブルの<emphasis>全ての</emphasis>行を削除し、テーブルを空にします。
システムは削除前に確認を求めるようなことは行いません！␞␞   </para>␞
