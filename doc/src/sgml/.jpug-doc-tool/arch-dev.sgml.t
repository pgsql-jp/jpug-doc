␝ <chapter id="overview">␟  <title>Overview of PostgreSQL Internals</title>␟  <title>PostgreSQL内部の概要</title>␞␞␞
␝  <note>␟   <title>Author</title>␟   <title>作者</title>␞␞   <para>␞
␝   <para>␟    This chapter originated as part of
    <xref linkend="sim98"/> Stefan Simkovics'
    Master's Thesis prepared at Vienna University of Technology under the direction
    of O.Univ.Prof.Dr. Georg Gottlob and Univ.Ass. Mag. Katrin Seyr.␟本章は<xref linkend="sim98"/>として、 ウィーン工科大学にてO.Univ.Prof.Dr. Georg GottlobとUniv.Ass.Mag. Katrin Seyr.の指導の下にStefan Simkovicsが書いた修士論文の一部が基になっています。␞␞   </para>␞
␝  <para>␟   This chapter gives an overview of the internal structure of the
   ...
   at which the results are returned to the client.␟本章では<productname>PostgreSQL</productname>のバックエンドの内部構造の概要を説明します。
次からの節を読んだ後には、問い合わせがどのように処理されるかの概念がつかめているはずです。
バックエンドが問い合わせを受け取った時点からクライアントに結果を返す時点の間に引き起こる操作の一般的な流れを理解してもらうのが、この章の目的です。␞␞  </para>␞
␝  <sect1 id="query-path">␟   <title>The Path of a Query</title>␟   <title>問い合わせの経路</title>␞␞␞
␝   <para>␟    Here we give a short overview of the stages a query has to pass
    to obtain a result.␟ここでは、問い合わせが結果を得るためにたどる過程を簡単に説明します。␞␞   </para>␞
␝     <para>␟      A connection from an application program to the <productname>PostgreSQL</productname>
      ...
      server.␟アプリケーションプログラムから<productname>PostgreSQL</productname>サーバに接続が確立されなくてはなりません。
アプリケーションプログラムはサーバに問い合わせを送り、そしてサーバから送り返される結果を待ちます。␞␞     </para>␞
␝     <para>␟      The <firstterm>parser stage</firstterm> checks the query
      transmitted by the application
      program for correct syntax and creates
      a <firstterm>query tree</firstterm>.␟<firstterm>構文解析過程</firstterm>で、アプリケーションプログラムから送られた問い合わせの構文が正しいかをチェックし、<firstterm>問い合わせツリー</firstterm>を作成します。␞␞     </para>␞
␝     <para>␟      The <firstterm>rewrite system</firstterm> takes
      ...
      transformations given in the <firstterm>rule bodies</firstterm>.␟<firstterm>書き換えシステム</firstterm>は構文解析過程で作られた問い合わせツリーを受け取り、問い合わせツリーに適用するための（<firstterm>システムカタログ</firstterm>に格納されている）<firstterm>ルール</firstterm>を探します。
そして<firstterm>ルール本体</firstterm>で与えられた変換を実行します。␞␞     </para>␞
␝     <para>␟      One application of the rewrite system is in the realization of
      ...
      the <firstterm>view definition</firstterm> instead.␟書き換えシステムの適用例の1つとして<firstterm>ビュー</firstterm>の具現化が挙げられます。
ビュー（すなわち<firstterm>仮想テーブル</firstterm>）に対して問い合わせがあると、書き換えシステムが代わってユーザの問い合わせを、<firstterm>ビュー定義</firstterm>で与えられた<firstterm>実テーブル</firstterm>にアクセスする問い合わせに書き換えます。␞␞     </para>␞
␝     <para>␟      The <firstterm>planner/optimizer</firstterm> takes
      the (rewritten) query tree and creates a
      <firstterm>query plan</firstterm> that will be the input to the
      <firstterm>executor</firstterm>.␟<firstterm>プランナ/オプティマイザ</firstterm>は、（書き換えられた）問い合わせツリーを見て、<firstterm>エグゼキュータ</firstterm>に渡すための<firstterm>問い合わせ計画</firstterm>を作ります。␞␞     </para>␞
␝     <para>␟      It does so by first creating all possible <firstterm>paths</firstterm>
      ...
      path is expanded into a complete plan that the executor can use.␟そのためにまず同じ結果をもたらす全ての可能な限りの<firstterm>経路</firstterm>を作ります。
...
一番コストの小さな経路は、エグゼキュータが実行できるように完全な計画に拡張されます。␞␞     </para>␞
␝     <para>␟      The executor recursively steps through
      ...
      evaluates <firstterm>qualifications</firstterm> and finally hands back the rows derived.␟エグゼキュータは再帰的に<firstterm>計画ツリー</firstterm>上を進み、計画で示されている方法で行を抽出します。
エグゼキュータはリレーションをスキャンする間<firstterm>保存システム</firstterm>を利用して<firstterm>ソート</firstterm>と<firstterm>結合</firstterm>を実行し、<firstterm>検索条件</firstterm>の評価を行い、最後に得られた行を返します。␞␞     </para>␞
␝   <para>␟    In the following sections we will cover each of the above listed items
    in more detail to give a better understanding of <productname>PostgreSQL</productname>'s internal
    control and data structures.␟これからの節では、<productname>PostgreSQL</productname>の内部制御とデータ構造をより良く理解するために、上に記載した事柄をさらに詳しく説明します。␞␞   </para>␞
␝  <sect1 id="connect-estab">␟   <title>How Connections Are Established</title>␟   <title>接続の確立</title>␞␞␞
␝   <para>␟    <productname>PostgreSQL</productname> implements a
    ...
    to ensure data integrity throughout concurrent data access.␟<productname>PostgreSQL</productname>は<quote>1プロセスに1ユーザ</quote>のクライアント/サーバモデルを実装しています。
...
このバックエンドプロセスは<firstterm>セマフォ</firstterm>と<glossterm linkend="glossary-shared-memory">共有メモリ</glossterm>を活用してお互いに連絡を取り合い、<glossterm linkend="glossary-instance">instance</glossterm>の他のプロセスと通信し、同時にデータにアクセスしても整合性が保たれるようにします。␞␞   </para>␞
␝   <para>␟    The client process can be any program that understands the
    ...
    <application>JDBC</application> driver.␟クライアントプロセスは<xref linkend="protocol"/>に記載された<productname>PostgreSQL</productname>プロトコルを理解できるどんなプログラムでも構いません。
多くのクライアントは<application>libpq</application> C言語ライブラリに基づいていますが、Java <application>JDBC</application>ドライバのようにいくつかの独立したプロトコル実装も存在します。␞␞   </para>␞
␝   <para>␟    Once a connection is established, the client process can send a query
    ...
    by transmitting them over the established connection.␟いったん接続が確立されると、クライアントプロセスは接続されたバックエンドプロセスに問い合わせを送ることができます。
問い合わせは平文で送信されます。
つまり、クライアントでは構文解析を行いません。
バックエンドプロセスは問い合わせの構文解析を行い、<firstterm>実行計画</firstterm>を作り、そして計画を実行し、抽出した行を確立された接続を通じてクライアントに返します。␞␞   </para>␞
␝  <sect1 id="parser-stage">␟   <title>The Parser Stage</title>␟   <title>構文解析過程</title>␞␞␞
␝   <para>␟    The <firstterm>parser stage</firstterm> consists of two parts:␟<firstterm>構文解析過程</firstterm>は2つの部分から構成されています。␞␞␞
␝      <para>␟       The <firstterm>parser</firstterm> defined in
       <filename>gram.y</filename> and <filename>scan.l</filename> is
       built using the Unix tools <application>bison</application>
       and <application>flex</application>.␟<filename>gram.y</filename>と<filename>scan.l</filename>で定義されている<firstterm>パーサ</firstterm>は、Unixのツール<application>bison</application>と<application>flex</application>を使って構築されます。␞␞      </para>␞
␝      <para>␟       The <firstterm>transformation process</firstterm> does
       modifications and augmentations to the data structures returned by the parser.␟<firstterm>変換プロセス</firstterm>は、パーサから返されたデータ構造の変更や追加を行います。␞␞      </para>␞
␝   <sect2 id="parser-stage-parser">␟    <title>Parser</title>␟    <title>パーサ</title>␞␞␞
␝    <para>␟     The parser has to check the query string (which arrives as plain
     ...
     and <application>flex</application>.␟パーサは、（平文のテキストとして渡される）問い合わせ文字列が正しい構文になっているかチェックしなければいけません。
もし構文が正しい場合は<firstterm>構文解析ツリー</firstterm>が作られて返されます。
正しくない場合はエラーが返されます。
パーサと字句解析はUnixでよく知られたツールの<application>bison</application>と<application>flex</application>を使用して実装されています。␞␞    </para>␞
␝    <para>␟     The <firstterm>lexer</firstterm> is defined in the file
     ...
     is generated and handed to the parser.␟<firstterm>字句解析</firstterm>はファイル<filename>scan.l</filename>で定義され、<firstterm>識別子</firstterm>や<firstterm>SQLキーワード</firstterm>などの確認を担当します。
検出された全てのキーワードや識別子に対し<firstterm>トークン</firstterm>が生成されパーサに渡されます。␞␞    </para>␞
␝    <para>␟     The parser is defined in the file <filename>gram.y</filename> and
     ...
     used to build up the parse tree.␟パーサはファイル<filename>gram.y</filename>の中で定義され、<firstterm>文法ルール</firstterm>とルールが実行された時に実行される<firstterm>アクション</firstterm>の組から構成されています。
アクションのコード（実際はC言語コードです）は構文解析ツリーを作るのに使われます。␞␞    </para>␞
␝    <para>␟     The file <filename>scan.l</filename> is transformed to the C
     ...
     or <application>bison</application> is called.␟ファイル<filename>scan.l</filename>はプログラム<application>flex</application>を使ってCのソースファイル<filename>scan.c</filename>に変換されます。
...
␝      <para>␟       The mentioned transformations and compilations are normally done
       automatically using the <firstterm>makefiles</firstterm>
       shipped with the <productname>PostgreSQL</productname>
       source distribution.␟ここで言及した書き換えやコンパイルは通常<productname>PostgreSQL</productname>のソースと一緒に配布される<firstterm>makefile</firstterm>を使って自動的に行われます。␞␞      </para>␞
␝    <para>␟     A detailed description of <application>bison</application> or
     ...
     understand what happens there.␟<application>bison</application>または<filename>gram.y</filename>で定義される文法ルールの詳細は本稿では説明しきれません。
...
その知識なしではそこで何が起こっているのかを理解することは難しいでしょう。␞␞    </para>␞
␝   <sect2 id="parser-stage-transformation-process">␟     <title>Transformation Process</title>␟     <title>書き換えプロセス</title>␞␞␞
␝    <para>␟     The parser stage creates a parse tree using only fixed rules about
     ...
     information is called the <firstterm>query tree</firstterm>.␟構文解析過程ではSQLの構文構造に関する固定ルールのみを使って構文解析ツリーを作成します。
...
この情報を表すために作成されるデータ構造を<firstterm>問い合わせツリー</firstterm>と呼びます。␞␞    </para>␞
␝    <para>␟     The reason for separating raw parsing from semantic analysis is that
     ...
     transformation process be invoked.␟語義解釈と入力の構文解釈を切り分ける理由は、システムカタログの参照はトランザクション内でのみ行うことができますが、問い合わせ文字列を受け取ってすぐにトランザクションを開始することは好ましくないと考えられるからです。
...
これ以降に限り書き換えプロセスを起動することができます。␞␞    </para>␞
␝    <para>␟     The query tree created by the transformation process is structurally
     ...
     results is added to the query tree.␟書き換えプロセスで作成された問い合わせツリーはほとんどの箇所で加工されていない構文解析ツリーに構造的には似ていますが、細部では数多くの相違が存在します。
...
さらに、列の実際のデータ型と式の結果についての情報が問い合わせツリーに書き加えられます。␞␞    </para>␞
␝  <sect1 id="rule-system">␟   <title>The <productname>PostgreSQL</productname> Rule System</title>␟   <title><productname>PostgreSQL</productname>ルールシステム</title>␞␞␞
␝   <para>␟    <productname>PostgreSQL</productname> supports a powerful
    ...
    rule system consisted of two implementations:␟<productname>PostgreSQL</productname>には、<firstterm>ビュー</firstterm>と理解の仕方でどうとも取れる<firstterm>ビューの更新</firstterm>の仕様に対応する強力な<firstterm>ルールシステム</firstterm>があります。
元々<productname>PostgreSQL</productname>のルールシステムは2つの実装で構成されていました。␞␞␞
␝      <para>␟       The first one worked using <firstterm>row level</firstterm> processing and was
       ...
       transformed into <productname>Postgres95</productname>.␟初めの1つは<firstterm>行レベル</firstterm>の処理を使って動き、<firstterm>エグゼキュータ</firstterm>の内部に深く実装されていました。
個別の行がアクセスされる度にルールシステムが呼ばれていました。
この実装は1995年、最後の<productname>Berkeley Postgres</productname>プロジェクトの公式リリースが<productname>Postgres95</productname>へ移行する時に削除されました。␞␞      </para>␞
␝      <para>␟       The second implementation of the rule system is a technique
       ...
       <firstterm>planner/optimizer</firstterm>. This technique is still implemented.␟ルールシステムの2つ目の実装は<firstterm>問い合わせ書き換え</firstterm>と呼ばれる手法です。
<firstterm>書き換えシステム</firstterm>は<firstterm>構文解析過程</firstterm>と<firstterm>プランナ/オプティマイザ</firstterm>の間にあるモジュールです。
この手法は現在でも実装されています。␞␞      </para>␞
␝   <para>␟    The query rewriter is discussed in some detail in
    ...
    can be thought of as a form of macro expansion.␟問い合わせの書き換えについては<xref linkend="rules"/>にて詳しく説明されますので、ここでは取り扱いません。
書き換えの入出力はともに問い合わせツリーである、つまり、ツリー内の表現の仕方や語義をどの程度詳しく判断するかには変更はない、ということを指摘するのに留めます。
書き換えはマクロの拡張と捉えることもできます。␞␞   </para>␞
␝  <sect1 id="planner-optimizer">␟   <title>Planner/Optimizer</title>␟   <title>プランナ/オプティマイザ</title>␞␞␞
␝   <para>␟    The task of the <firstterm>planner/optimizer</firstterm> is to
    ...
    selecting the execution plan that is expected to run the fastest.␟<firstterm>プランナ/オプティマイザ</firstterm>の役割は最適な実行計画を作ることです。
ある与えられたSQL問い合わせは（それがある問い合わせツリーになるのですが）、同じ結果をもたらす、多くの異なった方法で実際には実行できます。
もしもコンピュータの演算として可能であれば、問い合わせオプティマイザは可能な実行計画をすべて検証し、実行するとした場合に一番早く結果をもたらすと想定される実行計画を選択します。␞␞   </para>␞
␝    <para>␟     In some situations, examining each possible way in which a query
     ...
     exceeds a threshold (see <xref linkend="guc-geqo-threshold"/>).␟場合によっては、問い合わせがどう実行されるか、可能性のある全ての手段を検証するため、膨大な時間とメモリを消費する可能性があります。
特に数多くの結合操作に問い合わせが関わった時です。
相応な（必ずしも最適ではありませんが）問い合わせ計画を、相応な時間内で決定するため<productname>PostgreSQL</productname>は結合の数が閾値を越えた場合、<firstterm>遺伝的問い合わせオプティマイザ</firstterm>（<xref linkend="geqo"/>参照）を使用します（<xref linkend="guc-geqo-threshold"/>を参照ください）。␞␞    </para>␞
␝   <para>␟    The planner's search procedure actually works with data structures
    ...
    and plans.␟このプランナの検索手順は、実際には<firstterm>経路</firstterm>という名前のデータ構造を使用します。
...
本節の残りでは、経路と計画の違いについて無視します。␞␞   </para>␞
␝   <sect2 id="planner-optimizer-generating-possible-plans">␟    <title>Generating Possible Plans</title>␟    <title>実行可能な計画の生成</title>␞␞␞
␝    <para>␟     The planner/optimizer starts by generating plans for scanning each
     ...
     might be useful for merge joining (see below).␟プランナ/オプティマイザは、問い合わせの中で使用される個々のリレーション（テーブル）をスキャンするための計画を生成することから始めます。
...
さらに他のインデックスが存在し、問い合わせの中で条件がインデックスのキーに一致した場合、なおその上に計画が検討されます。インデックススキャン計画は、問い合わせの （もし存在すれば）<literal>ORDER BY</literal>句に一致するソート順、もしくはマージ結合に便利なソート順を所有するインデックスに対して生成されます（以下を参照してください）。␞␞    </para>␞
␝    <para>␟     If the query requires joining two or more relations,
     plans for joining relations are considered
     after all feasible plans have been found for scanning single relations.
     The three available join strategies are:␟問い合わせが２つ以上のリレーションの結合を必要とすると、リレーションを結合する計画は、単一のリレーションをスキャンするために全ての実行可能な計画が探し出された後に検討されます。３つの実行可能な結合戦略を示します。␞␞␞
␝       <para>␟        <firstterm>nested loop join</firstterm>: The right relation is scanned
        ...
        row of the left relation as keys for the index scan of the right.)␟<firstterm>ネステッドループ結合</firstterm>：
左側のリレーションの中で見つけられた行ごとに右側のリレーションが1回スキャンされます。
この戦略は実装が簡単ですが、時間がかかる場合があります。
（とは言っても右側のリレーションがインデックススキャンによってスキャン可能であればよい戦略になります。
右側のインデックススキャンのキーとして左側のリレーションの現在の行の値を使用することができます。）␞␞       </para>␞
␝       <para>␟        <firstterm>merge join</firstterm>: Each relation is sorted on the join
        ...
        index on the join key.␟<firstterm>マージ結合</firstterm>：
結合を開始する前に、それぞれのリレーションを結合属性でソートします。
そして、2つのリレーションを並行してスキャンし、一致する行を結合行の形にまとめます。
それぞれのリレーションがたった1回しかスキャンされなくて済むのでこの結合は魅力的です。
要求されるソートは、明示的なソート段階、または、結合キー上のインデックスを使用して適切な順序でリレーションをスキャンすることにより行われます。␞␞       </para>␞
␝       <para>␟        <firstterm>hash join</firstterm>: the right relation is first scanned
        ...
        locate the matching rows in the table.␟<firstterm>ハッシュ結合</firstterm>：
右側のリレーションがハッシュキーとして結合属性を用いて初めにスキャンされ、ハッシュテーブルに読み込まれます。
次に左側のリレーションがスキャンされ、見つかったそれぞれの行に相応しい値が、右側のリレーションの行を探し出すためのハッシュキーとして使われます。␞␞       </para>␞
␝    <para>␟     When the query involves more than two relations, the final result
     ...
     cheapest one.␟問い合わせが3つ以上のリレーションを含む場合、それぞれ2つの入力を持つ結合段階のツリーによって最終結果を構築しなければなりません。
プランナは最も低コストな計画を見つけ出すために、あり得る異なった結合順序を検証します。␞␞    </para>␞
␝    <para>␟     If the query uses fewer than <xref linkend="guc-geqo-threshold"/>
     ...
     (estimated to be) the cheapest is chosen.␟問い合わせが<xref linkend="guc-geqo-threshold"/>より少ないリレーションを使用する場合、最適な結合シーケンスを見つけ出すため、完璧に近い検索が行われます。
...
すべての有効な計画はプランナが考慮したすべての結合ペアに対し生成され、最も安価な（と評価された）ものが選択されます。␞␞    </para>␞
␝    <para>␟     When <varname>geqo_threshold</varname> is exceeded, the join
     sequences considered are determined by heuristics, as described
     in <xref linkend="geqo"/>.  Otherwise the process is the same.␟<varname>geqo_threshold</varname>を上回ると、考慮された結合シーケンスは<xref linkend="geqo"/>に記載されているように経験則で決定されます。
そうでない時、処理は変わりません。␞␞    </para>␞
␝    <para>␟     The finished plan tree consists of sequential or index scans of
     ...
     tree.␟最終的な計画ツリーは基になっているリレーションのシーケンシャルもしくはインデックススキャン、そして必要に応じてネステッドループ、マージ、またはハッシュ結合のノード、さらにはソートまたは集約関数計算ノードのような必要とされる補助の手順から構成されます。
...
プランナの1つの責任は、<literal>WHERE</literal>句から選択条件を付加して計画ツリーの最も適切なノードに対し必要とされる出力式を計算することです。␞␞    </para>␞
␝  <sect1 id="executor">␟   <title>Executor</title>␟   <title>エグゼキュータ</title>␞␞␞
␝   <para>␟    The <firstterm>executor</firstterm> takes the plan created by the
    planner/optimizer and recursively processes it to extract the required set
    of rows.  This is essentially a demand-pull pipeline mechanism.
    Each time a plan node is called, it must deliver one more row, or
    report that it is done delivering rows.␟<firstterm>エグゼキュータ</firstterm>は、プランナ/オプティマイザで作成された計画を受け取り、必要な行の集合を抽出するために再帰的に処理します。
これは本質的に要求引き寄せ型（demand-pull）パイプライン機能です。
計画ノードが呼ばれる度にもう1つの行を引き渡すか、行を引き渡したことの報告を行わなければなりません。␞␞   </para>␞
␝   <para>␟    To provide a concrete example, assume that the top
    ...
    that it can deliver them in sorted order in response to later demands.␟具体的な例を提供する目的で頂点のノードが<literal>MergeJoin</literal>ノードである場合を想定しましょう。
...
つまりソート順における最初の結果です。
後での要求に答えるためソート順に引き渡すことができるように残っている行は保存されます。␞␞   </para>␞
␝   <para>␟    The <literal>MergeJoin</literal> node similarly demands the first row
    ...
    no more join rows can be formed.␟<literal>MergeJoin</literal>ノードは同じようにしてその右副計画から最初の行を要求します。
...
最終的にはある副計画もしくは他の計画が使いきられ、<literal>MergeJoin</literal>ノードがこれ以上の結合行を生成できないという意味のNULLを返すことになります。␞␞   </para>␞
␝   <para>␟    Complex queries can involve many levels of plan nodes, but the general
    ...
    the planner.␟複雑な問い合わせは多くの階層となった計画ノードに関わるかもしれませんが、概略的な取り扱い方は同じです。
それぞれのノードは呼び出される度に次の出力行を計算して返します。
それぞれのノードは同時にプランナによって割り当てられたいかなる選択式や射影式でも適用する責任があります。␞␞   </para>␞
␝   <para>␟    The executor mechanism is used to evaluate all five basic SQL query
    ...
    top-level plan node called <literal>ModifyTable</literal>.␟エグゼキュータ機構は5つの基本的なSQL問い合わせの種類すべてを検証するために用いられます。
...
<command>INSERT ... SELECT</command>、<command>UPDATE</command>、<command>DELETE</command>、<command>MERGE</command>は、実質的には<literal>ModifyTable</literal>と呼ばれる特別な最上位階層の計画ノードの下の<command>SELECT</command>です。␞␞   </para>␞
␝   <para>␟    <command>INSERT ... SELECT</command> feeds the rows up
    ...
    inserts, updates or deletes the target row, as required.␟<command>INSERT ... SELECT</command>は挿入のために<literal>ModifyTable</literal>に行を入力します。
...
このデータは<literal>ModifyTable</literal>ノードに入力され、実行までの<literal>WHEN</literal>句を判断し、必要に応じて対象行を挿入、更新、または削除するためにこの情報を利用します。␞␞   </para>␞
␝   <para>␟    A simple <command>INSERT ... VALUES</command> command creates a
    trivial plan tree consisting of a single <literal>Result</literal>
    node, which computes just one result row, feeding that up
    to <literal>ModifyTable</literal> to perform the insertion.␟単純な<command>INSERT ... VALUES</command>コマンドは、1つの<literal>Result</literal>ノードからなる単純な計画ツリーを生成し、そのノードは結果としての行を1つだけ計算し、挿入を実行するためにその行が<literal>ModifyTable</literal>に入力されます。␞␞   </para>␞
