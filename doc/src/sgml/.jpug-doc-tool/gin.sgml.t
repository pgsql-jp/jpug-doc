...
␝<sect1 id="gin">␟<title>GIN Indexes</title>␟<title>GINインデックス</title>␞␞␞
␝<sect2 id="gin-intro">␟ <title>Introduction</title>␟ <title>はじめに</title>␞␞␞
␝ <para>␟  <acronym>GIN</acronym> stands for Generalized Inverted Index.
  ...
  and the queries could be searches for documents containing specific words.␟<acronym>GIN</acronym>とは汎用転置インデックス(Generalized Inverted Index)を表します。
<acronym>GIN</acronym>は、以下のような状況を取り扱うために設計されました。(1)インデックス対象の項目が複合型である。(2)そのインデックスにより処理される問い合わせは、複合型の項目内に存在する要素の値を検索する必要がある。
例えば、項目は文書であり、問い合わせは特定の単語を含む文書の検索です。␞␞ </para>␞
␝ <para>␟  We use the word <firstterm>item</firstterm> to refer to a composite value that
  ...
  not item values per se.␟ここでは、インデックス対象の複合型の値を<firstterm>項目</firstterm>と呼びます。また、要素値を<firstterm>キー</firstterm>と呼びます。
<acronym>GIN</acronym>は項目の値自体ではなく、常にキーを格納し検索します。␞␞ </para>␞
␝ <para>␟  A <acronym>GIN</acronym> index stores a set of (key, posting list) pairs,
  ...
  where the same key appears many times.␟<acronym>GIN</acronym>インデックスは(キー、ポスティングリスト(posting list))の組み合わせの集合を格納します。
ここで<firstterm>ポスティングリスト</firstterm>はキーが発生した行IDの集合です。
項目は１つ以上のキーを含むことができますので、同じ行IDが複数のポスティングリストに現れることがあり得ます。
キー値はそれぞれ一度のみ格納されます。
このため<acronym>GIN</acronym>インデックスの容量は、同じキーが何度も現れる場合に非常に小さくなります。␞␞ </para>␞
␝ <para>␟  <acronym>GIN</acronym> is generalized in the sense that the
  ...
  some of the key values in a query actually satisfies the query.␟<acronym>GIN</acronym>インデックスは、<acronym>GIN</acronym>アクセスメソッドが高速化対象の操作を把握する必要がないという意味で汎用化されています。
その代わり、特定のデータ型に対して定義された独自の戦略を使用します。
戦略は、インデックス付けされた項目と問い合わせ条件からキーを抽出する方法および問い合わせ内のいくつかのキー値を含む行が実際に問い合わせを満たすかどうかを決定する方法を定義します。␞␞ </para>␞
␝ <para>␟  One advantage of <acronym>GIN</acronym> is that it allows the development
  of custom data types with the appropriate access methods, by
  an expert in the domain of the data type, rather than a database expert.
  This is much the same advantage as using <acronym>GiST</acronym>.␟<acronym>GIN</acronym>の利点の1つは、データベース専門家ではなくデータ型の分野における専門家により、適切なアクセスメソッドを持つ独自のデータ型を開発できるという点です。
これは<acronym>GiST</acronym>の使用とほぼ同じ利点です。␞␞ </para>␞
␝ <para>␟  The <acronym>GIN</acronym>
  implementation in <productname>PostgreSQL</productname> is primarily
  maintained by Teodor Sigaev and Oleg Bartunov. There is more
  information about <acronym>GIN</acronym> on their
  <ulink url="http://www.sai.msu.su/~megera/wiki/Gin">website</ulink>.␟<productname>PostgreSQL</productname>における<acronym>GIN</acronym>の実装は、主にTeodor SigaevとOleg Bartunovにより保守されています。
<acronym>GIN</acronym>に関する情報は彼らの<ulink url="http://www.sai.msu.su/~megera/wiki/Gin">webサイト</ulink>により多く記載されています。␞␞ </para>␞
␝<sect2 id="gin-builtin-opclasses">␟ <title>Built-in Operator Classes</title>␟ <title>組み込み演算子クラス</title>␞␞␞
␝ <para>␟  The core <productname>PostgreSQL</productname> distribution
  ...
  provide additional <acronym>GIN</acronym> operator classes.)␟<productname>PostgreSQL</productname>のコア配布物は<xref linkend="gin-builtin-opclasses-table"/>に示す<acronym>GIN</acronym>演算子クラスを含みます。
(<xref linkend="contrib"/>に記載された追加モジュールの中には追加の<acronym>GIN</acronym>演算子クラスを提供するものもあります。)␞␞ </para>␞
␝  <table id="gin-builtin-opclasses-table">␟   <title>Built-in <acronym>GIN</acronym> Operator Classes</title>␟   <title>組み込み<acronym>GIN</acronym>演算子クラス</title>␞␞   <tgroup cols="2">␞
␝     <row>␟      <entry>Name</entry>␟      <entry>名前</entry>␞␞␞
␝      <entry>Name</entry>␟      <entry>Indexable Operators</entry>␟      <entry>インデックス可能な演算子</entry>␞␞     </row>␞
␝ <para>␟  Of the two operator classes for type <type>jsonb</type>, <literal>jsonb_ops</literal>
  is the default.  <literal>jsonb_path_ops</literal> supports fewer operators but
  offers better performance for those operators.
  See <xref linkend="json-indexing"/> for details.␟<type>jsonb</type>型の2つの演算子クラスのうち、<literal>jsonb_ops</literal>がデフォルトです。
<literal>jsonb_path_ops</literal>はより少数の演算子しかサポートしませんが、その演算子に対してはより良いパフォーマンスを提供します。
詳細は<xref linkend="json-indexing"/>を参照してください。␞␞ </para>␞
␝<sect2 id="gin-extensibility">␟ <title>Extensibility</title>␟ <title>拡張性</title>␞␞␞
␝ <para>␟   The <acronym>GIN</acronym> interface has a high level of abstraction,
   requiring the access method implementer only to implement the semantics of
   the data type being accessed.  The <acronym>GIN</acronym> layer itself
   takes care of concurrency, logging and searching the tree structure.␟<acronym>GIN</acronym>インタフェースは高度に抽象化されています。
アクセスメソッド実装者に要求されることは、アクセスするデータ型の意味を実装することだけです。
<acronym>GIN</acronym>層自体が同時実行性、ログ処理、ツリー構造の検索処理に関する面倒を見ます。␞␞ </para>␞
␝ <para>␟   All it takes to get a <acronym>GIN</acronym> access method working is to
   ...
   extensibility with generality, code reuse, and a clean interface.␟<acronym>GIN</acronym>アクセスメソッドを動作させるために必要なことは、少数のユーザ定義関数を実装することだけです。
これは、ツリー内のキーの動作とキーとインデックス付けされる項目、インデックス可能な問い合わせ間の関係を定義します。
すなわち、<acronym>GIN</acronym>は、一般化、コード再利用、整理されたインタフェースによる拡張性を組み合わせます。␞␞ </para>␞
␝ <para>␟   There are two methods that an operator class for
   <acronym>GIN</acronym> must provide:␟<acronym>GIN</acronym>用の演算子クラスが提供しなければならないメソッドは2つあります。␞␞␞
␝      <para>␟       Returns a palloc'd array of keys given an item to be indexed.  The
       ...
       The return value can be <symbol>NULL</symbol> if the item contains no keys.␟インデックス対象値に与えられる、pallocで割り当てられたキーの配列を返します。
...
項目がキーを含まない場合、戻り値は<symbol>NULL</symbol>になるかもしれません。␞␞      </para>␞
␝      <para>␟       Returns a palloc'd array of keys given a value to be queried; that is,
       ...
       <literal>*nullFlags</literal> can be left <symbol>NULL</symbol> (its initial value)
       if all keys are non-null.
       The return value can be <symbol>NULL</symbol> if the <literal>query</literal> contains no keys.␟問い合わせ対象の値に与えられる、pallocで割り当てられたキーの配列を返します。
...
<literal>query</literal>がキーを含まない場合、戻り値を<symbol>NULL</symbol>にすることができます。␞␞      </para>␞
␝      <para>␟       <literal>searchMode</literal> is an output argument that allows
       ...
       <filename>access/gin.h</filename>.␟<literal>searchMode</literal>は出力引数です。
...
このモードを設定するために使用する記号は<filename>access/gin.h</filename>で定義されています。␞␞      </para>␞
␝      <para>␟       <literal>pmatch</literal> is an output argument for use when partial match
       ...
       not support partial match.␟<literal>pmatch</literal>は部分一致が提供されている場合に使用する出力引数です。
使用するには、<function>extractQuery</function>が<literal>*nkeys</literal>個の<type>bool</type>の配列を割り当て、そのアドレスを<literal>*pmatch</literal>に格納しなければなりません。
...
呼び出し前に変数は<symbol>NULL</symbol>に初期化されますので、この引数は部分一致が提供されていない演算子クラスでは、単に無視できます。␞␞      </para>␞
␝      <para>␟       <literal>extra_data</literal> is an output argument that allows
       ...
       the appropriate element to the <function>comparePartial</function> method.␟<literal>extra_data</literal>は、<function>extractQuery</function>が<function>consistent</function>と<function>comparePartial</function>メソッドに追加データを渡すことができるようにする出力引数です。
...
もし<literal>*extra_data</literal>が設定されれば、配列全部が<function>consistent</function>メソッドに、適切な要素が<function>comparePartial</function>メソッドに渡されます。␞␞      </para>␞
␝␟  An operator class must also provide a function to check if an indexed item
  ...
  disabled.␟演算子クラスは、インデックス付けされた項目が問い合わせに一致するか確認する関数も提供しなければなりません。
それは2つの方法で行なわれます。
...
␝      <para>␟       Returns true if an indexed item satisfies the query operator with
       ...
       <function>extractQuery</function>, or <symbol>NULL</symbol> if none.␟インデックス付けられた項目が戦略番号<literal>n</literal>を持つ問い合わせ演算子を満たす（または、recheck印が返されたときはたぶん満たすかもしれない）場合に真を返します。
...
<literal>extra_data</literal>は<function>extractQuery</function>により返された追加データ配列で、ない場合は<symbol>NULL</symbol>です。␞␞      </para>␞
␝      <para>␟       When <function>extractQuery</function> returns a null key in
       ...
       the difference between a regular value match and a null match.␟<function>extractQuery</function>が<literal>queryKeys[]</literal>内でNULLキーを返す時、インデックス対象項目がNULLキーを含む場合は対応する<literal>check[]</literal>要素は真です、
つまり、<literal>check[]</literal>の意味は<literal>IS NOT DISTINCT FROM</literal>のようなものです。
<function>consistent</function>関数は、通常の値の合致とNULL合致との違いを通知する必要がある場合、対応する<literal>nullFlags[]</literal>要素を検査することができます。␞␞      </para>␞
␝      <para>␟       On success, <literal>*recheck</literal> should be set to true if the heap
       ...
       query operator directly against the originally indexed item.␟成功の場合、<literal>*recheck</literal>は、問い合わせ演算子に対してヒープタプルを再検査する必要があればtrue、インデックス検査が的確であればfalseに設定する必要があります。
つまり、falseの戻り値はヒープタプルが問い合わせに一致しないことを保証します。
trueの戻り値で、<literal>*recheck</literal>がfalseに設定された場合はヒープタプルが問い合わせに一致することを保証します。
trueの戻り値で、<literal>*recheck</literal>がtrueに設定された場合はヒープタプルが問い合わせに一致する可能性があることを意味します。
したがって、それを取り出し、元のインデックス付けされた項目を直接問い合わせ演算子で評価することで再検査する必要があることを意味します。␞␞      </para>␞
␝      <para>␟       <function>triConsistent</function> is similar to <function>consistent</function>,
       ...
       the function must return <literal>GIN_MAYBE</literal>.␟<function>triConsistent</function>は<function>consistent</function>と似ていますが、<literal>check</literal>ベクトルの論理値の代わりに、各キーに対して3つの可能な値があります。<literal>GIN_TRUE</literal>、<literal>GIN_FALSE</literal>、<literal>GIN_MAYBE</literal>です。
...
結果が<literal>GIN_MAYBE</literal>項目に依存する、すなわち、分かっている問い合わせキーに基づいて、一致することもしないことも確認できない場合には、関数は<literal>GIN_MAYBE</literal>を返さなければなりません。␞␞      </para>␞
␝      <para>␟       When there are no <literal>GIN_MAYBE</literal> values in the <literal>check</literal>
       vector, a <literal>GIN_MAYBE</literal> return value is the equivalent of
       setting the <literal>recheck</literal> flag in the
       Boolean <function>consistent</function> function.␟<literal>check</literal>ベクトルに<literal>GIN_MAYBE</literal>値がなければ、<literal>GIN_MAYBE</literal>戻り値は論理値の<function>consistent</function>関数で<literal>recheck</literal>フラグを設定することと同じです。␞␞      </para>␞
␝ <para>␟  In addition, GIN must have a way to sort the key values stored in the index.
  The operator class can define the sort ordering by specifying a comparison
  method:␟さらに、GINにはインデックス内に格納されているキー値をソートする方法がなければなりません。
演算子クラスは比較メソッドを指定することでソート順を定義できます。␞␞␞
␝      <para>␟       Compares two keys (not indexed items!) and returns an integer less than
       zero, zero, or greater than zero, indicating whether the first key is
       less than, equal to, or greater than the second.  Null keys are never
       passed to this function.␟キー（インデックス付けされる項目ではありません）を比較し、0より小さい、0、または0より大きい整数を返します。
それぞれ、最初のキーが2番目のキーより、小さい、等しい、または大きいことを示します。
NULLキーがこの関数に渡されることはありません。␞␞      </para>␞
␝␟  Alternatively, if the operator class does not provide a <function>compare</function>
  ...
  function.␟あるいは、演算子クラスが<function>compare</function>メソッドを提供しない場合には、GINはそのインデックスキーデータ型に対するデフォルトのbtree演算子クラスを探し、その比較関数を使います。
btree演算子クラスを探すのは処理に多少掛かりますので、GIN演算子クラスの中で比較関数を指定することを勧めます。それはただ一つのデータ型に対するものであることを意味します。
しかし、(<literal>array_ops</literal>のような)多様GIN演算子クラスでは、通常は単一の比較関数を指定できません。␞␞ </para>␞
␝ <para>␟  An operator class for <acronym>GIN</acronym> can optionally supply the
  following methods:␟省略可能ですが、<acronym>GIN</acronym>に対する演算子クラスは以下のメソッドを提供します。␞␞␞
␝      <para>␟       Compare a partial-match query key to an index key.  Returns an integer
       ...
       Null keys are never passed to this function.␟問い合わせキーとインデックスキーの部分一致を比較します。
符号が結果を示す整数が返ります。
...
␝      <para>␟       Since both key extraction of indexed values and representation of the
       ...
       user-specified parameters.␟インデックス付けされた値からのキーの抽出にも<acronym>GIN</acronym>でのキーの表現にも柔軟性がありますので、ユーザに固有のパラメータに依存するかもしれません。␞␞      </para>␞
␝ <para>␟  To support <quote>partial match</quote> queries, an operator class must
  ...
  <xref linkend="gin-partial-match"/> for details.␟<quote>部分一致</quote>問い合わせをサポートするためには、演算子クラスは<function>comparePartial</function>メソッドを提供しなければなりません。
またその<function>extractQuery</function>は、部分一致問い合わせであった時に<literal>pmatch</literal>パラメータを設定しなければなりません。
詳細については<xref linkend="gin-partial-match"/>を参照してください。␞␞ </para>␞
␝ <para>␟  The actual data types of the various <literal>Datum</literal> values mentioned
  ...
  though the actual type might be something else depending on the operator.␟上記の各種<literal>Datum</literal>値の実データ型は、演算子クラスに依存して変動します。
...
しかしながら、この3つのサポート関数のSQL宣言では、実際の型は演算子に依存して何か他のものであるとしても、<literal>query</literal>引数には演算子クラスのインデックス付けされたデータ型を使うことを勧めます。␞␞ </para>␞
␝<sect2 id="gin-implementation">␟ <title>Implementation</title>␟ <title>実装</title>␞␞␞
␝ <para>␟  Internally, a <acronym>GIN</acronym> index contains a B-tree index
  ...
  these components of a GIN index.␟<acronym>GIN</acronym>インデックスはキー全体に対するB-treeインデックスを持ちます。
そのキーはそれぞれインデックス対象項目の要素（例えば配列のメンバ）であり、リーフページ内のタプルはそれぞれ、ヒープポインタのB-treeへのポインタ（<quote>ポスティングツリー(posting tree)</quote>）か、もしリストがキー値と共に単一インデックスタプルに合う程度十分に小さければヒープポインタの単純なリスト（<quote>ポスティングリスト(posting list)</quote>）です。
<xref linkend="gin-internals-figure"/>にGINインデックスのこれらのコンポーネントを示します。␞␞ </para>␞
␝ <para>␟  As of <productname>PostgreSQL</productname> 9.1, null key values can be
  ...
  items to do so.␟<productname>PostgreSQL</productname> 9.1からNULLキー値をインデックスに含められるようになりました。
またプレースホルダとしてのNULLが、NULLまたは<function>extractValue</function>によるとキーを含まないインデックス対象項目についてインデックスに含められます。
これにより空の項目を見つけ出すための検索を行うことができます。␞␞ </para>␞
␝ <para>␟  Multicolumn <acronym>GIN</acronym> indexes are implemented by building
  a single B-tree over composite values (column number, key value).  The
  key values for different columns can be of different types.␟複数列に対する<acronym>GIN</acronym>インデックスは複合型の値（列番号、キー値）全体について単一のB-treeを構築することで実装されます。
異なる列に対するキー値は別の型となるかもしれません。␞␞ </para>␞
␝ <figure id="gin-internals-figure">␟  <title>GIN Internals</title>␟  <title>GINの内部</title>␞␞  <mediaobject>␞
␝ <sect3 id="gin-fast-update">␟  <title>GIN Fast Update Technique</title>␟  <title>GIN高速更新手法</title>␞␞␞
␝  <para>␟   Updating a <acronym>GIN</acronym> index tends to be slow because of the
   ...
   process instead of in foreground query processing.␟1つのヒープ行の挿入または更新によりインデックスへの挿入が多く発生するという、転置インデックスの本質的な性質のため<acronym>GIN</acronym>インデックスの更新は低速になりがちです。
（各キー用のヒープ行はインデックス付けされた項目から取り出されます。）
<acronym>GIN</acronym>は、新しいタプルを一時的なソートされていない、待機中の項目リストに挿入することにより、この作業の大部分を遅延させることができるようになりました。
テーブルがバキュームまたは自動解析された時、<function>gin_clean_pending_list</function>関数が呼ばれたとき、または、待機中のリスト(pending list)が<xref linkend="guc-gin-pending-list-limit"/>よりも大きくなった時、初期のインデックス作成の際に使用されるものと同様の一括挿入技法を使用して、項目は主<acronym>GIN</acronym>データ構造に移動されます。
これは、バキュームのオーバーヘッドが追加されることを考慮したとしても、<acronym>GIN</acronym>インデックスの更新速度を著しく向上します。
さらに、フォアグラウンドの問い合わせ処理ではなくバックグラウンド処理でこのオーバーヘッド作業を実行することができます。␞␞  </para>␞
␝  <para>␟   The main disadvantage of this approach is that searches must scan the list
   ...
   Proper use of autovacuum can minimize both of these problems.␟この手法の大きな欠点は、検索時に通常のインデックス検索に加え待機中の項目リストのスキャンを行わなければならない点です。
...
自動バキュームを適切に使用することで、これらの両方の問題を最小化することができます。␞␞  </para>␞
␝  <para>␟   If consistent response time is more important than update speed,
   use of pending entries can be disabled by turning off the
   <literal>fastupdate</literal> storage parameter for a
   <acronym>GIN</acronym> index.  See <xref linkend="sql-createindex"/>
   for details.␟一貫した応答時間が更新速度より重要な場合、<acronym>GIN</acronym>インデックスに対する<literal>fastupdate</literal>格納パラメータを無効にすることにより、待機中の項目の使用を無効にすることができます。
詳細は<xref linkend="sql-createindex"/>を参照してください。␞␞  </para>␞
␝ <sect3 id="gin-partial-match">␟  <title>Partial Match Algorithm</title>␟  <title>部分一致アルゴリズム</title>␞␞␞
␝  <para>␟   GIN can support <quote>partial match</quote> queries, in which the query
   ...
   that could match.␟GINは<quote>部分一致</quote>問い合わせをサポートすることができます。
...
<function>comparePartial</function>は一致するインデックスキーではゼロを、一致しないが検索すべき範囲内にあればゼロ未満の値を、インデックスキーが一致可能な範囲を超えた場合はゼロより大きな値を返さなければなりません。␞␞  </para>␞
␝<sect2 id="gin-tips">␟<title>GIN Tips and Tricks</title>␟<title>GINの小技</title>␞␞␞
␝  <varlistentry>␟   <term>Create vs. insert</term>␟   <term>作成と挿入</term>␞␞   <listitem>␞
␝    <para>␟     Insertion into a <acronym>GIN</acronym> index can be slow
     due to the likelihood of many keys being inserted for each item.
     So, for bulk insertions into a table it is advisable to drop the GIN
     index and recreate it after finishing bulk insertion.␟各項目に対して多くのキーが挿入される可能性がありますので、<acronym>GIN</acronym>インデックスへの挿入は低速になることがあります。
ですので、テーブルに対する大量の挿入では、GINインデックスを削除し、大量の挿入が終わった段階で再作成することを勧めます。␞␞    </para>␞
␝    <para>␟     When <literal>fastupdate</literal> is enabled for <acronym>GIN</acronym>
     ...
     best to drop and recreate the index.␟<acronym>GIN</acronym>では<literal>fastupdate</literal>が有効である場合、このペナルティはそうでない場合よりも少なくなります。
（<xref linkend="gin-fast-update"/>を参照してください。）
しかし非常に大規模な更新では、インデックスの削除と再作成がまだ最善かもしれません。␞␞    </para>␞
␝    <para>␟     Build time for a <acronym>GIN</acronym> index is very sensitive to
     the <varname>maintenance_work_mem</varname> setting; it doesn't pay to
     skimp on work memory during index creation.␟<acronym>GIN</acronym>インデックスの構築時間は<varname>maintenance_work_mem</varname>の設定に非常に敏感です。
インデックス作成時に作業メモリをより少なく使用しようとはしません。␞␞    </para>␞
␝    <para>␟     During a series of insertions into an existing <acronym>GIN</acronym>
     ...
     if a foreground cleanup does occur, it will take even longer.␟<literal>fastupdate</literal>が有効な既存の<acronym>GIN</acronym>インデックスに対して挿入を繰り返す間、待機中の項目リストが<varname>gin_pending_list_limit</varname>より大きくなると、システムはこのリストを整理します。
...
しかし、整理処理の閾値を大きくすることは、フォアグラウンドで整理処理が発生した時により長い時間がかかることを意味します。␞␞    </para>␞
␝    <para>␟     <varname>gin_pending_list_limit</varname> can be overridden for individual
     GIN indexes by changing storage parameters, which allows each
     GIN index to have its own cleanup threshold.
     For example, it's possible to increase the threshold only for the GIN
     index which can be updated heavily, and decrease it otherwise.␟<varname>gin_pending_list_limit</varname>は格納パラメータを変更することで個々のGINインデックスに対して上書きでき、それにより各GINインデックスが自身の整理閾値を持てます。
例えば、頻繁に更新される可能性のあるGINインデックスの閾値のみを増やして、それ以外は減らすことができます。␞␞    </para>␞
␝    <para>␟     The primary goal of developing <acronym>GIN</acronym> indexes was
     ...
     fast.)␟<acronym>GIN</acronym>インデックス開発の主な目的は、スケーラビリティが高い全文検索のサポートを<productname>PostgreSQL</productname>で作成することでした。
...
（インデックス検索自体は非常に高速であることに注意してください。）␞␞    </para>␞
␝    <para>␟     To facilitate controlled execution of such queries,
     ...
     the whole result set, chosen at random.␟こうした問い合わせの実行を簡単に制御できるように、<acronym>GIN</acronym>は返される行数に対して設定可能なソフト上限、<varname>gin_fuzzy_search_limit</varname>設定パラメータを持ちます。
これはデフォルトでは0です（無制限を意味します）。
非0の制限が設定された場合、返されるセットは結果セット全体からランダムに選んだサブセットになります。␞␞    </para>␞
␝    <para>␟     <quote>Soft</quote> means that the actual number of returned results
     could differ somewhat from the specified limit, depending on the query
     and the quality of the system's random number generator.␟<quote>ソフト</quote>は、問い合わせとシステムの乱数ジェネレータの品質に依存して、返される結果の実際の数が指定した上限より多少異なることを意味します。␞␞    </para>␞
␝    <para>␟     From experience, values in the thousands (e.g., 5000 &mdash; 20000)
     work well.␟経験上、数千（例えば5000から20000）の値がうまく動作します。␞␞    </para>␞
␝<sect2 id="gin-limit">␟ <title>Limitations</title>␟ <title>制限事項</title>␞␞␞
␝ <para>␟  <acronym>GIN</acronym> assumes that indexable operators are strict.  This
  ...
  or query value are supported.␟<acronym>GIN</acronym>は、インデックス付け可能な演算子は厳密であると仮定します。
...
しかし非NULLの複合型項目内または問い合わせ値内のNULLキー値はサポートされます。␞␞ </para>␞
␝<sect2 id="gin-examples">␟ <title>Examples</title>␟ <title>例</title>␞␞␞
␝ <para>␟  The core <productname>PostgreSQL</productname> distribution
  ...
  <acronym>GIN</acronym> operator classes:␟<productname>PostgreSQL</productname>のコア配布物は以前<xref linkend="gin-builtin-opclasses-table"/>に示した<acronym>GIN</acronym>演算子クラスを含みます。
以下の<filename>contrib</filename>モジュールにも<acronym>GIN</acronym>演算子クラスが含まれています。␞␞␞
␝   <listitem>␟    <para>B-tree equivalent functionality for several data types</para>␟    <para>さまざまなデータ型に対するB-tree等価の機能</para>␞␞   </listitem>␞
␝   <listitem>␟    <para>Module for storing (key, value) pairs</para>␟    <para>（キー、値）の組み合わせを格納するモジュール</para>␞␞   </listitem>␞
␝   <listitem>␟    <para>Enhanced support for <type>int[]</type></para>␟    <para><type>int[]</type>に対する高度サポート</para>␞␞   </listitem>␞
␝   <listitem>␟    <para>Text similarity using trigram matching</para>␟    <para>トライグラム一致を使用したテキスト類似度</para>␞␞   </listitem>␞
