...
␝<sect1 id="hash-index">␟<title>Hash Indexes</title>␟<title>ハッシュインデックス</title>␞␞␞
␝<sect2 id="hash-intro">␟ <title>Overview</title>␟ <title>概要</title>␞␞␞
␝ <para>␟  <productname>PostgreSQL</productname>
  ...
  being indexed.␟<productname>PostgreSQL</productname>には、クラッシュから完全に回復可能なディスク上の永続的なハッシュインデックスの実装が含まれています。
明確な線形順序付けを持たないものも含め、すべてのデータ型がハッシュインデックスでインデックス可能です。
ハッシュインデックスは、インデックスされる値のハッシュ値のみを保存するので、インデックス対象の列のサイズは制限となりません。␞␞ </para>␞
␝ <para>␟  Hash indexes support only single-column indexes and do not allow
  uniqueness checking.␟ハッシュインデックスは単一列のインデックスのみをサポートし、唯一性のチェックはできません。␞␞ </para>␞
␝ <para>␟  Hash indexes support only the <literal>=</literal> operator,
  so WHERE clauses that specify range operations will not be able to take
  advantage of hash indexes.␟ハッシュインデックスは<literal>=</literal>演算子のみをサポートしており、範囲演算を指定するWHERE句はハッシュインデックスの恩恵をこうむることができません。␞␞ </para>␞
␝ <para>␟  Each hash index tuple stores just the 4-byte hash value, not the actual
  ...
  take part in bitmap index scans and backward scans.␟各インデックスタプルは単なる4バイトのハッシュ値で、実際の列の値ではありません。
そのため、UUIDやURLのような大きなデータをインデックスすると、ハッシュインデックスはB-treeよりもずっと小さくなるかも知れません。
また、列値が含まれていないため、すべてのハッシュスキャンは損失がある(lossy)ものになります。
ハッシュインデックスはビットマップインデックススキャンおよび後方スキャンに参加できます。␞␞ </para>␞
␝ <para>␟  Hash indexes are best optimized for SELECT and UPDATE-heavy workloads
  ...
  larger than shared_buffers/RAM.␟ハッシュインデックスは、大きなテーブルに対して同値スキャンを使用するSELECTとUPDATEを多用するワークロードに対して最適です。
...
「論理的なI/O」における時間短縮は、共有バッファ/RAMよりもインデックス/データが大きな時にはより顕著になります。␞␞ </para>␞
␝ <para>␟  Hash indexes have been designed to cope with uneven distributions of
  ...
  accesses required, for some data.␟ハッシュインデックスはハッシュ値の均等ではない分布を想定して設計されています。
...
そのため、不均等なハッシュインデックスは、いくつかのデータに対してアクセスが必要なブロックの数の点で、Bツリーよりも悪いかも知れません。␞␞ </para>␞
␝ <para>␟  As a result of the overflow cases, we can say that hash indexes are
  ...
  not be suitable in many cases.␟オーバーフローが発生する場合を考慮すると、ハッシュインデックスは一意か、ほぼ一意に近いデータあるいは、それぞれのハッシュバケットに少数の行があるデータがもっとも適していると言えます。
問題を回避できる方法の１つは部分インデックス条件を使って極端に一意ではない値を排除することですが、多くの場合にこれが適しているとは言えないかも知れません。␞␞ </para>␞
␝ <para>␟  Like B-Trees, hash indexes perform simple index tuple deletion. This
  ...
  Deletion of dead index pointers also occurs during VACUUM.␟Bツリーのように、ハッシュインデックスは単純なインデックスタプルの削除を行います。
...
不要インデックスポインタの削除もVACUUM中に発生します。␞␞ </para>␞
␝ <para>␟  If it can, VACUUM will also try to squeeze the index tuples onto as
  ...
  There is no provision for reducing the number of buckets, either.␟可能ならば、VACUUMはインデックスタプルをできるだけ少ないオーバーフローページに押し込むことも試み、オーバーフローの連結を最小限に抑えます。
あるオーバーフローページが空になったらそのオーバーフローページは再利用できますが、オペレーティングシステムに返却することはありません。
今の所、REINDEXで再構築する以外にハッシュインデックスを縮小するようにする予定はありません。
バケット数を少なくする予定もありません。␞␞ </para>␞
␝ <para>␟  Hash indexes may expand the number of bucket pages as the number of
  ...
  key-to-bucket-number mapping.␟ハッシュインデックスはインデックスされた行数が増えるとバケットページ数も拡張します。
ハッシュキーからバケット番号へのマッピングは、インデックスが徐々に拡張できるように選択されます。
新しいバケットがインデックスに追加されることになったら、存在しているバケットの厳密に一つが「分割」される必要があります。
更新されたハッシュキーからバケット番号へのマッピングにしたがって、タプルが新しいバケットに転送されます。␞␞ </para>␞
␝ <para>␟  The expansion occurs in the foreground, which could increase execution
  time for user inserts. Thus, hash indexes may not be suitable for tables
  with rapidly increasing number of rows.␟その拡張はフォアグラウンドで行われるので、ユーザが挿入を実行するのにかかる時間を増加させるでしょう。
ですから、ハッシュインデックスは行数が急激に拡張するテーブルには適していないかもしれません。␞␞ </para>␞
␝<sect2 id="hash-implementation">␟ <title>Implementation</title>␟ <title>実装</title>␞␞␞
␝ <para>␟  There are four kinds of pages in a hash index: the meta page (page zero),
  ...
  purposes, bitmap pages are regarded as a subset of the overflow pages.␟ハッシュインデックスには4種類のページがあります。
静的に確保された制御情報を持つメタページ(ページ0)、主バケットページ、オーバーフローページ、解放されて再利用が可能なオーバーフローページを追跡するビットマップページ、です。
アドレッシング目的という点では、ビットマップページはオーバーフローページのサブセットと見なされます。␞␞ </para>␞
␝ <para>␟  Both scanning the index and inserting tuples require locating the bucket
  ...
  cache refresh.␟インデックスのスキャンおよびタプルの挿入は両者とも、与えられたタプルが置かれるべきバケットを特定する必要があります。
...
最後にキャッシュが更新されてから目的のバケットが分割されていない限り、これは正しいバケットのマッピングを生成します。␞␞ </para>␞
␝ <para>␟  Primary bucket pages and overflow pages are allocated independently since
  ...
  move primary bucket pages around after they are created.␟特定のインデックスにおいて、バケット数に対して必要なオーバーフローページは増減する可能性があるため、主バケットページとオーバーフローページは独立して確保されます。
ハッシュコードは可変数のオーバーフローページをサポートするために一連の興味深いアドレッシング規則を使用しており、主バケットページを作成後に移動する必要もありません。␞␞ </para>␞
␝ <para>␟  Each row in the table indexed is represented by a single index tuple in
  ...
  the relative ordering of hash codes across different index pages of a bucket.␟インデックス付されたテーブル内の各行はハッシュインデックスにおいては単一のインデックスタプルで表現されています。
...
しかし、バケット内の異なるインデックスページ間においては、ハッシュコードの相対的な順序付けに何も前提はないことに留意してください。␞␞ </para>␞
␝ <para>␟  The bucket splitting algorithms to expand the hash index are too complex to
  ...
  successfully.␟ハッシュインデックスを拡張するためにバケットを分割するアルゴリズムは複雑過ぎてここで言及するには及びませんが、より詳細が<filename>src/backend/access/hash/README</filename>に記載されています。
分割アルゴリズムはクラッシュ耐性があり、正常に完了していなくても再スタートできます。␞␞ </para>␞
