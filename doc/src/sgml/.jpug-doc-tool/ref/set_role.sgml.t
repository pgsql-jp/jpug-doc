...
␝  <refname>SET ROLE</refname>␟  <refpurpose>set the current user identifier of the current session</refpurpose>␟  <refpurpose>現在のセッションにおける現在のユーザ識別子を設定する</refpurpose>␞␞ </refnamediv>␞
␝ <refsect1>␟  <title>Description</title>␟  <title>説明</title>␞␞␞
␝  <para>␟   This command sets the current user
   ...
   session user (the <firstterm>authenticated user</firstterm>), respectively.␟このコマンドは現在のSQLセッションにおける現在のユーザ識別子を<replaceable class="parameter">role_name</replaceable>に設定します。
ロール名は識別子あるいは文字列リテラルのどちらを使用しても記述することができます。
<command>SET ROLE</command>の後、SQLコマンドに対する権限検査は、指定されたロールで普通にログインした場合と同様に行われます。
<command>SET ROLE</command>と<command>SET SESSION AUTHORIZATION</command>は例外であることに注意してください。これらに対する権限検査は、それぞれ現在のセッションユーザと初期セッションユーザ(<firstterm>認証されたユーザ</firstterm>)を使い続けます。␞␞  </para>␞
␝  <para>␟   The current session user must have the <literal>SET</literal> option for the
   ...
   (If the session user is a superuser, any role can be selected.)␟現在のセッションユーザは、直接的にまたは<literal>SET</literal>オプションを使用したメンバシップの連鎖を介して間接的に、指定する<replaceable class="parameter">role_name</replaceable>に対して<literal>SET</literal>オプションを持つことが必要です。
（セッションユーザがスーパーユーザであった場合、任意のロールを選択することができます。）␞␞  </para>␞
␝  <para>␟   The <literal>SESSION</literal> and <literal>LOCAL</literal> modifiers act the same
   as for the regular <link linkend="sql-set"><command>SET</command></link>
   command.␟<literal>SESSION</literal>および<literal>LOCAL</literal>修飾子は通常の<link linkend="sql-set"><command>SET</command></link>コマンドと同様に動作します。␞␞  </para>␞
␝  <para>␟   <literal>SET ROLE NONE</literal> sets the current user identifier to the
   ...
   forms can be executed by any user.␟<literal>SET ROLE NONE</literal>は現在のユーザ識別子を<function>session_user</function>により返される現在のセッションユーザ識別子に設定します。
...
この構文はすべてのユーザが実行できます。␞␞  </para>␞
␝ <refsect1>␟  <title>Notes</title>␟  <title>注釈</title>␞␞␞
␝  <para>␟   Using this command, it is possible to either add privileges or restrict
   ...
   exercised either with or without <literal>SET ROLE</literal>.␟このコマンドを使用して、権限を追加することも制限することもできます。
...
ロールに<literal>WITH INHERIT FALSE, SET FALSE</literal>が付与されている場合、<literal>SET ROLE</literal>の有無にかかわらず、その権限を行使できません。␞␞  </para>␞
␝  <para>␟   <command>SET ROLE</command> has effects comparable to
   ...
   allowed to a later <command>SET ROLE</command>.␟<command>SET ROLE</command>の影響は<link linkend="sql-set-session-authorization"><command>SET SESSION AUTHORIZATION</command></link>と似ていますが、行われる権限検査はかなり異なります。
また、<command>SET SESSION AUTHORIZATION</command>は、それ以降に実行する<command>SET ROLE</command>コマンドでどのロールに変更できるかを決定しますが、<command>SET ROLE</command>を使用してロールを変更した場合、それ以降に実行する<command>SET ROLE</command>コマンドで変更可能なロール群は変更されません。␞␞  </para>␞
␝  <para>␟   <command>SET ROLE</command> does not process session variables as specified by
   the role's <link linkend="sql-alterrole"><command>ALTER ROLE</command></link> settings;  this only happens during
   login.␟<command>SET ROLE</command>はロールの<link linkend="sql-alterrole"><command>ALTER ROLE</command></link>設定で指定されたセッション変数を処理しません。
これはログイン時のみ適用されます。␞␞  </para>␞
␝  <para>␟   <command>SET ROLE</command> cannot be used within a
   <literal>SECURITY DEFINER</literal> function.␟<command>SET ROLE</command>を<literal>SECURITY DEFINER</literal>関数内で使用することはできません。␞␞  </para>␞
...
␝  <para>␟   <productname>PostgreSQL</productname>
   ...
   <literal>RESET</literal> syntax.␟<productname>PostgreSQL</productname>では、識別子構文（<literal>"<replaceable>rolename</replaceable>"</literal>）を使用できます。
...
<literal>SESSION</literal>、<literal>LOCAL</literal>修飾子、および、<literal>RESET</literal>構文は<productname>PostgreSQL</productname>の拡張です。␞␞  </para>␞
