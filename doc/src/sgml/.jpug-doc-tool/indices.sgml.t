...
␝<chapter id="indexes">␟ <title>Indexes</title>␟ <title>インデックス</title>␞␞␞
␝ <para>␟  Indexes are a common way to enhance database performance.  An index
  ...
  sensibly.␟インデックスは、データベースの性能を向上させるための一般的な方法です。
データベースサーバでインデックスを使用すると、インデックスを使用しない場合に比べてかなり速く、特定の行を検出し抽出することができます。
しかし、インデックスを使用すると、データベースシステム全体にオーバーヘッドを追加することにもなるため、注意して使用する必要があります。␞␞ </para>␞
␝ <sect1 id="indexes-intro">␟  <title>Introduction</title>␟  <title>はじめに</title>␞␞␞
␝  <para>␟   Suppose we have a table similar to this:␟次のようなテーブルを考えてみましょう。␞␞<programlisting>␞
␝</programlisting>␟   and the application issues many queries of the form:␟アプリケーションはこの形式の多くの問い合わせを発行します。␞␞<programlisting>␞
␝</programlisting>␟   With no advance preparation, the system would have to scan the entire
   ...
   might only have to walk a few levels deep into a search tree.␟事前に準備を行っていなければ、システムで一致する項目を全て検出するためには、<structname>test1</structname>テーブル全体を1行ごとにスキャンする必要があります。
...
例えば、検索ツリーを数層分検索するだけで済む可能性もあります。␞␞  </para>␞
␝  <para>␟   A similar approach is used in most non-fiction books:  terms and
   ...
   will be useful.␟ほとんどのノンフィクションの本で、同じような手法が使われています。
...
読者がよく調べそうな項目を予想するのが作者の仕事であるように、どのインデックスが実用的であるかを予測するのはデータベースプログラマの仕事です。␞␞  </para>␞
␝  <para>␟   The following command can be used to create an index on the
   <structfield>id</structfield> column, as discussed:␟上述のように<structfield>id</structfield>列にインデックスを作成する場合は、以下のようなコマンドが使用できます。␞␞<programlisting>␞
␝</programlisting>␟   The name <structname>test1_id_index</structname> can be chosen
   freely, but you should pick something that enables you to remember
   later what the index was for.␟<structname>test1_id_index</structname>というインデックス名には、何を選んでも構いませんが、そのインデックスを何のために作成したかを後で思い出せるような名前を選ぶべきです。␞␞  </para>␞
␝  <para>␟   To remove an index, use the <command>DROP INDEX</command> command.
   Indexes can be added to and removed from tables at any time.␟インデックスを削除するには、<command>DROP INDEX</command>コマンドを使用します。
テーブルのインデックスは、いつでも追加および削除できます。␞␞  </para>␞
␝  <para>␟   Once an index is created, no further intervention is required: the
   ...
   planner might choose <emphasis>not</emphasis> to use an index.␟いったんインデックスを作成すれば、それ以上の処理は必要はありません。
...
インデックスが使われているかどうか、およびプランナがインデックスを<emphasis>使わない</emphasis>と判断した状況および理由を調べる方法については、<xref linkend="performance-tips"/>を参照してください。␞␞  </para>␞
␝  <para>␟   Indexes can also benefit <command>UPDATE</command> and
   <command>DELETE</command> commands with search conditions.
   Indexes can moreover be used in join searches.  Thus,
   an index defined on a column that is part of a join condition can
   also significantly speed up queries with joins.␟インデックスは、<command>UPDATE</command>や<command>DELETE</command>コマンドの検索条件でも使用できます。
さらに、インデックスは結合問い合わせでも使用されます。
したがって、結合条件で記述されている列にインデックスを定義すれば、結合を伴った問い合わせにかかる時間もかなり短縮できます。␞␞  </para>␞
␝  <para>␟   In general, <productname>PostgreSQL</productname> indexes can be used
   to optimize queries that contain one or more <literal>WHERE</literal>
   or <literal>JOIN</literal> clauses of the form␟一般に、<productname>PostgreSQL</productname>のインデックスは、以下の形式の1つ以上の<literal>WHERE</literal>句または<literal>JOIN</literal>句を含む問い合わせを最適化するために使用できます。␞␞␞
␝␟   Here, the <replaceable>indexed-column</replaceable> is whatever
   ...
   table.␟ここで、<replaceable>indexed-column</replaceable>はインデックスが定義されている列または式であれば何でもかまいません。
<replaceable>indexable-operator</replaceable>は、インデックスの<firstterm>演算子クラス</firstterm>のメンバである演算子です。
（詳細は後述。）
また、<replaceable>comparison-value</replaceable>は揮発性でなく、インデックスのテーブルを参照しない式であれば何でもかまいません。␞␞  </para>␞
␝  <para>␟   In some cases the query planner can extract an indexable clause of
   this form from another SQL construct.  A simple example is that if
   the original clause was␟場合によっては、問い合わせプランナは、別のSQL構造からこの形式のインデックス可能な句を抽出できます。
単純な例は、元の句が以下の通りであり、␞␞␞
␝␟   then it can be flipped around into indexable form if the
   original <replaceable>operator</replaceable> has a commutator
   operator that is a member of the index's operator class.␟元の<replaceable>operator</replaceable>にインデックスの演算子クラスのメンバである交代演算子がある場合、インデックス可能な形式に切り替えることができます。␞␞  </para>␞
␝  <para>␟   Creating an index on a large table can take a long time.  By default,
   ...
   for more information see <xref linkend="sql-createindex-concurrently"/>.␟大規模テーブルに対するインデックス作成が長時間にわたる可能性があります。
...
<xref linkend="sql-createindex-concurrently"/>の情報を参照してください。␞␞  </para>␞
␝  <para>␟   After an index is created, the system has to keep it synchronized with the
   ...
   should be removed.␟インデックスが作成された後、システムでは、テーブルとインデックスとの間で常に同期を取っておく必要があります。
これにより、データ操作の処理にオーバーヘッドが加わります。
インデックスはまた、<link linkend="storage-hot">ヒープ専用タプル</link>の作成を防いでしまいます。
したがって、めったに使用されないインデックスや、まったく使用されなくなったインデックスは、削除しておいた方が良いでしょう。␞␞  </para>␞
␝ <sect1 id="indexes-types">␟  <title>Index Types</title>␟  <title>インデックスの種類</title>␞␞␞
␝   <indexterm>
    <primary>index</primary>
...
␝  <para>␟   <productname>PostgreSQL</productname> provides several index types:
   ...
   For example, to create a Hash index:␟<productname>PostgreSQL</productname>では、B-tree、Hash、GiST、SP-GiST、GIN、BRIN、そして<link linkend="bloom">ブルーム</link>拡張といった複数の種類のインデックスを使用可能です。
...
例えば、ハッシュインデックスを作成するには、次のようにします。␞␞<programlisting>␞
␝  <para>␟   B-trees can handle equality and range queries on data that can be sorted
   into some ordering.
   In particular, the <productname>PostgreSQL</productname> query planner
   will consider using a B-tree index whenever an indexed column is
   involved in a comparison using one of these operators:␟B-treeインデックスは、ある順番でソート可能なデータに対する等価性や範囲を問い合わせることを扱うことができます。
...
␝␟   Constructs equivalent to combinations of these operators, such as
   ...
   NULL</literal> condition on an index column can be used with a B-tree index.␟また、<literal>BETWEEN</literal>や<literal>IN</literal>などのこれらの演算子の組み合わせと等価な式もB-treeインデックス検索で実装することができます。
インデックスの付いた列に対する<literal>IS NULL</literal>や<literal>IS NOT NULL</literal>でもB-treeインデックスを使用することができます。␞␞  </para>␞
␝  <para>␟   The optimizer can also use a B-tree index for queries involving the
   ...
   upper/lower case conversion.␟オプティマイザは、パターンマッチ演算子<literal>LIKE</literal>、<literal>~</literal>を含む問い合わせでも、そのパターンが定数であり、先頭文字列を指定している<emphasis>のであれば</emphasis>B-treeインデックスを使用することができます。
...
なお、<literal>ILIKE</literal>と<literal>~*</literal>でもB-treeインデックスを使用することができますが、パターンが英字以外の文字、つまり、大文字小文字の違いの影響がない文字で始まる場合のみです。␞␞  </para>␞
␝   <indexterm>
    <primary>index</primary>
...
␝  <para>␟   B-tree indexes can also be used to retrieve data in sorted order.
   This is not always faster than a simple scan and sort, but it is
   often helpful.␟B-treeインデックスをソートされた順序でデータを受けとるために使用することもできます。
これは常に単純なスキャンとソート処理より高速になるものではありませんが、よく役に立つことがあります。␞␞  </para>␞
␝   <indexterm>
    <primary>index</primary>
...
␝  <para>␟   GiST indexes are not a single kind of index, but rather an infrastructure
   ...
   queries using these operators:␟GiSTインデックスは単一種類のインデックスではなく、多くの異なるインデックス戦略を実装することができる基盤です。
...
␝␟   (See <xref linkend="functions-geometry"/> for the meaning of
   ...
   projects.  For more information see <xref linkend="gist"/>.␟（これらの演算子の意味については<xref linkend="functions-geometry"/>を参照してください。）
...
詳細は<xref linkend="gist"/>を参照してください。␞␞  </para>␞
␝  <para>␟   GiST indexes are also capable of optimizing <quote>nearest-neighbor</quote>
   searches, such as␟GiSTインデックスは以下のような<quote>最近傍</quote>検索を最適化する機能も持ちます。␞␞<programlisting><![CDATA[␞
␝   <indexterm>
    <primary>index</primary>
...
␝</programlisting>␟   which finds the ten places closest to a given target point.  The ability
   to do this is again dependent on the particular operator class being used.
   In <xref linkend="gist-builtin-opclasses-table"/>, operators that can be
   used in this way are listed in the column <quote>Ordering Operators</quote>.␟これは指定された対象地点に最も近い１０箇所を見つけ出します。
この場合も、これができるかどうかは使用される特定の演算子クラスに依存します。
このように利用できる演算子は<xref linkend="gist-builtin-opclasses-table"/>の<quote>順序付け演算子</quote>列に表示されています。␞␞  </para>␞
␝  <para>␟   SP-GiST indexes, like GiST indexes, offer an infrastructure that supports
   ...
   queries using these operators:␟SP-GiSTインデックスは、GiSTインデックスと同様に様々な種類の検索を支援する基盤を提供します。
...
␝␟   (See <xref linkend="functions-geometry"/> for the meaning of
   ...
   For more information see <xref linkend="spgist"/>.␟（これらの演算子の意味については<xref linkend="functions-geometry"/>を参照してください。）
標準配布物に含まれるSP-GiST演算子クラスは<xref linkend="spgist-builtin-opclasses-table"/>に記載されています。
詳細は <xref linkend="spgist"/>を参照してください。␞␞  </para>␞
␝   <indexterm>
    <primary>index</primary>
...
␝  <para>␟   Like GiST, SP-GiST supports <quote>nearest-neighbor</quote> searches.
   ...
   column in <xref linkend="spgist-builtin-opclasses-table"/>.␟GiSTと同様に、SP-GiSTは<quote>最近傍</quote>検索をサポートします。
距離の順序付けをサポートするSP-GiST演算子クラスの場合、対応する演算子は<xref linkend="spgist-builtin-opclasses-table"/>の<quote>順序付け演算子</quote>列に一覧表示されます。␞␞  </para>␞
␝  <para>␟   GIN indexes are <quote>inverted indexes</quote> which are appropriate for
   ...
   component values.␟GINは<quote>転置インデックス</quote>であり、配列などのように複数の要素を持つデータ値に適しています。
転置インデックスは各要素値に対して別々のエントリを持っており、特定の要素値の存在について検査する問い合わせを効率的に処理できます。␞␞  </para>␞
␝  <para>␟   Like GiST and SP-GiST, GIN can support
   many different user-defined indexing strategies, and the particular
   operators with which a GIN index can be used vary depending on the
   indexing strategy.
   As an example, the standard distribution of
   <productname>PostgreSQL</productname> includes a GIN operator class
   for arrays, which supports indexed queries using these operators:␟GiSTやSP-GiST同様、GINも多くの異なるユーザ定義のインデックス戦略を持つことができ、GINが使用できる具体的な演算子はインデックス戦略によって変化します。
...
␝␟   (See <xref linkend="functions-array"/> for the meaning of
   ...
   classes are available in the <literal>contrib</literal> collection or as separate
   projects.  For more information see <xref linkend="gin"/>.␟（これらの演算子の意味については<xref linkend="functions-array"/>を参照してください。）
...
詳細は<xref linkend="gin"/>を参照してください。␞␞  </para>␞
␝  <para>␟   BRIN indexes (a shorthand for Block Range INdexes) store summaries about
   ...
   using these operators:␟BRINインデックス(ブロックレンジインデックス(Block Range INdex)を縮めたものです)はテーブルの連続的な物理ブロックの範囲に格納された値についての要約を格納します。
...
␝␟   The BRIN operator classes included in the standard distribution are
   ...
   For more information see <xref linkend="brin"/>.␟標準配布物に含まれるBRIN演算子クラスは<xref linkend="brin-builtin-opclasses-table"/>に記載されています。
詳細は<xref linkend="brin"/>を参照してください。␞␞  </para>␞
...
␝ <sect1 id="indexes-multicolumn">␟  <title>Multicolumn Indexes</title>␟  <title>複数列インデックス</title>␞␞␞
␝  <para>␟   An index can be defined on more than one column of a table.  For example, if
   you have a table of this form:␟インデックスは、テーブルの2つ以上の列に定義することができます。
例えば、以下のようなテーブルがあるとします。␞␞<programlisting>␞
␝</programlisting>␟   (say, you keep your <filename class="directory">/dev</filename>
   directory in a database...) and you frequently issue queries like:␟（例えば、<filename class="directory">/dev</filename>ディレクトリの内容をデータベースに保持していて）頻繁に下記のような問い合わせを発行するとします。␞␞<programlisting>␞
␝</programlisting>␟   then it might be appropriate to define an index on the columns
   <structfield>major</structfield> and
   <structfield>minor</structfield> together, e.g.:␟このような場合、<structfield>major</structfield>および<structfield>minor</structfield>という２つの列に1つのインデックスを定義する方が適切かもしれません。␞␞<programlisting>␞
␝  <para>␟   Currently, only the B-tree, GiST, GIN, and BRIN index types support
   ...
   file <filename>pg_config_manual.h</filename>.)␟現在、B-tree、GiST、GINおよびBRINインデックス型でのみ、複数キー列インデックスをサポートしています。
...
<filename>pg_config_manual.h</filename>ファイルを参照してください。）␞␞  </para>␞
␝  <para>␟   A multicolumn B-tree index can be used with query conditions that
   ...
   the planner would prefer a sequential table scan over using the index.␟複数列に対するB-treeインデックスをインデックス対象列の任意の部分集合を含む問い合わせ条件で使用することができます。
...
しかし、インデックス全体がスキャンされますので、ほとんどの場合、プランナはインデックスの使用よりもシーケンシャルテーブルスキャンを選択します。␞␞  </para>␞
␝  <para>␟   A multicolumn GiST index can be used with query conditions that
   ...
   there are many distinct values in additional columns.␟複数列GiSTインデックスは、インデックス対象列の任意の部分集合を含む問い合わせ条件で使用することができます。
他の列に対する条件は、インデックスで返される項目を制限します。
しかし、先頭列に対する条件が、インデックスのスキャン量を決定するもっとも重要なものです。
先頭列の個別値がわずかな場合、他の列が多くの個別値を持っていたとしても、相対的にGiSTインデックスは非効率的になります。␞␞  </para>␞
␝  <para>␟   A multicolumn GIN index can be used with query conditions that
   involve any subset of the index's columns. Unlike B-tree or GiST,
   index search effectiveness is the same regardless of which index column(s)
   the query conditions use.␟複数列GINインデックスは、インデックス対象列の任意の部分集合を含む問い合わせ条件で使用することができます。
B-treeやGiSTと異なり、インデックス検索の効果はどのインデックス列が問い合わせ条件で使用されているかに関係なく同じです。␞␞  </para>␞
␝  <para>␟   A multicolumn BRIN index can be used with query conditions that
   ...
   a different <literal>pages_per_range</literal> storage parameter.␟複数列BRINインデックスは、インデックス対象列の任意の部分集合を含む問い合わせ条件で使用することができます。
GINと同様に、またB-treeやGiSTとは異なり、インデックス検索の効果はどのインデックス列が問い合わせ条件で使用されているかに関係なく同じです。
一つのテーブルに対して複数列BRINインデックスを一つ持つ代わりに複数のBRINインデックスを持つ唯一の理由は、異なる<literal>pages_per_range</literal>ストレージパラメータを持つためです。␞␞  </para>␞
␝  <para>␟   Of course, each column must be used with operators appropriate to the index
   type; clauses that involve other operators will not be considered.␟当然ながら、インデックス種類に対して適切な演算子を各列に使用しなければなりません。
他の演算子を含む句は考慮されません。␞␞  </para>␞
␝  <para>␟   Multicolumn indexes should be used sparingly.  In most situations,
   ...
   merits of different index configurations.␟複数列インデックスは慎重に使用する必要があります。
多くの場合、単一列のインデックスで十分であり、また、その方がディスク領域と時間を節約できます。
テーブルの使用方法が極端に様式化されていない限り、4つ以上の列を使用しているインデックスは、不適切である可能性が高いでしょう。
異なるインデックス構成の利点に関するこの他の説明について<xref linkend="indexes-bitmap-scans"/>および<xref linkend="indexes-index-only-scans"/>も参照してください。␞␞  </para>␞
...
␝ <sect1 id="indexes-ordering">␟  <title>Indexes and <literal>ORDER BY</literal></title>␟  <title>インデックスと<literal>ORDER BY</literal></title>␞␞␞
␝  <para>␟   In addition to simply finding the rows to be returned by a query,
   ...
   matching rows in an unspecified, implementation-dependent order.␟単に問い合わせによって返される行を見つけ出すだけではなく、インデックスは、その行を指定した順番で取り出すことができます。
...
他の種類のインデックスでは指定なし、または、実装固有の順序でマッチした行を返します。␞␞  </para>␞
␝  <para>␟   The planner will consider satisfying an <literal>ORDER BY</literal> specification
   ...
   rows can be retrieved directly, without scanning the remainder at all.␟プランナは、<literal>ORDER BY</literal>指定を満足させるために、指定に一致し利用可能なインデックスでスキャンするか、または、テーブルを物理的な順番でスキャンし明示的なソートを行うかを考慮します。
...
しかし、もし<literal>ORDER BY</literal>に合うインデックスが存在すれば、残りの部分をスキャンすることなく、先頭の<replaceable>n</replaceable>行の取り出しを直接行うことができます。␞␞  </para>␞
␝  <para>␟   By default, B-tree indexes store their entries in ascending order
   ...
   <literal>NULLS FIRST</literal> is the default for <literal>ORDER BY DESC</literal>).␟デフォルトでは、B-treeインデックスは項目を昇順で格納し、NULLを最後に格納します。
...
また、インデックスを後方方向にスキャンすることもでき、この場合、<literal>ORDER BY x DESC</literal>（より冗長にいえば<literal>ORDER BY x DESC NULLS FIRST</literal>。<literal>NULLS FIRST</literal>が<literal>ORDER BY DESC</literal>のデフォルトだからです。）を満たす出力を生成します。␞␞  </para>␞
␝  <para>␟   You can adjust the ordering of a B-tree index by including the
   options <literal>ASC</literal>, <literal>DESC</literal>, <literal>NULLS FIRST</literal>,
   and/or <literal>NULLS LAST</literal> when creating the index; for example:␟インデックスを作成する時に、以下のように<literal>ASC</literal>、<literal>DESC</literal>、<literal>NULLS FIRST</literal>、<literal>NULLS LAST</literal>オプションを組み合わせて指定することにより、B-treeインデックスの順序を調整することができます。␞␞<programlisting>␞
␝</programlisting>␟   An index stored in ascending order with nulls first can satisfy
   ...
   it is scanned in.␟昇順かつNULL先頭という順で格納されたインデックスは、スキャンされる方向に依存して<literal>ORDER BY x ASC NULLS FIRST</literal>または<literal>ORDER BY x DESC NULLS LAST</literal>を満たすことができます。␞␞  </para>␞
␝  <para>␟   You might wonder why bother providing all four options, when two
   ...
   as <literal>(x ASC, y DESC)</literal> or <literal>(x DESC, y ASC)</literal>.␟4つの全方向を提供する理由が何か、後方方向へのスキャンの可能性があることを考慮した2方向で、すべての種類の<literal>ORDER BY</literal>を網羅できるのではないかと疑問を持つかもしれません。
...
しかし、インデックスが<literal>(x ASC, y DESC)</literal>または<literal>(x DESC, y ASC)</literal>として定義されていれば、取り出すことができます。␞␞  </para>␞
␝  <para>␟   Obviously, indexes with non-default sort orderings are a fairly
   ...
   sort ordering.␟明確なことですが、デフォルト以外のソート順を持つインデックスはかなり特殊な機能です。
しかし、特定の問い合わせにおいては恐ろしいほどの速度を向上させることがあります。
こうしたインデックスを維持する価値があるかどうかは、特殊なソート順を要求する問い合わせを使用する頻度に依存します。␞␞  </para>␞
...
␝ <sect1 id="indexes-bitmap-scans">␟  <title>Combining Multiple Indexes</title>␟  <title>複数のインデックスの組み合わせ</title>␞␞␞
␝  <para>␟   A single index scan can only use query clauses that use the index's
   ...
   directly use the index.␟単一のインデックススキャンは、インデックスの列をその演算子クラスの演算子で使用する問い合わせ句と、それを<literal>AND</literal>結合したものでのみ使用されます。
例えば、<literal>(a, b)</literal>というインデックスと<literal>WHERE a = 5 AND b = 6</literal>という問い合わせでは、インデックスが使用されます。
しかし、<literal>WHERE a = 5 OR b = 6</literal>のような問い合わせではインデックスは直接使用されません。␞␞  </para>␞
␝  <para>␟   Fortunately,
   ...
   the index results to identify the result rows.␟幸いにも、<productname>PostgreSQL</productname>は、単一のインデックススキャンでは実装できない場合を扱うために、複数のインデックス（同じインデックスの複数回使用を含む）を組み合わせる機能を持ちます。
...
<literal>WHERE x = 5 AND y = 6</literal>のような問い合わせに対して取り得る実装は、適切な問い合わせ句で各インデックスを使用し、インデックスの結果をANDでまとめ、結果行を識別することです。␞␞  </para>␞
␝  <para>␟   To combine multiple indexes, the system scans each needed index and
   ...
   used as well.␟複数のインデックスを組み合わせるために、システムは必要なインデックスそれぞれをスキャンし、インデックス条件に適合するものと報告されたテーブル行の位置を与えるためにメモリ上に<firstterm>ビットマップ</firstterm>を準備します。
...
この理由、および、追加のインデックススキャンそれぞれのために余計な時間が加わることから、プランナは追加のインデックスが同様に使用できる場合であっても、単純なインデックススキャンを選択することがあります。␞␞  </para>␞
␝  <para>␟   In all but the simplest applications, there are various combinations of
   ...
   best match the common types.␟もっとも単純なアプリケーション以外のほとんどすべてのアプリケーションでは、インデックスの有用な組み合わせはいろいろあります。
...
問い合わせの中の1つの頻度が他よりも少なければ、おそらく良く使用される種類にもっとも合うように2つだけインデックスを作成した方がよいでしょう。␞␞  </para>␞
...
␝ <sect1 id="indexes-unique">␟  <title>Unique Indexes</title>␟  <title>一意インデックス</title>␞␞␞
␝  <para>␟   Indexes can also be used to enforce uniqueness of a column's value,
   or the uniqueness of the combined values of more than one column.␟インデックスは、列値の一意性や、複数列を組み合わせた値の一意性を強制するためにも使用できます。␞␞<synopsis>␞
␝</synopsis>␟   Currently, only B-tree indexes can be declared unique.␟現在、一意インデックスとして宣言できるのはB-treeインデックスのみです。␞␞  </para>␞
␝  <para>␟   When an index is declared unique, multiple table rows with equal
   ...
   cases where all indexed columns are equal in multiple rows.␟一意インデックスが宣言された場合、同じインデックス値を有する複数のテーブル行は許されなくなります。
デフォルトでは、一意列のNULL値は同じ値とはみなされず、列に複数のNULLが許可されます。
<literal>NULLS NOT DISTINCT</literal>オプションはこれを変更し、インデックスでNULLが同等として扱われるようにします。
複数列の一意インデックスは、インデックス列の全てが複数の行で同一の場合のみ拒絶されます。␞␞  </para>␞
␝  <para>␟   <productname>PostgreSQL</productname> automatically creates a unique
   ...
   that enforces the constraint.␟<productname>PostgreSQL</productname>では、テーブルに一意性制約または主キーが定義されると、自動的に一意インデックスを作成します。
このインデックスが、主キーや一意性制約（適切ならば複数列のインデックスで）となる列に対して作成され、この制約を強制する機構となります。␞␞  </para>␞
␝   <para>␟    There's no need to manually
    create indexes on unique columns; doing so would just duplicate
    the automatically-created index.␟手作業で一意列に対しインデックスを作成する必要はありません。
これは、単に自動作成されるインデックスを二重にするだけです。␞␞   </para>␞
...
␝ <sect1 id="indexes-expressional">␟  <title>Indexes on Expressions</title>␟  <title>式に対するインデックス</title>␞␞␞
␝  <para>␟   An index column need not be just a column of the underlying table,
   ...
   access to tables based on the results of computations.␟インデックス列は、基礎をなすテーブルにある列である必要はなく、そのテーブルの１つ以上の列から計算される関数やスカラ式とすることもできます。
この機能は、ある演算結果に基づいた高速テーブルアクセスを行う時に有用です。␞␞  </para>␞
␝  <para>␟   For example, a common way to do case-insensitive comparisons is to
   use the <function>lower</function> function:␟例えば、大文字小文字を区別せずに比較するための一般的な方法である、<function>lower</function>関数での使用例を以下に示します。␞␞<programlisting>␞
␝</programlisting>␟   This query can use an index if one has been
   defined on the result of the <literal>lower(col1)</literal>
   function:␟<literal>lower(col1)</literal>関数の結果にインデックスが定義されていれば、この問い合わせでインデックスを使用することができます。␞␞<programlisting>␞
␝  <para>␟   If we were to declare this index <literal>UNIQUE</literal>, it would prevent
   ...
   are not definable as simple unique constraints.␟このインデックスを<literal>UNIQUE</literal>と宣言したとすると、<literal>col1</literal>の値が同一となる行だけでなく、<literal>col1</literal>の大文字小文字だけが違う行の生成を防ぐことになります。
したがって、式に対するインデックスを使用して、単なる一意性制約では定義できないような制約を強制することができます。␞␞  </para>␞
␝  <para>␟   As another example, if one often does queries like:␟別の例として、以下のような問い合わせが頻繁に行われる場合を考えます。␞␞<programlisting>␞
␝</programlisting>␟   then it might be worth creating an index like this:␟この場合、以下のようなインデックスを作成する価値があるでしょう。␞␞<programlisting>␞
␝  <para>␟   The syntax of the <command>CREATE INDEX</command> command normally requires
   ...
   a function call, as in the first example.␟２番目の例に示すように<command>CREATE INDEX</command>コマンドの構文は通常、インデックス式を括弧で括る必要があります。
最初の例のように、式が単なる関数呼び出しの場合には括弧を省略することができます。␞␞  </para>␞
␝  <para>␟   Index expressions are relatively expensive to maintain, because the
   ...
   is more important than insertion and update speed.␟派生した式が、行挿入と<link linkend="storage-hot">非HOT更新</link>の度に計算されなければならないので、インデックス式は相対的に見て維持が高価です。
...
したがって、式に対するインデックスは取り出し速度が挿入、更新速度より重要な場合に有用です。␞␞  </para>␞
...
␝ <sect1 id="indexes-partial">␟  <title>Partial Indexes</title>␟  <title>部分インデックス</title>␞␞␞
␝  <para>␟   A <firstterm>partial index</firstterm> is an index built over a
   ...
   feature, but there are several situations in which they are useful.␟<firstterm>部分インデックス</firstterm>とは、テーブルの部分集合に構築されるインデックスです。
部分集合は、（部分インデックスの<firstterm>述語</firstterm>と呼ばれる）条件式で定義されます。
部分インデックスには、その述語を満たすテーブル行のみに対するエントリが含まれます。
部分インデックスは特別な機能です。
しかし、これらが有用となる状況が複数あります。␞␞  </para>␞
␝  <para>␟   One major reason for using a partial index is to avoid indexing common
   ...
   possible application of this idea.␟部分インデックスを利用する主な目的は、頻出値に対してインデックスを作成しないようにすることです。
...
<xref linkend="indexes-partial-ex1"/>にこの概念に基づいた用例を示します。␞␞  </para>␞
␝  <example id="indexes-partial-ex1">␟   <title>Setting up a Partial Index to Exclude Common Values</title>␟   <title>頻出値を除外するための部分インデックスの作成</title>␞␞␞
␝   <para>␟    Suppose you are storing web server access logs in a database.
    ...
    organization's subnet.␟ウェブサーバのアクセスログをデータベースに格納しているとします。
多くのアクセスは、社内のIPアドレスの範囲内から発信されています。
しかし、範囲外のアドレス（例えば、社員がダイアルアップ接続している場所）からの発信もあります。
主に範囲外からのアクセスをIPアドレスで検索する場合、社内のサブネットに該当するIPアドレスの範囲にインデックスを作成する必要はないでしょう。␞␞   </para>␞
␝   <para>␟    Assume a table like this:␟以下のようなテーブルがあると想定します。␞␞<programlisting>␞
␝   <para>␟    To create a partial index that suits our example, use a command
    such as this:␟この例に適する部分インデックスを作成するには、以下のようなコマンドを使用します。␞␞<programlisting>␞
␝   <para>␟    A typical query that can use this index would be:␟このインデックスを使用できる問い合わせの典型的な例は、以下のようなものです。␞␞<programlisting>␞
␝</programlisting>␟    Here the query's IP address is covered by the partial index.  The
    following query cannot use the partial index, as it uses an IP address
    that is excluded from the index:␟この問い合わせのIPアドレスは部分インデックスでカバーされています。
以下の問い合わせは、インデックスから除外されているIPアドレスを使用しているので、部分インデックスを使用できません。␞␞<programlisting>␞
␝   <para>␟    Observe that this kind of partial index requires that the common
    ...
    maintenance effort.␟このような部分インデックスを使用するには、あらかじめ頻出値が何であるかを知っている必要があることに注意してください。
値の分布が変わらない場合に、このような部分インデックスが最善です。
データの分布が新しくなった場合はインデックスの再作成によって調整できますが、これはメンテナンス作業を増やしてしまいます。␞␞   </para>␞
␝  <para>␟   Another possible use for a partial index is to exclude values from the
   index that the
   ...
   scenario will require a lot of care and experimentation.␟部分インデックスを使用する有効な他の方法としては、一般的な問い合わせに必要のない値をインデックスから取り除くことです。
...
以上のことから明白なように、このようなケースで部分インデックスを作成する際は、細心の注意を払い、十分な検証を行う必要があります。␞␞  </para>␞
␝  <example id="indexes-partial-ex2">␟   <title>Setting up a Partial Index to Exclude Uninteresting Values</title>␟   <title>必要のない値を除外するための部分インデックスの作成</title>␞␞␞
␝   <para>␟    If you have a table that contains both billed and unbilled orders,
    ...
    command to create the index would look like this:␟請求済み注文書および未請求注文書からなる、１つのテーブルがあるとします。
そして、未請求注文書の方がテーブル全体に対する割合が小さく、かつその部分へのアクセス数が最も多かったとします。
このような場合、未請求の行のみにインデックスを作成することにより、性能を向上させることができます。
インデックスの作成には、以下のようなコマンドを使用します。␞␞<programlisting>␞
␝   <para>␟    A possible query to use this index would be:␟このインデックスを使用する問い合わせの例としては、次のものが考えられます。␞␞<programlisting>␞
␝</programlisting>␟    However, the index can also be used in queries that do not involve
    <structfield>order_nr</structfield> at all, e.g.:␟しかし、このインデックスは、<structfield>order_nr</structfield>をまったく使用しない問い合わせでも使用することができます。
以下は、その例です。␞␞<programlisting>␞
␝</programlisting>␟    This is not as efficient as a partial index on the
    ...
    could be a win.␟この問い合わせでは、システムがインデックス全体を検索する必要があるため、<structfield>amount</structfield>列に部分インデックスを作成した場合ほど効率は良くありません。
しかし、未請求注文書データが比較的少ない場合は、この部分インデックスを未請求注文書を検出するためだけに使用した方が効率が良い可能性があります。␞␞   </para>␞
␝   <para>␟    Note that this query cannot use this index:␟以下の問い合わせでは、このインデックスを使用できないことに注意してください。␞␞<programlisting>␞
␝</programlisting>␟    The order 3501 might be among the billed or unbilled
    orders.␟注文番号3501は請求済みかもしれませんし、未請求かもしれないからです。␞␞   </para>␞
␝  <para>␟   <xref linkend="indexes-partial-ex2"/> also illustrates that the
   ...
   <quote>x &lt; 2</quote> for all possible values of the parameter.␟<xref linkend="indexes-partial-ex2"/>でもわかるように、インデックスが付けられた列名と、述語で使用されている列名は、一致している必要はありません。
...
たとえば、<quote>x &lt; ?</quote>と指定されたパラメータを持つ、プリペアド問い合わせでは、どのようなパラメータ値であっても<quote>x &lt; 2</quote>を表しません。␞␞  </para>␞
␝  <para>␟   A third possible use for partial indexes does not require the
   ...
   those that do not.␟部分インデックスの考えられる３つ目の用法では、問い合わせでインデックスをまったく使用しません。
この考え方は、テーブルの部分集合に一意インデックスを作成するというものです。
<xref linkend="indexes-partial-ex3"/>を参照してください。
これにより、インデックスの述語を満たさない行を制約することなく、その述語を満たす行での一意性を強制します。␞␞  </para>␞
␝  <example id="indexes-partial-ex3">␟   <title>Setting up a Partial Unique Index</title>␟   <title>一意な部分インデックスの作成</title>␞␞␞
␝   <para>␟    Suppose that we have a table describing test outcomes.  We wish
    ...
    <quote>unsuccessful</quote> entries.  Here is one way to do it:␟テストの結果が格納されているテーブルがあるとします。
与えられた件名（subject）および対象（target）の組み合わせに対して、<quote>成功</quote>のエントリが確実に1つしかないようにします。
<quote>失敗</quote>のエントリは、複数あっても構いません。
以下に、これを実行する一例を示します。␞␞<programlisting>␞
␝</programlisting>␟    This is a particularly efficient approach when there are few
    ...
    with an <literal>IS NULL</literal> restriction.␟これは、成功するテストが少なく、失敗するテストが多い場合に特に有効な手法です。
また、<literal>IS NULL</literal>制限を使用して一意の部分インデックスを作成することにより、ひとつの列にNULL値をひとつのみ許可できます。␞␞   </para>␞
␝  <para>␟   Finally, a partial index can also be used to override the system's
   ...
   for a bug report.␟最後に、部分インデックスは、システムの問い合わせ計画の選択を変更するためにも使用できます。
...
まったく不適切な計画を選択するようであれば、バグとして報告してください。␞␞  </para>␞
␝  <para>␟   Keep in mind that setting up a partial index indicates that you
   ...
   linkend="indexes-partial-ex4"/>.␟部分インデックスを作成するには、少なくとも問い合わせプランナと同等の知識を持っていること、特に、インデックスが有益となる状況を理解している必要があることに留意してください。
...
<xref linkend="indexes-partial-ex4"/>のように、かなり逆効果な場合があります。␞␞  </para>␞
␝  <example id="indexes-partial-ex4">␟   <title>Do Not Use Partial Indexes as a Substitute for Partitioning</title>␟   <title>パーティショニングの代わりに部分インデックスを使用しない</title>␞␞␞
␝   <para>␟    You might be tempted to create a large set of non-overlapping partial
    indexes, for example␟例えば、重複しない部分インデックスの大きなセットを作りたいと思うかもしれません。␞␞␞
␝␟    This is a bad idea!  Almost certainly, you'll be better off with a
    single non-partial index, declared like␟これは良くないアイディアです！ほとんどの場合、以下のように宣言された、部分的でない単一のインデックスを使用する方が良いでしょう。␞␞␞
␝␟    (Put the category column first, for the reasons described in
    ...
    query.␟（<xref linkend="indexes-multicolumn"/>で説明されている理由から、最初にcategory列を指定します。）
この大きなインデックスでの検索は、小さなインデックスでの検索よりも2,3ツリーレベルを下に移動する必要がありますが、部分インデックスの適切な1つを選択するためにプランナがおこなう作業よりも、ほぼ確実にコストが削減できます。
この問題の核心は、システムが部分インデックス間の関係を理解していないことと、現在の問い合わせに適用出来るかどうかそれぞれ苦労してテストすることです。␞␞   </para>␞
␝   <para>␟    If your table is large enough that a single index really is a bad idea,
    ...
    far better performance is possible.␟テーブルが非常に大きくて、単一のインデックスが本当に悪いアイデアである場合は、代わりにパーティショニングを使用する必要があります（<xref linkend="ddl-partitioning"/>を参照してください）。
このメカニズムにより、テーブルとインデックスが重複していないことが、システムで認識されるため、パフォーマンスが大幅に向上します。␞␞   </para>␞
␝  <para>␟   More information about partial indexes can be found in <xref
   linkend="ston89b"/>, <xref linkend="olson93"/>, and <xref
   linkend="seshadri95"/>.␟部分インデックスの詳細については、<xref linkend="ston89b"/>、<xref linkend="olson93"/>、および<xref linkend="seshadri95"/>を参照してください。␞␞  </para>␞
...
␝ <sect1 id="indexes-index-only-scans">␟  <title>Index-Only Scans and Covering Indexes</title>␟  <title>インデックスオンリースキャンとカバリングインデックス</title>␞␞␞
␝  <para>␟   All indexes in <productname>PostgreSQL</productname>
   ...
   so far.)␟<productname>PostgreSQL</productname>におけるすべてのインデックスは<firstterm>二次的な</firstterm>インデックス、つまり各インデックスはテーブルの主要なデータ領域（<productname>PostgreSQL</productname>の用語ではテーブルの<firstterm>ヒープ</firstterm>と呼ばれます）とは別に格納されています。
...
（<xref linkend="indexes-bitmap-scans"/>で説明したように、ビットマップインデックスはヒープアクセスをソートした順で行うことでこのコストを緩和しようとするものですが、それはある程度までしかできません。）␞␞  </para>␞
␝  <para>␟   To solve this performance problem, <productname>PostgreSQL</productname>
   ...
   when this method can be used:␟このパフォーマンス問題を解決するため、<productname>PostgreSQL</productname>は<firstterm>インデックスオンリースキャン</firstterm>をサポートします。
...
␝     <para>␟      The index type must support index-only scans.  B-tree indexes always
      ...
      original data value.␟インデックスの種類がインデックスオンリースキャンをサポートしている必要があります。
...
その反例として、GINインデックスでは、各インデックスエントリが通常は元のデータ値の一部しか保持していないため、インデックスオンリースキャンをサポートすることができません。␞␞     </para>␞
␝     <para>␟      The query must reference only columns stored in the index.  For
      example, given an index on columns <literal>x</literal>
      and <literal>y</literal> of a table that also has a
      column <literal>z</literal>, these queries could use index-only scans:␟問い合わせはインデックスに格納されている列だけを参照しなければなりません。
例えばテーブルの列<literal>x</literal>と<literal>y</literal>にインデックスがあり、そのテーブルにはさらに列<literal>z</literal>がある場合、次の問い合わせはインデックスオンリースキャンを使用できます。␞␞<programlisting>␞
␝</programlisting>␟      but these queries could not:␟しかし、以下の問い合わせはインデックスオンリースキャンを使用できません。␞␞<programlisting>␞
␝</programlisting>␟      (Expression indexes and partial indexes complicate this rule,
      as discussed below.)␟（以下で説明するように、式インデックスや部分インデックスは、この規則を複雑にします。）␞␞     </para>␞
␝  <para>␟   If these two fundamental requirements are met, then all the data values
   ...
   cached in memory all the time.␟この2つの基本的な要件が満たされるなら、問い合わせで要求されるすべてのデータ値はインデックスから利用できるので、インデックスオンリースキャンが物理的に可能になります。
...
ほとんどの状況では、可視性マップは常にメモリ内にキャッシュされて残っています。␞␞  </para>␞
␝   <indexterm>
...
␝  <para>␟   In short, while an index-only scan is possible given the two fundamental
   ...
   make this type of scan very useful in practice.␟要するに、2つの基本的条件が満たされていればインデックスオンリースキャンが可能ですが、テーブルのヒープページのかなりの部分に対し、その全可視のビットがセットされている場合にのみ、性能が向上します。
しかし大部分の行が変化しないテーブルは一般的であり、現実にはこの種のスキャンは非常に有効です。␞␞  </para>␞
␝   </indexterm>␟   To make effective use of the index-only scan feature, you might choose to
   ...
   queries like␟インデックスオンリースキャンの機能を有効に利用するため、<firstterm>カバリングインデックス</firstterm>の作成を選択できます。
...
例えば、次のような問い合わせをよく実行する場合を考えます。␞␞<programlisting>␞
␝</programlisting>␟   the traditional approach to speeding up such queries would be to create
   an index on <literal>x</literal> only.  However, an index defined as␟このような問い合わせを高速化する伝統的な手法は、<literal>x</literal>のみにインデックスを作成することです。
しかし、次のようなインデックス定義では、␞␞<programlisting>␞
␝</programlisting>␟   could handle these queries as index-only scans,
   because <literal>y</literal> can be obtained from the index without
   visiting the heap.␟<literal>y</literal>はヒープにアクセスしなくてもインデックスから取得できるため、この問い合わせをインデックスオンリースキャンとして処理できます。␞␞  </para>␞
␝  <para>␟   Because column <literal>y</literal> is not part of the index's search
   ...
   machinery.  Also, if the index is a unique index, that is␟<literal>y</literal>列はインデックスの検索キーの一部ではないため、インデックスが処理できるデータ型である必要はありません。
単にインデックスに格納されているだけで、インデックス機構によって解釈されることはありません。
また、インデックスが一意インデックスの場合は、␞␞<programlisting>␞
␝</programlisting>␟   the uniqueness condition applies to just column <literal>x</literal>,
   ...
   this.)␟<literal>x</literal>列のみに一意性条件が適用されます。
<literal>x</literal>と<literal>y</literal>の組み合わせではありません。
（<literal>INCLUDE</literal>句は、インデックスを設定するための代替構文を提供する<literal>UNIQUE</literal>や<literal>PRIMARY KEY</literal>の制約として書くことも出来ます。）␞␞  </para>␞
␝  <para>␟   It's wise to be conservative about adding non-key payload columns to an
   ...
   support included columns.␟キー以外のペイロード列、特に幅の広い列をインデックスに追加することについては慎重になることが賢明です。
...
他の制限は、式を列に含めることは、現在のところサポートされていません。また、列を含めるサポートは現在のところB-treeとGiSTとSP-GiSTインデックスのみサポートされています。␞␞  </para>␞
␝  <para>␟   Before <productname>PostgreSQL</productname> had
   ...
   that is writing␟<literal>INCLUDE</literal>機能がない以前の<productname>PostgreSQL</productname>では、ペイロード列を通常のインデックス列としてカバリングインデックスを作成することがありました。␞␞<programlisting>␞
␝</programlisting>␟   even though they had no intention of ever using <literal>y</literal> as
   ...
   enforce uniqueness on the key column(s).␟これは、<literal>y</literal>を<literal>WHERE</literal>句の一部で使用するつもりがなかったとしても書いています。
余分な列が末尾の列である限り、これはうまく機能します。
それらを先頭側の列にすることは、<xref linkend="indexes-multicolumn"/>で説明されている理由から賢明ではありません。
しかし、この方法では、キー列に一意性を強制するインデックスがサポートされません。␞␞  </para>␞
␝  <para>␟   <firstterm>Suffix truncation</firstterm> always removes non-key
   ...
   small.␟<firstterm>末尾消去</firstterm>は、常に上位のB-treeレベルから非キーの列を削除します。
...
ただし、ペイロード列を非キー列として明示的に定義すると<emphasis>確実に</emphasis>上位レベルのタプルが小さくなります。␞␞  </para>␞
␝  <para>␟   In principle, index-only scans can be used with expression indexes.
   For example, given an index on <literal>f(x)</literal>
   where <literal>x</literal> is a table column, it should be possible to
   execute␟原則として、インデックスオンリースキャンは式インデックスでも使うことができます。
例えば、<literal>x</literal>がテーブルの列で、<literal>f(x)</literal>上にインデックスがある場合、次の問い合わせをインデックスオンリースキャンとして実行できるはずです。␞␞<programlisting>␞
␝</programlisting>␟   as an index-only scan; and this is very attractive
   ...
   adding <literal>x</literal> as an included column, for example␟そして、関数<literal>f()</literal>の計算が高価なら、この方法は非常に魅力的です。
...
␝</programlisting>␟   An additional caveat, if the goal is to avoid
   ...
   versions of <productname>PostgreSQL</productname>.␟目的が<literal>f(x)</literal>の再計算を避けることの場合、さらなる注意として、プランナはインデックス可能な<literal>WHERE</literal>句にない<literal>f(x)</literal>の使用を必ずしもインデックス列とマッチしないという事があります。
上記のような単純な問い合わせの場合は通常は正しく処理できるでしょうが、結合を含む問い合わせでは駄目でしょう。
これらの欠点は<productname>PostgreSQL</productname>の将来のバージョンで解決されるかもしれません。␞␞  </para>␞
␝  <para>␟   Partial indexes also have interesting interactions with index-only scans.
   Consider the partial index shown in <xref linkend="indexes-partial-ex3"/>:␟部分インデックスもインデックスオンリースキャンとの間に興味深い関係があります。
<xref linkend="indexes-partial-ex3"/>に示す部分インデックスを考えます。␞␞<programlisting>␞
␝</programlisting>␟   In principle, we could do an index-only scan on this index to satisfy a
   query like␟原則として、次のような問い合わせに対して、このインデックスを使ったインデックスオンリースキャンが可能です。␞␞<programlisting>␞
␝</programlisting>␟   But there's a problem: the <literal>WHERE</literal> clause refers
   ...
   generated, but older versions will not.␟しかし、<literal>WHERE</literal>句で参照される<literal>success</literal>がインデックスの結果列として利用できないという問題があります。
...
<productname>PostgreSQL</productname>のバージョン9.6およびそれ以降ではこのような場合を認識し、インデックスオンリースキャンを生成可能ですが、それより古いバージョンではできません。␞␞  </para>␞
...
␝ <sect1 id="indexes-opclass">␟  <title>Operator Classes and Operator Families</title>␟  <title>演算子クラスと演算子族</title>␞␞␞
␝  <para>␟   An index definition can specify an <firstterm>operator
   class</firstterm> for each column of an index.␟インデックス定義では、インデックスの各列に<firstterm>演算子クラス</firstterm>を指定することができます。␞␞<synopsis>␞
␝</synopsis>␟   The operator class identifies the operators to be used by the index
   ...
   <literal>NULLS FIRST</literal>/<literal>NULLS LAST</literal>).␟演算子クラスにより、その列のインデックスで使用される演算子が特定されます。
...
（これはソートオプション<literal>COLLATE</literal>、<literal>ASC</literal>/<literal>DESC</literal>、<literal>NULLS FIRST</literal>/<literal>NULLS LAST</literal>を付けることで変更できます。）␞␞  </para>␞
␝  <para>␟   There are also some built-in operator classes besides the default ones:␟以下のように、デフォルトの演算子クラスとは別に、組み込み演算子クラスがいくつかあります。␞␞␞
␝     <para>␟      The operator classes <literal>text_pattern_ops</literal>,
      ...
      <type>varchar</type> column like this:␟<literal>text_pattern_ops</literal>、<literal>varchar_pattern_ops</literal>、<literal>bpchar_pattern_ops</literal>演算子クラスは、それぞれ、<type>text</type>、<type>varchar</type>、<type>char</type>型上のB-treeインデックスをサポートします。
...
例えば、以下のように<type>varchar</type>のインデックスを作成できます。␞␞<programlisting>␞
␝</programlisting>␟      Note that you should also create an index with the default operator
      ...
      is usable for pattern-matching queries in the C locale.␟また、通常の<literal>&lt;</literal>、<literal>&lt;=</literal>、<literal>&gt;</literal>、または<literal>&gt;=</literal>比較を含む問い合わせでインデックスを使いたい場合も、デフォルトの演算子クラスでインデックスを作成しなければならないことに注意してください。
...
Cロケールでのパターンマッチ問い合わせでは、デフォルト演算子クラスを使用したインデックスが使用できるためです。␞␞     </para>␞
␝  <para>␟    The following query shows all defined operator classes:␟以下の問い合わせは、定義済みの演算子クラスを全て返します。␞␞␞
␝  <para>␟   An operator class is actually just a subset of a larger structure called an
   ...
   associated with any single class within the family.␟実際のところ演算子クラスは、<firstterm>演算子族</firstterm>と呼ばれる、より大きな構造の一部でしかありません。
複数のデータ型が似たような動作を行う場合、データ型を跨る演算子を定義し、インデックスで使用可能とすることが有用な場合がよくあります。
このためには、各型に対する演算子クラスが同一の演算子族にまとめられている必要があります。
データ型を跨る演算子は演算子族の要素です。演算子族内の1つの演算子クラスに結びついているわけではありません。␞␞  </para>␞
␝  <para>␟    This expanded version of the previous query shows the operator family
    each operator class belongs to:␟以下は前述の問い合わせを拡張したバージョンで、各演算子クラスが属する演算子族を示します。␞␞<programlisting>␞
␝  <para>␟    This query shows all defined operator families and all
    the operators included in each family:␟以下の問い合わせは、定義済みの演算子族と各演算子族に含まれる演算子をすべて表示します。␞␞<programlisting>␞
␝   <para>␟    <xref linkend="app-psql"/> has
    commands <command>\dAc</command>, <command>\dAf</command>,
    and <command>\dAo</command>, which provide slightly more sophisticated
    versions of these queries.␟<xref linkend="app-psql"/>には<command>\dAc</command>、<command>\dAf</command>、<command>\dAo</command>コマンドがあり、これらのクエリのもう少し洗練されたバージョンを提供します。␞␞   </para>␞
␝ <sect1 id="indexes-collations">␟  <title>Indexes and Collations</title>␟  <title>インデックスと照合順序</title>␞␞␞
␝  <para>␟   An index can support only one collation per index column.
   If multiple collations are of interest, multiple indexes may be needed.␟インデックスはインデックス列当たり１つの照合順序のみをサポートすることができます。
複数の照合順序を考慮しなければならない場合、複数のインデックスが必要になるかもしれません。␞␞  </para>␞
␝  <para>␟   Consider these statements:␟以下の文を考えてみます。␞␞<programlisting>␞
␝</programlisting>␟   The index automatically uses the collation of the
   underlying column.  So a query of the form␟このインデックスは自動的に背後にある列の照合順序を使用することになり、␞␞<programlisting>␞
␝</programlisting>␟   could use the index, because the comparison will by default use the
   collation of the column.  However, this index cannot accelerate queries
   that involve some other collation.  So if queries of the form, say,␟という形式の問い合わせでは、この比較はデフォルトで列の照合順序を使用しますので、このインデックスを使用することになります。
しかし、このインデックスは何らかの他の照合順序を含む問い合わせを高速化することはできません。
このため␞␞<programlisting>␞
␝</programlisting>␟   are also of interest, an additional index could be created that supports
   the <literal>"y"</literal> collation, like this:␟という形式の問い合わせも考慮しなければならない場合は、以下のように<literal>"y"</literal>照合順序をサポートする追加のインデックスを作成することになります。␞␞<programlisting>␞
...
␝ <sect1 id="indexes-examine">␟  <title>Examining Index Usage</title>␟  <title>インデックス使用状況の検証</title>␞␞␞
␝  <para>␟   Although indexes in <productname>PostgreSQL</productname> do not need
   ...
   in a running server, as described in <xref linkend="monitoring-stats"/>.␟<productname>PostgreSQL</productname>では、インデックスのメンテナンスやチューニングは必要ありませんが、どのインデックスが実際の問い合わせで使われているかを確認することは、やはり重要です。
個々のコマンドでのインデックスの使用状況は、<xref linkend="sql-explain"/>コマンドで検証できます。
この目的のための用例を<xref linkend="using-explain"/>に示します。
また、<xref linkend="monitoring-stats"/>に示す通り、稼働中のサーバにおけるインデックス使用状況の全体的な統計情報を取り出すこともできます。␞␞  </para>␞
␝  <para>␟   It is difficult to formulate a general procedure for determining
   ...
   The rest of this section gives some tips for that:␟どのインデックスを作成すべきかを判断するための一般的な手順を定めることは困難です。
前節では、例として典型的なケースをいくつか記述してきました。
十分な検証がしばしば必要です。
本節の残りで、検証のためのヒントをいくつか説明しておきます。␞␞  </para>␞
␝    <para>␟     Always run <xref linkend="sql-analyze"/>
     first.  This command
     ...
     and <xref linkend="autovacuum"/> for more information.␟まず、必ず<xref linkend="sql-analyze"/>コマンドを実行してください。
...
より詳細な情報は<xref linkend="vacuum-for-statistics"/>と<xref linkend="autovacuum"/>を参照してください。␞␞    </para>␞
␝    <para>␟     Use real data for experimentation.  Using test data for setting
     up indexes will tell you what indexes you need for the test data,
     but that is all.␟検証には、実際に使用するデータを使ってください。
テストデータを使ってインデックスを作成した場合、テストデータに必要なインデックスはわかりますが、それ以上はわかりません。␞␞    </para>␞
␝    <para>␟     It is especially fatal to use very small test data sets.
     ...
     is no plan that can beat sequentially fetching 1 disk page.␟非常に小さなテストデータを使用することも、結果に特に致命的な影響を与えます。
100,000行から1,000行を選択する場合は、インデックスが使用される可能性がありますが、100行から1行を選択する場合はインデックスはまず使用されません。
なぜなら、100行はおそらく1つのディスクページに収まるため、1ページを逐次読み取るよりも高速な計画は存在しないからです。␞␞    </para>␞
␝    <para>␟     Also be careful when making up test data, which is often
     ...
     that real data would have.␟また、アプリケーションがまだ実動していない場合、テストデータを作成しなければならないことがよくありますが、その際にも注意が必要です。
非常に類似した値や、完全にランダムな値、またはソートされた順序で値が挿入されている場合は、その統計情報は、実際のデータの分布とかけ離れたものになってしまいます。␞␞    </para>␞
␝    <para>␟     When indexes are not used, it can be useful for testing to force
     ...
     the previous sections.)␟インデックスが使用されていない場合、テストのためにインデックスを強制的に使用するようにすると便利です。
...
（どのような問い合わせで、どのようなインデックスを使用できるかは、前節までで説明済みです。）␞␞    </para>␞
␝    <para>␟     If forcing index usage does use the index, then there are two
     ...
     command can be useful here.␟強制的にインデックスを使うように設定することで、インデックスを使用するようになった場合は、次の2つの可能性が考えられます。
システムの判断が正しく、インデックスの使用が実際には適切ではないという可能性と、問い合わせ計画のコスト推定が実情を反映していない可能性です。
したがって、インデックスを使った問い合わせの実行時間と、使わない場合の実行時間を計測する必要があります。
この場合、<command>EXPLAIN ANALYZE</command>コマンドが便利です。␞␞    </para>␞
␝    <para>␟     If it turns out that the cost estimates are wrong, there are,
     ...
     <xref linkend="sql-altertable"/>).␟コスト推定が間違っていると判明した場合、やはり2つの可能性が考えられます。
...
（<xref linkend="sql-altertable"/>を参照してください）。␞␞    </para>␞
␝    <para>␟     If you do not succeed in adjusting the costs to be more
     appropriate, then you might have to resort to forcing index usage
     explicitly.  You might also want to contact the
     <productname>PostgreSQL</productname> developers to examine the issue.␟コストを適切に調節できない場合は、明示的にインデックスの使用を強制する必要が考えられます。
あるいは、<productname>PostgreSQL</productname>開発者に問題の調査を依頼することになるかもしれません。␞␞    </para>␞
