...
␝<sect1 id="gist">␟<title>GiST Indexes</title>␟<title>GiSTインデックス</title>␞␞␞
␝<sect2 id="gist-intro">␟ <title>Introduction</title>␟ <title>はじめに</title>␞␞␞
␝ <para>␟   <acronym>GiST</acronym> stands for Generalized Search Tree.  It is a
   balanced, tree-structured access method, that acts as a base template in
   which to implement arbitrary indexing schemes. B-trees, R-trees and many
   other indexing schemes can be implemented in <acronym>GiST</acronym>.␟<acronym>GiST</acronym>は汎用検索ツリー（Generalized Search Tree）を表します。
これは、均衡な、ツリー構造のアクセスメソッドで、任意のインデックスの枠組みを実装する基本的なテンプレートとして動作します。
B-tree、R-treeやその他多くのインデックスの枠組みを<acronym>GiST</acronym>で実装することができます。␞␞ </para>␞
␝ <para>␟  One advantage of <acronym>GiST</acronym> is that it allows the development
  of custom data types with the appropriate access methods, by
  an expert in the domain of the data type, rather than a database expert.␟<acronym>GiST</acronym>の利点の1つは、データベースの専門家ではなく、データ型分野の専門家によって、適切なアクセスメソッドで独自のデータ型を開発することができることです。␞␞ </para>␞
␝  <para>␟    Some of the information here is derived from the University of California
    ...
    <ulink url="http://www.sai.msu.su/~megera/postgres/gist/">web site</ulink>.␟ここで示す情報の一部は、カリフォルニア大学バークレイ校のGiSTインデックスプロジェクト、<ulink url="http://gist.cs.berkeley.edu/">ウェブサイト</ulink>およびMarcel Kornackerの論文、<ulink url="http://www.sai.msu.su/~megera/postgres/gist/papers/concurrency/access-methods-for-next-generation.pdf.gz">Access Methods for Next-Generation Database Systems</ulink>から派生したものです。
<productname>PostgreSQL</productname>における<acronym>GiST</acronym>の実装は、主に、Teodor SigaevとOleg Bartunovによって保守されています。
そして、彼らの<ulink url="http://www.sai.msu.su/~megera/postgres/gist/">ウェブサイト</ulink>にも多くの情報があります。␞␞  </para>␞
␝<sect2 id="gist-builtin-opclasses">␟ <title>Built-in Operator Classes</title>␟ <title>組み込み演算子クラス</title>␞␞␞
␝ <para>␟  The core <productname>PostgreSQL</productname> distribution
  ...
  provide additional <acronym>GiST</acronym> operator classes.)␟<productname>PostgreSQL</productname>のコア配布物は<xref linkend="gist-builtin-opclasses-table"/>に示す<acronym>GiST</acronym>演算子クラスを含みます。
(<xref linkend="contrib"/>に記載された追加モジュールの中には追加の<acronym>GiST</acronym>演算子クラスを提供するものもあります。)␞␞ </para>␞
␝  <table id="gist-builtin-opclasses-table">␟   <title>Built-in <acronym>GiST</acronym> Operator Classes</title>␟   <title>組み込み<acronym>GiST</acronym>演算子クラス</title>␞␞   <tgroup cols="3">␞
...
␝ <para>␟  For historical reasons, the <literal>inet_ops</literal> operator class is
  ...
  for example␟歴史的な理由から、<literal>inet_ops</literal>演算子クラスは型<type>inet</type>と<type>cidr</type>のデフォルトクラスではありません。
これを使用するには、<command>CREATE INDEX</command>でクラス名を指定します。
例えば、以下のようにします。␞␞<programlisting>␞
␝<sect2 id="gist-extensibility">␟ <title>Extensibility</title>␟ <title>拡張性</title>␞␞␞
␝ <para>␟   Traditionally, implementing a new index access method meant a lot of
   ...
   takes care of concurrency, logging and searching the tree structure.␟伝統的に、新しいインデックスアクセスメソッドの実装は、非常に難しい作業を意味していました。
...
<acronym>GiST</acronym>層自身が同時実行性、ログ処理、ツリー構造の検索処理に関する注意を行います。␞␞ </para>␞
␝ <para>␟   This extensibility should not be confused with the extensibility of the
   ...
   and hash indexes only support equality queries.␟この拡張性と、他の、扱うことができるデータを対象とした標準検索ツリーの拡張性とを混同すべきではありません。
...
しかし、B-treeは範囲述語（<literal>&lt;</literal>、<literal>=</literal>、<literal>&gt;</literal>）のみをサポートし、ハッシュインデックスは等価性問い合わせのみをサポートします。␞␞ </para>␞
␝ <para>␟   So if you index, say, an image collection with a
   ...
   horses</quote> or <quote>find all over-exposed images</quote>.␟ですから、<productname>PostgreSQL</productname>のB-treeで例えば画像群をインデックス付けする場合、<quote>画像xは画像yと同じか</quote>、<quote>画像xは画像yより小さいか</quote>、<quote>画像xは画像yより大きいか</quote>といった問い合わせのみ発行することができます。
この文脈でどのように<quote>同じか</quote>や<quote>より小さいか</quote>、<quote>より大きいか</quote>を定義するかに依存して、これが有意なこともあるでしょう。
しかし、<acronym>GiST</acronym>を基にしたインデックスを使用すれば、問題分野に特化した、おそらくは、<quote>馬の画像を全て見つけたい</quote>、<quote>露出オーバーの写真をすべて見つけたい</quote>といった質問に答えられる手段を作成することができます。␞␞ </para>␞
␝ <para>␟   All it takes to get a <acronym>GiST</acronym> access method up and running
   ...
   reuse, and a clean interface.␟<acronym>GiST</acronym>アクセスメソッドを有効にし、実行するために行なわなければならないことは、ツリーのキーの動作を定義する、複数のユーザ定義のメソッドを実装することです。
当然ながら、これらのメソッドは手の込んだ問い合わせをサポートするためかなり意匠を凝らす必要があります。
しかし、すべての標準的な問い合わせ（B-treeやR-treeなど）ではこれらは、相対的に見てごく簡単です。
まとめると、<acronym>GiST</acronym>は汎用性、コード再利用、整理されたインタフェースと拡張性を兼ね備えたものです。␞␞  </para>␞
␝ <para>␟   There are five methods that an index operator class for
   ...
   speed up building a <acronym>GiST</acronym> index.␟<acronym>GiST</acronym>用の演算子クラスが提供しなければならないメソッドが5つ、オプションで提供可能なメソッドが6つあります。
...
オプションの11番目のメソッド<function>sortsupport</function>は、<acronym>GiST</acronym>インデックスの構築を高速にするのに使われます。␞␞ </para>␞
␝      <para>␟       Given an index entry <literal>p</literal> and a query value <literal>q</literal>,
       ...
       structures.␟インデックス項目<literal>p</literal>と問い合わせ値<literal>q</literal>が与えられると、この関数はインデックス項目が問い合わせと<quote>一貫性</quote>があるかどうか、つまり、述語<quote><replaceable>indexed_column</replaceable><replaceable>indexable_operator</replaceable> <literal>q</literal></quote>が、インデックス項目で表現される行に対して真かどうかを決定します。
...
この規則により、<acronym>GiST</acronym>はインデックス構造が非可逆な場合でも可逆な場合でもサポートすることができます。␞␞      </para>␞
...
␝    /*␟     * determine return value as a function of strategy, key and query.
     ...
     * nodes).␟     * strategy、keyおよびqueryの関数として戻り値を決定してください。
     *
...
␝␟    *recheck = true;        /* or false if check is exact */␟    *recheck = true;        /* もしくは検査が正確であれば偽 */␞␞␞
␝␟       Here, <varname>key</varname> is an element in the index and <varname>query</varname>
       ...
       <command>CREATE OPERATOR CLASS</command> command.␟ここで、<varname>key</varname>はインデックス要素であり、<varname>query</varname>はインデックスに対して検索される値です。
<literal>StrategyNumber</literal>パラメータは、演算子クラスのどの演算子が適用されるかを示します。
これは<command>CREATE OPERATOR CLASS</command>コマンドの演算子番号の1つに一致します。␞␞      </para>␞
␝      <para>␟       Depending on which operators you have included in the class, the data
       ...
       might be something else depending on the operator.␟演算子の右辺にはいかなる型も来ることがあり、それは左辺に現れるインデックス付けされたデータ型とは違うものかもしれませんので、このクラスにどの演算子を含めたかに依存して、<varname>query</varname>のデータ型は演算子に応じて変動することがあります。
(上のコードの骨格は型が1つだけ可能であることを仮定しています。
そうでなければ、<varname>query</varname>引数の値を取得するのは演算子に依存しないといけないでしょう。)
<function>consistent</function>関数のSQL宣言では、実際の型は演算子に依存して何か他のものであるとしても、<varname>query</varname>引数の演算子クラスのインデックス付けされたデータ型を使うことをお勧めします。␞␞      </para>␞
␝      <para>␟       This method consolidates information in the tree.  Given a set of
       ...
       all the given entries.␟このメソッドはツリー内の情報を統合します。
項目の集合が与えられると、この関数は与えられた項目すべてを表現するインデックス項目を新しく生成します。␞␞      </para>␞
␝      <para>␟        The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞␞
␝␟        And the matching code in the C module could then follow this skeleton:␟そして、Cモジュール内の対応するコードは以下のような骨格に従うことになります。␞␞␞
␝      <para>␟        As you can see, in this skeleton we're dealing with a data type
        where <literal>union(X, Y, Z) = union(union(X, Y), Z)</literal>. It's easy
        enough to support data types where this is not the case, by
        implementing the proper union algorithm in this
        <acronym>GiST</acronym> support method.␟ご覧になったように、この骨格で<literal>union(X, Y, Z) = union(union(X, Y), Z)</literal>であるようなデータ型を処理しています。
この<acronym>GiST</acronym>サポートメソッドに適切なunionアルゴリズムを実装することで、このような場合以外のデータ型をサポートすることは非常に容易です。␞␞      </para>␞
␝      <para>␟        The result of the <function>union</function> function must be a value of the
        ...
        no type change.␟<function>union</function>関数の結果は、(インデックス付けされた列の型とは異なるかもしれないし、異ならないかもしれませんが)それが何であれインデックスの格納型の値でなければなりません。
<function>union</function>関数は新たに<function>palloc()</function>されたメモリへのポインタを返さなければなりません。
型の変更がなかったとしても、入力値をそのまま返すことはできません。␞␞      </para>␞
␝      <para>␟       As shown above, the <function>union</function> function's
       ...
       result value into that variable, but this is no longer necessary.)␟上に示したように、<function>union</function>関数の1番目の<type>internal</type>引数は実際は<structname>GistEntryVector</structname>のポインタです。
2番目の引数は整数の変数へのポインタであり、無視できます。
(<function>union</function>関数がその結果値の大きさをその変数に保存するのに必要だったのですが、これはもはや必要ではありません。)␞␞      </para>␞
␝      <para>␟       Converts a data item into a format suitable for physical storage in
       ...
       in the index without modification.␟データ項目をインデックスページ内の物理的な格納に適した形式に変換します。
<function>compress</function>メソッドが省略されている場合、データ項目は変更されずにインデックスに格納されます。␞␞      </para>␞
␝      <para>␟        The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞␞
␝␟        And the matching code in the C module could then follow this skeleton:␟そして、Cモジュール内の対応するコードは以下のような骨格に従うことになります。␞␞␞
␝    {␟        /* replace entry-&gt;key with a compressed version */␟        /* 圧縮バージョンで entry-&gt;key を差し替え */␞␞        compressed_data_type *compressed_data = palloc(sizeof(compressed_data_type));␞
␝␟        /* fill *compressed_data from entry-&gt;key ... */␟        /* entry-&gt;key ... から *compressed_data を補填 */␞␞␞
␝    {␟        /* typically we needn't do anything with non-leaf entries */␟        /* 通常非リーフ項目に対して行うことはない */␞␞        retval = entry;␞
␝      <para>␟       You have to adapt <replaceable>compressed_data_type</replaceable> to the specific
       type you're converting to in order to compress your leaf nodes, of
       course.␟当然ながら<replaceable>compressed_data_type</replaceable>を、リーフノードを圧縮するために変換する特定の型に適合させなければなりません。␞␞      </para>␞
␝      <para>␟       Converts the stored representation of a data item into a format that
       ...
       require full reconstruction of the data.)␟データ項目の格納された表現を、演算子クラスの他のGiSTメソッドで操作できる形式に変換します。
...
他のGiSTメソッドはすべてのデータを再構築することは必要としないかもしれないので、<function>decompress</function>は<function>fetch</function>と等価であるとは限りません。）␞␞      </para>␞
␝      <para>␟        The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞␞
␝␟        And the matching code in the C module could then follow this skeleton:␟そして、Cモジュール内の対応するコードは以下のような骨格に従うことになります。␞␞␞
␝␟        The above skeleton is suitable for the case where no decompression
        is needed.  (But, of course, omitting the method altogether is even
        easier, and is recommended in such cases.)␟上記骨格は、伸長を必要としない場合に適したものです。
（ただし、もちろん、このメソッドを完全に省略する方がさらに簡単なので、このような場合は省略することをお勧めします。）␞␞      </para>␞
␝      <para>␟       Returns a value indicating the <quote>cost</quote> of inserting the new
       entry into a particular branch of the tree.  Items will be inserted
       down the path of least <function>penalty</function> in the tree.
       Values returned by <function>penalty</function> should be non-negative.
       If a negative value is returned, it will be treated as zero.␟新しい項目をツリーの特定の分岐点に挿入するための<quote>コスト</quote>を示す値を返します。
項目は、ツリー内で<function>penalty</function>が最小の経路に挿入されます。
<function>penalty</function>から返される値は非負でなければなりません。
負の値が返された場合、ゼロとして扱われます。␞␞      </para>␞
␝      <para>␟        The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞␞
␝LANGUAGE C STRICT;  -- in some cases penalty functions need not be strict␟LANGUAGE C STRICT;  -- in some cases penalty functions need not be strict␟LANGUAGE C STRICT;  -- penalty関数は厳密である必要がない場合もあります␞␞</programlisting>␞
␝␟        And the matching code in the C module could then follow this skeleton:␟そして、Cモジュール内の対応するコードは以下のような骨格に従うことになります。␞␞␞
␝␟        For historical reasons, the <function>penalty</function> function doesn't
        ...
        address of that argument.␟歴史的な理由により、<function>penalty</function>関数は単純に<type>float</type>の結果を返しません。
その代わり、3番目の引数で指定された場所に値を格納しなければなりません。
その引数のアドレスを戻すのが慣例ですが、戻り値それ自体は無視されます。␞␞      </para>␞
␝      <para>␟        The <function>penalty</function> function is crucial to good performance of
        the index. It'll get used at insertion time to determine which branch
        to follow when choosing where to add the new entry in the tree. At
        query time, the more balanced the index, the quicker the lookup.␟<function>penalty</function>関数は優れた性能のインデックスではきわめて重要です。
これは、挿入の段階で新しい項目をツリーに追加する場所を決定する際にどの分岐に従うかを決定するために使用されます。
問い合わせの際、インデックスのバランスが良ければ、検索が速くなります。␞␞      </para>␞
␝      <para>␟       When an index page split is necessary, this function decides which
       entries on the page are to stay on the old page, and which are to move
       to the new page.␟インデックスページ分割が必要になった時、この関数は、ページ内のどの項目を古いページに残すか、および、どれを新しいページに移動するかを決定します。␞␞      </para>␞
␝      <para>␟        The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞␞
␝␟        And the matching code in the C module could then follow this skeleton:␟そして、Cモジュール内の対応するコードは以下のような骨格に従うことになります。␞␞␞
␝␟    /* Initialize the raw entry vector. */␟    /* 項目自体のベクトルの初期化 */␞␞    raw_entryvec = (GISTENTRY **) malloc(entryvec-&gt;n * sizeof(void *));␞
␝        /*␟         * Choose where to put the index entries and update unionL and unionR
         * accordingly. Append the entries to either v-&gt;spl_left or
         * v-&gt;spl_right, and care about the counters.␟         * インデックス項目の格納場所を決定し、それに合わせてunionLとunionRを更新
         * します。v-&gt;spl_left もしくは v-&gt;spl_right のどちらかに項目を追加します。
         * カウンタに留意してください。␞␞         */␞
␝            /*␟             * Same on the right␟             * 右と同じ␞␞             */␞
␝␟       Notice that the <function>picksplit</function> function's result is delivered
       ...
       value per se is ignored, though it's conventional to pass back the
       address of <structname>v</structname>.␟<function>picksplit</function>関数の結果は渡された<structname>v</structname>構造体を修正することで返されることに注意してください。
<structname>v</structname>のアドレスを戻すのが慣例ですが、戻り値それ自体は無視されます。␞␞      </para>␞
␝      <para>␟        Like <function>penalty</function>, the <function>picksplit</function> function
        is crucial to good performance of the index.  Designing suitable
        <function>penalty</function> and <function>picksplit</function> implementations
        is where the challenge of implementing well-performing
        <acronym>GiST</acronym> indexes lies.␟<function>penalty</function>同様、<function>picksplit</function>関数も優れた性能のインデックスのためにきわめて重要です。
<function>penalty</function>と<function>picksplit</function>の実装を適切に設計することが、性能が良い<acronym>GiST</acronym>インデックスの実装を行うことにつながります。␞␞      </para>␞
␝      <para>␟       Returns true if two index entries are identical, false otherwise.
       (An <quote>index entry</quote> is a value of the index's storage type,
       not necessarily the original indexed column's type.)␟２つのインデックス項目が同一の場合に真、さもなくば偽を返します。
(<quote>インデックス項目</quote>はインデックスの格納型の値であり、必ずしも元のインデックス付けされた列の型という訳ではありません。)␞␞      </para>␞
␝      <para>␟        The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞␞
␝␟        And the matching code in the C module could then follow this skeleton:␟そして、Cモジュール内の対応するコードは以下のような骨格に従うことになります。␞␞␞
␝␟        For historical reasons, the <function>same</function> function doesn't
        ...
        at the location indicated by the third argument.  The return
        value per se is ignored, though it's conventional to pass back the
        address of that argument.␟歴史的な理由により、<function>same</function>関数は単純に論理値の結果を返しません。
...
␝      <para>␟       Given an index entry <literal>p</literal> and a query value <literal>q</literal>,
       ...
       smallest distance that any child entry could have.␟インデックス項目<literal>p</literal>と問い合わせ値<literal>q</literal>を与えると、この関数は問い合わせ値からのインデックス項目の<quote>距離</quote>を決定します。
...
内部ツリーノードでは、結果はすべての子項目が持つ中から最も最小の距離でなければなりません。␞␞      </para>␞
␝      <para>␟        The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞␞
␝␟        And the matching code in the C module could then follow this skeleton:␟そして、Cモジュール内の対応するコードは以下のような骨格に従うことになります。␞␞␞
␝    /*␟     * determine return value as a function of strategy, key and query.␟     * strategy、keyおよびqueryの関数として戻り値を決定してください。␞␞     */␞
␝␟       The arguments to the <function>distance</function> function are identical to
       the arguments of the <function>consistent</function> function.␟<function>distance</function>関数の引数は<function>consistent</function>関数の引数と同一です。␞␞      </para>␞
␝      <para>␟       Some approximation is allowed when determining the distance, so long
       ...
       fetching the tuple from the heap, and reorder the tuples if necessary.␟距離の決定において、その結果がエントリの実際の距離よりも大きくならない限り、多少の概算は許されます。
...
この場合、エグゼキュータはヒープからタプルを取得した後で正確な距離を計算し、必要ならタプルを並べ替えます。␞␞      </para>␞
␝      <para>␟       If the distance function returns <literal>*recheck = true</literal> for any
       ...
       recommended that <function>distance</function> functions return these values.)␟距離関数がリーフノードについて<literal>*recheck = true</literal>を返す場合、元の順序づけ演算子の戻り型は<type>float8</type>または<type>float4</type>でなければならず、また距離関数の結果の値は元の順序づけ演算子の戻り型と比較可能でなければなりません。
...
（無限大とマイナス無限大は内部的にNULLなどの場合を処理するために利用するので、<function>distance</function>関数がこれらの値を戻すことは薦められません。）␞␞      </para>␞
␝      <para>␟       Converts the compressed index representation of a data item into the
       original data type, for index-only scans. The returned data must be an
       exact, non-lossy copy of the originally indexed value.␟インデックスオンリースキャンで使用するため、データ項目の圧縮されたインデックス表現を元のデータ型に変換します。
返されたデータは元のインデックス値の正確で、何も失われていない複製でなければなりません。␞␞      </para>␞
␝      <para>␟        The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞␞
␝␟        The argument is a pointer to a <structname>GISTENTRY</structname> struct. On
        ...
        necessarily be a no-op.␟引数は<structname>GISTENTRY</structname>構造体へのポインタです。
...
これは、必然的にno-opになるからです。␞␞       </para>␞
␝       <para>␟        The matching code in the C module could then follow this skeleton:␟Cモジュールにおける対応するコードは次の骨格に従うことになります。␞␞␞
␝    /*␟     * Convert 'fetched_data' into the a Datum of the original datatype.␟     * fetched_dataを元のデータ型のデータに変換する。␞␞     */␞
␝␟    /* fill *retval from fetched_data. */␟    /* fetched_dataを使って*retvalに値を入れる。 */␞␞    gistentryinit(*retval, PointerGetDatum(converted_datum),␞
␝      <para>␟       If the compress method is lossy for leaf entries, the operator class
       cannot support index-only scans, and must not define
       a <function>fetch</function> function.␟compressメソッドがリーフエントリに対してデータ損失がある場合、演算子クラスはインデックスオンリースキャンをサポートすることができず、<function>fetch</function>関数を定義してはいけません。␞␞      </para>␞
␝      <para>␟       Allows definition of user-visible parameters that control operator
       class behavior.␟演算子クラスの振舞いを制御するユーザに可視のパラメータの集合を定義します。␞␞      </para>␞
␝      <para>␟        The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞␞
...
␝       <para>␟        An example implementation of my_options() and parameters use
        from other support functions are given below:␟my_options()の実装と他のサポート関数からのパラメータの使用の例は以下の通りです。␞␞␞
␝{␟    int32   vl_len_;    /* varlena header (do not touch directly!) */
    int     int_param;  /* integer parameter */
    double  real_param; /* real parameter */
    MyEnumType enum_param; /* enum parameter */
    int     str_param;  /* string parameter */␟    int32   vl_len_;    /* varlenaヘッダ(直接触らないこと!) */
    int     int_param;  /* 整数パラメータ */
    double  real_param; /* 実数パラメータ */
    MyEnumType enum_param; /* enumパラメータ */
    int     str_param;  /* 文字列パラメータ */␞␞} MyOptionsStruct;␞
␝␟/* String representation of enum values */␟/* enum値の文字列表現 */␞␞static relopt_enum_elt_def myEnumValues[] =␞
␝    {"auto", MY_ENUM_AUTO},␟    {(const char *) NULL}   /* list terminator */␟    {(const char *) NULL}   /* リストの終端 */␞␞};␞
␝/*␟ * Sample validator: checks that string is not longer than 8 bytes.␟ * 有効性検査の例: 文字列が8バイトより長くないことを検査します。␞␞ */␞
␝/*␟ * Sample filler: switches characters to lower case.␟ * 充填の例: 文字を小文字に交換します。␞␞ */␞
␝    /*␟     * Normally, when opclass contains 'options' method, then options are always
     * passed to support functions.  However, if you add 'options' method to
     * existing opclass, previously defined indexes have no options, so the
     * check is required.␟     * 通常は、演算子クラスが'options'メソッドを含む場合、optionsは常にサポート関数に
     * 渡されます。しかしながら、'options'メソッドを既存の演算子クラスに追加した場合、
     * 前に定義されたインデックスにoptionsがない場合、検査が必要です。␞␞     */␞
␝␟    /* the rest implementation of support function */␟    /* サポート関数の残りの実装 */␞␞}␞
␝      <para>␟       Since the representation of the key in <acronym>GiST</acronym> is
       flexible, it may depend on user-specified parameters.  For instance,
       the length of key signature may be specified.  See
       <literal>gtsvector_options()</literal> for example.␟<acronym>GiST</acronym>でのキーの表現には柔軟性がありますので、ユーザに固有のパラメータに依存するかもしれません。
例えば、キーの署名の長さが指定されるかもしれません。
例については<literal>gtsvector_options()</literal>を参照してください。␞␞      </para>␞
␝      <para>␟       Returns a comparator function to sort data in a way that preserves
       ...
       preserves locality of the inputs.␟ある程度局所性を保つ方法でデータをソートする比較関数を返します。
<command>CREATE INDEX</command>と<command>REINDEX</command>コマンドで使われます。
作成されるインデックスの質は、比較関数により決定された順序が入力の局所性をどれだけよく保っているかに依存します。␞␞      </para>␞
␝      <para>␟       The <function>sortsupport</function> method is optional. If it is not
       ...
       <function>picksplit</function> functions, which is much slower.␟<function>sortsupport</function>メソッドは省略可能です。
提供されなければ、<command>CREATE INDEX</command>は各タプルを<function>penalty</function>関数と<function>picksplit</function>関数を使ってツリーに挿入することでインデックスを構築します。これはずっと遅いです。␞␞      </para>␞
␝      <para>␟       The <acronym>SQL</acronym> declaration of the function must look like
       this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞␞
␝␟       The argument is a pointer to a <structname>SortSupport</structname>
       ...
       <filename>src/include/utils/sortsupport.h</filename>.␟引数は<structname>SortSupport</structname>構造体へのポインタです。
...
完全なAPIは<filename>src/include/utils/sortsupport.h</filename>で定義されています。␞␞       </para>␞
␝       <para>␟        The matching code in the C module could then follow this skeleton:␟Cモジュールにおける対応するコードは次の骨格に従うことになります。␞␞␞
␝{␟  /* establish order between x and y by computing some sorting value z */␟  /* ソートのための値zを計算することでxとyの間に順序を確立する */␞␞␞
␝  <para>␟   All the GiST support methods are normally called in short-lived memory
   ...
   will accumulate for the duration of the operation.␟すべてのGiSTサポートメソッドは通常短期間有効なメモリコンテキストで呼び出されます。
...
さもないと操作の間リークが蓄積されます。␞␞  </para>␞
␝<sect2 id="gist-implementation">␟ <title>Implementation</title>␟ <title>実装</title>␞␞␞
␝ <sect3 id="gist-buffering-build">␟  <title>GiST Index Build Methods</title>␟  <title>GiSTインデックス構築法</title>␞␞␞
␝  <para>␟   The simplest way to build a GiST index is just to insert all the entries,
   ...
   and <firstterm>buffered</firstterm> modes.␟GiSTインデックスを構築する一番簡単な方法は、全項目を単に1つ1つ挿入することです。
インデックスタプルがインデックス全体に分散し、インデックスがキャッシュに収まらない程大規模である場合、大量のランダムI/Oを必要としますので、これは大規模なインデックスに対して低速になりがちです。
<productname>PostgreSQL</productname>はGiSTインデックスの初期構築のために他に2つの方法をサポートします。<firstterm>ソート処理</firstterm>モードと<firstterm>バッファ処理</firstterm>モードです。␞␞  </para>␞
␝  <para>␟   The sorted method is only available if each of the opclasses used by the
   ...
   usually the best, so it is used by default.␟ソート処理法は、インデックスで使われる演算子クラスのそれぞれが、<xref linkend="gist-extensibility"/>に記載されているように<function>sortsupport</function>を提供している場合にのみ利用可能です。
もしそうであれば、この方法が普通は最善ですので、デフォルトで使われます。␞␞  </para>␞
␝  <para>␟   The buffered method works by not inserting tuples directly into the index
   ...
   whole does not.␟バッファ処理法はタプルを直ちに直接インデックスに挿入しないことで動作します。
これは、順序付けられていないデータ群に対して必要とされるランダムI/Oの量を劇的に減らすかもしれません。
十分に順序付けられたデータ群では、一度にわずかなページ数のみが新しいタプルを受け取り、そのためインデックス全体がキャッシュに収まらなくてもこれらのページがキャッシュ内に収まりますので、利点はより小さく、または利点がなくなります。␞␞  </para>␞
␝  <para>␟   The buffered method needs to call the <function>penalty</function>
   ...
   data and the operator class implementation.␟バッファ処理法は、<function>penalty</function>関数を単純な方法よりもより多く呼び出すことが必要で、余計にCPUリソースを消費します。
またバッファは、最大作成されるインデックスと同じサイズまで、一時的にディスク容量を必要とします。
バッファ処理は作成されるインデックスの品質にも、良くも悪くも、影響を与えます。
この影響は、入力データの分布や演算子クラスの実装等、様々な要因に依存します。␞␞  </para>␞
␝  <para>␟   If sorting is not possible, then by default a GiST index build switches
   ...
   is ordered.␟ソートが可能でない場合、デフォルトでは、インデックスのサイズが<xref linkend="guc-effective-cache-size"/>に達した時にGiSTインデックス構築はバッファ処理法に切り替わります。
バッファ処理はCREATE INDEXコマンドの<literal>buffering</literal>パラメータによって、手動で強制あるいは無効にできます。
デフォルトの動作は大抵の場合良好です。
しかし、入力データが順序付けされている場合、バッファ処理を無効にすることで構築が多少高速になります。␞␞  </para>␞
␝<sect2 id="gist-examples">␟ <title>Examples</title>␟ <title>例</title>␞␞␞
␝ <para>␟  The <productname>PostgreSQL</productname> source distribution includes
  ...
  operator classes:␟<productname>PostgreSQL</productname>のソース配布物には<acronym>GiST</acronym>を使用したインデックスメソッドの実装のいくつかの事例が含まれています。
...
␝   <listitem>␟    <para>B-tree equivalent functionality for several data types</para>␟    <para>いくつかのデータ型に対するB-tree等価機能</para>␞␞   </listitem>␞
␝   <listitem>␟    <para>Indexing for multidimensional cubes</para>␟    <para>多次元の立方体用のインデックス</para>␞␞   </listitem>␞
␝   <listitem>␟    <para>Module for storing (key, value) pairs</para>␟    <para>（キー、値）の組み合わせを格納するモジュール</para>␞␞   </listitem>␞
␝   <listitem>␟    <para>RD-Tree for one-dimensional array of int4 values</para>␟    <para>int4値の1次元配列用のRD-Tree</para>␞␞   </listitem>␞
␝   <listitem>␟    <para>Indexing for tree-like structures</para>␟    <para>疑似ツリー構造用のインデックス</para>␞␞   </listitem>␞
␝   <listitem>␟    <para>Text similarity using trigram matching</para>␟    <para>トライグラム一致を使用したテキストの類似性</para>␞␞   </listitem>␞
␝   <listitem>␟    <para>Indexing for <quote>float ranges</quote></para>␟    <para><quote>浮動小数点範囲</quote>のインデックス</para>␞␞   </listitem>␞
