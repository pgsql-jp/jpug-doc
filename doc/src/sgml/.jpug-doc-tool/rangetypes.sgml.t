...
␝<sect1 id="rangetypes">␟ <title>Range Types</title>␟ <title>範囲型</title>␞␞␞
␝ <para>␟  Range types are data types representing a range of values of some
  ...
  within, before, or after a range of values.␟範囲型は、ある要素型(その範囲の<firstterm>派生元型</firstterm>と呼ばれます)の値の範囲を表わすデータ型です。
...
派生元型には完全な順序がなければなりません。これは、要素の値が範囲の前、中間、後のどこにあるのか明確に定義されている必要があるからです。␞␞ </para>␞
␝ <para>␟  Range types are useful because they represent many element values in a
  ...
  ranges from an instrument, and so forth can also be useful.␟範囲型は、一つの範囲内の多くの要素の値を表現できる、また、範囲の重なりなどの概念が明確に表現できる、などの理由で便利です。
スケジューリングのために時刻と日付の範囲を使うのがもっとも簡単な例ですが、価格の範囲、機器による測定値の範囲などといったものにも利用できるでしょう。␞␞ </para>␞
␝ <para>␟  Every range type has a corresponding multirange type. A multirange is
  ...
  of their own.␟すべての範囲型には、対応する多重範囲型があります。
多重範囲は、連続していない、空でない、NULLでない範囲の順序付きリストです。
ほとんどの範囲演算子は多重範囲でも機能し、いくつかの独自の機能を持っています。␞␞ </para>␞
␝ <sect2 id="rangetypes-builtin">␟  <title>Built-in Range and Multirange Types</title>␟  <title>組み込みの範囲型と多重範囲型</title>␞␞␞
␝ <para>␟  PostgreSQL comes with the following built-in range types:␟PostgreSQLには、以下の組み込みの範囲型があります。␞␞  <itemizedlist>␞
␝      <para>␟       <type>int4range</type> &mdash; Range of <type>integer</type>,
       <type>int4multirange</type> &mdash; corresponding Multirange␟<type>int4range</type>&mdash;<type>integer</type>の範囲、<type>int4multirange</type>&mdash;対応する多重範囲␞␞      </para>␞
␝      <para>␟       <type>int8range</type> &mdash; Range of <type>bigint</type>,
       <type>int8multirange</type> &mdash; corresponding Multirange␟<type>int8range</type>&mdash;<type>bigint</type>の範囲、<type>int8multirange</type>&mdash;対応する多重範囲␞␞      </para>␞
␝      <para>␟       <type>numrange</type> &mdash; Range of <type>numeric</type>,
       <type>nummultirange</type> &mdash; corresponding Multirange␟<type>numrange</type>&mdash;<type>numeric</type>の範囲、<type>nummultirange</type>&mdash;対応する多重範囲␞␞      </para>␞
␝      <para>␟       <type>tsrange</type> &mdash; Range of <type>timestamp without time zone</type>,
       <type>tsmultirange</type> &mdash; corresponding Multirange␟<type>tsrange</type>&mdash;<type>timestamp without time zone</type>の範囲、<type>tsmultirange</type>&mdash;対応する多重範囲␞␞      </para>␞
␝      <para>␟       <type>tstzrange</type> &mdash; Range of <type>timestamp with time zone</type>,
       <type>tstzmultirange</type> &mdash; corresponding Multirange␟<type>tstzrange</type>&mdash;<type>timestamp with time zone</type>の範囲、<type>tstzmultirange</type>&mdash;対応する多重範囲␞␞      </para>␞
␝      <para>␟       <type>daterange</type> &mdash; Range of <type>date</type>,
       <type>datemultirange</type> &mdash; corresponding Multirange␟<type>daterange</type>&mdash;<type>date</type>の範囲、<type>datemultirange</type>&mdash;対応する多重範囲␞␞      </para>␞
␝  </itemizedlist>␟  In addition, you can define your own range types;
  see <xref linkend="sql-createtype"/> for more information.␟この他にも、独自の範囲型を定義することができます。詳しくは<xref linkend="sql-createtype"/>を参照してください。␞␞ </para>␞
␝ <sect2 id="rangetypes-examples">␟  <title>Examples</title>␟  <title>例</title>␞␞␞
␝-- Containment␟-- Containment␟-- 含有␞␞SELECT int4range(10, 20) @&gt; 3;␞
␝-- Overlaps␟-- Overlaps␟-- 重なり␞␞SELECT numrange(11.1, 22.2) &amp;&amp; numrange(20.0, 30.0);␞
␝-- Extract the upper bound␟-- Extract the upper bound␟-- 上限の取得␞␞SELECT upper(int8range(15, 25));␞
␝-- Compute the intersection␟-- Compute the intersection␟-- 共通部分の計算␞␞SELECT int4range(10, 20) * int4range(15, 25);␞
␝-- Is the range empty?␟-- Is the range empty?␟-- 範囲は空か␞␞SELECT isempty(numrange(1, 5));␞
␝␟   See <xref linkend="range-operators-table"/>
   and <xref linkend="range-functions-table"/> for complete lists of
   operators and functions on range types.␟範囲型についての演算子と関数の完全なリストについては、<xref linkend="range-operators-table"/>と<xref linkend="range-functions-table"/>を参照してください。␞␞  </para>␞
␝ <sect2 id="rangetypes-inclusivity">␟  <title>Inclusive and Exclusive Bounds</title>␟  <title>閉じた境界と開いた境界</title>␞␞␞
␝  <para>␟   Every non-empty range has two bounds, the lower bound and the upper
   ...
   point is not included in the range.␟空でない範囲には必ず2つの境界、つまり下限値と上限値があります。
これらの値の間にある値はすべてその範囲に含まれます。
閉じた境界とは、その境界値自体が範囲に含まれることを意味し、開いた境界とは、その境界値が範囲に含まれないことを意味します。␞␞  </para>␞
␝  <para>␟   In the text form of a range, an inclusive lower bound is represented by
   ...
   (See <xref linkend="rangetypes-io"/> for more details.)␟範囲を文字列の形式で表すとき、閉じた下限値は<quote><literal>[</literal></quote>で、開いた下限値は<quote><literal>(</literal></quote>で表します。
同様に、閉じた上限値は<quote><literal>]</literal></quote>で、開いた上限値は<quote><literal>)</literal></quote>で表します。
(詳しくは <xref linkend="rangetypes-io"/>を参照してください。)␞␞  </para>␞
␝  <para>␟   The functions <literal>lower_inc</literal>
   and <literal>upper_inc</literal> test the inclusivity of the lower
   and upper bounds of a range value, respectively.␟関数<literal>lower_inc</literal>および<literal>upper_inc</literal>はそれぞれ、範囲の下限値と上限値が閉じているかどうかを検査します。␞␞  </para>␞
␝ <sect2 id="rangetypes-infinite">␟  <title>Infinite (Unbounded) Ranges</title>␟  <title>無限の(境界のない)範囲</title>␞␞␞
␝  <para>␟   The lower bound of a range can be omitted, meaning that all
   ...
   element type's +/-infinity values.␟例えば、<literal>(,3]</literal>のように、範囲の下限値は省略することができ、このとき、上限値より小さいすべての値はその範囲に含まれることになります。
...
省略された値を+/-無限大と考えることができますが、特殊な範囲型の値であり、いかなる範囲の要素型の+/-無限大の値も超えていると考えられます。␞␞  </para>␞
␝  <para>␟   Element types that have the notion of <quote>infinity</quote> can
   ...
   <literal>[today,)</literal> and <literal>[today,]</literal>.␟<quote>無限大</quote>の概念がある要素型では、それを明示的な境界値として使用できます。
例えば、timestampの範囲で<literal>[today,infinity)</literal>は特殊な<type>timestamp</type>値、<literal>infinity</literal>を含みませんが、一方、<literal>[today,infinity]</literal>は<literal>[today,)</literal>や<literal>[today,]</literal>と同じように<literal>infinity</literal>を含みます。␞␞  </para>␞
␝  <para>␟   The functions <literal>lower_inf</literal>
   and <literal>upper_inf</literal> test for infinite lower
   and upper bounds of a range, respectively.␟関数<literal>lower_inf</literal>および<literal>upper_inf</literal>はそれぞれ範囲の下限値と上限値が無限大かどうかを検査します。␞␞  </para>␞
␝ <sect2 id="rangetypes-io">␟  <title>Range Input/Output</title>␟  <title>範囲の入出力</title>␞␞␞
␝  <para>␟   The input for a range value must follow one of the following patterns:␟範囲値の入力は、以下の形式の一つに従わなければなりません。␞␞<synopsis>␞
␝</synopsis>␟   The parentheses or brackets indicate whether the lower and upper bounds
   are exclusive or inclusive, as described previously.
   Notice that the final pattern is <literal>empty</literal>, which
   represents an empty range (a range that contains no points).␟前にも述べたとおり、丸括弧と大括弧は下限値と上限値が開いているか閉じているかを表します。
最後の形式が<literal>empty</literal>であることに注意してください。これは空の範囲(範囲に含まれる値が1つもない)を表します。␞␞  </para>␞
␝  <para>␟   The <replaceable>lower-bound</replaceable> may be either a string
   ...
   indicate no upper bound.␟<replaceable>lower-bound</replaceable>は、その派生元型の有効な入力値となる文字列か、あるいは省略して下限値がないことを指定するかのいずれかです。
同様に、<replaceable>upper-bound</replaceable>は、その派生元型の有効な入力値となる文字列か、あるいは省略して上限値がないことを指定するかのいずれかです。␞␞  </para>␞
␝  <para>␟   Each bound value can be quoted using <literal>"</literal> (double quote)
   ...
   nothing means an infinite bound.␟境界値は<literal>"</literal>(二重引用符)で括ることができます。
...
なお、境界値として空文字列を指定するには<literal>""</literal>と書いてください。何も書かないと、境界値が無限大であることになってしまいます。␞␞  </para>␞
␝  <para>␟   Whitespace is allowed before and after the range value, but any whitespace
   ...
   significant.)␟境界値の前後に空白文字を入れることができますが、括弧内にある空白文字はすべて下限値あるいは上限値の一部とみなされます。
(このことは、要素型によっては重要かもしれませんし、重要でないかもしれません。)␞␞  </para>␞
␝   <para>␟    These rules are very similar to those for writing field values in
    composite-type literals.  See <xref linkend="rowtypes-io-syntax"/> for
    additional commentary.␟これらの規則は、複合型のリテラルにフィールド値を記述する時と非常によく似ています。
詳細な解説は<xref linkend="rowtypes-io-syntax"/>を参照してください。␞␞   </para>␞
␝  <para>␟  Examples:␟  例：␞␞<programlisting>␞
␝-- includes 3, does not include 7, and does include all points in between␟-- includes 3, does not include 7, and does include all points in between␟-- 3を含み、7を含まない。その間の数はすべて含まれる␞␞SELECT '[3,7)'::int4range;␞
␝-- does not include either 3 or 7, but includes all points in between␟-- does not include either 3 or 7, but includes all points in between␟-- 3も7も含まないが、その間の数はすべて含まれる␞␞SELECT '(3,7)'::int4range;␞
␝-- includes only the single point 4␟-- includes only the single point 4␟-- 1つの値、4だけを含む␞␞SELECT '[4,4]'::int4range;␞
␝-- includes no points (and will be normalized to 'empty')␟-- includes no points (and will be normalized to 'empty')␟-- 含まれる点は何もない('empty'に正規化される)␞␞SELECT '[4,4)'::int4range;␞
␝  <para>␟   The input for a multirange is curly brackets (<literal>{</literal> and
   ...
   quoted as above however.)␟多重範囲の入力は、カンマで区切られた0個以上の有効範囲を含む中カッコ(<literal>{</literal>および<literal>}</literal>)です。
カッコとカンマの前後に空白を使用できます。
これは配列構文を連想させることを意図したものですが、多重範囲ははるかに単純で、次元が1つしかなく、内容を引用符で囲む必要はありません。
（ただし、範囲の境界は前述のように引用符で囲むことができます。）␞␞  </para>␞
␝  <para>␟  Examples:␟例えば、␞␞<programlisting>␞
␝ <sect2 id="rangetypes-construct">␟  <title>Constructing Ranges and Multiranges</title>␟  <title>範囲と多重範囲のコンストラクタ</title>␞␞␞
␝  <para>␟   Each range type has a constructor function with the same name as the range
   ...
   <quote><literal>[]</literal></quote>.
   For example:␟範囲型には、その範囲型と同じ名前のコンストラクタ関数があります。
...
例えば、␞␞␞
␝-- inclusivity/exclusivity of bounds.␟-- The full form is: lower bound, upper bound, and text argument indicating
-- inclusivity/exclusivity of bounds.␟-- 完全な形式では、下限値、上限値、そして境界が閉じているか開いているかを
-- 示す文字列の引数を指定する␞␞SELECT numrange(1.0, 14.0, '(]');␞
␝-- If the third argument is omitted, '[)' is assumed.␟-- If the third argument is omitted, '[)' is assumed.␟-- 3番目の引数が省略されると、'[)'を指定したのと同じになる␞␞SELECT numrange(1.0, 14.0);␞
␝-- canonical form, since int8range is a discrete range type (see below).␟-- Although '(]' is specified here, on display the value will be converted to
-- canonical form, since int8range is a discrete range type (see below).␟-- ここでは'(]'を指定しているが、int8rangeは離散的な範囲型(下記参照)なので
-- 正規化された形式に変換されて表示される␞␞SELECT int8range(1, 14, '(]');␞
␝-- Using NULL for either bound causes the range to be unbounded on that side.␟-- Using NULL for either bound causes the range to be unbounded on that side.␟-- 境界値にNULLを指定すると、範囲の上限、あるいは下限がないことになる␞␞SELECT numrange(NULL, 2.2);␞
␝  <para>␟   Each range type also has a multirange constructor with the same name as the
   multirange type.  The constructor function takes zero or more arguments
   which are all ranges of the appropriate type.
   For example:␟各範囲型には、多重範囲型と同じ名前の多重範囲コンストラクタもあります。
コンストラクタ関数は、すべて適切な型の範囲である0個以上の引数を取ります。
例えば、␞␞␞
␝ <sect2 id="rangetypes-discrete">␟  <title>Discrete Range Types</title>␟  <title>離散的な範囲型</title>␞␞␞
␝  <para>␟   A discrete range is one whose element type has a well-defined
   ...
   since the step size is normally not of interest.)␟離散的な範囲とは、<type>integer</type>や<type>date</type>のように明確に定義された<quote>ステップ</quote>のある要素型の範囲のことです。
...
(<type>timestamp</type>の精度は限界があるので、理論的には離散的として取り扱うことも可能ですが、ステップの大きさについて関心がないのが普通ですから、連続的であると考える方が良いでしょう。)␞␞  </para>␞
␝  <para>␟   Another way to think about a discrete range type is that there is a clear
   ...
   for a range over numeric.␟離散的な範囲型に関するもう1つの考え方は、各要素の値について、<quote>次</quote>あるいは<quote>前</quote>の値というのものが明確に考えられるか、ということです。
これを知っていれば、範囲の境界の閉じた表現、あるいは開いた表現について、その値の次、あるいは前の値を使って、表現を変換することができます。
例えば、整数の範囲型<literal>[4,8]</literal>と<literal>(3,9)</literal>は同じ値の集合を意味しますが、これがnumericの範囲型であったならそうではありません。␞␞  </para>␞
␝  <para>␟   A discrete range type should have a <firstterm>canonicalization</firstterm>
   ...
   represent the same set of values in reality.␟離散的な範囲型はその要素型で使いたいステップのサイズを認識する<firstterm>正規化</firstterm>関数を持つべきです。
正規化関数は同等な値の範囲型を、同一の表現に、特に、閉じた境界、開いた境界について一定の形式に変換します。
正規化関数が指定されない場合、異なる形式の範囲は必ず等しくないものとして扱われます。これは例え、それらが現実的に同じ値の集合であったとしても、等しくないとされます。␞␞  </para>␞
␝  <para>␟   The built-in range types <type>int4range</type>, <type>int8range</type>,
   ...
   however.␟組み込みの範囲型である<type>int4range</type>、<type>int8range</type>、および<type>daterange</type>はいずれも閉じた下限値と開いた上限値、つまり<literal>[)</literal>の正規化形式を使います。
しかし、ユーザ定義の範囲型はこれとは別の方式を使うことができます。␞␞  </para>␞
␝ <sect2 id="rangetypes-defining">␟  <title>Defining New Range Types</title>␟  <title>新しい範囲型の定義</title>␞␞␞
␝  <para>␟   Users can define their own range types. The most common reason to do
   this is to use ranges over subtypes not provided among the built-in
   range types.
   For example, to define a new range type of subtype <type>float8</type>:␟独自の範囲型を定義することもできます。
...
␝␟   Because <type>float8</type> has no meaningful
   ...
   function in this example.␟<type>float8</type>には意味のある<quote>ステップ</quote>がないので、この例では正規化関数を定義していません。␞␞  </para>␞
␝  <para>␟   When you define your own range you automatically get a corresponding
   multirange type.␟独自の範囲を定義すると、対応する多重範囲型が自動的に取得されます。␞␞  </para>␞
␝  <para>␟   Defining your own range type also allows you to specify a different
   subtype B-tree operator class or collation to use, so as to change the sort
   ordering that determines which values fall into a given range.␟独自の範囲型を定義すると、派生元型とは異なるB-tree演算子クラスや照合順を指定でき、どの値が指定の範囲に入るかを決定するソート順を変更することもできます。␞␞  </para>␞
␝  <para>␟   If the subtype is considered to have discrete rather than continuous
   ...
   or perhaps throw an error instead.␟派生元型が、連続的ではなく離散的な値を持つと考えられる場合は、<command>CREATE TYPE</command>コマンドで<literal>canonical</literal>(正規化)関数を指定する必要があります。
...
例えば<type>timestamp</type>の範囲型をステップのサイズを1時間として定義することができます。このとき、正規化関数は1時間の倍数になっていない境界値を丸める必要があります。あるいは、その代わりにエラーを投げることもできます。␞␞  </para>␞
␝  <para>␟   In addition, any range type that is meant to be used with GiST or SP-GiST
   ...
   greater than its second according to the sort ordering.␟また、GiSTまたはSP-GiSTインデックスと一緒に使われる範囲型は、派生元型の差分、つまり<literal>subtype_diff</literal>関数を定義すべきです。
...
可能な限りにおいて、<literal>subtype_diff</literal>関数は、選択した演算子クラスと照合順が示唆するソート順と矛盾しないようにすべき、つまり、ソート順で、1番目の引数が2番目の引数より上に来る場合は、必ず差分関数の結果は正になるべきです。␞␞  </para>␞
␝  <para>␟   A less-oversimplified example of a <literal>subtype_diff</literal> function is:␟<literal>subtype_diff</literal>関数の単純化されすぎていない例を以下に示します。␞␞  </para>␞
␝  <para>␟   See <xref linkend="sql-createtype"/> for more information about creating
   range types.␟範囲型の作成について、より詳細な情報は<xref linkend="sql-createtype"/>を参照してください。␞␞  </para>␞
␝  <indexterm>
    <primary>range type</primary>
...
␝ <sect2 id="rangetypes-indexing">␟  <title>Indexing</title>␟  <title>インデックス</title>␞␞␞
␝  <para>␟   GiST and SP-GiST indexes can be created for table columns of range types.
   GiST indexes can be also created for table columns of multirange types.
   For instance, to create a GiST index:␟範囲型のテーブル列にGiSTおよびSP-GiSTインデックスを作成することができます。
GiSTインデックスは、多重範囲型のテーブル列に対しても作成できます。
例えば、GiSTインデックスを作成するには、␞␞<programlisting>␞
␝</programlisting>␟   A GiST or SP-GiST index on ranges can accelerate queries involving these
   range operators:
   <literal>=</literal>,
   <literal>&amp;&amp;</literal>,
   <literal>&lt;@</literal>,
   ...
   <literal>&amp;&lt;</literal>, and
   <literal>&amp;&gt;</literal>.
   See <xref linkend="range-operators-table"/> for more information.␟範囲型に関するGiSTあるいはSP-GiSTインデックスがあると、以下の範囲演算子を含む検索を高速に実行できます。
...
より詳細な情報は<xref linkend="range-operators-table"/>を参照してください。␞␞  </para>␞
␝  <para>␟   In addition, B-tree and hash indexes can be created for table columns of
   ...
   actual indexes.␟さらに、範囲型のテーブル列にB-treeおよびハッシュインデックスを作ることもできます。
...
範囲型のB-treeとハッシュのサポートは実際にインデックスを作ることよりも、むしろ、検索時に内部的にソートやハッシュをできるようにするのが主な目的です。␞␞  </para>␞
␝  <indexterm>
    <primary>range type</primary>
...
␝ <sect2 id="rangetypes-constraint">␟  <title>Constraints on Ranges</title>␟  <title>範囲の制約</title>␞␞␞
␝  <para>␟   While <literal>UNIQUE</literal> is a natural constraint for scalar
   ...
   range type. For example:␟<literal>UNIQUE</literal>はスカラ値には自然な制約ですが、範囲型には通常は適当ではありません。
...
␝␟   That constraint will prevent any overlapping values from existing
   in the table at the same time:␟この制約は、テーブル上で重なりのある値が同時に存在することを防ぎます。␞␞␞
␝  <para>␟   You can use the <link linkend="btree-gist"><literal>btree_gist</literal></link>
   ...
   are equal:␟<link linkend="btree-gist"><literal>btree_gist</literal></link>の拡張を使って通常のスカラのデータ型について排他制約を定義することができます。
