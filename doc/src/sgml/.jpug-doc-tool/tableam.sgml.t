...
␝<chapter id="tableam">␟ <title>Table Access Method Interface Definition</title>␟ <title>テーブルアクセスメソッドのインタフェース定義</title>␞␞␞
␝ <para>␟  This chapter explains the interface between the core
  ...
  add-on code.␟本章は、<productname>PostgreSQL</productname>のコアシステムと、テーブルの格納を制御する<firstterm>テーブルアクセスメソッド</firstterm>とのインタフェースを説明します。
コアシステムはこのアクセスメソッドについて、ここで指定されたことのみを把握しています。これにより、追加コードを記述することで全く新しいアクセスメソッド種類を開発することができます。␞␞ </para>␞
␝ <para>␟  Each table access method is described by a row in the <link
  ...
  linkend="sql-drop-access-method"/> SQL commands.␟各テーブルアクセスメソッドは<link linkend="catalog-pg-am"><structname>pg_am</structname></link>システムカタログの行で記述されます。
...
␝ <para>␟  A table access method handler function must be declared to accept a single
  ...
  <filename>src/include/access/tableam.h</filename></ulink> file.␟テーブルアクセスメソッドのハンドラ関数は<type>internal</type>型の引数を一つ取り、<type>table_am_handler</type>疑似型を返すように宣言されなければなりません。
...
詳細は、<ulink url="https://git.postgresql.org/gitweb/?p=postgresql.git;a=blob;f=src/include/access/tableam.h;hb=HEAD"><filename>src/include/access/tableam.h</filename></ulink>ファイルを参照してください。␞␞ </para>␞
␝ <para>␟  To implement an access method, an implementor will typically need to
  ...
  <ulink url="https://git.postgresql.org/gitweb/?p=postgresql.git;a=blob;f=src/include/executor/tuptable.h;hb=HEAD">
   ...
   to access the columns of the tuple.␟アクセスメソッドを実装するには開発者は通常、タプルテーブルスロットの<acronym>AM</acronym>固有の型を実装する必要があります（<ulink url="https://git.postgresql.org/gitweb/?p=postgresql.git;a=blob;f=src/include/executor/tuptable.h;hb=HEAD"><filename>src/include/executor/tuptable.h</filename></ulink>を参照してください）。
これはアクセスメソッド外のコードが、AMのタプルへの参照を保持できるようにして、そのタプルの列にアクセスできるようにするものです。␞␞ </para>␞
␝ <para>␟  Currently, the way an AM actually stores data is fairly unconstrained. For
  ...
  <xref linkend="storage-page-layout"/>.␟今のところAMが実際にデータを格納する方法は全く制限されていません。
例えば、postgresの共有バッファキャッシュを使うことも、必須ではありませんが、可能です。
使う場合、おそらく<xref linkend="storage-page-layout"/>に記述された<productname>PostgreSQL</productname>の標準ページレイアウトを使うには有意義でしょう。␞␞ </para>␞
␝ <para>␟  One fairly large constraint of the table access method API is that,
  ...
  optional), the block number needs to provide locality.␟現在のテーブルアクセスメソッドAPIのそれなりに大きい制約は、AMが更新および/またはインデックスに対応したい場合、各タプルがブロック番号とアイテム番号から成るタプル識別子（<acronym>TID</acronym>）を持つ必要があることです（<xref linkend="storage-page-layout"/>も参照してください）。
<acronym>TIDs</acronym>の下位要素が、例えば<literal>heap</literal>に対して持つのと同じ意味を持つことは、厳密には必要ありません。しかし、ビットマップスキャン対応（これは任意です）を望むなら、ブロック番号は局所性を備える必要があります。␞␞ </para>␞
␝ <para>␟  For crash safety, an AM can use postgres' <link
  ...
  implemented.␟クラッシュ安全性のために、AMはpostgresの<link linkend="wal"><acronym>WAL</acronym></link>、あるいは、カスタム実装を使うことができます。
<acronym>WAL</acronym>を選んだ場合、<link linkend="generic-wal">汎用WALレコード</link>を利用するか、<link linkend="custom-rmgr">カスタムWALリソースマネージャ</link>を実装することができます。␞␞ </para>␞
␝ <para>␟  To implement transactional support in a manner that allows different table
  access methods be accessed within a single transaction, it likely is
  necessary to closely integrate with the machinery in
  <filename>src/backend/access/transam/xlog.c</filename>.␟異なるテーブルアクセスメソッドが単一トランザクション内でアクセスできるという類のトランザクション対応を実装するには、おそらく<filename>src/backend/access/transam/xlog.c</filename>の仕組みと注意深く統合することが必要でしょう。␞␞ </para>␞
␝ <para>␟  Any developer of a new <literal>table access method</literal> can refer to
  ...
  its implementation.␟新<literal>テーブルアクセスメソッド</literal>の開発者は、実装の詳細について、<filename>src/backend/access/heap/heapam_handler.c</filename>にある既存の<literal>heap</literal>の実装を参照できます。␞␞ </para>␞
