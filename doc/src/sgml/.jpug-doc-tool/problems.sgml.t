␝<sect1 id="bug-reporting">␟ <title>Bug Reporting Guidelines</title>␟ <title>バグレポートガイドライン</title>␞␞␞
␝ <para>␟  When you find a bug in <productname>PostgreSQL</productname> we want to
  ...
  will work on every platform under every circumstance.␟<productname>PostgreSQL</productname>に関してバグを発見した場合、ぜひ我々に連絡してください。
最大限の注意を払っても、全てのプラットフォーム、全ての環境で<productname>PostgreSQL</productname>の機能全てが正常に動くことは保証できませんので、バグレポートは<productname>PostgreSQL</productname>をより信頼性の高いものにするために、大変重要になります。␞␞ </para>␞
␝ <para>␟  The following suggestions are intended to assist you in forming bug reports
  that can be handled in an effective fashion. No one is required to follow
  them but doing so tends to be to everyone's advantage.␟下記の助言は、有効に活用され得るバグレポートを作成する際に、作成者を支援することを狙ったものです。
これに従う義務はありませんが、沿った方がより有益なものとなるでしょう。␞␞ </para>␞
␝ <para>␟  We cannot promise to fix every bug right away. If the bug is obvious, critical,
  ...
  If you need help immediately, consider obtaining a commercial support contract.␟私たちは、すべてのバグを直ちに修正することを約束することはできません。
...
早急に処置が必要な場合は、商用サポートの購入を検討してください。␞␞ </para>␞
␝ <sect2 id="bug-reporting-identifying-bugs">␟  <title>Identifying Bugs</title>␟  <title>バグの特定</title>␞␞␞
␝  <para>␟   Before you report a bug, please read and re-read the
   ...
   the following circumstances:␟バグ報告を行う前に、ドキュメントを読み、もう一度読み返し、実行しようとしている処理が実行可能かどうか確認してください。
実行可能かどうかが不明な場合は、その旨を報告してください。
それはドキュメントのバグです。
また、ドキュメントに書かれていることと実際の結果が異なる場合にはバグとなります。
以下のような状況が考えられます。
ただし、これらに限定しているわけではありません。␞␞␞
␝     <para>␟      A program terminates with a fatal signal or an operating system
      error message that would point to a problem in the program. (A
      counterexample might be a <quote>disk full</quote> message,
      since you have to fix that yourself.)␟プログラムが致命的なシグナル、またはオペレーティングシステムのエラーメッセージで終了し、それがプログラム内部の問題を指摘している場合。
（逆に、<quote>disk full</quote>のようなメッセージはプログラムの問題ではありませんから、この場合は自分で修正してください。）␞␞     </para>␞
␝     <para>␟      A program produces the wrong output for any given input.␟あるプログラムで、入力された値に対して間違った結果を返す場合。␞␞     </para>␞
␝     <para>␟      A program refuses to accept valid input (as defined in the documentation).␟（ドキュメントで定義されている）有効な値を入力してもプログラムで受け付けない場合。␞␞     </para>␞
␝     <para>␟      A program accepts invalid input without a notice or error message.
      But keep in mind that your idea of invalid input might be our idea of
      an extension or compatibility with traditional practice.␟プログラムが、無効な入力値を通知やエラーメッセージなどを表示せずに受け入れる場合。
ただし、無効な入力と思われるものでも、拡張、あるいは過去の経緯による互換性と考えられている可能性があることに注意してください。␞␞     </para>␞
␝     <para>␟      <productname>PostgreSQL</productname> fails to compile, build, or
      install according to the instructions on supported platforms.␟サポートされているプラットフォームで、<productname>PostgreSQL</productname>が手順通りにコンパイル、ビルド、インストールできない場合。␞␞     </para>␞
␝␟   Here <quote>program</quote> refers to any executable, not only the backend process.␟ここでは、<quote>プログラム</quote>とはバックエンドプロセスだけではなく、すべての実行可能ファイルを意味します。␞␞  </para>␞
␝  <para>␟   Being slow or resource-hogging is not necessarily a bug. Read the
   ...
   specific feature is explicitly claimed.␟プログラムの実行が遅かったり、リソースを大量に使用するのは必ずしもバグではありません。
アプリケーションを改善するためには、ドキュメントを読んだり、どこかのメーリングリストで尋ねてみたりしてください。
標準<acronym>SQL</acronym>の要求に応じない場合も、その機能の互換性を明確にうたっていない限り、バグとは言えません。␞␞  </para>␞
␝  <para>␟   Before you continue, check on the TODO list and in the FAQ to see if your bug is
   already known. If you cannot decode the information on the TODO list, report your
   problem. The least we can do is make the TODO list clearer.␟以降に進む前に、TODOリストやFAQを参照して、そのバグが既知のものかどうか確認してください。
もしTODOリストの情報を読み取ることができなければ、問題を報告してください。
少なくともTODOリストを分かりやすくすることができます。␞␞  </para>␞
␝ <sect2 id="bug-reporting-what-to-report">␟  <title>What to Report</title>␟  <title>報告すべきこと</title>␞␞␞
␝  <para>␟   The most important thing to remember about bug reporting is to state all
   ...
   anyway.␟バグ報告で最も重要なことは、全ての事実を、そして事実のみを明確に記述することです。
...
ありのままの事実を報告することは、単刀直入（多くの場合は画面からメッセージをコピー&amp;ペーストを行うのみ）ですが、えてして、重要でないだろうと想像したり、省いても理解してもらえるだろうという思い込みによって、重要な情報がもれてしまう場合がかなり多くあります。␞␞  </para>␞
␝  <para>␟   The following items should be contained in every bug report:␟全てのバグ報告では、下記の内容が記述されていなければいけません。␞␞␞
␝     <para>␟      The exact sequence of steps <emphasis>from program
      ...
      we would probably miss the problem.␟問題を再現できるように、<emphasis>プログラムの起動から</emphasis>行った作業を順序通りに記述してください。
...
我々は、ユーザのデータベーススキーマをリバースエンジニアリングするための時間を取ることができませんし、推測してデータを構築したとしても、おそらく間違えることになるでしょう。␞␞     </para>␞
␝     <para>␟      The best format for a test case for SQL-related problems is a
      ...
      way.␟SQL関連の問題のテストケースの最適な書式は、<application>psql</application>フロントエンドに直接読み込ませて問題を再現できるファイルを用意することです。
...
どのような方法であれ、バグが再現できればよいのです。␞␞     </para>␞
␝     <para>␟      If your application uses some other client interface, such as <application>PHP</application>, then
      ...
      information is too inexact to be of use.␟アプリケーションが<application>PHP</application>など何か別のクライアントインタフェースを使用している場合、問題となる問い合わせを切り出してください。
...
こうした情報は不正確過ぎて役に立ちません。␞␞     </para>␞
␝     <para>␟      The output you got. Please do not say that it <quote>didn't work</quote> or
      ...
      from the terminal, if possible.␟得られた出力そのもの。
...
最も容易な方法は、出力をターミナルからコピーすることです。␞␞     </para>␞
␝      <para>␟       If you are reporting an error message, please obtain the most verbose
       ...
       details are logged.␟エラーメッセージを報告する場合、そのメッセージを最大限詳細に取得してください。
<application>psql</application>では、前もって<literal>\set VERBOSITY verbose</literal>を指定してください。
サーバログからメッセージを取りだす場合は、全ての詳細をログに取得できるように<xref linkend="guc-log-error-verbosity"/>実行時パラメータを<literal>verbose</literal>に設定してください。␞␞      </para>␞
␝      <para>␟       In case of fatal errors, the error message reported by the client might
       ...
       output, this would be a good time to start doing so.␟致命的なエラーが起こった場合、クライアント側で報告されるエラーメッセージには得られる情報が全て書かれているとは限りません。
データベースサーバのログも見てみてください。
もしログを取っていないならば、取る習慣を付けるいいタイミングです。␞␞      </para>␞
␝     <para>␟      The output you expected is very important to state. If you just write
      ...
      obviously omit this item.)␟どのような出力を望んでいたのかを記述することも非常に重要です。
...
（問題がプログラムのクラッシュである場合、この内容は言うまでもなく省略できます。）␞␞     </para>␞
␝     <para>␟      Any command line options and other start-up options, including
      ...
      out how that is done.␟すべてのコマンドラインオプションと起動時のオプション、デフォルトから変更した関連する環境変数や設定ファイル。
ここでも、正確な情報を提供してください。
OSの起動時にデータベースサーバを起動するようにパッケージされたディストリビューションを使用している場合は、それらがどのように実行されているかを確認する必要があります。␞␞     </para>␞
␝     <para>␟      Anything you did at all differently from the installation
      instructions.␟インストールの手順書から変更して実行したすべての内容。␞␞     </para>␞
␝     <para>␟      The <productname>PostgreSQL</productname> version. You can run the command
      ...
      snapshot, mention that, including the commit hash.␟<productname>PostgreSQL</productname>のバージョン。
...
Git版に対するバグ報告の場合は、その旨も記載し、コミットハッシュの情報も含めてください。␞␞     </para>␞
␝     <para>␟      If your version is older than &version; we will almost certainly
      ...
      commercial support contract.␟&version;よりもバージョンが古い場合、アップグレードすることをお勧めします。
新しいリリースでは多くのバグ修正や改良がなされているからです。
ですので、古めの<productname>PostgreSQL</productname>のリリースを使用していて遭遇した不具合が修正されている可能性がかなりあります。
古い<productname>PostgreSQL</productname>のリリースを使用しているサイトに対して、我々は限定されたサポートしか提供することができません。
それ以上のサポートが必要であれば、商用サポート契約を結ぶことを検討してください。␞␞     </para>␞
␝     <para>␟      Platform information. This includes the kernel name and version,
      ...
      on) is also necessary.␟プラットフォーム情報。
カーネル名とバージョン、Cライブラリ、プロセッサ、メモリ情報なども含めて報告してください。
多くの場合、ベンダ名とそのバージョンを明記するだけで十分ですが、<quote>Debian</quote>の正確な構成要素を全ての人間が把握している、であるとか、全ての人間がx86_64を使用しているなどの思い込みは止めてください。
インストールに関する問題の場合は、マシンのツール群（コンパイラや<application>make</application>など）の情報も必要となります。␞␞     </para>␞
␝␟   Do not be afraid if your bug report becomes rather lengthy. That is a fact of life.
   ...
   that outlines some more tips on reporting bugs.␟バグ報告が長文になってもそれは仕方がないことなので、気にしないでください。
最初に全ての情報を入手できる方が、開発者が事実を聞き出さなければいけない状況よりも良いです。
その一方、ファイルが大きいならば、その情報に誰か興味があるかを最初に尋ねるのが得策かもしれません。
<ulink url="https://www.chiark.greenend.org.uk/~sgtatham/bugs.html">記事</ulink>には、バグ報告に関するその他のコツの概要があります。␞␞  </para>␞
␝  <para>␟   Do not spend all your time to figure out which changes in the input make
   ...
   guessing why the bug exists. We will find that out soon enough.␟問題を解決する入力を見つけ出すための試行錯誤に時間をかけないでください。
これはおそらく問題解決の助けになりません。
バグが即座に修正されない場合、その時間を利用すれば、あなた自身のワークアラウンドを探して共有できます。
繰り返しになりますが、バグがなぜあるのかを解明するのに余計な時間をかける必要はありません。
開発者の方が十分速くそれを見つけ出します。␞␞  </para>␞
␝  <para>␟   When writing a bug report, please avoid confusing terminology.
   ...
   about whether the problem is on the client or server side.␟バグ報告をする際、理解しやすい用語を使用してください。
...
1つのバックエンドプロセスのクラッシュと、その親プロセス<quote>postgres</quote>のクラッシュとはかなり異なります。
...
問題がクライアント側かサーバ側かの切り分けを試みてください。␞␞  </para>␞
␝ <sect2 id="bug-reporting-where-to-report-bugs">␟  <title>Where to Report Bugs</title>␟  <title>バグ報告先</title>␞␞␞
␝  <para>␟   In general, send bug reports to the bug report mailing list at
   <email>pgsql-bugs@lists.postgresql.org</email>.
   You are requested to use a descriptive subject for your email
   message, perhaps parts of the error message.␟一般論として、バグ報告は<email>pgsql-bugs@lists.postgresql.org</email>というバグ報告用メーリングリストに送ってください。
バグ報告の題名には、エラーメッセージの一部分などわかりやすいものを使ってください。␞␞  </para>␞
␝  <para>␟   Another method is to fill in the bug report web-form available
   at the project's
   <ulink url="https://www.postgresql.org/">web site</ulink>.
   Entering a bug report this way causes it to be mailed to the
   <email>pgsql-bugs@lists.postgresql.org</email> mailing list.␟その他の方法として、プロジェクトの<ulink url="https://www.postgresql.org/">Webサイト</ulink>にあるバグ報告フォームの項目を埋める方法があります。
この方法で入力したバグ報告は、<email>pgsql-bugs@lists.postgresql.org</email>メーリングリストに送信されます。␞␞  </para>␞
␝  <para>␟   If your bug report has security implications and you'd prefer that it
   not become immediately visible in public archives, don't send it to
   <literal>pgsql-bugs</literal>.  Security issues can be
   reported privately to <email>security@postgresql.org</email>.␟バグ報告にセキュリティが関連する場合や公開アーカイブからすぐに閲覧できることを好まない場合、<literal>pgsql-bugs</literal>には送信しないでください。
セキュリティの問題については、非公開で<email>security@postgresql.org</email>に報告できます。␞␞  </para>␞
␝  <para>␟   Do not send bug reports to any of the user mailing lists, such as
   ...
   bug reports. More importantly, they are unlikely to fix them.␟<email>pgsql-sql@lists.postgresql.org</email>や<email>pgsql-general@lists.postgresql.org</email>などのユーザ向けのメーリングリストには決してバグ報告を送らないでください。
これらのメーリングリストはユーザからの質問に答えるためのもので、ほとんどの購読者はバグ報告を受け取りたくないと思われます。
さらに重要なのは、これらのリストの購読者によってバグが修正されることはほとんどないということです。␞␞  </para>␞
␝  <para>␟   Also, please do <emphasis>not</emphasis> send reports to
   ...
   if the problem needs more review.␟また、開発者向けの<email>pgsql-hackers@lists.postgresql.org</email>にもバグ報告を<emphasis>送らないで</emphasis>ください。
ここは<productname>PostgreSQL</productname>の開発に関して議論する場で、バグ報告とは切り離している方が良いとされています。
もしその問題により多くのレビューが必要な場合は、そのバグ報告を<literal>pgsql-hackers</literal>で議論することになります。␞␞  </para>␞
␝  <para>␟   If you have a problem with the documentation, the best place to report it
   ...
   with.␟ドキュメントに関して問題がある場合は、ドキュメント用のメーリングリスト、<email>pgsql-docs@lists.postgresql.org</email>が最適な報告先です。
その際、問題になった箇所がどの部分かを明記してください。␞␞  </para>␞
␝  <para>␟   If your bug is a portability problem on a non-supported platform,
   send mail to <email>pgsql-hackers@lists.postgresql.org</email>,
   so we (and you) can work on
   porting <productname>PostgreSQL</productname> to your platform.␟また、サポートされていないプラットフォームへの移植に関係するバグ報告である場合は<email>pgsql-hackers@lists.postgresql.org</email>に報告してください。
そのプラットフォームへ<productname>PostgreSQL</productname>を移植するように（報告者と一緒に）最善の努力をします。␞␞  </para>␞
␝   <para>␟    Due to the unfortunate amount of spam going around, all of the above
    ...
    <ulink url="https://lists.postgresql.org/"></ulink> for instructions.␟嘆かわしい量のスパムメールが出回っているため、これらのメーリングリストは購読しない限りモデレータ付きとなっています。
これはEメールが配送されるのにいくらか遅延があることを意味します。
メーリングリストを購読したい場合には、<ulink url="https://lists.postgresql.org/"></ulink>を訪ねて、その指示に従ってください。␞␞   </para>␞
