...
␝ <chapter id="triggers">␟  <title>Triggers</title>␟  <title>トリガ</title>␞␞␞
␝  <para>␟   This chapter provides general information about writing trigger functions.
   ...
   details of writing a trigger in it.␟本章ではトリガ関数の作成に関する一般的な情報を示します。
...
本章を読んだ後、好みの手続き言語に関する章を参照して、トリガ作成に関する言語特有の詳細を確認すべきです。␞␞  </para>␞
␝  <para>␟   It is also possible to write a trigger function in C, although
   ...
   plain SQL function language.␟また、C言語でトリガ関数を作成することができます。
しかし、ほとんどの方は、手続き言語のいずれかで作成する方が簡単であることに気づくでしょう。
現時点では、普通のSQL関数言語ではトリガ関数を作成することはできません。␞␞  </para>␞
␝  <sect1 id="trigger-definition">␟   <title>Overview of Trigger Behavior</title>␟   <title>トリガ動作の概要</title>␞␞␞
␝   <para>␟    A trigger is a specification that the database should automatically
    execute a particular function whenever a certain type of operation is
    performed.  Triggers can be attached to tables (partitioned or not),
    views, and foreign tables.␟トリガとは、データベースが、ある特定の操作が行われた時に常に自動的に実行しなければならない特定の機能に関する規定です。
トリガはテーブル（パーティション化されているかどうかにかかわらず）、ビュー、外部テーブルに付与することができます。␞␞  </para>␞
␝  <para>␟    On tables and foreign tables, triggers can be defined to execute either
    ...
    event.␟テーブルおよび外部テーブル上では、トリガを<command>INSERT</command>、<command>UPDATE</command>または<command>DELETE</command>操作の前後に、行を変更する度、あるいは<acronym>SQL</acronym>文ごとに実行するように定義することができます。
...
トリガイベントが起こると、トリガ関数がそのイベントを扱う適切な時点で呼び出されます。␞␞   </para>␞
␝   <para>␟    On views, triggers can be defined to execute instead of
    ...
    that will be fired are the ones attached to the base table(s).␟ビュー上では、トリガを<command>INSERT</command>、<command>UPDATE</command>または<command>DELETE</command>操作の代わりに実行するものとして定義できます。
...
その結果、発行されるトリガは、基底テーブルに付けられたトリガとなります。␞␞   </para>␞
␝   <para>␟    The trigger function must be defined before the trigger itself can be
    ...
    arguments.)␟トリガ関数は、トリガ自体が作成される前までに定義しておく必要があります。
トリガ関数は、引数を取らない、<literal>trigger</literal>型を返す関数として宣言される必要があります。
（トリガ関数は、通常の関数で使用される引数という形ではなく、<structname>TriggerData</structname>構造体で入力を受け取ります。）␞␞   </para>␞
␝   <para>␟    Once a suitable trigger function has been created, the trigger is
    established with
    <xref linkend="sql-createtrigger"/>.
    The same trigger function can be used for multiple triggers.␟適切なトリガ関数が作成されると、<xref linkend="sql-createtrigger"/>を使用してトリガを構築することができます。
同一のトリガ関数を複数のトリガに使用することができます。␞␞   </para>␞
␝   <para>␟    <productname>PostgreSQL</productname> offers both <firstterm>per-row</firstterm>
    ...
    defined at statement level, not per-row.␟<productname>PostgreSQL</productname>は、<firstterm>行単位の</firstterm>トリガと<firstterm>文単位の</firstterm>トリガの両方を提供します。
...
<command>TRUNCATE</command>に対するトリガは、行単位ではなく、文レベルにのみに定義することができます。␞␞   </para>␞
␝   <para>␟    Triggers are also classified according to whether they fire
    ...
    row in the view is identified as needing to be operated on.␟また、トリガはそれらが操作の<firstterm>前</firstterm>、<firstterm>後</firstterm>または<firstterm>代わり</firstterm>のどれで実行されるかに応じて分けられます。
...
つまり、ビュー上のそれぞれの行で処理が必要と判断された場合には、即座に発動します。␞␞   </para>␞
␝   <para>␟    The execution of an <literal>AFTER</literal> trigger can be deferred
    ...
    trigger causes an error, the effects of both will be rolled back.␟<firstterm>制約トリガ</firstterm>として定義されている場合、<literal>AFTER</literal>トリガの実行は、トリガ実行を文の終わりではなく、トランザクションの終わりまで保留することができます。
すべての場合において、トリガはトリガが引き金となった文と同じトランザクションの一部として実行されるため、文またはトリガのいずれかがエラーを引き起こした場合、両方の結果がロールバックされます。␞␞   </para>␞
␝   <para>␟    If an <command>INSERT</command> contains an <literal>ON CONFLICT
    ...
    including changes to <varname>EXCLUDED</varname> columns.␟<command>INSERT</command>が<literal>ON CONFLICT DO UPDATE</literal>句を含む場合、トリガの発生した行では、行レベルの<literal>BEFORE</literal> <command>INSERT</command>トリガが実行され、次に行レベルの<literal>BEFORE</literal> <command>UPDATE</command>トリガが実行される可能性があります。
<literal>BEFORE</literal> <command>INSERT</command>トリガによって行われた変更が、<varname>EXCLUDED</varname>列への変更を含めて、<literal>BEFORE</literal> <command>UPDATE</command>トリガから見えるので、トリガが冪等ではない場合、このような相互作用は複雑になる可能性があります。␞␞   </para>␞
␝   <para>␟    Note that statement-level
    ...
    triggers.␟文レベルの<command>UPDATE</command>トリガは<literal>ON CONFLICT DO UPDATE</literal>が指定されたとき、その<command>UPDATE</command>によって行が影響を受けたかどうかに関わらず（そしてその代替である<command>UPDATE</command>部分が実行されたかどうかに関わらず）実行されることに注意してください。
<literal>ON CONFLICT DO UPDATE</literal>句のある<command>INSERT</command>では、まず文レベルの<literal>BEFORE</literal> <command>INSERT</command>トリガ、次に文レベルの<literal>BEFORE</literal> <command>UPDATE</command>トリガ、次いで文レベルの<literal>AFTER</literal> <command>UPDATE</command>トリガ、最後に文レベルの<literal>AFTER</literal> <command>INSERT</command>トリガを実行します。␞␞   </para>␞
␝   <para>␟    A statement that targets a parent table in an inheritance or partitioning
    hierarchy does not cause the statement-level triggers of affected child
    tables to be fired; only the parent table's statement-level triggers are
    fired.  However, row-level triggers of any affected child tables will be
    fired.␟継承あるいはパーティショニング階層において、親テーブルをターゲットとする文は、影響を受けた子テーブルの文レベルトリガを発動しません。
すなわち、親テーブルの文レベルトリガのみが発動します。
しかし、影響を受けた子テーブルの行レベルトリガは発動します。␞␞   </para>␞
␝   <para>␟    If an <command>UPDATE</command> on a partitioned table causes a row to move
    ...
    will be fired.␟あるパーティション化されたテーブルに適用された<command>UPDATE</command>の結果、行が他のパーティションに移動することになるなら、元のパーティションで<command>DELETE</command>し、続いて新しいパーティションに<command>INSERT</command>する操作として実行されます。
...
文レベルのトリガに関しては、たとえ行の移動が起こったとしても<command>DELETE</command>トリガも<command>INSERT</command>トリガも発動されません。<command>UPDATE</command>文中に現れた対象テーブルに定義された<command>UPDATE</command>トリガだけが発動されます。␞␞   </para>␞
␝   <para>␟    No separate triggers are defined for <command>MERGE</command>. Instead,
    ...
    actions are performed.␟<command>MERGE</command>には個別のトリガは定義されません。
代わりに、文レベルまたは行レベルの<command>UPDATE</command>、<command>DELETE</command>および<command>INSERT</command>トリガが、<command>MERGE</command>問い合わせで指定されたアクション（文レベルトリガの場合）および実行されたアクション（行レベルトリガの場合）に応じて起動されます。␞␞   </para>␞
␝   <para>␟    While running a <command>MERGE</command> command, statement-level
    ...
    are fired for the same kind of action.␟<command>MERGE</command>コマンドの実行中、文レベルの<literal>BEFORE</literal>および<literal>AFTER</literal>トリガは、アクションが最終的に実行されるかどうかに関係なく、<command>MERGE</command>コマンドのアクションで指定されたイベントに対して発行されます。
これは、行を更新しない<command>UPDATE</command>文と同じですが、文レベルのトリガは発行されます。
行レベルのトリガは、行が実際に更新、挿入または削除されたときにのみ発行されます。
したがって、文レベルのトリガが特定のタイプのアクションに対して発行されるものの、同じ種類のアクションに対して行レベルのトリガが発行されないことは完全に合法です。␞␞   </para>␞
␝   <para>␟    Trigger functions invoked by per-statement triggers should always
    ...
    the following choices:␟文単位のトリガによって呼び出されるトリガ関数は常に<symbol>NULL</symbol>を返さなければなりません。
...
␝      <para>␟       It can return <symbol>NULL</symbol> to skip the operation for the
       current row. This instructs the executor to not perform the
       row-level operation that invoked the trigger (the insertion,
       modification, or deletion of a particular table row).␟<symbol>NULL</symbol>を返して、現在の行への操作をスキップできます。
これは、エグゼキュータにトリガの元になった行レベルの操作（特定のテーブル行の挿入、更新、削除）を行わないよう指示します。␞␞      </para>␞
␝      <para>␟       For row-level <command>INSERT</command>
       ...
       row being inserted or updated.␟行レベルの<command>INSERT</command>および<command>UPDATE</command>トリガの場合のみ、返される行が挿入される、もしくは実際に更新される行になります。
これにより、トリガ関数で、挿入される行もしくは更新される行を変更することができます。␞␞      </para>␞
␝␟    A row-level <literal>BEFORE</literal> trigger that does not intend to cause
    ...
    <command>DELETE</command> triggers).␟これらの動作をさせたくない行レベルの<literal>BEFORE</literal>トリガについては、渡された行（つまり、<command>INSERT</command>および<command>UPDATE</command>トリガでは<varname>NEW</varname>行、<command>DELETE</command>の場合は<varname>OLD</varname>行）と同じ行結果を返すように気を付ける必要があります。␞␞   </para>␞
␝   <para>␟    A row-level <literal>INSTEAD OF</literal> trigger should either return
    ...
    that was provided.␟行レベルの<literal>INSTEAD OF</literal>トリガは、ビューの元となった元テーブルのデータをまったく変更しないことを表す<symbol>NULL</symbol>、または、渡されたビューの行（<command>INSERT</command>と<command>UPDATE</command>操作の場合<varname>NEW</varname>行、<command>DELETE</command>操作の場合<varname>OLD</varname>行）を返さなければなりません。
...
これは<command>INSERT RETURNING</command>または<command>UPDATE RETURNING</command>で返されるデータを変更しますので、ビューが提供されたデータと正確に同じ結果を返さない場合に有益です。␞␞   </para>␞
␝   <para>␟    The return value is ignored for row-level triggers fired after an
    operation, and so they can return <symbol>NULL</symbol>.␟操作の後に発生する行レベルトリガでは戻り値は無視されますので、これらは<symbol>NULL</symbol>を返すことができます。␞␞   </para>␞
␝   <para>␟    Some considerations apply for generated
    ...
    overwritten.␟生成列に対してはいくつか考慮が必要です。<indexterm><primary>生成列</primary><secondary>トリガでの</secondary></indexterm>
...
<literal>BEFORE</literal>トリガでの生成列の値の変更は無視され、上書きされます。␞␞   </para>␞
␝   <para>␟    If more than one trigger is defined for the same event on the same
    ...
    triggers are not fired (for that row).␟同一リレーション、同一イベントに対して1つ以上のトリガが定義された場合、トリガはその名前のアルファベット順に発生します。
<literal>BEFORE</literal>トリガと<literal>INSTEAD OF</literal>トリガの場合では、各トリガで返される、変更された可能性がある行が次のトリガの入力となります。
もし、ある<literal>BEFORE</literal>トリガや<literal>INSTEAD OF</literal>トリガが<symbol>NULL</symbol>を返したら、(いまのところ)操作はその行で中断し、残りのトリガは発生しません。␞␞   </para>␞
␝   <para>␟    A trigger definition can also specify a Boolean <literal>WHEN</literal>
    ...
    <literal>WHEN</literal> conditions.␟トリガ定義は、トリガを発動するかどうかを<literal>WHEN</literal>句の論理条件で指定することも可能です。
...
<literal>INSTEAD OF</literal>トリガは<literal>WHEN</literal>条件をサポートしていません。␞␞   </para>␞
␝   <para>␟    Typically, row-level <literal>BEFORE</literal> triggers are used for checking or
    ...
    the operation doesn't have to be saved until end of statement.␟通常、行レベルの<literal>BEFORE</literal>トリガは、挿入あるいは更新される予定のデータの検査や変更のために使用されます。
...
トリガを<literal>BEFORE</literal>にするか<literal>AFTER</literal>にするかを決める時に特別な理由がないのであれば、操作の情報を行が終わるまで保持する必要がない分、<literal>BEFORE</literal>を使う方が効率的です。␞␞   </para>␞
␝   <para>␟    If a trigger function executes SQL commands then these
    ...
    to avoid infinite recursion in such scenarios.␟トリガ関数がSQLコマンドを処理する場合、これらの問い合わせがトリガを再度発行することがあります。
...
こうした状況で無限再帰を防ぐのは、トリガプログラマの責任です。␞␞   </para>␞
␝    <indexterm>
     <primary>trigger</primary>
...
␝   <para>␟    If a foreign key constraint specifies referential actions (that
    ...
    responsibility to avoid that.␟外部キー制約が参照アクション（すなわち、カスケード更新または削除）を指定している場合、これらのアクションは、参照テーブル上の通常のSQL更新または削除コマンドを介して実行されます。
特に、参照テーブルに存在するトリガは、これらの変更に対して起動されます。
このようなトリガがこれらのコマンドのいずれかの効果を変更またはブロックすると、最終的に参照整合性を破壊する可能性があります。
それを回避するのはトリガプログラマの責任です。␞␞   </para>␞
␝    </indexterm>␟    When a trigger is being defined, arguments can be specified for
    ...
    trigger.␟トリガを定義する時、そのトリガ用の引数を指定することができます。
...
また、<command>UPDATE</command>として定義すれば、最終更新イベントを追跡するために使用することも可能です。␞␞   </para>␞
␝   <para>␟    Each programming language that supports triggers has its own method
    ...
    for <command>UPDATE</command> and <command>DELETE</command> triggers.␟トリガをサポートするプログラミング言語はそれぞれ独自の方法で、トリガ関数で利用できるトリガの入力データを作成します。
...
行レベルトリガの入力データには、<command>INSERT</command>および<command>UPDATE</command>トリガの場合は<varname>NEW</varname>行が、<command>UPDATE</command>および<command>DELETE</command>トリガの場合は<varname>OLD</varname>行が含まれます。␞␞   </para>␞
␝   <para>␟    By default, statement-level triggers do not have any way to examine the
    ...
    commands issued within the trigger function.␟デフォルトでは、文レベルトリガには文によって変更された個々の行を検査するための手段がありません。
...
遷移テーブルを検査する方法も使用中のプログラミング言語に依存しますが、典型的な方法は、トリガ関数の中で発行するSQLコマンドでアクセスできる、読み込み専用の一時テーブルのように振る舞う遷移テーブルを作成することです。␞␞   </para>␞
␝  <sect1 id="trigger-datachanges">␟   <title>Visibility of Data Changes</title>␟   <title>データ変更の可視性</title>␞␞␞
␝   <para>␟    If you execute SQL commands in your trigger function, and these
    ...
    is fired for.  Briefly:␟トリガ関数内でSQLコマンドを実行し、このコマンドがトリガの元となったテーブルにアクセスする場合、データの可視性規則に注意する必要があります。
この規則が、SQLコマンドがトリガの発行原因となったデータ変更を見ることができるかどうかを決定するからです。
簡単に以下に示します。␞␞␞
␝      <para>␟       Statement-level triggers follow simple visibility rules: none of
       ...
       triggers.␟文レベルトリガは次に示す簡単な可視性規則に従います。
文によってなされた変更は、文レベルの<literal>BEFORE</literal>トリガでは不可視です。
一方、文レベルの<literal>AFTER</literal>トリガでは全ての変更が可視です。␞␞      </para>␞
␝      <para>␟       The data change (insertion, update, or deletion) causing the
       ...
       because it hasn't happened yet.␟当然ながら行レベルの<literal>BEFORE</literal>トリガ内のSQLコマンドでは、トリガの発生原因となったデータ変更（挿入、更新、削除）はまだ発生していませんので、可視<emphasis>ではありません</emphasis>。␞␞      </para>␞
␝      <para>␟       However, SQL commands executed in a row-level <literal>BEFORE</literal>
       ...
       affects multiple rows can visit the rows in any order.␟しかし、行レベルの<literal>BEFORE</literal>トリガで実行されるSQLコマンドは、その外側のコマンドで以前に処理された行へのデータ変更の影響を見る<emphasis>ことになるでしょう</emphasis>。
これらの変更イベントの順序は一般的に予測できませんので、注意が必要です。
複数行に影響するSQLコマンドはどのような順番でもその行を更新することができます。␞␞      </para>␞
␝      <para>␟       Similarly, a row-level <literal>INSTEAD OF</literal> trigger will see the
       effects of data changes made by previous firings of <literal>INSTEAD
       OF</literal> triggers in the same outer command.␟同様に、行レベルの<literal>INSTEAD OF</literal>トリガは、同じ外側のコマンドで以前に処理された<literal>INSTEAD OF</literal>トリガよる変更結果を見ることになるでしょう。␞␞      </para>␞
␝      <para>␟       When a row-level <literal>AFTER</literal> trigger is fired, all data
       changes made
       by the outer command are already complete, and are visible to
       the invoked trigger function.␟行レベルの<literal>AFTER</literal>トリガが発生すると、その外側のコマンドによってなされた全ての変更は既に完了していますので、呼び出されたトリガ関数から可視になります。␞␞      </para>␞
␝   <para>␟    If your trigger function is written in any of the standard procedural
    ...
    the calling command in any case.␟もし、あなたのトリガが標準的な手続き型言語のいずれかで記述されている時、上記の可視性は関数が<literal>VOLATILE</literal>で定義されている場合のみ適用されます。
<literal>STABLE</literal>、もしくは<literal>IMMUTABLE</literal>で定義されている関数は、どのようなケースにおいても、呼び出しコマンドによる変更は見ないでしょう。␞␞   </para>␞
␝   <para>␟    Further information about data visibility rules can be found in
    <xref linkend="spi-visibility"/>.  The example in <xref
    linkend="trigger-example"/> contains a demonstration of these rules.␟データ可視性規則に関する詳細は<xref linkend="spi-visibility"/>にあります。
<xref linkend="trigger-example"/>の例にこの規則を示します。␞␞   </para>␞
...
␝  <sect1 id="trigger-interface">␟   <title>Writing Trigger Functions in C</title>␟   <title>Cによるトリガ関数の作成</title>␞␞␞
␝   <para>␟    This section describes the low-level details of the interface to a
    ...
    trigger in that language.␟本節ではトリガ関数とのインタフェースについて低レベルな詳細を説明します。
この情報はC言語でトリガ関数を作成する時にのみ必要です。
高レベルな言語で作成すれば、こうした詳細は代わりに扱ってもらえます。
たいていの場合、Cでトリガを作成する前に手続き言語を使用することを検討すべきです。
各手続き言語の文書で、その言語を使用したトリガの作成方法を説明します。␞␞   </para>␞
␝   <para>␟    Trigger functions must use the <quote>version 1</quote> function manager
    interface.␟トリガ関数は<quote>version 1</quote>関数マネージャインタフェースを使わなくてはいけません。␞␞   </para>␞
␝   <para>␟    When a function is called by the trigger manager, it is not passed
    ...
    manager or not by executing the macro:␟関数がトリガマネージャから呼び出される時は、通常の引数が渡されるのではなく、<structname>TriggerData</structname>構造体を指す<quote>context</quote>ポインタが渡されます。
C関数は、トリガマネージャから呼び出されたのかどうかを以下のマクロを実行することで検査することができます。␞␞<programlisting>␞
␝</programlisting>␟    which expands to:␟これは以下に展開されます。␞␞<programlisting>␞
␝</programlisting>␟    If this returns true, then it is safe to cast
    ...
    *</literal> and make use of the pointed-to
    ...
    structure or any of the data it points to.␟もしこれが真を返す場合、<literal>fcinfo-&gt;context</literal>を<literal>TriggerData *</literal>型にキャストし、指された<structname>TriggerData</structname>構造体を使用することは安全です。
その関数は、<structname>TriggerData</structname>構造体やそれが指すどのようなデータも変更しては<emphasis>いけません</emphasis>。␞␞   </para>␞
␝   <para>␟    <structname>struct TriggerData</structname> is defined in
    <filename>commands/trigger.h</filename>:␟<structname>struct TriggerData</structname>は<filename>commands/trigger.h</filename>の中で定義されています。␞␞␞
␝␟    where the members are defined as follows:␟メンバは下記のように定義されています。␞␞␞
␝       <para>␟        Always <literal>T_TriggerData</literal>.␟常に<literal>T_TriggerData</literal>です。␞␞       </para>␞
␝       <para>␟        Describes the event for which the function is called. You can use the
        following macros to examine <literal>tg_event</literal>:␟その関数が呼び出されたイベントを記述します。
<literal>tg_event</literal>を調べるためには下記のマクロを使うことができます。␞␞␞
␝           <para>␟            Returns true if the trigger fired before the operation.␟トリガが操作の前に(before)発行された場合に真を返します。␞␞           </para>␞
␝           <para>␟            Returns true if the trigger fired after the operation.␟トリガが操作の後に(after)発行された場合に真を返します。␞␞           </para>␞
␝           <para>␟            Returns true if the trigger fired instead of the operation.␟トリガがINSTEAD OFで発行された場合に真を返します。␞␞           </para>␞
␝           <para>␟            Returns true if the trigger fired for a row-level event.␟トリガが行レベルのイベントで発行された場合に真を返します。␞␞           </para>␞
␝           <para>␟            Returns true if the trigger fired for a statement-level event.␟トリガが文レベルのイベントで発行された場合に真を返します。␞␞           </para>␞
␝           <para>␟            Returns true if the trigger was fired by an <command>INSERT</command> command.␟トリガが<command>INSERT</command>コマンドで発行された場合に真を返します。␞␞           </para>␞
␝           <para>␟            Returns true if the trigger was fired by an <command>UPDATE</command> command.␟トリガが<command>UPDATE</command>コマンドで発行された場合に真を返します。␞␞           </para>␞
␝           <para>␟            Returns true if the trigger was fired by a <command>DELETE</command> command.␟トリガが<command>DELETE</command>コマンドで発行された場合に真を返します。␞␞           </para>␞
␝           <para>␟            Returns true if the trigger was fired by a <command>TRUNCATE</command> command.␟トリガが<command>TRUNCATE</command>コマンドで発行された場合に真を返します。␞␞           </para>␞
␝       <para>␟        A pointer to a structure describing the relation that the trigger fired for.
        ...
        need a copy of the name).␟トリガの発行元のリレーションを記述する構造体へのポインタです。
...
名前のコピーが必要な場合は、<type>char*</type>を得るために<literal>SPI_getrelname(tg_relation)</literal>を使用してください）。␞␞       </para>␞
␝       <para>␟        A pointer to the row for which the trigger was fired. This is
        ...
        columns herein are unspecified.␟トリガが発行された行へのポインタです。
...
外部テーブルのトリガに対しては、システム列の値はここでは指定されません。␞␞       </para>␞
␝       <para>␟        A pointer to the new version of the row, if the trigger was
        ...
        skip the operation.  For triggers on foreign tables, values of system
        columns herein are unspecified.␟トリガが<command>UPDATE</command>で発行された場合は、行の新しいバージョンへのポインタです。
...
外部テーブルのトリガに対しては、システム列の値はここでは指定されません。␞␞       </para>␞
␝       <para>␟        A pointer to a structure of type <structname>Trigger</structname>,
        defined in <filename>utils/reltrigger.h</filename>:␟以下のように<filename>utils/reltrigger.h</filename>で定義された、<structname>Trigger</structname>構造体へのポインタです。␞␞␞
␝␟       where <structfield>tgname</structfield> is the trigger's name,
       ...
       only.␟ここで、<structfield>tgname</structfield>がトリガの名前、<structfield>tgnargs</structfield>が<structfield>tgargs</structfield>内の引数の数、<structfield>tgargs</structfield>は<command>CREATE TRIGGER</command>文で指定された引数へのポインタの配列です。
他のメンバは内部でのみ使用されます。␞␞       </para>␞
␝       <para>␟        The slot containing <structfield>tg_trigtuple</structfield>,
        or a <symbol>NULL</symbol> pointer if there is no such tuple.␟<structfield>tg_trigtuple</structfield>を含むスロット、またはタプルが存在しない場合は<symbol>NULL</symbol>ポインタです。␞␞       </para>␞
␝       <para>␟        The slot containing <structfield>tg_newtuple</structfield>,
        or a <symbol>NULL</symbol> pointer if there is no such tuple.␟<structfield>tg_newtuple</structfield>を含むスロット、またはタプルが存在しない場合は<symbol>NULL</symbol>ポインタです。␞␞       </para>␞
␝       <para>␟        A pointer to a structure of type <structname>Tuplestorestate</structname>
        ...
        if there is no <literal>OLD TABLE</literal> transition relation.␟<structfield>tg_relation</structfield>で指定するフォーマットの0以上の行を含む<structname>Tuplestorestate</structname>型の構造体へのポインタです。
<literal>OLD TABLE</literal>遷移リレーションが存在しない場合は<symbol>NULL</symbol>ポインタです。␞␞       </para>␞
␝       <para>␟        A pointer to a structure of type <structname>Tuplestorestate</structname>
        containing zero or more rows in the format specified by
        <structfield>tg_relation</structfield>, or a <symbol>NULL</symbol> pointer
        if there is no <literal>NEW TABLE</literal> transition relation.␟<structfield>tg_relation</structfield>で指定するフォーマットの0以上の行を含む<structname>Tuplestorestate</structname>型の構造体へのポインタです。
<literal>NEW TABLE</literal>遷移リレーションが存在しない場合は<symbol>NULL</symbol>ポインタです。␞␞       </para>␞
␝       <para>␟        For <literal>UPDATE</literal> triggers, a bitmap set indicating the
        ...
        columns that were not changed.␟<literal>UPDATE</literal>トリガに対しては、トリガコマンドにより更新された列を示すビットマップ集合です。
汎用のトリガ関数はこれを使って、変更されていない列を扱わないことで動作を最適化できます。␞␞       </para>␞
␝       <para>␟        As an example, to determine whether a column with attribute number
        <varname>attnum</varname> (1-based) is a member of this bitmap set,
        call <literal>bms_is_member(attnum -
        FirstLowInvalidHeapAttributeNumber,
        trigdata->tg_updatedcols))</literal>.␟例として、属性番号<varname>attnum</varname>(1始まり)の列がこのビットマップ集合のメンバであるかどうか判定するために、<literal>bms_is_member(attnum - FirstLowInvalidHeapAttributeNumber, trigdata->tg_updatedcols))</literal>を呼び出します。␞␞       </para>␞
␝       <para>␟        For triggers other than <literal>UPDATE</literal> triggers, this will
        be <symbol>NULL</symbol>.␟<literal>UPDATE</literal>トリガ以外のトリガに対しては、これは<symbol>NULL</symbol>になります。␞␞       </para>␞
␝   <para>␟    To allow queries issued through SPI to reference transition tables, see
    <xref linkend="spi-spi-register-trigger-data"/>.␟SPIを使って遷移テーブルを参照するクエリを発行する方法については、<xref linkend="spi-spi-register-trigger-data"/>を参照してください。␞␞   </para>␞
␝   <para>␟    A trigger function must return either a
    ...
    as appropriate, if you don't want to modify the row being operated on.␟トリガ関数は<structname>HeapTuple</structname>ポインタもしくは<symbol>NULL</symbol>ポインタ（SQLのNULLでは<emphasis>ありません</emphasis>。
したがって、<parameter>isNull</parameter>は真にはなりません）のどちらかを返さなければなりません。
操作対象の行を変更したくない場合は、注意して、<structfield>tg_trigtuple</structfield>か<structfield>tg_newtuple</structfield>の適切な方を返してください。␞␞   </para>␞
␝  <sect1 id="trigger-example">␟   <title>A Complete Trigger Example</title>␟   <title>完全なトリガの例</title>␞␞␞
␝   <para>␟    Here is a very simple example of a trigger function written in C.
    (Examples of triggers written in procedural languages can be found
    in the documentation of the procedural languages.)␟C言語で作成したトリガ関数に関するとても簡単な例をここに示します
（手続き言語で作成したトリガの例は、その手続き言語の文書に記載されています。）␞␞   </para>␞
␝   <para>␟    The function <function>trigf</function> reports the number of rows in the
    ...
    doesn't abort the transaction.)␟<function>trigf</function>関数は、<structname>ttest</structname>テーブル内にある行数を報告し、問い合わせが<structfield>x</structfield>にNULL値を挿入しようとしていた場合は、その操作をスキップします。
（つまり、このトリガは、トランザクションを中断させないNOT NULL制約のような動作をします。）␞␞   </para>␞
␝   <para>␟    First, the table definition:␟まず、以下のようにテーブルを定義します。␞␞<programlisting>␞
␝   <para>␟    This is the source code of the trigger function:␟以下がトリガ関数のソースコードです。␞␞<programlisting><![CDATA[␞
␝   <para>␟#include "executor/spi.h"       /* this is what you need to work with SPI */
#include "commands/trigger.h"   /* ... triggers ... */
#include "utils/rel.h"          /* ... and relations */␟#include "executor/spi.h"       /* これはSPIを使用する場合に必要なもの */
#include "commands/trigger.h"   /* これはトリガで必要なもの */
#include "utils/rel.h"          /* これはリレーションで必要なもの */␞␞␞
␝   <para>␟    /* make sure it's called as a trigger at all */␟    /* トリガとして呼び出されたかどうかを確認 */␞␞    if (!CALLED_AS_TRIGGER(fcinfo))␞
␝   <para>␟    /* tuple to return to executor */␟    /* エグゼキュータに返すタプル */␞␞    if (TRIGGER_FIRED_BY_UPDATE(trigdata->tg_event))␞
␝   <para>␟    /* check for null values */␟    /* NULL値をチェック */␞␞    if (!TRIGGER_FIRED_BY_DELETE(trigdata->tg_event)␞
␝   <para>␟    /* connect to SPI manager */␟    /* SPIマネージャに接続 */␞␞    if ((ret = SPI_connect()) < 0)␞
␝   <para>␟    /* get number of rows in table */␟    /* テーブル中の行数を取得 */␞␞    ret = SPI_exec("SELECT count(*) FROM ttest", 0);␞
␝   <para>␟    /* count(*) returns int8, so be careful to convert */␟    /* count(*)はint8を返すため、変換に注意してください */␞␞    i = DatumGetInt64(SPI_getbinval(SPI_tuptable->vals[0],␞
␝   <para>␟    After you have compiled the source code (see <xref
    linkend="dfunc"/>), declare the function and the triggers:␟ソースコードをコンパイル（<xref linkend="dfunc"/>を参照してください）した後に、以下の様に関数とトリガを宣言します。␞␞<programlisting>␞
␝   <para>␟    Now you can test the operation of the trigger:␟これで、トリガの操作を確認することができます。␞␞<screen>␞
␝-- Insertion skipped and AFTER trigger is not fired␟-- Insertion skipped and AFTER trigger is not fired␟-- 挿入操作はスキップされ、また、AFTERトリガも発行されません。␞␞␞
␝                                       ^^^^^^^^␟                             remember what we said about visibility.␟                             可視性の説明を思い出してください。␞␞INSERT 167793 1␞
␝                                       ^^^^^^␟                             remember what we said about visibility.␟                             可視性の説明を思い出してください。␞␞INSERT 167794 1␞
␝                                       ^^^^^^␟                             remember what we said about visibility.␟                             可視性の説明を思い出してください。␞␞DELETE 2␞
␝   <para>␟    There are more complex examples in
    <filename>src/test/regress/regress.c</filename> and
    in <xref linkend="contrib-spi"/>.␟<filename>src/test/regress/regress.c</filename>と<xref linkend="contrib-spi"/>にはもっと複雑な例があります。␞␞   </para>␞
