␝ <chapter id="pltcl">␟  <title>PL/Tcl &mdash; Tcl Procedural Language</title>␟  <title>PL/Tcl &mdash; Tcl手続き言語</title>␞␞␞
␝  <para>␟   PL/Tcl is a loadable procedural language for the
   <productname>PostgreSQL</productname> database system
   that enables the <ulink url="https://www.tcl.tk/">
   Tcl language</ulink> to be used to write
   <productname>PostgreSQL</productname> functions and procedures.␟PL/Tclとは、<productname>PostgreSQL</productname>データベースシステムにロード可能な手続き言語で、<ulink url="https://www.tcl.tk/">Tcl言語</ulink>を使った<productname>PostgreSQL</productname>関数やプロシージャを作成できます。␞␞  </para>␞
␝  <sect1 id="pltcl-overview">␟   <title>Overview</title>␟   <title>概要</title>␞␞␞
␝   <para>␟    PL/Tcl offers most of the capabilities a function writer has in
    ...
    Tcl.␟PL/Tclは、いくつか制限がありますが、C言語で書かれた関数と同じような能力を提供します。
さらに、Tclで利用できる、強力な文字列処理ライブラリを持っています。␞␞   </para>␞
␝   <para>␟    One compelling <emphasis>good</emphasis> restriction is that
    ...
    to use this language; it does not give them unlimited authority.␟すべてがTclインタプリタの安全なコンテキスト内で実行されるという制約はやむを得ないものですが、逆に<emphasis>良い</emphasis>制約でもあります。
...
つまり、無制限の権限は与えられません。␞␞   </para>␞
␝   <para>␟    The other notable implementation restriction is that Tcl functions
    cannot be used to create input/output functions for new data
    types.␟その他の注意すべき実装上の制約として、Tcl関数を使用して新しいデータ型用の入出力関数を作成することはできません。␞␞   </para>␞
␝   <para>␟    Sometimes it is desirable to write Tcl functions that are not restricted
    ...
    a user logged in as the database administrator.␟例えば、メールを送るTcl関数が必要な場合など、安全なTclに制約されないTcl関数を書くことが望ましい場合があります。
...
<application>PL/TclU</application>関数ではデータベース管理者としてログインしたユーザができるあらゆることの実行が可能となるので、作成する際に、この関数が意図された以外のことを行わないように細心の注意を払う必要があります。␞␞   </para>␞
␝   <para>␟    The shared object code for the <application>PL/Tcl</application> and
    ...
    <literal>CREATE EXTENSION pltclu</literal>.␟インストール時にTclサポートの設定が指定されていれば、<application>PL/Tcl</application>と<application>PL/TclU</application>呼び出しハンドラの共有オブジェクトコードは自動的に作成され、<productname>PostgreSQL</productname>のライブラリディレクトリにインストールされます。
...
例えば、<literal>CREATE EXTENSION pltcl</literal>あるいは<literal>CREATE EXTENSION pltclu</literal>です。␞␞   </para>␞
␝   <sect1 id="pltcl-functions">␟    <title>PL/Tcl Functions and Arguments</title>␟    <title>PL/Tcl関数と引数</title>␞␞␞
␝    <para>␟     To create a function in the <application>PL/Tcl</application> language, use
     the standard <xref linkend="sql-createfunction"/> syntax:␟<application>PL/Tcl</application>言語で関数を作成するには、以下の標準的な<xref linkend="sql-createfunction"/>構文を使用してください。␞␞␞
␝CREATE FUNCTION <replaceable>funcname</replaceable> (<replaceable>argument-types</replaceable>) RETURNS <replaceable>return-type</replaceable> AS $$␟    # PL/Tcl function body␟    # PL/Tcl関数本体␞␞$$ LANGUAGE pltcl;␞
␝␟     <application>PL/TclU</application> is the same, except that the language has to be specified as
     <literal>pltclu</literal>.␟<application>PL/TclU</application>でも、言語に<literal>pltclu</literal>を指定しなければならない点以外は同様です。␞␞    </para>␞
␝    <para>␟     The body of the function is simply a piece of Tcl script.
     ...
     from the Tcl code is ignored.␟関数本体は、単なる小さなTclスクリプトです。
...
プロシージャでは、Tclコードからの戻り値は無視されます。␞␞    </para>␞
␝    <para>␟     For example, a function
     returning the greater of two integer values could be defined as:␟例えば、2つの整数のうち大きな方を返す関数は以下のように定義できます。␞␞␞
␝␟     Note the clause <literal>STRICT</literal>, which saves us from
     ...
     result automatically.␟<literal>STRICT</literal>句に注意してください。
これによりプログラマは、入力にNULL値が与えられた場合を検討する手間を省くことができます。
NULLが渡された場合、関数はまったく呼び出されず、単にNULLという結果が自動的に返されます。␞␞    </para>␞
␝    <para>␟     In a nonstrict function,
     ...
     argument, rather than null:␟厳密（strict）でない関数では、引数の実際の値がNULLである場合、対応する<literal>$<replaceable>n</replaceable></literal>変数は空文字列に設定されます。
...
␝    <para>␟     As shown above,
     ...
     function is strict or not.␟上で示した通り、NULL値をPL/Tcl関数から返すためには、<literal>return_null</literal>を実行してください。
これは、関数が厳密かどうかに関係なく、実行することができます。␞␞    </para>␞
␝    <para>␟     Composite-type arguments are passed to the function as Tcl
     ...
     null value, it will not appear in the array. Here is an example:␟複合型の引数は、Tcl配列として関数に渡されます。
配列の要素名は複合型の属性名です。
渡された行の属性がNULL値の場合、その属性は配列内には現れません。
以下に例を示します。␞␞␞
␝    <para>␟     PL/Tcl functions can return composite-type results, too.  To do this,
     ...
     column names.  Here is an example:␟PL/Tcl関数は複合型の結果を返すこともできます。
このためには、Tclコードは期待する結果型と一致する列の名前/値のペアのリストを返さなければなりません。
そのリストで省略された列名は結果がNULLになり、期待されない列名があるとエラーが生じます。
例を示します。␞␞␞
␝    <para>␟     Output arguments of procedures are returned in the same way, for example:␟プロシージャの出力引数は同様に返されます。以下に例を示します。␞␞␞
␝     <para>␟      The result list can be made from an array representation of the
      desired tuple with the <literal>array get</literal> Tcl command.  For example:␟<literal>array get</literal> Tclコマンドを使って、希望するタプルの配列表現から結果リストを作成できます。
以下に例を示します。␞␞␞
␝    <para>␟     PL/Tcl functions can return sets.  To do this, the Tcl code should
     ...
     Here is an example returning a scalar type:␟PL/Tcl関数は集合を返すことができます。
...
␝   <sect1 id="pltcl-data">␟    <title>Data Values in PL/Tcl</title>␟    <title>PL/Tclにおけるデータの値</title>␞␞␞
␝    <para>␟     The argument values supplied to a PL/Tcl function's code are simply
     ...
     result type, or for the specified column of a composite result type.␟PL/Tcl関数コードに与えられる引数の値は、単に、テキスト形式（<command>SELECT</command>文によりそれを表示した場合と同じ形式）に変換された入力引数です。
逆に、<literal>return</literal>コマンドと<literal>return_next</literal>コマンドは、その関数宣言における戻り値の型、あるいは複合型の戻り値型の入力書式として受け付けることができる、任意の文字列を受け付けます。␞␞    </para>␞
...
␝   <sect1 id="pltcl-global">␟    <title>Global Data in PL/Tcl</title>␟    <title>PL/Tclにおけるグローバルデータ</title>␞␞␞
␝    <para>␟     Sometimes it
     is useful to have some global data that is held between two
     calls to a function or is shared between different functions.
     This is easily done in PL/Tcl, but there are some restrictions that
     must be understood.␟ある関数の複数の呼び出し間で保持される、もしくは、異なる関数間で共有されるような、いくつかのグローバルデータを持つことが有意な場合があります。
これはPL/Tclで簡単に実現できますが、理解する必要がある制限がいくつかあります。␞␞    </para>␞
␝    <para>␟     For security reasons, PL/Tcl executes functions called by any one SQL
     ...
     unintended.␟セキュリティ上の理由のため、PL/Tclは、任意のSQLロールによって呼び出された関数をそのロール用の別のTclインタプリタで実行します。
...
当然ながら、こうした関数が意図しない動作を行うために使われることのないよう注意しなければなりません。␞␞    </para>␞
␝    <para>␟     All PL/TclU functions used in a session execute in the same Tcl
     ...
     namely that of a database superuser.␟セッション内で使用されるすべてのPL/TclU関数は、当然ながらPL/Tcl関数とは別のインタプリタですが、同一のTclインタプリタ内で実行されます。
このためPL/TclU関数間ではグローバルデータは自動的に共有されます。
すべてのPL/TclU関数は同じ信頼レベル、すなわちデータベーススーパーユーザで実行されますので、これはセキュリティ上危険とはみなされません。␞␞    </para>␞
␝    <para>␟     To help protect PL/Tcl functions from unintentionally interfering
     ...
     security restrictions mentioned above.)␟PL/Tcl関数が予期しない相互作用に巻き込まれないようにするために、<function>upvar</function>コマンドを使用することによって、各関数でアクセスできるグローバルな配列を作成することができます。
...
このため、これらは上記のセキュリティ制限を迂回することはありません。）␞␞    </para>␞
␝    <para>␟     An example of using <literal>GD</literal> appears in the
     <function>spi_execp</function> example below.␟後述の<function>spi_execp</function>の例の中に<literal>GD</literal>の使用例があります。␞␞    </para>␞
␝   <sect1 id="pltcl-dbaccess">␟    <title>Database Access from PL/Tcl</title>␟    <title>PL/Tclからのデータベースアクセス</title>␞␞␞
␝    <para>␟     In this section, we follow the usual Tcl convention of using question
     ...
     the database from the body of a PL/Tcl function:␟この節では、通常のTclの規約に従い、構文の概要でオプションの要素を示すのに角括弧ではなく疑問符を使います。
下記のコマンドは、PL/Tcl関数内からデータベースアクセスを行う時に使用できるコマンドです。␞␞␞
␝       <para>␟        Executes an SQL command given as a string.  An error in the command
        ...
        described below.␟文字列として与えられたSQL問い合わせを実行します。
...
さらに、コマンドが<command>SELECT</command>文の場合、選択された列の値は以下のようにTclの変数に格納されます。␞␞       </para>␞
␝       <para>␟        The optional <literal>-count</literal> value tells
        ...
        completion, the same as when <literal>-count</literal> is omitted.␟オプションの<literal>-count</literal>値は、<function>spi_exec</function>に対し、問い合わせに<literal>LIMIT</literal>句が含まれているかのように、<replaceable>n</replaceable>行を取得すると停止するよう指示します。
<replaceable>n</replaceable>が0の場合、問い合わせは完了するまで実行されます。これは、<literal>-count</literal>が省略された場合と同じです。␞␞       </para>␞
␝       <para>␟        If the command is a <command>SELECT</command> statement, the values of the
        ...
        in use as a column name in the result.␟コマンドが<command>SELECT</command>文の場合、その結果得られた列の値は、列名にちなんだ名前のTcl変数に格納されます。
<literal>-array</literal>オプションが付与された場合は、列の値は指定された名前の連想配列の要素に格納され、その配列のインデックスとして列名が使用されます。
加えて、結果内での現在の行番号（ゼロから数えます）が<quote><literal>.tupno</literal></quote>という名前の配列要素に格納されます。ただし、その名前が結果内の列名として使われていない場合に限られます。␞␞       </para>␞
␝       <para>␟        If the command is a <command>SELECT</command> statement and no <replaceable>loop-body</replaceable>
        ...
        For example:␟問い合わせ文が<command>SELECT</command>文、かつ、<replaceable>loop-body</replaceable>スクリプトが付与されなかった場合、結果のうち最初の行だけがTclの変数または配列要素に格納されます。
...
␝</programlisting>␟        will set the Tcl variable <literal>$cnt</literal> to the number of rows in
        the <structname>pg_proc</structname> system catalog.␟これは、<literal>$cnt</literal> Tcl変数を、<structname>pg_proc</structname>システムカタログの行数に設定します。␞␞       </para>␞
␝       <para>␟        If the optional <replaceable>loop-body</replaceable> argument is given, it is
        ...
        For example:␟<replaceable>loop-body</replaceable>オプション引数が付与された場合、それは、問い合わせの結果内の行それぞれに対して一度だけ実行される小さなTclスクリプトです。
（<replaceable>loop-body</replaceable>は<command>SELECT</command>以外の問い合わせで付与された場合は無視されます。）
処理中の行の列値は、各繰り返しの前にTclの変数または配列要素に格納されます。
以下に例を示します。␞␞<programlisting>␞
␝</programlisting>␟        will print a log message for every row of <literal>pg_class</literal>.  This
        feature works similarly to other Tcl looping constructs; in
        particular <literal>continue</literal> and <literal>break</literal> work in the
        usual way inside the loop body.␟これは、<literal>pg_class</literal>の各行に対してログメッセージを出力します。
この機能は他のTclの繰り返し構文でも同様に動作します。
特にループ本体内の<literal>continue</literal>と<literal>break</literal>は通常通り動作します。␞␞       </para>␞
␝       <para>␟        If a column of a query result is null, the target
        variable for it is <quote>unset</quote> rather than being set.␟問い合わせの結果、列がNULLであった場合、対象となる変数は代入されずに、<quote>未設定状態</quote>になります。␞␞       </para>␞
␝       <para>␟        Prepares and saves a query plan for later execution.  The
        saved plan will be retained for the life of the current
        session.<indexterm><primary>preparing a query</primary>
        <secondary>in PL/Tcl</secondary></indexterm>␟後の実行のために問い合わせ計画の準備、保存を行います。
保存された計画は現在のセッションが終了するまで保持されます。
<indexterm><primary>問い合わせの準備</primary><secondary>PL/Tclにおける</secondary></indexterm>␞␞       </para>␞
␝       <para>␟        The query can use parameters, that is, placeholders for
        values to be supplied whenever the plan is actually executed.
        In the query string, refer to parameters
        by the symbols <literal>$1</literal> ... <literal>$<replaceable>n</replaceable></literal>.
        If the query uses parameters, the names of the parameter types
        must be given as a Tcl list.  (Write an empty list for
        <replaceable>typelist</replaceable> if no parameters are used.)␟問い合わせはパラメータ、つまり、計画が実際に実行される時に常に与えられる値用のプレースホルダを持つことができます。
...
（パラメータを使用しない場合は<replaceable>typelist</replaceable>には空のリストを指定してください。）␞␞       </para>␞
␝       <para>␟        The return value from <function>spi_prepare</function> is a query ID
        to be used in subsequent calls to <function>spi_execp</function>. See
        <function>spi_execp</function> for an example.␟<function>spi_prepare</function>の戻り値は問い合わせIDです。
このIDは後に<function>spi_execp</function>を呼び出す時に使用されます。
使用例については<function>spi_execp</function>を参照してください。␞␞       </para>␞
␝       <para>␟        Executes a query previously prepared with <function>spi_prepare</function>.
        ...
        if the query has no parameters.␟<function>spi_prepare</function>により事前に準備された問い合わせを実行します。
...
問い合わせにパラメータがない場合は、<replaceable>value-list</replaceable>を省略してください。␞␞       </para>␞
␝       <para>␟        The optional value for <literal>-nulls</literal> is a string of spaces and
        ...
        is not given, all the parameter values are nonnull.␟<literal>-nulls</literal>オプションの値は、空白文字と<literal>'n'</literal>という文字からなる文字列で、<function>spi_execp</function>に対し、どの引数がNULL値かを示します。
指定された場合、その文字列の長さは<replaceable>value-list</replaceable>の長さと正確に一致していなければなりません。
指定されない場合は、すべてのパラメータの値は非NULLです。␞␞       </para>␞
␝       <para>␟        Except for the way in which the query and its parameters are specified,
        ...
        and so is the result value.␟問い合わせとそのパラメータをどこで指定するのかという点を除き、<function>spi_execp</function>は<function>spi_exec</function>と同様に動作します。
<literal>-count</literal>、<literal>-array</literal>、<replaceable>loop-body</replaceable>オプションも、そして、結果の値も同じです。␞␞       </para>␞
␝       <para>␟        Here's an example of a PL/Tcl function using a prepared plan:␟ここで、プリペアド計画を使用した、PL/Tcl関数の例を示します。␞␞␞
␝    if {![ info exists GD(plan) ]} {␟        # prepare the saved plan on the first call␟        # 最初の呼び出しでは保存する計画を準備します。␞␞        set GD(plan) [ spi_prepare \␞
␝␟        We need backslashes inside the query string given to
        ...
        variable substitution.␟<function>spi_prepare</function>に与える問い合わせ文字列の内側では、<literal>$<replaceable>n</replaceable></literal>記号が確実にそのまま<function>spi_prepare</function>に渡され、Tcl変数の代入による置き換えが起こらないようにバックスラッシュが必要です。␞␞␞
␝       <para>␟        The Tcl script contained in <replaceable>command</replaceable> is
        ...
        example.␟<replaceable>command</replaceable>に含まれるTclスクリプトが、SQLサブトランザクション中で実行されます。
スクリプトがエラーを返すと、上位のTclコードにエラーを返す前に、そのサブトランザクションをロールバックします。
更なる詳細と使用例については<xref linkend="pltcl-subtransactions"/>を参照してください。␞␞       </para>␞
␝       <para>␟        Doubles all occurrences of single quote and backslash characters
        in the given string.  This can be used to safely quote strings
        that are to be inserted into SQL commands given
        to <function>spi_exec</function> or
        <function>spi_prepare</function>.
        For example, think about an SQL command string like:␟指定された文字列内のすべての単一引用符とバックスラッシュ文字を二重化します。
...
␝␟        where the Tcl variable <literal>val</literal> actually contains
        ...
        in the final command string:␟ここで、<literal>val</literal> Tcl変数に<literal>doesn't</literal>が実際に含まれているものとします。
これは最終的に以下の問い合わせ文字列になってしまいます。␞␞␞
␝␟        which would cause a parse error during
        <function>spi_exec</function> or
        <function>spi_prepare</function>.
        To work properly, the submitted command should contain:␟これでは、<function>spi_exec</function>または<function>spi_prepare</function>の実行中に解析エラーが発生してしまいます。
...
␝␟        One advantage of <function>spi_execp</function> is that you don't
        ...
        parsed as part of an SQL command string.␟<function>spi_execp</function>の持つ1つの利点は、パラメータはSQL問い合わせ文字列の一部として解析されることがありませんので、このようにパラメータの値を引用符付けする必要がないことです。␞␞       </para>␞
␝       <para>␟        Emits a log or error message. Possible levels are
        ...
        for more information.␟ログまたはエラーメッセージを発行します。
...
詳細については<xref linkend="runtime-config"/>および<xref linkend="pltcl-error-handling"/>を参照してください。␞␞       </para>␞
␝    <indexterm>
...
␝   <sect1 id="pltcl-trigger">␟    <title>Trigger Functions in PL/Tcl</title>␟    <title>PL/Tclのトリガ関数</title>␞␞␞
␝    <para>␟     Trigger functions can be written in PL/Tcl.
     <productname>PostgreSQL</productname> requires that a function that is to be called
     as a trigger must be declared as a function with no arguments
     and a return type of <literal>trigger</literal>.␟トリガ関数をPL/Tclで作成することができます。
<productname>PostgreSQL</productname>では、トリガとして呼び出される関数は、<literal>trigger</literal>型の戻り値を返す引数のない関数として宣言する必要があります。␞␞    </para>␞
␝    <para>␟     The information from the trigger manager is passed to the function body
     in the following variables:␟トリガマネージャからの情報は、以下の変数内に格納されて関数本体に渡されます。␞␞␞
␝        <para>␟         The name of the trigger from the <command>CREATE TRIGGER</command> statement.␟<command>CREATE TRIGGER</command>文によるトリガ名。␞␞        </para>␞
␝        <para>␟         The object ID of the table that caused the trigger function
         to be invoked.␟そのトリガ関数の呼び出しを発生させたテーブルのオブジェクトID。␞␞        </para>␞
␝        <para>␟         The name of the table that caused the trigger function
         to be invoked.␟そのトリガ関数の呼び出しを発生させたテーブルの名前。␞␞        </para>␞
␝        <para>␟         The schema of the table that caused the trigger function
         to be invoked.␟そのトリガプロシージャ呼び出しが発生したテーブルのスキーマ。␞␞        </para>␞
␝        <para>␟         A Tcl list of the table column names, prefixed with an empty list
         ...
         to their right.)␟先頭に空のリスト要素を持つ、テーブルの列名のTclリスト。
...
（また空のリスト要素は、右側の列の属性番号を正しくするために、削除された列の位置に現れます。）␞␞        </para>␞
␝        <para>␟         The string <literal>BEFORE</literal>, <literal>AFTER</literal>, or
         <literal>INSTEAD OF</literal>, depending on the type of trigger event.␟トリガイベントの種類に応じた、<literal>BEFORE</literal>、<literal>AFTER</literal>または<literal>INSTEAD OF</literal>という文字列。␞␞        </para>␞
␝        <para>␟         The string <literal>ROW</literal> or <literal>STATEMENT</literal> depending on the
         type of trigger event.␟トリガイベントの種類に応じた、<literal>ROW</literal>または<literal>STATEMENT</literal>という文字列。␞␞        </para>␞
␝        <para>␟         The string <literal>INSERT</literal>, <literal>UPDATE</literal>,
         <literal>DELETE</literal>, or <literal>TRUNCATE</literal> depending on the type of
         trigger event.␟トリガイベントの種類に応じた、<literal>INSERT</literal>、<literal>UPDATE</literal>、<literal>DELETE</literal>、または<literal>TRUNCATE</literal>の文字列。␞␞        </para>␞
␝        <para>␟         An associative array containing the values of the new table
         ...
         This is not set for statement-level triggers.␟<command>INSERT</command>/<command>UPDATE</command>動作の場合は新しいテーブル行の値を、<command>DELETE</command>動作の場合は空を持つ連想配列。
配列のインデックスは列名です。
NULLの列はこの配列内には現れません。
文レベルのトリガに対しては設定されません。␞␞        </para>␞
␝        <para>␟         An associative array containing the values of the old table
         ...
         name.  Columns that are null will not appear in the array.
         This is not set for statement-level triggers.␟<command>UPDATE</command>/<command>DELETE</command>動作の場合は古いテーブル行の値を、<command>INSERT</command>動作の場合は空を持つ連想配列。
配列のインデックスは列名です。
NULLの列はこの配列内には現れません。
文レベルのトリガに対しては設定されません。␞␞        </para>␞
␝        <para>␟         A Tcl list of the arguments to the function as given in the
         <command>CREATE TRIGGER</command> statement. These arguments are also accessible as
         <literal>$1</literal> ... <literal>$<replaceable>n</replaceable></literal> in the function body.␟<command>CREATE TRIGGER</command>文で指定された、関数への引数のTclリスト。
この引数は、関数本体から<literal>$1</literal> ... <literal>$<replaceable>n</replaceable></literal>としてもアクセスすることができます。␞␞        </para>␞
␝    <para>␟     The return value from a trigger function can be one of the strings
     ...
     The trigger return value is ignored for all other types of triggers.␟トリガ関数からの戻り値は、<literal>OK</literal>という文字列、<literal>SKIP</literal>という文字列、列名/値の組のリスト、の内の1つを取ることができます。
...
この他の種類のトリガでは戻り値は無視されます。␞␞    </para>␞
␝     <para>␟      The result list can be made from an array representation of the
      modified tuple with the <literal>array get</literal> Tcl command.␟結果リストは<literal>array get</literal> Tclコマンドによる変更されたタプルの配列表現から作ることができます。␞␞     </para>␞
␝    <para>␟     Here's a little example trigger function that forces an integer value
     in a table to keep track of the number of updates that are performed on the
     row. For new rows inserted, the value is initialized to 0 and then
     incremented on every update operation.␟ここで、テーブル内の整数値としてその行に対する更新数を記録させる、小さめのトリガプロシージャの例を示します。
...
␝␟     Notice that the trigger function itself does not know the column
     ...
     trigger function be reused with different tables.␟トリガ関数自身は列名を認識していない点に注目してください。
これはトリガの引数として与えられます。
これにより、このトリガ関数を別のテーブルで再利用することができます。␞␞    </para>␞
␝    <indexterm>
...
␝   <sect1 id="pltcl-event-trigger">␟    <title>Event Trigger Functions in PL/Tcl</title>␟    <title>PL/Tclにおけるイベントトリガ関数</title>␞␞␞
␝    <para>␟     Event trigger functions can be written in PL/Tcl.
     ...
     arguments and a return type of <literal>event_trigger</literal>.␟イベントトリガ関数をPL/Tclで作成することができます。
<productname>PostgreSQL</productname>では、イベントトリガとして呼び出される関数は、<literal>event_trigger</literal>型の戻り値を返す引数のない関数として宣言する必要があります。␞␞    </para>␞
␝    <para>␟     The information from the trigger manager is passed to the function body
     in the following variables:␟トリガマネージャからの情報は、以下の変数内に格納されて関数本体に渡されます。␞␞␞
␝        <para>␟         The name of the event the trigger is fired for.␟トリガが発行されたイベント名␞␞        </para>␞
␝        <para>␟         The command tag for which the trigger is fired.␟トリガが発行されたコマンドタグ␞␞        </para>␞
␝    <para>␟     The return value of the trigger function is ignored.␟トリガ関数の戻り値は無視されます。␞␞    </para>␞
␝    <para>␟     Here's a little example event trigger function that simply raises
     a <literal>NOTICE</literal> message each time a supported command is
     executed:␟サポートするコマンドが実行される度に、単に<literal>NOTICE</literal>メッセージを発行するイベントトリガ関数の例を、以下に示します。␞␞␞
␝   <sect1 id="pltcl-error-handling">␟    <title>Error Handling in PL/Tcl</title>␟    <title>PL/Tclのエラー処理</title>␞␞␞
␝    <para>␟     Tcl code within or called from a PL/Tcl function can raise an error,
     ...
     function's calling query.␟PL/Tcl関数中の、あるいはPL/Tcl関数から呼ばれるTclコードは、無効な演算の実行により、あるいはTclの<function>error</function>コマンドやPL/Tclの<function>elog</function>コマンドを使ってエラーを生成することにより、エラーとなることがありえます。
これらエラーはTclの<function>catch</function>コマンドを使ってTcl内で捕捉することができます。
あるエラーが捕捉されず、PL/Tcl関数実行のトップレベルに伝播することが許容されているなら、関数が呼び出している問い合わせにおけるSQエラーとして報告されます。␞␞    </para>␞
␝    <para>␟     Conversely, SQL errors that occur within PL/Tcl's
     ...
     it turns back into an SQL error.␟逆に、PL/Tclの<function>spi_exec</function>、<function>spi_prepare</function>、<function>spi_execp</function>コマンドの中で起きるSQLエラーは、Tclのエラーとして報告され、したがって、これらはTclの<function>catch</function>コマンドにより捕捉可能です。
（各々のPL/Tclコマンドは、エラー時にロールバックするSQL操作をサブトランザクション中で実行するので、部分的に完了した操作は自動的に後始末されます。）
ここでも同様に、捕捉されることなくトップレベルに伝播するならSQLエラーに戻ります。␞␞    </para>␞
␝    <para>␟     Tcl provides an <varname>errorCode</varname> variable that can represent
     ...
     <varname>funcname</varname>.␟Tclは、Tclプログラムで解釈しやすい形式でエラーに関する追加情報を表現できる<varname>errorCode</varname>変数を提供します。
...
出現しうるフィールドとしては、<varname>detail</varname>、<varname>hint</varname>、<varname>context</varname>、<varname>schema</varname>、<varname>table</varname>、<varname>column</varname>、<varname>datatype</varname>、<varname>constraint</varname>、<varname>statement</varname>、<varname>cursor_position</varname>、<varname>filename</varname>、<varname>lineno</varname>および<varname>funcname</varname>があります。␞␞    </para>␞
␝    <para>␟     A convenient way to work with PL/Tcl's <varname>errorCode</varname>
     information is to load it into an array, so that the field names become
     array subscripts.  Code for doing that might look like␟PL/Tclの<varname>errorCode</varname>情報を処理する便利な方法は、それを配列に読み込むことです。これによりフィールド名は配列の添え字になります。
これを行うコードは以下のようになります。␞␞<programlisting>␞
␝</programlisting>␟     (The double colons explicitly specify that <varname>errorCode</varname>
     is a global variable.)␟（二重コロンは<varname>errorCode</varname>がグローバル変数であることを明示的に指定します。）␞␞    </para>␞
␝    <indexterm>
...
␝   <sect1 id="pltcl-subtransactions">␟    <title>Explicit Subtransactions in PL/Tcl</title>␟    <title>PL/Tclにおける明示的サブトランザクション</title>␞␞␞
␝    <para>␟     Recovering from errors caused by database access as described in
     ...
     the form of explicit subtransactions.␟<xref linkend="pltcl-error-handling"/>で説明されているように、データベースアクセスによって生じたエラーからの回復により、操作のうちいくつかが失敗する前に他の操作が成功し、エラーからの回復後、データの一貫性が失われた望ましくない状態になってしまう可能性があります。
PL/Tclは明示的なトランザクションの手法でこの問題を解決する手段を提供しています。␞␞    </para>␞
␝    <para>␟     Consider a function that implements a transfer between two accounts:␟2つのアカウントの間の送金を実装する関数を考えます。␞␞<programlisting>␞
␝</programlisting>␟     If the second <command>UPDATE</command> statement results in an
     ...
     got rolled back.␟ふたつ目の<command>UPDATE</command>文で例外が発生する結果になると、この関数は失敗を記録しますが、それにもかかわらず、最初の<command>UPDATE</command>はコミットされます。
言い換えると、Joeのアカウントから資金が引き出されたのに、Maryのアカウントには転送されません。
これは、それぞれの<function>spi_exec</function>が別々のサブトランザクションになっていて、そのうち一つのサブトランザクションだけがロールバックされるからです。␞␞    </para>␞
␝    <para>␟     To handle such cases, you can wrap multiple database operations in an
     ...
     this.  We can rewrite our function as:␟このような状況に対応するには、複数のデータベース操作を、全体が成功するか、あるいは失敗する明示的なサブトランザクションで包みます。
PL/Tclは、これを管理するための<function>subtransaction</function>コマンドを提供しています。
関数を以下のように書き直せます。␞␞<programlisting>␞
␝</programlisting>␟     Note that use of <function>catch</function> is still required for this
     ...
     be rolled back together when an error is reported.␟この目的のために、<function>catch</function>が必要であることに注意してください。
...
エラーが報告された際に、スコープの内側で実行されたすべてのデータベース操作がロールバックされることを保証するだけです。␞␞    </para>␞
␝    <para>␟     A rollback of an explicit subtransaction occurs on any error reported
     ...
     not cause a rollback.␟明示的なサブトランザクションのロールバックは、Tclのコードの中でエラーが報告された際だけでなく、データベースアクセスに起因するエラーの際にも起こります。
ですから、<function>subtransaction</function>コマンド内の内側で起こった通常のTcl例外は、サブトランザクションのロールバックも引き起こします。
しかし、Tclコードからのエラーによらない脱出（たとえば<function>return</function>によるもの）は、ロールバックをもたらしません。␞␞    </para>␞
␝   <sect1 id="pltcl-transactions">␟    <title>Transaction Management</title>␟    <title>トランザクション制御</title>␞␞␞
␝    <para>␟     In a procedure called from the top level or an anonymous code block
     ...
     command for that.␟トップレベル、あるいは、トップレベルから呼ばれた無名コードブロック（<command>DO</command>コマンド）から呼ばれたプロシージャでは、トランザクション制御が可能です。
...
トランザクションが終了した後、新たなトランザクションが自動的に開始されますので、開始するための別途のコマンドはありません。␞␞    </para>␞
␝    <para>␟     Here is an example:␟以下に例を示します。␞␞<programlisting>␞
␝    <para>␟     Transactions cannot be ended when an explicit subtransaction is active.␟明示的なサブトランザクションの中ではトランザクションを終了することはできません。␞␞    </para>␞
␝   <sect1 id="pltcl-config">␟    <title>PL/Tcl Configuration</title>␟    <title>PL/Tclの設定</title>␞␞␞
␝       <indexterm>
...
␝    <para>␟     This section lists configuration parameters that
     affect <application>PL/Tcl</application>.␟この節では、<application>PL/Tcl</application>に影響がある設定パラメータを列挙します。␞␞    </para>␞
␝       <para>␟        This parameter, if set to a nonempty string, specifies the name
        ...
        function is called by a new SQL role.␟このパラメータが空文字以外に設定された場合、PL/Tclのための新しいTclインタプリタが作成された際に実行すべきパラメータなしのPL/Tcl関数の名前（スキーマ修飾される場合もあります）を指定します。
そうした関数は、追加のTclコードをロードするような、Tclセッションごとの初期化を実施できます。
データベースセッションの中で新しいPL/Tcl関数が最初に実行された際、あるいはPL/Tcl関数が新しいロールから呼び出されたためにインタプリタを追加で作成しなければならない際に、新しいTclインタプリタが作られます。␞␞       </para>␞
␝       <para>␟        The referenced function must be written in the <literal>pltcl</literal>
        ...
        call it, too.␟参照されている関数は<literal>pltcl</literal>言語で記述しなければならず、また<literal>SECURITY DEFINER</literal>としてマークされていてはいけません。
（この制約により、その関数が初期化すると想定しているインタプリタ内で実行されることが保証されます。）
また、現在のユーザはその関数を呼び出すことが許可されていなければなりません。␞␞       </para>␞
␝       <para>␟        If the function fails with an error it will abort the function call
        ...
        Tcl interpreter.␟関数がエラーで失敗すると、関数呼び出しをアボートし、その結果新しいインタプリタが作成され、エラーは呼び出し元のクエリに伝搬し、現在のトランザクションあるいはサブトランザクションがアボートします。
Tcl内でそれまでに行われた操作は取り消されません。
しかし、インタプリタは再使用できません。
言語が再び使用されると、新しいTclインタプリタ内で初期化が再び試みられます。␞␞       </para>␞
␝       <indexterm>
...
␝       <para>␟        Only superusers can change this setting.  Although this setting
        can be changed within a session, such changes will not affect Tcl
        interpreters that have already been created.␟スーパーユーザだけがこの設定を変更できます。
この設定はセッション内で変更できますが、すでに作成されたTclインタプリタには影響しません。␞␞       </para>␞
␝       <para>␟        This parameter is exactly like <varname>pltcl.start_proc</varname>,
        except that it applies to PL/TclU.  The referenced function must
        be written in the <literal>pltclu</literal> language.␟このパラメータはPL/TclUに適用される点を除けば、<varname>pltcl.start_proc</varname>と完全に類似しています。
参照される関数は<literal>pltclu</literal>言語で書かれていなければなりません。␞␞       </para>␞
␝   <sect1 id="pltcl-procnames">␟    <title>Tcl Procedure Names</title>␟    <title>Tclプロシージャ名</title>␞␞␞
␝    <para>␟     In <productname>PostgreSQL</productname>, the same function name can be used for
     ...
     when debugging.␟<productname>PostgreSQL</productname>では、その関数の引数の数または引数の型が異なっていれば、同じ関数名を異なる関数定義に使用することが可能です。
...
PL/Tclプログラマから見ますと、通常は問題にはなりませんが、デバッグの際に表面に現れます。␞␞    </para>␞
