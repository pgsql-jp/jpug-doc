␝<chapter id="nls">␟ <title>Native Language Support</title>␟ <title>各国語サポート</title>␞␞␞
␝ <sect1 id="nls-translator">␟  <title>For the Translator</title>␟  <title>翻訳者へ</title>␞␞␞
␝  <para>␟   <productname>PostgreSQL</productname>
   ...
   programmer at all
   to do this.  This section explains how to help.␟<productname>PostgreSQL</productname>プログラムは（サーバ、クライアントともに）メッセージが翻訳されていれば、使い慣れた言語でそのメッセージを出すことができます。
翻訳メッセージセットの作成と保守は、自分の言語を上手に話し<productname>PostgreSQL</productname>の成果に貢献したい人々の協力が必要です。
この作業にはプログラマである必要はまったくありません。
本節ではお手伝いの仕方を説明します。␞␞  </para>␞
␝  <sect2 id="nls-translator-requirements">␟   <title>Requirements</title>␟   <title>必要条件</title>␞␞␞
␝   <para>␟    We won't judge your language skills &mdash; this section is about
    ...
    the installation instructions.␟協力者の言語の熟練度については判断しません。
...
作業を試す際には、インストール手順の適切な部分に従ってください。␞␞   </para>␞
␝   <para>␟    If you want to start a new translation effort or want to do a
    ...
    it.)  <application>GNU Gettext 0.10.36</application> or later is currently recommended.␟新規に翻訳作業を始めるか、（後述の）メッセージカタログのマージを行いたい場合、GNU版と互換性を持った実装の<filename>xgettext</filename>と<filename>msgmerge</filename>という2つのプログラムがそれぞれ必要です。
将来はパッケージ化されたソース配布物を使用する場合に<filename>xgettext</filename>を必要としないように変更する予定です。
（Git版で作業をしているのであれば、まだこれが必要です。）
現在<application>GNU Gettext 0.10.36</application>以上を推奨します。␞␞   </para>␞
␝   <para>␟    Your local gettext implementation should come with its own
    documentation.  Some of that is probably duplicated in what
    follows, but for additional details you should look there.␟使用するマシンのgettextの実装については、文書が一緒に付いてくると思います。
以下のいくつかはおそらく重複していますが、追補すべき詳細についてはその文書を参照してください。␞␞   </para>␞
␝  <sect2 id="nls-translator-concepts">␟   <title>Concepts</title>␟   <title>概念</title>␞␞␞
␝   <para>␟    The pairs of original (English) messages and their (possibly)
    ...
    do not deal with MO files; in fact hardly anyone does.␟英語による元のメッセージとそれを基に（場合によって）翻訳されたメッセージの組み合わせは<firstterm>メッセージカタログ</firstterm>に、（関連するプログラムはメッセージカタログを共有することができますが）各プログラム、対象とする言語に対して一つずつ保持されます。
...
翻訳者は、MOファイルを扱いません。
実際に扱うことは困難です。␞␞   </para>␞
␝   <para>␟    The extension of the message catalog file is to no surprise either
    ...
    for psql) or <filename>fr.mo</filename> (MO file in French).␟メッセージカタログファイルの拡張子は想像していたかもしれませんが<filename>.po</filename>もしくは<filename>.mo</filename>です。
基本名（拡張子を除いた部分）は、プログラムが伴っている名前、もしくは翻訳目的とする言語の名前のいずれかで、状況によって変わります。
少し混乱するかもしれません。
例えば、<filename>psql.po</filename>（psql用のPOファイル）、もしくは<filename>fr.mo</filename>（フランス語用のMOファイル）です。␞␞   </para>␞
␝   <para>␟    The file format of the PO files is illustrated here:␟POファイルの書式を以下に示します。␞␞<programlisting>␞
␝</programlisting>␟    The msgid lines are extracted from the program source.  (They need not
    ...
    start at the beginning of the line.)␟msgid行はプログラムのソースから抽出されます。
（これは必要はありませんが最も一般的な方法です。）
msgstr行は初期状態では空であり、翻訳者によって有益な文字列が埋め込まれます。
この文字列には、C言語形式のエスケープ文字を含めることも、上に示したように複数行にまたがって続けることもできます。
（継続行は必ず行の先頭から始まらなければなりません。）␞␞   </para>␞
␝   <para>␟    The # character introduces a comment.  If whitespace immediately
    ...
    are intended to aid the translator.␟#文字はコメントの開始を示します。
#文字の直後に空白文字があった場合、それは翻訳者によって保守されるコメントです。
#の直後に非空白文字が付く、自動的に付与されるコメントもあります。
これらは、POファイルに対する操作を行う各種ツールによって保守され、翻訳者の補助を意図しています。␞␞<programlisting>␞
␝</programlisting>␟    The #. style comments are extracted from the source file where the
    ...
    tools that can verify this, which key off the c-format flag.␟#.スタイルのコメントはそのメッセージが使用されているソースファイルから抽出されます。
...
これを検証するツールがあり、それらはc-formatフラグを入力として受け取ります。␞␞   </para>␞
␝  <sect2 id="nls-translator-message-catalogs">␟   <title>Creating and Maintaining Message Catalogs</title>␟   <title>メッセージカタログの作成と保守</title>␞␞␞
␝   <para>␟    OK, so how does one create a <quote>blank</quote> message
    ...
    for translation.␟さて、<quote>空の</quote>メッセージカタログをどうやって作成するのでしょうか。
まず、翻訳したいメッセージを持つプログラムが存在するディレクトリに移動します。
<filename>nls.mk</filename>ファイルがあればこのプログラムは翻訳の準備が整っています。␞␞   </para>␞
␝   <para>␟    If there are already some <filename>.po</filename> files, then
    ...
    file.␟もし、<filename>.po</filename>ファイルが数個既にあれば、誰かがある翻訳作業を行っています。
...
翻訳対象とする言語用のファイルがあれば、それを基に作業を始めることができます。␞␞   </para>␞
␝   <para>␟    If you need to start a new translation effort, then first run the
    command:␟新規に翻訳を始める必要がある場合、以下のコマンドを最初に実行してください。␞␞<programlisting>␞
␝</programlisting>␟    This will create a file
    ...
    like:␟これは、<filename><replaceable>progname</replaceable>.pot</filename>ファイルを作成します。
...
新規の言語が利用可能になったことを知らせるために、<filename>po/LINGUAS</filename>を編集し、言語（もしくは言語と国）コードを既存の言語リストの後に以下のように追加してください。␞␞<programlisting>␞
␝</programlisting>␟    (Other languages can appear, of course.)␟（もちろん他の言語があるかもしれません。）␞␞   </para>␞
␝   <para>␟    As the underlying program or library changes, messages might be
    changed or added by the programmers.  In this case you do not need
    to start from scratch.  Instead, run the command:␟対象のプログラムやライブラリの変更に伴い、メッセージはプログラマによって変更、追加されます。
この場合は始めからやり直す必要はありません。
その代わりに以下のコマンドを実行してください。␞␞<programlisting>␞
␝</programlisting>␟    which will create a new blank message catalog file (the pot file
    ...
    is saved with a <filename>.po.new</filename> extension.␟そうすると新しい空のメッセージカタログファイル（最初に使用したpotファイル）が作成され、既存のPOファイルにマージされます。
このマージのアルゴリズムが特定のメッセージに関して確実でない場合、それは上で説明した<quote>fuzzy</quote>となります。
新規POファイルは<filename>.po.new</filename>という拡張子付きで保存されます。␞␞   </para>␞
␝  <sect2 id="nls-translator-editing-po">␟   <title>Editing the PO Files</title>␟   <title>POファイルの編集</title>␞␞␞
␝   <para>␟    The PO files can be edited with a regular text editor. There are also
    ...
    useful.␟POファイルは普通のテキストエディタで編集できます。
翻訳に特化した機能で作業を補助する、POファイルに特化したエディタもいくつかあります。
Emacsには（予想通り）POモードがあり、非常に使いやすいものです。␞␞   </para>␞
␝   <para>␟    The translator should only change the area between the quotes after
    the msgstr directive, add comments, and alter the fuzzy flag.␟翻訳者はmsgstrディレクティブの後の引用符の間の部分の変更、コメントの追加、fuzzyフラグの変更のみを行えばよいのです。␞␞   </para>␞
␝   <para>␟    The PO files need not be completely filled in.  The software will
    ...
    translation.␟POファイルを完全に埋めることは必要ありません。
...
これらは何が正しい翻訳になり得るかの参照のためにのみ提供されています。␞␞   </para>␞
␝   <para>␟    Here are some things to keep in mind while editing the
    translations:␟以下に、翻訳の編集を行う際に注意すべき点を示します。␞␞    <itemizedlist>␞
␝      <para>␟       Make sure that if the original ends with a newline, the
       translation does, too.  Similarly for tabs, etc.␟元の文字列の終端が改行の場合、翻訳も同様になっていることを確認してください。
タブなども同様です。␞␞      </para>␞
␝      <para>␟       If the original is a <function>printf</function> format string, the translation
       ...
       In that case you can modify the format specifiers like this:␟元が<function>printf</function>書式文字列の場合、翻訳も同じでなければなりません。
また、翻訳は同じ書式識別子を同じ順番で持たなければなりません。
言語固有の規則によっては不可能な場合や扱いづらい場合も起こります。
このような場合は、以下の書式指定子を使用することができます。␞␞<programlisting>␞
␝</programlisting>␟       Then the first placeholder will actually use the second
       ...
       internationalization.)␟そして、リストの最初のプレースホルダが実際にはこのリストの2番目の引数に使用されます。
...
メッセージ国際化以外ではほとんど使用されませんので、聞いたことがないかもしれません。）␞␞      </para>␞
␝      <para>␟       If the original string contains a linguistic mistake, report
       ...
       a comment in the PO file.␟元の文字列に言語上の間違いがある場合、それを報告し（もしくはプログラムソースを直して）、普通に翻訳してください。
修正された文字列は、プログラムのソースが修正された時にマージ可能になります。
元の文字列が事実と異なる場合、それを報告し（もしくは自ら直して）、翻訳を行わないでください。
その代わりに、POファイルのその文字列にコメントを付けてください。␞␞      </para>␞
␝      <para>␟       Maintain the style and tone of the original string.
       ...
       It might help to read <xref linkend="error-style-guide"/>.␟元の文字列のスタイルや調子を維持してください。
...
<xref linkend="error-style-guide"/>を読むと参考になります。␞␞      </para>␞
␝      <para>␟       If you don't know what a message means, or if it is ambiguous,
       ask on the developers' mailing list.  Chances are that English
       speaking end users might also not understand it or find it
       ambiguous, so it's best to improve the message.␟メッセージの意味がわからない時や、曖昧な場合は、開発者用のメーリングリストに問い合わせてください。
英語を話すエンドユーザも理解できない、または曖昧であると判断することができる機会となり、メッセージの改良を行う最善のものです。␞␞      </para>␞
␝ <sect1 id="nls-programmer">␟  <title>For the Programmer</title>␟  <title>プログラマへ</title>␞␞␞
␝  <sect2 id="nls-mechanics">␟   <title>Mechanics</title>␟   <title>仕組み</title>␞␞␞
␝  <para>␟   This section describes how to implement native language support in a
   program or library that is part of the
   <productname>PostgreSQL</productname> distribution.
   Currently, it only applies to C programs.␟本節では、<productname>PostgreSQL</productname>配布物の一部であるプログラムやライブラリにおける各国語サポートの実装方法を説明します。
現在はCプログラムにのみ適用できます。␞␞  </para>␞
␝  <procedure>␟   <title>Adding NLS Support to a Program</title>␟   <title>プログラムにNLSサポートを追加する</title>␞␞␞
␝    <para>␟     Insert this code into the start-up sequence of the program:␟プログラムの起動処理に以下のコードを追加してください。␞␞<programlisting>␞
␝</programlisting>␟     (The <replaceable>progname</replaceable> can actually be chosen
     freely.)␟（<replaceable>progname</replaceable>は実際には自由に選択できます。）␞␞    </para>␞
␝    <para>␟     Wherever a message that is a candidate for translation is found,
     a call to <function>gettext()</function> needs to be inserted.  E.g.:␟どこであろうと翻訳の候補となるメッセージが見つかったら、<function>gettext()</function>の呼び出しが追加される必要があります。
例えば、␞␞<programlisting>␞
␝</programlisting>␟     would be changed to:␟は、次のように変更されます。␞␞<programlisting>␞
␝</programlisting>␟     (<symbol>gettext</symbol> is defined as a no-op if NLS support is
     not configured.)␟（NLSサポートが組み込まれていない場合、<symbol>gettext</symbol>はノーオペレーション命令として定義されます。）␞␞    </para>␞
␝    <para>␟     This tends to add a lot of clutter.  One common shortcut is to use:␟これは混乱しがちになります。一般的なショートカットは以下のものです。␞␞<programlisting>␞
␝</programlisting>␟     Another solution is feasible if the program does much of its
     ...
     input strings.␟他の解決方法は、バックエンドにおける<function>ereport()</function>のように、そのプログラムが通信のほとんどを1つまたは数個の関数で行っている場合有効です。
その場合、この関数の内部で全ての入力文字列に対し<function>gettext</function>を呼び出すようにすることになります。␞␞    </para>␞
␝    <para>␟     Add a file <filename>nls.mk</filename> in the directory with the
     program sources.  This file will be read as a makefile.  The
     following variable assignments need to be made here:␟プログラムのソースのあるディレクトリに<filename>nls.mk</filename>を追加してください。
これはmakefileとして読まれます。
以下の変数への代入をここで設定する必要があります。␞␞␞
␝        <para>␟         The program name, as provided in the
         <function>textdomain()</function> call.␟<function>textdomain()</function>の呼び出しに使用されるプログラム名です。␞␞        </para>␞
␝        <para>␟         List of files that contain translatable strings, i.e., those
         ...
         line.␟翻訳可能文字列を含むファイルの一覧です。
つまり、これらは<function>gettext</function>もしくは他の解決法として印が付けられます。
結局、これはプログラムのほとんど全てのソースファイルを含むことになります。
この一覧があまりに長くなる場合、最初の<quote>file</quote>を<literal>+</literal>とし、2番目の単語を1行に対して1つのファイル名を持ったファイルとすることができます。␞␞        </para>␞
␝        <para>␟         The tools that generate message catalogs for the translators
         ...
         (identifying the singular and plural message arguments).␟翻訳者が作業を行う上で、どの関数呼び出しが翻訳可能文字列を含むかを知る必要に迫られた場合に、メッセージカタログを生成するツールです。
...
複数形メッセージをサポートする関数がある場合、その項目は（単一形および複数形メッセージ引数を特定する）<literal>func:1,2</literal>のようになります。␞␞        </para>␞
␝    <para>␟     Add a file <filename>po/LINGUAS</filename>, which will contain the list
     of provided translations &mdash; initially empty.␟提供された翻訳のリストを含むファイル<filename>po/LINGUAS</filename>を追加します。初めは空です。␞␞    </para>␞
␝  <para>␟   The build system will automatically take care of building and
   installing the message catalogs.␟ビルドシステムは、自動的にメッセージカタログの構築およびインストールを行います。␞␞  </para>␞
␝  <sect2 id="nls-guidelines">␟   <title>Message-Writing Guidelines</title>␟   <title>メッセージ記述の指針</title>␞␞␞
␝  <para>␟   Here are some guidelines for writing messages that are easily
   translatable.␟メッセージの翻訳を簡単にするために以下に指針をいくつか示します。␞␞␞
␝     <para>␟      Do not construct sentences at run-time, like:␟以下のように実行時に文章を構築することはしないでください。␞␞<programlisting>␞
␝</programlisting>␟      The word order within the sentence might be different in other
      ...
      a message text.␟文章内の単語の順番は言語によって異なる可能性があります。
...
番号、ファイル名、および実行時変数のみメッセージテキストに実行時に挿入するべきです。␞␞     </para>␞
␝     <para>␟      For similar reasons, this won't work:␟同様の理由で、以下も上手くいきません。␞␞<programlisting>␞
␝</programlisting>␟      because it assumes how the plural is formed.  If you figured you
      could solve it like this:␟これは複数形がどのように形成されるかを決めてかかっているからです。
もし、以下のようにして回避できたと考えると、␞␞<programlisting>␞
␝</programlisting>␟      then be disappointed.  Some languages have more than two forms,
      with some peculiar rules.  It's often best to design the message
      to avoid the issue altogether, for instance like this:␟失望することになります。
言語の中には、独特の規則によって2つ以上の形式になるものもあります。
問題全体を回避するためこのメッセージを設計することが最善です。たとえば以下のようにします。␞␞<programlisting>␞
␝     <para>␟      If you really want to construct a properly pluralized message,
      ...
      write something like this:␟適切に複数形を持つメッセージを構築したいと本当に思うのなら、これに対するサポートがありますが、多少厄介です。
<function>ereport()</function>内の主たる、または詳細なエラーメッセージを生成する場合、以下のように書くことができます。␞␞<programlisting>␞
␝</programlisting>␟      The first argument is the format string appropriate for English
      ...
      selected based on the run-time value of <replaceable>n</replaceable>.␟最初の引数は英文の単数形に適切な書式文字列で、二番目は英文の複数形に適切な書式文字列、そして三番目はどの複数形を使用するのかを決定する整数制御値です。
...
翻訳者にはグループとして２つの英文書式を参照し、<replaceable>n</replaceable>の実行時の値に基づいて選択される適切な１つでもって、複数の代替文字列を供給する機会があります。␞␞     </para>␞
␝     <para>␟      If you need to pluralize a message that isn't going directly to an
      ...
      documentation.␟<function>errmsg</function>あるいは<function>errdetail</function>報告に直接行かない複数形メッセージが必要であれば、基礎となっている関数、<function>ngettext</function>を使用する必要があります。
gettextのドキュメントを参照してください。␞␞     </para>␞
␝     <para>␟      If you want to communicate something to the translator, such as
      ...
      with <literal>translator</literal>, e.g.:␟メッセージをどのように他の出力と合わせる予定なのかなど翻訳者と何か連絡を取り合いたい場合、<literal>translator</literal>で始まるコメントを最初に付けてどうなるかを知らせてください。
以下のようにします。␞␞<programlisting>␞
␝</programlisting>␟      These comments are copied to the message catalog files so that
      the translators can see them.␟これらのコメントはメッセージカタログファイルにコピーされますので翻訳者は参照できます。␞␞     </para>␞
