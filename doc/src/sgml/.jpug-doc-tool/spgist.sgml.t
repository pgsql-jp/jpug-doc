...
␝<sect1 id="spgist">␟<title>SP-GiST Indexes</title>␟<title>SP-GiSTインデックス</title>␞␞␞
␝<sect2 id="spgist-intro">␟ <title>Introduction</title>␟ <title>はじめに</title>␞␞␞
␝ <para>␟  <acronym>SP-GiST</acronym> is an abbreviation for space-partitioned
  ...
  can be very fast.␟<acronym>SP-GiST</acronym>は、空間分割された(Space-Partitioned)<acronym>GiST</acronym>を短縮した語です。
...
分割規則によく適合した検索は非常に高速になります。␞␞ </para>␞
␝ <para>␟  These popular data structures were originally developed for in-memory
  ...
  even if it traverses many nodes.␟これらのよく使われるデータ構造は、元々はメモリ内での利用のために開発されたものでした。
...
<acronym>SP-GiST</acronym>によって解決される困難とは、探索木のノードをディスクのページにマップするときに、多数のノードを通り抜ける場合であっても、探索ではごく少数のディスクページにしかアクセスしないですむようにすることです。␞␞ </para>␞
␝ <para>␟  Like <acronym>GiST</acronym>, <acronym>SP-GiST</acronym> is meant to allow
  the development of custom data types with the appropriate access methods,
  by an expert in the domain of the data type, rather than a database expert.␟<acronym>GiST</acronym>と同じく、<acronym>SP-GiST</acronym>は適切なアクセスメソッドのある独自のデータ型の開発を可能にするためのもので、データベースのエキスパートよりもむしろ、そのデータ型の領域のエキスパートによる開発を可能にします。␞␞ </para>␞
␝ <para>␟  Some of the information here is derived from Purdue University's
  ...
  <!-- URL will be changed -->
  <ulink url="http://www.sai.msu.su/~megera/wiki/spgist_dev">web site</ulink>.␟ここで記述する情報の一部はPurdue大学のSP-GiSTインデックスプロジェクト<ulink url="https://www.cs.purdue.edu/spgist/">WEBサイト</ulink>によるものです。
<productname>PostgreSQL</productname>での<acronym>SP-GiST</acronym>の実装は、おもにTeodor SigaevとOleg Bartunovによって保守されており、詳しい情報は彼らの<ulink url="http://www.sai.msu.su/~megera/wiki/spgist_dev">WEBサイト</ulink>にあります。␞␞ </para>␞
␝<sect2 id="spgist-builtin-opclasses">␟ <title>Built-in Operator Classes</title>␟ <title>組み込み演算子クラス</title>␞␞␞
␝ <para>␟  The core <productname>PostgreSQL</productname> distribution
  includes the <acronym>SP-GiST</acronym> operator classes shown in
  <xref linkend="spgist-builtin-opclasses-table"/>.␟<productname>PostgreSQL</productname>のコア配布物には<xref linkend="spgist-builtin-opclasses-table"/>に示される<acronym>SP-GiST</acronym>の演算子クラスが含まれます。␞␞ </para>␞
␝  <table id="spgist-builtin-opclasses-table">␟   <title>Built-in <acronym>SP-GiST</acronym> Operator Classes</title>␟   <title>組み込み<acronym>SP-GiST</acronym>演算子クラス</title>␞␞   <tgroup cols="3">␞
␝     <row>␟      <entry>Name</entry>␟      <entry>名前</entry>␞␞␞
␝      <entry>Name</entry>␟      <entry>Indexable Operators</entry>␟      <entry>インデックス可能な演算子</entry>␞␞␞
␝      <entry>Indexable Operators</entry>␟      <entry>Ordering Operators</entry>␟      <entry>順序付け演算子</entry>␞␞     </row>␞
␝ <para>␟  Of the two operator classes for type <type>point</type>,
  ...
  may offer better performance in some applications.␟<type>point</type>型の2つの演算子クラスのうち、<literal>quad_point_ops</literal>がデフォルトです。
<literal>kd_point_ops</literal>は同じ演算子をサポートしますが、異なるインデックスデータ構造を使うため、アプリケーションによってはより良いパフォーマンスを提供することがあります。␞␞ </para>␞
␝ <para>␟  The <literal>quad_point_ops</literal>, <literal>kd_point_ops</literal> and
  ...
  search over indexed point or polygon data sets.␟<literal>quad_point_ops</literal>、<literal>kd_point_ops</literal>、<literal>poly_ops</literal>演算子クラスは<literal>&lt;-&gt;</literal>順序付け演算子をサポートしますので、インデックス付けされた点や多角形のデータ集合に対してk近傍(<literal>k-NN</literal>)探索が可能です。␞␞ </para>␞
␝<sect2 id="spgist-extensibility">␟ <title>Extensibility</title>␟ <title>拡張性</title>␞␞␞
␝ <para>␟  <acronym>SP-GiST</acronym> offers an interface with a high level of
  ...
  It also takes care of concurrency and logging considerations.␟<acronym>SP-GiST</acronym>は高度に抽象化されたインタフェースを提供します。アクセスメソッドの開発者は特定のデータ型専用のメソッドだけを開発する必要があります。
<acronym>SP-GiST</acronym>のコアは効率的なディスクマッピングと木構造の探索を担当します。
また、同時実行制御とログ出力も担当します。␞␞ </para>␞
␝ <para>␟  Leaf tuples of an <acronym>SP-GiST</acronym> tree usually contain values
  ...
  inner tuples that are passed through to reach the leaf level.␟<acronym>SP-GiST</acronym>のツリーのリーフタプルは、インデックスの付けられた列の損失のある表現を含むこともできますが、通常はインデックスの付けられた列と同じデータ型の値を含んでいます。
ルートレベルに格納されたリーフタプルは、インデックスが付けられた元のデータの値を直接表現していますが、より下のレベルのリーフタプルは、接尾辞など、部分的な値しか含んでいないかも知れません。
この場合、演算子クラスのサポート関数が、内部タプルをリーフレベルまでたどりながら集める情報を使って元の値を再構築できる必要があります。␞␞ </para>␞
␝ <para>␟  When an <acronym>SP-GiST</acronym> index is created with
  ...
  not discussed further here.␟<acronym>SP-GiST</acronym>インデックスが<literal>INCLUDE</literal>列を付けて作成された場合には、その列の値もリーフタプルに格納されます。
<literal>INCLUDE</literal>列は<acronym>SP-GiST</acronym>演算子クラスとは関係ありませんので、ここではこれ以上説明しません。␞␞ </para>␞
␝ <para>␟  Inner tuples are more complex, since they are branching points in the
  ...
  central point.␟内部タプルは、探索木の分岐点となるため、もっと複雑です。
...
省略可能ですが、内部タプルはそのすべてのメンバを記述する<firstterm>接頭辞</firstterm>の値を持つことができます。
...
そうすると、四分木の内部タプルはこの中心点の周りの象限に対応する4つのノードも含むことになるでしょう。␞␞ </para>␞
␝ <para>␟  Some tree algorithms require knowledge of level (or depth) of the current
  ...
  <firstterm>traverse values</firstterm>) during a tree descent.␟木構造のアルゴリズムには、現在のタプルのレベル(深さ)を知っていることが必要なものがあります。そこで、<acronym>SP-GiST</acronym>のコアは、演算子クラスが木構造をたどって下がるときにレベル数の管理を可能にしています。
また、必要であれば、表現される値を加算的に再構築すること、また木構造を下る間に追加データ（<firstterm>探索値</firstterm>と呼ばれます）を渡すこともサポートしています。␞␞ </para>␞
␝  <para>␟   The <acronym>SP-GiST</acronym> core code takes care of null entries.
   ...
   are therefore not discussed further here.␟<acronym>SP-GiST</acronym>のコアのコードはnullエントリについても対応しています。
...
従って、ここではこれ以上、NULLについて説明しません。␞␞  </para>␞
␝ <para>␟  There are five user-defined methods that an index operator class for
  ...
  opclass-specific parameters should be placed, and returns <type>void</type>.␟<acronym>SP-GiST</acronym>のインデックス演算子クラスが提供しなければならないユーザ定義メソッドは5つあり、加えて、オプションのメソッドが2つあります。
5つの必須メソッド全ては2つの<type>internal</type>引数を受け付けるというしきたりに従い、1つ目はサポートメソッドへの入力値を含むC構造体へのポインタで、一方2つ目は出力値が配置されるであろうC構造体へのポインタです。
4つの必須メソッドでは、その結果がすべて出力構造体の中にあるので、単に<type>void</type>を返します。ですが、<function>leaf_consistent</function>は<type>boolean</type>の結果を返します。
メソッドは、その入力構造体のどのフィールドも変更してはいけません。
どんな場合でも、出力構造体はユーザ定義メソッドを呼び出す前にゼロに初期化されます。
オプションの6番目のメソッド<function>compress</function>は、唯一の引数としてインデックス付けされる<type>datum</type>を受け付け、リーフタプルの物理格納に適した値を返します。
オプションの7番目のメソッド<function>options</function>は、演算子クラスに固有のパラメータを入れるC構造体への<type>internal</type>ポインタを受け付け、<type>void</type>を返します。␞␞ </para>␞
␝ <para>␟  The five mandatory user-defined methods are:␟5つの必須ユーザ定義メソッドは以下のとおりです。␞␞ </para>␞
␝      <para>␟       Returns static information about the index implementation, including
       the data type OIDs of the prefix and node label data types.␟接頭辞とノードラベルのデータ型のデータ型OIDを含め、インデックスの実装に関する静的情報を返します。␞␞      </para>␞
␝     <para>␟      The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞<programlisting>␞
␝</programlisting>␟      The first argument is a pointer to a <structname>spgConfigIn</structname>
      C struct, containing input data for the function.
      The second argument is a pointer to a <structname>spgConfigOut</structname>
      C struct, which the function must fill with result data.␟1番目の引数はCの<structname>spgConfigIn</structname>構造体へのポインタで、関数の入力データを含みます。
2番目の引数はCの<structname>spgConfigOut</structname>構造体へのポインタで、関数が結果のデータを入れます。␞␞<programlisting>␞
␝{␟    Oid         attType;        /* Data type to be indexed */␟    Oid         attType;        /* インデックス付けされるデータ型 */␞␞} spgConfigIn;␞
␝{␟    Oid         prefixType;     /* Data type of inner-tuple prefixes */
    Oid         labelType;      /* Data type of inner-tuple node labels */
    Oid         leafType;       /* Data type of leaf-tuple values */
    bool        canReturnData;  /* Opclass can reconstruct original data */
    bool        longValuesOK;   /* Opclass can cope with values &gt; 1 page */␟    Oid         prefixType;     /* 内部タプルの接頭辞のデータ型 */
    Oid         labelType;      /* 内部タプルのノードのラベルのデータ型 */
    Oid         leafType;       /* リーフタプル値のデータ型 */
    bool        canReturnData;  /* 演算子クラスは元のデータを再構築できる */
    bool        longValuesOK;   /* 演算子クラスは1ページより大きな値を扱える */␞␞} spgConfigOut;␞
␝␟      <structfield>attType</structfield> is passed in order to support polymorphic
      ...
      will always have the same value and so can be ignored.␟<structfield>attType</structfield>は多様のインデックス演算子クラスをサポートするために渡されます。
通常の固定データ型の演算子クラスでは、それは常に同じ値を持っているので無視できます。␞␞     </para>␞
␝     <para>␟      For operator classes that do not use prefixes,
      ...
      (see <xref linkend="spgist-limits"/>).␟接頭辞を使わない演算子クラスでは、<structfield>prefixType</structfield>を<literal>VOIDOID</literal>に設定することができます。
...
<structfield>attType</structfield>が可変長で、演算子クラスが接尾辞付けの繰り返しによって長い値を分割できるときにのみ、<structfield>longValuesOK</structfield>をtrueにします(<xref linkend="spgist-limits"/>参照)。␞␞     </para>␞
␝     <para>␟      <structfield>leafType</structfield> should match the index storage type
      ...
      <structfield>opckeytype</structfield>.␟<structfield>leafType</structfield>は、演算子クラスの<structfield>opckeytype</structfield>カタログエントリにより定義されたインデックス格納型と一致しなければなりません。
(<structfield>opckeytype</structfield>は0の場合もあり得て、それは格納型が演算子クラスの入力型と同じであることを意味しています。これが最も一般的な状況であることに注意してください。)
後方互換性のため、<function>config</function>メソッドは<structfield>leafType</structfield>を他の値に設定して、その値を使うことができます。ですが、インデックスの内容がカタログでは誤って特定されますので、これは非推奨です。
また、<structfield>leafType</structfield>を初期化しないまま(0)にできます。これは<structfield>opckeytype</structfield>から導かれたインデックス格納型を意味すると解釈されます。␞␞     </para>␞
␝     <para>␟      When <structfield>attType</structfield>
      ...
      to <structfield>leafType</structfield>.␟<structfield>attType</structfield>と<structfield>leafType</structfield>が異なる場合には、オプションのメソッド<function>compress</function>を提供しなければなりません。
メソッド<function>compress</function>は、インデックス付けされるデータを<structfield>attType</structfield>から<structfield>leafType</structfield>に変換する責任があります。␞␞     </para>␞
␝      <para>␟        Chooses a method for inserting a new value into an inner tuple.␟内部タプルに新しい値を挿入するときのメソッドを選択します。␞␞      </para>␞
␝     <para>␟      The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞<programlisting>␞
␝</programlisting>␟      The first argument is a pointer to a <structname>spgChooseIn</structname>
      C struct, containing input data for the function.
      The second argument is a pointer to a <structname>spgChooseOut</structname>
      C struct, which the function must fill with result data.␟1番目の引数はCの<structname>spgChooseIn</structname>構造体へのポインタで、関数の入力データを含みます。
2番目の引数はCの<structname>spgChooseOut</structname>構造体へのポインタで、関数が結果のデータを入れます。␞␞<programlisting>␞
␝{␟    Datum       datum;          /* original datum to be indexed */
    Datum       leafDatum;      /* current datum to be stored at leaf */
    int         level;          /* current level (counting from zero) */␟    Datum       datum;          /* インデックス付けされる元のデータ */
...
␝␟    /* Data from current inner tuple */
    ...
    Datum      *nodeLabels;     /* node label values (NULL if none) */␟    /* 現在の内部タプルからのデータ */
    ...
    Datum      *nodeLabels;     /* ノードのラベルの値（なければNULL） */␞␞} spgChooseIn;␞
␝{␟    spgMatchNode = 1,           /* descend into existing node */
    spgAddNode,                 /* add a node to the inner tuple */
    spgSplitTuple               /* split inner tuple (change its prefix) */␟    spgMatchNode = 1,           /* 既存のノードに下がる */
    spgAddNode,                 /* ノードに内部タプルを追加する */
    spgSplitTuple               /* 内部タプルを分割する(その接頭辞を変更する) */␞␞} spgChooseResultType;␞
␝{␟    spgChooseResultType resultType;     /* action code, see above */␟    spgChooseResultType resultType;     /* アクションコード、上記参照 */␞␞    union␞
␝    {␟        struct                  /* results for spgMatchNode */␟        struct                  /* spgMatchNodeの結果 */␞␞        {␞
␝        {␟            int         nodeN;      /* descend to this node (index from 0) */
            int         levelAdd;   /* increment level by this much */
            Datum       restDatum;  /* new leaf datum */␟            int         nodeN;      /* このノードに下がる(0からのインデックス) */
            int         levelAdd;   /* この分だけレベルを増やす */
            Datum       restDatum;  /* 新しいリーフデータ */␞␞        }           matchNode;␞
␝        }           matchNode;␟        struct                  /* results for spgAddNode */␟        struct                  /* spgAddNodeの結果 */␞␞        {␞
␝        {␟            Datum       nodeLabel;  /* new node's label */
            int         nodeN;      /* where to insert it (index from 0) */␟            Datum       nodeLabel;  /* 新しいノードのラベル */
            int         nodeN;      /* 挿入する場所(0からのインデックス) */␞␞        }           addNode;␞
␝        }           addNode;␟        struct                  /* results for spgSplitTuple */␟        struct                  /* spgSplitTupleの結果 */␞␞        {␞
␝        {␟            /* Info to form new upper-level inner tuple with one child tuple */
            ...
            Datum      *prefixNodeLabels;   /* their labels (or NULL for
                                             * no labels) */
            int         childNodeN;         /* which node gets child tuple */␟            /* 子タプルを1つ持つ新しい上位のレベルの内部タプルを生成するための情報 */
...
␝␟            /* Info to form new lower-level inner tuple with all old nodes */
            ...
            Datum       postfixPrefixDatum; /* if so, its value */␟            /* 古いノードをすべて持つ新しい低位の内部タプルを生成するための情報 */
            bool        postfixHasPrefix;   /* タプルは接頭辞を持つか */
            Datum       postfixPrefixDatum; /* そうならば、その値 */␞␞        }           splitTuple;␞
␝␟       <structfield>datum</structfield> is the original datum of
       ...
       NULL if there are no labels.␟<structfield>datum</structfield>はインデックスに挿入できた<structname>spgConfigIn</structname>.<structfield>attType</structfield>型の元データです。
...
<structfield>nNodes</structfield>は内部タプルが含む子ノードの数で、<structfield>nodeLabels</structfield>はそれらのラベル値の配列、あるいはラベルがなければNULLになります。␞␞      </para>␞
␝      <para>␟       The <function>choose</function> function can determine either that
       ...
       less restrictive prefix.␟<function>choose</function>関数は、新しい値が既存の子ノードの1つとマッチするか、新しい子ノードを追加する必要があるか、あるいは新しい値がタプルの接頭辞と適合しないので内部タプルを分割してより制限のない接頭辞を作成する必要があるか、を決定することができます。␞␞      </para>␞
␝      <para>␟       If the new value matches one of the existing child nodes,
       ...
       <structfield>leafDatum</structfield> at the next level.␟新しい値が既存の子ノードの1つにマッチしたときは、<structfield>resultType</structfield>を<literal>spgMatchNode</literal>にセットします。
...
<structfield>restDatum</structfield>は、演算子クラスがデータをあるレベルから次のレベルに変更しないのであれば、<structfield>datum</structfield>に等しくセットします。そうでなければ、次のレベルで<structfield>leafDatum</structfield>として使われる修正された値にセットします。␞␞      </para>␞
␝      <para>␟       If a new child node must be added,
       ...
       that call should result in an <literal>spgMatchNode</literal> result.␟新しい子ノードを追加しなければならないときは、<structfield>resultType</structfield>を<literal>spgAddNode</literal>にセットします。
...
ノードを追加した後で、<function>choose</function>関数を修正された内部タプルを使って再び呼び出しますが、このときは、<literal>spgMatchNode</literal>という結果になるはずです。␞␞      </para>␞
␝      <para>␟       If the new value is inconsistent with the tuple prefix,
       ...
       allow the insertion to descend to the next level.␟新しい値がタプルの接頭辞と適合しないときは、<structfield>resultType</structfield>を<literal>spgSplitTuple</literal>にセットします。
...
そのうち、<function>choose</function>が<literal>spgMatchNode</literal>を返し、次のレベルに下がる挿入が可能となります。␞␞      </para>␞
␝      <para>␟       Decides how to create a new inner tuple over a set of leaf tuples.␟リーフタプルの集合に対し、新しい内部タプルをどうやって作るかを決定します。␞␞      </para>␞
␝      <para>␟        The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞<programlisting>␞
␝</programlisting>␟      The first argument is a pointer to a <structname>spgPickSplitIn</structname>
      C struct, containing input data for the function.
      The second argument is a pointer to a <structname>spgPickSplitOut</structname>
      C struct, which the function must fill with result data.␟1番目の引数はCの<structname>spgPickSplitIn</structname>構造体へのポインタで、関数の入力データを含みます。
2番目の引数はCの<structname>spgPickSplitOut</structname>構造体へのポインタで、関数が結果のデータを入れます。␞␞<programlisting>␞
␝{␟    int         nTuples;        /* number of leaf tuples */
    Datum      *datums;         /* their datums (array of length nTuples) */
    int         level;          /* current level (counting from zero) */␟    int         nTuples;        /* リーフタプルの数 */
    Datum      *datums;         /* そのデータ(長さnTuplesの配列) */
    int         level;          /* (0から数えた)現在のレベル */␞␞} spgPickSplitIn;␞
␝{␟    bool        hasPrefix;      /* new inner tuple should have a prefix? */
    Datum       prefixDatum;    /* if so, its value */␟    bool        hasPrefix;      /* 新しい内部タプルは接頭辞を持つか */
    Datum       prefixDatum;    /* もしそうなら、その値 */␞␞␞
␝␟    int         nNodes;         /* number of nodes for new inner tuple */
    Datum      *nodeLabels;     /* their labels (or NULL for no labels) */␟    int         nNodes;         /* 新しい内部タプルのノード数 */
    Datum      *nodeLabels;     /* そのラベル(ラベルがなければNULL) */␞␞␞
␝␟    int        *mapTuplesToNodes;   /* node index for each leaf tuple */
    Datum      *leafTupleDatums;    /* datum to store in each new leaf tuple */␟    int        *mapTuplesToNodes;   /* 各リーフタプルへのノードのインデックス */
    Datum      *leafTupleDatums;    /* 新しい各リーフタプルに保存されているデータ */␞␞} spgPickSplitOut;␞
␝␟       <structfield>nTuples</structfield> is the number of leaf tuples provided.
       ...
       share, which will become the level of the new inner tuple.␟<structfield>nTuples</structfield>は与えられるリーフタプルの個数です。
<structfield>datums</structfield>は<structname>spgConfigOut</structname>.<structfield>leafType</structfield>型のそれらのデータ値の配列です。
<structfield>level</structfield>はすべてのリーフタプルが共有する現在のレベルで、これが新しい内部タプルのレベルになります。␞␞      </para>␞
␝      <para>␟       Set <structfield>hasPrefix</structfield> to indicate whether the new inner
       ...
       <structfield>leafTupleDatums</structfield> arrays.␟<structfield>hasPrefix</structfield>は新しい内部タプルが接頭辞を持つかどうかを示し、持つ場合は<structfield>prefixDatum</structfield>を接頭辞の値にセットします。
...
<function>picksplit</function>関数は、<structfield>nodeLabels</structfield>、<structfield>mapTuplesToNodes</structfield>、<structfield>leafTupleDatums</structfield>の配列についてpallocしなければならないことに注意してください。␞␞      </para>␞
␝      <para>␟       If more than one leaf tuple is supplied, it is expected that the
       ...
       See <xref linkend="spgist-all-the-same"/> for more information.␟2つ以上のリーフタプルを与えた場合、<function>picksplit</function>関数はそれらを2つ以上のノードに分類すると予想されます。そうでなければ、リーフタプルを複数のページにまたがって分割するという、この操作の究極の目的を実現できないからです。
...
詳細な情報は<xref linkend="spgist-all-the-same"/>を参照してください。␞␞      </para>␞
␝      <para>␟       <function>picksplit</function> can be applied to a single leaf tuple only
       ...
       more information.␟<function>config</function>関数が<structfield>longValuesOK</structfield>をtrueにセットし、1ページよりも大きな入力値を与える場合にのみ、<function>picksplit</function>を1つだけのリーフタプルに適用できます。
この場合の操作の重要な点は、接頭辞をはがして、新しい、より短いリーフデータの値を生成することです。
この呼出は、1ページに収まる短さのリーフデータが生成されるまで繰り返されます。
詳細な情報は<xref linkend="spgist-limits"/>を参照してください。␞␞      </para>␞
␝      <para>␟       Returns set of nodes (branches) to follow during tree search.␟ツリーの探索でたどるべきノード(枝)の集合を返します。␞␞      </para>␞
␝      <para>␟       The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞<programlisting>␞
␝</programlisting>␟      The first argument is a pointer to a <structname>spgInnerConsistentIn</structname>
      C struct, containing input data for the function.
      The second argument is a pointer to a <structname>spgInnerConsistentOut</structname>
      C struct, which the function must fill with result data.␟1番目の引数はCの<structname>spgInnerConsistentIn</structname>構造体へのポインタで、関数の入力データを含みます。
2番目の引数はCの<structname>spgInnerConsistentOut</structname>構造体へのポインタで、関数が結果のデータを入れます。␞␞␞
␝{␟    ScanKey     scankeys;       /* array of operators and comparison values */
    ScanKey     orderbys;       /* array of ordering operators and comparison
                                 * values */
    int         nkeys;          /* length of scankeys array */
    int         norderbys;      /* length of orderbys array */␟    ScanKey     scankeys;       /* 演算子と比較する値の配列 */
...
␝␟    Datum       reconstructedValue;     /* value reconstructed at parent */
    ...
    bool        returnData;     /* original data must be returned? */␟    Datum       reconstructedValue;     /* 親で再構築された値 */
...
␝␟    /* Data from current inner tuple */
    ...
    Datum       prefixDatum;    /* もしそうなら、接頭辞の値 */
    int         nNodes;         /* 内部タプルの中のノード数 */
    Datum      *nodeLabels;     /* ノードのラベルの値（なければNULL） */␞␞} spgInnerConsistentIn;␞
␝{␟    int         nNodes;         /* number of child nodes to be visited */
    int        *nodeNumbers;    /* their indexes in the node array */
    int        *levelAdds;      /* increment level by this much for each */
    Datum      *reconstructedValues;    /* associated reconstructed values */
    void      **traversalValues;        /* opclass-specific traverse values */
    double    **distances;              /* associated distances */␟    int         nNodes;         /* 訪れるべき子ノードの数 */
    ...
    double    **distances;              /* 関連する距離 */␞␞} spgInnerConsistentOut;␞
␝␟       The array <structfield>scankeys</structfield>, of length <structfield>nkeys</structfield>,
       ...
       inner tuple, and
       <structfield>nodeLabels</structfield> is an array of their label values, or
       NULL if the nodes do not have labels.␟配列<structfield>scankeys</structfield>は長さが<structfield>nkeys</structfield>で、インデックス検索の条件を記述します。
...
配列<structfield>orderbys</structfield>は長さが<structfield>norderbys</structfield>で、(もしあれば)順序付け演算子を同じように記述します。
...
<structfield>nNodes</structfield>は内部タプルに含まれる子ノードの数で、<structfield>nodeLabels</structfield>はそれらのラベル値の配列、あるいは、ノードがラベルを持たないならNULLです。␞␞      </para>␞
␝      <para>␟       <structfield>nNodes</structfield> must be set to the number of child nodes that
       ...
       Each traverse value must be a single palloc'd chunk.␟<structfield>nNodes</structfield>は探索で訪れる必要のある子ノードの数にセットされなければなりません。また、<structfield>nodeNumbers</structfield>はそれらの番号の配列にセットされなければなりません。
...
再構築された値は<structname>spgConfigOut</structname>.<structfield>leafType</structfield>型と仮定されます。
(しかしながら、コアシステムはそれらに対してコピー以外のことをしませんので、<structfield>leafType</structfield>と同じ<literal>typlen</literal>と<literal>typbyval</literal>属性を持っていれば十分です。)
順序付け検索を実行するなら、<structfield>orderbys</structfield>配列に従って<structfield>distances</structfield>に距離の値の配列を設定します(距離の最も近いノードが最初に処理されます)。
そうでなければNULLのままにします。
...
<function>inner_consistent</function>関数は、現在のメモリコンテキスト内の<structfield>nodeNumbers</structfield>、<structfield>levelAdds</structfield>、<structfield>distances</structfield>、<structfield>reconstructedValues</structfield>、<structfield>traversalValues</structfield>の配列についてpallocしなければならないことに注意してください。
ただし、<structfield>traversalValues</structfield>配列が指すすべての出力探索値は<structfield>traversalMemoryContext</structfield>内に割り当てられます。
それぞれの探索値は1つのpallocされた塊でなければなりません。␞␞      </para>␞
␝      <para>␟       Returns true if a leaf tuple satisfies a query.␟リーフタプルが問い合わせを満たす場合、trueを返します。␞␞      </para>␞
␝      <para>␟       The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞<programlisting>␞
␝</programlisting>␟      The first argument is a pointer to a <structname>spgLeafConsistentIn</structname>
      C struct, containing input data for the function.
      The second argument is a pointer to a <structname>spgLeafConsistentOut</structname>
      C struct, which the function must fill with result data.␟1番目の引数はCの<structname>spgLeafConsistentIn</structname>構造体へのポインタで、関数の入力データを含みます。
2番目の引数はCの<structname>spgLeafConsistentOut</structname>構造体へのポインタで、関数が結果のデータを入れます。␞␞<programlisting>␞
␝{␟    ScanKey     scankeys;       /* array of operators and comparison values */
    ScanKey     orderbys;       /* array of ordering operators and comparison
                                 * values */
    int         nkeys;          /* length of scankeys array */
...
␝␟    Datum       reconstructedValue;     /* value reconstructed at parent */
    ...
    int         level;          /* (0から数えた)現在のレベル */
    bool        returnData;     /* 元のデータを返さなければならないか */␞␞␞
␝␟    Datum       leafDatum;      /* datum in leaf tuple */␟    Datum       leafDatum;      /* リーフタプルのデータ */␞␞} spgLeafConsistentIn;␞
␝{␟    Datum       leafValue;        /* reconstructed original data, if any */
    bool        recheck;          /* set true if operator must be rechecked */
    bool        recheckDistances; /* set true if distances must be rechecked */
    double     *distances;        /* associated distances */␟    Datum       leafValue;        /* もしあれば、再構築された元のデータ */
    bool        recheck;          /* 演算子を再チェックする必要があればtrue */
    bool        recheckDistances; /* 距離を再チェックする必要があればtrue */
    double     *distances;        /* 関連する距離 */␞␞} spgLeafConsistentOut;␞
␝␟       The array <structfield>scankeys</structfield>, of length <structfield>nkeys</structfield>,
       describes the index search condition(s).  These conditions are
       combined with AND &mdash; only index entries that satisfy all of
       ...
       <structname>spgConfigOut</structname>.<structfield>leafType</structfield>
       stored in the current leaf tuple.␟配列<structfield>scankeys</structfield>は長さが<structfield>nkeys</structfield>で、インデックス探索の条件を記述します。
複数の条件はANDで結合されます。つまり、条件のすべてを満たすインデックスエントリのみが対象となります。
...
配列<structfield>orderbys</structfield>は長さが<structfield>norderbys</structfield>で、順序付け演算子を同じように記述します。
<structfield>reconstructedValue</structfield>は親タプルのために再構築された値で、ルートレベルの場合、あるいは親レベルの<function>inner_consistent</function>関数が値を返さなかった場合は<literal>(Datum) 0</literal>となります。
<structfield>traversalValue</structfield>は親インデックスのタプルの<function>inner_consistent</function>の前の呼び出しから渡された探索データへのポインタで、ルートレベルならNULLです。
...
<structfield>leafDatum</structfield>は現在のリーフタプルに格納されている<structname>spgConfigOut</structname>.<structfield>leafType</structfield>のキーの値です。␞␞      </para>␞
␝      <para>␟       The function must return <literal>true</literal> if the leaf tuple matches the
       ...
       fetching the tuple from the heap, and will reorder the tuples if needed.␟この関数は、リーフタプルが問い合わせにマッチすれば<literal>true</literal>を返し、マッチしなければ<literal>false</literal>を返します。
<literal>true</literal>の場合、<structfield>returnData</structfield>が<literal>true</literal>であれば、<structfield>leafValue</structfield>を、このリーフタプルにインデックス付けするために元々提供された(<structname>spgConfigIn</structname>.<structfield>attType</structfield>型の)値に設定しなければなりません。
...
順序付け検索を実行するなら、<structfield>orderbys</structfield>配列に従って<structfield>distances</structfield>に距離の値の配列を設定します。
そうでなければNULLのままにします。
返される距離の少なくとも1つが正確でないのなら、<structfield>recheckDistances</structfield>にtrueを設定します。
この場合、エグゼキュータはヒープからタプルを取得した後正確な距離を計算し、必要ならタプルを並べ替えます。␞␞      </para>␞
␝ <para>␟  The optional user-defined methods are:␟オプションのユーザ定義メソッドは以下です。␞␞ </para>␞
␝      <para>␟       Converts a data item into a format suitable for physical storage in
       a leaf tuple of the index.  It accepts a value of type
       <structname>spgConfigIn</structname>.<structfield>attType</structfield>
       and returns a value of type
       <structname>spgConfigOut</structname>.<structfield>leafType</structfield>.
       The output value must not contain an out-of-line TOAST pointer.␟インデックスのリーフタプルでデータ項目を物理ストレージに適した形式に変換します。
<structname>spgConfigIn</structname>.<structfield>attType</structfield>型の値を受け付け、<structname>spgConfigOut</structname>.<structfield>leafType</structfield>型の値を返します。
出力値は行に収まらないTOASTを含んでいてはいけません。␞␞      </para>␞
␝      <para>␟       Note: the <function>compress</function> method is only applied to
       ...
       using <function>compress</function>.␟注意: <function>compress</function>メソッドは格納される値にのみ適用されます。
適合するメソッドは<function>compress</function>を使って変換することなく、問い合わせの<structfield>scankeys</structfield>をそのまま受け取ります。␞␞      </para>␞
␝      <para>␟       Defines a set of user-visible parameters that control operator class
       behavior.␟演算子クラスの振舞いを制御するユーザに可視のパラメータの集合を定義します。␞␞      </para>␞
␝      <para>␟        The <acronym>SQL</acronym> declaration of the function must look like this:␟この関数の<acronym>SQL</acronym>宣言は以下のようになります。␞␞␞
␝      <para>␟       The function is passed a pointer to a <structname>local_relopts</structname>
       struct, which needs to be filled with a set of operator class
       specific options.  The options can be accessed from other support
       functions using the <literal>PG_HAS_OPCLASS_OPTIONS()</literal> and
       <literal>PG_GET_OPCLASS_OPTIONS()</literal> macros.␟関数には<structname>local_relopts</structname>構造体へのポインタが渡されますが、構造体を演算子クラスに固有のオプションの集合で満たすことが必要です。
オプションはマクロ<literal>PG_HAS_OPCLASS_OPTIONS()</literal>と<literal>PG_GET_OPCLASS_OPTIONS()</literal>を使って他のサポート関数からアクセスできます。␞␞      </para>␞
␝      <para>␟       Since the representation of the key in <acronym>SP-GiST</acronym> is
       flexible, it may depend on user-specified parameters.␟<acronym>SP-GiST</acronym>でのキーの表現には柔軟性がありますので、ユーザに固有のパラメータに依存するかもしれません。␞␞      </para>␞
␝  <para>␟   All the SP-GiST support methods are normally called in a short-lived
   ...
   constants into the passed parameter struct.)␟SP-GiSTのすべてのサポートメソッドは、通常は短期間有効なメモリコンテキスト内で呼び出されます。つまり、それぞれのタプルについて処理した後で<varname>CurrentMemoryContext</varname>はリセットされます。
したがって、pallocしたものすべてについてpfreeすることを気にかけることはあまり重要ではありません。
(<function>config</function>メソッドは例外で、メモリリークを避けるようにする必要があります。
しかし、通常は<function>config</function>メソッドは、パラメータとして渡された構造体に定数を代入する以外、何もする必要がありません。)␞␞  </para>␞
␝  <para>␟   If the indexed column is of a collatable data type, the index collation
   will be passed to all the support methods, using the standard
   <function>PG_GET_COLLATION()</function> mechanism.␟インデックス付けされた列が照合可能なデータ型の場合、インデックスの照合は、標準的な<function>PG_GET_COLLATION()</function>の仕組みを使ってすべてのサポートメソッドに渡されます。␞␞  </para>␞
␝<sect2 id="spgist-implementation">␟ <title>Implementation</title>␟ <title>実装</title>␞␞␞
␝  <para>␟   This section covers implementation details and other tricks that are
   useful for implementers of <acronym>SP-GiST</acronym> operator classes to
   know.␟この節では、<acronym>SP-GiST</acronym>の演算子クラスを実装する人にとって知っていると役に立つ、実装についての詳細とその他の秘訣について説明します。␞␞  </para>␞
␝ <sect3 id="spgist-limits">␟  <title>SP-GiST Limits</title>␟  <title>SP-GiSTの制限</title>␞␞␞
␝  <para>␟   Individual leaf tuples and inner tuples must fit on a single index page
   ...
   on an index page.␟それぞれのリーフタプルおよび内部タプルは1つのインデックスページ内(デフォルトで8kB)に収まらなければなりません。
...
そうでなければ、<acronym>SP-GiST</acronym>のコアは、インデックスページに収めるには大きすぎる値についてのインデックス付け要求を拒絶します。␞␞  </para>␞
␝  <para>␟   Likewise, it is the operator class's responsibility that inner tuples
   ...
   maximum size of a prefix value.␟同様に、内部タプルが大きくなりすぎてインデックスページに収まらない、ということにならないようにするのは、演算子クラスの責任です。
これにより、1つの内部タプルで使うことができる子ノードの数、および接頭辞の値の最大サイズが制限されます。␞␞  </para>␞
␝  <para>␟   Another limitation is that when an inner tuple's node points to a set
   ...
   extraordinary measures described in <xref linkend="spgist-all-the-same"/>.␟内部タプルのノードがリーフタプルの集合を指しているとき、それらのタプルはすべて同じインデックスページ内になければならない、という制限もあります。
...
演算子クラスの<function>picksplit</function>関数がそれをするのに失敗したときは、<acronym>SP-GiST</acronym>のコアは、<xref linkend="spgist-all-the-same"/>に記述されている特別な手段に頼ることになります。␞␞  </para>␞
␝  <para>␟   When <structfield>longValuesOK</structfield> is true, it is expected
   ...
   of <function>choose</function> method calls.␟<structfield>longValuesOK</structfield>が真であれば、<acronym>SP-GiST</acronym>ツリーの後続のレベルは、より多くの情報を接頭辞と内部タプルのノードラベルへと吸収し、要求されるリーフデータより小さくして、最終的には1ページに収まるようになることが期待されます。
演算子クラスのバグが無限の挿入ループを引き起こすのを防ぐために、<function>choose</function>メソッドの呼び出しの10サイクル以内でリーフデータが少しも小さくならなければ、<acronym>SP-GiST</acronym>コアはエラーを発生します。␞␞  </para>␞
␝ <sect3 id="spgist-null-labels">␟  <title>SP-GiST Without Node Labels</title>␟  <title>ノードラベルのないSP-GiST</title>␞␞␞
␝  <para>␟   Some tree algorithms use a fixed set of nodes for each inner tuple;
   ...
   for others in the same index.␟木構造のアルゴリズムには、それぞれの内部タプルに対して固定された集合のノードを使うものがあります。
...
原則として、ノードラベルは同じインデックス中の一部の内部タプルに使い、他の内部タプルには省略する、ということができます。␞␞  </para>␞
␝  <para>␟   When working with an inner tuple having unlabeled nodes, it is an error
   for <function>choose</function> to return <literal>spgAddNode</literal>, since the set
   of nodes is supposed to be fixed in such cases.␟ラベルのないノードを持つ内部タプルを処理するときに、<function>choose</function>が<literal>spgAddNode</literal>を返すのはエラーです。というのは、この場合、ノードの集合は固定されていると想定されるからです。␞␞  </para>␞
␝ <sect3 id="spgist-all-the-same">␟  <title><quote>All-the-Same</quote> Inner Tuples</title>␟  <title><quote>All-the-Same</quote>内部タプル</title>␞␞␞
␝  <para>␟   The <acronym>SP-GiST</acronym> core can override the results of the
   ...
   tuple does not have the node set that they might otherwise expect.␟<function>picksplit</function>が入力のリーフ値を少なくとも2つのノード分類に分割できなかった場合、<acronym>SP-GiST</acronym>のコアは演算子クラスの<function>picksplit</function>関数の結果を無効にすることがあります。
...
内部タプルには<literal>allTheSame</literal>のフラグがセットされ、<function>choose</function>関数および<function>inner_consistent</function>関数に対し、そのタプルが通常期待されるようなノードの集合を持っていないことを警告します。␞␞  </para>␞
␝  <para>␟   When dealing with an <literal>allTheSame</literal> tuple, a <function>choose</function>
   ...
   doesn't match the existing nodes.␟<literal>allTheSame</literal>の処理において、<function>choose</function>の<literal>spgMatchNode</literal>という結果は、新しい値は等価なノードのどれに割り当てられても良い、という意味に解釈されます。
...
挿入する値が既存のノードとマッチしない時は、<literal>spgSplitTuple</literal>のアクションを使わなければなりません。␞␞  </para>␞
␝  <para>␟   When dealing with an <literal>allTheSame</literal> tuple, the
   ...
   assumes about the meaning of the nodes.␟<literal>allTheSame</literal>のタプルの処理において、すべてのノードは等価なので、<function>inner_consistent</function>関数は、インデックス検索を続けるためのターゲットとして、すべてのノードを返すか、ノードを1つも返さないかのいずれかであるべきです。
このために、特殊ケースを扱うコードが必要になるかもしれませんし、必要ないかもしれません。それは、<function>inner_consistent</function>関数が、通常、ノードの意味についてどの程度のことを仮定しているかに依存します。␞␞  </para>␞
␝<sect2 id="spgist-examples">␟ <title>Examples</title>␟ <title>例</title>␞␞␞
␝ <para>␟  The <productname>PostgreSQL</productname> source distribution includes
  ...
  and <filename>src/backend/utils/adt/</filename> to see the code.␟<productname>PostgreSQL</productname>のソースコードの配布物には、<xref linkend="spgist-builtin-opclasses-table"/>に示すように、<acronym>SP-GiST</acronym>のインデックス演算子クラスの例がいくつか含まれています。
コードを見るには<filename>src/backend/access/spgist/</filename>と<filename>src/backend/utils/adt/</filename>を調べてみてください。␞␞ </para>␞
