␝<sect1 id="amcheck" xreflabel="amcheck">␟ <title>amcheck &mdash; tools to verify table and index consistency</title>␟ <title>amcheck &mdash; テーブルとインデックスの一貫性を検査するツール</title>␞␞␞
␝ <para>␟  The <filename>amcheck</filename> module provides functions that allow you to
  verify the logical consistency of the structure of relations.␟<filename>amcheck</filename>モジュールは、リレーションの構造の論理的な一貫性を検査する機能を提供します。␞␞ </para>␞
␝ <para>␟  The B-Tree checking functions verify various <emphasis>invariants</emphasis> in the
  ...
  pg_temp</literal>.␟B-Tree検査関数は、特定のリレーションの構造表現における様々な<emphasis>不変量</emphasis>を検査します。
インデックススキャンや、その他の重要な操作を担うアクセスメソッド関数の正しさは、これらの不変量を常に保つことに依存しています。
...
これらのチェック関数の実行中、<xref linkend="guc-search-path"/>は一時的に<literal>pg_catalog, pg_temp</literal>に変更されます。␞␞ </para>␞
␝ <para>␟  Verification is performed using the same procedures as those used by
  ...
  functions.␟検証は、インデックススキャン自身で使われるのと同じ手続きを用いて行われます。
その手続きは、ユーザ定義演算子クラスのコードかもしれません。
たとえば、B-Treeインデックスの検査は、一つ以上のB-Treeサポート関数1ルーチンを用いる比較に依存しています。
演算子クラスサポート関数の詳細については<xref linkend="xindex-support"/>をご覧ください。␞␞ </para>␞
␝ <para>␟  Unlike the B-Tree checking functions which report corruption by raising
  ...
  function may be unable to continue and may instead raise an error.␟エラーを起こすことによって破損を報告するB-Tree検査関数とは違って、ヒープ検査関数<function>verify_heapam</function>はテーブルを検査し、破損の検出ごとに1行が対応する行の集合を返そうとします。
にも関わらず、<function>verify_heapam</function>が依存する機能自体が破損していれば、この関数は続行することができず、代わりにエラーを引き起こすかもしれません。␞␞ </para>␞
␝ <para>␟  Permission to execute <filename>amcheck</filename> functions may be granted
  ...
  able to infer something of the data itself from such messages.␟非スーパーユーザに<filename>amcheck</filename>関数の実行許可を与えることができますが、そのような権限を許可する前に、データのセキュリティとプライバシー上の懸念を注意深く考慮すべきです。
これらの関数が生成する破損報告は、データの構造と見つかった破損の性質ほどには、破損データの内容に焦点を当てるわけではありませんが、とりわけ攻撃者が破損を引き起こすこともできるのであれば、これらの関数の実行権限を与えられた攻撃者はそうしたメッセージからデータ自身を推測できるかもしれません。␞␞ </para>␞
␝ <sect2 id="amcheck-functions">␟  <title>Functions</title>␟  <title>関数</title>␞␞␞
␝     <para>␟      <function>bt_index_check</function> tests that its target, a
      B-Tree index, respects a variety of invariants.  Example usage:␟<function>bt_index_check</function>は対象となるB-Treeインデックスが、様々な不変量を維持していることをテストします。
例を示します。␞␞<screen>␞
␝</screen>␟      This example shows a session that performs verification of the
      ...
      database where verification is supported.␟この例では、データベース<quote>test</quote>中のもっとも大きな10個のカタログインデックスの検証を行うセッションを示しています。
インデックスタプルに対応するヒープタプルの存在の検証が、ユニークインデックスであるインデックスの一部に対して要求されています。
エラーは出ていないので、テストしたすべてのインデックスは論理的に一貫していることがわかります。
当然のことながら、この問い合わせは、検証可能なデータベース中のすべてのインデックスに対して<function>bt_index_check</function>を呼び出すように変更できます。␞␞     </para>␞
␝     <para>␟      <function>bt_index_check</function> acquires an <literal>AccessShareLock</literal>
      ...
      impact on application performance and availability.␟<function>bt_index_check</function>は、対象となるインデックスと、それが所属するヒープリレーションに対して<literal>AccessShareLock</literal>を獲得します。
...
実行中の運用環境で定期的、軽量なデータ破損検査が必要な場合、<function>bt_index_check</function>を使うのが、検査の完全性と、アプリケーションの性能と稼働への影響を限定することとの間の最良のトレードオフになることがしばしばあります。␞␞     </para>␞
␝     <para>␟      <function>bt_index_parent_check</function> tests that its
      ...
      inconsistency or other problem.␟<function>bt_index_parent_check</function>は、対象となるB-Treeインデックスが様々な不変量を保っていることを検査します。
...
<function>bt_index_parent_check</function>は、論理的な非一貫性やその他の問題を発見した場合、一般的な習慣に従ってエラーを報告します。␞␞     </para>␞
␝     <para>␟      A <literal>ShareLock</literal> is required on the target index by
      ...
      only while running, not for the entire transaction.␟<function>bt_index_parent_check</function>は、対象インデックスに<literal>ShareLock</literal>を獲得することを必要とします。
...
この関数は実行中のみロックを保持し、トランザクション全体に保持するのではないことに注意してください。␞␞     </para>␞
␝     <para>␟      <function>bt_index_parent_check</function>'s additional
      ...
      replicas), unlike <function>bt_index_check</function>.␟<function>bt_index_parent_check</function>による追加の検査では、様々な病的な事象を検出する可能性があります。
これらの現象は、チェック対象のインデックスが使用している間違った実装がされたB-Tree演算子クラスによるものや、もしかしたら関連するB-Treeインデックスアクセスメソッドのコード中のまだ見つかっていないバグによるものなのかもしれません。
<function>bt_index_check</function>と違って、<function>bt_index_parent_check</function>は、ホットスタンバイモードが有効な場合（すなわち、読み出し専用物理レプリカ）では使用できません。␞␞     </para>␞
␝   <para>␟    <function>bt_index_check</function> and
    ...
    inconsistency.  Running:␟<function>bt_index_check</function>と<function>bt_index_parent_check</function>は両方とも、<literal>DEBUG1</literal>と<literal>DEBUG2</literal>の深刻度レベルで検証プロセスに関するログメッセージを出力します。
...
以下を実行すると、␞␞<programlisting>␞
␝</programlisting>␟    in an interactive <application>psql</application> session before
    running a verification query will display messages about the
    progress of verification with a manageable level of detail.␟検証問い合わせを実行する前に対話式の<application>psql</application>セッションで扱いやすい程度の詳細で検証の進行状況に関するメッセージを表示します。␞␞   </para>␞
␝     <para>␟      Checks a table, sequence, or materialized view for structural corruption,
      ...
      inconsistent with the rest of the database cluster.␟リレーションのページが不正なフォーマットのデータを含む構造上の破損と、ページは構造的に正しいものの、データベースクラスタの他の部分と一貫していない論理上の破損に関してテーブル、シーケンス、またはマテリアライズドビューを検査します。␞␞     </para>␞
␝     <para>␟      The following optional arguments are recognized:␟次のようなオプションの引数を認識します。␞␞     </para>␞
␝        <para>␟         If true, corruption checking stops at the end of the first block in
         which any corruptions are found.␟真なら、破損が見つかった最初のブロックの終端で破損検査は終了します。␞␞        </para>␞
␝        <para>␟         Defaults to false.␟デフォルトは偽です。␞␞        </para>␞
␝        <para>␟         If true, toasted values are checked against the target relation's
         TOAST table.␟真なら、TOASTされた値が対象リレーションのTOASTテーブルに対して検査されます。␞␞        </para>␞
␝        <para>␟         This option is known to be slow.  Also, if the toast table or its
...
もっとも、多くの場合には単にエラーが生じるだけでしょう。␞␞        </para>␞
␝        <para>␟         Defaults to false.␟デフォルトは偽です。␞␞        </para>␞
␝        <para>␟         If not <literal>none</literal>, corruption checking skips blocks that
         are marked as all-visible or all-frozen, as specified.
         Valid options are <literal>all-visible</literal>,
         <literal>all-frozen</literal> and <literal>none</literal>.␟<literal>none</literal>でなければ、指定されたとおりに破損検査はすべて可視あるいは凍結されていると印が付けられたブロックをスキップします。
可能なオプションは<literal>all-visible</literal>、<literal>all-frozen</literal>、<literal>none</literal>です。␞␞        </para>␞
␝        <para>␟         Defaults to <literal>none</literal>.␟デフォルトは<literal>none</literal>です。␞␞        </para>␞
␝        <para>␟         If specified, corruption checking begins at the specified block,
         ...
         target table.␟指定すると、破損検査は前のブロックをすべてスキップして指定ブロックから開始されます。
<parameter>startblock</parameter>を対象テーブルに含まれるブロックの範囲外で指定するとエラーになります。␞␞        </para>␞
␝        <para>␟         By default, checking begins at the first block.␟デフォルトでは最初のブロックから検索が始まります。␞␞        </para>␞
␝        <para>␟         If specified, corruption checking ends at the specified block,
         ...
         table.␟指定すると、残りのすべてのブロックをスキップして指定ブロックで破損検査が終了します。
<parameter>endblock</parameter>を対象テーブルに含まれるブロックの範囲外で指定するとエラーになります。␞␞        </para>␞
␝        <para>␟         By default, all blocks are checked.␟デフォルトではすべてのブロックが検査されます。␞␞        </para>␞
␝     <para>␟      For each corruption detected, <function>verify_heapam</function> returns
      a row with the following columns:␟個々の破損の検出に対して<function>verify_heapam</function>は以下の列を含む行を返します。␞␞     </para>␞
␝        <para>␟         The number of the block containing the corrupt page.␟破損ページを含むブロック番号。␞␞        </para>␞
␝        <para>␟         The OffsetNumber of the corrupt tuple.␟破損タプルのオフセット番号。␞␞        </para>␞
␝        <para>␟         The attribute number of the corrupt column in the tuple, if the
         corruption is specific to a column and not the tuple as a whole.␟タプル全体ではなく、破損が特定の列にのみ起こっているなら、タプル内の破損列の属性番号。␞␞        </para>␞
␝        <para>␟         A message describing the problem detected.␟検出された問題を記述するメッセージ。␞␞        </para>␞
␝ <sect2 id="amcheck-optional-heapallindexed-verification">␟  <title>Optional <parameter>heapallindexed</parameter> Verification</title>␟  <title>オプション<parameter>heapallindexed</parameter>検証</title>␞␞ <para>␞
␝ <para>␟  When the <parameter>heapallindexed</parameter> argument to B-Tree
  ...
  have entries that can be found in the existing structure.␟B-Tree検証関数の<parameter>heapallindexed</parameter>引数が<literal>true</literal>ならば、対象のインデックスリレーションと関連付けられたテーブルに対して追加の検証フェーズが実施されます。
...
<parameter>heapallindexed</parameter>検証の背後にある高レベルの原理は、新しいインデックスが既存のインデックスと等しいこと、対象のインデックスが既存の構造中に見つかったエントリのみを含むことです。␞␞ </para>␞
␝ <para>␟  The additional <parameter>heapallindexed</parameter> phase adds
  ...
  performed.␟追加の<parameter>heapallindexed</parameter>フェーズは大きなオーバーヘッドをもたらします。
典型的には、検証に数倍時間かかるようになります。
しかし、取得されたリレーションレベルのロックに対して、<parameter>heapallindexed</parameter>検証が実施されるときに変更はありません。␞␞ </para>␞
␝ <para>␟  The summarizing structure is bound in size by
  ...
  detected with each new verification attempt.␟サマライズ構造は、<varname>maintenance_work_mem</varname>によってその大きさが制限されます。
...
失われた、あるいは不正なタプルは次の検証の機会に検出されます。␞␞ </para>␞
␝ <sect2 id="amcheck-using-amcheck-effectively">␟  <title>Using <filename>amcheck</filename> Effectively</title>␟  <title><filename>amcheck</filename>を効果的に使う</title>␞␞␞
␝ <para>␟  <filename>amcheck</filename> can be effective at detecting various types of
  failure modes that <link
  linkend="app-initdb-data-checksums"><application>data
  checksums</application></link> will fail to catch.  These include:␟<filename>amcheck</filename>は、<link linkend="app-initdb-data-checksums"><application>データチェックサム</application></link>が検知できないような、様々なタイプの障害モードを効果的に検知できます。
以下のようなものがあります。␞␞␞
␝    <para>␟     Structural inconsistencies caused by incorrect operator class
     implementations.␟演算子クラスの正しくない実装によって引き起こされる構造の非一貫性。␞␞    </para>␞
␝    <para>␟     This includes issues caused by the comparison rules of operating
     ...
     only be detected on standby servers.␟オペレーティングシステムの照合順序の比較ルールの変更による問題も含まれます。
...
そうした一貫性の欠如は一般的にスタンバイサーバでのみ起こるので、通常スタンバイサーバでのみ検出されます。␞␞    </para>␞
␝    <para>␟     If a problem like this arises, it may not affect each individual
     ...
     operating system locales and collations.␟そうした問題が起きても、影響を受けた照合順序を使って順序付けられた個々のインデックスには影響ないかもしれません。
これは単純に、振る舞いにおける一貫性のなさにかかわらず<emphasis>インデックスされた</emphasis>値は同じ絶対的な順になるからです。
<productname>PostgreSQL</productname>がオペレーティングシステムのロケールと照合順序をどう使用するかについての更なる詳細については、<xref linkend="locale"/>と<xref linkend="collation"/>をご覧ください。␞␞    </para>␞
␝    <para>␟     Structural inconsistencies between indexes and the heap relations
     that are indexed (when <parameter>heapallindexed</parameter>
     verification is performed).␟インデックスとインデックス付されたヒープリレーションの間の構造的な非一貫性（<parameter>heapallindexed</parameter>検証が実施される場合）␞␞    </para>␞
␝    <para>␟     There is no cross-checking of indexes against their heap relation
     during normal operation.  Symptoms of heap corruption can be subtle.␟通常の操作においてはインデックスとそのヒープリレーションの間にはクロスチェックはありません。
ヒープの破壊による症状は些細なものかもしれません。␞␞    </para>␞
␝    <para>␟     Corruption caused by hypothetical undiscovered bugs in the
     underlying <productname>PostgreSQL</productname> access method
     code, sort code, or transaction management code.␟依拠する<productname>PostgreSQL</productname>のアクセスメソッド、あるいはソート、トランザクション管理コードにおける、潜在的なまだ見つかっていないバグによる破損。␞␞    </para>␞
␝    <para>␟     Automatic verification of the structural integrity of indexes
     ...
     linkend="regress-run"/> for details on running the tests.␟新規、あるいは提案中の <productname>PostgreSQL</productname>の機能が、論理的な非一貫性をもたらしかねないかどうか全般的にテストする際に、インデックスの構造的な一貫性の自動検証が役立ちます。
...
テストの実行に関する詳細は、<xref linkend="regress-run"/>をご覧ください。␞␞    </para>␞
␝    <para>␟     File system or storage subsystem faults where checksums happen to
     simply not be enabled.␟単にチェックサムが有効になっていないファイルシステムあるいはストレージサブシステムの障害。␞␞    </para>␞
␝    <para>␟     Note that <filename>amcheck</filename> examines a page as represented in some
     ...
     failure when a corrupt block is read into a buffer.␟<filename>amcheck</filename>は、ブロックをアクセスする際に共有バッファがヒットした場合、検査時に共有メモリバッファ上に表現されたページを調査します。
そのため、<filename>amcheck</filename>は、検査時にファイルシステムから読み込んだデータを調査するとは限りません。
チェックサムが有効な場合、<filename>amcheck</filename>は壊れたブロックをバッファに読み込んだ際にチェックサム障害によるエラーを報告するかもしれません。␞␞    </para>␞
␝    <para>␟     Corruption caused by faulty RAM, or the broader memory subsystem.␟欠陥のあるRAMあるいは、広範囲に渡るメモリサブシステムによる破損。␞␞    </para>␞
␝    <para>␟     <productname>PostgreSQL</productname> does not protect against correctable
     ...
     result in memory corruption.␟<productname>PostgreSQL</productname>は、訂正可能なメモリエラーからは身を守らないので、業界標準のエラー訂正コード(ECC)、あるいはもっと優れた保護機構を使ったRAMを使って運用する前提となっています。
しかし、ECCメモリは典型的には単一ビットエラーに対してのみ耐性があり、メモリ破損に起因する障害に対して<emphasis>完全な</emphasis>保護を提供すると考えるべきではありません。␞␞    </para>␞
␝    <para>␟     When <parameter>heapallindexed</parameter> verification is
     performed, there is generally a greatly increased chance of
     detecting single-bit errors, since strict binary equality is
     tested, and the indexed attributes within the heap are tested.␟<parameter>heapallindexed</parameter>検証が実施されると、一般に1ビットエラーを検出する可能性が非常に高くなります。
これは、バイナリ一致を厳密にテストすることと、またヒープ内のインデックス付けされたアトリビュートをテストすることによります。␞␞    </para>␞
␝ <para>␟  Structural corruption can happen due to faulty storage hardware, or
  ...
  checksums</application></link>.␟構造上の破損は故障したストレージハードウェア、あるいはリレーションファイルが無関係のソフトウェアによって上書きされたり変更されることによって起こることがあります。
この種の破損は<link linkend="checksums"><application>データページチェックサム</application></link>によっても検出することができます。␞␞ </para>␞
␝ <para>␟  Relation pages which are correctly formatted, internally consistent, and
  ...
  than the oldest valid Transaction ID in the database or cluster.␟正しくフォーマットされ、内部的に一貫していて、かつ自身の内部チェックサムが正しいリレーションページでも論理的に破損していることがあります。
ですから、この種の破損は<application>チェックサム</application>では検出できません。
例としては、主テーブルのTOASTされた値に対応するTOASTテーブルのエントリが存在しない、主テーブルのタプルのトランザクションIDがデータベースクラスタ内の有効な最古のトランザクションIDよりも古い、などがあります。␞␞ </para>␞
␝ <para>␟  Multiple causes of logical corruption have been observed in production
  ...
  user error.␟<productname>PostgreSQL</productname>サーバソフトウェアのバグ、欠陥があったり、よく考慮されていないバックアップ、リストアツール、ユーザのエラーなど、論理的破損の複数の原因が実運用システム上で観測されています。␞␞ </para>␞
␝ <para>␟  Corrupt relations are most concerning in live production environments,
  ...
  be problematic if the catalogs themselves are corrupted.␟破損したリレーションは、リスクの高い活動がもっとも忌避される環境である、運用中の実システム環境では最大の懸念事項です。
...
カタログ自身が破損していれば<link linkend="catalogs-overview">カタログテーブル</link>へのアクセスの実行も問題になるでしょう。␞␞ </para>␞
␝ <para>␟  In general, <filename>amcheck</filename> can only prove the presence of
  corruption; it cannot prove its absence.␟一般的に、<filename>amcheck</filename>は破損の存在を証明することはできますが、破損がないことを証明することはできません。␞␞ </para>␞
␝ <sect2 id="amcheck-repairing-corruption">␟  <title>Repairing Corruption</title>␟  <title>破損の修復</title>␞␞ <para>␞
␝ <para>␟  No error concerning corruption raised by <filename>amcheck</filename> should
  ...
  errors is often required.␟<filename>amcheck</filename>が報告するエラーが関与する破損で、偽陽性のものはありません。
<filename>amcheck</filename>は、定義により発生してはならないはずの条件下で発生したエラーを報告するので、<filename>amcheck</filename>の報告するエラーを注意深く解析することがしばしば求められます。␞␞ </para>␞
␝ <para>␟  There is no general method of repairing problems that
  ...
  may not be effective in repairing corruption.␟<filename>amcheck</filename>が検出した問題を修復する一般的な方法はありません。
不変条件違反の根本的な原因の説明が求められます。
<filename>amcheck</filename>が検出した破損の診断には、<xref linkend="pageinspect"/>が有用な役割を担うかもしれません。
<command>REINDEX</command>は破損の修復には効果的ではないかもしれません。␞␞ </para>␞
