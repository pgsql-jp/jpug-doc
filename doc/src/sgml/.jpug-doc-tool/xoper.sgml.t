...
␝ <sect1 id="xoper">␟  <title>User-Defined Operators</title>␟  <title>ユーザ定義の演算子</title>␞␞␞
␝  <para>␟   Every operator is <quote>syntactic sugar</quote> for a call to an
   ...
   that additional information.␟演算子は裏側で実際の作業を行う関数を呼び出す<quote>構文上の飾り</quote>です。
...
次節では、この追加情報について重点的に説明します。␞␞  </para>␞
␝  <para>␟   <productname>PostgreSQL</productname> supports prefix
   ...
   number and types of the provided operands.␟<productname>PostgreSQL</productname>では前置演算子、中置演算子をサポートしています。
演算子はオーバーロード可能です。
<indexterm><primary>オーバーロード</primary><secondary>演算子</secondary></indexterm>
つまり、同じ演算子名をオペランドの数と型が異なる演算子に対して使用することができるということです。
問い合わせが実行されると、システムは与えられたオペランドの数と型より呼び出すべき演算子を決定します。␞␞  </para>␞
␝  <para>␟   Here is an example of creating an operator for adding two complex
   numbers.  We assume we've already created the definition of type
   <type>complex</type> (see <xref linkend="xtypes"/>).  First we need a
   function that does the work, then we can define the operator:␟以下に2つの複素数の加算を行う演算子を作成する例を示します。
既に<type>complex</type>型の定義（<xref linkend="xtypes"/>を参照）を作成していることを前提としています。
まず、実作業を行う関数が必要です。
その後、演算子を定義できます。␞␞␞
␝  <para>␟   Now we could execute a query like this:␟これで以下のような問い合わせを実行できるようになります。␞␞␞
␝  <para>␟   We've shown how to create a binary operator here.  To create a prefix
   ...
   optimizer hints appear in the next section.␟ここでは二項演算子をどのように作成するのかを示しました。
...
<literal>commutator</literal>やその他のオプティマイザへのヒントについての詳細は次節で説明します。␞␞  </para>␞
...
␝  <sect1 id="xoper-optimization">␟   <title>Operator Optimization Information</title>␟   <title>演算子最適化に関する情報</title>␞␞␞
␝   <para>␟    A <productname>PostgreSQL</productname> operator definition can include
    ...
    they need to.␟<productname>PostgreSQL</productname>の演算子定義では、システムに演算子がどう振舞うかに関する有用なことを通知する、いくつかのオプション句を持つことができます。
...
使用された時よりも問い合わせの実行が遅くなるかもしれないというだけです。␞␞   </para>␞
␝   <para>␟    Additional optimization clauses might be added in future versions of
    <productname>PostgreSQL</productname>.  The ones described here are all
    the ones that release &version; understands.␟<productname>PostgreSQL</productname>の今後のバージョンで、最適化用の句はさらに追加される可能性があります。
ここで説明するものはすべて、バージョン&version;で有効なものです。␞␞   </para>␞
␝   <para>␟    It is also possible to attach a planner support function to the function
    that underlies an operator, providing another way of telling the system
    about the behavior of the operator.
    See <xref linkend="xfunc-optimization"/> for more information.␟演算子の基となる関数にプランナサポート関数を結び付けて、システムに演算子の振舞いを通知する別の方法を提供することも可能です。
より詳細な情報については<xref linkend="xfunc-optimization"/>を参照してください。␞␞   </para>␞
␝    <para>␟     The <literal>COMMUTATOR</literal> clause, if provided, names an operator that is the
     ...
     But operator <literal>-</literal> is usually not commutative with anything.␟<literal>COMMUTATOR</literal>句が与えられた場合、それは定義する演算子の交代演算子となる演算子の名前です。
...
しかし、通常、演算子<literal>-</literal>は交代演算子を持ちません。␞␞    </para>␞
␝    <para>␟     The left operand type of a commutable operator is the same as the
     ...
     be provided in the <literal>COMMUTATOR</literal> clause.␟交代可能な演算子の左オペランドの型は、その交代演算子の右オペランドの型と同一で、その逆もまた同様です。
したがって、<productname>PostgreSQL</productname>で交代演算子を検索する時に必要なものは交代演算子の名前のみになりますので、<literal>COMMUTATOR</literal>句でそれのみを与えておけば十分です。␞␞    </para>␞
␝    <para>␟     It's critical to provide commutator information for operators that
     ...
     operator with commutator information.␟インデックスや結合句で使用される演算子では交代演算子の情報を提供することが必須です。
...
<literal>=</literal>演算子の作成者がこれが有効であることを、交代演算子情報を持つ演算子であると印付けて指定しなければなりません。␞␞    </para>␞
␝    <para>␟     The <literal>NEGATOR</literal> clause, if provided, names an operator that is the
     ...
     An operator can never validly be its own negator.␟<literal>NEGATOR</literal>句が与えられた場合、それは定義する演算子の否定子となる演算子の名前です。
...
演算子が自身の否定子になることは決してありません。␞␞    </para>␞
␝   <para>␟    Unlike commutators, a pair of unary operators could validly be marked
    as each other's negators; that would mean (A x) equals NOT (B x)
    for all x.␟交代演算子と異なり、単項演算子の対は互いに否定子として有効に指定されます。
つまりすべてのxに対して(A x)がNOT (B x)と等しいことを意味します。␞␞   </para>␞
␝   <para>␟    An operator's negator must have the same left and/or right operand types
    as the operator to be defined, so just as with <literal>COMMUTATOR</literal>, only the operator
    name need be given in the <literal>NEGATOR</literal> clause.␟ある演算子の否定子は、その演算子定義の左オペランド、右オペランドと同じ型を持たなければなりません。
ですので、<literal>COMMUTATOR</literal>句と同様に演算子の名前のみ<literal>NEGATOR</literal>句で与えるだけで済みます。␞␞   </para>␞
␝   <para>␟    Providing a negator is very helpful to the query optimizer since
    ...
    <literal>NOT</literal> operations can be inserted as a consequence of other rearrangements.␟<literal>NOT (x = y)</literal>という式を<literal>x &lt;&gt; y</literal>という形に単純化させることが可能なので、否定子があると問い合わせオプティマイザにとって非常に役に立ちます。
他の再配置の結果として<literal>NOT</literal>操作が挿入されることがありますので、この現象は思ったより頻繁に起こります。␞␞   </para>␞
␝   <para>␟    The <literal>RESTRICT</literal> clause, if provided, names a restriction selectivity
    ...
    table will satisfy a <literal>WHERE</literal>-clause condition of the form:␟<literal>RESTRICT</literal>句が与えられた場合、それは、その演算子用の制限選択評価関数を指定します。
（演算子名ではなく関数名であることに注意してください。）
<literal>RESTRICT</literal>句は<type>boolean</type>型を返す二項演算子に対してのみ有効です。
制限選択評価の目的は、現在の演算子と特定の定数値についての<literal>WHERE</literal>句␞␞<programlisting>␞
␝</programlisting>␟    for the current operator and a particular constant value.
    ...
    <literal>COMMUTATOR</literal> is for...)␟の条件を満たすテーブル内の行の割合を推測することです。
この形式を持った<literal>WHERE</literal>句によって、どのくらいの行が除外されるのかを通知することで、オプティマイザの手助けをします。
（定数値が左項にあったら何が起こるかという疑問が生じるかもしれませんが、それは<literal>COMMUTATOR</literal>が存在する理由の1つでもあります。）␞␞   </para>␞
␝   <para>␟    Writing new restriction selectivity estimation functions is far beyond
    ...
    These are the standard restriction estimators:␟新しい制限選択評価関数の記述方法は本章の内容を超えていますが、幸いなことに、数多いユーザ定義の演算子に対し通常いくつかのシステム標準の評価関数を使用すれば事足ります。
システム標準の制限評価関数には下記のものがあります。␞␞    <simplelist>␞
␝    <simplelist>␟     <member><function>eqsel</function> for <literal>=</literal></member>
     ...
     <member><function>scalargesel</function> for <literal>&gt;=</literal></member>␟     <member><literal>=</literal>用の<function>eqsel</function></member>
     ...
     <member><literal>&gt;=</literal>用の<function>scalargesel</function></member>␞␞    </simplelist>␞
␝   <para>␟    You can frequently get away with using either <function>eqsel</function> or <function>neqsel</function> for
    ...
    they'll usually only match a small fraction of the entries in a table.␟非常に高いもしくは低い選択性を所有する演算子が、まったく等しいか等しくないかにかかわらず、<function>eqsel</function>または<function>neqsel</function>を使用しないことも往々にして可能です。
例えば、近似等号用の幾何演算子はテーブルのエントリの小部分にのみに合致すると仮定して<function>eqsel</function>を使用します。␞␞   </para>␞
␝   <para>␟    You can use <function>scalarltsel</function>, <function>scalarlesel</function>,
    ...
    estimates won't be as good as they could be.␟範囲比較のために数値スカラに変換することに多少の有意性があるデータ型を比較するために、<function>scalarltsel</function>、<function>scalarlesel</function>、<function>scalargtsel</function>、<function>scalargesel</function>を使用することも可能です。
...
これを行わなくても動きますが、オプティマイザは本来の推測機能を十分発揮することができません。␞␞   </para>␞
␝   <para>␟    Another useful built-in selectivity estimation function
    ...
    function, providing a different default estimate.)␟もう1つの便利な組み込みの選択評価関数は<function>matchingsel</function>で、入力データ型に対して標準的な最頻値やヒストグラム統計を収集する場合、ほぼすべての二項演算子に対して動作します。
デフォルトの評価は<function>eqsel</function>で設定されているデフォルトの評価の2倍に設定されており、等価性ほど厳密でないような比較演算子には最適です。
(裏で実行される<function>generic_restriction_selectivity</function>関数に異なるデフォルトの評価を与えて呼び出すこともできます。)␞␞   </para>␞
␝   <para>␟    There are additional selectivity estimation functions designed for geometric
    ...
    to use them (or even better, improve them) anyway.␟さらに<filename>src/backend/utils/adt/geo_selfuncs.c</filename>には、幾何演算子に対する選択評価関数<function>areasel</function>、<function>positionsel</function>、<function>contsel</function>があります。
本章の執筆時点では、これらは単なるスタブですが、ともかく使いたい（あるいは改良したい）こともあるでしょう。␞␞   </para>␞
␝    <para>␟     The <literal>JOIN</literal> clause, if provided, names a join selectivity
     estimation function for the operator.  (Note that this is a function
     ...
     selectivity estimator is to guess what fraction of the rows in a
     pair of tables will satisfy a <literal>WHERE</literal>-clause condition of the form:␟<literal>JOIN</literal>句が与えられた場合、それはその演算子用の結合選択評価関数の名前を指定します。
（これが演算子名ではなく関数名であることに注意してください。）
<literal>JOIN</literal>句は<type>boolean</type>型を返す二項演算子に対してのみ有効です。
結合選択評価の目的は、現在の演算子について、<literal>WHERE</literal>句␞␞<programlisting>␞
␝</programlisting>␟     for the current operator.  As with the <literal>RESTRICT</literal> clause, this helps
     the optimizer very substantially by letting it figure out which
     of several possible join sequences is likely to take the least work.␟を満たすテーブルの組み合わせの行の割合を推測することです。
<literal>RESTRICT</literal>句の使用と同様、これはいくつかの取り得る結合手順のうち、どれが最も仕事量が少ないように考えられるのかをオプティマイザに計算させることで、大きなオプティマイザへの援助となります。␞␞    </para>␞
␝    <para>␟     As before, this chapter will make no attempt to explain how to write
     a join selectivity estimator function, but will just suggest that
     you use one of the standard estimators if one is applicable:␟以前と同様、本章でも結合選択評価関数の作成方法は説明しません。
しかし適用できるものがあれば、単に標準の評価関数を使用することをお勧めします。␞␞     <simplelist>␞
␝     <simplelist>␟      <member><function>eqjoinsel</function> for <literal>=</literal></member>
      ...
      <member><function>contjoinsel</function> for 2D containment-based comparisons</member>␟      <member><literal>=</literal>用の<function>eqjoinsel</function> </member>
      ...
      <member>2次元包含関係を基にした比較用の<function>contjoinsel</function></member>␞␞     </simplelist>␞
␝    <para>␟     The <literal>HASHES</literal> clause, if present, tells the system that
     ...
     equality for some data type or pair of data types.␟<literal>HASHES</literal>句が存在する場合、それはシステムに対して、この演算子に基づいた結合にハッシュ結合方法を使っても問題がないことを伝えます。
<literal>HASHES</literal>句は<literal>boolean</literal>型を返す二項演算子にのみ有効です。
実際には、この演算子はあるデータ型またはデータ型の組み合わせの等価性を表現しなければなりません。␞␞    </para>␞
␝    <para>␟     The assumption underlying hash join is that the join operator can
     ...
     to arrange this property when hashing integers of different widths.␟ハッシュ結合の基礎となっている仮定は、結合演算子は左項と右項の値が同じハッシュコードを持つ時にのみ真を返すことができるということです。
...
例えば、サイズが異なる整数に対するハッシュでは、この性質を調整することで大変単純になります。␞␞    </para>␞
␝    <para>␟     To be marked <literal>HASHES</literal>, the join operator must appear
     ...
     hash functions before you can create the operator family.␟<literal>HASHES</literal>印を付けるためには、結合演算子はハッシュインデックスの演算子族内になければなりません。
...
もちろん、演算子族を作成する前に適切なハッシュ関数を作成しなければなりません。␞␞    </para>␞
␝    <para>␟     Care should be exercised when preparing a hash function, because there
     ...
     to ensure it generates the same hash value as positive zero.␟ハッシュ関数を準備する時には注意が必要です。
...
浮動小数点数値がマイナス0を持つ可能性があるのであれば、それがプラス0と同じハッシュコードを確実に生成するような処置が必要です。␞␞    </para>␞
␝    <para>␟     A hash-joinable operator must have a commutator (itself if the two
     ...
     allows better optimization.␟ハッシュ結合可能な演算子は、同一演算子族内に存在する交代演算子を持たなければなりません。
...
これにより、より優れた最適化が可能になります。␞␞    </para>␞
␝    <para>␟     The function underlying a hash-joinable operator must be marked
     immutable or stable.  If it is volatile, the system will never
     attempt to use the operator for a hash join.␟ハッシュ結合可能演算子の基となる関数はimmutableもしくはstableでなければなりません。
volatileの場合、システムはその演算子を決してハッシュ結合に使用しません。␞␞    </para>␞
␝    <para>␟     If a hash-joinable operator has an underlying function that is marked
     strict, the
     ...
     null result.)␟ハッシュ結合可能演算子の基となる関数が厳密（strict）な場合、その関数は完全、つまり2つの非NULL入力に対して、真または偽を返し、決してNULLを返さないものである必要があります。
...
もしくは、NULLという結果に対する準備をしていないといったエラーを生成するかもしれません。）␞␞    </para>␞
␝    <para>␟     The <literal>MERGES</literal> clause, if present, tells the system that
     ...
     returns <literal>boolean</literal>, and in practice the operator must represent
     equality for some data type or pair of data types.␟<literal>MERGES</literal>句が存在する場合、それはシステムに対して、この演算子に基づいた結合にマージ結合方法を使っても問題がないことを伝えます。
<literal>MERGES</literal>句は<literal>boolean</literal>型を返す二項演算子にのみ有効です。
実際には、演算子がデータ型またはデータ型の組み合わせの等価性を表すものであることが必要です。␞␞    </para>␞
␝    <para>␟     Merge join is based on the idea of sorting the left- and right-hand tables
     ...
     logically compatible sequence.␟マージ結合は、左側のテーブル、右側のテーブルを順序よくソートし、並列にスキャンするという考えに基づいています。
...
両方のデータ型を論理的な互換性を保つ順番にソートする演算子のみが必要です。␞␞    </para>␞
␝    <para>␟     To be marked <literal>MERGES</literal>, the join operator must appear
     ...
     it's worth looking for a matching operator family.␟<literal>MERGES</literal>印を付けるためには、結合演算子は、<literal>btree</literal>インデックス演算子族の等価性メンバとして存在しなければなりません。
演算子を作成する時には参照する演算子族がまだ存在しませんので、演算子の作成時にこれは強制されていません。
しかし、対応する演算子族が存在しない限り、実際にマージ結合に使用されることはありません。
このように、<literal>MERGES</literal>フラグは、プランナが対応する演算子族を検索すべきかどうかを決定する際のヒントとして動作します。␞␞    </para>␞
␝    <para>␟     A merge-joinable operator must have a commutator (itself if the two
     operand data types are the same, or a related equality operator
     if they are different) that appears in the same operator family.
     If this is not the case, planner errors might occur when the operator
     is used.  Also, it is a good idea (but not strictly required) for
     ...
     equality operators for every combination of the data types; this
     allows better optimization.␟マージ結合可能な演算子は、同一演算子族内に存在する交代演算子を持たなければなりません。
（２つの入力データ型が同じ場合はその演算子自体が交代演算子となります。異なる場合は関連する等価性演算子となります。）
これを満たさないと、演算子の使用時にプランナエラーが発生します。
また、複数のデータ型をサポートする<literal>btree</literal>演算子族に対して、データ型の組み合わせすべてに対する等価性演算子を持たせることを推奨します（必要ではありません）。
これにより、より優れた最適化が可能になります。␞␞    </para>␞
␝    <para>␟     The function underlying a merge-joinable operator must be marked
     immutable or stable.  If it is volatile, the system will never
     attempt to use the operator for a merge join.␟マージ結合可能演算子の背後にある関数はimmutableもしくはstableでなければなりません。
volatileの場合、システムはその演算子を決してマージ結合に使用しようとはしません。␞␞    </para>␞
